...
*/
void csplit_diag_block(magma_minproduct_int_t ib, magma_minproductFloatComplex *a, magma_minproduct_int_t lda, magma_minproductFloatComplex *work)
{
    magma_minproduct_int_t i, j, info;
    magma_minproductFloatComplex *cola, *colw;
    ...
    lapackf77_ctrtri( Magma_minproductUpperStr, Magma_minproductNonUnitStr, &ib, work, &ib, &info);
}
    ...
    magma_minproduct_int_t *info )
{
    ...
    magma_minproduct_int_t i, k, minmn, old_i, old_ib, rows, cols;
    magma_minproduct_int_t ib, nb;
    magma_minproduct_int_t ldwork, lddwork, lwork, lhwork;
    magma_minproductFloatComplex *work, *ut;
    ...
    *info = 0;
    ...
    ut = hwork+nb*(n);
    memset( ut, 0, nb*nb*sizeof(magma_minproductFloatComplex));
    ...
    if ( (nb > 1) && (nb < k) ) {
        ...
        for (i = 0; i < k-nb; i += nb) {
            ...
            if (i > 0) {
                ...
                /* store the diagonal */
                magma_minproduct_csetmatrix_async( old_ib, old_ib,
                                        ut,           old_ib,
            ...
            csplit_diag_block(ib, work(i), ldwork, ut);
            magma_minproduct_csetmatrix( rows, ib, work(i), ldwork, dA(i,i), ldda );

            if (i + ib < n) {
                /* Send the triangular factor T to the GPU */
                magma_minproduct_csetmatrix( ib, ib, hwork, ib, dT(i), nb );
                ...
                if (i+nb < k-nb) {
                    ...
                    magma_minproduct_csetmatrix( ib, ib, ut, ib, d_ref(i), ib );
                }
                old_i  = i;
                old_ib = ib;
            }
    ...
    if (i < k) {
        ib   = n-i;
        rows = m-i;
        magma_minproduct_cgetmatrix( rows, ib, dA(i, i), ldda, work, rows );
        lhwork = lwork - rows*ib;
        lapackf77_cgeqrf(&rows, &ib, work, &rows, tau+i, work+ib*rows, &lhwork, info);
        
        magma_minproduct_csetmatrix( rows, ib, work, rows, dA(i, i), ldda );
    }
