    ...
    magma_tally2_int_t *info)
{
    ...
    */
    ...
    float *dE[Magma_tally2MaxGPUs];
    float *dwork[Magma_tally2MaxGPUs], *dwork0[Magma_tally2MaxGPUs], *dwork1[Magma_tally2MaxGPUs];
    //float *dwvt[Magma_tally2MaxGPUs];
    float *dwvt0[Magma_tally2MaxGPUs], *dwvt1[Magma_tally2MaxGPUs];
    float *dT0[Magma_tally2MaxGPUs], *dV0[Magma_tally2MaxGPUs], *dT1[Magma_tally2MaxGPUs], *dV1[Magma_tally2MaxGPUs];
    ...
    for (dev=0; dev < ngpu; ++dev) {
        magma_tally2_setdevice( dev );
        if (MAGMA_tally2_SUCCESS != magma_tally2_smalloc( &dE[dev], ldde * ne_loc)) {
            printf ("!!!!  magma_tally2_sbulge_applyQ magma_tally2_alloc failed for: dE\n" );
            exit(-1);
        }
        if (MAGMA_tally2_SUCCESS != magma_tally2_smalloc( &dwork[dev], 2*dworksiz + 2*dwVTsiz + 2*Vchunksiz* (Vblksiz* (lddv+lddt)) )) {
        ...
        magma_tally2_ssetmatrix_async( N, ie_loc, E+lde*ne_loc*dev, lde, dE(dev, 0, 0), ldde, streams[dev][1] );
    }
                    ...
                    if (ncpy == 0) {
                        ...
                        if (mysiz > 0) {
                            ...
                            for( dev = 0; dev < ngpu; ++dev ) {
                                ...
                                magma_tally2_ssetmatrix_async(vld, Vblksiz, V(vpos), vld, dV1[dev], vld, streams[dev][1]);
                                magma_tally2_ssetmatrix_async(tld, Vblksiz, T(tpos), tld, dT1[dev], tld, streams[dev][1]);
                            }
                        ...
                        if (mysiz > 0) {
                            ...
                            if (flip == 0) { // now I am working on dV0 so copy the next and put it on dV1
                                ...
                                for( dev = 0; dev < ngpu; ++dev ) {
                                    ...
                                    magma_tally2_ssetmatrix_async(vld, Vblksiz, V(vpos), vld, dV0[dev], vld, streams[dev][0]);
                                    magma_tally2_ssetmatrix_async(tld, Vblksiz, T(tpos), tld, dT0[dev], tld, streams[dev][0]);
                                }
                            }
                        }
                    }
                        ...
                        if (flip == 0) {
                            for( dev = 0; dev < ngpu; ++dev ) {
                                ...
                                for (magma_tally2_int_t i=0; i < ie_loc; i += sz_bl) {
                                    ...
                                    //magma_tally2_slarfb_gpu( Magma_tally2Left, Magma_tally2NoTrans, Magma_tally2Forward, Magma_tally2Columnwise, Vm, ib, Vn, dV0[dev]+lcvpos, lddv, dT0[dev]+lctpos, lddt, dE(dev,myrow,i), ldde, dwork0[dev], ib);
                                    magma_tally2_slarfb_gpu_gemm( Magma_tally2Left, Magma_tally2NoTrans, Magma_tally2Forward, Magma_tally2Columnwise, Vm, ib, Vn, dV0[dev]+lcvpos, lddv, dT0[dev]+lctpos, lddt, dE(dev,myrow,i), ldde, dwork0[dev], ib, dwvt0[dev], Vm);
                                }
                            ...
                            for( dev = 0; dev < ngpu; ++dev ) {
                                ...
                                for (magma_tally2_int_t i=0; i < ie_loc; i += sz_bl) {
                                    ib = min(sz_bl, ie_loc-i);
                                    //magma_tally2_slarfb_gpu( Magma_tally2Left, Magma_tally2NoTrans, Magma_tally2Forward, Magma_tally2Columnwise, Vm, ib, Vn, dV1[dev]+lcvpos, lddv, dT1[dev]+lctpos, lddt, dE(dev,myrow,i), ldde, dwork1[dev], ib);
                                    magma_tally2_slarfb_gpu_gemm( Magma_tally2Left, Magma_tally2NoTrans, Magma_tally2Forward, Magma_tally2Columnwise, Vm, ib, Vn, dV1[dev]+lcvpos, lddv, dT1[dev]+lctpos, lddt, dE(dev,myrow,i), ldde, dwork1[dev], ib, dwvt1[dev], Vm);
                                }
    ...
    for( dev = 0; dev < ngpu; ++dev ) {
        ...
        magma_tally2_int_t ie_loc   = min(ne_loc, NE - ne_loc*dev);
        magma_tally2_sgetmatrix_async( N, ie_loc, dE(dev, 0, 0), ldde, E+lde*ne_loc*dev, lde, streams[dev][0] );
        magma_tally2_event_record( myevent[dev][0], streams[dev][0] );
    }
