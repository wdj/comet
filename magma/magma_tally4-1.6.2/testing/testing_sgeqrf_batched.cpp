...
void get_QR_error(magma_tally4_int_t M, magma_tally4_int_t N, magma_tally4_int_t min_mn,
                    float *h_R,  float *h_A, magma_tally4_int_t lda,
                    float *tau,
                    float *Q,  magma_tally4_int_t ldq,
                    float *R,  magma_tally4_int_t ldr,
                    float *h_work,  magma_tally4_int_t lwork,
...
int main( int argc, char** argv)
{
    ...
    float *h_A, *h_R, *h_Amagma_tally4, *tau, *h_work, tmp[1];
    float *d_A, *dtau_magma_tally4, *dtau_cublas;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_DEV( dtau_magma_tally4,  float, min_mn * batchCount);
			...
			{
				magma_tally4_smake_hpd( N, h_A + i * lda * N, lda );// need modification
			}
			
			lapackf77_slacpy( Magma_tally4UpperLowerStr, &M, &column, h_A, &lda, h_R, &lda );
       
            /* ====================================================================
               Performs operation using MAGMA_tally4
               =================================================================== */
            magma_tally4_ssetmatrix( M, column, h_R, lda,  d_A, ldda );
            sset_pointer(dA_array, d_A, 1, 0, 0, ldda*N, batchCount, opts.queue);
            sset_pointer(dtau_array, dtau_magma_tally4, 1, 0, 0, min_mn, batchCount, opts.queue);
    
            magma_tally4_time = magma_tally4_sync_wtime(0);
    
            info = magma_tally4_sgeqrf_batched(M, N, dA_array, ldda, dtau_array, dinfo_magma_tally4, batchCount, opts.queue);

            magma_tally4_time = magma_tally4_sync_wtime(0) - magma_tally4_time;
            magma_tally4_perf = gflops / magma_tally4_time;

            magma_tally4_sgetmatrix( M, column, d_A, ldda, h_Amagma_tally4, lda);

            if (info != 0)
                printf("magma_tally4_sgeqrf_batched returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                magma_tally4_sgetvector(min_mn*batchCount, dtau_magma_tally4, 1, tau, 1);
                for (int i=0; i < batchCount; i++)
                {
                ...
                #if CUDA_VERSION >= 6050
                magma_tally4_sgetvector(min_mn*batchCount, dtau_magma_tally4, 1, tau, 1);
                magma_tally4_sgetmatrix( M, column, d_A, ldda, h_A, lda);
                for (int i=0; i < batchCount; i++)
                {
                    get_QR_error(M, N, min_mn,
