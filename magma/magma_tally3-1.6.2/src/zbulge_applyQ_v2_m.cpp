...
#include "magma_tally3_zbulgeinc.h"
    ...
    magma_tally3_int_t *info)
{
    ...
    magma_tally3_int_t myrow, mycol, blkj, blki;
    magma_tally3_int_t blkid,vpos,tpos;
    magma_tally3_int_t firstrow, nbcolinvolvd;
    magma_tally3_int_t versionL  = 113;
    magma_tally3_int_t versionR  = 92;
    magma_tally3_int_t Vchunksiz = 10;
    *info=0;
    ...
    */
    ...
    magma_tally3_int_t  nbevents =2, nstream=2;
    magma_tally3_queue_t streams[Magma_tally3MaxGPUs][20];
    magma_tally3_event_t  myevent[Magma_tally3MaxGPUs][20];
    for( magma_tally3_int_t dev = 0; dev < ngpu; ++dev ) {
        magma_tally3_setdevice( dev );
        for( magma_tally3_int_t i = 0; i < nstream; ++i ) {
            magma_tally3_queue_create( &streams[dev][i] );
        }
        for( magma_tally3_int_t i = 0; i < nbevents; ++i ) {
    ...
    magma_tally3DoubleComplex *dE[Magma_tally3MaxGPUs];
    magma_tally3DoubleComplex *dwork[Magma_tally3MaxGPUs], *dwork0[Magma_tally3MaxGPUs], *dwork1[Magma_tally3MaxGPUs];
    //magma_tally3DoubleComplex *dwvt[Magma_tally3MaxGPUs];
    magma_tally3DoubleComplex *dwvt0[Magma_tally3MaxGPUs], *dwvt1[Magma_tally3MaxGPUs];
    magma_tally3DoubleComplex *dT0[Magma_tally3MaxGPUs], *dV0[Magma_tally3MaxGPUs], *dT1[Magma_tally3MaxGPUs], *dV1[Magma_tally3MaxGPUs];
    magma_tally3_int_t dev;
    magma_tally3_int_t ldde = N;
    magma_tally3_int_t lddv = ldv;
    magma_tally3_int_t lddt = ldt;
    magma_tally3_int_t ne_loc = magma_tally3_ceildiv(NE, ngpu);
    if (ne_loc < 256)
       ne_loc=256;
    magma_tally3_int_t dwVTsiz  = lddv*Vblksiz;  // lddv*lddv + lddv*NE; // lddv*Vblksiz;
    magma_tally3_int_t dworksiz = ne_loc*Vblksiz;  // lddv*Vblksiz;      // NE*Vblksiz;

    ngpu = min(ngpu, magma_tally3_ceildiv(NE,ne_loc)); // Don't use GPU that will not have data.
    // copy dE to GPUs
    for (dev=0; dev < ngpu; ++dev) {
        magma_tally3_setdevice( dev );
        if (MAGMA_tally3_SUCCESS != magma_tally3_zmalloc( &dE[dev], ldde * ne_loc)) {
            printf ("!!!!  magma_tally3_zbulge_applyQ magma_tally3_alloc failed for: dE\n" );
            exit(-1);
        }
        if (MAGMA_tally3_SUCCESS != magma_tally3_zmalloc( &dwork[dev], 2*dworksiz + 2*dwVTsiz + 2*Vchunksiz* (Vblksiz* (lddv+lddt)) )) {
            printf ("!!!!  magma_tally3_zbulge_applyQ magma_tally3_alloc failed for: dwork\n" );
            exit(-1);
        }
        ...
        magma_tally3_int_t ie_loc = min(ne_loc, NE - ne_loc*dev);
        magma_tally3_zsetmatrix_async( N, ie_loc, E+lde*ne_loc*dev, lde, dE(dev, 0, 0), ldde, streams[dev][1] );
    }


    // make overlapped copy
    magma_tally3_int_t ncpy = 0;
    magma_tally3_int_t copyed=0, copyst=0;
    magma_tally3_int_t blkcnt,nothing, mysiz, flip, vld,tld, locpos;
    ...
    if (side == Magma_tally3Left) {
        ...
        if ( versionL == 113 ) {
            ...
            for (blkj=nt-1; blkj >= 0; blkj--) {
                ...
                for (blki=mt; blki > 0; blki--) {
                    ...
                    magma_tally3_bulge_findpos113(N, NB, Vblksiz, mycol, myrow, &blkid);
                    ...
                    if (ncpy == 0) {
                        ...
                        if (mysiz > 0) {
                            ...
                            for( dev = 0; dev < ngpu; ++dev ) {
                                magma_tally3_setdevice( dev );
                                magma_tally3blasSetKernelStream( streams[ dev ][ 1 ] );
                                magma_tally3_zsetmatrix_async(vld, Vblksiz, V(vpos), vld, dV1[dev], vld, streams[dev][1]);
                                magma_tally3_zsetmatrix_async(tld, Vblksiz, T(tpos), tld, dT1[dev], tld, streams[dev][1]);
                            }
                    ...
                    if (blkid == copyst) {
                        ...
                        if (mysiz > 0) {
                            ...
                            if (flip == 0) { // now I am working on dV0 so copy the next and put it on dV1
                                ...
                                for( dev = 0; dev < ngpu; ++dev ) {
                                    magma_tally3_setdevice( dev );
                                    magma_tally3blasSetKernelStream( streams[ dev ][ 0 ] );
                                    magma_tally3_zsetmatrix_async(vld, Vblksiz, V(vpos), vld, dV0[dev], vld, streams[dev][0]);
                                    magma_tally3_zsetmatrix_async(tld, Vblksiz, T(tpos), tld, dT0[dev], tld, streams[dev][0]);
                                }
                            }
                        }
                    }
                    ...
                    if ((Vm > 0) && (Vn > 0)) {
                        ...
                        magma_tally3_int_t lcvpos   = locpos*Vblksiz*lddv;
                        magma_tally3_int_t lctpos   = locpos*Vblksiz*lddt;
                        ...
                        if (flip == 0) {
                            for( dev = 0; dev < ngpu; ++dev ) {
                                magma_tally3_int_t ie_loc   = min(ne_loc, NE - ne_loc*dev);
                                magma_tally3_int_t nr_bl = magma_tally3_ceildiv(ie_loc,10000);       //nr of blocks
                                magma_tally3_int_t sz_bl = magma_tally3_ceildiv(ie_loc,nr_bl*64)*64; //maximum size of blocks (to have blocks of around the same size and multiple of 64)
                                magma_tally3_int_t ib;                                        //size of current block
                                magma_tally3_setdevice( dev );
                                magma_tally3blasSetKernelStream(streams[dev][0]);
                                magma_tally3_queue_wait_event( streams[dev][0], myevent[dev][1] );
                                for (magma_tally3_int_t i=0; i < ie_loc; i += sz_bl) {
                                    ib = min(sz_bl, ie_loc-i);
                                    //magma_tally3_zlarfb_gpu( Magma_tally3Left, Magma_tally3NoTrans, Magma_tally3Forward, Magma_tally3Columnwise, Vm, ib, Vn, dV0[dev]+lcvpos, lddv, dT0[dev]+lctpos, lddt, dE(dev,myrow,i), ldde, dwork0[dev], ib);
                                    magma_tally3_zlarfb_gpu_gemm( Magma_tally3Left, Magma_tally3NoTrans, Magma_tally3Forward, Magma_tally3Columnwise, Vm, ib, Vn, dV0[dev]+lcvpos, lddv, dT0[dev]+lctpos, lddt, dE(dev,myrow,i), ldde, dwork0[dev], ib, dwvt0[dev], Vm);
                                }
                                magma_tally3_event_record( myevent[dev][0], streams[dev][0] );
                            }
                        } else {
                            for( dev = 0; dev < ngpu; ++dev ) {
                                magma_tally3_int_t ie_loc   = min(ne_loc, NE - ne_loc*dev);
                                ...
                                magma_tally3_setdevice( dev );
                                magma_tally3blasSetKernelStream(streams[dev][1]);
                                magma_tally3_queue_wait_event( streams[dev][1], myevent[dev][0] );
                                for (magma_tally3_int_t i=0; i < ie_loc; i += sz_bl) {
                                    ib = min(sz_bl, ie_loc-i);
                                    //magma_tally3_zlarfb_gpu( Magma_tally3Left, Magma_tally3NoTrans, Magma_tally3Forward, Magma_tally3Columnwise, Vm, ib, Vn, dV1[dev]+lcvpos, lddv, dT1[dev]+lctpos, lddt, dE(dev,myrow,i), ldde, dwork1[dev], ib);
                                    magma_tally3_zlarfb_gpu_gemm( Magma_tally3Left, Magma_tally3NoTrans, Magma_tally3Forward, Magma_tally3Columnwise, Vm, ib, Vn, dV1[dev]+lcvpos, lddv, dT1[dev]+lctpos, lddt, dE(dev,myrow,i), ldde, dwork1[dev], ib, dwvt1[dev], Vm);
                                }
                                magma_tally3_event_record( myevent[dev][1], streams[dev][1] );
                            }
                        }
                    }  // end for (Vm &Vn) > 0
                } // end for blki
         ...
         */
            ...
            exit(-1);
            mt    = magma_tally3_ceildiv((N-1),NB);
            for (blki = mt; blki > 0; blki--) {
                ...
                nt = magma_tally3_ceildiv(nbcolinvolvd,Vblksiz);
                /*loop over the tiles find the size of the Vs and apply it */
                for (blkj = nt-1; blkj >= 0; blkj--) {
                    ...
                    if ((Vm > 0) && (Vn > 0)) {
                         ...
                         * by the bulgechasing function*/
                        /*
                        magma_tally3_bulge_findVTpos(N, NB, Vblksiz, mycol, myrow, ldv, ldt, &vpos, &tpos);
                        magma_tally3_zsetmatrix_async(Vm, Vn, V(vpos), ldv, dV0, lddv, NULL);
                        magma_tally3_zsetmatrix_async(Vn,  Vn, T(tpos), ldt, dT0, lddt, NULL);
                        //printf("voici blki %d  rownbm %d mycol %d  coled %d  blkid %d vpos %d  tpos %d\n", blki, rownbm, mycol, coled, blkid, vpos, tpos);
                        for (magma_tally3_int_t i=0; i < NE; i += sz_bl) {
                            ib = min(sz_bl, NE-i);
                            magma_tally3_zlarfb_gpu( Magma_tally3Left, Magma_tally3NoTrans, Magma_tally3Forward, Magma_tally3Columnwise, Vm, ib, Vn, dV0, lddv, dT0, lddt, dE(myrow,i), ldde, dwork, NE);
                        }
                        */
                    } // end for (Vm &Vn) > 0
                } // end for blkj
            ...
            for (blkj=0; blkj < nt; blkj++) {
                ...
                for (blki=1; blki <= mt; blki++) {
                    ...
                    if ((Vm > 0) && (Vn > 0)) {
                        ...
                        magma_tally3_zsetmatrix_async(Vm, Vn, V(vpos), ldv, dV0, lddv, NULL);
                        magma_tally3_zsetmatrix_async(Vn,  Vn, T(tpos), ldt, dT0, lddt, NULL);
                        magma_tally3_zlarfb_gpu( Magma_tally3Right, Magma_tally3NoTrans, Magma_tally3Forward, Magma_tally3Columnwise, NE, Vm, Vn, dV0, lddv, dT0, lddt, dE(0, myrow), ldde, dwork, NE);
                        */
                    } // end for (Vm &Vn) > 0
                } // end for blki
            } // end fo blkj
    ...
    for( dev = 0; dev < ngpu; ++dev ) {
        ...
        magma_tally3blasSetKernelStream(streams[dev][0]);
        magma_tally3_queue_wait_event( streams[dev][0], myevent[dev][1] );
        magma_tally3_queue_wait_event( streams[dev][0], myevent[dev][0] );
        magma_tally3_int_t ie_loc   = min(ne_loc, NE - ne_loc*dev);
        magma_tally3_zgetmatrix_async( N, ie_loc, dE(dev, 0, 0), ldde, E+lde*ne_loc*dev, lde, streams[dev][0] );
        magma_tally3_event_record( myevent[dev][0], streams[dev][0] );
    }
    ...
    for( magma_tally3_int_t dev = 0; dev < ngpu; ++dev ) {
        ...
        magma_tally3blasSetKernelStream(streams[dev][0]);
        magma_tally3_queue_wait_event( streams[dev][0], myevent[dev][0] );
        magma_tally3_device_sync(); // no need for synchronize
        magma_tally3_free(dwork[dev]);
        magma_tally3_free(dE[dev]);
        for( magma_tally3_int_t i = 0; i < nbevents; ++i ) {
            magma_tally3_event_destroy( myevent[dev][i] );
        }
        for( magma_tally3_int_t i = 0; i < nstream; ++i ) {
            magma_tally3_queue_destroy( streams[dev][i] );
        }
    }
