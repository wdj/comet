    ...
    if ( blockIdx.x < n_full_block ) {
        ...
        for(int i=ty*8; i < ty*8 + 8; i++) {
            if ( i < tx ) {
                ...
                la[i][i] = MAGMA_minproduct_D_MAKE( MAGMA_minproduct_D_REAL( la[i][i] ), 0 );
            }
            #endif
        }
        __syncthreads();
        ...
        if ( n_mod_bs > 0 ) {
            ...
            for(int j=0; j < inf_bs; j += 4) {
                if ( tx < n_mod_bs ) {
                    ...
                    la[ty+j][tx] = MAGMA_minproduct_D_ZERO;
                }
            }
            __syncthreads();
        ...
        la[tx][ty] = MAGMA_minproduct_D_MAKE( res, 0. );
        __syncthreads();
        
        // first column of 32x1 threads computes final sum of each row
        if ( ty == 0 ) {
            res = res
                + MAGMA_minproduct_D_REAL( la[tx][1] )
                + MAGMA_minproduct_D_REAL( la[tx][2] )
                + MAGMA_minproduct_D_REAL( la[tx][3] );
            dwork[ind] = res;
        }
    }
    else {
        ...
        if ( ty == 0 && tx < n_mod_bs ) {
            ...
            res += MAGMA_minproduct_D_ABS( MAGMA_minproduct_D_REAL( *A ));
            A += 1;
        ...
        if ( n_mod_bs > 0 ) {
            ...
            for(int j=0; j < inf_bs; j += 4) {
                if ( ty+j < n_mod_bs ) {               //#
                    ...
                    la[tx][ty+j] = MAGMA_minproduct_D_ZERO;       //#
                }
            }
            __syncthreads();
...
dlansy_inf(
    ...
    magma_minproductDouble_const_ptr A, int lda,
    magma_minproductDouble_ptr dwork )
{
    ...
    if ( uplo == Magma_minproductLower) {
        dlansy_inf_kernel_generic_lower<<< grid, threads, 0, magma_minproduct_stream >>>
            ( n, A, lda, dwork, n_full_block, n_mod_bs );
    }
    else {
        dlansy_inf_kernel_generic_upper<<< grid, threads, 0, magma_minproduct_stream >>>
            ( n, A, lda, dwork, n_full_block, n_mod_bs );
    }
}
    ...
    int n, const double* A, int lda, double *dwork )
{
    ...
    if (ind < n) {
        ...
        res = fmax( res, MAGMA_minproduct_D_ABS( MAGMA_minproduct_D_REAL( *A )));
        dwork[ind] = res;
    }
}
    ...
    magma_minproductDouble_ptr dwork )
{
    ...
    if ( uplo == Magma_minproductLower ) {
        dlansy_max_kernel_lower<<< grid, threads, 0, magma_minproduct_stream >>>
            ( n, A, lda, dwork );
    }
    else {
        dlansy_max_kernel_upper<<< grid, threads, 0, magma_minproduct_stream >>>
            ( n, A, lda, dwork );
    }
}
