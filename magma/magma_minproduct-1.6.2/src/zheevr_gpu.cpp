...
*******     (workspace) If FAST_HEMV is defined DZ should be (LDDZ, max(1,N)) in both cases.
    ...
    isuppz  INTEGER ARRAY, dimension ( 2*max(1,M) )
            ...
            ISUPPZ( 2*i ).
            __Implemented only for__ RANGE = Magma_minproductRangeAll or Magma_minproductRangeI and IU - IL = N - 1
    ...
    ldwz    INTEGER
            ...
            JOBZ = Magma_minproductVec, LDWZ >= max(1,N).
...
magma_minproduct_zheevr_gpu(
    ...
    double *w,
    magma_minproductDoubleComplex_ptr dZ, magma_minproduct_int_t lddz, magma_minproduct_int_t *isuppz,
    ...
    magma_minproduct_int_t *info)
{
    ...
    magma_minproduct_int_t indrd, indre;
    magma_minproduct_int_t imax;
    magma_minproduct_int_t lopt, itmp1, indree, indrdd;
    magma_minproduct_int_t lower, wantz, tryrac;
    magma_minproduct_int_t i, j, jj, i__1;
    magma_minproduct_int_t alleig, valeig, indeig;
    magma_minproduct_int_t iscale, indibl, indifl;
    magma_minproduct_int_t indiwo, indisp, indtau;
    magma_minproduct_int_t indrwk, indwk;
    magma_minproduct_int_t llwork, llrwork, nsplit;
    magma_minproduct_int_t lquery, ieeeok;
    ...
    *info = 0;
    ...
    work[0] = MAGMA_minproduct_Z_MAKE( lwmin, 0 );
    rwork[0] = lrwmin;
    iwork[0] = liwmin;
    ...
    *m = 0;
    ...
    if (n <= 128) {
        ...
        magma_minproduct_zmalloc_cpu( &a, n*n );
        magma_minproduct_zgetmatrix(n, n, dA, ldda, a, n);
        lapackf77_zheevr(jobz_, range_, uplo_,
                         &n, a, &n, &vl, &vu, &il, &iu, &abstol, m,
                         w, wZ, &ldwz, isuppz, work, &lwork,
                         rwork, &lrwork, iwork, &liwork, info);
        magma_minproduct_zsetmatrix( n,  n,  a,    n, dA, ldda);
        magma_minproduct_zsetmatrix( n, *m, wZ, ldwz, dZ, lddz);
        magma_minproduct_free_cpu(a);
        return *info;
    }

    if (MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc( &dwork, n )) {
        fprintf (stderr, "!!!! device memory allocation error (magma_minproduct_zheevr_gpu)\n");
    ...
    anrm = magma_minproductblas_zlanhe(Magma_minproductMaxNorm, uplo, n, dA, ldda, dwork);
    iscale = 0;
    sigma  = 1;
    if (anrm > 0. && anrm < rmin) {
        iscale = 1;
    ...
    if (iscale == 1) {
        d__1 = 1.;
        magma_minproductblas_zlascl(uplo, 0, 0, 1., sigma, n, n, dA,
                         ldda, info);
...
#ifdef FAST_HEMV
    magma_minproduct_zhetrd2_gpu(uplo, n, dA, ldda, &rwork[indrd], &rwork[indre],
                      &work[indtau], wA, ldwa, &work[indwk], llwork, dZ, lddz*n, &iinfo);
#else
    magma_minproduct_zhetrd_gpu (uplo, n, dA, ldda, &rwork[indrd], &rwork[indre],
                      &work[indtau], wA, ldwa, &work[indwk], llwork, &iinfo);
#endif

    lopt = n + (magma_minproduct_int_t)MAGMA_minproduct_Z_REAL(work[indwk]);
    ...
    if (! wantz) {
        ...
        if (*info == 0 && wantz) {
            magma_minproduct_zsetmatrix( n, *m, wZ, ldwz, dZ, lddz );
            magma_minproduct_zunmtr_gpu(Magma_minproductLeft, uplo, Magma_minproductNoTrans, n, *m, dA, ldda, &work[indtau],
    ...
    if (wantz) {
        for (j = 1; j <= *m-1; ++j) {
            ...
            if (i != 0) {
                ...
                magma_minproduct_zswap(n, dZ + (i-1)*lddz, ione, dZ + (j-1)*lddz, ione);
            }
        }
    }
    
    /* Set WORK[0] to optimal complex workspace size. */
    work[1] = MAGMA_minproduct_Z_MAKE( lopt, 0 );
