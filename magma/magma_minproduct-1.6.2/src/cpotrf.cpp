...
#define dA(i_, j_) (dA + (j_)*ldda + (i_))
    ...
    *info = 0;
    ...
    if ( ngpu > 1 ) {
        ...
        return magma_minproduct_cpotrf_m(ngpu, uplo, n, A, lda, info);
    }
    ...
    if (nb <= 1 || nb >= n) {
        ...
        if (upper) {
            ...
            for (j=0; j < n; j += nb) {
                ...
                jb = min(nb, (n-j));
                magma_minproduct_csetmatrix_async( jb, (n-j), A(j, j), lda, dA(j, j), ldda, stream[1]);
                ...
                lapackf77_cpotrf(Magma_minproductUpperStr, &jb, A(j, j), &lda, info);
                if (*info != 0) {
                    *info = *info + j;
                    break;
                }
            ...
            for (j=0; j < n; j += nb) {
                ...
                //  for non-positive-definiteness. Computing MIN
                jb = min(nb, (n-j));
                magma_minproduct_csetmatrix_async( (n-j), jb, A(j, j), lda, dA(j, j), ldda, stream[1]);
                ...
                magma_minproduct_cgetmatrix_async( jb, j,
                                        dA(j, 0), ldda,
                                        A(j, 0),  lda, stream[2] );

                magma_minproduct_queue_sync( stream[0] );
                lapackf77_cpotrf(Magma_minproductLowerStr, &jb, A(j, j), &lda, info);
                if (*info != 0) {
                    *info = *info + j;
                    break;
                }
