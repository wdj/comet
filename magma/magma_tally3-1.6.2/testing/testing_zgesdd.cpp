...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( VT,    magma_tally3DoubleComplex, ldv*N );   // N x N (jobz=A) or min(M,N) x N
            TESTING_MALLOC_CPU( U,     magma_tally3DoubleComplex, ldu*N_U ); // M x M (jobz=A) or M x min(M,N)
            ...
            if (info != 0)
                printf("magma_tally3_zgesdd returned error %d: %s.\n",
            ...
            if ( opts.lapack ) {
                ...
                lapackf77_zgesdd( lapack_vec_const_tally3(jobz), &M, &N,
