...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lapackf77_zlacpy( Magma_tally3FullStr, &M, &N, h_A, &lda, h_R, &lda );
            ...
            magma_tally3_zgelqf_gpu( M, N, d_A, ldda, tau, h_work, lwork, &info);
            gpu_time = magma_tally3_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally3_zgelqf_gpu returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                magma_tally3DoubleComplex *Q, *L;
                double *work;
                TESTING_MALLOC_CPU( Q,    magma_tally3DoubleComplex, ldq*N );       // K by N
                TESTING_MALLOC_CPU( L,    magma_tally3DoubleComplex, ldl*min_mn );  // M by K
