...
} magma_tally3gpu_dswapblk_params_t;

__global__ void magma_tally3gpu_dswapblkrm( magma_tally3gpu_dswapblk_params_t params )
{
...
__global__ void magma_tally3gpu_dswapblkcm( magma_tally3gpu_dswapblk_params_t params )
{
    unsigned int y = threadIdx.x + blockDim.x*blockIdx.x;
    ...
    magma_tally3_queue_t queue )
{
    ...
    if ( order == Magma_tally3ColMajor ) {
        for( k=(i1-1); k<i2; k+=BLOCK_SIZE )
        {
            ...
            magma_tally3gpu_dswapblk_params_t params = { dA+k, dB, n, ldda, lddb, sb };
            ...
            magma_tally3gpu_dswapblkcm<<< blocks, blocksize, 0, queue >>>( params );
        }
    }
    else {
        for( k=(i1-1); k<i2; k+=BLOCK_SIZE )
        {
            magma_tally3_int_t sb = min(BLOCK_SIZE, i2-k);
            magma_tally3gpu_dswapblk_params_t params = { dA+k*ldda, dB, n, ldda, lddb, sb };
            for( magma_tally3_int_t j = 0; j < sb; j++ )
            {
                im = ipiv[(k+j)*inci] - 1;
                if ( (k+j) == im )
            ...
            magma_tally3gpu_dswapblkrm<<< blocks, blocksize, 0, queue >>>( params );
        }
    }
}
...
/**
    @see magma_tally3blas_dswapblk_q
    @ingroup magma_tally3_dblas2
    ********************************************************************/
extern "C" void 
