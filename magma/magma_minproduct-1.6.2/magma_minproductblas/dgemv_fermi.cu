    ...
    double       * __restrict__ y, int incy)
{
    ...
    if ( threadIdx.x < npart ) {
        ...
        sx[threadIdx.x] = MAGMA_minproduct_D_ZERO;
    }
    __syncthreads();
...
magma_minproductblas_dgemv(
    magma_minproduct_trans_t trans, magma_minproduct_int_t m, magma_minproduct_int_t n, double alpha,
    ...
    magma_minproductDouble_ptr dy, magma_minproduct_int_t incy)
{
    ...
    if ( arch < 200  ) {
        ...
        magma_minproduct_dgemv( trans, m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
        #else
        magma_minproductblas_dgemv_tesla( trans, m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
        #endif
        return;
    }
    ...
    if ( trans == Magma_minproductNoTrans ) {
        ...
        dgemvn_kernel1_fermi<<< grid, threads, 0, magma_minproduct_stream >>>
            ( m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    }
    else if ( trans == Magma_minproductTrans ) {
        dim3 grid    ( 1, n, 1 );
        dim3 threads ( BLK_X, 1, 1 );
        dgemvt_kernel_fermi<<< grid, threads, 0, magma_minproduct_stream >>>
            ( m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    }
    else if ( trans == Magma_minproductConjTrans ) {
        dim3 grid    ( 1, n, 1 );
        dim3 threads ( BLK_X, 1, 1 );
        dgemvc_kernel_fermi<<< grid, threads, 0, magma_minproduct_stream >>>
            ( m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    }
}
