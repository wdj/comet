...
/**
    ...
    This version stores the T matrices, for later use in magma_minproduct_dorghr.
    ...
    magma_minproduct_int_t *info)
{
    ...
    magma_minproduct_int_t nb = magma_minproduct_get_dgehrd_nb(n);
    ...
    *info = 0;
    iws = n*(nb + nb*ngpu);
    work[0] = MAGMA_minproduct_D_MAKE( iws, 0 );
    ...
    if (nb == 1 || nb >= nh) {
        ...
        for( d = 0; d < ngpu; ++d ) {
            ...
            ldwork = nlocal*ldda   // A
                   ...
                   + nb*ldda       // W
                   + nb*nb;        // Ti
            if ( MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc( &data.A[d], ldwork )) {
        ...
        magma_minproduct_dsetmatrix_1D_col_bcyclic( n, n, A, lda, data.A, ldda, ngpu, nb );
        ...
        for (i = ilo; i < ihi - 1 - nb; i += nb) {
            ...
            if ( i > ilo ) {
                ...
                magma_minproduct_dgetmatrix( ihi-i, nb,
                                  dA(dpanel, i, di), ldda,
                                  A(i,i),            lda );
            }
            
            // add 1 to i for 1-based index
            magma_minproduct_dlahr2_m( ihi, i+1, nb, A(0,i), lda,
                            &tau[i], &T[i*nb], nb, work, n, &data );
            
            magma_minproduct_dlahru_m( n, ihi, i, nb, A, lda, &data );
            
            // copy first i rows above panel to host
            magma_minproduct_setdevice( dpanel );
            magma_minproduct_dgetmatrix_async( i, nb,
                                    dA(dpanel, 0, di), ldda,
        ...
        for( i2 = i; i2 < n; i2 += nb ) {
            ...
            di = (i2 / nb) / ngpu * nb;
            magma_minproduct_setdevice( d );
            magma_minproduct_dgetmatrix( n, ib,
                              dA(d, 0, di), ldda,
                              A(0,i2),      lda );
        }
    }
