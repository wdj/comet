...
int main( int argc, char** argv)
{
    ...
    double           eps;
    magma_minproductDoubleComplex *h_A, *h_R, *h_Q, *h_work, *work;
    magma_minproductDoubleComplex_ptr d_R, dwork;
    ...
    printf("2 - uses ZHEMV from MAGMA_minproduct BLAS that requires extra space\n\n");
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_DEV( d_R,     magma_minproductDoubleComplex, ldda*N );
            ...
            if (opts.version == 1) {
                magma_minproduct_zhetrd_gpu( opts.uplo, N, d_R, ldda, diag, offdiag,
                                  tau, h_R, lda, h_work, lwork, &info );
            }
            else {
                magma_minproduct_zhetrd2_gpu( opts.uplo, N, d_R, ldda, diag, offdiag,
                                   tau, h_R, lda, h_work, lwork, dwork, ldwork, &info );
            }
            gpu_time = magma_minproduct_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_minproduct_zhetrd_gpu returned error %d: %s.\n",
            ...
            /* =====================================================================
               ...
               =================================================================== */
            if ( opts.check ) {
                TESTING_MALLOC_CPU( h_Q,  magma_minproductDoubleComplex, lda*N );
                ...
                magma_minproduct_zgetmatrix( N, N, d_R, ldda, h_Q, lda );
