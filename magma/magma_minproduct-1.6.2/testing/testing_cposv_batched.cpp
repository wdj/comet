...
int main(int argc, char **argv)
{
    ...
    magma_minproductFloatComplex *h_A, *h_B, *h_X;
    ...
    magma_minproductFloatComplex **dB_array = NULL;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_DEV( d_B, magma_minproductFloatComplex, lddb*nrhs*batchCount );
            ...
            magma_minproduct_csetmatrix( N, N*batchCount,    h_A, lda, d_A, ldda );
            magma_minproduct_csetmatrix( N, nrhs*batchCount, h_B, ldb, d_B, lddb );
            ...
            info = magma_minproduct_cposv_batched(opts.uplo, N, nrhs, dA_array, ldda, dB_array, lddb, dinfo_array, batchCount, queue); 
            ...
            {
                if(cpu_info[i] != 0 ){
                    printf("magma_minproduct_cposv_batched matrix %d returned internal error %d\n",i, (int)cpu_info[i] );
                }
            }
            if (info != 0)
                printf("magma_minproduct_cposv_batched returned argument error %d: %s.\n", (int) info, magma_minproduct_strerror( info ));
            
            //=====================================================================
            // Residual
            //=====================================================================
            magma_minproduct_cgetmatrix( N, nrhs*batchCount, d_B, lddb, h_X, ldb );
            ...
            if ( opts.lapack ) {
                ...
                for(magma_minproduct_int_t s=0; s<batchCount; s++)
                {
                    lapackf77_cposv( lapack_uplo_const(opts.uplo), &N, &nrhs, h_A + s * lda * N, &lda, h_B + s * ldb * nrhs, &ldb, &info );
                }
