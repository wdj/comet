...
static int diff_matrix( magma_minproduct_int_t m, magma_minproduct_int_t n, double *A, magma_minproduct_int_t lda, double *B, magma_minproduct_int_t ldb )
{
    for( magma_minproduct_int_t j = 0; j < n; j++ ) {
        for( magma_minproduct_int_t i = 0; i < m; i++ ) {
            if ( ! MAGMA_minproduct_D_EQUAL( A[lda*j+i], B[ldb*j+i] ) )
                return 1;
        }
    }
    return 0;
...
static void init_matrix( magma_minproduct_int_t m, magma_minproduct_int_t n, double *A, magma_minproduct_int_t lda, magma_minproduct_int_t offset )
{
    assert( lda >= m );
    for( magma_minproduct_int_t j = 0; j < n; ++j ) {
        for( magma_minproduct_int_t i=0; i < m; ++i ) {
            A[i + j*lda] = MAGMA_minproduct_D_MAKE( offset + (i+1) + (j+1)/10000., 0 );
        }
    }
}
...
int main( int argc, char** argv)
{
    ...
    double *h_A1, *h_A2;
    double *h_R1, *h_R2;
    magma_minproductDouble_ptr d_A1, d_A2;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lda    = N;
            ldda   = ((N+31)/32)*32;
            nb     = (opts.nb > 0 ? opts.nb : magma_minproduct_get_dgetrf_nb( N ));
            nb     = min( N, nb );
            ...
            magma_minproduct_dsetmatrix( N, N, h_A1, lda, d_A1, ldda );
            magma_minproduct_dsetmatrix( N, N, h_A2, lda, d_A2, ldda );
            ...
            for( j=0; j < nb; j++) {
                if ( j != (ipiv[j]-1)) {
                    ...
                    #else
                        magma_minproduct_dswap( N, d_A1, ldda*j, 1, d_A2, ldda*(ipiv[j]-1), 1, opts.queue );
                    #endif
                }
            }
            ...
            magma_minproduct_dgetmatrix( N, N, d_A1, ldda, h_R1, lda );
            magma_minproduct_dgetmatrix( N, N, d_A2, ldda, h_R2, lda );
            ...
            for( j=0; j < nb; j++) {
                if ( j != (ipiv[j]-1)) {
                    ...
                    #else
                        magma_minproduct_dswap( N, d_A1, j, ldda, d_A2, ipiv[j]-1, ldda, opts.queue );
                    #endif
                }
            }
            time = magma_minproduct_sync_wtime( queue ) - time;
            ...
            for( j=0; j < nb; j++) {
                if ( j != (ipiv[j]-1)) {
                    magma_minproductblas_dswap( N, d_A1+ldda*j, 1, d_A2+ldda*(ipiv[j]-1), 1);
                }
            }
            time = magma_minproduct_sync_wtime( queue ) - time;
            ...
            for( j=0; j < nb; j++) {
                if ( j != (ipiv[j]-1)) {
                    magma_minproductblas_dswap( N, d_A1+j, ldda, d_A2+ipiv[j]-1, ldda );
                }
            }
            time = magma_minproduct_sync_wtime( queue ) - time;
            ...
            time = magma_minproduct_sync_wtime( queue );
            magma_minproductblas_dswapblk( Magma_minproductRowMajor, N, d_A1, ldda, d_A2, ldda, 1, nb, ipiv, 1, 0);
            time = magma_minproduct_sync_wtime( queue ) - time;
            ...
            time = magma_minproduct_sync_wtime( queue );
            magma_minproductblas_dswapblk( Magma_minproductColMajor, N, d_A1, ldda, d_A2, ldda, 1, nb, ipiv, 1, 0);
            time = magma_minproduct_sync_wtime( queue ) - time;
            ...
            time = magma_minproduct_sync_wtime( queue );
            magma_minproductblas_dlaswp( N, d_A1, ldda, 1, nb, ipiv, 1);
            time = magma_minproduct_sync_wtime( queue ) - time;
            ...
            magma_minproductblas_dlaswp2( N, d_A1, ldda, 1, nb, d_ipiv, 1 );
            time = magma_minproduct_sync_wtime( queue ) - time;
            ...
            time = magma_minproduct_sync_wtime( queue );
            magma_minproductblas_dlaswpx( N, d_A1, ldda, 1, 1, nb, ipiv, 1);
            time = magma_minproduct_sync_wtime( queue ) - time;
            ...
            time = magma_minproduct_sync_wtime( queue );
            magma_minproductblas_dlaswpx( N, d_A1, 1, ldda, 1, nb, ipiv, 1);
            time = magma_minproduct_sync_wtime( queue ) - time;
            ...
            time = magma_minproduct_sync_wtime( queue );
            magma_minproduct_dcopymatrix( N, nb, d_A1, ldda, d_A2, ldda );
            time = magma_minproduct_sync_wtime( queue ) - time;
            // copy reads 1 matrix and writes 1 matrix, so has half gbytes of swap
            col_perf6 = 0.5 * gbytes / time;
            
            time = magma_minproduct_sync_wtime( queue );
            magma_minproduct_dcopymatrix( nb, N, d_A1, ldda, d_A2, ldda );
            time = magma_minproduct_sync_wtime( queue ) - time;
            // copy reads 1 matrix and writes 1 matrix, so has half gbytes of swap
