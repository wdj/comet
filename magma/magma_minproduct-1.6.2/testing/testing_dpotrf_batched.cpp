...
int main( int argc, char** argv)
{
    ...
    for( int i = 0; i < opts.ntest; ++i ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lapackf77_dlacpy( Magma_minproductUpperLowerStr, &N, &(columns), h_A, &lda, h_R, &lda );
            magma_minproduct_dsetmatrix( N, columns, h_A, lda, d_A, ldda );
            ...
            info = magma_minproduct_dpotrf_batched( opts.uplo, N, d_A_array, ldda, dinfo_magma_minproduct, batchCount, queue);
            ...
            {
                if(cpu_info[i] != 0 ){
                    printf("magma_minproduct_dpotrf_batched matrix %d returned internal error %d\n",i, (int)cpu_info[i] );
                }
            }
            if (info != 0)
                printf("magma_minproduct_dpotrf_batched returned argument error %d: %s.\n", (int) info, magma_minproduct_strerror( info ));
            ...
            if ( opts.lapack ) {
                ...
                for(int i=0; i<batchCount; i++)
                {
                   lapackf77_dpotrf( lapack_uplo_const(opts.uplo), &N, h_A + i * lda * N, &lda, &info );
                }
                ...
                /* =====================================================================
                   ...
                   =================================================================== */
                 magma_minproduct_dgetmatrix( N, columns, d_A, ldda, h_R, lda );
