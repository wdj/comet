...
magma_minproduct_zgetrf_mc(magma_minproduct_context *cntxt,
        int *m, int *n,
...
/*  -- MAGMA_minproduct (version 1.6.1) --
    ...
    if (cntxt->num_cores == 1 && cntxt->num_gpus == 1)
      {
    //int result = magma_minproduct_zgetrf(*m, *n, a, *lda, ipiv, info);
    //return result;
      }
    ...
    cuDoubleComplex fone = MAGMA_minproduct_Z_ONE;
    cuDoubleComplex mone = MAGMA_minproduct_Z_NEG_ONE;
    ...
    *info = 0;
    ...
    int nb = (EN_BEE==-1)? magma_minproduct_get_zpotrf_nb(*n): EN_BEE;
