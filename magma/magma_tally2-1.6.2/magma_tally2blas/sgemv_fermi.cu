    ...
    float       * __restrict__ y, int incy)
{
    ...
    if ( threadIdx.x < npart ) {
        ...
        sx[threadIdx.x] = MAGMA_tally2_S_ZERO;
    }
    __syncthreads();
...
magma_tally2blas_sgemv(
    magma_tally2_trans_t trans, magma_tally2_int_t m, magma_tally2_int_t n, float alpha,
    ...
    magma_tally2Float_ptr dy, magma_tally2_int_t incy)
{
    ...
    if ( arch < 200  ) {
        ...
        magma_tally2_sgemv( trans, m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
        #else
        magma_tally2blas_sgemv_tesla( trans, m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
        #endif
        return;
    }
    ...
    if ( trans == Magma_tally2NoTrans ) {
        ...
        sgemvn_kernel1_fermi<<< grid, threads, 0, magma_tally2_stream >>>
            ( m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    }
    else if ( trans == Magma_tally2Trans ) {
        dim3 grid    ( 1, n, 1 );
        dim3 threads ( BLK_X, 1, 1 );
        sgemvt_kernel_fermi<<< grid, threads, 0, magma_tally2_stream >>>
            ( m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    }
    else if ( trans == Magma_tally2ConjTrans ) {
        dim3 grid    ( 1, n, 1 );
        dim3 threads ( BLK_X, 1, 1 );
        sgemvc_kernel_fermi<<< grid, threads, 0, magma_tally2_stream >>>
            ( m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    }
}
