/*
    -- MAGMA_tally3 (version 1.1) --
       ...
       @generated from magma_tally3_z_precond_wrapper.cpp normal z -> s, Mon May  4 11:57:23 2015
       @author Hartwig Anzt
    ...
    magma_tally3_queue_t queue )
{
    ...
    magma_tally3_s_solver_par psolver_par;
    ...
    magma_tally3_s_preconditioner pprecond;
    pprecond.solver = Magma_tally3_NONE;

    switch( precond->solver ) {
        case  Magma_tally3_CG:
                CHECK( magma_tally3_scg_res( A, b, x, &psolver_par, queue )); break;
        case  Magma_tally3_BICGSTAB:
                CHECK( magma_tally3_sbicgstab( A, b, x, &psolver_par, queue )); break;
        case  Magma_tally3_GMRES:
                CHECK( magma_tally3_sfgmres( A, b, x, &psolver_par, &pprecond, queue )); break;
        case  Magma_tally3_JACOBI:
                CHECK( magma_tally3_sjacobi( A, b, x, &psolver_par, queue )); break;
        case  Magma_tally3_BAITER:
                CHECK( magma_tally3_sbaiter( A, b, x, &psolver_par, queue )); break;
        default:
                CHECK( magma_tally3_scg_res( A, b, x, &psolver_par, queue )); break;
    ...
    magma_tally3_queue_t queue )
{
    ...
    if ( precond->solver == Magma_tally3_JACOBI ) {
        return magma_tally3_sjacobisetup_diagscal( A, &(precond->d), queue );
    }
    else if ( precond->solver == Magma_tally3_PASTIX ) {
        //return magma_tally3_spastixsetup( A, b, precond, queue );
        return MAGMA_tally3_ERR_NOT_SUPPORTED;
    }
    else if ( precond->solver == Magma_tally3_ILU ) {
        return magma_tally3_scumilusetup( A, precond, queue );
    }
    else if ( precond->solver == Magma_tally3_ICC ) {
        return magma_tally3_scumiccsetup( A, precond, queue );
    }
    else if ( precond->solver == Magma_tally3_AICC ) {
        return magma_tally3_sitericsetup( A, b, precond, queue );
    }
    else if ( precond->solver == Magma_tally3_AILU ) {
        return magma_tally3_siterilusetup( A, b, precond, queue );
    }
    ...
    magma_tally3_queue_t queue )
{
    ...
    magma_tally3_s_matrix tmp={Magma_tally3_CSR};
    ...
    if ( precond->solver == Magma_tally3_JACOBI ) {
        CHECK( magma_tally3_sjacobi_diagscal( A.num_rows, precond->d, b, x, queue ));
    }
    else if ( precond->solver == Magma_tally3_PASTIX ) {
        //CHECK( magma_tally3_sapplypastix( b, x, precond, queue ));
        return MAGMA_tally3_ERR_NOT_SUPPORTED;
    }
    else if ( precond->solver == Magma_tally3_ILU ) {
        CHECK( magma_tally3_svinit( &tmp, Magma_tally3_DEV, A.num_rows, b.num_cols, MAGMA_tally3_S_ZERO, queue ));
    }
    else if ( precond->solver == Magma_tally3_ICC ) {
        CHECK( magma_tally3_svinit( &tmp, Magma_tally3_DEV, A.num_rows, b.num_cols, MAGMA_tally3_S_ZERO, queue ));
    }
    else if ( precond->solver == Magma_tally3_NONE ) {
        magma_tally3_scopy( b.num_rows*b.num_cols, b.dval, 1, x->dval, 1 );      //  x = b
    }
    else {
        printf( "error: preconditioner type not yet supported.\n" );
    ...
    magma_tally3_queue_t queue )
{
    ...
    if ( precond->solver == Magma_tally3_JACOBI ) {
        ...
        CHECK( magma_tally3_sapplycumilu_l( b, x, precond, queue ));
    }
    else if ( ( precond->solver == Magma_tally3_ILU ||
                precond->solver == Magma_tally3_AILU ) && precond->maxiter < 50 ) {
        magma_tally3_scopy( b.num_rows*b.num_cols, b.dval, b.num_cols, x->dval, b.num_cols );
        magma_tally3_s_solver_par solver_par;
        solver_par.maxiter = precond->maxiter;
        //magma_tally3_sjacobiiter_sys( precond->L, b, precond->d, precond->work1, x, &solver_par, queue );
        CHECK( magma_tally3_sjacobispmvupdate(precond->maxiter, precond->L, precond->work1, b, precond->d, x, queue ));
    }
    else if ( ( precond->solver == Magma_tally3_ICC ||
                precond->solver == Magma_tally3_AICC ) && precond->maxiter >= 50 )  {
        CHECK( magma_tally3_sapplycumicc_l( b, x, precond, queue ));
    }
    else if ( ( precond->solver == Magma_tally3_ICC ||
                precond->solver == Magma_tally3_AICC ) && precond->maxiter < 50 )  {
        magma_tally3_scopy( b.num_rows*b.num_cols, b.dval, b.num_cols, x->dval, b.num_cols );
        ...
        CHECK( magma_tally3_sapplycustomprecond_l( b, x, precond, queue ));
    }
    else {
        printf( "error: preconditioner type not yet supported.\n" );
        magma_tally3blasSetKernelStream( orig_queue );
    ...
    magma_tally3_queue_t queue )
{
    ...
    if ( precond->solver == Magma_tally3_JACOBI ) {
        magma_tally3_scopy( b.num_rows*b.num_cols, b.dval, 1, x->dval, 1 );    // x = b
                ...
                precond->solver == Magma_tally3_AILU ) && precond->maxiter >= 50 ) {
        CHECK( magma_tally3_sapplycumilu_r( b, x, precond, queue ));
    }
    else if ( ( precond->solver == Magma_tally3_ILU ||
                precond->solver == Magma_tally3_AILU ) && precond->maxiter < 50 ) {
        magma_tally3_scopy( b.num_rows*b.num_cols, b.dval, b.num_cols, x->dval, b.num_cols );
        magma_tally3_s_solver_par solver_par;
        solver_par.maxiter = precond->maxiter;
        //magma_tally3_sjacobiiter_sys( precond->U, b, precond->d2, precond->work2, x, &solver_par, queue );
        CHECK( magma_tally3_sjacobispmvupdate(precond->maxiter, precond->U, precond->work2, b, precond->d2, x, queue ));
    }

    else if ( ( precond->solver == Magma_tally3_ICC ||
                precond->solver == Magma_tally3_AICC ) && precond->maxiter >= 50 ) {
        CHECK( magma_tally3_sapplycumicc_r( b, x, precond, queue ));
    }
    else if ( ( precond->solver == Magma_tally3_ICC ||
               precond->solver == Magma_tally3_AICC ) && precond->maxiter < 50 ) {
        magma_tally3_scopy( b.num_rows*b.num_cols, b.dval, b.num_cols, x->dval, b.num_cols );
        ...
        CHECK( magma_tally3_sapplycustomprecond_r( b, x, precond, queue ));
    }
    else {
        printf( "error: preconditioner type not yet supported.\n" );
        magma_tally3blasSetKernelStream( orig_queue );
