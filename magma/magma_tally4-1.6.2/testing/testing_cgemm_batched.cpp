...
int main( int argc, char** argv)
{
    ...
    float          magma_tally4_error, cublas_error, magma_tally4_err, cublas_err, Cnorm, work[1];
    ...
    magma_tally4FloatComplex *h_A, *h_B, *h_C, *h_Cmagma_tally4, *h_Ccublas;
    magma_tally4FloatComplex *d_A, *d_B, *d_C;
    ...
    magma_tally4FloatComplex **A_array = NULL;
    magma_tally4FloatComplex **B_array = NULL;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_C,  magma_tally4FloatComplex, sizeC  );
            TESTING_MALLOC_CPU( h_Cmagma_tally4,  magma_tally4FloatComplex, sizeC  );
            TESTING_MALLOC_CPU( h_Ccublas, magma_tally4FloatComplex, sizeC  );

            TESTING_MALLOC_DEV( d_A, magma_tally4FloatComplex, ldda*An*batchCount );
            TESTING_MALLOC_DEV( d_B, magma_tally4FloatComplex, lddb*Bn*batchCount );
            TESTING_MALLOC_DEV( d_C, magma_tally4FloatComplex, lddc*N*batchCount  );
            ...
            /* =====================================================================
               ...
               =================================================================== */
            magma_tally4_csetmatrix( Am, An*batchCount, h_A, lda, d_A, ldda );
            magma_tally4_csetmatrix( Bm, Bn*batchCount, h_B, ldb, d_B, lddb );
            magma_tally4_csetmatrix( M, N*batchCount, h_C, ldc, d_C, lddc );
            ...
            magma_tally4blas_cgemm_batched(opts.transA, opts.transB, M, N, K,
                             alpha, A_array, ldda,
                                    B_array, lddb,
            ...
            magma_tally4_cgetmatrix( M, N*batchCount, d_C, lddc, h_Cmagma_tally4, ldc );
            ...
            cublasCgemmBatched(handle, cublas_trans_const_tally4(opts.transA), cublas_trans_const_tally4(opts.transB), M, N, K,
                               &alpha, (const magma_tally4FloatComplex**) A_array, ldda,
                               (const magma_tally4FloatComplex**) B_array, lddb,
                               &beta,  C_array, lddc, batchCount );

            cublas_time = magma_tally4_sync_wtime( NULL ) - cublas_time;
            cublas_perf = gflops / cublas_time;
            
            magma_tally4_cgetmatrix( M, N*batchCount, d_C, lddc, h_Ccublas, ldc );
            ...
            if ( opts.lapack ) {
                ...
                for(int s=0; s<batchCount; s++)
                {
                    ...
                    blasf77_caxpy( &C_batchSize, &c_neg_one, h_C + s*C_batchSize, &ione, h_Cmagma_tally4 + s*C_batchSize, &ione );
                    magma_tally4_err = lapackf77_clange( "M", &M, &N, h_Cmagma_tally4 + s*C_batchSize, &ldc, work ) / Cnorm; 
                ...
                // compute relative error for magma_tally4, relative to cublas
                    ...
                    blasf77_caxpy( &sizeC, &c_neg_one, h_Ccublas, &ione, h_Cmagma_tally4, &ione );
                    magma_tally4_error = lapackf77_clange( "M", &M, &NN, h_Cmagma_tally4, &ldc, work ) / Cnorm;
