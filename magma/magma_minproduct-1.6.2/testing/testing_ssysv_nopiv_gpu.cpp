...
int main(int argc, char **argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            bool nopiv = true;
            if ( nopiv ) {
                magma_minproduct_smake_hpd( N, h_A, lda );  // SPD / HPD does not require pivoting
            }
            else {
                magma_minproduct_smake_symmetric( N, h_A, lda );  // symmetric/symmetric generally requires pivoting
            }
            
            magma_minproduct_ssetmatrix( N, N,    h_A, lda, d_A, ldda );
            magma_minproduct_ssetmatrix( N, nrhs, h_B, ldb, d_B, lddb );
            ...
            magma_minproduct_ssysv_nopiv_gpu( opts.uplo, N, nrhs, d_A, ldda, d_B, lddb, &info );
            gpu_time = magma_minproduct_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_minproduct_sgesv_gpu returned error %d: %s.\n",
            ...
            if ( opts.lapack ) {
                ...
                lwork = (magma_minproduct_int_t) MAGMA_minproduct_S_REAL( temp );
