...
zlarfg_device(
    int n,
    magma_tally3DoubleComplex* dalpha, magma_tally3DoubleComplex* dx, int incx,
    magma_tally3DoubleComplex* dtau,  double* swork, double* sscale, magma_tally3DoubleComplex* scale)
{
    ...
    magma_tally3_max_reduce<BLOCK_SIZE>( tx, swork );
    ...
    if ( *sscale > 0 ) {
        ...
        magma_tally3_sum_reduce<BLOCK_SIZE>( tx, swork );

    }
    
    if ( tx == 0 ) {
        magma_tally3DoubleComplex alpha = *dalpha;
        ...
        if ( swork[0] == 0 && imag(alpha) == 0 ) {
            ...
            *dtau   = MAGMA_tally3_Z_MAKE( (beta - real(alpha)) / beta, -imag(alpha) / beta );
            *dalpha = MAGMA_tally3_Z_MAKE( beta, 0 );
            *scale = 1 / (alpha - beta);
        }
    }
...
static __device__
void zlarfx_device( int m, int n,  magma_tally3DoubleComplex *v, magma_tally3DoubleComplex *tau,
                         magma_tally3DoubleComplex *dc, magma_tally3_int_t ldc, magma_tally3DoubleComplex* sum)
{


    if(n <=0) return ;
    if (MAGMA_tally3_Z_EQUAL(*tau, MAGMA_tally3_Z_ZERO) )  return; // check singularity
    ...
    for(int k=0;k<n;k++)
    {
        ...
        if(tx<BLOCK_SIZE)
        {
            ...
            for( int j = tx+1; j < m; j += BLOCK_SIZE ){
                lsum += MAGMA_tally3_Z_MUL( MAGMA_tally3_Z_CNJG( v[j] ), dc[j+ldc*k] );
            }
...
__global__
void zgeqr2_sm_kernel_batched( int m, int n, magma_tally3DoubleComplex** dA_array, magma_tally3_int_t lda,
                               magma_tally3DoubleComplex **dtau_array)
{

    magma_tally3DoubleComplex* dA = dA_array[blockIdx.z];
    magma_tally3DoubleComplex* dtau = dtau_array[blockIdx.z];
...
static __device__
void zgeqr2_device( magma_tally3_int_t m, magma_tally3_int_t n, magma_tally3DoubleComplex* dA, magma_tally3_int_t lda,
                               magma_tally3DoubleComplex *dtau, 
                               magma_tally3DoubleComplex *sdata,
...
__global__
void zgeqr2_kernel_batched( int m, int n, magma_tally3DoubleComplex** dA_array, magma_tally3_int_t lda,
                               magma_tally3DoubleComplex **dtau_array)
{
...
/**
    ...
    @param
    dwork   (workspace) COMPLEX_16 array, dimension (N) * ( sizeof(double) + sizeof(magma_tally3DoubleComplex)) 
...
magma_tally3_zgeqr2_batched(magma_tally3_int_t m, magma_tally3_int_t n, magma_tally3DoubleComplex **dA_array,
                  magma_tally3_int_t lda, magma_tally3DoubleComplex **dtau_array,
                  magma_tally3_int_t *info_array, magma_tally3_int_t batchCount, magma_tally3_queue_t queue)
{
    ...
    if(sizeof(magma_tally3DoubleComplex)*(m*k) <= 128 /*sizeof(magma_tally3DoubleComplex) * 128 * k*/) // there are some static shared memory besides of dynamic ones 
    {   
        ...
        zgeqr2_sm_kernel_batched<<< blocks, threads, sizeof(magma_tally3DoubleComplex)*(m*k), queue >>>
    ...
    else
    {
        ...
        if(sizeof(magma_tally3DoubleComplex)*(m) < 42000)
            zgeqr2_kernel_batched<<< blocks, threads, sizeof(magma_tally3DoubleComplex)*(m), queue >>>
                                      (m, k, dA_array, lda, dtau_array);
        else
