...
// Version 2 is MAGMA_minproduct  slaln2, which is exactly the same, but thread safe.
#define VERSION 2
...
/**
    ...
    If trans = Magma_minproductNoTrans, lambda is an eigenvalue for the lower 1x1 or 2x2 block,
    and it solves
        ( [ That u      ] - lambda*I ) * x = 0,
        ( [ 0    lambda ]            )
    ...
    If trans = Magma_minproductTrans, lambda is an eigenvalue for the upper 1x1 or 2x2 block,
    and it solves
    ...
    trans   magma_minproduct_trans_t
      ...
      -     = Magma_minproductNoTrans:    Solve (T - lambda*I)    * x = 0  (No transpose)
      -     = Magma_minproductTrans:      Solve (T - lambda*I)**T * x = 0  (Transpose)
    ...
    cnorm   (input) REAL array, dimension (N)
            ...
            of T.  If TRANS = Magma_minproductNoTrans, CNORM(j) must be greater than or equal
    ...
    @ingroup magma_minproduct_sgeev_aux
    ********************************************************************/
extern "C"
magma_minproduct_int_t magma_minproduct_slaqtrsd(
...
#define W(i,j)  (W + (i) + (j)*2)
    ...
    // constants
    const magma_minproduct_int_t c_false = false;
    const magma_minproduct_int_t c_true  = true;
    const magma_minproduct_int_t ione = 1;
    const magma_minproduct_int_t itwo = 2;
    const float   c_zero = 0.;
    const float   c_one  = 1.;
    
    // .. Local Scalars ..
    magma_minproduct_int_t notran;
    magma_minproduct_int_t ierr, j, j1, j2, jnxt, k, len;
    float beta, bignum, ovfl, rec, smin,
    ...
    *info = 0;
    if ( ! notran && trans != Magma_minproductTrans ) {
        *info = -1;
    }
    else if ( n < 0 ) {
        *info = -2;
    ...
    if ( notran ) {
        ...
        if ( n >= 2 && *T(n-1,n-2) != c_zero ) {
            wi = sqrt( fabsf(*T(n-1,n-2)) ) * sqrt( fabsf(*T(n-2,n-1)) );
        }
        ...
        if ( wi == c_zero ) {
            ...
            *x(n-1,0) = c_one;
            ...
            //magma_minproduct_s_snprint( buf, sizeof(buf), n, 1, x, ldx );
            ...
            for( j=n-2; j >= 0; --j ) {
                ...
                if ( j1 == j2 ) {
                    ...
                    if ( scale != 1. ) {
                        len = n;
                        blasf77_sscal( &len, &scale, x(0,0), &ione );
                    }
                    *x(j,0) = *W(0,0);
                    ...
                    len = j;
                    tmp = -(*W(0,0));
                    blasf77_saxpy( &len, &tmp, T(0,j), &ione, x(0,0), &ione );
                }
                else {
                    ...
                    *x(j,  0) = *W(1,0);
                    ...
                    // Update right-hand side
                    len = (j+1)-2;
                    tmp = -(*W(0,0));  blasf77_saxpy( &len, &tmp, T(0,j-1), &ione, x(0,0), &ione );
                    tmp = -(*W(1,0));  blasf77_saxpy( &len, &tmp, T(0,j  ), &ione, x(0,0), &ione );
                }
            }
            ...
            // [ ( T(n-1,n-2) T(n-1,n-1) )               ]
            if ( fabsf(*T(n-2,n-1)) >= fabsf(*T(n-1,n-2)) ) {
                *x(n-2,0) = c_one;
            ...
            *x(n-2,1) = c_zero;
            ...
            //magma_minproduct_s_snprint( buf, sizeof(buf), n, 2, x, ldx );
            ...
            for( j=n-3; j >= 0; --j ) {
                ...
                if ( j1 == j2 ) {
                    ...
                    if ( scale != 1. ) {
                        len = n;
                        blasf77_sscal( &len, &scale, x(0,0), &ione );
                        blasf77_sscal( &len, &scale, x(0,1), &ione );
                    }
                    *x(j,0) = *W(0,0);
                    *x(j,1) = *W(0,1);
                    ...
                    len = j;
                    tmp = -(*W(0,0));  blasf77_saxpy( &len, &tmp, T(0,j), &ione, x(0,0), &ione );
                    tmp = -(*W(0,1));  blasf77_saxpy( &len, &tmp, T(0,j), &ione, x(0,1), &ione );
                }
                else {
                    // 2-by-2 diagonal block
                    #if VERSION == 1
                    ...
                    *x(j,  1) = *W(1,1);
                    ...
                    tmp = -(*W(0,0));  blasf77_saxpy( &len, &tmp, T(0,j-1), &ione, x(0,0), &ione );
                    tmp = -(*W(1,0));  blasf77_saxpy( &len, &tmp, T(0,j  ), &ione, x(0,0), &ione );
                    tmp = -(*W(0,1));  blasf77_saxpy( &len, &tmp, T(0,j-1), &ione, x(0,1), &ione );
                    tmp = -(*W(1,1));  blasf77_saxpy( &len, &tmp, T(0,j  ), &ione, x(0,1), &ione );
                }
            }
        ...
        wi = 0.;
        if ( n >= 2 && *T(1,0) != c_zero ) {
            wi = sqrt( fabsf(*T(0,1)) ) * sqrt( fabsf(*T(1,0)) );
        }
        smin = max( ulp*(fabsf(wr) + fabsf(wi)), smlnum );
            ...
            for( j=1; j < n; ++j ) {
                ...
                if ( j1 == j2 ) {
                    ...
                    if ( cnorm[j] > vcrit ) {
                        rec = c_one / vmax;
                        len = n;
                        blasf77_sscal( &len, &rec, x(0,0), &ione );
                        vmax = c_one;
                        vcrit = bignum;
                    }

                    len = j-1;
                    *x(j,0) -= magma_minproduct_cblas_sdot( len, T(1,j), ione, x(1,0), ione );
                    ...
                    *x(j,0) = *W(0,0);
                    ...
                    len = j-1;
                    *x(j,  0) -= magma_minproduct_cblas_sdot( len, T(1,j),   ione, x(1,0), ione );
                    *x(j+1,0) -= magma_minproduct_cblas_sdot( len, T(1,j+1), ione, x(1,0), ione );
                    ...
                    *x(j+1,0) = *W(1,0);
                    ...
                    vmax = max( max( fabsf(*x(j,0)), fabsf(*x(j+1,0)) ), vmax );
                    vcrit = bignum / vmax;
                }
            }
        }  // end real eigenvalue
            ...
            for( j=2; j < n; ++j ) {
                ...
                if ( j1 == j2 ) {
                    ...
                    if ( cnorm[j] > vcrit ) {
                        ...
                        len = n;
                        blasf77_sscal( &len, &rec, x(0,0), &ione );
                        blasf77_sscal( &len, &rec, x(0,1), &ione );
                        vmax = c_one;
                        vcrit = bignum;
                    }

                    len = j-2;
                    *x(j,0) -= magma_minproduct_cblas_sdot( len, T(2,j), ione, x(2, 0), ione );
                    *x(j,1) -= magma_minproduct_cblas_sdot( len, T(2,j), ione, x(2, 1), ione );
                    ...
                    *x(j,0) = *W(0,0);
                    *x(j,1) = *W(0,1);
                    vmax = max( max( fabsf(*x(j,0)), fabsf(*x(j,1)) ), vmax );
                    vcrit = bignum / vmax;
                }
                else {
                    // 2-by-2 diagonal block
                    ...
                    *x(j,  0) -= magma_minproduct_cblas_sdot( len, T(2,j),   ione, x(2,0), ione );
                    *x(j,  1) -= magma_minproduct_cblas_sdot( len, T(2,j),   ione, x(2,1), ione );
                    *x(j+1,0) -= magma_minproduct_cblas_sdot( len, T(2,j+1), ione, x(2,0), ione );
                    *x(j+1,1) -= magma_minproduct_cblas_sdot( len, T(2,j+1), ione, x(2,1), ione );
                    ...
                    *x(j+1,0) = *W(1,0);
                    *x(j+1,1) = *W(1,1);
                    vmax = max( max( max( max( fabsf(*W(0,0)), fabsf(*W(0,1)) ),
                                               fabsf(*W(1,0)) ), fabsf(*W(1,1)) ), vmax );
                    vcrit = bignum / vmax;
                }
            }
        }  // end complex eigenvalue
