...
int main( int argc, char** argv)
{
    ...
    magma_minproductFloatComplex_ptr d_A, dT;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( opts.version == 2 ) {
                ...
                magma_minproduct_cgeqrf2_gpu( M, N, d_A, ldda, tau, &info );
            }
            else {
                nb = magma_minproduct_get_cgeqrf_nb( M );
                size = (2*min(M, N) + (N+31)/32*32 )*nb;
                TESTING_MALLOC_DEV( dT, magma_minproductFloatComplex, size );
                if ( opts.version == 1 ) {
                    // stores dT, V blocks have zeros, R blocks inverted & stored in dT
                    magma_minproduct_cgeqrf_gpu( M, N, d_A, ldda, tau, dT, &info );
                }
                #ifdef HAVE_CUBLAS
                else if ( opts.version == 3 ) {
                    // stores dT, V blocks have zeros, R blocks stored in dT
                    magma_minproduct_cgeqrf3_gpu( M, N, d_A, ldda, tau, dT, &info );
                }
                #endif
                else {
            ...
            if (info != 0)
                printf("magma_minproduct_cgeqrf returned error %d: %s.\n",
            ...
            if ( opts.check && opts.version == 2 ) {
                ...
                magma_minproductFloatComplex *x, *b, *hwork;
                magma_minproductFloatComplex_ptr d_B;
                const magma_minproductFloatComplex c_zero    = MAGMA_minproduct_C_ZERO;
                const magma_minproductFloatComplex c_one     = MAGMA_minproduct_C_ONE;
                const magma_minproductFloatComplex c_neg_one = MAGMA_minproduct_C_NEG_ONE;
                const magma_minproduct_int_t ione = 1;
                ...
                TESTING_MALLOC_CPU( b, magma_minproductFloatComplex, M );
                lapackf77_clarnv( &ione, ISEED, &N, x );
                blasf77_cgemv( "Notrans", &M, &N, &c_one, h_A, &lda, x, &ione, &c_zero, b, &ione );
                // copy to GPU
                TESTING_MALLOC_DEV( d_B, magma_minproductFloatComplex, M );
                magma_minproduct_csetvector( M, b, 1, d_B, 1 );

                if ( opts.version == 1 ) {
                    // allocate hwork
                    magma_minproduct_cgeqrs_gpu( M, N, 1,
                                      d_A, ldda, tau, dT,
                                      d_B, M, tmp, -1, &info );
                    lwork = (magma_minproduct_int_t)MAGMA_minproduct_C_REAL( tmp[0] );
                    TESTING_MALLOC_CPU( hwork, magma_minproductFloatComplex, lwork );
                    ...
                    if (info != 0)
                        printf("magma_minproduct_cgeqrs returned error %d: %s.\n",
                               (int) info, magma_minproduct_strerror( info ));
                    ...
                    // allocate hwork
                    magma_minproduct_cgeqrs3_gpu( M, N, 1,
                                       d_A, ldda, tau, dT,
                                       d_B, M, tmp, -1, &info );
                    lwork = (magma_minproduct_int_t)MAGMA_minproduct_C_REAL( tmp[0] );
                    TESTING_MALLOC_CPU( hwork, magma_minproductFloatComplex, lwork );
                    ...
                    magma_minproduct_cgeqrs3_gpu( M, N, 1,
                                       ...
                                       d_B, M, hwork, lwork, &info );
                    if (info != 0)
                        printf("magma_minproduct_cgeqrs3 returned error %d: %s.\n",
                               (int) info, magma_minproduct_strerror( info ));
                    TESTING_FREE_CPU( hwork );
                }
                #endif
                ...
                magma_minproduct_cgetvector( N, d_B, 1, x, 1 );
