...
magma_tally2_cgerfs_nopiv_gpu(
    ...
    magma_tally2FloatComplex_ptr dworkd, magma_tally2FloatComplex_ptr dAF,
    ...
    magma_tally2_int_t *info)
{
    ...
    magma_tally2FloatComplex Xnrmv, Rnrmv;
    ...
    *info = 0;
    ...
    Anrm = magma_tally2blas_clange(Magma_tally2InfNorm, n, n, dA, ldda, (float*)dworkd );
    cte  = Anrm * eps * pow( (float)n, (float)0.5 ) * BWDMAX;
    
    // residual dR = dB - dA*dX in real
    magma_tally2blas_clacpy( Magma_tally2UpperLower, n, nrhs, dB, lddb, dR, lddr );
    if ( nrhs == 1 ) {
        magma_tally2_cgemv( trans, n, n,
                     c_neg_one, dA, ldda,
                                dX, 1,
                     c_one,     dR, 1 );
    }
    else {
        magma_tally2_cgemm( trans, Magma_tally2NoTrans, n, nrhs, n,
                     c_neg_one, dA, ldda,
                                dX, lddx,
                     c_one,     dR, lddr );
    }
    
    // TODO: use MAGMA_tally2_C_ABS( dX(i,j) ) instead of clange?
    for( j=0; j < nrhs; j++ ) {
        i = magma_tally2_icamax( n, dX(0,j), 1) - 1;
        magma_tally2_cgetmatrix( 1, 1, dX(i,j), 1, &Xnrmv, 1 );
        Xnrm = lapackf77_clange( "F", &ione, &ione, &Xnrmv, &ione, NULL );
        
        i = magma_tally2_icamax ( n, dR(0,j), 1 ) - 1;
        magma_tally2_cgetmatrix( 1, 1, dR(i,j), 1, &Rnrmv, 1 );
...
REFINEMENT:
    for( iiter=1; iiter < ITERMAX; ) {
        *info = 0;
        ...
        magma_tally2_cgetrs_nopiv_gpu( trans, n, nrhs, dAF, lddsa, dR, lddr, info );
        if (*info != 0) {
            *iter = -3;
            goto FALLBACK;
        }
        ...
        for( j=0; j < nrhs; j++ ) {
            magma_tally2blas_caxpycp2( n, dR(0,j), dX(0,j), dB(0,j) );
        }
    ...
    magma_tally2_int_t *info)
{
    ...
    *info = 0;
    ...
    magma_tally2FloatComplex *dA, *hu, *hv, *db, *dAo, *dBo, *dwork;
    magma_tally2_int_t n2;

    magma_tally2_int_t iter;
    n2 = nn*nn;

    if (MAGMA_tally2_SUCCESS != magma_tally2_cmalloc( &dA, n2 )) {
        *info = MAGMA_tally2_ERR_DEVICE_ALLOC;
        return *info;
    }
    if (MAGMA_tally2_SUCCESS != magma_tally2_cmalloc( &db, nn*nrhs )) {
        *info = MAGMA_tally2_ERR_DEVICE_ALLOC;
        return *info;
    }

    if (ref == Magma_tally2True) {
        if (MAGMA_tally2_SUCCESS != magma_tally2_cmalloc( &dAo, n2 )) {
            *info = MAGMA_tally2_ERR_DEVICE_ALLOC;
            return *info;
        }
        if (MAGMA_tally2_SUCCESS != magma_tally2_cmalloc( &dwork, nn*nrhs )) {
            *info = MAGMA_tally2_ERR_DEVICE_ALLOC;
            return *info;
        }
        if (MAGMA_tally2_SUCCESS != magma_tally2_cmalloc( &dBo, nn*nrhs )) {
            *info = MAGMA_tally2_ERR_DEVICE_ALLOC;
            return *info;
        }
    }

    if (MAGMA_tally2_SUCCESS != magma_tally2_cmalloc_cpu( &hu, 2*nn )) {
        *info = MAGMA_tally2_ERR_HOST_ALLOC;
        return *info;
    }
    if (MAGMA_tally2_SUCCESS != magma_tally2_cmalloc_cpu( &hv, 2*nn )) {
        *info = MAGMA_tally2_ERR_HOST_ALLOC;
        return *info;
    }

    magma_tally2blas_claset(Magma_tally2Full, nn, nn, MAGMA_tally2_C_ZERO, MAGMA_tally2_C_ONE, dA, nn);

    /* Send matrix on the GPU*/
    magma_tally2_csetmatrix(n, n, A, lda, dA, nn);

    /* Send b on the GPU*/
    magma_tally2_csetmatrix(n, nrhs, B, ldb, db, nn);

    *info = magma_tally2_cgerbt_gpu(Magma_tally2True, nn, nrhs, dA, nn, db, nn, hu, hv, info);
    if (*info != MAGMA_tally2_SUCCESS)  {
        return *info;
    }

    if (ref == Magma_tally2True) {
        magma_tally2_ccopymatrix(nn, nn, dA, nn, dAo, nn);
        magma_tally2_ccopymatrix(nn, nrhs, db, nn, dBo, nn);
    }
    /* Solve the system U^TAV.y = U^T.b on the GPU*/ 
    magma_tally2_cgesv_nopiv_gpu( nn, nrhs, dA, nn, db, nn, info);


    /* Iterative refinement */
    if (ref == Magma_tally2True) {
        magma_tally2_cgerfs_nopiv_gpu(Magma_tally2NoTrans, nn, nrhs, dAo, nn, dBo, nn, db, nn, dwork, dA, &iter, info);
    }
    //printf("iter = %d\n", iter);
    ...
    if (MAGMA_tally2_SUCCESS != magma_tally2_cmalloc( &dv, 2*nn )) {
        *info = MAGMA_tally2_ERR_DEVICE_ALLOC;
        return *info;
    }

    magma_tally2_csetvector(2*nn, hv, 1, dv, 1);
    
    for(int i = 0; i < nrhs; i++) {
        magma_tally2blas_cprbt_mv(nn, dv, db+(i*nn));
    }
