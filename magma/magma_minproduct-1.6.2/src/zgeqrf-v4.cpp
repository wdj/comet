...
{
    magma_minproductDoubleComplex *da[Magma_minproductMaxGPUs];
    ...
    *info = 0;
    ...
    int lquery = (lwork == -1);
    if (ngpu < 0 || ngpu > Magma_minproductMaxGPUs) {
        *info = -1;
    } else if (m < 0) {
        *info = -2;
    } else if (n < 0) {
    ...
    for (i=0; i < ngpu; i++) {
        ...
        if (MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc( &da[i], ldda*n_local[i] )) {
    ...
    if (m > nb && n > nb) {
        ...
        magma_minproduct_zsetmatrix_1D_col_bcyclic(m, n, A, lda, da, ldda, ngpu, nb);

        /* Factor using the GPU interface */
        magma_minproduct_zgeqrf2_mgpu( ngpu, m, n, da, ldda, tau, info);
