...
/* ////////////////////////////////////////////////////////////////////////////
   -- Testing magma_tally4_zhemm_mgpu
*/
int main( int argc, char** argv)
{
    ...
    magma_tally4DoubleComplex calpha    = MAGMA_tally4_Z_MAKE( 3.456, 5.678 );
    magma_tally4DoubleComplex cbeta     = MAGMA_tally4_Z_MAKE( 1.234, 2.456 );
    ...
    magma_tally4DoubleComplex *hA, *hX, *hB, *hR;
    magma_tally4DoubleComplex_ptr dA[Magma_tally4MaxGPUs], dX[Magma_tally4MaxGPUs], dB[Magma_tally4MaxGPUs], dwork[Magma_tally4MaxGPUs], hwork[Magma_tally4MaxGPUs+1];
    magma_tally4DoubleComplex_ptr dA2;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int offset = 0; offset < N; offset += min(N,nb) ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            ldda  = ((M + 31)/32)*32;
            size  = lda*M;
            gflops = FLOPS_ZHEMM( Magma_tally4Left, (double)msize, (double)N ) / 1e9;
            
            magma_tally4_int_t dworksiz = ldda*N*3;
            magma_tally4_int_t hworksiz = lda*N;
            
            TESTING_MALLOC_CPU( hA, magma_tally4DoubleComplex, lda*M );
            TESTING_MALLOC_CPU( hX, magma_tally4DoubleComplex, lda*N );
            TESTING_MALLOC_CPU( hB, magma_tally4DoubleComplex, lda*N );
            
            TESTING_MALLOC_PIN( hR, magma_tally4DoubleComplex, lda*N );

            for( int d = 0; d < opts.ngpu; ++d ) {
                magma_tally4_int_t mlocal = ((M / nb) / opts.ngpu + 1) * nb;
                magma_tally4_setdevice( d );
                TESTING_MALLOC_DEV( dA[d],    magma_tally4DoubleComplex, ldda*mlocal );
                TESTING_MALLOC_DEV( dX[d],    magma_tally4DoubleComplex, ldda*N      );
                TESTING_MALLOC_DEV( dB[d],    magma_tally4DoubleComplex, ldda*N      );
                TESTING_MALLOC_DEV( dwork[d], magma_tally4DoubleComplex, dworksiz    );
                
                TESTING_MALLOC_PIN( hwork[d], magma_tally4DoubleComplex, hworksiz    );
            }
            TESTING_MALLOC_PIN( hwork[opts.ngpu], magma_tally4DoubleComplex, lda*N );
        
            if ( opts.check ) {
                magma_tally4_setdevice( 0 );
                TESTING_MALLOC_DEV( dA2, magma_tally4DoubleComplex, ldda*M );
            }

            lapackf77_zlarnv( &ione, iseed, &size, hA );
            magma_tally4_zmake_hermitian( M, hA, lda );
            ...
            /* ====================================================================
               ...
               =================================================================== */
            magma_tally4_zsetmatrix_1D_col_bcyclic( M, M, hA, lda, dA, ldda, opts.ngpu, nb );
            for( int d = 0; d < opts.ngpu; ++d ) {
                magma_tally4_setdevice( d );
                //magma_tally4blasSetKernelStream( streams[ d ][  0 ] );
                magma_tally4_zsetmatrix( M, N, hX, lda, dX[d], ldda );
                //if (d == 0) magma_tally4_zsetmatrix( M, N, hB, lda, dB[d], ldda ); // this is wrong coz when offset != 0 the gpu who do the beta*C may be not 0 so this should be related to stdev(starting device who own i=0 first col)
                magma_tally4_zsetmatrix( M, N, hB, lda, dB[d], ldda );
            }
        
            //memset(hR, 0, lda*N*sizeof(magma_tally4DoubleComplex));
            ...
            if ( opts.check && iter == 0 ) {
                ...
                magma_tally4_zsetmatrix( M, M, hA, lda, dA2, ldda );
                magma_tally4_zsetmatrix( M, N, hX, lda, dX[0], ldda );
                magma_tally4_zsetmatrix( M, N, hB, lda, dwork[0], ldda );
            ...
            if ( opts.check ) {
                ...
                //printf( "A =" ); magma_tally4_zprint( M, M, hA, lda );
                //printf( "X =" ); magma_tally4_zprint( M, N, hX, lda );
                //printf( "B =" ); magma_tally4_zprint( M, N, hB, lda );
                ...
                */
                ...
                for(magma_tally4_int_t dev=0; dev < opts.ngpu; ++dev) {
                    ...
                    magma_tally4_zgetmatrix( M, N, dB[dev], ldda, hR, lda );
                    ...
                    //printf( "R ="  ); magma_tally4_zprint( M, N, hR, lda );
                    if (firstprint == 0) {
