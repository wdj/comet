...
void zlag2c_kernel(
    ...
    magma_minproductFloatComplex *SA,       int ldsa,
    double rmax )
{
    ...
    if ( ind < m ) {
        ...
        if ( full ) {
            ...
            for( int j=0; j < BLK_Y; ++j ) {
                tmp = A[j*lda];
                if (   (MAGMA_minproduct_Z_REAL(tmp) < neg_rmax) || (MAGMA_minproduct_Z_REAL(tmp) > rmax)
                    #if defined(PRECISION_z) || defined(PRECISION_c)
                    || (MAGMA_minproduct_Z_IMAG(tmp) < neg_rmax) || (MAGMA_minproduct_Z_IMAG(tmp) > rmax)
                    #endif
                    )
                {
                    flag = 1;
                }
                SA[j*ldsa] = MAGMA_minproduct_C_MAKE( MAGMA_minproduct_Z_REAL(tmp), MAGMA_minproduct_Z_IMAG(tmp) );
            }
        }
        else {
...
/**
    @see magma_minproductblas_zlag2c_q
    @ingroup magma_minproduct_zaux2
    ********************************************************************/
extern "C" void
magma_minproductblas_zlag2c(
    ...
    magma_minproduct_int_t *info )
{
    magma_minproductblas_zlag2c_q( m, n, A, lda, SA, ldsa, info, magma_minproduct_stream );
}
