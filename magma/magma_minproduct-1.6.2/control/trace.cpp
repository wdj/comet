...
struct event_log
{
    ...
    int           ngpu;
    int           nqueue;
    magma_minproduct_queue_t queues   [ MAX_GPU_QUEUES ];
    int           gpu_id   [ MAX_GPU_QUEUES ];
    magma_minproduct_event_t gpu_first[ MAX_GPU_QUEUES ];
#if TRACE_METHOD == 2
    double        gpu_start[ MAX_GPU_QUEUES ][ MAX_EVENTS ];
#else
    magma_minproduct_event_t gpu_start[ MAX_GPU_QUEUES ][ MAX_EVENTS ];
#endif
    magma_minproduct_event_t gpu_end  [ MAX_GPU_QUEUES ][ MAX_EVENTS ];
...
// ----------------------------------------
void trace_init( int ncore, int ngpu, int nqueue, magma_minproduct_queue_t* queues )
{
    if ( ncore > MAX_CORES ) {
    ...
    glog.cpu_first = magma_minproduct_wtime();
}
...
void trace_cpu_start( int core, const char* tag, const char* lbl )
{
    ...
    glog.cpu_start[core][id] = magma_minproduct_wtime();
    magma_minproduct_strlcpy( glog.cpu_tag  [core][id], tag, MAX_LABEL_LEN );
    magma_minproduct_strlcpy( glog.cpu_label[core][id], lbl, MAX_LABEL_LEN );
}
...
{
    int id = glog.cpu_id[core];
    glog.cpu_end[core][id] = magma_minproduct_wtime();
    if ( id+1 < MAX_EVENTS ) {
...
void trace_gpu_start( int dev, int s, const char* tag, const char* lbl )
{
...
#if TRACE_METHOD == 2
    glog.gpu_start[t][id] = magma_minproduct_wtime();
#else
    #ifdef HAVE_CUBLAS
    ...
    #endif
#endif
    magma_minproduct_strlcpy( glog.gpu_tag  [t][id], tag, MAX_LABEL_LEN );
    magma_minproduct_strlcpy( glog.gpu_label[t][id], lbl, MAX_LABEL_LEN );
}
...
void trace_finalize( const char* filename, const char* cssfile )
{
    ...
    double time = magma_minproduct_wtime() - glog.cpu_first;
