/*
    -- MAGMA_tally2 (version 1.6.1) --
       ...
       @generated from zunmqr_m.cpp normal z -> c, Fri Jan 30 19:00:16 2015
...
/**
    ...
    as returned by CGEQRF. Q is of order M if SIDE = Magma_tally2Left and of order N
...
#define dwork(gpui, ind)       (dw[gpui] + maxnlocal*lddc + 2*lddac*lddar + 2*((nb+1)*nb) + (ind)*(lddwork*nb))
    ...
    const char* side_  = lapack_side_const_tally2( side );
    const char* trans_ = lapack_trans_const_tally2( trans );

    // TODO fix memory leak (alloc after argument checks)
    magma_tally2_int_t nb = 128;
    magma_tally2FloatComplex *T;
    magma_tally2_cmalloc_pinned(&T, nb*nb);
    //printf("calling cunmqr_m with nb=%d\n", (int) nb);
    ...
    magma_tally2_int_t ind_c;
    magma_tally2_device_t igpu;
    ...
    *info = 0;
    ...
    magma_tally2_int_t left   = (side == Magma_tally2Left);
    magma_tally2_int_t notran = (trans == Magma_tally2NoTrans);
    magma_tally2_int_t lquery = (lwork == -1);

    /* NQ is the order of Q and NW is the minimum dimension of WORK */
    magma_tally2_int_t nq, nw;
    if (left) {
        nq = m;
        nw = n;
    } else {
    ...
    magma_tally2_int_t lwkopt = max(1,nw) * nb;
    if (*info == 0) {
    ...
    magma_tally2_int_t lddc = (m+63)/64*64;
    magma_tally2_int_t lddac = nq;
    magma_tally2_int_t lddar = nb;
    magma_tally2_int_t lddwork = nw;

    magma_tally2_int_t nlocal[ Magma_tally2MaxGPUs ] = { 0 };

    magma_tally2_int_t nb_l=256;
    magma_tally2_int_t nbl = (n-1)/nb_l+1; // number of blocks
    magma_tally2_int_t maxnlocal = (nbl+ngpu-1)/ngpu*nb_l;

    ngpu = min(ngpu, (n+nb_l-1)/nb_l); // Don't use GPU that will not have data.

    magma_tally2_int_t ldw = maxnlocal*lddc // dC
                    + 2*lddac*lddar // 2*dA
                    + 2*(nb + 1 + lddwork)*nb; // 2*(dT and dwork)

    for (igpu = 0; igpu < ngpu; ++igpu) {
        magma_tally2_setdevice(igpu);
        if (MAGMA_tally2_SUCCESS != magma_tally2_cmalloc( &dw[igpu], ldw )) {
    ...
    if (left) {
        //copy C to mgpus
        for (magma_tally2_int_t i = 0; i < nbl; ++i) {
            magma_tally2_int_t igpu = i%ngpu;
            magma_tally2_setdevice(igpu);
            magma_tally2_int_t kb = min(nb_l, n-i*nb_l);
        ...
        for (magma_tally2_int_t i = i1; (i3 < 0 ? i >= i2 : i < i2); i += i3) {
            // start the copy of A panel
            magma_tally2_int_t kb = min(nb, k - i);
            for (igpu = 0; igpu < ngpu; ++igpu) {
                magma_tally2_setdevice(igpu);
                magma_tally2_event_sync(event[igpu][ind_c]); // check if the new data can be copied
                magma_tally2_csetmatrix_async(nq-i, kb,
                                       A(i, i),                 lda,
                                       dA_c(igpu, ind_c, i, 0), lddac, stream[igpu][0] );
                // set upper triangular part of dA to identity
                magma_tally2blas_claset_band_q( Magma_tally2Upper, kb, kb, kb, c_zero, c_one, dA_c(igpu, ind_c, i, 0), lddac, stream[igpu][0] );
            }

            /* Form the triangular factor of the block reflector
             H = H(i) H(i+1) . . . H(i+ib-1) */
            magma_tally2_int_t nqi = nq - i;
            ...
            for (igpu = 0; igpu < ngpu; ++igpu) {
                magma_tally2_setdevice(igpu);
                magma_tally2_queue_sync( stream[igpu][0] ); // check if the data was copied
                magma_tally2blasSetKernelStream(stream[igpu][1]);
                magma_tally2_clarfb_gpu( side, trans, Magma_tally2Forward, Magma_tally2Columnwise,
                                 m-i, nlocal[igpu], kb,
                                 dA_c(igpu, ind_c, i, 0), lddac, dT(igpu, ind_c), kb,
                                 dC(igpu, i, 0), lddc,
                                 dwork(igpu, ind_c), lddwork);
                magma_tally2_event_record(event[igpu][ind_c], stream[igpu][1] );
            }
        ...
        for (magma_tally2_int_t i = 0; i < nbl; ++i) {
            ...
            magma_tally2_setdevice(igpu);
            magma_tally2_int_t kb = min(nb_l, n-i*nb_l);
            magma_tally2_cgetmatrix( m, kb,
                              dC(igpu, 0, i/ngpu*nb_l), lddc,
                              C(0, i*nb_l), ldc );
//            magma_tally2_cgetmatrix_async( m, kb,
//                                   dC(igpu, 0, i/ngpu*nb_l), lddc,
//                                   C(0, i*nb_l), ldc, stream[igpu][0] );
        ...
        *info = MAGMA_tally2_ERR_NOT_IMPLEMENTED;
        ...
        for (i = i1; (i3 < 0 ? i >= i2 : i < i2); i += i3) {
            ...
            magma_tally2_csetmatrix( i__4, ib, A(i, i), lda, dA(i, 0), ldda );
            magma_tally2blas_claset_band( Magma_tally2Upper, ib, ib, ib, c_zero, c_one, dA(i, 0), ldda );
            ...
            // Apply H or H'; First copy T to the GPU
            magma_tally2_csetmatrix( ib, ib, T, ib, dT, ib );
            magma_tally2_clarfb_gpu( side, trans, Magma_tally2Forward, Magma_tally2Columnwise,
            mi, ni, ib,
            dA(i, 0), ldda, dT, ib,
            dC(ic, jc), lddc,
