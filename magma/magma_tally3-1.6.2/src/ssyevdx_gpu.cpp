...
/**
    ...
    dA      REAL array on the GPU,
            ...
            On exit, if JOBZ = Magma_tally3Vec, then if INFO = 0, the first mout columns
    ...
    mout    INTEGER
            ...
            If RANGE = Magma_tally3RangeAll, MOUT = N, and if RANGE = Magma_tally3RangeI, MOUT = IU-IL+1.
    ...
    lwork   INTEGER
            ...
            NB can be obtained through magma_tally3_get_ssytrd_nb(N).
    \n
...
magma_tally3_ssyevdx_gpu(
    ...
    magma_tally3_int_t *mout, float *w,
    ...
    magma_tally3_int_t *info)
{
    ...
    *info = 0;
    ...
    if (n <= 128) {
        ...
        magma_tally3_sgetmatrix( n, n, dA, ldda, A, lda );
        // TODO: deal with range and mout. Use lapack ssyevx? (There is no lapack ssyevdx.)
        lapackf77_ssyevd( lapack_vec_const_tally3(jobz), lapack_uplo_const_tally3(uplo),
                          &n, A, &lda,
                          w, work, &lwork,
                          iwork, &liwork, info );
        *mout = n;
    ...
    // slansy      requires n
    magma_tally3_int_t ldwork = max( ldda*ceildiv(n,64) + 2*ldda*nb, lddc*n );
    ldwork = max( ldwork, n );
    if ( wantz ) {
        // sstedx requires 3n^2/2
        ldwork = max( ldwork, 3*n*(n/2 + 1) );
    }
    if (MAGMA_tally3_SUCCESS != magma_tally3_smalloc( &dwork, ldwork )) {
    ...
    anrm = magma_tally3blas_slansy( Magma_tally3MaxNorm, uplo, n, dA, ldda, dwork );
    iscale = 0;
    sigma  = 1;
    if (anrm > 0. && anrm < rmin) {
        iscale = 1;
    ...
    if (iscale == 1) {
        magma_tally3blas_slascl( uplo, 0, 0, 1., sigma, n, n, dA, ldda, info );
    }
...
#ifdef FAST_SYMV
    magma_tally3_ssytrd2_gpu( uplo, n, dA, ldda, w, &work[inde],
                       &work[indtau], wA, ldwa, &work[indwrk], llwork,
                       dwork, ldwork, &iinfo );
#else
    magma_tally3_ssytrd_gpu(  uplo, n, dA, ldda, w, &work[inde],
                       &work[indtau], wA, ldwa, &work[indwrk], llwork,
                       &iinfo );
#endif
    ...
    if (! wantz) {
        ...
        magma_tally3_smove_eig( range, n, w, &il, &iu, vl, vu, mout );
    }
    else {
        timer_start( time );
        ...
        magma_tally3_ssetmatrix( n, *mout, &work[indwrk + n* (il-1) ], n, dwork, lddc );

        magma_tally3_sormtr_gpu( Magma_tally3Left, uplo, Magma_tally3NoTrans, n, *mout, dA, ldda, &work[indtau],
                          dwork, lddc, wA, ldwa, &iinfo );

        magma_tally3_scopymatrix( n, *mout, dwork, lddc, dA, ldda );
