/*
    -- MAGMA_minproduct (version 1.6.1) --
       ...
       @generated from zlange.cu normal z -> s, Fri Jan 30 19:00:09 2015
       @author Mark Gates
*/
    ...
    magma_minproductFloat_ptr dwork )  //, magma_minproduct_int_t lwork )
{
    ...
    if ( norm == Magma_minproductInfNorm ) {
        dim3 grid( (m-1)/64 + 1 );
        slange_inf_kernel<<< grid, threads, 0, magma_minproduct_stream >>>( m, n, dA, ldda, dwork );
        ...
        dim3 grid( (m-1)/64 + 1 );
        slange_max_kernel<<< grid, threads, 0, magma_minproduct_stream >>>( m, n, dA, ldda, dwork );
        max_nan_kernel<<< 1, 512, 0, magma_minproduct_stream >>>( m, dwork );
        cudaMemcpy( &result, &dwork[0], sizeof(float), cudaMemcpyDeviceToHost );
    }
    else if ( norm == Magma_minproductOneNorm ) {
        dim3 grid( n );
        slange_one_kernel<<< grid, threads, 0, magma_minproduct_stream >>>( m, n, dA, ldda, dwork );
