...
__global__ void
magma_minproduct_zpipelined_correction( 
    int n,  
    int k,
    ...
    magma_minproductDoubleComplex * v )
{
    ...
    temp[ i ] = ( i < k ) ? skp[ i ] * skp[ i ] : MAGMA_minproduct_Z_MAKE( 0.0, 0.0);
    __syncthreads();
    ...
    if( i == 0 ){
        tmp = MAGMA_minproduct_Z_REAL( temp[ i ] );
        zz = MAGMA_minproduct_Z_REAL( skp[(k)] );
        skp[k] = MAGMA_minproduct_Z_MAKE( sqrt(zz-tmp),0.0 );
    }
}

__global__ void
magma_minproduct_zpipelined_copyscale( 
    int n,  
    int k,
    magma_minproductDoubleComplex * skp, 
    magma_minproductDoubleComplex * r,
    magma_minproductDoubleComplex * v )
{
    ...
    magma_minproductDoubleComplex rr=skp[k];
...
__global__ void
magma_minproduct_zpipelineddznrm2_kernel( 
    int m, 
    magma_minproductDoubleComplex * da, 
    int ldda, 
    magma_minproductDoubleComplex * dxnorm )
{
    const int i = threadIdx.x;
    magma_minproductDoubleComplex_ptr dx = da + blockIdx.x * ldda;
    ...
    if (i==0)
        dxnorm[blockIdx.x] = MAGMA_minproduct_Z_MAKE( sqrt(sum[0]), 0.0 );
}

//----------------------------------------------------------------------------//

__global__ void
magma_minproduct_zpipelinedscale( 
    int n, 
    magma_minproductDoubleComplex * r, 
    ...
    magma_minproduct_queue_t queue )
{
    ...
    unsigned int Ms =   Bs.x * sizeof( magma_minproductDoubleComplex ); 

    dim3 Gs2( magma_minproduct_ceildiv( n, BLOCK_SIZE ) );


    magma_minproduct_zpipelined_correction<<<Gs, Bs, Ms, queue >>>
                                            ( n, k, skp, r, v );
    magma_minproduct_zpipelined_copyscale<<<Gs2, Bs, 0, queue >>>
                                            ( n, k, skp, r, v );
...
magma_minproduct_dznrm2scale(
    ...
    magma_minproductDoubleComplex_ptr r, 
    int lddr, 
    magma_minproductDoubleComplex_ptr drnorm,
    magma_minproduct_queue_t queue )
{
    dim3  blocks( 1 );
    dim3 threads( 512 );
    magma_minproduct_zpipelineddznrm2_kernel<<< blocks, threads, 0, queue >>>
                                ( m, r, lddr, drnorm );

    dim3 Bs( BLOCK_SIZE );
    dim3 Gs2( magma_minproduct_ceildiv( m, BLOCK_SIZE ) );
    magma_minproduct_zpipelinedscale<<<Gs2, Bs, 0, queue >>>( m, r, drnorm );
