...
} magma_tally4gpu_cswapblk_params_t;

__global__ void magma_tally4gpu_cswapblkrm( magma_tally4gpu_cswapblk_params_t params )
{
    unsigned int y = threadIdx.x + blockDim.x*blockIdx.x;
    if( y < params.n )
    {
        magma_tally4FloatComplex *A = params.A + y - params.ldda;
        magma_tally4FloatComplex *B = params.B + y;
        ...
        for( int i = 0; i < params.npivots; i++ )
        {
            ...
            if ( params.ipiv[i] == -1 )
                continue;
            magma_tally4FloatComplex  tmp1 = *A;
            magma_tally4FloatComplex *tmp2 = B + params.ipiv[i]*params.lddb;
            *A    = *tmp2;
            *tmp2 =  tmp1;
        }
    }
...
__global__ void magma_tally4gpu_cswapblkcm( magma_tally4gpu_cswapblk_params_t params )
{
    unsigned int y = threadIdx.x + blockDim.x*blockIdx.x;
    ...
    if( y < params.n )
    {
        magma_tally4FloatComplex *A = params.A + offset1 - 1;
        magma_tally4FloatComplex *B = params.B + offset2;
        ...
        for( int i = 0; i < params.npivots; i++ )
        {
            ...
            if ( params.ipiv[i] == -1 )
                continue;
            magma_tally4FloatComplex  tmp1 = *A;
            magma_tally4FloatComplex *tmp2 = B + params.ipiv[i];
            *A    = *tmp2;
            *tmp2 =  tmp1;
        }
    }
    ...
    magma_tally4_queue_t queue )
{
    ...
    if ( order == Magma_tally4ColMajor ) {
        for( k=(i1-1); k<i2; k+=BLOCK_SIZE )
        {
            ...
            magma_tally4gpu_cswapblk_params_t params = { dA+k, dB, n, ldda, lddb, sb };
            ...
            magma_tally4gpu_cswapblkcm<<< blocks, blocksize, 0, queue >>>( params );
        }
    }
    else {
        for( k=(i1-1); k<i2; k+=BLOCK_SIZE )
        {
            magma_tally4_int_t sb = min(BLOCK_SIZE, i2-k);
            magma_tally4gpu_cswapblk_params_t params = { dA+k*ldda, dB, n, ldda, lddb, sb };
            for( magma_tally4_int_t j = 0; j < sb; j++ )
            {
                im = ipiv[(k+j)*inci] - 1;
                if ( (k+j) == im )
            ...
            magma_tally4gpu_cswapblkrm<<< blocks, blocksize, 0, queue >>>( params );
        }
    }
}
...
/**
    @see magma_tally4blas_cswapblk_q
    @ingroup magma_tally4_cblas2
    ********************************************************************/
extern "C" void 
