...
int main( int argc, char** argv )
{
    ...
    double *C, *R, *A, *W, *tau;
    magma_minproductDouble_ptr dC, dA, dT;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int itran = 0; itran < 2; ++itran ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {        
            ...
            magma_minproduct_dsetmatrix( lda, k, A,  lda, dA, lda );
            magma_minproduct_dgeqrf_gpu( lda, k, dA, lda, tau, dT, &info );
            magma_minproduct_dgetmatrix( lda, k, dA, lda, A,  lda );
            if (info != 0)
            ...
            lwork = -1;
            magma_minproduct_dormqr_gpu( side[iside], trans[itran],
                              m, n, k,
                              dA, lda, tau, dC, ldc, W, lwork, dT, nb, &info );
            if (info != 0)
                printf("magma_minproduct_dormqr_gpu (lwork query) returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
            lwork = (magma_minproduct_int_t) MAGMA_minproduct_D_REAL( W[0] );
            ...
            if (info != 0)
                printf("magma_minproduct_dormqr_gpu returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
