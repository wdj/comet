...
int main( int argc, char** argv)
{
    ...
    for( int i = 0; i < opts.ntest; ++i ) {
      ...
      for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lapackf77_dlacpy( Magma_tally4UpperLowerStr, &M, &columns, h_A, &lda, h_R, &lda );
            magma_tally4_dsetmatrix( M, columns, h_R, lda, d_A, ldda );
            ...
            info1 = magma_tally4_dgetrf_batched( M, N, dA_array, ldda, dipiv_array, dinfo_array, batchCount, queue);
            info2 = magma_tally4_dgetri_outofplace_batched( min(M,N), dA_array, ldda, dipiv_array, dinvA_array, ldda, dinfo_array, batchCount, queue);
            ...
            {
                if(cpu_info[i] != 0 ){
                    printf("magma_tally4_dgetrf_batched matrix %d returned error %d\n", i, (int)cpu_info[i] );
                }
            }
            if (info1 != 0) printf("magma_tally4_dgetrf_batched returned argument error %d: %s.\n", (int) info1, magma_tally4_strerror( info1 ));
            if (info2 != 0) printf("magma_tally4_dgetri_batched returned argument error %d: %s.\n", (int) info2, magma_tally4_strerror( info2 ));
            /* =====================================================================
            ...
            if ( opts.lapack ) {
                ...
                lwork = magma_tally4_int_t( MAGMA_tally4_D_REAL( tmp ));
                TESTING_MALLOC_CPU( work,  double, lwork  );
                lapackf77_dlacpy( Magma_tally4UpperLowerStr, &M, &columns, h_R, &lda, h_A, &lda );
            ...
            if ( opts.check ) {
                ...
                magma_tally4_dgetmatrix( min(M,N), N*batchCount, d_invA, ldda, h_R, lda );
                int stop=0;
                n2     = lda*N;
                for(int i=0; i < batchCount; i++)
                {
