...
/**
    Purpose
    -------
    If VECT = Magma_tally2Q, DORMBR overwrites the general real M-by-N matrix C with
                                 SIDE = Magma_tally2Left     SIDE = Magma_tally2Right
    TRANS = Magma_tally2NoTrans:        Q*C                  C*Q
    TRANS = Magma_tally2Trans:     Q**H*C               C*Q**H
    
    If VECT = Magma_tally2P, DORMBR overwrites the general real M-by-N matrix C with
                                 SIDE = Magma_tally2Left     SIDE = Magma_tally2Right
    ...
    *info = 0;
    ...
    if (*info == 0) {
        if (nw > 0) {
            ...
            nb = magma_tally2_get_dgebrd_nb( min( m, n ));
            lwkopt = max(1, nw*nb);
        }
        else {
            lwkopt = 1;
    ...
    if (applyq) {
        ...
        if (nq >= k) {
            ...
            #else
            magma_tally2_dormqr( side, trans,
                          m, n, k, A, lda, tau, C, ldc, work, lwork, &iinfo);
            #endif
        }
        else if (nq > 1) {
        ...
        if (nq > k) {
            ...
            #if VERSION == 1
            lapackf77_dormlq( lapack_side_const_tally2(side), lapack_trans_const_tally2(transt),
                              &m, &n, &k, A, &lda, tau, C, &ldc, work, &lwork, &iinfo);
            #else
            magma_tally2_dormlq( side, transt,
                          m, n, k, A, lda, tau, C, ldc, work, lwork, &iinfo);
            #endif
        }
        else if (nq > 1) {
