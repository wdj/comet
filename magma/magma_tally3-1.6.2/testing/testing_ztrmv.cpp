...
int main( int argc, char** argv)
{
    ...
    magma_tally3DoubleComplex *h_A, *h_x, *h_xcublas;
    magma_tally3DoubleComplex_ptr d_A, d_x;
    ...
    printf("uplo = %s, transA = %s, diag = %s \n",
           ...
           lapack_diag_const_tally3(opts.diag) );
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_x,       magma_tally3DoubleComplex, N      );
            TESTING_MALLOC_CPU( h_xcublas, magma_tally3DoubleComplex, N      );
            
            TESTING_MALLOC_DEV( d_A, magma_tally3DoubleComplex, ldda*Ak );
            TESTING_MALLOC_DEV( d_x, magma_tally3DoubleComplex, N       );
            ...
            magma_tally3_zsetvector( N, h_x, 1, d_x, 1 );
            
            cublas_time = magma_tally3_sync_wtime( NULL );
            cublasZtrmv( opts.handle, cublas_uplo_const_tally3(opts.uplo), cublas_trans_const_tally3(opts.transA),
                         cublas_diag_const_tally3(opts.diag),
                         N, 
                         d_A, ldda,
                         d_x, 1 );
            ...
            magma_tally3_zgetvector( N, d_x, 1, h_xcublas, 1 );
            ...
            if ( opts.lapack ) {
                ...
                blasf77_ztrmv( lapack_uplo_const_tally3(opts.uplo), lapack_trans_const_tally3(opts.transA), lapack_diag_const_tally3(opts.diag), 
                               &N,
                               h_A, &lda,
                               h_x, &ione );
