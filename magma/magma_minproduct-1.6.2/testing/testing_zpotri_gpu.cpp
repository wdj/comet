...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            //magma_minproduct_zsetmatrix( N, N, h_R, lda, d_A, ldda );
            
            gpu_time = magma_minproduct_wtime();
            magma_minproduct_zpotri_gpu( opts.uplo, N, d_A, ldda, &info );
            gpu_time = magma_minproduct_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_minproduct_zpotri_gpu returned error %d: %s.\n",
