/*
    -- MAGMA_tally4 (version 1.6.1) --
       ...
       @generated from zsymv.cu normal z -> c, Fri Jan 30 19:00:10 2015
    ...
    magma_tally4FloatComplex * __restrict__ work)
{
    ...
    __shared__ magma_tally4FloatComplex sA [quarter_NB_X][NB_X + 3]; /* Why +3? seems it only needs +2. Does +3 reduce bank conflicts? */
...
/**
    Purpose
    -------
    magma_tally4blas_csymv_work performs the matrix-vector operation:
    ...
    uplo    magma_tally4_uplo_t.
            ...
            follows:
      -     = Magma_tally4Upper:  Only the upper triangular part of A is to be referenced.
    ...
    MAGMA_tally4 implements csymv through two steps:
    ...
    magma_tally4blas_csymv is a wrapper routine allocating the workspace inside the
    routine and provides the same interface as cublas.
    
    If users need to call csymv frequently, we suggest using
    magma_tally4blas_csymv_work instead of magma_tally4blas_csymv. As the overhead to
    allocate and free in device memory in magma_tally4blas_csymv would hurt performance.
    ...
    magma_tally4_queue_t queue )
{
    ...
    if ( arch < 200 ) {
        //magma_tally4_csymv( uplo, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    ...
    magma_tally4_int_t blocks = ceildiv( n, NB_X );
    magma_tally4_int_t lwmin  = ldda*blocks;
     ...
     */
    if ( (n == 0) || ( MAGMA_tally4_C_EQUAL(alpha, MAGMA_tally4_C_ZERO) && MAGMA_tally4_C_EQUAL(beta, MAGMA_tally4_C_ONE) ) )
        return info;
...
// end magma_tally4blas_csymv_work


/**
    Purpose
    -------
    magma_tally4blas_csymv performs the matrix-vector operation:
    ...
    magma_tally4FloatComplex_ptr dy, magma_tally4_int_t incy)
{
    ...
    magma_tally4_int_t blocks = ceildiv( n, NB_X );
    magma_tally4_int_t lwork  = ldda*blocks;

    magma_tally4_cmalloc( &dwork, lwork );
    if ( dwork == NULL ) {
    ...
    magma_tally4blas_csymv_work( uplo, n, alpha, dA, ldda, dx, incx, beta, dy, incy,
                          dwork, lwork, magma_tally4_stream );
    
    magma_tally4_free( dwork );
    
    return info;
}
// end magma_tally4blas_csymv
