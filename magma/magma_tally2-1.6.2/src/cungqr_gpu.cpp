...
#define dT(j)   (dT + (j)*nb)
    ...
    magma_tally2FloatComplex_ptr dV, dW;
    ...
    *info = 0;
    ...
    lwork = n*nb;
    lpanel = (m - kk)*(n - kk);
    magma_tally2_cmalloc_cpu( &work, lwork + lpanel );
    if ( work == NULL ) {
    ...
    if (MAGMA_tally2_SUCCESS != magma_tally2_cmalloc( &dV, ldda*nb )) {
    ...
    if (kk < n) {
        ...
        n_kk = n - kk;
        k_kk = k - kk;
        magma_tally2_cgetmatrix( m_kk, k_kk,
    ...
    if (kk > 0) {
        ...
        for (i = ki; i >= 0; i -= nb) {
            ...
            magma_tally2_ccopymatrix_async( mi, ib,
                                     dA(i,i), ldda,
                                     dV,      ldda, stream );
