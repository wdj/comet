...
/**
    ...
    The difference to magma_tally3_sbicgstab is that we use specifically designed
    ...
    magma_tally3_queue_t queue )
{
    ...
    // workspace
    magma_tally3_s_matrix q={Magma_tally3_CSR}, r={Magma_tally3_CSR}, rr={Magma_tally3_CSR}, p={Magma_tally3_CSR}, v={Magma_tally3_CSR}, s={Magma_tally3_CSR}, t={Magma_tally3_CSR};
    float *d1=NULL, *d2=NULL, *skp=NULL;
    d1 = NULL;
    d2 = NULL;
    skp = NULL;
    CHECK( magma_tally3_smalloc( &d1, dofs*(2) ));
    CHECK( magma_tally3_smalloc( &d2, dofs*(2) ));
    // array for the parameters
    CHECK( magma_tally3_smalloc( &skp, 8 ));
    // skp = [alpha|beta|omega|rho_old|rho|nom|tmp1|tmp2]
    CHECK( magma_tally3_svinit( &q, Magma_tally3_DEV, dofs*6, 1, c_zero, queue ));
    ...
    magma_tally3_scopy( dofs, r.dval, 1, q(0), 1 );                            // rr = r
    magma_tally3_scopy( dofs, r.dval, 1, q(1), 1 );                            // q = r
    betanom = nom0;
    nom = nom0*nom0;
    ...
    CHECK( magma_tally3_smalloc_cpu( &skp_h, 8 ));
    ...
    skp_h[3]=rho_old;
    skp_h[4]=rho_new;
    skp_h[5]=MAGMA_tally3_S_MAKE(nom, 0.0);
    magma_tally3_ssetvector( 8, skp_h, 1, skp, 1 );
    CHECK( magma_tally3_s_spmv( c_one, A, r, c_zero, v, queue ));             // z = A r
    den = MAGMA_tally3_S_REAL( magma_tally3_sdot(dofs, v.dval, 1, r.dval, 1) );// den = z dot r
    ...
    do
    {
        ...
        CHECK( magma_tally3_sbicgmerge1( dofs, skp, v.dval, r.dval, p.dval, queue ));

        CHECK( magma_tally3_s_spmv( c_one, A, p, c_zero, v, queue ));         // v = Ap

        CHECK( magma_tally3_smdotc( dofs, 1, q.dval, v.dval, d1, d2, skp, queue ));
        CHECK( magma_tally3_sbicgmerge4(  1, skp, queue ));
        CHECK( magma_tally3_sbicgmerge2( dofs, skp, r.dval, v.dval, s.dval, queue )); // s=r-alpha*v

        CHECK( magma_tally3_s_spmv( c_one, A, s, c_zero, t, queue ));         // t=As

        CHECK( magma_tally3_smdotc( dofs, 2, q.dval+4*dofs, t.dval, d1, d2, skp+6, queue ));
        CHECK( magma_tally3_sbicgmerge4(  2, skp, queue ));

        CHECK( magma_tally3_sbicgmerge_xrbeta( dofs, d1, d2, q.dval, r.dval, p.dval,
                                                    s.dval, t.dval, x->dval, skp, queue ));

        // check stopping criterion (asynchronous copy)
        magma_tally3_sgetvector_async( 1 , skp+5, 1,
                                                        skp_h+5, 1, stream[1] );
        betanom = sqrt(MAGMA_tally3_S_REAL(skp_h[5]));
...
cleanup:
    magma_tally3_smfree(&q, queue );  // frees all vectors
