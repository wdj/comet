...
int main( int argc, char** argv)
{
    ...
    magma_tally3FloatComplex *h_A, *h_Q, *h_PT, *h_work;
    magma_tally3FloatComplex *taup, *tauq;
    float      *diag, *offdiag;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( tauq,    magma_tally3FloatComplex, minmn );
            TESTING_MALLOC_CPU( taup,    magma_tally3FloatComplex, minmn );
            TESTING_MALLOC_CPU( diag,    float, minmn   );
            TESTING_MALLOC_CPU( offdiag, float, minmn-1 );
            
            TESTING_MALLOC_PIN( h_Q,     magma_tally3FloatComplex, lda*N  );
            TESTING_MALLOC_PIN( h_work,  magma_tally3FloatComplex, lhwork );
            
            /* Initialize the matrices */
            lapackf77_clarnv( &ione, ISEED, &n2, h_A );
            lapackf77_clacpy( Magma_tally3UpperLowerStr, &M, &N, h_A, &lda, h_Q, &lda );
            
            /* ====================================================================
               Performs operation using MAGMA_tally3
               =================================================================== */
            gpu_time = magma_tally3_wtime();
            magma_tally3_cgebrd( M, N, h_Q, lda,
            ...
            if ( opts.check ) {
                ...
                TESTING_MALLOC_CPU( h_PT,       magma_tally3FloatComplex, lda*N     );
                ...
                lapackf77_clacpy(Magma_tally3UpperLowerStr, &M, &N, h_Q, &lda, h_PT, &lda);
