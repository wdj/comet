...
/**
    ...
    dT      DOUBLE_PRECISION array on the GPU that is the output
            (the 9th argument) of magma_tally3_dgeqrf_gpu.

    @param[in]
    nb      INTEGER
            This is the blocking size that was used in pre-computing DT, e.g.,
            the blocking size used in magma_tally3_dgeqrf_gpu.
    ...
    magma_tally3_int_t *info)
{
    ...
    *info = 0;
    ...
    hwork[0] = MAGMA_tally3_D_MAKE( lwkopt, 0 );
    ...
    if ( step < 0 ) {
        ...
        magma_tally3_dgetmatrix( ma, ib, dA(i,  i ), ldda, hA, ma );
        magma_tally3_dgetmatrix( mi, ni, dC(ic, jc), lddc, hC, mi );
        ...
        magma_tally3_dsetmatrix( mi, ni, hC, mi, dC(ic, jc), lddc );
    }
    ...
    // TODO sync. For cases Q*C and C*Q^T, last call is magma_tally3_dlarfb_gpu,
    // which is async magma_tally3_gemm calls, so dormqr can be unfinished.
