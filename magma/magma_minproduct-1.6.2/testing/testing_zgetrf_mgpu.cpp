/*
    -- clMAGMA_minproduct (version 1.6.1) --
...
void init_matrix( int m, int n, magma_minproductDoubleComplex *h_A, magma_minproduct_int_t lda )
{
...
double get_LU_error(magma_minproduct_int_t M, magma_minproduct_int_t N,
                    magma_minproductDoubleComplex *LU, magma_minproduct_int_t lda,
                    magma_minproduct_int_t *ipiv)
{
    magma_minproduct_int_t min_mn = min(M,N);
    magma_minproduct_int_t ione   = 1;
    magma_minproduct_int_t i, j;
    magma_minproductDoubleComplex alpha = MAGMA_minproduct_Z_ONE;
    magma_minproductDoubleComplex beta  = MAGMA_minproduct_Z_ZERO;
    magma_minproductDoubleComplex *A, *L, *U;
    double work[1], matnorm, residual;
    
    TESTING_MALLOC_CPU( A, magma_minproductDoubleComplex, lda*N    );
    TESTING_MALLOC_CPU( L, magma_minproductDoubleComplex, M*min_mn );
    TESTING_MALLOC_CPU( U, magma_minproductDoubleComplex, min_mn*N );
    memset( L, 0, M*min_mn*sizeof(magma_minproductDoubleComplex) );
    memset( U, 0, min_mn*N*sizeof(magma_minproductDoubleComplex) );
    ...
    lapackf77_zlacpy( Magma_minproductLowerStr, &M, &min_mn, LU, &lda, L, &M      );
    lapackf77_zlacpy( Magma_minproductUpperStr, &min_mn, &N, LU, &lda, U, &min_mn );
    for(j=0; j<min_mn; j++)
        L[j+j*M] = MAGMA_minproduct_Z_MAKE( 1., 0. );
    ...
    for( j = 0; j < N; j++ ) {
        for( i = 0; i < M; i++ ) {
            LU[i+j*lda] = MAGMA_minproduct_Z_SUB( LU[i+j*lda], A[i+j*lda] );
        }
    }
...
int main( int argc, char** argv )
{
    ...
    double           error;
    magma_minproductDoubleComplex *h_A;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for( int dev=0; dev < ngpu; dev++ ) {
                ...
                TESTING_MALLOC_DEV( d_lA[dev], magma_minproductDoubleComplex, ldda*ldn_local );
            }
            ...
            /* ====================================================================
               ...
               =================================================================== */
            init_matrix( M, N, h_A, lda );
            magma_minproduct_zsetmatrix_1D_col_bcyclic( M, N, h_A, lda, d_lA, ldda, ngpu, nb );
    
            gpu_time = magma_minproduct_wtime();
            magma_minproduct_zgetrf_mgpu( ngpu, M, N, d_lA, ldda, ipiv, &info );
            gpu_time = magma_minproduct_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_minproduct_zgetrf_mgpu returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
                       
            magma_minproduct_zgetmatrix_1D_col_bcyclic( M, N, d_lA, ldda, h_A, lda, ngpu, nb );
