...
magma_tally4_cgetrf(
    magma_tally4_int_t m, magma_tally4_int_t n, magma_tally4FloatComplex *A, magma_tally4_int_t lda,
    magma_tally4_int_t *ipiv,
    magma_tally4_int_t *info)
{
    #define dAT(i_, j_) (dAT + (i_)*nb*ldda + (j_)*nb)

    magma_tally4FloatComplex *dAT, *dA, *da, *work;
    ...
    *info = 0;
    ...
    if ( (nb <= 1) || (nb >= min(m,n)) ) {
        ...
        magma_tally4_int_t maxm, maxn, ldda, maxdim;
        magma_tally4_int_t i, j, rows, cols, s = min(m, n)/nb;
        ...
        if ( ngpu > 1 ) {
            ...
            magma_tally4_cgetrf_m(ngpu, m, n, A, lda, ipiv, info);
            return *info;
        }
        ...
        int NB = (magma_tally4_int_t)(0.8*freeMem/maxm-h*nb);
        ...
        if (maxdim*maxdim < 2*maxm*maxn) {
            ...
            if (MAGMA_tally4_SUCCESS != magma_tally4_cmalloc( &dA, nb*maxm + maxdim*maxdim )) {
            ...
            ldda = maxdim;
            magma_tally4_csetmatrix( m, n, A, lda, da, ldda );
            ...
            if (MAGMA_tally4_SUCCESS != magma_tally4_cmalloc( &dA, (nb + maxn)*maxm )) {
                /* alloc failed so call non-GPU-resident version */
                magma_tally4_cgetrf_m(ngpu, m, n, A, lda, ipiv, info);
                return *info;
            }
            ...
            magma_tally4_csetmatrix( m, n, A, lda, da, maxm );
            
            if (MAGMA_tally4_SUCCESS != magma_tally4_cmalloc( &dAT, maxm*maxn )) {
                /* alloc failed so call non-GPU-resident version */
            ...
            magma_tally4blas_ctranspose( m, n, da, maxm, dAT, ldda );
        }
        ...
        for( j = 0; j < s; j++ ) {
            ...
            if (j > 0) {
                magma_tally4blas_ctranspose( nb, cols, dAT(j,j), ldda, dA, cols );

                // make sure that gpu queue is empty
                magma_tally4_device_sync();

                magma_tally4_cgetmatrix_async( m-j*nb, nb, dA, cols, work, lda,
                                        stream[0]);
            ...
            // upload j-th panel
            magma_tally4_csetmatrix_async( m-j*nb, nb, work, lda, dA, cols,
                                    stream[0]);

            for( i=j*nb; i < j*nb + nb; ++i ) {
                ipiv[i] += j*nb;
            }
            magma_tally4blas_claswp( n, dAT, ldda, j*nb + 1, j*nb + nb, ipiv, 1 );

            magma_tally4_queue_sync( stream[0] );
            magma_tally4blas_ctranspose( cols, nb, dA, cols, dAT(j,j), ldda );
        ...
        if ( nb0 > 0 ) {
            ...
            magma_tally4blas_ctranspose( nb0, rows, dAT(s,s), ldda, dA, cols );
            magma_tally4_cgetmatrix( rows, nb0, dA, cols, work, lda );
            ...
            magma_tally4blas_claswp( n, dAT, ldda, s*nb + 1, s*nb + nb0, ipiv, 1 );

            // upload j-th panel
            magma_tally4_csetmatrix( rows, nb0, work, lda, dA, cols );
            magma_tally4blas_ctranspose( rows, nb0, dA, cols, dAT(s,s), ldda );
        ...
        if (maxdim*maxdim < 2*maxm*maxn) {
            magma_tally4blas_ctranspose_inplace( ldda, dAT, ldda );
            magma_tally4_cgetmatrix( m, n, da, ldda, A, lda );
        }
        else {
            magma_tally4blas_ctranspose( n, m, dAT, ldda, da, maxm );
            magma_tally4_cgetmatrix( m, n, da, maxm, A, lda );
