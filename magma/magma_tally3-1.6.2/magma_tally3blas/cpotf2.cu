...
void cpotf2_csscal(magma_tally3_int_t n, magma_tally3FloatComplex *x, magma_tally3_int_t incx);
void cpotf2_cdotc(magma_tally3_int_t n, magma_tally3FloatComplex *x, magma_tally3_int_t incx);

#if defined(PRECISION_z) || defined(PRECISION_c)
void clacgv(magma_tally3_int_t n, magma_tally3FloatComplex *x, magma_tally3_int_t incx);
#endif
    ...
    if (uplo == Magma_tally3Upper) {
        for(j = 0; j < n; j++) {
            ...
            if (j < n) {
                ...
                clacgv(j, dA(0, j), 1);
                #endif
                magma_tally3_cgemv( Magma_tally3Trans, j, n-j-1,
                             alpha, dA(0, j+1), ldda,
                                    dA(0, j),   1,
                             beta,  dA(j, j+1), ldda);
        ...
        for(j = 0; j < n; j++) {
            ...
            if (j < n) {
                ...
                #endif
                magma_tally3_cgemv( Magma_tally3NoTrans, n-j-1, j,
                             alpha, dA(j+1, 0), ldda,
                                    dA(j,0),    ldda,
                             beta,  dA(j+1, j), 1 );
...
__global__ void kernel_cdotc(int n, magma_tally3FloatComplex *x, int incx, int threadSize)
{
    int tx = threadIdx.x;
...
*/
    ...
    kernel_cdotc<<< 1, threadSize, threadSize * sizeof(float), magma_tally3_stream>>> (n, x, incx, threadSize);
}

__global__ void kernel_csscal(int n, magma_tally3FloatComplex *x, int incx)
{
...
*/
    ...
    kernel_csscal<<< grid, threads, 0, magma_tally3_stream >>> (n, x, incx);
}


#if defined(PRECISION_z) || defined(PRECISION_c)

__global__ void kernel_clacgv(int n, magma_tally3FloatComplex *x, int incx)
{
...
void clacgv(magma_tally3_int_t n, magma_tally3FloatComplex *x, magma_tally3_int_t incx)
{
    ...
    dim3 grid(num_blocks,1);
    kernel_clacgv<<< grid, threads, 0, magma_tally3_stream >>> (n, x, incx);
}
