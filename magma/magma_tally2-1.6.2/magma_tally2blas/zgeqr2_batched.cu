...
zlarfg_device(
    int n,
    magma_tally2DoubleComplex* dalpha, magma_tally2DoubleComplex* dx, int incx,
    magma_tally2DoubleComplex* dtau,  double* swork, double* sscale, magma_tally2DoubleComplex* scale)
{
    ...
    magma_tally2_max_reduce<BLOCK_SIZE>( tx, swork );
    ...
    if ( *sscale > 0 ) {
        ...
        magma_tally2_sum_reduce<BLOCK_SIZE>( tx, swork );

    }
    
    if ( tx == 0 ) {
        magma_tally2DoubleComplex alpha = *dalpha;
        ...
        if ( swork[0] == 0 && imag(alpha) == 0 ) {
            ...
            *dtau   = MAGMA_tally2_Z_MAKE( (beta - real(alpha)) / beta, -imag(alpha) / beta );
            *dalpha = MAGMA_tally2_Z_MAKE( beta, 0 );
            *scale = 1 / (alpha - beta);
        }
    }
...
static __device__
void zlarfx_device( int m, int n,  magma_tally2DoubleComplex *v, magma_tally2DoubleComplex *tau,
                         magma_tally2DoubleComplex *dc, magma_tally2_int_t ldc, magma_tally2DoubleComplex* sum)
{


    if(n <=0) return ;
    if (MAGMA_tally2_Z_EQUAL(*tau, MAGMA_tally2_Z_ZERO) )  return; // check singularity
    ...
    for(int k=0;k<n;k++)
    {
        ...
        if(tx<BLOCK_SIZE)
        {
            ...
            for( int j = tx+1; j < m; j += BLOCK_SIZE ){
                lsum += MAGMA_tally2_Z_MUL( MAGMA_tally2_Z_CNJG( v[j] ), dc[j+ldc*k] );
            }
        ...
        magma_tally2DoubleComplex z__1 = - MAGMA_tally2_Z_CNJG(*tau) * sum[0];
        /*  C := C - v * w  */
        if(tx<BLOCK_SIZE)
        {    
           for( int j = tx+1; j<m ; j += BLOCK_SIZE )
...
__global__
void zgeqr2_sm_kernel_batched( int m, int n, magma_tally2DoubleComplex** dA_array, magma_tally2_int_t lda,
                               magma_tally2DoubleComplex **dtau_array)
{

    magma_tally2DoubleComplex* dA = dA_array[blockIdx.z];
    magma_tally2DoubleComplex* dtau = dtau_array[blockIdx.z];
...
static __device__
void zgeqr2_device( magma_tally2_int_t m, magma_tally2_int_t n, magma_tally2DoubleComplex* dA, magma_tally2_int_t lda,
                               magma_tally2DoubleComplex *dtau, 
                               magma_tally2DoubleComplex *sdata,
...
__global__
void zgeqr2_kernel_batched( int m, int n, magma_tally2DoubleComplex** dA_array, magma_tally2_int_t lda,
                               magma_tally2DoubleComplex **dtau_array)
{
...
/**
    ...
    @param
    dwork   (workspace) COMPLEX_16 array, dimension (N) * ( sizeof(double) + sizeof(magma_tally2DoubleComplex)) 
...
magma_tally2_zgeqr2_batched(magma_tally2_int_t m, magma_tally2_int_t n, magma_tally2DoubleComplex **dA_array,
                  magma_tally2_int_t lda, magma_tally2DoubleComplex **dtau_array,
                  magma_tally2_int_t *info_array, magma_tally2_int_t batchCount, magma_tally2_queue_t queue)
{
    ...
    if(sizeof(magma_tally2DoubleComplex)*(m*k) <= 128 /*sizeof(magma_tally2DoubleComplex) * 128 * k*/) // there are some static shared memory besides of dynamic ones 
    {   
        ...
        zgeqr2_sm_kernel_batched<<< blocks, threads, sizeof(magma_tally2DoubleComplex)*(m*k), queue >>>
    ...
    else
    {
        ...
        if(sizeof(magma_tally2DoubleComplex)*(m) < 42000)
            zgeqr2_kernel_batched<<< blocks, threads, sizeof(magma_tally2DoubleComplex)*(m), queue >>>
                                      (m, k, dA_array, lda, dtau_array);
        else
