...
static void magma_tally2_dsygst_m_1_L_col_update(magma_tally2_int_t nk, magma_tally2_int_t nb, double* dA_col, magma_tally2_int_t ldda,
                                          double* dC1, magma_tally2_int_t lddc1, double* dC2, magma_tally2_int_t lddc2);

static void magma_tally2_dsygst_m_1_U_row_update(magma_tally2_int_t nk, magma_tally2_int_t nb, double* dA_row, magma_tally2_int_t ldda,
                                          double* dC1, magma_tally2_int_t lddc1, double* dC2, magma_tally2_int_t lddc2);
...
#define dwork(gpui, i, j) (dw[gpui] + dima*ldda + lddbc*lddbr + (j)*nb*nb + (i)*nb)
    ...
    magma_tally2_int_t nb = magma_tally2_get_dsygst_nb_m(n);
    ...
    *info = 0;
    ...
    for (magma_tally2_int_t igpu = 0; igpu < ngpu; ++igpu) {
        ...
        if (MAGMA_tally2_SUCCESS != magma_tally2_dmalloc( &dw[igpu], (dima*ldda + lddbc*lddbr + n*nb) )) {
    ...
    if (itype == 1) {
        if (upper) {
            ...
            for (magma_tally2_int_t k = 0; k < nbl; ++k) {
                ...
                magma_tally2_dsetmatrix_async(kb, n-k*nb,
                                       A(k, k),              lda,
            ...
            for (magma_tally2_int_t k = 0; k < nbl; ++k) {
                ...
                if (k > 0) {
                    ...
                    magma_tally2_dsyr2k(Magma_tally2Upper, Magma_tally2ConjTrans, kb, nb,
                    ...
                    magma_tally2_dgemm(Magma_tally2ConjTrans, Magma_tally2NoTrans, kb, nk-kb, nb, c_neg_one, dwork(igpu_p, 0, k), nb,
                                dB_r(igpu_p, ind_k1, k+1), lddbr, c_one, dA(igpu_p, k/ngpu, k+1), ldda );

                    magma_tally2_dgemm(Magma_tally2ConjTrans, Magma_tally2NoTrans, kb, nk-kb, nb, c_neg_one, dB_r(igpu_p, ind_k1, k), lddbr,
                    ...
                    for (magma_tally2_int_t j=k+1; j < nbl; ++j) {
                        ...
                        if (igpu != igpu_p) {
                            ...
                            magma_tally2_dsygst_m_1_U_row_update(n-j*nb, nb, dA(igpu, j/ngpu, j), ldda,
                ...
                if (k+1 < nbl) {
                    ...
                    magma_tally2_dtrsm(Magma_tally2Left, uplo, Magma_tally2ConjTrans, Magma_tally2NonUnit,
                                kb, n-(k+1)*nb,
                ...
                if (k > 0) {
                    ...
                    for (magma_tally2_int_t j=k+ngpu; j < nbl; j += ngpu) {
                        ...
                        magma_tally2_dsygst_m_1_U_row_update(n-j*nb, nb, dA(igpu_p, j/ngpu, j), ldda,
                ...
                if (k+1 < nbl) {
                    ...
                    for (magma_tally2_int_t igpu = 0; igpu < ngpu; ++igpu) {
                        magma_tally2_setdevice(igpu);
                        magma_tally2_dsetmatrix_async(kb, n-(k+1)*nb,
                                               A(k, k+1),          lda,
            ...
            if (n > nb) {
                ...
                for (magma_tally2_int_t j = 1; j < nbl; ++j) {
                    ...
                    for (magma_tally2_int_t igpu = 0; igpu < ngpu; ++igpu) {
                        magma_tally2_setdevice(igpu);
                        if (nloc[igpu] > 0) {
                            magma_tally2_dsetmatrix_async(jb, n-j*nb,
                                                   B(j, j),            ldb,
                            ...
                            magma_tally2_dtrsm(Magma_tally2Right, uplo, Magma_tally2NoTrans, Magma_tally2NonUnit, nloc[igpu], jb, c_one, dB_r(igpu, j%2, j), lddbr,
                                        dA(igpu, 0, j), ldda );

                            if ( j < nbl-1 ) {
                                magma_tally2_dgemm(Magma_tally2NoTrans, Magma_tally2NoTrans, nloc[igpu], n-(j+1)*nb, nb, c_neg_one, dA(igpu, 0, j), ldda,
                    ...
                    for (magma_tally2_int_t k = 0; k < j; ++k) {
                        ...
                        magma_tally2_setdevice(igpu);
                        magma_tally2_int_t kb = min(nb, n-k*nb);
                        magma_tally2_dgetmatrix_async(kb, jb,
            ...
            for (magma_tally2_int_t k = 0; k < nbl; ++k) {
                ...
                magma_tally2_setdevice(igpu);
                magma_tally2_int_t kb = min(nb, n-k*nb);
                magma_tally2_dsetmatrix_async((n-k*nb), kb,
                                       A(k, k),              lda,
            ...
            for (magma_tally2_int_t k = 0; k < nbl; ++k) {
                ...
                if (k > 0) {
                    ...
                    magma_tally2_dsyr2k(Magma_tally2Lower, Magma_tally2NoTrans, kb, nb,
                    ...
                    magma_tally2_dgemm(Magma_tally2NoTrans, Magma_tally2ConjTrans, nk-kb, kb, nb, c_neg_one, dwork(igpu_p, k+1, 0), n,
                                dB_c(igpu_p, k, ind_k1), lddbc, c_one, dA(igpu_p, k+1, k/ngpu), ldda );

                    magma_tally2_dgemm(Magma_tally2NoTrans, Magma_tally2ConjTrans, nk-kb, kb, nb, c_neg_one, dB_c(igpu_p, k+1, ind_k1), lddbc,
                    ...
                    for (magma_tally2_int_t j=k+1; j < nbl; ++j) {
                        ...
                        if (igpu != igpu_p) {
                            ...
                            magma_tally2_dsygst_m_1_L_col_update(n-j*nb, nb, dA(igpu, j, j/ngpu), ldda,
                ...
                if (k+1 < nbl) {
                    ...
                    magma_tally2_dtrsm(Magma_tally2Right, uplo, Magma_tally2ConjTrans, Magma_tally2NonUnit,
                                n-(k+1)*nb, kb,
                ...
                if (k > 0) {
                    ...
                    for (magma_tally2_int_t j=k+ngpu; j < nbl; j += ngpu) {
                        ...
                        magma_tally2_dsygst_m_1_L_col_update(n-j*nb, nb, dA(igpu_p, j, j/ngpu), ldda,
                ...
                if (k+1 < nbl) {
                    ...
                    for (magma_tally2_int_t igpu = 0; igpu < ngpu; ++igpu) {
                        magma_tally2_setdevice(igpu);
                        magma_tally2_dsetmatrix_async((n-(k+1)*nb), kb,
                                               A(k+1, k),          lda,
            ...
            if (n > nb) {
                ...
                for (magma_tally2_int_t j = 1; j < nbl; ++j) {
                    ...
                    for (magma_tally2_int_t igpu = 0; igpu < ngpu; ++igpu) {
                        magma_tally2_setdevice(igpu);
                        if (nloc[igpu] > 0) {
                            magma_tally2_dsetmatrix_async((n-j*nb), jb,
                                                   B(j, j),            ldb,
                            ...
                            magma_tally2blasSetKernelStream(stream[igpu][j%2]);
                            magma_tally2_dtrsm(Magma_tally2Left, uplo, Magma_tally2NoTrans, Magma_tally2NonUnit, jb, nloc[igpu], c_one, dB_c(igpu, j, j%2), lddbc,
                                        dA(igpu, j, 0), ldda );

                            if ( j < nbl-1 ) {
                                magma_tally2_dgemm(Magma_tally2NoTrans, Magma_tally2NoTrans, n-(j+1)*nb, nloc[igpu], nb, c_neg_one, dB_c(igpu, j+1, j%2), lddbc,
                    ...
                    for (magma_tally2_int_t k = 0; k < j; ++k) {
                        ...
                        magma_tally2_setdevice(igpu);
                        magma_tally2_int_t kb = min(nb, n-k*nb);
                        magma_tally2_dgetmatrix_async(jb, kb,
        ...
        if (upper) {
            ...
            if (n > nb) {
                ...
                for (magma_tally2_int_t k = 0; k < nbl; ++k) {
                    ...
                    for (magma_tally2_int_t j = k; j < nbl; ++j) {
                        ...
                        magma_tally2_setdevice(igpu);
                        magma_tally2_int_t jb = min(nb, n-j*nb);
                        magma_tally2_dsetmatrix_async(kb, jb,
                                               A(k, j),              lda,
                    ...
                    for (magma_tally2_int_t igpu = 0; igpu < ngpu; ++igpu) {
                        magma_tally2_setdevice(igpu);
                        magma_tally2_dsetmatrix_async(k*nb + kb, kb,
                                               B(0, k),            ldb,
                        ...
                        magma_tally2_dgemm(Magma_tally2NoTrans, Magma_tally2NoTrans, k*nb, nloc[igpu], kb, c_one, dB_c(igpu, 0, k%2), lddbc,
                                    dA(igpu, k, iloc[igpu]), ldda, c_one, dA(igpu, 0, iloc[igpu]), ldda );

                        magma_tally2_dtrmm(Magma_tally2Left, uplo, Magma_tally2NoTrans, Magma_tally2NonUnit, kb, nloc[igpu], c_one, dB_c(igpu, k, k%2), lddbc,
                                    dA(igpu, k, iloc[igpu]), ldda );

                        magma_tally2_event_record( event[igpu][0], stream[igpu][k%2]);

                        if (igpu == igpu_p) {
                            magma_tally2_dgetmatrix_async(k*nb, kb,
            ...
            for (magma_tally2_int_t k = 0; k < nbl; ++k) {
                ...
                if (k > 0) {
                    for (magma_tally2_int_t igpu = 0; igpu < ngpu; ++igpu) {
                        ...
                        magma_tally2_dsetmatrix_async(k*nb, kb,
                                               A(0, k),           lda,
                    ...
                    magma_tally2_dtrmm(Magma_tally2Right, uplo, Magma_tally2ConjTrans, Magma_tally2NonUnit,
                                k*nb, kb,
                                c_one, dB_c(igpu_p, k, ind_k), lddbc,
                    ...
                    for (magma_tally2_int_t j = 0; j < k; ++j) {
                        ...
                        magma_tally2_dsyr2k(uplo, Magma_tally2NoTrans,
                                     nb, kb,
                                     c_one, dwork(igpu, j, 0), n,
                                     dB_c(igpu, j, ind_k), lddbc,
                                     d_one, dA(igpu, j, j/ngpu), ldda);

                        magma_tally2_dgemm(Magma_tally2NoTrans, Magma_tally2ConjTrans, j*nb, nb, kb, c_one, dB_c(igpu, 0, ind_k), lddbc,
                                    dwork(igpu, j, 0), n, c_one, dA(igpu, 0, j/ngpu), ldda );

                        magma_tally2_dgemm(Magma_tally2NoTrans, Magma_tally2ConjTrans, j*nb, nb, kb, c_one, dwork(igpu, 0, 0), n,
                ...
                for (magma_tally2_int_t igpu = 0; igpu < ngpu; ++igpu) {
                    ...
                    magma_tally2_queue_sync(stream[igpu][0]); // sync B copy (conflicts with dsygst)
                }
            ...
            for (magma_tally2_int_t j = 0; j < nbl; ++j) {
                ...
                magma_tally2_setdevice(igpu);
                magma_tally2_int_t jb = min(nb, n-j*nb);
                magma_tally2_dgetmatrix_async(j*nb+jb, jb,
            ...
            if (n > nb) {
                ...
                for (magma_tally2_int_t k = 0; k < nbl; ++k) {
                    ...
                    for (magma_tally2_int_t j = k; j < nbl; ++j) {
                        ...
                        magma_tally2_dsetmatrix_async(jb, kb,
                                               A(j, k),              lda,
                    ...
                    for (magma_tally2_int_t igpu = 0; igpu < ngpu; ++igpu) {
                        magma_tally2_setdevice(igpu);
                        magma_tally2_dsetmatrix_async(kb, k*nb +kb,
                                               B(k, 0),            ldb,
                        ...
                        magma_tally2_dgemm(Magma_tally2NoTrans, Magma_tally2NoTrans, nloc[igpu], k*nb, kb, c_one, dA(igpu, iloc[igpu], k), ldda,
                                    dB_r(igpu, k%2, 0), lddbr, c_one, dA(igpu, iloc[igpu], 0), ldda );

                        magma_tally2_dtrmm(Magma_tally2Right, uplo, Magma_tally2NoTrans, Magma_tally2NonUnit, nloc[igpu], kb, c_one, dB_r(igpu, k%2, k), lddbr,
                                    dA(igpu, iloc[igpu], k), ldda );

                        magma_tally2_event_record( event[igpu][0], stream[igpu][k%2]);

                        if (igpu == igpu_p) {
                            magma_tally2_dgetmatrix_async(kb, k*nb,
            ...
            for (magma_tally2_int_t k = 0; k < nbl; ++k) {
                ...
                if (k > 0) {
                    for (magma_tally2_int_t igpu = 0; igpu < ngpu; ++igpu) {
                        ...
                        magma_tally2_dsetmatrix_async(kb, k*nb,
                                               A(k, 0),           lda,
                    ...
                    magma_tally2_dtrmm(Magma_tally2Left, uplo, Magma_tally2ConjTrans, Magma_tally2NonUnit,
                                kb, k*nb,
                                c_one, dB_r(igpu_p, ind_k, k), lddbr,
                    ...
                    for (magma_tally2_int_t j = 0; j < k; ++j) {
                        ...
                        magma_tally2_dsyr2k(uplo, Magma_tally2ConjTrans,
                                     nb, kb,
                                     c_one, dwork(igpu, 0, j), nb,
                                     dB_r(igpu, ind_k, j), lddbr,
                                     d_one, dA(igpu, j/ngpu, j), ldda);

                        magma_tally2_dgemm(Magma_tally2ConjTrans, Magma_tally2NoTrans, nb, j*nb, kb, c_one, dwork(igpu, 0, j), nb,
                                    dB_r(igpu, ind_k, 0), lddbr, c_one, dA(igpu, j/ngpu, 0), ldda );

                        magma_tally2_dgemm(Magma_tally2ConjTrans, Magma_tally2NoTrans, nb, j*nb, kb, c_one, dB_r(igpu, ind_k, j), lddbr,
            ...
            for (magma_tally2_int_t j = 0; j < nbl; ++j) {
                ...
                magma_tally2_setdevice(igpu);
                magma_tally2_int_t jb = min(nb, n-j*nb);
                magma_tally2_dgetmatrix_async(jb, j*nb+jb,
...
inline static void magma_tally2_dsygst_m_1_U_row_update(
    magma_tally2_int_t nk, magma_tally2_int_t nb,
    double* dA_row, magma_tally2_int_t ldda,
    double* dC1, magma_tally2_int_t lddc1,
    double* dC2, magma_tally2_int_t lddc2)
{
    ...
    magma_tally2_dgemm(Magma_tally2ConjTrans, Magma_tally2NoTrans, kb, nk-kb, nb, c_neg_one, dC1, lddc1,
                dC2+kb*lddc2, lddc2, c_one, dA_row+kb*ldda, ldda );

    magma_tally2_dgemm(Magma_tally2ConjTrans, Magma_tally2NoTrans, kb, nk-kb, nb, c_neg_one, dC2, lddc2,
                dC1+kb*lddc1, lddc1, c_one, dA_row+kb*ldda, ldda );
}


inline static void magma_tally2_dsygst_m_1_L_col_update(
    magma_tally2_int_t nk, magma_tally2_int_t nb,
    double* dA_col, magma_tally2_int_t ldda,
    double* dC1, magma_tally2_int_t lddc1,
    double* dC2, magma_tally2_int_t lddc2)
{
    ...
    magma_tally2_dgemm(Magma_tally2NoTrans, Magma_tally2ConjTrans, nk-kb, kb, nb, c_neg_one, dC1+kb, lddc1,
                dC2, lddc2, c_one, dA_col+kb, ldda );

    magma_tally2_dgemm(Magma_tally2NoTrans, Magma_tally2ConjTrans, nk-kb, kb, nb, c_neg_one, dC2+kb, lddc2,
