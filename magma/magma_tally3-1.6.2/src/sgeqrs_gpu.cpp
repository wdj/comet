    ...
    magma_tally3_int_t *info)
{
    ...
    *info = 0;
    ...
    // TODO: this assumes that, on exit from magma_tally3_sormqr_gpu, hwork contains
    ...
    // magma_tally3_strsm and drop the ssetmatrix.
    if ( nrhs == 1 ) {
        blasf77_strsv( Magma_tally3UpperStr, Magma_tally3NoTransStr, Magma_tally3NonUnitStr,
                       &ib, hwork,         &rows,
                            hwork+rows*ib, &ione);
    } else {
        blasf77_strsm( Magma_tally3LeftStr, Magma_tally3UpperStr, Magma_tally3NoTransStr, Magma_tally3NonUnitStr,
                       &ib, &nrhs,
                       &c_one, hwork,         &rows,
                               hwork+rows*ib, &rows);
    }
    ...
    magma_tally3_ssetmatrix( ib, nrhs, hwork+rows*ib, rows, dwork+i, lddwork );

    // update c
    if (nrhs == 1)
        magma_tally3_sgemv( Magma_tally3NoTrans, i, ib,
    ...
    if (nb < k) {
        for (i = start; i >= 0; i -= nb) {
            ...
            if (i + ib < n) {
                if (nrhs == 1) {
                    magma_tally3_sgemv( Magma_tally3NoTrans, ib, ib,
                                 c_one,  dT(i), ib,
                                         dB+i,      1,
                                 c_zero, dwork+i,  1);
                    magma_tally3_sgemv( Magma_tally3NoTrans, i, ib,
    ...
    magma_tally3_scopymatrix( (n), nrhs,
                       dwork, lddwork,
                       dB,    lddb );
