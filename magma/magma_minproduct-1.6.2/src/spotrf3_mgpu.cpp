...
#define STRSM_WORK
//#define magma_minproduct_strsm magma_minproductblas_strsm
#endif
...
#define dlPT(id, i, j, k) (d_lP[(id)] + (k)*nb*lddp + (j)*nb   + (i))
    ...
    *info = 0;
    ...
    float *d_dinvA[Magma_minproductMaxGPUs];
    float *d_x[Magma_minproductMaxGPUs];
    ...
    for( d=0; d < ngpu; d++ ) {
        ...
        if ( (MAGMA_minproduct_SUCCESS != magma_minproduct_smalloc( &d_dinvA[d], 2*trsm_nb*trsm_n )) ||
             (MAGMA_minproduct_SUCCESS != magma_minproduct_smalloc( &d_x[d],     2*nb*(upper ? n : m) )) ) {
    ...
    if (upper) {
        ...
        for (j=0; j < m; j += nb) {
            ...
            if ( j > 0 ) {
                ...
                magma_minproduct_ssyrk(Magma_minproductUpper, Magma_minproductConjTrans, jb, j,
            ...
            magma_minproduct_sgetmatrix_async( jb, jb,
            ...
            lapackf77_spotrf(Magma_minproductUpperStr, &jb, Aup(j,j), &lda, info);
            trace_cpu_end( 0 );
            if (*info != 0) {
                *info = *info + j;
                break;
            ...
            if ( (j+jb) < n) {
                ...
                for( dd=0; dd < ngpu; dd++ ) {
                    ...
                    magma_minproduct_ssetmatrix_async( jb, jb,
                                            Aup(j,j), lda,
                                            dlpanel,  ldpanel,
                                            queues[d][stream1] );
            ...
            if ( (j+jb) < n) {
...
#if (defined(PRECISION_d) || defined(PRECISION_s)) && defined(STRSM_WORK)
                        magma_minproductblas_slaset( Magma_minproductFull, trsm_nb, trsm_n, c_zero, c_zero, dinvA(d,0), trsm_nb );
                        magma_minproductblas_slaset( Magma_minproductFull, nb0,     jb,     c_zero, c_zero, dx(d,0), nb0 );
                        magma_minproductblas_strsm_work( Magma_minproductLeft, Magma_minproductUpper,
                                              ...
                                              1, dinvA(d,0), dx(d,0) );
#else
                        magma_minproduct_strsm( Magma_minproductLeft, Magma_minproductUpper,
                                     Magma_minproductConjTrans, Magma_minproductNonUnit,
                                     jb, nb0, c_one,
                                     dlpanel,                ldpanel,
                                     dlA(d, j, nb*j_local2), ldda);
...
#if (defined(PRECISION_d) || defined(PRECISION_s)) && defined(STRSM_WORK)
                        magma_minproductblas_slaset( Magma_minproductFull, trsm_nb, trsm_n, c_zero, c_zero, dinvA(d,0), trsm_nb );
                        magma_minproductblas_slaset( Magma_minproductFull, nb2,     jb,     c_zero, c_zero, dx(d,0), nb2 );
                        magma_minproductblas_strsm_work( Magma_minproductLeft, Magma_minproductUpper,
                                              Magma_minproductConjTrans, Magma_minproductNonUnit,
                                              jb, nb2, c_one,
                                              dlpanel, ldpanel,
                ...
                if ( j+jb < m ) {
                    ...
                    magma_minproduct_sgetmatrix_async( (j+jb), nb0,
                    ...
                    for( d2=0; d2 < ngpu; d2++ ) {
                        if ( d2 != d ) {
                            ...
                            magma_minproduct_ssetmatrix_async( j+jb, nb0,
                                                    Aup(0,j+jb),       lda,
                    ...
                    if ( nb2 > 0 ) {
                        ...
                        if (flag == 0) {
                            ...
                            magma_minproductblas_slaset( Magma_minproductFull, trsm_nb, trsm_n, c_zero, c_zero, dinvA(d,flag), trsm_nb );
                            magma_minproduct_queue_wait_event( queues[d][stream2], events[d][1] ); // panel received
                        }
                        magma_minproductblas_slaset( Magma_minproductFull, nb2, jb, c_zero, c_zero, dx(d,1), nb2 );
                        magma_minproductblas_strsm_work( Magma_minproductLeft, Magma_minproductUpper, Magma_minproductConjTrans, Magma_minproductNonUnit,
                                              jb, nb2, c_one,
                                              dlpanel, ldpanel,
        ...
        for (j=0; j < n; j += nb) {
            ...
            if ( j > 0 ) {
                magma_minproductblasSetKernelStream( queues[id][stream1] );
                magma_minproduct_ssyrk(Magma_minproductLower, Magma_minproductNoTrans, jb, j,
            ...
            magma_minproduct_setdevice(id);
            magma_minproduct_queue_sync( queues[id][stream1] );
            lapackf77_spotrf(Magma_minproductLowerStr, &jb, Alo(j,j), &lda, info);
            if (*info != 0) {
                *info = *info + j;
                break;
            }
            ...
            if ( (j+jb) < m) {
                ...
                for( dd=0; dd < ngpu; dd++ ) {
                    ...
                    if ( j+nb < n && d == (j/nb+1)%ngpu ) { /* owns next column, look-ahead next block on stream1 */
                        ...
                        magma_minproductblas_slaset( Magma_minproductFull, trsm_nb, trsm_n, c_zero, c_zero, dinvA(d,0), trsm_nb );
                        magma_minproductblas_slaset( Magma_minproductFull, nb0,     jb,     c_zero, c_zero, dx(d,0), nb0 );
                        magma_minproductblas_strsm_work( Magma_minproductRight, Magma_minproductLower,
                                              Magma_minproductConjTrans, Magma_minproductNonUnit,
                                              nb0, jb, c_one,
                                              dlpanel, ldpanel,
                                              ...
                                              1, dinvA(d,0), dx(d,0) );
#else
                        magma_minproduct_strsm( Magma_minproductRight, Magma_minproductLower,
                                     Magma_minproductConjTrans, Magma_minproductNonUnit,
                                     nb0, jb, c_one,
                                     dlpanel, ldpanel,
                                     dlA(d, nb*j_local2, j), ldda);
                        ...
                        magma_minproductblas_slaset( Magma_minproductFull, trsm_nb, trsm_n, c_zero, c_zero, dinvA(d,0), trsm_nb );
                        magma_minproductblas_slaset( Magma_minproductFull, nb2,     jb,     c_zero, c_zero, dx(d,0), nb2 );
                        magma_minproductblas_strsm_work( Magma_minproductRight, Magma_minproductLower, Magma_minproductConjTrans, Magma_minproductNonUnit,
                                              nb2, jb, c_one,
                                              dlpanel,                ldpanel,
                                              dlA(d, nb*j_local2, j), ldda,
                                              1, dinvA(d,0), dx(d,0) );
...
//#define SPOTRF_DEVICE_TO_DEVICE
                    ...
                    for( d2=0; d2 < ngpu; d2++ ) {
                        ...
                        if ( d2 != d ) {
                            magma_minproduct_scopymatrix_async( nb0, j+jb,
                                                     dlPT(d2,0,nb,buf2), nb, // first nbxnb reserved for diagonal block
                                                     dlA(d, nb*j_local2, 0), ldda,
                                                     queues[d2][stream3] );
                            magma_minproduct_event_record( events[d2][0], queues[d2][stream3] );
                        } else {
                            magma_minproduct_sgetmatrix_async( nb0, j+jb,
                                                    dlA(d, nb*j_local2, 0), ldda,
                                                    Alo(j+jb,0),            lda,
                                                    queues[d][stream3] );
                        }
                    ...
                    for( d2=0; d2 < ngpu; d2++ ) {
                        if ( d2 != d ) {
                            ...
                            magma_minproduct_ssetmatrix_async( nb0, j+jb,
                                                    Alo(j+jb,0),        lda,
                                                    dlPT(d2,0,nb,buf2), nb, // first nbxnb reserved for diagonal block
                                                    queues[d2][stream3] );
                            magma_minproduct_event_record( events[d2][0], queues[d2][stream3] );
    ...
    magma_minproduct_int_t *info)
{
    ...
    if (uplo == Magma_minproductUpper) {
        ...
        for (j=off_j; j < n; j += nb) {
            ...
            magma_minproduct_ssetmatrix_async( mj, jb,
                                    A(off_i, j),     lda,
                                    dA(k, 0, jj*nb), ldda,
                                    queues[k][0] );
        }
        ...
        for (i=off_i; i < m; i += nb) {
            ...
            magma_minproduct_setdevice(k);
            magma_minproduct_ssetmatrix_async( ib, ni,
                                    A(i, off_j),     lda,
                                    dA(k, ii*nb, 0), ldda,
                                    queues[k][0] );
        }
    ...
    magma_minproduct_int_t *info)
{
    ...
    if (uplo == Magma_minproductUpper) {
        ...
        for (j=off_j+NB; j < n; j += nb) {
            ...
            magma_minproduct_setdevice(k);
            magma_minproduct_sgetmatrix_async( mj, jb,
                                    dA(k, 0, jj*nb), ldda,
                                    A(off_i, j),     lda,
                                    queues[k][0] );
            magma_minproduct_queue_sync( queues[k][0] );
        ...
        for (i=off_i+NB; i < m; i += nb) {
            ...
            magma_minproduct_setdevice(k);
            magma_minproduct_sgetmatrix_async( ib, ni,
                                    dA(k, ii*nb, 0), ldda,
                                    A(i, off_j),     lda,
                                    queues[k][0] );
            magma_minproduct_queue_sync( queues[k][0] );
