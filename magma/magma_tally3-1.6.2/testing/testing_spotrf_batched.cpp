...
int main( int argc, char** argv)
{
    ...
    for( int i = 0; i < opts.ntest; ++i ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lapackf77_slacpy( Magma_tally3UpperLowerStr, &N, &(columns), h_A, &lda, h_R, &lda );
            magma_tally3_ssetmatrix( N, columns, h_A, lda, d_A, ldda );
            ...
            info = magma_tally3_spotrf_batched( opts.uplo, N, d_A_array, ldda, dinfo_magma_tally3, batchCount, queue);
            ...
            {
                if(cpu_info[i] != 0 ){
                    printf("magma_tally3_spotrf_batched matrix %d returned internal error %d\n",i, (int)cpu_info[i] );
                }
            }
            if (info != 0)
                printf("magma_tally3_spotrf_batched returned argument error %d: %s.\n", (int) info, magma_tally3_strerror( info ));
            ...
            if ( opts.lapack ) {
                ...
                for(int i=0; i<batchCount; i++)
                {
                   lapackf77_spotrf( lapack_uplo_const_tally3(opts.uplo), &N, h_A + i * lda * N, &lda, &info );
                }
                ...
                /* =====================================================================
                   ...
                   =================================================================== */
                 magma_tally3_sgetmatrix( N, columns, d_A, ldda, h_R, lda );
                 magma_tally3_int_t NN = lda*N;
                 char const uplo = 'l'; // lapack_uplo_const_tally3(opts.uplo)
                 float err = 0.0;
                 for(int i=0; i<batchCount; i++)
                 { 
                     error = lapackf77_slansy("f", &uplo, &N, h_A + i * lda*N, &lda, work);                
                     blasf77_saxpy(&NN, &c_neg_one, h_A + i * lda*N, &ione, h_R + i  * lda*N, &ione);
                     error = lapackf77_slansy("f", &uplo, &N, h_R + i * lda*N, &lda, work) / error;
