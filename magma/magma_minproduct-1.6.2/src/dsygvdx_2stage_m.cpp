    ...
    magma_minproduct_int_t *info)
{
    ...
    *info = 0;
    ...
    magma_minproduct_dpotrf_m( ngpu, uplo, n, B, ldb, info );
    if (*info != 0) {
        *info = n + *info;
        return *info;
    }
    ...
    magma_minproduct_dsygst_m( ngpu, itype, uplo, n, A, lda, B, ldb, info );

    timer_stop( time );
    timer_printf( "time dsygst_m = %6.2f\n", time );
    timer_start( time );

    magma_minproduct_dsyevdx_2stage_m( ngpu, jobz, range, uplo, n, A, lda, vl, vu, il, iu, mout, w, work, lwork, iwork, liwork, info );
    ...
    if (wantz && *info == 0) {
        ...
        if (itype == 1 || itype == 2) {
            ...
            magma_minproduct_dtrsm_m( ngpu, Magma_minproductLeft, uplo, trans, Magma_minproductNonUnit, n, *mout, d_one, B, ldb, A, lda );
        }
        else if (itype == 3) {
            /* For B*A*x=(lambda)*x;
            ...
            //magma_minproduct_dtrmm_m(ngpu, Magma_minproductLeft, uplo, trans, Magma_minproductNonUnit, n, *mout, d_one, B, ldb, A, lda);
            #ifdef ENABLE_DEBUG
            ...
            magma_minproduct_dtrmm( Magma_minproductLeft, uplo, trans, Magma_minproductNonUnit,
