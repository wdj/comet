    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( norm[inorm] == Magma_minproductOneNorm )
                ...
                lwork = M;
            // read whole matrix
            gbytes = M*N*sizeof(magma_minproductDoubleComplex) / 1e9;
            ...
            //h_A[ 1 + 1*lda ] = MAGMA_minproduct_Z_NAN;
            ...
            norm_magma_minproduct = magma_minproductblas_zlange( norm[inorm], M, N, d_A, ldda, d_work );
            gpu_time = magma_minproduct_wtime() - gpu_time;
            gpu_perf = gbytes / gpu_time;
            if (norm_magma_minproduct < 0)
                printf("magma_minproductblas_zlange returned error %f: %s.\n",
            ...
            //         MAGMA_minproduct_Z_REAL( h_A[1+1*lda] ),
            //         MAGMA_minproduct_Z_IMAG( h_A[1+1*lda] ) );
