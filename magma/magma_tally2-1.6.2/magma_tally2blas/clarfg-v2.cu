...
__global__
void magma_tally2_clarfg_gpu_kernel( int n, magma_tally2FloatComplex* dx0, magma_tally2FloatComplex* dx,
...
#else
    float alphar = MAGMA_tally2_C_REAL(alpha);
    float alphai = MAGMA_tally2_C_IMAG(alpha);
    if ( xnorm != 0 || alphai != 0) {
        if (i == 0) {
    ...
    magma_tally2FloatComplex_ptr dAkk)
{
    ...
    //magma_tally2blas_scnrm2_cols(n, 1, dx0, n, dxnorm);
    magma_tally2blas_scnrm2_cols(n-1, 1, dx0+1, n, dxnorm);

    magma_tally2_clarfg_gpu_kernel<<< blocks, threads,
