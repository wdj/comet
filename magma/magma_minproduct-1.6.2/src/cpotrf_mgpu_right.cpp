    ...
    magma_minproduct_int_t *info )
{
    ...
    magma_minproductFloatComplex z_one = MAGMA_minproduct_C_MAKE(  1.0, 0.0 );
    magma_minproductFloatComplex mz_one = MAGMA_minproduct_C_MAKE( -1.0, 0.0 );
    float             one =  1.0;
    float             m_one = -1.0;
    const char* uplo_ = lapack_uplo_const( uplo );

    magma_minproduct_int_t j, nb, d, id, j_local, blkid, crosspoint, prevtrsmrows=0, nqueue = 5;
    magma_minproductFloatComplex *panel, *tmppanel0, *tmppanel1, *tmppanel, *tmpprevpanel;
    magma_minproductFloatComplex *d_lP[Magma_minproductMaxGPUs], *dlpanel, *dlpanels[Magma_minproductMaxGPUs];
    magma_minproduct_int_t rows, trsmrows, igpu, n_local[Magma_minproductMaxGPUs], ldpanel;
    magma_minproduct_queue_t queues[Magma_minproductMaxGPUs][10];
    ...
    *info = 0;
    ...
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_cmalloc_pinned( &panel, 2 * nb * ldpanel )) {
    ...
    if ((nb <= 1) || (nb >= n)) {
        // Use unblocked code.
        magma_minproduct_cgetmatrix( n, n, dlA(0, 0, 0), ldda, panel, ldpanel);
        lapackf77_cpotrf( uplo_, &n, panel, &ldpanel, info);
        magma_minproduct_csetmatrix( n, n, panel, ldpanel, dlA(0, 0, 0), ldda );
    } else {
        for( d = 0; d < ngpu; d++ ) {
            ...
            if (MAGMA_minproduct_SUCCESS != magma_minproduct_cmalloc( &d_lP[d], nb * ldda )) {
                for( j = 0; j < d; j++ ) {
                    magma_minproduct_setdevice(j);
                    magma_minproduct_free( d_lP[d] );
                }
                *info = MAGMA_minproduct_ERR_DEVICE_ALLOC;
                return *info;
            }
            for( j=0; j < nqueue; j++ ) {
                magma_minproduct_queue_create( &queues[d][j] );
            }
        }
        ...
        if (uplo == Magma_minproductUpper) {
            ...
            #if defined (ENABLE_TIMER)
            real_Double_t tget = magma_minproduct_wtime(), tset = 0.0, ttot = 0.0;
            #endif
            if ( n > nb ) {
                // send first panel to cpu
                magma_minproduct_setdevice(0);
                tmppanel = tmppanel0;
                magma_minproduct_cgetmatrix_async(n, nb,
                        dlA(0, 0, 0), ldda,
                        tmppanel(0),  ldpanel,
                        queues[0][0] );
            }
            ...
            tget = magma_minproduct_wtime()-tget;
            #endif
            ...
            for (j = 0; (j + nb) < n; j += nb) {
                ...
                magma_minproduct_queue_sync( queues[id][0] );
                if (j > 0 && prevtrsmrows > crosspoint) {
                    #if defined (ENABLE_TIMER)
                    tcnp = magma_minproduct_wtime();
                    #endif

                    tmpprevpanel = ((blkid - 1) % 2) == 0 ? tmppanel0 : tmppanel1;

                    blasf77_cgemm( Magma_minproductNoTransStr, Magma_minproductConjTransStr,
                            &rows, &nb, &nb,
                    ...
                    #if defined (ENABLE_TIMER)
                    tcnp = magma_minproduct_wtime() - tcnp;
                    ttot_cnp += tcnp;
                    #endif
                }

                #if defined (ENABLE_TIMER)
                tcchol = magma_minproduct_wtime();
                #endif
                lapackf77_cpotrf(Magma_minproductLowerStr, &nb, tmppanel(j), &ldpanel, info);
                if (*info != 0) {
                    *info = *info + j;
                    break;
                }
                ...
                #if defined (ENABLE_TIMER)
                tcchol = magma_minproduct_wtime() - tcchol;
                ttot_cchol += tcchol;
                tctrsm = magma_minproduct_wtime();
                #endif

                trsmrows = rows - nb;

                if (trsmrows > 0) {
                    blasf77_ctrsm(Magma_minproductRightStr, Magma_minproductLowerStr, Magma_minproductConjTransStr, Magma_minproductNonUnitStr,
                                  &trsmrows, &nb,
                                  &z_one, tmppanel(j), &ldpanel,
                ...
                #if defined (ENABLE_TIMER)
                tctrsm = magma_minproduct_wtime() - tctrsm;
                ttot_ctrsm += tctrsm;
                tctm = magma_minproduct_wtime();
                #endif

                d = (id + 1) % ngpu;
                // send current panel to gpus
                for (igpu = 0; igpu < ngpu; igpu++, d = (d + 1) % ngpu ) {
                    magma_minproduct_int_t myrows = 0;
                    magma_minproduct_int_t row_offset = 0;
                    if ( d == id ) {
                    ...
                    if (myrows > 0) {
                        magma_minproduct_setdevice(d);
                        magma_minproduct_csetmatrix_async(myrows, nb,
                ...
                for (igpu = 0; igpu < ngpu; igpu++, d = (d + 1) % ngpu ) {
                    magma_minproduct_setdevice(d);
                    magma_minproduct_queue_sync( queues[d][0] );
                }

                #if defined (ENABLE_TIMER)
                tctm = magma_minproduct_wtime() - tctm;
                ttot_ctm += tctm;
                #endif
                ...
                if ( (j + nb) < n) {
                    ...
                    if (j + nb + nb < n) {
                        d = (id + 1) % ngpu;
                        magma_minproduct_setdevice(d);
                        magma_minproduct_int_t j_local2 = (j + nb) / (nb * ngpu) * nb;
                        if (trsmrows <= crosspoint) {
                            #if defined (ENABLE_TIMER)
                            tmnp = magma_minproduct_wtime();
                            #endif
                            ...
                            magma_minproductblasSetKernelStream( queues[d][STREAM_ID(j_local2)] );
                            #define CHERK_ON_DIAG
                            #ifdef  CHERK_ON_DIAG
                            magma_minproduct_cherk( Magma_minproductLower, Magma_minproductNoTrans,
                                    nb, nb,
                                    m_one, dlpanel, ldda,
                            ...
                            #if defined (ENABLE_TIMER)
                            magma_minproduct_device_sync();
                            tmnp = magma_minproduct_wtime() - tmnp;
                            ttot_mnp += tmnp;
                            #endif
                        }
                        // send next panel to cpu
                        magma_minproduct_queue_sync( queues[d][STREAM_ID(j_local2)] ); // make sure lookahead is done
                        tmppanel = ((blkid+1) % 2 == 0) ? tmppanel0 : tmppanel1;
                        magma_minproduct_cgetmatrix_async(rows-nb, nb,
                    ...
                    if (n - offset > 0) {
                        ...
                        #if defined (ENABLE_TIMER)
                        for( d=0; d < ngpu; d++ ) therk[d] = magma_minproduct_wtime();
                        #endif

                        //magma_minproductblasSetKernelStream( queues[d] );
                        //magma_minproduct_cherk(Magma_minproductLower, Magma_minproductNoTrans, n - offset, nb,
                        //        m_one, dlpanel, ldda,
                        ...
                        #endif
                                        (ngpu, Magma_minproductLower, Magma_minproductNoTrans,
                                         nb, n - offset, nb,
                        ...
                        for( d=0; d < ngpu; d++ ) {
                            magma_minproduct_setdevice(d);
                            magma_minproduct_device_sync();
                            therk[d] = magma_minproduct_wtime() - therk[d];
                            ttot_herk[d] += therk[d];
                        }
                        #endif
                    }
            ...
            for( d = 0; d < ngpu; d++ ) {
                ...
                for( id=0; id < nqueue; id++ ) {
                    magma_minproduct_queue_sync( queues[d][id] );
                }
            }
            #if defined (ENABLE_TIMER)
            ...
            if (j < n && *info == 0) {
                ...
                magma_minproduct_setdevice(id);
                #if defined (ENABLE_TIMER)
                tset = magma_minproduct_wtime();
                #endif
                magma_minproduct_cgetmatrix(rows, rows, dlA(id, j, j_local), ldda, panel(j), ldpanel);
                lapackf77_cpotrf(Magma_minproductLowerStr, &rows, panel(j), &ldpanel, info);
                magma_minproduct_csetmatrix(rows, rows, panel(j), ldpanel, dlA(id, j, j_local), ldda);
                #if defined (ENABLE_TIMER)
                tset = magma_minproduct_wtime() - tset;
                #endif
            }
            #if defined (ENABLE_TIMER)
        ...
        for( d = 0; d < ngpu; d++ ) {
            magma_minproduct_setdevice(d);
            for( j=0; j < nqueue; j++ ) {
                magma_minproduct_queue_destroy( queues[d][j] );
            }
            magma_minproduct_free( d_lP[d] );
        }
    } // end of not lapack
    ...
    // free workspace
    magma_minproduct_free_pinned( panel );
