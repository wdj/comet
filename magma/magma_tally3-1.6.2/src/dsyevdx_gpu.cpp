    ...
    magma_tally3_int_t *info)
{
    ...
    *info = 0;
    ...
    if (n <= 128) {
        ...
        magma_tally3_dgetmatrix( n, n, dA, ldda, A, lda );
        // TODO: deal with range and mout. Use lapack dsyevx? (There is no lapack dsyevdx.)
        lapackf77_dsyevd( lapack_vec_const_tally3(jobz), lapack_uplo_const_tally3(uplo),
                          &n, A, &lda,
                          w, work, &lwork,
                          iwork, &liwork, info );
        *mout = n;
    ...
    if (MAGMA_tally3_SUCCESS != magma_tally3_dmalloc( &dwork, ldwork )) {
    ...
    anrm = magma_tally3blas_dlansy( Magma_tally3MaxNorm, uplo, n, dA, ldda, dwork );
    iscale = 0;
    sigma  = 1;
    if (anrm > 0. && anrm < rmin) {
        iscale = 1;
    ...
    if (iscale == 1) {
        magma_tally3blas_dlascl( uplo, 0, 0, 1., sigma, n, n, dA, ldda, info );
    }
...
#ifdef FAST_SYMV
    magma_tally3_dsytrd2_gpu( uplo, n, dA, ldda, w, &work[inde],
                       &work[indtau], wA, ldwa, &work[indwrk], llwork,
                       dwork, ldwork, &iinfo );
#else
    magma_tally3_dsytrd_gpu(  uplo, n, dA, ldda, w, &work[inde],
                       &work[indtau], wA, ldwa, &work[indwrk], llwork,
                       &iinfo );
#endif
    ...
    if (! wantz) {
        ...
        magma_tally3_dmove_eig( range, n, w, &il, &iu, vl, vu, mout );
    }
    else {
        timer_start( time );
        ...
        magma_tally3_dsetmatrix( n, *mout, &work[indwrk + n* (il-1) ], n, dwork, lddc );

        magma_tally3_dormtr_gpu( Magma_tally3Left, uplo, Magma_tally3NoTrans, n, *mout, dA, ldda, &work[indtau],
                          dwork, lddc, wA, ldwa, &iinfo );

        magma_tally3_dcopymatrix( n, *mout, dwork, lddc, dA, ldda );
