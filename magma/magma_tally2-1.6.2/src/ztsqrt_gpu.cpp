...
/**
    ...
    @ingroup magma_tally2_zgeqrf_tile
    ********************************************************************/
    ...
    magma_tally2_int_t *info )
{
    ...
    magma_tally2_int_t i, k, ldwork, lddwork, old_i, old_ib, rows, cols;
    magma_tally2_int_t nbmin, ib, ldda;
    
    /* Function Body */
    *info = 0;
    magma_tally2_int_t nb = magma_tally2_get_zgeqrf_nb(*m);
    
    magma_tally2_int_t lwkopt = (*n+*m) * nb;
    work[0] = (magma_tally2DoubleComplex) lwkopt;
    ...
    magma_tally2_int_t lhwork = *lwork - (*m)*nb;
    ...
    for (i = 0; i < k; i += nb) {
        ...
        magma_tally2_zgetmatrix_async( rows, ib,
                            ...
                            // A1(i,i), (*lda)*sizeof(magma_tally2DoubleComplex),
                            // the diagonal of A1 is in d_ref generated and
                            // passed from magma_tally2_zgeqrf_gpu
        magma_tally2_zgetmatrix_async( ib, ib,
                                d_ref(i), ib,
        ...
        if (i > 0) {
            ...
            magma_tally2_zssrfb(*m, cols, &old_ib,
        ...
        magma_tally2_zsetmatrix( *m, ib, work_A2, ldwork, A2(0,i), *lda );
        
        if (i + ib < *n) {
            // Send the triangular factor T from hwork to the GPU in t_ref(i)
            magma_tally2_zsetmatrix( ib, ib, hwork, ib, t_ref(i), lddwork );
            
            if (i+nb < k) {
                /* Apply H' to A(i:m,i+ib:i+2*ib) from the left */
                // if we can do one more step, first update T1
                magma_tally2_zssrfb(*m, ib, &ib,
