...
class magma_tally3_clatrsd_task: public magma_tally3_task
{
public:
    magma_tally3_clatrsd_task(
        ...
        const magma_tally3FloatComplex *in_T, magma_tally3_int_t in_ldt,
        magma_tally3FloatComplex  in_lambda,
        magma_tally3FloatComplex *in_x, //magma_tally3_int_t in_incx,
        magma_tally3FloatComplex *in_scale,
        float *in_cnorm
    ):
        uplo  ( in_uplo   ),
        trans ( in_trans  ),
    ...
    virtual void run()
    {
        ...
        float s;
        magma_tally3_clatrsd( uplo, trans, diag, normin, n,
                       T, ldt, lambda, x, &s, cnorm, &info );
        *scale = MAGMA_tally3_C_MAKE( s, 0 );
        if ( info != 0 ) {
...
private:
    ...
    const magma_tally3FloatComplex *T;
...
class cgemm_task: public magma_tally3_task
{
public:
    cgemm_task(
        ...
        magma_tally3FloatComplex  in_alpha,
        const magma_tally3FloatComplex *in_A, magma_tally3_int_t in_lda,
        const magma_tally3FloatComplex *in_B, magma_tally3_int_t in_ldb,
        magma_tally3FloatComplex  in_beta,
        magma_tally3FloatComplex *in_C, magma_tally3_int_t in_ldc
    ):
        transA( in_transA ),
        transB( in_transB ),
        m     ( in_m      ),
    ...
    virtual void run()
    {
        blasf77_cgemm( lapack_trans_const_tally3(transA), lapack_trans_const_tally3(transB),
...
/**
    ...
    @ingroup magma_tally3_cgeev_comp
    ********************************************************************/
extern "C"
magma_tally3_int_t magma_tally3_ctrevc3_mt(
    ...
    magma_tally3_int_t *info )
{
    ...
    *info = 0;
    ...
    rwork[0] = 0.;
    for( j=1; j < n; ++j ) {
        rwork[j] = magma_tally3_cblas_scasum( j, T(0,j), ione );
    }
    ...
    magma_tally3_int_t gemm_nb = magma_tally3_int_t( ceil( ceil( ((float)n) / nthread ) / 16. ) * 16. );
    if ( gemm_nb % 512 == 0 ) {
        gemm_nb += 32;
    }
    ...
    if ( rightv ) {
        ...
        for( ki=n-1; ki >= 0; --ki ) {
            ...
            //smin = max( ulp*MAGMA_tally3_C_ABS1( *T(ki,ki) ), smlnum );
            ...
            *work(ki,iv) = c_one;
            ...
            if ( ki > 0 ) {
                queue.push_task( new magma_tally3_clatrsd_task(
            ...
            if ( ! over ) {
                ...
                remax = 1. / MAGMA_tally3_C_ABS1( *VR(ii,is) );
                ...
                remax = 1. / MAGMA_tally3_C_ABS1( *VR(ii,ki) );
                ...
                if ( (iv == 1) || (ki == 0) ) {
                    ...
                    for( k = iv; k <= nb; ++k ) {
                        ...
                        remax = 1. / MAGMA_tally3_C_ABS1( *work(ii,nb+k) );
    ...
    if ( leftv ) {
        ...
        for( ki=0; ki < n; ++ki ) {
            ...
            *work(ki,iv) = c_one;
            ...
            // Form right-hand side.
            for( k = ki + 1; k < n; ++k ) {
                *work(k,iv) = -MAGMA_tally3_C_CNJG( *T(ki,k) );
            }
            ...
            if ( ! over ) {
                ...
                remax = 1. / MAGMA_tally3_C_ABS1( *VL(ii,is) );
                ...
                remax = 1. / MAGMA_tally3_C_ABS1( *VL(ii,ki) );
