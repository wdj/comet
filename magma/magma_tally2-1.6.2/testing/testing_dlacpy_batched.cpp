...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_tally2_setvector( ntile, sizeof(double*), hAarray, 1, dAarray, 1 );
            magma_tally2_setvector( ntile, sizeof(double*), hBarray, 1, dBarray, 1 );
            
            gpu_time = magma_tally2_sync_wtime( 0 );
            magma_tally2blas_dlacpy_batched( Magma_tally2UpperLower, mb, nb, dAarray, ldda, dBarray, ldda, ntile, queue );
            ...
            for( int tile = 0; tile < ntile; ++tile ) {
                ...
                lapackf77_dlacpy( Magma_tally2UpperLowerStr, &mb, &nb,
                                  &h_A[offset], &lda,
                                  &h_B[offset], &lda );
            }
