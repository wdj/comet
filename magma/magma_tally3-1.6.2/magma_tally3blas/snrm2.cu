...
__global__ void
magma_tally3blas_snrm2_kernel( int m, float *dA, int ldda, float *dxnorm )
{
...
//==============================================================================
__global__ void
magma_tally3blas_snrm2_check_kernel( int m, float *dA, int ldda, float *dxnorm, 
                               float *lsticc )
{
    const int tx = threadIdx.x;
    float *dx = dA + blockIdx.x * ldda;
    ...
    magma_tally3Float_ptr dlsticc) 
{
    ...
    magma_tally3blas_snrm2_check_kernel<<< blocks, threads >>>( m, dA, ldda, dxnorm, dlsticc );
}


//==============================================================================
__global__ void
magma_tally3blas_snrm2_smkernel( int m, int n, float *dA, int ldda,
                           float *dxnorm )
{
    const int tx = threadIdx.x;
    ...
    float *dxnorm)
{
    ...
    magma_tally3blas_snrm2_smkernel<<< blocks, threads, 0, magma_tally3_stream >>>( m, n, dA, ldda, dxnorm );
}

//==============================================================================
extern "C"
__global__ void
magma_tally3_snrm2_adjust_kernel(float *xnorm, float *c)
{
    const int tx = threadIdx.x;
    ...
    temp = MAGMA_tally3_S_ABS( c[tx] ) / xnorm[0];
    sum[tx] = -temp * temp;
...
*/
extern "C" void
magma_tally3blas_snrm2_adjust(magma_tally3_int_t k, magma_tally3Float_ptr dxnorm, magma_tally3Float_ptr dc)
{
    magma_tally3_snrm2_adjust_kernel<<< 1, k, 0, magma_tally3_stream >>> (dxnorm, dc);
}
...
__global__ void
magma_tally3_snrm2_row_check_adjust_kernel(
    ...
    float *C, int ldc, float *lsticc)
{
    ...
    if (tx < n) {
        float temp = MAGMA_tally3_S_ABS( C[tx*ldc] ) / xnorm[tx];
    ...
    magma_tally3Float_ptr dlsticc)
{
    ...
    magma_tally3_snrm2_row_check_adjust_kernel<<< nblocks, BS >>> (k, tol, dxnorm, dxnorm2, dC, lddc, dlsticc);
}
    ...
    magma_tally3Float_ptr dxnorm) 
{
    ...
    magma_tally3blas_snrm2_kernel<<< blocks, threads, 0, magma_tally3_stream >>>( m, dA, ldda, dxnorm );

    // The following would do the computation on one SM
    // magma_tally3blas_snrm2_sm(m, n, dA, ldda, dxnorm);
}
