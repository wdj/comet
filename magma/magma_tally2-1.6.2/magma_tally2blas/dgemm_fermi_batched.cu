/*
    -- MAGMA_tally2 (version 1.6.1) --
       ...
       @generated from zgemm_fermi_batched.cu normal z -> d, Fri Jan 30 19:00:10 2015
...
#include "dgemm_fermi_kernels_batched.h"
...
/**
    ...
    dA      DOUBLE_PRECISION array of DIMENSION ( LDA, ka ), where ka is
            k  when  transA = Magma_tally2NoTrans,  and is  m  otherwise.
            Before entry with  transA = Magma_tally2NoTrans,  the leading  m by k
    ...
    ldda    INTEGER.
            ...
            in the calling (sub) program. When  transA = Magma_tally2NoTrans then
            LDA must be at least  max( 1, m ), otherwise  LDA must be at
            least  max( 1, k ).
    
    @param[in]
    dB      DOUBLE_PRECISION array of DIMENSION ( LDB, kb ), where kb is
            n  when  transB = Magma_tally2NoTrans,  and is  k  otherwise.
            Before entry with  transB = Magma_tally2NoTrans,  the leading  k by n
    ...
    lddb    INTEGER.
            ...
            in the calling (sub) program. When  transB = Magma_tally2NoTrans then
    ...
    double **dC_array, magma_tally2_int_t lddc, magma_tally2_int_t batchCount, magma_tally2_queue_t queue )
{
    ...
    if      ( transA != Magma_tally2NoTrans && transA != Magma_tally2Trans && transA != Magma_tally2ConjTrans )
        info = -1;
    else if ( transB != Magma_tally2NoTrans && transB != Magma_tally2Trans && transB != Magma_tally2ConjTrans )
        info = -2;
    else if ( m < 0 )
        info = -3;
    else if ( n < 0 )
        ...
        info = -5;
    else if ( transA == Magma_tally2NoTrans ? ldda < m : ldda < k )
        info = -8;
    else if ( transB == Magma_tally2NoTrans ? lddb < k : lddb < n )
        info = -10;
    else if ( lddc < m )
        info = -13;
    ...
    if ( arch < 200  ) {
        ...
        // magma_tally2blas for [sd] precisions, cublas for [zc] precisions.
        #if defined(PRECISION_z) || defined(PRECISION_c)
        magma_tally2_dgemm(
            transA, transB,
            m, n, k, alpha, dA_array[0], ldda, dB_array[0], lddb, beta, dC_array[0], lddc );
        #else
        magma_tally2blas_dgemm_tesla(
    ...
    if      ( transA == Magma_tally2Trans )
        TransA = 1;
    else if ( transA == Magma_tally2NoTrans )
        TransA = 0;
                    
    if      ( transB == Magma_tally2Trans )
        TransB = 1;
    else if ( transB == Magma_tally2NoTrans )
        TransB = 0;


    #ifdef TEXTURE_1D
        // allocate CPU ptr to copy the GPU array to it in order to bind texture
        double *cpuAarray[2], *cpuBarray[2];
        magma_tally2_getvector( min(2,batchCount), sizeof(double*), dA_array, 1, cpuAarray, 1);
        magma_tally2_getvector( min(2,batchCount), sizeof(double*), dB_array, 1, cpuBarray, 1);
        magma_tally2_int_t matrixA_size = batchCount > 1 ? cpuAarray[1] - cpuAarray[0] : 0;
        magma_tally2_int_t matrixB_size = batchCount > 1 ? cpuBarray[1] - cpuBarray[0] : 0;
        magma_tally2_int_t Am = ( ! TransA ? m : k);
        magma_tally2_int_t An = (!TransA ? k : m);
        magma_tally2_int_t Bm = ( ! TransB ? k : n);
        magma_tally2_int_t Bn = (!TransB ? n : k);
             ...
             sizeB >= CUBLAS_MAX_1DBUF_SIZE )
        {
            ...
            return;
            /*
            magma_tally2_dgemm( transA, transB, m, n, k, alpha,
        ...
        cudaError_t err;
        err = cudaBindTexture(&offsetA, tex_ref_A, cpuAarray[0], sizeA*sizeof(double));
        if ( err != cudaSuccess ) {
            fprintf( stderr, "cannot bind A to texture: %s (%d)\n", cudaGetErrorString(err), err );
            return;
        }	
        err = cudaBindTexture(&offsetB, tex_ref_B, cpuBarray[0], sizeB*sizeof(double));
        if ( err != cudaSuccess ) {
    ...
    if ( TransA == 0 && TransB == 0 ) {
        dim3 dimGrid( (m - 1)/BLK_M_nn + 1,
                      (n - 1)/BLK_N_nn + 1 ,
                      batchCount );
        dgemm_kernel_fermi_nn_batched<<< dimGrid, dimBlock, 0, queue >>>(
        ...
        dim3 dimGrid( (m - 1)/BLK_M_nt + 1,
                      (n - 1)/BLK_N_nt + 1 ,
                      batchCount );
        dgemm_kernel_fermi_nt_batched<<< dimGrid, dimBlock, 0, queue >>>(
            m, n, k, dA_array, ldda, dB_array, lddb, dC_array, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
        ...
        dim3 dimGrid( (m - 1)/BLK_M_nc + 1,
                      (n - 1)/BLK_N_nc + 1 ,
                      batchCount );
        dgemm_kernel_fermi_nc_batched<<< dimGrid, dimBlock, 0, queue >>>(
            m, n, k, dA_array, ldda, dB_array, lddb, dC_array, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
        ...
        dim3 dimGrid( (m - 1)/BLK_M_tn + 1,
                      (n - 1)/BLK_N_tn + 1 ,
                      batchCount );
        dgemm_kernel_fermi_tn_batched<<< dimGrid, dimBlock, 0, queue >>>(
            m, n, k, dA_array, ldda, dB_array, lddb, dC_array, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
        ...
        dim3 dimGrid( (m - 1)/BLK_M_tt + 1,
                      (n - 1)/BLK_N_tt + 1 ,
                      batchCount );
        dgemm_kernel_fermi_tt_batched<<< dimGrid, dimBlock, 0, queue >>>(
            m, n, k, dA_array, ldda, dB_array, lddb, dC_array, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
        ...
        dim3 dimGrid( (m - 1)/BLK_M_tc + 1,
                      (n - 1)/BLK_N_tc + 1 ,
                      batchCount );
        dgemm_kernel_fermi_tc_batched<<< dimGrid, dimBlock, 0, queue >>>(
            m, n, k, dA_array, ldda, dB_array, lddb, dC_array, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
        ...
        dim3 dimGrid( (m - 1)/BLK_M_cn + 1,
                      (n - 1)/BLK_N_cn + 1 ,
                      batchCount );
        dgemm_kernel_fermi_cn_batched<<< dimGrid, dimBlock, 0, queue >>>(
            m, n, k, dA_array, ldda, dB_array, lddb, dC_array, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
        ...
        dim3 dimGrid( (m - 1)/BLK_M_ct + 1,
                      (n - 1)/BLK_N_ct + 1 ,
                      batchCount );
        dgemm_kernel_fermi_ct_batched<<< dimGrid, dimBlock, 0, queue >>>(
            m, n, k, dA_array, ldda, dB_array, lddb, dC_array, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
        ...
        dim3 dimGrid( (m - 1)/BLK_M_cc + 1,
                      (n - 1)/BLK_N_cc + 1 ,
                      batchCount );
        dgemm_kernel_fermi_cc_batched<<< dimGrid, dimBlock, 0, queue >>>(
            m, n, k, dA_array, ldda, dB_array, lddb, dC_array, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
