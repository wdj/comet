...
#define dTup(d, i, j) (   dt[(d)] + (j)*nb    + (i))
    ...
    magma_tally4DoubleComplex *dwork[Magma_tally4MaxGPUs], *dt[Magma_tally4MaxGPUs];
    ...
    *info = 0;
    ...
    for (d=0; d < ngpu; d++ ) {
        ...
        if (MAGMA_tally4_SUCCESS != magma_tally4_zmalloc( &dt[d], NB*lddla + max(2,ngpu)*nb*ldda )) {
    ...
    if (upper) {
        ...
        for( J=0; J < n; J += NB ) {
            ...
            magma_tally4_zhtodpo( ngpu, uplo, JB, n, J, J, nb, A, lda, dwork, NB, stream, &iinfo);
            ...
            for( j=0; j < J; j += nb ) {
                ...
                for( d=0; d < ngpu; d++ ) {
                    magma_tally4_setdevice(d);
                    magma_tally4_zsetmatrix_async( nb, JB,
                                            A(j, J),       lda,
                                            dTup(d, 0, J), nb,
                                            stream[d][0] );
                    n_local[d] = 0;
                ...
                for( jj=0; jj < JB; jj += nb ) { // jj is 'local' column index within the big panel
                    ...
                    magma_tally4_zherk(Magma_tally4Upper, Magma_tally4ConjTrans, jb, nb,
            ...
            // using three streams
            magma_tally4_zpotrf3_mgpu(ngpu, uplo, JB, n-J, J, J, nb,
            ...
            magma_tally4_zdtohpo(ngpu, uplo, JB, n, J, J, nb, NB, A, lda, dwork, NB, stream, &iinfo);
            //magma_tally4_zdtohpo(ngpu, uplo, JB, n, J, J, nb, 0, A, lda, dwork, NB, stream, &iinfo);
        }
    } else {
        /* ========================================================= *
        ...
        for( J=0; J < n; J += NB ) {
            ...
            magma_tally4_zhtodpo( ngpu, uplo, n, JB, J, J, nb, A, lda, dwork, lddla, stream, &iinfo);
            ...
            for( j=0; j < J; j += nb ) {
                ...
                for( d=0; d < ngpu; d++ ) {
                    magma_tally4_setdevice(d);
                    magma_tally4_zsetmatrix_async( JB, nb,
                                            A(J, j),     lda,
                                            dT(d, J, 0), ldda,
                                            stream[d][0] );
                    n_local[d] = 0;
                ...
                for( jj=0; jj < JB; jj += nb ) { /* diagonal */
                    ...
                    magma_tally4_zherk(Magma_tally4Lower, Magma_tally4NoTrans, jb, nb,
            ...
            h = (JB+nb-1)/nb; // big diagonal of big panel will be on CPU
            // using three streams
            magma_tally4_zpotrf3_mgpu(ngpu, uplo, n-J, JB, J, J, nb,
            ...
            magma_tally4_zdtohpo( ngpu, uplo, n, JB, J, J, nb, JB, A, lda, dwork, lddla, stream, &iinfo);
