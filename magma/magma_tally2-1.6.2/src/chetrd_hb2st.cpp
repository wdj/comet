...
static void *magma_tally2_chetrd_hb2st_parallel_section(void *arg);

static void magma_tally2_ctile_bulge_parallel(
    magma_tally2_int_t my_core_id, magma_tally2_int_t cores_num,
    magma_tally2FloatComplex *A, magma_tally2_int_t lda,
    magma_tally2FloatComplex *V, magma_tally2_int_t ldv,
    magma_tally2FloatComplex *TAU, magma_tally2_int_t n, magma_tally2_int_t nb, magma_tally2_int_t nbtiles,
    magma_tally2_int_t grsiz, magma_tally2_int_t Vblksiz, volatile magma_tally2_int_t *prog);

static void magma_tally2_ctile_bulge_computeT_parallel(
    magma_tally2_int_t my_core_id, magma_tally2_int_t cores_num,
    magma_tally2FloatComplex *V, magma_tally2_int_t ldv, magma_tally2FloatComplex *TAU,
    magma_tally2FloatComplex *T, magma_tally2_int_t ldt,
    magma_tally2_int_t n, magma_tally2_int_t nb, magma_tally2_int_t Vblksiz);

//////////////////////////////////////////////////////////////////////////////////////////////////////////

typedef struct magma_tally2_cbulge_data_s {
    magma_tally2_int_t threads_num;
    magma_tally2_int_t n;
    magma_tally2_int_t nb;
    magma_tally2_int_t nbtiles;
    magma_tally2_int_t grsiz;
    magma_tally2_int_t Vblksiz;
    magma_tally2_int_t compT;
    ...
    magma_tally2_int_t ldt;
    volatile magma_tally2_int_t *prog;
    pthread_barrier_t barrier;
} magma_tally2_cbulge_data;

void magma_tally2_cbulge_data_init(
    magma_tally2_cbulge_data *cbulge_data_S,
    magma_tally2_int_t threads_num, magma_tally2_int_t n, magma_tally2_int_t nb, magma_tally2_int_t nbtiles,
    magma_tally2_int_t grsiz, magma_tally2_int_t Vblksiz, magma_tally2_int_t compT,
    magma_tally2FloatComplex *A, magma_tally2_int_t lda,
    magma_tally2FloatComplex *V, magma_tally2_int_t ldv, magma_tally2FloatComplex *TAU,
    magma_tally2FloatComplex *T, magma_tally2_int_t ldt,
    volatile magma_tally2_int_t* prog)
...
void magma_tally2_cbulge_data_destroy(magma_tally2_cbulge_data *cbulge_data_S)
{
    pthread_barrier_destroy(&(cbulge_data_S->barrier));
}
typedef struct magma_tally2_cbulge_id_data_s {
    magma_tally2_int_t id;
    magma_tally2_cbulge_data* data;
} magma_tally2_cbulge_id_data;

void magma_tally2_cbulge_id_data_init(magma_tally2_cbulge_id_data *id_data, magma_tally2_int_t id, magma_tally2_cbulge_data* data)
{
    id_data->id = id;
    id_data->data = data;
}
...
/**
    ...
    uplo    magma_tally2_uplo_t
      -     = Magma_tally2Upper:  Upper triangles of A is stored;
      -     = Magma_tally2Lower:  Lower triangles of A is stored.
    ...
    V       COMPLEX array, dimension (BLKCNT, LDV, VBLKSIZ)
            ...
            BLKCNT is the number of block and it is returned by the funtion MAGMA_tally2_BULGE_GET_BLKCNT.
...
magma_tally2_chetrd_hb2st(
    ...
    magma_tally2FloatComplex *A, magma_tally2_int_t lda, float *d, float *e,
    magma_tally2FloatComplex *V, magma_tally2_int_t ldv, magma_tally2FloatComplex *TAU,
    magma_tally2_int_t compT, magma_tally2FloatComplex *T, magma_tally2_int_t ldt)
{
    #ifdef ENABLE_TIMER
    ...
    magma_tally2_int_t INgrsiz=1;
    magma_tally2_int_t blkcnt = magma_tally2_bulge_get_blkcnt(n, nb, Vblksiz);
    magma_tally2_int_t nbtiles = magma_tally2_ceildiv(n, nb);

    memset(T,   0, blkcnt*ldt*Vblksiz*sizeof(magma_tally2FloatComplex));
    memset(TAU, 0, blkcnt*Vblksiz*sizeof(magma_tally2FloatComplex));
    memset(V,   0, blkcnt*ldv*Vblksiz*sizeof(magma_tally2FloatComplex));

    volatile magma_tally2_int_t* prog;
    magma_tally2_malloc_cpu((void**) &prog, (2*nbtiles+threads+10)*sizeof(magma_tally2_int_t));
    memset((void *) prog, 0, (2*nbtiles+threads+10)*sizeof(magma_tally2_int_t));

    magma_tally2_cbulge_id_data* arg;
    magma_tally2_malloc_cpu((void**) &arg, threads*sizeof(magma_tally2_cbulge_id_data));

    pthread_t* thread_id;
    magma_tally2_malloc_cpu((void**) &thread_id, threads*sizeof(pthread_t));
    pthread_attr_t thread_attr;

    magma_tally2_cbulge_data data_bulge;
    magma_tally2_cbulge_data_init(&data_bulge, threads, n, nb, nbtiles, INgrsiz, Vblksiz, compT,
    ...
    //timing
    #ifdef ENABLE_TIMER
    timeblg = magma_tally2_wtime();
    #endif

    // Launch threads
    for (magma_tally2_int_t thread = 1; thread < threads; thread++) {
        magma_tally2_cbulge_id_data_init(&(arg[thread]), thread, &data_bulge);
        pthread_create(&thread_id[thread], &thread_attr, magma_tally2_chetrd_hb2st_parallel_section, &arg[thread]);
    }
    magma_tally2_cbulge_id_data_init(&(arg[0]), 0, &data_bulge);
    magma_tally2_chetrd_hb2st_parallel_section(&arg[0]);
    ...
    // timing
    #ifdef ENABLE_TIMER
    timeblg = magma_tally2_wtime()-timeblg;
    printf("  time BULGE+T = %f\n", timeblg);
    #endif

    magma_tally2_free_cpu(thread_id);
    magma_tally2_free_cpu(arg);
    magma_tally2_free_cpu((void *) prog);
    magma_tally2_cbulge_data_destroy(&data_bulge);
    ...
    if (uplo == Magma_tally2Lower) {
        for (magma_tally2_int_t i=0; i < n-1; i++) {
            d[i] = MAGMA_tally2_C_REAL( A[i*lda  ] );
            e[i] = MAGMA_tally2_C_REAL( A[i*lda+1] );
        }
        d[n-1] = MAGMA_tally2_C_REAL(A[(n-1)*lda]);
    } else { /* Magma_tally2Upper not tested yet */
        for (magma_tally2_int_t i=0; i < n-1; i++) {
            d[i] = MAGMA_tally2_C_REAL( A[i*lda+nb]   );
            e[i] = MAGMA_tally2_C_REAL( A[i*lda+nb-1] );
        }
        d[n-1] = MAGMA_tally2_C_REAL(A[(n-1)*lda+nb]);
    } /* end Magma_tally2Upper */
#else
    if ( uplo == Magma_tally2Lower ) {
        for (magma_tally2_int_t i=0; i < n-1; i++) {
            d[i] = A[i*lda];   // diag
...
static void *magma_tally2_chetrd_hb2st_parallel_section(void *arg)
{
    magma_tally2_int_t my_core_id  = ((magma_tally2_cbulge_id_data*)arg) -> id;
    magma_tally2_cbulge_data* data = ((magma_tally2_cbulge_id_data*)arg) -> data;

    magma_tally2_int_t allcores_num   = data -> threads_num;
    magma_tally2_int_t n              = data -> n;
    magma_tally2_int_t nb             = data -> nb;
    magma_tally2_int_t nbtiles        = data -> nbtiles;
    magma_tally2_int_t grsiz          = data -> grsiz;
    magma_tally2_int_t Vblksiz        = data -> Vblksiz;
    magma_tally2_int_t compT          = data -> compT;
    magma_tally2FloatComplex *A      = data -> A;
    magma_tally2_int_t lda            = data -> lda;
    magma_tally2FloatComplex *V      = data -> V;
    magma_tally2_int_t ldv            = data -> ldv;
    magma_tally2FloatComplex *TAU    = data -> TAU;
    magma_tally2FloatComplex *T      = data -> T;
    magma_tally2_int_t ldt            = data -> ldt;
    volatile magma_tally2_int_t* prog = data -> prog;

    pthread_barrier_t* barrier = &(data -> barrier);

    //magma_tally2_int_t sys_corenbr    = 1;
...
//#define PRINTAFFINITY
    ...
    if (compT == 1) {
        ...
        if (allcores_num == 1) {
            ...
            //=========================
            #ifdef ENABLE_TIMER
            timeB = magma_tally2_wtime();
            #endif
            
            magma_tally2_ctile_bulge_parallel(0, 1, A, lda, V, ldv, TAU, n, nb, nbtiles, grsiz, Vblksiz, prog);

            #ifdef ENABLE_TIMER
            timeB = magma_tally2_wtime()-timeB;
            ...
            //=========================
            #ifdef ENABLE_TIMER
            timeT = magma_tally2_wtime();
            #endif

            magma_tally2_ctile_bulge_computeT_parallel(0, 1, V, ldv, TAU, T, ldt, n, nb, Vblksiz);

            #ifdef ENABLE_TIMER
            timeT = magma_tally2_wtime()-timeT;
            printf("  Finish T's     timing= %f\n", timeT);
            #endif
        } else { // allcore_num > 1
            magma_tally2_int_t id  = my_core_id;
            magma_tally2_int_t tot = allcores_num;
                ...
                magma_tally2_ctile_bulge_parallel(id, tot, A, lda, V, ldv, TAU, n, nb, nbtiles, grsiz, Vblksiz, prog);
                ...
                magma_tally2_ctile_bulge_computeT_parallel(id, tot, V, ldv, TAU, T, ldt, n, nb, Vblksiz);
                pthread_barrier_wait(barrier);
        ...
        magma_tally2_ctile_bulge_parallel(my_core_id, allcores_num, A, lda, V, ldv, TAU, n, nb, nbtiles, grsiz, Vblksiz, prog);
        pthread_barrier_wait(barrier);
...
static void magma_tally2_ctile_bulge_parallel(
    ...
    magma_tally2_int_t grsiz, magma_tally2_int_t Vblksiz, volatile magma_tally2_int_t *prog)
{
    magma_tally2_int_t sweepid, myid, shift, stt, st, ed, stind, edind;
    magma_tally2_int_t blklastind, colpt;
    magma_tally2_int_t stepercol;
    magma_tally2_int_t i, j, m, k;
    magma_tally2_int_t thgrsiz, thgrnb, thgrid, thed;
    magma_tally2_int_t coreid;
    magma_tally2_int_t colblktile, maxrequiredcores, colpercore, mycoresnb;
    magma_tally2_int_t fin;
    ...
    thgrsiz = n;
    stepercol = magma_tally2_ceildiv(shift, grsiz);
    thgrnb  = magma_tally2_ceildiv(n-1, thgrsiz);
                            ...
                            while(fin == 0) {
                                if (myid == 1) {
                                    if ( prog[myid+shift-1] == (sweepid-1) ) {
                                        magma_tally2_ctrdtype1cbHLsym_withQ_v2(n, nb, A, lda, V, ldv, TAU, stind, edind, sweepid, Vblksiz, work);
                                    ...
                                    if ( (prog[myid-1] == sweepid) && (prog[myid+shift-1] == (sweepid-1)) ) {
                                        if (myid%2 == 0)
                                            magma_tally2_ctrdtype2cbHLsym_withQ_v2(n, nb, A, lda, V, ldv, TAU, stind, edind, sweepid, Vblksiz, work);
                                        else
                                            magma_tally2_ctrdtype3cbHLsym_withQ_v2(n, nb, A, lda, V, ldv, TAU, stind, edind, sweepid, Vblksiz, work);
    ...
    magma_tally2_int_t n, magma_tally2_int_t nb, magma_tally2_int_t Vblksiz)
{
    ...
    magma_tally2_int_t myrow, mycol, blkj, blki, firstrow;
    magma_tally2_int_t blkid, vpos, taupos, tpos;
    magma_tally2_int_t blkpercore, myid;
    ...
    for (blkj=nt-1; blkj >= 0; blkj--) {
        ...
        for (blki=mt; blki > 0; blki--) {
            ...
            magma_tally2_bulge_findVTAUTpos(n, nb, Vblksiz, mycol, myrow, ldv, ldt, &vpos, &taupos, &tpos, &blkid);
