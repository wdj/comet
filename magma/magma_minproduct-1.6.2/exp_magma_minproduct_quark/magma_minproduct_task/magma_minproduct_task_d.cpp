...
void magma_minproduct_task_dlaswp(Schedule* sched_obj   )
{
    ...
    magma_minproductblas_dlaswp(  n, dA,  lda,  i1,  i2,  ipiv,  inci );
     //magma_minproduct_task_dlaswp(gpu_ncols, dAT(K,K+A_N), dAT_LD, c_one, nb, &ipiv[K], c_one);
#if (dbglevel >=1)
ca_trace_end_1gpu('W');
ca_trace_end_cpu('C');
#endif
...
void magma_minproduct_task_dtrsm(Schedule* sched_obj  )
{
    ...
    magma_minproduct_dtrsm( side,  uplo,  trans,  diag,  m,  n, alpha, dA,  lda, dB,  ldb );
     //magma_minproduct_task_dtrsm('R', 'U', 'N', 'U', gpu_ncols, nb, c_one, dAT(K,K), dAT_LD, dAT(K,K+A_N), dAT_LD);
#if (dbglevel >=1)
ca_trace_end_1gpu('U');
ca_trace_end_cpu('C');
#endif
...
void magma_minproduct_task_dgemm(Schedule* sched_obj  )
{
    ...
    magma_minproduct_dgemm( transA,  transB,  m,  n,  k, alpha, dA,  lda, dB,  ldb, beta, dC,  ldc );
     //magma_minproduct_task_dgemm('N','N', gpu_ncols, gpu_nrows, nb, c_neg_one, dAT(K,K+A_N), dAT_LD, dAT(K+1,K), dAT_LD, c_one, dAT(K+1,K+A_N), dAT_LD);
