...
clarfg_device(
    ...
    magma_tally3FloatComplex* dtau,  float* swork, float* sscale, magma_tally3FloatComplex* scale)
{
...
static __device__
void clarfx_device( int m, int n,  magma_tally3FloatComplex *v, magma_tally3FloatComplex *tau,
                         magma_tally3FloatComplex *dc, magma_tally3_int_t ldc, magma_tally3FloatComplex* sum)
{


    if(n <=0) return ;
    if (MAGMA_tally3_C_EQUAL(*tau, MAGMA_tally3_C_ZERO) )  return; // check singularity
    ...
    for(int k=0;k<n;k++)
    {
        ...
        if(tx<BLOCK_SIZE)
        {
            ...
            for( int j = tx+1; j < m; j += BLOCK_SIZE ){
                lsum += MAGMA_tally3_C_MUL( MAGMA_tally3_C_CNJG( v[j] ), dc[j+ldc*k] );
            }
...
__global__
void cgeqr2_sm_kernel_batched( int m, int n, magma_tally3FloatComplex** dA_array, magma_tally3_int_t lda,
                               magma_tally3FloatComplex **dtau_array)
{

    magma_tally3FloatComplex* dA = dA_array[blockIdx.z];
    magma_tally3FloatComplex* dtau = dtau_array[blockIdx.z];
...
static __device__
void cgeqr2_device( magma_tally3_int_t m, magma_tally3_int_t n, magma_tally3FloatComplex* dA, magma_tally3_int_t lda,
                               magma_tally3FloatComplex *dtau, 
                               magma_tally3FloatComplex *sdata,
                               magma_tally3FloatComplex *sum,
                               float *swork,
                               magma_tally3FloatComplex *scale,
                               float *sscale)
{
...
__global__
void cgeqr2_kernel_batched( int m, int n, magma_tally3FloatComplex** dA_array, magma_tally3_int_t lda,
                               magma_tally3FloatComplex **dtau_array)
{
...
/**
    ...
    @param
    dwork   (workspace) COMPLEX array, dimension (N) * ( sizeof(float) + sizeof(magma_tally3FloatComplex)) 
...
magma_tally3_cgeqr2_batched(magma_tally3_int_t m, magma_tally3_int_t n, magma_tally3FloatComplex **dA_array,
                  magma_tally3_int_t lda, magma_tally3FloatComplex **dtau_array,
                  magma_tally3_int_t *info_array, magma_tally3_int_t batchCount, magma_tally3_queue_t queue)
{
    ...
    if(sizeof(magma_tally3FloatComplex)*(m*k) <= 128 /*sizeof(magma_tally3FloatComplex) * 128 * k*/) // there are some static shared memory besides of dynamic ones 
    {   
        ...
        cgeqr2_sm_kernel_batched<<< blocks, threads, sizeof(magma_tally3FloatComplex)*(m*k), queue >>>
    ...
    else
    {
        ...
        if(sizeof(magma_tally3FloatComplex)*(m) < 42000)
            cgeqr2_kernel_batched<<< blocks, threads, sizeof(magma_tally3FloatComplex)*(m), queue >>>
                                      (m, k, dA_array, lda, dtau_array);
        else
