...
#define dTup(d, i, j) (   dt[(d)] + (j)*nb    + (i))
    ...
    float *dwork[Magma_tally3MaxGPUs], *dt[Magma_tally3MaxGPUs];
    magma_tally3_int_t     ldda, lddla, nb, iinfo, n_local[Magma_tally3MaxGPUs], J2, d, ngpu0 = ngpu;
    magma_tally3_int_t     j, jj, jb, J, JB, NB, h;
    magma_tally3_queue_t   stream[Magma_tally3MaxGPUs][3];
    magma_tally3_event_t   event[Magma_tally3MaxGPUs][5];
    magma_tally3_timer_t time_total=0, time_sum=0, time=0;
    ...
    *info = 0;
    ...
    NB = (magma_tally3_int_t)((0.8*freeMem - max(2,ngpu)*nb*ldda - (n+nb)*nb)/lddla); /* number of columns in the big panel */
    //NB = min(5*nb,n);
    ...
    for (d=0; d < ngpu; d++ ) {
        ...
        if (MAGMA_tally3_SUCCESS != magma_tally3_smalloc( &dt[d], NB*lddla + max(2,ngpu)*nb*ldda )) {
        ...
        for( j=0; j < 3; j++ )
            magma_tally3_queue_create( &stream[d][j] );
        for( j=0; j < 5; j++ )
            magma_tally3_event_create( &event[d][j]  );
        magma_tally3_device_sync(); // synch the device
    }
    ...
    if (upper) {
        ...
        for( J=0; J < n; J += NB ) {
            ...
            magma_tally3_shtodpo( ngpu, uplo, JB, n, J, J, nb, A, lda, dwork, NB, stream, &iinfo);
            ...
            for( j=0; j < J; j += nb ) {
                ...
                for( d=0; d < ngpu; d++ ) {
                    magma_tally3_setdevice(d);
                    magma_tally3_ssetmatrix_async( nb, JB,
                                            A(j, J),       lda,
                                            dTup(d, 0, J), nb,
                                            stream[d][0] );
                    n_local[d] = 0;
                ...
                for( jj=0; jj < JB; jj += nb ) { // jj is 'local' column index within the big panel
                    ...
                    magma_tally3_setdevice(d);
                    magma_tally3blasSetKernelStream(stream[d][J2%2]); // the last stream (2) used to process off-diagonal
                    J2 = nb*J2;
                    ...
                    magma_tally3_ssyrk(Magma_tally3Upper, Magma_tally3ConjTrans, jb, nb,
                ...
                if ( n > J+JB ) {
                    for( d=0; d < ngpu; d++ ) {
                        magma_tally3_setdevice(d);
                        magma_tally3blasSetKernelStream(stream[d][2]);
                ...
                for( d=0; d < ngpu; d++ ) {
                    magma_tally3_setdevice(d);
                    for( jj=0; jj < 3; jj++ )
                        magma_tally3_queue_sync( stream[d][jj] );
            ...
            // using three streams
            magma_tally3_spotrf3_mgpu(ngpu, uplo, JB, n-J, J, J, nb,
            ...
            magma_tally3_sdtohpo(ngpu, uplo, JB, n, J, J, nb, NB, A, lda, dwork, NB, stream, &iinfo);
            //magma_tally3_sdtohpo(ngpu, uplo, JB, n, J, J, nb, 0, A, lda, dwork, NB, stream, &iinfo);
        }
    } else {
        /* ========================================================= *
        ...
        for( J=0; J < n; J += NB ) {
            ...
            magma_tally3_shtodpo( ngpu, uplo, n, JB, J, J, nb, A, lda, dwork, lddla, stream, &iinfo);
            ...
            for( j=0; j < J; j += nb ) {
                ...
                for( d=0; d < ngpu; d++ ) {
                    magma_tally3_setdevice(d);
                    magma_tally3_ssetmatrix_async( JB, nb,
                                            A(J, j),     lda,
                                            dT(d, J, 0), ldda,
                                            stream[d][0] );
                    n_local[d] = 0;
                ...
                for( jj=0; jj < JB; jj += nb ) { /* diagonal */
                    ...
                    magma_tally3_setdevice(d);
                    magma_tally3blasSetKernelStream(stream[d][J2%2]);
                    ...
                    magma_tally3_ssyrk(Magma_tally3Lower, Magma_tally3NoTrans, jb, nb,
            ...
            h = (JB+nb-1)/nb; // big diagonal of big panel will be on CPU
            // using three streams
            magma_tally3_spotrf3_mgpu(ngpu, uplo, n-J, JB, J, J, nb,
            ...
            magma_tally3_sdtohpo( ngpu, uplo, n, JB, J, J, nb, JB, A, lda, dwork, lddla, stream, &iinfo);
    ...
    for (d=0; d < ngpu; d++ ) {
        ...
        for( j=0; j < 3; j++ ) {
            magma_tally3_queue_destroy( stream[d][j] );
        }
        magma_tally3_free( dt[d] );

        for( j=0; j < 5; j++ ) {
            magma_tally3_event_destroy( event[d][j] );
        }
    }
