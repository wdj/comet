    ...
    magma_minproduct_int_t batchCount, cublasHandle_t myhandle, magma_minproduct_queue_t queue )
{
    ...
    //===============================================
    if(m < nb){
        printf("magma_minproduct_cgetrf_panel_nopiv_batched_q m < nb %d < %d \n",(int) m, (int) nb);
        return -101;
    }

#if 0
    arginfo = magma_minproduct_cgetf2_nopiv_batched(
                       m, nb,
                       dA_array, ldda,
    ...
    if((m-nb) > 0){
        ...
        magma_minproductblas_ctrsm_work_batched(Magma_minproductRight, Magma_minproductUpper, Magma_minproductNoTrans, Magma_minproductNonUnit,
                              1, m-nb, nb, 
