...
__global__ void
magma_tally4_dreduce_kernel_spmv1(    
    int Gs,
    int n, 
    double * vtmp,
    double * vtmp2 )
...
__global__ void
magma_tally4_dbicgmerge_spmv1_kernel(  
    int n,
    double * dval, 
    ...
    double * vtmp)
{
    ...
    temp[ Idx ] = ( i < n ) ? v[ i ] * r[ i ] : MAGMA_tally4_D_MAKE( 0.0, 0.0);
    __syncthreads();
    if ( Idx < 128 ){
...
__global__ void
magma_tally4_dbicgstab_alphakernel(  
                    double * skp ){
    ...
    magma_tally4_queue_t queue )
{
    ...
    if ( A.storage_type == Magma_tally4_CSR)
        magma_tally4_dbicgmerge_spmv1_kernel<<<Gs, Bs, Ms>>>
    ...
    while( Gs.x > 1 ) {
        ...
        magma_tally4_dreduce_kernel_spmv1<<< Gs_next.x/2, Bs.x/2, Ms/2 >>> 
                            ( Gs.x, n, aux1, aux2 );
    ...
    magma_tally4_dcopyvector( 1, aux1, 1, skp, 1 );
    dim3 Bs2( 2 );
    dim3 Gs2( 1 );
    magma_tally4_dbicgstab_alphakernel<<<Gs2, Bs2, 0>>>( skp );
...
__global__ void
magma_tally4_dreduce_kernel_spmv2( 
    int Gs,
    int n, 
    double * vtmp,
    double * vtmp2 )
{
    ...
    for( j=0; j<2; j++){
        int i = blockIdx.x * ( blockSize * 2 ) + Idx;   
        temp[Idx+j*(blockSize)] = MAGMA_tally4_D_MAKE( 0.0, 0.0);
        while (i < Gs ) {
...
__global__ void
magma_tally4_dbicgmerge_spmv2_kernel(  
    int n,
    double * dval, 
    magma_tally4_index_t * drowptr, 
    magma_tally4_index_t * dcolind,
    ...
    double * vtmp )
{
    ...
    if (i<n){
        ...
        for( j=0; j<2; j++)
            temp[Idx+j*blockDim.x] =MAGMA_tally4_D_MAKE( 0.0, 0.0);
    }
    __syncthreads();
    if ( Idx < 128 ){
        for( j=0; j<2; j++){
...
__global__ void
magma_tally4_dbicgstab_omegakernel(  
                    double * skp ){
    int i = blockIdx.x * blockDim.x + threadIdx.x;
...
/**
    ...
    @param[in]
    ds          magma_tally4Double_ptr
                input vector s

    @param[in]
    dt          magma_tally4Double_ptr
                output vector t
    ...
    magma_tally4_queue_t queue )
{
    ...
    int b = 1;        
    if ( A.storage_type == Magma_tally4_CSR)
        magma_tally4_dbicgmerge_spmv2_kernel<<<Gs, Bs, Ms>>>
    ...
    while( Gs.x > 1 ) {
        Gs_next.x = ( Gs.x+Bs.x-1 )/ Bs.x ;
        if ( Gs_next.x == 1 ) Gs_next.x = 2;
        magma_tally4_dreduce_kernel_spmv2<<< Gs_next.x/2, Bs.x/2, Ms/2 >>> 
                    ( Gs.x, n, aux1, aux2 );
        Gs_next.x = Gs_next.x /2;
        Gs.x = Gs_next.x;
        b = 1 - b;
    ...
    magma_tally4_dcopyvector( 1, aux1, 1, skp+6, 1 );
    magma_tally4_dcopyvector( 1, aux1+n, 1, skp+7, 1 );
    dim3 Bs2( 2 );
    dim3 Gs2( 1 );
    magma_tally4_dbicgstab_omegakernel<<<Gs2, Bs2, 0>>>( skp );
...
__global__ void
magma_tally4_dbicgmerge_xrbeta_kernel(  
    int n, 
    double * rr,
    double * r,
    double * p,
...
__global__ void
magma_tally4_dbicgstab_betakernel(  
    double * skp )
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    magma_tally4_queue_t queue )
{
    ...
    magma_tally4Double_ptr aux1 = d1, aux2 = d2;
    int b = 1;        
    magma_tally4_dbicgmerge_xrbeta_kernel<<<Gs, Bs, Ms>>>
    ...
    magma_tally4_dcopyvector( 1, aux1+n, 1, skp+5, 1 );
    dim3 Bs2( 2 );
    dim3 Gs2( 1 );
    magma_tally4_dbicgstab_betakernel<<<Gs2, Bs2, 0>>>( skp );
