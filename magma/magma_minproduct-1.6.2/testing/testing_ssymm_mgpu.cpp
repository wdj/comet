...
/* ////////////////////////////////////////////////////////////////////////////
   -- Testing magma_minproduct_ssymm_mgpu
*/
int main( int argc, char** argv)
{
    ...
    float calpha    = MAGMA_minproduct_S_MAKE( 3.456, 5.678 );
    float cbeta     = MAGMA_minproduct_S_MAKE( 1.234, 2.456 );
    ...
    float *hA, *hX, *hB, *hR;
    magma_minproductFloat_ptr dA[Magma_minproductMaxGPUs], dX[Magma_minproductMaxGPUs], dB[Magma_minproductMaxGPUs], dwork[Magma_minproductMaxGPUs], hwork[Magma_minproductMaxGPUs+1];
    magma_minproductFloat_ptr dA2;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int offset = 0; offset < N; offset += min(N,nb) ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            ldda  = ((M + 31)/32)*32;
            size  = lda*M;
            gflops = FLOPS_SSYMM( Magma_minproductLeft, (float)msize, (float)N ) / 1e9;
            ...
            magma_minproduct_smake_symmetric( M, hA, lda );
            ...
            /* ====================================================================
               ...
               =================================================================== */
            magma_minproduct_ssetmatrix_1D_col_bcyclic( M, M, hA, lda, dA, ldda, opts.ngpu, nb );
            for( int d = 0; d < opts.ngpu; ++d ) {
                magma_minproduct_setdevice( d );
                //magma_minproductblasSetKernelStream( streams[ d ][  0 ] );
                magma_minproduct_ssetmatrix( M, N, hX, lda, dX[d], ldda );
                //if (d == 0) magma_minproduct_ssetmatrix( M, N, hB, lda, dB[d], ldda ); // this is wrong coz when offset != 0 the gpu who do the beta*C may be not 0 so this should be related to stdev(starting device who own i=0 first col)
                magma_minproduct_ssetmatrix( M, N, hB, lda, dB[d], ldda );
            }
            ...
            if ( opts.check && iter == 0 ) {
                ...
                magma_minproduct_ssetmatrix( M, M, hA, lda, dA2, ldda );
                magma_minproduct_ssetmatrix( M, N, hX, lda, dX[0], ldda );
                magma_minproduct_ssetmatrix( M, N, hB, lda, dwork[0], ldda );
            ...
            if ( opts.check ) {
                ...
                //printf( "A =" ); magma_minproduct_sprint( M, M, hA, lda );
                //printf( "X =" ); magma_minproduct_sprint( M, N, hX, lda );
                //printf( "B =" ); magma_minproduct_sprint( M, N, hB, lda );
                ...
                */
                ...
                for(magma_minproduct_int_t dev=0; dev < opts.ngpu; ++dev) {
                    ...
                    magma_minproduct_sgetmatrix( M, N, dB[dev], ldda, hR, lda );
                    ...
                    //printf( "R ="  ); magma_minproduct_sprint( M, N, hR, lda );
                    if (firstprint == 0) {
