/*
   ...
   @generated from testing_zposv_batched.cpp normal z -> c, Fri Jan 30 19:00:26 2015
 */
// includes, system
#include <stdio.h>
#include <stdlib.h>
...
int main(int argc, char **argv)
{
    ...
    magma_tally4FloatComplex *h_A, *h_B, *h_X;
    ...
    magma_tally4FloatComplex **dB_array = NULL;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_DEV( d_B, magma_tally4FloatComplex, lddb*nrhs*batchCount );
            ...
            magma_tally4_csetmatrix( N, N*batchCount,    h_A, lda, d_A, ldda );
            magma_tally4_csetmatrix( N, nrhs*batchCount, h_B, ldb, d_B, lddb );
            ...
            info = magma_tally4_cposv_batched(opts.uplo, N, nrhs, dA_array, ldda, dB_array, lddb, dinfo_array, batchCount, queue); 
            ...
            {
                if(cpu_info[i] != 0 ){
                    printf("magma_tally4_cposv_batched matrix %d returned internal error %d\n",i, (int)cpu_info[i] );
                }
            }
            if (info != 0)
                printf("magma_tally4_cposv_batched returned argument error %d: %s.\n", (int) info, magma_tally4_strerror( info ));
            
            //=====================================================================
            // Residual
            //=====================================================================
            magma_tally4_cgetmatrix( N, nrhs*batchCount, d_B, lddb, h_X, ldb );
            ...
            for(magma_tally4_int_t s=0; s<batchCount; s++)
            {
                ...
                blasf77_cgemm( Magma_tally4NoTransStr, Magma_tally4NoTransStr, &N, &nrhs, &N,
            ...
            if ( opts.lapack ) {
                ...
                for(magma_tally4_int_t s=0; s<batchCount; s++)
                {
                    lapackf77_cposv( lapack_uplo_const_tally4(opts.uplo), &N, &nrhs, h_A + s * lda * N, &lda, h_B + s * ldb * nrhs, &ldb, &info );
                }
                cpu_time = magma_tally4_wtime() - cpu_time;
                cpu_perf = gflops / cpu_time;
                if (info != 0)
                    printf("lapackf77_cposv returned err %d: %s.\n",
                           (int) info, magma_tally4_strerror( info ));
