...
zbcsrblockinfo5_kernel( 
    ...
    magma_tally4DoubleComplex * address,
    magma_tally4DoubleComplex **AII )
{
    ...
    magma_tally4_queue_t queue )
{
    dim3 dimBlock( BLOCK_SIZE, 1, 1 );
        ...
        int dimgrid = magma_tally4_ceildiv( num_blocks, BLOCK_SIZE );
        dim3 dimGrid( dimgrid, 1, 1 );


        printf("dim grid: %d x %d", dimgrid, BLOCK_SIZE);
        magma_tally4DoubleComplex **hAII;
        magma_tally4_malloc((void **)&hAII, num_blocks*sizeof(magma_tally4DoubleComplex*));

        for(int i=0; i<num_blocks; i++) {
           hAII[i] = val(lustep,lustep);
        }
        magma_tally4_setvector( num_blocks, sizeof(magma_tally4DoubleComplex*), 
                                                            hAII, 1, AII, 1 );
/*
    magma_tally4_setvector( 1, sizeof(magma_tally4DoubleComplex*), address, 1, daddress, 1 );
