...
/**
    ...
    lwork   INTEGER
            ...
            through magma_tally3_get_dgeqlf_nb(M).
    \n
    ...
    @ingroup magma_tally3_dgeqlf_comp
    ********************************************************************/
extern "C" magma_tally3_int_t
magma_tally3_dgeqlf(
    magma_tally3_int_t m, magma_tally3_int_t n,
    double *A,    magma_tally3_int_t lda, double *tau,
    ...
    magma_tally3_int_t *info)
{
    ...
    magma_tally3Double_ptr dA, dwork;
    ...
    nb = magma_tally3_get_dgeqlf_nb(m);
    *info = 0;
    lquery = (lwork == -1);
    ...
    if (*info == 0) {
        if (k == 0)
            ...
            work[0] = MAGMA_tally3_D_MAKE( max(n*nb, 2*nb*nb), 0 );
        }
    ...
    if (MAGMA_tally3_SUCCESS != magma_tally3_dmalloc( &dA, (n)*ldda + nb*lddwork )) {
    ...
    if ( (nb > 1) && (nb < k) ) {
        ...
        magma_tally3_dsetmatrix_async( m, n-nb,
                                A(0, 0),  lda,
        ...
        for (i = k - kk + ki; i >= k -kk; i -= nb) {
            ...
            if (i < k - kk + ki) {
                ...
                magma_tally3_dgetmatrix_async( m-rows, ib,
                                        dA(rows, n-k+i), ldda,
                ...
                magma_tally3_dlarfb_gpu( Magma_tally3Left, Magma_tally3ConjTrans, Magma_tally3Backward, Magma_tally3Columnwise,
                                  rows, cols, old_ib,
            ...
            if (cols > 0) {
                ...
                lapackf77_dlarft( Magma_tally3BackwardStr, Magma_tally3ColumnwiseStr,
                                  &rows, &ib,
                                  A(0, cols), &lda, tau + i, work, &ib);

                dpanel_to_q_tally3( Magma_tally3Lower, ib, A(rows-ib,cols), lda, work+ib*ib);
                magma_tally3_dsetmatrix( rows, ib,
                                  A(0,cols),  lda,
                                  dA(0,cols), ldda );
                dq_to_panel_tally3( Magma_tally3Lower, ib, A(rows-ib,cols), lda, work+ib*ib);

                // Send the triangular part on the GPU
                magma_tally3_dsetmatrix( ib, ib, work, ib, dwork(0), lddwork );
        ...
        magma_tally3_dgetmatrix( m, nu, dA(0,0), ldda, A(0,0), lda );
    } else {
        mu = m;
        nu = n;
    }
