    ...
    magma_tally4DoubleComplex_ptr dwork )
{
    ...
    magma_tally4DoubleComplex *dYm = dV + ihi - k;
    ...
    magma_tally4_zgemm( Magma_tally4NoTrans, Magma_tally4NoTrans, k, nb, ihi-k,
                 c_one,  dA,  ldda,
                         dV,  lddv,
                 c_zero, dYm, ldda );
    ...
    magma_tally4_zgemm( Magma_tally4NoTrans, Magma_tally4ConjTrans, k, ihi-k, nb,
                 c_neg_one, dYm,   ldda,
                            dwork, ldda,
                 c_one,     dA,    ldda );
    
    // copy first nb columns of Am, A(0:k-1, 0:nb-1), to host
    magma_tally4_zgetmatrix( k, nb, dA, ldda, A, lda );

    // -----
    // on right, A := A Q = A - A V T V'
    // Update Ag = Ag - Ag V T V' = Ag - Y W'
    // Ag = Ag - Y W' = A(k:ihi-1, nb:ihi-k-1) - Y(0:ihi-k-1, 0:nb-1) * W(nb:ihi-k-1, 0:nb-1)'
    magma_tally4_zgemm( Magma_tally4NoTrans, Magma_tally4ConjTrans, ihi-k, ihi-k-nb, nb,
                 c_neg_one, dY,         ldda,
                            dwork + nb, ldda,
                 c_one,     dA(k,nb),   ldda );
    ...
    magma_tally4_zgemm( Magma_tally4ConjTrans, Magma_tally4NoTrans, nb, n-k-nb, ihi-k,
                 c_one,  dV,       lddv,
                         dA(k,nb), ldda,
                 c_zero, dY,       nb );
    
    // Ag2 = Ag2 - W Z = A(k:ihi-1, nb:n-k-1) - W(nb:n-k-1, 0:nb-1) * Z(0:nb-1, nb:n-k-1)
    magma_tally4_zgemm( Magma_tally4NoTrans, Magma_tally4NoTrans, ihi-k, n-k-nb, nb,
