...
int main( int argc, char** argv )
{
    ...
    float          magma_tally2_error, norm_invA, work[1];
    ...
    magma_tally2FloatComplex *h_A, *h_dinvA;
    magma_tally2FloatComplex_ptr d_A, d_dinvA;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_DEV( d_dinvA, magma_tally2FloatComplex, size_inv );
            TESTING_MALLOC_CPU( h_dinvA, magma_tally2FloatComplex, size_inv );
            ...
            magma_tally2blas_ctrtri_diag( opts.uplo, opts.diag, N, d_A, ldda, d_dinvA );
            magma_tally2_time = magma_tally2_sync_wtime( NULL ) - magma_tally2_time;
            magma_tally2_perf = gflops / magma_tally2_time;
            
            magma_tally2_cgetvector( size_inv, d_dinvA, 1, h_dinvA, 1 );
            
            if ( opts.verbose ) {
                printf( "A%d=", (int) N );
                magma_tally2_cprint( N, N, h_A, lda );
                printf( "d_dinvA%d=", (int) N );
                magma_tally2_cprint( min(N+4, nb), min(N+4, nblock*nb), h_dinvA, nb );
            }
            ...
            if ( opts.check ) {
                ...
                for( int i=0; i < N; i += nb ) {
                    ...
                    magma_tally2_error = max( magma_tally2_error, lapackf77_clantr( "M", uplo_, Magma_tally2NonUnitStr, &jb, &jb, h_dinvA(0, i), &nb,  work ));
                    norm_invA   = max( norm_invA,   lapackf77_clantr( "M", uplo_, Magma_tally2NonUnitStr, &jb, &jb, h_A(i, i),     &lda, work ));
                }
