/*
    -- MAGMA_tally4 (version 1.1) --
       ...
       @generated from magma_tally4_zmconverter.cpp normal z -> d, Tue May  5 14:02:10 2015
       @author Hartwig Anzt
*/
    ...
    magma_tally4_queue_t queue )
{
    ...
    for( i=0; i<*n; i++ ) {
        ...
        for( j=(*row)[i]; j<(*row)[i+1]; j++ ) {
            if ( MAGMA_tally4_D_REAL((*val)[j]) != 0 ) {
                nnz_new++;
                nnz_this_row++;
            }
        }
    ...
    CHECK( magma_tally4_dmalloc_cpu( valn, nnz_new ));
    ...
    magma_tally4_queue_t queue )
{
    ...
    magma_tally4_d_matrix hA={Magma_tally4_CSR}, hB={Magma_tally4_CSR};
    magma_tally4_d_matrix A_d={Magma_tally4_CSR}, B_d={Magma_tally4_CSR};
    ...
    if ( A.memory_location == Magma_tally4_CPU ) {
        ...
        if ( old_format == Magma_tally4_CSR ) {
            ...
            if ( new_format == Magma_tally4_CSR ) {
                ...
                CHECK( magma_tally4_dmalloc_cpu( &B->val, A.nnz ));
            ...
            // CSR to CSRL
                ...
                B->nnz = numzeros;
                CHECK( magma_tally4_dmalloc_cpu( &B->val, numzeros ));
                CHECK( magma_tally4_index_malloc_cpu( &B->row, A.num_rows+1 ));
                ...
                for( magma_tally4_int_t i=0; i < A.num_rows; i++) {
                    ...
                    for( magma_tally4_int_t j=A.row[i]; j < A.row[i+1]; j++) {
                        if ( A.col[j] < i) {
                            ...
                            B->val[numzeros] = MAGMA_tally4_D_MAKE(1.0, 0.0);
                            B->col[numzeros] = A.col[j];
                            numzeros++;
                        }
                        else if ( A.col[j] == i ) {
            ...
            // CSR to ELLPACKT (using row-major storage)
                ...
                //printf( "Conversion to ELLPACK with %d elements per row: ",
                                                                // maxrowlength );
                //fflush(stdout);
                CHECK( magma_tally4_dmalloc_cpu( &B->val, maxrowlength*A.num_rows ));
            ...
            // CSR to ELLRT (also ELLPACKRT)
                ...
                CHECK( magma_tally4_dmalloc_cpu( &B->val, rowlength*A.num_rows ));
            ...
            // so the rowlength is padded (SELLP) to a multiple of the alignment
                ...
                //fflush(stdout);
                CHECK( magma_tally4_dmalloc_cpu( &B->val, B->row[slices] ));
            ...
            // CSR to DENSE
                ...
                // conversion
                CHECK( magma_tally4_dmalloc_cpu( &B->val, A.num_rows*A.num_cols ));
            ...
            // CSR to BCSR
                ...
                CHECK( magma_tally4_dmconvert(A_d, &B_d, Magma_tally4_CSR, Magma_tally4_BCSR, queue ) );            
        ...
        // anything to CSR
            ...
            // CSRCSC to CSR
              ...
              //  magma_tally4_dmconvert( A, B, Magma_tally4_CSR, Magma_tally4_CSR, queue );
            }
            ...
            // ELL/ELLPACK to CSR
                ...
                CHECK( magma_tally4_d_csr_compressor(&A.val, &row_tmp, &A.col,
            ...
            // ELL (column-major) to CSR
                ...
                // conversion
                CHECK( magma_tally4_dmalloc_cpu( &val_tmp, A.num_rows*A.max_nnz_row ));
                CHECK( magma_tally4_index_malloc_cpu( &row_tmp, A.num_rows+1 ));
                ...
                //now use AA_ELL, IA_ELL, row_tmp as CSR with some zeros.
                //The CSR compressor removes these
                CHECK( magma_tally4_d_csr_compressor(&val_tmp, &row_tmp, &col_tmp,
                           &B->val, &B->row, &B->col, &B->num_rows, queue ));
            ...
            // ELLD (ELLPACK with diagonal element first) to CSR
                ...
                CHECK( magma_tally4_dmalloc_cpu( &val_tmp2, A.num_rows*A.max_nnz_row ));
                ...
                //now use AA_ELL, IA_ELL, row_tmp as CSR with some zeros.
                //The CSR compressor removes these
                CHECK( magma_tally4_d_csr_compressor(&val_tmp2, &row_tmp, &col_tmp2,
                           &B->val, &B->row, &B->col, &B->num_rows, queue ));
                B->nnz = B->row[B->num_rows];
            }
            ...
            // SELLP to CSR
                ...
                // conversion
                CHECK( magma_tally4_dmalloc_cpu( &val_tmp, A.max_nnz_row*(A.num_rows+C) ));
                ...
                for(magma_tally4_int_t i=0; i < A.max_nnz_row*(A.num_rows+C); i++ ) {
                    val_tmp[ i ] = MAGMA_tally4_D_MAKE(0., 0.);
                    col_tmp[ i ] =  0;
                }
            ...
            // DENSE to CSR
                ...
                B->nnz=0;
                for( magma_tally4_int_t i=0; i<(A.num_rows)*(A.num_cols); i++ ) {
                    if ( MAGMA_tally4_D_REAL(A.val[i]) != 0.0 )
                        (B->nnz)++;
                }
                CHECK( magma_tally4_dmalloc_cpu( &B->val, B->nnz));
                ...
                for(i=0; i<(A.num_rows)*(A.num_cols); i++)
                {
                    ...
                    if ( MAGMA_tally4_D_REAL(A.val[i]) != 0 )
                    {
                        (B->val)[j] = A.val[i];
            ...
            // BCSR to CSR
                ...
                CHECK( magma_tally4_dmtransfer(A, &A_d, Magma_tally4_CPU, Magma_tally4_DEV, queue ) );
                CHECK( magma_tally4_dmconvert(A_d, &B_d, Magma_tally4_BCSR, Magma_tally4_CSR, queue ) );
                CHECK( magma_tally4_dmtransfer(B_d, B, Magma_tally4_DEV, Magma_tally4_CPU, queue ) );
            }
        ...
        if ( old_format == Magma_tally4_CSR && new_format == Magma_tally4_CSR ) {
            CHECK( magma_tally4_dmtransfer( A, B, Magma_tally4_DEV, Magma_tally4_DEV, queue ));
        }
        // CSR to DENSE
        if ( old_format == Magma_tally4_CSR && new_format == Magma_tally4_DENSE ) {
            ...
            CHECK( magma_tally4_dmalloc( &B->dval, A.num_rows*A.num_cols ));
        ...
        // DENSE to CSR
            ...
            //magma_tally4_dprint_gpu( A.num_rows, 1, nnz_per_row, A.num_rows )
            cusparseDnnz( cusparseHandle, CUSPARSE_DIRECTION_COLUMN,
                          A.num_rows, A.num_cols,
                          descr,
                          A.dval, A.num_rows, nnz_per_row, &intnnz );

            CHECK( magma_tally4_dmalloc( &B->dval, B->nnz ));
        ...
        // CSR to BCSR
            ...
            CHECK( magma_tally4_dmalloc( &B->dval, nnzb*size_b*size_b ));
        ...
        // CSR to COO
            ...
            magma_tally4_dcopyvector( A.nnz, A.dval, 1, B->dval, 1 );
        ...
        // COO to CSR
            ...
            magma_tally4_dcopyvector( A.nnz, A.val, 1, B->val, 1 );
            ...
            CHECK( magma_tally4_dmconvert( hA, &hB, old_format, new_format, queue ));
            CHECK( magma_tally4_dmtransfer( hB, B, Magma_tally4_CPU, A.memory_location, queue ));
        }
    }
