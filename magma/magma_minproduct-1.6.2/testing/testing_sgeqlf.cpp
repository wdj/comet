/*
    -- MAGMA_minproduct (version 1.6.1) --
       ...
       @generated from testing_zgeqlf.cpp normal z -> s, Fri Jan 30 19:00:25 2015
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_minproduct_sgeqlf( M, N, h_R, lda, tau, h_work, lwork, &info);
            gpu_time = magma_minproduct_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_minproduct_sgeqlf returned error %d: %s.\n",
