...
const magma_minproductDoubleComplex MAGMA_minproduct_Z_NAN
    = MAGMA_minproduct_Z_MAKE( std::numeric_limits<double>::quiet_NaN(),
                    std::numeric_limits<double>::quiet_NaN() );

const magma_minproductDoubleComplex MAGMA_minproduct_Z_INF
    = MAGMA_minproduct_Z_MAKE( std::numeric_limits<double>::infinity(),
                    std::numeric_limits<double>::infinity() );

/** @return true if either real(x) or imag(x) is NAN. */
inline bool magma_minproduct_z_isnan( magma_minproductDoubleComplex x )
{
#ifdef COMPLEX
    return isnan( MAGMA_minproduct_Z_REAL( x )) ||
           isnan( MAGMA_minproduct_Z_IMAG( x ));
#else
    return isnan( x );
#endif
}
...
inline bool magma_minproduct_z_isinf( magma_minproductDoubleComplex x )
{
#ifdef COMPLEX
    return isinf( MAGMA_minproduct_Z_REAL( x )) ||
           isinf( MAGMA_minproduct_Z_IMAG( x ));
#else
    return isinf( x );
#endif
}
...
/**
    ...
    magma_minproduct_znan_inf checks a matrix that is located on the CPU host
    ...
    ********************************************************************/
extern "C"
magma_minproduct_int_t magma_minproduct_znan_inf(
    ...
    if ( uplo == Magma_minproductLower ) {
        for( int j = 0; j < n; ++j ) {
            for( int i = j; i < m; ++i ) {  // i >= j
                if      ( magma_minproduct_z_isnan( *A(i,j) )) { c_nan++; }
                else if ( magma_minproduct_z_isinf( *A(i,j) )) { c_inf++; }
            }
        }
    }
...
/**
    ...
    ********************************************************************/
extern "C"
magma_minproduct_int_t magma_minproduct_znan_inf_gpu(
    magma_minproduct_uplo_t uplo, magma_minproduct_int_t m, magma_minproduct_int_t n,
    ...
    magma_minproduct_int_t *cnt_inf )
{
    ...
    magma_minproduct_int_t cnt = magma_minproduct_znan_inf( uplo, m, n, A, lda, cnt_nan, cnt_inf );
