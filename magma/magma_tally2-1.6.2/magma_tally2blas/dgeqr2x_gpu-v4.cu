    ...
    magma_tally2_int_t *info)
{
    ...
    *info = 0;
    ...
    k = min(m,n);
    magma_tally2blas_dnrm2_cols(m, k, dA(0,0), ldda, dnorm);
    ...
    for (magma_tally2_int_t b=0; b < k; b += BS) {
        for (i = b; i < min(k, b+BS); ++i) {
            ...
            if (i-b > 0) {
                ...
                if ( i-1 > 0 ) {
                    magma_tally2_dgemv_kernel3<<< i-1, BLOCK_SIZE, 0, magma_tally2_stream >>>
                        ( m-i+1, dA(i-1,0), ldda, dA(i-1, i-1), work, dtau+i-1);
                    magma_tally2_dtrmv_kernel2<<< i-1, i-1, 0, magma_tally2_stream >>>
                        ( dT(0,0), k, work, dT(0,i-1), dtau+i-1);
                }

                /* dwork = V**H c */
                magma_tally2_dgemv_kernel1<<< i-b, BLOCK_SIZE, 0, magma_tally2_stream >>>
                    (m-b, dA(b, b),  ldda, dA(b,i), work);

                /* dwork = T**H work */
                magma_tally2_dtrmv_tkernel<<< i-b, i-b, 0, magma_tally2_stream >>>
                ...
                if ( m-b > 0 ) {
                    ...
                    magma_tally2_dgemv_kernel2<<< blocks3, threads3, 0, magma_tally2_stream >>>
            ...
            if ( i > 0 ) {
                magma_tally2_dnrm2_adjust_kernel<<< 1, i, 0, magma_tally2_stream >>>(dnorm+i, dA(0, i));
            }
            ...
            /*  Generate elementary reflector H(i) to annihilate A(i+1:m,i)
                ...
                3. update T */
            magma_tally2_dlarfgx_gpu(m-i, dA(i, i), dA(min(i+1,m),i), dtau+i,
                              dnorm+i, ddA + i + i*n, i);

            if (i == 0) {
                double tt = MAGMA_tally2_D_ONE;
                magma_tally2blas_dlacpy(Magma_tally2UpperLower, 1, 1, dtau, 1, dT(0,0), 1);
                magma_tally2_dsetmatrix_async(1, 1, &tt, 1, dA(i, i), 1, magma_tally2_stream);
            }
        }
        if ( i-1 > 0 ) {
            magma_tally2_dgemv_kernel3<<< i-1, BLOCK_SIZE, 0, magma_tally2_stream >>>
        ...
        //magma_tally2_dlarfb2_gpu( Magma_tally2Left, Magma_tally2ConjTrans, Magma_tally2Forward, Magma_tally2Columnwise,
