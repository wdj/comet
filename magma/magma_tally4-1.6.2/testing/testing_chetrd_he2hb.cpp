...
extern "C" void ccheck_eig_(char *JOBZ, int  *MATYPE, int  *N, int  *NB,
                       magma_tally4FloatComplex* A, int  *LDA, float *AD, float *AE, float *D1, float *EIG,
                    magma_tally4FloatComplex *Z, int  *LDZ, magma_tally4FloatComplex *WORK, float *RWORK, float *RESU);
#endif
...
int main( int argc, char** argv)
{
    ...
    magma_tally4FloatComplex *h_A, *h_R, *h_work, *dT1;
    magma_tally4FloatComplex *tau;
    float *D, *E;

    /* Matrix size */
    magma_tally4_int_t N, n2, lda, lwork, lwork0;  //ldt
...
#if defined(CHECKEIG)
#if defined(PRECISION_z)  || defined(PRECISION_d)
    magma_tally4_int_t WANTZ=0;
    magma_tally4_int_t THREADS=1;
#endif
#endif

    magma_tally4_int_t NE = 0;
    magma_tally4_int_t NB = 0;
    magma_tally4_int_t ngpu = 1;
    ...
    NB = opts.nb;
    if (NB < 1)
        NB  = 64; //64; //magma_tally4_get_chetrd_he2hb_nb(N);

    // what is NE ?
    if (NE < 1)
        NE  = 64; //N;  //magma_tally4_get_chetrd_he2hb_nb(N);  // N not yet initialized
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            /* We suppose the magma_tally4 NB is bigger than lapack NB */
            lwork0 = N*NB;
        
            /* Allocate host memory for the matrix */
            TESTING_MALLOC_CPU( h_A,    magma_tally4FloatComplex, lda*N  );
            TESTING_MALLOC_CPU( tau,    magma_tally4FloatComplex, N-1    );
            
            TESTING_MALLOC_PIN( h_R,    magma_tally4FloatComplex, lda*N  );
            TESTING_MALLOC_PIN( h_work, magma_tally4FloatComplex, lwork0 );
            TESTING_MALLOC_PIN( D, float, N );
            TESTING_MALLOC_PIN( E, float, N );
            
            //TESTING_MALLOC_DEV( dT1, magma_tally4FloatComplex, (2*min(N,N)+(N+31)/32*32)*NB );
            TESTING_MALLOC_DEV( dT1, magma_tally4FloatComplex, (N*NB) );
            ...
            magma_tally4_cmake_hermitian( N, h_A, lda );
            ...
            /*
            magma_tally4_chetrd_he2hb( opts.uplo, N, NB, h_R, lda, tau, h_work, lwork0, dT1, THREADS, &info);
            tband = magma_tally4_wtime - gpu_time();
            printf("  Finish BAND  N %d  NB %d  ngpu %d timing= %f\n", N, NB, ngpu, tband);
            magma_tally4_chetrd_bhe2trc_v5(THREADS, WANTZ, opts.uplo, NE, N, NB, h_R, lda, D, E, dT1, ldt);
            */
    
            /*
            magma_tally4_chetrd_he2hb( opts.uplo, N, NB, h_R, lda, tau, h_work, lwork, dT1, THREADS, &info);
            tband = magma_tally4_wtime - gpu_time();
            printf("  Finish BAND  N %d  NB %d  ngpu %d timing= %f\n", N, NB, ngpu, tband);
            magma_tally4_chetrd_bhe2trc(THREADS, WANTZ, opts.uplo, NE, N, NB, h_R, lda, D, E, dT1, ldt);
            */

            magma_tally4_range_t range = Magma_tally4RangeAll;
            magma_tally4_int_t fraction_ev = 100;
            magma_tally4_int_t il, iu, m1;
            float vl=0., vu=0.;
            ...
            magma_tally4FloatComplex *hh_work;
            magma_tally4_int_t *iwork;
            magma_tally4_int_t /*nb,*/ /*lwork,*/ liwork;
            magma_tally4_int_t threads = magma_tally4_get_parallel_numthreads();
            #if defined(PRECISION_z) || defined(PRECISION_c)
                float *rwork;
                magma_tally4_int_t lrwork;
                lwork  = magma_tally4_cbulge_get_lq2(N, threads) + 2*N + N*N;
                lrwork = 1 + 5*N +2*N*N;
                TESTING_MALLOC_PIN( rwork, float, lrwork );
            #else
                lwork  = magma_tally4_cbulge_get_lq2(N, threads) + 1 + 6*N + 2*N*N;
            #endif
            liwork = 3 + 5*N;
            //nb = magma_tally4_get_chetrd_nb(N);
            TESTING_MALLOC_PIN( hh_work, magma_tally4FloatComplex, lwork  );
            TESTING_MALLOC_CPU( iwork,   magma_tally4_int_t,        liwork );
    
            if (ngpu == 1) {
                printf("calling cheevdx_2stage 1 GPU\n");
                magma_tally4_cheevdx_2stage( opts.jobz, range, opts.uplo, N,
                                h_R, lda,
                                vl, vu, il, iu,
                                &m1, D,
                                hh_work, lwork,
                ...
                magma_tally4_cheevdx_2stage_m(ngpu, opts.jobz, range, opts.uplo, N,
                                h_R, lda,
                                vl, vu, il, iu,
                                &m1, D,
                                hh_work, lwork,
            ...
            */
            ...
            if ( opts.check ) {
                ...
                int    lwork2 = 256*N;
                magma_tally4FloatComplex *work2, *AINIT;
                float *rwork2, *D2;
                // TODO free this memory !
                magma_tally4_cmalloc_cpu( &work2, lwork2 );
                magma_tally4_smalloc_cpu( &rwork2, N );
                magma_tally4_smalloc_cpu( &D2, N );
                magma_tally4_cmalloc_cpu( &AINIT, N*lda );
                memcpy(AINIT, h_A, N*lda*sizeof(magma_tally4FloatComplex));
                ...
                */
                ...
                //magma_tally4FloatComplex mydz=0.0, mydo=1.0;
                //magma_tally4FloatComplex *Z;
                // magma_tally4_cmalloc_cpu( &Z, N*lda );
                ...
                magma_tally4FloatComplex *WORKAJETER;
                float *RWORKAJETER, *RESU;
                // TODO free this memory !
                magma_tally4_cmalloc_cpu( &WORKAJETER, (2* N * N + N)  );
                magma_tally4_smalloc_cpu( &RWORKAJETER, N  );
                magma_tally4_smalloc_cpu( &RESU, 10 );
                int MATYPE;
