...
int main( int argc, char** argv )
{
    ...
    magma_minproductFloatComplex *hA, *hR, *hT, *tau, *h_work;
    magma_minproductFloatComplex_ptr dA, dT;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            // okay that magma_minproduct_cgeqrf_gpu has special structure for R; R isn't used here.
            magma_minproduct_csetmatrix( m, n, hA, lda, dA, ldda );
            magma_minproduct_cgeqrf_gpu( m, n, dA, ldda, tau, dT, &info );
            if ( info != 0 )
                printf("magma_minproduct_cgeqrf_gpu returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
            magma_minproduct_cgetmatrix( m, n, dA, ldda, hA, lda );
            lapackf77_clacpy( Magma_minproductFullStr, &m, &n, hA, &lda, hR, &lda );
            magma_minproduct_cgetmatrix( nb, min_mn, dT, nb, hT, nb );
            
            gpu_time = magma_minproduct_wtime();
            magma_minproduct_cungqr_m( m, n, k, hR, lda, tau, hT, nb, &info );
            gpu_time = magma_minproduct_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if ( info != 0 )
                printf("magma_minproduct_cungqr_gpu returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
