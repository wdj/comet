/*
    -- MAGMA_tally2 (version 1.6.1) --
       ...
       @generated from zhetrf_nopiv_tally2_gpu.cpp normal z -> s, Fri Jan 30 19:00:17 2015
*/
    ...
    magma_tally2_int_t *info)
{
    ...
    *info = 0;
    ...
    // CPU workspace
    float *A;
    if (MAGMA_tally2_SUCCESS != magma_tally2_smalloc_pinned( &A, nb*nb )) {
    ...
    if (MAGMA_tally2_SUCCESS != magma_tally2_smalloc( &dW, (1+nb)*ldda )) {
    ...
    if (upper) {
        ...
        for (j=0; j<n; j += nb) {
            ...
            magma_tally2_sgetmatrix_async(jb, jb, dA(j, j), ldda, A(j,j), nb, stream[1]);
            trace_gpu_end( 0, 0 );

            // factorize the diagonal block
            magma_tally2_queue_sync(stream[1]);
            trace_cpu_start( 0, "potrf", "potrf" );
            ssytrf_nopiv_tally2_cpu(Magma_tally2Upper, jb, ib, A(j, j), nb, info);
            trace_cpu_end( 0 );
            if (*info != 0){
                *info = *info + j;
                break;
            ...
            magma_tally2_ssetmatrix_async(jb, jb, A(j, j), nb, dA(j, j), ldda, stream[0]);
            trace_gpu_end( 0, 0 );
        ...
        for (j=0; j<n; j+=nb) {
            ...
            magma_tally2_queue_sync(stream[1]);
            trace_cpu_start( 0, "potrf", "potrf" );
            ssytrf_nopiv_tally2_cpu(Magma_tally2Lower, jb, ib, A(j, j), nb, info);
            trace_cpu_end( 0 );
            if (*info != 0){
                *info = *info + j;
                break;
