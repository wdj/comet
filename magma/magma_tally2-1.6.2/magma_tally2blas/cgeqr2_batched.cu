...
clarfg_device(
    int n,
    magma_tally2FloatComplex* dalpha, magma_tally2FloatComplex* dx, int incx,
    magma_tally2FloatComplex* dtau,  float* swork, float* sscale, magma_tally2FloatComplex* scale)
{
    ...
    if ( tx == 0 ) {
        magma_tally2FloatComplex alpha = *dalpha;
        ...
        if ( swork[0] == 0 && imag(alpha) == 0 ) {
            ...
            *dtau   = MAGMA_tally2_C_MAKE( (beta - real(alpha)) / beta, -imag(alpha) / beta );
            *dalpha = MAGMA_tally2_C_MAKE( beta, 0 );
            *scale = 1 / (alpha - beta);
        }
    }
...
static __device__
void clarfx_device( int m, int n,  magma_tally2FloatComplex *v, magma_tally2FloatComplex *tau,
                         magma_tally2FloatComplex *dc, magma_tally2_int_t ldc, magma_tally2FloatComplex* sum)
{


    if(n <=0) return ;
    if (MAGMA_tally2_C_EQUAL(*tau, MAGMA_tally2_C_ZERO) )  return; // check singularity
    ...
    for(int k=0;k<n;k++)
    {
        ...
        if(tx<BLOCK_SIZE)
        {
            ...
            for( int j = tx+1; j < m; j += BLOCK_SIZE ){
                lsum += MAGMA_tally2_C_MUL( MAGMA_tally2_C_CNJG( v[j] ), dc[j+ldc*k] );
            }
...
__global__
void cgeqr2_sm_kernel_batched( int m, int n, magma_tally2FloatComplex** dA_array, magma_tally2_int_t lda,
                               magma_tally2FloatComplex **dtau_array)
{

    magma_tally2FloatComplex* dA = dA_array[blockIdx.z];
    magma_tally2FloatComplex* dtau = dtau_array[blockIdx.z];
...
static __device__
void cgeqr2_device( magma_tally2_int_t m, magma_tally2_int_t n, magma_tally2FloatComplex* dA, magma_tally2_int_t lda,
                               magma_tally2FloatComplex *dtau, 
                               magma_tally2FloatComplex *sdata,
...
__global__
void cgeqr2_kernel_batched( int m, int n, magma_tally2FloatComplex** dA_array, magma_tally2_int_t lda,
                               magma_tally2FloatComplex **dtau_array)
{
...
/**
    ...
    @param
    dwork   (workspace) COMPLEX array, dimension (N) * ( sizeof(float) + sizeof(magma_tally2FloatComplex)) 
...
magma_tally2_cgeqr2_batched(magma_tally2_int_t m, magma_tally2_int_t n, magma_tally2FloatComplex **dA_array,
                  magma_tally2_int_t lda, magma_tally2FloatComplex **dtau_array,
                  magma_tally2_int_t *info_array, magma_tally2_int_t batchCount, magma_tally2_queue_t queue)
{
    ...
    if(sizeof(magma_tally2FloatComplex)*(m*k) <= 128 /*sizeof(magma_tally2FloatComplex) * 128 * k*/) // there are some static shared memory besides of dynamic ones 
    {   
        ...
        cgeqr2_sm_kernel_batched<<< blocks, threads, sizeof(magma_tally2FloatComplex)*(m*k), queue >>>
    ...
    else
    {
        ...
        if(sizeof(magma_tally2FloatComplex)*(m) < 42000)
            cgeqr2_kernel_batched<<< blocks, threads, sizeof(magma_tally2FloatComplex)*(m), queue >>>
                                      (m, k, dA_array, lda, dtau_array);
        else
