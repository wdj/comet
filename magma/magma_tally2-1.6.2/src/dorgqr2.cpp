...
#define dA(i,j) (dA + (i) + (j)*ldda)
    ...
    *info = 0;
    ...
    ldda    = ((m + 31) / 32) * 32;
    lddwork = ((n + 31) / 32) * 32;
    if (MAGMA_tally2_SUCCESS != magma_tally2_dmalloc( &dA, ldda*n + ldda*nb + lddwork*nb + nb*nb)) {
    ...
    if (kk < n) {
        ...
        */
        lapackf77_dlacpy( Magma_tally2UpperLowerStr, &m_kk, &k_kk, A(kk,kk), &lda, V, &m_kk);
        lapackf77_dlaset( Magma_tally2UpperLowerStr, &m_kk, &n_kk, &c_zero, &c_one, A(kk, kk), &lda );

        lapackf77_dlarft( Magma_tally2ForwardStr, Magma_tally2ColumnwiseStr,
                          &m_kk, &k_kk,
                          V, &m_kk, &tau[kk], work, &k_kk);
        lapackf77_dlarfb( Magma_tally2LeftStr, Magma_tally2NoTransStr, Magma_tally2ForwardStr, Magma_tally2ColumnwiseStr,
                          &m_kk, &n_kk, &k_kk,
