...
int main(int argc, char **argv)
{
    ...
    float alpha = MAGMA_tally2_S_MAKE(  1.5, -2.3 );
    float beta  = MAGMA_tally2_S_MAKE( -0.6,  0.8 );
    float *A, *X, *Y, *Ylapack, *Ycublas, *Ymagma_tally2, *Ymagma_tally21, *hwork;
    magma_tally2Float_ptr dA, dX, dY;
    magma_tally2Float_ptr d_lA[Magma_tally2MaxGPUs], dwork[Magma_tally2MaxGPUs];
    ...
    magma_tally2_int_t nb = 64;  // required by magma_tally2blas_ssymv_mgpu implementation
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int ioffset=0; ioffset < noffsets; ioffset += 1 ) {
        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( Ymagma_tally2,  float, vecsize );
            TESTING_MALLOC_CPU( Ymagma_tally21, float, vecsize );
            ...
            // TODO make magma_tally2_smalloc_bcyclic helper function?
            for( dev=0; dev < opts.ngpu; dev++ ) {
            ...
            magma_tally2_smake_symmetric( Noffset, A, lda );
            ...
            /* =====================================================================
               ...
               =================================================================== */
            magma_tally2_setdevice( opts.device );
            magma_tally2_ssetmatrix( Noffset, Noffset, A, lda, dA, ldda );
            magma_tally2_ssetvector( Noffset, X, incx, dX, incx );
            magma_tally2_ssetvector( Noffset, Y, incx, dY, incx );
            
            cuda_time = magma_tally2_sync_wtime(0);
            cublasSsymv( opts.handle, cublas_uplo_const_tally2(opts.uplo), N,
            ...
            magma_tally2_sgetvector( Noffset, dY, incx, Ycublas, incx );
            ...
            magma_tally2blas_ssymv_work( opts.uplo, N,
                                  alpha, dA + offset + offset*ldda, ldda,
                                         dX + offset, incx,
            ...
            magma_tally2_sgetvector( Noffset, dY, incx, Ymagma_tally21, incx );
            
            /* =====================================================================
               Performs operation using MAGMA_tally2BLAS (multi-GPU)
               =================================================================== */
            magma_tally2_ssetmatrix_1D_col_bcyclic( Noffset, Noffset, A, lda, d_lA, ldda, opts.ngpu, nb );
            blasf77_scopy( &Noffset, Y, &incx, Ymagma_tally2, &incx );
            
            // workspaces do NOT need to be zero -- set to NAN to prove
            for( dev=0; dev < opts.ngpu; ++dev ) {
                magma_tally2_setdevice( dev );
                magma_tally2blas_slaset( Magma_tally2Full, ldwork, 1, MAGMA_tally2_S_NAN, MAGMA_tally2_S_NAN, dwork[dev], ldwork );
            }
            lapackf77_slaset( "Full", &lhwork, &ione, &MAGMA_tally2_S_NAN, &MAGMA_tally2_S_NAN, hwork, &lhwork );
            
            mgpu_time = magma_tally2_sync_wtime(0);
            
            magma_tally2_int_t info;
            info = magma_tally2blas_ssymv_mgpu(
                opts.uplo, N,
                alpha,
                d_lA, ldda, offset,
                X + offset, incx,
                ...
                opts.ngpu, nb, queues );
            if ( info != 0 )
                printf("magma_tally2blas_ssymv_mgpu returned error %d: %s.\n",
                       (int) info, magma_tally2_strerror( info ));
            
            info = magma_tally2blas_ssymv_mgpu_sync(
                opts.uplo, N,
                alpha,
                d_lA, ldda, offset,
                X + offset, incx,
                ...
                opts.ngpu, nb, queues );
            if ( info != 0 )
                printf("magma_tally2blas_ssymv_sync returned error %d: %s.\n",
                       (int) info, magma_tally2_strerror( info ));
            ...
            if ( opts.lapack ) {
                ...
                blasf77_ssymv( lapack_uplo_const_tally2(opts.uplo), &N,
                ...
                blasf77_saxpy( &Noffset, &c_neg_one, Ymagma_tally2, &incx, Ylapack, &incx );
            ...
            blasf77_saxpy( &Noffset, &c_neg_one, Ymagma_tally2, &incx, Ycublas, &incx );
