    ...
    magma_tally3_queue_t queue )
{
    ...
    CHECK( magma_tally3_cvinit( &rt,Magma_tally3_DEV, A.num_rows, b.num_cols, c_zero, queue ));
    ...
    CHECK( magma_tally3_cvinit( &h, Magma_tally3_DEV, A.num_rows, b.num_cols, c_zero, queue ));
    ...
    magma_tally3_ccopy( dofs, h.dval, 1, p.dval, 1 );                    // p = h
    nom = MAGMA_tally3_C_REAL( magma_tally3_cdotc(dofs, r.dval, 1, h.dval, 1) );
    ...
    do
    {
        ...
        gammanew = MAGMA_tally3_C_REAL( magma_tally3_cdotc(dofs, r.dval, 1, h.dval, 1) );
                                                            // gn = < r,h>

        if ( solver_par->numiter==1 ) {
            magma_tally3_ccopy( dofs, h.dval, 1, p.dval, 1 );                    // p = h
        } else {
            beta = MAGMA_tally3_C_MAKE(gammanew/gammaold, 0.);       // beta = gn/go
            magma_tally3_cscal(dofs, beta, p.dval, 1);            // p = beta*p
            magma_tally3_caxpy(dofs, c_one, h.dval, 1, p.dval, 1); // p = p + h
        }
