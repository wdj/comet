/*
    -- MAGMA_minproduct (version 1.6.1) --
       ...
       @generated from zgehrd.cpp normal z -> c, Fri Jan 30 19:00:18 2015
...
/**
    ...
    This version stores the T matrices in dT, for later use in magma_minproduct_cunghr.
    ...
    magma_minproduct_int_t *info)
{
    ...
    magma_minproduct_int_t nb = magma_minproduct_get_cgehrd_nb(n);
    magma_minproduct_int_t ldda = ((n+31)/32)*32;

    magma_minproduct_int_t i, nh, iws;
    magma_minproduct_int_t iinfo;
    magma_minproduct_int_t lquery;

    *info = 0;
    iws = n*nb;
    work[0] = MAGMA_minproduct_C_MAKE( iws, 0 );
    ...
    if (nb == 1 || nb > nh) {
        ...
        if (MAGMA_minproduct_SUCCESS != magma_minproduct_cmalloc( &dwork, 2*nb*ldda + n*ldda )) {
            *info = MAGMA_minproduct_ERR_DEVICE_ALLOC;
            return *info;
        }
        magma_minproductFloatComplex *dV = dwork + nb*ldda;
        magma_minproductFloatComplex *dA = dwork + nb*ldda*2;
        magma_minproductFloatComplex *dTi;
        
        magma_minproductFloatComplex *T;
        magma_minproduct_cmalloc_cpu( &T, nb*nb );
        if ( T == NULL ) {
        ...
        magma_minproductblas_claset( Magma_minproductFull, nb, nb, c_zero, c_zero, dV, ldda );
        ...
        magma_minproductblas_claset( Magma_minproductFull, nb, n, c_zero, c_zero, dT, nb );
        
        // Copy the matrix to the GPU
        magma_minproduct_csetmatrix( n, n-ilo, A(0,ilo), lda, dA, ldda );
        ...
        for (i = ilo; i < ihi-1 - nb; i += nb) {
            ...
            magma_minproduct_cgetmatrix( ihi-i, nb,
                              dA(i,i-ilo), ldda,
                              A(i,i), lda );
            
            // add 1 to i for 1-based index
            magma_minproduct_clahr2( ihi, i+1, nb,
                          dA(0,i-ilo), ldda,
                          dV,          ldda,
                          A(0,i),      lda,
                          &tau[i], T, nb, work, n);
            ...
            dTi = dT + (i - ilo)*nb;
            magma_minproduct_csetmatrix( nb, nb, T, nb, dTi, nb );
            
            magma_minproduct_clahru( n, ihi, i, nb,
                          A(0,i),      lda,
                          dA(0,i-ilo), ldda, // dA
        ...
        magma_minproduct_cgetmatrix( n, n-i,
                          dA(0,i-ilo), ldda,
                          A(0,i), lda );
