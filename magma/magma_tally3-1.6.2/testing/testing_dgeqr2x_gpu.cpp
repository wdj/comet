...
int main( int argc, char** argv)
{
    ...
    magma_tally3Double_ptr d_A,  d_T, ddA, dtau;
    magma_tally3Double_ptr d_A2, d_T2, ddA2, dtau2;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            // todo replace with magma_tally3_dlaset
            magma_tally3blas_dlaset( Magma_tally3Full, N, N, c_zero, c_zero, ddA,  N );
            magma_tally3blas_dlaset( Magma_tally3Full, N, N, c_zero, c_zero, d_T,  N );
            magma_tally3blas_dlaset( Magma_tally3Full, N, N, c_zero, c_zero, ddA2, N );
            magma_tally3blas_dlaset( Magma_tally3Full, N, N, c_zero, c_zero, d_T2, N );
        
            lwork = -1;
            lapackf77_dgeqrf(&M, &N, NULL, &M, NULL, tmp, &lwork, &info);
            lwork = (magma_tally3_int_t)MAGMA_tally3_D_REAL( tmp[0] );
            lwork = max(lwork, N*N);
            ...
            magma_tally3_dsetmatrix( M, N, h_R, lda, d_A2, ldda );
            ...
            if (opts.version == 1)
                magma_tally3_dgeqr2x_gpu(M, N, d_A, ldda, dtau, d_T, ddA, dwork, &info);
            else if (opts.version == 2)
                magma_tally3_dgeqr2x2_gpu(M, N, d_A, ldda, dtau, d_T, ddA, dwork, &info);
            else if (opts.version == 3)
                magma_tally3_dgeqr2x3_gpu(M, N, d_A, ldda, dtau, d_T, ddA, dwork, &info);
            else {
                printf( "call magma_tally3_dgeqr2x4_gpu\n" );
                /*
                ...
                */
                magma_tally3_dgeqr2x4_gpu(M, N, d_A, ldda, dtau, d_T, ddA, dwork, NULL, &info);
                //magma_tally3_dgeqr2x4_gpu(M, N, d_A, ldda, dtau, d_T, ddA, dwork, &info, stream[1]);
                //magma_tally3_dgeqr2x4_gpu(M, N, d_A2, ldda, dtau2, d_T2, ddA2, dwork2, &info, stream[0]);
                //magma_tally3_dgeqr2x4_gpu(M, N, d_A2, ldda, dtau2, d_T2, ddA2, dwork2, &info, NULL);
                //gflops *= 2;
            }
            ...
            if (info != 0) {
                printf("magma_tally3_dgeqr2x_gpu version %d returned error %d: %s.\n",
                ...
                if ( opts.check ) {
                    /* =====================================================================
                       ...
                       =================================================================== */
                    magma_tally3_dgetmatrix( M, N, d_A, ldda, h_R, M );
                    magma_tally3_dgetmatrix( N, N, ddA, N,    h_T, N );
                    magma_tally3_dgetmatrix( min_mn, 1, dtau, min_mn,   tau, min_mn );
                    ...
                    lapackf77_dlacpy( Magma_tally3UpperLowerStr, &M, &N, h_R, &M, h_A, &lda );
                    lapackf77_dlarft( Magma_tally3ForwardStr, Magma_tally3ColumnwiseStr,
                                      &M, &N, h_A, &lda, tau, h_work, &N);
                    //magma_tally3_dgeqr2(&M, &N, h_A, &lda, tau, h_work, &info);
                    ...
                    magma_tally3_dgetmatrix( N, N, d_T, N, h_T, N );
    
                    double terr = 0.;
                    for(int col=0; col < N; col++)
                        for(int row=0; row <= col; row++)
                            terr += (  MAGMA_tally3_D_ABS(h_work[row + col*N] - h_T[row + col*N])*
                                       MAGMA_tally3_D_ABS(h_work[row + col*N] - h_T[row + col*N])  );
                    terr = sqrt( terr );
