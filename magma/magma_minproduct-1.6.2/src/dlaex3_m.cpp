...
magma_minproduct_int_t magma_minproduct_get_dlaex3_m_k()  { return  512; }
magma_minproduct_int_t magma_minproduct_get_dlaex3_m_nb() { return 1024; }
...
#define dQ(id, ii) (dwork[id] + n2*n2_loc +    2*(n2*nb) + (ii)*(n2_loc*nb))
    ...
    if (ngpu == 1) {
        ...
        magma_minproduct_dlaex3(k, n, n1, d, Q, ldq, rho,
...
//#define CHECK_CPU
#ifdef CHECK_CPU
    double *hwS[2][Magma_minproductMaxGPUs], *hwQ[2][Magma_minproductMaxGPUs], *hwQ2[Magma_minproductMaxGPUs];
    ...
    nb = magma_minproduct_get_dlaex3_m_nb();

    if (n1 >= magma_minproduct_get_dlaex3_m_k()) {
#ifdef CHECK_CPU
        for (igpu = 0; igpu < ngpu; ++igpu) {
            magma_minproduct_dmalloc_pinned( &(hwS[0][igpu]), n2*nb );
            magma_minproduct_dmalloc_pinned( &(hwS[1][igpu]), n2*nb );
            magma_minproduct_dmalloc_pinned( &(hwQ2[igpu]), n2*n2_loc );
            magma_minproduct_dmalloc_pinned( &(hwQ[0][igpu]), n2_loc*nb );
            magma_minproduct_dmalloc_pinned( &(hwQ[1][igpu]), n2_loc*nb );
        }
#endif
        for (igpu = 0; igpu < ngpu-1; igpu += 2) {
            ...
            magma_minproduct_dsetmatrix_async( ni_loc[igpu], n12,
                                    Q2+n1_loc*(igpu/2), n1,
            ...
            magma_minproduct_dsetmatrix_async( ni_loc[igpu+1], n23,
...
#pragma omp single
            {
                ...
                if (valeig)
                    magma_minproduct_dvrange(k, d, &iil, &iiu, vl, vu);
                else if (indeig)
                    magma_minproduct_dirange(k, indxq, &iil, &iiu, il, iu);
                else {
                    iil = 1;
                    iiu = k;
                }
...
#pragma omp barrier
                ...
                for (j = ib; j < ie; ++j) {
                    for (i = 0; i < k; ++i)
                        s[id*k + i] = w[i] / *Q(i,j);
                    temp = magma_minproduct_cblas_dnrm2( k, s+id*k, 1 );
                    for (i = 0; i < k; ++i) {
    ...
    if (k == 2) {
        ...
        for (j = iil-1; j < iiu; ++j) {
            for (i = 0; i < k; ++i)
                s[i] = w[i] / *Q(i,j);
            temp = magma_minproduct_cblas_dnrm2( k, s, 1 );
            for (i = 0; i < k; ++i) {
                magma_minproduct_int_t iii = indx[i] - 1;
                *Q(i,j) = s[iii] / temp;
            }
    ...
    if (rk > 0) {
        if (n1 < magma_minproduct_get_dlaex3_m_k()) {
            ...
            for (igpu = 0; igpu < ngpu-1; igpu += 2) {
                if (n23 != 0) {
                    magma_minproduct_setdevice(igpu+1);
                    magma_minproduct_dsetmatrix_async( n23, ib,
                                            Q(ctot[0],iil-1), ldq,
                                            dS(igpu+1,0),     n23, queues[igpu+1][0] );
                }
                if (n12 != 0) {
                    magma_minproduct_setdevice(igpu);
                    magma_minproduct_dsetmatrix_async( n12, ib,
                                            Q(0,iil-1), ldq,
            ...
            for (i = 0; i < rk; i += nb) {
                ...
                if (i+nb < rk) {
                    ...
                    for (igpu = 0; igpu < ngpu-1; igpu += 2) {
                        if (n23 != 0) {
                            magma_minproduct_setdevice(igpu+1);
                            magma_minproduct_dsetmatrix_async( n23, ib2,
                                                    Q(ctot[0],iil-1+i+nb), ldq,
                                                    dS(igpu+1,(ind+1)%2),  n23, queues[igpu+1][(ind+1)%2] );
                        }
                        if (n12 != 0) {
                            magma_minproduct_setdevice(igpu);
                            magma_minproduct_dsetmatrix_async( n12, ib2,
                                                    Q(0,iil-1+i+nb),    ldq,
                ...
                for (igpu = 0; igpu < ngpu-1; igpu += 2) {
                    if (n23 != 0) {
                        ...
                        magma_minproduct_dgemm(Magma_minproductNoTrans, Magma_minproductNoTrans, ni_loc[igpu+1], ib, n23, d_one, dQ2(igpu+1), n2_loc,
                    ...
                    if (n12 != 0) {
                        ...
                        magma_minproduct_dgemm(Magma_minproductNoTrans, Magma_minproductNoTrans, ni_loc[igpu], ib, n12, d_one, dQ2(igpu), n1_loc,
                ...
                for (igpu = 0; igpu < ngpu-1; igpu += 2) {
                    if (n23 != 0) {
                        magma_minproduct_setdevice(igpu+1);
                        magma_minproduct_dgetmatrix( ni_loc[igpu+1], ib, dQ(igpu+1, ind), n2_loc,
                                          Q(n1+n2_loc*(igpu/2),iil-1+i), ldq );
//                        magma_minproduct_dgetmatrix_async( ni_loc[igpu+1], ib, dQ(igpu+1, ind), n2_loc,
//                                                Q(n1+n2_loc*(igpu/2),iil-1+i), ldq, queues[igpu+1][ind] );
                    }
                    if (n12 != 0) {
                        magma_minproduct_setdevice(igpu);
                        magma_minproduct_dgetmatrix( ni_loc[igpu], ib, dQ(igpu, ind), n1_loc,
                                          Q(n1_loc*(igpu/2),iil-1+i), ldq );
//                        magma_minproduct_dgetmatrix_async( ni_loc[igpu], ib, dQ(igpu, ind), n1_loc,
