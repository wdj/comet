...
extern "C" void ccheck_eig_(char *JOBZ, int  *MATYPE, int  *N, int  *NB,
                       magma_minproductFloatComplex* A, int  *LDA, float *AD, float *AE, float *D1, float *EIG,
                    magma_minproductFloatComplex *Z, int  *LDZ, magma_minproductFloatComplex *WORK, float *RWORK, float *RESU);
#endif
...
int main( int argc, char** argv)
{
    ...
    magma_minproductFloatComplex *h_A, *h_R, *h_work, *dT1;
    magma_minproductFloatComplex *tau;
    float *D, *E;

    /* Matrix size */
    magma_minproduct_int_t N, n2, lda, lwork, lwork0;  //ldt
...
#if defined(CHECKEIG)
#if defined(PRECISION_z)  || defined(PRECISION_d)
    magma_minproduct_int_t WANTZ=0;
    magma_minproduct_int_t THREADS=1;
#endif
#endif

    magma_minproduct_int_t NE = 0;
    magma_minproduct_int_t NB = 0;
    magma_minproduct_int_t ngpu = 1;
    ...
    NB = opts.nb;
    if (NB < 1)
        NB  = 64; //64; //magma_minproduct_get_chetrd_he2hb_nb(N);

    // what is NE ?
    if (NE < 1)
        NE  = 64; //N;  //magma_minproduct_get_chetrd_he2hb_nb(N);  // N not yet initialized
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            /* We suppose the magma_minproduct NB is bigger than lapack NB */
            lwork0 = N*NB;
        
            /* Allocate host memory for the matrix */
            TESTING_MALLOC_CPU( h_A,    magma_minproductFloatComplex, lda*N  );
            TESTING_MALLOC_CPU( tau,    magma_minproductFloatComplex, N-1    );
            
            TESTING_MALLOC_PIN( h_R,    magma_minproductFloatComplex, lda*N  );
            TESTING_MALLOC_PIN( h_work, magma_minproductFloatComplex, lwork0 );
            TESTING_MALLOC_PIN( D, float, N );
            TESTING_MALLOC_PIN( E, float, N );
            
            //TESTING_MALLOC_DEV( dT1, magma_minproductFloatComplex, (2*min(N,N)+(N+31)/32*32)*NB );
            TESTING_MALLOC_DEV( dT1, magma_minproductFloatComplex, (N*NB) );
            ...
            /*
            magma_minproduct_chetrd_he2hb( opts.uplo, N, NB, h_R, lda, tau, h_work, lwork0, dT1, THREADS, &info);
            tband = magma_minproduct_wtime - gpu_time();
            printf("  Finish BAND  N %d  NB %d  ngpu %d timing= %f\n", N, NB, ngpu, tband);
            magma_minproduct_chetrd_bhe2trc_v5(THREADS, WANTZ, opts.uplo, NE, N, NB, h_R, lda, D, E, dT1, ldt);
            */
    
            /*
            magma_minproduct_chetrd_he2hb( opts.uplo, N, NB, h_R, lda, tau, h_work, lwork, dT1, THREADS, &info);
            tband = magma_minproduct_wtime - gpu_time();
            printf("  Finish BAND  N %d  NB %d  ngpu %d timing= %f\n", N, NB, ngpu, tband);
            magma_minproduct_chetrd_bhe2trc(THREADS, WANTZ, opts.uplo, NE, N, NB, h_R, lda, D, E, dT1, ldt);
            */

            magma_minproduct_range_t range = Magma_minproductRangeAll;
            magma_minproduct_int_t fraction_ev = 100;
            magma_minproduct_int_t il, iu, m1;
            float vl=0., vu=0.;
            ...
            magma_minproductFloatComplex *hh_work;
            magma_minproduct_int_t *iwork;
            magma_minproduct_int_t /*nb,*/ /*lwork,*/ liwork;
            magma_minproduct_int_t threads = magma_minproduct_get_parallel_numthreads();
            #if defined(PRECISION_z) || defined(PRECISION_c)
                float *rwork;
                magma_minproduct_int_t lrwork;
                lwork  = magma_minproduct_cbulge_get_lq2(N, threads) + 2*N + N*N;
                lrwork = 1 + 5*N +2*N*N;
                TESTING_MALLOC_PIN( rwork, float, lrwork );
            #else
                lwork  = magma_minproduct_cbulge_get_lq2(N, threads) + 1 + 6*N + 2*N*N;
            #endif
            liwork = 3 + 5*N;
            //nb = magma_minproduct_get_chetrd_nb(N);
            TESTING_MALLOC_PIN( hh_work, magma_minproductFloatComplex, lwork  );
            TESTING_MALLOC_CPU( iwork,   magma_minproduct_int_t,        liwork );
    
            if (ngpu == 1) {
                printf("calling cheevdx_2stage 1 GPU\n");
                magma_minproduct_cheevdx_2stage( opts.jobz, range, opts.uplo, N,
                                h_R, lda,
                                vl, vu, il, iu,
                                &m1, D,
                                hh_work, lwork,
                ...
                magma_minproduct_cheevdx_2stage_m(ngpu, opts.jobz, range, opts.uplo, N,
                                h_R, lda,
                                vl, vu, il, iu,
                                &m1, D,
                                hh_work, lwork,
            ...
            */
            ...
            if ( opts.check ) {
                ...
                int    lwork2 = 256*N;
                magma_minproductFloatComplex *work2, *AINIT;
                float *rwork2, *D2;
                // TODO free this memory !
                magma_minproduct_cmalloc_cpu( &work2, lwork2 );
                magma_minproduct_smalloc_cpu( &rwork2, N );
                magma_minproduct_smalloc_cpu( &D2, N );
                magma_minproduct_cmalloc_cpu( &AINIT, N*lda );
                memcpy(AINIT, h_A, N*lda*sizeof(magma_minproductFloatComplex));
                ...
                */
                ...
                //magma_minproductFloatComplex mydz=0.0, mydo=1.0;
                //magma_minproductFloatComplex *Z;
                // magma_minproduct_cmalloc_cpu( &Z, N*lda );
                ...
                magma_minproductFloatComplex *WORKAJETER;
                float *RWORKAJETER, *RESU;
                // TODO free this memory !
                magma_minproduct_cmalloc_cpu( &WORKAJETER, (2* N * N + N)  );
                magma_minproduct_smalloc_cpu( &RWORKAJETER, N  );
                magma_minproduct_smalloc_cpu( &RESU, 10 );
                int MATYPE;
