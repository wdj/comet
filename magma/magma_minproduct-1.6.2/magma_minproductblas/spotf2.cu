...
void spotf2_sscal(magma_minproduct_int_t n, float *x, magma_minproduct_int_t incx);
void spotf2_sdot(magma_minproduct_int_t n, float *x, magma_minproduct_int_t incx);

#if defined(PRECISION_z) || defined(PRECISION_c)
void slacgv(magma_minproduct_int_t n, float *x, magma_minproduct_int_t incx);
#endif
    ...
    if (uplo == Magma_minproductUpper) {
        for(j = 0; j < n; j++) {
            ...
            if (j < n) {
                ...
                slacgv(j, dA(0, j), 1);
                #endif
                magma_minproduct_sgemv( Magma_minproductTrans, j, n-j-1,
                             alpha, dA(0, j+1), ldda,
                                    dA(0, j),   1,
                             beta,  dA(j, j+1), ldda);
        ...
        for(j = 0; j < n; j++) {
            ...
            if (j < n) {
                ...
                #endif
                magma_minproduct_sgemv( Magma_minproductNoTrans, n-j-1, j,
                             alpha, dA(j+1, 0), ldda,
                                    dA(j,0),    ldda,
                             beta,  dA(j+1, j), 1 );
...
*/
    ...
    kernel_sdot<<< 1, threadSize, threadSize * sizeof(float), magma_minproduct_stream>>> (n, x, incx, threadSize);
}
...
*/
    ...
    kernel_sscal<<< grid, threads, 0, magma_minproduct_stream >>> (n, x, incx);
}
...
void slacgv(magma_minproduct_int_t n, float *x, magma_minproduct_int_t incx)
{
    ...
    dim3 grid(num_blocks,1);
    kernel_slacgv<<< grid, threads, 0, magma_minproduct_stream >>> (n, x, incx);
}
