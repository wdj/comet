...
int main( int argc, char** argv )
{
    ...
    double *C, *R, *A, *W, *tau;
    magma_tally4Double_ptr dC, dA, dT;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int itran = 0; itran < 2; ++itran ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {        
            ...
            magma_tally4_dsetmatrix( lda, k, A,  lda, dA, lda );
            magma_tally4_dgeqrf_gpu( lda, k, dA, lda, tau, dT, &info );
            magma_tally4_dgetmatrix( lda, k, dA, lda, A,  lda );
            if (info != 0)
            ...
            lapackf77_dormqr( lapack_side_const_tally4( side[iside] ), lapack_trans_const_tally4( trans[itran] ),
                              &m, &n, &k,
            ...
            lwork = -1;
            magma_tally4_dormqr_gpu( side[iside], trans[itran],
                              m, n, k,
                              dA, lda, tau, dC, ldc, W, lwork, dT, nb, &info );
            if (info != 0)
                printf("magma_tally4_dormqr_gpu (lwork query) returned error %d: %s.\n",
                       (int) info, magma_tally4_strerror( info ));
            lwork = (magma_tally4_int_t) MAGMA_tally4_D_REAL( W[0] );
            ...
            if (info != 0)
                printf("magma_tally4_dormqr_gpu returned error %d: %s.\n",
                       (int) info, magma_tally4_strerror( info ));
