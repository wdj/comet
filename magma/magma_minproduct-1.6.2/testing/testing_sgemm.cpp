...
int main( int argc, char** argv)
{
    ...
    float *h_A, *h_B, *h_C, *h_Cmagma_minproduct, *h_Cdev;
    magma_minproductFloat_ptr d_A, d_B, d_C;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_Cmagma_minproduct,  float, ldc*N  );
            ...
            magma_minproduct_ssetmatrix( Am, An, h_A, lda, d_A, ldda );
            magma_minproduct_ssetmatrix( Bm, Bn, h_B, ldb, d_B, lddb );
            
            /* =====================================================================
               Performs operation using MAGMA_minproductBLAS (currently only with CUDA)
               =================================================================== */
            #ifdef HAVE_CUBLAS
                magma_minproduct_ssetmatrix( M, N, h_C, ldc, d_C, lddc );
                
                magma_minproduct_time = magma_minproduct_sync_wtime( NULL );
                magma_minproductblas_sgemm( opts.transA, opts.transB, M, N, K,
                                 alpha, d_A, ldda,
                                        d_B, lddb,
                                 beta,  d_C, lddc );
                ...
                magma_minproduct_sgetmatrix( M, N, d_C, lddc, h_Cmagma_minproduct, ldc );
            #endif
            ...
            magma_minproduct_ssetmatrix( M, N, h_C, ldc, d_C, lddc );
                ...
                magma_minproduct_sgemm( opts.transA, opts.transB, M, N, K,
                             alpha, d_A, 0, ldda,
                                    d_B, 0, lddb,
            ...
            magma_minproduct_sgetmatrix( M, N, d_C, lddc, h_Cdev, ldc );
            ...
            if ( opts.lapack ) {
                ...
                dev_error = lapackf77_slange( "F", &M, &N, h_Cdev, &ldc, work ) / Cnorm;
                ...
                #ifdef HAVE_CUBLAS
                    blasf77_saxpy( &sizeC, &c_neg_one, h_C, &ione, h_Cmagma_minproduct, &ione );
                    magma_minproduct_error = lapackf77_slange( "F", &M, &N, h_Cmagma_minproduct, &ldc, work ) / Cnorm;
                    ...
                    blasf77_saxpy( &sizeC, &c_neg_one, h_Cdev, &ione, h_Cmagma_minproduct, &ione );
                    magma_minproduct_error = lapackf77_slange( "F", &M, &N, h_Cmagma_minproduct, &ldc, work ) / Cnorm;
