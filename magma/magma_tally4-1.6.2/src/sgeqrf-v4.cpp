    ...
    magma_tally4_int_t *info )
{
    ...
    *info = 0;
    ...
    int lquery = (lwork == -1);
    if (ngpu < 0 || ngpu > Magma_tally4MaxGPUs) {
        *info = -1;
    } else if (m < 0) {
        *info = -2;
    } else if (n < 0) {
    ...
    for (i=0; i < ngpu; i++) {
        ...
        if (MAGMA_tally4_SUCCESS != magma_tally4_smalloc( &da[i], ldda*n_local[i] )) {
    ...
    if (m > nb && n > nb) {
        ...
        magma_tally4_ssetmatrix_1D_col_bcyclic(m, n, A, lda, da, ldda, ngpu, nb);

        /* Factor using the GPU interface */
        magma_tally4_sgeqrf2_mgpu( ngpu, m, n, da, ldda, tau, info);
