...
int main(int argc, char **argv)
{
    ...
    magma_tally2DoubleComplex *h_A, *h_B, *h_X, temp, *hwork;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            bool nopiv = true;
            if ( nopiv ) {
                magma_tally2_zmake_hpd( N, h_A, lda );  // SPD / HPD does not require pivoting
            }
            else {
                magma_tally2_zmake_hermitian( N, h_A, lda );  // symmetric/Hermitian generally requires pivoting
            }
            ...
            magma_tally2_zhesv_nopiv_gpu( opts.uplo, N, nrhs, d_A, ldda, d_B, lddb, &info );
            ...
            if ( opts.lapack ) {
                lwork = -1;
                lapackf77_zhesv( lapack_uplo_const_tally2(opts.uplo), &N,&nrhs,
                                 h_A, &lda, ipiv, h_B, &ldb, &temp, &lwork, &info );
                lwork = (magma_tally2_int_t) MAGMA_tally2_Z_REAL( temp );
                TESTING_MALLOC_PIN( hwork, magma_tally2DoubleComplex, lwork );
