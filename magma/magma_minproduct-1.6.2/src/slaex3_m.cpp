...
magma_minproduct_int_t magma_minproduct_get_slaex3_m_k()  { return  512; }
magma_minproduct_int_t magma_minproduct_get_slaex3_m_nb() { return 1024; }
...
/**
    ...
    @param
    queues  (device queues) magma_minproduct_queue_t array,
            dimension (Magma_minproductMaxGPUs,2)
    ...
    @ingroup magma_minproduct_ssyev_aux
    ********************************************************************/
...
#define dQ(id, ii) (dwork[id] + n2*n2_loc +    2*(n2*nb) + (ii)*(n2_loc*nb))
    ...
    if (ngpu == 1) {
        ...
        magma_minproduct_slaex3(k, n, n1, d, Q, ldq, rho,
    ...
    magma_minproduct_int_t ineg_one = -1;

    magma_minproduct_int_t iil, iiu, rk;
    magma_minproduct_int_t n1_loc, n2_loc, ib, nb, ib2, igpu;
    magma_minproduct_int_t ni_loc[Magma_minproductMaxGPUs];

    magma_minproduct_int_t i, ind, iq2, j, n12, n2, n23, tmp;
    float temp;
...
//#define CHECK_CPU
#ifdef CHECK_CPU
    float *hwS[2][Magma_minproductMaxGPUs], *hwQ[2][Magma_minproductMaxGPUs], *hwQ2[Magma_minproductMaxGPUs];
    ...
    nb = magma_minproduct_get_slaex3_m_nb();

    if (n1 >= magma_minproduct_get_slaex3_m_k()) {
#ifdef CHECK_CPU
        for (igpu = 0; igpu < ngpu; ++igpu) {
            magma_minproduct_smalloc_pinned( &(hwS[0][igpu]), n2*nb );
            magma_minproduct_smalloc_pinned( &(hwS[1][igpu]), n2*nb );
            magma_minproduct_smalloc_pinned( &(hwQ2[igpu]), n2*n2_loc );
            magma_minproduct_smalloc_pinned( &(hwQ[0][igpu]), n2_loc*nb );
            magma_minproduct_smalloc_pinned( &(hwQ[1][igpu]), n2_loc*nb );
        }
#endif
        for (igpu = 0; igpu < ngpu-1; igpu += 2) {
            ...
            magma_minproduct_ssetmatrix_async( ni_loc[igpu], n12,
                                    Q2+n1_loc*(igpu/2), n1,
...
#endif
            magma_minproduct_setdevice(igpu+1);
            magma_minproduct_ssetmatrix_async( ni_loc[igpu+1], n23,
...
#pragma omp parallel private(i, j, tmp, temp)
    {
        magma_minproduct_int_t id = omp_get_thread_num();
        magma_minproduct_int_t tot = omp_get_num_threads();

        magma_minproduct_int_t ib = (  id   * k) / tot; //start index of local loop
        magma_minproduct_int_t ie = ((id+1) * k) / tot; //end index of local loop
        magma_minproduct_int_t ik = ie - ib;           //number of local indices

        for (i = ib; i < ie; ++i)
            dlamda[i]=lapackf77_slamc3(&dlamda[i], &dlamda[i]) - dlamda[i];

        for (j = ib; j < ie; ++j) {
            magma_minproduct_int_t tmpp=j+1;
            magma_minproduct_int_t iinfo = 0;
...
#pragma omp single
            {
                ...
                magma_minproduct_int_t nk = n - k;
                lapackf77_slamrg( &k, &nk, d, &ione, &ineg_one, indxq);

                //compute the lower and upper bound of the non-deflated eigenvectors
                if (valeig)
                    magma_minproduct_svrange(k, d, &iil, &iiu, vl, vu);
                else if (indeig)
                    magma_minproduct_sirange(k, indxq, &iil, &iiu, il, iu);
                else {
                    iil = 1;
                    iiu = k;
                }
...
#pragma omp single
                ...
                for (j = 0; j < k; ++j) {
                    magma_minproduct_int_t i_tmp = min(j, ie);
                    for (i = ib; i < i_tmp; ++i)
...
#pragma omp barrier
                ...
                for (j = ib; j < ie; ++j) {
                    for (i = 0; i < k; ++i)
                        s[id*k + i] = w[i] / *Q(i,j);
                    temp = magma_minproduct_cblas_snrm2( k, s+id*k, 1 );
                    for (i = 0; i < k; ++i) {
                        magma_minproduct_int_t iii = indx[i] - 1;
    ...
    if (k == 2) {
        ...
        for (j = iil-1; j < iiu; ++j) {
            for (i = 0; i < k; ++i)
                s[i] = w[i] / *Q(i,j);
            temp = magma_minproduct_cblas_snrm2( k, s, 1 );
            for (i = 0; i < k; ++i) {
                magma_minproduct_int_t iii = indx[i] - 1;
                *Q(i,j) = s[iii] / temp;
            }
    ...
    if (rk > 0) {
        if (n1 < magma_minproduct_get_slaex3_m_k()) {
            ...
            for (igpu = 0; igpu < ngpu-1; igpu += 2) {
                if (n23 != 0) {
                    magma_minproduct_setdevice(igpu+1);
                    magma_minproduct_ssetmatrix_async( n23, ib,
                                            Q(ctot[0],iil-1), ldq,
                                            dS(igpu+1,0),     n23, queues[igpu+1][0] );
                }
                if (n12 != 0) {
                    magma_minproduct_setdevice(igpu);
                    magma_minproduct_ssetmatrix_async( n12, ib,
                                            Q(0,iil-1), ldq,
            ...
            for (i = 0; i < rk; i += nb) {
                ...
                if (i+nb < rk) {
                    ...
                    for (igpu = 0; igpu < ngpu-1; igpu += 2) {
                        if (n23 != 0) {
                            magma_minproduct_setdevice(igpu+1);
                            magma_minproduct_ssetmatrix_async( n23, ib2,
                                                    Q(ctot[0],iil-1+i+nb), ldq,
                                                    dS(igpu+1,(ind+1)%2),  n23, queues[igpu+1][(ind+1)%2] );
                        }
                        if (n12 != 0) {
                            magma_minproduct_setdevice(igpu);
                            magma_minproduct_ssetmatrix_async( n12, ib2,
                                                    Q(0,iil-1+i+nb),    ldq,
...
#endif
                        magma_minproduct_setdevice(igpu+1);
                        magma_minproduct_queue_sync( queues[igpu+1][ind] );
                    }
                    if (n12 != 0) {
#ifdef CHECK_CPU
...
#endif
                        magma_minproduct_setdevice(igpu);
                        magma_minproduct_queue_sync( queues[igpu][ind] );
                    }
                }
                for (igpu = 0; igpu < ngpu-1; igpu += 2) {
                    if (n23 != 0) {
...
#endif
                        magma_minproduct_setdevice(igpu+1);
                        magma_minproductblasSetKernelStream(queues[igpu+1][ind]);
                        magma_minproduct_sgemm(Magma_minproductNoTrans, Magma_minproductNoTrans, ni_loc[igpu+1], ib, n23, d_one, dQ2(igpu+1), n2_loc,
...
#endif
                        magma_minproduct_setdevice(igpu);
                        magma_minproductblasSetKernelStream(queues[igpu][ind]);
                        magma_minproduct_sgemm(Magma_minproductNoTrans, Magma_minproductNoTrans, ni_loc[igpu], ib, n12, d_one, dQ2(igpu), n1_loc,
                ...
                for (igpu = 0; igpu < ngpu-1; igpu += 2) {
                    if (n23 != 0) {
                        magma_minproduct_setdevice(igpu+1);
                        magma_minproduct_sgetmatrix( ni_loc[igpu+1], ib, dQ(igpu+1, ind), n2_loc,
                                          Q(n1+n2_loc*(igpu/2),iil-1+i), ldq );
//                        magma_minproduct_sgetmatrix_async( ni_loc[igpu+1], ib, dQ(igpu+1, ind), n2_loc,
//                                                Q(n1+n2_loc*(igpu/2),iil-1+i), ldq, queues[igpu+1][ind] );
                    }
                    if (n12 != 0) {
                        magma_minproduct_setdevice(igpu);
                        magma_minproduct_sgetmatrix( ni_loc[igpu], ib, dQ(igpu, ind), n1_loc,
                                          Q(n1_loc*(igpu/2),iil-1+i), ldq );
//                        magma_minproduct_sgetmatrix_async( ni_loc[igpu], ib, dQ(igpu, ind), n1_loc,
//                                                Q(n1_loc*(igpu/2),iil-1+i), ldq, queues[igpu][ind] );
            ...
            for (igpu = 0; igpu < ngpu; ++igpu) {
#ifdef CHECK_CPU
                magma_minproduct_free_pinned( hwS[1][igpu] );
                magma_minproduct_free_pinned( hwS[0][igpu] );
                magma_minproduct_free_pinned( hwQ2[igpu] );
                magma_minproduct_free_pinned( hwQ[1][igpu] );
                magma_minproduct_free_pinned( hwQ[0][igpu] );
#endif
                magma_minproduct_setdevice(igpu);
                magma_minproduct_queue_sync( queues[igpu][0] );
                magma_minproduct_queue_sync( queues[igpu][1] );
            }
            if ( n23 == 0 )
                lapackf77_slaset("A", &n2, &rk, &d_zero, &d_zero, Q(n1,iil-1), &ldq);
