    ...
    magma_tally2_int_t *info)
{
    ...
    *info = 0;
    ...
    #endif
    if (MAGMA_tally2_SUCCESS != magma_tally2_zmalloc( &dA, ldda*n + 2*lddw*nb + ldwork2 )) {
        *info = MAGMA_tally2_ERR_DEVICE_ALLOC;
        return *info;
    }

    magma_tally2DoubleComplex *dwork  = dA + ldda*n;
    #ifdef FAST_HEMV
    magma_tally2DoubleComplex *dwork2 = dwork + 2*lddw*nb;
    #endif
    ...
    if (upper) {
        ...
        for (i = n - nb; i >= kk; i -= nb) {
            ...
            magma_tally2_zlatrd2( uplo, i+nb, nb, A(0, 0), lda, e, tau,
                           ...
                           dwork2, ldwork2 );
            #else
            magma_tally2_zlatrd(  uplo, i+nb, nb, A(0, 0), lda, e, tau,
                           work, ldw, dA(0, 0), ldda, dwork, lddw );
            #endif
        ...
        for (i = 0; i < n-nx; i += nb) {
            ...
            magma_tally2_zlatrd2( uplo, n-i, nb, A(i, i), lda, &e[i], &tau[i],
                           ...
                           dwork2, ldwork2 );
            #else
            magma_tally2_zlatrd(  uplo, n-i, nb, A(i, i), lda, &e[i], &tau[i],
                           work, ldw, dA(i, i), ldda, dwork, lddw );
            #endif
