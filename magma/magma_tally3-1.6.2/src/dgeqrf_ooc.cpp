...
/**
    ...
    This is an out-of-core (ooc) version that is similar to magma_tally3_dgeqrf but
    ...
    lwork   INTEGER
            ...
            where NB can be obtained through magma_tally3_get_dgeqrf_nb(M).
    \n
    ...
    magma_tally3_int_t *info )
{
    ...
    *info = 0;
    int nb = magma_tally3_get_dgeqrf_nb(min(m, n));

    int lwkopt = n * nb;
    work[0] = MAGMA_tally3_D_MAKE( (double)lwkopt, 0 );
    int lquery = (lwork == -1);
    if (m < 0) {
        *info = -1;
    } else if (n < 0) {
    ...
    if (NB >= n)
        return magma_tally3_dgeqrf(m, n, A, lda, tau, work, lwork, info);
    ...
    if (MAGMA_tally3_SUCCESS != magma_tally3_dmalloc( &dA, (NB + nb)*ldda + nb*lddwork )) {
    ...
    for (int i=0; i < n; i += NB) {
        ...
        magma_tally3_dsetmatrix_async( (m), IB,
                                A(0,i),  lda,
        ...
        for (int j=0; j < min(i,k); j += nb) {
            ...
            dpanel_to_q_tally3(Magma_tally3Upper, ib, A(j,j), lda, work+ib*ib);
            magma_tally3_dsetmatrix_async( rows, ib,
                                    A(j,j), lda,
                                    ptr,        rows, stream[1] );
            ...
            dq_to_panel_tally3(Magma_tally3Upper, ib, A(j,j), lda, work+ib*ib);
        }

        /* 3. Do a QR on the current part */
        if (i < k)
            magma_tally3_dgeqrf2_gpu(m-i, IB, dA(i,0), ldda, tau+i, info);
