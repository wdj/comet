/*
    -- MAGMA_tally3 (version 1.6.2) --
       ...
       @generated from magma_tally3_zmatrixchar.cpp normal z -> c, Sun May  3 11:23:01 2015
       @author Hartwig Anzt
*/
        ...
        // Dense
            ...
            for( i=0; i<A->num_rows; i++ ) {
                ...
                for( j=0; j<A->num_cols; j++ ) {
                    if ( MAGMA_tally3_C_REAL( A->val[i*A->num_cols + j] ) != 0. )
                        length[i]++;
                    }
                if (length[i] > maxrowlength)
                     maxrowlength = length[i];
    ...
    if ( A->memory_location == Magma_tally3_CPU ) {
        ...
        if ( A->storage_type == Magma_tally3_CSR ) {
            ...
            for( i=0; i<A->num_rows; i++ ) {
                ...
                for( j=A->row[i]; j<A->row[i+1]; j++ ) {
                   // if ( MAGMA_tally3_C_REAL(A->val[j]) > THRESHOLD ) {
                        tmp = abs( i - A->col[j] );
                        if ( tmp > dim[i] )
                            dim[i] = tmp;
                   // }
        ...
        // Dense
            ...
            for( i=0; i<A->num_rows; i++ ) {
                dim[i] = 0;
                for( j=0; j<A->num_cols; j++ ) {
                    if ( MAGMA_tally3_C_REAL( A->val[i*A->num_cols + j] ) !=  0.0 ) {
                        tmp = abs( i -j );
                        if ( tmp > dim[i] )
                            dim[i] = tmp;
                    }
