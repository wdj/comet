...
int main( int argc, char** argv)
{
    ...
    magma_tally3DoubleComplex *h_A, *h_R, *h_Q, *h_work, *tau, *twork;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_tally3_zgehrd( N, ione, N, h_R, lda, tau, h_work, lwork, dT, &info);
            gpu_time = magma_tally3_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally3_zgehrd returned error %d: %s.\n",
