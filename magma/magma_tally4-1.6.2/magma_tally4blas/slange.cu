    ...
    magma_tally4Float_ptr dwork )  //, magma_tally4_int_t lwork )
{
    ...
    if ( norm == Magma_tally4InfNorm ) {
        dim3 grid( (m-1)/64 + 1 );
        slange_inf_kernel<<< grid, threads, 0, magma_tally4_stream >>>( m, n, dA, ldda, dwork );
        ...
        dim3 grid( (m-1)/64 + 1 );
        slange_max_kernel<<< grid, threads, 0, magma_tally4_stream >>>( m, n, dA, ldda, dwork );
        max_nan_kernel<<< 1, 512, 0, magma_tally4_stream >>>( m, dwork );
        cudaMemcpy( &result, &dwork[0], sizeof(float), cudaMemcpyDeviceToHost );
    }
    else if ( norm == Magma_tally4OneNorm ) {
        dim3 grid( n );
        slange_one_kernel<<< grid, threads, 0, magma_tally4_stream >>>( m, n, dA, ldda, dwork );
