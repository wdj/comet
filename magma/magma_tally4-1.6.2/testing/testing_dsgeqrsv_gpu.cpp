...
int main( int argc, char** argv)
{
    ...
    magma_tally4Double_ptr d_A, d_B, d_X, d_T;
    float  *d_SA, *d_SB;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lddx   = ((N+31)/32) * 32;
            nb     = max( magma_tally4_get_dgeqrf_nb( M ), magma_tally4_get_sgeqrf_nb( M ) );
            ...
            lhwork = -1;
            lapackf77_dgels( Magma_tally4NoTransStr, &M, &N, &nrhs,
                             NULL, &lda, NULL, &ldb, tmp, &lhwork, &info );
            lhwork = (magma_tally4_int_t) MAGMA_tally4_D_REAL( tmp[0] );
            ...
            lapackf77_dlacpy( Magma_tally4UpperLowerStr, &M, &N, h_A, &lda, h_A2, &lda );
            
            // make random RHS
            size = ldb*nrhs;
            lapackf77_dlarnv( &ione, ISEED, &size, h_B );
            lapackf77_dlacpy( Magma_tally4UpperLowerStr, &M, &nrhs, h_B, &ldb, h_R, &ldb );
            
            magma_tally4_dsetmatrix( M, N,    h_A, lda, d_A, ldda );
            magma_tally4_dsetmatrix( M, nrhs, h_B, ldb, d_B, lddb );
            
            //=====================================================================
            //              Mixed Precision Iterative Refinement - GPU
            //=====================================================================
            gpu_time = magma_tally4_wtime();
            magma_tally4_dsgeqrsv_gpu( M, N, nrhs,
                                d_A, ldda, d_B, lddb,
                                d_X, lddx, &qrsv_iters, &info );
            gpu_time = magma_tally4_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally4_dsgeqrsv returned error %d: %s.\n",
                       (int) info, magma_tally4_strerror( info ));
            
            // compute the residual
            magma_tally4_dgetmatrix( N, nrhs, d_X, lddx, h_X, ldb );
            blasf77_dgemm( Magma_tally4NoTransStr, Magma_tally4NoTransStr, &M, &nrhs, &N,
                           &c_neg_one, h_A, &lda,
                                       h_X, &ldb,
                           &c_one,     h_R, &ldb);
            ...
            gpu_time = magma_tally4_wtime();
            magma_tally4_dgels_gpu( Magma_tally4NoTrans, M, N, nrhs, d_A, ldda,
            ...
            magma_tally4blas_dlag2s( M, N,    d_A, ldda, d_SA, ldda, &info );
            magma_tally4blas_dlag2s( N, nrhs, d_B, lddb, d_SB, lddb, &info );
            
            gpu_time = magma_tally4_wtime();
            magma_tally4_sgels_gpu( Magma_tally4NoTrans, M, N, nrhs, d_SA, ldda,
            ...
            /* =====================================================================
               ...
               =================================================================== */
            lapackf77_dlacpy( Magma_tally4UpperLowerStr, &M, &nrhs, h_B, &ldb, h_X, &ldb );
