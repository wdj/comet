/*
   ...
   @generated from testing_zgetrf_batched.cpp normal z -> s, Fri Jan 30 19:00:26 2015
 */
// includes, system
#include <stdlib.h>
#include <stdio.h>
...
float get_LU_error(magma_tally3_int_t M, magma_tally3_int_t N,
                    ...
                    float *LU, magma_tally3_int_t *IPIV)
{
    magma_tally3_int_t min_mn = min(M,N);
    magma_tally3_int_t ione   = 1;
    magma_tally3_int_t i, j;
    float alpha = MAGMA_tally3_S_ONE;
    float beta  = MAGMA_tally3_S_ZERO;
    float *L, *U;
    ...
    lapackf77_slacpy( Magma_tally3LowerStr, &M, &min_mn, LU, &lda, L, &M      );
    lapackf77_slacpy( Magma_tally3UpperStr, &min_mn, &N, LU, &lda, U, &min_mn );

    for(j=0; j<min_mn; j++)
        L[j+j*M] = MAGMA_tally3_S_MAKE( 1., 0. );
    ...
    for( j = 0; j < N; j++ ) {
        for( i = 0; i < M; i++ ) {
            LU[i+j*lda] = MAGMA_tally3_S_SUB( LU[i+j*lda], A[i+j*lda] );
        }
    }
...
int main( int argc, char** argv)
{
    ...
    for( int i = 0; i < opts.ntest; ++i ) {
      ...
      for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lapackf77_slacpy( Magma_tally3UpperLowerStr, &M, &columns, h_A, &lda, h_R, &lda );
            
            /* ====================================================================
               Performs operation using MAGMA_tally3
               =================================================================== */

            magma_tally3_ssetmatrix( M, columns, h_R, lda, dA, ldda );
            sset_pointer(dA_array, dA, ldda, 0, 0, ldda*N, batchCount, queue);
            set_ipointer(dipiv_array, dipiv_magma_tally3, 1, 0, 0, min_mn, batchCount, queue);
            
            magma_tally3_time = magma_tally3_sync_wtime(0);
            info = magma_tally3_sgetrf_batched( M, N, dA_array, ldda, dipiv_array,  dinfo_magma_tally3, batchCount, queue);
            magma_tally3_time = magma_tally3_sync_wtime(0) - magma_tally3_time;
            magma_tally3_perf = gflops / magma_tally3_time;
            
            magma_tally3_sgetmatrix( M, N*batchCount, dA, ldda, h_Amagma_tally3, lda );
            ...
            {
                if(cpu_info[i] != 0 ){
                    printf("magma_tally3_sgetrf_batched matrix %d returned internal error %d\n",i, (int)cpu_info[i] );
                }
            }
            
            if (info != 0)
                printf("magma_tally3_sgetrf_batched returned argument error %d: %s.\n", (int) info, magma_tally3_strerror( info ));
