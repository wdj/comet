...
int main(int argc, char **argv)
{
    ...
    magma_tally3DoubleComplex *h_A, *h_B, *h_X, temp, *hwork;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            bool nopiv = true;
            if ( nopiv ) {
                magma_tally3_zmake_hpd( N, h_A, lda );  // SPD / HPD does not require pivoting
            }
            else {
                magma_tally3_zmake_hermitian( N, h_A, lda );  // symmetric/Hermitian generally requires pivoting
            }
            ...
            magma_tally3_zhesv_nopiv_gpu( opts.uplo, N, nrhs, d_A, ldda, d_B, lddb, &info );
            gpu_time = magma_tally3_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally3_zgesv_gpu returned error %d: %s.\n",
            ...
            if ( opts.lapack ) {
                lwork = -1;
                lapackf77_zhesv( lapack_uplo_const_tally3(opts.uplo), &N,&nrhs,
                                 h_A, &lda, ipiv, h_B, &ldb, &temp, &lwork, &info );
                lwork = (magma_tally3_int_t) MAGMA_tally3_Z_REAL( temp );
                TESTING_MALLOC_PIN( hwork, magma_tally3DoubleComplex, lwork );
