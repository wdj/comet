...
bool lessthan( magma_tally4FloatComplex a, magma_tally4FloatComplex b )
{
    return (MAGMA_tally4_C_REAL(a) < MAGMA_tally4_C_REAL(b)) ||
        (MAGMA_tally4_C_REAL(a) == MAGMA_tally4_C_REAL(b) && MAGMA_tally4_C_IMAG(a) < MAGMA_tally4_C_IMAG(b));
}
...
extern "C"
float magma_tally4_slapy2(float x, float y)
{
    float ret_val, d;
    float w, z, xabs, yabs;
    
    xabs = MAGMA_tally4_S_ABS(x);
    yabs = MAGMA_tally4_S_ABS(y);
    w    = max(xabs, yabs);
    z    = min(xabs, yabs);
    if (z == 0.) {
        ret_val = w;
...
extern "C"
float magma_tally4_szlapy2(magma_tally4FloatComplex x)
{
    return magma_tally4_slapy2( MAGMA_tally4_C_REAL(x), MAGMA_tally4_C_IMAG(x) );
}
...
int main( int argc, char** argv)
{
    ...
    float *w1i, *w2i;
    magma_tally4FloatComplex *w1copy, *w2copy;
    magma_tally4FloatComplex  c_neg_one = MAGMA_tally4_C_NEG_ONE;
    float tnrm, result[9];
    magma_tally4_int_t N, n2, lda, nb, lwork, info;
    ...
    if ( opts.check && !opts.lapack && opts.jobvl == Magma_tally4NoVec && opts.jobvr == Magma_tally4NoVec ) {
    ...
    printf("    N   CPU Time (sec)   GPU Time (sec)   |W_magma_tally4 - W_lapack| / |W_lapack|\n");
    printf("===========================================================================\n");
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( w1copy, magma_tally4FloatComplex, N );
            TESTING_MALLOC_CPU( w2copy, magma_tally4FloatComplex, N );
            ...
            lapackf77_slacpy( Magma_tally4UpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
            
            /* ====================================================================
               Performs operation using MAGMA_tally4
               =================================================================== */
            gpu_time = magma_tally4_wtime();
            magma_tally4_sgeev( opts.jobvl, opts.jobvr,
                         N, h_R, lda, w1, w1i,
                         VL, lda, VR, lda,
                         h_work, lwork, &info );
            gpu_time = magma_tally4_wtime() - gpu_time;
            if (info != 0)
                printf("magma_tally4_sgeev returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                if ( opts.jobvr == Magma_tally4Vec ) {
                    // Do test 1: | A * VR - VR * W | / ( n |A| )
                    // Note this writes result[1] also
                    lapackf77_sget22( Magma_tally4NoTransStr, Magma_tally4NoTransStr, Magma_tally4NoTransStr,
                    ...
                    for( int j = 0; j < N; ++j ) {
                        tnrm = 1.;
                        if (w1i[j] == 0.)
                            tnrm = magma_tally4_cblas_snrm2( N, &VR[j*lda], ione );
                        else if (w1i[j] > 0.)
                            tnrm = magma_tally4_slapy2( magma_tally4_cblas_snrm2( N, &VR[j*lda],     ione ),
                                                 magma_tally4_cblas_snrm2( N, &VR[(j+1)*lda], ione ));
                        
                        result[1] = max( result[1], min( ulpinv, MAGMA_tally4_S_ABS(tnrm-1.)/ulp ));
                        
                        if (w1i[j] > 0.) {
                            vmx  = vrmx = 0.;
                            for( int jj = 0; jj < N; ++jj ) {
                                vtst = magma_tally4_slapy2( VR[jj+j*lda], VR[jj+(j+1)*lda]);
                                if (vtst > vmx)
                                    vmx = vtst;
                                
                                if ( (VR[jj + (j+1)*lda])==0. &&
                                     MAGMA_tally4_S_ABS( VR[jj+j*lda] ) > vrmx)
                                {
                                    vrmx = MAGMA_tally4_S_ABS( VR[jj+j*lda] );
                                }
                            }
                            if (vrmx / vmx < 1. - ulp*2.)
                                result[1] = ulpinv;
                ...
                if ( opts.jobvl == Magma_tally4Vec ) {
                    // Do test 3: | A**T * VL - VL * W**T | / ( n |A| )
                    // Note this writes result[3] also
                    lapackf77_sget22( Magma_tally4TransStr, Magma_tally4NoTransStr, Magma_tally4TransStr,
                    ...
                    for( int j = 0; j < N; ++j ) {
                        tnrm = 1.;
                        if (w1i[j] == 0.)
                            tnrm = magma_tally4_cblas_snrm2( N, &VL[j*lda], ione );
                        else if (w1i[j] > 0.)
                            tnrm = magma_tally4_slapy2( magma_tally4_cblas_snrm2( N, &VL[j*lda],     ione ),
                                                 magma_tally4_cblas_snrm2( N, &VL[(j+1)*lda], ione ));
                        
                        result[3] = max( result[3], min( ulpinv, MAGMA_tally4_S_ABS(tnrm-1.)/ulp ));
                        
                        if (w1i[j] > 0.) {
                            vmx  = vrmx = 0.;
                            for( int jj = 0; jj < N; ++jj ) {
                                vtst = magma_tally4_slapy2( VL[jj+j*lda], VL[jj+(j+1)*lda]);
                                if (vtst > vmx)
                                    vmx = vtst;
                                
                                if ( (VL[jj + (j+1)*lda])==0. &&
                                     MAGMA_tally4_S_ABS( VL[jj+j*lda]) > vrmx)
                                {
                                    vrmx = MAGMA_tally4_S_ABS( VL[jj+j*lda] );
                                }
                            }
                            if (vrmx / vmx < 1. - ulp*2.)
                                result[3] = ulpinv;
            ...
            if ( opts.check == 2 ) {
                ...
                // this is really slow because it calls magma_tally4_zgeev multiple times
                float *LRE, DUM;
                ...
                magma_tally4_sgeev( Magma_tally4Vec, Magma_tally4Vec,
                             N, h_R, lda, w1, w1i,
                             VL, lda, VR, lda,
                             h_work, lwork, &info );
                if (info != 0)
                    printf("magma_tally4_zgeev (case V, V) returned error %d: %s.\n",
                           (int) info, magma_tally4_strerror( info ));
                ...
                //lapackf77_slacpy( Magma_tally4UpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                //magma_tally4_sgeev( Magma_tally4NoVec, Magma_tally4NoVec,
                //             N, h_R, lda, w2, w2i,
                //             &DUM, 1, &DUM, 1,
                //             h_work, lwork, &info );
                //if (info != 0)
                //    printf("magma_tally4_sgeev (case N, N) returned error %d: %s.\n",
                //           (int) info, magma_tally4_strerror( info ));
                //
                ...
                lapackf77_slacpy( Magma_tally4UpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                magma_tally4_sgeev( Magma_tally4NoVec, Magma_tally4Vec,
                             N, h_R, lda, w2, w2i,
                             &DUM, 1, LRE, lda,
                             h_work, lwork, &info );
                if (info != 0)
                    printf("magma_tally4_sgeev (case N, V) returned error %d: %s.\n",
                           (int) info, magma_tally4_strerror( info ));
                ...
                for( int j = 0; j < N; ++j )
                    for( int jj = 0; jj < N; ++jj )
                        if ( ! MAGMA_tally4_S_EQUAL( VR[j+jj*lda], LRE[j+jj*lda] ))
                            result[7] = 0;
                
                // ----------
                // Compute eigenvalues and left eigenvectors
                lapackf77_slacpy( Magma_tally4UpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                magma_tally4_sgeev( Magma_tally4Vec, Magma_tally4NoVec,
                             N, h_R, lda, w2, w2i,
                             LRE, lda, &DUM, 1,
                             h_work, lwork, &info );
                if (info != 0)
                    printf("magma_tally4_sgeev (case V, N) returned error %d: %s.\n",
                           (int) info, magma_tally4_strerror( info ));
                ...
                for( int j = 0; j < N; ++j )
                    for( int jj = 0; jj < N; ++jj )
                        if ( ! MAGMA_tally4_S_EQUAL( VL[j+jj*lda], LRE[j+jj*lda] ))
                            result[8] = 0;
            ...
            if ( opts.lapack ) {
                ...
                lapackf77_sgeev( lapack_vec_const_tally4(opts.jobvl), lapack_vec_const_tally4(opts.jobvr),
                                 &N, h_A, &lda, w2, w2i,
                                 VL, &lda, VR, &lda,
                                 h_work, &lwork, &info );
                ...
                // check | W_magma_tally4 - W_lapack | / | W |
                // need to sort eigenvalues first
                // copy them into complex vectors for ease
                for( int j=0; j < N; ++j ) {
                    w1copy[j] = MAGMA_tally4_C_MAKE( w1[j], w1i[j] );
                    w2copy[j] = MAGMA_tally4_C_MAKE( w2[j], w2i[j] );
                }
                ...
                for( int j=0; j < N; ++j ) {
                    for( int j2=j; j2 < N; ++j2 ) {
                        magma_tally4FloatComplex diff = MAGMA_tally4_C_SUB( w1copy[j], w2copy[j2] );
                        float diff2 = magma_tally4_szlapy2( diff ) / max( magma_tally4_szlapy2( w1copy[j] ), tol );
                        if ( diff2 < 100*tol ) {
                            if ( j != j2 ) {
                ...
                error  = magma_tally4_cblas_scnrm2( N, w1copy, 1 );
                error /= magma_tally4_cblas_scnrm2( N, w2copy, 1 );
