...
strtri_diag_lower_device(
    magma_tally2_diag_t diag, int n, const float *A, int lda, float *d_dinvA)
{
    int tx = threadIdx.x;
    int bx = blockIdx.x;
    int blk_ind = bx*IB;
    ...
    for( int j=0; j < IB; j++ ) {
        if (tx >= j && ind < n) {
            ...
            sB[tx + j*IB] = MAGMA_tally2_S_ZERO;
        }
    }
    __syncthreads();
    ...
    if (diag == Magma_tally2Unit) {
        sB[tx + tx*IB] = MAGMA_tally2_S_ONE;
    }
    else {
        if ( sB[tx + tx*IB] == MAGMA_tally2_S_ZERO ) {  // singular or outside matrix
            sB[tx + tx*IB] = MAGMA_tally2_S_ONE;
        }
        else {
            sB[tx + tx*IB] = MAGMA_tally2_S_ONE / sB[tx + tx*IB];
        }
    }
...
strtri_diag_lower_kernel_batched(
    magma_tally2_diag_t diag, int n, float const * const * dA_array, int lda, float **dinvA_array)
{
