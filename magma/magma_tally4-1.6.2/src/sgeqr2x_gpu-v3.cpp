    ...
    magma_tally4Float_ptr       dwork, magma_tally4_int_t ldwork )
{
    ...
    // W = W T^H = C^H V T^H
    magma_tally4_strmm( Magma_tally4Right, Magma_tally4Upper, Magma_tally4NoTrans, Magma_tally4NonUnit,
                 n, k,
                 c_one, dT,    lddt,
                        dwork, ldwork);
    ...
    *info = 0;
    ...
    for (int b=0; b < k; b += BLOCK_SIZE) {
        for (i = b; i < min(k, b+BLOCK_SIZE); ++i) {
            ...
            if ( i-b > 0)
                magma_tally4_slarfbx_gpu(m-b, i-b, dA(b, b), ldda,
                                  dT+b+b*k, k, dA(b, i), work);

            /*   Adjust the dnorm[i] to hold the norm of A(i:m,i) */
            //if ( i > 0 )
            //    magma_tally4blas_snrm2_adjust(i, dnorm+i, dA(0, i));
            magma_tally4blas_snrm2_cols(m-i, 1, dA(i,i), ldda, dnorm+i);
            ...
            /*  Generate elementary reflector H(i) to annihilate A(i+1:m,i)
                ...
                3. update T */
            magma_tally4_slarfgtx_gpu(m-i, dA(i, i), dA(min(i+1,m), i), dtau+i,
                               dnorm+i, ddA + i + i*(n), i,
