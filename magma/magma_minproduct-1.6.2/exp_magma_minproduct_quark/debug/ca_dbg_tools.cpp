...
#include "cudbg_magma_minproduct_wrap.h" 
#else
...
int i,j, Mp, Np;
    ...
    if ( magma_minproduct_is_devptr( A ) == 1 ) {
...
void ca_dbg_printMat_gpu(int M, int N, double *dA,int dA_LDA, char desc[] )
{
    ...
    if ( magma_minproduct_is_devptr( dA ) == 0 ) {
    ...
    //if(magma_minproduct_malloc_pinned((void **) &A, max(LDA,M)*N)!=MAGMA_minproduct_SUCCESS) {printf("Memory allocation failed for A in ca_dbg_printMat_gpu"); exit(1);}
   // if(magma_minproduct_dmalloc_pinned(&A, max(LDA,M)*N)!=MAGMA_minproduct_SUCCESS) {printf("Memory allocation failed for A in ca_dbg_printMat_gpu\n"); exit(1);}
    if(magma_minproduct_dmalloc_cpu(&A, max(LDA,M)*N)!=MAGMA_minproduct_SUCCESS) {printf("Memory allocation failed for A in ca_dbg_printMat_gpu\n"); exit(1);}
    magma_minproduct_dgetmatrix(M, N, dA, dA_LDA, A, LDA);
...
int LDA=dA_LDA;
    ...
    /*TODO slow*/
    //if (MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc_cpu(&A, max(LDA,M)*N)) {printf("Memory allocation failed for A in cudbg_dprint_transpose_gpu\n");exit(1);}
    //if(!(A =    (double *)    malloc(max(LDA,M)*N*sizeof(double)))) {printf("Memory allocation failed for A in cudbg_dprint_transpose_gpu\n");exit(1);}
    //if(magma_minproduct_dmalloc_pinned(&A, max(LDA,M)*N)!=MAGMA_minproduct_SUCCESS) {printf("Memory allocation failed for A in cudbg_dprint_transpose_gpu\n");exit(1);}
    if(magma_minproduct_dmalloc_cpu(&A, max(LDA,M)*N)!=MAGMA_minproduct_SUCCESS) {printf("Memory allocation failed for A in ca_dbg_printMat_gpu\n"); exit(1);}
    magma_minproduct_dgetmatrix(M, N, dA, dA_LDA, A, LDA);
