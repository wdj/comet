/*
    -- MAGMA_minproduct (version 1.6.1) --
       ...
       @generated from ztranspose_inplace.cu normal z -> c, Fri Jan 30 19:00:09 2015
...
__global__ void ctranspose_inplace_odd( int n, magma_minproductFloatComplex *matrix, int lda )
{
    __shared__ magma_minproductFloatComplex sA[ NB ][ NB+1 ];
    __shared__ magma_minproductFloatComplex sB[ NB ][ NB+1 ];
    ...
    magma_minproductFloatComplex *A = matrix + ii+i + (jj+j)*lda;
    if( ii == jj ) {
        if ( ii+i < n && jj+j < n ) {
            sA[j][i] = *A;
        }
        ...
        magma_minproductFloatComplex *B = matrix + jj+i + (ii+j)*lda;
        if ( ii+i < n && jj+j < n ) {
            sA[j][i] = *A;
        }
        if ( jj+i < n && ii+j < n ) {
...
__global__ void ctranspose_inplace_even( int n, magma_minproductFloatComplex *matrix, int lda )
{
    __shared__ magma_minproductFloatComplex sA[ NB ][ NB+1 ];
    __shared__ magma_minproductFloatComplex sB[ NB ][ NB+1 ];
...
/**
    @see magma_minproductblas_ctranspose_inplace_q
    @ingroup magma_minproduct_caux2
    ********************************************************************/
extern "C" void
magma_minproductblas_ctranspose_inplace(
    ...
    magma_minproductFloatComplex_ptr dA, magma_minproduct_int_t ldda )
{
    magma_minproductblas_ctranspose_inplace_q( n, dA, ldda, magma_minproduct_stream );
}
