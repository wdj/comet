...
/**
    ...
    @ingroup magma_minproduct_zgels_driver
    ********************************************************************/
    ...
    magma_minproduct_int_t *info)
{
    ...
    magma_minproductDoubleComplex *hworkd;
    magma_minproductFloatComplex  *hworks;
    magma_minproductDoubleComplex *tau;
    magma_minproductFloatComplex  *stau;
    magma_minproductDoubleComplex_ptr dworkd;
    magma_minproductFloatComplex_ptr  dworks;
    magma_minproductDoubleComplex_ptr dR, dT;
    magma_minproductFloatComplex_ptr  dSA, dSX, dST;
    ...
    *info = 0;
    ...
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_cmalloc( &dworks, size )) {
    ...
    /* dworkd(dR) = lddr*nrhs */
    ldworkd = lddr*nrhs;
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc( &dworkd, ldworkd )) {
    ...
    size = lhwork + minmn;
    magma_minproduct_cmalloc_cpu( &hworks, size );
    if ( hworks == NULL ) {
        magma_minproduct_free( dworks );
    ...
    Anrm = magma_minproductblas_zlange(Magma_minproductInfNorm, m, n, dA, ldda, (double*)dworkd );
    cte  = Anrm * eps * pow((double)n, 0.5) * BWDMAX;

    /*
     * Convert to single precision
     */
    magma_minproductblas_zlag2c( m, nrhs, dB, lddb, dSX, lddsx, info );
    if (*info != 0) {
        *iter = -2;
        goto FALLBACK;
    }

    magma_minproductblas_zlag2c( m, n, dA, ldda, dSA, lddsa, info );
    if (*info != 0) {
        *iter = -2;
        goto FALLBACK;
    }
    ...
    magma_minproduct_cgeqrf_gpu( m, n, dSA, lddsa, stau, dST, info );
    if (*info != 0) {
        *iter = -3;
        goto FALLBACK;
    }
    ...
    magma_minproduct_cgeqrs_gpu( m, n, nrhs, dSA, lddsa, stau, dST, dSX, lddsx, hworks, lhwork, info );
    if (*info != 0) {
        *iter = -3;
        goto FALLBACK;
    }
    ...
    magma_minproductblas_clag2z( n, nrhs, dSX, lddsx, dX, lddx, info );
    magma_minproductblas_zlacpy( Magma_minproductUpperLower, m, nrhs, dB, lddb, dR, lddr );
    if ( nrhs == 1 ) {
        magma_minproduct_zgemv( Magma_minproductNoTrans, m, n,
                     c_neg_one, dA, ldda,
                                dX, 1,
                     c_one,     dR, 1 );
    }
    else {
        magma_minproduct_zgemm( Magma_minproductNoTrans, Magma_minproductNoTrans, m, nrhs, n,
                     c_neg_one, dA, ldda,
                                dX, lddx,
                     c_one,     dR, lddr );
    }
    ...
    for( j=0; j < nrhs; j++ ) {
        ...
        i = magma_minproduct_izamax ( m, dR(0,j), 1 ) - 1;
...
REFINEMENT:
    ...
    for( iiter=1; iiter < ITERMAX; ) {
        *info = 0;
        ...
        magma_minproductblas_zlag2c( m, nrhs, dR, lddr, dSX, lddsx, info );
        if (*info != 0) {
            *iter = -2;
            goto FALLBACK;
        }
        ...
        for( j=0; j < nrhs; j++ ) {
            magma_minproductblas_zcaxpycp( n, dSX(0,j), dX(0,j), dB(0,j), dR(0,j) );
        }
        // dR = dB  (whole m rows)
        magma_minproductblas_zlacpy( Magma_minproductUpperLower, m, nrhs, dB, lddb, dR, lddr );
...
FALLBACK:
    ...
    if ( size > ldworkd ) {
        magma_minproduct_free( dworkd );
        if (MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc( &dworkd, size )) {
    ...
    size = lhwork + minmn;
    magma_minproduct_zmalloc_cpu( &hworkd, size );
    if ( hworkd == NULL ) {
        magma_minproduct_free( dworkd );
    ...
    magma_minproduct_zgeqrf_gpu( m, n, dA, ldda, tau, dT, info );
    if (*info == 0) {
        // if m > n, then dB won't fit in dX, so solve with dB and copy n rows to dX
        magma_minproduct_zgeqrs_gpu( m, n, nrhs, dA, ldda, tau, dT, dB, lddb, hworkd, lhwork, info );
        magma_minproductblas_zlacpy( Magma_minproductUpperLower, n, nrhs, dB, lddb, dX, lddx );
    }
