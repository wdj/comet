...
/* ////////////////////////////////////////////////////////////////////////////
   -- Testing magma_minproduct_zher2k_mgpu
*/
int main( int argc, char** argv)
{
    ...
    magma_minproductDoubleComplex alpha = MAGMA_minproduct_Z_MAKE( 1.2345, 4.321 );
    double beta = 3.14159;
    
    real_Double_t    gflops, gpu_perf, cpu_perf, gpu_time, cpu_time;
    double           error, work[1];
    magma_minproductDoubleComplex *hA, *hR, *hR2, *hV, *hW;
    magma_minproductDoubleComplex_ptr dV[Magma_minproductMaxGPUs], dW[Magma_minproductMaxGPUs], dA[Magma_minproductMaxGPUs];
    ...
    printf( "version 1: magma_minproductblas_zher2k_mgpu2     %s\n", (opts.version==1 ? "(enabled)" : ""));
    printf( "version 2: magma_minproductblas_zher2k_mgpu_spec %s\n", (opts.version==2 ? "(enabled)" : ""));
#ifdef ICHI
    printf( "version 3: magma_minproduct_zher2k_mgpu (Ichi)   %s\n", (opts.version==3 ? "(enabled)" : ""));
#endif
    printf( "\n" );
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        ...
        for( int offset = 0; offset < n; offset += min(k,nb) ) {
            for( int iter = 0; iter < opts.niter; ++iter ) {
                ...
                TESTING_MALLOC_CPU( hR,  magma_minproductDoubleComplex, lda*n   );
                TESTING_MALLOC_CPU( hR2, magma_minproductDoubleComplex, lda*n   );
                TESTING_MALLOC_CPU( hV,  magma_minproductDoubleComplex, lda*k*2 );
                //TESTING_MALLOC_CPU( hW,  magma_minproductDoubleComplex, lda*k   );
                for( int d = 0; d < ngpu; ++d ) {
                    magma_minproduct_int_t nlocal = ((n / nb) / ngpu + 1) * nb;
                    magma_minproduct_setdevice( d );
                    TESTING_MALLOC_DEV( dA[d], magma_minproductDoubleComplex, ldda*nlocal );
                    TESTING_MALLOC_DEV( dV[d], magma_minproductDoubleComplex, ldda*k*2    );
                    //TESTING_MALLOC_DEV( dW[d], magma_minproductDoubleComplex, ldda*k      );
                ...
                /* ====================================================================
                   ...
                   =================================================================== */
                magma_minproduct_zsetmatrix_1D_col_bcyclic( n, n, hA, lda, dA, ldda, ngpu, nb );
                for( int d = 0; d < ngpu; ++d ) {
                    magma_minproduct_setdevice( d );
                    dW[d] = dV[d] + ldda*k;
                    magma_minproduct_zsetmatrix( n, k, hV, lda, dV[d], ldda );
                    magma_minproduct_zsetmatrix( n, k, hW, lda, dW[d], ldda );
                }
                ...
                magma_minproduct_zgetmatrix_1D_col_bcyclic( n, n, dA, ldda, hR, lda, ngpu, nb );
