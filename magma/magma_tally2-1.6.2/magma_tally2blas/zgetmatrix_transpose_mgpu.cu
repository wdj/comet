...
#define dAT(d, j)   (dAT[(d)]   + (j)*nb)
    ...
    if (lda < m || ngpu*ldda < n || lddw < m){
        printf( "Wrong arguments in magma_tally2blas_zgetmatrix_transpose_mgpu (%d<%d), (%d*%d<%d), or (%d<%d).\n",
    ...
    for(j=0; j<n; j+=nb){
       ...
       ib = min(n-j, nb);
       magma_tally2blas_ztranspose_q( ib, m, dAT(d,j_local), ldda, dwork(d,id), lddw, queues[d][id] );
