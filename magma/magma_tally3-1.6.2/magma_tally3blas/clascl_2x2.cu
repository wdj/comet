...
__global__ void
clascl_2x2_lower(int m, const magma_tally3FloatComplex* W, int ldw, magma_tally3FloatComplex* A, int lda)
{
    int ind = blockIdx.x * NB + threadIdx.x;

    magma_tally3FloatComplex D21 = W( 1, 0 );
    magma_tally3FloatComplex D11 = MAGMA_tally3_C_DIV( W( 1, 1 ), D21 );
    magma_tally3FloatComplex D22 = MAGMA_tally3_C_DIV( W( 0, 0 ), MAGMA_tally3_C_CNJG( D21 ) );
    float T = 1.0 / ( MAGMA_tally3_C_REAL( D11*D22 ) - 1.0 );
    D21 = MAGMA_tally3_C_DIV( MAGMA_tally3_C_MAKE(T,0.0), D21 );

    if (ind < m) {
        A( ind, 0 ) = MAGMA_tally3_C_CNJG( D21 )*( D11*W( 2+ind, 0 )-W( 2+ind, 1 ) );
...
__global__ void
clascl_2x2_upper(int m, const magma_tally3FloatComplex *W, int ldw, magma_tally3FloatComplex* A, int lda)
{
    int ind = blockIdx.x * NB + threadIdx.x;

    magma_tally3FloatComplex D21 = W( m, 1 );
    magma_tally3FloatComplex D11 = MAGMA_tally3_C_DIV( W( m+1, 1 ), MAGMA_tally3_C_CNJG( D21 ) );
    magma_tally3FloatComplex D22 = MAGMA_tally3_C_DIV( W( m, 0 ), D21 );
    float T = 1.0 / ( MAGMA_tally3_C_REAL( D11*D22 ) - 1.0 );
    D21 = MAGMA_tally3_C_DIV( MAGMA_tally3_C_MAKE(T,0.0), D21 );

    if (ind < m) {
        A( ind, 0 ) = D21*( D11*W( ind, 0 )-W( ind, 1 ) );
        A( ind, 1 ) = MAGMA_tally3_C_CNJG( D21 )*( D22*W( ind, 1 )-W( ind, 0 ) );
    }
}
...
/**
    ...
    ********************************************************************/
extern "C" void
magma_tally3blas_clascl_2x2_q(
    magma_tally3_type_t type, magma_tally3_int_t m, 
    const magma_tally3FloatComplex *dW, magma_tally3_int_t lddw, 
...
/**
    @see magma_tally3blas_clascl2_q
    @ingroup magma_tally3_caux2
    ********************************************************************/
extern "C" void
...
{
    magma_tally3blas_clascl_2x2_q( type, m, dW, lddw, dA, ldda, info, magma_tally3_stream );
}
