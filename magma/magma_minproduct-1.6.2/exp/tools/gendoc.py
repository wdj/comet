...
def getlatexdoc(funcdoc):
  latexdoc = "\\textsf{magma_minproduct\_int\_t} "
...
def getoutputdoc(fdoc, output):
  ...
  doc += output.ret_type("magma_minproduct_int_t")
...
def main(argv):
  ...
  for fname in argv[1:]:
    ...
    of.write(odoc.preamble("MAGMA_minproduct"))

    funcdoc = getdoc(fname)
    #latexdoc = getlatexdoc(funcdoc)
    doc = getoutputdoc(funcdoc, odoc)
    of.write(doc)
    of.write(odoc.epilog("MAGMA_minproduct"))

    fl.append((funcdoc.funcname, ofname))

  idxf = open(os.path.join(outdir, "index.html"), "w")
  idxf.write("<html>\n<head><title>MAGMA_minproduct 1.0</title></head>\n<body>\n<h1>MAGMA_minproduct 1.0 Function Index</h1><ul>\n")
  for t in fl:
    funcname, ofname = t
