...
int main(int argc, char **argv)
{
    ...
    magma_tally2DoubleComplex **dB_array = NULL;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_tally2_zsetmatrix( N, N*batchCount,    h_A, lda, d_A, ldda );
            ...
            info = magma_tally2_zposv_batched(opts.uplo, N, nrhs, dA_array, ldda, dB_array, lddb, dinfo_array, batchCount, queue); 
            ...
            {
                if(cpu_info[i] != 0 ){
                    printf("magma_tally2_zposv_batched matrix %d returned internal error %d\n",i, (int)cpu_info[i] );
                }
            }
            if (info != 0)
                printf("magma_tally2_zposv_batched returned argument error %d: %s.\n", (int) info, magma_tally2_strerror( info ));
            ...
            if ( opts.lapack ) {
                ...
                for(magma_tally2_int_t s=0; s<batchCount; s++)
                {
                    lapackf77_zposv( lapack_uplo_const_tally2(opts.uplo), &N, &nrhs, h_A + s * lda * N, &lda, h_B + s * ldb * nrhs, &ldb, &info );
                }
