...
/**
    ...
    lwork   INTEGER
            ...
            through magma_minproduct_get_sgeqlf_nb(M).
    \n
    ...
    magma_minproduct_int_t *info)
{
    ...
    magma_minproductFloat_ptr dA, dwork;
    ...
    nb = magma_minproduct_get_sgeqlf_nb(m);
    *info = 0;
    lquery = (lwork == -1);
    ...
    if (*info == 0) {
        if (k == 0)
            ...
            work[0] = MAGMA_minproduct_S_MAKE( max(n*nb, 2*nb*nb), 0 );
        }
    ...
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_smalloc( &dA, (n)*ldda + nb*lddwork )) {
    ...
    if ( (nb > 1) && (nb < k) ) {
        ...
        magma_minproduct_ssetmatrix_async( m, n-nb,
                                A(0, 0),  lda,
        ...
        for (i = k - kk + ki; i >= k -kk; i -= nb) {
            ...
            if (i < k - kk + ki) {
                ...
                magma_minproduct_sgetmatrix_async( m-rows, ib,
                                        dA(rows, n-k+i), ldda,
                ...
                magma_minproduct_slarfb_gpu( Magma_minproductLeft, Magma_minproductConjTrans, Magma_minproductBackward, Magma_minproductColumnwise,
                                  rows, cols, old_ib,
            ...
            if (cols > 0) {
                ...
                lapackf77_slarft( Magma_minproductBackwardStr, Magma_minproductColumnwiseStr,
                                  &rows, &ib,
                                  A(0, cols), &lda, tau + i, work, &ib);

                spanel_to_q( Magma_minproductLower, ib, A(rows-ib,cols), lda, work+ib*ib);
                magma_minproduct_ssetmatrix( rows, ib,
                                  A(0,cols),  lda,
                                  dA(0,cols), ldda );
                sq_to_panel( Magma_minproductLower, ib, A(rows-ib,cols), lda, work+ib*ib);

                // Send the triangular part on the GPU
                magma_minproduct_ssetmatrix( ib, ib, work, ib, dwork(0), lddwork );
        ...
        magma_minproduct_sgetmatrix( m, nu, dA(0,0), ldda, A(0,0), lda );
    } else {
        mu = m;
        nu = n;
    }
