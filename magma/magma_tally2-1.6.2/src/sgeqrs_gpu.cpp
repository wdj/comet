...
/**
    ...
    tau     REAL array, dimension (N)
            ...
            reflector H(i), as returned by MAGMA_tally2_SGEQRF_GPU.
    ...
    dT      REAL array that is the output (the 6th argument)
            of magma_tally2_sgeqrf_gpu of size
    ...
    @ingroup magma_tally2_sgels_comp
    ********************************************************************/
    ...
    magma_tally2_int_t *info)
{
    ...
    *info = 0;
    ...
    /* B := Q' * B */
    magma_tally2_sormqr_gpu( Magma_tally2Left, Magma_tally2Trans,
                      m, nrhs, n,
                      dA(0,0), ldda, tau,
    ...
    // TODO: this assumes that, on exit from magma_tally2_sormqr_gpu, hwork contains
    ...
    // magma_tally2_strsm and drop the ssetmatrix.
    if ( nrhs == 1 ) {
        blasf77_strsv( Magma_tally2UpperStr, Magma_tally2NoTransStr, Magma_tally2NonUnitStr,
                       &ib, hwork,         &rows,
                            hwork+rows*ib, &ione);
    } else {
        blasf77_strsm( Magma_tally2LeftStr, Magma_tally2UpperStr, Magma_tally2NoTransStr, Magma_tally2NonUnitStr,
                       &ib, &nrhs,
                       &c_one, hwork,         &rows,
                               hwork+rows*ib, &rows);
    }
    ...
    magma_tally2_ssetmatrix( ib, nrhs, hwork+rows*ib, rows, dwork+i, lddwork );

    // update c
    if (nrhs == 1)
        magma_tally2_sgemv( Magma_tally2NoTrans, i, ib,
    ...
    if (nb < k) {
        for (i = start; i >= 0; i -= nb) {
            ...
            if (i + ib < n) {
                if (nrhs == 1) {
                    magma_tally2_sgemv( Magma_tally2NoTrans, ib, ib,
                                 c_one,  dT(i), ib,
                                         dB+i,      1,
                                 c_zero, dwork+i,  1);
                    magma_tally2_sgemv( Magma_tally2NoTrans, i, ib,
    ...
    magma_tally2_scopymatrix( (n), nrhs,
                       dwork, lddwork,
                       dB,    lddb );
