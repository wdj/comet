    ...
    float       * __restrict__ y, int incy)
{
    ...
    if ( threadIdx.x < npart ) {
        ...
        sx[threadIdx.x] = MAGMA_minproduct_S_ZERO;
    }
    __syncthreads();
...
magma_minproductblas_sgemv(
    magma_minproduct_trans_t trans, magma_minproduct_int_t m, magma_minproduct_int_t n, float alpha,
    ...
    magma_minproductFloat_ptr dy, magma_minproduct_int_t incy)
{
    ...
    if ( arch < 200  ) {
        ...
        magma_minproduct_sgemv( trans, m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
        #else
        magma_minproductblas_sgemv_tesla( trans, m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
        #endif
        return;
    }
    ...
    if ( trans == Magma_minproductNoTrans ) {
        ...
        sgemvn_kernel1_fermi<<< grid, threads, 0, magma_minproduct_stream >>>
            ( m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    }
    else if ( trans == Magma_minproductTrans ) {
        dim3 grid    ( 1, n, 1 );
        dim3 threads ( BLK_X, 1, 1 );
        sgemvt_kernel_fermi<<< grid, threads, 0, magma_minproduct_stream >>>
            ( m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    }
    else if ( trans == Magma_minproductConjTrans ) {
        dim3 grid    ( 1, n, 1 );
        dim3 threads ( BLK_X, 1, 1 );
        sgemvc_kernel_fermi<<< grid, threads, 0, magma_minproduct_stream >>>
            ( m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    }
}
