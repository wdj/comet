...
magma_tally3_zlarft_sm32x32_batched(magma_tally3_int_t n, magma_tally3_int_t k, magma_tally3DoubleComplex **v_array, magma_tally3_int_t ldv,
                    magma_tally3DoubleComplex **tau_array, magma_tally3DoubleComplex **T_array, magma_tally3_int_t ldt, magma_tally3_int_t batchCount, cublasHandle_t myhandle, magma_tally3_queue_t queue)
{
...
#define use_gemm_larft_sm32
    ...
    //magma_tally3blas_zgemm_batched( Magma_tally3ConjTrans, Magma_tally3NoTrans, k, k, n, MAGMA_tally3_Z_ONE, v_array, ldv, v_array, ldv, MAGMA_tally3_Z_ZERO, T_array, ldt, batchCount, queue);
    cublasZgemmBatched(myhandle, CUBLAS_OP_C, CUBLAS_OP_N, k, k, n,
                             &one, (const magma_tally3DoubleComplex**) v_array, ldv,
                                    (const magma_tally3DoubleComplex**) v_array, ldv,
                             &zero,  T_array, ldt, batchCount);

    magma_tally3blas_zlaset_batched(Magma_tally3Lower, k, k, MAGMA_tally3_Z_ZERO, MAGMA_tally3_Z_ZERO, T_array, ldt, batchCount, queue);
#else
    #if 1
    for(int i=0; i<k; i++)
    {
        ...
        #ifdef USE_GEMV2
        magma_tally3blas_zlarft_gemvrowwise_batched( n-i, i, 
                            tau_array,
                            v_array, ldv, 
                            T_array, ldt,
                            batchCount, queue);
        ...
        #else       
        magma_tally3blas_zlarft_gemvcolwise_batched( n-i, i, v_array, ldv, T_array, ldt, tau_array, batchCount, queue);
        #endif
    }
    #else
     ...
     magma_tally3blas_zlarft_ztrmv_sm32x32_batched(k, k, tau_array, T_array, ldt, T_array, ldt, batchCount, queue);
}
...
magma_tally3_zlarft_batched(magma_tally3_int_t n, magma_tally3_int_t k, magma_tally3_int_t stair_T, 
                ...
                magma_tally3DoubleComplex **work_array, magma_tally3_int_t lwork, magma_tally3_int_t batchCount, cublasHandle_t myhandle, 
                magma_tally3_queue_t queue)
{
    ...
    magma_tally3DoubleComplex **dTstep_array  = NULL;
    ...
    //magma_tally3DoubleComplex *Tstep =  k > nb ? work : T;
    if(k > nb)
    {
        magma_tally3_zdisplace_pointers(dTstep_array, work_array, lwork, 0, 0, batchCount, queue);
    }
    else
    {
        magma_tally3_zdisplace_pointers(dTstep_array, T_array, ldt, 0, 0, batchCount, queue);
    }
...
#ifdef RFT_MAG_GEM
    magma_tally3blas_zgemm_batched( Magma_tally3ConjTrans, Magma_tally3NoTrans, 
            k, k, n, 
            one,  v_array, ldv, 
                  v_array, ldv, 
    ...
    magma_tally3blas_zlaset_batched(Magma_tally3Lower, k, k, MAGMA_tally3_Z_ZERO, MAGMA_tally3_Z_ZERO, dTstep_array, ldtstep, batchCount, queue);
    // no need for it as T is expected to be lower zero
    //if(k > nb) magma_tally3blas_zlaset_batched(Magma_tally3Lower, k, k, MAGMA_tally3_Z_ZERO, MAGMA_tally3_Z_ZERO, dTstep_array, ldtstep, batchCount);
    ...
    for(j=0; j<k; j+=nb)
    {
        ...
        if(prev_n>0 && mycol>0){
            ...
            magma_tally3_zdisplace_pointers(dW1_displ, dTstep_array, ldtstep, 0, j, batchCount, queue);
            magma_tally3_zdisplace_pointers(dW2_displ, T_array,     ldt, 0, j, batchCount, queue);
#ifdef RFT_MAG_GEM
            magma_tally3blas_zgemm_batched( Magma_tally3NoTrans, Magma_tally3NoTrans, 
                    prev_n, mycol, prev_n, 
                    one,  T_array, ldt, 
                          dW1_displ, ldtstep, 
                    zero, dW2_displ, ldt, 
            ...
            cublasZgemmBatched(myhandle, CUBLAS_OP_N, CUBLAS_OP_N, 
                    prev_n, mycol, prev_n,
                    &one, (const magma_tally3DoubleComplex**) T_array, ldt,
                          (const magma_tally3DoubleComplex**) dW1_displ, ldtstep,
                    &zero, dW2_displ, ldt, batchCount);
#endif

            // update my rectangular portion (prev_n,mycol) using sequence of gemv 
            magma_tally3_zdisplace_pointers(dW1_displ, dTstep_array, ldtstep, j, j, batchCount, queue);
            magma_tally3_zdisplace_pointers(dW3_displ, tau_array,  1, j, 0, batchCount, queue);
            ...
            for(i=0; i<prev_n; i+=nb)
            {
                ...
                if(rows>0 && mycol>0)
                {
                    magma_tally3_zdisplace_pointers(dW2_displ, T_array,     ldt, i, j, batchCount, queue);
                    magma_tally3blas_zlarft_recztrmv_sm32x32_batched(rows, mycol, dW3_displ, dW2_displ, ldt, dW1_displ, ldtstep, batchCount, queue);
                }
            }
        }
        ...
        if(stair_T == 0){
            ...
            if(mycol>0)
            {
                magma_tally3_zdisplace_pointers(dW1_displ, dTstep_array, ldtstep, j, j, batchCount, queue);
                magma_tally3_zdisplace_pointers(dW3_displ, tau_array,  1, j, 0, batchCount, queue);
                magma_tally3_zdisplace_pointers(dW2_displ, T_array,     ldt, j, j, batchCount, queue);
                magma_tally3blas_zlarft_ztrmv_sm32x32_batched(mycol, mycol, dW3_displ, dW1_displ, ldtstep, dW2_displ, ldt, batchCount, queue);
