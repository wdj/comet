/*
    -- MAGMA_tally2 (version 1.1) --
       ...
       @generated from magma_tally2_zmconverter.cpp normal z -> d, Tue May  5 14:02:10 2015
       @author Hartwig Anzt
*/
    ...
    magma_tally2_queue_t queue )
{
    ...
    for( i=0; i<*n; i++ ) {
        ...
        for( j=(*row)[i]; j<(*row)[i+1]; j++ ) {
            if ( MAGMA_tally2_D_REAL((*val)[j]) != 0 ) {
                nnz_new++;
                nnz_this_row++;
            }
        }
    ...
    CHECK( magma_tally2_dmalloc_cpu( valn, nnz_new ));
    ...
    magma_tally2_queue_t queue )
{
    ...
    magma_tally2_d_matrix hA={Magma_tally2_CSR}, hB={Magma_tally2_CSR};
    magma_tally2_d_matrix A_d={Magma_tally2_CSR}, B_d={Magma_tally2_CSR};
    ...
    if ( A.memory_location == Magma_tally2_CPU ) {
        ...
        if ( old_format == Magma_tally2_CSR ) {
            ...
            // CSR to CSRL
                ...
                B->nnz = numzeros;
                CHECK( magma_tally2_dmalloc_cpu( &B->val, numzeros ));
                CHECK( magma_tally2_index_malloc_cpu( &B->row, A.num_rows+1 ));
                ...
                for( magma_tally2_int_t i=0; i < A.num_rows; i++) {
                    ...
                    for( magma_tally2_int_t j=A.row[i]; j < A.row[i+1]; j++) {
                        if ( A.col[j] < i) {
                            ...
                            B->val[numzeros] = MAGMA_tally2_D_MAKE(1.0, 0.0);
                            B->col[numzeros] = A.col[j];
                            numzeros++;
                        }
                        else if ( A.col[j] == i ) {
            ...
            // CSR to ELLPACKT (using row-major storage)
                ...
                //printf( "Conversion to ELLPACK with %d elements per row: ",
                                                                // maxrowlength );
                //fflush(stdout);
                CHECK( magma_tally2_dmalloc_cpu( &B->val, maxrowlength*A.num_rows ));
            ...
            // so the rowlength is padded (SELLP) to a multiple of the alignment
                ...
                //fflush(stdout);
                CHECK( magma_tally2_dmalloc_cpu( &B->val, B->row[slices] ));
            ...
            // CSR to BCSR
                ...
                CHECK( magma_tally2_dmconvert(A_d, &B_d, Magma_tally2_CSR, Magma_tally2_BCSR, queue ) );            
        ...
        // anything to CSR
            ...
            // CSRCSC to CSR
              ...
              //  magma_tally2_dmconvert( A, B, Magma_tally2_CSR, Magma_tally2_CSR, queue );
            }
            ...
            // ELL/ELLPACK to CSR
                ...
                CHECK( magma_tally2_d_csr_compressor(&A.val, &row_tmp, &A.col,
            ...
            // ELL (column-major) to CSR
                ...
                // conversion
                CHECK( magma_tally2_dmalloc_cpu( &val_tmp, A.num_rows*A.max_nnz_row ));
                CHECK( magma_tally2_index_malloc_cpu( &row_tmp, A.num_rows+1 ));
                ...
                //now use AA_ELL, IA_ELL, row_tmp as CSR with some zeros.
                //The CSR compressor removes these
                CHECK( magma_tally2_d_csr_compressor(&val_tmp, &row_tmp, &col_tmp,
                           &B->val, &B->row, &B->col, &B->num_rows, queue ));
            ...
            // ELLD (ELLPACK with diagonal element first) to CSR
                ...
                CHECK( magma_tally2_dmalloc_cpu( &val_tmp2, A.num_rows*A.max_nnz_row ));
                ...
                //now use AA_ELL, IA_ELL, row_tmp as CSR with some zeros.
                //The CSR compressor removes these
                CHECK( magma_tally2_d_csr_compressor(&val_tmp2, &row_tmp, &col_tmp2,
                           &B->val, &B->row, &B->col, &B->num_rows, queue ));
                B->nnz = B->row[B->num_rows];
            }
            ...
            // SELLP to CSR
                ...
                // conversion
                CHECK( magma_tally2_dmalloc_cpu( &val_tmp, A.max_nnz_row*(A.num_rows+C) ));
                ...
                for(magma_tally2_int_t i=0; i < A.max_nnz_row*(A.num_rows+C); i++ ) {
                    val_tmp[ i ] = MAGMA_tally2_D_MAKE(0., 0.);
                    col_tmp[ i ] =  0;
                }
            ...
            // DENSE to CSR
                ...
                B->nnz=0;
                for( magma_tally2_int_t i=0; i<(A.num_rows)*(A.num_cols); i++ ) {
                    if ( MAGMA_tally2_D_REAL(A.val[i]) != 0.0 )
                        (B->nnz)++;
                }
                CHECK( magma_tally2_dmalloc_cpu( &B->val, B->nnz));
                ...
                for(i=0; i<(A.num_rows)*(A.num_cols); i++)
                {
                    ...
                    if ( MAGMA_tally2_D_REAL(A.val[i]) != 0 )
                    {
                        (B->val)[j] = A.val[i];
            ...
            // BCSR to CSR
                ...
                CHECK( magma_tally2_dmtransfer(A, &A_d, Magma_tally2_CPU, Magma_tally2_DEV, queue ) );
                CHECK( magma_tally2_dmconvert(A_d, &B_d, Magma_tally2_BCSR, Magma_tally2_CSR, queue ) );
                CHECK( magma_tally2_dmtransfer(B_d, B, Magma_tally2_DEV, Magma_tally2_CPU, queue ) );
            }
        ...
        if ( old_format == Magma_tally2_CSR && new_format == Magma_tally2_CSR ) {
            CHECK( magma_tally2_dmtransfer( A, B, Magma_tally2_DEV, Magma_tally2_DEV, queue ));
        }
        // CSR to DENSE
        ...
        // DENSE to CSR
            ...
            //magma_tally2_dprint_gpu( A.num_rows, 1, nnz_per_row, A.num_rows )
            cusparseDnnz( cusparseHandle, CUSPARSE_DIRECTION_COLUMN,
                          A.num_rows, A.num_cols,
                          descr,
                          A.dval, A.num_rows, nnz_per_row, &intnnz );

            CHECK( magma_tally2_dmalloc( &B->dval, B->nnz ));
        ...
        // CSR to BCSR
            ...
            CHECK( magma_tally2_dmalloc( &B->dval, nnzb*size_b*size_b ));
        ...
        // COO to CSR
            ...
            magma_tally2_dcopyvector( A.nnz, A.val, 1, B->val, 1 );
            ...
            CHECK( magma_tally2_dmconvert( hA, &hB, old_format, new_format, queue ));
            CHECK( magma_tally2_dmtransfer( hB, B, Magma_tally2_CPU, A.memory_location, queue ));
        }
    }
