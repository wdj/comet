...
/**
    ...
    magma_tally2_zprint prints a matrix that is located on the CPU host.
    ...
    const magma_tally2DoubleComplex *A, magma_tally2_int_t lda )
{
    ...
    for( int i = 0; i < m; ++i ) {
        for( int j = 0; j < n; ++j ) {
            if ( MAGMA_tally2_Z_EQUAL( *A(i,j), c_zero )) {
                #ifdef COMPLEX
                printf( "   0.              " );
                #else
                printf( "   0.    " );
                ...
                #ifdef COMPLEX
                printf( " %8.4f+%8.4fi", MAGMA_tally2_Z_REAL( *A(i,j) ), MAGMA_tally2_Z_IMAG( *A(i,j) ));
                #else
                printf( " %8.4f", MAGMA_tally2_Z_REAL( *A(i,j) ));
                #endif
            }
        }
        if ( m > 1 ) {
...
/**
    Purpose
    -------
    magma_tally2_zprint_gpu prints a matrix that is located on the GPU device.
...
void magma_tally2_zprint_gpu(
    magma_tally2_int_t m, magma_tally2_int_t n,
    const magma_tally2DoubleComplex *dA, magma_tally2_int_t ldda )
{
    magma_tally2_int_t info = 0;
    if ( m < 0 )
        info = -1;
