...
int main( int argc, char** argv)
{
    ...
    magma_tally4_norm_t norm[] = { Magma_tally4MaxNorm, Magma_tally4OneNorm, Magma_tally4InfNorm };
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( norm[inorm] == Magma_tally4OneNorm )
                lwork = N;
            else
                lwork = M;
            // read whole matrix
            gbytes = M*N*sizeof(magma_tally4FloatComplex) / 1e9;
            ...
            // MAGMA_tally4 propogates NaN; LAPACK does not necesarily.
            //h_A[ 1 + 1*lda ] = MAGMA_tally4_C_NAN;
            ...
            norm_magma_tally4 = magma_tally4blas_clange( norm[inorm], M, N, d_A, ldda, d_work );
            gpu_time = magma_tally4_wtime() - gpu_time;
            gpu_perf = gbytes / gpu_time;
            if (norm_magma_tally4 < 0)
                printf("magma_tally4blas_clange returned error %f: %s.\n",
            ...
            norm_lapack = lapackf77_clange( lapack_norm_const_tally4(norm[inorm]), &M, &N, h_A, &lda, h_work );
            ...
            //         norm_magma_tally4, norm_lapack,
            //         MAGMA_tally4_C_REAL( h_A[1+1*lda] ),
            //         MAGMA_tally4_C_IMAG( h_A[1+1*lda] ) );
            ...
            printf("%5d %5d   %4c   %7.2f (%7.2f)   %7.2f (%7.2f)   %#9.3g   %s\n",
                   (int) M, (int) N, lapacke_norm_const_tally4(norm[inorm]),
