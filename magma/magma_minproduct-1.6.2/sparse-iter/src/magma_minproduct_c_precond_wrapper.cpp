/*
    -- MAGMA_minproduct (version 1.1) --
       ...
       @generated from magma_minproduct_z_precond_wrapper.cpp normal z -> c, Mon May  4 11:57:23 2015
       @author Hartwig Anzt
    ...
    magma_minproduct_queue_t queue )
{
    ...
    magma_minproduct_c_solver_par psolver_par;
    ...
    magma_minproduct_c_preconditioner pprecond;
    pprecond.solver = Magma_minproduct_NONE;

    switch( precond->solver ) {
        case  Magma_minproduct_CG:
                CHECK( magma_minproduct_ccg_res( A, b, x, &psolver_par, queue )); break;
        case  Magma_minproduct_BICGSTAB:
                CHECK( magma_minproduct_cbicgstab( A, b, x, &psolver_par, queue )); break;
        case  Magma_minproduct_GMRES:
                CHECK( magma_minproduct_cfgmres( A, b, x, &psolver_par, &pprecond, queue )); break;
        case  Magma_minproduct_JACOBI:
                CHECK( magma_minproduct_cjacobi( A, b, x, &psolver_par, queue )); break;
        case  Magma_minproduct_BAITER:
                CHECK( magma_minproduct_cbaiter( A, b, x, &psolver_par, queue )); break;
        default:
                CHECK( magma_minproduct_ccg_res( A, b, x, &psolver_par, queue )); break;
    ...
    magma_minproduct_queue_t queue )
{
    ...
    if ( precond->solver == Magma_minproduct_JACOBI ) {
        return magma_minproduct_cjacobisetup_diagscal( A, &(precond->d), queue );
    }
    else if ( precond->solver == Magma_minproduct_PASTIX ) {
        //return magma_minproduct_cpastixsetup( A, b, precond, queue );
        return MAGMA_minproduct_ERR_NOT_SUPPORTED;
    }
    else if ( precond->solver == Magma_minproduct_ILU ) {
        return magma_minproduct_ccumilusetup( A, precond, queue );
    }
    else if ( precond->solver == Magma_minproduct_ICC ) {
        return magma_minproduct_ccumiccsetup( A, precond, queue );
    }
    else if ( precond->solver == Magma_minproduct_AICC ) {
        return magma_minproduct_citericsetup( A, b, precond, queue );
    }
    else if ( precond->solver == Magma_minproduct_AILU ) {
        return magma_minproduct_citerilusetup( A, b, precond, queue );
    }
    ...
    magma_minproduct_queue_t queue )
{
    ...
    magma_minproduct_c_matrix tmp={Magma_minproduct_CSR};
    ...
    if ( precond->solver == Magma_minproduct_JACOBI ) {
        CHECK( magma_minproduct_cjacobi_diagscal( A.num_rows, precond->d, b, x, queue ));
    }
    else if ( precond->solver == Magma_minproduct_PASTIX ) {
        //CHECK( magma_minproduct_capplypastix( b, x, precond, queue ));
        return MAGMA_minproduct_ERR_NOT_SUPPORTED;
    }
    else if ( precond->solver == Magma_minproduct_ILU ) {
        CHECK( magma_minproduct_cvinit( &tmp, Magma_minproduct_DEV, A.num_rows, b.num_cols, MAGMA_minproduct_C_ZERO, queue ));
    }
    else if ( precond->solver == Magma_minproduct_ICC ) {
        CHECK( magma_minproduct_cvinit( &tmp, Magma_minproduct_DEV, A.num_rows, b.num_cols, MAGMA_minproduct_C_ZERO, queue ));
    }
    else if ( precond->solver == Magma_minproduct_NONE ) {
        magma_minproduct_ccopy( b.num_rows*b.num_cols, b.dval, 1, x->dval, 1 );      //  x = b
    }
    else {
        printf( "error: preconditioner type not yet supported.\n" );
    ...
    magma_minproduct_queue_t queue )
{
    ...
    if ( precond->solver == Magma_minproduct_JACOBI ) {
        ...
        CHECK( magma_minproduct_capplycumilu_l( b, x, precond, queue ));
    }
    else if ( ( precond->solver == Magma_minproduct_ILU ||
                precond->solver == Magma_minproduct_AILU ) && precond->maxiter < 50 ) {
        magma_minproduct_ccopy( b.num_rows*b.num_cols, b.dval, b.num_cols, x->dval, b.num_cols );
        magma_minproduct_c_solver_par solver_par;
        solver_par.maxiter = precond->maxiter;
        //magma_minproduct_cjacobiiter_sys( precond->L, b, precond->d, precond->work1, x, &solver_par, queue );
        CHECK( magma_minproduct_cjacobispmvupdate(precond->maxiter, precond->L, precond->work1, b, precond->d, x, queue ));
    }
    else if ( ( precond->solver == Magma_minproduct_ICC ||
                precond->solver == Magma_minproduct_AICC ) && precond->maxiter >= 50 )  {
        CHECK( magma_minproduct_capplycumicc_l( b, x, precond, queue ));
    }
    else if ( ( precond->solver == Magma_minproduct_ICC ||
                precond->solver == Magma_minproduct_AICC ) && precond->maxiter < 50 )  {
        magma_minproduct_ccopy( b.num_rows*b.num_cols, b.dval, b.num_cols, x->dval, b.num_cols );
        ...
        CHECK( magma_minproduct_capplycustomprecond_l( b, x, precond, queue ));
    }
    else {
        printf( "error: preconditioner type not yet supported.\n" );
        magma_minproductblasSetKernelStream( orig_queue );
    ...
    magma_minproduct_queue_t queue )
{
    ...
    if ( precond->solver == Magma_minproduct_JACOBI ) {
        magma_minproduct_ccopy( b.num_rows*b.num_cols, b.dval, 1, x->dval, 1 );    // x = b
                ...
                precond->solver == Magma_minproduct_AILU ) && precond->maxiter >= 50 ) {
        CHECK( magma_minproduct_capplycumilu_r( b, x, precond, queue ));
    }
    else if ( ( precond->solver == Magma_minproduct_ILU ||
                precond->solver == Magma_minproduct_AILU ) && precond->maxiter < 50 ) {
        magma_minproduct_ccopy( b.num_rows*b.num_cols, b.dval, b.num_cols, x->dval, b.num_cols );
        magma_minproduct_c_solver_par solver_par;
        solver_par.maxiter = precond->maxiter;
        //magma_minproduct_cjacobiiter_sys( precond->U, b, precond->d2, precond->work2, x, &solver_par, queue );
        CHECK( magma_minproduct_cjacobispmvupdate(precond->maxiter, precond->U, precond->work2, b, precond->d2, x, queue ));
    }

    else if ( ( precond->solver == Magma_minproduct_ICC ||
                precond->solver == Magma_minproduct_AICC ) && precond->maxiter >= 50 ) {
        CHECK( magma_minproduct_capplycumicc_r( b, x, precond, queue ));
    }
    else if ( ( precond->solver == Magma_minproduct_ICC ||
               precond->solver == Magma_minproduct_AICC ) && precond->maxiter < 50 ) {
        magma_minproduct_ccopy( b.num_rows*b.num_cols, b.dval, b.num_cols, x->dval, b.num_cols );
        ...
        CHECK( magma_minproduct_capplycustomprecond_r( b, x, precond, queue ));
    }
    else {
        printf( "error: preconditioner type not yet supported.\n" );
        magma_minproductblasSetKernelStream( orig_queue );
