...
__global__ void
magma_tally3_sreduce_kernel_spmv1(    
    int Gs,
    int n, 
    float * vtmp,
    float * vtmp2 )
...
__global__ void
magma_tally3_sbicgmerge_spmv1_kernel(  
    int n,
    float * dval, 
...
__global__ void
magma_tally3_sbicgstab_alphakernel(  
                    float * skp ){
    ...
    magma_tally3_queue_t queue )
{
    ...
    if ( A.storage_type == Magma_tally3_CSR)
        magma_tally3_sbicgmerge_spmv1_kernel<<<Gs, Bs, Ms>>>
    ...
    while( Gs.x > 1 ) {
        ...
        magma_tally3_sreduce_kernel_spmv1<<< Gs_next.x/2, Bs.x/2, Ms/2 >>> 
                            ( Gs.x, n, aux1, aux2 );
    ...
    magma_tally3_scopyvector( 1, aux1, 1, skp, 1 );
    dim3 Bs2( 2 );
    dim3 Gs2( 1 );
    magma_tally3_sbicgstab_alphakernel<<<Gs2, Bs2, 0>>>( skp );
...
__global__ void
magma_tally3_sreduce_kernel_spmv2( 
    int Gs,
    int n, 
    float * vtmp,
    float * vtmp2 )
...
__global__ void
magma_tally3_sbicgmerge_spmv2_kernel(  
    int n,
    float * dval, 
    magma_tally3_index_t * drowptr, 
    magma_tally3_index_t * dcolind,
...
__global__ void
magma_tally3_sbicgstab_omegakernel(  
                    float * skp ){
    int i = blockIdx.x * blockDim.x + threadIdx.x;
...
/**
    ...
    @param[in]
    ds          magma_tally3Float_ptr
                input vector s

    @param[in]
    dt          magma_tally3Float_ptr
                output vector t
    ...
    magma_tally3_queue_t queue )
{
    ...
    int b = 1;        
    if ( A.storage_type == Magma_tally3_CSR)
        magma_tally3_sbicgmerge_spmv2_kernel<<<Gs, Bs, Ms>>>
    ...
    while( Gs.x > 1 ) {
        Gs_next.x = ( Gs.x+Bs.x-1 )/ Bs.x ;
        if ( Gs_next.x == 1 ) Gs_next.x = 2;
        magma_tally3_sreduce_kernel_spmv2<<< Gs_next.x/2, Bs.x/2, Ms/2 >>> 
                    ( Gs.x, n, aux1, aux2 );
        Gs_next.x = Gs_next.x /2;
        Gs.x = Gs_next.x;
        b = 1 - b;
    ...
    magma_tally3_scopyvector( 1, aux1, 1, skp+6, 1 );
    magma_tally3_scopyvector( 1, aux1+n, 1, skp+7, 1 );
    dim3 Bs2( 2 );
    dim3 Gs2( 1 );
    magma_tally3_sbicgstab_omegakernel<<<Gs2, Bs2, 0>>>( skp );
...
__global__ void
magma_tally3_sbicgmerge_xrbeta_kernel(  
    int n, 
    float * rr,
    float * r,
    float * p,
...
__global__ void
magma_tally3_sbicgstab_betakernel(  
    float * skp )
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    magma_tally3_queue_t queue )
{
    ...
    magma_tally3Float_ptr aux1 = d1, aux2 = d2;
    int b = 1;        
    magma_tally3_sbicgmerge_xrbeta_kernel<<<Gs, Bs, Ms>>>
    ...
    magma_tally3_scopyvector( 1, aux1+n, 1, skp+5, 1 );
    dim3 Bs2( 2 );
    dim3 Gs2( 1 );
    magma_tally3_sbicgstab_betakernel<<<Gs2, Bs2, 0>>>( skp );
