...
/**
    ...
    Other versions (magma_tally4_cgeqrf_gpu and magma_tally4_cgeqrf3_gpu) store the
    ...
    *info = 0;
    ...
    if (nb >= nbmin && nb < k && nx < k) {
        ...
        for (i = 0; i < k-nx; i += nb) {
            ...
            cpanel_to_q_tally4( Magma_tally4Upper, ib, work_ref(i), ldwork, hwork+ib*ib );

            /* download the i-th V matrix */
            magma_tally4_csetmatrix_async( rows, ib, work_ref(i), ldwork, dA(i,i), ldda, stream[0] );

            /* download the T matrix */
            magma_tally4_queue_sync( stream[1] );
            magma_tally4_csetmatrix_async( ib, ib, hwork, ib, dwork, lddwork, stream[0] );
            magma_tally4_queue_sync( stream[0] );
            ...
            if (i + ib < n) {
                if (i+nb < k-nx) {
                    ...
                    cq_to_panel_tally4( Magma_tally4Upper, ib, work_ref(i), ldwork, hwork+ib*ib );
                }
                else {
                    magma_tally4_clarfb_gpu( Magma_tally4Left, Magma_tally4ConjTrans, Magma_tally4Forward, Magma_tally4Columnwise,
                                      rows, n-i-ib, ib,
    ...
    if (i < k) {
        ib   = n-i;
        rows = m-i;
        magma_tally4_cgetmatrix_async( rows, ib, dA(i, i), ldda, work, rows, stream[1] );
        magma_tally4_queue_sync( stream[1] );
        lhwork = lwork - rows*ib;
        lapackf77_cgeqrf(&rows, &ib, work, &rows, tau+i, work+ib*rows, &lhwork, info);
        
        magma_tally4_csetmatrix_async( rows, ib, work, rows, dA(i, i), ldda, stream[1] );
    }
