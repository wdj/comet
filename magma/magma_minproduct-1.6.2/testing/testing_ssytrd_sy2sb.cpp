/*
    -- MAGMA_minproduct (version 1.6.1) --
       ...
       @generated from testing_zhetrd_he2hb.cpp normal z -> s, Fri Jan 30 19:00:26 2015
...
int main( int argc, char** argv)
{
    ...
    NB = opts.nb;
    if (NB < 1)
        NB  = 64; //64; //magma_minproduct_get_ssytrd_sy2sb_nb(N);

    // what is NE ?
    if (NE < 1)
        NE  = 64; //N;  //magma_minproduct_get_ssytrd_sy2sb_nb(N);  // N not yet initialized
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            //TESTING_MALLOC_DEV( dT1, float, (2*min(N,N)+(N+31)/32*32)*NB );
            ...
            /*
            magma_minproduct_ssytrd_sy2sb( opts.uplo, N, NB, h_R, lda, tau, h_work, lwork0, dT1, THREADS, &info);
            tband = magma_minproduct_wtime - gpu_time();
            printf("  Finish BAND  N %d  NB %d  ngpu %d timing= %f\n", N, NB, ngpu, tband);
            magma_minproduct_ssytrd_bsy2trc_v5(THREADS, WANTZ, opts.uplo, NE, N, NB, h_R, lda, D, E, dT1, ldt);
            */
    
            /*
            magma_minproduct_ssytrd_sy2sb( opts.uplo, N, NB, h_R, lda, tau, h_work, lwork, dT1, THREADS, &info);
            tband = magma_minproduct_wtime - gpu_time();
            printf("  Finish BAND  N %d  NB %d  ngpu %d timing= %f\n", N, NB, ngpu, tband);
            magma_minproduct_ssytrd_bsy2trc(THREADS, WANTZ, opts.uplo, NE, N, NB, h_R, lda, D, E, dT1, ldt);
            */
            ...
            magma_minproduct_int_t threads = magma_minproduct_get_parallel_numthreads();
                ...
                lwork  = magma_minproduct_sbulge_get_lq2(N, threads) + 2*N + N*N;
                lrwork = 1 + 5*N +2*N*N;
                TESTING_MALLOC_PIN( rwork, float, lrwork );
            #else
                lwork  = magma_minproduct_sbulge_get_lq2(N, threads) + 1 + 6*N + 2*N*N;
            #endif
            liwork = 3 + 5*N;
            //nb = magma_minproduct_get_ssytrd_nb(N);
            ...
            if (ngpu == 1) {
                ...
                magma_minproduct_ssyevdx_2stage( opts.jobz, range, opts.uplo, N,
                                h_R, lda,
                                vl, vu, il, iu,
                                &m1, D,
                                hh_work, lwork,
                ...
                magma_minproduct_ssyevdx_2stage_m(ngpu, opts.jobz, range, opts.uplo, N,
                                h_R, lda,
                                vl, vu, il, iu,
                                &m1, D,
                                hh_work, lwork,
            ...
            */
            ...
            if ( opts.check ) {
                ...
                magma_minproduct_smalloc_cpu( &work2, lwork2 );
                magma_minproduct_smalloc_cpu( &rwork2, N );
                magma_minproduct_smalloc_cpu( &D2, N );
                magma_minproduct_smalloc_cpu( &AINIT, N*lda );
                ...
                */
                ...
                //float *Z;
                // magma_minproduct_smalloc_cpu( &Z, N*lda );
                ...
                // TODO free this memory !
                magma_minproduct_smalloc_cpu( &WORKAJETER, (2* N * N + N)  );
