...
int main( int argc, char** argv)
{
    ...
    double *h_A, *h_B, *h_Bcublas, *h_Bmagma_tally4, *h_Blapack, *h_X;
    double *d_A, *d_B;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_Bmagma_tally4,  double, sizeB   );
            ...
            magma_tally4_dmalloc( &dinvA, dinvA_batchSize * batchCount);
            magma_tally4_dmalloc( &dwork, dwork_batchSize * batchCount );
    
            dset_pointer(dwork_array, dwork, lddb, 0, 0, dwork_batchSize, batchCount, opts.queue);
            dset_pointer(dinvA_array, dinvA, magma_tally4_roundup( Ak, TRI_NB ), 0, 0, dinvA_batchSize, batchCount, opts.queue);

            memset(h_Bmagma_tally4, 0, batchCount*ldb*N*sizeof(double));
            magma_tally4blas_dlaset( Magma_tally4Full, lddb, N*batchCount, c_zero, c_zero, dwork, lddb);
            ...
            /* =====================================================================
               ...
               =================================================================== */
            magma_tally4_dsetmatrix( Ak, Ak*batchCount, h_A, lda, d_A, ldda );
            magma_tally4_dsetmatrix( M,  N*batchCount, h_B, ldb, d_B, lddb );
            ...
            magma_tally4_time = magma_tally4_sync_wtime( NULL );
                ...
                magma_tally4_dgetmatrix( M, N*batchCount, dwork, lddb, h_Bmagma_tally4, ldb );
            #else
                ...
                magma_tally4_time = magma_tally4_sync_wtime( NULL ) - magma_tally4_time;
                magma_tally4_perf = gflops / magma_tally4_time;
                magma_tally4_dgetmatrix( M, N*batchCount, d_B, lddb, h_Bmagma_tally4, ldb );
            #endif
            ...
            magma_tally4_dgetmatrix( M, N*batchCount, d_B, lddb, h_Bcublas, ldb );
            ...
            /* =====================================================================
               ...
               =================================================================== */
            // ||b - 1/alpha*A*x|| / (||A||*||x||)
            double alpha2 = MAGMA_tally4_D_DIV( c_one, alpha );
            double normR, normX, normA=0.;
            magma_tally4_error  = 0.0;
            cublas_error = 0.0;

            memcpy( h_X, h_Bmagma_tally4, sizeB*sizeof(double) );
            ...
            for (int s=0; s < batchCount; s++) {
                ...
                normX = lapackf77_dlange( "M", &M, &N, h_Bmagma_tally4 + s * ldb * N, &ldb, work );
