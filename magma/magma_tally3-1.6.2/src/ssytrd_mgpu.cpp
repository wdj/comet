...
#define dW(id, i, j) (dW[(id)] + (j)*ldda + (i))
    ...
    magma_tally3_int_t nb = magma_tally3_get_ssytrd_nb(n), ib, ib2;
    ...
    float *hwork = NULL;
    magma_tally3Float_ptr dwork2[Magma_tally3MaxGPUs] = { NULL };
    magma_tally3Float_ptr dA[Magma_tally3MaxGPUs]     = { NULL };
    magma_tally3Float_ptr dW[Magma_tally3MaxGPUs]     = { NULL };
    ...
    *info = 0;
    ...
    for( dev=0; dev < ngpu; dev++ ) {
        ...
        if ( MAGMA_tally3_SUCCESS != magma_tally3_smalloc( &dA[dev],     nlocal*ldda + 3*lddw*nb ) ||
             MAGMA_tally3_SUCCESS != magma_tally3_smalloc( &dwork2[dev], ldwork2 ) ) {
    ...
    if ( MAGMA_tally3_SUCCESS != magma_tally3_smalloc_pinned( &hwork, lhwork ) ) {
    ...
    if (upper) {
        ...
        if (1 <= n-nx) {
            magma_tally3_shtodhe( ngpu, uplo, n, nb, A, lda, dA, ldda, queues, &iinfo );
        }
        ...
        for (i = nb*((n-1)/nb); i >= nx; i -= nb) {
            ...
            magma_tally3_slatrd_mgpu( ngpu, uplo, i+ib, ib, nb,
                               A(0, 0), lda, e, tau,
                               work, ldwork,
                               dA, ldda, 0,
                               dW, i+ib,
            ...
            magma_tally3_ssyr2k_mgpu( ngpu, Magma_tally3Upper, Magma_tally3NoTrans, nb, i, ib,
                               c_neg_one, dW, i+ib, 0,
                               d_one,     dA, ldda, 0,
                               nqueue, queues);
            ...
            if (i-nb >= nx ) {
                ...
                magma_tally3_sgetmatrix_async( (i-nb)+ib2, ib2,
                                        dA(dev, 0, ii), ldda,
                                        A(0, i-nb),     lda,
                                        queues[dev][0] );
            }
        ...
        if ( nx > 0 ) {
            if (1 <= n-nx) { /* else A is already on CPU */
                for (i=0; i < nx; i += nb) {
                    ...
                    magma_tally3_setdevice( dev );
                    magma_tally3_sgetmatrix_async( nx, ib,
                                            dA(dev, 0, ii), ldda,
                                            A(0, i),        lda,
                                            queues[dev][0] );
                }
        ...
        for (i = 0; i < n-nx; i += nb) {
            ...
            if (i != 0) {
                ...
                magma_tally3_sgetmatrix_async( n-i, ib,
                                        dA(dev, i, ii), ldda,
                                        A(i,i),         lda,
                                        queues[dev][0] );
                trace_gpu_end( dev, 0 );
            ...
            magma_tally3_slatrd_mgpu( ngpu, uplo, n-i, ib, nb,
                               A(i, i), lda, &e[i], &tau[i],
                               work, ldwork,
                               dA, ldda, i,
                               dW, n-i,
            ...
            magma_tally3_ssyr2k_mgpu( ngpu, Magma_tally3Lower, Magma_tally3NoTrans, nb, n-i-ib, ib,
                               c_neg_one, dW, n-i, ib,
        ...
        if ( i < n ) {
            ...
            if ( i > 0 ) {
                for (; i < n; i += nb) {
                    ...
                    magma_tally3_setdevice( dev );
                    magma_tally3_sgetmatrix_async( i_n, ib,
                                            dA(dev, iii, ii), ldda,
                                            A(iii, i),        lda,
                                            queues[dev][0] );
                }
...
#define dC(id, i, j)  (dC[(id)] + (j)*lddc + (i))
    ...
    for( i=0; i < n; i += nb ) {
        ...
        magma_tally3_ssyr2k( uplo, trans, ib, k,
