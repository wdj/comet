...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            //magma_tally2blas_csymmetrize( opts.uplo, N-2, d_A+1+ldda, ldda );  // inset by 1 row & col
            magma_tally2blas_csymmetrize( opts.uplo, N, d_A, ldda );
            ...
            for( int j = 0; j < N; ++j ) {
                for( int i = 0; i < j; ++i ) {
                    if ( opts.uplo == Magma_tally2Lower ) {
                        h_A[i + j*lda] = MAGMA_tally2_C_CNJG( h_A[j + i*lda] );
                    }
                    else {
                        h_A[j + i*lda] = MAGMA_tally2_C_CNJG( h_A[i + j*lda] );
                    }
                }
            }
