...
int main(int argc, char **argv)
{
    ...
    magma_minproductDoubleComplex *h_A, *h_B, *h_X, temp, *hwork;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            bool nopiv = true;
            if ( nopiv ) {
                magma_minproduct_zmake_hpd( N, h_A, lda );  // SPD / HPD does not require pivoting
            }
            else {
                magma_minproduct_zmake_hermitian( N, h_A, lda );  // symmetric/Hermitian generally requires pivoting
            }
            ...
            magma_minproduct_zhesv_nopiv_gpu( opts.uplo, N, nrhs, d_A, ldda, d_B, lddb, &info );
            gpu_time = magma_minproduct_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_minproduct_zgesv_gpu returned error %d: %s.\n",
            ...
            if ( opts.lapack ) {
                ...
                lwork = (magma_minproduct_int_t) MAGMA_minproduct_Z_REAL( temp );
                TESTING_MALLOC_PIN( hwork, magma_minproductDoubleComplex, lwork );
