/*
    -- MAGMA_tally2 (version 1.6.2) --
       ...
       @generated from magma_tally2_zvinit.cpp normal z -> s, Sun May  3 11:23:01 2015
       @author Hartwig Anzt
*/
...
/**
    ...
    Allocates memory for magma_tally2_s_matrix and initializes it
    ...
    magma_tally2_queue_t queue )
{
    ...
    if ( mem_loc == Magma_tally2_CPU ) {
        x->memory_location = Magma_tally2_CPU;
        CHECK( magma_tally2_smalloc_cpu( &x->val, x->nnz ));
        for( magma_tally2_int_t i=0; i<x->nnz; i++)
             x->val[i] = values;
    }
    else if ( mem_loc == Magma_tally2_DEV ) {
        x->memory_location = Magma_tally2_DEV;
        CHECK( magma_tally2_smalloc( &x->val, x->nnz ));
        magma_tally2blas_slaset(Magma_tally2Full, x->num_rows, x->num_cols, values, values, x->val, x->num_rows);
    }
