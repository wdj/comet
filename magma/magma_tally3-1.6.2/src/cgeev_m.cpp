 ...
 * Version2 - MAGMA_tally3 without dT    (magma_tally3_cgehrd2 and lapack_cunghr)
 * Version3 - MAGMA_tally3 with dT       (magma_tally3_cgehrd  and magma_tally3_cunghr)
 * Version4 - Multi-GPU magma_tally3_cgehrd_m with T on CPU, copied to dT, single-GPU magma_tally3_cunghr
 * Version5 - Multi-GPU magma_tally3_cgehrd_m with T on CPU, multi-GPU magma_tally3_cunghr_m
 */
#define Version5
...
/**
    ...
    @ingroup magma_tally3_cgeev_driver
    ********************************************************************/
    ...
    magma_tally3_int_t *info )
{
    ...
    magma_tally3_int_t scalea, minwrk, irwork, lquery, wantvl, wantvr, select[1];
    ...
    *info = 0;
    ...
    if (*info == 0) {
        minwrk = (1+nb)*n;
        work[0] = MAGMA_tally3_C_MAKE( minwrk, 0 );
    ...
    magma_tally3FloatComplex *dT;
    if (MAGMA_tally3_SUCCESS != magma_tally3_cmalloc( &dT, nb*n )) {
    ...
    if (MAGMA_tally3_SUCCESS != magma_tally3_cmalloc_cpu( &T, nb*n )) {
    ...
    smlnum = magma_tally3_ssqrt( smlnum ) / eps;
    bignum = 1. / smlnum;
    ...
    if (wantvl) {
        ...
        lapackf77_clacpy( Magma_tally3LowerStr, &n, &n, A, &lda, VL, &ldvl );
         ...
         *  - including N reserved for gebal/gebak, unused by cunghr */
            ...
            // Version 3 - LAPACK consistent MAGMA_tally3 HRD + T matrices stored
            magma_tally3_cunghr( n, ilo, ihi, VL, ldvl, &work[itau], dT, nb, &ierr );
        #elif defined(Version5)
            // Version 5 - Multi-GPU, T on host
            magma_tally3_cunghr_m( n, ilo, ihi, VL, ldvl, &work[itau], T, nb, &ierr );
        #endif
        ...
        #elif defined(Version3) || defined(Version4)
            // Version 3 - LAPACK consistent MAGMA_tally3 HRD + T matrices stored
            magma_tally3_cunghr( n, ilo, ihi, VR, ldvr, &work[itau], dT, nb, &ierr );
        #elif defined(Version5)
            // Version 5 - Multi-GPU, T on host
            magma_tally3_cunghr_m( n, ilo, ihi, VR, ldvr, &work[itau], T, nb, &ierr );
        #endif
    ...
    if (wantvl || wantvr) {
        ...
        irwork = ibal + n;
        #if TREVC_VERSION == 1
        lapackf77_ctrevc( lapack_side_const_tally3(side), "B", select, &n, A, &lda, VL, &ldvl,
                          VR, &ldvr, &n, &nout, &work[iwrk], &rwork[irwork], &ierr );
        #elif TREVC_VERSION == 2
        liwrk = lwork - iwrk;
        lapackf77_ctrevc3( lapack_side_const_tally3(side), "B", select, &n, A, &lda, VL, &ldvl,
                           VR, &ldvr, &n, &nout, &work[iwrk], &liwrk, &rwork[irwork], &ierr );
        #elif TREVC_VERSION == 3
        magma_tally3_ctrevc3( side, Magma_tally3BacktransVec, select, n, A, lda, VL, ldvl,
                       VR, ldvr, n, &nout, &work[iwrk], liwrk, &rwork[irwork], &ierr );
        #elif TREVC_VERSION == 4
        magma_tally3_ctrevc3_mt( side, Magma_tally3BacktransVec, select, n, A, lda, VL, ldvl,
                          VR, ldvr, n, &nout, &work[iwrk], liwrk, &rwork[irwork], &ierr );
        #elif TREVC_VERSION == 5
        magma_tally3_ctrevc3_mt_gpu( side, Magma_tally3BacktransVec, select, n, A, lda, VL, ldvl,
                              VR, ldvr, n, &nout, &work[iwrk], liwrk, &rwork[irwork], &ierr );
        #else
    ...
    if (wantvl) {
        ...
        for (i = 0; i < n; ++i) {
            scl = 1. / magma_tally3_cblas_scnrm2( n, VL(0,i), 1 );
            blasf77_csscal( &n, &scl, VL(0,i), &ione );
            for (k = 0; k < n; ++k) {
                /* Computing 2nd power */
                d__1 = MAGMA_tally3_C_REAL( *VL(k,i) );
                d__2 = MAGMA_tally3_C_IMAG( *VL(k,i) );
                rwork[irwork + k] = d__1*d__1 + d__2*d__2;
            }
            k = blasf77_isamax( &n, &rwork[irwork], &ione ) - 1;  // subtract 1; k is 0-based
            tmp = MAGMA_tally3_C_CNJG( *VL(k,i) ) / magma_tally3_ssqrt( rwork[irwork + k] );
            blasf77_cscal( &n, &tmp, VL(0,i), &ione );
            *VL(k,i) = MAGMA_tally3_C_MAKE( MAGMA_tally3_C_REAL( *VL(k,i) ), 0 );
        }
    }
    ...
    if (wantvr) {
        ...
        for (i = 0; i < n; ++i) {
            scl = 1. / magma_tally3_cblas_scnrm2( n, VR(0,i), 1 );
            blasf77_csscal( &n, &scl, VR(0,i), &ione );
            for (k = 0; k < n; ++k) {
                /* Computing 2nd power */
                d__1 = MAGMA_tally3_C_REAL( *VR(k,i) );
                d__2 = MAGMA_tally3_C_IMAG( *VR(k,i) );
                rwork[irwork + k] = d__1*d__1 + d__2*d__2;
            }
            k = blasf77_isamax( &n, &rwork[irwork], &ione ) - 1;  // subtract 1; k is 0-based
            tmp = MAGMA_tally3_C_CNJG( *VR(k,i) ) / magma_tally3_ssqrt( rwork[irwork + k] );
            blasf77_cscal( &n, &tmp, VR(0,i), &ione );
            *VR(k,i) = MAGMA_tally3_C_MAKE( MAGMA_tally3_C_REAL( *VR(k,i) ), 0 );
        }
    }
...
CLEANUP:
    ...
    work[0] = MAGMA_tally3_C_MAKE( (float) minwrk, 0. );  // TODO use optwrk as in dgeev
