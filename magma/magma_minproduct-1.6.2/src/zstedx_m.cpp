    ...
    magma_minproduct_int_t *info)
{
    ...
    magma_minproduct_int_t i, j, smlsiz;
    magma_minproduct_int_t liwmin, lrwmin;
    ...
    *info = 0;
    ...
    if (n < smlsiz) {
        ...
        magma_minproduct_dstedx_m(ngpu, range, n, vl, vu, il, iu, d, e, rwork, n,
                       rwork+n*n, lrwork-n*n, iwork, liwork, info);

        for (j=0; j < n; ++j)
            for (i=0; i < n; ++i) {
                *(Z+i+ldz*j) = MAGMA_minproduct_Z_MAKE( *(rwork+i+n*j), 0 );
            }
    }
