...
__global__ void
slascl_diag_lower(int m, int n, magma_minproductFloat_const_ptr D, int ldd, 
                                      magma_minproductFloat_ptr A, int lda)
{
...
__global__ void
slascl_diag_upper(int m, int n, magma_minproductFloat_const_ptr D, int ldd, 
                                      magma_minproductFloat_ptr A, int lda)
{
    int ind = blockIdx.x * NB + threadIdx.x;
...
/**
    ...
    ********************************************************************/
extern "C" void
magma_minproductblas_slascl_diag_q(
    ...
    magma_minproduct_int_t *info, magma_minproduct_queue_t queue )
{
    *info = 0;
    ...
    if (type == Magma_minproductLower) {
        slascl_diag_lower <<< grid, threads, 0, queue >>> (m, n, dD, lddd, dA, ldda);
    }
    else if (type == Magma_minproductUpper) {
        slascl_diag_upper <<< grid, threads, 0, queue >>> (m, n, dD, lddd, dA, ldda);
    }
}
...
{
    magma_minproductblas_slascl_diag_q( type, m, n, dD, lddd, dA, ldda, info, magma_minproduct_stream );
}
