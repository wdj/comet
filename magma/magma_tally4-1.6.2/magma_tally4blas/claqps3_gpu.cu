...
__global__ void
magma_tally4_cswap_gemv_kernel(int m, int rk, int n, const magma_tally4FloatComplex * __restrict__ V, int ldv,
                     const magma_tally4FloatComplex * __restrict__ x, int ldx, magma_tally4FloatComplex *c, magma_tally4FloatComplex *b)
{
    const int i = threadIdx.x;
    const int j = i + BLOCK_SIZE1 * blockIdx.x;
    magma_tally4FloatComplex lsum, tmp;
    ...
    if (j < m){
       ...
       if (j>=rk) 
          for(int k=0; k<n; k++)
              lsum += MAGMA_tally4_C_MUL( V[k*ldv], MAGMA_tally4_C_CNJG(x[k*ldx]));
...
__global__ void
magma_tally4_cgemv_kernel(int m, int n, const magma_tally4FloatComplex * __restrict__ V, int ldv,
                     const magma_tally4FloatComplex * __restrict__ x, magma_tally4FloatComplex *b, magma_tally4FloatComplex *c)
{
    const int i = threadIdx.x;
    const int j = i + BLOCK_SIZE1 * blockIdx.x;
    ...
    if (j < m){
        for(int k=0; k<n; k++)
            lsum += MAGMA_tally4_C_MUL( V[k*ldv], x[k]);
...
__global__
void magma_tally4_cscale_kernel(int n, magma_tally4FloatComplex* dx0,
                         magma_tally4FloatComplex *dtau, float *dxnorm, magma_tally4FloatComplex* dAkk)
{
   const int i = threadIdx.x;
   ...
   magma_tally4FloatComplex *dx = dx0;
   ...
   for( int k = i; k < n; k += BLOCK_SIZE ) {
             ...
             lsum += re*re;
        #else
             re = MAGMA_tally4_C_REAL( dx[k] );
             float im = MAGMA_tally4_C_IMAG( dx[k] );
             lsum += re*re + im*im;
        #endif
   }
   sum[i] = lsum;
            ...
            if ( beta == 0 ) {
              *dtau = MAGMA_tally4_C_ZERO;
               ...
               *dAkk    = beta;
               ...
               scale = 1. / (tmp - beta);
#else
               float alphar =  MAGMA_tally4_C_REAL(tmp), alphai = MAGMA_tally4_C_IMAG(tmp);
               beta  = -copysign( beta, alphar );

               // todo: deal with badly scaled vectors (see lapack's larfg)
               *dtau = MAGMA_tally4_C_MAKE((beta - alphar)/beta, -alphai/beta);
               *dAkk = MAGMA_tally4_C_MAKE(beta, 0.);

               tmp = MAGMA_tally4_C_MAKE( alphar - beta, alphai);
               scale = MAGMA_tally4_C_DIV( MAGMA_tally4_C_ONE, tmp);
#endif
            }
   }
...
__global__ void
magma_tally4_cgemv_kernel_adjust(int n, int k, magma_tally4FloatComplex * A, int lda, 
                          magma_tally4FloatComplex *B, int ldb, magma_tally4FloatComplex *C,
                          float *xnorm, float *xnorm2, magma_tally4FloatComplex *Akk, int *lsticc, int *lsticcs)
{
    const int i = threadIdx.x;
    ...
    if (j<n) {
      B += j;
      sum = MAGMA_tally4_C_CNJG( B[(k-1)*ldb] );
      // sum = MAGMA_tally4_C_ZERO;
      for(int m=0; m<k-1; m++) {
         sum += MAGMA_tally4_C_MUL( MAGMA_tally4_C_CNJG( B[m*ldb] ), A[m*lda] );
      }
      C[j*lda] -= sum;

      oldnorm = xnorm[j];
      temp = MAGMA_tally4_C_ABS( C[j*lda] ) / oldnorm;
...
/*
    ...
    if (blockIdx.x==0) {
       //if (2.*temp < oldnorm) {
           ...
           magma_tally4FloatComplex *dx = C+blockIdx.x*lda+1;
...
__global__ void
magma_tally4blas_scnrm2_check_kernel(int m, magma_tally4FloatComplex *da, int ldda, 
...
#define dF(i_, j_) (dF + (i_) + (j_)*(lddf))
    ...
    magma_tally4FloatComplex tauk;
    magma_tally4_int_t pvt, itemp;

    magma_tally4FloatComplex_ptr dAkk = dauxv;
    dauxv += 1;
    ...
    // float tol3z = magma_tally4_ssqrt( lapackf77_slamch("Epsilon"));
    ...
    while( k < nb && lsticc == 0 ) {
        ...
        pvt = k - 1 + magma_tally4_isamax( n-k, &dvn1[k], ione );

        if (pvt != k) {
            magma_tally4blas_cswap( k, dF(pvt,0), lddf, dF(k,0), lddf);
            itemp     = jpvt[pvt];
            jpvt[pvt] = jpvt[k];
            jpvt[k]   = itemp;
        ...
        magma_tally4_cswap_gemv_kernel<<< (m + BLOCK_SIZE1-1) / BLOCK_SIZE1, BLOCK_SIZE1, 0, magma_tally4_stream >>> 
                              ( m, rk, k, dA(0, 0), ldda, dF(k,  0), lddf, dA(0, k), dA(0,pvt));
                                 
        /*  Generate elementary reflector H(k). */
        magma_tally4_cscale_kernel<<< 1, BLOCK_SIZE, 0, magma_tally4_stream >>>
        ...
        if (k < n-1) {
            magma_tally4_cgetvector( 1, &dtau[k], 1, &tauk, 1 );
            magma_tally4_cgemv( Magma_tally4ConjTrans, m-rk, n,
                         tauk,   dA( rk,  0 ), ldda,
                                 dA( rk,  k   ), 1,
                         c_zero, dauxv, 1 );
            if (k==0) 
               magma_tally4blas_clacpy(Magma_tally4UpperLower, n-k-1, 1, dauxv+k+1, n-k-1, dF( k+1, k   ), n-k-1);
        }
        ...
        if (k > 0) {
            ...
            magma_tally4_cgemv_kernel<<< (n-k-1 + BLOCK_SIZE1 -1)/BLOCK_SIZE1, BLOCK_SIZE1, 0, magma_tally4_stream >>>
        ...
        if (k < n-1) {
            ...
            magma_tally4_cgemv_kernel_adjust<<<(n-k-1 + BLOCK_SIZE2-1)/BLOCK_SIZE2, BLOCK_SIZE2, 0, magma_tally4_stream>>>
            ...
            // can be done in the kernel directly (magma_tally4blas_scnrm2_check_kernel)
            // if (k==16) lsticc = 1;
        }
...
/*
        if (rk < min(m, n+offset)-1){
           magma_tally4blas_scnrm2_row_check_adjust(n-k-1, tol3z, &dvn1[k+1], 
...
*/
        ...
        if (k>=n-1)
           magma_tally4blas_clacpy(Magma_tally4UpperLower, 1, 1, dAkk, 1, dA(rk, k), 1);
    ...
    *kb = k + 1;
    ...
    if( lsticc > 0 ) {
        ...
        //magma_tally4blas_scnrm2_check(m-rk-1, n-*kb, A(rk+1,rk+1), lda,
        //                       &dvn1[rk+1], &dvn2[rk+1], dlsticcs);
       
        // There is a bug when we get to recompute  
        magma_tally4blas_scnrm2_check_kernel<<< n-*kb, BLOCK_SIZE >>>
