...
void init_matrix( int m, int n, double *h_A, magma_tally2_int_t lda )
{
...
double get_LU_error(magma_tally2_int_t M, magma_tally2_int_t N,
                    double *LU, magma_tally2_int_t lda,
                    magma_tally2_int_t *ipiv)
{
    magma_tally2_int_t min_mn = min(M,N);
    magma_tally2_int_t ione   = 1;
    magma_tally2_int_t i, j;
    double alpha = MAGMA_tally2_D_ONE;
    double beta  = MAGMA_tally2_D_ZERO;
    double *A, *L, *U;
    ...
    lapackf77_dlacpy( Magma_tally2LowerStr, &M, &min_mn, LU, &lda, L, &M      );
    lapackf77_dlacpy( Magma_tally2UpperStr, &min_mn, &N, LU, &lda, U, &min_mn );
    for(j=0; j<min_mn; j++)
        L[j+j*M] = MAGMA_tally2_D_MAKE( 1., 0. );
    ...
    for( j = 0; j < N; j++ ) {
        for( i = 0; i < M; i++ ) {
            LU[i+j*lda] = MAGMA_tally2_D_SUB( LU[i+j*lda], A[i+j*lda] );
        }
    }
...
int main( int argc, char** argv )
{
    ...
    double           error;
    double *h_A;
    double *d_lA[ Magma_tally2MaxGPUs ];
    magma_tally2_int_t *ipiv;
    magma_tally2_int_t M, N, n2, lda, ldda, n_local, ngpu;
    magma_tally2_int_t info, min_mn, nb, ldn_local;
    magma_tally2_int_t status = 0;

    magma_tally2_int_t P=-1;    /*Number of threads in the CPU part*/
    double d_cpu=-1;    /*pourcentgae of the matrix to allocate in the cpu part*/
    magma_tally2_int_t Pr=-1;  /*Number of threads for the panel*/
    magma_tally2_int_t async_nb; /*Block size*/
    
    double *WORK;
    magma_tally2_int_t WORK_LD, WORK_n;
    ...
    d_cpu = magma_tally2_amc_recommanded_dcpu(opts.nthread, CPU_PEAK, opts.ngpu, GPU_PEAK);
    #endif
    if(opts.fraction_dcpu!=0){ /*Overwrite the one computed with the model*/
    d_cpu = opts.fraction_dcpu;
    }
    magma_tally2_assert(d_cpu > 0 && d_cpu<=1.0,
    ...
    for( int i = 0; i < opts.ntest; ++i ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            n2     = lda*N;
            ldda   = ((M+31)/32)*32;
            //nb     = magma_tally2_get_dgetrf_nb( M );
            ...
            magma_tally2_setlapack_numthreads(P);
            /* =====================================================================
            ...
            init_matrix( M, N, h_A, lda );
            magma_tally2_dsetmatrix_1D_col_bcyclic( M, N, h_A, lda, d_lA, ldda, ngpu, nb );
    
            gpu_time1 = magma_tally2_wtime();
            magma_tally2_dgetrf_mgpu( ngpu, M, N, d_lA, ldda, ipiv, &info );
            gpu_time1 = magma_tally2_wtime() - gpu_time1;
            gpu_perf1 = gflops / gpu_time1;
            if (info != 0)
                printf("magma_tally2_dgetrf_mgpu returned error %d: %s.\n",
                       (int) info, magma_tally2_strerror( info ));
                       
            magma_tally2_dgetmatrix_1D_col_bcyclic( M, N, d_lA, ldda, h_A, lda, ngpu, nb );
            ...
            /* ====================================================================
               Performs operation using MAGMA_tally2_Async: This interface allocate workspace internally
               =================================================================== */

            /*For the benchmark we have 2 approaches*/
            /*1. use directly magma_tally2_amc */
            /*2. use magma_tally2_amc_work and add pinned memory time*/
            ...
            magma_tally2_setlapack_numthreads(1);

            magma_tally2_amc_init(P, d_cpu, Pr, nb);
            gpu_time2 = magma_tally2_wtime();
            magma_tally2_dgetrf_async_mgpu( ngpu, M, N, d_lA, ldda, ipiv, &info );
            gpu_time2 = magma_tally2_wtime() - gpu_time2;
            gpu_perf2 = gflops / gpu_time2;
            magma_tally2_amc_finalize();
            if (info != 0)
                printf("magma_tally2_dgetrf_mgpu returned error %d: %s.\n",
                       (int) info, magma_tally2_strerror( info ));
            ...
            */
            ...
            /* ====================================================================
               Performs operation using MAGMA_tally2_Async_Work
               =================================================================== */
            ...
            //Allocate workspace
            alloc_time = magma_tally2_wtime();
            if (MAGMA_tally2_SUCCESS != magma_tally2_dmalloc_pinned(&WORK, WORK_LD*WORK_n)) { 
            //if (MAGMA_tally2_SUCCESS != magma_tally2_dmalloc_cpu(&WORK, WORK_LD*WORK_n)) {
                info = MAGMA_tally2_ERR_HOST_ALLOC;
                printf("magma_tally2_dmalloc_pinned returned error %d: %s.\n     ", (int) info);
            }
            ...
            dlpanelT_n = M;
             dlpanelT = (double **)    malloc(ngpu*sizeof(double*));
              for(int dev=0;dev<ngpu;dev++){
                 ...
                 if (MAGMA_tally2_SUCCESS != magma_tally2_dmalloc(&dlpanelT[dev], dlpanelT_m*dlpanelT_n)) { 
            
                        info = MAGMA_tally2_ERR_DEVICE_ALLOC; 
                        printf("magma_tally2_dmalloc returned error %d: %s.\n     ", (int) info);
                }
              }

            alloc_time = magma_tally2_wtime() - alloc_time;

            //First touch the workspace with each thread. This may be needed to avoid using numactl --interleave
            //magma_tally2_amc_dmemset(WORK, 0.0, WORK_LD*WORK_n, 256, P); //nb
            ...
            magma_tally2_amc_init(P, d_cpu, Pr, nb);
            gpu_time3 = magma_tally2_wtime();
            magma_tally2_dgetrf_mgpu_work_amc( ngpu, M, N, d_lA, ldda, ipiv, &info, WORK, WORK_LD, WORK_n, dlpanelT, dlpanelT_m, dlpanelT_n);
            gpu_time3 = magma_tally2_wtime() - gpu_time3;
            ...
            //Free workspace
            free_time = magma_tally2_wtime();
            magma_tally2_free_pinned(WORK);
            free_time = magma_tally2_wtime() - free_time;

            for(int dev=0;dev<ngpu;dev++){
                magma_tally2_setdevice(dev);
                magma_tally2_free(dlpanelT[dev]);
            }
