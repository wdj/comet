...
/*
*
*  magma_tally4_dgetrf_mgpu_amc:
*
*  This function allocate the workspace and call magma_tally4_dgetrf_mgpu_amc
...
/*  -- MAGMA_tally4 (version 1.6.1) -- 
    ...
    magma_tally4_dgetrf_mgpu_amc computes an LU factorization of a general M-by-N matrix A 
...
//    ca_dbg_printMat_transpose_gpu(m, n, dA, dA_LD,"dA = LU"); 
    ...
    return *info; 
}   /* End of magma_tally4_dgetrf_mgpu_amc */
...
/*
*
*  magma_tally4_dgetrf_mgpu_work_amc
*
...
/*  -- MAGMA_tally4 (version 1.6.1) -- 
    ...
    magma_tally4_int_t nrows, ncols; 
     ...
     magma_tally4_int_t I, J, K, M, N, U_K; 
...
for(dd=0;dd<num_gpus;dd++){
    ...
    t1 = magma_tally4_wtime();
     ...
     for(I=K+1; I<=M-1; I+=B_rows){ 
        ...
        magma_tally4_insert_core_dtrsm_gatherv('R', 'U', 'N', 'N', i_nrows, nb, c_one, A(0,K), A_LD, A(I,K), A_LD, colptr(K));
         ...
         for(J=K+A_N;J<min(K+A_N+num_gpus,N);J++){
          ...
          for(J=K+1;J<=min(K+A_N-1, N-1);J++){ 
                   ...
                   //// magma_tally4_schedule_set_task_priority(INT_MAX - N*K - 2);
                    ...
                    B_rows = max(B_rows,4); /*maximun of 4*/ 
                     ...
                     for(I=K+2; I<=M-1; I+=B_rows){ 
                        ...
                        i_nrows = min(B_rows*nb, m-I*nb);
                        magma_tally4_insert_core_dtrsm_gatherv('R', 'U', 'N', 'N', i_nrows, ncols, c_one, A(K+1,K+1), A_LD, A(I,K+1), A_LD, colptr(K+1));
           ...
           if(K+A_N<N) { 
             ...
             magma_tally4_insert_dev_queue_sync(dd, mstream[dd][1], dlAT(K+1,K+A_N)); 
             magma_tally4_insert_dev_dgetmatrix_async_transpose(dd, gpu_nrows, ncols, dlAT(K+1,K+A_N), dlAT_LD, A(K+1,K+A_N), A_LD, mstream[dd][2], dlAP_get[dd], dlAP_LD, colptr(K+A_N));
