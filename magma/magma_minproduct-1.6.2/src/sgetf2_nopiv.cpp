/*
    -- MAGMA_minproduct (version 1.6.1) --
       ...
       @generated from zgetf2_nopiv.cpp normal z -> s, Fri Jan 30 19:00:14 2015
    ...
    magma_minproduct_int_t *info)
{
    ...
    magma_minproduct_int_t min_mn, i__2, i__3;
    float z__1;
    ...
    *info = 0;
    ...
    for (j = 1; j <= min_mn; ++j) {
        ...
        if ( ! MAGMA_minproduct_S_EQUAL( *A(j,j), c_zero)) {
            ...
            if (j < m) {
                if (MAGMA_minproduct_S_ABS( *A(j,j) ) >= sfmin) {
                    i__2 = m - j;
                    z__1 = MAGMA_minproduct_S_DIV(c_one, *A(j,j));
                    blasf77_sscal(&i__2, &z__1, A(j+1,j), &ione);
                }
                else {
                    i__2 = m - j;
                    for (i = 1; i <= i__2; ++i) {
                        *A(j+i,j) = MAGMA_minproduct_S_DIV( *A(j+i,j), *A(j,j) );
                    }
                }
            }
        }
