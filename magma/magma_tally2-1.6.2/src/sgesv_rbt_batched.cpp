                  ...
                  magma_tally2_int_t batchCount, magma_tally2_queue_t queue)
{
    ...
    float *hu, *hv;
    if (MAGMA_tally2_SUCCESS != magma_tally2_smalloc_cpu( &hu, 2*n )) {
        info = MAGMA_tally2_ERR_HOST_ALLOC;
        return info;
    }

    if (MAGMA_tally2_SUCCESS != magma_tally2_smalloc_cpu( &hv, 2*n )) {
        info = MAGMA_tally2_ERR_HOST_ALLOC;
        return info;
    }
    ...
    info = magma_tally2_sgerbt_batched(Magma_tally2True, n, nrhs, dA_array, n, dB_array, n, hu, hv, &info, batchCount, queue);
    if (info != MAGMA_tally2_SUCCESS)  {
        return info;
    }


    info = magma_tally2_sgetrf_nopiv_batched( n, n, dA_array, ldda, info_array, batchCount, queue);
    if ( (info != MAGMA_tally2_SUCCESS) ){
        return info;
    }
    ...
    info = magma_tally2_sgetrs_nopiv_batched( Magma_tally2NoTrans, n, nrhs, dA_array, ldda, dB_array, lddb, info_array, batchCount, queue );
    ...
    magma_tally2_ssetvector(2*n, hv, 1, dv, 1);

    for(int i = 0; i < nrhs; i++)
        magma_tally2blas_sprbt_mv_batched(n, dv, dB_array+(i), batchCount, queue);
