...
//
// We work around this bug by putting replacement routines in the magma_minproduct/lib/libblas_fix.a library.
...
float test( magma_minproduct_int_t m, magma_minproduct_int_t n )
{
...
#define dA(i,j) (dA + (i) + (j)*lda)
    ...
    magma_minproduct_int_t lda = MAX(m,n);
    ...
    for( magma_minproduct_int_t i = 0; i < lda; ++i ) {
    ...
    double error;
    magma_minproduct_int_t status;
    
    // can repeat multiple times, but shows same results every time
    status = 0;
    for( magma_minproduct_int_t i=0; i < 1; ++i ) {
...
{
    magma_minproduct_int_t m = 100;
    magma_minproduct_int_t n = m;
    if ( argc > 1 ) {
        n = atoi( argv[1] );
    }
