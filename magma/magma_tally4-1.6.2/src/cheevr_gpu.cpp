...
*******     (workspace) If FAST_HEMV is defined DZ should be (LDDZ, max(1,N)) in both cases.
    ...
    isuppz  INTEGER ARRAY, dimension ( 2*max(1,M) )
            ...
            ISUPPZ( 2*i ).
            __Implemented only for__ RANGE = Magma_tally4RangeAll or Magma_tally4RangeI and IU - IL = N - 1
    ...
    ldwz    INTEGER
            ...
            JOBZ = Magma_tally4Vec, LDWZ >= max(1,N).
...
magma_tally4_cheevr_gpu(
    ...
    float *w,
    magma_tally4FloatComplex_ptr dZ, magma_tally4_int_t lddz, magma_tally4_int_t *isuppz,
    ...
    magma_tally4_int_t *info)
{
    ...
    magma_tally4_int_t indrd, indre;
    magma_tally4_int_t imax;
    magma_tally4_int_t lopt, itmp1, indree, indrdd;
    magma_tally4_int_t lower, wantz, tryrac;
    magma_tally4_int_t i, j, jj, i__1;
    magma_tally4_int_t alleig, valeig, indeig;
    magma_tally4_int_t iscale, indibl, indifl;
    magma_tally4_int_t indiwo, indisp, indtau;
    magma_tally4_int_t indrwk, indwk;
    magma_tally4_int_t llwork, llrwork, nsplit;
    magma_tally4_int_t lquery, ieeeok;
    ...
    *m = 0;
    ...
    if (n <= 128) {
        ...
        magma_tally4_cmalloc_cpu( &a, n*n );
        magma_tally4_cgetmatrix(n, n, dA, ldda, a, n);
        lapackf77_cheevr(jobz_, range_, uplo_,
                         &n, a, &n, &vl, &vu, &il, &iu, &abstol, m,
                         w, wZ, &ldwz, isuppz, work, &lwork,
                         rwork, &lrwork, iwork, &liwork, info);
        magma_tally4_csetmatrix( n,  n,  a,    n, dA, ldda);
        magma_tally4_csetmatrix( n, *m, wZ, ldwz, dZ, lddz);
        magma_tally4_free_cpu(a);
        return *info;
    }

    if (MAGMA_tally4_SUCCESS != magma_tally4_smalloc( &dwork, n )) {
        fprintf (stderr, "!!!! device memory allocation error (magma_tally4_cheevr_gpu)\n");
    ...
    anrm = magma_tally4blas_clanhe(Magma_tally4MaxNorm, uplo, n, dA, ldda, dwork);
    iscale = 0;
    sigma  = 1;
    if (anrm > 0. && anrm < rmin) {
        iscale = 1;
    ...
    if (iscale == 1) {
        d__1 = 1.;
        magma_tally4blas_clascl(uplo, 0, 0, 1., sigma, n, n, dA,
                         ldda, info);
...
#ifdef FAST_HEMV
    magma_tally4_chetrd2_gpu(uplo, n, dA, ldda, &rwork[indrd], &rwork[indre],
                      &work[indtau], wA, ldwa, &work[indwk], llwork, dZ, lddz*n, &iinfo);
#else
    magma_tally4_chetrd_gpu (uplo, n, dA, ldda, &rwork[indrd], &rwork[indre],
                      &work[indtau], wA, ldwa, &work[indwk], llwork, &iinfo);
#endif

    lopt = n + (magma_tally4_int_t)MAGMA_tally4_C_REAL(work[indwk]);
    ...
    if (! wantz) {
        ...
        if (*info == 0 && wantz) {
            magma_tally4_csetmatrix( n, *m, wZ, ldwz, dZ, lddz );
            magma_tally4_cunmtr_gpu(Magma_tally4Left, uplo, Magma_tally4NoTrans, n, *m, dA, ldda, &work[indtau],
    ...
    if (wantz) {
        for (j = 1; j <= *m-1; ++j) {
            ...
            if (i != 0) {
                ...
                magma_tally4_cswap(n, dZ + (i-1)*lddz, ione, dZ + (j-1)*lddz, ione);
            }
        }
    }
    
    /* Set WORK[0] to optimal complex workspace size. */
    work[1] = MAGMA_tally4_C_MAKE( lopt, 0 );
