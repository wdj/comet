/*
    -- MAGMA_tally4 (version 1.6.1) --
       ...
       @generated from zhemv.cu normal z -> s, Fri Jan 30 19:00:09 2015
...
/**
    Purpose
    -------
    magma_tally4blas_ssymv_work performs the matrix-vector operation:
    ...
    MAGMA_tally4 implements ssymv through two steps:
    ...
    magma_tally4blas_ssymv is a wrapper routine allocating the workspace inside the
    routine and provides the same interface as cublas.
    
    If users need to call ssymv frequently, we suggest using
    magma_tally4blas_ssymv_work instead of magma_tally4blas_ssymv. As the overhead to
    allocate and free in device memory in magma_tally4blas_ssymv would hurt performance.
    ...
    magma_tally4_queue_t queue )
{
    ...
    if ( arch < 200 ) {
        magma_tally4_ssymv( uplo, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
     ...
     */
    if ( (n == 0) || ( MAGMA_tally4_S_EQUAL(alpha, MAGMA_tally4_S_ZERO) && MAGMA_tally4_S_EQUAL(beta, MAGMA_tally4_S_ONE) ) )
        return info;
    ...
    if ( upper ) {
        ssymv_kernel_U<<< grid, threads, 0, queue >>>
            (n, dA, ldda, dx, incx, dwork);
        ssymv_kernel_U_sum<<< grid, threads_sum, 0, queue >>>
            (n, alpha, ldda, beta, dy, incy, dwork);
    }
    else {
        ssymv_kernel_L<<< grid, threads, 0, queue >>>
            (n, dA, ldda, dx, incx, dwork);
        ssymv_kernel_L_sum<<< grid, threads_sum, 0, queue >>>
            (n, alpha, ldda, beta, dy, incy, dwork);
    }
    
    return info;
}
// end magma_tally4blas_ssymv_work


/**
    Purpose
    -------
    magma_tally4blas_ssymv performs the matrix-vector operation:
    ...
    magma_tally4Float_ptr dy, magma_tally4_int_t incy)
{
    ...
    magma_tally4_smalloc( &dwork, lwork );
    if ( dwork == NULL ) {
    ...
    magma_tally4blas_ssymv_work( uplo, n, alpha, dA, ldda, dx, incx, beta, dy, incy,
