...
int main( int argc, char** argv)
{
    ...
    magma_tally2DoubleComplex *h_A, *h_B, *h_R;
    magma_tally2DoubleComplex_ptr d_A, d_B;
    magma_tally2_int_t M, N, size, lda, ldda, ldb, lddb;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            gbytes = sizeof(magma_tally2DoubleComplex) * 2.*M*N / 1e9;
            
            TESTING_MALLOC_CPU( h_A, magma_tally2DoubleComplex, lda*N  );  // input:  M x N
            TESTING_MALLOC_CPU( h_B, magma_tally2DoubleComplex, ldb*M  );  // output: N x M
            TESTING_MALLOC_CPU( h_R, magma_tally2DoubleComplex, ldb*M  );  // output: N x M
            
            TESTING_MALLOC_DEV( d_A, magma_tally2DoubleComplex, ldda*N );  // input:  M x N
            TESTING_MALLOC_DEV( d_B, magma_tally2DoubleComplex, lddb*M );  // output: N x M
            
            /* Initialize the matrix */
            for( int j = 0; j < N; ++j ) {
                for( int i = 0; i < M; ++i ) {
                    h_A[i + j*lda] = MAGMA_tally2_Z_MAKE( i + j/10000., j );
                }
            }
            for( int j = 0; j < M; ++j ) {
                for( int i = 0; i < N; ++i ) {
                    h_B[i + j*ldb] = MAGMA_tally2_Z_MAKE( i + j/10000., j );
                }
            }
            magma_tally2_zsetmatrix( N, M, h_B, ldb, d_B, lddb );
            ...
            /* ====================================================================
               Performs operation using MAGMA_tally2, out-of-place
               =================================================================== */
            magma_tally2_zsetmatrix( M, N, h_A, lda, d_A, ldda );
            magma_tally2_zsetmatrix( N, M, h_B, ldb, d_B, lddb );
            
            gpu_time = magma_tally2_sync_wtime( 0 );
            //magma_tally2blas_ztranspose( M-2, N-2, d_A+1+ldda, ldda, d_B+1+lddb, lddb );  // inset by 1 row & col
            magma_tally2blas_ztranspose( M, N, d_A, ldda, d_B, lddb );
            gpu_time = magma_tally2_sync_wtime( 0 ) - gpu_time;
            gpu_perf = gbytes / gpu_time;
            
            /* ====================================================================
               Performs operation using MAGMA_tally2, in-place
               =================================================================== */
            if ( M == N ) {
                magma_tally2_zsetmatrix( M, N, h_A, lda, d_A, ldda );
                
                gpu_time2 = magma_tally2_sync_wtime( 0 );
                //magma_tally2blas_ztranspose_inplace( N-2, d_A+1+ldda, ldda );  // inset by 1 row & col
                magma_tally2blas_ztranspose_inplace( N, d_A, ldda );
