...
/* ////////////////////////////////////////////////////////////////////////////
   -- Testing magma_tally3_dsymm_mgpu
*/
int main( int argc, char** argv)
{
    ...
    double calpha    = MAGMA_tally3_D_MAKE( 3.456, 5.678 );
    double cbeta     = MAGMA_tally3_D_MAKE( 1.234, 2.456 );
    ...
    double *hA, *hX, *hB, *hR;
    magma_tally3Double_ptr dA[Magma_tally3MaxGPUs], dX[Magma_tally3MaxGPUs], dB[Magma_tally3MaxGPUs], dwork[Magma_tally3MaxGPUs], hwork[Magma_tally3MaxGPUs+1];
    magma_tally3Double_ptr dA2;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int offset = 0; offset < N; offset += min(N,nb) ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            ldda  = ((M + 31)/32)*32;
            size  = lda*M;
            gflops = FLOPS_DSYMM( Magma_tally3Left, (double)msize, (double)N ) / 1e9;
            ...
            magma_tally3_dmake_symmetric( M, hA, lda );
            ...
            /* ====================================================================
               ...
               =================================================================== */
            magma_tally3_dsetmatrix_1D_col_bcyclic( M, M, hA, lda, dA, ldda, opts.ngpu, nb );
            for( int d = 0; d < opts.ngpu; ++d ) {
                magma_tally3_setdevice( d );
                //magma_tally3blasSetKernelStream( streams[ d ][  0 ] );
                magma_tally3_dsetmatrix( M, N, hX, lda, dX[d], ldda );
                //if (d == 0) magma_tally3_dsetmatrix( M, N, hB, lda, dB[d], ldda ); // this is wrong coz when offset != 0 the gpu who do the beta*C may be not 0 so this should be related to stdev(starting device who own i=0 first col)
                magma_tally3_dsetmatrix( M, N, hB, lda, dB[d], ldda );
            }
            ...
            if ( opts.check && iter == 0 ) {
                ...
                magma_tally3_dsetmatrix( M, M, hA, lda, dA2, ldda );
                magma_tally3_dsetmatrix( M, N, hX, lda, dX[0], ldda );
                magma_tally3_dsetmatrix( M, N, hB, lda, dwork[0], ldda );
            ...
            if ( opts.check ) {
                ...
                //printf( "A =" ); magma_tally3_dprint( M, M, hA, lda );
                //printf( "X =" ); magma_tally3_dprint( M, N, hX, lda );
                //printf( "B =" ); magma_tally3_dprint( M, N, hB, lda );
                ...
                */
                ...
                for(magma_tally3_int_t dev=0; dev < opts.ngpu; ++dev) {
                    ...
                    magma_tally3_dgetmatrix( M, N, dB[dev], ldda, hR, lda );
                    ...
                    //printf( "R ="  ); magma_tally3_dprint( M, N, hR, lda );
                    if (firstprint == 0) {
