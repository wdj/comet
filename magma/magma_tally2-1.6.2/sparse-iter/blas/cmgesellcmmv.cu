    ...
    magma_tally2FloatComplex * dy)
{
    ...
    if(row < num_rows ){
        magma_tally2FloatComplex dot1 = MAGMA_tally2_C_MAKE(0.0, 0.0);
        magma_tally2FloatComplex dot2 = MAGMA_tally2_C_MAKE(0.0, 0.0);
        int offset = drowptr[ bdx ];
        int max_ = (drowptr[ bdx+1 ]-offset)/blocksize;  
            // number of elements each thread handles
    ...
    magma_tally2_queue_t queue )
{
    ...
    if ( (texture==1) && (precision==1) && (kepler==1) ) {
        ...
        int Ms = num_vecs * blocksize*alignment * sizeof( magma_tally2FloatComplex );


        if ( alignment == 1) {
            dim3 block( blocksize, num_vecs/2, 1 );
            if ( beta == MAGMA_tally2_C_MAKE( 0.0, 0.0 ) )
