    ...
    magma_tally2_int_t *info )
{
    ...
    magma_tally2DoubleComplex *dwork[Magma_tally2MaxGPUs]={NULL}, *dpanel[Magma_tally2MaxGPUs]={NULL};
    magma_tally2DoubleComplex *hwork=NULL, *hpanel=NULL;
    ...
    *info = 0;
    ...
    for( dev=0; dev < ngpu; dev++ ) {
        ...
        if ( MAGMA_tally2_SUCCESS != magma_tally2_zmalloc( &(dwork[dev]), (lddwork + ldda)*nb )) {
    ...
    if ( MAGMA_tally2_SUCCESS != magma_tally2_zmalloc_pinned( &hwork, lwork )) {
    ...
    if ( nb < min_mn ) {
        ...
        for( i = 0; i < min_mn-nb; i += nb ) {
            ...
            zpanel_to_q_tally2( Magma_tally2Upper, ib, hpanel(i), ldhpanel, hwork + ib*ib );
            ...
            for( dev=0; dev < ngpu; dev++ ) {
                ...
                magma_tally2_zsetmatrix_async( rows, ib,
                                        hpanel(i),   ldhpanel,
            ...
            // TODO: if zpanel_to_q_tally2 copied whole block, wouldn't need to restore
            // -- just send the copy to the GPUs.
            // TODO: also, could zero out the lower triangle and use Azzam's larfb w/ gemm.
            
            /* Restore the panel */
            zq_to_panel_tally2( Magma_tally2Upper, ib, hpanel(i), ldhpanel, hwork + ib*ib );
    ...
    if (i < min_mn) {
        ...
        for( j=i; j < n; j += nb ) {
            ...
            ib = min( n-j, nb );
            magma_tally2_setdevice( panel_dev );
            magma_tally2_zgetmatrix( rows, ib,
                              dlA(panel_dev, i, i_local), ldda,
                              hwork + (j-i)*rows,         rows );
        }
        ...
        for( j=i; j < n; j += nb ) {
            ...
            ib = min( n-j, nb );
            magma_tally2_setdevice( panel_dev );
            magma_tally2_zsetmatrix( rows, ib,
                              hwork + (j-i)*rows,         rows,
                              dlA(panel_dev, i, i_local), ldda );
        }
    }
