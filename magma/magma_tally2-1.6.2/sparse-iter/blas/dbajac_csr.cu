...
__global__ void
magma_tally2_dbajac_csr_ls_kernel(int localiters, int n, 
                            double * valD, 
...
__global__ void
magma_tally2_dbajac_csr_kernel(    
    int n, 
    double * valD, 
    magma_tally2_index_t * rowD, 
    magma_tally2_index_t * colD, 
    ...
    magma_tally2_queue_t queue )
{
    ...
    if ( R.nnz > 0 ) { 
        if ( localiters == 1 )
        magma_tally2_dbajac_csr_kernel<<< grid, block, 0, queue >>>
            ( D.num_rows, D.dval, D.drow, D.dcol, 
                            R.dval, R.drow, R.dcol, b.dval, x->dval );
        else
            magma_tally2_dbajac_csr_ls_kernel<<< grid, block, 0, queue >>>
