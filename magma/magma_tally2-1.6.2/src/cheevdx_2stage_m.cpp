...
/**
    ...
    @ingroup magma_tally2_cheev_driver
    ********************************************************************/
    ...
    magma_tally2_int_t *info)
{
    ...
    *info = 0;
    ...
    magma_tally2_int_t nb = magma_tally2_get_cbulge_nb(n, parallel_threads);
    magma_tally2_int_t Vblksiz = magma_tally2_cbulge_get_Vblksiz(n, nb, parallel_threads);
    ...
    magma_tally2_int_t lq2 = magma_tally2_cbulge_get_lq2(n, parallel_threads);
    ...
    if (n == 1) {
        w[0] = MAGMA_tally2_C_REAL(A[0]);
        if (wantz) {
            A[0] = MAGMA_tally2_C_ONE;
        }
        return *info;
    }
    ...
    magma_tally2_timer_t time=0, time_total=0, time_alloc=0, time_dist=0, time_band=0;
    timer_start( time_total );

#ifdef HE2HB_SINGLEGPU
    magma_tally2FloatComplex *dT1;
    if (MAGMA_tally2_SUCCESS != magma_tally2_cmalloc( &dT1, n*nb)) {
        *info = MAGMA_tally2_ERR_DEVICE_ALLOC;
        return *info;
    }
    timer_start( time_band );
    magma_tally2_chetrd_he2hb(uplo, n, nb, A, lda, &work[indtau1], &work[indwrk], llwork, dT1, info);
    timer_stop( time_band );
    timer_printf( "    1 GPU seq code time chetrd_he2hb only = %7.4f\n", time_band );
    magma_tally2_free(dT1);
#else
    magma_tally2_int_t nstream = max(3,ngpu+2);
    magma_tally2_queue_t streams[Magma_tally2MaxGPUs][20];
    magma_tally2FloatComplex *da[Magma_tally2MaxGPUs], *dT1[Magma_tally2MaxGPUs];
    magma_tally2_int_t ldda = ((n+31)/32)*32;

    magma_tally2_int_t ver = 0;
    magma_tally2_int_t distblk = max(256, 4*nb);
    ...
    for( magma_tally2_int_t dev = 0; dev < ngpu; ++dev ) {
        magma_tally2_int_t mlocal = ((n / distblk) / ngpu + 1) * distblk;
        magma_tally2_setdevice( dev );
        // TODO check malloc
        magma_tally2_cmalloc(&da[dev], ldda*mlocal );
        magma_tally2_cmalloc(&dT1[dev], (n*nb) );
    ...
    magma_tally2_csetmatrix_1D_col_bcyclic( n, n, A, lda, da, ldda, ngpu, distblk );
    magma_tally2_setdevice(0);
    timer_stop( time_dist );

    timer_start( time_band );
    if (ver == 30) {
        magma_tally2_chetrd_he2hb_mgpu_spec(uplo, n, nb, A, lda, &work[indtau1], &work[indwrk], llwork, da, ldda, dT1, nb, ngpu, distblk, streams, nstream, info);
    } else {
        magma_tally2_chetrd_he2hb_mgpu(uplo, n, nb, A, lda, &work[indtau1], &work[indwrk], llwork, da, ldda, dT1, nb, ngpu, distblk, streams, nstream, info);
    }
    timer_stop( time_band );
    ...
    magma_tally2FloatComplex* A2 = &work[indwrk];
    memset(A2, 0, n*lda2*sizeof(magma_tally2FloatComplex));

    for (magma_tally2_int_t j = 0; j < n-nb; j++) {
        len = nb+1;
        blasf77_ccopy( &len, A(j,j), &ione, A2(0,j), &ione );
        memset(A(j,j), 0, (nb+1)*sizeof(magma_tally2FloatComplex));
        *A(nb+j,j) = c_one;
    }
    for (magma_tally2_int_t j = 0; j < nb; j++) {
        len = nb-j;
        blasf77_ccopy( &len, A(j+n-nb,j+n-nb), &ione, A2(0,j+n-nb), &ione );
        memset(A(j+n-nb,j+n-nb), 0, (nb-j)*sizeof(magma_tally2FloatComplex));
    }
    ...
    magma_tally2_chetrd_hb2st(uplo, n, nb, Vblksiz, A2, lda2, w, &rwork[inde], &work[indV2], ldv, &work[indTAU2], wantz, &work[indT2], ldt);
    ...
    if (! wantz) {
        ...
        magma_tally2_smove_eig(range, n, w, &il, &iu, vl, vu, m);
        ...
        magma_tally2_cstedx_m(ngpu, range, n, vl, vu, il, iu, w, &rwork[inde],
        ...
        magma_tally2_smove_eig(range, n, w, &il, &iu, vl, vu, m);
/*
        magma_tally2FloatComplex *dZ;
        magma_tally2_int_t lddz = n;

        if (MAGMA_tally2_SUCCESS != magma_tally2_cmalloc( &dZ, *m*lddz)) {
            *info = MAGMA_tally2_ERR_DEVICE_ALLOC;
            return *info;
        }

        magma_tally2_cbulge_back(uplo, n, nb, *m, Vblksiz, &work[indwrk + n * (il-1)], n, dZ, lddz,
                          &work[indV2], ldv, &work[indTAU2], &work[indT2], ldt, info);

        magma_tally2_cgetmatrix( n, *m, dZ, lddz, &work[indwrk], n);

        magma_tally2_free(dZ);

*/

        magma_tally2_cbulge_back_m(ngpu, uplo, n, nb, *m, Vblksiz, &work[indwrk + n * (il-1)], n,
                            &work[indV2], ldv, &work[indTAU2], &work[indT2], ldt, info);
        ...
        magma_tally2_cunmqr_m(ngpu, Magma_tally2Left, Magma_tally2NoTrans, n-nb, *m, n-nb, A+nb, lda, &work[indtau1],
