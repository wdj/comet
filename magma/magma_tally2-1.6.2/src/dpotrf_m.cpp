...
#define dTup(d, i, j) (   dt[(d)] + (j)*nb    + (i))
    ...
    double *dwork[Magma_tally2MaxGPUs], *dt[Magma_tally2MaxGPUs];
    magma_tally2_int_t     ldda, lddla, nb, iinfo, n_local[Magma_tally2MaxGPUs], J2, d, ngpu0 = ngpu;
    magma_tally2_int_t     j, jj, jb, J, JB, NB, h;
    magma_tally2_queue_t   stream[Magma_tally2MaxGPUs][3];
    magma_tally2_event_t   event[Magma_tally2MaxGPUs][5];
    magma_tally2_timer_t time_total=0, time_sum=0, time=0;
    ...
    *info = 0;
    ...
    NB = (magma_tally2_int_t)((0.8*freeMem - max(2,ngpu)*nb*ldda - (n+nb)*nb)/lddla); /* number of columns in the big panel */
    //NB = min(5*nb,n);
    ...
    for (d=0; d < ngpu; d++ ) {
        ...
        if (MAGMA_tally2_SUCCESS != magma_tally2_dmalloc( &dt[d], NB*lddla + max(2,ngpu)*nb*ldda )) {
        ...
        magma_tally2_device_sync(); // synch the device
    }
    ...
    if (upper) {
        ...
        for( J=0; J < n; J += NB ) {
            ...
            magma_tally2_dhtodpo( ngpu, uplo, JB, n, J, J, nb, A, lda, dwork, NB, stream, &iinfo);
            ...
            for( j=0; j < J; j += nb ) {
                ...
                for( d=0; d < ngpu; d++ ) {
                    magma_tally2_setdevice(d);
                    magma_tally2_dsetmatrix_async( nb, JB,
                                            A(j, J),       lda,
                                            dTup(d, 0, J), nb,
                                            stream[d][0] );
                    n_local[d] = 0;
                ...
                for( jj=0; jj < JB; jj += nb ) { // jj is 'local' column index within the big panel
                    ...
                    magma_tally2_setdevice(d);
                    magma_tally2blasSetKernelStream(stream[d][J2%2]); // the last stream (2) used to process off-diagonal
                    J2 = nb*J2;
                    ...
                    magma_tally2_dsyrk(Magma_tally2Upper, Magma_tally2ConjTrans, jb, nb,
                ...
                if ( n > J+JB ) {
                    for( d=0; d < ngpu; d++ ) {
                        magma_tally2_setdevice(d);
                        magma_tally2blasSetKernelStream(stream[d][2]);
                ...
                for( d=0; d < ngpu; d++ ) {
                    magma_tally2_setdevice(d);
                    for( jj=0; jj < 3; jj++ )
                        magma_tally2_queue_sync( stream[d][jj] );
            ...
            // using three streams
            magma_tally2_dpotrf3_mgpu(ngpu, uplo, JB, n-J, J, J, nb,
            ...
            magma_tally2_ddtohpo(ngpu, uplo, JB, n, J, J, nb, NB, A, lda, dwork, NB, stream, &iinfo);
            //magma_tally2_ddtohpo(ngpu, uplo, JB, n, J, J, nb, 0, A, lda, dwork, NB, stream, &iinfo);
        }
    } else {
        /* ========================================================= *
        ...
        for( J=0; J < n; J += NB ) {
            ...
            magma_tally2_dhtodpo( ngpu, uplo, n, JB, J, J, nb, A, lda, dwork, lddla, stream, &iinfo);
            ...
            for( j=0; j < J; j += nb ) {
                ...
                for( d=0; d < ngpu; d++ ) {
                    magma_tally2_setdevice(d);
                    magma_tally2_dsetmatrix_async( JB, nb,
                                            A(J, j),     lda,
                                            dT(d, J, 0), ldda,
                                            stream[d][0] );
                    n_local[d] = 0;
                ...
                for( jj=0; jj < JB; jj += nb ) { /* diagonal */
                    ...
                    magma_tally2_setdevice(d);
                    magma_tally2blasSetKernelStream(stream[d][J2%2]);
                    ...
                    magma_tally2_dsyrk(Magma_tally2Lower, Magma_tally2NoTrans, jb, nb,
            ...
            h = (JB+nb-1)/nb; // big diagonal of big panel will be on CPU
            // using three streams
            magma_tally2_dpotrf3_mgpu(ngpu, uplo, n-J, JB, J, J, nb,
            ...
            magma_tally2_ddtohpo( ngpu, uplo, n, JB, J, J, nb, JB, A, lda, dwork, lddla, stream, &iinfo);
