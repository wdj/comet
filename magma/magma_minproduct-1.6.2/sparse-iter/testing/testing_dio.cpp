...
int main(  int argc, char** argv )
{
    ...
    real_Double_t res;
    magma_minproduct_d_matrix A={Magma_minproduct_CSR}, A2={Magma_minproduct_CSR}, 
    ...
    while(  i < argc ) {
        ...
        // write to file
        CHECK( magma_minproduct_dwrite_csrtomtx( A, filename, queue ));
        // read from file
        CHECK( magma_minproduct_d_csr_mtx( &A2, filename, queue ));
        ...
        //visualize
        printf("A2:\n");
        CHECK( magma_minproduct_dprint_matrix( A2, queue ));
        
        //visualize
        CHECK( magma_minproduct_dmconvert(A2, &A4, Magma_minproduct_CSR, Magma_minproduct_CSRL, queue ));
        printf("A4:\n");
        CHECK( magma_minproduct_dprint_matrix( A4, queue ));
        CHECK( magma_minproduct_dmconvert(A4, &A5, Magma_minproduct_CSR, Magma_minproduct_ELL, queue ));
        printf("A5:\n");
        CHECK( magma_minproduct_dprint_matrix( A5, queue ));

        // pass it to another application and back
        magma_minproduct_int_t m, n;
        magma_minproduct_index_t *row, *col;
        double *val=NULL;
        CHECK( magma_minproduct_dcsrget( A2, &m, &n, &row, &col, &val, queue ));
        CHECK( magma_minproduct_dcsrset( m, n, row, col, val, &A3, queue ));
        ...
        CHECK( magma_minproduct_dmdiff( A, A3, &res, queue ));
        printf("# ||A-B||_F = %8.2e\n", res);
        if ( res < .000001 )
        ...
        magma_minproduct_dmfree(&A4, queue );
        magma_minproduct_dmfree(&A5, queue );
