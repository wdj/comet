    ...
    magma_minproduct_int_t *info )
{
    ...
    float *dwork[Magma_minproductMaxGPUs]={NULL}, *dpanel[Magma_minproductMaxGPUs]={NULL};
    ...
    *info = 0;
    ...
    for( dev=0; dev < ngpu; dev++ ) {
        ...
        if ( MAGMA_minproduct_SUCCESS != magma_minproduct_smalloc( &(dwork[dev]), (lddwork + ldda)*nb )) {
    ...
    if ( MAGMA_minproduct_SUCCESS != magma_minproduct_smalloc_pinned( &hwork, lwork )) {
    ...
    if ( nb < min_mn ) {
        ...
        for( i = 0; i < min_mn-nb; i += nb ) {
            ...
            magma_minproduct_sgetmatrix_async( rows, ib,
            ...
            spanel_to_q( Magma_minproductUpper, ib, hpanel(i), ldhpanel, hwork + ib*ib );
            ...
            /* Restore the panel */
            sq_to_panel( Magma_minproductUpper, ib, hpanel(i), ldhpanel, hwork + ib*ib );
    ...
    if (i < min_mn) {
        ...
        for( j=i; j < n; j += nb ) {
            ...
            ib = min( n-j, nb );
            magma_minproduct_setdevice( panel_dev );
            magma_minproduct_sgetmatrix( rows, ib,
                              dlA(panel_dev, i, i_local), ldda,
                              hwork + (j-i)*rows,         rows );
        }
        ...
        for( j=i; j < n; j += nb ) {
            ...
            ib = min( n-j, nb );
            magma_minproduct_setdevice( panel_dev );
            magma_minproduct_ssetmatrix( rows, ib,
                              hwork + (j-i)*rows,         rows,
                              dlA(panel_dev, i, i_local), ldda );
        }
    }
