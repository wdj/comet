/*
    -- MAGMA_tally4 (version 1.6.1) --
       ...
       @generated from zhemv_mgpu.cu normal z -> c, Fri Jan 30 19:00:10 2015
...
chemv_kernel_L_mgpu(
    int n,
    magma_tally4FloatComplex const * __restrict__ A, int lda,
    magma_tally4FloatComplex const * __restrict__ x, int incx,
    magma_tally4FloatComplex       * __restrict__ work,
    int my_gpu_id,
    int ngpu,
    int block_offset )
{
    ...
    magma_tally4FloatComplex psum, psum_t;
    magma_tally4FloatComplex total = MAGMA_tally4_C_ZERO;
    ...
    __shared__ magma_tally4FloatComplex sA [quarter_NB_X][NB_X + 2];  // TODO +3 used in chemv (single GPU); why?
    __shared__ magma_tally4FloatComplex sx_blk[NB_X];  // for x[ blk ]
    __shared__ magma_tally4FloatComplex sx_jj [NB_X];  // for x[ jj ], which cycles over all blocks left of diag

    magma_tally4FloatComplex rA[4];
    magma_tally4FloatComplex psums_t[4];
...
/**************************************************************
    ...
    Note beta*y is not included here; see magma_tally4blas_chemv_mgpu_sync.
...
chemv_kernel_L_mgpu_sum(
    ...
    int lda,
    magma_tally4FloatComplex       * __restrict__ y, int incy,
    magma_tally4FloatComplex const * __restrict__ work,
    int my_gpu_id,
    int ngpu,
    int block_offset)
{
    ...
    if ( ind >= block_offset && ind < n+block_offset ) {
        magma_tally4FloatComplex Ax = MAGMA_tally4_C_ZERO;
        ...
        y[ind * incy] = alpha*Ax;  // see magma_tally4blas_chemv_sync for beta*y
    }
}
...
/**
    Purpose
    -------
    magma_tally4blas_chemv_mgpu performs the matrix-vector operation:
    ...
    uplo    magma_tally4_uplo_t.
            ...
            follows:
      -     = Magma_tally4Upper:  Only the upper triangular part of A is to be referenced. **Not currently supported.**
      -     = Magma_tally4Lower:  Only the lower triangular part of A is to be referenced.
    ...
    \n
            Before entry with  UPLO = Magma_tally4Upper, the leading n by n
            upper triangular part of the array A must contain the upper
            triangular part of the Hermitian matrix and the strictly
            lower triangular part of A is not referenced.
            Before entry with UPLO = Magma_tally4Lower, the leading n by n
    ...
    @param[in]
    queues  magma_tally4_queue_t array of dimension (ngpu).
...
magma_tally4blas_chemv_mgpu(
    ...
    magma_tally4FloatComplex alpha,
    magma_tally4FloatComplex_const_ptr const d_lA[], magma_tally4_int_t ldda,
    magma_tally4_int_t offset,
    magma_tally4FloatComplex const *x,       magma_tally4_int_t incx,
    magma_tally4FloatComplex beta,                                         // unused, see magma_tally4blas_chemv_mgpu_sync
    magma_tally4FloatComplex       *y,       magma_tally4_int_t incy,             // unused
    ...
    magma_tally4_queue_t queues[] )
{
    ...
    magma_tally4_int_t offset_block_id = offset / NB_X;
    magma_tally4_int_t offset_gpu_id   = offset_block_id % ngpu;
    magma_tally4_int_t block_offset    = offset % NB_X;
    
    magma_tally4_int_t blocks = ceildiv( n + block_offset, NB_X );
    magma_tally4_int_t ldwmin = ldda*(blocks + 1);
    magma_tally4_int_t lhwmin = n*ngpu;
    ...
    for(dev=0; dev < ngpu; dev++) {
        ...
        magma_tally4_int_t num_blocks_skipped = offset_block_id / ngpu;
        if ( dev < offset_gpu_id ) {
            num_blocks_skipped += 1;
        }
        
        // shift dA to first block >= offset block that is owned by this GPU
        magma_tally4FloatComplex const *dA_dev    = d_lA[dev] + offset_block_id*NB_X + num_blocks_skipped*NB_X*ldda;
        ...
        magma_tally4FloatComplex       *dx_dev    = dwork[dev];
        magma_tally4FloatComplex       *dwork_dev = dwork[dev] + ldda;
        
        // renumber GPUs starting from the offset block
        magma_tally4_int_t new_gpu_id = (dev + ngpu - offset_gpu_id) % ngpu;
        
        dim3 grid( blocks, 1 );
        
        // copy x to each GPU
        magma_tally4_csetvector_async( n, x, incx, dx_dev + block_offset, 1, queues[dev] );
        ...
        if ( upper ) {
            chemv_kernel_U_mgpu<<< grid, threads, 0, queues[dev] >>>(
                n, dA_dev, ldda, dx_dev, 1, dwork_dev,
                new_gpu_id, ngpu, block_offset );
            
            chemv_kernel_U_mgpu_sum<<< grid, threads_sum, 0, queues[dev] >>>(
                n, alpha, ldda, dx_dev, 1, dwork_dev,
                new_gpu_id, ngpu, block_offset );
        }
        else {
            chemv_kernel_L_mgpu<<< grid, threads, 0, queues[dev] >>>(
                n, dA_dev, ldda, dx_dev, 1, dwork_dev,
                new_gpu_id, ngpu, block_offset );
            
            chemv_kernel_L_mgpu_sum<<< grid, threads_sum, 0, queues[dev] >>>(
                n, alpha, ldda, dx_dev, 1, dwork_dev,
                new_gpu_id, ngpu, block_offset );
        }
    }
    ...
    for(dev=0; dev < ngpu; dev++) {
        ...
        magma_tally4_setdevice( dev );
        magma_tally4FloatComplex       *dx_dev    = dwork[dev];
        magma_tally4_cgetvector_async( n, dx_dev + block_offset, 1, &hwork[dev*n], 1, queues[dev] );
    }
    
    // see magma_tally4blas_chemv_mgpu_sync for final row sums
...
/**
    ...
    For convenience, the parameters are identical to magma_tally4blas_chemv_mgpu
    (though some are unused here).
    
    @see magma_tally4blas_chemv_mgpu
    @ingroup magma_tally4_cblas2
    ********************************************************************/
extern "C" magma_tally4_int_t
magma_tally4blas_chemv_mgpu_sync(
    magma_tally4_uplo_t uplo,                                               // unused, see magma_tally4blas_chemv_mgpu
    magma_tally4_int_t n,
    magma_tally4FloatComplex alpha,                                        // unused
    magma_tally4FloatComplex_const_ptr const d_lA[], magma_tally4_int_t ldda,
    magma_tally4_int_t offset,                                              // unused
    magma_tally4FloatComplex const *x,       magma_tally4_int_t incx,             // unused
    magma_tally4FloatComplex beta,
    magma_tally4FloatComplex       *y,       magma_tally4_int_t incy,             // unused
    magma_tally4FloatComplex       *hwork,   magma_tally4_int_t lhwork,
    magma_tally4FloatComplex_ptr    dwork[], magma_tally4_int_t ldwork,           // unused
    magma_tally4_int_t ngpu,
    magma_tally4_int_t nb,                                                  // unused
    magma_tally4_queue_t queues[] )
{
     ...
     */
    magma_tally4_int_t info = 0;
    //if ( (! upper) && (uplo != Magma_tally4Lower) ) {  // unused
    //    info = -1;
    //} else
    if ( n < 0 ) {
        info = -2;
    ...
    for( dev=0; dev < ngpu; ++dev ) {
        magma_tally4_setdevice( dev );
        magma_tally4_queue_sync( queues[dev] );
