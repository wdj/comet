    ...
    magma_tally4_int_t *info)
{
    ...
    */
    ...
    magma_tally4_event_t myevent[2];
    ...
    magma_tally4FloatComplex *dwork, *dwork0, *dwork1, *dwvt0, *dwvt1;
    magma_tally4FloatComplex *dT0, *dV0, *dT1, *dV1;
    magma_tally4_int_t lddv = ldv;
    magma_tally4_int_t lddt = ldt;
    magma_tally4_int_t lddw = 0;
    magma_tally4_int_t lddwork  = ((NE+31)/32)*32;
    magma_tally4_int_t dwVTsiz  = lddv*Vblksiz; // lddv*lddv + lddv*lddwork; (v2) // lddv*Vblksiz; (v1,v3)
    magma_tally4_int_t dworksiz = lddwork*Vblksiz;  // lddv*Vblksiz; (v2)   // NE*Vblksiz=lddwork*Vblksiz; (v1,v3)

    if (MAGMA_tally4_SUCCESS != magma_tally4_cmalloc( &dwork, 2*dworksiz + 2*dwVTsiz +  2*Vchunksiz* (Vblksiz* (lddv+lddt)) )) {
    ...
    //magma_tally4_int_t nr_bl = magma_tally4_ceildiv(NE,10000);        //nr of blocks
    magma_tally4_int_t sz_bl = NE; //magma_tally4_ceildiv(NE,nr_bl*64)*64; //maximum size of blocks (to have blocks of around the same size and multiple of 64)
                ...
                for (blki=mt; blki > 0; blki--) {
                    ...
                    if (ncpy == 0) {
                        ...
                        if (mysiz > 0) {
                            ...
                            magma_tally4_csetmatrix_async(vld, Vblksiz, V(vpos), vld, dV1, vld, stream[1]);
                            magma_tally4_csetmatrix_async(tld, Vblksiz, T(tpos), tld, dT1, tld, stream[1]);
                    ...
                    if (blkid == copyst) {
                        ...
                        if (mysiz > 0) {
                            ...
                            if (flip == 0) { // now I am working on dV0 so copy the next and put it on dV1
                                ...
                                magma_tally4_csetmatrix_async(vld, Vblksiz, V(vpos), vld, dV0, vld, stream[0]);
                                magma_tally4_csetmatrix_async(tld, Vblksiz, T(tpos), tld, dT0, tld, stream[0]);
                            }
                        }
                    }
                    ...
                    if ((Vm > 0) && (Vn > 0)) {
                        ...
                        if (flip == 0) {
                            magma_tally4blasSetKernelStream(stream[0]);
                            magma_tally4_queue_wait_event( stream[0], myevent[1] );
                            for (magma_tally4_int_t i=0; i < NE; i += sz_bl) {
                                ib = min(sz_bl, NE-i);
                                lddw = min(lddwork,sz_bl);
                                //magma_tally4_clarfb_gpu( Magma_tally4Left, Magma_tally4NoTrans, Magma_tally4Forward, Magma_tally4Columnwise, Vm, ib, Vn, dV0+lcvpos, lddv, dT0+lctpos, lddt, dE(myrow,i), ldde, dwork0, lddw);
                                magma_tally4_clarfb_gpu_gemm( Magma_tally4Left, Magma_tally4NoTrans, Magma_tally4Forward, Magma_tally4Columnwise, Vm, ib, Vn, dV0+lcvpos, lddv, dT0+lctpos, lddt, dE(myrow,i), ldde, dwork0, lddw, dwvt0, lddv);
                            }
                            magma_tally4_event_record( myevent[0], stream[0] );
                        } else {
                            magma_tally4blasSetKernelStream(stream[1]);
                            magma_tally4_queue_wait_event( stream[1], myevent[0] );
                            for (magma_tally4_int_t i=0; i < NE; i += sz_bl) {
                                ib = min(sz_bl, NE-i);
                                lddw = min(lddwork,sz_bl);
                                //magma_tally4_clarfb_gpu( Magma_tally4Left, Magma_tally4NoTrans, Magma_tally4Forward, Magma_tally4Columnwise, Vm, ib, Vn, dV1+lcvpos, lddv, dT1+lctpos, lddt, dE(myrow,i), ldde, dwork1, lddw);
                                magma_tally4_clarfb_gpu_gemm( Magma_tally4Left, Magma_tally4NoTrans, Magma_tally4Forward, Magma_tally4Columnwise, Vm, ib, Vn, dV1+lcvpos, lddv, dT1+lctpos, lddt, dE(myrow,i), ldde, dwork1, lddw, dwvt1, lddv);
                            }
                            magma_tally4_event_record( myevent[1], stream[1] );
                        }
                    }  // end for (Vm &Vn) > 0
                } // end for blki
            } // end for blkj
    ...
    magma_tally4_event_destroy( myevent[0] );
    magma_tally4_event_destroy( myevent[1] );
