...
// copied from scnrm2.cu in trunk/magma_minproductblas
// ----------------------------------------
...
__global__ void
magma_minproduct_clobpcg_res_kernel( 
    ...
    if( row<num_rows){
        for( int i=0; i<num_vecs; i++ ){ 
            ...
            R[row + i*num_rows] = R[row + i*num_rows] 
                                    + MAGMA_minproduct_C_MAKE( -evals[i], 0.0 )
                                                * X[ row + i*num_rows ];
        }
    }
}
...
/*
magma_minproductblas_scnrm2_kernel( 
    int m, 
    ...
    magma_minproduct_queue_t queue )
{
    ...
    magma_minproduct_clobpcg_res_kernel<<< grid, threads, 0, queue >>>
