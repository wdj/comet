...
int main( int argc, char** argv)
{
    ...
    magma_tally3FloatComplex *d_A, *d_invA;
    ...
    for( int i = 0; i < opts.ntest; ++i ) {
      ...
      for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_DEV(  d_invA,  magma_tally3FloatComplex, ldda*N * batchCount);
            ...
            magma_tally3_csetmatrix( M, columns, h_R, lda, d_A, ldda );
            ...
            info1 = magma_tally3_cgetrf_batched( M, N, dA_array, ldda, dipiv_array, dinfo_array, batchCount, queue);
            info2 = magma_tally3_cgetri_outofplace_batched( min(M,N), dA_array, ldda, dipiv_array, dinvA_array, ldda, dinfo_array, batchCount, queue);
            ...
            if (info1 != 0) printf("magma_tally3_cgetrf_batched returned argument error %d: %s.\n", (int) info1, magma_tally3_strerror( info1 ));
            if (info2 != 0) printf("magma_tally3_cgetri_batched returned argument error %d: %s.\n", (int) info2, magma_tally3_strerror( info2 ));
            /* =====================================================================
            ...
            if ( opts.lapack ) {
                ...
                lwork = magma_tally3_int_t( MAGMA_tally3_C_REAL( tmp ));
                TESTING_MALLOC_CPU( work,  magma_tally3FloatComplex, lwork  );
                lapackf77_clacpy( Magma_tally3UpperLowerStr, &M, &columns, h_R, &lda, h_A, &lda );
            ...
            if ( opts.check ) {
                ...
                magma_tally3_cgetmatrix( min(M,N), N*batchCount, d_invA, ldda, h_R, lda );
                int stop=0;
                n2     = lda*N;
                for(int i=0; i < batchCount; i++)
                {
