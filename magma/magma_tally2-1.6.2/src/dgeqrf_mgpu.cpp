    ...
    magma_tally2_int_t *info )
{
    ...
    double *dwork[Magma_tally2MaxGPUs]={NULL}, *dpanel[Magma_tally2MaxGPUs]={NULL};
    ...
    *info = 0;
    ...
    for( dev=0; dev < ngpu; dev++ ) {
        ...
        if ( MAGMA_tally2_SUCCESS != magma_tally2_dmalloc( &(dwork[dev]), (lddwork + ldda)*nb )) {
    ...
    if ( MAGMA_tally2_SUCCESS != magma_tally2_dmalloc_pinned( &hwork, lwork )) {
    ...
    if ( nb < min_mn ) {
        ...
        for( i = 0; i < min_mn-nb; i += nb ) {
            ...
            dpanel_to_q_tally2( Magma_tally2Upper, ib, hpanel(i), ldhpanel, hwork + ib*ib );
            ...
            for( dev=0; dev < ngpu; dev++ ) {
                ...
                magma_tally2_dsetmatrix_async( rows, ib,
                                        hpanel(i),   ldhpanel,
            ...
            // TODO: if dpanel_to_q_tally2 copied whole block, wouldn't need to restore
            // -- just send the copy to the GPUs.
            // TODO: also, could zero out the lower triangle and use Azzam's larfb w/ gemm.
            
            /* Restore the panel */
            dq_to_panel_tally2( Magma_tally2Upper, ib, hpanel(i), ldhpanel, hwork + ib*ib );

            if (i + ib < n) {
                /* Send the T matrix to the GPU. */
                for( dev=0; dev < ngpu; dev++ ) {
                    magma_tally2_setdevice( dev );
                    magma_tally2_dsetmatrix_async( ib, ib,
                                            hwork,      ib,
    ...
    if (i < min_mn) {
        ...
        for( j=i; j < n; j += nb ) {
            ...
            ib = min( n-j, nb );
            magma_tally2_setdevice( panel_dev );
            magma_tally2_dgetmatrix( rows, ib,
                              dlA(panel_dev, i, i_local), ldda,
                              hwork + (j-i)*rows,         rows );
        }
