...
/**
    ...
    as returned by CGEQLF. Q is of order M if SIDE = Magma_tally3Left and of order N
    ...
    magma_tally3_int_t *info)
{
    ...
    *info  = 0;
    ...
    if (*info == 0) {
        nb = magma_tally3_get_cgelqf_nb( min( m, n ));
        lwkopt = max(1,nw)*nb;
    ...
    if ( nb >= k ) {
        /* Use CPU code */
        lapackf77_cunmql( lapack_side_const_tally3(side), lapack_trans_const_tally3(trans),
        ...
        magma_tally3FloatComplex *dwork, *dV, *dT, *dC;
        magma_tally3_cmalloc( &dwork, (nw + nq + nb)*nb + lddc*n );
        if ( dwork == NULL ) {
        ...
        magma_tally3_cmalloc_pinned( &T, 2*nb*nb );
        if ( T == NULL ) {
        ...
        magma_tally3_csetmatrix( m, n, C, ldc, dC, lddc );
        ...
        for (i = i1; (step < 0 ? i >= i2 : i < i2); i += step) {
            ...
            /* 1) set lower triangle of panel in A to identity,
               ...
               3) restore A                                      */
            cpanel_to_q_tally3( Magma_tally3Lower, ib, A(nq_i-ib,i), lda, T2 );
            magma_tally3_csetmatrix( nq_i,  ib, A(0,      i), lda, dV, nq_i );
            cq_to_panel_tally3( Magma_tally3Lower, ib, A(nq_i-ib,i), lda, T2 );
            ...
            magma_tally3_csetmatrix( ib, ib, T, ib, dT, ib );
            magma_tally3_clarfb_gpu( side, trans, Magma_tally3Backward, Magma_tally3Columnwise,
                              mi, ni, ib,
                              dV, nq_i,
                              dT, ib,
                              dC, lddc,
        ...
        magma_tally3_cgetmatrix( m, n, dC, lddc, C, ldc );
