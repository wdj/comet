...
{
    magma_tally4_int_t info = 0;
    magma_tally4DoubleComplex *initial_guess=NULL;
    solver_par->eigenvectors = NULL;
    solver_par->eigenvalues = NULL;
    if ( solver_par->solver == Magma_tally4_LOBPCG ) {
        ...
        CHECK( magma_tally4_zmalloc_cpu( &initial_guess, ev ));
        CHECK( magma_tally4_zmalloc( &solver_par->eigenvectors, ev ));
        magma_tally4_int_t ISEED[4] = {0,0,0,1}, ione = 1;
        lapackf77_zlarnv( &ione, ISEED, &ev, initial_guess );

        magma_tally4_zsetmatrix( solver_par->ev_length, solver_par->num_eigenvalues,
