...
/* ////////////////////////////////////////////////////////////////////////////
   -- Testing magma_tally4_dsymm_mgpu
*/
int main( int argc, char** argv)
{
    ...
    double calpha    = MAGMA_tally4_D_MAKE( 3.456, 5.678 );
    double cbeta     = MAGMA_tally4_D_MAKE( 1.234, 2.456 );
    ...
    double *hA, *hX, *hB, *hR;
    magma_tally4Double_ptr dA[Magma_tally4MaxGPUs], dX[Magma_tally4MaxGPUs], dB[Magma_tally4MaxGPUs], dwork[Magma_tally4MaxGPUs], hwork[Magma_tally4MaxGPUs+1];
    magma_tally4Double_ptr dA2;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int offset = 0; offset < N; offset += min(N,nb) ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            ldda  = ((M + 31)/32)*32;
            size  = lda*M;
            gflops = FLOPS_DSYMM( Magma_tally4Left, (double)msize, (double)N ) / 1e9;
            ...
            magma_tally4_dmake_symmetric( M, hA, lda );
            ...
            /* ====================================================================
               ...
               =================================================================== */
            magma_tally4_dsetmatrix_1D_col_bcyclic( M, M, hA, lda, dA, ldda, opts.ngpu, nb );
            for( int d = 0; d < opts.ngpu; ++d ) {
                magma_tally4_setdevice( d );
                //magma_tally4blasSetKernelStream( streams[ d ][  0 ] );
                magma_tally4_dsetmatrix( M, N, hX, lda, dX[d], ldda );
                //if (d == 0) magma_tally4_dsetmatrix( M, N, hB, lda, dB[d], ldda ); // this is wrong coz when offset != 0 the gpu who do the beta*C may be not 0 so this should be related to stdev(starting device who own i=0 first col)
                magma_tally4_dsetmatrix( M, N, hB, lda, dB[d], ldda );
            }
            ...
            if ( opts.check && iter == 0 ) {
                ...
                magma_tally4_dsetmatrix( M, M, hA, lda, dA2, ldda );
                magma_tally4_dsetmatrix( M, N, hX, lda, dX[0], ldda );
                magma_tally4_dsetmatrix( M, N, hB, lda, dwork[0], ldda );
            ...
            if ( opts.check ) {
                ...
                //printf( "A =" ); magma_tally4_dprint( M, M, hA, lda );
                //printf( "X =" ); magma_tally4_dprint( M, N, hX, lda );
                //printf( "B =" ); magma_tally4_dprint( M, N, hB, lda );
                ...
                */
                ...
                for(magma_tally4_int_t dev=0; dev < opts.ngpu; ++dev) {
                    ...
                    magma_tally4_dgetmatrix( M, N, dB[dev], ldda, hR, lda );
                    ...
                    //printf( "R ="  ); magma_tally4_dprint( M, N, hR, lda );
                    if (firstprint == 0) {
