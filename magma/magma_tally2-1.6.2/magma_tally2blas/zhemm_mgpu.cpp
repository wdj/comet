...
extern "C"
void magma_tally2blas_zhemm_mgpu_com(
    ...
    magma_tally2DoubleComplex *work[],     magma_tally2_int_t worksiz,  // TODO unused
    ...
    magma_tally2_int_t gnode[Magma_tally2MaxGPUs][Magma_tally2MaxGPUs+2], magma_tally2_int_t nbcmplx )
{
    ...
    if ( side != Magma_tally2Left || uplo != Magma_tally2Lower ) {
    ...
    magma_tally2DoubleComplex_ptr dwork2[Magma_tally2MaxGPUs];


    magma_tally2_int_t maxgsize    = n*m;
    magma_tally2_int_t lddwork = lddc;
    magma_tally2_int_t ldwork  = m;
    ...
    magma_tally2_int_t dev, devperm, myblk, mycolsize, myblkoffst;
    magma_tally2_int_t gmaster;
    magma_tally2_int_t masterdev, lcdev, lccolsize, myngpu;

    magma_tally2_int_t stdev       = (offset/nb)%ngpu;  
    magma_tally2_int_t blockoffset = offset % nb;  
    magma_tally2_int_t fstblksiz   = 0;
    if(blockoffset>0){
        fstblksiz   = min(m, (nb - blockoffset));
    }
    //magma_tally2_int_t nbblk       = magma_tally2_ceildiv(m, nb);
    magma_tally2_int_t nbblk       = magma_tally2_ceildiv((m+blockoffset), nb);
    magma_tally2_int_t remm        = m- fstblksiz;
    magma_tally2_int_t nbblkoffst  = offset/nb;


    magma_tally2_int_t nblstblks = -1;
    magma_tally2_int_t devlstblk = -1;
    magma_tally2_int_t lstblksiz = remm%nb;
    if(lstblksiz>0){
        nblstblks = nbblk%ngpu;
        devlstblk = (nblstblks-1+ngpu)%ngpu;
    }

    magma_tally2_int_t nbcmplxactive =  0;
    magma_tally2_int_t cmplxisactive[Magma_tally2MaxGPUs];
    magma_tally2_int_t gpuisactive[Magma_tally2MaxGPUs];
    memset(gpuisactive, 0, Magma_tally2MaxGPUs*sizeof(magma_tally2_int_t));
    memset(cmplxisactive, 0, Magma_tally2MaxGPUs*sizeof(magma_tally2_int_t));


    for( magma_tally2_int_t dev = 0; dev < ngpu; ++dev ) {
        magma_tally2_setdevice( dev );
        magma_tally2blasSetKernelStream( queues[ dev ][ 0 ] );
        cudaMemset(dwork(dev,0,0), 0, (lddwork)*(n)*sizeof(magma_tally2DoubleComplex) );
        // put all dC on all dev to 0 except the one which
        // hold i==0 because this one has to multiply by beta.
        if(dev!=stdev){
           cudaMemset(dC(dev,0,0), 0, (lddc)*(n)*sizeof(magma_tally2DoubleComplex) );
        }
    }

    magma_tally2_int_t newoffset = offset;
    // 1. symmetrize
    if(blockoffset>0){
        newoffset  = offset+fstblksiz; // newoffset is adjusted over nb
        magma_tally2_int_t myblkoffst = (nbblkoffst/ngpu)+(nbblkoffst%ngpu > stdev?1:0);
        //printf("STDEV %d  voici offset %d remm %d   myblockoffset %d    siz %d \n", stdev, offset, remm, myblkoffst, fstblksiz);
        magma_tally2_setdevice( stdev );
        magma_tally2blasSetKernelStream( queues[ stdev ][ 0 ] );
        magma_tally2blas_zsymmetrize_tiles(  Magma_tally2Lower,  fstblksiz,  dA(stdev, offset, myblkoffst*nb+blockoffset),  ldda,  1,  ngpu*nb,  nb  );         
    }

    for( magma_tally2_int_t dev = 0; dev < ngpu; ++dev ) {
        magma_tally2_int_t newstdev      = (newoffset/nb)%ngpu;
        magma_tally2_int_t nbblk = remm/nb; // number of block of size nb. if m%nb>0 then a last block exist and is of size ib=m%nb
        magma_tally2_int_t myblk = (nbblk/ngpu) + (nbblk%ngpu > ((dev-newstdev+ngpu)%ngpu) ?  1:0 );
        magma_tally2_int_t devperm   = (dev-newstdev+ngpu)%ngpu;
        magma_tally2_int_t nbblkoffst = newoffset/nb;
        magma_tally2_int_t myblkoffst = (nbblkoffst/ngpu)+(nbblkoffst%ngpu > dev?1:0);
        ...
        magma_tally2_setdevice( dev );
        magma_tally2blasSetKernelStream( queues[ dev ][ 0 ] );
        magma_tally2blas_zsymmetrize_tiles(  Magma_tally2Lower,  nb,  dA(dev, newoffset+devperm*nb, myblkoffst*nb),  ldda,  myblk,  ngpu*nb,  nb  );
        if(remm%nb>0){
            magma_tally2_int_t nblstblks = (nbblk+1)%ngpu;
            magma_tally2_int_t devlstblk = (nblstblks-1+ngpu)%ngpu;
            ...
            if(devperm==devlstblk)
                magma_tally2blas_zsymmetrize(  Magma_tally2Lower,  remm % nb,  dA(dev, newoffset+nbblk*nb, myblkoffst*nb+ myblk*nb),  ldda );  // last partial tile
        }
    }
...
/*
    magma_tally2_int_t siz = m+offset;
    magma_tally2DoubleComplex *R;
    magma_tally2_zmalloc_cpu( &R, siz*siz );
    // collecte back A
    magma_tally2blas_zgetmatrix_1D_bcyclic( siz, siz, dA, ldda, R, siz, ngpu, nb );
    magma_tally2_setdevice( 0 );
    magma_tally2blasSetKernelStream( queues[ dev ][ 0 ] );
    //magma_tally2_zgetmatrix( siz, siz, dA[0], ldda, R, siz );
    FILE *trace_file;
...
*/
    ...
    if(ngpu>1){
        for( magma_tally2_int_t i = fstblksiz; i < m; i += nb ) {
            magma_tally2_int_t ib     = min( nb, m-i );      // block size
            magma_tally2_int_t ioff   = i + offset;          // start global index in parent matrix
            //magma_tally2_int_t dev    = (ioff / nb) % ngpu;
            magma_tally2_int_t nbblkoffst = offset/nb;
            magma_tally2_int_t nbblk      = magma_tally2_ceildiv(i, nb);
            for( magma_tally2_int_t dev = 0; dev < ngpu; ++dev ) {


                magma_tally2_int_t myblk = (nbblk/ngpu) + (nbblk%ngpu > ((dev-stdev+ngpu)%ngpu) ?  1:0 );
                magma_tally2_int_t myblkoffst = (nbblkoffst/ngpu)+(nbblkoffst%ngpu > dev?1:0);

                magma_tally2_int_t myrowsize = myblk * nb;
                magma_tally2_int_t coloffset = myblkoffst*nb;
                if(dev==stdev) {
                ...
                if(myrowsize>0){
                    magma_tally2_setdevice( dev );
                    magma_tally2blasSetKernelStream( queues[ dev ][ 1 ] );    
                    magma_tally2_zgemm( Magma_tally2ConjTrans, Magma_tally2NoTrans, myrowsize, n, ib,
        ...
        for( magma_tally2_int_t dev = 0; dev < ngpu; ++dev ) {
            magma_tally2_setdevice( dev );
            magma_tally2_event_record(redevents[dev][1], queues[dev][1]);
        }
    }
    ...
    if(blockoffset>0){
        magma_tally2_int_t ib     = min( nb-blockoffset, m );  // block size
        magma_tally2_int_t iblock = (offset / nb) / ngpu;          // local block id
        magma_tally2_int_t di     = iblock*nb+blockoffset;       // local index in parent matrix
        magma_tally2_setdevice( stdev );
        magma_tally2blasSetKernelStream( queues[ stdev ][ 0 ] );        
        //printf("DEV %d COL GEMM first   ioff %d  di %d   m %d   n %d   ib %d \n", stdev, offset, di, m, n, ib);
        magma_tally2_zgemm( Magma_tally2NoTrans, Magma_tally2NoTrans, m, n, ib,
    ...
    for( magma_tally2_int_t i = fstblksiz; i < m; i += nb ) {
        magma_tally2_int_t ib     = min( nb, m-i );      // block size
        magma_tally2_int_t ioff   = i + offset;          // start global index in parent matrix
        magma_tally2_int_t iblock = (ioff / nb) / ngpu;  // local block id
        magma_tally2_int_t dev    = (ioff / nb) % ngpu;
        magma_tally2_int_t di     = iblock*nb;           // local index in parent matrix
        ...
        magma_tally2blasSetKernelStream( queues[ dev ][ 0 ] );
        if(i==0){
           magma_tally2_zgemm( Magma_tally2NoTrans, Magma_tally2NoTrans, m-i, n, ib,
        ...
        magma_tally2_event_record(redevents[dev][0], queues[dev][0]);
        ...
        if(ngpu==1){
            ...
            // magma_tally2blasSetKernelStream( queues[ dev ][ 0 ] );
            magma_tally2_zgemm( Magma_tally2ConjTrans, Magma_tally2NoTrans, i, n, ib,
    ...
    if(ngpu>1){
        for( magma_tally2_int_t dev = 0; dev < ngpu; ++dev ) {
            magma_tally2_int_t nbblk    = magma_tally2_ceildiv((m+blockoffset), nb);
            magma_tally2_int_t nbblkrow = nbblk-1; 
            magma_tally2_int_t devperm  = (dev-stdev+ngpu)%ngpu;
            magma_tally2_int_t myblk = (nbblkrow/ngpu) + (nbblkrow%ngpu > devperm ?  1:0 );
            magma_tally2_int_t myrowsize = myblk * nb;
             if(dev==stdev) {
                myrowsize = myrowsize - blockoffset;
            }
            ...
            if(myrowsize>0){
                magma_tally2_setdevice( dev );
                magma_tally2blasSetKernelStream( queues[ dev ][ 0 ] );
                magma_tally2_queue_wait_event(queues[ dev ][ 0 ], redevents[dev][1]);
                //magma_tally2_queue_sync( queues[ dev ][ 1 ] );
                // for each dev add the computed ROW block each on its placment with dC
                for( magma_tally2_int_t blki = 0; blki < myblk; ++blki){
                    magma_tally2_int_t gbblki = (blki*ngpu + devperm)*nb - blockoffset;
                    magma_tally2_int_t lcblki = blki*nb;
                    magma_tally2_int_t ib     = nb;// min(nb, m-gbblki);
                    if(dev==stdev){
                    ...
                    magma_tally2blas_zgeadd(ib, n, c_one, 
    ...
    // INITIALIZE COMM
    for( magma_tally2_int_t cmplxid = 0; cmplxid < nbcmplx; ++cmplxid ) {
        masterdev     = -1;
        gnode[cmplxid][Magma_tally2MaxGPUs+1] = -1;
        myngpu = gnode[cmplxid][Magma_tally2MaxGPUs];
        for( magma_tally2_int_t idev = 0; idev < myngpu; ++idev ) {
            ...
            if(mycolsize>0){
                ...
                if(masterdev==-1) {
                    ...
                    gnode[cmplxid][Magma_tally2MaxGPUs+1] = masterdev;
                }
            }
        }
    }
...
*/
    ...
    for( magma_tally2_int_t cmplxid = 0; cmplxid < nbcmplx; ++cmplxid ) {
        myngpu    = gnode[cmplxid][Magma_tally2MaxGPUs];
        masterdev = gnode[cmplxid][Magma_tally2MaxGPUs+1];
            ...
            for( magma_tally2_int_t idev = 0; idev < myngpu; ++idev ) {
                ...
                if(mycolsize>0){
                    ...
                    if(dev!=masterdev){
                        ...
                        magma_tally2_queue_wait_event(queues[ dev ][ 0 ], redevents[dev][0]);
                        // sending to the master of my complex
                        magma_tally2_zcopymatrix_async(
                            m, n,
                            &dC[dev][0], lddc,
                            &dwork2[masterdev][maxgsize*dev], m, queues[dev][0] );
                        magma_tally2_event_record(redevents[dev][masterdev], queues[dev][0]);
...
*/
    ...
    for( magma_tally2_int_t cmplxid = 0; cmplxid < nbcmplx; ++cmplxid ) {
        ...
        //check if complex is active
        if(masterdev!=-1){ 
            magma_tally2_setdevice( masterdev ); 
            // addition is done on stream 0 sequentially
            magma_tally2blasSetKernelStream( queues[ masterdev ][ 0 ] );
            // wait the geadd of my ROW and COL GEMM is done
            magma_tally2_queue_wait_event(queues[ masterdev ][ 0 ], redevents[masterdev][0]);
            // ========================================
            //     local addition
            // ========================================
            for( magma_tally2_int_t l = 0; l < myngpu; ++l ) {
                ...
                if((lcdev!=masterdev)&&(lccolsize>0)){
                    ...
                    magma_tally2_queue_wait_event(queues[ masterdev ][ 0 ], redevents[lcdev][masterdev]);
                    magma_tally2blas_zgeadd(m, n, c_one, 
            ...
            magma_tally2_event_record(redevents[masterdev][masterdev], queues[masterdev][0]);
            // ========================================
            //
            // ========================================
            //      send to other masters
            // ========================================
            for( magma_tally2_int_t k = 0; k < nbcmplx; ++k ) {
                if(k!=cmplxid){
                    gmaster = gnode[k][Magma_tally2MaxGPUs+1];
                    if(gmaster!=-1){ //complex is active
                        ...
                        magma_tally2_queue_wait_event(queues[ masterdev ][ gmaster ], redevents[masterdev][masterdev]);
                        magma_tally2_zcopymatrix_async(
                            m, n,
                            &dC[masterdev][0], lddc,
                            &dwork2[gmaster][maxgsize*masterdev], m, queues[masterdev][gmaster] );
                        magma_tally2_event_record(redevents[masterdev][gmaster], queues[masterdev][gmaster]);
                        magma_tally2_event_record(redevents[masterdev][masterdev], queues[masterdev][gmaster]);
                      } // end of gmaster!=-1
                } // end of k!=cmplxid
            }// for k = 0: nbcmplx
            // ========================================
...
*/
    ...
    for( magma_tally2_int_t cmplxid = 0; cmplxid < nbcmplx; ++cmplxid ) {
        ...
        if(masterdev!=-1){ 
            ...
            magma_tally2_queue_wait_event(queues[ masterdev ][ 0 ], redevents[masterdev][masterdev]);
            // ========================================
            ...
            for( magma_tally2_int_t k = 0; k < nbcmplx; ++k ) {
                if(k!=cmplxid){
                    ...
                    if(gmaster!=-1){ //complex is active
                        ...
                        magma_tally2_queue_wait_event(queues[ masterdev ][ 0 ], redevents[gmaster][masterdev]);
                        magma_tally2blas_zgeadd(m, n, c_one, 
            ...
            for( magma_tally2_int_t l = 0; l < myngpu; ++l ) {
                ...
                if((lcdev!=masterdev)&&(lccolsize>0)){
                    ...
                    magma_tally2_event_record(redevents[masterdev][lcdev], queues[masterdev][0]);
                }
            }// for l=1:myngpu
            // ========================================
        }// end of if masterdev!=-1 maening complex is active
    ...
    for( magma_tally2_int_t cmplxid = 0; cmplxid < nbcmplx; ++cmplxid ) {
        ...
        if(masterdev!=-1){ 
            for( magma_tally2_int_t l = 0; l < myngpu; ++l ) {
                ...
                lccolsize     = gpuisactive[lcdev];
                if(lccolsize>0){
                    magma_tally2_setdevice( lcdev );
                    magma_tally2_queue_wait_event(queues[ lcdev ][ 0 ], redevents[lcdev][0]);
                    magma_tally2_queue_wait_event(queues[ lcdev ][ 0 ], redevents[masterdev][lcdev]);
                }
            }// for l=1:myngpu
        }// end of if masterdev!=-1 maening complex is active
    }// for cmplxid
