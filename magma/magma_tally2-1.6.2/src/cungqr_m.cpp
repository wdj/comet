...
/**
    ...
    T       COMPLEX array, dimension (NB, min(M,N)).
            ...
            magma_tally2_cgeqrf_gpu (except stored on the CPU, not the GPU).
...
#define dT(d,i,j) (dT[d] + (i) + (j)*nb)
    ...
    *info = 0;
    ...
    magma_tally2FloatComplex *dA[ Magma_tally2MaxGPUs ] = { NULL };
    magma_tally2FloatComplex *dT[ Magma_tally2MaxGPUs ] = { NULL };
    magma_tally2FloatComplex *dV[ Magma_tally2MaxGPUs ] = { NULL };
    magma_tally2FloatComplex *dW[ Magma_tally2MaxGPUs ] = { NULL };
    ...
    for( int d = 0; d < ngpu; ++d ) {
        ...
        ldwork = nlocal[d]*ldda  // dA
               ...
               + nb*ldda         // dV
               + nb*lddwork;     // dW
        if ( MAGMA_tally2_SUCCESS != magma_tally2_cmalloc( &dA[d], ldwork )) {
    ...
    if (kk < n) {
        ...
        magma_tally2blas_claset( Magma_tally2Full, kk, n - kk, c_zero, c_zero, dA(dpanel, 0, di), ldda );
        trace_cpu_end( 0 );
    }
    ...
    if (kk > 0) {
        ...
        for( int d = 0; d < ngpu; ++d ) {
            ...
            magma_tally2_csetmatrix_async( nb, min(m,n), T, nb, dT[d], nb, stream[d] );
            trace_gpu_end( d, 0 );
        }
        ...
        for( i = ki; i >= 0; i -= nb ) {
            ...
            magma_tally2blas_claset( Magma_tally2Full, i,  ib, c_zero, c_zero, dA(dpanel, 0, di), ldda );
            magma_tally2blas_claset( Magma_tally2Full, mi, ib, c_zero, c_one,  dA(dpanel, i, di), ldda );
    ...
    magma_tally2_cgetmatrix_1D_col_bcyclic( m, n, dA, ldda, A, lda, ngpu, nb );
    trace_cpu_end( 0 );
