...
int main( int argc, char** argv )
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lapackf77_dlacpy( Magma_tally4UpperLowerStr, &m, &n, hA, &lda, hR, &lda );
            ...
            // okay that magma_tally4_dgeqrf_gpu has special structure for R; R isn't used here.
            magma_tally4_dsetmatrix( m, n, hA, lda, dA, ldda );
            magma_tally4_dgeqrf_gpu( m, n, dA, ldda, tau, dT, &info );
            if ( info != 0 )
                printf("magma_tally4_dgeqrf_gpu returned error %d: %s.\n",
                       (int) info, magma_tally4_strerror( info ));
            magma_tally4_dgetmatrix( m, n, dA, ldda, hA, lda );
            lapackf77_dlacpy( Magma_tally4FullStr, &m, &n, hA, &lda, hR, &lda );
            magma_tally4_dgetmatrix( nb, min_mn, dT, nb, hT, nb );
            
            gpu_time = magma_tally4_wtime();
            magma_tally4_dorgqr_m( m, n, k, hR, lda, tau, hT, nb, &info );
            gpu_time = magma_tally4_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if ( info != 0 )
                printf("magma_tally4_dorgqr_gpu returned error %d: %s.\n",
                       (int) info, magma_tally4_strerror( info ));
