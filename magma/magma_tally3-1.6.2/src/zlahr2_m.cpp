    ...
    struct zgehrd_data_tally3 *data )
{
    ...
    magma_tally3DoubleComplex ei = MAGMA_tally3_Z_ZERO;
    ...
    for( d = 0; d < ngpu; ++d ) {
        ...
        magma_tally3blas_zlaset( Magma_tally3Full, nb, nb, c_zero, c_zero, dV(d,k,0), ldv );
    }
    ...
    for (i = 0; i < nb; ++i) {
        ...
        if (i > 0) {
            ...
            tmp = MAGMA_tally3_Z_NEGATE( tau[i-1] );
        ...
        *A(k+i+1,i) = c_one;
        ...
        for( d = 0; d < ngpu; ++d ) {
            ...
            magma_tally3_zsetvector_async( n_k_i_1,
                                    A(k+i+1,i), 1,
            ...
            magma_tally3blas_zlacpy( Magma_tally3Full, nb, nblocks-lblock,
            ...
            if ( dn-dki1 > 0 ) {
                magma_tally3_zgemv( Magma_tally3NoTrans, n-k, dn-dki1,
                ...
                magma_tally3_zgetvector_async( n-k,
                                        dY(d, k, i), 1,
        ...
        scale = MAGMA_tally3_Z_NEGATE( tau[i] );
        blasf77_zgemv( "Conj", &n_k_i_1, &i,
                       &scale,  A(k+i+1,0), &lda,
                                A(k+i+1,i), &ione,
                       &c_zero, T(0,i), &ione );
    ...
    *A(k+nb,nb-1) = ei;
    ...
    for( d = 0; d < ngpu; ++d ) {
        ...
        // which are summed in separate loop below
        if ( dn-dki1 > 0 ) {
            magma_tally3_zgemm( Magma_tally3NoTrans, Magma_tally3NoTrans, k, nb, dn-dki1,
            ...
            magma_tally3_zgetmatrix_async( k, nb,
                                    dY(d, 0, 0),  ldda,
    ...
    for( d = 0; d < ngpu; ++d ) {
        magma_tally3_setdevice( d );
        magma_tally3_zsetmatrix_async( n, nb, Y, ldy, dY(d, 0, 0), ldda, data->streams[d] );
        magma_tally3_zsetmatrix_async( nb, nb, T, nb, dTi(d),      nb,   data->streams[d] );
    }
