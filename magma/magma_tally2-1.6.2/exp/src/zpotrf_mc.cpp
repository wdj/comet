...
static void SCHED_zgemm(Quark* quark)
{
  ...
  cuDoubleComplex one = MAGMA_tally2_Z_ONE;

  quark_unpack_args_8(quark, UPPER, M, N, K, A1, LDA, A2, A3);

  if (UPPER) {

    blasf77_zgemm(Magma_tally2ConjTransStr, Magma_tally2NoTransStr, 
...
void SCHED_zsyrk(Quark* quark)
{
  ...
  if (UPPER) {
    ...
    blasf77_zherk(Magma_tally2UpperStr, Magma_tally2ConjTransStr, &N, &K, &mone, A1, &LDA, &one, 
      A2, &LDA);

  } else {

    blasf77_zherk(Magma_tally2LowerStr, Magma_tally2NoTransStr, &N, &K, &mone, A1, &LDA, &one, 
      A2, &LDA);
...
void SCHED_zpotrf(Quark* quark)
{
  ...
  if (UPPER) {
    ...
    lapackf77_zpotrf(Magma_tally2UpperStr, &N, A, &LDA, &info);

  } else {

    lapackf77_zpotrf(Magma_tally2LowerStr, &N, A, &LDA, &info);
...
void SCHED_ztrsm(Quark* quark)
{
  ...
  if (UPPER) {
    ...
    blasf77_ztrsm(Magma_tally2LeftStr, Magma_tally2UpperStr, Magma_tally2ConjTransStr, Magma_tally2NonUnitStr,
...
magma_tally2_zpotrf_mc(magma_tally2_context *cntxt, char *uplo,
        magma_tally2_int_t *n,
...
/*  -- MAGMA_tally2 (version 1.6.1) --
  ...
  {
    //magma_tally2_int_t result = magma_tally2_zpotrf(*uplo, *n, a, *lda, info);
    //return result;
  }

  // check arguments
  magma_tally2_int_t upper = (magma_tally2_int_t) lsame_(uplo, "U");                                          
  *info = 0;
  ...
  magma_tally2_int_t ii,jj,kk;
  magma_tally2_int_t temp,temp2,temp3;

  char label[10000];

  magma_tally2_int_t iinfo[2];
  iinfo[1] = 0;
  ii = -1;
  ...
  for (i = 0; i < (*n); i += nb) {
      ...
      if (i < (*n)/2) {
        ...
        if (upper) {
          ...
          QUARK_Insert_Task(quark, SCHED_zsyrk, 0,
            sizeof(magma_tally2_int_t),             &upper,    VALUE,
            sizeof(magma_tally2_int_t),             &temp2,    VALUE,
            sizeof(magma_tally2_int_t),             &i,        VALUE,
          ...
          for (k = 0; k < i; k += nb) {
            ...
            if (upper) {
              QUARK_Insert_Task(quark, SCHED_zgemm, 0,
                sizeof(magma_tally2_int_t),             &upper,    VALUE,
                sizeof(magma_tally2_int_t),             &nb,       VALUE,
                sizeof(magma_tally2_int_t),             &temp,     VALUE,
                sizeof(magma_tally2_int_t),             &nb,       VALUE,
    ...
    QUARK_Insert_Task(quark, SCHED_zpotrf, 0,
      ...
      sizeof(cuDoubleComplex)*(*n)*(*n), A(i,i),    INOUT,
      sizeof(magma_tally2_int_t),             lda,       VALUE,
      sizeof(magma_tally2_int_t),             iinfo,     OUTPUT,
      strlen(label)+1,         label,     VALUE | TASKLABEL,
