      ...
      for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( uplo[iuplo] == Magma_tally4Lower || uplo[iuplo] == Magma_tally4Upper ) {
                ...
                gbytes = sizeof(magma_tally4DoubleComplex) * 0.5*2.*N*(N+1) / 1e9;
            }
            else {
            ...
            //magma_tally4blas_zlascl( uplo[iuplo], 1, 1, cfrom, cto, M-2, N-2, d_A+1+ldda, ldda, &info );  // inset by 1 row & col
            magma_tally4blas_zlascl( uplo[iuplo], 1, 1, cfrom, cto, M, N, d_A, ldda, &info );
            gpu_time = magma_tally4_sync_wtime( 0 ) - gpu_time;
            gpu_perf = gbytes / gpu_time;
            if (info != 0)
                printf("magma_tally4blas_zlascl returned error %d: %s.\n",
            ...
            //lapackf77_zlascl( lapack_uplo_const_tally4( uplo[iuplo] ), &ione, &ione, &cfrom, &cto, &M2, &N2, h_A+1+lda, &lda, &info );
            lapackf77_zlascl( lapack_uplo_const_tally4( uplo[iuplo] ), &ione, &ione, &cfrom, &cto, &M, &N, h_A, &lda, &info );
            ...
            //magma_tally4_zprint( M, N, h_R, lda );
