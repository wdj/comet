/*
    -- MAGMA_tally2 (version 1.6.1) --
       ...
       @generated from zungqr2.cpp normal z -> s, Fri Jan 30 19:00:15 2015
...
#define dA(i,j) (dA + (i) + (j)*ldda)
    ...
    *info = 0;
    ...
    ldda    = ((m + 31) / 32) * 32;
    lddwork = ((n + 31) / 32) * 32;
    if (MAGMA_tally2_SUCCESS != magma_tally2_smalloc( &dA, ldda*n + ldda*nb + lddwork*nb + nb*nb)) {
    ...
    if (kk < n) {
        ...
        */
        lapackf77_slacpy( Magma_tally2UpperLowerStr, &m_kk, &k_kk, A(kk,kk), &lda, V, &m_kk);
        lapackf77_slaset( Magma_tally2UpperLowerStr, &m_kk, &n_kk, &c_zero, &c_one, A(kk, kk), &lda );

        lapackf77_slarft( Magma_tally2ForwardStr, Magma_tally2ColumnwiseStr,
                          &m_kk, &k_kk,
                          V, &m_kk, &tau[kk], work, &k_kk);
        lapackf77_slarfb( Magma_tally2LeftStr, Magma_tally2NoTransStr, Magma_tally2ForwardStr, Magma_tally2ColumnwiseStr,
                          &m_kk, &n_kk, &k_kk,
                          V, &m_kk, work, &k_kk, A(kk, kk), &lda, work+k_kk*k_kk, &n_kk );
        
        if (kk > 0) {
            magma_tally2_ssetmatrix( m_kk, n_kk,
                              A(kk, kk),  lda,
                              dA(kk, kk), ldda );
    ...
    if (kk > 0) {
        ...
        for (i = ki; i >= 0; i -= nb) {
            ...
            magma_tally2_ssetmatrix_async( mi, ib,
                                    A(i, i), lda,
                                    dV,      ldda, stream );
            lapackf77_slarft( Magma_tally2ForwardStr, Magma_tally2ColumnwiseStr,
                              &mi, &ib,
            ...
            magma_tally2blas_slaset( Magma_tally2Full, mi, ib, c_zero, c_one,  dA(i, i), ldda );
