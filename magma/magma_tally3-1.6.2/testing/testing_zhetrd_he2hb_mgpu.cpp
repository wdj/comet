...
int main( int argc, char** argv)
{
    ...
    magma_tally3DoubleComplex *h_A, *h_R, *h_work;
    magma_tally3DoubleComplex *tau;
    double *D, *E;
    magma_tally3_int_t N, n2, lda, ldda, lwork, ldt, info, nstream;
    ...
    magma_tally3_int_t distblk = 0;

    magma_tally3_opts opts;
    parse_opts( argc, argv, &opts );
    
    magma_tally3_int_t WANTZ = (opts.jobz == Magma_tally3Vec);
    double tol = opts.tolerance * lapackf77_dlamch("E");
    if (opts.nb == 0)
        opts.nb = 64; //magma_tally3_get_zhetrd_he2hb_nb(N);

    if (NE < 1)
        NE = N; //64; //magma_tally3_get_zhetrd_he2hb_nb(N);

    nstream = max(3, opts.ngpu+2);
    magma_tally3_queue_t streams[Magma_tally3MaxGPUs][20];
    magma_tally3DoubleComplex_ptr da[Magma_tally3MaxGPUs], dT1[Magma_tally3MaxGPUs];
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_PIN( h_A,    magma_tally3DoubleComplex, lda*N );
            ...
            for( magma_tally3_int_t dev = 0; dev < opts.ngpu; ++dev ) {
                magma_tally3_int_t mlocal = ((N / distblk) / opts.ngpu + 1) * distblk;
                magma_tally3_setdevice( dev );
                TESTING_MALLOC_DEV( da[dev],  magma_tally3DoubleComplex, ldda*mlocal );
                TESTING_MALLOC_DEV( dT1[dev], magma_tally3DoubleComplex, N*opts.nb        );
            }
            ...
            magma_tally3_zsetmatrix_1D_col_bcyclic( N, N, h_R, lda, da, ldda, opts.ngpu, distblk);
            //magma_tally3DoubleComplex_ptr dabis;
            //TESTING_MALLOC_DEV( dabis,  magma_tally3DoubleComplex, ldda*N );
            //magma_tally3_zsetmatrix(N, N, h_R, lda, dabis, ldda);
            ...
            for (int count=0; count < 1; ++count) {
                ...
                // magma_tally3_zhetrd_he2hb(opts.uplo, N, opts.nb, h_R, lda, tau, h_work, lwork, dT1[0], &info);
            ...
            // magma_tally3_zhetrd_bhe2trc_v5(opts.nthread, WANTZ, opts.uplo, NE, N, opts.nb, h_R, lda, D, E, dT1[0], ldt);
            // magma_tally3_zhetrd_bhe2trc(opts.nthread, WANTZ, opts.uplo, NE, N, opts.nb, h_R, lda, D, E, dT1[0], ldt);
            ...
            if (info != 0)
                printf("magma_tally3_zhetrd_he2hb returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                cpu_time = magma_tally3_wtime();
                // check results
                zcheck_eig_( lapack_vec_const_tally3(opts.jobz), &MATYPE, &N, &opts.nb,
