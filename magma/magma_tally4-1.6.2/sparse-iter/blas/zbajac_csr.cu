...
__global__ void
magma_tally4_zbajac_csr_ls_kernel(int localiters, int n, 
                            magma_tally4DoubleComplex * valD, 
                            magma_tally4_index_t * rowD, 
                            magma_tally4_index_t * colD, 
                            magma_tally4DoubleComplex * valR, 
    ...
    if(index<n){
        ...
        magma_tally4DoubleComplex bl, tmp = zero, v = zero; 
        ...
        __shared__ magma_tally4DoubleComplex local_x[ BLOCKSIZE ];
...
__global__ void
magma_tally4_zbajac_csr_kernel(    
    int n, 
    magma_tally4DoubleComplex * valD, 
    magma_tally4_index_t * rowD, 
    magma_tally4_index_t * colD, 
    ...
    magma_tally4_queue_t queue )
{
    ...
    if ( R.nnz > 0 ) { 
        if ( localiters == 1 )
        magma_tally4_zbajac_csr_kernel<<< grid, block, 0, queue >>>
            ( D.num_rows, D.dval, D.drow, D.dcol, 
                            R.dval, R.drow, R.dcol, b.dval, x->dval );
        else
            magma_tally4_zbajac_csr_ls_kernel<<< grid, block, 0, queue >>>
