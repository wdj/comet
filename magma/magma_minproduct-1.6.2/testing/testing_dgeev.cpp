...
bool lessthan( magma_minproductDoubleComplex a, magma_minproductDoubleComplex b )
{
    return (MAGMA_minproduct_Z_REAL(a) < MAGMA_minproduct_Z_REAL(b)) ||
        (MAGMA_minproduct_Z_REAL(a) == MAGMA_minproduct_Z_REAL(b) && MAGMA_minproduct_Z_IMAG(a) < MAGMA_minproduct_Z_IMAG(b));
}
...
extern "C"
double magma_minproduct_dlapy2(double x, double y)
{
    double ret_val, d;
...
extern "C"
double magma_minproduct_dzlapy2(magma_minproductDoubleComplex x)
{
    return magma_minproduct_dlapy2( MAGMA_minproduct_Z_REAL(x), MAGMA_minproduct_Z_IMAG(x) );
}
...
int main( int argc, char** argv)
{
    ...
    double *w1i, *w2i;
    magma_minproductDoubleComplex *w1copy, *w2copy;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( w1copy, magma_minproductDoubleComplex, N );
            TESTING_MALLOC_CPU( w2copy, magma_minproductDoubleComplex, N );
            ...
            magma_minproduct_dgeev( opts.jobvl, opts.jobvr,
                         N, h_R, lda, w1, w1i,
                         VL, lda, VR, lda,
                         h_work, lwork, &info );
            gpu_time = magma_minproduct_wtime() - gpu_time;
            if (info != 0)
                printf("magma_minproduct_dgeev returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                if ( opts.jobvr == Magma_minproductVec ) {
                    ...
                    lapackf77_dget22( Magma_minproductNoTransStr, Magma_minproductNoTransStr, Magma_minproductNoTransStr,
                    ...
                    for( int j = 0; j < N; ++j ) {
                        tnrm = 1.;
                        if (w1i[j] == 0.)
                            tnrm = magma_minproduct_cblas_dnrm2( N, &VR[j*lda], ione );
                        else if (w1i[j] > 0.)
                            tnrm = magma_minproduct_dlapy2( magma_minproduct_cblas_dnrm2( N, &VR[j*lda],     ione ),
                                                 magma_minproduct_cblas_dnrm2( N, &VR[(j+1)*lda], ione ));
                        
                        result[1] = max( result[1], min( ulpinv, MAGMA_minproduct_D_ABS(tnrm-1.)/ulp ));
                        
                        if (w1i[j] > 0.) {
                            vmx  = vrmx = 0.;
                            for( int jj = 0; jj < N; ++jj ) {
                                vtst = magma_minproduct_dlapy2( VR[jj+j*lda], VR[jj+(j+1)*lda]);
                                if (vtst > vmx)
                                    vmx = vtst;
                                
                                if ( (VR[jj + (j+1)*lda])==0. &&
                                     MAGMA_minproduct_D_ABS( VR[jj+j*lda] ) > vrmx)
                                {
                                    vrmx = MAGMA_minproduct_D_ABS( VR[jj+j*lda] );
                                }
                            }
                            if (vrmx / vmx < 1. - ulp*2.)
                                result[1] = ulpinv;
                ...
                if ( opts.jobvl == Magma_minproductVec ) {
                    ...
                    lapackf77_dget22( Magma_minproductTransStr, Magma_minproductNoTransStr, Magma_minproductTransStr,
                    ...
                    for( int j = 0; j < N; ++j ) {
                        tnrm = 1.;
                        if (w1i[j] == 0.)
                            tnrm = magma_minproduct_cblas_dnrm2( N, &VL[j*lda], ione );
                        else if (w1i[j] > 0.)
                            tnrm = magma_minproduct_dlapy2( magma_minproduct_cblas_dnrm2( N, &VL[j*lda],     ione ),
                                                 magma_minproduct_cblas_dnrm2( N, &VL[(j+1)*lda], ione ));
                        
                        result[3] = max( result[3], min( ulpinv, MAGMA_minproduct_D_ABS(tnrm-1.)/ulp ));
                        
                        if (w1i[j] > 0.) {
                            vmx  = vrmx = 0.;
                            for( int jj = 0; jj < N; ++jj ) {
                                vtst = magma_minproduct_dlapy2( VL[jj+j*lda], VL[jj+(j+1)*lda]);
                                if (vtst > vmx)
                                    vmx = vtst;
                                
                                if ( (VL[jj + (j+1)*lda])==0. &&
                                     MAGMA_minproduct_D_ABS( VL[jj+j*lda]) > vrmx)
                                {
                                    vrmx = MAGMA_minproduct_D_ABS( VL[jj+j*lda] );
                                }
                            }
                            if (vrmx / vmx < 1. - ulp*2.)
                                result[3] = ulpinv;
            ...
            if ( opts.check == 2 ) {
                ...
                // this is really slow because it calls magma_minproduct_zgeev multiple times
                double *LRE, DUM;
                ...
                magma_minproduct_dgeev( Magma_minproductVec, Magma_minproductVec,
                             N, h_R, lda, w1, w1i,
                             VL, lda, VR, lda,
                             h_work, lwork, &info );
                if (info != 0)
                    printf("magma_minproduct_zgeev (case V, V) returned error %d: %s.\n",
                           (int) info, magma_minproduct_strerror( info ));
                ...
                //lapackf77_dlacpy( Magma_minproductUpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                //magma_minproduct_dgeev( Magma_minproductNoVec, Magma_minproductNoVec,
                //             N, h_R, lda, w2, w2i,
                //             &DUM, 1, &DUM, 1,
                //             h_work, lwork, &info );
                //if (info != 0)
                //    printf("magma_minproduct_dgeev (case N, N) returned error %d: %s.\n",
                //           (int) info, magma_minproduct_strerror( info ));
                //
                ...
                lapackf77_dlacpy( Magma_minproductUpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                magma_minproduct_dgeev( Magma_minproductNoVec, Magma_minproductVec,
                             N, h_R, lda, w2, w2i,
                             &DUM, 1, LRE, lda,
                             h_work, lwork, &info );
                if (info != 0)
                    printf("magma_minproduct_dgeev (case N, V) returned error %d: %s.\n",
                           (int) info, magma_minproduct_strerror( info ));
                ...
                for( int j = 0; j < N; ++j )
                    for( int jj = 0; jj < N; ++jj )
                        if ( ! MAGMA_minproduct_D_EQUAL( VR[j+jj*lda], LRE[j+jj*lda] ))
                            result[7] = 0;
                
                // ----------
                // Compute eigenvalues and left eigenvectors
                lapackf77_dlacpy( Magma_minproductUpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                magma_minproduct_dgeev( Magma_minproductVec, Magma_minproductNoVec,
                             N, h_R, lda, w2, w2i,
                             LRE, lda, &DUM, 1,
                             h_work, lwork, &info );
                if (info != 0)
                    printf("magma_minproduct_dgeev (case V, N) returned error %d: %s.\n",
                           (int) info, magma_minproduct_strerror( info ));
                ...
                for( int j = 0; j < N; ++j )
                    for( int jj = 0; jj < N; ++jj )
                        if ( ! MAGMA_minproduct_D_EQUAL( VL[j+jj*lda], LRE[j+jj*lda] ))
                            result[8] = 0;
            ...
            if ( opts.lapack ) {
                ...
                for( int j=0; j < N; ++j ) {
                    w1copy[j] = MAGMA_minproduct_Z_MAKE( w1[j], w1i[j] );
                    w2copy[j] = MAGMA_minproduct_Z_MAKE( w2[j], w2i[j] );
                }
                ...
                for( int j=0; j < N; ++j ) {
                    for( int j2=j; j2 < N; ++j2 ) {
                        magma_minproductDoubleComplex diff = MAGMA_minproduct_Z_SUB( w1copy[j], w2copy[j2] );
                        double diff2 = magma_minproduct_dzlapy2( diff ) / max( magma_minproduct_dzlapy2( w1copy[j] ), tol );
                        if ( diff2 < 100*tol ) {
                            if ( j != j2 ) {
                ...
                error  = magma_minproduct_cblas_dznrm2( N, w1copy, 1 );
                error /= magma_minproduct_cblas_dznrm2( N, w2copy, 1 );
