...
{
    magma_minproduct_int_t nb, n_local[Magma_minproductMaxGPUs];
    magma_minproduct_int_t maxm;
    magma_minproduct_int_t i, j, d, lddat, lddwork;
    magma_minproductDoubleComplex *d_lAT[Magma_minproductMaxGPUs];
    magma_minproductDoubleComplex *d_panel[Magma_minproductMaxGPUs], *work;
    magma_minproduct_queue_t streaml[Magma_minproductMaxGPUs][2];
    ...
    *info = 0;
    ...
    if (nb <= 1 || nb >= n) {
        ...
        magma_minproduct_zgetmatrix( m, n, d_lA[0], ldda, work, m );
        lapackf77_zgetrf(&m, &n, work, &m, ipiv, info);
        magma_minproduct_zsetmatrix( m, n, work, m, d_lA[0], ldda );
        ...
        for (i=0; i < ngpu; i++) {
            ...
            /* workspaces */
            if (MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc( &d_panel[i], (3+ngpu)*nb*maxm )) {
                for( j=0; j <= i; j++ ) {
                    magma_minproduct_setdevice(j);
                }
                for( j=0; j < i; j++ ) {
                    magma_minproduct_setdevice(j);
                    magma_minproduct_free( d_panel[j] );
                    magma_minproduct_free( d_lAT[j]   );
                }
            ...
            if (MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc( &d_lAT[i], lddat*maxm )) {
                for( j=0; j <= i; j++ ) {
                    magma_minproduct_setdevice(j);
                    magma_minproduct_free( d_panel[j] );
                }
            ...
            /* create the streams */
            magma_minproduct_queue_create( &streaml[i][0] );
            magma_minproduct_queue_create( &streaml[i][1] );
            
            magma_minproductblasSetKernelStream(streaml[i][1]);
            magma_minproductblas_ztranspose( m, n_local[i], d_lA[i], ldda, d_lAT[i], lddat );
        }
        for (i=0; i < ngpu; i++) {
            magma_minproduct_setdevice(i);
            magma_minproduct_queue_sync(streaml[i][0]);
        ...
        /* cpu workspace */
        lddwork = maxm;
        if (MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc_pinned( &work, lddwork*nb*ngpu )) {
            for (i=0; i < ngpu; i++ ) {
                magma_minproduct_setdevice(i);
                magma_minproduct_free( d_panel[i] );
                magma_minproduct_free( d_lAT[i]   );
            }
            *info = MAGMA_minproduct_ERR_HOST_ALLOC;
            return *info;
        }
        ...
        magma_minproduct_zgetrf2_mgpu(ngpu, m, n, nb, 0, d_lAT, lddat, ipiv, d_panel, work, maxm,
                           streaml, info);
        ...
        for( d=0; d < ngpu; d++ ) {
            ...
            /* save on output */
            magma_minproductblas_ztranspose( n_local[d], m, d_lAT[d], lddat, d_lA[d], ldda );
            magma_minproduct_device_sync();
            magma_minproduct_free( d_lAT[d]   );
            magma_minproduct_free( d_panel[d] );
            magma_minproduct_queue_destroy( streaml[d][0] );
            magma_minproduct_queue_destroy( streaml[d][1] );
        } /* end of for d=1,..,ngpu */
