...
void magma_tally2_dgetf2_swap(
    magma_tally2_int_t n, double *x, magma_tally2_int_t i, magma_tally2_int_t j, magma_tally2_int_t incx);

void magma_tally2_dscal_dger(
    ...
    magma_tally2_int_t *info )
{
    ...
    *info = 0;
    ...
    for( j=0; j < min_mn; j++ ) {
        ...
        jp = j - 1 + magma_tally2_idamax(m-j, dA(j,j), 1);
        ...
        //if ( dA(jp, j) != 0.0) {
            ...
            if (jp != j) {
                magma_tally2_dgetf2_swap(n, dA, j, jp, ldda);
            }
            
            // Compute elements J+1:M of J-th column.
            if (j < m) {
                magma_tally2_dscal_dger(m-j, n-j, dA(j, j), ldda);
            }
        //}
        //else if (*info == 0) {
        //    *info = j;
...
void magma_tally2_dgetf2_swap(magma_tally2_int_t n, double *x, magma_tally2_int_t i, magma_tally2_int_t j, magma_tally2_int_t incx)
{
/*
...
*/
    ...
    kernel_dswap<<< grid, threads, 0, magma_tally2_stream >>>(n, x, i, j, incx);
}
...
void kernel_dscal_dger(int m, int n, double *A, int lda)
{
    ...
    if (tid < m && tid > 0) {
        ...
        #pragma unroll
        for(int i=1; i < n; i++) {
            A[tid + i*lda] += (MAGMA_tally2_D_NEG_ONE) * shared_y[i] * reg;
        }
    }
}


void magma_tally2_dscal_dger(magma_tally2_int_t m, magma_tally2_int_t n, double *A, magma_tally2_int_t lda)
{
/*
...
*/
    ...
    kernel_dscal_dger<<< grid, threads, shared_size, magma_tally2_stream>>>(m, n, A, lda);
}
