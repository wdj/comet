...
/**
    ...
    T       COMPLEX_16 array, dimension (NB, min(M,N)).
            ...
            magma_minproduct_zgeqrf_gpu (except stored on the CPU, not the GPU).
...
#define dT(d,i,j) (dT[d] + (i) + (j)*nb)
    ...
    *info = 0;
    ...
    magma_minproductDoubleComplex *dA[ Magma_minproductMaxGPUs ] = { NULL };
    magma_minproductDoubleComplex *dT[ Magma_minproductMaxGPUs ] = { NULL };
    magma_minproductDoubleComplex *dV[ Magma_minproductMaxGPUs ] = { NULL };
    magma_minproductDoubleComplex *dW[ Magma_minproductMaxGPUs ] = { NULL };
    ...
    for( int d = 0; d < ngpu; ++d ) {
        ...
        ldwork = nlocal[d]*ldda  // dA
               ...
               + nb*ldda         // dV
               + nb*lddwork;     // dW
        if ( MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc( &dA[d], ldwork )) {
    ...
    if (kk < n) {
        ...
        magma_minproductblas_zlaset( Magma_minproductFull, kk, n - kk, c_zero, c_zero, dA(dpanel, 0, di), ldda );
        trace_cpu_end( 0 );
    }
    ...
    if (kk > 0) {
        ...
        for( int d = 0; d < ngpu; ++d ) {
            ...
            magma_minproduct_zsetmatrix_async( nb, min(m,n), T, nb, dT[d], nb, stream[d] );
            trace_gpu_end( d, 0 );
        }
        ...
        for( i = ki; i >= 0; i -= nb ) {
            ...
            magma_minproductblas_zlaset( Magma_minproductFull, i,  ib, c_zero, c_zero, dA(dpanel, 0, di), ldda );
            magma_minproductblas_zlaset( Magma_minproductFull, mi, ib, c_zero, c_one,  dA(dpanel, i, di), ldda );
    ...
    magma_minproduct_zgetmatrix_1D_col_bcyclic( m, n, dA, ldda, A, lda, ngpu, nb );
    trace_cpu_end( 0 );
