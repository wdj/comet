/*
    -- MAGMA_tally3 (version 1.6.1) --
       ...
       @generated from zgeqrf_gpu.cpp normal z -> s, Fri Jan 30 19:00:15 2015
*/
...
*/
void ssplit_diag_block(magma_tally3_int_t ib, float *a, magma_tally3_int_t lda, float *work)
{
    ...
    lapackf77_strtri( Magma_tally3UpperStr, Magma_tally3NonUnitStr, &ib, work, &ib, &info);
}
...
/**
    ...
    dT      (workspace) REAL array on the GPU,
            ...
            where NB can be obtained through magma_tally3_get_sgeqrf_nb(M).
    ...
    magma_tally3_int_t *info )
{
    ...
    magma_tally3_int_t i, k, minmn, old_i, old_ib, rows, cols;
    magma_tally3_int_t ib, nb;
    magma_tally3_int_t ldwork, lddwork, lwork, lhwork;
    float *work, *ut;
    ...
    *info = 0;
    ...
    if (MAGMA_tally3_SUCCESS != magma_tally3_smalloc_pinned( &work, lwork )) {
    ...
    if ( (nb > 1) && (nb < k) ) {
        ...
        for (i = 0; i < k-nb; i += nb) {
            ...
            if (i > 0) {
                ...
                magma_tally3_slarfb_gpu( Magma_tally3Left, Magma_tally3ConjTrans, Magma_tally3Forward, Magma_tally3Columnwise,
                                  m-old_i, cols, old_ib,
                                  dA(old_i, old_i         ), ldda, dT(old_i), nb,
                                  dA(old_i, old_i+2*old_ib), ldda, dd_ref(0),    lddwork);
                
                /* store the diagonal */
                magma_tally3_ssetmatrix_async( old_ib, old_ib,
                                        ut,           old_ib,
            ...
            magma_tally3_ssetmatrix( rows, ib, work(i), ldwork, dA(i,i), ldda );

            if (i + ib < n) {
                /* Send the triangular factor T to the GPU */
                magma_tally3_ssetmatrix( ib, ib, hwork, ib, dT(i), nb );
                ...
                if (i+nb < k-nb) {
                    ...
                    magma_tally3_ssetmatrix( ib, ib, ut, ib, d_ref(i), ib );
                }
                old_i  = i;
                old_ib = ib;
            }
    ...
    if (i < k) {
        ib   = n-i;
        rows = m-i;
        magma_tally3_sgetmatrix( rows, ib, dA(i, i), ldda, work, rows );
        lhwork = lwork - rows*ib;
        lapackf77_sgeqrf(&rows, &ib, work, &rows, tau+i, work+ib*rows, &lhwork, info);
        
        magma_tally3_ssetmatrix( rows, ib, work, rows, dA(i, i), ldda );
    }
