...
int main( int argc, char** argv)
{
    ...
    NB = opts.nb;
    if (NB < 1)
        NB  = 64; //64; //magma_minproduct_get_dsytrd_sy2sb_nb(N);

    // what is NE ?
    if (NE < 1)
        NE  = 64; //N;  //magma_minproduct_get_dsytrd_sy2sb_nb(N);  // N not yet initialized
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            /*
            magma_minproduct_dsytrd_sy2sb( opts.uplo, N, NB, h_R, lda, tau, h_work, lwork0, dT1, THREADS, &info);
            tband = magma_minproduct_wtime - gpu_time();
            printf("  Finish BAND  N %d  NB %d  ngpu %d timing= %f\n", N, NB, ngpu, tband);
            magma_minproduct_dsytrd_bsy2trc_v5(THREADS, WANTZ, opts.uplo, NE, N, NB, h_R, lda, D, E, dT1, ldt);
            */
    
            /*
            magma_minproduct_dsytrd_sy2sb( opts.uplo, N, NB, h_R, lda, tau, h_work, lwork, dT1, THREADS, &info);
            tband = magma_minproduct_wtime - gpu_time();
            printf("  Finish BAND  N %d  NB %d  ngpu %d timing= %f\n", N, NB, ngpu, tband);
            magma_minproduct_dsytrd_bsy2trc(THREADS, WANTZ, opts.uplo, NE, N, NB, h_R, lda, D, E, dT1, ldt);
            */
            ...
            magma_minproduct_int_t threads = magma_minproduct_get_parallel_numthreads();
                ...
                lwork  = magma_minproduct_dbulge_get_lq2(N, threads) + 2*N + N*N;
                lrwork = 1 + 5*N +2*N*N;
                TESTING_MALLOC_PIN( rwork, double, lrwork );
            #else
                lwork  = magma_minproduct_dbulge_get_lq2(N, threads) + 1 + 6*N + 2*N*N;
            #endif
            liwork = 3 + 5*N;
            ...
            if (ngpu == 1) {
                ...
                magma_minproduct_dsyevdx_2stage( opts.jobz, range, opts.uplo, N,
                                h_R, lda,
                                vl, vu, il, iu,
                                &m1, D,
                                hh_work, lwork,
                ...
                magma_minproduct_dsyevdx_2stage_m(ngpu, opts.jobz, range, opts.uplo, N,
                                h_R, lda,
                                vl, vu, il, iu,
                                &m1, D,
                                hh_work, lwork,
            ...
            */
            ...
            if ( opts.check ) {
                ...
                magma_minproduct_dmalloc_cpu( &work2, lwork2 );
                magma_minproduct_dmalloc_cpu( &rwork2, N );
                magma_minproduct_dmalloc_cpu( &D2, N );
                magma_minproduct_dmalloc_cpu( &AINIT, N*lda );
                ...
                lapackf77_dsyev( "N", "L", &N, h_A, &lda, D2, work2, &lwork2,
                    #if defined(PRECISION_z) || defined (PRECISION_c)
                    rwork2,
                    #endif
                    &info );
                ...
                */
                ...
                //double *Z;
                // magma_minproduct_dmalloc_cpu( &Z, N*lda );
                ...
                // TODO free this memory !
                magma_minproduct_dmalloc_cpu( &WORKAJETER, (2* N * N + N)  );
