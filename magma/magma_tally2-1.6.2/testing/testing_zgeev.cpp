...
int main( int argc, char** argv)
{
    ...
    magma_tally2DoubleComplex *h_A, *h_R, *VL, *VR, *h_work, *w1, *w2;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( w1,     magma_tally2DoubleComplex, N );
            TESTING_MALLOC_CPU( w2,     magma_tally2DoubleComplex, N );
            ...
            TESTING_MALLOC_PIN( VL,     magma_tally2DoubleComplex, n2 );
            TESTING_MALLOC_PIN( VR,     magma_tally2DoubleComplex, n2 );
            ...
            magma_tally2_zgeev( opts.jobvl, opts.jobvr,
                         N, h_R, lda, w1,
                         VL, lda, VR, lda,
                         h_work, lwork, rwork, &info );
            gpu_time = magma_tally2_wtime() - gpu_time;
            if (info != 0)
                printf("magma_tally2_zgeev returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                if ( opts.jobvr == Magma_tally2Vec ) {
                    ...
                    lapackf77_zget22( Magma_tally2NoTransStr, Magma_tally2NoTransStr, Magma_tally2NoTransStr,
                                      &N, h_A, &lda, VR, &lda, w1,
                    ...
                    result[1] = -1.;
                    for( int j = 0; j < N; ++j ) {
                        tnrm = magma_tally2_cblas_dznrm2( N, &VR[j*lda], ione );
                        ...
                        for( int jj = 0; jj <N; ++jj ) {
                            ...
                            if (MAGMA_tally2_Z_IMAG(VR[jj + j*lda])==0. &&
                                fabs( MAGMA_tally2_Z_REAL(VR[jj+j*lda]) ) > vrmx)
                            {
                                vrmx = fabs( MAGMA_tally2_Z_REAL( VR[jj+j*lda] ) );
                            }
                        }
                        if (vrmx / vmx < 1. - ulp*2.)
                            result[1] = ulpinv;
                ...
                if ( opts.jobvl == Magma_tally2Vec ) {
                    ...
                    lapackf77_zget22( Magma_tally2ConjTransStr, Magma_tally2NoTransStr, Magma_tally2ConjTransStr,
                                      &N, h_A, &lda, VL, &lda, w1,
                    ...
                    result[3] = -1.;
                    for( int j = 0; j < N; ++j ) {
                        tnrm = magma_tally2_cblas_dznrm2( N, &VL[j*lda], ione );
                        ...
                        for( int jj = 0; jj < N; ++jj ) {
                            ...
                            if (MAGMA_tally2_Z_IMAG(VL[jj + j*lda])==0. &&
                                fabs( MAGMA_tally2_Z_REAL( VL[jj + j*lda] ) ) > vrmx)
                            {
                                vrmx = fabs( MAGMA_tally2_Z_REAL( VL[jj+j*lda]) );
                            }
                        }
                        if (vrmx / vmx < 1. - ulp*2.)
                            result[3] = ulpinv;
            ...
            if ( opts.check == 2 ) {
                ...
                magma_tally2DoubleComplex *LRE, DUM;
                TESTING_MALLOC_PIN( LRE, magma_tally2DoubleComplex, n2 );
                ...
                magma_tally2_zgeev( Magma_tally2Vec, Magma_tally2Vec,
                             N, h_R, lda, w1,
                             VL, lda, VR, lda,
                             h_work, lwork, rwork, &info );
                if (info != 0)
                ...
                // lapackf77_zlacpy( Magma_tally2UpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                // magma_tally2_zgeev( Magma_tally2NoVec, Magma_tally2NoVec,
                //              N, h_R, lda, w2,
                //              &DUM, 1, &DUM, 1,
                //              h_work, lwork, rwork, &info );
                // if (info != 0)
                //     printf("magma_tally2_zgeev (case N, N) returned error %d: %s.\n",
                //            (int) info, magma_tally2_strerror( info ));
                // 
                // // Do test 5: W(full) = W(partial, W only)
                // result[4] = 1;
                // for( int j = 0; j < N; ++j )
                //     if ( ! MAGMA_tally2_Z_EQUAL( w1[j], w2[j] ))
                //         result[4] = 0;
                
                // ----------
                // Compute eigenvalues and right eigenvectors
                lapackf77_zlacpy( Magma_tally2UpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                magma_tally2_zgeev( Magma_tally2NoVec, Magma_tally2Vec,
                             N, h_R, lda, w2,
                             &DUM, 1, LRE, lda,
                             h_work, lwork, rwork, &info );
                if (info != 0)
                    printf("magma_tally2_zgeev (case N, V) returned error %d: %s.\n",
                           (int) info, magma_tally2_strerror( info ));
                ...
                for( int j = 0; j < N; ++j )
                    for( int jj = 0; jj < N; ++jj )
                        if ( ! MAGMA_tally2_Z_EQUAL( VR[j+jj*lda], LRE[j+jj*lda] ))
                            result[7] = 0;
                
                // ----------
                // Compute eigenvalues and left eigenvectors
                lapackf77_zlacpy( Magma_tally2UpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                magma_tally2_zgeev( Magma_tally2Vec, Magma_tally2NoVec,
                             N, h_R, lda, w2,
                             LRE, lda, &DUM, 1,
                             h_work, lwork, rwork, &info );
                if (info != 0)
                    printf("magma_tally2_zgeev (case V, N) returned error %d: %s.\n",
                           (int) info, magma_tally2_strerror( info ));
                ...
                for( int j = 0; j < N; ++j )
                    for( int jj = 0; jj < N; ++jj )
                        if ( ! MAGMA_tally2_Z_EQUAL( VL[j+jj*lda], LRE[j+jj*lda] ))
                            result[8] = 0;
            ...
            if ( opts.lapack ) {
                ...
                lapackf77_zgeev( lapack_vec_const_tally2(opts.jobvl), lapack_vec_const_tally2(opts.jobvr),
                                 &N, h_A, &lda, w2,
                                 VL, &lda, VR, &lda,
                                 h_work, &lwork, rwork, &info );
