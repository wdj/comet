/*
    -- MAGMA_minproduct (version 1.6.1) --
       ...
       @generated from testing_zungqr_m.cpp normal z -> s, Fri Jan 30 19:00:25 2015
...
int main( int argc, char** argv )
{
    ...
    magma_minproductFloat_ptr dA, dT;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lapackf77_slacpy( Magma_minproductUpperLowerStr, &m, &n, hA, &lda, hR, &lda );
            ...
            // okay that magma_minproduct_sgeqrf_gpu has special structure for R; R isn't used here.
            magma_minproduct_ssetmatrix( m, n, hA, lda, dA, ldda );
            magma_minproduct_sgeqrf_gpu( m, n, dA, ldda, tau, dT, &info );
            if ( info != 0 )
                printf("magma_minproduct_sgeqrf_gpu returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
            magma_minproduct_sgetmatrix( m, n, dA, ldda, hA, lda );
            lapackf77_slacpy( Magma_minproductFullStr, &m, &n, hA, &lda, hR, &lda );
            magma_minproduct_sgetmatrix( nb, min_mn, dT, nb, hT, nb );
            
            gpu_time = magma_minproduct_wtime();
            magma_minproduct_sorgqr_m( m, n, k, hR, lda, tau, hT, nb, &info );
            gpu_time = magma_minproduct_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if ( info != 0 )
                printf("magma_minproduct_sorgqr_gpu returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
