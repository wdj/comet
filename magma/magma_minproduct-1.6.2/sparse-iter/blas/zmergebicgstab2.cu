...
__global__ void
magma_minproduct_zreduce_kernel_spmv1(    
    int Gs,
    int n, 
...
__global__ void
magma_minproduct_zbicgmerge_spmv1_kernel(  
    int n,
    ...
    magma_minproductDoubleComplex * vtmp)
{
    ...
    temp[ Idx ] = ( i < n ) ? v[ i ] * r[ i ] : MAGMA_minproduct_Z_MAKE( 0.0, 0.0);
    __syncthreads();
    if ( Idx < 128 ){
...
__global__ void
magma_minproduct_zbicgstab_alphakernel(  
...
/**
    ...
    @param[in]
    dp          magma_minproductDoubleComplex_ptr
                input vector p
    ...
    @param[in]
    dv          magma_minproductDoubleComplex_ptr
                output vector v
    ...
    magma_minproduct_queue_t queue )
{
    ...
    if ( A.storage_type == Magma_minproduct_CSR)
        magma_minproduct_zbicgmerge_spmv1_kernel<<<Gs, Bs, Ms>>>
    ...
    while( Gs.x > 1 ) {
        ...
        magma_minproduct_zreduce_kernel_spmv1<<< Gs_next.x/2, Bs.x/2, Ms/2 >>> 
                            ( Gs.x, n, aux1, aux2 );
    ...
    magma_minproduct_zcopyvector( 1, aux1, 1, skp, 1 );
    dim3 Bs2( 2 );
    dim3 Gs2( 1 );
    magma_minproduct_zbicgstab_alphakernel<<<Gs2, Bs2, 0>>>( skp );
...
__global__ void
magma_minproduct_zreduce_kernel_spmv2( 
    int Gs,
    int n, 
    magma_minproductDoubleComplex * vtmp,
    magma_minproductDoubleComplex * vtmp2 )
{
    ...
    for( j=0; j<2; j++){
        int i = blockIdx.x * ( blockSize * 2 ) + Idx;   
        temp[Idx+j*(blockSize)] = MAGMA_minproduct_Z_MAKE( 0.0, 0.0);
        while (i < Gs ) {
...
__global__ void
magma_minproduct_zbicgmerge_spmv2_kernel(  
    int n,
    magma_minproductDoubleComplex * dval, 
    magma_minproduct_index_t * drowptr, 
    magma_minproduct_index_t * dcolind,
    ...
    magma_minproductDoubleComplex * vtmp )
{
    ...
    // 2 vectors 
    if (i<n){
            magma_minproductDoubleComplex tmp2 = t[i];
            temp[Idx] = s[i] * tmp2;
            temp[Idx+blockDim.x] = tmp2 * tmp2;
    }
    else{
        for( j=0; j<2; j++)
            temp[Idx+j*blockDim.x] =MAGMA_minproduct_Z_MAKE( 0.0, 0.0);
    }
    __syncthreads();
    if ( Idx < 128 ){
        for( j=0; j<2; j++){
...
__global__ void
magma_minproduct_zbicgstab_omegakernel(  
                    magma_minproductDoubleComplex * skp ){
    int i = blockIdx.x * blockDim.x + threadIdx.x;
...
/**
    ...
    @param[in]
    ds          magma_minproductDoubleComplex_ptr
                input vector s

    @param[in]
    dt          magma_minproductDoubleComplex_ptr
                output vector t
    ...
    magma_minproduct_queue_t queue )
{
    ...
    int b = 1;        
    if ( A.storage_type == Magma_minproduct_CSR)
        magma_minproduct_zbicgmerge_spmv2_kernel<<<Gs, Bs, Ms>>>
    ...
    while( Gs.x > 1 ) {
        Gs_next.x = ( Gs.x+Bs.x-1 )/ Bs.x ;
        if ( Gs_next.x == 1 ) Gs_next.x = 2;
        magma_minproduct_zreduce_kernel_spmv2<<< Gs_next.x/2, Bs.x/2, Ms/2 >>> 
                    ( Gs.x, n, aux1, aux2 );
        Gs_next.x = Gs_next.x /2;
        Gs.x = Gs_next.x;
        b = 1 - b;
    ...
    magma_minproduct_zcopyvector( 1, aux1, 1, skp+6, 1 );
    magma_minproduct_zcopyvector( 1, aux1+n, 1, skp+7, 1 );
    dim3 Bs2( 2 );
    dim3 Gs2( 1 );
    magma_minproduct_zbicgstab_omegakernel<<<Gs2, Bs2, 0>>>( skp );
...
__global__ void
magma_minproduct_zbicgmerge_xrbeta_kernel(  
    int n, 
    magma_minproductDoubleComplex * rr,
    magma_minproductDoubleComplex * r,
    magma_minproductDoubleComplex * p,
    magma_minproductDoubleComplex * s,
    magma_minproductDoubleComplex * t,
    ...
    magma_minproductDoubleComplex * vtmp )
{
    ...
    if( i<n ){
        magma_minproductDoubleComplex sl;
        sl = s[i];
    ...
    // 2 vectors 
    if (i<n){
            magma_minproductDoubleComplex tmp2 = r[i];
            temp[Idx] = rr[i] * tmp2;
            temp[Idx+blockDim.x] = tmp2 * tmp2;
    }
    else{
...
__global__ void
magma_minproduct_zbicgstab_betakernel(  
    magma_minproductDoubleComplex * skp )
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
...
/**
    ...
    @param[in]
    rr          magma_minproductDoubleComplex_ptr
                input vector rr
...
magma_minproduct_zbicgmerge_xrbeta(
    ...
    magma_minproductDoubleComplex_ptr d1,
    magma_minproductDoubleComplex_ptr d2,
    magma_minproductDoubleComplex_ptr rr,
    ...
    magma_minproduct_queue_t queue )
{
    ...
    magma_minproductDoubleComplex_ptr aux1 = d1, aux2 = d2;
    int b = 1;        
    magma_minproduct_zbicgmerge_xrbeta_kernel<<<Gs, Bs, Ms>>>
    ...
    magma_minproduct_zcopyvector( 1, aux1+n, 1, skp+5, 1 );
    dim3 Bs2( 2 );
    dim3 Gs2( 1 );
    magma_minproduct_zbicgstab_betakernel<<<Gs2, Bs2, 0>>>( skp );
