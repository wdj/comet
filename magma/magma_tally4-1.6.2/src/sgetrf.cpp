...
magma_tally4_sgetrf(
    magma_tally4_int_t m, magma_tally4_int_t n, float *A, magma_tally4_int_t lda,
    ...
    *info = 0;
    ...
    if ( (nb <= 1) || (nb >= min(m,n)) ) {
        ...
        if ( ngpu > 1 ) {
            ...
            magma_tally4_sgetrf_m(ngpu, m, n, A, lda, ipiv, info);
            return *info;
        }
        ...
        if (maxdim*maxdim < 2*maxm*maxn) {
            ...
            if (MAGMA_tally4_SUCCESS != magma_tally4_smalloc( &dA, nb*maxm + maxdim*maxdim )) {
            ...
            ldda = maxdim;
            magma_tally4_ssetmatrix( m, n, A, lda, da, ldda );
            ...
            if (MAGMA_tally4_SUCCESS != magma_tally4_smalloc( &dA, (nb + maxn)*maxm )) {
                /* alloc failed so call non-GPU-resident version */
                magma_tally4_sgetrf_m(ngpu, m, n, A, lda, ipiv, info);
                return *info;
            }
            ...
            magma_tally4_ssetmatrix( m, n, A, lda, da, maxm );
            
            if (MAGMA_tally4_SUCCESS != magma_tally4_smalloc( &dAT, maxm*maxn )) {
                /* alloc failed so call non-GPU-resident version */
            ...
            magma_tally4blas_stranspose( m, n, da, maxm, dAT, ldda );
        }
        ...
        for( j = 0; j < s; j++ ) {
            ...
            if (j > 0) {
                magma_tally4blas_stranspose( nb, cols, dAT(j,j), ldda, dA, cols );

                // make sure that gpu queue is empty
                magma_tally4_device_sync();

                magma_tally4_sgetmatrix_async( m-j*nb, nb, dA, cols, work, lda,
                                        stream[0]);
            ...
            // upload j-th panel
            magma_tally4_ssetmatrix_async( m-j*nb, nb, work, lda, dA, cols,
                                    stream[0]);

            for( i=j*nb; i < j*nb + nb; ++i ) {
                ipiv[i] += j*nb;
            }
            magma_tally4blas_slaswp( n, dAT, ldda, j*nb + 1, j*nb + nb, ipiv, 1 );

            magma_tally4_queue_sync( stream[0] );
            magma_tally4blas_stranspose( cols, nb, dA, cols, dAT(j,j), ldda );
        ...
        if ( nb0 > 0 ) {
            ...
            magma_tally4blas_stranspose( nb0, rows, dAT(s,s), ldda, dA, cols );
            magma_tally4_sgetmatrix( rows, nb0, dA, cols, work, lda );
            ...
            magma_tally4blas_slaswp( n, dAT, ldda, s*nb + 1, s*nb + nb0, ipiv, 1 );

            // upload j-th panel
            magma_tally4_ssetmatrix( rows, nb0, work, lda, dA, cols );
            magma_tally4blas_stranspose( rows, nb0, dA, cols, dAT(s,s), ldda );
        ...
        if (maxdim*maxdim < 2*maxm*maxn) {
            ...
            magma_tally4blas_stranspose( n, m, dAT, ldda, da, maxm );
            magma_tally4_sgetmatrix( m, n, da, maxm, A, lda );
