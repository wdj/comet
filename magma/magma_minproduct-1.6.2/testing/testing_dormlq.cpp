    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int itran = 0; itran < 2; ++itran ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_minproduct_dgelqf( k, mm, A, lda, tau, W, lwork_max, &info );
            if (info != 0)
                printf("magma_minproduct_dgelqf returned error %d: %s.\n",
            ...
            lwork = -1;
            magma_minproduct_dormlq( side[iside], trans[itran],
                          m, n, k,
                          A, lda, tau, R, ldc, W, lwork, &info );
            if (info != 0)
                printf("magma_minproduct_dormlq (lwork query) returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
            ...
            if (info != 0)
                printf("magma_minproduct_dormlq returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
