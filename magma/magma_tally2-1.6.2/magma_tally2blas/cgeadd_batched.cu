...
cgeadd_batched_kernel(
    ...
    const magma_tally2FloatComplex * const *dAarray, int ldda,
    magma_tally2FloatComplex              **dBarray, int lddb )
{
    // dA and dB iterate across row i
    const magma_tally2FloatComplex *dA = dAarray[ blockIdx.y ];
    magma_tally2FloatComplex       *dB = dBarray[ blockIdx.y ];
    int i = blockIdx.x*blockDim.x + threadIdx.x;
    if ( i < m ) {
        dA += i;
        dB += i;
        const magma_tally2FloatComplex *dAend = dA + n*ldda;
        while( dA < dAend ) {
            *dB = alpha*(*dA) + (*dB);
            dA += ldda;
            dB += lddb;
...
/**
    @see magma_tally2blas_cgeadd_batched_q
    @ingroup magma_tally2_caux2
    ********************************************************************/
extern "C" void
