/*
    -- MAGMA_minproduct (version 1.6.2) --
       ...
       @generated from magma_minproduct_zmcsrcompressor_gpu.cu normal z -> s, Sun May  3 11:22:58 2015
       @author Hartwig Anzt
...
__global__ void
magma_minproduct_smcsrgpu_kernel1( int num_rows,
                 float *A_val,
    ...
    if(row<num_rows){
        float zero = MAGMA_minproduct_S_ZERO;
...
__global__ void
magma_minproduct_smcsrgpu_kernel2( int num_rows,
                 magma_minproduct_index_t *B_rowptr,
                 magma_minproduct_index_t *A_rowptr ){
...
__global__ void
magma_minproduct_smcsrgpu_kernel3( int num_rows,
                 float *B_val,
                 magma_minproduct_index_t *B_rowptr,
                 magma_minproduct_index_t *B_colind,
    ...
    magma_minproduct_queue_t queue )
{
    ...
    magma_minproduct_s_matrix B={Magma_minproduct_CSR}, B2={Magma_minproduct_CSR};
    magma_minproduct_s_matrix dA={Magma_minproduct_CSR}, CSRA={Magma_minproduct_CSR};
    ...
    if ( A->memory_location == Magma_minproduct_DEV && A->storage_type == Magma_minproduct_CSR ) {
        ...
        magma_minproduct_smcsrgpu_kernel1<<< grid1, BLOCK_SIZE1, 0, queue >>>
                ( A->num_rows, A->dval, A->drow, A->dcol, B.dval, B.drow, B.dcol );

        // correct the row pointer
        dim3 grid2( 1, 1, 1);
        magma_minproduct_smcsrgpu_kernel2<<< grid2, BLOCK_SIZE2, 0, queue >>>
        ...
        magma_minproduct_smcsrgpu_kernel3<<< grid1, BLOCK_SIZE1, 0, queue >>>
        ...
        CHECK( magma_minproduct_smconvert( *A, &CSRA, A->storage_type, Magma_minproduct_CSR, queue ));
        CHECK( magma_minproduct_smtransfer( *A, &dA, A->memory_location, Magma_minproduct_DEV, queue ));

        CHECK( magma_minproduct_smcsrcompressor_gpu( &dA, queue ));

        magma_minproduct_smfree( &dA, queue );
        magma_minproduct_smfree( A, queue );
        CHECK( magma_minproduct_smtransfer( dA, &CSRA, Magma_minproduct_DEV, A_location, queue ));
        CHECK( magma_minproduct_smconvert( CSRA, A, Magma_minproduct_CSR, A_storage, queue ));
        magma_minproduct_smfree( &dA, queue );
