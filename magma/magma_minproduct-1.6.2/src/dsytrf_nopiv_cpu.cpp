/*
    -- MAGMA_minproduct (version 1.6.1) --
       ...
       @generated from zhetrf_nopiv_cpu.cpp normal z -> d, Fri Jan 30 19:00:17 2015
...
int dsyrk_d(magma_minproduct_uplo_t uplo, magma_minproduct_int_t m, magma_minproduct_int_t n,
            double alpha, double *A, magma_minproduct_int_t lda,
            double beta,  double *C, magma_minproduct_int_t ldc,
            double *D, magma_minproduct_int_t incD)
{
    double *Aik;
    double *Dkk;
    double *Akj;
...
// stores D*L'
int dsyrk_d_workspace(magma_minproduct_uplo_t uplo, magma_minproduct_int_t n, magma_minproduct_int_t k,
                      double alpha, double *A, magma_minproduct_int_t lda,
                      double beta,  double *C, magma_minproduct_int_t ldc,
                      double *work, magma_minproduct_int_t ldw)
{
    ...
    if ( uplo == Magma_minproductLower )
    {
         blasf77_dgemm( Magma_minproductNoTransStr, Magma_minproductNoTransStr, 
                        &n, &n, &k,
                        &c_mone, A,    &lda,
                                 work, &ldw,
                        &c_one,  C,    &ldc );
...
int dsytrf_diag_nopiv(magma_minproduct_uplo_t uplo, magma_minproduct_int_t n, 
                      double *A, magma_minproduct_int_t lda)
{
    ...
    if ( uplo == Magma_minproductLower )
    {
        ...
        for (magma_minproduct_int_t k=n-1; k>0; k--) {
            ...
            // scale off-diagonals
            alpha = done / MAGMA_minproduct_D_REAL( Akk );
            blasf77_dscal(&k, &alpha, Ak1k, &ione);

            // update remaining
            alpha = - MAGMA_minproduct_D_REAL( Akk );
            blasf77_dsyr(Magma_minproductLowerStr, &k, 
        ...
        for (magma_minproduct_int_t k=n-1; k>0; k--) {
            ...
            #endif
            blasf77_dsyr(Magma_minproductUpperStr, &k, 
...
dsytrf_nopiv_cpu(magma_minproduct_uplo_t uplo, magma_minproduct_int_t n, magma_minproduct_int_t ib,
                 double *A, magma_minproduct_int_t lda,
                 magma_minproduct_int_t *info)
{
    ...
    double mzone = -MAGMA_minproduct_D_ONE;
            ...
            *info = dsytrf_diag_nopiv(uplo, sb, &A(i, i), lda);
            ...
            if ( i + sb < n ) {
                ...
                for (magma_minproduct_int_t k=0; k<sb; k++) {
                    ...
                    for (magma_minproduct_int_t ii=i+sb; ii<n; ii++) A(i+k, ii) = MAGMA_minproduct_D_CNJG(A(ii, i+k));
                    #endif
                    alpha = done / MAGMA_minproduct_D_REAL(A(i+k, i+k));
                    blasf77_dscal(&height, &alpha, &A(i+sb, i+k), &ione);
                    A(i+k, i+k) = MAGMA_minproduct_D_MAKE(MAGMA_minproduct_D_REAL(A(i+k, i+k)), 0.0);
                }

                /* Update the trailing submatrix A22 = A22 - A21 * D11 * A21' */
                #ifdef DSYRK_D_WORKSPACE
                dsyrk_d_workspace(Magma_minproductLower, height, sb,
                                  mzone, &A(i+sb, i), lda,    // A21
                                  zone,  &A(i+sb, i+sb), lda, // A22
                                         &A(i, i+sb), lda);   // workspace, I am writing on upper part :)
                #else
                dsyrk_d(Magma_minproductLower, height, sb,
                        mzone, &A(i+sb, i), lda,    // A21
                        zone,  &A(i+sb, i+sb), lda, // A22
                               &A(i, i), lda+1);    // D11
                #endif
            ...
            *info = dsytrf_diag_nopiv(uplo, sb, &A(i, i), lda);
            ...
            if ( i + sb < n ) {
                ...
                for (magma_minproduct_int_t k=0; k<sb; k++) {
                    #define DSYRK_D_WORKSPACE
                    #ifdef DSYRK_D_WORKSPACE
                    for (magma_minproduct_int_t ii=i+sb; ii<n; ii++) A(ii, i+k) = MAGMA_minproduct_D_CNJG(A(i+k, ii));
                    #endif
                    alpha = done / MAGMA_minproduct_D_REAL(A(i+k, i+k));
                ...
                /* Update the trailing submatrix A22 = A22 - A21 * D11 * A21' */
                #ifdef DSYRK_D_WORKSPACE
                dsyrk_d_workspace(Magma_minproductUpper, height, sb,
                                  mzone, &A(i, i+sb), lda,    // A21
                                  zone,  &A(i+sb, i+sb), lda, // A22
                                         &A(i+sb, i), lda);   // workspace, I am writing on upper part :)
                #else
                dsyrk_d(Magma_minproductUpper, height, sb,
                        mzone, &A(i, i+sb), lda,    // A21
                        zone,  &A(i+sb, i+sb), lda, // A22
                               &A(i, i), lda+1);    // D11
                #endif
