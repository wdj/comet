...
/**
    ...
    lwork   INTEGER
            ...
            through magma_tally4_get_sgeqlf_nb(M).
    \n
    ...
    @ingroup magma_tally4_sgeqlf_comp
    ********************************************************************/
    ...
    magma_tally4_int_t *info)
{
    ...
    magma_tally4Float_ptr dA, dwork;
    ...
    *info = 0;
    ...
    if (*info == 0) {
        if (k == 0)
            ...
            work[0] = MAGMA_tally4_S_MAKE( max(n*nb, 2*nb*nb), 0 );
        }
    ...
    if (MAGMA_tally4_SUCCESS != magma_tally4_smalloc( &dA, (n)*ldda + nb*lddwork )) {
    ...
    if ( (nb > 1) && (nb < k) ) {
        ...
        for (i = k - kk + ki; i >= k -kk; i -= nb) {
            ...
            if (i < k - kk + ki) {
                ...
                magma_tally4_sgetmatrix_async( m-rows, ib,
                                        dA(rows, n-k+i), ldda,
                ...
                magma_tally4_slarfb_gpu( Magma_tally4Left, Magma_tally4ConjTrans, Magma_tally4Backward, Magma_tally4Columnwise,
                                  rows, cols, old_ib,
            ...
            if (cols > 0) {
                ...
                lapackf77_slarft( Magma_tally4BackwardStr, Magma_tally4ColumnwiseStr,
                                  &rows, &ib,
                                  A(0, cols), &lda, tau + i, work, &ib);

                spanel_to_q_tally4( Magma_tally4Lower, ib, A(rows-ib,cols), lda, work+ib*ib);
                magma_tally4_ssetmatrix( rows, ib,
                                  A(0,cols),  lda,
                                  dA(0,cols), ldda );
                sq_to_panel_tally4( Magma_tally4Lower, ib, A(rows-ib,cols), lda, work+ib*ib);

                // Send the triangular part on the GPU
                magma_tally4_ssetmatrix( ib, ib, work, ib, dwork(0), lddwork );
        ...
        magma_tally4_sgetmatrix( m, nu, dA(0,0), ldda, A(0,0), lda );
    } else {
        mu = m;
        nu = n;
    }
