...
dtrtri_diag_lower_device(
    magma_tally4_diag_t diag, int n, const double *A, int lda, double *d_dinvA)
{
    int tx = threadIdx.x;
    int bx = blockIdx.x;
    int blk_ind = bx*IB;
    ...
    for( int j=0; j < IB; j++ ) {
        if (tx >= j && ind < n) {
            ...
            sB[tx + j*IB] = MAGMA_tally4_D_ZERO;
        }
    }
    __syncthreads();
    ...
    if (diag == Magma_tally4Unit) {
        sB[tx + tx*IB] = MAGMA_tally4_D_ONE;
    }
    else {
        if ( sB[tx + tx*IB] == MAGMA_tally4_D_ZERO ) {  // singular or outside matrix
            sB[tx + tx*IB] = MAGMA_tally4_D_ONE;
        }
        else {
            sB[tx + tx*IB] = MAGMA_tally4_D_ONE / sB[tx + tx*IB];
        }
    }
...
dtrtri_diag_lower_kernel_batched(
    magma_tally4_diag_t diag, int n, double const * const * dA_array, int lda, double **dinvA_array)
{
