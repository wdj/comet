    ...
    magma_tally2DoubleComplex *A, int lda )
{
    ...
    bool full = (iby + BLK_Y <= n && (ind >= iby + BLK_Y || ind + BLK_X <= iby || MAGMA_tally2_Z_EQUAL( offdiag, diag )));
...
*/
__global__
void zlaset_full_kernel_tally2(
    int m, int n,
    magma_tally2DoubleComplex offdiag, magma_tally2DoubleComplex diag,
    magma_tally2DoubleComplex *dA, int ldda )
{
    zlaset_full_device(m, n, offdiag, diag, dA, ldda);
}

__global__
void zlaset_lower_kernel_tally2(
    int m, int n,
    magma_tally2DoubleComplex offdiag, magma_tally2DoubleComplex diag,
    magma_tally2DoubleComplex *dA, int ldda )
{
...
__global__
void zlaset_upper_kernel_tally2(
    int m, int n,
    magma_tally2DoubleComplex offdiag, magma_tally2DoubleComplex diag,
    magma_tally2DoubleComplex *dA, int ldda )
{
...
*/
__global__
void zlaset_full_kernel_tally2_batched(
    int m, int n,
    magma_tally2DoubleComplex offdiag, magma_tally2DoubleComplex diag,
    magma_tally2DoubleComplex **dAarray, int ldda )
{
...
__global__
void zlaset_lower_kernel_tally2_batched(
    int m, int n,
    magma_tally2DoubleComplex offdiag, magma_tally2DoubleComplex diag,
    magma_tally2DoubleComplex **dAarray, int ldda )
{
...
__global__
void zlaset_upper_kernel_tally2_batched(
    int m, int n,
    magma_tally2DoubleComplex offdiag, magma_tally2DoubleComplex diag,
    magma_tally2DoubleComplex **dAarray, int ldda )
{
...
/**
    ...
    ********************************************************************/
extern "C"
void magma_tally2blas_zlaset_q(
    ...
    if (uplo == Magma_tally2Lower) {
        for( int i=0; i < super_grid.x; ++i ) {
            ...
            for( int j=0; j < super_grid.y && j <= i; ++j ) {  // from left to diagonal
                ...
                if ( i == j ) {  // diagonal super block
                    zlaset_lower_kernel_tally2<<< grid, threads, 0, queue >>>
                        ( mm, nn, offdiag, diag, dA(i*super_NB, j*super_NB), ldda );
                }
                else {           // off diagonal super block
                    zlaset_full_kernel_tally2<<< grid, threads, 0, queue >>>
        ...
        for( int i=0; i < super_grid.x; ++i ) {
            ...
            for( int j=i; j < super_grid.y; ++j ) {  // from diagonal to right
                ...
                grid.y = magma_tally2_ceildiv( nn, BLK_Y );
                if ( i == j ) {  // diagonal super block
                    zlaset_upper_kernel_tally2<<< grid, threads, 0, queue >>>
                        ( mm, nn, offdiag, diag, dA(i*super_NB, j*super_NB), ldda );
                }
                else {           // off diagonal super block
                    zlaset_full_kernel_tally2<<< grid, threads, 0, queue >>>
        ...
        if ( m == ldda &&
             MAGMA_tally2_Z_EQUAL( offdiag, MAGMA_tally2_Z_ZERO ) &&
             MAGMA_tally2_Z_EQUAL( diag,    MAGMA_tally2_Z_ZERO ) )
        {
            size_t size = m*n;
            cudaError_t err = cudaMemsetAsync( dA, 0, size*sizeof(magma_tally2DoubleComplex), queue );
...
/**
    @see magma_tally2blas_zlaset_q
    @ingroup magma_tally2_zaux2
    ********************************************************************/
extern "C"
void magma_tally2blas_zlaset(
    magma_tally2_uplo_t uplo, magma_tally2_int_t m, magma_tally2_int_t n,
    magma_tally2DoubleComplex offdiag, magma_tally2DoubleComplex diag,
    magma_tally2DoubleComplex_ptr dA, magma_tally2_int_t ldda )
{
    magma_tally2blas_zlaset_q( uplo, m, n, offdiag, diag, dA, ldda, magma_tally2_stream );
}


////////////////////////////////////////////////////////////////////////////////////////
extern "C"
void magma_tally2blas_zlaset_batched(
    magma_tally2_uplo_t uplo, magma_tally2_int_t m, magma_tally2_int_t n,
    magma_tally2DoubleComplex offdiag, magma_tally2DoubleComplex diag,
    ...
    magma_tally2_int_t batchCount, magma_tally2_queue_t queue)
{
    ...
    if (uplo == Magma_tally2Lower) {
        zlaset_lower_kernel_tally2_batched<<< grid, threads, 0, queue >>> (m, n, offdiag, diag, dAarray, ldda);
    }
    else if (uplo == Magma_tally2Upper) {
        zlaset_upper_kernel_tally2_batched<<< grid, threads, 0, queue >>> (m, n, offdiag, diag, dAarray, ldda);
    }
    else {
        zlaset_full_kernel_tally2_batched<<< grid, threads, 0, queue >>> (m, n, offdiag, diag, dAarray, ldda);
    }
}
