    ...
    magma_tally2_int_t *info )
{
    ...
    double z_one = MAGMA_tally2_D_MAKE(  1.0, 0.0 );
    double mz_one = MAGMA_tally2_D_MAKE( -1.0, 0.0 );
    double             one =  1.0;
    double             m_one = -1.0;
    ...
    double *d_lP[Magma_tally2MaxGPUs], *dlpanel, *dlpanels[Magma_tally2MaxGPUs];
    ...
    *info = 0;
    ...
    if (MAGMA_tally2_SUCCESS != magma_tally2_dmalloc_pinned( &panel, 2 * nb * ldpanel )) {
    ...
    if ((nb <= 1) || (nb >= n)) {
        // Use unblocked code.
        magma_tally2_dgetmatrix( n, n, dlA(0, 0, 0), ldda, panel, ldpanel);
        lapackf77_dpotrf( uplo_, &n, panel, &ldpanel, info);
        magma_tally2_dsetmatrix( n, n, panel, ldpanel, dlA(0, 0, 0), ldda );
    } else {
        for( d = 0; d < ngpu; d++ ) {
            ...
            if (MAGMA_tally2_SUCCESS != magma_tally2_dmalloc( &d_lP[d], nb * ldda )) {
                for( j = 0; j < d; j++ ) {
        ...
        if (uplo == Magma_tally2Upper) {
            ...
            if ( n > nb ) {
                ...
                magma_tally2_setdevice(0);
                tmppanel = tmppanel0;
                magma_tally2_dgetmatrix_async(n, nb,
                        dlA(0, 0, 0), ldda,
                        tmppanel(0),  ldpanel,
                        queues[0][0] );
            }
            ...
            for (j = 0; (j + nb) < n; j += nb) {
                ...
                if (j > 0 && prevtrsmrows > crosspoint) {
                    ...
                    blasf77_dgemm( Magma_tally2NoTransStr, Magma_tally2ConjTransStr,
                            &rows, &nb, &nb,
                ...
                #endif
                lapackf77_dpotrf(Magma_tally2LowerStr, &nb, tmppanel(j), &ldpanel, info);
                if (*info != 0) {
                    *info = *info + j;
                    break;
                }
                ...
                if (trsmrows > 0) {
                    blasf77_dtrsm(Magma_tally2RightStr, Magma_tally2LowerStr, Magma_tally2ConjTransStr, Magma_tally2NonUnitStr,
                                  &trsmrows, &nb,
                                  &z_one, tmppanel(j), &ldpanel,
                ...
                for (igpu = 0; igpu < ngpu; igpu++, d = (d + 1) % ngpu ) {
                    ...
                    if (myrows > 0) {
                        magma_tally2_setdevice(d);
                        magma_tally2_dsetmatrix_async(myrows, nb,
                ...
                if ( (j + nb) < n) {
                    ...
                    if (j + nb + nb < n) {
                        ...
                        if (trsmrows <= crosspoint) {
                            ...
                            #ifdef  DSYRK_ON_DIAG
                            magma_tally2_dsyrk( Magma_tally2Lower, Magma_tally2NoTrans,
                                    nb, nb,
                                    m_one, dlpanel, ldda,
                        ...
                        magma_tally2_dgetmatrix_async(rows-nb, nb,
                    ...
                    if (n - offset > 0) {
                        ...
                        //magma_tally2_dsyrk(Magma_tally2Lower, Magma_tally2NoTrans, n - offset, nb,
                        //        m_one, dlpanel, ldda,
            ...
            if (j < n && *info == 0) {
                ...
                #endif
                magma_tally2_dgetmatrix(rows, rows, dlA(id, j, j_local), ldda, panel(j), ldpanel);
                lapackf77_dpotrf(Magma_tally2LowerStr, &rows, panel(j), &ldpanel, info);
                magma_tally2_dsetmatrix(rows, rows, panel(j), ldpanel, dlA(id, j, j_local), ldda);
                #if defined (ENABLE_TIMER)
