...
#define dF(i_, j_) (dF + (i_) + (j_)*(lddf))
    ...
    magma_tally2_smalloc( &lsticcs, 1+256*(n+255)/256 );
    ...
    while( k < nb && lsticc == 0 ) {
        ...
        if (pvt != k) {
             magma_tally2blas_sswap( k+1, dF(pvt,0), lddf, dF(k,0), lddf);
        ...
        if (k > 0) {
            magma_tally2blas_sgemv_conjv( m-rk, k,
        ...
        magma_tally2_slarfg_gpu(m-rk, dA(rk, k), dA(rk + 1, k), &dtau[k], &dvn1[k], &dAkk[k]);
        magma_tally2_ssetvector( 1, &c_one,   1, dA(rk, k), 1 );

        /* Compute Kth column of F:
           Compute  F(K+1:N,K) := tau(K)*A(RK:M,K+1:N)'*A(RK:M,K) on the GPU */
        if (k < n-1 || k > 0 ) magma_tally2_sgetvector( 1, &dtau[k], 1, &tauk, 1 );
        if (k < n-1) {
        ...
        if (k > 0) {
            ...
            magma_tally2_sgemv( Magma_tally2ConjTrans, m-rk, k,
                         z__1,   dA(rk, 0), ldda,
                                 dA(rk, k), ione,
                         c_zero, dauxv, ione );*/

            magma_tally2_sgemv_kernel3<<< k, BLOCK_SIZE, 0, magma_tally2_stream >>>(m-rk, dA(rk, 0), ldda,
                                                                      dA(rk, k), dauxv, dtau+k);

            /* I think we only need stricly lower-triangular part */
            magma_tally2_sgemv( Magma_tally2NoTrans, n-k-1, k,
                         c_one, dF(k+1,0), lddf,
                                dauxv,     ione,
                         c_one, dF(k+1,k), ione );
        }
       ...
       /* Update the current row of A:
        ...
        //*dA(rk, k) = Akk;
        //magma_tally2_ssetvector( 1, &Akk, 1, dA(rk, k), 1 );
        //magma_tally2blas_slacpy(Magma_tally2UpperLower, 1, 1, dAkk, 1, dA(rk, k), 1);

        ++k;
    }
    // restore the diagonals
    magma_tally2_scopymatrix( 1, k, dAkk, 1, dA(offset, 0), ldda+1 );
