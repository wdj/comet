...
/**
    ...
    @ingroup magma_tally4_zgeev_driver
    ********************************************************************/
    ...
    magma_tally4_int_t *info )
{
    ...
    *info = 0;
    ...
    /* Compute workspace */
    nb = magma_tally4_get_zgehrd_nb( n );
    if (*info == 0) {
        minwrk = (1+nb)*n;
        work[0] = MAGMA_tally4_Z_MAKE( minwrk, 0 );
    ...
    magma_tally4DoubleComplex *dT;
    if (MAGMA_tally4_SUCCESS != magma_tally4_zmalloc( &dT, nb*n )) {
    ...
    if (MAGMA_tally4_SUCCESS != magma_tally4_zmalloc_cpu( &T, nb*n )) {
    ...
    smlnum = magma_tally4_dsqrt( smlnum ) / eps;
    bignum = 1. / smlnum;
    ...
    if (wantvl) {
        ...
        lapackf77_zlacpy( Magma_tally4LowerStr, &n, &n, A, &lda, VL, &ldvl );
         ...
         *  - including N reserved for gebal/gebak, unused by zunghr */
            ...
            // Version 3 - LAPACK consistent MAGMA_tally4 HRD + T matrices stored
            magma_tally4_zunghr( n, ilo, ihi, VL, ldvl, &work[itau], dT, nb, &ierr );
        #elif defined(Version5)
            // Version 5 - Multi-GPU, T on host
            magma_tally4_zunghr_m( n, ilo, ihi, VL, ldvl, &work[itau], T, nb, &ierr );
        #endif
        ...
        #elif defined(Version3) || defined(Version4)
            // Version 3 - LAPACK consistent MAGMA_tally4 HRD + T matrices stored
            magma_tally4_zunghr( n, ilo, ihi, VR, ldvr, &work[itau], dT, nb, &ierr );
        #elif defined(Version5)
            // Version 5 - Multi-GPU, T on host
            magma_tally4_zunghr_m( n, ilo, ihi, VR, ldvr, &work[itau], T, nb, &ierr );
        #endif
    ...
    if (wantvl || wantvr) {
        ...
        magma_tally4_ztrevc3_mt( side, Magma_tally4BacktransVec, select, n, A, lda, VL, ldvl,
                          VR, ldvr, n, &nout, &work[iwrk], liwrk, &rwork[irwork], &ierr );
        #elif TREVC_VERSION == 5
        magma_tally4_ztrevc3_mt_gpu( side, Magma_tally4BacktransVec, select, n, A, lda, VL, ldvl,
                              VR, ldvr, n, &nout, &work[iwrk], liwrk, &rwork[irwork], &ierr );
        #else
    ...
    if (wantvl) {
        ...
        for (i = 0; i < n; ++i) {
            scl = 1. / magma_tally4_cblas_dznrm2( n, VL(0,i), 1 );
            blasf77_zdscal( &n, &scl, VL(0,i), &ione );
            for (k = 0; k < n; ++k) {
                /* Computing 2nd power */
                d__1 = MAGMA_tally4_Z_REAL( *VL(k,i) );
                d__2 = MAGMA_tally4_Z_IMAG( *VL(k,i) );
                rwork[irwork + k] = d__1*d__1 + d__2*d__2;
            }
            k = blasf77_idamax( &n, &rwork[irwork], &ione ) - 1;  // subtract 1; k is 0-based
            tmp = MAGMA_tally4_Z_CNJG( *VL(k,i) ) / magma_tally4_dsqrt( rwork[irwork + k] );
            blasf77_zscal( &n, &tmp, VL(0,i), &ione );
            *VL(k,i) = MAGMA_tally4_Z_MAKE( MAGMA_tally4_Z_REAL( *VL(k,i) ), 0 );
        }
    }
    ...
    if (wantvr) {
        ...
        for (i = 0; i < n; ++i) {
            scl = 1. / magma_tally4_cblas_dznrm2( n, VR(0,i), 1 );
            blasf77_zdscal( &n, &scl, VR(0,i), &ione );
            for (k = 0; k < n; ++k) {
                /* Computing 2nd power */
                d__1 = MAGMA_tally4_Z_REAL( *VR(k,i) );
                d__2 = MAGMA_tally4_Z_IMAG( *VR(k,i) );
                rwork[irwork + k] = d__1*d__1 + d__2*d__2;
            }
            k = blasf77_idamax( &n, &rwork[irwork], &ione ) - 1;  // subtract 1; k is 0-based
            tmp = MAGMA_tally4_Z_CNJG( *VR(k,i) ) / magma_tally4_dsqrt( rwork[irwork + k] );
            blasf77_zscal( &n, &tmp, VR(0,i), &ione );
            *VR(k,i) = MAGMA_tally4_Z_MAKE( MAGMA_tally4_Z_REAL( *VR(k,i) ), 0 );
        }
    }
...
CLEANUP:
    ...
    work[0] = MAGMA_tally4_Z_MAKE( (double) minwrk, 0. );  // TODO use optwrk as in dgeev
