...
#include "magma_tally2_sbulgeinc.h"
    ...
    magma_tally2_int_t *info)
{
    ...
    */
    ...
    magma_tally2_event_t myevent[2];
    ...
    magma_tally2_int_t lddw = 0;
    magma_tally2_int_t lddwork  = ((NE+31)/32)*32;
    magma_tally2_int_t dwVTsiz  = lddv*Vblksiz; // lddv*lddv + lddv*lddwork; (v2) // lddv*Vblksiz; (v1,v3)
    magma_tally2_int_t dworksiz = lddwork*Vblksiz;  // lddv*Vblksiz; (v2)   // NE*Vblksiz=lddwork*Vblksiz; (v1,v3)

    if (MAGMA_tally2_SUCCESS != magma_tally2_smalloc( &dwork, 2*dworksiz + 2*dwVTsiz +  2*Vchunksiz* (Vblksiz* (lddv+lddt)) )) {
       printf ("!!!!  magma_tally2_sbulge_applyQ magma_tally2_alloc failed for: dwork\n" );
       exit(-1);
    }
    ...
    //magma_tally2_int_t nr_bl = magma_tally2_ceildiv(NE,10000);        //nr of blocks
    magma_tally2_int_t sz_bl = NE; //magma_tally2_ceildiv(NE,nr_bl*64)*64; //maximum size of blocks (to have blocks of around the same size and multiple of 64)
                ...
                for (blki=mt; blki > 0; blki--) {
                    ...
                    if (ncpy == 0) {
                        ...
                        if (mysiz > 0) {
                            ...
                            magma_tally2_ssetmatrix_async(vld, Vblksiz, V(vpos), vld, dV1, vld, stream[1]);
                            magma_tally2_ssetmatrix_async(tld, Vblksiz, T(tpos), tld, dT1, tld, stream[1]);
                    ...
                    if (blkid == copyst) {
                        ...
                        if (mysiz > 0) {
                            ...
                            if (flip == 0) { // now I am working on dV0 so copy the next and put it on dV1
                                ...
                                magma_tally2_ssetmatrix_async(vld, Vblksiz, V(vpos), vld, dV0, vld, stream[0]);
                                magma_tally2_ssetmatrix_async(tld, Vblksiz, T(tpos), tld, dT0, tld, stream[0]);
                            }
                        }
                    }
                    ...
                    if ((Vm > 0) && (Vn > 0)) {
                        ...
                        if (flip == 0) {
                            magma_tally2blasSetKernelStream(stream[0]);
                            magma_tally2_queue_wait_event( stream[0], myevent[1] );
                            for (magma_tally2_int_t i=0; i < NE; i += sz_bl) {
                                ib = min(sz_bl, NE-i);
                                lddw = min(lddwork,sz_bl);
                                //magma_tally2_slarfb_gpu( Magma_tally2Left, Magma_tally2NoTrans, Magma_tally2Forward, Magma_tally2Columnwise, Vm, ib, Vn, dV0+lcvpos, lddv, dT0+lctpos, lddt, dE(myrow,i), ldde, dwork0, lddw);
                                magma_tally2_slarfb_gpu_gemm( Magma_tally2Left, Magma_tally2NoTrans, Magma_tally2Forward, Magma_tally2Columnwise, Vm, ib, Vn, dV0+lcvpos, lddv, dT0+lctpos, lddt, dE(myrow,i), ldde, dwork0, lddw, dwvt0, lddv);
                            }
                            magma_tally2_event_record( myevent[0], stream[0] );
                        } else {
                            magma_tally2blasSetKernelStream(stream[1]);
                            magma_tally2_queue_wait_event( stream[1], myevent[0] );
                            for (magma_tally2_int_t i=0; i < NE; i += sz_bl) {
                                ib = min(sz_bl, NE-i);
                                lddw = min(lddwork,sz_bl);
                                //magma_tally2_slarfb_gpu( Magma_tally2Left, Magma_tally2NoTrans, Magma_tally2Forward, Magma_tally2Columnwise, Vm, ib, Vn, dV1+lcvpos, lddv, dT1+lctpos, lddt, dE(myrow,i), ldde, dwork1, lddw);
                                magma_tally2_slarfb_gpu_gemm( Magma_tally2Left, Magma_tally2NoTrans, Magma_tally2Forward, Magma_tally2Columnwise, Vm, ib, Vn, dV1+lcvpos, lddv, dT1+lctpos, lddt, dE(myrow,i), ldde, dwork1, lddw, dwvt1, lddv);
                            }
                            magma_tally2_event_record( myevent[1], stream[1] );
                        }
                    }  // end for (Vm &Vn) > 0
                } // end for blki
            } // end for blkj
            ...
            for (blki = mt; blki > 0; blki--) {
                ...
                for (blkj = nt-1; blkj >= 0; blkj--) {
                     ...
                     * by the bulgechasing function*/
                        ...
                        magma_tally2_ssetmatrix_async(Vm, Vn, V(vpos), ldv, dV0, lddv, NULL);
                        magma_tally2_ssetmatrix_async(Vn,  Vn, T(tpos), ldt, dT0, lddt, NULL);
                        //printf("voici blki %d  rownbm %d mycol %d  coled %d  blkid %d vpos %d  tpos %d\n", blki, rownbm, mycol, coled, blkid, vpos, tpos);
                        for (magma_tally2_int_t i=0; i < NE; i += sz_bl) {
                            ib = min(sz_bl, NE-i);
                            magma_tally2_slarfb_gpu( Magma_tally2Left, Magma_tally2NoTrans, Magma_tally2Forward, Magma_tally2Columnwise, Vm, ib, Vn, dV0, lddv, dT0, lddt, dE(myrow,i), ldde, dwork, NE);
                        }
                    } // end for (Vm &Vn) > 0
                } // end for blkj
            } // end for blki
            ...
            for (blkj=0; blkj < nt; blkj++) {
                ...
                for (blki=1; blki <= mt; blki++) {
                    ...
                    if ((Vm > 0) && (Vn > 0)) {
                        ...
                        magma_tally2_ssetmatrix_async(Vm, Vn, V(vpos), ldv, dV0, lddv, NULL);
                        magma_tally2_ssetmatrix_async(Vn,  Vn, T(tpos), ldt, dT0, lddt, NULL);
                        magma_tally2_slarfb_gpu( Magma_tally2Right, Magma_tally2NoTrans, Magma_tally2Forward, Magma_tally2Columnwise, NE, Vm, Vn, dV0, lddv, dT0, lddt, dE(0, myrow), ldde, dwork, NE);
                    } // end for (Vm &Vn) > 0
                } // end for blki
            } // end fo blkj
        } // end of version 91
    ...
    magma_tally2_event_destroy( myevent[0] );
    magma_tally2_event_destroy( myevent[1] );
