...
/**
    ...
    @param[in,out]
    precond_par magma_tally4_d_precond_par*
    ...
    @ingroup magma_tally4sparse_dsyev
    ********************************************************************/
...
#define magma_tally4_d_bspmv_tuned(m, n, alpha, A, X, beta, AX, queue)       {        \
            magma_tally4_d_matrix x={Magma_tally4_CSR}, ax={Magma_tally4_CSR};                                       \
            ...
            CHECK( magma_tally4_d_spmv(alpha, A, x, beta, ax, queue ));                   \
}
...
//**************************************************************
    ...
    double c_one = MAGMA_tally4_D_ONE, c_zero = MAGMA_tally4_D_ZERO;
    double c_mone = MAGMA_tally4_D_MAKE(-1.0, 0.0);
    ...
    CHECK( magma_tally4_dmalloc_pinned( &hwork   ,        lwork ));
    CHECK( magma_tally4_dmalloc(        &blockAX   ,        m*n ));
    CHECK( magma_tally4_dmalloc(        &blockAR   ,        m*n ));
    CHECK( magma_tally4_dmalloc(        &blockAP   ,        m*n ));
    CHECK( magma_tally4_dmalloc(        &blockR    ,        m*n ));
    CHECK( magma_tally4_dmalloc(        &blockP    ,        m*n ));
    CHECK( magma_tally4_dmalloc(        &blockW    ,        m*n ));
    CHECK( magma_tally4_dmalloc(        &dwork     ,        m*n ));
...
//**********************************************************+
    ...
    CHECK( magma_tally4_dmalloc_pinned(&hW, n*n));
    CHECK( magma_tally4_dmalloc_pinned(&gevectors, 9*n*n));
    CHECK( magma_tally4_dmalloc_pinned(&h_gramB  , 9*n*n));

    // === Allocate GPU workspace ===
    CHECK( magma_tally4_dmalloc(&gramM, n * n));
    CHECK( magma_tally4_dmalloc(&gramA, 9 * n * n));
    CHECK( magma_tally4_dmalloc(&gramB, 9 * n * n));
    ...
    magma_tally4_dgegqr_gpu(ikind, m, n, blockX, m, dwork, hwork, &info );

    //magma_tally4_dorthomgs( m, n, blockX, queue );
    
    magma_tally4_d_bspmv_tuned(m, n, c_one, A, blockX, c_zero, blockAX, queue );

    // === Compute the Gram matrix = (X, AX) & its eigenstates ===
    magma_tally4_dgemm(Magma_tally4ConjTrans, Magma_tally4NoTrans, n, n, m,
                c_one,  blockX, m, blockAX, m, c_zero, gramM, n);

    magma_tally4_dsyevd_gpu( Magma_tally4Vec, Magma_tally4Upper,
    ...
    // === Update  X =  X * evectors ===
    magma_tally4_dgemm(Magma_tally4NoTrans, Magma_tally4NoTrans, m, n, n,
    ...
    for(iterationNumber = 1; iterationNumber < maxIterations; iterationNumber++)
        {
            ...
            magma_tally4blas_dlacpy( Magma_tally4UpperLower, m, n, blockAX, m, blockR, m);

/*
            for(magma_tally4_int_t i=0; i<n; i++) {
               magma_tally4_daxpy(m, MAGMA_tally4_D_MAKE(-evalues[i],0), blockX+i*m, 1, blockR+i*m, 1);
            }
  */
            ...
            CHECK( magma_tally4_dlobpcg_res( m, n, eval_gpu, blockX, blockR, eval_gpu, queue ));

            magma_tally4blas_dnrm2_cols(m, n, blockR, m, residualNorms(0, iterationNumber));

            // === remove the residuals corresponding to already converged evectors
            CHECK( magma_tally4_dcompact(m, n, blockR, m,
            ...
            //magma_tally4blas_dlacpy( Magma_tally4UpperLower, m, cBlockSize, blockR, m, blockW, m);
            //SWAP(blockW, blockR);
            
                // preconditioner
            magma_tally4_d_matrix bWv={Magma_tally4_CSR}, bRv={Magma_tally4_CSR};
            ...
            CHECK( magma_tally4_d_applyprecond_left( A, bRv, &bWv, precond_par, queue ));
            CHECK( magma_tally4_d_applyprecond_right( A, bWv, &bRv, precond_par, queue ));
            
            // === make the preconditioned residuals orthogonal to X
            if( precond_par->solver != Magma_tally4_NONE){
                magma_tally4_dgemm(Magma_tally4ConjTrans, Magma_tally4NoTrans, n, cBlockSize, m,
                            c_one, blockX, m, blockR, m, c_zero, gramB(0,0), ldgram);
                magma_tally4_dgemm(Magma_tally4NoTrans, Magma_tally4NoTrans, m, cBlockSize, n,
                            c_mone, blockX, m, gramB(0,0), ldgram, c_one, blockR, m);
            }

            // === make the active preconditioned residuals orthonormal

            magma_tally4_dgegqr_gpu(ikind, m, cBlockSize, blockR, m, dwork, hwork, &info );
#if defined(PRECISION_s)
// re-orthogonalization
            SWAP(blockX, dwork);
            magma_tally4_dgegqr_gpu(ikind, m, cBlockSize, blockR, m, dwork, hwork, &info );
#endif
            //magma_tally4_dorthomgs( m, cBlockSize, blockR, queue );

            // === compute AR
            magma_tally4_d_bspmv_tuned(m, cBlockSize, c_one, A, blockR, c_zero, blockAR, queue );

            if (!restart) {
                // === compact P & AP as well
                CHECK( magma_tally4_dcompactActive(m, n, blockP,  m, activeMask, queue ));
                CHECK( magma_tally4_dcompactActive(m, n, blockAP, m, activeMask, queue ));
                ...
                magma_tally4_dgemm(Magma_tally4ConjTrans, Magma_tally4NoTrans, cBlockSize, cBlockSize, m,
                            c_one, blockR, m, blockP, m, c_zero, gramB(0,0), ldgram);
                magma_tally4_dgemm(Magma_tally4NoTrans, Magma_tally4NoTrans, m, cBlockSize, cBlockSize,
                            c_mone, blockR, m, gramB(0,0), ldgram, c_one, blockP, m);
                */

                // === Make P orthonormal & properly change AP (without multiplication by A)
                magma_tally4_dgegqr_gpu(ikind, m, cBlockSize, blockP, m, dwork, hwork, &info );
#if defined(PRECISION_s)
// re-orthogonalization
                SWAP(blockX, dwork);
                magma_tally4_dgegqr_gpu(ikind, m, cBlockSize, blockP, m, dwork, hwork, &info );
#endif
                //magma_tally4_dorthomgs( m, cBlockSize, blockP, queue );

                //magma_tally4_d_bspmv_tuned(m, cBlockSize, c_one, A, blockP, c_zero, blockAP, queue );
                magma_tally4_dsetmatrix( cBlockSize, cBlockSize, hwork, cBlockSize, dwork, cBlockSize);
...
//                magma_tally4_dtrsm( Magma_tally4Right, Magma_tally4Upper, Magma_tally4NoTrans, Magma_tally4NonUnit,
  //                           m, cBlockSize, c_one, dwork, cBlockSize, blockAP, m);
            ...
            magma_tally4blas_dtrsm( Magma_tally4Right, Magma_tally4Upper, Magma_tally4NoTrans, Magma_tally4NonUnit,
                        m, cBlockSize, c_one, dwork, cBlockSize, blockAP, m);
#else
            magma_tally4_dtrsm( Magma_tally4Right, Magma_tally4Upper, Magma_tally4NoTrans, Magma_tally4NonUnit, m,
            ...
            magma_tally4blas_dlaset(Magma_tally4Full, ldgram, ldgram, c_zero, c_one, gramB, ldgram);  // identity

            if (!restart) {
                magma_tally4_dgemm(Magma_tally4ConjTrans, Magma_tally4NoTrans, cBlockSize, n, m,
            ...
            magma_tally4_dgetmatrix(gramDim, gramDim, gramB, ldgram, h_gramB, ldgram);
            ...
            magma_tally4blas_dlaset(Magma_tally4Full, ldgram, ldgram, c_zero, c_one, gramA, ldgram);  // identity
            ...
            */
            ...
            if (restart==0) {
                magma_tally4_dgetmatrix(gramDim, gramDim, gramA, ldgram, gevectors, ldgram);
            }
            else {
                gramDim = n+cBlockSize;
                magma_tally4_dgetmatrix(gramDim, gramDim, gramA, ldgram, gevectors, ldgram);
            ...
            for(magma_tally4_int_t k=0; k<n; k++)
                *gevectors(k,k) = MAGMA_tally4_D_MAKE(evalues[k], 0);
            ...
            magma_tally4_dsetmatrix(gramDim, gramDim, gevectors, ldgram, gramA, ldgram);

            if (restart == 0) {
                // === contribution from P to the new X (in new search direction P)
                magma_tally4_dgemm(Magma_tally4NoTrans, Magma_tally4NoTrans, m, n, cBlockSize,
            ...
            SWAP(dwork, blockX);
            //magma_tally4_daxpy(m*n, c_one, blockP, 1, blockX, 1);
            CHECK( magma_tally4_dlobpcg_maxpy( m, n, blockP, blockX, queue ));
            ...
            SWAP(dwork, blockAX);
            //magma_tally4_daxpy(m*n, c_one, blockAP, 1, blockAX, 1);
            CHECK( magma_tally4_dlobpcg_maxpy( m, n, blockAP, blockAX, queue ));
    ...
    if (blockX != origX)
        magma_tally4blas_dlacpy( Magma_tally4UpperLower, m, n, blockX, m, origX, m);
