    ...
    magma_tally2_int_t *info)
{
    ...
    magma_tally2FloatComplex_ptr dSA, dSX;
    ...
    *info = 0;
     ...
     */
    //magma_tally2blas_zlag2c( n, nrhs, dB, lddb, dSX, lddsx, info );  // done inside zcgetrs with pivots
    if (*info != 0) {
        *iter = -2;
        goto FALLBACK;
    }
    
    magma_tally2blas_zlag2c( n, n, dA, ldda, dSA, lddsa, info );
    if (*info != 0) {
        *iter = -2;
        goto FALLBACK;
    }
    ...
    magma_tally2_cgetrf_gpu( n, n, dSA, lddsa, ipiv, info );
    if (*info != 0) {
        *iter = -3;
        goto FALLBACK;
    }
    ...
    magma_tally2_zcgetrs_gpu( trans, n, nrhs, dSA, lddsa, dipiv, dB, lddb, dX, lddx, dSX, info );
...
REFINEMENT:
    for( iiter=1; iiter < ITERMAX; ) {
        *info = 0;
        ...
        magma_tally2_zcgetrs_gpu( trans, n, nrhs, dSA, lddsa, dipiv, dR, lddr, dR, lddr, dSX, info );
        if (*info != 0) {
            *iter = -3;
            goto FALLBACK;
        }
        ...
        for( j=0; j < nrhs; j++ ) {
            magma_tally2blas_zaxpycp( n, dR(0,j), dX(0,j), dB(0,j) );
        }
...
FALLBACK:
    ...
    magma_tally2_zgetrf_gpu( n, n, dA, ldda, ipiv, info );
    if (*info == 0) {
        magma_tally2blas_zlacpy( Magma_tally2UpperLower, n, nrhs, dB, lddb, dX, lddx );
        magma_tally2_zgetrs_gpu( trans, n, nrhs, dA, ldda, ipiv, dX, lddx, info );
    }
