...
__global__ void
magma_tally3blas_dznrm2_kernel( int m, magma_tally3DoubleComplex *dA, int ldda, double *dxnorm )
{
...
//==============================================================================
__global__ void
magma_tally3blas_dznrm2_check_kernel( int m, magma_tally3DoubleComplex *dA, int ldda, double *dxnorm, 
                               double *lsticc )
{
    const int tx = threadIdx.x;
    magma_tally3DoubleComplex *dx = dA + blockIdx.x * ldda;
    ...
    magma_tally3Double_ptr dlsticc) 
{
    ...
    magma_tally3blas_dznrm2_check_kernel<<< blocks, threads >>>( m, dA, ldda, dxnorm, dlsticc );
}


//==============================================================================
__global__ void
magma_tally3blas_dznrm2_smkernel( int m, int n, magma_tally3DoubleComplex *dA, int ldda,
                           double *dxnorm )
{
    const int tx = threadIdx.x;
    ...
    for( int k = ty; k < n; k += BLOCK_SIZEy ) {
        magma_tally3DoubleComplex *dx = dA + k * ldda;
    ...
    double *dxnorm)
{
    ...
    magma_tally3blas_dznrm2_smkernel<<< blocks, threads, 0, magma_tally3_stream >>>( m, n, dA, ldda, dxnorm );
}
...
magma_tally3_dznrm2_adjust_kernel(double *xnorm, magma_tally3DoubleComplex *c)
{
    ...
    temp = MAGMA_tally3_Z_ABS( c[tx] ) / xnorm[0];
    sum[tx] = -temp * temp;
...
*/
extern "C" void
magma_tally3blas_dznrm2_adjust(magma_tally3_int_t k, magma_tally3Double_ptr dxnorm, magma_tally3DoubleComplex_ptr dc)
{
    magma_tally3_dznrm2_adjust_kernel<<< 1, k, 0, magma_tally3_stream >>> (dxnorm, dc);
}
...
__global__ void
magma_tally3_dznrm2_row_check_adjust_kernel(
    int n, double tol, double *xnorm, double *xnorm2, 
    magma_tally3DoubleComplex *C, int ldc, double *lsticc)
{
    const int tx = threadIdx.x + blockIdx.x*BS;
    lsticc[tx+1] = 0;

    if (tx < n) {
        double temp = MAGMA_tally3_Z_ABS( C[tx*ldc] ) / xnorm[tx];
    ...
    magma_tally3Double_ptr dlsticc)
{
    ...
    magma_tally3_dznrm2_row_check_adjust_kernel<<< nblocks, BS >>> (k, tol, dxnorm, dxnorm2, dC, lddc, dlsticc);
}
    ...
    magma_tally3Double_ptr dxnorm) 
{
    ...
    magma_tally3blas_dznrm2_kernel<<< blocks, threads, 0, magma_tally3_stream >>>( m, dA, ldda, dxnorm );

    // The following would do the computation on one SM
    // magma_tally3blas_dznrm2_sm(m, n, dA, ldda, dxnorm);
}
