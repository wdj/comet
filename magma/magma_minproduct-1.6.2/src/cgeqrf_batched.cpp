...
#define dA(i, j)  (dA + (i) + (j)*ldda)   // A(i, j) means at i row, j column
    ...
    magma_minproduct_int_t nb = 32;
    magma_minproduct_int_t nnb = 8;
    magma_minproduct_int_t i, k, ib=nb, jb=nnb;
    magma_minproduct_int_t ldw, ldt, ldr, offset; 
    ...
    magma_minproductFloatComplex **dW5_displ = NULL;

    magma_minproductFloatComplex *dwork = NULL;
    magma_minproductFloatComplex *dT   = NULL;
    magma_minproductFloatComplex *dR   = NULL;
    magma_minproductFloatComplex **dR_array = NULL;
    magma_minproductFloatComplex **dT_array = NULL; 
    magma_minproductFloatComplex **cpuAarray = NULL;
    magma_minproductFloatComplex **cpuTarray = NULL;
    ...
    magma_minproduct_malloc((void**)&dW4_displ, batchCount * sizeof(*dW4_displ));  // used in clarfb
    magma_minproduct_malloc((void**)&dW5_displ, batchCount * sizeof(*dW5_displ));
    magma_minproduct_malloc((void**)&dR_array, batchCount * sizeof(*dR_array));
    magma_minproduct_malloc((void**)&dT_array, batchCount * sizeof(*dT_array));

    ldt = ldr = min(nb, min_mn);
    magma_minproduct_cmalloc(&dwork,  (2 * nb * n) * batchCount);
    magma_minproduct_cmalloc(&dR,  ldr * n   * batchCount);
    magma_minproduct_cmalloc(&dT,  ldt * ldt * batchCount);
    magma_minproduct_malloc_cpu((void**) &cpuAarray, batchCount*sizeof(magma_minproductFloatComplex*));
    magma_minproduct_malloc_cpu((void**) &cpuTarray, batchCount*sizeof(magma_minproductFloatComplex*));
    ...
    if ( dW0_displ == NULL || dW1_displ == NULL || dW2_displ == NULL || dW3_displ == NULL || 
        ...
        magma_minproduct_free(dR_array);
        magma_minproduct_free(dT_array);
        magma_minproduct_free(dR);
    ...
    magma_minproductblas_claset_q(Magma_minproductFull, ldr, n*batchCount  , MAGMA_minproduct_C_ZERO, MAGMA_minproduct_C_ZERO, dR, ldr, queue);
    magma_minproductblas_claset_q(Magma_minproductFull, ldt, ldt*batchCount, MAGMA_minproduct_C_ZERO, MAGMA_minproduct_C_ZERO, dT, ldt, queue);
    ...
    magma_minproduct_getvector( batchCount, sizeof(magma_minproductFloatComplex*), dT_array, 1, cpuTarray, 1);
    ...
    for(i=0; i<min_mn;i+=nb)
    {
            ...
            magma_minproduct_cdisplace_pointers(dW0_displ, dA_array, ldda, i, i, batchCount, queue); 
            magma_minproduct_cdisplace_pointers(dW2_displ, tau_array, 1, i, 0, batchCount, queue);
            ...
            magma_minproduct_cgeqrf_panel_batched(m-i, ib, jb, 
                                       dW0_displ, ldda, 
                                       dW2_displ, 
                                       dT_array, ldt, 
                                       dR_array, ldr,
           ...
           cgeqrf_copy_upper_batched(ib, jb, dW0_displ, ldda, dR_array, ldr, batchCount, queue);
            ...
            if( (n-ib-i) > 0)
            {
                ...
                magma_minproductblas_claset_batched(Magma_minproductUpper, ib, ib, MAGMA_minproduct_C_ZERO, MAGMA_minproduct_C_ONE, dW0_displ, ldda, batchCount, queue); 
                magma_minproduct_cdisplace_pointers(dW2_displ, tau_array, 1, i, 0, batchCount, queue); 

                // it is faster since it is using BLAS-3 GEMM routines, different from lapack implementation 
                magma_minproduct_clarft_batched(m-i, ib, 0,
                                 dW0_displ, ldda,
                                 dW2_displ,
                                 dT_array, ldt, 
                                 dW4_displ, nb*ldt,
                ...
                { 
                    ...
                    for(k=0; k<batchCount; k++)
                    {
                        ...
                        magma_minproduct_clarfb_gpu_gemm(Magma_minproductLeft, Magma_minproductConjTrans, Magma_minproductForward, Magma_minproductColumnwise,
                                    m-i, n-i-ib, ib,
            ...
            magma_minproductblas_clacpy_batched(Magma_minproductUpper, ib, ib, dR_array, ldr, dW0_displ, ldda, batchCount, queue);

    }

    for(k=0; k<nbstreams; k++){
        magma_minproduct_queue_destroy( stream[k] );
    }
