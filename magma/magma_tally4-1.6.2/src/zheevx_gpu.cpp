    ...
    magma_tally4_int_t *info)
{
    ...
    *info = 0;
    ...
    work[0] = MAGMA_tally4_Z_MAKE( lopt, 0 );
    ...
    *m = 0;
    ...
    if (MAGMA_tally4_SUCCESS != magma_tally4_dmalloc( &dwork, n )) {
        fprintf (stderr, "!!!! device memory allocation error (magma_tally4_zheevx_gpu)\n");
...
#ifdef FAST_HEMV
    magma_tally4_zhetrd2_gpu(uplo, n, dA, ldda, &rwork[indd], &rwork[inde],
                      &work[indtau], wA, ldwa, &work[indwrk], llwork, dZ, lddz*n, &iinfo);
#else
    magma_tally4_zhetrd_gpu (uplo, n, dA, ldda, &rwork[indd], &rwork[inde],
                      &work[indtau], wA, ldwa, &work[indwrk], llwork, &iinfo);
#endif

    lopt = n + (magma_tally4_int_t)MAGMA_tally4_Z_REAL(work[indwrk]);
    ...
    if ((alleig || (indeig && il == 1 && iu == n)) && abstol <= 0.) {
        ...
        if (! wantz) {
            ...
            if (*info == 0) {
                ...
                magma_tally4_zsetmatrix( n, n, wZ, ldwz, dZ, lddz );
            }
        }
        if (*info == 0) {
            *m = n;
