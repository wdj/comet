/*
    -- MAGMA_minproduct (version 1.6.1) --
       ...
       @generated from zhetrf.cpp normal z -> c, Fri Jan 30 19:00:16 2015
*/
    ...
    magma_minproduct_int_t *ipiv, magma_minproduct_int_t *info)
{
    ...
    magma_minproduct_int_t nb = magma_minproduct_get_chetrf_nb(n);
    magma_minproduct_int_t iinfo = 0, nk, kb;
    ...
    *info = 0;
    ...
    magma_minproductFloatComplex *dA, *dW;
    if ((MAGMA_minproduct_SUCCESS != magma_minproduct_cmalloc( &dA, n*ldda  )) ||
        (MAGMA_minproduct_SUCCESS != magma_minproduct_cmalloc( &dW, (1+nb)*ldda ))) {
        *info = MAGMA_minproduct_ERR_DEVICE_ALLOC;
        return *info;
    }
    magma_minproduct_queue_t stream[2];
    magma_minproduct_event_t event[2];
    magma_minproduct_queue_create( &stream[0] );
    magma_minproduct_queue_create( &stream[1] );
    magma_minproduct_event_create( &event[0] );
    magma_minproduct_event_create( &event[1] );
    trace_init( 1, 1, 2, (CUstream_st**)stream );

    /* copy matrix to GPU */
    trace_gpu_start( 0, 0, "set", "setA" );
    //magma_minproduct_csetmatrix_async( n, n, A(0,0), lda, dA(0,0), ldda, stream[0] );
    if ( upper ) {
       for (int k = 0; k < n; k+=nb ) {
           kb = min(nb, n-k);
           magma_minproduct_csetmatrix_async( k+kb, kb, A(0,k), lda, dA(0,k), ldda, stream[0] );
       }
    } else {
       for (int k = 0; k < n; k+=nb ) {
           kb = min(nb, n-k);
           magma_minproduct_csetmatrix_async( n-k, kb, A(k,k), lda, dA(k,k), ldda, stream[0] );
       }
    }
    trace_gpu_end( 0, 0 );
    ...
    if ( upper ) {
        ...
        for (int k = n-1; k >= 0; k-=kb ) {
            ...
            if ( k+1 > nb ) {
                ...
                magma_minproduct_clahef_gpu( Magma_minproductUpper, nk, kb, &kb, A( 0, 0 ), lda, dA( 0, 0 ), ldda, 
                ...
                magma_minproduct_cgetmatrix( nk, nk, dA( 0, 0 ),ldda, A( 0, 0 ),lda );
                lapackf77_chetf2( Magma_minproductUpperStr, &nk, A( 0, 0 ), &lda, &ipiv[0], &iinfo );
                kb = k+1;
            }
        ...
        for (int k = 0; k < n; k += kb ) {
            ...
            if ( k < n-nb ) {
                ...
                magma_minproduct_clahef_gpu( Magma_minproductLower, nk, nb, &kb, A( k, k ), lda, dA( k, k ), ldda,
                                  &ipiv[k], dW, ldda, stream, event, &iinfo );

            } else {
                /* Use unblocked code to factorize columns k:n of A */
                magma_minproduct_queue_sync( stream[0] );
                magma_minproduct_cgetmatrix( nk,nk, dA(k,k),ldda, A(k,k),lda );
                lapackf77_chetf2( Magma_minproductLowerStr, &nk, A( k, k ), &lda, &ipiv[k], &iinfo );
            }
            /* Set INFO on the first occurrence of a zero pivot */
    ...
    magma_minproduct_event_destroy( event[0] );
    magma_minproduct_event_destroy( event[1] );
