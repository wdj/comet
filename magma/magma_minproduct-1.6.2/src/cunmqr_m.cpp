...
#define dwork(gpui, ind)       (dw[gpui] + maxnlocal*lddc + 2*lddac*lddar + 2*((nb+1)*nb) + (ind)*(lddwork*nb))
    ...
    magma_minproduct_cmalloc_pinned(&T, nb*nb);
    ...
    *info = 0;
    ...
    for (igpu = 0; igpu < ngpu; ++igpu) {
        ...
        if (MAGMA_minproduct_SUCCESS != magma_minproduct_cmalloc( &dw[igpu], ldw )) {
    ...
    if (left) {
        ...
        for (magma_minproduct_int_t i = i1; (i3 < 0 ? i >= i2 : i < i2); i += i3) {
            ...
            for (igpu = 0; igpu < ngpu; ++igpu) {
                ...
                magma_minproduct_csetmatrix_async(nq-i, kb,
                                       A(i, i),                 lda,
                                       dA_c(igpu, ind_c, i, 0), lddac, stream[igpu][0] );
                // set upper triangular part of dA to identity
                magma_minproductblas_claset_band_q( Magma_minproductUpper, kb, kb, kb, c_zero, c_one, dA_c(igpu, ind_c, i, 0), lddac, stream[igpu][0] );
            }
        ...
        for (magma_minproduct_int_t i = 0; i < nbl; ++i) {
            ...
            magma_minproduct_setdevice(igpu);
            magma_minproduct_int_t kb = min(nb_l, n-i*nb_l);
            magma_minproduct_cgetmatrix( m, kb,
                              dC(igpu, 0, i/ngpu*nb_l), lddc,
                              C(0, i*nb_l), ldc );
//            magma_minproduct_cgetmatrix_async( m, kb,
//                                   dC(igpu, 0, i/ngpu*nb_l), lddc,
//                                   C(0, i*nb_l), ldc, stream[igpu][0] );
        ...
        *info = MAGMA_minproduct_ERR_NOT_IMPLEMENTED;
        ...
        for (i = i1; (i3 < 0 ? i >= i2 : i < i2); i += i3) {
            ...
            magma_minproduct_csetmatrix( i__4, ib, A(i, i), lda, dA(i, 0), ldda );
            magma_minproductblas_claset_band( Magma_minproductUpper, ib, ib, ib, c_zero, c_one, dA(i, 0), ldda );
            ...
            // Apply H or H'; First copy T to the GPU
            magma_minproduct_csetmatrix( ib, ib, T, ib, dT, ib );
            magma_minproduct_clarfb_gpu( side, trans, Magma_minproductForward, Magma_minproductColumnwise,
            mi, ni, ib,
            dA(i, 0), ldda, dT, ib,
            dC(ic, jc), lddc,
