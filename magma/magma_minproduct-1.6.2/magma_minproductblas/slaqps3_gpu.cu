...
__global__ void
magma_minproduct_sswap_gemv_kernel(int m, int rk, int n, const float * __restrict__ V, int ldv,
                     const float * __restrict__ x, int ldx, float *c, float *b)
{
    ...
    if (j < m){
       ...
       if (j>=rk) 
          for(int k=0; k<n; k++)
              lsum += MAGMA_minproduct_S_MUL( V[k*ldv], MAGMA_minproduct_S_CNJG(x[k*ldx]));
...
__global__ void
magma_minproduct_sgemv_kernel(int m, int n, const float * __restrict__ V, int ldv,
                     const float * __restrict__ x, float *b, float *c)
{
    ...
    if (j < m){
        for(int k=0; k<n; k++)
            lsum += MAGMA_minproduct_S_MUL( V[k*ldv], x[k]);
...
__global__
void magma_minproduct_sscale_kernel(int n, float* dx0,
                         float *dtau, float *dxnorm, float* dAkk)
{
   ...
   for( int k = i; k < n; k += BLOCK_SIZE ) {
             ...
             lsum += re*re;
        #else
             re = MAGMA_minproduct_S_REAL( dx[k] );
             float im = MAGMA_minproduct_S_IMAG( dx[k] );
             lsum += re*re + im*im;
        #endif
   }
   sum[i] = lsum;
            ...
            if ( beta == 0 ) {
              *dtau = MAGMA_minproduct_S_ZERO;
               ...
               *dAkk    = beta;
               ...
               scale = 1. / (tmp - beta);
#else
               float alphar =  MAGMA_minproduct_S_REAL(tmp), alphai = MAGMA_minproduct_S_IMAG(tmp);
               beta  = -copysign( beta, alphar );

               // todo: deal with badly scaled vectors (see lapack's larfg)
               *dtau = MAGMA_minproduct_S_MAKE((beta - alphar)/beta, -alphai/beta);
               *dAkk = MAGMA_minproduct_S_MAKE(beta, 0.);

               tmp = MAGMA_minproduct_S_MAKE( alphar - beta, alphai);
               scale = MAGMA_minproduct_S_DIV( MAGMA_minproduct_S_ONE, tmp);
#endif
            }
   }
   ...
   for(int j=i; j<n; j+=BLOCK_SIZE)
      dx0[j] = MAGMA_minproduct_S_MUL(dx0[j], scale);

   /* === Make temporary the first element to 1; value is stored in dAkk === */
   if (i==0)
     dx0[0] = MAGMA_minproduct_S_ONE;
}
...
__global__ void
magma_minproduct_sgemv_kernel_adjust(int n, int k, float * A, int lda, 
                          ...
                          float *xnorm, float *xnorm2, float *Akk, int *lsticc, int *lsticcs)
{
    ...
    if (j<n) {
      B += j;
      sum = MAGMA_minproduct_S_CNJG( B[(k-1)*ldb] );
      // sum = MAGMA_minproduct_S_ZERO;
      for(int m=0; m<k-1; m++) {
         sum += MAGMA_minproduct_S_MUL( MAGMA_minproduct_S_CNJG( B[m*ldb] ), A[m*lda] );
      }
      C[j*lda] -= sum;

      oldnorm = xnorm[j];
      temp = MAGMA_minproduct_S_ABS( C[j*lda] ) / oldnorm;
...
__global__ void
magma_minproductblas_snrm2_check_kernel(int m, float *da, int ldda, 
                              ...
                              int *dlsticc, int *dlsticcs)
{
    ...
    for( int j = i; j < m; j += BLOCK_SIZE ) {
        ...
        lsum += re*re;
#else
        re = MAGMA_minproduct_S_REAL( dx[j] );
        float im = MAGMA_minproduct_S_IMAG( dx[j] );
        lsum += re*re + im*im;
#endif
...
#define dF(i_, j_) (dF + (i_) + (j_)*(lddf))
    ...
    magma_minproductFloat_ptr dAkk = dauxv;
    dauxv += 1;
    ...
    while( k < nb && lsticc == 0 ) {
        ...
        if (pvt != k) {
            magma_minproductblas_sswap( k, dF(pvt,0), lddf, dF(k,0), lddf);
            itemp     = jpvt[pvt];
            jpvt[pvt] = jpvt[k];
            jpvt[k]   = itemp;
        ...
        magma_minproduct_sswap_gemv_kernel<<< (m + BLOCK_SIZE1-1) / BLOCK_SIZE1, BLOCK_SIZE1, 0, magma_minproduct_stream >>> 
                              ( m, rk, k, dA(0, 0), ldda, dF(k,  0), lddf, dA(0, k), dA(0,pvt));
                                 
        /*  Generate elementary reflector H(k). */
        magma_minproduct_sscale_kernel<<< 1, BLOCK_SIZE, 0, magma_minproduct_stream >>>
        ...
        if (k < n-1) {
            magma_minproduct_sgetvector( 1, &dtau[k], 1, &tauk, 1 );
            magma_minproduct_sgemv( Magma_minproductConjTrans, m-rk, n,
                         tauk,   dA( rk,  0 ), ldda,
                                 dA( rk,  k   ), 1,
                         c_zero, dauxv, 1 );
            if (k==0) 
               magma_minproductblas_slacpy(Magma_minproductUpperLower, n-k-1, 1, dauxv+k+1, n-k-1, dF( k+1, k   ), n-k-1);
        }
        ...
        if (k > 0) {
            ...
            magma_minproduct_sgemv_kernel<<< (n-k-1 + BLOCK_SIZE1 -1)/BLOCK_SIZE1, BLOCK_SIZE1, 0, magma_minproduct_stream >>>
        ...
        if (k < n-1) {
            ...
            magma_minproduct_sgemv_kernel_adjust<<<(n-k-1 + BLOCK_SIZE2-1)/BLOCK_SIZE2, BLOCK_SIZE2, 0, magma_minproduct_stream>>>
            ...
            // can be done in the kernel directly (magma_minproductblas_snrm2_check_kernel)
            // if (k==16) lsticc = 1;
        }
...
/*
        if (rk < min(m, n+offset)-1){
           magma_minproductblas_snrm2_row_check_adjust(n-k-1, tol3z, &dvn1[k+1], 
...
*/
        ...
        if (k>=n-1)
           magma_minproductblas_slacpy(Magma_minproductUpperLower, 1, 1, dAkk, 1, dA(rk, k), 1);
    ...
    *kb = k + 1;
    ...
    if( lsticc > 0 ) {
        ...
        //magma_minproductblas_snrm2_check(m-rk-1, n-*kb, A(rk+1,rk+1), lda,
        //                       &dvn1[rk+1], &dvn2[rk+1], dlsticcs);
       
        // There is a bug when we get to recompute  
        magma_minproductblas_snrm2_check_kernel<<< n-*kb, BLOCK_SIZE >>>
