...
/**
    ...
    @ingroup magma_tally3_zgeqp3_comp
    ********************************************************************/
...
#define dA(i, j) (dwork + (i) + (j)*(ldda))
    ...
    magma_tally3DoubleComplex   *dwork, *df;
    ...
    *info = 0;
    ...
    nb = magma_tally3_get_zgeqp3_nb(min(m, n));
    minmn = min(m,n);
    if (*info == 0) {
        if (minmn == 0) {
            lwkopt = 1;
    ...
    if (MAGMA_tally3_SUCCESS != magma_tally3_zmalloc( &dwork, ldwork )) {
    ...
    if (nfxd > 0) {
        ...
        if (na < n) {
            n_j = n - na;
            lapackf77_zunmqr( Magma_tally3LeftStr, Magma_tally3ConjTransStr, &m, &n_j, &na,
    ...
    if (nfxd < minmn) {
        ...
        if (nb < sminmn) {
            ...
            magma_tally3_zsetmatrix_async( m, sn,
                                    A (0,j), lda,
                                    dA(0,j), ldda, stream );
        }
        ...
        for (j = nfxd; j < n; ++j) {
            rwork[j] = magma_tally3_cblas_dznrm2( sm, A(nfxd,j), ione );
            rwork[n + j] = rwork[j];
        }
        ...
        if (nb < sminmn) {
            ...
            while(j < topbmn) {
                ...
                if (j > nfxd) {
                    ...
                    magma_tally3_zgetmatrix( m-j, jb,
                                      dA(j,j), ldda,
                                      A (j,j), lda );
                    
                    // Get the rows
                    magma_tally3_zgetmatrix( jb, n_j - jb,
                                      dA(j,j + jb), ldda,
                                      A (j,j + jb), lda );
                }

                magma_tally3_zlaqps( m, n_j, j, jb, &fjb,
                              A (0, j), lda,
                              dA(0, j), ldda,
        ...
        if (j < minmn) {
            n_j = n - j;
            if (j > nfxd) {
                magma_tally3_zgetmatrix( m-j, n_j,
                                  dA(j,j), ldda,
                                  A (j,j), lda );
            }
