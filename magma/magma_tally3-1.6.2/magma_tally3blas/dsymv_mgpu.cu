    ...
    int block_offset )
{
    ...
    double psum, psum_t;
    double total = MAGMA_tally3_D_ZERO;
...
/**************************************************************
    ...
    Note beta*y is not included here; see magma_tally3blas_dsymv_mgpu_sync.
    ...
    int block_offset)
{
    ...
    if ( ind >= block_offset && ind < n+block_offset ) {
        ...
        y[ind * incy] = alpha*Ax;  // see magma_tally3blas_dsymv_sync for beta*y
    }
}
...
/**
    Purpose
    -------
    magma_tally3blas_dsymv_mgpu performs the matrix-vector operation:
...
magma_tally3blas_dsymv_mgpu(
    ...
    double alpha,
    magma_tally3Double_const_ptr const d_lA[], magma_tally3_int_t ldda,
    magma_tally3_int_t offset,
    double const *x,       magma_tally3_int_t incx,
    double beta,                                         // unused, see magma_tally3blas_dsymv_mgpu_sync
    double       *y,       magma_tally3_int_t incy,             // unused
    ...
    magma_tally3_queue_t queues[] )
{
    ...
    for(dev=0; dev < ngpu; dev++) {
        ...
        // copy x to each GPU
        magma_tally3_dsetvector_async( n, x, incx, dx_dev + block_offset, 1, queues[dev] );
    ...
    for(dev=0; dev < ngpu; dev++) {
        ...
        magma_tally3_setdevice( dev );
        double       *dx_dev    = dwork[dev];
        magma_tally3_dgetvector_async( n, dx_dev + block_offset, 1, &hwork[dev*n], 1, queues[dev] );
    }
    
    // see magma_tally3blas_dsymv_mgpu_sync for final row sums
...
/**
    ...
    For convenience, the parameters are identical to magma_tally3blas_dsymv_mgpu
    (though some are unused here).
    
    @see magma_tally3blas_dsymv_mgpu
    @ingroup magma_tally3_dblas2
    ********************************************************************/
extern "C" magma_tally3_int_t
magma_tally3blas_dsymv_mgpu_sync(
    magma_tally3_uplo_t uplo,                                               // unused, see magma_tally3blas_dsymv_mgpu
    magma_tally3_int_t n,
    double alpha,                                        // unused
    magma_tally3Double_const_ptr const d_lA[], magma_tally3_int_t ldda,
    magma_tally3_int_t offset,                                              // unused
    double const *x,       magma_tally3_int_t incx,             // unused
    double beta,
    double       *y,       magma_tally3_int_t incy,             // unused
    double       *hwork,   magma_tally3_int_t lhwork,
    magma_tally3Double_ptr    dwork[], magma_tally3_int_t ldwork,           // unused
    magma_tally3_int_t ngpu,
