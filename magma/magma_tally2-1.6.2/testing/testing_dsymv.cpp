...
int main(int argc, char **argv)
{
    ...
    double *A, *X, *Y, *Yatomics, *Ycublas, *Ymagma_tally2;
    magma_tally2Double_ptr dA, dX, dY, dwork;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( Ymagma_tally2,   double, sizeY );
            ...
            magma_tally2blas_dlaset( Magma_tally2Full, ldwork, 1, MAGMA_tally2_D_NAN, MAGMA_tally2_D_NAN, dwork, ldwork );
            magma_tally2blas_dlaset( Magma_tally2Full, ldda,   N, MAGMA_tally2_D_NAN, MAGMA_tally2_D_NAN, dA,    ldda   );
            
            /* Initialize the matrix */
            lapackf77_dlarnv( &ione, ISEED, &sizeA, A );
            magma_tally2_dmake_symmetric( N, A, lda );
            ...
            if ( opts.uplo == Magma_tally2Upper ) {
                lapackf77_dlaset( "Lower", &N1, &N1, &MAGMA_tally2_D_NAN, &MAGMA_tally2_D_NAN, &A[1], &lda );
            }
            else {
                lapackf77_dlaset( "Upper", &N1, &N1, &MAGMA_tally2_D_NAN, &MAGMA_tally2_D_NAN, &A[lda], &lda );
            }
            ...
            magma_tally2_dsetvector( N, X, incx, dX, incx );
            magma_tally2_dsetvector( N, Y, incy, dY, incy );
            
            cublas_time = magma_tally2_sync_wtime( 0 );
            cublasDsymv( opts.handle, cublas_uplo_const_tally2(opts.uplo),
                         N, &alpha, dA, ldda, dX, incx, &beta, dY, incy );
            cublas_time = magma_tally2_sync_wtime( 0 ) - cublas_time;
            cublas_perf = gflops / cublas_time;
            
            magma_tally2_dgetvector( N, dY, incy, Ycublas, incy );
            ...
            magma_tally2_dgetvector( N, dY, incy, Yatomics, incy );
            ...
            if ( opts.version == 1 ) {
                magma_tally2blas_dsymv_work( opts.uplo, N, alpha, dA, ldda, dX, incx, beta, dY, incy, dwork, ldwork, opts.queue );
            }
            else {
                // non-work interface (has added overhead)
                magma_tally2blas_dsymv( opts.uplo, N, alpha, dA, ldda, dX, incx, beta, dY, incy );
            }
            magma_tally2_time = magma_tally2_sync_wtime( 0 ) - magma_tally2_time;
            magma_tally2_perf = gflops / magma_tally2_time;
            
            magma_tally2_dgetvector( N, dY, incy, Ymagma_tally2, incy );
            
            /* =====================================================================
               Performs operation using CPU BLAS
               =================================================================== */
            cpu_time = magma_tally2_wtime();
            blasf77_dsymv( lapack_uplo_const_tally2(opts.uplo), &N, &alpha, A, &lda, X, &incx, &beta, Y, &incy );
            ...
            /* =====================================================================
               Check the result
               =================================================================== */
            blasf77_daxpy( &N, &c_neg_one, Y, &incy, Ymagma_tally2, &incy );
            magma_tally2_error = lapackf77_dlange( "M", &N, &ione, Ymagma_tally2, &N, work ) / N;
