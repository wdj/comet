...
*/
extern "C"
void magma_minproductblas_sgemvn_batched(
    int m, int n, 
    ...
    int batchCount, magma_minproduct_queue_t queue)
{
    ...
    if( m > 512 || n > 512)
    {
        fprintf( stderr, "m=%d, n=%d, sgemv_batched nontranspose assume row && column lower than %d. Plz call magma_minproductblas_sgemv instead", m, n, 512);
        return ;
    }
...
extern "C"
void magma_minproductblas_sgemvt_batched(
    int m, int n, 
    float alpha, float **dA_array, int lda, 
    float **x_array,  int incx,
    float beta, float **y_array,  int incy, 
    ...
    float beta, float  **y_array, int incy)
{
    ...
    for(int i=0; i<m1; i+= sgemv_bs)
    {
        res += MAGMA_minproduct_S_CNJG (A_ptr[i]) * x_ptr[i*incx];
    }
    ...
    if(m > m1)
    {
        if( tx + m1 <  m )
        {
            res  += MAGMA_minproduct_S_CNJG(A_ptr[m1]) * x_ptr[m1*incx];
        }
        else
        {
            res  = res;
...
extern "C"
void magma_minproductblas_sgemvc_batched(
    int m, int n, 
    float alpha, float **dA_array, int lda, 
    float **x_array,  int incx,
    float beta, float **y_array,  int incy, 
...
extern "C"
void magma_minproductblas_sgemv_batched(
...
{       
    ...
    if ( trans == Magma_minproductNoTrans ) {
        ...
        magma_minproductblas_sgemvn_batched(m, n, alpha, dA_array, ldda, dx_array, incx, beta, dy_array, incy, batchCount, queue);
            
    }
    else if ( trans == Magma_minproductTrans ) {
        magma_minproductblas_sgemvt_batched(m, n, alpha, dA_array, ldda, dx_array, incx, beta, dy_array, incy, batchCount, queue);
    }
    else if ( trans == Magma_minproductConjTrans ) {
#if defined(PRECISION_z) || defined (PRECISION_c)
        magma_minproductblas_sgemvc_batched(m, n, alpha, dA_array, ldda, dx_array, incx, beta, dy_array, incy, batchCount, queue);
#else
        magma_minproductblas_sgemvt_batched(m, n, alpha, dA_array, ldda, dx_array, incx, beta, dy_array, incy, batchCount, queue);
#endif
    }
