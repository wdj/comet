...
int main(  int argc, char** argv )
{
    ...
    real_Double_t res;
    magma_tally3_d_matrix A={Magma_tally3_CSR}, B={Magma_tally3_CSR}, B2={Magma_tally3_CSR}, 
    A_d={Magma_tally3_CSR}, B_d={Magma_tally3_CSR}, C_d={Magma_tally3_CSR};

    double one = MAGMA_tally3_D_MAKE(1.0, 0.0);
    double mone = MAGMA_tally3_D_MAKE(-1.0, 0.0);
    ...
    if ( strcmp("LAPLACE2D", argv[i]) == 0 && i+1 < argc ) {   // Laplace test
        i++;
        magma_tally3_int_t laplace_size = atoi( argv[i] );
        CHECK( magma_tally3_dm_5stencil(  laplace_size, &B, queue ));
    } else {                        // file-matrix test
        CHECK( magma_tally3_d_csr_mtx( &B,  argv[i], queue ));
    }
    printf( "# matrix info: %d-by-%d with %d nonzeros\n",
    ...
    CHECK( magma_tally3_dcuspaxpy( &one, A_d, &one, B_d, &C_d, queue ));

    magma_tally3_dmfree(&B_d, queue );

    CHECK( magma_tally3_dcuspaxpy( &mone, A_d, &one, C_d, &B_d, queue ));
    
    CHECK( magma_tally3_dmtransfer( B_d, &B2, Magma_tally3_DEV, Magma_tally3_CPU, queue ));

    magma_tally3_dmfree(&A_d, queue );
    magma_tally3_dmfree(&B_d, queue );
    magma_tally3_dmfree(&C_d, queue );

    // check difference
    CHECK( magma_tally3_dmdiff( B, B2, &res, queue ));
    printf("# ||A-B||_F = %8.2e\n", res);
    if ( res < .000001 )
