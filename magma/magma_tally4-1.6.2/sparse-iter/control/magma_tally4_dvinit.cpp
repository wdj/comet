/*
    -- MAGMA_tally4 (version 1.6.2) --
       ...
       @generated from magma_tally4_zvinit.cpp normal z -> d, Sun May  3 11:23:01 2015
       @author Hartwig Anzt
*/
...
/**
    ...
    Allocates memory for magma_tally4_d_matrix and initializes it
    ...
    magma_tally4_queue_t queue )
{
    ...
    if ( mem_loc == Magma_tally4_CPU ) {
        x->memory_location = Magma_tally4_CPU;
        CHECK( magma_tally4_dmalloc_cpu( &x->val, x->nnz ));
        for( magma_tally4_int_t i=0; i<x->nnz; i++)
             x->val[i] = values;
    }
    else if ( mem_loc == Magma_tally4_DEV ) {
        x->memory_location = Magma_tally4_DEV;
        CHECK( magma_tally4_dmalloc( &x->val, x->nnz ));
        magma_tally4blas_dlaset(Magma_tally4Full, x->num_rows, x->num_cols, values, values, x->val, x->num_rows);
    }
