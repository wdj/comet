...
int main( int argc, char** argv)
{
    ...
    if ( arch < 200 ) {
        ...
        for( int iuplo = 0; iuplo < 2; ++iuplo ) {
            printf( "Testing that magma_tally2blas_clanhe( %s, %s, ... ) returns -1 error...\n",
                    lapack_norm_const_tally2( norm[inorm] ),
                    lapack_uplo_const_tally2( uplo[iuplo] ));
            norm_magma_tally2 = magma_tally2blas_clanhe( norm[inorm], uplo[iuplo], 1, NULL, 1, NULL );
            if ( norm_magma_tally2 != -1 ) {
                printf( "expected magma_tally2blas_clanhe to return -1 error, but got %f\n", norm_magma_tally2 );
                status = 1;
            }
        }}
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int iuplo = 0; iuplo < 2; ++iuplo ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            gbytes = 0.5*(N+1)*N*sizeof(magma_tally2FloatComplex) / 1e9;
            ...
            norm_magma_tally2 = magma_tally2blas_clanhe( norm[inorm], uplo[iuplo], N, d_A, ldda, d_work );
            ...
            if (norm_magma_tally2 < 0)
                printf("magma_tally2blas_clanhe returned error %f: %s.\n",
