...
/**
    ...
    @ingroup magma_minproduct_dgeqp3_comp
    ********************************************************************/
    ...
    magma_minproduct_int_t *info )
{
    ...
    *info = 0;
    ...
    nb = magma_minproduct_get_dgeqp3_nb(min(m, n));
    minmn = min(m,n);
    if (*info == 0) {
        if (minmn == 0) {
            lwkopt = 1;
    ...
    #endif
    magma_minproductDouble_ptr df;
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc( &df, (n+1)*nb )) {
    ...
    if (nfxd > 0) {
        ...
        if (na < n) {
            n_j = n - na;
            lapackf77_dormqr( Magma_minproductLeftStr, Magma_minproductConjTransStr, &m, &n_j, &na,
    ...
    if (nfxd < minmn) {
        ...
        magma_minproductblas_dnrm2_cols(sm, sn, dA(nfxd,nfxd), ldda, &rwork[nfxd]);
        ...
        //if (nb < sminmn)
        {
            ...
            while(j < topbmn) {
                ...
                //magma_minproduct_dlaqps_gpu    // this is a cpp-file
                magma_minproduct_dlaqps2_gpu   // this is a cuda-file
                    ( m, n_j, j, jb, &fjb,
                      dA(0, j), ldda,
        ...
        if (j < minmn) {
            n_j = n - j;
            if (j > nfxd) {
                magma_minproduct_dgetmatrix( m-j, n_j,
                                  dA(j,j), ldda,
                                   A(j,j), lda );
            }
