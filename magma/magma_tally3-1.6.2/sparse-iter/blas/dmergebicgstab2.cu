...
__global__ void
magma_tally3_dreduce_kernel_spmv1(    
    int Gs,
    int n, 
    double * vtmp,
    double * vtmp2 )
...
__global__ void
magma_tally3_dbicgmerge_spmv1_kernel(  
    int n,
    double * dval, 
    ...
    double * vtmp)
{
    ...
    temp[ Idx ] = ( i < n ) ? v[ i ] * r[ i ] : MAGMA_tally3_D_MAKE( 0.0, 0.0);
    __syncthreads();
    if ( Idx < 128 ){
...
__global__ void
magma_tally3_dbicgstab_alphakernel(  
                    double * skp ){
    ...
    magma_tally3_queue_t queue )
{
    ...
    if ( A.storage_type == Magma_tally3_CSR)
        magma_tally3_dbicgmerge_spmv1_kernel<<<Gs, Bs, Ms>>>
    ...
    while( Gs.x > 1 ) {
        ...
        magma_tally3_dreduce_kernel_spmv1<<< Gs_next.x/2, Bs.x/2, Ms/2 >>> 
                            ( Gs.x, n, aux1, aux2 );
    ...
    magma_tally3_dcopyvector( 1, aux1, 1, skp, 1 );
    dim3 Bs2( 2 );
    dim3 Gs2( 1 );
    magma_tally3_dbicgstab_alphakernel<<<Gs2, Bs2, 0>>>( skp );
...
__global__ void
magma_tally3_dreduce_kernel_spmv2( 
    int Gs,
    int n, 
    double * vtmp,
    double * vtmp2 )
{
    ...
    for( j=0; j<2; j++){
        int i = blockIdx.x * ( blockSize * 2 ) + Idx;   
        temp[Idx+j*(blockSize)] = MAGMA_tally3_D_MAKE( 0.0, 0.0);
        while (i < Gs ) {
...
__global__ void
magma_tally3_dbicgmerge_spmv2_kernel(  
    int n,
    double * dval, 
    magma_tally3_index_t * drowptr, 
    magma_tally3_index_t * dcolind,
    ...
    double * vtmp )
{
    ...
    if (i<n){
        ...
        for( j=0; j<2; j++)
            temp[Idx+j*blockDim.x] =MAGMA_tally3_D_MAKE( 0.0, 0.0);
    }
    __syncthreads();
    if ( Idx < 128 ){
        for( j=0; j<2; j++){
...
__global__ void
magma_tally3_dbicgstab_omegakernel(  
                    double * skp ){
    int i = blockIdx.x * blockDim.x + threadIdx.x;
...
/**
    ...
    @param[in]
    ds          magma_tally3Double_ptr
                input vector s

    @param[in]
    dt          magma_tally3Double_ptr
                output vector t
    ...
    magma_tally3_queue_t queue )
{
    ...
    int b = 1;        
    if ( A.storage_type == Magma_tally3_CSR)
        magma_tally3_dbicgmerge_spmv2_kernel<<<Gs, Bs, Ms>>>
    ...
    while( Gs.x > 1 ) {
        Gs_next.x = ( Gs.x+Bs.x-1 )/ Bs.x ;
        if ( Gs_next.x == 1 ) Gs_next.x = 2;
        magma_tally3_dreduce_kernel_spmv2<<< Gs_next.x/2, Bs.x/2, Ms/2 >>> 
                    ( Gs.x, n, aux1, aux2 );
        Gs_next.x = Gs_next.x /2;
        Gs.x = Gs_next.x;
        b = 1 - b;
    ...
    magma_tally3_dcopyvector( 1, aux1, 1, skp+6, 1 );
    magma_tally3_dcopyvector( 1, aux1+n, 1, skp+7, 1 );
    dim3 Bs2( 2 );
    dim3 Gs2( 1 );
    magma_tally3_dbicgstab_omegakernel<<<Gs2, Bs2, 0>>>( skp );
...
__global__ void
magma_tally3_dbicgmerge_xrbeta_kernel(  
    int n, 
    double * rr,
    double * r,
    double * p,
...
__global__ void
magma_tally3_dbicgstab_betakernel(  
    double * skp )
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
...
/**
    ...
    @param[in]
    rr          magma_tally3Double_ptr
                input vector rr

    @param[in]
    r           magma_tally3Double_ptr
                input/output vector r

    @param[in]
    p           magma_tally3Double_ptr
                input vector p

    @param[in]
    s           magma_tally3Double_ptr
                input vector s

    @param[in]
    t           magma_tally3Double_ptr
                input vector t
...
magma_tally3_dbicgmerge_xrbeta(
    ...
    magma_tally3Double_ptr d1,
    magma_tally3Double_ptr d2,
    magma_tally3Double_ptr rr,
    magma_tally3Double_ptr r,
    magma_tally3Double_ptr p,
    magma_tally3Double_ptr s,
    magma_tally3Double_ptr t,
    ...
    magma_tally3_queue_t queue )
{
    ...
    magma_tally3Double_ptr aux1 = d1, aux2 = d2;
    int b = 1;        
    magma_tally3_dbicgmerge_xrbeta_kernel<<<Gs, Bs, Ms>>>
    ...
    magma_tally3_dcopyvector( 1, aux1+n, 1, skp+5, 1 );
    dim3 Bs2( 2 );
    dim3 Gs2( 1 );
    magma_tally3_dbicgstab_betakernel<<<Gs2, Bs2, 0>>>( skp );
