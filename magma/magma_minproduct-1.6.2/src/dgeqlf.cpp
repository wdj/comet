...
/**
    ...
    lwork   INTEGER
            ...
            through magma_minproduct_get_dgeqlf_nb(M).
    \n
    ...
    @ingroup magma_minproduct_dgeqlf_comp
    ********************************************************************/
    ...
    magma_minproduct_int_t *info)
{
    ...
    magma_minproductDouble_ptr dA, dwork;
    ...
    nb = magma_minproduct_get_dgeqlf_nb(m);
    *info = 0;
    lquery = (lwork == -1);
    ...
    if (*info == 0) {
        if (k == 0)
            ...
            work[0] = MAGMA_minproduct_D_MAKE( max(n*nb, 2*nb*nb), 0 );
        }
    ...
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc( &dA, (n)*ldda + nb*lddwork )) {
    ...
    if ( (nb > 1) && (nb < k) ) {
        ...
        for (i = k - kk + ki; i >= k -kk; i -= nb) {
            ...
            if (i < k - kk + ki) {
                ...
                magma_minproduct_dgetmatrix_async( m-rows, ib,
                                        dA(rows, n-k+i), ldda,
                ...
                magma_minproduct_dlarfb_gpu( Magma_minproductLeft, Magma_minproductConjTrans, Magma_minproductBackward, Magma_minproductColumnwise,
                                  rows, cols, old_ib,
            ...
            if (cols > 0) {
                ...
                lapackf77_dlarft( Magma_minproductBackwardStr, Magma_minproductColumnwiseStr,
                                  &rows, &ib,
                                  A(0, cols), &lda, tau + i, work, &ib);

                dpanel_to_q( Magma_minproductLower, ib, A(rows-ib,cols), lda, work+ib*ib);
                magma_minproduct_dsetmatrix( rows, ib,
                                  A(0,cols),  lda,
                                  dA(0,cols), ldda );
                dq_to_panel( Magma_minproductLower, ib, A(rows-ib,cols), lda, work+ib*ib);

                // Send the triangular part on the GPU
                magma_minproduct_dsetmatrix( ib, ib, work, ib, dwork(0), lddwork );
        ...
        magma_minproduct_dgetmatrix( m, nu, dA(0,0), ldda, A(0,0), lda );
    } else {
        mu = m;
        nu = n;
    }
