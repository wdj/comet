...
__global__ void
magma_tally3_zmcsrgpu_kernel1( int num_rows,
                 magma_tally3DoubleComplex *A_val,
                 magma_tally3_index_t *A_rowptr,
                 magma_tally3_index_t *A_colind,
                 magma_tally3DoubleComplex *B_val,
    ...
    if(row<num_rows){
        magma_tally3DoubleComplex zero = MAGMA_tally3_Z_ZERO;
...
__global__ void
magma_tally3_zmcsrgpu_kernel2( int num_rows,
                 magma_tally3_index_t *B_rowptr,
                 magma_tally3_index_t *A_rowptr ){
...
__global__ void
magma_tally3_zmcsrgpu_kernel3( int num_rows,
                 magma_tally3DoubleComplex *B_val,
                 magma_tally3_index_t *B_rowptr,
                 magma_tally3_index_t *B_colind,
    ...
    magma_tally3_queue_t queue )
{
    ...
    magma_tally3_z_matrix B={Magma_tally3_CSR}, B2={Magma_tally3_CSR};
    magma_tally3_z_matrix dA={Magma_tally3_CSR}, CSRA={Magma_tally3_CSR};
    ...
    if ( A->memory_location == Magma_tally3_DEV && A->storage_type == Magma_tally3_CSR ) {
        ...
        magma_tally3_zmcsrgpu_kernel1<<< grid1, BLOCK_SIZE1, 0, queue >>>
                ( A->num_rows, A->dval, A->drow, A->dcol, B.dval, B.drow, B.dcol );

        // correct the row pointer
        dim3 grid2( 1, 1, 1);
        magma_tally3_zmcsrgpu_kernel2<<< grid2, BLOCK_SIZE2, 0, queue >>>
        ...
        magma_tally3_zmcsrgpu_kernel3<<< grid1, BLOCK_SIZE1, 0, queue >>>
        ...
        CHECK( magma_tally3_zmconvert( *A, &CSRA, A->storage_type, Magma_tally3_CSR, queue ));
        CHECK( magma_tally3_zmtransfer( *A, &dA, A->memory_location, Magma_tally3_DEV, queue ));

        CHECK( magma_tally3_zmcsrcompressor_gpu( &dA, queue ));

        magma_tally3_zmfree( &dA, queue );
        magma_tally3_zmfree( A, queue );
        CHECK( magma_tally3_zmtransfer( dA, &CSRA, Magma_tally3_DEV, A_location, queue ));
        CHECK( magma_tally3_zmconvert( CSRA, A, Magma_tally3_CSR, A_storage, queue ));
        magma_tally3_zmfree( &dA, queue );
