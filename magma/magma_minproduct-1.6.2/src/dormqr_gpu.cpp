...
/**
    ...
    dT      DOUBLE_PRECISION array on the GPU that is the output
            (the 9th argument) of magma_minproduct_dgeqrf_gpu.

    @param[in]
    nb      INTEGER
            This is the blocking size that was used in pre-computing DT, e.g.,
            the blocking size used in magma_minproduct_dgeqrf_gpu.
    ...
    magma_minproduct_int_t *info)
{
    ...
    *info = 0;
    ...
    hwork[0] = MAGMA_minproduct_D_MAKE( lwkopt, 0 );
    ...
    if ( step < 0 ) {
        ...
        magma_minproduct_dgetmatrix( ma, ib, dA(i,  i ), ldda, hA, ma );
        magma_minproduct_dgetmatrix( mi, ni, dC(ic, jc), lddc, hC, mi );
        ...
        magma_minproduct_dsetmatrix( mi, ni, hC, mi, dC(ic, jc), lddc );
    }
    ...
    // TODO sync. For cases Q*C and C*Q^T, last call is magma_minproduct_dlarfb_gpu,
    // which is async magma_minproduct_gemm calls, so dormqr can be unfinished.
