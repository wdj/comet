...
int main(int argc, char **argv)
{
    ...
    double *A, *X, *Y, *Ylapack, *Ycublas, *Ymagma_minproduct, *Ymagma_minproduct1, *hwork;
    magma_minproductDouble_ptr dA, dX, dY;
    magma_minproductDouble_ptr d_lA[Magma_minproductMaxGPUs], dwork[Magma_minproductMaxGPUs];
    ...
    magma_minproduct_int_t nb = 64;  // required by magma_minproductblas_dsymv_mgpu implementation
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int ioffset=0; ioffset < noffsets; ioffset += 1 ) {
        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( Ymagma_minproduct,  double, vecsize );
            TESTING_MALLOC_CPU( Ymagma_minproduct1, double, vecsize );
            ...
            // TODO make magma_minproduct_dmalloc_bcyclic helper function?
            for( dev=0; dev < opts.ngpu; dev++ ) {
            ...
            magma_minproduct_dmake_symmetric( Noffset, A, lda );
            ...
            /* =====================================================================
               ...
               =================================================================== */
            magma_minproduct_setdevice( opts.device );
            magma_minproduct_dsetmatrix( Noffset, Noffset, A, lda, dA, ldda );
            magma_minproduct_dsetvector( Noffset, X, incx, dX, incx );
            magma_minproduct_dsetvector( Noffset, Y, incx, dY, incx );
            ...
            magma_minproduct_dgetvector( Noffset, dY, incx, Ycublas, incx );
            ...
            magma_minproductblas_dsymv_work( opts.uplo, N,
                                  alpha, dA + offset + offset*ldda, ldda,
                                         dX + offset, incx,
            ...
            magma_minproduct_dgetvector( Noffset, dY, incx, Ymagma_minproduct1, incx );
            
            /* =====================================================================
               Performs operation using MAGMA_minproductBLAS (multi-GPU)
               =================================================================== */
            magma_minproduct_dsetmatrix_1D_col_bcyclic( Noffset, Noffset, A, lda, d_lA, ldda, opts.ngpu, nb );
            blasf77_dcopy( &Noffset, Y, &incx, Ymagma_minproduct, &incx );
            
            // workspaces do NOT need to be zero -- set to NAN to prove
            for( dev=0; dev < opts.ngpu; ++dev ) {
                magma_minproduct_setdevice( dev );
                magma_minproductblas_dlaset( Magma_minproductFull, ldwork, 1, MAGMA_minproduct_D_NAN, MAGMA_minproduct_D_NAN, dwork[dev], ldwork );
            }
            lapackf77_dlaset( "Full", &lhwork, &ione, &MAGMA_minproduct_D_NAN, &MAGMA_minproduct_D_NAN, hwork, &lhwork );
            
            mgpu_time = magma_minproduct_sync_wtime(0);
            
            magma_minproduct_int_t info;
            info = magma_minproductblas_dsymv_mgpu(
                opts.uplo, N,
                alpha,
                d_lA, ldda, offset,
                X + offset, incx,
                ...
                opts.ngpu, nb, queues );
            if ( info != 0 )
                printf("magma_minproductblas_dsymv_mgpu returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
            
            info = magma_minproductblas_dsymv_mgpu_sync(
                opts.uplo, N,
                alpha,
                d_lA, ldda, offset,
                X + offset, incx,
                ...
                opts.ngpu, nb, queues );
            if ( info != 0 )
                printf("magma_minproductblas_dsymv_sync returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
            ...
            if ( opts.lapack ) {
                ...
                blasf77_daxpy( &Noffset, &c_neg_one, Ymagma_minproduct, &incx, Ylapack, &incx );
            ...
            blasf77_daxpy( &Noffset, &c_neg_one, Ymagma_minproduct, &incx, Ycublas, &incx );
