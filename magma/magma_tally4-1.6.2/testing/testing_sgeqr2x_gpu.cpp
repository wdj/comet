...
int main( int argc, char** argv)
{
    ...
    magma_tally4Float_ptr d_A,  d_T, ddA, dtau;
    magma_tally4Float_ptr d_A2, d_T2, ddA2, dtau2;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            // todo replace with magma_tally4_slaset
            magma_tally4blas_slaset( Magma_tally4Full, N, N, c_zero, c_zero, ddA,  N );
            magma_tally4blas_slaset( Magma_tally4Full, N, N, c_zero, c_zero, d_T,  N );
            magma_tally4blas_slaset( Magma_tally4Full, N, N, c_zero, c_zero, ddA2, N );
            magma_tally4blas_slaset( Magma_tally4Full, N, N, c_zero, c_zero, d_T2, N );
        
            lwork = -1;
            lapackf77_sgeqrf(&M, &N, NULL, &M, NULL, tmp, &lwork, &info);
            lwork = (magma_tally4_int_t)MAGMA_tally4_S_REAL( tmp[0] );
            lwork = max(lwork, N*N);
            ...
            lapackf77_slacpy( Magma_tally4UpperLowerStr, &M, &N, h_A, &lda, h_R, &lda );
            magma_tally4_ssetmatrix( M, N, h_R, lda,  d_A, ldda );
            magma_tally4_ssetmatrix( M, N, h_R, lda, d_A2, ldda );
            ...
            if (opts.version == 1)
                magma_tally4_sgeqr2x_gpu(M, N, d_A, ldda, dtau, d_T, ddA, dwork, &info);
            else if (opts.version == 2)
                magma_tally4_sgeqr2x2_gpu(M, N, d_A, ldda, dtau, d_T, ddA, dwork, &info);
            else if (opts.version == 3)
                magma_tally4_sgeqr2x3_gpu(M, N, d_A, ldda, dtau, d_T, ddA, dwork, &info);
            else {
                printf( "call magma_tally4_sgeqr2x4_gpu\n" );
                /*
                ...
                */
                magma_tally4_sgeqr2x4_gpu(M, N, d_A, ldda, dtau, d_T, ddA, dwork, NULL, &info);
                //magma_tally4_sgeqr2x4_gpu(M, N, d_A, ldda, dtau, d_T, ddA, dwork, &info, stream[1]);
                //magma_tally4_sgeqr2x4_gpu(M, N, d_A2, ldda, dtau2, d_T2, ddA2, dwork2, &info, stream[0]);
                //magma_tally4_sgeqr2x4_gpu(M, N, d_A2, ldda, dtau2, d_T2, ddA2, dwork2, &info, NULL);
                //gflops *= 2;
            }
            ...
            if (info != 0) {
                printf("magma_tally4_sgeqr2x_gpu version %d returned error %d: %s.\n",
                ...
                if ( opts.check ) {
                    /* =====================================================================
                       ...
                       =================================================================== */
                    magma_tally4_sgetmatrix( M, N, d_A, ldda, h_R, M );
                    magma_tally4_sgetmatrix( N, N, ddA, N,    h_T, N );
                    magma_tally4_sgetmatrix( min_mn, 1, dtau, min_mn,   tau, min_mn );
                    ...
                    lapackf77_slacpy( Magma_tally4UpperLowerStr, &M, &N, h_R, &M, h_A, &lda );
                    lapackf77_slarft( Magma_tally4ForwardStr, Magma_tally4ColumnwiseStr,
                                      &M, &N, h_A, &lda, tau, h_work, &N);
                    //magma_tally4_sgeqr2(&M, &N, h_A, &lda, tau, h_work, &info);
                    ...
                    magma_tally4_sgetmatrix( N, N, d_T, N, h_T, N );
    
                    float terr = 0.;
                    for(int col=0; col < N; col++)
                        for(int row=0; row <= col; row++)
                            terr += (  MAGMA_tally4_S_ABS(h_work[row + col*N] - h_T[row + col*N])*
                                       MAGMA_tally4_S_ABS(h_work[row + col*N] - h_T[row + col*N])  );
                    terr = sqrt( terr );
