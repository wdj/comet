...
/**
    ...
    as returned by CGEQRF. Q is of order M if SIDE = Magma_minproductLeft and of order N
    ...
    lwork   INTEGER
            ...
            LWORK >= (M-K+NB)*(N+NB) + N*NB if SIDE = Magma_minproductLeft, and
            LWORK >= (N-K+NB)*(M+NB) + M*NB if SIDE = Magma_minproductRight,
    ...
    dT      COMPLEX array on the GPU that is the output
            (the 9th argument) of magma_minproduct_cgeqrf_gpu.

    @param[in]
    nb      INTEGER
            This is the blocking size that was used in pre-computing DT, e.g.,
            the blocking size used in magma_minproduct_cgeqrf_gpu.
    ...
    magma_minproduct_int_t *info)
{
    ...
    magma_minproduct_int_t i, lddwork;
    magma_minproduct_int_t i1, i2, step, ib, ic, jc, ma, mi, ni, nq, nw;
    ...
    *info = 0;
    ...
    hwork[0] = MAGMA_minproduct_C_MAKE( lwkopt, 0 );
    ...
    if ( step < 0 ) {
        ...
        magma_minproductFloatComplex* hA = hwork;
        magma_minproductFloatComplex* hC = hwork + ma*ib;
        magma_minproductFloatComplex* hW = hwork + ma*ib + mi*ni;
        magma_minproduct_int_t lhwork = lwork - (ma*ib + mi*ni);
        
        magma_minproduct_cgetmatrix( ma, ib, dA(i,  i ), ldda, hA, ma );
        magma_minproduct_cgetmatrix( mi, ni, dC(ic, jc), lddc, hC, mi );
        ...
        magma_minproduct_csetmatrix( mi, ni, hC, mi, dC(ic, jc), lddc );
    }
    ...
    if (nb < k) {
        for( i=i1; (step < 0 ? i >= i2 : i < i2); i += step ) {
            ...
            magma_minproduct_clarfb_gpu( side, trans, Magma_minproductForward, Magma_minproductColumnwise,
                              mi, ni, ib,
                              dA(i,  i ), ldda, dT(i), nb,
    ...
    // TODO sync. For cases Q*C and C*Q^T, last call is magma_minproduct_clarfb_gpu,
    // which is async magma_minproduct_gemm calls, so cunmqr can be unfinished.
