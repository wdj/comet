    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( norm[inorm] == Magma_tally2OneNorm )
                ...
                lwork = M;
            // read whole matrix
            gbytes = M*N*sizeof(magma_tally2FloatComplex) / 1e9;
            ...
            //h_A[ 1 + 1*lda ] = MAGMA_tally2_C_NAN;
            ...
            norm_magma_tally2 = magma_tally2blas_clange( norm[inorm], M, N, d_A, ldda, d_work );
            gpu_time = magma_tally2_wtime() - gpu_time;
            gpu_perf = gbytes / gpu_time;
            if (norm_magma_tally2 < 0)
                printf("magma_tally2blas_clange returned error %f: %s.\n",
            ...
            norm_lapack = lapackf77_clange( lapack_norm_const_tally2(norm[inorm]), &M, &N, h_A, &lda, h_work );
            ...
            //         MAGMA_tally2_C_REAL( h_A[1+1*lda] ),
            //         MAGMA_tally2_C_IMAG( h_A[1+1*lda] ) );
