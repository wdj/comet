    ...
    magma_minproduct_int_t *info)
{
    ...
    double *tau;
    float  *stau;
    magma_minproductDouble_ptr dworkd;
    magma_minproductFloat_ptr  dworks;
    magma_minproductDouble_ptr dR, dT;
    magma_minproductFloat_ptr  dSA, dSX, dST;
    double Xnrmv, Rnrmv;
    double          Anrm, Xnrm, Rnrm, cte, eps;
    magma_minproduct_int_t     i, j, iiter, lddsa, lddsx, lddr, nb, lhwork, minmn, size, ldworkd;
    ...
    *info = 0;
    ...
    nb   = magma_minproduct_get_sgeqrf_nb(m);
    minmn= min(m, n);
    ...
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_smalloc( &dworks, size )) {
    ...
    /* dworkd(dR) = lddr*nrhs */
    ldworkd = lddr*nrhs;
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc( &dworkd, ldworkd )) {
        magma_minproduct_free( dworks );
    ...
    size = lhwork + minmn;
    magma_minproduct_smalloc_cpu( &hworks, size );
    if ( hworks == NULL ) {
        magma_minproduct_free( dworks );
        magma_minproduct_free( dworkd );
    ...
    Anrm = magma_minproductblas_dlange(Magma_minproductInfNorm, m, n, dA, ldda, (double*)dworkd );
    cte  = Anrm * eps * pow((double)n, 0.5) * BWDMAX;

    /*
     * Convert to single precision
     */
    magma_minproductblas_dlag2s( m, nrhs, dB, lddb, dSX, lddsx, info );
    if (*info != 0) {
        *iter = -2;
        goto FALLBACK;
    }

    magma_minproductblas_dlag2s( m, n, dA, ldda, dSA, lddsa, info );
    if (*info != 0) {
        *iter = -2;
        goto FALLBACK;
    }
    ...
    magma_minproduct_sgeqrf_gpu( m, n, dSA, lddsa, stau, dST, info );
    if (*info != 0) {
        *iter = -3;
        goto FALLBACK;
    }
    ...
    magma_minproduct_sgeqrs_gpu( m, n, nrhs, dSA, lddsa, stau, dST, dSX, lddsx, hworks, lhwork, info );
    if (*info != 0) {
        *iter = -3;
        goto FALLBACK;
    }
    ...
    magma_minproductblas_slag2d( n, nrhs, dSX, lddsx, dX, lddx, info );
    magma_minproductblas_dlacpy( Magma_minproductUpperLower, m, nrhs, dB, lddb, dR, lddr );
    if ( nrhs == 1 ) {
        magma_minproduct_dgemv( Magma_minproductNoTrans, m, n,
                     c_neg_one, dA, ldda,
                                dX, 1,
                     c_one,     dR, 1 );
    }
    else {
        magma_minproduct_dgemm( Magma_minproductNoTrans, Magma_minproductNoTrans, m, nrhs, n,
                     c_neg_one, dA, ldda,
                                dX, lddx,
                     c_one,     dR, lddr );
    }
    ...
    for( j=0; j < nrhs; j++ ) {
        ...
        i = magma_minproduct_idamax ( m, dR(0,j), 1 ) - 1;
    ...
    *iter = 0;
    ...
    magma_minproduct_free( dworks );
    magma_minproduct_free( dworkd );
    magma_minproduct_free_cpu( hworks );
    return *info;
...
REFINEMENT:
    ...
    for( iiter=1; iiter < ITERMAX; ) {
        *info = 0;
        ...
        magma_minproductblas_dlag2s( m, nrhs, dR, lddr, dSX, lddsx, info );
        if (*info != 0) {
            *iter = -2;
            goto FALLBACK;
        }
        ...
        for( j=0; j < nrhs; j++ ) {
            magma_minproductblas_dsaxpycp( n, dSX(0,j), dX(0,j), dB(0,j), dR(0,j) );
        }
        // dR = dB  (whole m rows)
        magma_minproductblas_dlacpy( Magma_minproductUpperLower, m, nrhs, dB, lddb, dR, lddr );
...
FALLBACK:
    ...
    nb   = magma_minproduct_get_dgeqrf_nb( m );
    size = (2*min(m, n) + (n+31)/32*32 )*nb;
    if ( size > ldworkd ) {
        magma_minproduct_free( dworkd );
        if (MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc( &dworkd, size )) {
    ...
    size = lhwork + minmn;
    magma_minproduct_dmalloc_cpu( &hworkd, size );
    if ( hworkd == NULL ) {
        magma_minproduct_free( dworkd );
    ...
    if (*info == 0) {
        ...
        magma_minproduct_dgeqrs_gpu( m, n, nrhs, dA, ldda, tau, dT, dB, lddb, hworkd, lhwork, info );
        magma_minproductblas_dlacpy( Magma_minproductUpperLower, n, nrhs, dB, lddb, dX, lddx );
    }

    magma_minproduct_free( dworkd );
    magma_minproduct_free_cpu( hworkd );
    return *info;
}
