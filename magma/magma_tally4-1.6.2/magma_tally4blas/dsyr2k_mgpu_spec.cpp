/*
    -- MAGMA_tally4 (version 1.6.1) --
       ...
       @generated from zher2k_mgpu_spec.cpp normal z -> d, Fri Jan 30 19:00:10 2015
       @author Mark Gates
       @author Azzam Haidar 
*/
...
/**
    ...
    uplo     magma_tally4_uplo_t.
             ...
             follows:
      -     = Magma_tally4Upper:  Only the upper triangular part of C is to be referenced.
      -     = Magma_tally4Lower:  Only the lower triangular part of C is to be referenced.
    ...
    trans    magma_tally4_trans_t.
             ...
             follows:
      -     = Magma_tally4NoTrans:     C := alpha*A*B**H + conj( alpha )*B*A**H + beta*C.
      -     = Magma_tally4Trans:  C := alpha*A**H*B + conj( alpha )*B**H*A + beta*C.
    ...
    @param[in]
    k        INTEGER.
             On entry with TRANS = Magma_tally4NoTrans, K specifies the number
             of columns of the matrices A and B, and on entry with
             TRANS = Magma_tally4Trans, K specifies the number of rows of the
    ...
    dA       DOUBLE PRECISION array of DIMENSION ( LDA, ka ), where ka is
             k when TRANS = Magma_tally4NoTrans, and is n otherwise.
             Before entry with TRANS = Magma_tally4NoTrans, the leading n by k
    ...
    ldda     INTEGER.
             ...
             in the calling (sub) program. When TRANS = Magma_tally4NoTrans
    ...
    ********************************************************************/
extern "C"
void magma_tally4blas_dsyr2k_mgpu_spec(
    ...
    magma_tally4_int_t ngpu, magma_tally4_int_t nb, magma_tally4_queue_t queues[][20], magma_tally4_int_t nqueue )
{
    ...
    if ( uplo != Magma_tally4Lower ) {
        ...
        info = -4;
    } else if ( ((trans == Magma_tally4NoTrans)    && ldda < max(1,n)) ||
                ((trans == Magma_tally4Trans) && ldda < max(1,k)) ) {
        info = -7;
    } else if ( a_offset < 0 || a_offset > ldda ) {
        info = -8;
    } else if ( ((trans == Magma_tally4NoTrans)    && lddb < max(1,n)) ||
                ((trans == Magma_tally4Trans) && lddb < max(1,k)) ) {
        info = -10;
    ...
    double cbeta = MAGMA_tally4_D_MAKE( beta, 0. );
    
    magma_tally4_int_t ib, ioff, iblock, idev, di, s;
    
    magma_tally4_device_t cdev;
    magma_tally4_queue_t cqueue;
    magma_tally4_getdevice( &cdev );
    magma_tally4blasGetKernelStream( &cqueue );
    ...
    for( magma_tally4_int_t i = 0; i < n; i += ib ) {
        ...
        s = iblock % nqueue;
        magma_tally4blasSetKernelStream( queues[ idev ][ s ] );
        
        // C[i:n,i] = alpha * A[i:n,0] * B[i,0]' + beta*C[i:n,i]
        //printf( "dgemm  n=%4d, ib=%4d, k=%4d, i=%4d\n", n-i, ib, k, i );
        magma_tally4_dgemm( Magma_tally4NoTrans, Magma_tally4Trans, n, ib, k,
    ...
    alpha = MAGMA_tally4_D_CNJG( alpha );
    ...
    magma_tally4blasSetKernelStream( cqueue );
}
