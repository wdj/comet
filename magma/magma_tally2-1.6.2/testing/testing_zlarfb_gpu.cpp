    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int itran = 0; itran < 2; ++itran ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {            
            ...
            magma_tally2DoubleComplex *C, *R, *V, *T, *W;
            TESTING_MALLOC_CPU( C, magma_tally2DoubleComplex, ldc*N );
            TESTING_MALLOC_CPU( R, magma_tally2DoubleComplex, ldc*N );
            TESTING_MALLOC_CPU( V, magma_tally2DoubleComplex, ldv*K );
            TESTING_MALLOC_CPU( T, magma_tally2DoubleComplex, ldt*K );
            TESTING_MALLOC_CPU( W, magma_tally2DoubleComplex, ldw*K );
            
            magma_tally2DoubleComplex_ptr dC, dV, dT, dW;
            TESTING_MALLOC_DEV( dC, magma_tally2DoubleComplex, ldc*N );
            TESTING_MALLOC_DEV( dV, magma_tally2DoubleComplex, ldv*K );
            TESTING_MALLOC_DEV( dT, magma_tally2DoubleComplex, ldt*K );
            TESTING_MALLOC_DEV( dW, magma_tally2DoubleComplex, ldw*K );
            
            // C is M x N.
            size = ldc*N;
            lapackf77_zlarnv( &ione, ISEED, &size, C );
            //printf( "C=" );  magma_tally2_zprint( M, N, C, ldc );
            ...
            if ( storev[istor] == Magma_tally2Columnwise ) {
                if ( direct[idir] == Magma_tally2Forward ) {
                    lapackf77_zlaset( Magma_tally2UpperStr, &K, &K, &c_zero, &c_one, V, &ldv );
                }
                else {
                    lapackf77_zlaset( Magma_tally2LowerStr, &K, &K, &c_zero, &c_one, &V[(ldv-K)], &ldv );
                }
            }
            else {
                ...
                std::swap( ldv, nv );
                if ( direct[idir] == Magma_tally2Forward ) {
                    lapackf77_zlaset( Magma_tally2LowerStr, &K, &K, &c_zero, &c_one, V, &ldv );
                }
                else {
                    lapackf77_zlaset( Magma_tally2UpperStr, &K, &K, &c_zero, &c_one, &V[(nv-K)*ldv], &ldv );
                }
            }
            //printf( "# ldv %d, nv %d\n", ldv, nv );
            //printf( "V=" );  magma_tally2_zprint( ldv, nv, V, ldv );
            ...
            if ( direct[idir] == Magma_tally2Forward ) {
                lapackf77_zlaset( Magma_tally2LowerStr, &k1, &k1, &c_zero, &c_zero, &T[1], &ldt );
            }
            else {
                lapackf77_zlaset( Magma_tally2UpperStr, &k1, &k1, &c_zero, &c_zero, &T[1*ldt], &ldt );
            }
            //printf( "T=" );  magma_tally2_zprint( K, K, T, ldt );
            
            magma_tally2_zsetmatrix( M,   N,  C, ldc, dC, ldc );
            magma_tally2_zsetmatrix( ldv, nv, V, ldv, dV, ldv );
            magma_tally2_zsetmatrix( K,   K,  T, ldt, dT, ldt );
            
            lapackf77_zlarfb( lapack_side_const_tally2( side[iside] ), lapack_trans_const_tally2( trans[itran] ),
                              lapack_direct_const_tally2( direct[idir] ), lapack_storev_const_tally2( storev[istor] ),
                              &M, &N, &K,
                              V, &ldv, T, &ldt, C, &ldc, W, &ldw );
            //printf( "HC=" );  magma_tally2_zprint( M, N, C, ldc );
            
            magma_tally2_zlarfb_gpu( side[iside], trans[itran], direct[idir], storev[istor],
                              M, N, K,
                              dV, ldv, dT, ldt, dC, ldc, dW, ldw );
            magma_tally2_zgetmatrix( M, N, dC, ldc, R, ldc );
            //printf( "dHC=" );  magma_tally2_zprint( M, N, R, ldc );
