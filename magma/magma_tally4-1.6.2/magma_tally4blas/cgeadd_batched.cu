...
cgeadd_batched_kernel(
    ...
    const magma_tally4FloatComplex * const *dAarray, int ldda,
    magma_tally4FloatComplex              **dBarray, int lddb )
{
    // dA and dB iterate across row i
    const magma_tally4FloatComplex *dA = dAarray[ blockIdx.y ];
    magma_tally4FloatComplex       *dB = dBarray[ blockIdx.y ];
    int i = blockIdx.x*blockDim.x + threadIdx.x;
    if ( i < m ) {
        dA += i;
        dB += i;
        const magma_tally4FloatComplex *dAend = dA + n*ldda;
        while( dA < dAend ) {
            *dB = alpha*(*dA) + (*dB);
            dA += ldda;
            dB += lddb;
...
/**
    @see magma_tally4blas_cgeadd_batched_q
    @ingroup magma_tally4_caux2
    ********************************************************************/
extern "C" void
