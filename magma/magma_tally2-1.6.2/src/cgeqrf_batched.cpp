...
#define dA(i, j)  (dA + (i) + (j)*ldda)   // A(i, j) means at i row, j column
    ...
    magma_tally2FloatComplex **dW0_displ = NULL;
    ...
    magma_tally2FloatComplex **dW5_displ = NULL;

    magma_tally2FloatComplex *dwork = NULL;
    magma_tally2FloatComplex *dT   = NULL;
    magma_tally2FloatComplex *dR   = NULL;
    magma_tally2FloatComplex **dR_array = NULL;
    magma_tally2FloatComplex **dT_array = NULL; 
    magma_tally2FloatComplex **cpuAarray = NULL;
    magma_tally2FloatComplex **cpuTarray = NULL;
    ...
    magma_tally2_malloc((void**)&dW4_displ, batchCount * sizeof(*dW4_displ));  // used in clarfb
    ...
    magma_tally2_cmalloc(&dwork,  (2 * nb * n) * batchCount);
    magma_tally2_cmalloc(&dR,  ldr * n   * batchCount);
    magma_tally2_cmalloc(&dT,  ldt * ldt * batchCount);
    magma_tally2_malloc_cpu((void**) &cpuAarray, batchCount*sizeof(magma_tally2FloatComplex*));
    magma_tally2_malloc_cpu((void**) &cpuTarray, batchCount*sizeof(magma_tally2FloatComplex*));
    ...
    magma_tally2blas_claset_q(Magma_tally2Full, ldr, n*batchCount  , MAGMA_tally2_C_ZERO, MAGMA_tally2_C_ZERO, dR, ldr, queue);
    magma_tally2blas_claset_q(Magma_tally2Full, ldt, ldt*batchCount, MAGMA_tally2_C_ZERO, MAGMA_tally2_C_ZERO, dT, ldt, queue);
    ...
    magma_tally2_getvector( batchCount, sizeof(magma_tally2FloatComplex*), dA_array, 1, cpuAarray, 1);
    magma_tally2_getvector( batchCount, sizeof(magma_tally2FloatComplex*), dT_array, 1, cpuTarray, 1);
    ...
    for(i=0; i<min_mn;i+=nb)
    {
            ...
            magma_tally2_cdisplace_pointers(dW0_displ, dA_array, ldda, i, i, batchCount, queue); 
            magma_tally2_cdisplace_pointers(dW2_displ, tau_array, 1, i, 0, batchCount, queue);
            ...
            magma_tally2_cgeqrf_panel_batched(m-i, ib, jb, 
                                       dW0_displ, ldda, 
                                       dW2_displ, 
                                       dT_array, ldt, 
                                       dR_array, ldr,
           ...
           cgeqrf_copy_upper_batched(ib, jb, dW0_displ, ldda, dR_array, ldr, batchCount, queue);
            ...
            if( (n-ib-i) > 0)
            {
                ...
                magma_tally2blas_claset_batched(Magma_tally2Upper, ib, ib, MAGMA_tally2_C_ZERO, MAGMA_tally2_C_ONE, dW0_displ, ldda, batchCount, queue); 
                magma_tally2_cdisplace_pointers(dW2_displ, tau_array, 1, i, 0, batchCount, queue); 

                // it is faster since it is using BLAS-3 GEMM routines, different from lapack implementation 
                magma_tally2_clarft_batched(m-i, ib, 0,
                                 dW0_displ, ldda,
                                 dW2_displ,
                                 dT_array, ldt, 
                                 dW4_displ, nb*ldt,
                ...
                { 
                    ...
                    for(k=0; k<batchCount; k++)
                    {
                        ...
                        magma_tally2_clarfb_gpu_gemm(Magma_tally2Left, Magma_tally2ConjTrans, Magma_tally2Forward, Magma_tally2Columnwise,
                                    m-i, n-i-ib, ib,
                ...
                else
                {
                    ...
                    magma_tally2_cdisplace_pointers(dW1_displ, dA_array, ldda, i, i+ib, batchCount, queue); 
            ...
            magma_tally2blas_clacpy_batched(Magma_tally2Upper, ib, ib, dR_array, ldr, dW0_displ, ldda, batchCount, queue);
