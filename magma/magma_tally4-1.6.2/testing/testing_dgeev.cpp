...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_tally4_dgeev( opts.jobvl, opts.jobvr,
                         N, h_R, lda, w1, w1i,
                         VL, lda, VR, lda,
                         h_work, lwork, &info );
            gpu_time = magma_tally4_wtime() - gpu_time;
            if (info != 0)
                printf("magma_tally4_dgeev returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                if ( opts.jobvr == Magma_tally4Vec ) {
                    ...
                    lapackf77_dget22( Magma_tally4NoTransStr, Magma_tally4NoTransStr, Magma_tally4NoTransStr,
                    ...
                    for( int j = 0; j < N; ++j ) {
                        tnrm = 1.;
                        if (w1i[j] == 0.)
                            tnrm = magma_tally4_cblas_dnrm2( N, &VR[j*lda], ione );
                        else if (w1i[j] > 0.)
                            tnrm = magma_tally4_dlapy2( magma_tally4_cblas_dnrm2( N, &VR[j*lda],     ione ),
                                                 magma_tally4_cblas_dnrm2( N, &VR[(j+1)*lda], ione ));
                        
                        result[1] = max( result[1], min( ulpinv, MAGMA_tally4_D_ABS(tnrm-1.)/ulp ));
                        
                        if (w1i[j] > 0.) {
                            vmx  = vrmx = 0.;
                            for( int jj = 0; jj < N; ++jj ) {
                                vtst = magma_tally4_dlapy2( VR[jj+j*lda], VR[jj+(j+1)*lda]);
                                if (vtst > vmx)
                                    vmx = vtst;
                                
                                if ( (VR[jj + (j+1)*lda])==0. &&
                                     MAGMA_tally4_D_ABS( VR[jj+j*lda] ) > vrmx)
                                {
                                    vrmx = MAGMA_tally4_D_ABS( VR[jj+j*lda] );
                                }
                            }
                            if (vrmx / vmx < 1. - ulp*2.)
                                result[1] = ulpinv;
                ...
                if ( opts.jobvl == Magma_tally4Vec ) {
                    ...
                    lapackf77_dget22( Magma_tally4TransStr, Magma_tally4NoTransStr, Magma_tally4TransStr,
                    ...
                    for( int j = 0; j < N; ++j ) {
                        tnrm = 1.;
                        if (w1i[j] == 0.)
                            tnrm = magma_tally4_cblas_dnrm2( N, &VL[j*lda], ione );
                        else if (w1i[j] > 0.)
                            tnrm = magma_tally4_dlapy2( magma_tally4_cblas_dnrm2( N, &VL[j*lda],     ione ),
                                                 magma_tally4_cblas_dnrm2( N, &VL[(j+1)*lda], ione ));
                        
                        result[3] = max( result[3], min( ulpinv, MAGMA_tally4_D_ABS(tnrm-1.)/ulp ));
                        
                        if (w1i[j] > 0.) {
                            vmx  = vrmx = 0.;
                            for( int jj = 0; jj < N; ++jj ) {
                                vtst = magma_tally4_dlapy2( VL[jj+j*lda], VL[jj+(j+1)*lda]);
                                if (vtst > vmx)
                                    vmx = vtst;
                                
                                if ( (VL[jj + (j+1)*lda])==0. &&
                                     MAGMA_tally4_D_ABS( VL[jj+j*lda]) > vrmx)
                                {
                                    vrmx = MAGMA_tally4_D_ABS( VL[jj+j*lda] );
                                }
                            }
                            if (vrmx / vmx < 1. - ulp*2.)
                                result[3] = ulpinv;
            ...
            if ( opts.check == 2 ) {
                ...
                magma_tally4_dgeev( Magma_tally4Vec, Magma_tally4Vec,
                             N, h_R, lda, w1, w1i,
                             VL, lda, VR, lda,
                             h_work, lwork, &info );
                if (info != 0)
                ...
                //lapackf77_dlacpy( Magma_tally4UpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                //magma_tally4_dgeev( Magma_tally4NoVec, Magma_tally4NoVec,
                //             N, h_R, lda, w2, w2i,
                //             &DUM, 1, &DUM, 1,
                //             h_work, lwork, &info );
                //if (info != 0)
                //    printf("magma_tally4_dgeev (case N, N) returned error %d: %s.\n",
                //           (int) info, magma_tally4_strerror( info ));
                //
                ...
                lapackf77_dlacpy( Magma_tally4UpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                magma_tally4_dgeev( Magma_tally4NoVec, Magma_tally4Vec,
                             N, h_R, lda, w2, w2i,
                             &DUM, 1, LRE, lda,
                             h_work, lwork, &info );
                if (info != 0)
                    printf("magma_tally4_dgeev (case N, V) returned error %d: %s.\n",
                           (int) info, magma_tally4_strerror( info ));
                ...
                for( int j = 0; j < N; ++j )
                    for( int jj = 0; jj < N; ++jj )
                        if ( ! MAGMA_tally4_D_EQUAL( VR[j+jj*lda], LRE[j+jj*lda] ))
                            result[7] = 0;
                
                // ----------
                // Compute eigenvalues and left eigenvectors
                lapackf77_dlacpy( Magma_tally4UpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                magma_tally4_dgeev( Magma_tally4Vec, Magma_tally4NoVec,
                             N, h_R, lda, w2, w2i,
                             LRE, lda, &DUM, 1,
                             h_work, lwork, &info );
                if (info != 0)
                    printf("magma_tally4_dgeev (case V, N) returned error %d: %s.\n",
                           (int) info, magma_tally4_strerror( info ));
                ...
                for( int j = 0; j < N; ++j )
                    for( int jj = 0; jj < N; ++jj )
                        if ( ! MAGMA_tally4_D_EQUAL( VL[j+jj*lda], LRE[j+jj*lda] ))
                            result[8] = 0;
            ...
            if ( opts.lapack ) {
                ...
                lapackf77_dgeev( lapack_vec_const_tally4(opts.jobvl), lapack_vec_const_tally4(opts.jobvr),
                                 &N, h_A, &lda, w2, w2i,
                                 VL, &lda, VR, &lda,
                                 h_work, &lwork, &info );
                ...
                for( int j=0; j < N; ++j ) {
                    w1copy[j] = MAGMA_tally4_Z_MAKE( w1[j], w1i[j] );
                    w2copy[j] = MAGMA_tally4_Z_MAKE( w2[j], w2i[j] );
                }
