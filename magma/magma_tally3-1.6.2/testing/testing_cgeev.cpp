...
extern "C"
float magma_tally3_dlapy2(float x, float y)
{
    float ret_val, d;
    float w, z, xabs, yabs;
    
    xabs = MAGMA_tally3_D_ABS(x);
    yabs = MAGMA_tally3_D_ABS(y);
    w    = max(xabs, yabs);
    z    = min(xabs, yabs);
    if (z == 0.) {
        ret_val = w;
...
extern "C"
float magma_tally3_dzlapy2(magma_tally3FloatComplex x)
{
    return magma_tally3_dlapy2( MAGMA_tally3_C_REAL(x), MAGMA_tally3_C_IMAG(x) );
}
...
int main( int argc, char** argv)
{
    ...
    magma_tally3FloatComplex *h_A, *h_R, *VL, *VR, *h_work, *w1, *w2;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lda   = N;
            n2    = lda*N;
            nb    = magma_tally3_get_cgehrd_nb(N);
            lwork = N*(1 + nb);
            ...
            TESTING_MALLOC_CPU( w1,     magma_tally3FloatComplex, N );
            TESTING_MALLOC_CPU( w2,     magma_tally3FloatComplex, N );
            ...
            TESTING_MALLOC_PIN( VL,     magma_tally3FloatComplex, n2 );
            TESTING_MALLOC_PIN( VR,     magma_tally3FloatComplex, n2 );
            ...
            magma_tally3_cgeev( opts.jobvl, opts.jobvr,
                         N, h_R, lda, w1,
                         VL, lda, VR, lda,
                         h_work, lwork, rwork, &info );
            gpu_time = magma_tally3_wtime() - gpu_time;
            if (info != 0)
                printf("magma_tally3_cgeev returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                if ( opts.jobvr == Magma_tally3Vec ) {
                    ...
                    lapackf77_cget22( Magma_tally3NoTransStr, Magma_tally3NoTransStr, Magma_tally3NoTransStr,
                                      &N, h_A, &lda, VR, &lda, w1,
                    ...
                    result[1] = -1.;
                    for( int j = 0; j < N; ++j ) {
                        tnrm = magma_tally3_cblas_scnrm2( N, &VR[j*lda], ione );
                        result[1] = max( result[1], min( ulpinv, fabs(tnrm-1.)/ulp ));
                        
                        vmx = vrmx = 0.;
                        for( int jj = 0; jj <N; ++jj ) {
                            vtst = magma_tally3_dzlapy2(VR[jj + j*lda]);
                            if (vtst > vmx)
                                vmx = vtst;
                            
                            if (MAGMA_tally3_C_IMAG(VR[jj + j*lda])==0. &&
                                fabs( MAGMA_tally3_C_REAL(VR[jj+j*lda]) ) > vrmx)
                            {
                                vrmx = fabs( MAGMA_tally3_C_REAL( VR[jj+j*lda] ) );
                            }
                        }
                        if (vrmx / vmx < 1. - ulp*2.)
                            result[1] = ulpinv;
                ...
                if ( opts.jobvl == Magma_tally3Vec ) {
                    ...
                    lapackf77_cget22( Magma_tally3ConjTransStr, Magma_tally3NoTransStr, Magma_tally3ConjTransStr,
                                      &N, h_A, &lda, VL, &lda, w1,
                    ...
                    result[3] = -1.;
                    for( int j = 0; j < N; ++j ) {
                        tnrm = magma_tally3_cblas_scnrm2( N, &VL[j*lda], ione );
                        result[3] = max( result[3], min( ulpinv, fabs(tnrm - 1.)/ ulp ));
                        
                        vmx = vrmx = 0.;
                        for( int jj = 0; jj < N; ++jj ) {
                            vtst = magma_tally3_dzlapy2(VL[jj + j*lda]);
                            if (vtst > vmx)
                                vmx = vtst;
                            
                            if (MAGMA_tally3_C_IMAG(VL[jj + j*lda])==0. &&
                                fabs( MAGMA_tally3_C_REAL( VL[jj + j*lda] ) ) > vrmx)
                            {
                                vrmx = fabs( MAGMA_tally3_C_REAL( VL[jj+j*lda]) );
                            }
                        }
                        if (vrmx / vmx < 1. - ulp*2.)
                            result[3] = ulpinv;
            ...
            if ( opts.check == 2 ) {
                ...
                // this is really slow because it calls magma_tally3_cgeev multiple times
                magma_tally3FloatComplex *LRE, DUM;
                TESTING_MALLOC_PIN( LRE, magma_tally3FloatComplex, n2 );
                ...
                magma_tally3_cgeev( Magma_tally3Vec, Magma_tally3Vec,
                             N, h_R, lda, w1,
                             VL, lda, VR, lda,
                             h_work, lwork, rwork, &info );
                if (info != 0)
                    printf("magma_tally3_cgeev (case V, V) returned error %d: %s.\n",
                           (int) info, magma_tally3_strerror( info ));
                ...
                // lapackf77_clacpy( Magma_tally3UpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                // magma_tally3_cgeev( Magma_tally3NoVec, Magma_tally3NoVec,
                //              N, h_R, lda, w2,
                //              &DUM, 1, &DUM, 1,
                //              h_work, lwork, rwork, &info );
                // if (info != 0)
                //     printf("magma_tally3_cgeev (case N, N) returned error %d: %s.\n",
                //            (int) info, magma_tally3_strerror( info ));
                // 
                // // Do test 5: W(full) = W(partial, W only)
                // result[4] = 1;
                // for( int j = 0; j < N; ++j )
                //     if ( ! MAGMA_tally3_C_EQUAL( w1[j], w2[j] ))
                //         result[4] = 0;
                
                // ----------
                // Compute eigenvalues and right eigenvectors
                lapackf77_clacpy( Magma_tally3UpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                magma_tally3_cgeev( Magma_tally3NoVec, Magma_tally3Vec,
                             N, h_R, lda, w2,
                             &DUM, 1, LRE, lda,
                             h_work, lwork, rwork, &info );
                if (info != 0)
                    printf("magma_tally3_cgeev (case N, V) returned error %d: %s.\n",
                           (int) info, magma_tally3_strerror( info ));
                ...
                for( int j = 0; j < N; ++j )
                    for( int jj = 0; jj < N; ++jj )
                        if ( ! MAGMA_tally3_C_EQUAL( VR[j+jj*lda], LRE[j+jj*lda] ))
                            result[7] = 0;
                
                // ----------
                // Compute eigenvalues and left eigenvectors
                lapackf77_clacpy( Magma_tally3UpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                magma_tally3_cgeev( Magma_tally3Vec, Magma_tally3NoVec,
                             N, h_R, lda, w2,
                             LRE, lda, &DUM, 1,
                             h_work, lwork, rwork, &info );
                if (info != 0)
                    printf("magma_tally3_cgeev (case V, N) returned error %d: %s.\n",
                           (int) info, magma_tally3_strerror( info ));
                ...
                for( int j = 0; j < N; ++j )
                    for( int jj = 0; jj < N; ++jj )
                        if ( ! MAGMA_tally3_C_EQUAL( VL[j+jj*lda], LRE[j+jj*lda] ))
                            result[8] = 0;
            ...
            if ( opts.lapack ) {
                ...
                lapackf77_cgeev( lapack_vec_const_tally3(opts.jobvl), lapack_vec_const_tally3(opts.jobvr),
                                 &N, h_A, &lda, w2,
                                 VL, &lda, VR, &lda,
                                 h_work, &lwork, rwork, &info );
                ...
                for( int j=0; j < N; ++j ) {
                    for( int j2=j; j2 < N; ++j2 ) {
                        ...
                        float diff2 = magma_tally3_dzlapy2( diff ) / max( magma_tally3_dzlapy2( w1copy[j] ), tol );
                        if ( diff2 < 100*tol ) {
                            if ( j != j2 ) {
