    ...
    magma_tally3_int_t *ipiv, magma_tally3_int_t *info)
{
    ...
    magma_tally3_int_t nb = magma_tally3_get_dsytrf_nb(n);
    ...
    *info = 0;
    ...
    if ((MAGMA_tally3_SUCCESS != magma_tally3_dmalloc( &dA, n*ldda  )) ||
        (MAGMA_tally3_SUCCESS != magma_tally3_dmalloc( &dW, (1+nb)*ldda ))) {
    ...
    //magma_tally3_dsetmatrix_async( n, n, A(0,0), lda, dA(0,0), ldda, stream[0] );
    if ( upper ) {
       for (int k = 0; k < n; k+=nb ) {
           kb = min(nb, n-k);
           magma_tally3_dsetmatrix_async( k+kb, kb, A(0,k), lda, dA(0,k), ldda, stream[0] );
       }
    } else {
       for (int k = 0; k < n; k+=nb ) {
           kb = min(nb, n-k);
           magma_tally3_dsetmatrix_async( n-k, kb, A(k,k), lda, dA(k,k), ldda, stream[0] );
       }
    }
    trace_gpu_end( 0, 0 );
    ...
    if ( upper ) {
        ...
        for (int k = n-1; k >= 0; k-=kb ) {
            ...
            if ( k+1 > nb ) {
                ...
                magma_tally3_dlasyf_gpu( Magma_tally3Upper, nk, kb, &kb, A( 0, 0 ), lda, dA( 0, 0 ), ldda, 
                ...
                magma_tally3_dgetmatrix( nk, nk, dA( 0, 0 ),ldda, A( 0, 0 ),lda );
                lapackf77_dsytf2( Magma_tally3UpperStr, &nk, A( 0, 0 ), &lda, &ipiv[0], &iinfo );
                kb = k+1;
            }
        ...
        for (int k = 0; k < n; k += kb ) {
            ...
            if ( k < n-nb ) {
                ...
                magma_tally3_dlasyf_gpu( Magma_tally3Lower, nk, nb, &kb, A( k, k ), lda, dA( k, k ), ldda,
                                  &ipiv[k], dW, ldda, stream, event, &iinfo );

            } else {
                /* Use unblocked code to factorize columns k:n of A */
                magma_tally3_queue_sync( stream[0] );
                magma_tally3_dgetmatrix( nk,nk, dA(k,k),ldda, A(k,k),lda );
                lapackf77_dsytf2( Magma_tally3LowerStr, &nk, A( k, k ), &lda, &ipiv[k], &iinfo );
            }
            /* Set INFO on the first occurrence of a zero pivot */
