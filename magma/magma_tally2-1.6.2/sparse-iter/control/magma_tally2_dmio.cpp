/*
    -- MAGMA_tally2 (version 1.6.2) --
       ...
       @generated from magma_tally2_zmio.cpp normal z -> d, Sun May  3 11:23:01 2015
       @author Hartwig Anzt
*/
...
extern "C"
magma_tally2_int_t read_d_csr_from_mtx(
    ...
    magma_tally2_queue_t queue )
{
    ...
    *nnz      = num_nonzeros;
    ...
    CHECK( magma_tally2_dmalloc_cpu( &coo_val, *nnz ) );
    ...
    if (mm_is_real(matcode) || mm_is_integer(matcode)) {
        for(magma_tally2_int_t i = 0; i < *nnz; ++i) {
            ...
            coo_row[i] = ROW - 1;
            coo_col[i] = COL - 1;
            coo_val[i] = MAGMA_tally2_D_MAKE( VAL, 0.);
        }
        ...
        for(magma_tally2_int_t i = 0; i < *nnz; ++i) {
            ...
            coo_row[i] = ROW - 1;
            coo_col[i] = COL - 1;
            coo_val[i] = MAGMA_tally2_D_MAKE( 1.0, 0.);
        }
       ...
       for(magma_tally2_int_t i = 0; i < *nnz; ++i) {
            ...
            coo_row[i] = ROW - 1;
            coo_col[i] = COL - 1;
            coo_val[i] = MAGMA_tally2_D_MAKE( VAL, VALC);
        }
    ...
    if (mm_is_symmetric(matcode)) { // duplicate off diagonal entries
        ...
        CHECK( magma_tally2_dmalloc_cpu( &new_val, true_nonzeros ) );
    ...
    */
    CHECK( magma_tally2_dmalloc_cpu( val, *nnz ) );
    ...
    magma_tally2_queue_t queue )
{
    ...
    CHECK( magma_tally2_dwrite_csr_mtx( B, Magma_tally2ColMajor, filename, queue ));
cleanup:
    return info;
}
    ...
    magma_tally2_queue_t queue )
{
    ...
    if ( MajorType == Magma_tally2ColMajor ) {
        ...
        CHECK( magma_tally2_d_cucsrtranspose( A, &B, queue ));
        magma_tally2_dprint_matrix( A, queue );
        
        
        magma_tally2_dprint_matrix( B, queue );
        ...
        for(i=0; i < B.num_cols; i++) {
            ...
            for(j=0; j < rowtemp2 - rowtemp1; j++) {
                fprintf( fp, "%d %d %.16g %.16g\n",
                    ...
                    MAGMA_tally2_D_REAL((B.val)[rowtemp1+j]),
                    MAGMA_tally2_D_IMAG((B.val)[rowtemp1+j]) );
        ...
        for(i=0; i < A.num_cols; i++) {
            ...
            for(j=0; j < rowtemp2 - rowtemp1; j++) {
                fprintf( fp, "%d %d %.16g %.16g\n",
                    ...
                    MAGMA_tally2_D_REAL((A.val)[rowtemp1+j]),
                    MAGMA_tally2_D_IMAG((A.val)[rowtemp1+j]) );
        ...
        for(i=0; i < n_col; i++) {
            ...
            for(j=0; j < rowtemp2 - rowtemp1; j++) {
                printf( "%d %d %.6e %.6e\n",
                    ...
                    MAGMA_tally2_D_REAL((new_val)[rowtemp1+j]),
                    MAGMA_tally2_D_IMAG((new_val)[rowtemp1+j]) );
        ...
        for(i=0; i < n_col; i++) {
            ...
            for(j=0; j < rowtemp2 - rowtemp1; j++) {
                printf( "%d %d %.6e %.6e\n",
                    ...
                    MAGMA_tally2_D_REAL((*val)[rowtemp1+j]),
                    MAGMA_tally2_D_IMAG((*val)[rowtemp1+j]) );
    ...
    magma_tally2_queue_t queue )
{
    ...
    #define magma_tally2_dprintval( tmp )       {                                  \
        if ( MAGMA_tally2_D_EQUAL( tmp, c_zero )) {                                \
            ...
            printf( " %8.4f", MAGMA_tally2_D_REAL( tmp ));                         \
        }                                                                   \
    }
    #endif
    //**************************************************************
    ...
    magma_tally2_d_matrix C={Magma_tally2_CSR};
        ...
        if ( A.storage_type == Magma_tally2_DENSE ) {
            for( i=0; i < (A.num_rows); i++ ) {
                for( j=0; j < A.num_cols; j++ ) {
                    magma_tally2_dprintval( A.val[i*(A.num_cols)+j] );
                }
                printf( "\n" );
            }
        }
            ...
            if ( A.num_rows < 11 && A.num_cols < 11 ) {
                CHECK( magma_tally2_dmconvert( A, &C, A.storage_type, Magma_tally2_DENSE, queue ));
                CHECK( magma_tally2_dprint_matrix(  C, queue ));
            ...
            CHECK( magma_tally2_dmconvert( A, &C, A.storage_type, Magma_tally2_CSR, queue ));
            CHECK( magma_tally2_dprint_matrix(  C, queue ));
        }
    }
    else {
        magma_tally2_d_matrix C={Magma_tally2_CSR};
        CHECK( magma_tally2_dmtransfer( A, &C, A.memory_location, Magma_tally2_CPU, queue ));
        CHECK( magma_tally2_dprint_matrix(  C, queue ));
    }
    ...
    magma_tally2_queue_t queue )
{
    ...
    CHECK( magma_tally2_dmalloc_cpu( &coo_val, A->nnz ) );
    ...
    */
    CHECK( magma_tally2_dmalloc_cpu( &A->val, A->nnz ));
    ...
    if ( csr_compressor > 0) { // run the CSR compressor to remove zeros
        ...
        CHECK( magma_tally2_dmtransfer( *A, &B, Magma_tally2_CPU, Magma_tally2_CPU, queue ));
        CHECK( magma_tally2_d_csr_compressor(
            &(A->val), &(A->row), &(A->col),
        ...
        CHECK( magma_tally2_dmtransfer( B, A, Magma_tally2_CPU, Magma_tally2_CPU, queue ));
        //printf("done.\n");
