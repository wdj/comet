    ...
    magma_minproduct_int_t *info )
{
    ...
    magma_minproductFloatComplex *dwork[Magma_minproductMaxGPUs]={NULL}, *dpanel[Magma_minproductMaxGPUs]={NULL};
    magma_minproductFloatComplex *hwork=NULL, *hpanel=NULL;
    ...
    *info = 0;
    ...
    for( dev=0; dev < ngpu; dev++ ) {
        ...
        if ( MAGMA_minproduct_SUCCESS != magma_minproduct_cmalloc( &(dwork[dev]), (lddwork + ldda)*nb )) {
    ...
    if ( MAGMA_minproduct_SUCCESS != magma_minproduct_cmalloc_pinned( &hwork, lwork )) {
    ...
    if ( nb < min_mn ) {
        ...
        for( i = 0; i < min_mn-nb; i += nb ) {
            ...
            magma_minproduct_cgetmatrix_async( rows, ib,
            ...
            cpanel_to_q( Magma_minproductUpper, ib, hpanel(i), ldhpanel, hwork + ib*ib );
            ...
            for( dev=0; dev < ngpu; dev++ ) {
                ...
                magma_minproduct_csetmatrix_async( rows, ib,
                                        hpanel(i),   ldhpanel,
            ...
            /* Restore the panel */
            cq_to_panel( Magma_minproductUpper, ib, hpanel(i), ldhpanel, hwork + ib*ib );

            if (i + ib < n) {
                /* Send the T matrix to the GPU. */
                for( dev=0; dev < ngpu; dev++ ) {
                    magma_minproduct_setdevice( dev );
                    magma_minproduct_csetmatrix_async( ib, ib,
                                            hwork,      ib,
                ...
                for( dev=0; dev < ngpu; dev++ ) {
                    ...
                    if (dev == la_dev && i+nb < min_mn-nb) {
                        ...
                        magma_minproduct_clarfb_gpu( Magma_minproductLeft, Magma_minproductConjTrans, Magma_minproductForward, Magma_minproductColumnwise,
                                          rows, ib, ib,
                                          dpanel[dev],             ldda,       // V
                                          dwork[dev],              lddwork,    // T
    ...
    if (i < min_mn) {
        ...
        for( j=i; j < n; j += nb ) {
            ...
            ib = min( n-j, nb );
            magma_minproduct_setdevice( panel_dev );
            magma_minproduct_cgetmatrix( rows, ib,
                              dlA(panel_dev, i, i_local), ldda,
                              hwork + (j-i)*rows,         rows );
        }
        ...
        for( j=i; j < n; j += nb ) {
            ...
            ib = min( n-j, nb );
            magma_minproduct_setdevice( panel_dev );
            magma_minproduct_csetmatrix( rows, ib,
                              hwork + (j-i)*rows,         rows,
                              dlA(panel_dev, i, i_local), ldda );
        }
    }
