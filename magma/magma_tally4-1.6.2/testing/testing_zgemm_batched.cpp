...
int main( int argc, char** argv)
{
    ...
    magma_tally4DoubleComplex *h_A, *h_B, *h_C, *h_Cmagma_tally4, *h_Ccublas;
    magma_tally4DoubleComplex *d_A, *d_B, *d_C;
    ...
    magma_tally4DoubleComplex **B_array = NULL;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_Ccublas, magma_tally4DoubleComplex, sizeC  );

            TESTING_MALLOC_DEV( d_A, magma_tally4DoubleComplex, ldda*An*batchCount );
            TESTING_MALLOC_DEV( d_B, magma_tally4DoubleComplex, lddb*Bn*batchCount );
            ...
            /* =====================================================================
               ...
               =================================================================== */
            magma_tally4_zsetmatrix( Am, An*batchCount, h_A, lda, d_A, ldda );
            magma_tally4_zsetmatrix( Bm, Bn*batchCount, h_B, ldb, d_B, lddb );
            magma_tally4_zsetmatrix( M, N*batchCount, h_C, ldc, d_C, lddc );
            ...
            magma_tally4blas_zgemm_batched(opts.transA, opts.transB, M, N, K,
                             alpha, A_array, ldda,
                                    B_array, lddb,
            ...
            magma_tally4_zgetmatrix( M, N*batchCount, d_C, lddc, h_Cmagma_tally4, ldc );
            ...
            cublasZgemmBatched(handle, cublas_trans_const_tally4(opts.transA), cublas_trans_const_tally4(opts.transB), M, N, K,
                               &alpha, (const magma_tally4DoubleComplex**) A_array, ldda,
                               (const magma_tally4DoubleComplex**) B_array, lddb,
                               &beta,  C_array, lddc, batchCount );

            cublas_time = magma_tally4_sync_wtime( NULL ) - cublas_time;
            cublas_perf = gflops / cublas_time;
            
            magma_tally4_zgetmatrix( M, N*batchCount, d_C, lddc, h_Ccublas, ldc );
            ...
            if ( opts.lapack ) {
                ...
                for(int s=0; s<batchCount; s++)
                {
                    ...
                    blasf77_zaxpy( &C_batchSize, &c_neg_one, h_C + s*C_batchSize, &ione, h_Cmagma_tally4 + s*C_batchSize, &ione );
                    magma_tally4_err = lapackf77_zlange( "M", &M, &N, h_Cmagma_tally4 + s*C_batchSize, &ldc, work ) / Cnorm; 
                ...
                // compute relative error for magma_tally4, relative to cublas
                    ...
                    blasf77_zaxpy( &sizeC, &c_neg_one, h_Ccublas, &ione, h_Cmagma_tally4, &ione );
                    magma_tally4_error = lapackf77_zlange( "M", &M, &NN, h_Cmagma_tally4, &ldc, work ) / Cnorm;
