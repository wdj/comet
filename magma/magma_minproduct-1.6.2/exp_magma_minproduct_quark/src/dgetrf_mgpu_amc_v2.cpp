...
#include "magma_minproduct_amc.h"
...
#define PRECISION_d 
  ...
  #define magma_minproduct_dgemm magma_minproductblas_dgemm 
...
/*  -- MAGMA_minproduct (version 1.6.1) -- 
    ...
    =====================================================================    */ 
     ...
     /*Workspace*/
     double *AWORK;
     magma_minproduct_int_t AWORK_LD, AWORK_n;
    
     double **dlpanelT;
     magma_minproduct_int_t dlpanelT_m, dlpanelT_n;
...
//    magma_minproduct_event_t *A_event; /*Controling bucket*/
    ...
    *info = 0; 
    ...
    args = magma_minproduct_amc_args_get_default();

    if(args->nb==0)
     nb    = magma_minproduct_get_dgetrf_nb(m) ;//magma_minproduct dgetrf block size
    else
     nb = args->nb;
...
#if (dbglevel >=1)
    t1 = magma_minproduct_wtime();
#endif
    
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc_pinned(&AWORK, AWORK_LD*AWORK_n)) { 
    //if (MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc_cpu(&AWORK, AWORK_LD*AWORK_n)) {
    ...
    dlpanelT_n = m;
     dlpanelT = (double **)    malloc(num_gpus*sizeof(double*));
      for(dd=0;dd<num_gpus;dd++){
         magma_minproduct_setdevice(dd);

         if (MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc(&dlpanelT[dd], dlpanelT_m*dlpanelT_n)) { 
...
#if (dbglevel >=1)
    printf("[DBG] Time memory malloc (pinned):%f\n",magma_minproduct_wtime()-t1); 
    t1 = magma_minproduct_wtime();
#endif

    /*First touch the workspace by each thread*/
    //magma_minproduct_amc_dmemset(AWORK, 0.0, AWORK_LD*AWORK_n, nb, nbcores);
    ...
    *info = magma_minproduct_dgetrf_mgpu_work_amc(num_gpus, m, n, dlA, dlA_LD, ipiv, info, AWORK, AWORK_LD, AWORK_n, dlpanelT, dlpanelT_m, dlpanelT_n);
    ...
    magma_minproduct_free_pinned(AWORK);

    for(dd=0;dd<num_gpus;dd++){
        magma_minproduct_setdevice(dd);
        magma_minproduct_free(dlpanelT[dd]);
    }
...
//    ca_dbg_printMat_transpose_gpu(m, n, dA, dA_LD,"dA = LU"); 
    ...
    return *info; 
}   /* End of MAGMA_minproduct_DGETRF_REC_ASYNC_GPU */
...
/*  -- MAGMA_minproduct (version 1.6.1) -- 
    ...
    magma_minproduct_int_t mindim; 
    magma_minproduct_int_t nrows, ncols; 
    //double *work; 
 
 
     magma_minproduct_int_t dm_max, dn_max; 
     magma_minproduct_int_t I, J, K, M, N, U_K, L; 
     magma_minproduct_int_t i;
     //magma_minproduct_int_t A_m, A_n, A_N; 
     //magma_minproduct_int_t Am_max, An_max; 
     //magma_minproduct_int_t A_nb; 
     
 
     //magma_minproduct_int_t A_K; 
     double **dlAT; 
     magma_minproduct_int_t dlAT_LD; 
      
      
     double *dlAP_get[Magma_minproductMaxGPUs]; //*dlAP_set[Magma_minproductMaxGPUs]
      
     
     double *dlAP_set[Magma_minproductMaxGPUs];
     magma_minproduct_int_t dlAP_LD;
     ...
     //magma_minproduct_int_t nrows, ncols; 
     magma_minproduct_int_t gpu_nrows, gpu_ncols; 
    ...
    /*Workspace*/
    // magma_minproduct_int_t AWORK_NMAX;
    // magma_minproduct_int_t AWORK_m, AWORK_n, AWORK_N;
     ...
     magma_minproduct_int_t dlpanelT_NMAX;
     magma_minproduct_int_t dlpanelT_LD;

     amc_args_t *args;
    /*magma_minproduct_event_t *A_event;*/ /*Control bucket*/
     magma_minproduct_queue_t mstream[Magma_minproductMaxGPUs][3]; /*0: H2D, 1: compute, 2:D2H*/
     int dd;
...
//     double *tmpdA;
    ...
    *info = 0; 
    ...
    /* create the streams */
    //mstream = (magma_minproduct_queue_t *)    malloc(num_gpus*sizeof(magma_minproduct_queue_t));

    for(dd=0;dd<num_gpus;dd++){
       magma_minproduct_setdevice(dd); //required
       magma_minproduct_queue_create(&mstream[dd][0]);
       magma_minproduct_queue_create(&mstream[dd][1]);
       magma_minproduct_queue_create(&mstream[dd][2]);

       /*Set the stream for internal computations*/
       //magma_minproductblasSetKernelStream(mstream[dd][1]);
      // magma_minproductblasSetKernelStream(0); /*Use 0 instead of mstream[dd][1], Magma_minproductsetkernelStream is not thread safe*/ /*TODO: mae it safe*/
    }
    ...
    /// dlAP_set = (double **)    malloc(num_gpus*sizeof(double*));
     ...
     for(dd=0;dd<num_gpus;dd++){
         ...
         if (MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc( &dlAP_set[dd], dlAP_LD*nb)) { 
                *info = MAGMA_minproduct_ERR_DEVICE_ALLOC; 
                return *info; 
        } 
        
        /*
        if (MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc(&tmpdA, dlAP_LD*nb)) { 
                *info = MAGMA_minproduct_ERR_DEVICE_ALLOC; 
                return *info; 
        }
        */
        if ( magma_minproduct_is_devptr(dlAP_set[dd] ) == 0 ) {
        ...
        printf("0.4\n");
         ...
         if (MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc(&dlAP_get[dd], dlAP_LD*nb)) { 
                //magma_minproduct_free(dlAP_set); //TODO: free all previous buffers
                *info = MAGMA_minproduct_ERR_DEVICE_ALLOC; 
                return *info; 
        }
     }
    ...
    for(dd=0;dd<num_gpus;dd++){
        ...
        if (MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc(&dlAT[dd], dlAT_LD*dm_max )) { 
                for(J=0;J<dd;J++){
                    magma_minproduct_setdevice(J);
                    magma_minproduct_free( dlAP_set[J]); 
                    magma_minproduct_free( dlAP_get[J]);
                    //magma_minproduct_free(dlpanel[J]);
                    magma_minproduct_free(dlAT[J]);
                }
                //free(dlAP_set); 
                //free(dlAP_get);
                //free(dlpanel);
    ...
    //magma_minproduct_dgetmatrix(A_m, A_n, dA, dA_LD, A, A_LD); 
    magma_minproduct_dgetmatrix_1D_col_bcyclic(A_m, A_n, dlA, dlA_LD, A, A_LD, num_gpus, nb);
...
#endif
        //magma_minproductblasSetKernelStream(&mstream[dd]);
        ///magma_minproductblas_dtranspose2(dlAT(0,J), dlAT_LD, dlA(0,J), dlA_LD, m, gpu_ncols);
        magma_minproductblas_dtranspose2(dlAT[dd], dlAT_LD, dlA[dd], dlA_LD, m, gpu_ncols);
...
for(dd=0;dd<num_gpus;dd++){
    magma_minproduct_setdevice(dd);
    //magma_minproductblasSetKernelStream(&mstream[dd]);        
    magma_minproductblas_dtranspose2(dlAT[dd], dlAT_LD, dlA[dd], dlA_LD, m, n_local[dd]);
}
...
*/
     ...
     magma_minproduct_schedule_init(nbcores, num_gpus); 
     ...
     magma_minproduct_schedule_set_task_priority(INT_MAX-1);
     
     /*Schedule the first panel factorization*/ 
#ifdef USE_CALU
     magma_minproduct_insert_core_dtslu(A_m, nb, A(0,K), A_LD, ipiv(0), &iinfo, panel_num_threads, colptr(K));
     ...
     for(I=K+1; I<=M-1; I+=B_rows){ 
        ...
        magma_minproduct_insert_core_dtrsm('R', 'U', 'N', 'N', i_nrows, nb, c_one, A(0,K), A_LD, A(I,K), A_LD, colptr(K));
        //dtrsm("R", "U", "N", "N", &nrowPblock, &panel_NB, &dONE, &(A[M*pos+pos]), &LDA, &(A[lpos]), &LDA); //
     }

     
#else
     magma_minproduct_insert_core_dgetrf_rec(A_m, nb, A(0,K), A_LD, ipiv(0), &iinfo, panel_num_threads, colptr(K));  
#endif
     
     //magma_minproduct_insert_core_dgetrf(A_m, nb, A(0,K), A_LD, ipiv(0), &iinfo, colptr(K)); 
     ...
     for(dd=0;dd<num_gpus;dd++){
        ...
        ///magma_minproduct_insert_dev_dsetmatrix_transpose(dd, A_m, nb, A(0,K), A_LD, dlpanel(dd,K), dlpanel_LD, dlAP_set[dd], dlAP_LD, colptr(K), dlpanel[dd]);
        magma_minproduct_insert_dev_dsetmatrix_async_transpose(dd, A_m, nb, A(0,K), A_LD, dlpanelT(dd,K,K), dlpanelT_LD, mstream[dd][0], dlAP_set[dd], dlAP_LD, colptr(K), dlpanelT[dd]);
         /*TODO: test*/
        //magma_minproduct_setdevice(dd);

        
        //printf("M:%d, nb:%d, A_Ld:%d, dlAP_LD:%d\n",A_m, nb, A_LD, dlAP_LD);
        //magma_minproduct_dsetmatrix(A_m, nb, A(0,K), A_LD, dlAP_set[dd], dlAP_LD);
        ...
        // cudaMemcpy(&tmpdA, dlAP_set[dd], sizeof(double*), cudaMemcpyHostToDevice);
        // magma_minproduct_insert_dev_dsetmatrix_transpose(dd, A_m, nb, A(0,K), A_LD, dlpanel(dd,K), dlpanel_LD, tmpdA, dlAP_LD, colptr(K), dlpanel[dd]);
     }
#if (dbglevel==10) 
    magma_minproduct_schedule_barrier();
    //printf("4.5: int to continue"); scanf("%d", &I);
     ...
     dd = GID(K);
     //magma_minproduct_insert_dev_dsetmatrix_transpose(dd, A_m, nb, A(0,K), A_LD, dlAT(0,K), dlAT_LD, dlAP_set[dd], dlAP_LD, colptr(K), dlAT(0,K)); 
     magma_minproduct_insert_dev_dsetmatrix_async_transpose(dd, A_m, nb, A(0,K), A_LD, dlAT(0,K), dlAT_LD, mstream[dd][0], dlAP_set[dd], dlAP_LD, colptr(K), dlAT(0,K)); 
 ...
 printf("5.0\n");
     ...
     for(K=0;K<=N-1;K++){ 
         ...
         /*Set the priority max for the GPU computations*/
            magma_minproduct_schedule_set_task_priority(INT_MAX);
            //// magma_minproduct_schedule_set_task_priority(INT_MAX - N*K);
         ...
         for(J=K+A_N;J<min(K+A_N+num_gpus,N);J++){
            ...
            /*Determine the device which own the first column of the group of columns to update*/
             dd = GID(J);
              ...
              { 
                  ...
                  magma_minproduct_insert_dev_dlaswp(dd, gpu_ncols, dlAT(K, J), dlAT_LD, ONE, nb, ipiv(K), ONE, dlAT(K, J-1)); /*non blocking*/                  
                  //printf("debug barrier\n");
                  //magma_minproduct_schedule_barrier();
                  //&(dlpanel[dd][dlpanel_LD*nb*K])
                  magma_minproduct_insert_dev_dtrsm(dd, Magma_minproductRight,  Magma_minproductUpper, Magma_minproductNoTrans, Magma_minproductUnit, gpu_ncols, nb, c_one, dlpanelT(dd,K,K), dlpanelT_LD, dlAT(K,J), dlAT_LD);/*non blocking*/ 
 
                  /* aij^T = aij^T - (lik.ukj)^T = aij^T - ukj^T.lik^T*/ //&(dlpanel[dd][dlpanel_LD*nb*(K+1)])
                  magma_minproduct_insert_dev_dgemm(dd, Magma_minproductNoTrans,Magma_minproductNoTrans, gpu_ncols, gpu_nrows, nb, c_neg_one, dlAT(K,J), dlAT_LD, dlpanelT(dd,K+1,K), dlpanelT_LD, c_one, dlAT(K+1,J), dlAT_LD);/*non blocking*/    
                 ...
                 //// { 
                    ...
                    ////magma_minproduct_insert_dgetmatrix_transpose(gpu_nrows, ncols, dAT(K+1,K+A_N), dAT_LD, A(K+1,K+A_N), A_LD, dAP, dAP_LD, colptr(K+A_N)); //blocking
                 //// }
          ...
          for(J=K+1;J<=min(K+A_N-1, N-1);J++){ 
               ...
               if(J==K+1)
                   magma_minproduct_schedule_set_task_priority(INT_MAX -2 );
                  //// magma_minproduct_schedule_set_task_priority(INT_MAX - N*K -1);
               else
                   magma_minproduct_schedule_set_task_priority(INT_MAX -3 - J );//- N*K
                  //// magma_minproduct_schedule_set_task_priority(INT_MAX - N*K -3 -J);
               //magma_minproduct_schedule_set_task_priority(INT_MAX - J);

               /*dependency colptr(J): make sure column J is sent from GPU, and all previous update was done*/
               magma_minproduct_insert_core_dlaswp(ncols, A(K,J), A_LD, ONE, nb, ipiv(K), ONE, colptr(J)); 
 
               magma_minproduct_insert_core_dtrsm('L', 'L', 'N', 'U', nb, ncols, c_one, A(K,K), A_LD, A(K,J), A_LD, colptr(J)); 
             ...
             /*Compute the number of blocs rows to group together before the update. To avoid scheduling overhead.*/
              ...
              //printf("B_rows:%d\n",B_rows);
               for(I=K+1; I<=M-1; I+=B_rows){ 
                    ...
                    magma_minproduct_insert_core_dgemm('N','N', i_nrows, ncols, nb, c_neg_one, A(I,K), A_LD, A(K,J), A_LD, c_one, A(I,J), A_LD, colptr(K), colptr(J)); 
               } 
               ...
               { 
                    ...
                    magma_minproduct_schedule_set_task_priority(INT_MAX -1);
                    ///magma_minproduct_schedule_set_task_priority(0); //TEST: testing prio_0
                   //// magma_minproduct_schedule_set_task_priority(INT_MAX - N*K - 2);
#ifdef USE_CALU
                    magma_minproduct_insert_core_dtslu(nrows, ncols, A(K+1,K+1), A_LD, ipiv(K+1), &iinfo, panel_num_threads, colptr(K+1));
                    ...
                    B_rows = max(B_rows,4); /*maximun of 4*/ 
                     ...
                     for(I=K+2; I<=M-1; I+=B_rows){ 
                        ...
                        i_nrows = min(B_rows*nb, m-I*nb);
                        magma_minproduct_insert_core_dtrsm('R', 'U', 'N', 'N', i_nrows, ncols, c_one, A(K+1,K+1), A_LD, A(I,K+1), A_LD, colptr(K+1));
                        //dtrsm("R", "U", "N", "N", &nrowPblock, &panel_NB, &dONE, &(A[M*pos+pos]), &LDA, &(A[lpos]), &LDA); //
                     }

#else
                   magma_minproduct_insert_core_dgetrf_rec(nrows, ncols, A(K+1,K+1), A_LD, ipiv(K+1), &iinfo, panel_num_threads, colptr(K+1)); 
#endif
                   // magma_minproduct_insert_core_dgetrf(nrows, ncols, A(K+1,K+1), A_LD, ipiv(K+1), &iinfo, colptr(K+1)); 
 
                    /*Transfer the factorized panel in the buffer of GPU (dlpanel)*/

                     for(dd=0;dd<num_gpus;dd++){
                         //&(dlpanel[dd][dlpanel_LD*nb*(K+1)])
                        ///magma_minproduct_insert_dev_dsetmatrix_transpose(dd, nrows, ncols, A(K+1, K+1), A_LD, dlpanel(dd, K+1), dlpanel_LD, dlAP_set[dd], dlAP_LD, colptr(K+1), dlpanel[dd]);
                        magma_minproduct_insert_dev_dsetmatrix_async_transpose(dd, nrows, ncols, A(K+1, K+1), A_LD, dlpanelT(dd, K+1, K+1), dlpanelT_LD, mstream[dd][0], dlAP_set[dd], dlAP_LD, colptr(K+1), dlpanelT[dd]);
                     }
                    ...
                    ///magma_minproduct_insert_dsetmatrix_transpose(nrows, ncols, A(U_K, K+1), A_LD, dAT(U_K, K+1), dAT_LD, dAP, dAP_LD, A(K+1,K+1), dAT(K+1,K+1)); 
                    //magma_minproduct_insert_dev_dsetmatrix_transpose(nrows, ncols, A(U_K, K+1), A_LD, dAT(U_K, K+1), dAT_LD, dAP_set, dAP_LD, colptr(K+1), dAT(K+1,K+1));
 
                    
                     /*Transfer also the factorized panel on its right position in the final matrix (transposition included)*/ 
                     /*TODO: this may use cudaMemcpyDeviceToDevice and initiate the transfer from dlpanel*/
                     dd = GID(K+1);
                     ///magma_minproduct_insert_dev_dsetmatrix_transpose(dd, nrows, ncols, A(U_K, K+1), A_LD, dlAT(U_K,K+1), dlAT_LD, dlAP_set[dd], dlAP_LD, colptr(K+1), dlAT(K+1,K+1));
                     magma_minproduct_insert_dev_dsetmatrix_async_transpose(dd, nrows, ncols, A(U_K, K+1), A_LD, dlAT(U_K,K+1), dlAT_LD, mstream[dd][0], dlAP_set[dd], dlAP_LD, colptr(K+1), dlAT(K+1,K+1));
          ...
          for(L=K+A_N;L<K+A_N+A_NBT;L++)
          {
               if(L<N) { 
                 ...
                 ///magma_minproduct_insert_dev_dgetmatrix_transpose(dd, gpu_nrows, ncols, dlAT(K+1,K+A_N), dlAT_LD, A(K+1,K+A_N), A_LD, dlAP_get[dd], dlAP_LD, colptr(K+A_N)); //blocking
             
                 /*make sure the computations are done on stream 1 and send a block column on stream 2*/
                 magma_minproduct_insert_dev_queue_sync(dd, mstream[dd][1], dlAT(K+1,L)); 
                 magma_minproduct_insert_dev_dgetmatrix_async_transpose(dd, gpu_nrows, ncols, dlAT(K+1,L), dlAT_LD, A(K+1,L), A_LD, mstream[dd][2], dlAP_get[dd], dlAP_LD, colptr(L));
...
/*
 ...
 magma_minproduct_schedule_barrier(); 
    ...
    for(dd=0;dd<=min(num_gpus-1, K-1);dd++){
        ...
        if(gpu_ncols>0){
            magma_minproduct_setdevice(dd);
            magma_minproductblas_dlaswp(gpu_ncols, dlAT(K, J), dlAT_LD, ONE, nrows, ipiv(K), ONE);
        }
       ...
       // magma_minproduct_insert_dlaswp(ncols, dAT(K, 0), dAT_LD, ONE, nrows, ipiv(K), ONE, dAT(K-1,0)); 
...
/*Shutdown the scheduler*/
     magma_minproduct_schedule_delete();
...
/*update permutation vector indexes*/ 
    ...
    for(dd=0;dd<num_gpus;dd++){
        ...
        magma_minproduct_setdevice(dd);
        //magma_minproductblasSetKernelStream(&mstream[dd]);
        magma_minproductblas_dtranspose2(dlA[dd], dlA_LD, dlAT[dd], dlAT_LD, n_local[dd], m);
    }

 /*no need for synchro, since dtranspose is blocking*/
/*
    if (m == n) {
      magma_minproductblas_dtranspose_inplace(m, dAT, dAT_LD); //( m, dAT, dAT_LD ); 
      dA = dAT; 
   } 
   else { 
      magma_minproductblas_dtranspose2( dA, dA_LD, dAT, dAT_LD, n, m ); 
...
//   free(A_event);
   ...
   /*
   for(I=0;I<A_SIZE;I++){
       magma_minproduct_free_pinned(A[I]); 
    }
   */
    ...
    for(dd=0;dd<num_gpus;dd++){
        magma_minproduct_setdevice(dd);
       magma_minproduct_queue_destroy(mstream[dd][0]);
       magma_minproduct_queue_destroy(mstream[dd][1]);
       magma_minproduct_queue_destroy(mstream[dd][2]);
    }
...
//    free(k_local);
    for(dd=0;dd<num_gpus;dd++){
        magma_minproduct_setdevice(dd);
        magma_minproduct_free( dlAP_set[dd]); 
        magma_minproduct_free( dlAP_get[dd]);
        magma_minproduct_free(dlAT[dd]);
    }
    ...
    return *info; 
}   /* End of MAGMA_minproduct_DGETRF_REC_ASYNC_WORK_GPU */
