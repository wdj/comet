...
/**
    ...
    lwork   INTEGER
            ...
            optimal blocksize given by magma_tally4_get_zhetrd_nb().
    \n
...
#define dW(id, i, j) (dW[(id)] + (j)*ldda + (i))
    ...
    magma_tally4_int_t nb = magma_tally4_get_zhetrd_nb(n), ib, ib2;
    ...
    magma_tally4DoubleComplex *hwork = NULL;
    magma_tally4DoubleComplex_ptr dwork2[Magma_tally4MaxGPUs] = { NULL };
    magma_tally4DoubleComplex_ptr dA[Magma_tally4MaxGPUs]     = { NULL };
    magma_tally4DoubleComplex_ptr dW[Magma_tally4MaxGPUs]     = { NULL };
    ...
    *info = 0;
    ...
    for( dev=0; dev < ngpu; dev++ ) {
        ...
        if ( MAGMA_tally4_SUCCESS != magma_tally4_zmalloc( &dA[dev],     nlocal*ldda + 3*lddw*nb ) ||
             MAGMA_tally4_SUCCESS != magma_tally4_zmalloc( &dwork2[dev], ldwork2 ) ) {
    ...
    if ( MAGMA_tally4_SUCCESS != magma_tally4_zmalloc_pinned( &hwork, lhwork ) ) {
    ...
    if (upper) {
        ...
        if (1 <= n-nx) {
            magma_tally4_zhtodhe( ngpu, uplo, n, nb, A, lda, dA, ldda, queues, &iinfo );
        }
        ...
        for (i = nb*((n-1)/nb); i >= nx; i -= nb) {
            ...
            magma_tally4_zlatrd_mgpu( ngpu, uplo, i+ib, ib, nb,
                               A(0, 0), lda, e, tau,
                               work, ldwork,
                               dA, ldda, 0,
                               dW, i+ib,
            ...
            magma_tally4_zher2k_mgpu( ngpu, Magma_tally4Upper, Magma_tally4NoTrans, nb, i, ib,
                               c_neg_one, dW, i+ib, 0,
                               d_one,     dA, ldda, 0,
                               nqueue, queues);
            ...
            if (i-nb >= nx ) {
                ...
                magma_tally4_zgetmatrix_async( (i-nb)+ib2, ib2,
                                        dA(dev, 0, ii), ldda,
                                        A(0, i-nb),     lda,
                                        queues[dev][0] );
            }
            ...
            for (j = i; j < i+ib; ++j) {
                if ( j > 0 ) {
                    *A(j-1,j) = MAGMA_tally4_Z_MAKE( e[j - 1], 0 );
                }
                d[j] = MAGMA_tally4_Z_REAL( *A(j, j) );
            }
        } /* end of for i=... */
        ...
        if ( nx > 0 ) {
            if (1 <= n-nx) { /* else A is already on CPU */
                for (i=0; i < nx; i += nb) {
                    ...
                    magma_tally4_setdevice( dev );
                    magma_tally4_zgetmatrix_async( nx, ib,
                                            dA(dev, 0, ii), ldda,
                                            A(0, i),        lda,
                                            queues[dev][0] );
                }
        ...
        for (i = 0; i < n-nx; i += nb) {
            ...
            if (i != 0) {
                ...
                magma_tally4_zgetmatrix_async( n-i, ib,
                                        dA(dev, i, ii), ldda,
                                        A(i,i),         lda,
                                        queues[dev][0] );
                trace_gpu_end( dev, 0 );
            ...
            magma_tally4_zlatrd_mgpu( ngpu, uplo, n-i, ib, nb,
                               A(i, i), lda, &e[i], &tau[i],
                               work, ldwork,
                               dA, ldda, i,
                               dW, n-i,
            ...
            magma_tally4_zher2k_mgpu( ngpu, Magma_tally4Lower, Magma_tally4NoTrans, nb, n-i-ib, ib,
                               c_neg_one, dW, n-i, ib,
            ...
            for (j = i; j < i+ib; ++j) {
                if ( j+1 < n ) {
                    *A(j+1,j) = MAGMA_tally4_Z_MAKE( e[j], 0 );
                }
                d[j] = MAGMA_tally4_Z_REAL( *A(j, j) );
            }
        } /* for i=... */
        ...
        if ( i < n ) {
            ...
            if ( i > 0 ) {
                for (; i < n; i += nb) {
                    ...
                    magma_tally4_setdevice( dev );
                    magma_tally4_zgetmatrix_async( i_n, ib,
                                            dA(dev, iii, ii), ldda,
                                            A(iii, i),        lda,
                                            queues[dev][0] );
                }
...
#define dC(id, i, j)  (dC[(id)] + (j)*lddc + (i))
    ...
    for( i=0; i < n; i += nb ) {
        ...
        magma_tally4_zher2k( uplo, trans, ib, k,
