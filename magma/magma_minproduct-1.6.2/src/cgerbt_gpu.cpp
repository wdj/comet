        ...
        magma_minproductFloatComplex* u, magma_minproductFloatComplex* v)
{
    ...
    for (idx=0; idx<n; idx++){
        ...
        u[idx] = MAGMA_minproduct_C_MAKE(u1,u1);

        v[idx] = MAGMA_minproduct_C_MAKE(v1,v1);
    }
}
    ...
    magma_minproduct_int_t *info)
{
    ...
    *info = 0;
    ...
    magma_minproductFloatComplex *du, *dv;

    /* Allocate memory for the buterfly matrices */
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_cmalloc( &du, 2*n )) {
    ...
    magma_minproduct_csetvector( 2*n, U, 1, du, 1);
    magma_minproduct_csetvector( 2*n, V, 1, dv, 1);

    /* Perform Partial Random Butterfly Transformation on the GPU*/
    magma_minproductblas_cprbt(n, dA, ldda, du, dv);

    /* Compute U^T.b on the GPU*/
    for(int i= 0; i < nrhs; i++)
        magma_minproductblas_cprbt_mtv(n, du, dB+(i*lddb));
