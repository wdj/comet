/*
    -- MAGMA_tally2 (version 1.1) --
       ...
       @generated from magma_tally2_z_precond_wrapper.cpp normal z -> c, Mon May  4 11:57:23 2015
       @author Hartwig Anzt
    ...
    magma_tally2_queue_t queue )
{
    ...
    magma_tally2_c_solver_par psolver_par;
    ...
    magma_tally2_c_preconditioner pprecond;
    pprecond.solver = Magma_tally2_NONE;

    switch( precond->solver ) {
        case  Magma_tally2_CG:
                CHECK( magma_tally2_ccg_res( A, b, x, &psolver_par, queue )); break;
        case  Magma_tally2_BICGSTAB:
                CHECK( magma_tally2_cbicgstab( A, b, x, &psolver_par, queue )); break;
        case  Magma_tally2_GMRES:
                CHECK( magma_tally2_cfgmres( A, b, x, &psolver_par, &pprecond, queue )); break;
        case  Magma_tally2_JACOBI:
                CHECK( magma_tally2_cjacobi( A, b, x, &psolver_par, queue )); break;
        case  Magma_tally2_BAITER:
                CHECK( magma_tally2_cbaiter( A, b, x, &psolver_par, queue )); break;
        default:
                CHECK( magma_tally2_ccg_res( A, b, x, &psolver_par, queue )); break;
    ...
    magma_tally2_queue_t queue )
{
    ...
    if ( precond->solver == Magma_tally2_JACOBI ) {
        return magma_tally2_cjacobisetup_diagscal( A, &(precond->d), queue );
    }
    else if ( precond->solver == Magma_tally2_PASTIX ) {
        //return magma_tally2_cpastixsetup( A, b, precond, queue );
        return MAGMA_tally2_ERR_NOT_SUPPORTED;
    }
    else if ( precond->solver == Magma_tally2_ILU ) {
        return magma_tally2_ccumilusetup( A, precond, queue );
    }
    else if ( precond->solver == Magma_tally2_ICC ) {
        return magma_tally2_ccumiccsetup( A, precond, queue );
    }
    else if ( precond->solver == Magma_tally2_AICC ) {
        return magma_tally2_citericsetup( A, b, precond, queue );
    }
    else if ( precond->solver == Magma_tally2_AILU ) {
        return magma_tally2_citerilusetup( A, b, precond, queue );
    }
    ...
    magma_tally2_queue_t queue )
{
    ...
    magma_tally2_c_matrix tmp={Magma_tally2_CSR};
    ...
    if ( precond->solver == Magma_tally2_JACOBI ) {
        CHECK( magma_tally2_cjacobi_diagscal( A.num_rows, precond->d, b, x, queue ));
    }
    else if ( precond->solver == Magma_tally2_PASTIX ) {
        //CHECK( magma_tally2_capplypastix( b, x, precond, queue ));
        return MAGMA_tally2_ERR_NOT_SUPPORTED;
    }
    else if ( precond->solver == Magma_tally2_ILU ) {
        CHECK( magma_tally2_cvinit( &tmp, Magma_tally2_DEV, A.num_rows, b.num_cols, MAGMA_tally2_C_ZERO, queue ));
    }
    else if ( precond->solver == Magma_tally2_ICC ) {
        CHECK( magma_tally2_cvinit( &tmp, Magma_tally2_DEV, A.num_rows, b.num_cols, MAGMA_tally2_C_ZERO, queue ));
    }
    else if ( precond->solver == Magma_tally2_NONE ) {
        magma_tally2_ccopy( b.num_rows*b.num_cols, b.dval, 1, x->dval, 1 );      //  x = b
    }
    else {
        printf( "error: preconditioner type not yet supported.\n" );
    ...
    magma_tally2_queue_t queue )
{
    ...
    if ( precond->solver == Magma_tally2_JACOBI ) {
        ...
        CHECK( magma_tally2_capplycumilu_l( b, x, precond, queue ));
    }
    else if ( ( precond->solver == Magma_tally2_ILU ||
                precond->solver == Magma_tally2_AILU ) && precond->maxiter < 50 ) {
        magma_tally2_ccopy( b.num_rows*b.num_cols, b.dval, b.num_cols, x->dval, b.num_cols );
        magma_tally2_c_solver_par solver_par;
        solver_par.maxiter = precond->maxiter;
        //magma_tally2_cjacobiiter_sys( precond->L, b, precond->d, precond->work1, x, &solver_par, queue );
        CHECK( magma_tally2_cjacobispmvupdate(precond->maxiter, precond->L, precond->work1, b, precond->d, x, queue ));
    }
    else if ( ( precond->solver == Magma_tally2_ICC ||
                precond->solver == Magma_tally2_AICC ) && precond->maxiter >= 50 )  {
        CHECK( magma_tally2_capplycumicc_l( b, x, precond, queue ));
    }
    else if ( ( precond->solver == Magma_tally2_ICC ||
                precond->solver == Magma_tally2_AICC ) && precond->maxiter < 50 )  {
        magma_tally2_ccopy( b.num_rows*b.num_cols, b.dval, b.num_cols, x->dval, b.num_cols );
        ...
        CHECK( magma_tally2_capplycustomprecond_l( b, x, precond, queue ));
    }
    else {
        printf( "error: preconditioner type not yet supported.\n" );
        magma_tally2blasSetKernelStream( orig_queue );
    ...
    magma_tally2_queue_t queue )
{
    ...
    if ( precond->solver == Magma_tally2_JACOBI ) {
        magma_tally2_ccopy( b.num_rows*b.num_cols, b.dval, 1, x->dval, 1 );    // x = b
                ...
                precond->solver == Magma_tally2_AILU ) && precond->maxiter >= 50 ) {
        CHECK( magma_tally2_capplycumilu_r( b, x, precond, queue ));
    }
    else if ( ( precond->solver == Magma_tally2_ILU ||
                precond->solver == Magma_tally2_AILU ) && precond->maxiter < 50 ) {
        magma_tally2_ccopy( b.num_rows*b.num_cols, b.dval, b.num_cols, x->dval, b.num_cols );
        magma_tally2_c_solver_par solver_par;
        solver_par.maxiter = precond->maxiter;
        //magma_tally2_cjacobiiter_sys( precond->U, b, precond->d2, precond->work2, x, &solver_par, queue );
        CHECK( magma_tally2_cjacobispmvupdate(precond->maxiter, precond->U, precond->work2, b, precond->d2, x, queue ));
    }

    else if ( ( precond->solver == Magma_tally2_ICC ||
                precond->solver == Magma_tally2_AICC ) && precond->maxiter >= 50 ) {
        CHECK( magma_tally2_capplycumicc_r( b, x, precond, queue ));
    }
    else if ( ( precond->solver == Magma_tally2_ICC ||
               precond->solver == Magma_tally2_AICC ) && precond->maxiter < 50 ) {
        magma_tally2_ccopy( b.num_rows*b.num_cols, b.dval, b.num_cols, x->dval, b.num_cols );
        ...
        CHECK( magma_tally2_capplycustomprecond_r( b, x, precond, queue ));
    }
    else {
        printf( "error: preconditioner type not yet supported.\n" );
        magma_tally2blasSetKernelStream( orig_queue );
