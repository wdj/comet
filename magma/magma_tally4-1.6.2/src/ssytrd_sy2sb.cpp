    ...
    magma_tally4_int_t *info)
{
    ...
    *info = 0;
    ...
    float *dA;
    if (MAGMA_tally4_SUCCESS != magma_tally4_smalloc( &dA, (n + 2*nb)*ldda )) {
    ...
    if (upper) {
        ...
        if (1 <= n-nb) {
            ...
            magma_tally4_ssetmatrix_async( (n-nb), (n-nb),
                                    A(nb+1, nb+1),  lda,
        ...
        for (i = 1; i <= n-nb; i += nb) {
             ...
             if (i > 1 ) {
                 // spanel_to_q_tally4 copy the upper oof diagonal part of
                 ...
                 spanel_to_q_tally4(Magma_tally4Upper, pn-1, A(i, i+1), lda, work);

                 trace_gpu_start( 0, 1, "get", "get panel" );
                 //magma_tally4_queue_sync( stream[0] );
                 magma_tally4_queue_wait_event(stream[1], Pupdate_event);  //, 0);
                 magma_tally4_sgetmatrix_async( (pm+pn), pn,
                                         dA( i, i), ldda,
                                         A ( i, i), lda, stream[1] );
                 trace_gpu_end( 0, 1 );

                 trace_gpu_start( 0, 2, "her2k", "her2k" );
                 magma_tally4_ssyr2k(Magma_tally4Lower, Magma_tally4NoTrans, pm_old-pn_old, pn_old, c_neg_one,
                 ...
                 trace_cpu_end( 0 );
                 sq_to_panel_tally4(Magma_tally4Upper, pn-1, A(i, i+1), lda, work);
             }
             ...
             spanel_to_q_tally4(Magma_tally4Upper, pk, A(indi, indj), lda, work);
             trace_cpu_end( 0 );

             /* Send V from the CPU to the GPU */
             trace_gpu_start( 0, 0, "set", "set V and T" );
             magma_tally4_ssetmatrix_async( pm, pk,
                                     A(indi, indj),  lda,
                                     dA(indi, indj), ldda, stream[0] );

             /* Send the triangular factor T to the GPU */
             magma_tally4_ssetmatrix_async( pk, pk,
                                     hT,       nb,
                                     dT(i), lddt, stream[0] );
             trace_gpu_end( 0, 0 );
             ...
             // because below we made a restore sq_to_panel_tally4 and this restore need
             ...
             magma_tally4_sgemm(Magma_tally4NoTrans, Magma_tally4NoTrans, pm, pk, pk,
             ...
             magma_tally4_ssymm(Magma_tally4Left, uplo, pm, pk,
                         c_one, dA(indi, indi), ldda,
                         dwork, pm,
                         c_zero, dW, pm);
             trace_gpu_end( 0, 2 );
             /* restore the panel */
             sq_to_panel_tally4(Magma_tally4Upper, pk, A(indi, indj), lda, work);
             
             /* dwork = V*T already ==> dwork' = T'*V'
              * compute T'*V'*X ==> dwork'*W ==>
              * dwork + pm*nb = ((T' * V') * X) = dwork' * X = dwork' * W */
             trace_gpu_start( 0, 2, "gemm", "work = T'*V'*X" );
             magma_tally4_sgemm(Magma_tally4ConjTrans, Magma_tally4NoTrans, pk, pk, pm,
                         c_one, dwork, pm,
                         dW, pm,
             ...
             if (i + nb <= n-nb) {
                 ...
                 magma_tally4_sgemm(Magma_tally4NoTrans, Magma_tally4ConjTrans, pm, pn, pn, c_neg_one,
                             dA(indi, indj), ldda,
                             dW,                 pm, c_one,
                             dA(indi, indi), ldda);
                 trace_gpu_end( 0, 2 );
                 ...
                 magma_tally4_ssyr2k(Magma_tally4Lower, Magma_tally4NoTrans, pk, pk, c_neg_one,
                              dA(indi, indj), ldda,
                              dW,                 pm, d_one,
                              dA(indi, indi), ldda);
                 trace_gpu_end( 0, 2 );
        ...
        pk = min(pm,pn);
        if (1 <= n-nb) {
            spanel_to_q_tally4(Magma_tally4Upper, pk-1, A(n-pk+1, n-pk+2), lda, work);
            trace_gpu_start( 0, 2, "get", "get last block" );
            magma_tally4_sgetmatrix( pk, pk,
                              dA(n-pk+1, n-pk+1), ldda,
                              A(n-pk+1, n-pk+1),  lda );
            trace_gpu_end( 0, 2 );
            sq_to_panel_tally4(Magma_tally4Upper, pk-1, A(n-pk+1, n-pk+2), lda, work);
        }
    }// end of LOWER
