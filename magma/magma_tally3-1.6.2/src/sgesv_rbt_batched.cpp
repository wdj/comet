                  ...
                  magma_tally3_int_t batchCount, magma_tally3_queue_t queue)
{
    ...
    float *hu, *hv;
    if (MAGMA_tally3_SUCCESS != magma_tally3_smalloc_cpu( &hu, 2*n )) {
        info = MAGMA_tally3_ERR_HOST_ALLOC;
        return info;
    }

    if (MAGMA_tally3_SUCCESS != magma_tally3_smalloc_cpu( &hv, 2*n )) {
        info = MAGMA_tally3_ERR_HOST_ALLOC;
        return info;
    }
    ...
    info = magma_tally3_sgerbt_batched(Magma_tally3True, n, nrhs, dA_array, n, dB_array, n, hu, hv, &info, batchCount, queue);
    if (info != MAGMA_tally3_SUCCESS)  {
        return info;
    }


    info = magma_tally3_sgetrf_nopiv_batched( n, n, dA_array, ldda, info_array, batchCount, queue);
    if ( (info != MAGMA_tally3_SUCCESS) ){
        return info;
    }
    ...
    info = magma_tally3_sgetrs_nopiv_batched( Magma_tally3NoTrans, n, nrhs, dA_array, ldda, dB_array, lddb, info_array, batchCount, queue );


    /* The solution of A.x = b is Vy computed on the GPU */
    float *dv;

    if (MAGMA_tally3_SUCCESS != magma_tally3_smalloc( &dv, 2*n )) {
        info = MAGMA_tally3_ERR_DEVICE_ALLOC;
        return info;
    }

    magma_tally3_ssetvector(2*n, hv, 1, dv, 1);

    for(int i = 0; i < nrhs; i++)
        magma_tally3blas_sprbt_mv_batched(n, dv, dB_array+(i), batchCount, queue);
