...
// ----------------------------------------
// Convert MAGMA_tally3 constants to CUBLAS v1 constants, which are the same as lapack.
// These must be static to avoid conflict with CUBLAS v2 translators.
#define cublas_trans_const_tally3( magma_tally3_const ) lapacke_trans_const_tally3( magma_tally3_const )
#define cublas_side_const_tally3(  magma_tally3_const ) lapacke_side_const_tally3(  magma_tally3_const )
#define cublas_diag_const_tally3(  magma_tally3_const ) lapacke_diag_const_tally3(  magma_tally3_const )
#define cublas_uplo_const_tally3(  magma_tally3_const ) lapacke_uplo_const_tally3(  magma_tally3_const )
...
*/
extern "C"
magma_tally3DoubleComplex magma_tally3_zdotc(
    magma_tally3_int_t n,
    magma_tally3DoubleComplex_const_ptr dx, magma_tally3_int_t incx,
...
*/
extern "C"
magma_tally3DoubleComplex magma_tally3_zdotu(
    magma_tally3_int_t n,
    magma_tally3DoubleComplex_const_ptr dx, magma_tally3_int_t incx,
    magma_tally3DoubleComplex_const_ptr dy, magma_tally3_int_t incy )
{
...
magma_tally3_zrotm(
    magma_tally3_int_t n,
    double *dx, magma_tally3_int_t incx,
    double *dy, magma_tally3_int_t incy,
    const double *param )
{
...
/** Perform matrix-vector product.
        \f$ y = \alpha A   x + \beta y \f$  (transA == Magma_tally3NoTrans), or \n
        \f$ y = \alpha A^T x + \beta y \f$  (transA == Magma_tally3Trans),   or \n
        \f$ y = \alpha A^H x + \beta y \f$  (transA == Magma_tally3ConjTrans).
    ...
    @param[in]
    dx      COMPLEX_16 array on GPU device.
            If transA == Magma_tally3NoTrans, the n element vector x of dimension (1 + (n-1)*incx); \n
    ...
    @param[in,out]
    dy      COMPLEX_16 array on GPU device.
            If transA == Magma_tally3NoTrans, the m element vector y of dimension (1 + (m-1)*incy); \n
...
{
    cublasZgemv(
        cublas_trans_const_tally3( transA ),
        m, n,
        alpha, dA, ldda,
               dx, incx,
        beta,  dy, incy );
...
/** Perform triangular matrix-vector product.
        \f$ x = A   x \f$  (trans == Magma_tally3NoTrans), or \n
        \f$ x = A^T x \f$  (trans == Magma_tally3Trans),   or \n
        \f$ x = A^H x \f$  (trans == Magma_tally3ConjTrans).
    ...
    magma_tally3DoubleComplex_ptr       dx, magma_tally3_int_t incx )
{
    cublasZtrmv(
        cublas_uplo_const_tally3( uplo ),
        cublas_trans_const_tally3( trans ),
        cublas_diag_const_tally3( diag ),
        n,
        dA, ldda,
        dx, incx );
}
...
/** Solve triangular matrix-vector system (one right-hand side).
        \f$ A   x = b \f$  (trans == Magma_tally3NoTrans), or \n
        \f$ A^T x = b \f$  (trans == Magma_tally3Trans),   or \n
        \f$ A^H x = b \f$  (trans == Magma_tally3ConjTrans).
...
/** Perform matrix-matrix product, \f$ C = \alpha op(A) op(B) + \beta C \f$.
    ...
    @param[in]
    dA      COMPLEX_16 array on GPU device.
            If transA == Magma_tally3NoTrans, the m-by-k matrix A of dimension (ldda,k), ldda >= max(1,m); \n
    ...
    dB      COMPLEX_16 array on GPU device.
            If transB == Magma_tally3NoTrans, the k-by-n matrix B of dimension (lddb,n), lddb >= max(1,k); \n
    ...
    magma_tally3DoubleComplex_ptr       dC, magma_tally3_int_t lddc )
{
    cublasZgemm(
        cublas_trans_const_tally3( transA ),
        cublas_trans_const_tally3( transB ),
        m, n, k,
        alpha, dA, ldda,
               dB, lddb,
        beta,  dC, lddc );
...
/** Perform symmetric matrix-matrix product.
        \f$ C = \alpha A B + \beta C \f$ (side == Magma_tally3Left), or \n
        \f$ C = \alpha B A + \beta C \f$ (side == Magma_tally3Right),   \n
    ...
    @param[in]
    dA      COMPLEX_16 array on GPU device.
            If side == Magma_tally3Left, the m-by-m symmetric matrix A of dimension (ldda,m), ldda >= max(1,m); \n
...
/** Perform symmetric rank-k update.
        \f$ C = \alpha A A^T + \beta C \f$ (trans == Magma_tally3NoTrans), or \n
        \f$ C = \alpha A^T A + \beta C \f$ (trans == Magma_tally3Trans),      \n
    ...
    @param[in]
    k       Number of columns of A (for Magma_tally3NoTrans) or rows of A (for Magma_tally3Trans). k >= 0.
    ...
    @param[in]
    dA      COMPLEX_16 array on GPU device.
            If trans == Magma_tally3NoTrans, the n-by-k matrix A of dimension (ldda,k), ldda >= max(1,n); \n
...
/** Perform symmetric rank-2k update.
        \f$ C = \alpha A B^T + \alpha B A^T \beta C \f$ (trans == Magma_tally3NoTrans), or \n
        \f$ C = \alpha A^T B + \alpha B^T A \beta C \f$ (trans == Magma_tally3Trans),      \n
        where \f$ C \f$ is symmetric.
    ...
    @param[in]
    k       Number of columns of A and B (for Magma_tally3NoTrans) or rows of A and B (for Magma_tally3Trans). k >= 0.
    ...
    @param[in]
    dB      COMPLEX_16 array on GPU device.
            If trans == Magma_tally3NoTrans, the n-by-k matrix B of dimension (lddb,k), lddb >= max(1,n); \n
...
/** Perform Hermitian matrix-matrix product.
    ...
    @param[in]
    dA      COMPLEX_16 array on GPU device.
            If side == Magma_tally3Left, the m-by-m Hermitian matrix A of dimension (ldda,m), ldda >= max(1,m); \n
...
/** Perform triangular matrix-matrix product.
        \f$ B = \alpha op(A) B \f$ (side == Magma_tally3Left), or \n
        \f$ B = \alpha B op(A) \f$ (side == Magma_tally3Right),   \n
    ...
    @param[in]
    dA      COMPLEX_16 array on GPU device.
            If side == Magma_tally3Left, the n-by-n triangular matrix A of dimension (ldda,n), ldda >= max(1,n); \n
...
/** Solve triangular matrix-matrix system (multiple right-hand sides).
        \f$ op(A) X = \alpha B \f$ (side == Magma_tally3Left), or \n
        \f$ X op(A) = \alpha B \f$ (side == Magma_tally3Right),   \n
        where \f$ A \f$ is triangular.
    ...
    @param[in]
    dA      COMPLEX_16 array on GPU device.
            If side == Magma_tally3Left, the m-by-m triangular matrix A of dimension (ldda,m), ldda >= max(1,m); \n
