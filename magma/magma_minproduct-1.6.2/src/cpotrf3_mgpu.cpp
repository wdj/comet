/*
    -- MAGMA_minproduct (version 1.6.1) --
       ...
       @generated from zpotrf3_mgpu.cpp normal z -> c, Fri Jan 30 19:00:15 2015
...
#define dlPT(id, i, j, k) (d_lP[(id)] + (k)*nb*lddp + (j)*nb   + (i))
    ...
    magma_minproduct_int_t     j, jb, nb0, nb2, d, dd, id, j_local, j_local2, buf;
    ...
    magma_minproductFloatComplex *dlpanel;
    magma_minproduct_int_t n_local[Magma_minproductMaxGPUs], ldpanel;
    const magma_minproduct_int_t stream1 = 0, stream2 = 1, stream3 = 2;
    ...
    *info = 0;
    ...
    magma_minproductFloatComplex *d_dinvA[Magma_minproductMaxGPUs];
    magma_minproductFloatComplex *d_x[Magma_minproductMaxGPUs];
    ...
    for( d=0; d < ngpu; d++ ) {
        ...
        if ( (MAGMA_minproduct_SUCCESS != magma_minproduct_cmalloc( &d_dinvA[d], 2*trsm_nb*trsm_n )) ||
             (MAGMA_minproduct_SUCCESS != magma_minproduct_cmalloc( &d_x[d],     2*nb*(upper ? n : m) )) ) {
    ...
    if (upper) {
        ...
        for (j=0; j < m; j += nb) {
            ...
            if ( j > 0 ) {
                magma_minproductblasSetKernelStream( queues[id][stream1] );
                trace_gpu_start( id, stream1, "syrk", "syrk" );
                magma_minproduct_cherk(Magma_minproductUpper, Magma_minproductConjTrans, jb, j,
            ...
            if ( j > 0 ) {
                ...
                for( dd=0; dd < ngpu; dd++ ) {
                    ...
                    if ( n_local[d] > nb0 ) {
                        magma_minproduct_setdevice(d);
                        magma_minproductblasSetKernelStream( queues[d][stream2] );
                        if ( d == id ) {
                            dlpanel = dlA(d,0,nb*j_local);
                            ldpanel = ldda;
                            // the GPU owns the row from start, and no need of synch.
                            //magma_minproduct_queue_wait_event( queues[d][stream2], events[d][0] ); // rows arrived at gpu
                            magma_minproduct_queue_wait_event( queues[d][stream2], events[d][4] ); // wait for look-ahead trsm to finish
                        } else {
                        ...
                        magma_minproduct_cgemm(Magma_minproductConjTrans, Magma_minproductNoTrans,
                                    jb, n_local[d]-nb0, j,
                                    c_neg_one, dlpanel,        ldpanel,
                                               dlA(d, 0, nb0), ldda,
                                    c_one,     dlA(d, j, nb0), ldda);
                        trace_gpu_end( d, stream2 );
                        magma_minproduct_event_record( events[d][2], queues[d][stream2] );
                    }
                    d = (d+1)%ngpu;
                }
            }
            ...
            magma_minproduct_setdevice(id);
            magma_minproduct_queue_sync( queues[id][stream1] );
            trace_cpu_start( 0, "getrf", "getrf" );
            lapackf77_cpotrf(Magma_minproductUpperStr, &jb, Aup(j,j), &lda, info);
            trace_cpu_end( 0 );
            if (*info != 0) {
                *info = *info + j;
                break;
            ...
            if ( (j+jb) < n) {
                ...
                for( dd=0; dd < ngpu; dd++ ) {
                    ...
                    magma_minproduct_csetmatrix_async( jb, jb,
                                            Aup(j,j), lda,
                                            dlpanel,  ldpanel,
                                            queues[d][stream1] );
                    trace_gpu_end( d, stream1 );
                    magma_minproduct_event_record( events[d][1], queues[d][stream1] );
                    d = (d+1)%ngpu;
                }
            } else {
                magma_minproduct_setdevice(id);
            ...
            if ( (j+jb) < n) {
                ...
                for( dd=0; dd < ngpu; dd++ ) {
                    ...
                    if ( j+jb < m && d == (j/nb+1)%ngpu ) {
                        ...
                        nb0 = min(nb, nb2);
                        magma_minproductblasSetKernelStream( queues[d][stream1] );
                        magma_minproduct_queue_wait_event( queues[d][stream1], events[d][2] ); // wait for gemm update
                        trace_gpu_start( d, stream1, "trsm", "trsm" );
#if (defined(PRECISION_d) || defined(PRECISION_s)) && defined(CTRSM_WORK)
                        magma_minproductblas_claset( Magma_minproductFull, trsm_nb, trsm_n, c_zero, c_zero, dinvA(d,0), trsm_nb );
                        magma_minproductblas_claset( Magma_minproductFull, nb0,     jb,     c_zero, c_zero, dx(d,0), nb0 );
                        magma_minproductblas_ctrsm_work( Magma_minproductLeft, Magma_minproductUpper,
                                              Magma_minproductConjTrans, Magma_minproductNonUnit,
                                              jb, nb0, c_one,
                                              dlpanel, ldpanel,
                                              dlA(d, j, nb*j_local2), ldda,
                                              1, dinvA(d,0), dx(d,0) );
#else
                        magma_minproduct_ctrsm( Magma_minproductLeft, Magma_minproductUpper,
                                     Magma_minproductConjTrans, Magma_minproductNonUnit,
                                     jb, nb0, c_one,
                                     dlpanel,                ldpanel,
                                     dlA(d, j, nb*j_local2), ldda);
#endif
                        magma_minproduct_event_record( events[d][4], queues[d][stream1] );
                        trace_gpu_end( d, stream1 );
                    } else if ( nb2 > 0 ) {
                        /* update all the blocks on stream2 */
                        magma_minproduct_queue_wait_event( queues[d][stream2], events[d][1] ); // wait for cholesky factor
                        trace_gpu_start( d, stream2, "trsm", "trsm" );
                        magma_minproductblasSetKernelStream( queues[d][stream2] );
#if (defined(PRECISION_d) || defined(PRECISION_s)) && defined(CTRSM_WORK)
                        magma_minproductblas_claset( Magma_minproductFull, trsm_nb, trsm_n, c_zero, c_zero, dinvA(d,0), trsm_nb );
                        magma_minproductblas_claset( Magma_minproductFull, nb2,     jb,     c_zero, c_zero, dx(d,0), nb2 );
                        magma_minproductblas_ctrsm_work( Magma_minproductLeft, Magma_minproductUpper,
                                              Magma_minproductConjTrans, Magma_minproductNonUnit,
                                              jb, nb2, c_one,
                                              dlpanel, ldpanel,
                ...
                if ( j+jb < m ) {
                    ...
                    magma_minproduct_setdevice(d);
                    /* lookahead done */
                    magma_minproduct_queue_wait_event( queues[d][stream3], events[d][4] );
                
                    trace_gpu_start( d, stream3, "comm", "row to CPU" );
                    magma_minproduct_cgetmatrix_async( (j+jb), nb0,
                                            dlA(d, 0, nb*j_local2), ldda,
                                            Aup(0,j+jb),            lda,
                                            queues[d][stream3] );
                    trace_gpu_end( d, stream3 );
                    magma_minproduct_event_record( events[d][3], queues[d][stream3] );
                    /* needed on pluto */
                    //magma_minproduct_queue_sync( queues[d][stream3] );
                
                    /* broadcast rows to gpus on stream2 */
                    buf2 = ((j+jb)/nb)%ngpu;
                    for( d2=0; d2 < ngpu; d2++ ) {
                        if ( d2 != d ) {
                            magma_minproduct_setdevice(d2);
                            trace_gpu_start( d2, stream3, "comm", "row to GPUs" );
                            magma_minproduct_queue_wait_event( queues[d2][stream3], events[d][3] ); // rows arrived at cpu on stream3
                            magma_minproduct_csetmatrix_async( j+jb, nb0,
                                                    Aup(0,j+jb),       lda,
                                                    dlP(d2,nb,0,buf2), lddp,
                                                    queues[d2][stream3] );
                            trace_gpu_end( d2, stream3 );
                            magma_minproduct_event_record( events[d2][0], queues[d2][stream3] );
                        }
                    }
                    ...
                    if ( nb2 > 0 ) {
                        ...
                        int flag = 0;
                        if (flag == 0) {
                            magma_minproduct_queue_wait_event( queues[d][stream2], events[d][4] ); // lookahead -> diagonal inversion
                        } else {
                            magma_minproductblas_claset( Magma_minproductFull, trsm_nb, trsm_n, c_zero, c_zero, dinvA(d,flag), trsm_nb );
                            magma_minproduct_queue_wait_event( queues[d][stream2], events[d][1] ); // panel received
                        }
                        magma_minproductblas_claset( Magma_minproductFull, nb2, jb, c_zero, c_zero, dx(d,1), nb2 );
                        magma_minproductblas_ctrsm_work( Magma_minproductLeft, Magma_minproductUpper, Magma_minproductConjTrans, Magma_minproductNonUnit,
                                              jb, nb2, c_one,
                                              dlpanel, ldpanel,
        ...
        for (j=0; j < n; j += nb) {
            ...
            if ( j > 0 ) {
                magma_minproductblasSetKernelStream( queues[id][stream1] );
                magma_minproduct_cherk(Magma_minproductLower, Magma_minproductNoTrans, jb, j,
            ...
            if ( j > 0 ) {
                ...
                for( dd=0; dd < ngpu; dd++ ) {
                    ...
                    if ( nb0 < n_local[d] ) {
                        ...
                        magma_minproduct_cgemm( Magma_minproductNoTrans, Magma_minproductConjTrans,
                                     n_local[d]-nb0, jb, j,
            ...
            magma_minproduct_setdevice(id);
            magma_minproduct_queue_sync( queues[id][stream1] );
            lapackf77_cpotrf(Magma_minproductLowerStr, &jb, Alo(j,j), &lda, info);
            if (*info != 0) {
                *info = *info + j;
                break;
            }
            ...
            if ( (j+jb) < m) {
                ...
                for( dd=0; dd < ngpu; dd++ ) {
                    ...
                    if ( j+nb < n && d == (j/nb+1)%ngpu ) { /* owns next column, look-ahead next block on stream1 */
                        if ( j > 0 ) magma_minproduct_queue_wait_event( queues[d][stream1], events[d][2] ); // wait for gemm update
                        magma_minproductblasSetKernelStream( queues[d][stream1] );
#if (defined(PRECISION_d) || defined(PRECISION_s)) && defined(CTRSM_WORK)
                        magma_minproductblas_claset( Magma_minproductFull, trsm_nb, trsm_n, c_zero, c_zero, dinvA(d,0), trsm_nb );
                        magma_minproductblas_claset( Magma_minproductFull, nb0,     jb,     c_zero, c_zero, dx(d,0), nb0 );
                        magma_minproductblas_ctrsm_work( Magma_minproductRight, Magma_minproductLower,
                                              Magma_minproductConjTrans, Magma_minproductNonUnit,
                                              nb0, jb, c_one,
                                              dlpanel, ldpanel,
                        ...
                        magma_minproduct_queue_wait_event( queues[d][stream2], events[d][1] ); // wait for the cholesky factor
                        magma_minproductblasSetKernelStream( queues[d][stream2] );
#if (defined(PRECISION_d) || defined(PRECISION_s)) && defined(CTRSM_WORK)
                        magma_minproductblas_claset( Magma_minproductFull, trsm_nb, trsm_n, c_zero, c_zero, dinvA(d,0), trsm_nb );
                        magma_minproductblas_claset( Magma_minproductFull, nb2,     jb,     c_zero, c_zero, dx(d,0), nb2 );
                        magma_minproductblas_ctrsm_work( Magma_minproductRight, Magma_minproductLower, Magma_minproductConjTrans, Magma_minproductNonUnit,
                                              nb2, jb, c_one,
                                              dlpanel,                ldpanel,
                                              dlA(d, nb*j_local2, j), ldda,
                                              1, dinvA(d,0), dx(d,0) );
...
//#define CPOTRF_DEVICE_TO_DEVICE
                    ...
                    for( d2=0; d2 < ngpu; d2++ ) {
                        magma_minproduct_setdevice(d2);
                        magma_minproduct_queue_wait_event( queues[d2][stream3], events[d][4] );
                        if ( d2 != d ) {
                            magma_minproduct_ccopymatrix_async( nb0, j+jb,
                                                     dlPT(d2,0,nb,buf2), nb, // first nbxnb reserved for diagonal block
                                                     dlA(d, nb*j_local2, 0), ldda,
                                                     queues[d2][stream3] );
                            magma_minproduct_event_record( events[d2][0], queues[d2][stream3] );
                        } else {
                            magma_minproduct_cgetmatrix_async( nb0, j+jb,
                                                    dlA(d, nb*j_local2, 0), ldda,
                                                    Alo(j+jb,0),            lda,
                                                    queues[d][stream3] );
                        }
                    ...
                    for( d2=0; d2 < ngpu; d2++ ) {
                        if ( d2 != d ) {
                            magma_minproduct_setdevice(d2);
                            magma_minproduct_queue_wait_event( queues[d2][stream3], events[d][3] ); // getmatrix done
                            magma_minproduct_csetmatrix_async( nb0, j+jb,
                                                    Alo(j+jb,0),        lda,
                                                    dlPT(d2,0,nb,buf2), nb, // first nbxnb reserved for diagonal block
                                                    queues[d2][stream3] );
                            magma_minproduct_event_record( events[d2][0], queues[d2][stream3] );
    ...
    for( d=0; d < ngpu; d++ ) {
        magma_minproduct_setdevice(d);
        for( j=0; j < 3; j++ ) {
            magma_minproduct_queue_sync( queues[d][j] );
        }
#if (defined(PRECISION_d) || defined(PRECISION_s)) && defined(CTRSM_WORK)
        magma_minproduct_free( d_dinvA[d] );
        magma_minproduct_free( d_x[d] );
#endif
    }
    ...
    magma_minproduct_int_t *info)
{
    ...
    if (uplo == Magma_minproductUpper) {
        magma_minproduct_int_t j, jj, jb, mj;
        ...
        for (j=off_j; j < n; j += nb) {
            ...
            magma_minproduct_csetmatrix_async( mj, jb,
                                    A(off_i, j),     lda,
                                    dA(k, 0, jj*nb), ldda,
                                    queues[k][0] );
        }
        ...
        magma_minproduct_int_t i, ii, ib, ni;
        ...
        for (i=off_i; i < m; i += nb) {
            ...
            magma_minproduct_setdevice(k);
            magma_minproduct_csetmatrix_async( ib, ni,
                                    A(i, off_j),     lda,
                                    dA(k, ii*nb, 0), ldda,
                                    queues[k][0] );
        }
    ...
    for( k=0; k < ngpu; k++ ) {
        magma_minproduct_setdevice(k);
        magma_minproduct_queue_sync( queues[k][0] );
    }
    magma_minproduct_setdevice( orig_dev );
    ...
    magma_minproduct_int_t *info)
{
    ...
    if (uplo == Magma_minproductUpper) {
        ...
        for (j=off_j+NB; j < n; j += nb) {
            ...
            magma_minproduct_setdevice(k);
            magma_minproduct_cgetmatrix_async( mj, jb,
                                    dA(k, 0, jj*nb), ldda,
                                    A(off_i, j),     lda,
                                    queues[k][0] );
            magma_minproduct_queue_sync( queues[k][0] );
        ...
        for (i=off_i+NB; i < m; i += nb) {
            ...
            magma_minproduct_setdevice(k);
            magma_minproduct_cgetmatrix_async( ib, ni,
                                    dA(k, ii*nb, 0), ldda,
                                    A(i, off_j),     lda,
                                    queues[k][0] );
            magma_minproduct_queue_sync( queues[k][0] );
