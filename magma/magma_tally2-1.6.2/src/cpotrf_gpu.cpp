...
#define dA(i, j) (dA + (j)*ldda + (i))
    ...
    *info = 0;
    ...
    if ((nb <= 1) || (nb >= n)) {
        /* Use unblocked code. */
        magma_tally2_cgetmatrix_async( n, n, dA, ldda, work, n, stream[1] );
        magma_tally2_queue_sync( stream[1] );
        lapackf77_cpotrf(uplo_, &n, work, &n, info);
        magma_tally2_csetmatrix_async( n, n, work, n, dA, ldda, stream[1] );
    }
    else {
        /* Use blocked code. */
        if (upper) {
            ...
            for (j=0; j < n; j += nb) {
                ...
                lapackf77_cpotrf(Magma_tally2UpperStr, &jb, work, &jb, info);
            ...
            for (j=0; j < n; j += nb) {
                ...
                magma_tally2_queue_sync( stream[0] );
                lapackf77_cpotrf(Magma_tally2LowerStr, &jb, work, &jb, info);
                magma_tally2_csetmatrix_async( jb, jb,
                                        work,     jb,
                                        dA(j, j), ldda, stream[1] );
                if (*info != 0) {
