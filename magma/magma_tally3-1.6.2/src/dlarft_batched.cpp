...
magma_tally3_dlarft_sm32x32_batched(magma_tally3_int_t n, magma_tally3_int_t k, double **v_array, magma_tally3_int_t ldv,
                    double **tau_array, double **T_array, magma_tally3_int_t ldt, magma_tally3_int_t batchCount, cublasHandle_t myhandle, magma_tally3_queue_t queue)
{
...
#define use_gemm_larft_sm32
    ...
    //magma_tally3blas_dgemm_batched( Magma_tally3ConjTrans, Magma_tally3NoTrans, k, k, n, MAGMA_tally3_D_ONE, v_array, ldv, v_array, ldv, MAGMA_tally3_D_ZERO, T_array, ldt, batchCount, queue);
    ...
    magma_tally3blas_dlaset_batched(Magma_tally3Lower, k, k, MAGMA_tally3_D_ZERO, MAGMA_tally3_D_ZERO, T_array, ldt, batchCount, queue);
#else
    #if 1
    for(int i=0; i<k; i++)
    {
        ...
        #ifdef USE_GEMV2
        magma_tally3blas_dlarft_gemvrowwise_batched( n-i, i, 
                            tau_array,
                            v_array, ldv, 
                            T_array, ldt,
                            batchCount, queue);
        ...
        #else       
        magma_tally3blas_dlarft_gemvcolwise_batched( n-i, i, v_array, ldv, T_array, ldt, tau_array, batchCount, queue);
        #endif
    }
    #else
     ...
     magma_tally3blas_dlarft_dtrmv_sm32x32_batched(k, k, tau_array, T_array, ldt, T_array, ldt, batchCount, queue);
}
...
magma_tally3_dlarft_batched(magma_tally3_int_t n, magma_tally3_int_t k, magma_tally3_int_t stair_T, 
                ...
                double **work_array, magma_tally3_int_t lwork, magma_tally3_int_t batchCount, cublasHandle_t myhandle, 
                magma_tally3_queue_t queue)
{
    ...
    if(k > nb)
    {
        magma_tally3_ddisplace_pointers(dTstep_array, work_array, lwork, 0, 0, batchCount, queue);
    }
    else
    {
        magma_tally3_ddisplace_pointers(dTstep_array, T_array, ldt, 0, 0, batchCount, queue);
    }
...
#ifdef RFT_MAG_GEM
    magma_tally3blas_dgemm_batched( Magma_tally3ConjTrans, Magma_tally3NoTrans, 
            k, k, n, 
            one,  v_array, ldv, 
                  v_array, ldv, 
    ...
    magma_tally3blas_dlaset_batched(Magma_tally3Lower, k, k, MAGMA_tally3_D_ZERO, MAGMA_tally3_D_ZERO, dTstep_array, ldtstep, batchCount, queue);
    // no need for it as T is expected to be lower zero
    //if(k > nb) magma_tally3blas_dlaset_batched(Magma_tally3Lower, k, k, MAGMA_tally3_D_ZERO, MAGMA_tally3_D_ZERO, dTstep_array, ldtstep, batchCount);
    ...
    for(j=0; j<k; j+=nb)
    {
        ...
        if(prev_n>0 && mycol>0){
            ...
            magma_tally3_ddisplace_pointers(dW1_displ, dTstep_array, ldtstep, 0, j, batchCount, queue);
            magma_tally3_ddisplace_pointers(dW2_displ, T_array,     ldt, 0, j, batchCount, queue);
#ifdef RFT_MAG_GEM
            magma_tally3blas_dgemm_batched( Magma_tally3NoTrans, Magma_tally3NoTrans, 
                    prev_n, mycol, prev_n, 
                    one,  T_array, ldt, 
                          dW1_displ, ldtstep, 
                    zero, dW2_displ, ldt, 
            ...
            magma_tally3_ddisplace_pointers(dW1_displ, dTstep_array, ldtstep, j, j, batchCount, queue);
            magma_tally3_ddisplace_pointers(dW3_displ, tau_array,  1, j, 0, batchCount, queue);
            ...
            for(i=0; i<prev_n; i+=nb)
            {
                ...
                if(rows>0 && mycol>0)
                {
                    magma_tally3_ddisplace_pointers(dW2_displ, T_array,     ldt, i, j, batchCount, queue);
                    magma_tally3blas_dlarft_recdtrmv_sm32x32_batched(rows, mycol, dW3_displ, dW2_displ, ldt, dW1_displ, ldtstep, batchCount, queue);
                }
            }
        }
        ...
        if(stair_T == 0){
            ...
            if(mycol>0)
            {
                magma_tally3_ddisplace_pointers(dW1_displ, dTstep_array, ldtstep, j, j, batchCount, queue);
                magma_tally3_ddisplace_pointers(dW3_displ, tau_array,  1, j, 0, batchCount, queue);
                magma_tally3_ddisplace_pointers(dW2_displ, T_array,     ldt, j, j, batchCount, queue);
                magma_tally3blas_dlarft_dtrmv_sm32x32_batched(mycol, mycol, dW3_displ, dW1_displ, ldtstep, dW2_displ, ldt, batchCount, queue);
