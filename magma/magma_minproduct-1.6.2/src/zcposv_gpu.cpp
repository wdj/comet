    ...
    magma_minproduct_int_t *info)
{
    ...
    *info = 0;
    ...
    Anrm = magma_minproductblas_zlanhe(Magma_minproductInfNorm, uplo, n, dA, ldda, (double*)dworkd );
    cte  = Anrm * eps * pow((double)n, 0.5) * BWDMAX;

    /*
     * Convert to single precision
     */
    magma_minproductblas_zlag2c( n, nrhs, dB, lddb, dSX, lddsx, info );
    if (*info != 0) {
        *iter = -2;
        goto FALLBACK;
    }

    magma_minproductblas_zlat2c( uplo, n, dA, ldda, dSA, lddsa, info );
    if (*info != 0) {
        *iter = -2;
        goto FALLBACK;
    }
    ...
    magma_minproduct_cpotrf_gpu( uplo, n, dSA, lddsa, info );
    if (*info != 0) {
        *iter = -3;
        goto FALLBACK;
    }
    ...
    magma_minproduct_cpotrs_gpu( uplo, n, nrhs, dSA, lddsa, dSX, lddsx, info );
    ...
    if ( nrhs == 1 ) {
        magma_minproduct_zhemv( uplo, n,
                     c_neg_one, dA, ldda,
                                dX, 1,
                     c_one,     dR, 1 );
    }
    else {
        magma_minproduct_zhemm( Magma_minproductLeft, uplo, n, nrhs,
                     c_neg_one, dA, ldda,
                                dX, lddx,
                     c_one,     dR, lddr );
    }
...
REFINEMENT:
    for( iiter=1; iiter < ITERMAX; ) {
        *info = 0;
        ...
        magma_minproductblas_zlag2c( n, nrhs, dR, lddr, dSX, lddsx, info );
        if (*info != 0) {
            *iter = -2;
            goto FALLBACK;
        }
...
FALLBACK:
    ...
    if (*info == 0) {
        ...
        magma_minproduct_zpotrs_gpu( uplo, n, nrhs, dA, ldda, dX, lddx, info );
    }
