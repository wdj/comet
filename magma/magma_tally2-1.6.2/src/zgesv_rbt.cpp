...
/**
    ...
    @ingroup magma_tally2_zgesv_driver
    ********************************************************************/
extern "C" magma_tally2_int_t
magma_tally2_zgerfs_nopiv_gpu(
    ...
    magma_tally2DoubleComplex_ptr dworkd, magma_tally2DoubleComplex_ptr dAF,
    ...
    magma_tally2_int_t *info)
{
    ...
    *info = 0;
    ...
    Anrm = magma_tally2blas_zlange(Magma_tally2InfNorm, n, n, dA, ldda, (double*)dworkd );
    cte  = Anrm * eps * pow( (double)n, (double)0.5 ) * BWDMAX;
    
    // residual dR = dB - dA*dX in double precision
    magma_tally2blas_zlacpy( Magma_tally2UpperLower, n, nrhs, dB, lddb, dR, lddr );
    if ( nrhs == 1 ) {
        magma_tally2_zgemv( trans, n, n,
                     c_neg_one, dA, ldda,
                                dX, 1,
                     c_one,     dR, 1 );
    }
    else {
        magma_tally2_zgemm( trans, Magma_tally2NoTrans, n, nrhs, n,
                     c_neg_one, dA, ldda,
                                dX, lddx,
                     c_one,     dR, lddr );
    }
    ...
    for( j=0; j < nrhs; j++ ) {
        ...
        i = magma_tally2_izamax ( n, dR(0,j), 1 ) - 1;
...
REFINEMENT:
    for( iiter=1; iiter < ITERMAX; ) {
        *info = 0;
        ...
        magma_tally2_zgetrs_nopiv_gpu( trans, n, nrhs, dAF, lddsa, dR, lddr, info );
        if (*info != 0) {
            *iter = -3;
            goto FALLBACK;
        }
        ...
        for( j=0; j < nrhs; j++ ) {
            magma_tally2blas_zaxpycp2( n, dR(0,j), dX(0,j), dB(0,j) );
        }
    ...
    magma_tally2_int_t *info)
{
    ...
    *info = 0;
    ...
    magma_tally2DoubleComplex *dA, *hu, *hv, *db, *dAo, *dBo, *dwork;
    magma_tally2_int_t n2;

    magma_tally2_int_t iter;
    n2 = nn*nn;

    if (MAGMA_tally2_SUCCESS != magma_tally2_zmalloc( &dA, n2 )) {
        *info = MAGMA_tally2_ERR_DEVICE_ALLOC;
        return *info;
    }
    if (MAGMA_tally2_SUCCESS != magma_tally2_zmalloc( &db, nn*nrhs )) {
        *info = MAGMA_tally2_ERR_DEVICE_ALLOC;
        return *info;
    }

    if (ref == Magma_tally2True) {
        if (MAGMA_tally2_SUCCESS != magma_tally2_zmalloc( &dAo, n2 )) {
            *info = MAGMA_tally2_ERR_DEVICE_ALLOC;
            return *info;
        }
        if (MAGMA_tally2_SUCCESS != magma_tally2_zmalloc( &dwork, nn*nrhs )) {
            *info = MAGMA_tally2_ERR_DEVICE_ALLOC;
            return *info;
        }
        if (MAGMA_tally2_SUCCESS != magma_tally2_zmalloc( &dBo, nn*nrhs )) {
            *info = MAGMA_tally2_ERR_DEVICE_ALLOC;
            return *info;
        }
    }

    if (MAGMA_tally2_SUCCESS != magma_tally2_zmalloc_cpu( &hu, 2*nn )) {
        *info = MAGMA_tally2_ERR_HOST_ALLOC;
        return *info;
    }
    if (MAGMA_tally2_SUCCESS != magma_tally2_zmalloc_cpu( &hv, 2*nn )) {
        *info = MAGMA_tally2_ERR_HOST_ALLOC;
        return *info;
    }

    magma_tally2blas_zlaset(Magma_tally2Full, nn, nn, MAGMA_tally2_Z_ZERO, MAGMA_tally2_Z_ONE, dA, nn);

    /* Send matrix on the GPU*/
    magma_tally2_zsetmatrix(n, n, A, lda, dA, nn);

    /* Send b on the GPU*/
    magma_tally2_zsetmatrix(n, nrhs, B, ldb, db, nn);

    *info = magma_tally2_zgerbt_gpu(Magma_tally2True, nn, nrhs, dA, nn, db, nn, hu, hv, info);
    if (*info != MAGMA_tally2_SUCCESS)  {
        return *info;
    }

    if (ref == Magma_tally2True) {
        magma_tally2_zcopymatrix(nn, nn, dA, nn, dAo, nn);
        magma_tally2_zcopymatrix(nn, nrhs, db, nn, dBo, nn);
    }
    /* Solve the system U^TAV.y = U^T.b on the GPU*/ 
    magma_tally2_zgesv_nopiv_gpu( nn, nrhs, dA, nn, db, nn, info);


    /* Iterative refinement */
    if (ref == Magma_tally2True) {
        magma_tally2_zgerfs_nopiv_gpu(Magma_tally2NoTrans, nn, nrhs, dAo, nn, dBo, nn, db, nn, dwork, dA, &iter, info);
    }
    //printf("iter = %d\n", iter);

    /* The solution of A.x = b is Vy computed on the GPU */
    magma_tally2DoubleComplex *dv;

    if (MAGMA_tally2_SUCCESS != magma_tally2_zmalloc( &dv, 2*nn )) {
        *info = MAGMA_tally2_ERR_DEVICE_ALLOC;
        return *info;
    }

    magma_tally2_zsetvector(2*nn, hv, 1, dv, 1);
    
    for(int i = 0; i < nrhs; i++) {
        magma_tally2blas_zprbt_mv(nn, dv, db+(i*nn));
    }

    magma_tally2_zgetmatrix(n, nrhs, db, nn, B, ldb);
