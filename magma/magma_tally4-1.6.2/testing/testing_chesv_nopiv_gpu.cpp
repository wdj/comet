...
int main(int argc, char **argv)
{
    ...
    magma_tally4FloatComplex *h_A, *h_B, *h_X, temp, *hwork;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            bool nopiv = true;
            if ( nopiv ) {
                magma_tally4_cmake_hpd( N, h_A, lda );  // SPD / HPD does not require pivoting
            }
            else {
                magma_tally4_cmake_hermitian( N, h_A, lda );  // symmetric/Hermitian generally requires pivoting
            }
            
            magma_tally4_csetmatrix( N, N,    h_A, lda, d_A, ldda );
            magma_tally4_csetmatrix( N, nrhs, h_B, ldb, d_B, lddb );
            ...
            magma_tally4_chesv_nopiv_gpu( opts.uplo, N, nrhs, d_A, ldda, d_B, lddb, &info );
            gpu_time = magma_tally4_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally4_cgesv_gpu returned error %d: %s.\n",
            ...
            if ( opts.lapack ) {
                lwork = -1;
                lapackf77_chesv( lapack_uplo_const_tally4(opts.uplo), &N,&nrhs,
                                 h_A, &lda, ipiv, h_B, &ldb, &temp, &lwork, &info );
                lwork = (magma_tally4_int_t) MAGMA_tally4_C_REAL( temp );
                TESTING_MALLOC_PIN( hwork, magma_tally4FloatComplex, lwork );
