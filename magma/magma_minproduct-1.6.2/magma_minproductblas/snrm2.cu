...
__global__ void
magma_minproductblas_snrm2_kernel( int m, float *dA, int ldda, float *dxnorm )
{
...
//==============================================================================
__global__ void
magma_minproductblas_snrm2_check_kernel( int m, float *dA, int ldda, float *dxnorm, 
                               float *lsticc )
{
    const int tx = threadIdx.x;
    float *dx = dA + blockIdx.x * ldda;
    ...
    magma_minproductFloat_ptr dlsticc) 
{
    ...
    magma_minproductblas_snrm2_check_kernel<<< blocks, threads >>>( m, dA, ldda, dxnorm, dlsticc );
}


//==============================================================================
__global__ void
magma_minproductblas_snrm2_smkernel( int m, int n, float *dA, int ldda,
                           float *dxnorm )
{
    const int tx = threadIdx.x;
    ...
    float *dxnorm)
{
    ...
    magma_minproductblas_snrm2_smkernel<<< blocks, threads, 0, magma_minproduct_stream >>>( m, n, dA, ldda, dxnorm );
}

//==============================================================================
extern "C"
__global__ void
magma_minproduct_snrm2_adjust_kernel(float *xnorm, float *c)
{
    const int tx = threadIdx.x;
    ...
    temp = MAGMA_minproduct_S_ABS( c[tx] ) / xnorm[0];
    sum[tx] = -temp * temp;
...
*/
extern "C" void
magma_minproductblas_snrm2_adjust(magma_minproduct_int_t k, magma_minproductFloat_ptr dxnorm, magma_minproductFloat_ptr dc)
{
    magma_minproduct_snrm2_adjust_kernel<<< 1, k, 0, magma_minproduct_stream >>> (dxnorm, dc);
}
...
__global__ void
magma_minproduct_snrm2_row_check_adjust_kernel(
    ...
    float *C, int ldc, float *lsticc)
{
    ...
    if (tx < n) {
        float temp = MAGMA_minproduct_S_ABS( C[tx*ldc] ) / xnorm[tx];
    ...
    magma_minproductFloat_ptr dlsticc)
{
    ...
    magma_minproduct_snrm2_row_check_adjust_kernel<<< nblocks, BS >>> (k, tol, dxnorm, dxnorm2, dC, lddc, dlsticc);
}
    ...
    magma_minproductFloat_ptr dxnorm) 
{
    ...
    magma_minproductblas_snrm2_kernel<<< blocks, threads, 0, magma_minproduct_stream >>>( m, dA, ldda, dxnorm );

    // The following would do the computation on one SM
    // magma_minproductblas_snrm2_sm(m, n, dA, ldda, dxnorm);
}
