...
magma_tally2_ziteric_csr_kernel(    
    ...
    {     
        ...
        // modify entry
        if (i == j)
            val[il-1] = MAGMA_tally2_Z_MAKE(sqrt(abs(MAGMA_tally2_Z_REAL(s))), 0.0);
        else
            val[il-1] =  s / val[iu-1];
...
magma_tally2_ziteric_csr( 
    ...
    magma_tally2_ziteric_csr_kernel<<< grid, block, 0, magma_tally2_stream >>>
            ( A.num_rows, A.nnz, 
