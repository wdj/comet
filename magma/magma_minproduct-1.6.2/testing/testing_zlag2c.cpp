...
int main( int argc, char** argv )
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            gbytes = (real_Double_t) m*n * (sizeof(magma_minproductDoubleComplex) + sizeof(magma_minproductFloatComplex)) / 1e9;
            ...
            magma_minproduct_csetmatrix( m, n, SA, lda, dSA, ldda );
            ...
            /* ====================================================================
               Performs operation using MAGMA_minproduct zlag2c
               =================================================================== */            
            gpu_time = magma_minproduct_sync_wtime(0);
            magma_minproductblas_zlag2c( m, n, dA, ldda, dSA, ldda, &info );
            gpu_time = magma_minproduct_sync_wtime(0) - gpu_time;
            gpu_perf = gbytes / gpu_time;
            if (info != 0)
                printf("magma_minproductblas_zlag2c returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
            
            magma_minproduct_cgetmatrix( m, n, dSA, ldda, SR, lda );
            ...
            /* ====================================================================
               Performs operation using MAGMA_minproduct clag2z
               =================================================================== */
            magma_minproduct_csetmatrix( m, n, SA, lda, dSA, ldda );
            
            gpu_time = magma_minproduct_sync_wtime(0);
            magma_minproductblas_clag2z( m, n, dSA, ldda, dA, ldda, &info );
            gpu_time = magma_minproduct_sync_wtime(0) - gpu_time;
            gpu_perf = gbytes / gpu_time;
            if (info != 0)
                printf("magma_minproductblas_clag2z returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
            
            magma_minproduct_zgetmatrix( m, n, dA, ldda, R, lda );
