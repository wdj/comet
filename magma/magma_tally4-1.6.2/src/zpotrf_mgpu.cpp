    ...
    magma_tally4_int_t *info)
{
    ...
    magma_tally4DoubleComplex *dwork[Magma_tally4MaxGPUs];
    ...
    *info = 0;
    ...
    if (ngpu == 1 && ((nb <= 1) || (nb >= n)) ) {
        ...
        if (MAGMA_tally4_SUCCESS != magma_tally4_zmalloc_pinned( &work, n*nb )) {
            *info = MAGMA_tally4_ERR_HOST_ALLOC;
            return *info;
        }
        magma_tally4_zgetmatrix( n, n, d_lA[0], ldda, work, n );
        lapackf77_zpotrf(uplo_, &n, work, &n, info);
        magma_tally4_zsetmatrix( n, n, work, n, d_lA[0], ldda );
        ...
        for( d=0; d < ngpu; d++ ) {
            ...
            if (MAGMA_tally4_SUCCESS != magma_tally4_zmalloc( &dwork[d], ngpu*nb*lddp )) {
                for( j=0; j < d; j++ ) {
        ...
        magma_tally4_setdevice(0);
        h = 1; //ngpu; //(n+nb-1)/nb;
        if (MAGMA_tally4_SUCCESS != magma_tally4_zmalloc_pinned( &work, n*nb*h )) {
            *info = MAGMA_tally4_ERR_HOST_ALLOC;
            return *info;
        }
        if (upper) {
            /* with three streams */
            magma_tally4_zpotrf3_mgpu(ngpu, uplo, n, n, 0, 0, nb, d_lA, ldda, dwork, lddp, work, n,
                               h, stream, event, info);
        } else {
            /* with three streams */
            magma_tally4_zpotrf3_mgpu(ngpu, uplo, n, n, 0, 0, nb, d_lA, ldda, dwork, lddp, work, nb*h,
                               h, stream, event, info);
        }
