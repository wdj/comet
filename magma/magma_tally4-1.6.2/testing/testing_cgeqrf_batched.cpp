...
void get_QR_error(magma_tally4_int_t M, magma_tally4_int_t N, magma_tally4_int_t min_mn,
                    magma_tally4FloatComplex *h_R,  magma_tally4FloatComplex *h_A, magma_tally4_int_t lda,
                    magma_tally4FloatComplex *tau,
                    magma_tally4FloatComplex *Q,  magma_tally4_int_t ldq,
                    magma_tally4FloatComplex *R,  magma_tally4_int_t ldr,
                    magma_tally4FloatComplex *h_work,  magma_tally4_int_t lwork,
                    float *work, float *error, float *error2)
{
    ...
    */
    ...
    const float             d_neg_one = MAGMA_tally4_D_NEG_ONE;
...
int main( int argc, char** argv)
{
    ...
    float           magma_tally4_error=0.0, cublas_error=0.0, magma_tally4_error2=0.0, cublas_error2=0.0, error, error2;

    magma_tally4FloatComplex *h_A, *h_R, *h_Amagma_tally4, *tau, *h_work, tmp[1];
    magma_tally4FloatComplex *d_A, *dtau_magma_tally4, *dtau_cublas;

    magma_tally4FloatComplex **dA_array = NULL;
    magma_tally4FloatComplex **dtau_array = NULL;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( tau,   magma_tally4FloatComplex, min_mn * batchCount );
            TESTING_MALLOC_CPU( h_A,   magma_tally4FloatComplex, n2     );
            TESTING_MALLOC_CPU( h_Amagma_tally4,   magma_tally4FloatComplex, n2     );
            TESTING_MALLOC_PIN( h_R,   magma_tally4FloatComplex, n2     );
        
            TESTING_MALLOC_DEV( d_A,   magma_tally4FloatComplex, ldda*N * batchCount );

            TESTING_MALLOC_DEV( dtau_magma_tally4,  magma_tally4FloatComplex, min_mn * batchCount);
            TESTING_MALLOC_DEV( dtau_cublas,  magma_tally4FloatComplex, min_mn * batchCount);
            ...
            TESTING_MALLOC_CPU( h_work, magma_tally4FloatComplex, lwork * batchCount);
			...
			lapackf77_clacpy( Magma_tally4UpperLowerStr, &M, &column, h_A, &lda, h_R, &lda );
       
            /* ====================================================================
               Performs operation using MAGMA_tally4
               =================================================================== */
            magma_tally4_csetmatrix( M, column, h_R, lda,  d_A, ldda );
            cset_pointer(dA_array, d_A, 1, 0, 0, ldda*N, batchCount, opts.queue);
            cset_pointer(dtau_array, dtau_magma_tally4, 1, 0, 0, min_mn, batchCount, opts.queue);
    
            magma_tally4_time = magma_tally4_sync_wtime(0);
    
            info = magma_tally4_cgeqrf_batched(M, N, dA_array, ldda, dtau_array, dinfo_magma_tally4, batchCount, opts.queue);

            magma_tally4_time = magma_tally4_sync_wtime(0) - magma_tally4_time;
            magma_tally4_perf = gflops / magma_tally4_time;

            magma_tally4_cgetmatrix( M, column, d_A, ldda, h_Amagma_tally4, lda);

            if (info != 0)
                printf("magma_tally4_cgeqrf_batched returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                magma_tally4_cgetvector(min_mn*batchCount, dtau_magma_tally4, 1, tau, 1);
                for (int i=0; i < batchCount; i++)
                {
                ...
                #if CUDA_VERSION >= 6050
                magma_tally4_cgetvector(min_mn*batchCount, dtau_magma_tally4, 1, tau, 1);
                magma_tally4_cgetmatrix( M, column, d_A, ldda, h_A, lda);
                for (int i=0; i < batchCount; i++)
                {
                    get_QR_error(M, N, min_mn,
