...
magma_minproduct_zgeqrf2(magma_minproduct_context *cntxt, magma_minproduct_int_t m, magma_minproduct_int_t n, 
...
/*  -- MAGMA_minproduct (version 1.6.1) --
    ...
    *info = 0;
    ...
    magma_minproduct_qr_params *qr_params = (magma_minproduct_qr_params *)cntxt->params;
    int nb = qr_params->nb;
    ...
    if (nb >= nbmin && nb < k && nx < k) {
        ...
        magma_minproduct_zgeqrf_mc(cntxt, &rows, &ib, a_ref(i,i), &lda, 
        ...
        zpanel_to_q(Magma_minproductUpper, ib, a_ref(i,i), lda, qr_params->w+cnt*qr_params->nb*qr_params->nb);
            cublasSetMatrix(rows, ib, sizeof(cuDoubleComplex),
                            a_ref(i,i), lda, da_ref(i,i), ldda);
        if (qr_params->flag == 1)
          zq_to_panel(Magma_minproductUpper, ib, a_ref(i,i), lda, qr_params->w+cnt*qr_params->nb*qr_params->nb);
