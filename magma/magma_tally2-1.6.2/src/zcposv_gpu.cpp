    ...
    magma_tally2_int_t *info)
{
    ...
    *info = 0;
    ...
    Anrm = magma_tally2blas_zlanhe(Magma_tally2InfNorm, uplo, n, dA, ldda, (double*)dworkd );
    cte  = Anrm * eps * pow((double)n, 0.5) * BWDMAX;

    /*
     * Convert to single precision
     */
    magma_tally2blas_zlag2c( n, nrhs, dB, lddb, dSX, lddsx, info );
    if (*info != 0) {
        *iter = -2;
        goto FALLBACK;
    }

    magma_tally2blas_zlat2c( uplo, n, dA, ldda, dSA, lddsa, info );
    if (*info != 0) {
        *iter = -2;
        goto FALLBACK;
    }
    ...
    magma_tally2_cpotrf_gpu( uplo, n, dSA, lddsa, info );
    if (*info != 0) {
        *iter = -3;
        goto FALLBACK;
    }
    ...
    magma_tally2_cpotrs_gpu( uplo, n, nrhs, dSA, lddsa, dSX, lddsx, info );
    ...
    if ( nrhs == 1 ) {
        magma_tally2_zhemv( uplo, n,
                     c_neg_one, dA, ldda,
                                dX, 1,
                     c_one,     dR, 1 );
    }
    else {
        magma_tally2_zhemm( Magma_tally2Left, uplo, n, nrhs,
                     c_neg_one, dA, ldda,
                                dX, lddx,
                     c_one,     dR, lddr );
    }
...
REFINEMENT:
    for( iiter=1; iiter < ITERMAX; ) {
        *info = 0;
        ...
        magma_tally2blas_zlag2c( n, nrhs, dR, lddr, dSX, lddsx, info );
        if (*info != 0) {
            *iter = -2;
            goto FALLBACK;
        }
...
FALLBACK:
    ...
    if (*info == 0) {
        ...
        magma_tally2_zpotrs_gpu( uplo, n, nrhs, dA, ldda, dX, lddx, info );
    }
