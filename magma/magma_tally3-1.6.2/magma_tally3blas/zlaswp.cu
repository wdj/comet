...
__global__ void zlaswp_kernel(
    int n, magma_tally3DoubleComplex *dAT, int ldda, zlaswp_params_t params )
{
    int tid = threadIdx.x + blockDim.x*blockIdx.x;
    if ( tid < n ) {
        dAT += tid;
        magma_tally3DoubleComplex *A1  = dAT;
        
        for( int i1 = 0; i1 < params.npivots; ++i1 ) {
            int i2 = params.ipiv[i1];
            magma_tally3DoubleComplex *A2 = dAT + i2*ldda;
            magma_tally3DoubleComplex temp = *A1;
            *A1 = *A2;
            *A2 = temp;
...
magma_tally3blas_zlaswp(
    ...
    const magma_tally3_int_t *ipiv, magma_tally3_int_t inci )
{
    magma_tally3blas_zlaswp_q( n, dAT, ldda, k1, k2, ipiv, inci, magma_tally3_stream );
}
...
__global__ void zlaswpx_kernel(
    int n, magma_tally3DoubleComplex *dA, int ldx, int ldy, zlaswp_params_t params )
{
    int tid = threadIdx.x + blockDim.x*blockIdx.x;
    if ( tid < n ) {
        dA += tid*ldy;
        magma_tally3DoubleComplex *A1  = dA;
        
        for( int i1 = 0; i1 < params.npivots; ++i1 ) {
            int i2 = params.ipiv[i1];
            magma_tally3DoubleComplex *A2 = dA + i2*ldx;
            magma_tally3DoubleComplex temp = *A1;
            *A1 = *A2;
            *A2 = temp;
            A1 += ldx;  // A1 = dA + i1*ldx
...
/**
    @see magma_tally3blas_zlaswpx_q
    @ingroup magma_tally3_zaux2
    ********************************************************************/
extern "C" void
magma_tally3blas_zlaswpx(
    ...
    const magma_tally3_int_t *ipiv, magma_tally3_int_t inci )
{
    return magma_tally3blas_zlaswpx_q( n, dA, ldx, ldy, k1, k2, ipiv, inci, magma_tally3_stream );
}
...
// batches of pivots. On Fermi, it is faster than magma_tally3blas_zlaswp
// (including copying pivots to the GPU).

__global__ void zlaswp2_kernel(
    int n, magma_tally3DoubleComplex *dAT, int ldda, int npivots,
...
/**
    @see magma_tally3blas_zlaswp2_q
    @ingroup magma_tally3_zaux2
    ********************************************************************/
extern "C" void
magma_tally3blas_zlaswp2(
    ...
    magma_tally3Int_const_ptr d_ipiv, magma_tally3_int_t inci )
{
    magma_tally3blas_zlaswp2_q( n, dAT, ldda, k1, k2, d_ipiv, inci, magma_tally3_stream );
}
