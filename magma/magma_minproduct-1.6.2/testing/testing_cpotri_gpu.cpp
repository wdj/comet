...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_minproduct_cpotrf_gpu( opts.uplo, N, d_A, ldda, &info );
            
            // check for exact singularity
            //magma_minproduct_cgetmatrix( N, N, d_A, ldda, h_R, lda );
            //h_R[ 10 + 10*lda ] = MAGMA_minproduct_C_MAKE( 0.0, 0.0 );
            //magma_minproduct_csetmatrix( N, N, h_R, lda, d_A, ldda );
            
            gpu_time = magma_minproduct_wtime();
            magma_minproduct_cpotri_gpu( opts.uplo, N, d_A, ldda, &info );
            gpu_time = magma_minproduct_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_minproduct_cpotri_gpu returned error %d: %s.\n",
