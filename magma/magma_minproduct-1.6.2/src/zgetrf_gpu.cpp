    ...
    magma_minproduct_int_t *info)
{
    ...
    magma_minproductDoubleComplex *dAT, *dAP, *work;
    ...
    *info = 0;
    ...
    if (nb <= 1 || nb >= min(m,n)) {
        /* Use CPU code. */
        magma_minproduct_zmalloc_cpu( &work, m * n );
        if ( work == NULL ) {
            *info = MAGMA_minproduct_ERR_HOST_ALLOC;
            return *info;
        }
        magma_minproduct_zgetmatrix( m, n, dA, ldda, work, m );
        lapackf77_zgetrf(&m, &n, work, &m, ipiv, info);
        magma_minproduct_zsetmatrix( m, n, work, m, dA, ldda );
        ...
        if (MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc( &dAP, nb*maxm )) {
        ...
        if ( m == n ) {
            dAT = dA;
            lddat = ldda;
            magma_minproductblas_ztranspose_inplace( m, dAT, ldda );
        }
        else {
            lddat = maxn;  // N-by-M
            if (MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc( &dAT, lddat*maxm )) {
                magma_minproduct_free( dAP );
                *info = MAGMA_minproduct_ERR_DEVICE_ALLOC;
                return *info;
            }
            magma_minproductblas_ztranspose( m, n, dA, ldda, dAT, lddat );
        }

        ldwork = maxm;
        if (MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc_pinned( &work, ldwork*nb )) {
            magma_minproduct_free( dAP );
            if ( ! (m == n))
        ...
        for( j=0; j < s; j++ ) {
            // download j-th panel
            cols = maxm - j*nb;
            magma_minproductblas_ztranspose( nb, m-j*nb, dAT(j,j), lddat, dAP, cols );

            // make sure that the transpose has completed
            magma_minproduct_queue_sync( stream[1] );
            magma_minproduct_zgetmatrix_async( m-j*nb, nb, dAP, cols, work, ldwork,
                                    stream[0]);
            ...
            // upload j-th panel
            magma_minproduct_zsetmatrix_async( m-j*nb, nb, work, ldwork, dAP, maxm,
                                    stream[0]);

            for( i=j*nb; i < j*nb + nb; ++i ) {
                ipiv[i] += j*nb;
            }
            magma_minproductblas_zlaswp( n, dAT, lddat, j*nb + 1, j*nb + nb, ipiv, 1 );

            magma_minproduct_queue_sync( stream[0] );
            magma_minproductblas_ztranspose( m-j*nb, nb, dAP, maxm, dAT(j,j), lddat );
        ...
        if ( nb0 > 0 ) {
            ...
            magma_minproductblas_ztranspose( nb0, rows, dAT(s,s), lddat, dAP, maxm );
            magma_minproduct_zgetmatrix( rows, nb0, dAP, maxm, work, ldwork );
            ...
            magma_minproductblas_zlaswp( n, dAT, lddat, s*nb + 1, s*nb + nb0, ipiv, 1 );
    
            // upload j-th panel
            magma_minproduct_zsetmatrix( rows, nb0, work, ldwork, dAP, maxm );
            magma_minproductblas_ztranspose( rows, nb0, dAP, maxm, dAT(s,s), lddat );
        ...
        // undo transpose
        if ( m == n ) {
            magma_minproductblas_ztranspose_inplace( m, dAT, lddat );
        }
        else {
            magma_minproductblas_ztranspose( n, m, dAT, lddat, dA, ldda );
            magma_minproduct_free( dAT );
        }
