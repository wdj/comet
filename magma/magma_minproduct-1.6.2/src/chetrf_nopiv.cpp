...
/**
    ...
    @ingroup magma_minproduct_chesv_comp
    ******************************************************************* */
    ...
    *info = 0;
    ...
    if (upper) {
        ...
        for (j=0; j<n; j+=nb) {
            ...
            magma_minproduct_csetmatrix_async(j+jb, jb, A(0, j), lda, dA(0, j), ldda, stream[0]);
            trace_gpu_end( 0, 0 );
        }
        ...
        for (j=0; j<n; j += nb) {
            ...
            if ( j!=0) {
                ...
                magma_minproduct_cgetmatrix_async(jb, jb, dA(j, j), ldda, A(j,j), lda, stream[1]);
            }
            trace_gpu_end( 0, 0 );
            ...
            chetrf_nopiv_cpu(Magma_minproductUpper, jb, ib, A(j, j), lda, info);
            trace_cpu_end( 0 );
            if (*info != 0){
                *info = *info + j;
                break;
            ...
            trace_gpu_start( 0, 0, "set", "set" );
            magma_minproduct_csetmatrix_async(jb, jb, A(j, j), lda, dA(j, j), ldda, stream[0]);
            trace_gpu_end( 0, 0 );
            
            // copy j-th column of U back to CPU
            trace_gpu_start( 0, 1, "get", "get" );                                                                         
            magma_minproduct_cgetmatrix_async(j, jb, dA(0, j), ldda, A(0, j), lda, stream[1]);                                        
            trace_gpu_end( 0, 1 );
            ...
            if ( (j+jb) < n) {
                ...
                magma_minproduct_ctrsm(Magma_minproductLeft, Magma_minproductUpper, Magma_minproductConjTrans, Magma_minproductUnit, 
                            jb, (n-j-jb), 
                            zone, dA(j, j),    ldda, 
                            dA(j, j+jb), ldda);
                magma_minproduct_ccopymatrix( jb, n-j-jb, dA( j, j+jb ), ldda, dWt( 0, j+jb ), nb );
                
                // update the trailing submatrix with D
                magma_minproductblas_clascl_diag(Magma_minproductUpper, jb, n-j-jb,
                                      dA(j,    j), ldda,
                                      dA(j, j+jb), ldda,
                                      &iinfo);
                trace_gpu_end( 0, 0 );
                ...
                for (k=j+jb; k<n; k+=nb) {
                    ...
                    magma_minproduct_cgemm(Magma_minproductConjTrans, Magma_minproductNoTrans, kb, n-k, jb,
                                mzone, dWt(0, k), nb, 
                                       dA(j, k), ldda,
                                zone,  dA(k, k), ldda);
                    if (k==j+jb) {
        ...
        for (j=0; j<n; j+=nb) {
            jb = min(nb, (n-j));
            trace_gpu_start( 0, 0, "set", "set" );
            magma_minproduct_csetmatrix_async((n-j), jb, A(j, j), lda, dA(j, j), ldda, stream[0]);
            trace_gpu_end( 0, 0 );
        }
        ...
        for (j=0; j<n; j+=nb) {
            ...
            magma_minproduct_queue_sync(stream[1]);
            trace_cpu_start( 0, "potrf", "potrf" );
            chetrf_nopiv_cpu(Magma_minproductLower, jb, ib, A(j, j), lda, info);
            trace_cpu_end( 0 );
            if (*info != 0){
                *info = *info + j;
                break;
            ...
            trace_gpu_start( 0, 1, "get", "get" );
            magma_minproduct_cgetmatrix_async(jb, j, dA(j, 0), ldda, A(j, 0), lda, stream[1]);
            trace_gpu_end( 0, 1 ); 
            ...
            if ( (j+jb) < n) {
                ...
                magma_minproductblasSetKernelStream( stream[0] );
                trace_gpu_start( 0, 0, "trsm", "trsm" );
                magma_minproduct_ctrsm(Magma_minproductRight, Magma_minproductLower, Magma_minproductConjTrans, Magma_minproductUnit, 
                            (n-j-jb), jb, 
                            zone, dA(j,    j), ldda, 
                            dA(j+jb, j), ldda);
                magma_minproduct_ccopymatrix( n-j-jb,jb, dA( j+jb, j ), ldda, dW( j+jb, 0 ), ldda );
                
                // update the trailing submatrix with D
                magma_minproductblas_clascl_diag(Magma_minproductLower, n-j-jb, jb,
                                      dA(j,    j), ldda,
                                      dA(j+jb, j), ldda,
                                      &iinfo);
                trace_gpu_end( 0, 0 );
                ...
                for (k=j+jb; k<n; k+=nb) {
                    magma_minproduct_int_t kb = min(nb,n-k);
                    magma_minproduct_cgemm(Magma_minproductNoTrans, Magma_minproductConjTrans, n-k, kb, jb,
                                mzone, dA(k, j), ldda, 
                                       dW(k, 0), ldda,
                                zone,  dA(k, k), ldda);
                    if (k==j+jb) {
