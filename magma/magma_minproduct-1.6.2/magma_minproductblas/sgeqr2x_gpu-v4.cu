    ...
    for (magma_minproduct_int_t b=0; b < k; b += BS) {
        for (i = b; i < min(k, b+BS); ++i) {
            ...
            if (i-b > 0) {
                ...
                if ( i-1 > 0 ) {
                    magma_minproduct_sgemv_kernel3<<< i-1, BLOCK_SIZE, 0, magma_minproduct_stream >>>
                        ( m-i+1, dA(i-1,0), ldda, dA(i-1, i-1), work, dtau+i-1);
                    magma_minproduct_strmv_kernel2<<< i-1, i-1, 0, magma_minproduct_stream >>>
                        ( dT(0,0), k, work, dT(0,i-1), dtau+i-1);
                }

                /* dwork = V**H c */
                magma_minproduct_sgemv_kernel1<<< i-b, BLOCK_SIZE, 0, magma_minproduct_stream >>>
                    (m-b, dA(b, b),  ldda, dA(b,i), work);

                /* dwork = T**H work */
                magma_minproduct_strmv_tkernel<<< i-b, i-b, 0, magma_minproduct_stream >>>
                ...
                if ( m-b > 0 ) {
                    ...
                    magma_minproduct_sgemv_kernel2<<< blocks3, threads3, 0, magma_minproduct_stream >>>
            ...
            if ( i > 0 ) {
                magma_minproduct_snrm2_adjust_kernel<<< 1, i, 0, magma_minproduct_stream >>>(dnorm+i, dA(0, i));
            }
            ...
            /*  Generate elementary reflector H(i) to annihilate A(i+1:m,i)
                ...
                3. update T */
            magma_minproduct_slarfgx_gpu(m-i, dA(i, i), dA(min(i+1,m),i), dtau+i,
                              dnorm+i, ddA + i + i*n, i);

            if (i == 0) {
                float tt = MAGMA_minproduct_S_ONE;
                magma_minproductblas_slacpy(Magma_minproductUpperLower, 1, 1, dtau, 1, dT(0,0), 1);
                magma_minproduct_ssetmatrix_async(1, 1, &tt, 1, dA(i, i), 1, magma_minproduct_stream);
            }
        }
        if ( i-1 > 0 ) {
            magma_minproduct_sgemv_kernel3<<< i-1, BLOCK_SIZE, 0, magma_minproduct_stream >>>
