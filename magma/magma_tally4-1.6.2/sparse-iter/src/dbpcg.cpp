    ...
    magma_tally4_queue_t queue )
{
    ...
    // GPU workspace
    magma_tally4_d_matrix r={Magma_tally4_CSR}, rt={Magma_tally4_CSR}, p={Magma_tally4_CSR}, q={Magma_tally4_CSR}, h={Magma_tally4_CSR};
    ...
    CHECK( magma_tally4_dmalloc_cpu(&alpha, num_vecs));
    CHECK( magma_tally4_dmalloc_cpu(&beta, num_vecs));
    ...
    CHECK( magma_tally4_dvinit( &r, Magma_tally4_DEV, dofs*num_vecs, 1, c_zero, queue ));
    CHECK( magma_tally4_dvinit( &rt, Magma_tally4_DEV, dofs*num_vecs, 1, c_zero, queue ));
    CHECK( magma_tally4_dvinit( &p, Magma_tally4_DEV, dofs*num_vecs, 1, c_zero, queue ));
    CHECK( magma_tally4_dvinit( &q, Magma_tally4_DEV, dofs*num_vecs, 1, c_zero, queue ));
    CHECK( magma_tally4_dvinit( &h, Magma_tally4_DEV, dofs*num_vecs, 1, c_zero, queue ));

    // solver setup
    CHECK(  magma_tally4_dresidualvec( A, b, *x, &r, nom0, queue));

    // preconditioner
    CHECK( magma_tally4_d_applyprecond_left( A, r, &rt, precond_par, queue ));
    CHECK( magma_tally4_d_applyprecond_right( A, rt, &h, precond_par, queue ));

    magma_tally4_dcopy( dofs*num_vecs, h.dval, 1, p.dval, 1 );                 // p = h

    for( i=0; i<num_vecs; i++) {
        nom[i] = MAGMA_tally4_D_REAL( magma_tally4_ddot(dofs, r(i), 1, h(i), 1) );
        nom0[i] = magma_tally4_dnrm2( dofs, r(i), 1 );
    }
                                          
    CHECK( magma_tally4_d_spmv( c_one, A, p, c_zero, q, queue ));             // q = A p

    for( i=0; i<num_vecs; i++)
        den[i] = MAGMA_tally4_D_REAL( magma_tally4_ddot(dofs, p(i), 1, q(i), 1) );  // den = p dot q
    ...
    do
    {
        ...
        for( i=0; i<num_vecs; i++)
            gammanew[i] = MAGMA_tally4_D_REAL( magma_tally4_ddot(dofs, r(i), 1, h(i), 1) );  // gn = < r,h>
        ...
        if ( solver_par->numiter==1 ) {
            ...
            for( i=0; i<num_vecs; i++) {
                beta[i] = MAGMA_tally4_D_MAKE(gammanew[i]/gammaold[i], 0.);       // beta = gn/go
                magma_tally4_dscal(dofs, beta[i], p(i), 1);            // p = beta*p
                magma_tally4_daxpy(dofs, c_one, h(i), 1, p(i), 1); // p = p + h
            }
        }

        CHECK( magma_tally4_d_spmv( c_one, A, p, c_zero, q, queue ));   // q = A p

     //   magma_tally4_d_bspmv_tuned( dofs, num_vecs, c_one, A, p.dval, c_zero, q.dval, queue );


        for( i=0; i<num_vecs; i++) {
            den[i] = MAGMA_tally4_D_REAL(magma_tally4_ddot(dofs, p(i), 1, q(i), 1));
                // den = p dot q

            alpha[i] = MAGMA_tally4_D_MAKE(gammanew[i]/den[i], 0.);
            magma_tally4_daxpy(dofs,  alpha[i], p(i), 1, x->dval+dofs*i, 1); // x = x + alpha p
            magma_tally4_daxpy(dofs, -alpha[i], q(i), 1, r(i), 1);      // r = r - alpha q
            gammaold[i] = gammanew[i];

            res[i] = magma_tally4_dnrm2( dofs, r(i), 1 );
        }
...
cleanup:
    ...
    magma_tally4_dmfree(&rt, queue );
    magma_tally4_dmfree(&p, queue );
    magma_tally4_dmfree(&q, queue );
    magma_tally4_dmfree(&h, queue );
