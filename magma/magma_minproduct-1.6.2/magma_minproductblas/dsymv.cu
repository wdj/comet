...
/**
    Purpose
    -------
    magma_minproductblas_dsymv_work performs the matrix-vector operation:
    ...
    MAGMA_minproduct implements dsymv through two steps:
    ...
    magma_minproductblas_dsymv is a wrapper routine allocating the workspace inside the
    routine and provides the same interface as cublas.
    
    If users need to call dsymv frequently, we suggest using
    magma_minproductblas_dsymv_work instead of magma_minproductblas_dsymv. As the overhead to
    allocate and free in device memory in magma_minproductblas_dsymv would hurt performance.
    ...
    magma_minproduct_queue_t queue )
{
    ...
    if ( arch < 200 ) {
        magma_minproduct_dsymv( uplo, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
     ...
     */
    if ( (n == 0) || ( MAGMA_minproduct_D_EQUAL(alpha, MAGMA_minproduct_D_ZERO) && MAGMA_minproduct_D_EQUAL(beta, MAGMA_minproduct_D_ONE) ) )
        return info;
...
// end magma_minproductblas_dsymv_work


/**
    Purpose
    -------
    magma_minproductblas_dsymv performs the matrix-vector operation:
    ...
    magma_minproductDouble_ptr dy, magma_minproduct_int_t incy)
{
    ...
    magma_minproduct_dmalloc( &dwork, lwork );
    if ( dwork == NULL ) {
    ...
    magma_minproductblas_dsymv_work( uplo, n, alpha, dA, ldda, dx, incx, beta, dy, incy,
...
// end magma_minproductblas_dsymv
