...
{
    magma_tally3DoubleComplex *da[Magma_tally3MaxGPUs];
    ...
    *info = 0;
    ...
    for (i=0; i < ngpu; i++) {
        ...
        if (MAGMA_tally3_SUCCESS != magma_tally3_zmalloc( &da[i], ldda*n_local[i] )) {
    ...
    if (m > nb && n > nb) {
        ...
        magma_tally3_zsetmatrix_1D_col_bcyclic(m, n, A, lda, da, ldda, ngpu, nb);

        /* Factor using the GPU interface */
        magma_tally3_zgeqrf2_mgpu( ngpu, m, n, da, ldda, tau, info);
