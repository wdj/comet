/*
    -- MAGMA_tally4 (version 1.6.1) --
       ...
       @generated from zhemv_mgpu_upper.cu normal z -> c, Fri Jan 30 19:00:10 2015
    ...
    int block_offset)
{
    ...
    __shared__ magma_tally4FloatComplex sx_jj [NB_X];  // for x[ jj ], which cycles over all blocks right of diag
    ...
    for(int jj=next; jj < gridDim.x; jj += ngpu) {
        ...
        if ( ty4 < 4 && (partial == 0 || tx4 + ty4*quarter_NB_X < partial) ) {
            ...
            work[jj*NB_X + tx4 + ty4_nb4] = psum_t;  //MAGMA_tally4_C_MAKE( tx4, blk );  // store at work( jj*NB_X + tx4 + ty4*16, blk )
        }
        __syncthreads();
    }
    ...
    if ( ty == 0 && (partial == 0 || tx < partial) ) {
        total = sA16(0, tx)
              ...
              + sA16(2, tx)
              + sA16(3, tx);
        work[blk*NB_X + tx] = total;  //MAGMA_tally4_C_MAKE( tx, blk );  // store at work( blk*NB_X + tx, blk )
    }
