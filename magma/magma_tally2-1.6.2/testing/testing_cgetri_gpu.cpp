...
int main( int argc, char** argv )
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            n2     = lda*N;
            ldda   = ((N+31)/32)*32;
            ldwork = N * magma_tally2_get_cgetri_nb( N );
            ...
            lwork = int( MAGMA_tally2_C_REAL( tmp ));
            ...
            /* Factor the matrix. Both MAGMA_tally2 and LAPACK will use this factor. */
            magma_tally2_csetmatrix( N, N, h_A, lda, d_A, ldda );
            magma_tally2_cgetrf_gpu( N, N, d_A, ldda, ipiv, &info );
            ...
            //h_A[ 10 + 10*lda ] = MAGMA_tally2_C_MAKE( 0.0, 0.0 );
            //magma_tally2_csetmatrix( N, N, h_A, lda, d_A, ldda );
            ...
            magma_tally2_cgetri_gpu( N, d_A, ldda, ipiv, dwork, ldwork, &info );
            gpu_time = magma_tally2_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally2_cgetri_gpu returned error %d: %s.\n",
                       (int) info, magma_tally2_strerror( info ));
            
            magma_tally2_cgetmatrix( N, N, d_A, ldda, h_R, lda );
