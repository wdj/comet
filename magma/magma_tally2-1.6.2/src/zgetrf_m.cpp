...
#define dPT(d,i,j) (dPT[d] + (i)*nb*nb + (j)*nb*maxm)
    ...
    magma_tally2_timer_t time=0, time_total=0, time_alloc=0, time_set=0, time_get=0, time_comp=0;
    timer_start( time_total );
    real_Double_t flops;

    magma_tally2DoubleComplex c_one     = MAGMA_tally2_Z_ONE;
    magma_tally2DoubleComplex c_neg_one = MAGMA_tally2_Z_NEG_ONE;
    magma_tally2DoubleComplex *dAT[Magma_tally2MaxGPUs], *dA[Magma_tally2MaxGPUs], *dPT[Magma_tally2MaxGPUs];
    magma_tally2_int_t        iinfo = 0, nb, nbi, maxm, n_local[Magma_tally2MaxGPUs], ldn_local;
    magma_tally2_int_t        N, M, NB, NBk, I, d, ngpu0 = ngpu;
    magma_tally2_int_t        ii, jj, h, offset, ib, rows;
    
    magma_tally2_queue_t stream[Magma_tally2MaxGPUs][2];
    ...
    *info = 0;
    ...
    freeMem /= sizeof(magma_tally2DoubleComplex);
    
    /* number of columns in the big panel */
    h = 1+(2+ngpu0);
    NB = (magma_tally2_int_t)(0.8*freeMem/maxm-h*nb);
    const char* ngr_nb_char = getenv("MAGMA_tally2_NGR_NB");
    if ( ngr_nb_char != NULL )
    ...
    if ( (nb <= 1) || (nb >= min(m,n)) ) {
        ...
        for( d=0; d < ngpu; d++ ) {
            ...
            if (MAGMA_tally2_SUCCESS != magma_tally2_zmalloc( &dA[d], (ldn_local+h*nb)*maxm )) {
            ...
            magma_tally2_queue_create( &stream[d][0] );
            magma_tally2_queue_create( &stream[d][1] );
            magma_tally2_event_create( &event[d][0] );
            magma_tally2_event_create( &event[d][1] );
        }
        ...
        for( I=0; I < n; I += NB ) {
            ...
            timer_start( time );
            magma_tally2blas_zsetmatrix_transpose_mgpu(ngpu, stream, A(0,I), lda,
                                                dAT, ldn_local, dA, maxm, M, N, nb);
            for( d=0; d < ngpu; d++ ) {
                magma_tally2_setdevice(d);
                magma_tally2_queue_sync( stream[d][0] );
                magma_tally2_queue_sync( stream[d][1] );
            ...
            for( offset = 0; offset < min(m,I); offset += NB ) {
                ...
                for( d=0; d < ngpu; d++ ) {
                    magma_tally2_setdevice(d);
                    nbi  = min( nb, NBk );
                    magma_tally2_zsetmatrix_async( (M-offset), nbi,
                                            A(offset,offset), lda,
                                            dA[d],            (maxm-offset), stream[d][0] );
                    
                    /* make sure the previous update finished */
                    magma_tally2blasSetKernelStream(stream[d][0]);
                    //magma_tally2_queue_sync( stream[d][1] );
                    magma_tally2_queue_wait_event( stream[d][0], event[d][0] );
                    
                    /* transpose */
                    magma_tally2blas_ztranspose( M-offset, nbi, dA[d], maxm-offset, dPT(d,0,0), nb );
                }
                
                /* applying the pivot from the previous big-panel */
                for( d=0; d < ngpu; d++ ) {
                    magma_tally2_setdevice(d);
                    magma_tally2blas_zlaswp_q( ldn_local, dAT(d,0,0), ldn_local, offset+1, offset+NBk, ipiv, 1, stream[d][1] );
                }
                ...
                for( jj=0, ib=offset/nb; jj < NBk; jj += nb, ib++ ) {
                    ...
                    for( d=0; d < ngpu; d++ ) {
                        ...
                        if ( jj+nb < NBk ) {
                            magma_tally2_zsetmatrix_async( (M-ii-nb), min(nb,NBk-jj-nb),
                                                    A(ii+nb,ii+nb), lda,
                            ...
                            magma_tally2blasSetKernelStream(stream[d][0]);
                            //magma_tally2_queue_sync( stream[d][1] );
                            magma_tally2_queue_wait_event( stream[d][0], event[d][(1+jj/nb)%2] );
                            
                            /* transpose next column */
                            magma_tally2blas_ztranspose( M-ii-nb, nb, dA[d], rows-nb, dPT(d,0,(1+jj/nb)%2), nb );
                        }
                        
                        /* update with the block column */
                        magma_tally2blasSetKernelStream(stream[d][1]);
                        magma_tally2_ztrsm( Magma_tally2Right, Magma_tally2Upper, Magma_tally2NoTrans, Magma_tally2Unit,
                        ...
                        magma_tally2_event_record( event[d][(jj/nb)%2], stream[d][1] );
            ...
            /* calling magma_tally2-gpu interface to panel-factorize the big panel */
            if ( M > I ) {
                magma_tally2_zgetrf2_mgpu(ngpu, M-I, N, nb, I, dAT, ldn_local, ipiv+I, dA, A(0,I), lda,
                                   stream, &iinfo);
                if ( iinfo < 0 ) {
                    *info = iinfo;
                    break;
            ...
            timer_start( time );
            magma_tally2blas_zgetmatrix_transpose_mgpu(ngpu, stream, dAT, ldn_local, A(0,I), lda, dA, maxm, M, N, nb);
            for( d=0; d < ngpu; d++ ) {
                magma_tally2_setdevice(d);
                magma_tally2_queue_sync( stream[d][0] );
                magma_tally2_queue_sync( stream[d][1] );
        ...
        for( d=0; d < ngpu0; d++ ) {
            ...
            magma_tally2_event_destroy( event[d][0] );
            magma_tally2_event_destroy( event[d][1] );
            magma_tally2_queue_destroy( stream[d][0] );
            magma_tally2_queue_destroy( stream[d][1] );
        }
        magma_tally2_setdevice( orig_dev );
        magma_tally2blasSetKernelStream( orig_stream );
    }
    if ( *info >= 0 )
        magma_tally2_zgetrf_piv(m, n, NB, A, lda, ipiv, info);
    return *info;
} /* magma_tally2_zgetrf_m */
...
magma_tally2_zgetrf_piv(
    ...
    magma_tally2_int_t *info)
{
    magma_tally2_int_t I, k1, k2, incx, minmn;
    *info = 0;
    if (m < 0)
        *info = -1;
    else if (n < 0)
...
// ----------------------------------------------------------------------
extern "C" magma_tally2_int_t
magma_tally2_zgetrf2_piv(
    magma_tally2_int_t m, magma_tally2_int_t n, magma_tally2_int_t start, magma_tally2_int_t end,
    magma_tally2DoubleComplex *A, magma_tally2_int_t lda, magma_tally2_int_t *ipiv,
    magma_tally2_int_t *info)
{
    magma_tally2_int_t I, k1, k2, nb, incx, minmn;
