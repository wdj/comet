...
int main( int argc, char** argv)
{
    ...
    magma_minproductFloat_ptr d_R, dwork;
    float *tau;
    float          *diag, *offdiag;
    float           result[2] = {0., 0.};
    ...
    printf("2 - uses SSYMV from MAGMA_minproduct BLAS that requires extra space\n\n");
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if (opts.version == 1) {
                magma_minproduct_ssytrd_gpu( opts.uplo, N, d_R, ldda, diag, offdiag,
                                  tau, h_R, lda, h_work, lwork, &info );
            }
            else {
                magma_minproduct_ssytrd2_gpu( opts.uplo, N, d_R, ldda, diag, offdiag,
                                   tau, h_R, lda, h_work, lwork, dwork, ldwork, &info );
            }
            gpu_time = magma_minproduct_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_minproduct_ssytrd_gpu returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                magma_minproduct_sgetmatrix( N, N, d_R, ldda, h_Q, lda );
