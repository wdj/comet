...
magma_minproduct_get_ztrsm_m_nb() { return 128; }
...
/**
    ...
    lda     INTEGER.
            ...
            When SIDE = Magma_minproductLeft  then LDA >= max( 1, m ),
            when SIDE = Magma_minproductRight then LDA >= max( 1, n ).
...
magma_minproduct_ztrsm_m(
    ...
    magma_minproduct_trans_t transa, magma_minproduct_diag_t diag,
...
#define dA(gpui, i, j) (dw[gpui] + dimb*lddb + (i)*nb + (j)*nb*ldda)
    ...
    magma_minproductDoubleComplex  alpha_;
    magma_minproductDoubleComplex* dw[Magma_minproductMaxGPUs];
    magma_minproduct_queue_t stream [Magma_minproductMaxGPUs][3];
    magma_minproduct_int_t lside;
    magma_minproduct_int_t upper;
    magma_minproduct_int_t notransp;
    magma_minproduct_int_t nrowa;
    magma_minproduct_int_t nb = magma_minproduct_get_ztrsm_m_nb();
    magma_minproduct_int_t igpu = 0;
    magma_minproduct_int_t info;
    magma_minproduct_int_t k, j, kb, jb;
    magma_minproduct_int_t ldda, dima, lddb, dimb;
    ...
    lside = (side == Magma_minproductLeft);
    if (lside) {
        nrowa = m;
    } else {
        nrowa = n;
    ...
    notransp = (transa == Magma_minproductNoTrans);

    info = 0;
    if (! lside && side != Magma_minproductRight) {
        info = 1;
    } else if (! upper && uplo != Magma_minproductLower) {
        info = 2;
    } else if (! notransp && transa != Magma_minproductTrans
               && transa != Magma_minproductConjTrans) {
        info = 3;
    ...
    if (MAGMA_minproduct_Z_REAL(alpha) == 0. && MAGMA_minproduct_Z_IMAG(alpha) == 0.) {
    ...
    magma_minproduct_int_t nbl = (n-1)/nb+1; // number of blocks in a row
    magma_minproduct_int_t mbl = (m-1)/nb+1; // number of blocks in a column
    ...
    for (igpu = 0; igpu < ngpu; ++igpu) {
        ...
        if (MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc( &dw[igpu], (dimb*lddb + dima*ldda) )) {
    ...
    if (lside) {
        if (notransp) {
            ...
            if (upper) {
                ...
                magma_minproduct_int_t nloc[Magma_minproductMaxGPUs];
                for (igpu = 0; igpu < ngpu; ++igpu)
                    nloc[igpu] = 0;
                ...
                for (k = 0; k < nbl; ++k) {
                    ...
                    kb = min(nb, n-k*nb);
                    nloc[igpu] += kb;
                    magma_minproduct_zsetmatrix_async( m, kb,
                                            B(0, k),              ldb,
                ...
                for (j = mbl-1; j >= 0; --j) {
                    if (j > 0) {
                        ...
                        for (igpu = 0; igpu < ngpu; ++igpu) {
                            magma_minproduct_setdevice(igpu);
                            magma_minproduct_zsetmatrix_async( j*nb, jb,
                                                    A(0, j-1),            lda,
                    ...
                    for (igpu = 0; igpu < ngpu; ++igpu) {
                        ...
                        magma_minproduct_ztrsm(side, uplo, transa, diag, jb, nloc[igpu], alpha_, dA(igpu, j, j%2), ldda,
                                    dB(igpu, j, 0), lddb );
                    }
                    ...
                    if (j > 0) {
                        for (igpu = 0; igpu < ngpu; ++igpu) {
                            magma_minproduct_setdevice(igpu);
                            magma_minproductblasSetKernelStream(stream[igpu][j%2]);
                            magma_minproduct_zgemm(transa, Magma_minproductNoTrans, j*nb, nloc[igpu], jb, c_neg_one, dA(igpu, 0, j%2), ldda,
                                        dB(igpu, j, 0), lddb, alpha_, dB(igpu, 0, 0), lddb );
                        }
                    }

                    for (igpu = 0; igpu < ngpu; ++igpu) {
                        magma_minproduct_queue_sync( stream[igpu][j%2] );
                    }
                ...
                for (j = 0; j < mbl; ++j) {
                    if ((j+1)*nb < m) {
                        ...
                        for (igpu = 0; igpu < ngpu; ++igpu) {
                            magma_minproduct_setdevice(igpu);
                            magma_minproduct_zsetmatrix_async( (m-(j+1)*nb), jb,
                                                    A(j+1, j+1),            lda,
                    ...
                    if ( j < mbl-1 ) {
                        for (igpu = 0; igpu < ngpu; ++igpu) {
                            magma_minproduct_setdevice(igpu);
                            magma_minproductblasSetKernelStream(stream[igpu][j%2]);
                            magma_minproduct_zgemm(transa, Magma_minproductNoTrans, m-(j+1)*nb, nloc[igpu], nb, c_neg_one, dA(igpu, j+1, j%2), ldda,
            ...
            if (upper) {
                ...
                for (igpu = 0; igpu < ngpu; ++igpu) {
                    magma_minproduct_setdevice(igpu);
                    magma_minproduct_zsetmatrix_async( jb, m,
                                            A(0, 0),        lda,
                                            dA(igpu, 0, 0), ldda, stream[igpu][0] );
                }
                for (j = 0; j < mbl; ++j) {
                    if ((j+1)*nb < m) {
                        ...
                        for (igpu = 0; igpu < ngpu; ++igpu) {
                            magma_minproduct_setdevice(igpu);
                            magma_minproduct_zsetmatrix_async( jb, m-(j+1)*nb,
                                                    A(j+1, j+1),            lda,
                    ...
                    for (igpu = 0; igpu < ngpu; ++igpu) {
                        magma_minproduct_setdevice(igpu);
                        magma_minproductblasSetKernelStream(stream[igpu][j%2]);
                        magma_minproduct_ztrsm(side, uplo, transa, diag, jb, nloc[igpu], alpha_, dA(igpu, j%2, j), ldda,
                                    dB(igpu, j, 0), lddb );
                    }
                    ...
                    if ( j < mbl-1 ) {
                        for (igpu = 0; igpu < ngpu; ++igpu) {
                            magma_minproduct_setdevice(igpu);
                            magma_minproductblasSetKernelStream(stream[igpu][j%2]);
                            magma_minproduct_zgemm(transa, Magma_minproductNoTrans, m-(j+1)*nb, nloc[igpu], nb, c_neg_one, dA(igpu, j%2, j+1), ldda,
                                        dB(igpu, j, 0), lddb, alpha_, dB(igpu, j+1, 0), lddb );
                        }
                    }
                ...
                for (j = mbl-1; j >= 0; --j) {
                    if (j > 0) {
                        ...
                        for (igpu = 0; igpu < ngpu; ++igpu) {
                            magma_minproduct_setdevice(igpu);
                            magma_minproduct_zsetmatrix_async( jb, j*nb,
                                                    A(j-1, 0),            lda,
                    ...
                    if (j > 0) {
                        for (igpu = 0; igpu < ngpu; ++igpu) {
                            magma_minproduct_setdevice(igpu);
                            magma_minproductblasSetKernelStream(stream[igpu][j%2]);
                            magma_minproduct_zgemm(transa, Magma_minproductNoTrans, j*nb, nloc[igpu], jb, c_neg_one, dA(igpu, j%2, 0), ldda,
                                        dB(igpu, j, 0), lddb, alpha_, dB(igpu, 0, 0), lddb );
                        }
                    }
        ...
        if (notransp) {
            ...
            if (upper) {
                ...
                magma_minproduct_int_t mloc[Magma_minproductMaxGPUs];
                for (igpu = 0; igpu < ngpu; ++igpu)
                    mloc[igpu] = 0;
                ...
                for (igpu = 0; igpu < ngpu; ++igpu) {
                    magma_minproduct_setdevice(igpu);
                    magma_minproduct_zsetmatrix_async( kb, n,
                                            A(0, 0),        lda,
                                            dA(igpu, 0, 0), ldda, stream[igpu][0] );
                }
                for (k = 0; k < nbl; ++k) {
                    ...
                    for (igpu = 0; igpu < ngpu; ++igpu) {
                        ...
                        magma_minproduct_ztrsm(side, uplo, transa, diag, mloc[igpu], kb, alpha_, dA(igpu, k%2, k), ldda,
                                    dB(igpu, 0, k), lddb );
                    }
                    ...
                    if ( k < nbl-1 ) {
                        for (igpu = 0; igpu < ngpu; ++igpu) {
                            magma_minproduct_setdevice(igpu);
                            magma_minproductblasSetKernelStream(stream[igpu][k%2]);
                            magma_minproduct_zgemm(Magma_minproductNoTrans, transa, mloc[igpu], n-(k+1)*nb, nb, c_neg_one, dB(igpu, 0, k), lddb,
                                        dA(igpu, k%2, k+1), ldda, alpha_, dB(igpu, 0, k+1), lddb );
                        }
                    }

                    for (igpu = 0; igpu < ngpu; ++igpu) {
                        magma_minproduct_queue_sync( stream[igpu][k%2] );
                    }
                ...
                for (k = nbl-1; k >= 0; --k) {
                    ...
                    if (k > 0) {
                        for (igpu = 0; igpu < ngpu; ++igpu) {
                            magma_minproduct_setdevice(igpu);
                            magma_minproductblasSetKernelStream(stream[igpu][k%2]);
                            magma_minproduct_zgemm(Magma_minproductNoTrans, transa, mloc[igpu], k*nb, kb, c_neg_one, dB(igpu, 0, k), lddb,
            ...
            if (upper) {
                ...
                for (igpu = 0; igpu < ngpu; ++igpu) {
                    magma_minproduct_setdevice(igpu);
                    magma_minproduct_zsetmatrix_async( n, kb,
                                            A(0, nbl-1),            lda,
                ...
                for (k = nbl-1; k >= 0; --k) {
                    ...
                    for (igpu = 0; igpu < ngpu; ++igpu) {
                        magma_minproduct_setdevice(igpu);
                        magma_minproductblasSetKernelStream(stream[igpu][k%2]);
                        magma_minproduct_ztrsm(side, uplo, transa, diag, mloc[igpu], kb, alpha_, dA(igpu, k, k%2), ldda,
                                    dB(igpu, 0, k), lddb );
                    }
    ...
    for (igpu = 0; igpu < ngpu; ++igpu) {
        magma_minproduct_setdevice(igpu);
        magma_minproduct_queue_sync( stream[igpu][2] );
