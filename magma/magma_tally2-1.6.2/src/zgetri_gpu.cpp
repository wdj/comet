    ...
    magma_tally2_int_t *info )
{
    ...
    magma_tally2DoubleComplex_ptr dL = dwork;
    magma_tally2_int_t lddl = n;
    magma_tally2_int_t nb   = magma_tally2_get_zgetri_nb(n);
    ...
    *info = 0;
    ...
    magma_tally2_ztrtri_gpu( Magma_tally2Upper, Magma_tally2NonUnit, n, dA, ldda, info );
    if ( *info != 0 )
        return *info;
    ...
    for( j = jmax; j >= 0; j -= nb ) {
        ...
        magma_tally2blas_zlacpy( Magma_tally2Full, n-j, jb,
                          dA(j,j), ldda,
                          dL(j,0), lddl );
        magma_tally2blas_zlaset( Magma_tally2Lower, n-j-1, jb, c_zero, c_zero, dA(j+1,j), ldda );
        ...
        if ( j+jb < n ) {
            magma_tally2_zgemm( Magma_tally2NoTrans, Magma_tally2NoTrans, n, jb, n-j-jb,
                         c_neg_one, dA(0,j+jb), ldda,
                                    dL(j+jb,0), lddl,
                         c_one,     dA(0,j),    ldda );
        }
        // TODO use magma_tally2blas work interface
        magma_tally2_ztrsm( Magma_tally2Right, Magma_tally2Lower, Magma_tally2NoTrans, Magma_tally2Unit,
                     n, jb, c_one,
                     dL(j,0), lddl,
                     dA(0,j), ldda );
    }
    ...
    for( j = n-2; j >= 0; --j ) {
        jp = ipiv[j] - 1;
        if ( jp != j ) {
            magma_tally2blas_zswap( n, dA(0,j), 1, dA(0,jp), 1 );
        }
    }
