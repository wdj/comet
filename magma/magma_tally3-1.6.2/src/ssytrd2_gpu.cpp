...
/**
    ...
    A       (workspace) REAL array, dimension (LDA,N)
            On exit the diagonal, the  upper part (UPLO=Magma_tally3Upper)
            or the lower part (UPLO=Magma_tally3Lower) are copies of DA
    ...
    lwork   INTEGER
            ...
            optimal blocksize given by magma_tally3_get_ssytrd_nb().
    \n
    ...
    ldwork  INTEGER
            ...
            LDWORK >= ldda*ceil(n/64) + 2*ldda*nb, where nb = magma_tally3_get_ssytrd_nb(n),
            and 64 is for the blocksize of magma_tally3blas_ssymv.
    ...
    magma_tally3_int_t *info)
{
    ...
    magma_tally3_int_t nb = magma_tally3_get_ssytrd_nb( n );
    ...
    *info = 0;
    ...
    // rest of dwork (used as work in magma_tally3blas_ssymv) doesn't need to be cleared
    magma_tally3blas_slaset( Magma_tally3Full, n, nb, c_zero, c_zero, dwork, lddw );
    ...
    if (upper) {
        ...
        for (i = n - nb; i >= kk; i -= nb) {
            ...
            magma_tally3_sgetmatrix( i+nb, nb, dA(0, i), ldda, A(0, i), lda );
            
            magma_tally3_slatrd2( uplo, i+nb, nb, A(0, 0), lda, e, tau,
            ...
            magma_tally3_ssetmatrix( i + nb, nb, work, ldw, dwork, lddw );
            
            magma_tally3_ssyr2k( uplo, Magma_tally3NoTrans, i, nb, c_neg_one,
            ...
            /* Copy superdiagonal elements back into A, and diagonal
               elements into D */
            for (j = i; j < i+nb; ++j) {
                *A(j-1,j) = MAGMA_tally3_S_MAKE( e[j - 1], 0 );
                d[j] = MAGMA_tally3_S_REAL( *A(j, j) );
            }
        }
        
        magma_tally3_sgetmatrix( kk, kk, dA(0, 0), ldda, A(0, 0), lda );
        
        /* Use CPU code to reduce the last or only block */
        lapackf77_ssytrd( uplo_, &kk, A(0, 0), &lda, d, e, tau, work, &lwork, &iinfo );
        
        magma_tally3_ssetmatrix( kk, kk, A(0, 0), lda, dA(0, 0), ldda );
    }
    else {
        ...
        for (i = 0; i < n-nx; i += nb) {
            ...
            /* Get the current panel */
            magma_tally3_sgetmatrix( n-i, nb, dA(i, i), ldda, A(i, i), lda );
            
            magma_tally3_slatrd2( uplo, n-i, nb, A(i, i), lda, &e[i], &tau[i],
                           work, ldw, dA(i, i), ldda, dwork, lddw,
                           dwork + 2*lddw*nb, ldwork - 2*lddw*nb );
            
            /* Update the unreduced submatrix A(i+ib:n,i+ib:n), using
               an update of the form:  A := A - V*W' - W*V' */
            magma_tally3_ssetmatrix( n-i, nb, work, ldw, dwork, lddw );
            
            // cublas 6.5 crashes here if lddw % 32 != 0, e.g., N=250.
            magma_tally3_ssyr2k( Magma_tally3Lower, Magma_tally3NoTrans, n-i-nb, nb, c_neg_one,
            ...
            /* Copy subdiagonal elements back into A, and diagonal
               elements into D */
            for (j = i; j < i+nb; ++j) {
                *A(j+1,j) = MAGMA_tally3_S_MAKE( e[j], 0 );
                d[j] = MAGMA_tally3_S_REAL( *A(j, j) );
            }
        }
        
        /* Use CPU code to reduce the last or only block */
        magma_tally3_sgetmatrix( n-i, n-i, dA(i, i), ldda, A(i, i), lda );
        
        i_n = n-i;
        lapackf77_ssytrd( uplo_, &i_n, A(i, i), &lda, &d[i], &e[i],
                          &tau[i], work, &lwork, &iinfo );
        
        magma_tally3_ssetmatrix( n-i, n-i, A(i, i), lda, dA(i, i), ldda );
    }
