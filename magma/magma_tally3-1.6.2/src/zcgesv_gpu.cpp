    ...
    magma_tally3_int_t *info)
{
    ...
    magma_tally3FloatComplex_ptr dSA, dSX;
    ...
    *info = 0;
     ...
     */
    //magma_tally3blas_zlag2c( n, nrhs, dB, lddb, dSX, lddsx, info );  // done inside zcgetrs with pivots
    if (*info != 0) {
        *iter = -2;
        goto FALLBACK;
    }
    
    magma_tally3blas_zlag2c( n, n, dA, ldda, dSA, lddsa, info );
    if (*info != 0) {
        *iter = -2;
        goto FALLBACK;
    }
    ...
    magma_tally3_cgetrf_gpu( n, n, dSA, lddsa, ipiv, info );
    if (*info != 0) {
        *iter = -3;
        goto FALLBACK;
    }
    ...
    {
        magma_tally3_int_t *newipiv;
        magma_tally3_imalloc_cpu( &newipiv, n );
        if ( newipiv == NULL ) {
            *iter = -3;
            goto FALLBACK;
        }
        swp2pswp_tally3( trans, n, ipiv, newipiv );
        magma_tally3_setvector( n, sizeof(magma_tally3_int_t), newipiv, 1, dipiv, 1 );
        magma_tally3_free_cpu( newipiv );
    }
    
    // solve dSA*dSX = dB in single precision
    // converts dB to dSX and applies pivots, solves, then converts result back to dX
    magma_tally3_zcgetrs_gpu( trans, n, nrhs, dSA, lddsa, dipiv, dB, lddb, dX, lddx, dSX, info );
...
REFINEMENT:
    for( iiter=1; iiter < ITERMAX; ) {
        *info = 0;
        ...
        magma_tally3_zcgetrs_gpu( trans, n, nrhs, dSA, lddsa, dipiv, dR, lddr, dR, lddr, dSX, info );
        if (*info != 0) {
            *iter = -3;
            goto FALLBACK;
        }
        ...
        for( j=0; j < nrhs; j++ ) {
            magma_tally3blas_zaxpycp( n, dR(0,j), dX(0,j), dB(0,j) );
        }
...
FALLBACK:
    ...
    magma_tally3_zgetrf_gpu( n, n, dA, ldda, ipiv, info );
    if (*info == 0) {
        magma_tally3blas_zlacpy( Magma_tally3UpperLower, n, nrhs, dB, lddb, dX, lddx );
        magma_tally3_zgetrs_gpu( trans, n, nrhs, dA, ldda, ipiv, dX, lddx, info );
    }
