...
static magma_tally2_int_t check_orthogonality(magma_tally2_int_t M, magma_tally2_int_t N, double *Q, magma_tally2_int_t LDQ, double eps);
static magma_tally2_int_t check_reduction(magma_tally2_uplo_t uplo, magma_tally2_int_t N, magma_tally2_int_t bw, double *A, double *D, magma_tally2_int_t LDA, double *Q, double eps );
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            n2     = N*N;
            #if defined(PRECISION_z) || defined(PRECISION_c)
            lwork  = magma_tally2_dbulge_get_lq2(N, threads) + 2*N + N*N;
            lrwork = 1 + 5*N +2*N*N;
            #else
            lwork  = magma_tally2_dbulge_get_lq2(N, threads) + 1 + 6*N + 2*N*N;
            #endif
            liwork = 3 + 5*N;
            ...
            magma_tally2_dmake_symmetric( N, h_A, N );
            ...
            if (opts.warmup) {
                ...
                // ==================================================================
                lapackf77_dlacpy( Magma_tally2UpperLowerStr, &N, &N, h_A, &N, h_R, &N );
                if (opts.ngpu == 1) {
                    //printf("calling dsyevdx_2stage 1 GPU\n");
                    magma_tally2_dsyevdx_2stage(opts.jobz, range, opts.uplo, N, 
                                    h_R, N, 
                                    vl, vu, il, iu, 
                                    &m1, w1, 
                                    h_work, lwork, 
                    ...
                    magma_tally2_dsyevdx_2stage_m(opts.ngpu, opts.jobz, range, opts.uplo, N, 
                                    h_R, N, 
                                    vl, vu, il, iu, 
                                    &m1, w1, 
                                    h_work, lwork, 
            ...
            if ( opts.check && opts.jobz != Magma_tally2NoVec ) {
                ...
                //printf("\n");
                //printf("------ TESTS FOR MAGMA_tally2 DSYEVD ROUTINE -------  \n");
...
static magma_tally2_int_t check_orthogonality(magma_tally2_int_t M, magma_tally2_int_t N, double *Q, magma_tally2_int_t LDQ, double eps)
{
    ...
    double *Id;
    magma_tally2_dmalloc_cpu( &Id, minMN*minMN );
...
static magma_tally2_int_t check_reduction(magma_tally2_uplo_t uplo, magma_tally2_int_t N, magma_tally2_int_t bw, double *A, double *D, magma_tally2_int_t LDA, double *Q, double eps )
{
    ...
    magma_tally2_dmalloc_cpu( &TEMP, N*N );
    magma_tally2_dmalloc_cpu( &Residual, N*N );
    ...
    Rnorm = lapackf77_dlansy("1", lapack_uplo_const_tally2(uplo), &N, Residual, &N, work);
    Anorm = lapackf77_dlansy("1", lapack_uplo_const_tally2(uplo), &N, A,        &LDA, work);
