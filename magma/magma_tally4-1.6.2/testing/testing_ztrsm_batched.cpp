...
int main( int argc, char** argv)
{
    ...
    double          magma_tally4_error, cublas_error, lapack_error, work[1];
    ...
    magma_tally4DoubleComplex *h_A, *h_B, *h_Bcublas, *h_Bmagma_tally4, *h_Blapack, *h_X;
    magma_tally4DoubleComplex *d_A, *d_B;
    magma_tally4DoubleComplex **d_A_array = NULL;
    magma_tally4DoubleComplex **d_B_array = NULL;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_Blapack, magma_tally4DoubleComplex, sizeB   );
            TESTING_MALLOC_CPU( h_Bcublas, magma_tally4DoubleComplex, sizeB   );
            TESTING_MALLOC_CPU( h_Bmagma_tally4,  magma_tally4DoubleComplex, sizeB   );
            TESTING_MALLOC_CPU( ipiv,      magma_tally4_int_t,        Ak      );
            
            TESTING_MALLOC_DEV( d_A,       magma_tally4DoubleComplex, ldda*Ak*batchCount );
            TESTING_MALLOC_DEV( d_B,       magma_tally4DoubleComplex, lddb*N*batchCount  );
            ...
            magma_tally4DoubleComplex* dwork=NULL; // invA and work are workspace in ztrsm
 
            magma_tally4_int_t dinvA_batchSize = magma_tally4_roundup( Ak, TRI_NB )*TRI_NB;
            magma_tally4_int_t dwork_batchSize = lddb*N;
            magma_tally4_zmalloc( &dinvA, dinvA_batchSize * batchCount);
            magma_tally4_zmalloc( &dwork, dwork_batchSize * batchCount );
    
            zset_pointer(dwork_array, dwork, lddb, 0, 0, dwork_batchSize, batchCount, opts.queue);
            zset_pointer(dinvA_array, dinvA, magma_tally4_roundup( Ak, TRI_NB ), 0, 0, dinvA_batchSize, batchCount, opts.queue);

            memset(h_Bmagma_tally4, 0, batchCount*ldb*N*sizeof(magma_tally4DoubleComplex));
            magma_tally4blas_zlaset( Magma_tally4Full, lddb, N*batchCount, c_zero, c_zero, dwork, lddb);
            ...
            memcpy( h_Blapack, h_B, sizeB*sizeof(magma_tally4DoubleComplex) );
            
            /* =====================================================================
               Performs operation using MAGMA_tally4BLAS
               =================================================================== */
            magma_tally4_zsetmatrix( Ak, Ak*batchCount, h_A, lda, d_A, ldda );
            magma_tally4_zsetmatrix( M,  N*batchCount, h_B, ldb, d_B, lddb );
            ...
            magma_tally4_time = magma_tally4_sync_wtime( NULL );
                ...
                magma_tally4_zgetmatrix( M, N*batchCount, dwork, lddb, h_Bmagma_tally4, ldb );
            #else
                ...
                magma_tally4_time = magma_tally4_sync_wtime( NULL ) - magma_tally4_time;
                magma_tally4_perf = gflops / magma_tally4_time;
                magma_tally4_zgetmatrix( M, N*batchCount, d_B, lddb, h_Bmagma_tally4, ldb );
            #endif
            ...
            // CUBLAS version <= 6.0 has magma_tally4DoubleComplex **            dA_array, no cast needed.
            // CUBLAS version    6.5 has magma_tally4DoubleComplex const**       dA_array, requiring cast.
            // Correctly, it should be   magma_tally4DoubleComplex const* const* dA_array, to avoid requiring cast.
                ...
                cublasZtrsmBatched(
                    ...
                    M, N, &alpha,
                    (const magma_tally4DoubleComplex**) d_A_array, ldda,
                    d_B_array, lddb, batchCount);
                cublas_time = magma_tally4_sync_wtime( NULL ) - cublas_time;
                cublas_perf = gflops / cublas_time;
            #endif

            magma_tally4_zgetmatrix( M, N*batchCount, d_B, lddb, h_Bcublas, ldb );
            ...
            /* =====================================================================
               ...
               =================================================================== */
            // ||b - 1/alpha*A*x|| / (||A||*||x||)
            magma_tally4DoubleComplex alpha2 = MAGMA_tally4_Z_DIV( c_one, alpha );
            double normR, normX, normA=0.;
            magma_tally4_error  = 0.0;
            cublas_error = 0.0;

            memcpy( h_X, h_Bmagma_tally4, sizeB*sizeof(magma_tally4DoubleComplex) );
            ...
            for (int s=0; s < batchCount; s++) {
                ...
                normX = lapackf77_zlange( "M", &M, &N, h_Bmagma_tally4 + s * ldb * N, &ldb, work );
                double magma_tally4_err = normR/(normX*normA);
            ...
            memcpy( h_X, h_Bcublas, sizeB*sizeof(magma_tally4DoubleComplex) );
            // check cublas
            #if CUDA_VERSION >= 6050
            for (int s=0; s < batchCount; s++) {
                normA = lapackf77_zlange( "M", &Ak, &Ak, h_A + s * lda * Ak, &lda, work );
            ...
            if ( opts.lapack ) {
                ...
                lapack_error = 0.0;
                memcpy( h_X, h_Blapack, sizeB*sizeof(magma_tally4DoubleComplex) );
                for (int s=0; s < batchCount; s++) {
                    blasf77_ztrmm(
                        lapack_side_const_tally4(opts.side), lapack_uplo_const_tally4(opts.uplo),
                        lapack_trans_const_tally4(opts.transA), lapack_diag_const_tally4(opts.diag),
