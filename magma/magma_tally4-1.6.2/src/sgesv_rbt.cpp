...
magma_tally4_sgerfs_nopiv_gpu(
    ...
    magma_tally4Float_ptr dworkd, magma_tally4Float_ptr dAF,
    ...
    magma_tally4_int_t *info)
{
    ...
    *info = 0;
    ...
    Anrm = magma_tally4blas_slange(Magma_tally4InfNorm, n, n, dA, ldda, (float*)dworkd );
    cte  = Anrm * eps * pow( (float)n, (float)0.5 ) * BWDMAX;
    
    // residual dR = dB - dA*dX in real
    magma_tally4blas_slacpy( Magma_tally4UpperLower, n, nrhs, dB, lddb, dR, lddr );
    if ( nrhs == 1 ) {
        magma_tally4_sgemv( trans, n, n,
                     c_neg_one, dA, ldda,
                                dX, 1,
                     c_one,     dR, 1 );
    }
    else {
        magma_tally4_sgemm( trans, Magma_tally4NoTrans, n, nrhs, n,
                     c_neg_one, dA, ldda,
                                dX, lddx,
                     c_one,     dR, lddr );
    }
    
    // TODO: use MAGMA_tally4_S_ABS( dX(i,j) ) instead of slange?
    for( j=0; j < nrhs; j++ ) {
        i = magma_tally4_isamax( n, dX(0,j), 1) - 1;
        magma_tally4_sgetmatrix( 1, 1, dX(i,j), 1, &Xnrmv, 1 );
        Xnrm = lapackf77_slange( "F", &ione, &ione, &Xnrmv, &ione, NULL );
        
        i = magma_tally4_isamax ( n, dR(0,j), 1 ) - 1;
        magma_tally4_sgetmatrix( 1, 1, dR(i,j), 1, &Rnrmv, 1 );
...
REFINEMENT:
    for( iiter=1; iiter < ITERMAX; ) {
        *info = 0;
        ...
        magma_tally4_sgetrs_nopiv_gpu( trans, n, nrhs, dAF, lddsa, dR, lddr, info );
        if (*info != 0) {
            *iter = -3;
            goto FALLBACK;
        }
        ...
        for( j=0; j < nrhs; j++ ) {
            magma_tally4blas_saxpycp2( n, dR(0,j), dX(0,j), dB(0,j) );
        }
    ...
    magma_tally4_int_t *info)
{
    ...
    *info = 0;
    ...
    if (MAGMA_tally4_SUCCESS != magma_tally4_smalloc( &dA, n2 )) {
        *info = MAGMA_tally4_ERR_DEVICE_ALLOC;
        return *info;
    }
    if (MAGMA_tally4_SUCCESS != magma_tally4_smalloc( &db, nn*nrhs )) {
        *info = MAGMA_tally4_ERR_DEVICE_ALLOC;
        return *info;
    }

    if (ref == Magma_tally4True) {
        if (MAGMA_tally4_SUCCESS != magma_tally4_smalloc( &dAo, n2 )) {
            *info = MAGMA_tally4_ERR_DEVICE_ALLOC;
            return *info;
        }
        if (MAGMA_tally4_SUCCESS != magma_tally4_smalloc( &dwork, nn*nrhs )) {
            *info = MAGMA_tally4_ERR_DEVICE_ALLOC;
            return *info;
        }
        if (MAGMA_tally4_SUCCESS != magma_tally4_smalloc( &dBo, nn*nrhs )) {
            *info = MAGMA_tally4_ERR_DEVICE_ALLOC;
            return *info;
        }
    }

    if (MAGMA_tally4_SUCCESS != magma_tally4_smalloc_cpu( &hu, 2*nn )) {
        *info = MAGMA_tally4_ERR_HOST_ALLOC;
        return *info;
    }
    if (MAGMA_tally4_SUCCESS != magma_tally4_smalloc_cpu( &hv, 2*nn )) {
        *info = MAGMA_tally4_ERR_HOST_ALLOC;
        return *info;
    }

    magma_tally4blas_slaset(Magma_tally4Full, nn, nn, MAGMA_tally4_S_ZERO, MAGMA_tally4_S_ONE, dA, nn);

    /* Send matrix on the GPU*/
    magma_tally4_ssetmatrix(n, n, A, lda, dA, nn);

    /* Send b on the GPU*/
    magma_tally4_ssetmatrix(n, nrhs, B, ldb, db, nn);

    *info = magma_tally4_sgerbt_gpu(Magma_tally4True, nn, nrhs, dA, nn, db, nn, hu, hv, info);
    if (*info != MAGMA_tally4_SUCCESS)  {
        return *info;
    }

    if (ref == Magma_tally4True) {
        magma_tally4_scopymatrix(nn, nn, dA, nn, dAo, nn);
        magma_tally4_scopymatrix(nn, nrhs, db, nn, dBo, nn);
    }
    /* Solve the system U^TAV.y = U^T.b on the GPU*/ 
    magma_tally4_sgesv_nopiv_gpu( nn, nrhs, dA, nn, db, nn, info);


    /* Iterative refinement */
    if (ref == Magma_tally4True) {
        magma_tally4_sgerfs_nopiv_gpu(Magma_tally4NoTrans, nn, nrhs, dAo, nn, dBo, nn, db, nn, dwork, dA, &iter, info);
    }
    //printf("iter = %d\n", iter);
    ...
    if (MAGMA_tally4_SUCCESS != magma_tally4_smalloc( &dv, 2*nn )) {
        *info = MAGMA_tally4_ERR_DEVICE_ALLOC;
        return *info;
    }

    magma_tally4_ssetvector(2*nn, hv, 1, dv, 1);
    
    for(int i = 0; i < nrhs; i++) {
        magma_tally4blas_sprbt_mv(nn, dv, db+(i*nn));
    }

    magma_tally4_sgetmatrix(n, nrhs, db, nn, B, ldb);
