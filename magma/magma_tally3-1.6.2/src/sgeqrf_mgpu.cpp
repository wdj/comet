    ...
    magma_tally3_int_t *info )
{
    ...
    float *dwork[Magma_tally3MaxGPUs]={NULL}, *dpanel[Magma_tally3MaxGPUs]={NULL};
    ...
    *info = 0;
    ...
    for( dev=0; dev < ngpu; dev++ ) {
        ...
        if ( MAGMA_tally3_SUCCESS != magma_tally3_smalloc( &(dwork[dev]), (lddwork + ldda)*nb )) {
    ...
    if ( MAGMA_tally3_SUCCESS != magma_tally3_smalloc_pinned( &hwork, lwork )) {
    ...
    if ( nb < min_mn ) {
        ...
        for( i = 0; i < min_mn-nb; i += nb ) {
            ...
            spanel_to_q_tally3( Magma_tally3Upper, ib, hpanel(i), ldhpanel, hwork + ib*ib );
            ...
            // TODO: if spanel_to_q_tally3 copied whole block, wouldn't need to restore
            // -- just send the copy to the GPUs.
            // TODO: also, could zero out the lower triangle and use Azzam's larfb w/ gemm.
            
            /* Restore the panel */
            sq_to_panel_tally3( Magma_tally3Upper, ib, hpanel(i), ldhpanel, hwork + ib*ib );
    ...
    if (i < min_mn) {
        ...
        for( j=i; j < n; j += nb ) {
            ...
            ib = min( n-j, nb );
            magma_tally3_setdevice( panel_dev );
            magma_tally3_sgetmatrix( rows, ib,
                              dlA(panel_dev, i, i_local), ldda,
                              hwork + (j-i)*rows,         rows );
        }
