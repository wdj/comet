...
{
    magma_minproduct_int_t info = 0;
    magma_minproductDoubleComplex *initial_guess=NULL;
    solver_par->eigenvectors = NULL;
    solver_par->eigenvalues = NULL;
    if ( solver_par->solver == Magma_minproduct_LOBPCG ) {
        CHECK( magma_minproduct_dmalloc_cpu( &solver_par->eigenvalues ,
        ...
        CHECK( magma_minproduct_zmalloc_cpu( &initial_guess, ev ));
        CHECK( magma_minproduct_zmalloc( &solver_par->eigenvectors, ev ));
        magma_minproduct_int_t ISEED[4] = {0,0,0,1}, ione = 1;
        lapackf77_zlarnv( &ione, ISEED, &ev, initial_guess );

        magma_minproduct_zsetmatrix( solver_par->ev_length, solver_par->num_eigenvalues,
