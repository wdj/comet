...
/**
    ...
    @ingroup magma_tally3_dgesvd_driver
    ********************************************************************/
        ...
        if (m >= n && minmn > 0) {
            ...
            if (m >= mnthr) {
                if (wantqn) {
                    ...
                    wrkbl  =              n +   n * nb;   //magma_tally3_ilaenv( 1, "DGEQRF", " ",   m, n, -1, -1 )
                    ...
                    wrkbl  =              n +   n * nb;   //magma_tally3_ilaenv( 1, "DGEQRF", " ",   m, n, -1, -1 )
                    wrkbl  = max(wrkbl,   n +   n * nb ); //magma_tally3_ilaenv( 1, "DORGQR", " ",   m, n,  n, -1 ))
                    wrkbl  = max(wrkbl, 3*n + 2*n * nb ); // dgebrd
                    wrkbl  = max(wrkbl, 3*n +   n * nb ); //magma_tally3_ilaenv( 1, "DORMBR", "QLN", n, n,  n, -1 ))
                    wrkbl  = max(wrkbl, 3*n +   n * nb ); //magma_tally3_ilaenv( 1, "DORMBR", "PRT", n, n,  n, -1 ))
                    ...
                    wrkbl  = max(wrkbl,   n +   m * nb ); //magma_tally3_ilaenv( 1, "DORGQR", " ",   m, m,  n, -1 ))
                    maxwrk = wrkbl + n*n;
                }
            }
            else {
                ...
                if (wantqn) {
                    ...
                    wrkbl  = max(wrkbl, 3*n + n * nb ); //magma_tally3_ilaenv( 1, "DORMBR", "QLN", m, n, n, -1 ))
                    wrkbl  = max(wrkbl, 3*n + n * nb ); //magma_tally3_ilaenv( 1, "DORMBR", "PRT", n, n, n, -1 ))
                    ...
                    wrkbl  = max(wrkbl, 3*n + m * nb ); //magma_tally3_ilaenv( 1, "DORMBR", "QLN", m, m, n, -1 ))
                    wrkbl  = max(wrkbl, 3*n + n * nb ); //magma_tally3_ilaenv( 1, "DORMBR", "PRT", n, n, n, -1 ))
                    maxwrk = max(wrkbl, 3*n + bdspac);
                    minwrk = maxwrk;  // lapack was: 3*n + max(m, bdspac)
                }
            ...
            if (n >= mnthr) {
                if (wantqn) {
                    ...
                    wrkbl  =              m +   m * nb;  //magma_tally3_ilaenv( 1, "DGELQF", " ",   m, n, -1, -1 )
                    ...
                    wrkbl  =              m +   m * nb;   //magma_tally3_ilaenv( 1, "DGELQF", " ",   m, n, -1, -1 )
                    wrkbl  = max(wrkbl,   m +   m * nb ); //magma_tally3_ilaenv( 1, "DORGLQ", " ",   m, n,  m, -1 ))
                    wrkbl  = max(wrkbl, 3*m + 2*m * nb ); // dgebrd
                    wrkbl  = max(wrkbl, 3*m +   m * nb ); //magma_tally3_ilaenv( 1, "DORMBR", "QLN", m, m,  m, -1 ))
                    wrkbl  = max(wrkbl, 3*m +   m * nb ); //magma_tally3_ilaenv( 1, "DORMBR", "PRT", m, m,  m, -1 ))
                    ...
                    // include preferred size for dorgqr
                    wrkbl  = max(wrkbl,   m +   n * nb ); //magma_tally3_ilaenv( 1, "DORGLQ", " ",   n, n,  m, -1 ))
                    maxwrk = wrkbl + m*m;
                }
            }
            else {
                ...
                if (wantqn) {
                    ...
                    wrkbl  = max(wrkbl, 3*m + m * nb ); //magma_tally3_ilaenv( 1, "DORMBR", "QLN", m, m, n, -1 ))
                    wrkbl  = max(wrkbl, 3*m + m * nb ); //magma_tally3_ilaenv( 1, "DORMBR", "PRT", m, n, m, -1 ))
                    ...
                    wrkbl  = max(wrkbl, 3*m + n * nb ); //magma_tally3_ilaenv( 1, "DORMBR", "QLN", m, m, n, -1 ))
                    wrkbl  = max(wrkbl, 3*m + m * nb ); //magma_tally3_ilaenv( 1, "DORMBR", "PRT", n, n, m, -1 ))
                    maxwrk = max(wrkbl, 3*m + bdspac);
                    minwrk = maxwrk;  // lapack was: 3*m + max(n, bdspac)
                }
            }
    ...
    if (m >= n) {
        ...
        if (m >= mnthr) {
            if (wantqn) {
                ...
                #else
                magma_tally3_dgebrd(n, n, A(1,1), lda, s, &work[ie], &work[itauq], &work[itaup], &work[nwork], lnwork, &ierr);
                #endif
                nwork = ie + n;
                ...
                #else
                magma_tally3_dgebrd(n, n, &work[ir], ldwrkr, s, &work[ie], &work[itauq], &work[itaup], &work[nwork], lnwork, &ierr);
                #endif
                ...
                #else
                magma_tally3_dormbr(Magma_tally3Q, Magma_tally3Left,  Magma_tally3NoTrans, n, n, n, &work[ir], ldwrkr, &work[itauq], &work[iu], n,    &work[nwork], lnwork, &ierr);
                magma_tally3_dormbr(Magma_tally3P, Magma_tally3Right, Magma_tally3Trans,   n, n, n, &work[ir], ldwrkr, &work[itaup], VT,        ldvt, &work[nwork], lnwork, &ierr);
                #endif
                ...
                lapackf77_dormbr("P", "R", "T", &n, &n, &n, &work[ir], &ldwrkr, &work[itaup], VT, &ldvt, &work[nwork], &lnwork, &ierr);
                #else
                magma_tally3_dormbr(Magma_tally3Q, Magma_tally3Left,  Magma_tally3NoTrans, n, n, n, &work[ir], ldwrkr, &work[itauq], U,  ldu,  &work[nwork], lnwork, &ierr);
                magma_tally3_dormbr(Magma_tally3P, Magma_tally3Right, Magma_tally3Trans,   n, n, n, &work[ir], ldwrkr, &work[itaup], VT, ldvt, &work[nwork], lnwork, &ierr);
                #endif
                ...
                #else
                magma_tally3_dormbr(Magma_tally3Q, Magma_tally3Left,  Magma_tally3NoTrans, n, n, n, A(1,1), lda, &work[itauq], &work[iu], ldwrku, &work[nwork], lnwork, &ierr);
                magma_tally3_dormbr(Magma_tally3P, Magma_tally3Right, Magma_tally3Trans,   n, n, n, A(1,1), lda, &work[itaup], VT,        ldvt,   &work[nwork], lnwork, &ierr);
                #endif
            ...
            #else
            magma_tally3_dgebrd(m, n, A(1,1), lda, s, &work[ie], &work[itauq], &work[itaup], &work[nwork], lnwork, &ierr);
            #endif
            ...
            if (wantqn) {
                ...
                if (lwork >= m*n + 3*n + bdspac) {
                    ...
                    #else
                    magma_tally3_dormbr(Magma_tally3Q, Magma_tally3Left, Magma_tally3NoTrans, m, n, n, A(1,1), lda, &work[itauq], &work[iu], ldwrku, &work[nwork], lnwork, &ierr);
                    #endif
                ...
                lapackf77_dormbr("P", "R", "T", &n, &n, &n, A(1,1), &lda, &work[itaup], VT, &ldvt, &work[nwork], &lnwork, &ierr);
                #else
                magma_tally3_dormbr(Magma_tally3Q, Magma_tally3Left,  Magma_tally3NoTrans, m, n, n, A(1,1), lda, &work[itauq], U,  ldu,  &work[nwork], lnwork, &ierr);
                magma_tally3_dormbr(Magma_tally3P, Magma_tally3Right, Magma_tally3Trans,   n, n, n, A(1,1), lda, &work[itaup], VT, ldvt, &work[nwork], lnwork, &ierr);
                #endif
            }
            else if (wantqa) {
                ...
                #else
                magma_tally3_dormbr(Magma_tally3Q, Magma_tally3Left,  Magma_tally3NoTrans, m, m, n, A(1,1), lda, &work[itauq], U,  ldu,  &work[nwork], lnwork, &ierr);
                magma_tally3_dormbr(Magma_tally3P, Magma_tally3Right, Magma_tally3Trans,   n, n, m, A(1,1), lda, &work[itaup], VT, ldvt, &work[nwork], lnwork, &ierr);
                #endif
            }
        }
    }
        ...
        if (n >= mnthr) {
            if (wantqn) {
                ...
                #else
                magma_tally3_dgebrd(m, m, A(1,1), lda, s, &work[ie], &work[itauq], &work[itaup], &work[nwork], lnwork, &ierr);
                #endif
                nwork = ie + m;
                ...
                #else
                magma_tally3_dgebrd(m, m, &work[il], ldwrkl, s, &work[ie], &work[itauq], &work[itaup], &work[nwork], lnwork, &ierr);
                #endif
                ...
                #else
                magma_tally3_dormbr(Magma_tally3Q, Magma_tally3Left,  Magma_tally3NoTrans, m, m, m, &work[il], ldwrkl, &work[itauq], U,          ldu, &work[nwork], lnwork, &ierr);
                magma_tally3_dormbr(Magma_tally3P, Magma_tally3Right, Magma_tally3Trans,   m, m, m, &work[il], ldwrkl, &work[itaup], &work[ivt], m,   &work[nwork], lnwork, &ierr);
                #endif
                ...
                #else
                magma_tally3_dormbr(Magma_tally3Q, Magma_tally3Left,  Magma_tally3NoTrans, m, m, m, &work[il], ldwrkl, &work[itauq], U,  ldu,  &work[nwork], lnwork, &ierr);
                magma_tally3_dormbr(Magma_tally3P, Magma_tally3Right, Magma_tally3Trans,   m, m, m, &work[il], ldwrkl, &work[itaup], VT, ldvt, &work[nwork], lnwork, &ierr);
                #endif
                ...
                #else
                magma_tally3_dormbr(Magma_tally3Q, Magma_tally3Left,  Magma_tally3NoTrans, m, m, m, A(1,1), lda, &work[itauq], U,          ldu,    &work[nwork], lnwork, &ierr);
                magma_tally3_dormbr(Magma_tally3P, Magma_tally3Right, Magma_tally3Trans,   m, m, m, A(1,1), lda, &work[itaup], &work[ivt], ldwkvt, &work[nwork], lnwork, &ierr);
                #endif
            ...
            if (wantqn) {
                ...
                if (lwork >= m*n + 3*m + bdspac) {
                    ...
                    #else
                    magma_tally3_dormbr(Magma_tally3P, Magma_tally3Right, Magma_tally3Trans, m, n, m, A(1,1), lda, &work[itaup], &work[ivt], ldwkvt, &work[nwork], lnwork, &ierr);
                    #endif
                ...
                #else
                magma_tally3_dormbr(Magma_tally3Q, Magma_tally3Left,  Magma_tally3NoTrans, m, m, n, A(1,1), lda, &work[itauq], U,  ldu,  &work[nwork], lnwork, &ierr);
                magma_tally3_dormbr(Magma_tally3P, Magma_tally3Right, Magma_tally3Trans,   m, n, m, A(1,1), lda, &work[itaup], VT, ldvt, &work[nwork], lnwork, &ierr);
                #endif
            }
            else if (wantqa) {
                /* Path 5ta (N > M, JOBZ='A') */
