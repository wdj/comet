...
__global__ void
magma_tally3blas_scnrm2_kernel( int m, magma_tally3FloatComplex *dA, int ldda, float *dxnorm )
{
...
//==============================================================================
__global__ void
magma_tally3blas_scnrm2_check_kernel( int m, magma_tally3FloatComplex *dA, int ldda, float *dxnorm, 
                               float *lsticc )
{
    const int tx = threadIdx.x;
    magma_tally3FloatComplex *dx = dA + blockIdx.x * ldda;
    ...
    magma_tally3Float_ptr dlsticc) 
{
    ...
    magma_tally3blas_scnrm2_check_kernel<<< blocks, threads >>>( m, dA, ldda, dxnorm, dlsticc );
}


//==============================================================================
__global__ void
magma_tally3blas_scnrm2_smkernel( int m, int n, magma_tally3FloatComplex *dA, int ldda,
                           float *dxnorm )
{
    const int tx = threadIdx.x;
    ...
    for( int k = ty; k < n; k += BLOCK_SIZEy ) {
        magma_tally3FloatComplex *dx = dA + k * ldda;
        ...
        sum[tx][ty] = lsum;
        magma_tally3_sum_reduce_2d< BLOCK_SIZEx, BLOCK_SIZEy+1 >( tx, ty, sum );
    ...
    float *dxnorm)
{
    ...
    magma_tally3blas_scnrm2_smkernel<<< blocks, threads, 0, magma_tally3_stream >>>( m, n, dA, ldda, dxnorm );
}
...
magma_tally3_scnrm2_adjust_kernel(float *xnorm, magma_tally3FloatComplex *c)
{
    ...
    temp = MAGMA_tally3_C_ABS( c[tx] ) / xnorm[0];
    sum[tx] = -temp * temp;
...
*/
extern "C" void
magma_tally3blas_scnrm2_adjust(magma_tally3_int_t k, magma_tally3Float_ptr dxnorm, magma_tally3FloatComplex_ptr dc)
{
    magma_tally3_scnrm2_adjust_kernel<<< 1, k, 0, magma_tally3_stream >>> (dxnorm, dc);
}
...
__global__ void
magma_tally3_scnrm2_row_check_adjust_kernel(
    int n, float tol, float *xnorm, float *xnorm2, 
    magma_tally3FloatComplex *C, int ldc, float *lsticc)
{
    const int tx = threadIdx.x + blockIdx.x*BS;
    lsticc[tx+1] = 0;

    if (tx < n) {
        float temp = MAGMA_tally3_C_ABS( C[tx*ldc] ) / xnorm[tx];
    ...
    if (tx == 0)
        lsticc[0] = 0;
    magma_tally3_sum_reduce_n( blockDim.x, tx, lsticc );
}
    ...
    magma_tally3Float_ptr dlsticc)
{
    ...
    magma_tally3_scnrm2_row_check_adjust_kernel<<< nblocks, BS >>> (k, tol, dxnorm, dxnorm2, dC, lddc, dlsticc);
}
    ...
    magma_tally3Float_ptr dxnorm) 
{
    ...
    magma_tally3blas_scnrm2_kernel<<< blocks, threads, 0, magma_tally3_stream >>>( m, dA, ldda, dxnorm );

    // The following would do the computation on one SM
    // magma_tally3blas_scnrm2_sm(m, n, dA, ldda, dxnorm);
}
