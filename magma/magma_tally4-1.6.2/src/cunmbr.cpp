...
/**
    Purpose
    -------
    If VECT = Magma_tally4Q, CUNMBR overwrites the general complex M-by-N matrix C with
                                 SIDE = Magma_tally4Left     SIDE = Magma_tally4Right
    TRANS = Magma_tally4NoTrans:        Q*C                  C*Q
    TRANS = Magma_tally4_ConjTrans:     Q**H*C               C*Q**H
    
    If VECT = Magma_tally4P, CUNMBR overwrites the general complex M-by-N matrix C with
                                 SIDE = Magma_tally4Left     SIDE = Magma_tally4Right
    ...
    *info = 0;
    ...
    if (*info == 0) {
        if (nw > 0) {
            ...
            nb = magma_tally4_get_cgebrd_nb( min( m, n ));
            lwkopt = max(1, nw*nb);
        }
        else {
            lwkopt = 1;
