    ...
    if ( ind < m ) {
        ...
        if ( full ) {
            ...
            for( int j=0; j < BLK_Y; ++j ) {
                A[j*lda] = MAGMA_tally3_D_MAKE( MAGMA_tally3_S_REAL( SA[j*ldsa] ), MAGMA_tally3_S_IMAG( SA[j*ldsa] ));
            }
        }
        else {
...
magma_tally3blas_slag2d(
    ...
    magma_tally3_int_t *info)
{
    magma_tally3blas_slag2d_q( m, n, SA, ldsa, A, lda, info, magma_tally3_stream );
}
