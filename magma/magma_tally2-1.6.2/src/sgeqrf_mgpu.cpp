/*
    -- MAGMA_tally2 (version 1.6.1) --
       ...
       @generated from zgeqrf_mgpu.cpp normal z -> s, Fri Jan 30 19:00:16 2015
    ...
    magma_tally2_int_t *info )
{
    ...
    float *dwork[Magma_tally2MaxGPUs]={NULL}, *dpanel[Magma_tally2MaxGPUs]={NULL};
    float *hwork=NULL, *hpanel=NULL;
    magma_tally2_queue_t stream[Magma_tally2MaxGPUs][2]={{NULL}};
    magma_tally2_event_t panel_event[Magma_tally2MaxGPUs]={NULL};

    magma_tally2_int_t i, j, min_mn, dev, ldhpanel, lddwork, rows;
    magma_tally2_int_t ib, nb;
    magma_tally2_int_t lhwork, lwork;
    magma_tally2_int_t panel_dev, i_local, i_nb_local, n_local[Magma_tally2MaxGPUs], la_dev, dpanel_offset;
    ...
    *info = 0;
    ...
    nb = magma_tally2_get_sgeqrf_nb( m );
    ...
    for( dev=0; dev < ngpu; dev++ ) {
        ...
        if ( MAGMA_tally2_SUCCESS != magma_tally2_smalloc( &(dwork[dev]), (lddwork + ldda)*nb )) {
    ...
    if ( MAGMA_tally2_SUCCESS != magma_tally2_smalloc_pinned( &hwork, lwork )) {
    ...
    for( dev=0; dev < ngpu; dev++ ) {
        magma_tally2_setdevice( dev );
        magma_tally2_queue_create( &stream[dev][0] );
        magma_tally2_queue_create( &stream[dev][1] );
        magma_tally2_event_create( &panel_event[dev] );
    }
    ...
    if ( nb < min_mn ) {
        ...
        for( i = 0; i < min_mn-nb; i += nb ) {
            ...
            magma_tally2_setdevice( panel_dev );
            magma_tally2_queue_wait_event( stream[panel_dev][1], panel_event[panel_dev] );
            magma_tally2_sgetmatrix_async( rows, ib,
                                    dlA(panel_dev, i, i_local), ldda,
                                    hpanel(i),                  ldhpanel, stream[panel_dev][1] );
            magma_tally2_queue_sync( stream[panel_dev][1] );
            ...
            spanel_to_q_tally2( Magma_tally2Upper, ib, hpanel(i), ldhpanel, hwork + ib*ib );
            ...
            for( dev=0; dev < ngpu; dev++ ) {
                ...
                magma_tally2_ssetmatrix_async( rows, ib,
                                        hpanel(i),   ldhpanel,
                                        dpanel[dev], ldda, stream[dev][0] );
            }
            for( dev=0; dev < ngpu; dev++ ) {
                magma_tally2_setdevice( dev );
                magma_tally2_queue_sync( stream[dev][0] );
            }

            // TODO: if spanel_to_q_tally2 copied whole block, wouldn't need to restore
            // -- just send the copy to the GPUs.
            // TODO: also, could zero out the lower triangle and use Azzam's larfb w/ gemm.
            
            /* Restore the panel */
            sq_to_panel_tally2( Magma_tally2Upper, ib, hpanel(i), ldhpanel, hwork + ib*ib );
            ...
            if (i + ib < n) {
                ...
                for( dev=0; dev < ngpu; dev++ ) {
                    magma_tally2_setdevice( dev );
                    magma_tally2blasSetKernelStream( stream[dev][0] );
                    if (dev == la_dev && i+nb < min_mn-nb) {
                        // If not last panel,
                        // for look-ahead panel, apply H' to A(i:m,i+ib:i+2*ib)
                        i_nb_local = (i+nb)/(nb*ngpu)*nb;
                        magma_tally2_slarfb_gpu( Magma_tally2Left, Magma_tally2ConjTrans, Magma_tally2Forward, Magma_tally2Columnwise,
                                          rows, ib, ib,
                                          dpanel[dev],             ldda,       // V
                                          dwork[dev],              lddwork,    // T
                                          dlA(dev, i, i_nb_local), ldda,       // C
                                          dwork[dev]+ib,           lddwork );  // work
                        magma_tally2_event_record( panel_event[dev], stream[dev][0] );
    ...
    if (i < min_mn) {
        ...
        for( j=i; j < n; j += nb ) {
            ...
            ib = min( n-j, nb );
            magma_tally2_setdevice( panel_dev );
            magma_tally2_sgetmatrix( rows, ib,
                              dlA(panel_dev, i, i_local), ldda,
                              hwork + (j-i)*rows,         rows );
        }
        ...
        for( j=i; j < n; j += nb ) {
            ...
            ib = min( n-j, nb );
            magma_tally2_setdevice( panel_dev );
            magma_tally2_ssetmatrix( rows, ib,
                              hwork + (j-i)*rows,         rows,
                              dlA(panel_dev, i, i_local), ldda );
        }
    }
...
CLEANUP:
    ...
    for( dev=0; dev < ngpu; dev++ ) {
        magma_tally2_setdevice( dev );
        magma_tally2_queue_destroy( stream[dev][0]   );
        magma_tally2_queue_destroy( stream[dev][1]   );
        magma_tally2_event_destroy( panel_event[dev] );
