/*
    -- MAGMA_minproduct (version 1.6.2) --
       ...
       @generated from magma_minproduct_zmcsrcompressor_gpu.cu normal z -> d, Sun May  3 11:22:58 2015
       @author Hartwig Anzt
...
__global__ void
magma_minproduct_dmcsrgpu_kernel1( int num_rows,
                 double *A_val,
    ...
    if(row<num_rows){
        double zero = MAGMA_minproduct_D_ZERO;
...
__global__ void
magma_minproduct_dmcsrgpu_kernel2( int num_rows,
                 magma_minproduct_index_t *B_rowptr,
                 magma_minproduct_index_t *A_rowptr ){
...
__global__ void
magma_minproduct_dmcsrgpu_kernel3( int num_rows,
                 double *B_val,
                 magma_minproduct_index_t *B_rowptr,
                 magma_minproduct_index_t *B_colind,
    ...
    magma_minproduct_queue_t queue )
{
    ...
    magma_minproduct_d_matrix B={Magma_minproduct_CSR}, B2={Magma_minproduct_CSR};
    magma_minproduct_d_matrix dA={Magma_minproduct_CSR}, CSRA={Magma_minproduct_CSR};
    ...
    if ( A->memory_location == Magma_minproduct_DEV && A->storage_type == Magma_minproduct_CSR ) {
        ...
        magma_minproduct_dmcsrgpu_kernel1<<< grid1, BLOCK_SIZE1, 0, queue >>>
                ( A->num_rows, A->dval, A->drow, A->dcol, B.dval, B.drow, B.dcol );

        // correct the row pointer
        dim3 grid2( 1, 1, 1);
        magma_minproduct_dmcsrgpu_kernel2<<< grid2, BLOCK_SIZE2, 0, queue >>>
        ...
        magma_minproduct_dmcsrgpu_kernel3<<< grid1, BLOCK_SIZE1, 0, queue >>>
        ...
        CHECK( magma_minproduct_dmconvert( *A, &CSRA, A->storage_type, Magma_minproduct_CSR, queue ));
        CHECK( magma_minproduct_dmtransfer( *A, &dA, A->memory_location, Magma_minproduct_DEV, queue ));

        CHECK( magma_minproduct_dmcsrcompressor_gpu( &dA, queue ));

        magma_minproduct_dmfree( &dA, queue );
        magma_minproduct_dmfree( A, queue );
        CHECK( magma_minproduct_dmtransfer( dA, &CSRA, Magma_minproduct_DEV, A_location, queue ));
        CHECK( magma_minproduct_dmconvert( CSRA, A, Magma_minproduct_CSR, A_storage, queue ));
        magma_minproduct_dmfree( &dA, queue );
