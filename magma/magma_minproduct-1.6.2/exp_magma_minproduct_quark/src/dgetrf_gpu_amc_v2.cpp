...
/*
*
*  ASYNC_DGETRF_REC_GPU:
*
*  This function allocate the workspace and call ASYNC_DGETRF_REC_ASYNC_WORK_GPU
*  REC: recursif Algorithm
*  ASYNC: asynchronous algorithm
*/

extern "C" magma_minproduct_int_t magma_minproduct_dgetrf_gpu_amc_v2(magma_minproduct_int_t m, magma_minproduct_int_t n,  
...
//    magma_minproduct_event_t *A_event; /*Controling bucket*/
    ...
    *info = 0; 
    ...
    magma_minproduct_dgetrf_gpu_work_amc_v2(m, n, dA, dA_LD, ipiv, info, AWORK, AWORK_LD, AWORK_n);
...
magma_minproduct_dgetrf_gpu_work_amc_v2(
...
/*  -- MAGMA_minproduct (version 1.6.1) -- 
    ...
    //double *work; 
     ...
     double *dAT; 
     magma_minproduct_int_t dAT_LD; 
      
      
     double *dAP_set,*dAP_get; 
     magma_minproduct_int_t dAP_LD; 
    ...
    // magma_minproduct_int_t AWORK_m, AWORK_n, AWORK_N;
     ...
     int A_m, A_n, A_N, A_NMAX, A_LD, A_NBT;
     double *A;
    ...
    *info = 0; 
    ...
    ca_dbg_printMat(m, A_n, A, A_LD,"A after first getMatrix"); 
     ...
     if (MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc(&dAP_set, dAP_LD*nb)) { 
            *info = MAGMA_minproduct_ERR_DEVICE_ALLOC; 
            return *info; 
    } 

     if (MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc(&dAP_get, dAP_LD*nb)) { 
            magma_minproduct_free(dAP_set);
            *info = MAGMA_minproduct_ERR_DEVICE_ALLOC; 
            return *info; 
    }
    ...
    if ((m == n) ){  //&& (m % 32 == 0) && (dA_LD%32 == 0)
         dAT = dA;
         dAT_LD= dA_LD;
       magma_minproductblas_dtranspose_inplace(m, dAT, dAT_LD); 
    } 
    else { 
    ...
    /*Make sure m and n are multiple of 32*/
     ...
     if (MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc(&dAT, dm_max*dn_max )) { 
        magma_minproduct_free(dAP_set); 
        magma_minproduct_free(dAP_get);
        *info = MAGMA_minproduct_ERR_DEVICE_ALLOC; 
        return *info; 
     }

     dAT_LD = dn_max; 
     magma_minproductblas_dtranspose2( dAT, dAT_LD, dA, dA_LD, m, n );  
   }
    ...
    ca_dbg_printMat_transpose_gpu(m, n, dAT, dAT_LD,"matrix dAT to factorize"); 
     ...
     magma_minproduct_schedule_init(nbcores, 1); 
     ...
     magma_minproduct_insert_core_dgetrf_rec(A_m, nb, A(K,K), A_LD, ipiv(0), &iinfo, panel_num_threads, colptr(K));  
     ...
     magma_minproduct_insert_dsetmatrix_transpose(A_m, nb, A(0,K), A_LD, dAT(0,K), dAT_LD, dAP_set, dAP_LD, colptr(K), dAT(K,K)); 
         ...
         A_N = NSplit(N-K, dcpu);
          ...
          { 
             ...
             //// magma_minproduct_schedule_set_task_priority(INT_MAX - N*K);
              ...
              magma_minproduct_insert_dlaswp(gpu_ncols, dAT(K, K+A_N), dAT_LD, ONE, nb, ipiv(K), ONE, dAT(K, K+A_N-1)); /*non blocking*/                  
              //printf("debug barrier\n");
              //magma_minproduct_schedule_barrier();
              magma_minproduct_insert_dtrsm(Magma_minproductRight,  Magma_minproductUpper, Magma_minproductNoTrans, Magma_minproductUnit, gpu_ncols, nb, c_one, dAT(K,K), dAT_LD, dAT(K,K+A_N), dAT_LD);/*non blocking*/ 
 
              /* aij^T = aij^T - (lik.ukj)^T = aij^T - ukj^T.lik^T*/ 
              magma_minproduct_insert_dgemm(Magma_minproductNoTrans,Magma_minproductNoTrans, gpu_ncols, gpu_nrows, nb, c_neg_one, dAT(K,K+A_N), dAT_LD, dAT(K+1,K), dAT_LD, c_one, dAT(K+1,K+A_N), dAT_LD);/*non blocking*/    
              ...
              //printf("B_rows:%d\n",B_rows);
               ...
               { 
                   ...
                   // magma_minproduct_insert_core_dgetrf(nrows, ncols, A(K+1,K+1), A_LD, ipiv(K+1), &iinfo, colptr(K+1)); 
                    ...
                    magma_minproduct_insert_dsetmatrix_transpose(nrows, ncols, A(U_K, K+1), A_LD, dAT(U_K, K+1), dAT_LD, dAP_set, dAP_LD, colptr(K+1), dAT(K+1,K+1));
          ...
          for(L=K+A_N;L<K+A_N+A_NBT;L++)
          {
               if(L<N) { 
                  ...
                  magma_minproduct_insert_dgetmatrix_transpose(gpu_nrows, ncols, dAT(K+1,L), dAT_LD, A(K+1,L), A_LD, dAP_get, dAP_LD, colptr(L)); //blocking
 ...
 magma_minproduct_schedule_barrier(); 
    ...
    ca_trace_start();
       ...
       // magma_minproduct_insert_dlaswp(ncols, dAT(K, 0), dAT_LD, ONE, nrows, ipiv(K), ONE, dAT(K-1,0)); 
        magma_minproductblas_dlaswp(ncols, dAT(K, 0), dAT_LD, ONE, nrows, ipiv(K), ONE);
