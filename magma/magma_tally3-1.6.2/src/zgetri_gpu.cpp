    ...
    magma_tally3_int_t *info )
{
    ...
    magma_tally3DoubleComplex_ptr dL = dwork;
    magma_tally3_int_t lddl = n;
    magma_tally3_int_t nb   = magma_tally3_get_zgetri_nb(n);
    ...
    *info = 0;
    ...
    magma_tally3_ztrtri_gpu( Magma_tally3Upper, Magma_tally3NonUnit, n, dA, ldda, info );
    if ( *info != 0 )
        return *info;
    ...
    for( j = jmax; j >= 0; j -= nb ) {
        ...
        magma_tally3blas_zlacpy( Magma_tally3Full, n-j, jb,
                          dA(j,j), ldda,
                          dL(j,0), lddl );
        magma_tally3blas_zlaset( Magma_tally3Lower, n-j-1, jb, c_zero, c_zero, dA(j+1,j), ldda );
        ...
        if ( j+jb < n ) {
            magma_tally3_zgemm( Magma_tally3NoTrans, Magma_tally3NoTrans, n, jb, n-j-jb,
                         c_neg_one, dA(0,j+jb), ldda,
                                    dL(j+jb,0), lddl,
                         c_one,     dA(0,j),    ldda );
        }
        // TODO use magma_tally3blas work interface
        magma_tally3_ztrsm( Magma_tally3Right, Magma_tally3Lower, Magma_tally3NoTrans, Magma_tally3Unit,
                     n, jb, c_one,
                     dL(j,0), lddl,
                     dA(0,j), ldda );
    }
    ...
    for( j = n-2; j >= 0; --j ) {
        jp = ipiv[j] - 1;
        if ( jp != j ) {
            magma_tally3blas_zswap( n, dA(0,j), 1, dA(0,jp), 1 );
        }
    }
