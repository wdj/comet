...
#define Q(i_,j_) (Q + (i_) + (j_)*ldq)
    ...
    magma_tally4_int_t igpu, j, k, matsiz, msd2, smlsiz;
    ...
    *info = 0;
    ...
    if (ngpu > 1) {
        ...
        tmp = tmp * tmp2 + 2 * magma_tally4_get_dlaex3_m_nb()*(tmp + tmp2);
    }

    for (igpu = 0; igpu < ngpu; ++igpu) {
        magma_tally4_setdevice(igpu);
        if (ngpu == 1) {
            if (MAGMA_tally4_SUCCESS != magma_tally4_dmalloc( &dw[igpu], 3*n*(n/2 + 1) )) {
                *info = MAGMA_tally4_ERR_DEVICE_ALLOC;
                return *info;
            }
        }
        else {
            if (MAGMA_tally4_SUCCESS != magma_tally4_dmalloc( &dw[igpu], tmp )) {
                *info = MAGMA_tally4_ERR_DEVICE_ALLOC;
                return *info;
            }
        }
    ...
    for (i=0; i < subpbs-1; ++i) {
        submat = iwork[i];
        d[submat-1] -= MAGMA_tally4_D_ABS(e[submat-1]);
        d[submat] -= MAGMA_tally4_D_ABS(e[submat-1]);
    }
    ...
    while (subpbs > 1) {
        ...
        for (i=0; i < subpbs-1; i += 2) {
            ...
            magma_tally4_dlaex1_m(ngpu, matsiz, &d[submat], Q(submat, submat), ldq,
