...
*/
extern "C"
void magma_tally3blas_sgemvn_batched(
    int m, int n, 
    ...
    int batchCount, magma_tally3_queue_t queue)
{
    ...
    if( m > 512 || n > 512)
    {
        fprintf( stderr, "m=%d, n=%d, sgemv_batched nontranspose assume row && column lower than %d. Plz call magma_tally3blas_sgemv instead", m, n, 512);
        return ;
    }
...
extern "C"
void magma_tally3blas_sgemvt_batched(
    int m, int n, 
    float alpha, float **dA_array, int lda, 
    float **x_array,  int incx,
    float beta, float **y_array,  int incy, 
    ...
    float beta, float  **y_array, int incy)
{
    ...
    for(int i=0; i<m1; i+= sgemv_bs)
    {
        res += MAGMA_tally3_S_CNJG (A_ptr[i]) * x_ptr[i*incx];
    }
    ...
    if(m > m1)
    {
        if( tx + m1 <  m )
        {
            res  += MAGMA_tally3_S_CNJG(A_ptr[m1]) * x_ptr[m1*incx];
        }
        else
        {
            res  = res;
...
extern "C"
void magma_tally3blas_sgemvc_batched(
    int m, int n, 
    float alpha, float **dA_array, int lda, 
    float **x_array,  int incx,
    float beta, float **y_array,  int incy, 
...
extern "C"
void magma_tally3blas_sgemv_batched(
...
{       
    ...
    if ( trans == Magma_tally3NoTrans ) {
        ...
        magma_tally3blas_sgemvn_batched(m, n, alpha, dA_array, ldda, dx_array, incx, beta, dy_array, incy, batchCount, queue);
            
    }
    else if ( trans == Magma_tally3Trans ) {
        magma_tally3blas_sgemvt_batched(m, n, alpha, dA_array, ldda, dx_array, incx, beta, dy_array, incy, batchCount, queue);
    }
    else if ( trans == Magma_tally3ConjTrans ) {
#if defined(PRECISION_z) || defined (PRECISION_c)
        magma_tally3blas_sgemvc_batched(m, n, alpha, dA_array, ldda, dx_array, incx, beta, dy_array, incy, batchCount, queue);
#else
        magma_tally3blas_sgemvt_batched(m, n, alpha, dA_array, ldda, dx_array, incx, beta, dy_array, incy, batchCount, queue);
#endif
    }
