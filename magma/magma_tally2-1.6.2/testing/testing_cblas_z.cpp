/*
    -- MAGMA_tally2 (version 1.6.1) --
       ...
       correct.
       This is derived from the testing_blas_z.cpp code that checks MAGMA_tally2's
...
double blasf77_dzasum( const magma_tally2_int_t* n,
                       const magma_tally2DoubleComplex* x, const magma_tally2_int_t* incx );

double blasf77_dznrm2( const magma_tally2_int_t* n,
                       const magma_tally2DoubleComplex* x, const magma_tally2_int_t* incx );

magma_tally2DoubleComplex blasf77_zdotc( const magma_tally2_int_t* n,
                                  const magma_tally2DoubleComplex* x, const magma_tally2_int_t* incx,
                                  const magma_tally2DoubleComplex* y, const magma_tally2_int_t* incy );

magma_tally2DoubleComplex blasf77_zdotu( const magma_tally2_int_t* n,
                                  const magma_tally2DoubleComplex* x, const magma_tally2_int_t* incx,
                                  const magma_tally2DoubleComplex* y, const magma_tally2_int_t* incy );
...
int main( int argc, char** argv )
{
    ...
    magma_tally2DoubleComplex  *A, *B;
    double diff, error;
    magma_tally2_int_t ISEED[4] = {0,0,0,1};
    magma_tally2_int_t m, n, k, size, maxn, ld;
    magma_tally2DoubleComplex x2_m, x2_c;  // complex x for magma_tally2, cblas/fortran blas respectively
    double x_m, x_c;  // x for magma_tally2, cblas/fortran blas respectively
    ...
    for( int iincy = 0; iincy < ninc; ++iincy ) {
        ...
        ld = max( 1, maxn );
        size = ld*maxn;
        magma_tally2_zmalloc_pinned( &A,  size );  assert( A   != NULL );
        magma_tally2_zmalloc_pinned( &B,  size );  assert( B   != NULL );
        ...
        if ( incx > 0 && incx == incy ) {  // positive, no incy
            ...
            error = 0;
            for( int j = 0; j < k; ++j ) {
                x_m = magma_tally2_cblas_dzasum( m, A(0,j), incx );
        ...
        if ( incx > 0 && incx == incy ) {  // positive, no incy
            ...
            error = 0;
            for( int j = 0; j < k; ++j ) {
                x_m = magma_tally2_cblas_dznrm2( m, A(0,j), incx );
        ...
        for( int j = 0; j < k; ++j ) {
            ...
            x2_m = magma_tally2_cblas_zdotc( m, A(0,j), incx, B(0,j), incy );
        ...
        for( int j = 0; j < k; ++j ) {
            // MAGMA_tally2 implementation, not just wrapper
            x2_m = magma_tally2_cblas_zdotu( m, A(0,j), incx, B(0,j), incy );
