...
void init_matrix( int m, int n, magma_tally3DoubleComplex *h_A, magma_tally3_int_t lda )
{
...
double get_LU_error(magma_tally3_int_t M, magma_tally3_int_t N,
                    magma_tally3DoubleComplex *LU, magma_tally3_int_t lda,
                    magma_tally3_int_t *ipiv)
{
    ...
    magma_tally3DoubleComplex *A, *L, *U;
    double work[1], matnorm, residual;
    
    TESTING_MALLOC_CPU( A, magma_tally3DoubleComplex, lda*N    );
...
int main( int argc, char** argv )
{
    ...
    magma_tally3DoubleComplex_ptr d_lA[ Magma_tally3MaxGPUs ];
    magma_tally3_int_t *ipiv;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for( int dev=0; dev < ngpu; dev++ ) {
                ...
                TESTING_MALLOC_DEV( d_lA[dev], magma_tally3DoubleComplex, ldda*ldn_local );
            }
            ...
            /* ====================================================================
               ...
               =================================================================== */
            init_matrix( M, N, h_A, lda );
            magma_tally3_zsetmatrix_1D_col_bcyclic( M, N, h_A, lda, d_lA, ldda, ngpu, nb );
    
            gpu_time = magma_tally3_wtime();
            magma_tally3_zgetrf_mgpu( ngpu, M, N, d_lA, ldda, ipiv, &info );
            gpu_time = magma_tally3_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally3_zgetrf_mgpu returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
                       
            magma_tally3_zgetmatrix_1D_col_bcyclic( M, N, d_lA, ldda, h_A, lda, ngpu, nb );
