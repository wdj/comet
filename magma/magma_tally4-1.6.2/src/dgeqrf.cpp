...
/**
    ...
    lwork   INTEGER
            ...
            where NB can be obtained through magma_tally4_get_dgeqrf_nb(M).
    \n
    ...
    magma_tally4_int_t *info )
{
    ...
    *info = 0;
    ...
    work[0] = MAGMA_tally4_D_MAKE( (double)lwkopt, 0 );
    int lquery = (lwork == -1);
    if (m < 0) {
        *info = -1;
    } else if (n < 0) {
    ...
    if ( ngpu > 1 ) {
        ...
        return magma_tally4_dgeqrf4(ngpu, m, n, A, lda, tau, work, lwork, info);
    }

    // allocate space for dA, dwork, and dT
    if (MAGMA_tally4_SUCCESS != magma_tally4_dmalloc( &dA, n*ldda + nb*lddwork + nb*nb )) {
        /* Switch to the "out-of-core" (out of GPU-memory) version */
        return magma_tally4_dgeqrf_ooc(m, n, A, lda, tau, work, lwork, info);
    }
    ...
    if ( (nb > 1) && (nb < k) ) {
        ...
        magma_tally4_dsetmatrix_async( m, n-nb,
                                A(0,nb),  lda,
        ...
        for (i = 0; i < k-nb; i += nb) {
            ...
            if (i > 0) {
                ...
                magma_tally4_dgetmatrix_async( m-i, ib,
                                        dA(i,i), ldda,
                                        A(i,i),  lda, stream[0] );
                ...
                magma_tally4_dgetmatrix_async( i, ib,
                                        dA(0,i), ldda,
                                        A(0,i),  lda, stream[1] );
            ...
            dpanel_to_q_tally4(Magma_tally4Upper, ib, A(i,i), lda, work+ib*ib);

            /* download the i-th V matrix */
            magma_tally4_dsetmatrix_async( rows, ib, A(i,i), lda, dA(i,i), ldda, stream[0] );

            /* download the T matrix */
            magma_tally4_queue_sync( stream[1] );
            magma_tally4_dsetmatrix_async( ib, ib, work, ib, dT, nb, stream[0] );
            magma_tally4_queue_sync( stream[0] );
            ...
            if (i + ib < n) {
                if (i+ib < k-nb) {
                    ...
                    dq_to_panel_tally4(Magma_tally4Upper, ib, A(i,i), lda, work+ib*ib);
                }
                else {
    ...
    if (i < k) {
        ib = n-i;
        if (i != 0) {
            magma_tally4_dgetmatrix_async( m, ib, dA(0,i), ldda, A(0,i), lda, stream[1] );
            magma_tally4_queue_sync( stream[1] );
        }
        magma_tally4_int_t rows = m-i;
        lapackf77_dgeqrf(&rows, &ib, A(i,i), &lda, tau+i, work, &lwork, info);
