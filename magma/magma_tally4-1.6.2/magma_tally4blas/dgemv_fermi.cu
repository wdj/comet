    ...
    double       * __restrict__ y, int incy)
{
    ...
    if ( threadIdx.x < npart ) {
        ...
        sx[threadIdx.x] = MAGMA_tally4_D_ZERO;
    }
    __syncthreads();
...
magma_tally4blas_dgemv(
    magma_tally4_trans_t trans, magma_tally4_int_t m, magma_tally4_int_t n, double alpha,
    ...
    magma_tally4Double_ptr dy, magma_tally4_int_t incy)
{
    ...
    if ( arch < 200  ) {
        ...
        magma_tally4_dgemv( trans, m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
        #else
        magma_tally4blas_dgemv_tesla( trans, m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
        #endif
        return;
    }
    ...
    if ( trans == Magma_tally4NoTrans ) {
        ...
        dgemvn_kernel1_fermi<<< grid, threads, 0, magma_tally4_stream >>>
            ( m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    }
    else if ( trans == Magma_tally4Trans ) {
        dim3 grid    ( 1, n, 1 );
        dim3 threads ( BLK_X, 1, 1 );
        dgemvt_kernel_fermi<<< grid, threads, 0, magma_tally4_stream >>>
            ( m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    }
    else if ( trans == Magma_tally4ConjTrans ) {
        dim3 grid    ( 1, n, 1 );
        dim3 threads ( BLK_X, 1, 1 );
        dgemvc_kernel_fermi<<< grid, threads, 0, magma_tally4_stream >>>
            ( m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    }
}
