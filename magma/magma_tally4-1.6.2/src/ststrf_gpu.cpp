...
#define hL2(i)  (hL2 + (i)*ib*ldhl          )
    ...
    magma_tally4Float_ptr dAT, dUT;
    magma_tally4Float_ptr dAp, dUp;
#ifndef WITHOUTTRTRI
    ...
    *info = 0;
    ...
    if ( ib >= mindim ) {
        ...
        if ( order == Magma_tally4RowMajor ) {
            magma_tally4_ssetmatrix( m, n, hU, ldhu, dwork, lddwork );
            magma_tally4blas_stranspose( m, n, dwork, lddwork, dU, lddu );

            magma_tally4_ssetmatrix( m, n, hA, ldha, dwork, lddwork );
            magma_tally4blas_stranspose( m, n, dwork, lddwork, dA, ldda );
        } else {
            magma_tally4_ssetmatrix( m, n, hU, ldhu, dU, lddu );
            magma_tally4_ssetmatrix( m, n, hA, ldha, dA, ldda );
        }
        magma_tally4_ssetmatrix( p*ib, n, hL, ldhl, dL, lddl );
        ...
        if ( order == Magma_tally4ColMajor ) {
            magma_tally4blas_sgetmo_in( dU, dUT, lddu, m,  n );
        ...
        for( i=0; i < s; i++ ) {
            ...
            if ( i > 0 ) {
                // download i-th panel
                magma_tally4blas_stranspose( sb, ii, UT(0,i), lddu, dUp, lddu );
                magma_tally4blas_stranspose( sb, m,  AT(0,i), ldda, dAp, ldda );
                
                magma_tally4_sgetmatrix( ii, sb, dUp, lddu, hU(0, i), ldhu );
                magma_tally4_sgetmatrix( m,  sb, dAp, ldda, hA(0, i), ldha );
...
#ifndef NOSWAPBLK
            magma_tally4blas_sswapblk( Magma_tally4RowMajor, n-(ii+sb),
                                UT(i, i+1), lddu,
                                AT(0, i+1), ldda,
                                1, sb, ipiv+ii, 1, nb );
            ...
            for (j=0; j < ib; j++) {
                ...
                if ( im != (j) ) {
                    im = im - nb;
                    assert( (im >= 0) && (im < m) );
                    magma_tally4blas_sswap( n-(ii+sb), UT(i, i+1)+j*lddu, 1, AT(0, i+1)+im*ldda, 1 );
                } else {
                    ipiv[ip] += ii;
                }
                ip++;
...
#endif
            // upload i-th panel
            magma_tally4_ssetmatrix( sb, sb, hU(i, i), ldhu, dUp, lddu );
            magma_tally4_ssetmatrix( m, sb, hA(0, i), ldha, dAp, ldda );
            magma_tally4_ssetmatrix( p*ib, sb, hL(i), ldhl, L(i), lddl );
            magma_tally4blas_stranspose( sb, sb, dUp, lddu, UT(i,i), lddu );
            magma_tally4blas_stranspose( m,  sb, dAp, ldda, AT(0,i), ldda );
        ...
        if ( order == Magma_tally4ColMajor ) {
            magma_tally4blas_sgetmo_out( dU, dUT, lddu, m,  n );
