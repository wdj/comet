...
__global__ void 
magma_tally2_citeric_csr_kernel(    
    magma_tally2_int_t n, 
    magma_tally2_int_t nnz, 
    magma_tally2_index_t *Arowidx, 
    magma_tally2_index_t *Acolidx, 
    const magma_tally2FloatComplex * __restrict__  A_val,
    ...
    {     
        ...
        // modify entry
        if (i == j)
            val[il-1] = MAGMA_tally2_C_MAKE(sqrt(abs(MAGMA_tally2_C_REAL(s))), 0.0);
        else
            val[il-1] =  s / val[iu-1];
...
/**
    ...
    The input format of the initial guess matrix A is Magma_tally2_CSRCOO,
...
magma_tally2_citeric_csr( 
    ...
    magma_tally2_citeric_csr_kernel<<< grid, block, 0, magma_tally2_stream >>>
            ( A.num_rows, A.nnz, 
