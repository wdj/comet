...
int main( int argc, char** argv)
{
    ...
    float *h_A, *h_B, *h_Bcublas, *h_Bmagma_tally2, *h_Blapack, *h_X;
    float *d_A, *d_B;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_Bmagma_tally2,  float, sizeB   );
            ...
            magma_tally2_smalloc( &dinvA, dinvA_batchSize * batchCount);
            magma_tally2_smalloc( &dwork, dwork_batchSize * batchCount );
    
            sset_pointer(dwork_array, dwork, lddb, 0, 0, dwork_batchSize, batchCount, opts.queue);
            sset_pointer(dinvA_array, dinvA, magma_tally2_roundup( Ak, TRI_NB ), 0, 0, dinvA_batchSize, batchCount, opts.queue);

            memset(h_Bmagma_tally2, 0, batchCount*ldb*N*sizeof(float));
            magma_tally2blas_slaset( Magma_tally2Full, lddb, N*batchCount, c_zero, c_zero, dwork, lddb);
            ...
            /* =====================================================================
               ...
               =================================================================== */
            magma_tally2_ssetmatrix( Ak, Ak*batchCount, h_A, lda, d_A, ldda );
            magma_tally2_ssetmatrix( M,  N*batchCount, h_B, ldb, d_B, lddb );
            ...
            magma_tally2_time = magma_tally2_sync_wtime( NULL );
                ...
                magma_tally2_sgetmatrix( M, N*batchCount, dwork, lddb, h_Bmagma_tally2, ldb );
            #else
                ...
                magma_tally2_time = magma_tally2_sync_wtime( NULL ) - magma_tally2_time;
                magma_tally2_perf = gflops / magma_tally2_time;
                magma_tally2_sgetmatrix( M, N*batchCount, d_B, lddb, h_Bmagma_tally2, ldb );
            #endif
            ...
            magma_tally2_sgetmatrix( M, N*batchCount, d_B, lddb, h_Bcublas, ldb );
            ...
            /* =====================================================================
               ...
               =================================================================== */
            // ||b - 1/alpha*A*x|| / (||A||*||x||)
            float alpha2 = MAGMA_tally2_S_DIV( c_one, alpha );
            float normR, normX, normA=0.;
            magma_tally2_error  = 0.0;
            cublas_error = 0.0;

            memcpy( h_X, h_Bmagma_tally2, sizeB*sizeof(float) );
            ...
            for (int s=0; s < batchCount; s++) {
                ...
                normX = lapackf77_slange( "M", &M, &N, h_Bmagma_tally2 + s * ldb * N, &ldb, work );
