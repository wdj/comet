...
zgeadd_batched_kernel(
    ...
    const magma_tally3DoubleComplex * const *dAarray, int ldda,
    magma_tally3DoubleComplex              **dBarray, int lddb )
{
    // dA and dB iterate across row i
    const magma_tally3DoubleComplex *dA = dAarray[ blockIdx.y ];
    magma_tally3DoubleComplex       *dB = dBarray[ blockIdx.y ];
    int i = blockIdx.x*blockDim.x + threadIdx.x;
    if ( i < m ) {
        dA += i;
        dB += i;
        const magma_tally3DoubleComplex *dAend = dA + n*ldda;
        while( dA < dAend ) {
            *dB = alpha*(*dA) + (*dB);
            dA += ldda;
            dB += lddb;
...
/**
    @see magma_tally3blas_zgeadd_batched_q
    @ingroup magma_tally3_zaux2
    ********************************************************************/
extern "C" void
