/*
    -- MAGMA_minproduct (version 1.6.1) --
       ...
       @generated from testing_zgesvd.cpp normal z -> s, Fri Jan 30 19:00:26 2015
       @author Mark Gates
        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            switch( opts.svd_work ) {
                case 0: {  // query for workspace size
                    lwork = -1;
                    magma_minproduct_sgesvd( jobu, jobvt, M, N,
                                  h_R, lda, S1, U, ldu, VT, ldv, dummy, lwork,
                                  #ifdef COMPLEX
                                  rwork,
                                  #endif
                                  &info );
                    lwork = (int) MAGMA_minproduct_S_REAL( dummy[0] );
                    break;
                }
            ...
            TESTING_MALLOC_CPU( VT,    float, ldv*N );   // N x N (jobvt=A) or min(M,N) x N
            TESTING_MALLOC_CPU( U,     float, ldu*N_U ); // M x M (jobu=A)  or M x min(M,N)
            ...
            if (info != 0)
                printf("magma_minproduct_sgesvd returned error %d: %s.\n",
            ...
            if ( opts.lapack ) {
                ...
                lapackf77_sgesvd( lapack_vec_const(jobu), lapack_vec_const(jobvt), &M, &N,
                ...
                if (info != 0)
                    printf("lapackf77_sgesvd returned error %d: %s.\n",
                           (int) info, magma_minproduct_strerror( info ));
