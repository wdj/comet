...
int main( int argc, char** argv) 
{
    ...
    printf("  N     CPU GFlop/s (sec)   MAGMA_tally3 GFlop/s (sec)   ||R_magma_tally3 - R_lapack||_F / ||R_lapack||_F\n");
    printf("=============================================================================================\n");
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC(    h_A, magma_tally3DoubleComplex, n2 );
            TESTING_HOSTALLOC( h_R, magma_tally3DoubleComplex, n2 );
            ...
            for(j = 0; j < ngpu; j++){
                ...
                TESTING_DEVALLOC( d_lA[j], magma_tally3DoubleComplex, ldda * ldn_local );
            }
            ...
            //magma_tally3_zprint( N,N, h_A, lda );
            //if( opts.uplo == Magma_tally3Upper) {
                for(j = 0; j < N; j += nb){
                    k = (j / nb) % ngpu;
                    magma_tally3_setdevice(k);
                    nk = min(nb, N - j);
                    magma_tally3_zsetmatrix( N, nk,
                            h_A + j * lda,                       lda,
            ...
            //magma_tally3_zpotrf_mgpu(ngpu, opts.uplo, N, d_lA, ldda, &info);
            magma_tally3_zpotrf_mgpu_right(ngpu, opts.uplo, N, d_lA, ldda, &info);
            gpu_time = magma_tally3_wtime() - gpu_time;
            if (info != 0)
                printf("magma_tally3_zpotrf_gpu returned error %d.\n", (int) info);
            ...
            if ( opts.check && info == 0) {
                ...
                magma_tally3_setdevice(0);
                //printf( " ==== MAGMA_tally3 ====\n" );
                //magma_tally3_zprint( N,N, h_R, lda );
