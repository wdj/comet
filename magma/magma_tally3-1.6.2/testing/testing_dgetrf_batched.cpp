...
int main( int argc, char** argv)
{
    ...
    double *h_A, *h_R, *h_Amagma_tally3;
    double *dA;
    ...
    for( int i = 0; i < opts.ntest; ++i ) {
      ...
      for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU(    h_Amagma_tally3,  double, n2     );
            ...
            lapackf77_dlacpy( Magma_tally3UpperLowerStr, &M, &columns, h_A, &lda, h_R, &lda );
            
            /* ====================================================================
               Performs operation using MAGMA_tally3
               =================================================================== */

            magma_tally3_dsetmatrix( M, columns, h_R, lda, dA, ldda );
            ...
            info = magma_tally3_dgetrf_batched( M, N, dA_array, ldda, dipiv_array,  dinfo_magma_tally3, batchCount, queue);
            magma_tally3_time = magma_tally3_sync_wtime(0) - magma_tally3_time;
            magma_tally3_perf = gflops / magma_tally3_time;
            
            magma_tally3_dgetmatrix( M, N*batchCount, dA, ldda, h_Amagma_tally3, lda );
            ...
            {
                if(cpu_info[i] != 0 ){
                    printf("magma_tally3_dgetrf_batched matrix %d returned internal error %d\n",i, (int)cpu_info[i] );
                }
            }
            
            if (info != 0)
                printf("magma_tally3_dgetrf_batched returned argument error %d: %s.\n", (int) info, magma_tally3_strerror( info ));
