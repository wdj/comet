...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_tally3_smake_hpd( N, h_B, lda );
            lapackf77_slacpy( Magma_tally3FullStr, &N, &N, h_A, &lda, h_R, &lda );
            lapackf77_slacpy( Magma_tally3FullStr, &N, &N, h_B, &lda, h_S, &lda );
            ...
            if ( opts.check && opts.jobz != Magma_tally3NoVec ) {
                ...
                if ( opts.itype == 1 || opts.itype == 2 ) {
                    ...
                    blasf77_ssymm("R", lapack_uplo_const_tally3(opts.uplo), &N, &N, &c_neg_one, h_B, &lda, h_work, &N, &c_one, h_S, &lda);
                    result[1] = lapackf77_slange("1", &N, &N, h_S, &lda, rwork) / N;
                }
                else if ( opts.itype == 3 ) {
                    lapackf77_slacpy( Magma_tally3FullStr, &N, &N, h_B, &lda, h_S, &lda);
                    blasf77_ssyrk(lapack_uplo_const_tally3(opts.uplo), "N", &N, &N, &d_neg_one, h_R, &lda, &d_one, h_S, &lda);
                    result[1] = lapackf77_slansy("1", lapack_uplo_const_tally3(opts.uplo), &N, h_S, &lda, rwork) / N
                              / lapackf77_slansy("1", lapack_uplo_const_tally3(opts.uplo), &N, h_B, &lda, rwork);
                }
                
                result[0] = 1.;
                result[0] /= lapackf77_slansy("1", lapack_uplo_const_tally3(opts.uplo), &N, h_A, &lda, rwork);
                result[0] /= lapackf77_slange("1", &N, &N, h_R, &lda, rwork);
                
                if ( opts.itype == 1 ) {
                    blasf77_ssymm("L", lapack_uplo_const_tally3(opts.uplo), &N, &N, &c_one, h_A, &lda, h_R, &lda, &c_zero, h_work, &N);
                    for(int i=0; i < N; ++i)
                        blasf77_sscal(&N, &w1[i], &h_R[i*N], &ione);
                    blasf77_ssymm("L", lapack_uplo_const_tally3(opts.uplo), &N, &N, &c_neg_one, h_B, &lda, h_R, &lda, &c_one, h_work, &N);
                    result[0] *= lapackf77_slange("1", &N, &N, h_work, &lda, rwork)/N;
                }
                else if ( opts.itype == 2 ) {
                    blasf77_ssymm("L", lapack_uplo_const_tally3(opts.uplo), &N, &N, &c_one, h_B, &lda, h_R, &lda, &c_zero, h_work, &N);
                    for(int i=0; i < N; ++i)
                        blasf77_sscal(&N, &w1[i], &h_R[i*N], &ione);
                    blasf77_ssymm("L", lapack_uplo_const_tally3(opts.uplo), &N, &N, &c_one, h_A, &lda, h_work, &N, &c_neg_one, h_R, &lda);
                    ...
                    for(int i=0; i < N; ++i)
                        blasf77_sscal(&N, &w1[i], &h_R[i*N], &ione);
                    blasf77_ssymm("L", lapack_uplo_const_tally3(opts.uplo), &N, &N, &c_one, h_B, &lda, h_work, &N, &c_neg_one, h_R, &lda);
                    result[0] *= lapackf77_slange("1", &N, &N, h_R, &lda, rwork)/N;
                }
                ...
                */
                ...
                //lapackf77_slacpy( Magma_tally3FullStr, &N, &N, h_B, &lda, h_S, &lda );
                //
                //magma_tally3_ssygvd( opts.itype, Magma_tally3NoVec, opts.uplo,
