...
magma_minproduct_dgetrf(
    magma_minproduct_int_t m, magma_minproduct_int_t n, double *A, magma_minproduct_int_t lda,
    ...
    *info = 0;
    ...
    if ( (nb <= 1) || (nb >= min(m,n)) ) {
        ...
        if ( ngpu > 1 ) {
            ...
            magma_minproduct_dgetrf_m(ngpu, m, n, A, lda, ipiv, info);
            return *info;
        }
        ...
        if (maxdim*maxdim < 2*maxm*maxn) {
            ...
            if (MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc( &dA, nb*maxm + maxdim*maxdim )) {
            ...
            if (MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc( &dA, (nb + maxn)*maxm )) {
                /* alloc failed so call non-GPU-resident version */
                magma_minproduct_dgetrf_m(ngpu, m, n, A, lda, ipiv, info);
                return *info;
            }
            ...
            magma_minproduct_dsetmatrix( m, n, A, lda, da, maxm );
            
            if (MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc( &dAT, maxm*maxn )) {
                /* alloc failed so call non-GPU-resident version */
            ...
            magma_minproductblas_dtranspose( m, n, da, maxm, dAT, ldda );
        }
        ...
        for( j = 0; j < s; j++ ) {
            ...
            if (j > 0) {
                magma_minproductblas_dtranspose( nb, cols, dAT(j,j), ldda, dA, cols );

                // make sure that gpu queue is empty
                magma_minproduct_device_sync();

                magma_minproduct_dgetmatrix_async( m-j*nb, nb, dA, cols, work, lda,
                                        stream[0]);
            ...
            // upload j-th panel
            magma_minproduct_dsetmatrix_async( m-j*nb, nb, work, lda, dA, cols,
                                    stream[0]);

            for( i=j*nb; i < j*nb + nb; ++i ) {
                ipiv[i] += j*nb;
            }
            magma_minproductblas_dlaswp( n, dAT, ldda, j*nb + 1, j*nb + nb, ipiv, 1 );

            magma_minproduct_queue_sync( stream[0] );
            magma_minproductblas_dtranspose( cols, nb, dA, cols, dAT(j,j), ldda );
        ...
        if ( nb0 > 0 ) {
            ...
            magma_minproductblas_dtranspose( nb0, rows, dAT(s,s), ldda, dA, cols );
            magma_minproduct_dgetmatrix( rows, nb0, dA, cols, work, lda );
            ...
            magma_minproductblas_dlaswp( n, dAT, ldda, s*nb + 1, s*nb + nb0, ipiv, 1 );

            // upload j-th panel
            magma_minproduct_dsetmatrix( rows, nb0, work, lda, dA, cols );
            magma_minproductblas_dtranspose( rows, nb0, dA, cols, dAT(s,s), ldda );
        ...
        if (maxdim*maxdim < 2*maxm*maxn) {
            magma_minproductblas_dtranspose_inplace( ldda, dAT, ldda );
            magma_minproduct_dgetmatrix( m, n, da, ldda, A, lda );
        }
        else {
            magma_minproductblas_dtranspose( n, m, dAT, ldda, da, maxm );
            magma_minproduct_dgetmatrix( m, n, da, maxm, A, lda );
