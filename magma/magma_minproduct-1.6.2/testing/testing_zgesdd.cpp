...
int main( int argc, char** argv)
{
    ...
    magma_minproductDoubleComplex *h_A, *h_R, *U, *VT, *h_work;
    magma_minproductDoubleComplex dummy[1];
    double *S1, *S2;
    #ifdef COMPLEX
    magma_minproduct_int_t lrwork=0;
    double *rwork;
    #endif
    magma_minproduct_int_t *iwork;
    
    magma_minproduct_int_t M, N, N_U, M_VT, lda, ldu, ldv, n2, min_mn, max_mn, info, nb, lwork;
    magma_minproduct_int_t ione     = 1;
    magma_minproduct_int_t ISEED[4] = {0,0,0,1};
    magma_minproduct_vec_t jobz;
    magma_minproduct_int_t status = 0;

    MAGMA_minproduct_UNUSED( max_mn );  // used only in complex
    ...
    magma_minproduct_vec_t jobs[] = { Magma_minproductNoVec, Magma_minproductSomeVec, Magma_minproductOverwriteVec, Magma_minproductAllVec };
    
    if ( opts.check && ! opts.all && (jobz == Magma_minproductNoVec)) {
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            min_mn = min(M, N);
            max_mn = max(M, N);
            N_U  = (jobz == Magma_minproductAllVec ? M : min_mn);
            M_VT = (jobz == Magma_minproductAllVec ? N : min_mn);
            lda = M;
            ldu = M;
            ldv = M_VT;
            n2 = lda*N;
            ...
            magma_minproduct_int_t x = max(M,N);
            magma_minproduct_int_t y = min(M,N);
            #ifdef COMPLEX
            ...
            switch( opts.svd_work ) {
                case 0: {  // query for workspace size
                    lwork = -1;
                    magma_minproduct_zgesdd( jobz, M, N,
                                  NULL, lda, NULL, NULL, ldu, NULL, ldv, dummy, lwork,
                                  #ifdef COMPLEX
                                  NULL,
                                  #endif
                                  NULL, &info );
                    lwork = (int) MAGMA_minproduct_Z_REAL( dummy[0] );
                    break;
                }
            ...
            TESTING_MALLOC_CPU( VT,    magma_minproductDoubleComplex, ldv*N );   // N x N (jobz=A) or min(M,N) x N
            TESTING_MALLOC_CPU( U,     magma_minproductDoubleComplex, ldu*N_U ); // M x M (jobz=A) or M x min(M,N)
            TESTING_MALLOC_CPU( S1,    double, min_mn );
            TESTING_MALLOC_CPU( S2,    double, min_mn );
            TESTING_MALLOC_CPU( iwork, magma_minproduct_int_t, 8*min_mn );
            ...
            if (info != 0)
                printf("magma_minproduct_zgesdd returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                magma_minproductDoubleComplex *U2  = NULL;
                magma_minproductDoubleComplex *VT2 = NULL;
                if ( jobz == Magma_minproductSomeVec || jobz == Magma_minproductAllVec ) {
                    U2  = U;
                    VT2 = VT;
                }
                else if ( jobz == Magma_minproductOverwriteVec ) {
                    if ( M >= N ) {
                        U2  = h_R;
                        ldu = lda;
                        VT2 = VT;
                ...
                magma_minproduct_int_t lwork_err = M+N;
                if ( U2 != NULL ) {
                ...
                magma_minproductDoubleComplex *h_work_err;
                TESTING_MALLOC_CPU( h_work_err, magma_minproductDoubleComplex, lwork_err );
