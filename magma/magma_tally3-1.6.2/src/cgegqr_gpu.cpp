...
/**
    ...
    ikind   INTEGER
            ...
            2:  This version uses a standard LAPACK-based orthogonalization through
                MAGMA_tally3's QR panel factorization (magma_tally3_cgeqr2x3_gpu) and magma_tally3_cungqr
            3:  MGS
    ...
    magma_tally3_int_t *info )
{
    ...
    *info = 0;
    ...
    if (ikind == 1) {
        ...
        magma_tally3FloatComplex *U, *VT, *vt, *R, *G, *hwork, *tau;
        float *S;
        ...
        magma_tally3_cmalloc_cpu( &hwork, 32 + 2*n*n + 2*n);
        if ( hwork == NULL ) {
        ...
        float *rwork;
        magma_tally3_smalloc_cpu( &rwork, 5*n);
        if ( rwork == NULL ) {
            *info = MAGMA_tally3_ERR_HOST_ALLOC;
            return *info;
        }
        ...
        do {
            ...
            magma_tally3_cgemm(Magma_tally3ConjTrans, Magma_tally3NoTrans, n, n, m, c_one, dA, ldda, dA, ldda, c_zero, dwork, n );
            magma_tally3_cgetmatrix(n, n, dwork, n, G, n);
            ...
            magma_tally3_csetmatrix(n, n, VT, n, dwork, n);
            magma_tally3_ctrsm( Magma_tally3Right, Magma_tally3Upper, Magma_tally3NoTrans, Magma_tally3NonUnit, m, n, c_one, dwork, n, dA, ldda);
            if (mins > 0.00001f)
                cn = maxs/mins;
        ...
        magma_tally3FloatComplex_ptr dtau = dwork + 2*n*n;
        magma_tally3FloatComplex_ptr d_T  = dwork;
        magma_tally3FloatComplex_ptr ddA  = dwork + n*n;
        magma_tally3FloatComplex *tau  = work+n*n;

        magma_tally3blas_claset( Magma_tally3Full, n, n, c_zero, c_zero, d_T, n );
        magma_tally3_cgeqr2x3_gpu(m, n, dA, ldda, dtau, d_T, ddA,
                           (float *)(dwork+min_mn+2*n*n), info);
        magma_tally3_cgetmatrix( min_mn, 1, dtau, min_mn, tau, min_mn);
        magma_tally3_cgetmatrix( n, n, ddA, n, work, n);
        magma_tally3_cungqr_gpu( m, n, n, dA, ldda, tau, d_T, nb, info );
        // ================== end of ikind == 2 ===================================================
    }
    else if (ikind == 3) {
        // ================== MGS               ===================================================
        for (magma_tally3_int_t j = 0; j < n; j++) {
            for (magma_tally3_int_t i = 0; i < j; i++) {
                *work(i, j) = magma_tally3_cdotc(m, dA(0,i), 1, dA(0,j), 1);
                magma_tally3_caxpy(m, -(*work(i,j)),  dA(0,i), 1, dA(0,j), 1);
            }
            for (magma_tally3_int_t i = j; i < n; i++)
                *work(i, j) = MAGMA_tally3_C_ZERO;
            //*work(j,j) = MAGMA_tally3_C_MAKE( magma_tally3_scnrm2(m, dA(0,j), 1), 0. );
            *work(j,j) = magma_tally3_cdotc(m, dA(0,j), 1, dA(0,j), 1);
            *work(j,j) = MAGMA_tally3_C_MAKE( sqrt(MAGMA_tally3_C_REAL( *work(j,j) )), 0.);
            magma_tally3_cscal(m, 1./ *work(j,j), dA(0,j), 1);
        }
        // ================== end of ikind == 3 ===================================================
    }
    else if (ikind == 4) {
        // ================== Cholesky QR       ===================================================
        magma_tally3_cgemm(Magma_tally3ConjTrans, Magma_tally3NoTrans, n, n, m, c_one, dA, ldda, dA, ldda, c_zero, dwork, n );
        magma_tally3_cgetmatrix(n, n, dwork, n, work, n);
        lapackf77_cpotrf("u", &n, work, &n, info);
        magma_tally3_csetmatrix(n, n, work, n, dwork, n);
        magma_tally3_ctrsm( Magma_tally3Right, Magma_tally3Upper, Magma_tally3NoTrans, Magma_tally3NonUnit, m, n, c_one, dwork, n, dA, ldda);
        // ================== end of ikind == 4 ===================================================
    }
