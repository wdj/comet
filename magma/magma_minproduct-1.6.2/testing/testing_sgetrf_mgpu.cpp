...
float get_LU_error(magma_minproduct_int_t M, magma_minproduct_int_t N,
                    float *LU, magma_minproduct_int_t lda,
                    magma_minproduct_int_t *ipiv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            /* ====================================================================
               ...
               =================================================================== */
            init_matrix( M, N, h_A, lda );
            magma_minproduct_ssetmatrix_1D_col_bcyclic( M, N, h_A, lda, d_lA, ldda, ngpu, nb );
    
            gpu_time = magma_minproduct_wtime();
            magma_minproduct_sgetrf_mgpu( ngpu, M, N, d_lA, ldda, ipiv, &info );
            gpu_time = magma_minproduct_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_minproduct_sgetrf_mgpu returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
                       
            magma_minproduct_sgetmatrix_1D_col_bcyclic( M, N, d_lA, ldda, h_A, lda, ngpu, nb );
