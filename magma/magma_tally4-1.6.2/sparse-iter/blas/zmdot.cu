    ...
    int k )
{
   ...
   if( i < n ){
    for( int j=0; j<k; j++)
      d[ i+j*n ] = MAGMA_tally4_Z_MAKE( 0.0, 0.0 );
    }
}

// dot product
__global__ void
magma_tally4_zdot_kernel( 
    int Gs,
    int n, 
...
__global__ void
magma_tally4_zblockreduce_kernel( 
    int Gs,
    int n, 
    int k,
    magma_tally4DoubleComplex * vtmp,
...
__global__ void
magma_tally4_zreduce_kernel_fast( int Gs,
                           int n, 
                           magma_tally4DoubleComplex * vtmp,
                           magma_tally4DoubleComplex * vtmp2 ){
...
__global__ void
magma_tally4_zblockreduce_kernel_fast( 
    int Gs,
    int n, 
    int k,
    magma_tally4DoubleComplex * vtmp,
    ...
    magma_tally4_queue_t queue )
{
    ...
    dim3 Gs_next;
    int Ms =  (k)* (local_block_size) * sizeof( magma_tally4DoubleComplex ); // k vecs 
    magma_tally4DoubleComplex_ptr aux1 = d1, aux2 = d2;
    int b = 1;        

    if (k>1) {
        magma_tally4_zblockdot_kernel<<<Gs, Bs, Ms>>>( Gs.x, n, k, v, r, d1 );
    }
    else {
        magma_tally4_zdot_kernel<<<Gs, Bs, Ms>>>( Gs.x, n, v, r, d1 );
    }
/*
    // not necessary to zero GPU mem
    magma_tally4_zgpumemzero<<<Gs, Bs, 0>>>( d1, n*k,1 );
    magma_tally4_zgpumemzero<<<Gs, Bs, 0>>>( d2, n*k,1 );
    //magma_tally4blas_zlaset( Magma_tally4UpperLower, n, k, d1, n );
    //magma_tally4blas_zlaset( Magma_tally4UpperLower, n, k, d2, n );
    while( Gs.x > 1 ) {
        Gs_next.x = ( Gs.x+Bs.x-1 )/ Bs.x ;
        magma_tally4_zblockreduce_kernel<<< Gs_next.x, Bs.x, Ms >>> 
                                        ( Gs.x, n, k, aux1, aux2 );
        Gs.x = Gs_next.x;
        b = 1 - b;
    ...
    for( int j=0; j<k; j++) {
            magma_tally4_zcopyvector( 1, aux1+j*n, 1, skp+j, 1 );
    }
*/
    ...
    if ( k>1) {
        while( Gs.x > 1 ) {
            ...
            magma_tally4_zblockreduce_kernel_fast<<< Gs_next.x/2, Bs.x/2, Ms/2 >>> 
                        ( Gs.x, n, k, aux1, aux2 );
        ...
        while( Gs.x > 1 ) {
            Gs_next.x = ( Gs.x+Bs.x-1 )/ Bs.x ;
            if ( Gs_next.x == 1 ) Gs_next.x = 2;
            magma_tally4_zreduce_kernel_fast<<< Gs_next.x/2, Bs.x/2, Ms/2 >>> 
                        ( Gs.x, n, aux1, aux2 );
            Gs_next.x = Gs_next.x /2;
            Gs.x = Gs_next.x;
            b = 1 - b;
    ...
    magma_tally4_queue_t queue )
{
    ...
    while( rows_left > (chunk_size) ) {
        magma_tally4_zmdotc( n, chunk_size, v+offset*n, r, d1, d2, skp+offset, queue );
        offset = offset + chunk_size;
        rows_left = rows_left-chunk_size;

    }
    // process rest
    magma_tally4_zmdotc( n, rows_left, v+offset*n, r, d1, d2, skp+offset, queue ); 
