...
*******     (workspace) If FAST_HEMV is defined DZ should be (LDDZ, max(1,N)) in both cases.
    ...
    isuppz  INTEGER ARRAY, dimension ( 2*max(1,M) )
            ...
            ISUPPZ( 2*i ).
            __Implemented only for__ RANGE = Magma_tally2RangeAll or Magma_tally2RangeI and IU - IL = N - 1
    ...
    ldwz    INTEGER
            ...
            JOBZ = Magma_tally2Vec, LDWZ >= max(1,N).
...
magma_tally2_cheevr_gpu(
    ...
    float *w,
    magma_tally2FloatComplex_ptr dZ, magma_tally2_int_t lddz, magma_tally2_int_t *isuppz,
    ...
    magma_tally2_int_t *info)
{
    ...
    const char* range_ = lapack_range_const_tally2( range );
    
    magma_tally2_int_t ione = 1;
    float szero = 0.;
    float sone = 1.;
    
    magma_tally2_int_t indrd, indre;
    magma_tally2_int_t imax;
    magma_tally2_int_t lopt, itmp1, indree, indrdd;
    magma_tally2_int_t lower, wantz, tryrac;
    magma_tally2_int_t i, j, jj, i__1;
    magma_tally2_int_t alleig, valeig, indeig;
    magma_tally2_int_t iscale, indibl, indifl;
    magma_tally2_int_t indiwo, indisp, indtau;
    magma_tally2_int_t indrwk, indwk;
    magma_tally2_int_t llwork, llrwork, nsplit;
    magma_tally2_int_t lquery, ieeeok;
    ...
    *info = 0;
    ...
    work[0] = MAGMA_tally2_C_MAKE( lwmin, 0 );
    rwork[0] = lrwmin;
    iwork[0] = liwmin;
    ...
    *m = 0;
    ...
    if (n <= 128) {
        ...
        magma_tally2_cmalloc_cpu( &a, n*n );
        magma_tally2_cgetmatrix(n, n, dA, ldda, a, n);
        lapackf77_cheevr(jobz_, range_, uplo_,
                         &n, a, &n, &vl, &vu, &il, &iu, &abstol, m,
                         w, wZ, &ldwz, isuppz, work, &lwork,
                         rwork, &lrwork, iwork, &liwork, info);
        magma_tally2_csetmatrix( n,  n,  a,    n, dA, ldda);
        magma_tally2_csetmatrix( n, *m, wZ, ldwz, dZ, lddz);
        magma_tally2_free_cpu(a);
        return *info;
    }

    if (MAGMA_tally2_SUCCESS != magma_tally2_smalloc( &dwork, n )) {
        fprintf (stderr, "!!!! device memory allocation error (magma_tally2_cheevr_gpu)\n");
    ...
    anrm = magma_tally2blas_clanhe(Magma_tally2MaxNorm, uplo, n, dA, ldda, dwork);
    iscale = 0;
    sigma  = 1;
    if (anrm > 0. && anrm < rmin) {
        iscale = 1;
    ...
    if (iscale == 1) {
        d__1 = 1.;
        magma_tally2blas_clascl(uplo, 0, 0, 1., sigma, n, n, dA,
                         ldda, info);
...
#ifdef FAST_HEMV
    magma_tally2_chetrd2_gpu(uplo, n, dA, ldda, &rwork[indrd], &rwork[indre],
                      &work[indtau], wA, ldwa, &work[indwk], llwork, dZ, lddz*n, &iinfo);
#else
    magma_tally2_chetrd_gpu (uplo, n, dA, ldda, &rwork[indrd], &rwork[indre],
                      &work[indtau], wA, ldwa, &work[indwk], llwork, &iinfo);
#endif

    lopt = n + (magma_tally2_int_t)MAGMA_tally2_C_REAL(work[indwk]);
    ...
    if (! wantz) {
        ...
        if (*info == 0 && wantz) {
            magma_tally2_csetmatrix( n, *m, wZ, ldwz, dZ, lddz );
            magma_tally2_cunmtr_gpu(Magma_tally2Left, uplo, Magma_tally2NoTrans, n, *m, dA, ldda, &work[indtau],
    ...
    if (wantz) {
        for (j = 1; j <= *m-1; ++j) {
            ...
            if (i != 0) {
                ...
                magma_tally2_cswap(n, dZ + (i-1)*lddz, ione, dZ + (j-1)*lddz, ione);
            }
        }
    }
    
    /* Set WORK[0] to optimal complex workspace size. */
    work[1] = MAGMA_tally2_C_MAKE( lopt, 0 );
