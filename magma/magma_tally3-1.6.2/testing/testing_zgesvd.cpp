...
int main( int argc, char** argv)
{
    ...
    magma_tally3DoubleComplex *h_A, *h_R, *U, *VT, *h_work;
    magma_tally3DoubleComplex dummy[1];
    double *S1, *S2;
    #ifdef COMPLEX
    double *rwork;
    #endif
    magma_tally3_int_t M, N, N_U, M_VT, lda, ldu, ldv, n2, min_mn, info, nb, lwork;
    magma_tally3_int_t ione     = 1;
    magma_tally3_int_t ISEED[4] = {0,0,0,1};
    magma_tally3_vec_t jobu, jobvt;
    ...
    if ( opts.check && ! opts.all && (jobu == Magma_tally3NoVec || jobvt == Magma_tally3NoVec)) {
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( jobu == Magma_tally3OverwriteVec && jobvt == Magma_tally3OverwriteVec ) {
            ...
            N = opts.nsize[itest];
            min_mn = min(M, N);
            N_U  = (jobu  == Magma_tally3AllVec ? M : min_mn);
            M_VT = (jobvt == Magma_tally3AllVec ? N : min_mn);
            lda = M;
            ldu = M;
            ldv = M_VT;
            n2 = lda*N;
            ...
            switch( opts.svd_work ) {
                case 0: {  // query for workspace size
                    lwork = -1;
                    magma_tally3_zgesvd( jobu, jobvt, M, N,
                                  h_R, lda, S1, U, ldu, VT, ldv, dummy, lwork,
                                  #ifdef COMPLEX
                                  rwork,
                                  #endif
                                  &info );
                    lwork = (int) MAGMA_tally3_Z_REAL( dummy[0] );
                    break;
                }
            ...
            TESTING_MALLOC_CPU( VT,    magma_tally3DoubleComplex, ldv*N );   // N x N (jobvt=A) or min(M,N) x N
            TESTING_MALLOC_CPU( U,     magma_tally3DoubleComplex, ldu*N_U ); // M x M (jobu=A)  or M x min(M,N)
            ...
            if (info != 0)
                printf("magma_tally3_zgesvd returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                magma_tally3DoubleComplex *U2  = NULL;
                magma_tally3DoubleComplex *VT2 = NULL;
                if ( jobu == Magma_tally3SomeVec || jobu == Magma_tally3AllVec ) {
                    U2 = U;
                }
                else if ( jobu == Magma_tally3OverwriteVec ) {
                    U2 = h_R;
                    ldu = lda;
                }
                if ( jobvt == Magma_tally3SomeVec || jobvt == Magma_tally3AllVec ) {
                    VT2 = VT;
                }
                else if ( jobvt == Magma_tally3OverwriteVec ) {
                    VT2 = h_R;
                    ldv = lda;
                }
                ...
                magma_tally3DoubleComplex *h_work_err;
                TESTING_MALLOC_CPU( h_work_err, magma_tally3DoubleComplex, lwork_err );
            ...
            if ( opts.lapack ) {
                ...
                lapackf77_zgesvd( lapack_vec_const_tally3(jobu), lapack_vec_const_tally3(jobvt), &M, &N,
                ...
                printf("   %c    %c %5d %5d  %7.2f         %7.2f         %8.2e",
                       lapack_vec_const_tally3(jobu)[0], lapack_vec_const_tally3(jobvt)[0],
