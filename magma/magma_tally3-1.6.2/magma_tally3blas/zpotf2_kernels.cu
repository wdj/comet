...
/////////////////////////////////////////////////////////////////////////////////////////////////
__global__ void zdotc_kernel_batched(int n, magma_tally3DoubleComplex **x_array, int incx, int offset, magma_tally3_int_t *info_array, int gbstep)
{
    int tx = threadIdx.x;

    magma_tally3DoubleComplex *x = x_array[blockIdx.z]+offset;
    ...
    if (tx == 0) {
        ...
        //MAGMA_tally3_Z_SET2REAL(x[n*incx], sqrt(xreal - sdata[0]));
        x[n*incx] = MAGMA_tally3_Z_MAKE(sqrt(xreal - sdata[0]), 0);
        if(x[n*incx] == MAGMA_tally3_Z_ZERO){
...
void magma_tally3_zpotf2_zdotc_batched(magma_tally3_int_t n, magma_tally3DoubleComplex **x_array, magma_tally3_int_t incx, magma_tally3_int_t offset, magma_tally3_int_t *info_array, magma_tally3_int_t gbstep, magma_tally3_int_t batchCount, magma_tally3_queue_t queue)
{
/*
    Specialized Zdotc
...
/////////////////////////////////////////////////////////////////////////////////////////////////
__global__ void zdscal_kernel_batched(int n, magma_tally3DoubleComplex **x_array, int incx, int offset, magma_tally3_int_t *info_array)
{
...
void magma_tally3_zpotf2_zdscal_batched(magma_tally3_int_t n, magma_tally3DoubleComplex **x_array, magma_tally3_int_t incx, magma_tally3_int_t offset, magma_tally3_int_t *info_array, magma_tally3_int_t batchCount, magma_tally3_queue_t queue)
{
/*
...
__global__ void zlacgv_kernel_batched(int n, magma_tally3DoubleComplex **x_array, int incx, int offset)
{
    int id = threadIdx.x;
...
void magma_tally3_zlacgv_batched(magma_tally3_int_t n, magma_tally3DoubleComplex **x_array, magma_tally3_int_t incx, int offset, int batchCount, magma_tally3_queue_t queue)
{
/*
    Purpose
    =======
...
static __device__ void zpotf2_device(int m, int n, 
                              ...
                              magma_tally3DoubleComplex beta, magma_tally3_int_t *info, int gbstep)
{
/*
...
*/
    ...
    int tx = threadIdx.x;
    magma_tally3DoubleComplex *sdata_A = shared_data;
    __shared__ magma_tally3DoubleComplex factor;
    ...
    {
        double res = MAGMA_tally3_D_ZERO;
        magma_tally3DoubleComplex res1 = MAGMA_tally3_Z_ZERO;

        //1) performs zdotc sum = A[iter, 0:iter-1]*conj(A[iter, 0:iter-1])
        //2) updates A[iter,iter] = sqrt(A[iter,iter]-sum);
        if(tx<iter)
        {
            res = MAGMA_tally3_Z_REAL (sdata_A[iter + tx * m] * MAGMA_tally3_Z_CNJG(sdata_A[iter + tx * m]));         
            sum[tx] = res;
        }
        else
        {
        ...
        __syncthreads();
        magma_tally3_sum_reduce<POTF2_TILE_SIZE>(tx, sum);//tried on K40: if m=32 n=32 the overall zpotf2_device routine time is 60ms n=16 time=25 n=8 time=20ms 
        ...
        if (tx == 0) {
              double xreal = MAGMA_tally3_Z_REAL(sdata_A[iter + iter * m]);        
              sdata_A[iter + iter * m] = MAGMA_tally3_Z_MAKE(sqrt(xreal - sum[0]), 0);
              if(sdata_A[iter + iter * m] == MAGMA_tally3_Z_ZERO){
                  *info = iter + gbstep + 1;
              }
        }
        __syncthreads();
        if(sdata_A[iter + iter * m] == MAGMA_tally3_Z_ZERO) return;
        __syncthreads();
        ...
        if(tx < iter)
        {
             sdata_A[iter + tx * m] = MAGMA_tally3_Z_CNJG(sdata_A[iter + tx * m]);
        }
        __syncthreads();  
        #endif
        ...
        if (tx == 0) {
            factor = MAGMA_tally3_Z_MAKE(1.0/MAGMA_tally3_Z_REAL(sdata_A[iter + iter * m]), 0.0);
        }
        __syncthreads();
...
__global__ void zpotf2_kernel_batched(int m, int n, 
                              magma_tally3DoubleComplex **dA_array, int lda, 
                              magma_tally3DoubleComplex alpha, 
...
__global__ void zpotf2_kernel(int m, int n, 
                              magma_tally3DoubleComplex *dA, int lda, 
                              magma_tally3DoubleComplex alpha, 
                              magma_tally3DoubleComplex beta,
    ...
    magma_tally3_int_t *info_array, magma_tally3_int_t gbstep, magma_tally3_int_t batchCount, magma_tally3_queue_t queue)
{
    ...
    int shared_mem_size = sizeof(magma_tally3DoubleComplex)*m*n; // + sizeof(double)*(POTF2_TILE_SIZE+1);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
extern "C" magma_tally3_int_t
magma_tally3_zpotf2_tile(
    magma_tally3_uplo_t uplo, magma_tally3_int_t m, magma_tally3_int_t n,
    magma_tally3DoubleComplex *dA, magma_tally3_int_t lda,
    magma_tally3_int_t *info)
{
    ...
    *info = 0;
    ...
    zpotf2_kernel<<<dimGrid, threads, shared_mem_size, magma_tally3_stream >>>(m, n, dA, lda, alpha, beta, info);
