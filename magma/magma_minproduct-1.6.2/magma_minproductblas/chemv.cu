...
/**
    Purpose
    -------
    magma_minproductblas_chemv_work performs the matrix-vector operation:
    ...
    MAGMA_minproduct implements chemv through two steps:
    ...
    magma_minproductblas_chemv is a wrapper routine allocating the workspace inside the
    routine and provides the same interface as cublas.
    
    If users need to call chemv frequently, we suggest using
    magma_minproductblas_chemv_work instead of magma_minproductblas_chemv. As the overhead to
    allocate and free in device memory in magma_minproductblas_chemv would hurt performance.
    ...
    magma_minproduct_queue_t queue )
{
    ...
    if ( arch < 200 ) {
        magma_minproduct_chemv( uplo, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
...
// end magma_minproductblas_chemv_work


/**
    Purpose
    -------
    magma_minproductblas_chemv performs the matrix-vector operation:
    ...
    magma_minproductFloatComplex_ptr dy, magma_minproduct_int_t incy)
{
    ...
    magma_minproductblas_chemv_work( uplo, n, alpha, dA, ldda, dx, incx, beta, dy, incy,
...
// end magma_minproductblas_chemv
