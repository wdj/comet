...
__global__
void magma_tally4_clarfg_gpu_kernel( int n, magma_tally4FloatComplex* dx0, magma_tally4FloatComplex* dx,
...
#else
    float alphar = MAGMA_tally4_C_REAL(alpha);
    float alphai = MAGMA_tally4_C_IMAG(alpha);
    if ( xnorm != 0 || alphai != 0) {
        if (i == 0) {
    ...
    magma_tally4FloatComplex_ptr dAkk)
{
    ...
    //magma_tally4blas_scnrm2_cols(n, 1, dx0, n, dxnorm);
    magma_tally4blas_scnrm2_cols(n-1, 1, dx0+1, n, dxnorm);

    magma_tally4_clarfg_gpu_kernel<<< blocks, threads,
