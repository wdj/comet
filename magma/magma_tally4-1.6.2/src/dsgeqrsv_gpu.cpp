...
/**
    ...
    @ingroup magma_tally4_dgels_driver
    ********************************************************************/
    ...
    magma_tally4_int_t *info)
{
    ...
    double *tau;
    float  *stau;
    magma_tally4Double_ptr dworkd;
    magma_tally4Float_ptr  dworks;
    magma_tally4Double_ptr dR, dT;
    magma_tally4Float_ptr  dSA, dSX, dST;
    double Xnrmv, Rnrmv;
    double          Anrm, Xnrm, Rnrm, cte, eps;
    magma_tally4_int_t     i, j, iiter, lddsa, lddsx, lddr, nb, lhwork, minmn, size, ldworkd;
    ...
    *info = 0;
    ...
    nb   = magma_tally4_get_sgeqrf_nb(m);
    minmn= min(m, n);
    ...
    if (MAGMA_tally4_SUCCESS != magma_tally4_smalloc( &dworks, size )) {
    ...
    /* dworkd(dR) = lddr*nrhs */
    ldworkd = lddr*nrhs;
    if (MAGMA_tally4_SUCCESS != magma_tally4_dmalloc( &dworkd, ldworkd )) {
        magma_tally4_free( dworks );
    ...
    size = lhwork + minmn;
    magma_tally4_smalloc_cpu( &hworks, size );
    if ( hworks == NULL ) {
        magma_tally4_free( dworks );
        magma_tally4_free( dworkd );
    ...
    Anrm = magma_tally4blas_dlange(Magma_tally4InfNorm, m, n, dA, ldda, (double*)dworkd );
    cte  = Anrm * eps * pow((double)n, 0.5) * BWDMAX;

    /*
     * Convert to single precision
     */
    magma_tally4blas_dlag2s( m, nrhs, dB, lddb, dSX, lddsx, info );
    if (*info != 0) {
        *iter = -2;
        goto FALLBACK;
    }

    magma_tally4blas_dlag2s( m, n, dA, ldda, dSA, lddsa, info );
    if (*info != 0) {
        *iter = -2;
        goto FALLBACK;
    }
    ...
    magma_tally4_sgeqrf_gpu( m, n, dSA, lddsa, stau, dST, info );
    if (*info != 0) {
        *iter = -3;
        goto FALLBACK;
    }
    ...
    magma_tally4_sgeqrs_gpu( m, n, nrhs, dSA, lddsa, stau, dST, dSX, lddsx, hworks, lhwork, info );
    if (*info != 0) {
        *iter = -3;
        goto FALLBACK;
    }
    ...
    magma_tally4blas_slag2d( n, nrhs, dSX, lddsx, dX, lddx, info );
    magma_tally4blas_dlacpy( Magma_tally4UpperLower, m, nrhs, dB, lddb, dR, lddr );
    if ( nrhs == 1 ) {
        magma_tally4_dgemv( Magma_tally4NoTrans, m, n,
                     c_neg_one, dA, ldda,
                                dX, 1,
                     c_one,     dR, 1 );
    }
    else {
        magma_tally4_dgemm( Magma_tally4NoTrans, Magma_tally4NoTrans, m, nrhs, n,
                     c_neg_one, dA, ldda,
                                dX, lddx,
                     c_one,     dR, lddr );
    }
    ...
    for( j=0; j < nrhs; j++ ) {
        ...
        i = magma_tally4_idamax ( m, dR(0,j), 1 ) - 1;
...
REFINEMENT:
    ...
    for( iiter=1; iiter < ITERMAX; ) {
        *info = 0;
        ...
        magma_tally4blas_dlag2s( m, nrhs, dR, lddr, dSX, lddsx, info );
        if (*info != 0) {
            *iter = -2;
            goto FALLBACK;
        }
        ...
        for( j=0; j < nrhs; j++ ) {
            magma_tally4blas_dsaxpycp( n, dSX(0,j), dX(0,j), dB(0,j), dR(0,j) );
        }
        // dR = dB  (whole m rows)
        magma_tally4blas_dlacpy( Magma_tally4UpperLower, m, nrhs, dB, lddb, dR, lddr );
...
FALLBACK:
    ...
    nb   = magma_tally4_get_dgeqrf_nb( m );
    size = (2*min(m, n) + (n+31)/32*32 )*nb;
    if ( size > ldworkd ) {
        magma_tally4_free( dworkd );
        if (MAGMA_tally4_SUCCESS != magma_tally4_dmalloc( &dworkd, size )) {
    ...
    size = lhwork + minmn;
    magma_tally4_dmalloc_cpu( &hworkd, size );
    if ( hworkd == NULL ) {
        magma_tally4_free( dworkd );
    ...
    magma_tally4_dgeqrf_gpu( m, n, dA, ldda, tau, dT, info );
    if (*info == 0) {
        // if m > n, then dB won't fit in dX, so solve with dB and copy n rows to dX
        magma_tally4_dgeqrs_gpu( m, n, nrhs, dA, ldda, tau, dT, dB, lddb, hworkd, lhwork, info );
        magma_tally4blas_dlacpy( Magma_tally4UpperLower, n, nrhs, dB, lddb, dX, lddx );
    }

    magma_tally4_free( dworkd );
    magma_tally4_free_cpu( hworkd );
    return *info;
}
