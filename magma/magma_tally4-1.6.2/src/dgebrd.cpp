...
/**
    ...
    @ingroup magma_tally4_dgesvd_comp
    ********************************************************************/
...
#define dA(i, j) (dA + (j)*ldda + (i))
    ...
    nb   = magma_tally4_get_dgebrd_nb(n);
    ldda = m;
    ...
    *info = 0;
    ...
    if (MAGMA_tally4_SUCCESS != magma_tally4_dmalloc( &dA, n*ldda + (m + n)*nb )) {
    ...
    for (i=0; i < (minmn - nx); i += nb) {
        ...
        if ( i > 0 ) {
            magma_tally4_dgetmatrix( nrow, nb, dA(i, i), ldda, A( i, i), lda );
            magma_tally4_dgetmatrix( nb, ncol - nb,
                              dA(i, i+nb), ldda,
                              A( i, i+nb), lda );
        }

        magma_tally4_dlabrd_gpu(nrow, ncol, nb,
        ...
        magma_tally4_dsetmatrix( nrow, nb, work  + nb, ldwrkx, dwork + nb, ldwrkx );
        magma_tally4_dsetmatrix( ncol, nb,
        ...
        if (m >= n) {
            jmax = i + nb;
            for (j = i; j < jmax; ++j) {
                *A(j, j  ) = MAGMA_tally4_D_MAKE( d[j], 0. );
                *A(j, j+1) = MAGMA_tally4_D_MAKE( e[j], 0. );
            }
        } else {
            jmax = i + nb;
            for (j = i; j < jmax; ++j) {
    ...
    if ( 0 < minmn - nx ) {
        magma_tally4_dgetmatrix( nrow, ncol, dA(i, i), ldda, A(i, i), lda );
    }
