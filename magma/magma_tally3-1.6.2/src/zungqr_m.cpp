...
/**
    ...
    T       COMPLEX_16 array, dimension (NB, min(M,N)).
            ...
            magma_tally3_zgeqrf_gpu (except stored on the CPU, not the GPU).
...
#define dT(d,i,j) (dT[d] + (i) + (j)*nb)
    ...
    *info = 0;
    ...
    magma_tally3DoubleComplex *dA[ Magma_tally3MaxGPUs ] = { NULL };
    magma_tally3DoubleComplex *dT[ Magma_tally3MaxGPUs ] = { NULL };
    magma_tally3DoubleComplex *dV[ Magma_tally3MaxGPUs ] = { NULL };
    magma_tally3DoubleComplex *dW[ Magma_tally3MaxGPUs ] = { NULL };
    ...
    for( int d = 0; d < ngpu; ++d ) {
        ...
        ldwork = nlocal[d]*ldda  // dA
               ...
               + nb*ldda         // dV
               + nb*lddwork;     // dW
        if ( MAGMA_tally3_SUCCESS != magma_tally3_zmalloc( &dA[d], ldwork )) {
    ...
    if (kk < n) {
        ...
        magma_tally3blas_zlaset( Magma_tally3Full, kk, n - kk, c_zero, c_zero, dA(dpanel, 0, di), ldda );
        trace_cpu_end( 0 );
    }
    ...
    if (kk > 0) {
        ...
        for( int d = 0; d < ngpu; ++d ) {
            ...
            magma_tally3_zsetmatrix_async( nb, min(m,n), T, nb, dT[d], nb, stream[d] );
            trace_gpu_end( d, 0 );
        }
        ...
        for( i = ki; i >= 0; i -= nb ) {
            ...
            for( int d = 0; d < ngpu; ++d ) {
                ...
                magma_tally3_zsetmatrix_async( mi, ib,
                                        A(i, i), lda,
                                        dV[d],   ldda, stream[d] );
                trace_gpu_end( d, 0 );
            }
            ...
            magma_tally3blas_zlaset( Magma_tally3Full, i,  ib, c_zero, c_zero, dA(dpanel, 0, di), ldda );
            magma_tally3blas_zlaset( Magma_tally3Full, mi, ib, c_zero, c_one,  dA(dpanel, i, di), ldda );
    ...
    magma_tally3_zgetmatrix_1D_col_bcyclic( m, n, dA, ldda, A, lda, ngpu, nb );
    trace_cpu_end( 0 );
