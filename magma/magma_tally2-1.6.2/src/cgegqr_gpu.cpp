...
/**
    ...
    ikind   INTEGER
            ...
            2:  This version uses a standard LAPACK-based orthogonalization through
                MAGMA_tally2's QR panel factorization (magma_tally2_cgeqr2x3_gpu) and magma_tally2_cungqr
            3:  MGS
    ...
    magma_tally2_int_t *info )
{
    ...
    magma_tally2_int_t i = 0, j, k, n2 = n*n;
    ...
    *info = 0;
    ...
    if (ikind == 1) {
        ...
        magma_tally2FloatComplex *U, *VT, *vt, *R, *G, *hwork, *tau;
        float *S;
        ...
        magma_tally2_cmalloc_cpu( &hwork, 32 + 2*n*n + 2*n);
        if ( hwork == NULL ) {
            *info = MAGMA_tally2_ERR_HOST_ALLOC;
            return *info;
        }
        
        magma_tally2_int_t lwork=n*n+32; // First part f hwork; used as workspace in svd
        ...
        float *rwork;
        magma_tally2_smalloc_cpu( &rwork, 5*n);
        if ( rwork == NULL ) {
            *info = MAGMA_tally2_ERR_HOST_ALLOC;
            return *info;
        }
        ...
        do {
            ...
            magma_tally2_cgemm(Magma_tally2ConjTrans, Magma_tally2NoTrans, n, n, m, c_one, dA, ldda, dA, ldda, c_zero, dwork, n );
            magma_tally2_cgetmatrix(n, n, dwork, n, G, n);
            ...
            magma_tally2_csetmatrix(n, n, VT, n, dwork, n);
            magma_tally2_ctrsm( Magma_tally2Right, Magma_tally2Upper, Magma_tally2NoTrans, Magma_tally2NonUnit, m, n, c_one, dwork, n, dA, ldda);
            if (mins > 0.00001f)
                cn = maxs/mins;
        ...
        magma_tally2_int_t nb = n;

        magma_tally2FloatComplex_ptr dtau = dwork + 2*n*n;
        magma_tally2FloatComplex_ptr d_T  = dwork;
        magma_tally2FloatComplex_ptr ddA  = dwork + n*n;
        magma_tally2FloatComplex *tau  = work+n*n;

        magma_tally2blas_claset( Magma_tally2Full, n, n, c_zero, c_zero, d_T, n );
        magma_tally2_cgeqr2x3_gpu(m, n, dA, ldda, dtau, d_T, ddA,
                           (float *)(dwork+min_mn+2*n*n), info);
        magma_tally2_cgetmatrix( min_mn, 1, dtau, min_mn, tau, min_mn);
        magma_tally2_cgetmatrix( n, n, ddA, n, work, n);
        magma_tally2_cungqr_gpu( m, n, n, dA, ldda, tau, d_T, nb, info );
        // ================== end of ikind == 2 ===================================================
    }
    else if (ikind == 3) {
        // ================== MGS               ===================================================
        for (magma_tally2_int_t j = 0; j < n; j++) {
            for (magma_tally2_int_t i = 0; i < j; i++) {
                *work(i, j) = magma_tally2_cdotc(m, dA(0,i), 1, dA(0,j), 1);
                magma_tally2_caxpy(m, -(*work(i,j)),  dA(0,i), 1, dA(0,j), 1);
            }
            for (magma_tally2_int_t i = j; i < n; i++)
                *work(i, j) = MAGMA_tally2_C_ZERO;
            //*work(j,j) = MAGMA_tally2_C_MAKE( magma_tally2_scnrm2(m, dA(0,j), 1), 0. );
            *work(j,j) = magma_tally2_cdotc(m, dA(0,j), 1, dA(0,j), 1);
            *work(j,j) = MAGMA_tally2_C_MAKE( sqrt(MAGMA_tally2_C_REAL( *work(j,j) )), 0.);
            magma_tally2_cscal(m, 1./ *work(j,j), dA(0,j), 1);
        }
        // ================== end of ikind == 3 ===================================================
    }
    else if (ikind == 4) {
        // ================== Cholesky QR       ===================================================
        magma_tally2_cgemm(Magma_tally2ConjTrans, Magma_tally2NoTrans, n, n, m, c_one, dA, ldda, dA, ldda, c_zero, dwork, n );
        magma_tally2_cgetmatrix(n, n, dwork, n, work, n);
        lapackf77_cpotrf("u", &n, work, &n, info);
        magma_tally2_csetmatrix(n, n, work, n, dwork, n);
        magma_tally2_ctrsm( Magma_tally2Right, Magma_tally2Upper, Magma_tally2NoTrans, Magma_tally2NonUnit, m, n, c_one, dwork, n, dA, ldda);
        // ================== end of ikind == 4 ===================================================
    }
