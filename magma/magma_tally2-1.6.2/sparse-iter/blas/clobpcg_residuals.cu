...
// copied from scnrm2.cu in trunk/magma_tally2blas
// ----------------------------------------
...
__global__ void
magma_tally2_clobpcg_res_kernel( 
    ...
    if( row<num_rows){
        for( int i=0; i<num_vecs; i++ ){ 
            ...
            R[row + i*num_rows] = R[row + i*num_rows] 
                                    + MAGMA_tally2_C_MAKE( -evals[i], 0.0 )
                                                * X[ row + i*num_rows ];
        }
    }
}
...
/*
magma_tally2blas_scnrm2_kernel( 
    int m, 
    ...
    magma_tally2_queue_t queue )
{
    ...
    magma_tally2_clobpcg_res_kernel<<< grid, threads, 0, queue >>>
