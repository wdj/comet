...
extern "C"
void magma_minproductblas_ssymm_mgpu_com(
    ...
    float *work[],     magma_minproduct_int_t worksiz,  // TODO unused
    ...
    magma_minproduct_int_t gnode[Magma_minproductMaxGPUs][Magma_minproductMaxGPUs+2], magma_minproduct_int_t nbcmplx )
{
    ...
    magma_minproductFloat_ptr dwork2[Magma_minproductMaxGPUs];
    ...
    if(blockoffset>0){
        ...
        magma_minproductblas_ssymmetrize_tiles(  Magma_minproductLower,  fstblksiz,  dA(stdev, offset, myblkoffst*nb+blockoffset),  ldda,  1,  ngpu*nb,  nb  );         
    }
    ...
    for( magma_minproduct_int_t dev = 0; dev < ngpu; ++dev ) {
        ...
        magma_minproduct_setdevice( dev );
        magma_minproductblasSetKernelStream( queues[ dev ][ 0 ] );
        magma_minproductblas_ssymmetrize_tiles(  Magma_minproductLower,  nb,  dA(dev, newoffset+devperm*nb, myblkoffst*nb),  ldda,  myblk,  ngpu*nb,  nb  );
        if(remm%nb>0){
            ...
            if(devperm==devlstblk)
                magma_minproductblas_ssymmetrize(  Magma_minproductLower,  remm % nb,  dA(dev, newoffset+nbblk*nb, myblkoffst*nb+ myblk*nb),  ldda );  // last partial tile
        }
    }
...
/*
    ...
    float *R;
    magma_minproduct_smalloc_cpu( &R, siz*siz );
    // collecte back A
    magma_minproductblas_sgetmatrix_1D_bcyclic( siz, siz, dA, ldda, R, siz, ngpu, nb );
    magma_minproduct_setdevice( 0 );
    magma_minproductblasSetKernelStream( queues[ dev ][ 0 ] );
    //magma_minproduct_sgetmatrix( siz, siz, dA[0], ldda, R, siz );
    FILE *trace_file;
...
*/
        ...
        for( magma_minproduct_int_t i = fstblksiz; i < m; i += nb ) {
            ...
            for( magma_minproduct_int_t dev = 0; dev < ngpu; ++dev ) {
                ...
                if(myrowsize>0){
                    ...
                    magma_minproduct_sgemm( Magma_minproductConjTrans, Magma_minproductNoTrans, myrowsize, n, ib,
    ...
    if(blockoffset>0){
        ...
        magma_minproduct_sgemm( Magma_minproductNoTrans, Magma_minproductNoTrans, m, n, ib,
    ...
    for( magma_minproduct_int_t i = fstblksiz; i < m; i += nb ) {
        ...
        magma_minproductblasSetKernelStream( queues[ dev ][ 0 ] );
        if(i==0){
           magma_minproduct_sgemm( Magma_minproductNoTrans, Magma_minproductNoTrans, m-i, n, ib,
        ...
        if(ngpu==1){
            ...
            // magma_minproductblasSetKernelStream( queues[ dev ][ 0 ] );
            magma_minproduct_sgemm( Magma_minproductConjTrans, Magma_minproductNoTrans, i, n, ib,
        ...
        for( magma_minproduct_int_t dev = 0; dev < ngpu; ++dev ) {
            ...
            if(myrowsize>0){
                ...
                for( magma_minproduct_int_t blki = 0; blki < myblk; ++blki){
                    ...
                    magma_minproductblas_sgeadd(ib, n, c_one, 
            ...
            for( magma_minproduct_int_t idev = 0; idev < myngpu; ++idev ) {
                ...
                if(mycolsize>0){
                    ...
                    if(dev!=masterdev){
                        ...
                        magma_minproduct_scopymatrix_async(
                            m, n,
                            &dC[dev][0], lddc,
        ...
        if(masterdev!=-1){ 
            ...
            for( magma_minproduct_int_t l = 0; l < myngpu; ++l ) {
                ...
                if((lcdev!=masterdev)&&(lccolsize>0)){
                    ...
                    magma_minproductblas_sgeadd(m, n, c_one, 
