...
void init_matrix( int nopiv, int m, int n, float *h_A, magma_minproduct_int_t lda )
{
    assert( m == n );
    ...
    if (nopiv) {
        ...
        magma_minproduct_smake_symmetric( n, h_A, lda );
    }
}
    ...
    magma_minproduct_int_t *ipiv )
{
    ...
    if (nopiv) {
        if (upper) {
            blasf77_strsm( Magma_minproductLeftStr, Magma_minproductUpperStr, 
                           Magma_minproductConjTransStr, Magma_minproductUnitStr, 
                           &n, &ione, &c_one,
                           A, &lda, x, &n );
            for (int i=0; i<n; i++) x[i] = MAGMA_minproduct_S_DIV( x[i], A[i+i*lda] );
            blasf77_strsm( Magma_minproductLeftStr, Magma_minproductUpperStr, 
                           Magma_minproductNoTransStr, Magma_minproductUnitStr, 
                           &n, &ione, &c_one,
                           A, &lda, x, &n );
        } else {
            blasf77_strsm( Magma_minproductLeftStr, Magma_minproductLowerStr, 
                           Magma_minproductNoTransStr, Magma_minproductUnitStr, 
                           &n, &ione, &c_one,
                           A, &lda, x, &n );
            for (int i=0; i<n; i++) x[i] = MAGMA_minproduct_S_DIV( x[i], A[i+i*lda] );
    ...
    norm_A = lapackf77_slange( Magma_minproductFullStr, &n, &n, A, &lda, work );
    norm_r = lapackf77_slange( Magma_minproductFullStr, &n, &ione, b, &n, work );
    norm_x = lapackf77_slange( Magma_minproductFullStr, &n, &ione, x, &n, work );
    
    //printf( "r=\n" ); magma_minproduct_sprint( 1, n, b, 1 );
...
float get_LDLt_error(int nopiv, magma_minproduct_uplo_t uplo, magma_minproduct_int_t N,
                      float *LD, magma_minproduct_int_t lda,
                      magma_minproduct_int_t *ipiv)
{
    ...
    if (uplo == Magma_minproductUpper) {
        for (j=N-1; j>=0; j--) {
            ...
            if (piv < 0) {
                ...
                D(j,j)     = LD(j,j);
                D(j,j-1)   = MAGMA_minproduct_S_CNJG(LD(j-1,j));
                D(j-1,j)   = LD(j-1,j);
        ...
        if (nopiv) {
            // compute W = D*U
            blasf77_sgemm(Magma_minproductNoTransStr, Magma_minproductNoTransStr, &N, &N, &N,
                          &c_one, D, &N, L, &N, &c_zero, LD, &lda);
            // compute D = U'*W
            blasf77_sgemm(Magma_minproductConjTransStr, Magma_minproductNoTransStr, &N, &N, &N,
            ...
            // compute D = W*U'
            blasf77_sgemm(Magma_minproductNoTransStr, Magma_minproductConjTransStr, &N, &N, &N,
    ...
    // compute norm of A
    matnorm = lapackf77_slange(Magma_minproductFullStr, &N, &N, A, &lda, work);

    for( j = 0; j < N; j++ ) {
        for( i = 0; i < N; i++ ) {
            D(i,j) = MAGMA_minproduct_S_SUB( D(i,j), A(i,j) );
        }
    }
    residual = lapackf77_slange(Magma_minproductFullStr, &N, &N, D, &N, work);
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( opts.lapack ) {
                ...
                lwork = (int)MAGMA_minproduct_S_REAL(temp);
            ...
            if (nopiv) {
                ...
                magma_minproduct_ssytrf_nopiv( opts.uplo, N, h_A, lda, &info);
                gpu_time = magma_minproduct_wtime() - gpu_time;
            } else if (cpu) {
                // CPU-interface to Bunch-Kauffman LDLt
                magma_minproduct_setdevice(0);
                gpu_time = magma_minproduct_wtime();
                magma_minproduct_ssytrf( opts.uplo, N, h_A, lda, ipiv, &info);
                gpu_time = magma_minproduct_wtime() - gpu_time;
            } else if (nopiv_gpu) {
                ...
                if (MAGMA_minproduct_SUCCESS != magma_minproduct_smalloc( &d_A, N*ldda  )) {
                    printf( " failed to allocate d_A(%dx%d)\n", (int) N, (int) ldda );
                    return 0;
                }
                magma_minproduct_ssetmatrix(N, N, h_A, lda, d_A, ldda);
                gpu_time = magma_minproduct_wtime();
                magma_minproduct_ssytrf_nopiv_gpu( opts.uplo, N, d_A, ldda, &info);
                gpu_time = magma_minproduct_wtime() - gpu_time;
                magma_minproduct_sgetmatrix(N, N, d_A, ldda, h_A, lda);
                magma_minproduct_free( d_A );
            } else if (row) {
                magma_minproduct_setdevice(0);
                //magma_minproduct_ssytrf_gpu_row( opts.uplo, N, h_A, lda, ipiv, work, lwork, &info);
            } else {
                magma_minproduct_setdevice(0);
                //magma_minproduct_ssytrf_hybrid( opts.uplo, N, h_A, lda, ipiv, work, lwork, &info);
            }
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_minproduct_ssytrf returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
