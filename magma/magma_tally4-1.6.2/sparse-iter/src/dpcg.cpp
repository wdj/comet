    ...
    magma_tally4_queue_t queue )
{
    ...
    CHECK( magma_tally4_dvinit( &rt,Magma_tally4_DEV, A.num_rows, b.num_cols, c_zero, queue ));
    ...
    CHECK( magma_tally4_dvinit( &h, Magma_tally4_DEV, A.num_rows, b.num_cols, c_zero, queue ));
    ...
    magma_tally4_dcopy( dofs, h.dval, 1, p.dval, 1 );                    // p = h
    nom = MAGMA_tally4_D_REAL( magma_tally4_ddot(dofs, r.dval, 1, h.dval, 1) );
    ...
    do
    {
        ...
        gammanew = MAGMA_tally4_D_REAL( magma_tally4_ddot(dofs, r.dval, 1, h.dval, 1) );
                                                            // gn = < r,h>

        if ( solver_par->numiter==1 ) {
            magma_tally4_dcopy( dofs, h.dval, 1, p.dval, 1 );                    // p = h
        } else {
            beta = MAGMA_tally4_D_MAKE(gammanew/gammaold, 0.);       // beta = gn/go
            magma_tally4_dscal(dofs, beta, p.dval, 1);            // p = beta*p
            magma_tally4_daxpy(dofs, c_one, h.dval, 1, p.dval, 1); // p = p + h
        }
