...
/**
    ...
    @ingroup magma_tally3_dgeev_driver
    ********************************************************************/
    ...
    magma_tally3_int_t *info )
{
    ...
    *info = 0;
    ...
    if (*info == 0) {
        minwrk = (2 +   nb)*n;
        optwrk = (2 + 2*nb)*n;
        work[0] = MAGMA_tally3_D_MAKE( (double) optwrk, 0. );
    ...
    double *dT;
    if (MAGMA_tally3_SUCCESS != magma_tally3_dmalloc( &dT, nb*n )) {
    ...
    double *T;
    if (MAGMA_tally3_SUCCESS != magma_tally3_dmalloc_cpu( &T, nb*n )) {
    ...
    if (wantvl) {
        ...
        lapackf77_dlacpy( Magma_tally3LowerStr, &n, &n, A, &lda, VL, &ldvl );
        ...
        flops_start( flop_unghr );
            ...
            // Version 3 - LAPACK consistent MAGMA_tally3 HRD + T matrices stored
            magma_tally3_dorghr( n, ilo, ihi, VL, ldvl, &work[itau], dT, nb, &ierr );
        #elif defined(Version5)
            // Version 5 - Multi-GPU, T on host
            magma_tally3_dorghr_m( n, ilo, ihi, VL, ldvl, &work[itau], T, nb, &ierr );
        #endif
        ...
        #elif defined(Version3) || defined(Version4)
            // Version 3 - LAPACK consistent MAGMA_tally3 HRD + T matrices stored
            magma_tally3_dorghr( n, ilo, ihi, VR, ldvr, &work[itau], dT, nb, &ierr );
        #elif defined(Version5)
            // Version 5 - Multi-GPU, T on host
            magma_tally3_dorghr_m( n, ilo, ihi, VR, ldvr, &work[itau], T, nb, &ierr );
        #endif
        time_sum += timer_stop( time_unghr );
        flop_sum += flops_stop( flop_unghr );
    ...
    if (wantvl || wantvr) {
        ...
        liwrk = lwork - iwrk;
        #if TREVC_VERSION == 1
        lapackf77_dtrevc( lapack_side_const_tally3(side), "B", select, &n, A, &lda, VL, &ldvl,
                          VR, &ldvr, &n, &nout, &work[iwrk], &ierr );
        #elif TREVC_VERSION == 2
        lapackf77_dtrevc3( lapack_side_const_tally3(side), "B", select, &n, A, &lda, VL, &ldvl,
                           VR, &ldvr, &n, &nout, &work[iwrk], &liwrk, &ierr );
        #elif TREVC_VERSION == 3
        magma_tally3_dtrevc3( side, Magma_tally3BacktransVec, select, n, A, lda, VL, ldvl,
                       VR, ldvr, n, &nout, &work[iwrk], liwrk, &ierr );
        #elif TREVC_VERSION == 4
        magma_tally3_dtrevc3_mt( side, Magma_tally3BacktransVec, select, n, A, lda, VL, ldvl,
                          VR, ldvr, n, &nout, &work[iwrk], liwrk, &ierr );
        #elif TREVC_VERSION == 5
        magma_tally3_dtrevc3_mt_gpu( side, Magma_tally3BacktransVec, select, n, A, lda, VL, ldvl,
                              VR, ldvr, n, &nout, &work[iwrk], liwrk, &ierr );
        #else
    ...
    if (wantvl) {
        ...
        for (i = 0; i < n; ++i) {
            if ( wi[i] == 0. ) {
                scl = 1. / magma_tally3_cblas_dnrm2( n, VL(0,i), 1 );
                blasf77_dscal( &n, &scl, VL(0,i), &ione );
            }
            else if ( wi[i] > 0. ) {
                d__1 = magma_tally3_cblas_dnrm2( n, VL(0,i), 1 );
                d__2 = magma_tally3_cblas_dnrm2( n, VL(0,i+1), 1 );
    ...
    if (wantvr) {
        ...
        for (i = 0; i < n; ++i) {
            if ( wi[i] == 0. ) {
                scl = 1. / magma_tally3_cblas_dnrm2( n, VR(0,i), 1 );
                blasf77_dscal( &n, &scl, VR(0,i), &ione );
            }
            else if ( wi[i] > 0. ) {
                d__1 = magma_tally3_cblas_dnrm2( n, VR(0,i), 1 );
                d__2 = magma_tally3_cblas_dnrm2( n, VR(0,i+1), 1 );
                scl = 1. / lapackf77_dlapy2( &d__1, &d__2 );
                blasf77_dscal( &n, &scl, VR(0,i), &ione );
