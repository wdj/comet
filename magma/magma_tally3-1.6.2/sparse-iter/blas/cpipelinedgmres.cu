...
__global__ void
magma_tally3_cpipelined_correction( 
    int n,  
    int k,
    ...
    magma_tally3FloatComplex * v )
{
    ...
    temp[ i ] = ( i < k ) ? skp[ i ] * skp[ i ] : MAGMA_tally3_C_MAKE( 0.0, 0.0);
    __syncthreads();
    ...
    if( i == 0 ){
        tmp = MAGMA_tally3_C_REAL( temp[ i ] );
        zz = MAGMA_tally3_C_REAL( skp[(k)] );
        skp[k] = MAGMA_tally3_C_MAKE( sqrt(zz-tmp),0.0 );
    }
}

__global__ void
magma_tally3_cpipelined_copyscale( 
    int n,  
    int k,
    magma_tally3FloatComplex * skp, 
    magma_tally3FloatComplex * r,
    magma_tally3FloatComplex * v )
{
    ...
    magma_tally3FloatComplex rr=skp[k];
...
__global__ void
magma_tally3_cpipelinedscnrm2_kernel( 
    int m, 
    magma_tally3FloatComplex * da, 
    int ldda, 
    magma_tally3FloatComplex * dxnorm )
{
    const int i = threadIdx.x;
    magma_tally3FloatComplex_ptr dx = da + blockIdx.x * ldda;
    ...
    if (i==0)
        dxnorm[blockIdx.x] = MAGMA_tally3_C_MAKE( sqrt(sum[0]), 0.0 );
}

//----------------------------------------------------------------------------//

__global__ void
magma_tally3_cpipelinesscale( 
    int n, 
    magma_tally3FloatComplex * r, 
    ...
    magma_tally3_queue_t queue )
{
    ...
    unsigned int Ms =   Bs.x * sizeof( magma_tally3FloatComplex ); 

    dim3 Gs2( magma_tally3_ceildiv( n, BLOCK_SIZE ) );


    magma_tally3_cpipelined_correction<<<Gs, Bs, Ms, queue >>>
                                            ( n, k, skp, r, v );
    magma_tally3_cpipelined_copyscale<<<Gs2, Bs, 0, queue >>>
                                            ( n, k, skp, r, v );
...
magma_tally3_scnrm2scale(
    ...
    magma_tally3FloatComplex_ptr r, 
    int lddr, 
    magma_tally3FloatComplex_ptr drnorm,
    magma_tally3_queue_t queue )
{
    dim3  blocks( 1 );
    dim3 threads( 512 );
    magma_tally3_cpipelinedscnrm2_kernel<<< blocks, threads, 0, queue >>>
                                ( m, r, lddr, drnorm );

    dim3 Bs( BLOCK_SIZE );
    dim3 Gs2( magma_tally3_ceildiv( m, BLOCK_SIZE ) );
    magma_tally3_cpipelinesscale<<<Gs2, Bs, 0, queue >>>( m, r, drnorm );
