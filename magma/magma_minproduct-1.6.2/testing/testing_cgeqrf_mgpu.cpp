...
int main( int argc, char** argv )
{
    ...
    magma_minproductFloatComplex_ptr d_lA[ Magma_minproductMaxGPUs ];
    magma_minproduct_int_t M, N, n2, lda, ldda, n_local, ngpu;
    magma_minproduct_int_t info, min_mn, nb, lhwork;
    magma_minproduct_int_t ione     = 1;
    magma_minproduct_int_t ISEED[4] = {0,0,0,1}, ISEED2[4];
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for( int dev = 0; dev < ngpu; dev++ ) {
                ...
                TESTING_MALLOC_DEV( d_lA[dev], magma_minproductFloatComplex, ldda*n_local );
            }
            ...
            /* =====================================================================
               ...
               =================================================================== */
            if ( opts.lapack ) {
                magma_minproductFloatComplex *tau2;
                TESTING_MALLOC_CPU( tau2, magma_minproductFloatComplex, min_mn );
            ...
            /* ====================================================================
               ...
               =================================================================== */
            magma_minproduct_csetmatrix_1D_col_bcyclic( M, N, h_R, lda, d_lA, ldda, ngpu, nb );
            
            gpu_time = magma_minproduct_wtime();
            magma_minproduct_cgeqrf2_mgpu( ngpu, M, N, d_lA, ldda, tau, &info );
            gpu_time = magma_minproduct_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_minproduct_cgeqrf2 returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
            
            magma_minproduct_cgetmatrix_1D_col_bcyclic( M, N, d_lA, ldda, h_R, lda, ngpu, nb );
            ...
            if ( opts.check == 1 && M >= N ) {
                /* =====================================================================
                   ...
                   =================================================================== */
                magma_minproduct_int_t lwork = n2+N;
                magma_minproductFloatComplex *h_W1, *h_W2, *h_W3;
                float *h_RW, results[2];
            
                TESTING_MALLOC_CPU( h_W1, magma_minproductFloatComplex, n2    ); // Q
                TESTING_MALLOC_CPU( h_W2, magma_minproductFloatComplex, n2    ); // R
                TESTING_MALLOC_CPU( h_W3, magma_minproductFloatComplex, lwork ); // WORK
