...
int main( int argc, char** argv)
{
    ...
    magma_minproductDoubleComplex *h_A, *h_A2, *h_B, *h_X, *h_R;
    magma_minproductDoubleComplex_ptr d_A, d_B, d_X, d_T;
    magma_minproductFloatComplex  *d_SA, *d_SB;
    magma_minproductDoubleComplex *h_workd, *tau, tmp[1];
    magma_minproductFloatComplex  *h_works;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lddx   = ((N+31)/32) * 32;
            nb     = max( magma_minproduct_get_zgeqrf_nb( M ), magma_minproduct_get_cgeqrf_nb( M ) );
            ...
            TESTING_MALLOC_CPU( h_workd, magma_minproductDoubleComplex, lhwork   );
            h_works = (magma_minproductFloatComplex*)h_workd;
            ...
            TESTING_MALLOC_DEV( d_T, magma_minproductDoubleComplex, ( 2*min_mn + (N+31)/32*32 )*nb );
            ...
            magma_minproduct_zcgeqrsv_gpu( M, N, nrhs,
                                d_A, ldda, d_B, lddb,
                                d_X, lddx, &qrsv_iters, &info );
            gpu_time = magma_minproduct_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_minproduct_zcgeqrsv returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
            
            // compute the residual
            magma_minproduct_zgetmatrix( N, nrhs, d_X, lddx, h_X, ldb );
            ...
            gpu_time = magma_minproduct_wtime();
            magma_minproduct_zgels_gpu( Magma_minproductNoTrans, M, N, nrhs, d_A, ldda,
            ...
            TESTING_MALLOC_DEV( d_SA, magma_minproductFloatComplex, ldda*N    );
            TESTING_MALLOC_DEV( d_SB, magma_minproductFloatComplex, lddb*nrhs );
            magma_minproductblas_zlag2c( M, N,    d_A, ldda, d_SA, ldda, &info );
            magma_minproductblas_zlag2c( N, nrhs, d_B, lddb, d_SB, lddb, &info );
            
            gpu_time = magma_minproduct_wtime();
            magma_minproduct_cgels_gpu( Magma_minproductNoTrans, M, N, nrhs, d_SA, ldda,
