...
*********   (workspace) If FAST_HEMV is defined DZ should be (LDDZ, max(1,N)) in both cases.
    ...
    lddz    INTEGER
            ...
            JOBZ = Magma_minproductVec, LDDZ >= max(1,N).
...
magma_minproduct_zheevx_gpu(
    ...
    double *rwork, magma_minproduct_int_t *iwork, magma_minproduct_int_t *ifail,
    magma_minproduct_int_t *info)
{
    ...
    const char* order_;
    magma_minproduct_int_t indd, inde;
    magma_minproduct_int_t imax;
    magma_minproduct_int_t lopt, itmp1, indee;
    magma_minproduct_int_t lower, wantz;
    magma_minproduct_int_t i, j, jj, i__1;
    magma_minproduct_int_t alleig, valeig, indeig;
    magma_minproduct_int_t iscale, indibl;
    magma_minproduct_int_t indiwk, indisp, indtau;
    magma_minproduct_int_t indrwk, indwrk;
    magma_minproduct_int_t llwork, nsplit;
    ...
    *info = 0;
    ...
    work[0] = MAGMA_minproduct_Z_MAKE( lopt, 0 );
    ...
    *m = 0;
    ...
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc( &dwork, n )) {
        fprintf (stderr, "!!!! device memory allocation error (magma_minproduct_zheevx_gpu)\n");
...
#ifdef FAST_HEMV
    magma_minproduct_zhetrd2_gpu(uplo, n, dA, ldda, &rwork[indd], &rwork[inde],
                      &work[indtau], wA, ldwa, &work[indwrk], llwork, dZ, lddz*n, &iinfo);
#else
    magma_minproduct_zhetrd_gpu (uplo, n, dA, ldda, &rwork[indd], &rwork[inde],
                      &work[indtau], wA, ldwa, &work[indwrk], llwork, &iinfo);
#endif

    lopt = n + (magma_minproduct_int_t)MAGMA_minproduct_Z_REAL(work[indwrk]);
    ...
    if ((alleig || (indeig && il == 1 && iu == n)) && abstol <= 0.) {
        ...
        if (! wantz) {
            ...
            if (*info == 0) {
                ...
                magma_minproduct_zsetmatrix( n, n, wZ, ldwz, dZ, lddz );
            }
        }
        if (*info == 0) {
            *m = n;
