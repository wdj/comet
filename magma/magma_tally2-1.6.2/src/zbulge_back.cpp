...
static void *magma_tally2_zapplyQ_parallel_section(void *arg);

static void magma_tally2_ztile_bulge_applyQ(magma_tally2_int_t core_id, magma_tally2_side_t side, magma_tally2_int_t n_loc, magma_tally2_int_t n, magma_tally2_int_t nb, magma_tally2_int_t Vblksiz,
                                     magma_tally2DoubleComplex *E, magma_tally2_int_t lde, magma_tally2DoubleComplex *V, magma_tally2_int_t ldv,
                                     magma_tally2DoubleComplex *TAU, magma_tally2DoubleComplex *T, magma_tally2_int_t ldt);

//////////////////////////////////////////////////////////////////////////////////////////////////////////

typedef struct magma_tally2_zapplyQ_data_s {
    magma_tally2_int_t threads_num;
    magma_tally2_int_t n;
    magma_tally2_int_t ne;
    magma_tally2_int_t n_gpu;
    magma_tally2_int_t nb;
    magma_tally2_int_t Vblksiz;
    magma_tally2DoubleComplex* E;
    magma_tally2_int_t lde;
    ...
    magma_tally2DoubleComplex* dE;
    magma_tally2_int_t ldde;
    pthread_barrier_t barrier;
} magma_tally2_zapplyQ_data;

void magma_tally2_zapplyQ_data_init(
    magma_tally2_zapplyQ_data *zapplyQ_data, magma_tally2_int_t threads_num, magma_tally2_int_t n, magma_tally2_int_t ne, magma_tally2_int_t n_gpu,
    magma_tally2_int_t nb, magma_tally2_int_t Vblksiz, magma_tally2DoubleComplex *E, magma_tally2_int_t lde,
    magma_tally2DoubleComplex *V, magma_tally2_int_t ldv, magma_tally2DoubleComplex *TAU,
    magma_tally2DoubleComplex *T, magma_tally2_int_t ldt, magma_tally2DoubleComplex *dE, magma_tally2_int_t ldde)
{
    ...
    magma_tally2_int_t count = zapplyQ_data->threads_num;
...
void magma_tally2_zapplyQ_data_destroy(
    magma_tally2_zapplyQ_data *zapplyQ_data)
{
    pthread_barrier_destroy(&(zapplyQ_data->barrier));
}

typedef struct magma_tally2_zapplyQ_id_data {
    magma_tally2_int_t id;
    magma_tally2_zapplyQ_data* data;
} magma_tally2_zapplyQ_id_data;

void magma_tally2_zapplyQ_id_data_init(
    magma_tally2_zapplyQ_id_data *zapplyQ_id_data,
    magma_tally2_int_t id, magma_tally2_zapplyQ_data* data)
{
...
//#endif
    ...
    magma_tally2_int_t gpu_cpu_perf = magma_tally2_get_zbulge_gcperf();
    if (threads > 1) {
...
//$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
    ...
    if (n_gpu < ne) {
        ...
        #endif
        magma_tally2_zapplyQ_data data_applyQ;
        magma_tally2_zapplyQ_data_init(&data_applyQ, threads, n, ne, n_gpu, nb, Vblksiz, Z, ldz, V, ldv, TAU, T, ldt, dZ, lddz);

        magma_tally2_zapplyQ_id_data* arg;
        magma_tally2_malloc_cpu((void**) &arg, threads*sizeof(magma_tally2_zapplyQ_id_data));
        ...
        for (magma_tally2_int_t thread = 1; thread < threads; thread++) {
            magma_tally2_zapplyQ_id_data_init(&(arg[thread]), thread, &data_applyQ);
            pthread_create(&thread_id[thread], &thread_attr, magma_tally2_zapplyQ_parallel_section, &arg[thread]);
        }
        magma_tally2_zapplyQ_id_data_init(&(arg[0]), 0, &data_applyQ);
        magma_tally2_zapplyQ_parallel_section(&arg[0]);
        ...
        magma_tally2_zapplyQ_data_destroy(&data_applyQ);


        magma_tally2_zsetmatrix(n, ne-n_gpu, Z + n_gpu*ldz, ldz, dZ + n_gpu*ldz, lddz);

        /*============================
         *  use only GPU
         *==========================*/
    } else {
        magma_tally2_zsetmatrix(n, ne, Z, ldz, dZ, lddz);
        magma_tally2_zbulge_applyQ_v2(Magma_tally2Left, ne, n, nb, Vblksiz, dZ, lddz, V, ldv, T, ldt, info);
...
static void *magma_tally2_zapplyQ_parallel_section(void *arg)
{
    magma_tally2_int_t my_core_id   = ((magma_tally2_zapplyQ_id_data*)arg) -> id;
    magma_tally2_zapplyQ_data* data = ((magma_tally2_zapplyQ_id_data*)arg) -> data;
    ...
    magma_tally2DoubleComplex *E         = data -> E;
    ...
    magma_tally2DoubleComplex *dE        = data -> dE;
    magma_tally2_int_t ldde           = data -> ldde;
    ...
    if (my_core_id == 0) {
        ...
        magma_tally2_zsetmatrix(n, n_gpu, E, lde, dE, ldde);
        magma_tally2_zbulge_applyQ_v2(Magma_tally2Left, n_gpu, n, nb, Vblksiz, dE, ldde, V, ldv, T, ldt, &info);
        magma_tally2_device_sync();
        ...
        magma_tally2DoubleComplex* E_loc = E + (n_gpu+ n_loc * (my_core_id-1))*lde;
        n_loc = min(n_loc,n_cpu - n_loc * (my_core_id-1));

        magma_tally2_ztile_bulge_applyQ(my_core_id, Magma_tally2Left, n_loc, n, nb, Vblksiz, E_loc, lde, V, ldv, TAU, T, ldt);
...
#define TAU(m)   &(TAU[(m)])
#define T(m)     &(T[(m)])
static void magma_tally2_ztile_bulge_applyQ(
    ...
    //, magma_tally2_int_t* info)
{
    ...
    magma_tally2DoubleComplex *work, *work2;

    magma_tally2_zmalloc_cpu(&work, lwork);
    magma_tally2_zmalloc_cpu(&work2, lwork);
    ...
    for (magma_tally2_int_t i = 0; i < nbchunk; i++) {
        ...
        if (side == Magma_tally2Left) {
            for (bg = nbGblk; bg > 0; bg--) {
                ...
                if (bg == nbGblk) rownbm = magma_tally2_ceildiv((n-(firstcolj)),nb);  // last blk has size=1 used for complex to handle A(N,N-1)
