...
//===================================================================================================
static __device__
void zlarft_gemvcolwise_device( int m, magma_minproductDoubleComplex *v, magma_minproductDoubleComplex *tau,
                         magma_minproductDoubleComplex *c, int ldc, magma_minproductDoubleComplex *T, int ldt, int step )
{
    ...
    if ( !MAGMA_minproduct_Z_EQUAL(*tau, MAGMA_minproduct_Z_ZERO) ) {
        if(thblk<step){    
            const int tx = threadIdx.x;
            magma_minproductDoubleComplex *dc = c + blockIdx.x * ldc;
            ...
            for( int j = tx+1; j < m; j += BLOCK_SIZE ){
                tmp +=  MAGMA_minproduct_Z_CNJG( v[j] ) * dc[j];
            }
            sum[tx] = tmp;
            magma_minproduct_sum_reduce< BLOCK_SIZE >( tx, sum );
            #if defined (use_gemm_larft)
            *(T+thblk) = MAGMA_minproduct_Z_CNJG(sum[0]);
            #else
            tmp = - MAGMA_minproduct_Z_CNJG(*tau) * sum[0]; 
            *(T+thblk) = MAGMA_minproduct_Z_CNJG(tmp); // T = - tau(tx) * V(tx:n,1:tx-1)' * V(tx:n,tx) = tmp'
            //*(T+thblk) = - MAGMA_minproduct_Z_CNJG(sum[0]) * (*tau); // T = - tau(tx) * V(tx:n,1:tx-1)' * V(tx:n,tx) = tmp'
            #endif
        }
        else{
            #if defined (use_gemm_larft)
            *(T+thblk) = MAGMA_minproduct_Z_ONE;
            #else
            *(T+thblk) = *tau;
            #endif
        }
    ...
    {
        *(T+thblk) = MAGMA_minproduct_Z_ZERO;
    }
}
//===================================================================================================
__global__
void zlarft_gemvcolwise_kernel( int m, magma_minproductDoubleComplex *v, int ldv, magma_minproductDoubleComplex *tau,
                          magma_minproductDoubleComplex *T, int ldt, int step )
{
    zlarft_gemvcolwise_device(m, v+step+step*ldv, tau+step, v+step, ldv, T+step*ldt, ldt, step);
}
//===================================================================================================
__global__
void zlarft_gemvcolwise_kernel_batched( int m, magma_minproductDoubleComplex **v_array, int ldv, magma_minproductDoubleComplex **tau_array,
                          magma_minproductDoubleComplex **T_array, int ldt, int step )
{
    ...
    magma_minproductDoubleComplex *tau)
{
    ...
    zlarft_gemvcolwise_kernel<<< grid, threads, 0, magma_minproduct_stream >>>( m, v, ldv, tau, T, ldt, step);
...
zlarft_gemvrowwise_device(
    int m, int i,
    magma_minproductDoubleComplex *tau, 
    magma_minproductDoubleComplex *v_ptr, int ldv, 
    magma_minproductDoubleComplex *x_ptr, int incx,
    magma_minproductDoubleComplex *T_ptr, int ldt,
    magma_minproductDoubleComplex *W, magma_minproductDoubleComplex* sdata)
{
    int tx = threadIdx.x; 
    int ty = threadIdx.y; 
    ...
    if(tx < zgemv_bs)
    {
        ...
        {
            res += MAGMA_minproduct_Z_CNJG (v_ptr[s]) * x_ptr[s*incx];
        }
    ...
    magma_minproduct_sum_reduce<zgemv_bs>(tx, &(sdata[ty*zgemv_bs+0]));
    ...
    magma_minproductDoubleComplex *T, int ldt)
{
    ...
    magma_minproductDoubleComplex *W =  T +i*ldt;

    magma_minproductDoubleComplex *sdata = (magma_minproductDoubleComplex*)shared_data;
...
zlarft_gemvrowwise_kernel_batched(
    ...
    magma_minproductDoubleComplex **T_array, int ldt)
{

    int batchid = blockIdx.z;

    magma_minproductDoubleComplex *W =  T_array[batchid] +i*ldt;
    ...
    magma_minproductDoubleComplex *W)
{
    ...
    zlarft_gemvrowwise_kernel <<< grid, threads, sizeof(magma_minproductDoubleComplex)*zgemv_bs*(i+1), magma_minproduct_stream>>>(m, i, tau, v, ldv, T, ldt);
}
//===================================================================================================
extern "C"
    ...
    magma_minproduct_int_t batchCount, magma_minproduct_queue_t queue)
{
    ...
    */
    zlarft_gemvrowwise_kernel_batched <<< grid, threads, sizeof(magma_minproductDoubleComplex)*zgemv_bs*(i+1), queue>>>(m, i,  tau_array, v_array, ldv, T_array, ldt);
}
//===================================================================================================
    ...
    magma_minproductDoubleComplex *T, int ldt)
{
    ...
    for(int i=1; i<k;i++)
    {
        ...
        magma_minproductDoubleComplex *v_ptr = v;

        v_ptr += i;

        magma_minproductDoubleComplex *x_ptr = v_ptr + i * ldv;
            
        res = MAGMA_minproduct_Z_ZERO;
    ...
    magma_minproductDoubleComplex *T, int ldt)
{
    ...
    zlarft_gemv_loop_inside_kernel<<<grid, threads, sizeof(magma_minproductDoubleComplex) * (zgemv_bs*(k+1)), magma_minproduct_stream>>>(n, k, tau, v, ldv, T, ldt); 
}
//===================================================================================================
extern "C"
    ...
    magma_minproductDoubleComplex **T_array, int ldt, magma_minproduct_int_t batchCount, magma_minproduct_queue_t queue)
{
    ...
    dim3 grid(1, 1, batchCount);
    dim3 threads(zgemv_bs, max(k,1), 1);
    zlarft_gemv_loop_inside_kernel_batched<<<grid, threads, sizeof(magma_minproductDoubleComplex) * (zgemv_bs*(k+1)), queue>>>(n, k, tau_array, v_array, ldv, T_array, ldt); 
}
//===================================================================================================
...
zlarft_ztrmv_sm32x32_device(
    int n, int k, magma_minproductDoubleComplex *tau,
    magma_minproductDoubleComplex *Tin, int ldtin,  magma_minproductDoubleComplex *Tout, int ldtout )
{
    int tx = threadIdx.x; 
    magma_minproductDoubleComplex *sdata = (magma_minproductDoubleComplex*)shared_data;
    magma_minproductDoubleComplex res;
...
zlarft_ztrmv_sm32x32_kernel_batched(
    int n, int k, magma_minproductDoubleComplex **tau_array,
    magma_minproductDoubleComplex **Tin_array, int ldtin,  magma_minproductDoubleComplex **Tout_array, int ldtout )
{
    int batchId = blockIdx.z;
    ...
    magma_minproductDoubleComplex *Tout, magma_minproduct_int_t ldtout)
{
    ...
    zlarft_ztrmv_sm32x32_kernel <<< grid, threads, sizeof(magma_minproductDoubleComplex)*(m*m), magma_minproduct_stream >>> (m, n,  tau, Tin, ldtin, Tout, ldtout);
}
//===================================================================================================
extern "C"
    ...
    magma_minproduct_int_t batchCount, magma_minproduct_queue_t queue)
{
    ...
    dim3 grid(1, 1, batchCount);
    dim3 threads(max(m,1), 1, 1);
    zlarft_ztrmv_sm32x32_kernel_batched <<< grid, threads, sizeof(magma_minproductDoubleComplex)*(m*m), queue >>> (m, n,  tau_array, Tin_array, ldtin, Tout_array, ldtout);
}
//===================================================================================================
...
zlarft_recztrmv_sm32x32_device(
    int m, int n, magma_minproductDoubleComplex *tau,
    magma_minproductDoubleComplex *Trec, int ldtrec, magma_minproductDoubleComplex *Ttri, int ldttri)
{
    int tx = threadIdx.x; 
    magma_minproductDoubleComplex *sdata = (magma_minproductDoubleComplex*)shared_data;
    magma_minproductDoubleComplex res;
...
zlarft_recztrmv_sm32x32_kernel_batched(
    int m, int n, magma_minproductDoubleComplex **tau_array,
    magma_minproductDoubleComplex **Trec_array, int ldtrec, magma_minproductDoubleComplex **Ttri_array, int ldttri)
{
    int batchId = blockIdx.z;
    ...
    magma_minproductDoubleComplex *Ttri, magma_minproduct_int_t ldttri)
{
    ...
    dim3 grid(1);
    dim3 threads(max(m,1), 1, 1);
    zlarft_recztrmv_sm32x32_kernel <<< grid, threads, sizeof(magma_minproductDoubleComplex)*(m*n), magma_minproduct_stream >>> (m, n,  tau, Trec, ldtrec, Ttri, ldttri);
}
//===================================================================================================
extern "C"
    ...
    magma_minproduct_int_t batchCount, magma_minproduct_queue_t queue)
{
    ...
    dim3 grid(1, 1, batchCount);
    dim3 threads(max(m,1), 1, 1);
    zlarft_recztrmv_sm32x32_kernel_batched <<< grid, threads, sizeof(magma_minproductDoubleComplex)*(m*n), queue >>> (m, n,  tau_array, Trec_array, ldtrec, Ttri_array, ldttri);
}
//===================================================================================================
