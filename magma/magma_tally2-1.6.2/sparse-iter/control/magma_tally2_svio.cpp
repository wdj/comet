/*
    -- MAGMA_tally2 (version 1.6.2) --
       ...
       @generated from magma_tally2_zvio.cpp normal z -> s, Sun May  3 11:23:01 2015
       @author Hartwig Anzt
*/
...
/**
    ...
    Visualizes part of a vector of type magma_tally2_s_matrix.
    ...
    magma_tally2_queue_t queue )
{
    ...
    magma_tally2_s_matrix y={Magma_tally2_CSR};
    ...
    if ( x.memory_location == Magma_tally2_CPU ) {
        ...
        for( magma_tally2_int_t i=offset; i<offset + visulen; i++ )
            magma_tally2_sprintval(x.val[i]);
    }
    else if ( x.memory_location == Magma_tally2_DEV ) {
        printf("located on DEV:\n");
        CHECK( magma_tally2_smtransfer( x, &y, Magma_tally2_DEV, Magma_tally2_CPU, queue ));
        for( magma_tally2_int_t i=offset; i<offset +  visulen; i++ )
            magma_tally2_sprintval(y.val[i]);
    ...
    magma_tally2_queue_t queue )
{
    ...
    CHECK( magma_tally2_smalloc_cpu( &x->val, length ));
    ...
    while( i<length )  // eof() is 'true' at the end of data
    {
        ...
        #define REAL
            ...
            float VAL2;
            fscanf(fid, " %f %f \n", &VAL1, &VAL2);
            VAL = MAGMA_tally2_S_MAKE(VAL1, VAL2);
        #else
            fscanf(fid, " %f \n", &VAL1);
            VAL = MAGMA_tally2_S_MAKE(VAL1, 0.0);
        #endif
        
        if ( VAL != MAGMA_tally2_S_ZERO )
            nnz++;
        x->val[i] = VAL;
        i++;
    }
    ...
    magma_tally2_queue_t queue )
{
    ...
    magma_tally2_s_matrix A={Magma_tally2_CSR}, B={Magma_tally2_CSR};
    magma_tally2_int_t entry=0;
     //   char *vfilename[] = {"/mnt/sparse_matrices/mtx/rail_79841_B.mtx"};
    CHECK( magma_tally2_s_csr_mtx( &A,  filename, queue  ));
    CHECK( magma_tally2_smconvert( A, &B, Magma_tally2_CSR, Magma_tally2_DENSE, queue ));
    CHECK( magma_tally2_svinit( x, Magma_tally2_CPU, A.num_cols, A.num_rows, MAGMA_tally2_S_ZERO, queue ));
