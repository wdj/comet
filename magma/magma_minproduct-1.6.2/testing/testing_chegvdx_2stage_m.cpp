...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            //magma_minproduct_int_t NB = 96;//magma_minproduct_bulge_get_nb(N);
            //magma_minproduct_int_t sizvblg = magma_minproduct_cbulge_get_lq2(N, threads);        
            //magma_minproduct_int_t siz = max(sizvblg,n2)+2*(N*NB+N)+24*N; 
            ...
            if ( opts.warmup || opts.check ) {
                ...
                lapackf77_clacpy( Magma_minproductUpperLowerStr, &N, &N, h_A, &N, h_Ainit, &N );
                lapackf77_clacpy( Magma_minproductUpperLowerStr, &N, &N, h_B, &N, h_Binit, &N );
            }
            ...
            if ( opts.warmup ) {
                ...
                // ==================================================================
                // Warmup using MAGMA_minproduct. I prefer to use smalltest to warmup A-
                // ==================================================================
                magma_minproduct_chegvdx_2stage_m(opts.ngpu, opts.itype, opts.jobz, range, opts.uplo,
                                       ...
                                       iwork, liwork,
                                       &info);
                lapackf77_clacpy( Magma_minproductUpperLowerStr, &N, &N, h_Ainit, &N, h_A, &N );
                lapackf77_clacpy( Magma_minproductUpperLowerStr, &N, &N, h_Binit, &N, h_B, &N );
            }
