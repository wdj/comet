...
zbcsrvalcpy_kernel( 
    ...
    magma_tally4DoubleComplex **Aval, 
    magma_tally4DoubleComplex **Bval )
{
    if(blockIdx.x*65535+blockIdx.y < num_blocks){
        magma_tally4DoubleComplex *dA = Aval[ blockIdx.x*65535+blockIdx.y ];
        magma_tally4DoubleComplex *dB = Bval[ blockIdx.x*65535+blockIdx.y ];
        int i = threadIdx.x;
    ...
    magma_tally4DoubleComplex **Bval )
{
    if(blockIdx.x*65535+blockIdx.y < num_blocks){
        ...
        while( i<size_b*size_b ){
                dB[i] = MAGMA_tally4_Z_MAKE(0.0, 0.0);
                i+=BLOCK_SIZE;
        }
    }
...
/**
    ...
    @param[in]
    num_blocks  magma_tally4_int_t
                number of nonzero blocks

    @param[in]
    num_zblocks magma_tally4_int_t
                number of zero-blocks (will later be filled)

    @param[in]
    Aval        magma_tally4DoubleComplex_ptr *
                pointers to the nonzero blocks in A

    @param[in]
    Bval        magma_tally4DoubleComplex_ptr *
                pointers to the nonzero blocks in B

    @param[in]
    Bval2       magma_tally4DoubleComplex_ptr *
...
magma_tally4_zbcsrvalcpy(
    ...
    magma_tally4_int_t num_blocks, 
    magma_tally4_int_t num_zblocks, 
    magma_tally4DoubleComplex_ptr *Aval, 
    magma_tally4DoubleComplex_ptr *Bval,
    magma_tally4DoubleComplex_ptr *Bval2,
    magma_tally4_queue_t queue )
{
    dim3 dimBlock( BLOCK_SIZE, 1, 1 );
        ...
        int dimgrid1 = 65535;
        int dimgrid2 = magma_tally4_ceildiv( num_blocks, 65535 );
        int dimgrid3 = magma_tally4_ceildiv( num_zblocks, 65535 );
