...
#define dTup(d, i, j) (   dt[(d)] + (j)*nb    + (i))
    ...
    magma_minproductFloatComplex *dwork[Magma_minproductMaxGPUs], *dt[Magma_minproductMaxGPUs];
    magma_minproduct_int_t     ldda, lddla, nb, iinfo, n_local[Magma_minproductMaxGPUs], J2, d, ngpu0 = ngpu;
    magma_minproduct_int_t     j, jj, jb, J, JB, NB, h;
    magma_minproduct_queue_t   stream[Magma_minproductMaxGPUs][3];
    magma_minproduct_event_t   event[Magma_minproductMaxGPUs][5];
    magma_minproduct_timer_t time_total=0, time_sum=0, time=0;
    ...
    *info = 0;
    ...
    NB = (magma_minproduct_int_t)((0.8*freeMem - max(2,ngpu)*nb*ldda - (n+nb)*nb)/lddla); /* number of columns in the big panel */
    //NB = min(5*nb,n);
    ...
    for (d=0; d < ngpu; d++ ) {
        ...
        if (MAGMA_minproduct_SUCCESS != magma_minproduct_cmalloc( &dt[d], NB*lddla + max(2,ngpu)*nb*ldda )) {
        ...
        magma_minproduct_device_sync(); // synch the device
    }
    ...
    if (upper) {
        ...
        for( J=0; J < n; J += NB ) {
            ...
            magma_minproduct_chtodpo( ngpu, uplo, JB, n, J, J, nb, A, lda, dwork, NB, stream, &iinfo);
            ...
            for( j=0; j < J; j += nb ) {
                ...
                for( d=0; d < ngpu; d++ ) {
                    magma_minproduct_setdevice(d);
                    magma_minproduct_csetmatrix_async( nb, JB,
                                            A(j, J),       lda,
                                            dTup(d, 0, J), nb,
                                            stream[d][0] );
                    n_local[d] = 0;
                ...
                for( jj=0; jj < JB; jj += nb ) { // jj is 'local' column index within the big panel
                    ...
                    magma_minproduct_setdevice(d);
                    magma_minproductblasSetKernelStream(stream[d][J2%2]); // the last stream (2) used to process off-diagonal
                    J2 = nb*J2;
                    ...
                    magma_minproduct_cherk(Magma_minproductUpper, Magma_minproductConjTrans, jb, nb,
                ...
                if ( n > J+JB ) {
                    for( d=0; d < ngpu; d++ ) {
                        magma_minproduct_setdevice(d);
                        magma_minproductblasSetKernelStream(stream[d][2]);
                ...
                for( d=0; d < ngpu; d++ ) {
                    magma_minproduct_setdevice(d);
                    for( jj=0; jj < 3; jj++ )
                        magma_minproduct_queue_sync( stream[d][jj] );
            ...
            // using three streams
            magma_minproduct_cpotrf3_mgpu(ngpu, uplo, JB, n-J, J, J, nb,
            ...
            magma_minproduct_cdtohpo(ngpu, uplo, JB, n, J, J, nb, NB, A, lda, dwork, NB, stream, &iinfo);
            //magma_minproduct_cdtohpo(ngpu, uplo, JB, n, J, J, nb, 0, A, lda, dwork, NB, stream, &iinfo);
        }
    } else {
        /* ========================================================= *
        ...
        for( J=0; J < n; J += NB ) {
            ...
            magma_minproduct_chtodpo( ngpu, uplo, n, JB, J, J, nb, A, lda, dwork, lddla, stream, &iinfo);
            ...
            for( j=0; j < J; j += nb ) {
                ...
                for( d=0; d < ngpu; d++ ) {
                    magma_minproduct_setdevice(d);
                    magma_minproduct_csetmatrix_async( JB, nb,
                                            A(J, j),     lda,
                                            dT(d, J, 0), ldda,
                                            stream[d][0] );
                    n_local[d] = 0;
                ...
                for( jj=0; jj < JB; jj += nb ) { /* diagonal */
                    ...
                    magma_minproduct_setdevice(d);
                    magma_minproductblasSetKernelStream(stream[d][J2%2]);
                    ...
                    magma_minproduct_cherk(Magma_minproductLower, Magma_minproductNoTrans, jb, nb,
            ...
            h = (JB+nb-1)/nb; // big diagonal of big panel will be on CPU
            // using three streams
            magma_minproduct_cpotrf3_mgpu(ngpu, uplo, n-J, JB, J, J, nb,
            ...
            magma_minproduct_cdtohpo( ngpu, uplo, n, JB, J, J, nb, JB, A, lda, dwork, lddla, stream, &iinfo);
