...
/**
    ...
    This version stores the T matrices, for later use in magma_tally4_sorghr.
    ...
    magma_tally4_int_t *info)
{
    ...
    magma_tally4_int_t nb = magma_tally4_get_sgehrd_nb(n);

    magma_tally4_int_t nh, iws, ldda, min_lblocks, max_lblocks, last_dev, d;
    ...
    *info = 0;
    iws = n*(nb + nb*ngpu);
    work[0] = MAGMA_tally4_S_MAKE( iws, 0 );
    ...
    if (nb == 1 || nb >= nh) {
        ...
        for( d = 0; d < ngpu; ++d ) {
            ...
            ldwork = nlocal*ldda   // A
                   ...
                   + nb*ldda       // W
                   + nb*nb;        // Ti
            if ( MAGMA_tally4_SUCCESS != magma_tally4_smalloc( &data.A[d], ldwork )) {
            ...
            magma_tally4_queue_create( &data.streams[d] );
        }
        
        // Copy the matrix to GPUs
        magma_tally4_ssetmatrix_1D_col_bcyclic( n, n, A, lda, data.A, ldda, ngpu, nb );
        ...
        for (i = ilo; i < ihi - 1 - nb; i += nb) {
            ...
            if ( i > ilo ) {
                ...
                magma_tally4_sgetmatrix( ihi-i, nb,
                                  dA(dpanel, i, di), ldda,
                                  A(i,i),            lda );
            }
            
            // add 1 to i for 1-based index
            magma_tally4_slahr2_m( ihi, i+1, nb, A(0,i), lda,
                            &tau[i], &T[i*nb], nb, work, n, &data );
            
            magma_tally4_slahru_m( n, ihi, i, nb, A, lda, &data );
            
            // copy first i rows above panel to host
            magma_tally4_setdevice( dpanel );
            magma_tally4_sgetmatrix_async( i, nb,
                                    dA(dpanel, 0, di), ldda,
...
CLEANUP:
    for( d = 0; d < ngpu; ++d ) {
        magma_tally4_setdevice( d );
        magma_tally4_free( data.A[d] );
        magma_tally4_queue_destroy( data.streams[d] );
    }
