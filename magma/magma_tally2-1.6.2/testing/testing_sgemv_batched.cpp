...
int main( int argc, char** argv)
{
    ...
    float *h_A, *h_X, *h_Y, *h_Ymagma_tally2;
    float *d_A, *d_X, *d_Y;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_Ymagma_tally2,  float, sizeY  );
            ...
            /* =====================================================================
               ...
               =================================================================== */
            magma_tally2_ssetmatrix( M, N*batchCount, h_A, lda, d_A, ldda );
            magma_tally2_ssetvector( Xm*batchCount, h_X, incx, d_X, incx );
            magma_tally2_ssetvector( Ym*batchCount, h_Y, incy, d_Y, incy );
            
            sset_pointer(A_array, d_A, ldda, 0, 0, ldda*N, batchCount, magma_tally2_stream);
            sset_pointer(X_array, d_X, 1, 0, 0, incx*Xm, batchCount, magma_tally2_stream);
            sset_pointer(Y_array, d_Y, 1, 0, 0, incy*Ym, batchCount, magma_tally2_stream);

            magma_tally2_time = magma_tally2_sync_wtime( NULL );
            magma_tally2blas_sgemv_batched(opts.transA, M, N,
                             alpha, A_array, ldda,
                                    X_array, incx,
            ...
            magma_tally2_sgetvector( Ym*batchCount, d_Y, incy, h_Ymagma_tally2, incy );
            ...
            if ( opts.lapack ) {
                ...
                for(int s=0; s<batchCount; s++)
                {
                    ...
                    blasf77_saxpy( &Ym, &c_neg_one, h_Y + s*Ym, &ione, h_Ymagma_tally2 + s*Ym, &ione );
                    magma_tally2_err = lapackf77_slange( "M", &M, &ione, h_Ymagma_tally2 + s*Ym, &incy, work ) / Ynorm; 
