...
#define dA(i, j) (dA+(j)*ldda + (i))
    ...
    *info = 0;
    ...
    if (nounit) {
        for (j=0; j < n; ++j) {
            if ( MAGMA_tally4_S_EQUAL( *dA(j,j), c_zero )) {
    ...
    if (nb <= 1 || nb >= n) {
        ...
        if (upper) {
            ...
            for (j=0; j < n; j += nb) {
                ...
                lapackf77_strtri( Magma_tally4UpperStr, diag_, &jb, work, &jb, info );
            ...
            for (j=nn-1; j >= 0; j -= nb) {
                ...
                /* Compute inverse of current diagonal block */
                lapackf77_strtri( Magma_tally4LowerStr, diag_, &jb, work, &jb, info );
