...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lapackf77_dlacpy( Magma_tally2FullStr, &M, &N, h_A, &lda, h_R, &lda );
            ...
            magma_tally2_dgelqf_gpu( M, N, d_A, ldda, tau, h_work, lwork, &info);
            gpu_time = magma_tally2_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally2_dgelqf_gpu returned error %d: %s.\n",
