...
void get_QR_error(magma_tally3_int_t M, magma_tally3_int_t N, magma_tally3_int_t min_mn,
                    magma_tally3FloatComplex *h_R,  magma_tally3FloatComplex *h_A, magma_tally3_int_t lda,
                    magma_tally3FloatComplex *tau,
                    magma_tally3FloatComplex *Q,  magma_tally3_int_t ldq,
                    magma_tally3FloatComplex *R,  magma_tally3_int_t ldr,
                    magma_tally3FloatComplex *h_work,  magma_tally3_int_t lwork,
...
int main( int argc, char** argv)
{
    ...
    magma_tally3FloatComplex *h_A, *h_R, *h_Amagma_tally3, *tau, *h_work, tmp[1];
    magma_tally3FloatComplex *d_A, *dtau_magma_tally3, *dtau_cublas;

    magma_tally3FloatComplex **dA_array = NULL;
    magma_tally3FloatComplex **dtau_array = NULL;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( tau,   magma_tally3FloatComplex, min_mn * batchCount );
            ...
            TESTING_MALLOC_DEV( dtau_magma_tally3,  magma_tally3FloatComplex, min_mn * batchCount);
            TESTING_MALLOC_DEV( dtau_cublas,  magma_tally3FloatComplex, min_mn * batchCount);
            ...
            TESTING_MALLOC_CPU( h_work, magma_tally3FloatComplex, lwork * batchCount);
			...
			{
				magma_tally3_cmake_hpd( N, h_A + i * lda * N, lda );// need modification
			}
			
			lapackf77_clacpy( Magma_tally3UpperLowerStr, &M, &column, h_A, &lda, h_R, &lda );
       
            /* ====================================================================
               Performs operation using MAGMA_tally3
               =================================================================== */
            magma_tally3_csetmatrix( M, column, h_R, lda,  d_A, ldda );
            cset_pointer(dA_array, d_A, 1, 0, 0, ldda*N, batchCount, opts.queue);
            cset_pointer(dtau_array, dtau_magma_tally3, 1, 0, 0, min_mn, batchCount, opts.queue);
    
            magma_tally3_time = magma_tally3_sync_wtime(0);
    
            info = magma_tally3_cgeqrf_batched(M, N, dA_array, ldda, dtau_array, dinfo_magma_tally3, batchCount, opts.queue);

            magma_tally3_time = magma_tally3_sync_wtime(0) - magma_tally3_time;
            magma_tally3_perf = gflops / magma_tally3_time;

            magma_tally3_cgetmatrix( M, column, d_A, ldda, h_Amagma_tally3, lda);

            if (info != 0)
                printf("magma_tally3_cgeqrf_batched returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                magma_tally3_cgetvector(min_mn*batchCount, dtau_magma_tally3, 1, tau, 1);
                for (int i=0; i < batchCount; i++)
                {
                ...
                #if CUDA_VERSION >= 6050
                magma_tally3_cgetvector(min_mn*batchCount, dtau_magma_tally3, 1, tau, 1);
                magma_tally3_cgetmatrix( M, column, d_A, ldda, h_A, lda);
                for (int i=0; i < batchCount; i++)
                {
                    get_QR_error(M, N, min_mn,
