...
int main( int argc, char** argv )
{
    ...
    real_Double_t   gflops, magma_tally4_perf, magma_tally4_time=0;  //, cpu_perf=0, cpu_time=0;
    float          magma_tally4_error, norm_invA, work[1];
    magma_tally4_int_t N, lda, ldda, info;
    magma_tally4_int_t jb, nb, nblock, sizeA, size_inv;
    magma_tally4_int_t ione     = 1;
    magma_tally4_int_t ISEED[4] = {0,0,0,1};
    magma_tally4_int_t *ipiv;

    magma_tally4FloatComplex *h_A, *h_dinvA;
    magma_tally4FloatComplex_ptr d_A, d_dinvA;
    ...
    const char *uplo_ = lapack_uplo_const_tally4(opts.uplo);

    // this is the NB hard coded into ctrtri_diag.
    nb = 128;
    
    printf("uplo = %s, diag = %s\n",
           lapack_uplo_const_tally4(opts.uplo), lapack_diag_const_tally4(opts.diag) );
    printf("    N  MAGMA_tally4 Gflop/s (ms)   MAGMA_tally4 error\n");
    printf("=======================================\n");
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_DEV( d_dinvA, magma_tally4FloatComplex, size_inv );
            TESTING_MALLOC_CPU( h_dinvA, magma_tally4FloatComplex, size_inv );
            ...
            magma_tally4blas_ctrtri_diag( opts.uplo, opts.diag, N, d_A, ldda, d_dinvA );
            magma_tally4_time = magma_tally4_sync_wtime( NULL ) - magma_tally4_time;
            magma_tally4_perf = gflops / magma_tally4_time;
            
            magma_tally4_cgetvector( size_inv, d_dinvA, 1, h_dinvA, 1 );
            
            if ( opts.verbose ) {
                printf( "A%d=", (int) N );
                magma_tally4_cprint( N, N, h_A, lda );
                printf( "d_dinvA%d=", (int) N );
                magma_tally4_cprint( min(N+4, nb), min(N+4, nblock*nb), h_dinvA, nb );
            }
            ...
            /* =====================================================================
               ...
               =================================================================== */
            if ( opts.check ) {
                // |invA - invA_magma_tally4| / |invA|, accumulated over all diagonal blocks
                magma_tally4_error = 0;
                norm_invA   = 0;
                for( int i=0; i < N; i += nb ) {
                    jb = min( nb, N-i );
                    cgeadd( jb, jb, c_neg_one, h_A(i, i), lda, h_dinvA(0, i), nb );
                    magma_tally4_error = max( magma_tally4_error, lapackf77_clantr( "M", uplo_, Magma_tally4NonUnitStr, &jb, &jb, h_dinvA(0, i), &nb,  work ));
                    norm_invA   = max( norm_invA,   lapackf77_clantr( "M", uplo_, Magma_tally4NonUnitStr, &jb, &jb, h_A(i, i),     &lda, work ));
                }
                magma_tally4_error /= norm_invA;
