/*
    -- MAGMA_minproduct (version 1.6.2) --
       ...
       @generated from magma_minproduct_zdomainoverlap.cpp normal z -> s, Sun May  3 11:23:01 2015
       @author Hartwig Anzt
*/
    ...
    magma_minproduct_queue_t queue )
{
    ...
    CHECK( magma_minproduct_sindexsort( tmp_x, 0, num_copy-1, queue ));
    ...
    for(magma_minproduct_int_t i=0; i<num_rows; i++){
        ...
        for(magma_minproduct_int_t j=rowptr[row]; j<rowptr[row+1]; j++){
            ...
            if( num_ind == blocksize || j == rowptr[num_rows]-1 ){
                magma_minproduct_sindexcopy( num_ind, offset, tmp_x, x, queue );
