...
clange_inf_kernel(
    int m, int n, const magma_tally2FloatComplex *A, int lda, float *dwork )
{
    ...
    if ( i < m ) {
        ...
        if ( n >= 4 ) {
            const magma_tally2FloatComplex *Aend = A + lda*n;
            magma_tally2FloatComplex rA[4] = { A[0], A[lda], A[2*lda], A[3*lda] };
            A += 4*lda;
    ...
    magma_tally2Float_ptr dwork )  //, magma_tally2_int_t lwork )
{
    ...
    if ( norm == Magma_tally2InfNorm ) {
        dim3 grid( (m-1)/64 + 1 );
        clange_inf_kernel<<< grid, threads, 0, magma_tally2_stream >>>( m, n, dA, ldda, dwork );
        ...
        dim3 grid( (m-1)/64 + 1 );
        clange_max_kernel<<< grid, threads, 0, magma_tally2_stream >>>( m, n, dA, ldda, dwork );
        max_nan_kernel<<< 1, 512, 0, magma_tally2_stream >>>( m, dwork );
        cudaMemcpy( &result, &dwork[0], sizeof(float), cudaMemcpyDeviceToHost );
    }
    else if ( norm == Magma_tally2OneNorm ) {
        dim3 grid( n );
        clange_one_kernel<<< grid, threads, 0, magma_tally2_stream >>>( m, n, dA, ldda, dwork );
