...
int main( int argc, char** argv)
{
    ...
    magma_minproductDoubleComplex *h_A, *h_R, *tau, *dtau, *h_work, tmp[1];
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            // warmup
            if ( opts.warmup ) {
                magma_minproduct_zgeqr2_gpu( M, N, d_A, ldda, dtau, dwork, &info );
                magma_minproduct_zsetmatrix( M, N, h_R, lda, d_A, ldda );
            }
            ...
            if (info != 0)
                printf("magma_minproduct_zgeqr2_gpu returned error %d: %s.\n",
