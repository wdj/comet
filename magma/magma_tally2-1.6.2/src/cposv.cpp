...
/**
    ...
    @ingroup magma_tally2_cposv_driver
    ********************************************************************/
    ...
    magma_tally2_int_t *info )
{
    ...
    *info = 0;
    ...
    magma_tally2FloatComplex *dA, *dB;
    if ( ngpu > 1 ) {
        goto CPU_INTERFACE;
    }
    ldda = ((n+31)/32)*32;
    lddb = ldda;
    if ( MAGMA_tally2_SUCCESS != magma_tally2_cmalloc( &dA, ldda*n )) {
        goto CPU_INTERFACE;
    }
    if ( MAGMA_tally2_SUCCESS != magma_tally2_cmalloc( &dB, lddb*nrhs )) {
        magma_tally2_free( dA );
        goto CPU_INTERFACE;
    }
    magma_tally2_csetmatrix( n, n, A, lda, dA, ldda );
    magma_tally2_cpotrf_gpu( uplo, n, dA, ldda, info );
    ...
    if ( *info == 0 ) {
        magma_tally2_csetmatrix( n, nrhs, B, ldb, dB, lddb );
        magma_tally2_cpotrs_gpu( uplo, n, nrhs, dA, ldda, dB, lddb, info );
        magma_tally2_cgetmatrix( n, nrhs, dB, lddb, B, ldb );
    }
    magma_tally2_free( dA );
    magma_tally2_free( dB );
    return *info;
...
CPU_INTERFACE:
    ...
    magma_tally2_cpotrf( uplo, n, A, lda, info );
    if ( *info == 0 ) {
        lapackf77_cpotrs( lapack_uplo_const_tally2(uplo), &n, &nrhs, A, &lda, B, &ldb, info );
    }
