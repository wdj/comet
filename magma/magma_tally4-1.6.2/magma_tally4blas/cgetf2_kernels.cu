...
__device__ int 
icamax_devfunc(int length, const magma_tally4FloatComplex *x, int incx, float *shared_x, int *shared_idx)
{
    ...
    for(int s =0 ; s < nchunk; s++)
    {
        if( (tx + s * zamax < length) && (tx < zamax) )
        {
            ...
            res1 = fabs(MAGMA_tally4_C_REAL(res)) + fabs(MAGMA_tally4_C_IMAG(res));
...
////////////////////////////////////////////////////////////////////////////////////////////////////
__global__ void 
icamax_kernel_batched(int length, int chunk, magma_tally4FloatComplex **x_array, int incx, 
                   int step, int lda, magma_tally4_int_t** ipiv_array, magma_tally4_int_t *info_array, int gbstep)
{

    magma_tally4FloatComplex *x_start = x_array[blockIdx.z];
    const magma_tally4FloatComplex *x = &(x_start[step + step * lda]); 
...
__global__ void
tree_icamax_kernel_batched(int length, magma_tally4FloatComplex **x_array, int incx, 
                   int step, int lda, magma_tally4_int_t** ipiv_array, magma_tally4_int_t *info_array, int gbstep, 
                   float** data_pool_array, magma_tally4_int_t** id_pool_array)
{
...
__global__ void
tree_icamax_kernel2_batched(int n, int step,  magma_tally4_int_t** ipiv_array, magma_tally4_int_t *info_array, int gbstep, float** data_pool_array, magma_tally4_int_t** id_pool_array)
{
    __shared__ float shared_x[zamax];
    __shared__ int    shared_idx[zamax];
...
magma_tally4_int_t magma_tally4_icamax_lg_batched(magma_tally4_int_t length, magma_tally4FloatComplex **x_array, magma_tally4_int_t incx, magma_tally4_int_t step,  magma_tally4_int_t lda,
...

{
    ...
    magma_tally4_smalloc(&data_pool, num_blocks * batchCount);
...
extern "C"
magma_tally4_int_t magma_tally4_icamax_batched(magma_tally4_int_t length, 
...
#else
    // the magma_tally4_icamax_lg_batched is faster but when cuda launch it as 2 kernels the white space time between these 2 kernels and the next kernel is larger than using the icamax_kernel for that today we are using only icamax_kernel
    if( length <= 10 * zamax )
    {  
        dim3 grid(1, 1, batchCount);
        int chunk = (length-1)/zamax + 1;
        icamax_kernel_batched<<< grid, zamax, zamax * (sizeof(float) + sizeof(magma_tally4_int_t)), queue >>>
                      (length, chunk, x_array, incx, step, lda, ipiv_array, info_array, gbstep);

    }
    else
    {
        magma_tally4_icamax_lg_batched(length, x_array, incx, step, lda, ipiv_array, info_array, gbstep, batchCount);
    }
#endif
...
__global__
void cswap_kernel_batched(magma_tally4_int_t n, magma_tally4FloatComplex **x_array, magma_tally4_int_t incx, magma_tally4_int_t step, magma_tally4_int_t** ipiv_array)
{

    magma_tally4FloatComplex *x = x_array[blockIdx.z];
    magma_tally4_int_t *ipiv = ipiv_array[blockIdx.z];
    ...
    if (id < n) {
        magma_tally4FloatComplex tmp = x[jp + incx*id];
...
////////////////////////////////////////////////////////////////////////////////////////////////////
extern "C"
magma_tally4_int_t magma_tally4_cswap_batched(magma_tally4_int_t n, magma_tally4FloatComplex **x_array, magma_tally4_int_t incx, magma_tally4_int_t step, 
...
*/
    if( n  > MAX_NTHREADS) 
    {
       printf("magma_tally4_cswap_batched nb=%d, > %d, not supported \n",n, MAX_NTHREADS);
       return -15;
    }
    dim3 grid(1,1, batchCount);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
__global__
void cscal_cgeru_kernel_batched(int m, int n, int step, magma_tally4FloatComplex **dA_array, int lda, magma_tally4_int_t *info_array, int gbstep)
{

    // checkinfo to avoid computation of the singular matrix
    if(info_array[blockIdx.z] != 0 ) return;

    magma_tally4FloatComplex *A_start = dA_array[blockIdx.z];
    magma_tally4FloatComplex *A = &(A_start[step + step * lda]); 
    magma_tally4FloatComplex *shared_y = shared_data;
    ...
    __syncthreads();
    if(shared_y[0] == MAGMA_tally4_C_ZERO) {
        info_array[blockIdx.z] = step + gbstep + 1; 
        return;
    }

    if (gbidx < m && gbidx > 0) {
        magma_tally4FloatComplex reg = MAGMA_tally4_C_ZERO;
        reg = A[gbidx];
        reg *= MAGMA_tally4_C_DIV(MAGMA_tally4_C_ONE, shared_y[0]);
        A[gbidx] = reg;
        #pragma unroll
        for(int i=1; i < n; i++) {
            ...
            A[gbidx + i*lda] += (MAGMA_tally4_C_NEG_ONE) * shared_y[i] * reg;
        }
    }
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////
extern "C"
magma_tally4_int_t magma_tally4_cscal_cgeru_batched(magma_tally4_int_t m, magma_tally4_int_t n, magma_tally4_int_t step,
...
*/
    ...
    if( n  > MAX_NTHREADS) 
    {
       printf("magma_tally4_cscal_cgeru_batched nb=%d, > %d, not supported \n",n, MAX_NTHREADS);
       return -15;
    }

    int nchunk = (m-1)/MAX_NTHREADS + 1;
    size_t shared_size = sizeof(magma_tally4FloatComplex)*(n);
...
////////////////////////////////////////////////////////////////////////////////////////////////////
__global__
void cgetf2trsm_kernel_batched(int ib, int n, magma_tally4FloatComplex **dA_array, int step, int lda)
{
        /*
    ...
    magma_tally4FloatComplex *A_start = dA_array[blockIdx.z];
    magma_tally4FloatComplex *A = &(A_start[step + step * lda]); 
    magma_tally4FloatComplex *B = &(A_start[step + (step+ib) * lda]); 
    magma_tally4FloatComplex *shared_a = shared_data;
    magma_tally4FloatComplex *shared_b = shared_data+ib*ib;
    ...
    if (tid < n) {
        ...
        for( d=0;  d<ib-1; d++) {
            for( i=d+1; i<ib; i++) {
                shared_b[i+tid*ib] += (MAGMA_tally4_C_NEG_ONE) * shared_a[i+d*ib] * shared_b[d+tid*ib];
            }
        }
    }
    __syncthreads();
...
////////////////////////////////////////////////////////////////////////////////////////////////////
extern "C" void
magma_tally4_cgetf2trsm_batched(magma_tally4_int_t ib, magma_tally4_int_t n, magma_tally4FloatComplex **dA_array,  magma_tally4_int_t step, magma_tally4_int_t lda,
                       magma_tally4_int_t batchCount, magma_tally4_queue_t queue)
{
/*
...
*/
    if( n == 0 || ib == 0 ) return;
    size_t shared_size = sizeof(magma_tally4FloatComplex)*(ib*(ib+n));
...
zupdate_device(int m, int step, magma_tally4FloatComplex* x, int ldx,  magma_tally4FloatComplex *A, int lda)
{
...
////////////////////////////////////////////////////////////////////////////////////////////////////
static __device__ void 
cscal5_device(int m, magma_tally4FloatComplex* x, magma_tally4FloatComplex alpha)
{
    int tid = threadIdx.x;
    int nchunk = (m-1)/MAX_NTHREADS + 1;    
    ...
    for(int s=0 ; s < nchunk; s++)
    {
        if( (tid + s * MAX_NTHREADS) < m ) {
            #if 0
            x[tid + s * MAX_NTHREADS] *= MAGMA_tally4_C_DIV(MAGMA_tally4_C_ONE, alpha);
            #else
...
////////////////////////////////////////////////////////////////////////////////////////////////////
__global__ void 
zcomputecolumn_kernel_shared_batched(int m, int paneloffset, int step, magma_tally4FloatComplex **dA_array, int lda, magma_tally4_int_t **ipiv_array, magma_tally4_int_t *info_array, int gbstep)
{
    int gboff = paneloffset+step;
    magma_tally4_int_t *ipiv                   = ipiv_array[blockIdx.z];
    magma_tally4FloatComplex *A_start = dA_array[blockIdx.z];
    magma_tally4FloatComplex *A0j     = &(A_start[paneloffset + (paneloffset+step) * lda]); 
    magma_tally4FloatComplex *A00     = &(A_start[paneloffset + paneloffset * lda]); 

    magma_tally4FloatComplex *shared_A = shared_data;
    __shared__ float  shared_x[zamax];
    __shared__ int     shared_idx[zamax];
    __shared__ magma_tally4FloatComplex alpha;
    int tid = threadIdx.x;
...
////////////////////////////////////////////////////////////////////////////////////////////////////
extern "C"
magma_tally4_int_t magma_tally4_ccomputecolumn_batched(magma_tally4_int_t m, magma_tally4_int_t paneloffset, magma_tally4_int_t step, 
                                        magma_tally4FloatComplex **dA_array,  magma_tally4_int_t lda,
...
*/
    ...
    size_t all_shmem_size = zamax*(sizeof(float)+sizeof(int)) + (m+2)*sizeof(magma_tally4FloatComplex);
    if( all_shmem_size >  (MAX_SHARED_ALLOWED*1024) ) // limit the shared memory to 44K leaving 4K for extra
    {
        printf("magma_tally4_ccomputecolumn_batched error out of shared memory \n");
        return -20;
    }

    size_t shared_size = sizeof(magma_tally4FloatComplex)*m;
    dim3 grid(1, 1, batchCount);
