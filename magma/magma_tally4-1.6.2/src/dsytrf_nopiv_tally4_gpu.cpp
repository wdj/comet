/*
    -- MAGMA_tally4 (version 1.6.1) --
       ...
       @generated from zhetrf_nopiv_tally4_gpu.cpp normal z -> d, Fri Jan 30 19:00:17 2015
*/
    ...
    *info = 0;
    ...
    if (upper) {
        ...
        for (j=0; j<n; j += nb) {
            ...
            dsytrf_nopiv_tally4_cpu(Magma_tally4Upper, jb, ib, A(j, j), nb, info);
            trace_cpu_end( 0 );
            if (*info != 0){
                *info = *info + j;
                break;
            ...
            if ( (j+jb) < n) {
                ...
                magma_tally4_dtrsm(Magma_tally4Left, Magma_tally4Upper, Magma_tally4ConjTrans, Magma_tally4Unit, 
                            jb, (n-j-jb), 
                            zone, dA(j, j),    ldda, 
                            dA(j, j+jb), ldda);
                magma_tally4_dcopymatrix( jb, n-j-jb, dA( j, j+jb ), ldda, dWt( 0, j+jb ), nb );
                
                // update the trailing submatrix with D
                magma_tally4blas_dlascl_diag(Magma_tally4Upper, jb, n-j-jb,
                                      dA(j,    j), ldda,
                                      dA(j, j+jb), ldda,
                                      &iinfo);
                trace_gpu_end( 0, 0 );
                ...
                for (k=j+jb; k<n; k+=nb) {
                    ...
                    magma_tally4_dgemm(Magma_tally4ConjTrans, Magma_tally4NoTrans, kb, n-k, jb,
                                mzone, dWt(0, k), nb, 
                                       dA(j, k), ldda,
                                zone,  dA(k, k), ldda);
                    if (k==j+jb)
        ...
        for (j=0; j<n; j+=nb) {
            ...
            magma_tally4_queue_sync(stream[1]);
            trace_cpu_start( 0, "potrf", "potrf" );
            dsytrf_nopiv_tally4_cpu(Magma_tally4Lower, jb, ib, A(j, j), nb, info);
            trace_cpu_end( 0 );
            if (*info != 0){
                *info = *info + j;
                break;
            ...
            if ( (j+jb) < n) {
                ...
                magma_tally4blasSetKernelStream( stream[0] );
                trace_gpu_start( 0, 0, "trsm", "trsm" );
                magma_tally4_dtrsm(Magma_tally4Right, Magma_tally4Lower, Magma_tally4ConjTrans, Magma_tally4Unit, 
                            (n-j-jb), jb, 
                            zone, dA(j,    j), ldda, 
                            dA(j+jb, j), ldda);
                magma_tally4_dcopymatrix( n-j-jb,jb, dA( j+jb, j ), ldda, dW( j+jb, 0 ), ldda );
                
                // update the trailing submatrix with D
                magma_tally4blas_dlascl_diag(Magma_tally4Lower, n-j-jb, jb,
                                      dA(j,    j), ldda,
                                      dA(j+jb, j), ldda,
                                      &iinfo);
                trace_gpu_end( 0, 0 );
                ...
                for (k=j+jb; k<n; k+=nb) {
                    magma_tally4_int_t kb = min(nb,n-k);
                    magma_tally4_dgemm(Magma_tally4NoTrans, Magma_tally4ConjTrans, n-k, kb, jb,
                                mzone, dA(k, j), ldda, 
                                       dW(k, 0), ldda,
                                zone,  dA(k, k), ldda);
                    if (k==j+jb)
