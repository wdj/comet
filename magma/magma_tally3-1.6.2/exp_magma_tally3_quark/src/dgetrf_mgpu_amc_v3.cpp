...
#include "magma_tally3_task_dev_d.h"
...
extern "C" magma_tally3_int_t magma_tally3_dgetrf_mgpu_amc_v3(magma_tally3_int_t num_gpus, magma_tally3_int_t m, magma_tally3_int_t n,  
...
//    magma_tally3_event_t *A_event; /*Controling bucket*/
    ...
    *info = magma_tally3_dgetrf_mgpu_work_amc_v3(num_gpus, m, n, dlA, dlA_LD, ipiv, info, AWORK, AWORK_LD, AWORK_n);
...
magma_tally3_dgetrf_mgpu_work_amc_v3(magma_tally3_int_t num_gpus,
...
/*  -- MAGMA_tally3 (version 1.6.1) -- 
    ...
    //double *work; 
     ...
     double *dlpanel[Magma_tally3MaxGPUs];
     magma_tally3_int_t dlpanel_LD;
...
//     double *tmpdA;
    ...
    *info = 0; 
    ...
    for(dd=0;dd<num_gpus;dd++){
       ...
       magma_tally3_task_dev_set_compute_stream(dd, 0); //set to mstream 1 later
    }
    ...
    // dlpanel = (double **)    malloc(num_gpus*sizeof(double*));
     ...
     for(dd=0;dd<num_gpus;dd++){
         ...
         if (MAGMA_tally3_SUCCESS != magma_tally3_dmalloc(&dlpanel[dd], nb*dm_max)) { 
...
for(dd=0;dd<num_gpus;dd++){
    magma_tally3_setdevice(dd);
    magma_tally3_task_dev_set_compute_stream(dd, mstream[dd][1]);
}
...
*/
     ...
     for(dd=0;dd<num_gpus;dd++){
        ...
        magma_tally3_insert_dev_dsetmatrix_async_transpose(dd, A_m, nb, A(0,K), A_LD, dlpanel(dd,K), dlpanel_LD, mstream[dd][0], dlAP_set[dd], dlAP_LD, colptr(K), dlpanel(dd,K)); //dlpanel[dd]
            ...
            /*Determine the number of columns to apply the update. */
             nr_local[dd] = numcols2p(dd, n - (K+1+A_N-1)*nb, nb, num_gpus);
              ...
              { 
                  ...
                  //magma_tally3_schedule_barrier();
                  //&(dlpanel[dd][dlpanel_LD*nb*K])
                  magma_tally3_insert_dev_dtrsm(dd, Magma_tally3Right,  Magma_tally3Upper, Magma_tally3NoTrans, Magma_tally3Unit, gpu_ncols, nb, c_one, dlpanel(dd,K), dlpanel_LD, dlAT(K,J), dlAT_LD);/*non blocking*/ 
 
                  /* aij^T = aij^T - (lik.ukj)^T = aij^T - ukj^T.lik^T*/ //&(dlpanel[dd][dlpanel_LD*nb*(K+1)])
                  magma_tally3_insert_dev_dgemm(dd, Magma_tally3NoTrans,Magma_tally3NoTrans, gpu_ncols, gpu_nrows, nb, c_neg_one, dlAT(K,J), dlAT_LD, dlpanel(dd,K+1), dlpanel_LD, c_one, dlAT(K+1,J), dlAT_LD);/*non blocking*/    
             ...
             /*Compute the number of blocs rows to group together before the update. To avoid scheduling overhead.*/
              ...
              //printf("B_rows:%d\n",B_rows);
               for(I=K+1; I<=M-1; I+=B_rows){ 
                    ...
                    magma_tally3_insert_core_dgemm('N','N', nrows, ncols, nb, c_neg_one, A(I,K), A_LD, A(K,J), A_LD, c_one, A(I,J), A_LD, colptr(K), colptr(J)); 
               } 
                   ...
                   // magma_tally3_insert_core_dgetrf(nrows, ncols, A(K+1,K+1), A_LD, ipiv(K+1), &iinfo, colptr(K+1)); 
                    ...
                    /*Transfer the factorized panel in the buffer of GPU (dlpanel)*/
                     ...
                     for(dd=0;dd<num_gpus;dd++){
                        ...
                        magma_tally3_insert_dev_dsetmatrix_async_transpose(dd, nrows, ncols, A(K+1, K+1), A_LD, dlpanel(dd, K+1), dlpanel_LD, mstream[dd][0], dlAP_set[dd], dlAP_LD, colptr(K+1), dlpanel(dd,K+1));//, dlpanel[dd]
                     }
                    ...
                    //magma_tally3_insert_dev_dsetmatrix_transpose(nrows, ncols, A(U_K, K+1), A_LD, dAT(U_K, K+1), dAT_LD, dAP_set, dAP_LD, colptr(K+1), dAT(K+1,K+1));
                     ...
                     magma_tally3_insert_dev_dsetmatrix_async_transpose(dd, nrows, ncols, A(U_K, K+1), A_LD, dlAT(U_K,K+1), dlAT_LD, mstream[dd][0], dlAP_set[dd], dlAP_LD, colptr(K+1), dlAT(0,K+1));///
                ...
                //// nr_local[dd]-=nb;
                  ...
                  /*if A_N==1, there is no look-ahead, so insert the panel here*/
                   if((A_N==1) && (L==K+A_N)){
                      ...
                      dd = GID(K+1);
                      ///magma_tally3_insert_dev_dsetmatrix_transpose(dd, nrows, ncols, A(U_K, K+1), A_LD, dlAT(U_K,K+1), dlAT_LD, dlAP_set[dd], dlAP_LD, colptr(K+1), dlAT(K+1,K+1));
                      magma_tally3_insert_dev_dsetmatrix_async_transpose(dd, nrows, ncols, A(U_K, K+1), A_LD, dlAT(U_K,K+1), dlAT_LD, mstream[dd][0], dlAP_set[dd], dlAP_LD, colptr(K+1), dlAT(0,K+1));///dlAT(K+1,K+1)
                   }
               }
 ...
 for(dd=0;dd<num_gpus;dd++){
        magma_tally3_setdevice(dd);
       magma_tally3_queue_sync(mstream[dd][0]);
       magma_tally3_queue_sync(mstream[dd][1]);
       magma_tally3_queue_sync(mstream[dd][2]);
}
