...
#define  dF(i, j) (dF  + (i) + (j)*(lddf))
    ...
    //double Akk;
    magma_tally4Double_ptr dAks;
    double tauk = MAGMA_tally4_D_ZERO;
    ...
    while( k < nb && lsticc == 0 ) {
        ...
        if (pvt != k) {
            /*if (pvt >= nb) {
                ...
                magma_tally4_dgetmatrix_async( m - offset - nb, 1,
            ...
            /*if (pvt < nb) {
                ...
                // 3. Restore the GPU
                magma_tally4_dsetmatrix_async( m - offset - nb, 1,
                                        A (offset + nb, pvt), lda,
            ...
            magma_tally4blas_dswap( i__1, dF(pvt, 0), lddf, dF(k, 0), lddf);
            itemp     = jpvt[pvt];
            jpvt[pvt] = jpvt[k];
            jpvt[k]   = itemp;
            //vn1[pvt] = vn1[k];
        ...
        if (k > 0) {
            ...
            for (j = 0; j < k; ++j) {
                *F(k,j) = MAGMA_tally4_D_CNJG( *F(k,j) );
            }
            #endif*/
...
//#define RIGHT_UPDATE
            ...
            i__1 = m - offset - nb;
            i__2 = k;
            magma_tally4_dgemv( Magma_tally4NoTrans, i__1, i__2,
            ...
            i__1 = m - rk;
            i__2 = k;
            /*blasf77_dgemv( Magma_tally4NoTransStr, &i__1, &i__2,
        ...
        magma_tally4_dlarfg_gpu( m-rk, dA(rk, k), dA(rk + 1, k), &tau[k], &vn1[k], &dAks[k]);

        //Akk = *A(rk, k);
        //*A(rk, k) = c_one;
        //magma_tally4_dgetvector( 1, &dAks[k],  1, &Akk,     1 );

        /* needed to avoid the race condition */
        if (k == 0) magma_tally4_dsetvector(  1,    &c_one,        1, dA(rk, k), 1 );
        else        magma_tally4_dcopymatrix( 1, 1, dA(offset, 0), 1, dA(rk, k), 1 );

        /* Compute Kth column of F:
           Compute  F(K+1:N,K) := tau(K)*A(RK:M,K+1:N)'*A(RK:M,K) on the GPU */
        if (k < n-1 || k > 0) magma_tally4_dgetvector( 1, &tau[k], 1, &tauk, 1 );
        if (k < n-1) {
            i__1 = m - rk;
            i__2 = n - k - 1;

            /* Send the vector to the GPU */
            //magma_tally4_dsetmatrix( i__1, 1, A(rk, k), lda, dA(rk,k), ldda );
            ...
            //                           A( RK,  K   ), 1,
            //                 CZERO,    F( K+1, K   ), 1 )
            //magma_tally4_dgetvector( 1, &tau[k], 1, &tauk, 1 );
            magma_tally4_dgemv( Magma_tally4ConjTrans, m-rk, n-k-1,
                         tauk,   dA( rk,  k+1 ), ldda,
                                 dA( rk,  k   ), 1,
                         c_zero, dF( k+1, k   ), 1 );
            //magma_tally4_dscal( m-rk, tau[k], F( k+1, k), 1 );
            //magma_tally4_int_t i__3 = nb-k-1;
            //magma_tally4_int_t i__4 = i__2 - i__3;
            //magma_tally4_int_t i__5 = nb-k;
            //magma_tally4_dgemv( Magma_tally4ConjTrans, i__1 - i__5, i__2 - i__3,
            //             tau[k], dA(rk +i__5, k+1+i__3), ldda,
            //                     dA(rk +i__5, k       ), ione,
            //             c_zero, dF(k+1+i__3, k       ), ione );
            
            //magma_tally4_dgetmatrix_async( i__2-i__3, 1,
            //                        dF(k + 1 +i__3, k), i__2,
            //                        F (k + 1 +i__3, k), i__2, stream );
            
            //blasf77_dgemv( Magma_tally4ConjTransStr, &i__1, &i__3,
            //               &tau[k], A(rk,  k+1), &lda,
            //                        A(rk,  k  ), &ione,
            //               &c_zero, F(k+1, k  ), &ione );
            
            //magma_tally4_queue_sync( stream );
            //blasf77_dgemv( Magma_tally4ConjTransStr, &i__5, &i__4,
        ...
        for (j = 0; j <= k; ++j) {
            magma_tally4_dsetvector( 1, &c_zero, 1, F(j, k), 1 );
        }*/
        ...
        if (k > 0) {
            ...
            i__1 = m - offset - nb;
            i__2 = k;
            magma_tally4_dgemv( Magma_tally4ConjTrans, i__1, i__2,
                         z__1,   dA(offset+nb, 0), lda,
                                 dA(offset+nb, k), ione,
                         c_zero, dauxv, ione );
            
            i__1 = k;
            magma_tally4_dgemv( Magma_tally4NoTrans, n-k-1, i__1,
                         c_one, F(k+1,0), ldf,
                                dauxv,     ione,
                         c_one, F(k+1,k), ione );
#else
            i__1 = m - rk;
            i__2 = k;
            //blasf77_dgemv( Magma_tally4ConjTransStr, &i__1, &i__2,
            //               &z__1,   A(rk, 0), &lda,
            //                        A(rk, k), &ione,
            //               &c_zero, auxv, &ione );
            ...
            //i__1 = k;
            //blasf77_dgemv( Magma_tally4NoTransStr, &n, &i__1,
            //               &c_one, F(0,0), &ldf,
            //                       auxv,   &ione,
            //               &c_one, F(0,k), &ione );
            /*magma_tally4_dgemv( Magma_tally4NoTrans, n, i__1,
                           c_one, F(0,0), ldf,
                                  auxv,   ione,
                           c_one, F(0,k), ione ); */
            /* I think we only need stricly lower-triangular part :) */
            magma_tally4_dgemv( Magma_tally4NoTrans, n-k-1, i__2,
                         c_one, dF(k+1,0), lddf,
                                dauxv,     ione,
                         c_one, dF(k+1,k), ione );
#endif
        ...
        if (k < n-1) {
            i__1 = n - k - 1;
            i__2 = k + 1;
            //blasf77_dgemm( Magma_tally4NoTransStr, Magma_tally4ConjTransStr, &ione, &i__1, &i__2,
            //               &c_neg_one, A(rk, 0  ), &lda,
            //                           F(k+1,0  ), &ldf,
            //               &c_one,     A(rk, k+1), &lda );
#ifdef RIGHT_UPDATE
            /* right-looking update of rows,                     */
            magma_tally4_dgemm( Magma_tally4NoTrans, Magma_tally4ConjTrans, nb-k, i__1, ione,
        ...
        if (rk < min(m, n+offset)-1 ) {
            magma_tally4blas_dnrm2_row_check_adjust(n-k-1, tol3z, &vn1[k+1], &vn2[k+1], dA(rk,k+1), ldda, dlsticcs);
        ...
        /*if (rk < lastrk) {
            for (j = k + 1; j < n; ++j) {
                if (vn1[j] != 0.) {
                    ...
                    temp = MAGMA_tally4_D_ABS( *A(rk,j) ) / vn1[j];
        ...
        //*A(rk, k) = Akk;
        //magma_tally4_dsetvector( 1, &Akk, 1, A(rk, k), 1 );
        //magma_tally4_dswap( 1, &dAks[k], 1, A(rk, k), 1 );
        
        ++k;
    }
    magma_tally4_dcopymatrix( 1, k, dAks, 1, dA(offset, 0), ldda+1 );
    ...
    *kb = k + 1;
    ...
    if (*kb < min(n, m - offset)) {
        ...
        /* Send F to the GPU
        magma_tally4_dsetmatrix( i__2, *kb,
                          F (*kb, 0), ldf,
                          dF(*kb, 0), i__2 ); */
    ...
    /*while( lsticc > 0 ) {
        ...
        i__1 = m - rk - 1;
        if (lsticc <= nb)
            vn1[lsticc] = magma_tally4_cblas_dnrm2( i__1, A(rk+1,lsticc), ione );
        else {
            // Where is the data, CPU or GPU ?
            double r1, r2;
            
            r1 = magma_tally4_cblas_dnrm2( nb-k, A(rk+1,lsticc), ione );
            r2 = magma_tally4_dnrm2(m-offset-nb, dA(offset + nb + 1, lsticc), ione);
