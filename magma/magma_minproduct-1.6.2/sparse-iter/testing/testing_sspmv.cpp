...
int main(  int argc, char** argv )
{
    ...
    magma_minproduct_s_matrix hA={Magma_minproduct_CSR}, hA_SELLP={Magma_minproduct_CSR}, hA_ELL={Magma_minproduct_CSR}, 
    dA={Magma_minproduct_CSR}, dA_SELLP={Magma_minproduct_CSR}, dA_ELL={Magma_minproduct_CSR};
    
    magma_minproduct_s_matrix hx={Magma_minproduct_CSR}, hy={Magma_minproduct_CSR}, dx={Magma_minproduct_CSR}, 
    ...
    float c_one  = MAGMA_minproduct_S_MAKE(1.0, 0.0);
    float c_zero = MAGMA_minproduct_S_MAKE(0.0, 0.0);
    ...
    while( i < argc ) {
        if ( strcmp("LAPLACE2D", argv[i]) == 0 && i+1 < argc ) {   // Laplace test
            ...
            CHECK( magma_minproduct_sm_5stencil(  laplace_size, &hA, queue ));
        } else {                        // file-matrix test
            CHECK( magma_minproduct_s_csr_mtx( &hA,  argv[i], queue ));
        }
        ...
        // init CPU vectors
        CHECK( magma_minproduct_svinit( &hx, Magma_minproduct_CPU, hA.num_rows, 1, c_zero, queue ));
        CHECK( magma_minproduct_svinit( &hy, Magma_minproduct_CPU, hA.num_rows, 1, c_zero, queue ));

        // init DEV vectors
        CHECK( magma_minproduct_svinit( &dx, Magma_minproduct_DEV, hA.num_rows, 1, c_one, queue ));
        CHECK( magma_minproduct_svinit( &dy, Magma_minproduct_DEV, hA.num_rows, 1, c_zero, queue ));
        ...
        // copy matrix to GPU
        CHECK( magma_minproduct_smtransfer( hA, &dA, Magma_minproduct_CPU, Magma_minproduct_DEV, queue ));
        // SpMV on GPU (CSR) -- this is the reference!
        start = magma_minproduct_sync_wtime( queue );
        for (j=0; j<10; j++)
            CHECK( magma_minproduct_s_spmv( c_one, dA, dx, c_zero, dy, queue ));
        end = magma_minproduct_sync_wtime( queue );
        printf( " > MAGMA_minproduct: %.2e seconds %.2e GFLOP/s    (standard CSR).\n",
                                        (end-start)/10, FLOPS*10/(end-start) );
        magma_minproduct_smfree(&dA, queue );
        CHECK( magma_minproduct_smtransfer( dy, &hrefvec , Magma_minproduct_DEV, Magma_minproduct_CPU, queue ));

        // convert to ELL and copy to GPU
        CHECK( magma_minproduct_smconvert(  hA, &hA_ELL, Magma_minproduct_CSR, Magma_minproduct_ELL, queue ));
        CHECK( magma_minproduct_smtransfer( hA_ELL, &dA_ELL, Magma_minproduct_CPU, Magma_minproduct_DEV, queue ));
        magma_minproduct_smfree(&hA_ELL, queue );
        magma_minproduct_smfree( &dy, queue );
        CHECK( magma_minproduct_svinit( &dy, Magma_minproduct_DEV, hA.num_rows, 1, c_zero, queue ));
        // SpMV on GPU (ELL)
        start = magma_minproduct_sync_wtime( queue );
        for (j=0; j<10; j++)
            CHECK( magma_minproduct_s_spmv( c_one, dA_ELL, dx, c_zero, dy, queue ));
        end = magma_minproduct_sync_wtime( queue );
        printf( " > MAGMA_minproduct: %.2e seconds %.2e GFLOP/s    (standard ELL).\n",
                                        (end-start)/10, FLOPS*10/(end-start) );
        magma_minproduct_smfree(&dA_ELL, queue );
        CHECK( magma_minproduct_smtransfer( dy, &hcheck , Magma_minproduct_DEV, Magma_minproduct_CPU, queue ));
        res = 0.0;
        for(magma_minproduct_int_t k=0; k<hA.num_rows; k++ )
            res=res + MAGMA_minproduct_S_REAL(hcheck.val[k]) - MAGMA_minproduct_S_REAL(hrefvec.val[k]);
        if ( res < .000001 )
            printf("# tester spmv ELL:  ok\n");
        else
            printf("# tester spmv ELL:  failed\n");
        magma_minproduct_smfree( &hcheck, queue );

        // convert to SELLP and copy to GPU
        CHECK( magma_minproduct_smconvert(  hA, &hA_SELLP, Magma_minproduct_CSR, Magma_minproduct_SELLP, queue ));
        CHECK( magma_minproduct_smtransfer( hA_SELLP, &dA_SELLP, Magma_minproduct_CPU, Magma_minproduct_DEV, queue ));
        magma_minproduct_smfree(&hA_SELLP, queue );
        magma_minproduct_smfree( &dy, queue );
        CHECK( magma_minproduct_svinit( &dy, Magma_minproduct_DEV, hA.num_rows, 1, c_zero, queue ));
        // SpMV on GPU (SELLP)
        start = magma_minproduct_sync_wtime( queue );
        for (j=0; j<10; j++)
            CHECK( magma_minproduct_s_spmv( c_one, dA_SELLP, dx, c_zero, dy, queue ));
        end = magma_minproduct_sync_wtime( queue );
        ...
        magma_minproduct_smfree(&dA_SELLP, queue );
        ...
        magma_minproduct_smfree(&hx, queue );
        magma_minproduct_smfree(&hy, queue );
        magma_minproduct_smfree(&hrefvec, queue );
        // free GPU memory
        magma_minproduct_smfree(&dA, queue );
        magma_minproduct_smfree(&dx, queue );
        magma_minproduct_smfree(&dy, queue );
