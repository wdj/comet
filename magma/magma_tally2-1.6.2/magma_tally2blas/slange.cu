    ...
    magma_tally2Float_ptr dwork )  //, magma_tally2_int_t lwork )
{
    ...
    if ( norm == Magma_tally2InfNorm ) {
        dim3 grid( (m-1)/64 + 1 );
        slange_inf_kernel<<< grid, threads, 0, magma_tally2_stream >>>( m, n, dA, ldda, dwork );
        ...
        dim3 grid( (m-1)/64 + 1 );
        slange_max_kernel<<< grid, threads, 0, magma_tally2_stream >>>( m, n, dA, ldda, dwork );
        max_nan_kernel<<< 1, 512, 0, magma_tally2_stream >>>( m, dwork );
        cudaMemcpy( &result, &dwork[0], sizeof(float), cudaMemcpyDeviceToHost );
    }
    else if ( norm == Magma_tally2OneNorm ) {
        dim3 grid( n );
        slange_one_kernel<<< grid, threads, 0, magma_tally2_stream >>>( m, n, dA, ldda, dwork );
