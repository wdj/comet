/*
    -- MAGMA_tally3 (version 1.6.2) --
       ...
       @generated from magma_tally3_zmcsrcompressor_gpu.cu normal z -> c, Sun May  3 11:22:58 2015
       @author Hartwig Anzt
...
__global__ void
magma_tally3_cmcsrgpu_kernel1( int num_rows,
                 magma_tally3FloatComplex *A_val,
                 magma_tally3_index_t *A_rowptr,
                 magma_tally3_index_t *A_colind,
                 magma_tally3FloatComplex *B_val,
    ...
    if(row<num_rows){
        magma_tally3FloatComplex zero = MAGMA_tally3_C_ZERO;
...
__global__ void
magma_tally3_cmcsrgpu_kernel2( int num_rows,
                 magma_tally3_index_t *B_rowptr,
                 magma_tally3_index_t *A_rowptr ){
...
__global__ void
magma_tally3_cmcsrgpu_kernel3( int num_rows,
                 magma_tally3FloatComplex *B_val,
                 magma_tally3_index_t *B_rowptr,
                 magma_tally3_index_t *B_colind,
    ...
    magma_tally3_queue_t queue )
{
    ...
    magma_tally3_c_matrix B={Magma_tally3_CSR}, B2={Magma_tally3_CSR};
    magma_tally3_c_matrix dA={Magma_tally3_CSR}, CSRA={Magma_tally3_CSR};
    ...
    if ( A->memory_location == Magma_tally3_DEV && A->storage_type == Magma_tally3_CSR ) {
        ...
        magma_tally3_cmcsrgpu_kernel1<<< grid1, BLOCK_SIZE1, 0, queue >>>
                ( A->num_rows, A->dval, A->drow, A->dcol, B.dval, B.drow, B.dcol );

        // correct the row pointer
        dim3 grid2( 1, 1, 1);
        magma_tally3_cmcsrgpu_kernel2<<< grid2, BLOCK_SIZE2, 0, queue >>>
        ...
        magma_tally3_cmcsrgpu_kernel3<<< grid1, BLOCK_SIZE1, 0, queue >>>
        ...
        CHECK( magma_tally3_cmconvert( *A, &CSRA, A->storage_type, Magma_tally3_CSR, queue ));
        CHECK( magma_tally3_cmtransfer( *A, &dA, A->memory_location, Magma_tally3_DEV, queue ));

        CHECK( magma_tally3_cmcsrcompressor_gpu( &dA, queue ));

        magma_tally3_cmfree( &dA, queue );
        magma_tally3_cmfree( A, queue );
        CHECK( magma_tally3_cmtransfer( dA, &CSRA, Magma_tally3_DEV, A_location, queue ));
        CHECK( magma_tally3_cmconvert( CSRA, A, Magma_tally3_CSR, A_storage, queue ));
        magma_tally3_cmfree( &dA, queue );
