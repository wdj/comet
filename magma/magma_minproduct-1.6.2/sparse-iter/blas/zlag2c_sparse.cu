...
__global__ void
magma_minproductint_zlag2c_sparse(
    int M,
    int N,
    const magma_minproductDoubleComplex_ptr A,
...
magma_minproductblas_zlag2c_sparse(
    ...
    magma_minproductFloatComplex *SA,
    ...
    magma_minproduct_queue_t queue )
{
    ...
    *info = 0;
    ...
    dim3 grid( magma_minproduct_ceildiv( M, BLOCKSIZE ) );


    cudaMemcpyToSymbol( flag, info, sizeof(flag) );    // flag = 0
    magma_minproductint_zlag2c_sparse<<< grid, BLOCKSIZE, 0, queue >>>
                                        ( M, N, A, SA ) ;
...
__global__ void
magma_minproduct_zlag2c_CSR_DENSE_kernel(
    int num_rows,
    int num_cols,
    magma_minproductDoubleComplex_ptr Aval,
    magma_minproductIndex_ptr Arow,
    magma_minproductIndex_ptr Acol,
    magma_minproductFloatComplex *Bval )
{
    ...
    if(row<num_rows){
        for( j=0; j<num_cols; j++)
            Bval[ j ] = MAGMA_minproduct_C_MAKE(0.0, 0.0);
        int start = Arow[ row ];
        int end = Arow[ row+1 ];
        for( j=start; j<end; j++ )
...
__global__ void
magma_minproduct_zlag2c_CSR_DENSE_kernel_1(
    int num_rows,
    int num_cols,
    magma_minproductFloatComplex_ptr Bval )
{
...
__global__ void
magma_minproduct_zlag2c_CSR_DENSE_kernel_2(
...
magma_minproduct_zlag2c_CSR_DENSE(
    ...
    magma_minproduct_c_sparse_matrix *B,
    magma_minproduct_queue_t queue )
{
    magma_minproduct_int_t stat;
    ...
    if ( A.memory_location == Magma_minproduct_DEV && A.storage_type == Magma_minproduct_CSR) {
        ...
        B->nnz = A.nnz;
        stat = magma_minproduct_cmalloc( &B->val, A.num_rows* A.num_cols );
        if ( stat != 0 )
        {printf("Memory Allocation Error converting matrix\n"); return -1;}
        
        dim3 Bs( BLOCKSIZE );
        dim3 Gs( magma_minproduct_ceildiv( A.num_rows, BLOCKSIZE ) );

        magma_minproduct_zlag2c_CSR_DENSE_kernel<<< Bs, Gs, 0, queue >>>
    ...
    magma_minproduct_queue_t queue )
{
    ...
    if ( A.memory_location == Magma_minproduct_DEV && A.storage_type == Magma_minproduct_CSR) {
        ...
        magma_minproduct_zlag2c_CSR_DENSE_kernel_1<<< Bs, Gs, 0, queue >>>
...
magma_minproduct_zlag2c_CSR_DENSE_convert(
    ...
    magma_minproduct_queue_t queue )
{
    if ( B->memory_location == Magma_minproduct_DEV && B->storage_type == Magma_minproduct_DENSE) {
        dim3 Bs( BLOCKSIZE );
        dim3 Gs( magma_minproduct_ceildiv( A.num_rows, BLOCKSIZE ) );

        magma_minproduct_zlag2c_CSR_DENSE_kernel_2<<< Bs, Gs, 0, queue >>>
        ( A.num_rows, A.num_cols, A.dval, A.drow, A.dcol, B->val );
    }
}
