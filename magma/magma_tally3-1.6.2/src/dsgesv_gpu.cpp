    ...
    magma_tally3_int_t *info)
{
    ...
    *info = 0;
     ...
     */
    //magma_tally3blas_dlag2s( n, nrhs, dB, lddb, dSX, lddsx, info );  // done inside dsgetrs with pivots
    if (*info != 0) {
        *iter = -2;
        goto FALLBACK;
    }
    
    magma_tally3blas_dlag2s( n, n, dA, ldda, dSA, lddsa, info );
    if (*info != 0) {
        *iter = -2;
        goto FALLBACK;
    }
    ...
    magma_tally3_sgetrf_gpu( n, n, dSA, lddsa, ipiv, info );
    if (*info != 0) {
        *iter = -3;
        goto FALLBACK;
    }
    ...
    magma_tally3_dsgetrs_gpu( trans, n, nrhs, dSA, lddsa, dipiv, dB, lddb, dX, lddx, dSX, info );
...
REFINEMENT:
    for( iiter=1; iiter < ITERMAX; ) {
        *info = 0;
        ...
        magma_tally3_dsgetrs_gpu( trans, n, nrhs, dSA, lddsa, dipiv, dR, lddr, dR, lddr, dSX, info );
        if (*info != 0) {
            *iter = -3;
            goto FALLBACK;
        }
        ...
        for( j=0; j < nrhs; j++ ) {
            magma_tally3blas_daxpycp( n, dR(0,j), dX(0,j), dB(0,j) );
        }
...
FALLBACK:
    ...
    magma_tally3_dgetrf_gpu( n, n, dA, ldda, ipiv, info );
    if (*info == 0) {
        magma_tally3blas_dlacpy( Magma_tally3UpperLower, n, nrhs, dB, lddb, dX, lddx );
        magma_tally3_dgetrs_gpu( trans, n, nrhs, dA, ldda, ipiv, dX, lddx, info );
    }
