    ...
    magma_tally3FloatComplex * __restrict__ work)
{
    ...
    __shared__ magma_tally3FloatComplex sA [quarter_NB_X][NB_X + 3]; /* Why +3? seems it only needs +2. Does +3 reduce bank conflicts? */
...
/**
    Purpose
    -------
    magma_tally3blas_csymv_work performs the matrix-vector operation:
    ...
    MAGMA_tally3 implements csymv through two steps:
    ...
    magma_tally3blas_csymv is a wrapper routine allocating the workspace inside the
    routine and provides the same interface as cublas.
    
    If users need to call csymv frequently, we suggest using
    magma_tally3blas_csymv_work instead of magma_tally3blas_csymv. As the overhead to
    allocate and free in device memory in magma_tally3blas_csymv would hurt performance.
    ...
    magma_tally3_queue_t queue )
{
    ...
    if ( arch < 200 ) {
        //magma_tally3_csymv( uplo, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
     ...
     */
    if ( (n == 0) || ( MAGMA_tally3_C_EQUAL(alpha, MAGMA_tally3_C_ZERO) && MAGMA_tally3_C_EQUAL(beta, MAGMA_tally3_C_ONE) ) )
        return info;
...
// end magma_tally3blas_csymv_work


/**
    Purpose
    -------
    magma_tally3blas_csymv performs the matrix-vector operation:
    ...
    magma_tally3FloatComplex_ptr dy, magma_tally3_int_t incy)
{
    ...
    magma_tally3_cmalloc( &dwork, lwork );
    if ( dwork == NULL ) {
    ...
    magma_tally3blas_csymv_work( uplo, n, alpha, dA, ldda, dx, incx, beta, dy, incy,
