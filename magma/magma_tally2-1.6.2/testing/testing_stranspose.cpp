/*
    -- MAGMA_tally2 (version 1.6.1) --
       ...
       @generated from testing_ztranspose.cpp normal z -> s, Fri Jan 30 19:00:24 2015
       @author Mark Gates
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for( int j = 0; j < N; ++j ) {
                for( int i = 0; i < M; ++i ) {
                    h_A[i + j*lda] = MAGMA_tally2_S_MAKE( i + j/10000., j );
                }
            }
            for( int j = 0; j < M; ++j ) {
                for( int i = 0; i < N; ++i ) {
                    h_B[i + j*ldb] = MAGMA_tally2_S_MAKE( i + j/10000., j );
                }
            }
            magma_tally2_ssetmatrix( N, M, h_B, ldb, d_B, lddb );
            ...
            //magma_tally2blas_stranspose( M-2, N-2, d_A+1+ldda, ldda, d_B+1+lddb, lddb );  // inset by 1 row & col
            magma_tally2blas_stranspose( M, N, d_A, ldda, d_B, lddb );
            ...
            if ( M == N ) {
                ...
                //magma_tally2blas_stranspose_inplace( N-2, d_A+1+ldda, ldda );  // inset by 1 row & col
                magma_tally2blas_stranspose_inplace( N, d_A, ldda );
