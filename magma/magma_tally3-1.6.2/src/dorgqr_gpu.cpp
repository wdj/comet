...
#define dT(j)   (dT + (j)*nb)
    ...
    *info = 0;
    ...
    lwork = n*nb;
    lpanel = (m - kk)*(n - kk);
    magma_tally3_dmalloc_cpu( &work, lwork + lpanel );
    if ( work == NULL ) {
    ...
    if (MAGMA_tally3_SUCCESS != magma_tally3_dmalloc( &dV, ldda*nb )) {
    ...
    if (kk < n) {
        ...
        n_kk = n - kk;
        k_kk = k - kk;
        magma_tally3_dgetmatrix( m_kk, k_kk,
    ...
    if (kk > 0) {
        ...
        for (i = ki; i >= 0; i -= nb) {
            ...
            magma_tally3_dcopymatrix_async( mi, ib,
                                     dA(i,i), ldda,
                                     dV,      ldda, stream );
