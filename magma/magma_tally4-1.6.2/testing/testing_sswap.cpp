...
static int diff_matrix( magma_tally4_int_t m, magma_tally4_int_t n, float *A, magma_tally4_int_t lda, float *B, magma_tally4_int_t ldb )
{
    for( magma_tally4_int_t j = 0; j < n; j++ ) {
        for( magma_tally4_int_t i = 0; i < m; i++ ) {
            if ( ! MAGMA_tally4_S_EQUAL( A[lda*j+i], B[ldb*j+i] ) )
                return 1;
        }
    }
    return 0;
...
static void init_matrix( magma_tally4_int_t m, magma_tally4_int_t n, float *A, magma_tally4_int_t lda, magma_tally4_int_t offset )
{
    assert( lda >= m );
    for( magma_tally4_int_t j = 0; j < n; ++j ) {
        for( magma_tally4_int_t i=0; i < m; ++i ) {
            A[i + j*lda] = MAGMA_tally4_S_MAKE( offset + (i+1) + (j+1)/10000., 0 );
        }
    }
}
...
int main( int argc, char** argv)
{
    ...
    float *h_A1, *h_A2;
    float *h_R1, *h_R2;
    magma_tally4Float_ptr d_A1, d_A2;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lda    = N;
            ldda   = ((N+31)/32)*32;
            nb     = (opts.nb > 0 ? opts.nb : magma_tally4_get_sgetrf_nb( N ));
            nb     = min( N, nb );
            ...
            magma_tally4_ssetmatrix( N, N, h_A1, lda, d_A1, ldda );
            magma_tally4_ssetmatrix( N, N, h_A2, lda, d_A2, ldda );
            ...
            for( j=0; j < nb; j++) {
                if ( j != (ipiv[j]-1)) {
                    ...
                    #else
                        magma_tally4_sswap( N, d_A1, ldda*j, 1, d_A2, ldda*(ipiv[j]-1), 1, opts.queue );
                    #endif
                }
            }
            ...
            magma_tally4_sgetmatrix( N, N, d_A1, ldda, h_R1, lda );
            magma_tally4_sgetmatrix( N, N, d_A2, ldda, h_R2, lda );
            ...
            for( j=0; j < nb; j++) {
                if ( j != (ipiv[j]-1)) {
                    ...
                    #else
                        magma_tally4_sswap( N, d_A1, j, ldda, d_A2, ipiv[j]-1, ldda, opts.queue );
                    #endif
                }
            }
            time = magma_tally4_sync_wtime( queue ) - time;
            ...
            for( j=0; j < nb; j++) {
                if ( j != (ipiv[j]-1)) {
                    magma_tally4blas_sswap( N, d_A1+ldda*j, 1, d_A2+ldda*(ipiv[j]-1), 1);
                }
            }
            time = magma_tally4_sync_wtime( queue ) - time;
            ...
            for( j=0; j < nb; j++) {
                if ( j != (ipiv[j]-1)) {
                    magma_tally4blas_sswap( N, d_A1+j, ldda, d_A2+ipiv[j]-1, ldda );
                }
            }
            time = magma_tally4_sync_wtime( queue ) - time;
            ...
            time = magma_tally4_sync_wtime( queue );
            magma_tally4blas_sswapblk( Magma_tally4RowMajor, N, d_A1, ldda, d_A2, ldda, 1, nb, ipiv, 1, 0);
            time = magma_tally4_sync_wtime( queue ) - time;
            ...
            time = magma_tally4_sync_wtime( queue );
            magma_tally4blas_sswapblk( Magma_tally4ColMajor, N, d_A1, ldda, d_A2, ldda, 1, nb, ipiv, 1, 0);
            time = magma_tally4_sync_wtime( queue ) - time;
            ...
            time = magma_tally4_sync_wtime( queue );
            magma_tally4blas_slaswp( N, d_A1, ldda, 1, nb, ipiv, 1);
            time = magma_tally4_sync_wtime( queue ) - time;
            ...
            magma_tally4blas_slaswp2( N, d_A1, ldda, 1, nb, d_ipiv, 1 );
            time = magma_tally4_sync_wtime( queue ) - time;
            ...
            time = magma_tally4_sync_wtime( queue );
            magma_tally4blas_slaswpx( N, d_A1, ldda, 1, 1, nb, ipiv, 1);
            time = magma_tally4_sync_wtime( queue ) - time;
            ...
            time = magma_tally4_sync_wtime( queue );
            magma_tally4blas_slaswpx( N, d_A1, 1, ldda, 1, nb, ipiv, 1);
            time = magma_tally4_sync_wtime( queue ) - time;
            ...
            time = magma_tally4_sync_wtime( queue );
            magma_tally4_scopymatrix( N, nb, d_A1, ldda, d_A2, ldda );
            time = magma_tally4_sync_wtime( queue ) - time;
            // copy reads 1 matrix and writes 1 matrix, so has half gbytes of swap
            col_perf6 = 0.5 * gbytes / time;
            
            time = magma_tally4_sync_wtime( queue );
            magma_tally4_scopymatrix( nb, N, d_A1, ldda, d_A2, ldda );
            time = magma_tally4_sync_wtime( queue ) - time;
            // copy reads 1 matrix and writes 1 matrix, so has half gbytes of swap
