...
// BLK_K gets defined in magma_tally2blas_dgemm_reduce,
    ...
    double      * __restrict__ dC, int ldc)
{
    ...
    if (blockIdx.x*BLK_M + threadIdx.y < m && blockIdx.y*BLK_N + threadIdx.z < n){
        ...
        for( int j = tx; j < k; j += BLK_K )
            lsum += MAGMA_tally2_D_CNJG( dA[j] )* dB[j];
        ...
        __syncthreads();
        if (threadIdx.x == 0) {
            if (MAGMA_tally2_D_EQUAL(beta, MAGMA_tally2_D_ZERO))
    ...
    magma_tally2Double_ptr       dC, magma_tally2_int_t lddc )
{
    ...
    if ( arch < 200  ) {
        ...
        dgemm_reduce_kernel<BLK_K> <<< blocks, threads, 0, magma_tally2_stream >>>
