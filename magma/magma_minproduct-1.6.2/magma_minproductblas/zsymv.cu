    ...
    magma_minproductDoubleComplex * __restrict__ work)
{
    ...
    __shared__ magma_minproductDoubleComplex sA [quarter_NB_X][NB_X + 3]; /* Why +3? seems it only needs +2. Does +3 reduce bank conflicts? */
...
/**
    Purpose
    -------
    magma_minproductblas_zsymv_work performs the matrix-vector operation:
    ...
    uplo    magma_minproduct_uplo_t.
            ...
            follows:
      -     = Magma_minproductUpper:  Only the upper triangular part of A is to be referenced.
    ...
    MAGMA_minproduct implements zsymv through two steps:
    ...
    magamblas_zsymv_work requires users to provide a workspace, while
    magma_minproductblas_zsymv is a wrapper routine allocating the workspace inside the
    routine and provides the same interface as cublas.
    
    If users need to call zsymv frequently, we suggest using
    magma_minproductblas_zsymv_work instead of magma_minproductblas_zsymv. As the overhead to
    allocate and free in device memory in magma_minproductblas_zsymv would hurt performance.
    ...
    magma_minproduct_queue_t queue )
{
    ...
    if ( arch < 200 ) {
        //magma_minproduct_zsymv( uplo, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    ...
    magma_minproduct_int_t blocks = ceildiv( n, NB_X );
    magma_minproduct_int_t lwmin  = ldda*blocks;
     ...
     */
    if ( (n == 0) || ( MAGMA_minproduct_Z_EQUAL(alpha, MAGMA_minproduct_Z_ZERO) && MAGMA_minproduct_Z_EQUAL(beta, MAGMA_minproduct_Z_ONE) ) )
        return info;
...
// end magma_minproductblas_zsymv_work


/**
    Purpose
    -------
    magma_minproductblas_zsymv performs the matrix-vector operation:
    ...
    magma_minproductDoubleComplex_ptr dy, magma_minproduct_int_t incy)
{
    ...
    magma_minproduct_int_t blocks = ceildiv( n, NB_X );
    magma_minproduct_int_t lwork  = ldda*blocks;

    magma_minproduct_zmalloc( &dwork, lwork );
    if ( dwork == NULL ) {
    ...
    magma_minproductblas_zsymv_work( uplo, n, alpha, dA, ldda, dx, incx, beta, dy, incy,
                          dwork, lwork, magma_minproduct_stream );
    
    magma_minproduct_free( dwork );
    
    return info;
}
// end magma_minproductblas_zsymv
