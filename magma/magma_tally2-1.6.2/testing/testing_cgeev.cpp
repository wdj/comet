...
bool lessthan( magma_tally2FloatComplex a, magma_tally2FloatComplex b )
{
    return (MAGMA_tally2_C_REAL(a) < MAGMA_tally2_C_REAL(b)) ||
        (MAGMA_tally2_C_REAL(a) == MAGMA_tally2_C_REAL(b) && MAGMA_tally2_C_IMAG(a) < MAGMA_tally2_C_IMAG(b));
}
...
extern "C"
float magma_tally2_dlapy2(float x, float y)
{
    float ret_val, d;
    float w, z, xabs, yabs;
    
    xabs = MAGMA_tally2_D_ABS(x);
    yabs = MAGMA_tally2_D_ABS(y);
    w    = max(xabs, yabs);
    z    = min(xabs, yabs);
    if (z == 0.) {
        ret_val = w;
...
extern "C"
float magma_tally2_dzlapy2(magma_tally2FloatComplex x)
{
    return magma_tally2_dlapy2( MAGMA_tally2_C_REAL(x), MAGMA_tally2_C_IMAG(x) );
}
...
int main( int argc, char** argv)
{
    ...
    magma_tally2FloatComplex *h_A, *h_R, *VL, *VR, *h_work, *w1, *w2;
    magma_tally2FloatComplex *w1copy, *w2copy;
    magma_tally2FloatComplex  c_neg_one = MAGMA_tally2_C_NEG_ONE;
    float *rwork;
    float tnrm, result[9];
    magma_tally2_int_t N, n2, lda, nb, lwork, info;
    ...
    if ( opts.check && !opts.lapack && opts.jobvl == Magma_tally2NoVec && opts.jobvr == Magma_tally2NoVec ) {
    ...
    printf("    N   CPU Time (sec)   GPU Time (sec)   |W_magma_tally2 - W_lapack| / |W_lapack|\n");
    printf("===========================================================================\n");
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( w1copy, magma_tally2FloatComplex, N );
            TESTING_MALLOC_CPU( w2copy, magma_tally2FloatComplex, N );
            TESTING_MALLOC_CPU( w1,     magma_tally2FloatComplex, N );
            TESTING_MALLOC_CPU( w2,     magma_tally2FloatComplex, N );
            ...
            TESTING_MALLOC_PIN( VL,     magma_tally2FloatComplex, n2 );
            TESTING_MALLOC_PIN( VR,     magma_tally2FloatComplex, n2 );
            TESTING_MALLOC_PIN( h_work, magma_tally2FloatComplex, lwork );
            
            /* Initialize the matrix */
            lapackf77_clarnv( &ione, ISEED, &n2, h_A );
            lapackf77_clacpy( Magma_tally2UpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
            
            /* ====================================================================
               Performs operation using MAGMA_tally2
               =================================================================== */
            gpu_time = magma_tally2_wtime();
            magma_tally2_cgeev( opts.jobvl, opts.jobvr,
                         N, h_R, lda, w1,
                         VL, lda, VR, lda,
                         h_work, lwork, rwork, &info );
            gpu_time = magma_tally2_wtime() - gpu_time;
            if (info != 0)
                printf("magma_tally2_cgeev returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                if ( opts.jobvr == Magma_tally2Vec ) {
                    // Do test 1: | A * VR - VR * W | / ( n |A| )
                    // Note this writes result[1] also
                    lapackf77_cget22( Magma_tally2NoTransStr, Magma_tally2NoTransStr, Magma_tally2NoTransStr,
                                      &N, h_A, &lda, VR, &lda, w1,
                    ...
                    result[1] = -1.;
                    for( int j = 0; j < N; ++j ) {
                        tnrm = magma_tally2_cblas_scnrm2( N, &VR[j*lda], ione );
                        result[1] = max( result[1], min( ulpinv, fabs(tnrm-1.)/ulp ));
                        
                        vmx = vrmx = 0.;
                        for( int jj = 0; jj <N; ++jj ) {
                            vtst = magma_tally2_dzlapy2(VR[jj + j*lda]);
                            if (vtst > vmx)
                                vmx = vtst;
                            
                            if (MAGMA_tally2_C_IMAG(VR[jj + j*lda])==0. &&
                                fabs( MAGMA_tally2_C_REAL(VR[jj+j*lda]) ) > vrmx)
                            {
                                vrmx = fabs( MAGMA_tally2_C_REAL( VR[jj+j*lda] ) );
                            }
                        }
                        if (vrmx / vmx < 1. - ulp*2.)
                            result[1] = ulpinv;
                ...
                if ( opts.jobvl == Magma_tally2Vec ) {
                    // Do test 3: | A**H * VL - VL * W**H | / ( n |A| )
                    // Note this writes result[3] also
                    lapackf77_cget22( Magma_tally2ConjTransStr, Magma_tally2NoTransStr, Magma_tally2ConjTransStr,
                                      &N, h_A, &lda, VL, &lda, w1,
                    ...
                    result[3] = -1.;
                    for( int j = 0; j < N; ++j ) {
                        tnrm = magma_tally2_cblas_scnrm2( N, &VL[j*lda], ione );
                        result[3] = max( result[3], min( ulpinv, fabs(tnrm - 1.)/ ulp ));
                        
                        vmx = vrmx = 0.;
                        for( int jj = 0; jj < N; ++jj ) {
                            vtst = magma_tally2_dzlapy2(VL[jj + j*lda]);
                            if (vtst > vmx)
                                vmx = vtst;
                            
                            if (MAGMA_tally2_C_IMAG(VL[jj + j*lda])==0. &&
                                fabs( MAGMA_tally2_C_REAL( VL[jj + j*lda] ) ) > vrmx)
                            {
                                vrmx = fabs( MAGMA_tally2_C_REAL( VL[jj+j*lda]) );
                            }
                        }
                        if (vrmx / vmx < 1. - ulp*2.)
                            result[3] = ulpinv;
            ...
            if ( opts.check == 2 ) {
                ...
                // this is really slow because it calls magma_tally2_cgeev multiple times
                magma_tally2FloatComplex *LRE, DUM;
                TESTING_MALLOC_PIN( LRE, magma_tally2FloatComplex, n2 );
                ...
                magma_tally2_cgeev( Magma_tally2Vec, Magma_tally2Vec,
                             N, h_R, lda, w1,
                             VL, lda, VR, lda,
                             h_work, lwork, rwork, &info );
                if (info != 0)
                    printf("magma_tally2_cgeev (case V, V) returned error %d: %s.\n",
                           (int) info, magma_tally2_strerror( info ));
                ...
                // lapackf77_clacpy( Magma_tally2UpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                // magma_tally2_cgeev( Magma_tally2NoVec, Magma_tally2NoVec,
                //              N, h_R, lda, w2,
                //              &DUM, 1, &DUM, 1,
                //              h_work, lwork, rwork, &info );
                // if (info != 0)
                //     printf("magma_tally2_cgeev (case N, N) returned error %d: %s.\n",
                //            (int) info, magma_tally2_strerror( info ));
                // 
                // // Do test 5: W(full) = W(partial, W only)
                // result[4] = 1;
                // for( int j = 0; j < N; ++j )
                //     if ( ! MAGMA_tally2_C_EQUAL( w1[j], w2[j] ))
                //         result[4] = 0;
                
                // ----------
                // Compute eigenvalues and right eigenvectors
                lapackf77_clacpy( Magma_tally2UpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                magma_tally2_cgeev( Magma_tally2NoVec, Magma_tally2Vec,
                             N, h_R, lda, w2,
                             &DUM, 1, LRE, lda,
                             h_work, lwork, rwork, &info );
                if (info != 0)
                    printf("magma_tally2_cgeev (case N, V) returned error %d: %s.\n",
                           (int) info, magma_tally2_strerror( info ));
                ...
                for( int j = 0; j < N; ++j )
                    for( int jj = 0; jj < N; ++jj )
                        if ( ! MAGMA_tally2_C_EQUAL( VR[j+jj*lda], LRE[j+jj*lda] ))
                            result[7] = 0;
                
                // ----------
                // Compute eigenvalues and left eigenvectors
                lapackf77_clacpy( Magma_tally2UpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                magma_tally2_cgeev( Magma_tally2Vec, Magma_tally2NoVec,
                             N, h_R, lda, w2,
                             LRE, lda, &DUM, 1,
                             h_work, lwork, rwork, &info );
                if (info != 0)
                    printf("magma_tally2_cgeev (case V, N) returned error %d: %s.\n",
                           (int) info, magma_tally2_strerror( info ));
                ...
                for( int j = 0; j < N; ++j )
                    for( int jj = 0; jj < N; ++jj )
                        if ( ! MAGMA_tally2_C_EQUAL( VL[j+jj*lda], LRE[j+jj*lda] ))
                            result[8] = 0;
            ...
            if ( opts.lapack ) {
                ...
                lapackf77_cgeev( lapack_vec_const_tally2(opts.jobvl), lapack_vec_const_tally2(opts.jobvr),
                                 &N, h_A, &lda, w2,
                                 VL, &lda, VR, &lda,
                                 h_work, &lwork, rwork, &info );
                ...
                // check | W_magma_tally2 - W_lapack | / | W |
                ...
                for( int j=0; j < N; ++j ) {
                    for( int j2=j; j2 < N; ++j2 ) {
                        magma_tally2FloatComplex diff = MAGMA_tally2_C_SUB( w1copy[j], w2copy[j2] );
                        float diff2 = magma_tally2_dzlapy2( diff ) / max( magma_tally2_dzlapy2( w1copy[j] ), tol );
                        if ( diff2 < 100*tol ) {
                            if ( j != j2 ) {
                ...
                error  = magma_tally2_cblas_scnrm2( N, w1copy, 1 );
                error /= magma_tally2_cblas_scnrm2( N, w2copy, 1 );
