/*
    -- MAGMA_tally4 (version 1.6.1) --
       ...
       @generated from zpotrf_mgpu.cpp normal z -> s, Fri Jan 30 19:00:13 2015
    ...
    magma_tally4_int_t *info)
{
    ...
    float *dwork[Magma_tally4MaxGPUs];
    ...
    *info = 0;
    ...
    if (ngpu == 1 && ((nb <= 1) || (nb >= n)) ) {
        ...
        if (MAGMA_tally4_SUCCESS != magma_tally4_smalloc_pinned( &work, n*nb )) {
            *info = MAGMA_tally4_ERR_HOST_ALLOC;
            return *info;
        }
        magma_tally4_sgetmatrix( n, n, d_lA[0], ldda, work, n );
        lapackf77_spotrf(uplo_, &n, work, &n, info);
        magma_tally4_ssetmatrix( n, n, work, n, d_lA[0], ldda );
        ...
        for( d=0; d < ngpu; d++ ) {
            ...
            if (MAGMA_tally4_SUCCESS != magma_tally4_smalloc( &dwork[d], ngpu*nb*lddp )) {
                for( j=0; j < d; j++ ) {
                    magma_tally4_setdevice(j);
                    magma_tally4_free( dwork[j] );
                }
        ...
        magma_tally4_setdevice(0);
        h = 1; //ngpu; //(n+nb-1)/nb;
        if (MAGMA_tally4_SUCCESS != magma_tally4_smalloc_pinned( &work, n*nb*h )) {
            *info = MAGMA_tally4_ERR_HOST_ALLOC;
            return *info;
        }
        if (upper) {
            /* with three streams */
            magma_tally4_spotrf3_mgpu(ngpu, uplo, n, n, 0, 0, nb, d_lA, ldda, dwork, lddp, work, n,
                               h, stream, event, info);
        } else {
            /* with three streams */
            magma_tally4_spotrf3_mgpu(ngpu, uplo, n, n, 0, 0, nb, d_lA, ldda, dwork, lddp, work, nb*h,
                               h, stream, event, info);
        }
        ...
        for( d=0; d < ngpu; d++ ) {
            magma_tally4_setdevice(d);
            for( j=0; j < 3; j++ ) {
                magma_tally4_queue_sync( stream[d][j] );
