...
static void *magma_tally2_sapplyQ_m_parallel_section(void *arg);
...
class magma_tally2_sapplyQ_m_data {

public:

    magma_tally2_sapplyQ_m_data(magma_tally2_int_t ngpu_, magma_tally2_int_t threads_num_, magma_tally2_int_t n_, magma_tally2_int_t ne_, magma_tally2_int_t n_gpu_,
                         magma_tally2_int_t nb_, magma_tally2_int_t Vblksiz_, float *E_, magma_tally2_int_t lde_,
                         float *V_, magma_tally2_int_t ldv_, float *TAU_,
                         float *T_, magma_tally2_int_t ldt_)
    :
    ngpu(ngpu_),
...
private:
    ...
    magma_tally2_sapplyQ_m_data(magma_tally2_sapplyQ_m_data& data); // disable copy
};

class magma_tally2_sapplyQ_m_id_data {

public:

    magma_tally2_sapplyQ_m_id_data()
    : id(-1), data(NULL)
    {}

    magma_tally2_sapplyQ_m_id_data(magma_tally2_int_t id_, magma_tally2_sapplyQ_m_data* data_)
    : id(id_), data(data_)
    {}

    magma_tally2_int_t id;
    magma_tally2_sapplyQ_m_data* data;
};
...
//n_gpu = ne;
    if (n_gpu < ne) {
        ...
        #endif
        magma_tally2_sapplyQ_m_data data_applyQ(ngpu, threads, n, ne, n_gpu, nb, Vblksiz, Z, ldz, V, ldv, TAU, T, ldt);

        magma_tally2_sapplyQ_m_id_data* arg;
        magma_tally2_malloc_cpu((void**) &arg, threads*sizeof(magma_tally2_sapplyQ_m_id_data));
        ...
        for (magma_tally2_int_t thread = 1; thread < threads; thread++) {
            arg[thread] = magma_tally2_sapplyQ_m_id_data(thread, &data_applyQ);
            pthread_create(&thread_id[thread], &thread_attr, magma_tally2_sapplyQ_m_parallel_section, &arg[thread]);
        }
        arg[0] = magma_tally2_sapplyQ_m_id_data(0, &data_applyQ);
        magma_tally2_sapplyQ_m_parallel_section(&arg[0]);
        ...
        magma_tally2_sbulge_applyQ_v2_m(ngpu, Magma_tally2Left, ne, n, nb, Vblksiz, Z, ldz, V, ldv, T, ldt, info);
...
static void *magma_tally2_sapplyQ_m_parallel_section(void *arg)
{
    magma_tally2_int_t my_core_id     = ((magma_tally2_sapplyQ_m_id_data*)arg) -> id;
    magma_tally2_sapplyQ_m_data* data = ((magma_tally2_sapplyQ_m_id_data*)arg) -> data;
    ...
    if (my_core_id == 0) {
        ...
        magma_tally2_sbulge_applyQ_v2_m(ngpu, Magma_tally2Left, n_gpu, n, nb, Vblksiz, E, lde, V, ldv, T, ldt, &info);
        magma_tally2_device_sync();
