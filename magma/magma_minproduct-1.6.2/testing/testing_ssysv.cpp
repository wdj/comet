...
void init_matrix( int m, int n, float *h_A, magma_minproduct_int_t lda )
{
    assert( m == n );
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( opts.lapack ) {
                ...
                lapackf77_slacpy( Magma_minproductUpperLowerStr, &N, &opts.nrhs, h_B, &ldb, h_X, &ldb );
            ...
            magma_minproduct_ssysv( opts.uplo, N, opts.nrhs, h_A, lda, ipiv, h_X, ldb, &info);
            gpu_time = magma_minproduct_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_minproduct_ssysv returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
