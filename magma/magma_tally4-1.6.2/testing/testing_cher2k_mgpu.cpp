...
/* ////////////////////////////////////////////////////////////////////////////
   -- Testing magma_tally4_cher2k_mgpu
*/
int main( int argc, char** argv)
{
    ...
    magma_tally4FloatComplex alpha = MAGMA_tally4_C_MAKE( 1.2345, 4.321 );
    float beta = 3.14159;
    
    real_Double_t    gflops, gpu_perf, cpu_perf, gpu_time, cpu_time;
    float           error, work[1];
    magma_tally4FloatComplex *hA, *hR, *hR2, *hV, *hW;
    magma_tally4FloatComplex_ptr dV[Magma_tally4MaxGPUs], dW[Magma_tally4MaxGPUs], dA[Magma_tally4MaxGPUs];
    ...
    printf( "version 1: magma_tally4blas_cher2k_mgpu2     %s\n", (opts.version==1 ? "(enabled)" : ""));
    printf( "version 2: magma_tally4blas_cher2k_mgpu_spec %s\n", (opts.version==2 ? "(enabled)" : ""));
#ifdef ICHI
    printf( "version 3: magma_tally4_cher2k_mgpu (Ichi)   %s\n", (opts.version==3 ? "(enabled)" : ""));
#endif
    printf( "\n" );
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        ...
        for( int offset = 0; offset < n; offset += min(k,nb) ) {
            for( int iter = 0; iter < opts.niter; ++iter ) {
                ...
                TESTING_MALLOC_CPU( hR2, magma_tally4FloatComplex, lda*n   );
                TESTING_MALLOC_CPU( hV,  magma_tally4FloatComplex, lda*k*2 );
                //TESTING_MALLOC_CPU( hW,  magma_tally4FloatComplex, lda*k   );
                for( int d = 0; d < ngpu; ++d ) {
                    magma_tally4_int_t nlocal = ((n / nb) / ngpu + 1) * nb;
                    magma_tally4_setdevice( d );
                    TESTING_MALLOC_DEV( dA[d], magma_tally4FloatComplex, ldda*nlocal );
                    TESTING_MALLOC_DEV( dV[d], magma_tally4FloatComplex, ldda*k*2    );
                    //TESTING_MALLOC_DEV( dW[d], magma_tally4FloatComplex, ldda*k      );
                ...
                /* ====================================================================
                   ...
                   =================================================================== */
                magma_tally4_csetmatrix_1D_col_bcyclic( n, n, hA, lda, dA, ldda, ngpu, nb );
                for( int d = 0; d < ngpu; ++d ) {
                    magma_tally4_setdevice( d );
                    dW[d] = dV[d] + ldda*k;
                    magma_tally4_csetmatrix( n, k, hV, lda, dV[d], ldda );
                    magma_tally4_csetmatrix( n, k, hW, lda, dW[d], ldda );
                }
                ...
                magma_tally4_cgetmatrix_1D_col_bcyclic( n, n, dA, ldda, hR, lda, ngpu, nb );
