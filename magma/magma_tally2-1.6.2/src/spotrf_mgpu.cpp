    ...
    magma_tally2_int_t *info)
{
    ...
    float *dwork[Magma_tally2MaxGPUs];
    ...
    *info = 0;
    ...
    if (ngpu == 1 && ((nb <= 1) || (nb >= n)) ) {
        ...
        if (MAGMA_tally2_SUCCESS != magma_tally2_smalloc_pinned( &work, n*nb )) {
            *info = MAGMA_tally2_ERR_HOST_ALLOC;
            return *info;
        }
        magma_tally2_sgetmatrix( n, n, d_lA[0], ldda, work, n );
        lapackf77_spotrf(uplo_, &n, work, &n, info);
        magma_tally2_ssetmatrix( n, n, work, n, d_lA[0], ldda );
        ...
        for( d=0; d < ngpu; d++ ) {
            ...
            if (MAGMA_tally2_SUCCESS != magma_tally2_smalloc( &dwork[d], ngpu*nb*lddp )) {
                for( j=0; j < d; j++ ) {
        ...
        magma_tally2_setdevice(0);
        h = 1; //ngpu; //(n+nb-1)/nb;
        if (MAGMA_tally2_SUCCESS != magma_tally2_smalloc_pinned( &work, n*nb*h )) {
            *info = MAGMA_tally2_ERR_HOST_ALLOC;
            return *info;
        }
        if (upper) {
            /* with three streams */
            magma_tally2_spotrf3_mgpu(ngpu, uplo, n, n, 0, 0, nb, d_lA, ldda, dwork, lddp, work, n,
                               h, stream, event, info);
        } else {
            /* with three streams */
            magma_tally2_spotrf3_mgpu(ngpu, uplo, n, n, 0, 0, nb, d_lA, ldda, dwork, lddp, work, nb*h,
                               h, stream, event, info);
        }
