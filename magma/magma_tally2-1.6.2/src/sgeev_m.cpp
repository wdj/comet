 ...
 * Version2 - MAGMA_tally2 without dT    (magma_tally2_zgehrd2 and lapack_zunghr)
 * Version3 - MAGMA_tally2 with dT       (magma_tally2_zgehrd  and magma_tally2_zunghr)
 * Version4 - Multi-GPU magma_tally2_zgehrd_m with T on CPU, copied to dT, single-GPU magma_tally2_zunghr
 * Version5 - Multi-GPU magma_tally2_zgehrd_m with T on CPU, multi-GPU magma_tally2_zunghr_m
 */
#define Version5
...
/**
    ...
    @ingroup magma_tally2_sgeev_driver
    ********************************************************************/
    ...
    magma_tally2_int_t *info )
{
    ...
    magma_tally2_int_t scalea, minwrk, optwrk, lquery, wantvl, wantvr, select[1];
    
    magma_tally2_side_t side = Magma_tally2Right;
    
    magma_tally2_timer_t time_total=0, time_gehrd=0, time_unghr=0, time_hseqr=0, time_trevc=0, time_sum=0;
    magma_tally2_flops_t flop_total=0, flop_gehrd=0, flop_unghr=0, flop_hseqr=0, flop_trevc=0, flop_sum=0;
    ...
    *info = 0;
    ...
    /* Compute workspace */
    nb = magma_tally2_get_sgehrd_nb( n );
    if (*info == 0) {
        minwrk = (2 +   nb)*n;
        optwrk = (2 + 2*nb)*n;
        work[0] = MAGMA_tally2_S_MAKE( (float) optwrk, 0. );
    ...
    float *dT;
    if (MAGMA_tally2_SUCCESS != magma_tally2_smalloc( &dT, nb*n )) {
    ...
    float *T;
    if (MAGMA_tally2_SUCCESS != magma_tally2_smalloc_cpu( &T, nb*n )) {
    ...
    if (wantvl) {
        ...
        lapackf77_slacpy( Magma_tally2LowerStr, &n, &n, A, &lda, VL, &ldvl );
        ...
        flops_start( flop_unghr );
            ...
            // Version 3 - LAPACK consistent MAGMA_tally2 HRD + T matrices stored
            magma_tally2_sorghr( n, ilo, ihi, VL, ldvl, &work[itau], dT, nb, &ierr );
        #elif defined(Version5)
            // Version 5 - Multi-GPU, T on host
            magma_tally2_sorghr_m( n, ilo, ihi, VL, ldvl, &work[itau], T, nb, &ierr );
        #endif
        ...
        #elif defined(Version3) || defined(Version4)
            // Version 3 - LAPACK consistent MAGMA_tally2 HRD + T matrices stored
            magma_tally2_sorghr( n, ilo, ihi, VR, ldvr, &work[itau], dT, nb, &ierr );
        #elif defined(Version5)
            // Version 5 - Multi-GPU, T on host
            magma_tally2_sorghr_m( n, ilo, ihi, VR, ldvr, &work[itau], T, nb, &ierr );
        #endif
        time_sum += timer_stop( time_unghr );
        flop_sum += flops_stop( flop_unghr );
    ...
    if (wantvl || wantvr) {
        ...
        liwrk = lwork - iwrk;
        #if TREVC_VERSION == 1
        lapackf77_strevc( lapack_side_const_tally2(side), "B", select, &n, A, &lda, VL, &ldvl,
                          VR, &ldvr, &n, &nout, &work[iwrk], &ierr );
        #elif TREVC_VERSION == 2
        lapackf77_strevc3( lapack_side_const_tally2(side), "B", select, &n, A, &lda, VL, &ldvl,
                           VR, &ldvr, &n, &nout, &work[iwrk], &liwrk, &ierr );
        #elif TREVC_VERSION == 3
        magma_tally2_strevc3( side, Magma_tally2BacktransVec, select, n, A, lda, VL, ldvl,
                       VR, ldvr, n, &nout, &work[iwrk], liwrk, &ierr );
        #elif TREVC_VERSION == 4
        magma_tally2_strevc3_mt( side, Magma_tally2BacktransVec, select, n, A, lda, VL, ldvl,
                          VR, ldvr, n, &nout, &work[iwrk], liwrk, &ierr );
        #elif TREVC_VERSION == 5
        magma_tally2_strevc3_mt_gpu( side, Magma_tally2BacktransVec, select, n, A, lda, VL, ldvl,
                              VR, ldvr, n, &nout, &work[iwrk], liwrk, &ierr );
        #else
    ...
    if (wantvl) {
        ...
        for (i = 0; i < n; ++i) {
            if ( wi[i] == 0. ) {
                scl = 1. / magma_tally2_cblas_snrm2( n, VL(0,i), 1 );
                blasf77_sscal( &n, &scl, VL(0,i), &ione );
            }
            else if ( wi[i] > 0. ) {
                d__1 = magma_tally2_cblas_snrm2( n, VL(0,i), 1 );
                d__2 = magma_tally2_cblas_snrm2( n, VL(0,i+1), 1 );
    ...
    if (wantvr) {
        ...
        for (i = 0; i < n; ++i) {
            if ( wi[i] == 0. ) {
                scl = 1. / magma_tally2_cblas_snrm2( n, VR(0,i), 1 );
                blasf77_sscal( &n, &scl, VR(0,i), &ione );
            }
            else if ( wi[i] > 0. ) {
                d__1 = magma_tally2_cblas_snrm2( n, VR(0,i), 1 );
                d__2 = magma_tally2_cblas_snrm2( n, VR(0,i+1), 1 );
                scl = 1. / lapackf77_slapy2( &d__1, &d__2 );
                blasf77_sscal( &n, &scl, VR(0,i), &ione );
