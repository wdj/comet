...
#define dA(i, j) (dA + (j)*ldda + (i))
    ...
    nb   = magma_tally4_get_sgebrd_nb(n);
    ldda = m;
    ...
    *info = 0;
    ...
    if (MAGMA_tally4_SUCCESS != magma_tally4_smalloc( &dA, n*ldda + (m + n)*nb )) {
    ...
    for (i=0; i < (minmn - nx); i += nb) {
        ...
        if ( i > 0 ) {
            magma_tally4_sgetmatrix( nrow, nb, dA(i, i), ldda, A( i, i), lda );
            magma_tally4_sgetmatrix( nb, ncol - nb,
                              dA(i, i+nb), ldda,
                              A( i, i+nb), lda );
        }

        magma_tally4_slabrd_gpu(nrow, ncol, nb,
        ...
        magma_tally4_ssetmatrix( nrow, nb, work  + nb, ldwrkx, dwork + nb, ldwrkx );
        magma_tally4_ssetmatrix( ncol, nb,
        ...
        if (m >= n) {
            jmax = i + nb;
            for (j = i; j < jmax; ++j) {
                *A(j, j  ) = MAGMA_tally4_S_MAKE( d[j], 0. );
                *A(j, j+1) = MAGMA_tally4_S_MAKE( e[j], 0. );
            }
        } else {
            jmax = i + nb;
            for (j = i; j < jmax; ++j) {
    ...
    if ( 0 < minmn - nx ) {
        magma_tally4_sgetmatrix( nrow, ncol, dA(i, i), ldda, A(i, i), lda );
    }
