...
int main( int argc, char** argv )
{
    ...
    magma_tally4FloatComplex *C, *R, *A, *work, *tau, *tauq, *taup;
    float *d, *e;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int itran = 0; itran < 2; ++itran ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            n = opts.nsize[itest];
            k = opts.ksize[itest];
            nb  = magma_tally4_get_cgebrd_nb( m );
            ldc = m;
            ...
            TESTING_MALLOC_CPU( A,    magma_tally4FloatComplex, lda*nn );
            TESTING_MALLOC_CPU( work, magma_tally4FloatComplex, lwork_max );
            TESTING_MALLOC_CPU( d,    float,             min(mm,nn) );
            TESTING_MALLOC_CPU( e,    float,             min(mm,nn) );
            TESTING_MALLOC_CPU( tauq, magma_tally4FloatComplex, min(mm,nn) );
            TESTING_MALLOC_CPU( taup, magma_tally4FloatComplex, min(mm,nn) );
            ...
            magma_tally4_cgebrd( mm, nn, A, lda, d, e, tauq, taup, work, lwork_max, &info );
            if (info != 0)
                printf("magma_tally4_cgebrd returned error %d: %s.\n",
            ...
            lapackf77_cunmbr( lapack_vect_const_tally4( vect[ivect] ),
            ...
            lwork = -1;
            magma_tally4_cunmbr( vect[ivect], side[iside], trans[itran],
                          m, n, k,
                          A, lda, tau, R, ldc, work, lwork, &info );
            if (info != 0)
                printf("magma_tally4_cunmbr (lwork query) returned error %d: %s.\n",
                       (int) info, magma_tally4_strerror( info ));
            lwork = (magma_tally4_int_t) MAGMA_tally4_C_REAL( work[0] );
            ...
            if (info != 0)
                printf("magma_tally4_cunmbr returned error %d: %s.\n",
                       (int) info, magma_tally4_strerror( info ));
