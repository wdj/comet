...
static void magma_minproduct_dsygst_m_1_L_col_update(magma_minproduct_int_t nk, magma_minproduct_int_t nb, double* dA_col, magma_minproduct_int_t ldda,
                                          double* dC1, magma_minproduct_int_t lddc1, double* dC2, magma_minproduct_int_t lddc2);

static void magma_minproduct_dsygst_m_1_U_row_update(magma_minproduct_int_t nk, magma_minproduct_int_t nb, double* dA_row, magma_minproduct_int_t ldda,
                                          double* dC1, magma_minproduct_int_t lddc1, double* dC2, magma_minproduct_int_t lddc2);
...
/**
    ...
    @ingroup magma_minproduct_dsyev_comp
    ********************************************************************/
...
#define dwork(gpui, i, j) (dw[gpui] + dima*ldda + lddbc*lddbr + (j)*nb*nb + (i)*nb)
    ...
    double    c_half     = MAGMA_minproduct_D_HALF;
    double    c_neg_half = MAGMA_minproduct_D_NEG_HALF;
    ...
    magma_minproduct_int_t nb = magma_minproduct_get_dsygst_nb_m(n);
    ...
    *info = 0;
    ...
    for (magma_minproduct_int_t igpu = 0; igpu < ngpu; ++igpu) {
        ...
        if (MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc( &dw[igpu], (dima*ldda + lddbc*lddbr + n*nb) )) {
    ...
    if (itype == 1) {
        if (upper) {
            ...
            for (magma_minproduct_int_t k = 0; k < nbl; ++k) {
                ...
                magma_minproduct_dsetmatrix_async(kb, n-k*nb,
                                       A(k, k),              lda,
            ...
            for (magma_minproduct_int_t k = 0; k < nbl; ++k) {
                ...
                if (k > 0) {
                    ...
                    magma_minproduct_dsyr2k(Magma_minproductUpper, Magma_minproductConjTrans, kb, nb,
                                 c_neg_one, dwork(igpu_p, 0, k), nb, dB_r(igpu_p, ind_k1, k), lddbr,
                                 d_one, dA(igpu_p, k/ngpu, k), ldda);

                    // copy Akk block on the CPU
                    magma_minproduct_dgetmatrix_async(kb, kb,
                                           dA(igpu_p, k/ngpu, k), ldda,
                                           A(k, k),                lda, stream[igpu_p][2] );

                    magma_minproduct_event_record( event[igpu_p][0], stream[igpu_p][2]);

                    magma_minproduct_dgemm(Magma_minproductConjTrans, Magma_minproductNoTrans, kb, nk-kb, nb, c_neg_one, dwork(igpu_p, 0, k), nb,
                                dB_r(igpu_p, ind_k1, k+1), lddbr, c_one, dA(igpu_p, k/ngpu, k+1), ldda );

                    magma_minproduct_dgemm(Magma_minproductConjTrans, Magma_minproductNoTrans, kb, nk-kb, nb, c_neg_one, dB_r(igpu_p, ind_k1, k), lddbr,
                    ...
                    for (magma_minproduct_int_t j=k+1; j < nbl; ++j) {
                        ...
                        if (igpu != igpu_p) {
                            ...
                            magma_minproduct_dsygst_m_1_U_row_update(n-j*nb, nb, dA(igpu, j/ngpu, j), ldda,
                ...
                if (k+1 < nbl) {
                    ...
                    magma_minproduct_dtrsm(Magma_minproductLeft, uplo, Magma_minproductConjTrans, Magma_minproductNonUnit,
                                kb, n-(k+1)*nb,
                ...
                if (k+1 < nbl) {
                    ...
                    magma_minproduct_dsymm(Magma_minproductLeft, uplo,
                                kb, n-(k+1)*nb,
                                c_neg_half, dA(igpu_p, k/ngpu, k), ldda,
                                dB_r(igpu_p, ind_k, k+1), lddbr,
                                c_one, dA(igpu_p, k/ngpu, k+1), ldda);

                    magma_minproduct_dgetmatrix_async(kb, n-(k+1)*nb,
                                           dA(igpu_p, k/ngpu, k+1), ldda,
                ...
                if (k > 0) {
                    ...
                    for (magma_minproduct_int_t j=k+ngpu; j < nbl; j += ngpu) {
                        ...
                        magma_minproduct_dsygst_m_1_U_row_update(n-j*nb, nb, dA(igpu_p, j/ngpu, j), ldda,
                ...
                if (k+1 < nbl) {
                    ...
                    for (magma_minproduct_int_t igpu = 0; igpu < ngpu; ++igpu) {
                        magma_minproduct_setdevice(igpu);
                        magma_minproduct_dsetmatrix_async(kb, n-(k+1)*nb,
                                               A(k, k+1),          lda,
            ...
            if (n > nb) {
                ...
                for (magma_minproduct_int_t j = 1; j < nbl; ++j) {
                    ...
                    for (magma_minproduct_int_t igpu = 0; igpu < ngpu; ++igpu) {
                        magma_minproduct_setdevice(igpu);
                        if (nloc[igpu] > 0) {
                            magma_minproduct_dsetmatrix_async(jb, n-j*nb,
                                                   B(j, j),            ldb,
                            ...
                            magma_minproduct_dtrsm(Magma_minproductRight, uplo, Magma_minproductNoTrans, Magma_minproductNonUnit, nloc[igpu], jb, c_one, dB_r(igpu, j%2, j), lddbr,
                                        dA(igpu, 0, j), ldda );

                            if ( j < nbl-1 ) {
                                magma_minproduct_dgemm(Magma_minproductNoTrans, Magma_minproductNoTrans, nloc[igpu], n-(j+1)*nb, nb, c_neg_one, dA(igpu, 0, j), ldda,
                    ...
                    for (magma_minproduct_int_t k = 0; k < j; ++k) {
                        ...
                        magma_minproduct_setdevice(igpu);
                        magma_minproduct_int_t kb = min(nb, n-k*nb);
                        magma_minproduct_dgetmatrix_async(kb, jb,
            ...
            for (magma_minproduct_int_t k = 0; k < nbl; ++k) {
                ...
                magma_minproduct_setdevice(igpu);
                magma_minproduct_int_t kb = min(nb, n-k*nb);
                magma_minproduct_dsetmatrix_async((n-k*nb), kb,
                                       A(k, k),              lda,
            ...
            for (magma_minproduct_int_t k = 0; k < nbl; ++k) {
                ...
                if (k > 0) {
                    ...
                    magma_minproduct_dsyr2k(Magma_minproductLower, Magma_minproductNoTrans, kb, nb,
                    ...
                    magma_minproduct_dgemm(Magma_minproductNoTrans, Magma_minproductConjTrans, nk-kb, kb, nb, c_neg_one, dwork(igpu_p, k+1, 0), n,
                                dB_c(igpu_p, k, ind_k1), lddbc, c_one, dA(igpu_p, k+1, k/ngpu), ldda );

                    magma_minproduct_dgemm(Magma_minproductNoTrans, Magma_minproductConjTrans, nk-kb, kb, nb, c_neg_one, dB_c(igpu_p, k+1, ind_k1), lddbc,
                    ...
                    for (magma_minproduct_int_t j=k+1; j < nbl; ++j) {
                        ...
                        if (igpu != igpu_p) {
                            ...
                            magma_minproduct_dsygst_m_1_L_col_update(n-j*nb, nb, dA(igpu, j, j/ngpu), ldda,
                ...
                if (k+1 < nbl) {
                    ...
                    magma_minproduct_dtrsm(Magma_minproductRight, uplo, Magma_minproductConjTrans, Magma_minproductNonUnit,
                                n-(k+1)*nb, kb,
                ...
                if (k+1 < nbl) {
                    ...
                    magma_minproduct_dsymm(Magma_minproductRight, uplo,
                                n-(k+1)*nb, kb,
                                c_neg_half, dA(igpu_p, k, k/ngpu), ldda,
                                dB_c(igpu_p, k+1, ind_k), lddbc,
                                c_one, dA(igpu_p, k+1, k/ngpu), ldda);

                    magma_minproduct_dgetmatrix_async(n-(k+1)*nb, kb,
                                           dA(igpu_p, k+1, k/ngpu), ldda,
                ...
                if (k > 0) {
                    ...
                    for (magma_minproduct_int_t j=k+ngpu; j < nbl; j += ngpu) {
                        ...
                        magma_minproduct_dsygst_m_1_L_col_update(n-j*nb, nb, dA(igpu_p, j, j/ngpu), ldda,
                ...
                if (k+1 < nbl) {
                    ...
                    for (magma_minproduct_int_t igpu = 0; igpu < ngpu; ++igpu) {
                        magma_minproduct_setdevice(igpu);
                        magma_minproduct_dsetmatrix_async((n-(k+1)*nb), kb,
                                               A(k+1, k),          lda,
            ...
            if (n > nb) {
                ...
                for (magma_minproduct_int_t j = 1; j < nbl; ++j) {
                    ...
                    for (magma_minproduct_int_t igpu = 0; igpu < ngpu; ++igpu) {
                        magma_minproduct_setdevice(igpu);
                        if (nloc[igpu] > 0) {
                            magma_minproduct_dsetmatrix_async((n-j*nb), jb,
                                                   B(j, j),            ldb,
                            ...
                            magma_minproductblasSetKernelStream(stream[igpu][j%2]);
                            magma_minproduct_dtrsm(Magma_minproductLeft, uplo, Magma_minproductNoTrans, Magma_minproductNonUnit, jb, nloc[igpu], c_one, dB_c(igpu, j, j%2), lddbc,
                                        dA(igpu, j, 0), ldda );

                            if ( j < nbl-1 ) {
                                magma_minproduct_dgemm(Magma_minproductNoTrans, Magma_minproductNoTrans, n-(j+1)*nb, nloc[igpu], nb, c_neg_one, dB_c(igpu, j+1, j%2), lddbc,
                    ...
                    for (magma_minproduct_int_t k = 0; k < j; ++k) {
                        ...
                        magma_minproduct_setdevice(igpu);
                        magma_minproduct_int_t kb = min(nb, n-k*nb);
                        magma_minproduct_dgetmatrix_async(jb, kb,
        ...
        if (upper) {
            ...
            if (n > nb) {
                ...
                for (magma_minproduct_int_t k = 0; k < nbl; ++k) {
                    ...
                    for (magma_minproduct_int_t j = k; j < nbl; ++j) {
                        ...
                        magma_minproduct_setdevice(igpu);
                        magma_minproduct_int_t jb = min(nb, n-j*nb);
                        magma_minproduct_dsetmatrix_async(kb, jb,
                                               A(k, j),              lda,
                    ...
                    for (magma_minproduct_int_t igpu = 0; igpu < ngpu; ++igpu) {
                        magma_minproduct_setdevice(igpu);
                        magma_minproduct_dsetmatrix_async(k*nb + kb, kb,
                                               B(0, k),            ldb,
                        ...
                        magma_minproduct_dgemm(Magma_minproductNoTrans, Magma_minproductNoTrans, k*nb, nloc[igpu], kb, c_one, dB_c(igpu, 0, k%2), lddbc,
                                    dA(igpu, k, iloc[igpu]), ldda, c_one, dA(igpu, 0, iloc[igpu]), ldda );

                        magma_minproduct_dtrmm(Magma_minproductLeft, uplo, Magma_minproductNoTrans, Magma_minproductNonUnit, kb, nloc[igpu], c_one, dB_c(igpu, k, k%2), lddbc,
                                    dA(igpu, k, iloc[igpu]), ldda );

                        magma_minproduct_event_record( event[igpu][0], stream[igpu][k%2]);

                        if (igpu == igpu_p) {
                            magma_minproduct_dgetmatrix_async(k*nb, kb,
            ...
            for (magma_minproduct_int_t k = 0; k < nbl; ++k) {
                ...
                if (k > 0) {
                    for (magma_minproduct_int_t igpu = 0; igpu < ngpu; ++igpu) {
                        ...
                        magma_minproduct_dsetmatrix_async(k*nb, kb,
                                               A(0, k),           lda,
                    ...
                    magma_minproduct_dtrmm(Magma_minproductRight, uplo, Magma_minproductConjTrans, Magma_minproductNonUnit,
                                k*nb, kb,
                                c_one, dB_c(igpu_p, k, ind_k), lddbc,
                    ...
                    for (magma_minproduct_int_t j = 0; j < k; ++j) {
                        ...
                        magma_minproduct_dsyr2k(uplo, Magma_minproductNoTrans,
                                     nb, kb,
                                     c_one, dwork(igpu, j, 0), n,
                                     dB_c(igpu, j, ind_k), lddbc,
                                     d_one, dA(igpu, j, j/ngpu), ldda);

                        magma_minproduct_dgemm(Magma_minproductNoTrans, Magma_minproductConjTrans, j*nb, nb, kb, c_one, dB_c(igpu, 0, ind_k), lddbc,
                                    dwork(igpu, j, 0), n, c_one, dA(igpu, 0, j/ngpu), ldda );

                        magma_minproduct_dgemm(Magma_minproductNoTrans, Magma_minproductConjTrans, j*nb, nb, kb, c_one, dwork(igpu, 0, 0), n,
                ...
                for (magma_minproduct_int_t igpu = 0; igpu < ngpu; ++igpu) {
                    ...
                    magma_minproduct_queue_sync(stream[igpu][0]); // sync B copy (conflicts with dsygst)
                }
            ...
            for (magma_minproduct_int_t j = 0; j < nbl; ++j) {
                ...
                magma_minproduct_setdevice(igpu);
                magma_minproduct_int_t jb = min(nb, n-j*nb);
                magma_minproduct_dgetmatrix_async(j*nb+jb, jb,
            ...
            if (n > nb) {
                ...
                for (magma_minproduct_int_t k = 0; k < nbl; ++k) {
                    ...
                    for (magma_minproduct_int_t j = k; j < nbl; ++j) {
                        ...
                        magma_minproduct_dsetmatrix_async(jb, kb,
                                               A(j, k),              lda,
                    ...
                    for (magma_minproduct_int_t igpu = 0; igpu < ngpu; ++igpu) {
                        magma_minproduct_setdevice(igpu);
                        magma_minproduct_dsetmatrix_async(kb, k*nb +kb,
                                               B(k, 0),            ldb,
                        ...
                        magma_minproduct_dgemm(Magma_minproductNoTrans, Magma_minproductNoTrans, nloc[igpu], k*nb, kb, c_one, dA(igpu, iloc[igpu], k), ldda,
                                    dB_r(igpu, k%2, 0), lddbr, c_one, dA(igpu, iloc[igpu], 0), ldda );

                        magma_minproduct_dtrmm(Magma_minproductRight, uplo, Magma_minproductNoTrans, Magma_minproductNonUnit, nloc[igpu], kb, c_one, dB_r(igpu, k%2, k), lddbr,
                                    dA(igpu, iloc[igpu], k), ldda );

                        magma_minproduct_event_record( event[igpu][0], stream[igpu][k%2]);

                        if (igpu == igpu_p) {
                            magma_minproduct_dgetmatrix_async(kb, k*nb,
            ...
            for (magma_minproduct_int_t k = 0; k < nbl; ++k) {
                ...
                if (k > 0) {
                    for (magma_minproduct_int_t igpu = 0; igpu < ngpu; ++igpu) {
                        ...
                        magma_minproduct_dsetmatrix_async(kb, k*nb,
                                               A(k, 0),           lda,
                    ...
                    magma_minproduct_dtrmm(Magma_minproductLeft, uplo, Magma_minproductConjTrans, Magma_minproductNonUnit,
                                kb, k*nb,
                                c_one, dB_r(igpu_p, ind_k, k), lddbr,
                    ...
                    for (magma_minproduct_int_t j = 0; j < k; ++j) {
                        ...
                        magma_minproduct_dsyr2k(uplo, Magma_minproductConjTrans,
                                     nb, kb,
                                     c_one, dwork(igpu, 0, j), nb,
                                     dB_r(igpu, ind_k, j), lddbr,
                                     d_one, dA(igpu, j/ngpu, j), ldda);

                        magma_minproduct_dgemm(Magma_minproductConjTrans, Magma_minproductNoTrans, nb, j*nb, kb, c_one, dwork(igpu, 0, j), nb,
                                    dB_r(igpu, ind_k, 0), lddbr, c_one, dA(igpu, j/ngpu, 0), ldda );

                        magma_minproduct_dgemm(Magma_minproductConjTrans, Magma_minproductNoTrans, nb, j*nb, kb, c_one, dB_r(igpu, ind_k, j), lddbr,
            ...
            for (magma_minproduct_int_t j = 0; j < nbl; ++j) {
                ...
                magma_minproduct_setdevice(igpu);
                magma_minproduct_int_t jb = min(nb, n-j*nb);
                magma_minproduct_dgetmatrix_async(jb, j*nb+jb,
...
inline static void magma_minproduct_dsygst_m_1_U_row_update(
    magma_minproduct_int_t nk, magma_minproduct_int_t nb,
    double* dA_row, magma_minproduct_int_t ldda,
    double* dC1, magma_minproduct_int_t lddc1,
    double* dC2, magma_minproduct_int_t lddc2)
{
    ...
    magma_minproduct_dgemm(Magma_minproductConjTrans, Magma_minproductNoTrans, kb, nk-kb, nb, c_neg_one, dC1, lddc1,
                dC2+kb*lddc2, lddc2, c_one, dA_row+kb*ldda, ldda );

    magma_minproduct_dgemm(Magma_minproductConjTrans, Magma_minproductNoTrans, kb, nk-kb, nb, c_neg_one, dC2, lddc2,
                dC1+kb*lddc1, lddc1, c_one, dA_row+kb*ldda, ldda );
}


inline static void magma_minproduct_dsygst_m_1_L_col_update(
    magma_minproduct_int_t nk, magma_minproduct_int_t nb,
    double* dA_col, magma_minproduct_int_t ldda,
    double* dC1, magma_minproduct_int_t lddc1,
    double* dC2, magma_minproduct_int_t lddc2)
{
    ...
    magma_minproduct_dgemm(Magma_minproductNoTrans, Magma_minproductConjTrans, nk-kb, kb, nb, c_neg_one, dC1+kb, lddc1,
                dC2, lddc2, c_one, dA_col+kb, ldda );

    magma_minproduct_dgemm(Magma_minproductNoTrans, Magma_minproductConjTrans, nk-kb, kb, nb, c_neg_one, dC2+kb, lddc2,
