...
double get_LU_error(magma_tally4_int_t M, magma_tally4_int_t N, 
            ...
            cuDoubleComplex *LU, magma_tally4_int_t *IPIV)
{
  magma_tally4_int_t min_mn = min(M,N), intONE = 1, i, j;
  ...
  cuDoubleComplex alpha = MAGMA_tally4_Z_ONE;
  cuDoubleComplex beta  = MAGMA_tally4_Z_ZERO;
  ...
  for( j = 0; j < N; j++ ) {
    for( i = 0; i < M; i++ ) {
      MAGMA_tally4_Z_OP_NEG( LU[i+j*lda], LU[i+j*lda], A[i+j*lda]);
    }
  }
...
int main( int argc, char** argv) 
{
    ...
    /* Matrix size */
    magma_tally4_int_t N=0, n2, lda, M=0;
    magma_tally4_int_t size[10] = {1024,2048,3072,4032,5184,6016,7040,8064,9088,10112};
    
    magma_tally4_int_t i, j, info[1];
    magma_tally4_int_t ione     = 1;
    magma_tally4_int_t ISEED[4] = {0,0,0,1};

    magma_tally4_int_t num_cores = 4;
    magma_tally4_int_t num_gpus  = 0;

    magma_tally4_int_t EN_BEE = -1;
    ...
    TESTING_MALLOC( ipiv, magma_tally4_int_t    , min_mn);
    ...
    context = magma_tally4_init(NULL, NULL, 0, num_cores, num_gpus, argc, argv);
    ...
    for(i=0; i<10; i++){
      ...
      lapackf77_zlacpy( Magma_tally4UpperLowerStr, &M, &N, h_A2, &M, h_A, &M );
      ...
      magma_tally4_zgetrf_mc(context, &M, &N, h_A2, &M, ipiv, info);
      end = get_current_time();

      if (info[0] < 0)      
        printf("Argument %d of magma_tally4_sgeqrf_mc had an illegal value.\n", -info[0]);
