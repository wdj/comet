    ...
    magma_tally3_queue_t queue )
{
    ...
    magma_tally3_d_matrix hA={Magma_tally3_CSR}, hACSR={Magma_tally3_CSR};
    magma_tally3_d_matrix hL={Magma_tally3_CSR}, hU={Magma_tally3_CSR};
    CHECK( magma_tally3_dmtransfer( A, &hA, A.memory_location, Magma_tally3_CPU, queue ));
    CHECK( magma_tally3_dmconvert( hA, &hACSR, hA.storage_type, Magma_tally3_CSR, queue ));

        // in case using fill-in
    if( precond->levels > 0 ){
        magma_tally3_d_matrix hAL={Magma_tally3_CSR}, hAUt={Magma_tally3_CSR};
        CHECK( magma_tally3_dsymbilu( &hACSR, precond->levels, &hAL, &hAUt,  queue ));
        magma_tally3_dmfree(&hAL, queue);
        magma_tally3_dmfree(&hAUt, queue);
    }

    CHECK( magma_tally3_dmtransfer(hACSR, &(precond->M), Magma_tally3_CPU, Magma_tally3_DEV, queue ));

    magma_tally3_dmfree( &hA, queue );
    magma_tally3_dmfree( &hACSR, queue );
    ...
    CHECK( magma_tally3_dmtransfer( precond->M, &hA, Magma_tally3_DEV, Magma_tally3_CPU, queue ));

    hL.diagorder_type = Magma_tally3_UNITY;
    CHECK( magma_tally3_dmconvert( hA, &hL , Magma_tally3_CSR, Magma_tally3_CSRL, queue ));
    hU.diagorder_type = Magma_tally3_VALUE;
    CHECK( magma_tally3_dmconvert( hA, &hU , Magma_tally3_CSR, Magma_tally3_CSRU, queue ));
    CHECK( magma_tally3_dmtransfer( hL, &(precond->L), Magma_tally3_CPU, Magma_tally3_DEV, queue ));
    CHECK( magma_tally3_dmtransfer( hU, &(precond->U), Magma_tally3_CPU, Magma_tally3_DEV, queue ));
    ...
    magma_tally3_queue_t queue )
{
    ...
    magma_tally3_d_matrix hA={Magma_tally3_CSR}, hL={Magma_tally3_CSR}, hU={Magma_tally3_CSR};
    
    if (precond->L.memory_location != Magma_tally3_DEV ){
        
        CHECK( magma_tally3_dmtransfer( precond->M, &hA,
    ...
    if( precond->maxiter < 50 ){
        ...
        // extract the diagonal of L into precond->d
        CHECK( magma_tally3_djacobisetup_diagscal( precond->L, &precond->d, queue ));
        CHECK( magma_tally3_dvinit( &precond->work1, Magma_tally3_DEV, precond->U.num_rows, 1, MAGMA_tally3_D_ZERO, queue ));
        
        // extract the diagonal of U into precond->d2
        CHECK( magma_tally3_djacobisetup_diagscal( precond->U, &precond->d2, queue ));
        CHECK( magma_tally3_dvinit( &precond->work2, Magma_tally3_DEV, precond->U.num_rows, 1, MAGMA_tally3_D_ZERO, queue ));
    }
    ...
    magma_tally3_queue_t queue )
{
    ...
    magma_tally3_d_matrix hA={Magma_tally3_CSR}, hACSR={Magma_tally3_CSR}, U={Magma_tally3_CSR};
    CHECK( magma_tally3_dmtransfer( A, &hA, A.memory_location, Magma_tally3_CPU, queue ));
    ...
    CHECK( magma_tally3_dmconvert( hACSR, &U, Magma_tally3_CSR, Magma_tally3_CSRL, queue ));
    magma_tally3_dmfree( &hACSR, queue );
    CHECK( magma_tally3_dmtransfer(U, &(precond->M), Magma_tally3_CPU, Magma_tally3_DEV, queue ));
...
/*
    ...
    magma_tally3_d_matrix hD, hR, hAt

    CHECK( magma_tally3_dcsrsplit( 256, hA, &hD, &hR, queue ));

    CHECK( magma_tally3_dmtransfer( hD, &precond->LD, Magma_tally3_CPU, Magma_tally3_DEV, queue ));
    CHECK( magma_tally3_dmtransfer( hR, &precond->L, Magma_tally3_CPU, Magma_tally3_DEV, queue ));

    magma_tally3_dmfree(&hD, queue );
    magma_tally3_dmfree(&hR, queue );

    CHECK( magma_tally3_d_cucsrtranspose(   hA, &hAt, queue ));

    CHECK( magma_tally3_dcsrsplit( 256, hAt, &hD, &hR, queue ));

    CHECK( magma_tally3_dmtransfer( hD, &precond->UD, Magma_tally3_CPU, Magma_tally3_DEV, queue ));
    CHECK( magma_tally3_dmtransfer( hR, &precond->U, Magma_tally3_CPU, Magma_tally3_DEV, queue ));
    
    magma_tally3_dmfree(&hD, queue );
    magma_tally3_dmfree(&hR, queue );
    magma_tally3_dmfree(&hA, queue );
    magma_tally3_dmfree(&hAt, queue );
*/
