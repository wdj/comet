...
__global__
void magma_minproduct_zlarfg_gpu_kernel( int n, magma_minproductDoubleComplex* dx0, magma_minproductDoubleComplex* dx,
...
#else
    double alphar = MAGMA_minproduct_Z_REAL(alpha);
    double alphai = MAGMA_minproduct_Z_IMAG(alpha);
    if ( xnorm != 0 || alphai != 0) {
        if (i == 0) {
    ...
    magma_minproductDoubleComplex_ptr dAkk)
{
    ...
    //magma_minproductblas_dznrm2_cols(n, 1, dx0, n, dxnorm);
    magma_minproductblas_dznrm2_cols(n-1, 1, dx0+1, n, dxnorm);

    magma_minproduct_zlarfg_gpu_kernel<<< blocks, threads,
