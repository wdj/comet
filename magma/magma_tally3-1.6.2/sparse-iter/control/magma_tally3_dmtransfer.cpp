/*
    -- MAGMA_tally3 (version 1.6.2) --
       ...
       @generated from magma_tally3_zmtransfer.cpp normal z -> d, Sun May  3 11:23:01 2015
       @author Hartwig Anzt
*/
    ...
    magma_tally3_queue_t queue )
{
    ...
    if ( src == Magma_tally3_CPU && dst == Magma_tally3_DEV ) {
        ...
        if ( A.storage_type == Magma_tally3_CSR || A.storage_type == Magma_tally3_CSC
            ...
            // data transfer
            magma_tally3_dsetvector( A.nnz, A.val, 1, B->dval, 1 );
        ...
        if ( A.storage_type == Magma_tally3_ELLPACKT || A.storage_type == Magma_tally3_ELL ) {
            ...
            B->diameter = A.diameter;
            // memory allocation
            CHECK( magma_tally3_dmalloc( &B->dval, A.num_rows * A.max_nnz_row ));
            CHECK( magma_tally3_index_malloc( &B->dcol, A.num_rows * A.max_nnz_row ));
            // data transfer
            magma_tally3_dsetvector( A.num_rows * A.max_nnz_row, A.val, 1, B->dval, 1 );
        ...
        if ( A.storage_type == Magma_tally3_ELLRT ) {
            ...
            // memory allocation
            CHECK( magma_tally3_dmalloc( &B->dval, A.num_rows * rowlength ));
            CHECK( magma_tally3_index_malloc( &B->dcol, A.num_rows * rowlength ));
            CHECK( magma_tally3_index_malloc( &B->drow, A.num_rows ));
            // data transfer
            magma_tally3_dsetvector( A.num_rows * rowlength, A.val, 1, B->dval, 1 );
        ...
        if ( A.storage_type == Magma_tally3_BCSR ) {
            ...
            // memory allocation
            CHECK( magma_tally3_dmalloc( &B->dval, size_b*size_b*A.numblocks ));
            CHECK( magma_tally3_index_malloc( &B->drow, r_blocks + 1 ));
            CHECK( magma_tally3_index_malloc( &B->dcol, A.numblocks ));
            // data transfer
            magma_tally3_dsetvector( size_b*size_b*A.numblocks, A.val, 1, B->dval, 1 );
        ...
        if ( A.storage_type == Magma_tally3_DENSE ) {
            ...
            // data transfer
            magma_tally3_dsetvector( A.num_rows*A.num_cols, A.val, 1, B->dval, 1 );
        }
    }
    ...
    if ( src == Magma_tally3_CPU && dst == Magma_tally3_CPU ) {
        ...
        if ( A.storage_type == Magma_tally3_ELLPACKT || A.storage_type == Magma_tally3_ELL ) {
            ...
            B->diameter = A.diameter;
            // memory allocation
            CHECK( magma_tally3_dmalloc_cpu( &B->val, A.num_rows*A.max_nnz_row ));
    ...
    if ( src == Magma_tally3_DEV && dst == Magma_tally3_CPU ) {
        ...
        if ( A.storage_type == Magma_tally3_CSR || A.storage_type == Magma_tally3_CSC
            ...
            CHECK( magma_tally3_index_malloc_cpu( &B->col, A.nnz ));
            // data transfer
            magma_tally3_dgetvector( A.nnz, A.dval, 1, B->val, 1 );
        ...
        if ( A.storage_type == Magma_tally3_ELL || A.storage_type == Magma_tally3_ELLPACKT ) {
            ...
            CHECK( magma_tally3_index_malloc_cpu( &B->col, A.num_rows*A.max_nnz_row ));
            // data transfer
            magma_tally3_dgetvector( A.num_rows*A.max_nnz_row, A.dval, 1, B->val, 1 );
        ...
        if ( A.storage_type == Magma_tally3_ELLRT ) {
            ...
            CHECK( magma_tally3_index_malloc_cpu( &B->col, rowlength*A.num_rows ));
            // data transfer
            magma_tally3_dgetvector( A.num_rows*rowlength, A.dval, 1, B->val, 1 );
        ...
        if ( A.storage_type == Magma_tally3_BCSR ) {
            ...
            magma_tally3_int_t r_blocks = ceil( (float)A.num_rows / (float)size_b );
                    // max number of blocks per column
            // memory allocation
            CHECK( magma_tally3_dmalloc_cpu( &B->val, A.numblocks*A.blocksize*A.blocksize ));
            CHECK( magma_tally3_index_malloc_cpu( &B->row, r_blocks+1 ));
            CHECK( magma_tally3_index_malloc_cpu( &B->col, A.numblocks ));
            // data transfer
            magma_tally3_dgetvector( A.numblocks * A.blocksize * A.blocksize, A.dval, 1, B->val, 1 );
        ...
        if ( A.storage_type == Magma_tally3_DENSE ) {
            ...
            CHECK( magma_tally3_dmalloc_cpu( &B->val, A.num_rows*A.num_cols ));
            // data transfer
            magma_tally3_dgetvector( A.num_rows*A.num_cols, A.dval, 1, B->val, 1 );
        }
    }
    ...
    if ( src == Magma_tally3_DEV && dst == Magma_tally3_DEV ) {
        ...
        if ( A.storage_type == Magma_tally3_ELL || A.storage_type == Magma_tally3_ELLPACKT ) {
            ...
            CHECK( magma_tally3_index_malloc( &B->dcol, A.num_rows * A.max_nnz_row ));
            // data transfer
            magma_tally3_dcopyvector( A.num_rows*A.max_nnz_row, A.dval, 1, B->dval, 1 );
        ...
        if ( A.storage_type == Magma_tally3_ELLRT ) {
            ...
            CHECK( magma_tally3_index_malloc( &B->drow, A.num_rows ));
            // data transfer
            magma_tally3_dcopyvector( A.num_rows * rowlength, A.dval, 1, B->dval, 1 );
        ...
        if ( A.storage_type == Magma_tally3_BCSR ) {
            ...
            CHECK( magma_tally3_index_malloc( &B->dcol, A.numblocks ));
            // data transfer
            magma_tally3_dcopyvector( size_b*size_b*A.numblocks, A.dval, 1, B->dval, 1 );
        ...
        if ( A.storage_type == Magma_tally3_DENSE ) {
            ...
            CHECK( magma_tally3_dmalloc( &B->dval, A.num_rows*A.num_cols ));
            // data transfer
            magma_tally3_dcopyvector( A.num_rows*A.num_cols, A.dval, 1, B->dval, 1 );
        }
    }
