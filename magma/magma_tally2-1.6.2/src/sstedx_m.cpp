...
#define Z(i_,j_) (Z + (i_) + (j_)*ldz)
    ...
    magma_tally2_int_t alleig, indeig, valeig, lquery;
    magma_tally2_int_t i, j, k, m;
    magma_tally2_int_t liwmin, lwmin;
    magma_tally2_int_t start, end, smlsiz;
        ...
        if (alleig) {
            ...
            while ( start < n ) {
                ...
                for (end = start+1; end < n; ++end) {
                    tiny = eps * sqrt( MAGMA_tally2_S_ABS(d[end-1]*d[end]));
                    if (MAGMA_tally2_S_ABS(e[end-1]) <= tiny)
                        break;
                }
                ...
                if (m > smlsiz) {
                    ...
                    magma_tally2_int_t mm = m-1;
                    lapackf77_slascl("G", &izero, &izero, &orgnrm, &d_one, &mm, &ione, &e[start], &mm, info);

                    magma_tally2_slaex0_m( ngpu, m, &d[start], &e[start], Z(start, start), ldz, work, iwork, Magma_tally2RangeAll, vl, vu, il, iu, info);
            ...
            magma_tally2_int_t nm = n-1;
            lapackf77_slascl("G", &izero, &izero, &orgnrm, &d_one, &nm, &ione, e, &nm, info);

            magma_tally2_slaex0_m(ngpu, n, d, e, Z, ldz, work, iwork, range, vl, vu, il, iu, info);
