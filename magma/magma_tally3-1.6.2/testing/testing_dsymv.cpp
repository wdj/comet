...
int main(int argc, char **argv)
{
    ...
    double          magma_tally3_error, atomics_error, cublas_error, work[1];
    ...
    double *A, *X, *Y, *Yatomics, *Ycublas, *Ymagma_tally3;
    magma_tally3Double_ptr dA, dX, dY, dwork;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_tally3blas_dlaset( Magma_tally3Full, ldwork, 1, MAGMA_tally3_D_NAN, MAGMA_tally3_D_NAN, dwork, ldwork );
            magma_tally3blas_dlaset( Magma_tally3Full, ldda,   N, MAGMA_tally3_D_NAN, MAGMA_tally3_D_NAN, dA,    ldda   );
            
            /* Initialize the matrix */
            lapackf77_dlarnv( &ione, ISEED, &sizeA, A );
            magma_tally3_dmake_symmetric( N, A, lda );
            ...
            if ( opts.uplo == Magma_tally3Upper ) {
                lapackf77_dlaset( "Lower", &N1, &N1, &MAGMA_tally3_D_NAN, &MAGMA_tally3_D_NAN, &A[1], &lda );
            }
            else {
                lapackf77_dlaset( "Upper", &N1, &N1, &MAGMA_tally3_D_NAN, &MAGMA_tally3_D_NAN, &A[lda], &lda );
            }
            ...
            magma_tally3_dsetvector( N, X, incx, dX, incx );
            magma_tally3_dsetvector( N, Y, incy, dY, incy );
            
            cublas_time = magma_tally3_sync_wtime( 0 );
            cublasDsymv( opts.handle, cublas_uplo_const_tally3(opts.uplo),
                         N, &alpha, dA, ldda, dX, incx, &beta, dY, incy );
            cublas_time = magma_tally3_sync_wtime( 0 ) - cublas_time;
            cublas_perf = gflops / cublas_time;
            
            magma_tally3_dgetvector( N, dY, incy, Ycublas, incy );
            ...
            magma_tally3_dgetvector( N, dY, incy, Yatomics, incy );
            ...
            if ( opts.version == 1 ) {
                magma_tally3blas_dsymv_work( opts.uplo, N, alpha, dA, ldda, dX, incx, beta, dY, incy, dwork, ldwork, opts.queue );
            }
            else {
                // non-work interface (has added overhead)
                magma_tally3blas_dsymv( opts.uplo, N, alpha, dA, ldda, dX, incx, beta, dY, incy );
            }
            magma_tally3_time = magma_tally3_sync_wtime( 0 ) - magma_tally3_time;
            magma_tally3_perf = gflops / magma_tally3_time;
            
            magma_tally3_dgetvector( N, dY, incy, Ymagma_tally3, incy );
            
            /* =====================================================================
               Performs operation using CPU BLAS
               =================================================================== */
            cpu_time = magma_tally3_wtime();
            blasf77_dsymv( lapack_uplo_const_tally3(opts.uplo), &N, &alpha, A, &lda, X, &incx, &beta, Y, &incy );
            ...
            /* =====================================================================
               Check the result
               =================================================================== */
            blasf77_daxpy( &N, &c_neg_one, Y, &incy, Ymagma_tally3, &incy );
            magma_tally3_error = lapackf77_dlange( "M", &N, &ione, Ymagma_tally3, &N, work ) / N;
