/*
    -- MAGMA_minproduct (version 1.6.1) --
       ...
       @author Mark Gates
       @generated from magma_minproduct_znan_inf.cpp normal z -> d, Fri Jan 30 19:00:20 2015
...
const double MAGMA_minproduct_D_NAN
    = MAGMA_minproduct_D_MAKE( std::numeric_limits<double>::quiet_NaN(),
                    std::numeric_limits<double>::quiet_NaN() );

const double MAGMA_minproduct_D_INF
    = MAGMA_minproduct_D_MAKE( std::numeric_limits<double>::infinity(),
                    std::numeric_limits<double>::infinity() );

/** @return true if either real(x) or imag(x) is NAN. */
inline bool magma_minproduct_d_isnan( double x )
{
#ifdef COMPLEX
    return isnan( MAGMA_minproduct_D_REAL( x )) ||
           isnan( MAGMA_minproduct_D_IMAG( x ));
#else
    return isnan( x );
#endif
}
...
inline bool magma_minproduct_d_isinf( double x )
{
#ifdef COMPLEX
    return isinf( MAGMA_minproduct_D_REAL( x )) ||
           isinf( MAGMA_minproduct_D_IMAG( x ));
#else
    return isinf( x );
#endif
}
...
/**
    ...
    magma_minproduct_dnan_inf checks a matrix that is located on the CPU host
    ...
    ********************************************************************/
extern "C"
magma_minproduct_int_t magma_minproduct_dnan_inf(
    ...
    if ( uplo == Magma_minproductLower ) {
        for( int j = 0; j < n; ++j ) {
            for( int i = j; i < m; ++i ) {  // i >= j
                if      ( magma_minproduct_d_isnan( *A(i,j) )) { c_nan++; }
                else if ( magma_minproduct_d_isinf( *A(i,j) )) { c_inf++; }
            }
        }
    }
...
/**
    ...
    ********************************************************************/
extern "C"
magma_minproduct_int_t magma_minproduct_dnan_inf_gpu(
    magma_minproduct_uplo_t uplo, magma_minproduct_int_t m, magma_minproduct_int_t n,
    ...
    magma_minproduct_int_t *cnt_inf )
{
    ...
    magma_minproduct_int_t cnt = magma_minproduct_dnan_inf( uplo, m, n, A, lda, cnt_nan, cnt_inf );
