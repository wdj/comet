...
magma_tally3_queue_t compute_stream[Magma_tally3MaxGPUs];
...
//    printf("doing dmalloc\n");
    ...
    magma_tally3_setdevice(deviceID);
...
//    printf("doing dmalloc %p\n",dep_ptr);
    ...
    //A[index]=NULL;
    magma_tally3_free_pinned(A[index]);
#if (dbglevel >=1)
ca_trace_end_gpu('0');
ca_trace_end_cpu('C');
#endif
...
//    double *tmpdA;
    ...
    magma_tally3_queue_sync(stream1);
...
void magma_tally3_task_dev_dsetmatrix(Schedule* sched_obj )
 {
    ...
    double *dA_dst;
    magma_tally3_int_t dA_LD;

    schedule_unpack_args_7(sched_obj, deviceID, m,  nb, A_src,  LDA, dA_dst,  dA_LD);

    magma_tally3_setdevice(deviceID);
    magma_tally3_dsetmatrix(m, nb, A_src, LDA, dA_dst, dA_LD);
 ...
 void magma_tally3_task_dev_dgetmatrix(Schedule* sched_obj )               
 {
    ...
    magma_tally3_setdevice(deviceID);
    magma_tally3_dgetmatrix( m,  nb, dA_src,  dA_LD, A_dst,  LDA);
...
//    double *tmpdA;
    ...
    magma_tally3_setdevice(deviceID);
    //magma_tally3_dsetmatrix_transpose( m,  nb, A_src,  LDA, dA_dst,  dA_LD);
      //task_setpanel(A_m, nb, A(0,K), A_LD, dA(0,K), dA_LD)
...
//            pthread_mutex_lock(&mutex_dAP);
    ...
    magma_tally3_dsetmatrix(m, nb, A_src, LDA, dwork, dwork_LD);
#if (dbglevel==10)
//            ca_dbg_printMat_gpu(m, nb, dwork, dwork_LD, "dwork after setMatrix");
#endif
    /*2.transpose dwork to dA*/
    magma_tally3blas_dtranspose2(dA_dst, dA_LD, dwork, dwork_LD, m, nb);
...
//    double *tmpdA;
    ...
    magma_tally3_queue_sync(compute_stream[deviceID]);
...
//            pthread_mutex_lock(&mutex_dAP);
    /*1. copy A to dwork: send to the GPU*/
    magma_tally3_dsetmatrix_async(m, nb, A_src, LDA, dwork, dwork_LD, stream1);
#if (dbglevel==10)
    magma_tally3_queue_sync(stream1);
    ca_dbg_printMat_gpu(m, nb, dwork, dwork_LD, "dwork after setMatrix");
#endif

    magma_tally3_queue_sync(stream1); //make sure dwork is set before the transpose

    /*2.transpose dwork to dA*/
    magma_tally3blas_dtranspose2s(dA_dst, dA_LD, dwork, dwork_LD, m, nb, compute_stream[deviceID]);
//    pthread_mutex_unlock(&mutex_dAP);
#if (dbglevel==10)
    ca_dbg_printMat_transpose_gpu(nb, m, dA_dst, dA_LD, "dA after setMatrix");
#endif
...
//    printf("Matrix_get_transpose m:%d, nb:%d, dep_ptr:%p\n",m,nb,dep_ptr);
    ...
    //magma_tally3_dgetmatrix_transpose( m,  nb, dA_src,  dA_LD, A_dst,  LDA);
     // task_getpanel(gpu_nrows, ncols, dAT(K,K+A_N), dAT_LD, A(K,K+A_N), A_LD);
#if (dbglevel==10)
...
//            pthread_mutex_lock(&mutex_dAP);
    ...
    magma_tally3blas_dtranspose2(dwork, dwork_LD, dA_src, dA_LD,  nb, m);
...
//            ca_dbg_printMat_gpu(m, nb, dwork, dwork_LD, "dwork after dA transpose");
    ...
    magma_tally3_dgetmatrix(m, nb, dwork, dwork_LD, A_dst, LDA);
...
//            pthread_mutex_lock(&mutex_dAP);
    /*1.transpose dA to dwork*/
    magma_tally3blas_dtranspose2s(dwork, dwork_LD, dA_src, dA_LD,  nb, m, compute_stream[deviceID]);
//    printf("Matrix_get_transpose m:%d, nb:%d 1:done\n",m,nb);
#if (dbglevel==10)
//            ca_dbg_printMat_gpu(m, nb, dwork, dwork_LD, "dwork after dA transpose");
#endif
    ...
    /*2. copy dwork to A: send the panel to GPU*/
    magma_tally3_dgetmatrix_async(m, nb, dwork, dwork_LD, A_dst, LDA, stream1);
//    printf("Matrix_get_transpose m:%d, nb:%d 2:done\n",m,nb);
//    pthread_mutex_unlock(&mutex_dAP);
...
void magma_tally3_task_dev_dlaswp(Schedule* sched_obj   )
{
    ...
    double *dA;
    magma_tally3_int_t lda;
    magma_tally3_int_t i1;
    magma_tally3_int_t i2;
    magma_tally3_int_t *ipiv;
    magma_tally3_int_t inci;
#if (dbglevel >=1)
    ca_trace_start();
#endif
    ...
    #if (dbglevel==10)
       ca_dbg_printMat_transpose_gpu(n, n, dA, lda, "A(n,n) before magma_tally3_dlaswp");
    #endif

    pthread_mutex_lock(&mutex_compute_stream);
    magma_tally3blasSetKernelStream(compute_stream[deviceID]);
    
    magma_tally3blas_dlaswp(  n, dA,  lda,  i1,  i2,  ipiv,  inci );

    pthread_mutex_unlock(&mutex_compute_stream);
     //task_magma_tally3_dlaswp(gpu_ncols, dAT(K,K+A_N), dAT_LD, c_one, nb, &ipiv[K], c_one);
    #if (dbglevel==10)
       ca_dbg_printMat_transpose_gpu(n, n, dA, lda, "A(n,n) after magma_tally3_dlaswp");
    #endif
...
void magma_tally3_task_dev_dtrsm(Schedule* sched_obj  )
{
    ...
    #if (dbglevel==10)
       ca_dbg_printMat_transpose_gpu(m, n, dB, ldb, "A(K,K:N) before magma_tally3_dtrsm");
    #endif
    ...
    magma_tally3_dtrsm( side,  uplo,  trans,  diag,  m,  n, alpha, dA,  lda, dB,  ldb );
     //task_magma_tally3_dtrsm('R', 'U', Magma_tally3NoTrans, 'U', gpu_ncols, nb, c_one, dAT(K,K), dAT_LD, dAT(K,K+A_N), dAT_LD);
    pthread_mutex_unlock(&mutex_compute_stream);

    #if (dbglevel==10)
       ca_dbg_printMat_transpose_gpu(m, n, dB, ldb, "A(K,K:N) after magma_tally3_dtrsm");
    #endif
...
#if (dbglevel==10)
            ca_dbg_printMat_transpose_gpu(m, k, dA, lda, "A before magma_tally3_dgemm");
            ca_dbg_printMat_transpose_gpu(k, n, dB, ldb, "B before magma_tally3_dgemm");
            ca_dbg_printMat_transpose_gpu(m, n, dC, ldc, "C before magma_tally3_dgemm");
#endif

        pthread_mutex_lock(&mutex_compute_stream);
    magma_tally3blasSetKernelStream(compute_stream[deviceID]);
    
    magma_tally3_dgemm( transA,  transB,  m,  n,  k, alpha, dA,  lda, dB,  ldb, beta, dC,  ldc );
     //task_magma_tally3_dgemm(Magma_tally3NoTrans,Magma_tally3NoTrans, gpu_ncols, gpu_nrows, nb, c_neg_one, dAT(K,K+A_N), dAT_LD, dAT(K+1,K), dAT_LD, c_one, dAT(K+1,K+A_N), dAT_LD);
    pthread_mutex_unlock(&mutex_compute_stream);
#if (dbglevel==10)
            ca_dbg_printMat_transpose_gpu(m, n, dC, ldc, "C after magma_tally3_dgemm");
#endif
#if (dbglevel >=1)
ca_trace_end_gpu('S');
ca_trace_end_cpu('C');
