...
magma_tally4_get_dtrsm_m_nb() { return 128; }
...
#define dA(gpui, i, j) (dw[gpui] + dimb*lddb + (i)*nb + (j)*nb*ldda)
    ...
    magma_tally4_int_t nb = magma_tally4_get_dtrsm_m_nb();
    ...
    if (MAGMA_tally4_D_REAL(alpha) == 0. && MAGMA_tally4_D_IMAG(alpha) == 0.) {
    ...
    for (igpu = 0; igpu < ngpu; ++igpu) {
        ...
        if (MAGMA_tally4_SUCCESS != magma_tally4_dmalloc( &dw[igpu], (dimb*lddb + dima*ldda) )) {
        ...
        if (notransp) {
            ...
            if (upper) {
                ...
                for (k = 0; k < nbl; ++k) {
                    ...
                    kb = min(nb, n-k*nb);
                    nloc[igpu] += kb;
                    magma_tally4_dsetmatrix_async( m, kb,
                                            B(0, k),              ldb,
                ...
                for (igpu = 0; igpu < ngpu; ++igpu) {
                    magma_tally4_setdevice(igpu);
                    magma_tally4_dsetmatrix_async( m, jb,
                                            A(0, mbl-1),            lda,
                ...
                for (j = mbl-1; j >= 0; --j) {
                    if (j > 0) {
                        ...
                        for (igpu = 0; igpu < ngpu; ++igpu) {
                            magma_tally4_setdevice(igpu);
                            magma_tally4_dsetmatrix_async( j*nb, jb,
                                                    A(0, j-1),            lda,
                    ...
                    for (igpu = 0; igpu < ngpu; ++igpu) {
                        ...
                        magma_tally4_dtrsm(side, uplo, transa, diag, jb, nloc[igpu], alpha_, dA(igpu, j, j%2), ldda,
                                    dB(igpu, j, 0), lddb );
                    }
                    ...
                    if (j > 0) {
                        for (igpu = 0; igpu < ngpu; ++igpu) {
                            magma_tally4_setdevice(igpu);
                            magma_tally4blasSetKernelStream(stream[igpu][j%2]);
                            magma_tally4_dgemm(transa, Magma_tally4NoTrans, j*nb, nloc[igpu], jb, c_neg_one, dA(igpu, 0, j%2), ldda,
                ...
                for (j = 0; j < mbl; ++j) {
                    if ((j+1)*nb < m) {
                        ...
                        for (igpu = 0; igpu < ngpu; ++igpu) {
                            magma_tally4_setdevice(igpu);
                            magma_tally4_dsetmatrix_async( (m-(j+1)*nb), jb,
                                                    A(j+1, j+1),            lda,
                    ...
                    if ( j < mbl-1 ) {
                        for (igpu = 0; igpu < ngpu; ++igpu) {
                            magma_tally4_setdevice(igpu);
                            magma_tally4blasSetKernelStream(stream[igpu][j%2]);
                            magma_tally4_dgemm(transa, Magma_tally4NoTrans, m-(j+1)*nb, nloc[igpu], nb, c_neg_one, dA(igpu, j+1, j%2), ldda,
            ...
            if (upper) {
                ...
                for (igpu = 0; igpu < ngpu; ++igpu) {
                    magma_tally4_setdevice(igpu);
                    magma_tally4_dsetmatrix_async( jb, m,
                                            A(0, 0),        lda,
                                            dA(igpu, 0, 0), ldda, stream[igpu][0] );
                }
                for (j = 0; j < mbl; ++j) {
                    if ((j+1)*nb < m) {
                        ...
                        for (igpu = 0; igpu < ngpu; ++igpu) {
                            magma_tally4_setdevice(igpu);
                            magma_tally4_dsetmatrix_async( jb, m-(j+1)*nb,
                                                    A(j+1, j+1),            lda,
                    ...
                    for (igpu = 0; igpu < ngpu; ++igpu) {
                        magma_tally4_setdevice(igpu);
                        magma_tally4blasSetKernelStream(stream[igpu][j%2]);
                        magma_tally4_dtrsm(side, uplo, transa, diag, jb, nloc[igpu], alpha_, dA(igpu, j%2, j), ldda,
                                    dB(igpu, j, 0), lddb );
                    }
                    ...
                    if ( j < mbl-1 ) {
                        for (igpu = 0; igpu < ngpu; ++igpu) {
                            magma_tally4_setdevice(igpu);
                            magma_tally4blasSetKernelStream(stream[igpu][j%2]);
                            magma_tally4_dgemm(transa, Magma_tally4NoTrans, m-(j+1)*nb, nloc[igpu], nb, c_neg_one, dA(igpu, j%2, j+1), ldda,
                                        dB(igpu, j, 0), lddb, alpha_, dB(igpu, j+1, 0), lddb );
                        }
                    }
                ...
                for (j = mbl-1; j >= 0; --j) {
                    if (j > 0) {
                        ...
                        for (igpu = 0; igpu < ngpu; ++igpu) {
                            magma_tally4_setdevice(igpu);
                            magma_tally4_dsetmatrix_async( jb, j*nb,
                                                    A(j-1, 0),            lda,
                    ...
                    if (j > 0) {
                        for (igpu = 0; igpu < ngpu; ++igpu) {
                            magma_tally4_setdevice(igpu);
                            magma_tally4blasSetKernelStream(stream[igpu][j%2]);
                            magma_tally4_dgemm(transa, Magma_tally4NoTrans, j*nb, nloc[igpu], jb, c_neg_one, dA(igpu, j%2, 0), ldda,
                                        dB(igpu, j, 0), lddb, alpha_, dB(igpu, 0, 0), lddb );
                        }
                    }
        ...
        if (notransp) {
            ...
            if (upper) {
                ...
                for (j = 0; j < mbl; ++j) {
                    ...
                    jb = min(nb, m-j*nb);
                    mloc[igpu] += jb;
                    magma_tally4_dsetmatrix_async( jb, n,
                                            B(j, 0),              ldb,
                ...
                for (igpu = 0; igpu < ngpu; ++igpu) {
                    magma_tally4_setdevice(igpu);
                    magma_tally4_dsetmatrix_async( kb, n,
                                            A(0, 0),        lda,
                                            dA(igpu, 0, 0), ldda, stream[igpu][0] );
                }
                for (k = 0; k < nbl; ++k) {
                    ...
                    for (igpu = 0; igpu < ngpu; ++igpu) {
                        ...
                        magma_tally4_dtrsm(side, uplo, transa, diag, mloc[igpu], kb, alpha_, dA(igpu, k%2, k), ldda,
                                    dB(igpu, 0, k), lddb );
                    }
                    ...
                    if ( k < nbl-1 ) {
                        for (igpu = 0; igpu < ngpu; ++igpu) {
                            magma_tally4_setdevice(igpu);
                            magma_tally4blasSetKernelStream(stream[igpu][k%2]);
                            magma_tally4_dgemm(Magma_tally4NoTrans, transa, mloc[igpu], n-(k+1)*nb, nb, c_neg_one, dB(igpu, 0, k), lddb,
                ...
                for (k = nbl-1; k >= 0; --k) {
                    ...
                    if (k > 0) {
                        for (igpu = 0; igpu < ngpu; ++igpu) {
                            magma_tally4_setdevice(igpu);
                            magma_tally4blasSetKernelStream(stream[igpu][k%2]);
                            magma_tally4_dgemm(Magma_tally4NoTrans, transa, mloc[igpu], k*nb, kb, c_neg_one, dB(igpu, 0, k), lddb,
            ...
            if (upper) {
                ...
                for (igpu = 0; igpu < ngpu; ++igpu) {
                    magma_tally4_setdevice(igpu);
                    magma_tally4_dsetmatrix_async( n, kb,
                                            A(0, nbl-1),            lda,
                ...
                for (k = nbl-1; k >= 0; --k) {
                    ...
                    for (igpu = 0; igpu < ngpu; ++igpu) {
                        magma_tally4_setdevice(igpu);
                        magma_tally4blasSetKernelStream(stream[igpu][k%2]);
                        magma_tally4_dtrsm(side, uplo, transa, diag, mloc[igpu], kb, alpha_, dA(igpu, k, k%2), ldda,
                                    dB(igpu, 0, k), lddb );
                    }
