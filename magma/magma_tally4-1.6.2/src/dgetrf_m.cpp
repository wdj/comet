...
#define dPT(d,i,j) (dPT[d] + (i)*nb*nb + (j)*nb*maxm)
    ...
    double *dAT[Magma_tally4MaxGPUs], *dA[Magma_tally4MaxGPUs], *dPT[Magma_tally4MaxGPUs];
    ...
    *info = 0;
    ...
    if ( (nb <= 1) || (nb >= min(m,n)) ) {
        ...
        for( d=0; d < ngpu; d++ ) {
            ...
            if (MAGMA_tally4_SUCCESS != magma_tally4_dmalloc( &dA[d], (ldn_local+h*nb)*maxm )) {
        ...
        for( I=0; I < n; I += NB ) {
            ...
            timer_start( time );
            magma_tally4blas_dsetmatrix_transpose_mgpu(ngpu, stream, A(0,I), lda,
            ...
            for( offset = 0; offset < min(m,I); offset += NB ) {
                ...
                for( d=0; d < ngpu; d++ ) {
                    magma_tally4_setdevice(d);
                    nbi  = min( nb, NBk );
                    magma_tally4_dsetmatrix_async( (M-offset), nbi,
                                            A(offset,offset), lda,
                    ...
                    /* transpose */
                    magma_tally4blas_dtranspose( M-offset, nbi, dA[d], maxm-offset, dPT(d,0,0), nb );
                }
                
                /* applying the pivot from the previous big-panel */
                for( d=0; d < ngpu; d++ ) {
                    magma_tally4_setdevice(d);
                    magma_tally4blas_dlaswp_q( ldn_local, dAT(d,0,0), ldn_local, offset+1, offset+NBk, ipiv, 1, stream[d][1] );
                }
                ...
                for( jj=0, ib=offset/nb; jj < NBk; jj += nb, ib++ ) {
                    ...
                    for( d=0; d < ngpu; d++ ) {
                        ...
                        if ( jj+nb < NBk ) {
                            magma_tally4_dsetmatrix_async( (M-ii-nb), min(nb,NBk-jj-nb),
                                                    A(ii+nb,ii+nb), lda,
                            ...
                            magma_tally4blas_dtranspose( M-ii-nb, nb, dA[d], rows-nb, dPT(d,0,(1+jj/nb)%2), nb );
                        }
            ...
            if ( M > I ) {
                magma_tally4_dgetrf2_mgpu(ngpu, M-I, N, nb, I, dAT, ldn_local, ipiv+I, dA, A(0,I), lda,
                                   stream, &iinfo);
                if ( iinfo < 0 ) {
                    *info = iinfo;
                    break;
            ...
            timer_start( time );
            magma_tally4blas_dgetmatrix_transpose_mgpu(ngpu, stream, dAT, ldn_local, A(0,I), lda, dA, maxm, M, N, nb);
            for( d=0; d < ngpu; d++ ) {
                magma_tally4_setdevice(d);
                magma_tally4_queue_sync( stream[d][0] );
                magma_tally4_queue_sync( stream[d][1] );
    ...
    if ( *info >= 0 )
        magma_tally4_dgetrf_piv(m, n, NB, A, lda, ipiv, info);
    return *info;
} /* magma_tally4_dgetrf_m */
