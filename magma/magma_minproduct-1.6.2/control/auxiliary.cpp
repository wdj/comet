...
// -------------------------
// Returns version of MAGMA_minproduct, as defined by
// MAGMA_minproduct_VERSION_MAJOR, MAGMA_minproduct_VERSION_MINOR, MAGMA_minproduct_VERSION_MICRO constants.
extern "C"
void magma_minproduct_version( magma_minproduct_int_t* major, magma_minproduct_int_t* minor, magma_minproduct_int_t* micro )
{
    if ( major != NULL && minor != NULL && micro != NULL ) {
        *major = MAGMA_minproduct_VERSION_MAJOR;
        *minor = MAGMA_minproduct_VERSION_MINOR;
        *micro = MAGMA_minproduct_VERSION_MICRO;
    }
}
...
/* ////////////////////////////////////////////////////////////////////////////
   -- Get number of GPUs to use from $MAGMA_minproduct_NUM_GPUS environment variable.
   @author Mark Gates
*/
extern "C"
magma_minproduct_int_t magma_minproduct_num_gpus( void )
{
    const char *ngpu_str = getenv("MAGMA_minproduct_NUM_GPUS");
    ...
    if ( ngpu_str != NULL ) {
        ...
        if ( ngpu < 1 || *endptr != '\0' ) {
            ngpu = 1;
            fprintf( stderr, "$MAGMA_minproduct_NUM_GPUS='%s' is an invalid number; using %d GPU.\n",
                     ngpu_str, (int) ngpu );
        }
        else if ( ngpu > Magma_minproductMaxGPUs || ngpu > ndevices ) {
            ngpu = min( ndevices, Magma_minproductMaxGPUs );
            fprintf( stderr, "$MAGMA_minproduct_NUM_GPUS='%s' exceeds Magma_minproductMaxGPUs=%d or available GPUs=%d; using %d GPUs.\n",
                     ngpu_str, Magma_minproductMaxGPUs, ndevices, (int) ngpu );
        }
...
{
  magma_minproduct_int_t i, newind, ind;
                               ...
                               magma_minproduct_int_t* dj0, magma_minproduct_int_t* dj1 )
{
    ...
    magma_minproduct_int_t jblock = (j0 / nb) / ngpu;
    magma_minproduct_int_t jdev   = (j0 / nb) % ngpu;
    if ( dev < jdev ) {
        jblock += 1;
    }
    *dj0 = jblock*nb;
