...
//#define ENABLE_TIMER3
    ...
    double neg_one = MAGMA_minproduct_D_NEG_ONE;
    double one  = MAGMA_minproduct_D_ONE;
    ...
    magma_minproduct_dmalloc( &dinvA, invA_msize * batchCount);
   ...
   /* check allocation */
    ...
    magma_minproductblas_dlaset_q(Magma_minproductFull, invA_msize, batchCount, MAGMA_minproduct_D_ZERO, MAGMA_minproduct_D_ZERO, dinvA, invA_msize, queue);
    ...
    for(i = 0; i < min_mn; i+=nb) 
    {
        ...
        magma_minproduct_ddisplace_pointers(dA_displ, dA_array, ldda, i, i, batchCount, queue);
        //===============================================
        //  panel factorization
        //===============================================
#if 0
        arginfo = magma_minproduct_dgetf2_batched(
                pm, ib,
                dA_displ, ldda,
...
#else
        arginfo = magma_minproduct_dgetrf_recpanel_batched(
                pm, ib, 16,
                dA_displ, ldda,
...
#define RUN_ALL
        ...
        magma_minproduct_ddisplace_pointers(dW0_displ, dA_array, ldda, i, 0, batchCount, queue);
        magma_minproduct_dlaswp_rowparallel_batched( i, dA_displ, ldda,
                dW0_displ, ldda,
                i, i+ib,
        ...
        if( (i + ib) < n)
        {
            ...
            magma_minproduct_ddisplace_pointers(dA_displ, dA_array, ldda, i, i+ib, batchCount, queue);
            dset_pointer(dwork_array, dwork, nb, 0, 0, dwork_msize, batchCount, queue); // I don't think it is needed Azzam
            magma_minproduct_dlaswp_rowparallel_batched( n-(i+ib), dA_displ, ldda,
                    dwork_array, nb,
                    i, i+ib,
                    pivinfo_array, batchCount, queue);
            ...
            magma_minproduct_ddisplace_pointers(dA_displ, dA_array, ldda, i, i, batchCount, queue);
            magma_minproduct_ddisplace_pointers(dW0_displ, dA_array, ldda, i, i+ib, batchCount, queue);
            ...
            {    
                ...
                {
                    magma_minproduct_ddisplace_pointers(dA_displ, dA_array,  ldda, i+ib,    i, batchCount, queue);
                    magma_minproduct_ddisplace_pointers(dW1_displ, dA_array, ldda,    i, i+ib, batchCount, queue);
                    magma_minproduct_ddisplace_pointers(dW2_displ, dA_array, ldda, i+ib, i+ib, batchCount, queue);
                    //printf("caling batched dgemm %d %d %d \n", m-i-ib, n-i-ib, ib);
                    magma_minproductblas_dgemm_batched( Magma_minproductNoTrans, Magma_minproductNoTrans, m-i-ib, n-i-ib, ib, 
