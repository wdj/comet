...
magma_tally4_cgeqrf_panel_batched(
    ...
    magma_tally4FloatComplex** dW3_displ,
    ...
    magma_tally4_int_t batchCount, cublasHandle_t myhandle, magma_tally4_queue_t queue)
{
            ...
            for( j=0; j<n; j+=nb)
            {
                ...
                magma_tally4_cdisplace_pointers(dW0_displ, dA_array, ldda, j, j, batchCount, queue); 
                magma_tally4_cdisplace_pointers(dW2_displ, tau_array, 1, j, 0, batchCount, queue);
                magma_tally4_cdisplace_pointers(dW3_displ, dR_array, ldr, j, j, batchCount, queue); // 
  
                //sub-panel factorization 
                magma_tally4_cgeqr2_batched(
                      m-j, jb,
                      dW0_displ, ldda,      
                      dW2_displ, 
                      info_array, 
                ...
                magma_tally4_cdisplace_pointers(dW0_displ, dA_array, ldda, j, j, batchCount, queue); 
                magma_tally4_cdisplace_pointers(dW3_displ, dR_array, ldr, j, j, batchCount, queue); 
                magma_tally4blas_clacpy_batched(Magma_tally4Upper, jb, jb, dW0_displ, ldda, dW3_displ, ldr, batchCount, queue);

                magma_tally4_cdisplace_pointers(dW0_displ, dA_array, ldda, j, j, batchCount, queue); 
                magma_tally4_cdisplace_pointers(dW3_displ, dR_array, ldr, j, j, batchCount, queue);
                magma_tally4blas_claset_batched(Magma_tally4Upper, jb, jb, MAGMA_tally4_C_ZERO, MAGMA_tally4_C_ONE, dW0_displ, ldda, batchCount, queue); 

                
                if( (n-j-jb) > 0) //update the trailing matrix inside the panel
                {

                    magma_tally4_clarft_sm32x32_batched(m-j, jb,
                                 dW0_displ, ldda,
                                 dW2_displ,
                                 dT_array, ldt, 
                                 batchCount, myhandle, queue);

                    magma_tally4_cdisplace_pointers(dW1_displ, dA_array, ldda, j, j + jb, batchCount, queue); 
