...
int main( int argc, char** argv)
{
    ...
    magma_minproductDoubleComplex *h_A, *h_R, *tau, *h_work;
    magma_minproductDoubleComplex_ptr d_A, dtau, d_work;
    magma_minproduct_int_t *jpvt;
    magma_minproduct_int_t M, N, K, n2, lda, lwork, j, info, min_mn, nb;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lda    = M;
            n2     = lda*N;
            nb     = magma_minproduct_get_zgeqp3_nb( min_mn );
            ...
            double *rwork;
            magma_minproductDouble_ptr drwork;
            TESTING_MALLOC_DEV( drwork, double, 2*N + (N+1)*nb);
            TESTING_MALLOC_CPU( rwork,  double, 2*N );
            #endif
            TESTING_MALLOC_CPU( jpvt,   magma_minproduct_int_t,        N      );
            ...
            TESTING_MALLOC_DEV( d_work, magma_minproductDoubleComplex, lwork  );
            
            /* Initialize the matrix */
            lapackf77_zlarnv( &ione, ISEED, &n2, h_R );

            /* Make h_A of rank K */
            magma_minproductDoubleComplex alpha = MAGMA_minproduct_Z_MAKE(  1., 0. );
            magma_minproductDoubleComplex beta  = MAGMA_minproduct_Z_MAKE(  0., 0. );
            ...
            /* copy A to gpu */
            magma_minproduct_zsetmatrix( M, N, h_R, lda, d_A, lda );

            /* call gpu-interface */
            gpu_time = magma_minproduct_wtime();
            #if defined(PRECISION_z) || defined(PRECISION_c)
            magma_minproduct_zgeqp3_gpu(M, N, d_A, lda, jpvt, dtau, d_work, lwork, drwork, &info);
            #else
            magma_minproduct_zgeqp3_gpu(M, N, d_A, lda, jpvt, dtau, d_work, lwork, &info);
            #endif
            gpu_time = magma_minproduct_wtime() - gpu_time;
            
            /* copy outputs to cpu */
            magma_minproduct_zgetmatrix( M, N, d_A, lda, h_R, lda );
            magma_minproduct_zgetvector( min_mn, dtau, 1, tau, 1 );
            
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_minproduct_zgeqp3 returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
