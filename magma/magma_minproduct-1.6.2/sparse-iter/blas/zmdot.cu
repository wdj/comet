    ...
    int k )
{
   ...
   if( i < n ){
    for( int j=0; j<k; j++)
      d[ i+j*n ] = MAGMA_minproduct_Z_MAKE( 0.0, 0.0 );
    }
}

// dot product
__global__ void
magma_minproduct_zdot_kernel( 
    int Gs,
    int n, 
...
__global__ void
magma_minproduct_zblockdot_kernel( 
    int Gs,
    int n, 
    int k,
    magma_minproductDoubleComplex * v,
...
__global__ void
magma_minproduct_zblockreduce_kernel( 
    int Gs,
    int n, 
    int k,
    magma_minproductDoubleComplex * vtmp,
...
__global__ void
magma_minproduct_zreduce_kernel_fast( int Gs,
                           int n, 
                           magma_minproductDoubleComplex * vtmp,
                           magma_minproductDoubleComplex * vtmp2 ){
...
__global__ void
magma_minproduct_zblockreduce_kernel_fast( 
    int Gs,
    int n, 
    int k,
    magma_minproductDoubleComplex * vtmp,
    ...
    magma_minproduct_queue_t queue )
{
    ...
    dim3 Gs_next;
    int Ms =  (k)* (local_block_size) * sizeof( magma_minproductDoubleComplex ); // k vecs 
    magma_minproductDoubleComplex_ptr aux1 = d1, aux2 = d2;
    int b = 1;        

    if (k>1) {
        magma_minproduct_zblockdot_kernel<<<Gs, Bs, Ms>>>( Gs.x, n, k, v, r, d1 );
    }
    else {
        magma_minproduct_zdot_kernel<<<Gs, Bs, Ms>>>( Gs.x, n, v, r, d1 );
    }
/*
    // not necessary to zero GPU mem
    magma_minproduct_zgpumemzero<<<Gs, Bs, 0>>>( d1, n*k,1 );
    magma_minproduct_zgpumemzero<<<Gs, Bs, 0>>>( d2, n*k,1 );
    //magma_minproductblas_zlaset( Magma_minproductUpperLower, n, k, d1, n );
    //magma_minproductblas_zlaset( Magma_minproductUpperLower, n, k, d2, n );
    while( Gs.x > 1 ) {
        Gs_next.x = ( Gs.x+Bs.x-1 )/ Bs.x ;
        magma_minproduct_zblockreduce_kernel<<< Gs_next.x, Bs.x, Ms >>> 
                                        ( Gs.x, n, k, aux1, aux2 );
        Gs.x = Gs_next.x;
        b = 1 - b;
    ...
    for( int j=0; j<k; j++) {
            magma_minproduct_zcopyvector( 1, aux1+j*n, 1, skp+j, 1 );
    }
*/
    ...
    if ( k>1) {
        while( Gs.x > 1 ) {
            ...
            magma_minproduct_zblockreduce_kernel_fast<<< Gs_next.x/2, Bs.x/2, Ms/2 >>> 
                        ( Gs.x, n, k, aux1, aux2 );
        ...
        while( Gs.x > 1 ) {
            Gs_next.x = ( Gs.x+Bs.x-1 )/ Bs.x ;
            if ( Gs_next.x == 1 ) Gs_next.x = 2;
            magma_minproduct_zreduce_kernel_fast<<< Gs_next.x/2, Bs.x/2, Ms/2 >>> 
                        ( Gs.x, n, aux1, aux2 );
            Gs_next.x = Gs_next.x /2;
            Gs.x = Gs_next.x;
            b = 1 - b;
    ...
    magma_minproduct_queue_t queue )
{
    ...
    while( rows_left > (chunk_size) ) {
        magma_minproduct_zmdotc( n, chunk_size, v+offset*n, r, d1, d2, skp+offset, queue );
        offset = offset + chunk_size;
        rows_left = rows_left-chunk_size;

    }
    // process rest
    magma_minproduct_zmdotc( n, rows_left, v+offset*n, r, d1, d2, skp+offset, queue ); 
