...
int main( int argc, char** argv)
{
    ...
    double offdiag = MAGMA_minproduct_D_MAKE( 1.2000, 6.7000 );
    double diag    = MAGMA_minproduct_D_MAKE( 3.1415, 2.7183 );
    magma_minproduct_int_t M, N, nb, cnt, size, lda, ldda;
        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for( int j=inset; j < N-inset; ++j ) {
                for( int k=0; k < nb; ++k ) {  // set k-th sub- or super-diagonal
                    ...
                    else if ( uplo[iuplo] == Magma_minproductLower && j+k < M-inset ) {
                        *h_A(j+k,j) = offdiag;
                        cnt += 1;
                    }
                    else if ( uplo[iuplo] == Magma_minproductUpper && j-k >= inset && j-k < M-inset ) {
                        *h_A(j-k,j) = offdiag;
                        cnt += 1;
                    }
                }
            ...
            int mm = M - 2*inset;
            int nn = N - 2*inset;
            magma_minproductblas_dlaset_band( uplo[iuplo], mm, nn, nb, offdiag, diag, d_A(inset,inset), ldda );
            ...
            //printf( "h_R=" );  magma_minproduct_dprint( M, N, h_R, lda );
            //printf( "h_A=" );  magma_minproduct_dprint( M, N, h_A, lda );
