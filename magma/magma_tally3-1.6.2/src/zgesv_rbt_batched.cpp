                  ...
                  magma_tally3_int_t batchCount, magma_tally3_queue_t queue)
{
    ...
    magma_tally3DoubleComplex *hu, *hv;
    if (MAGMA_tally3_SUCCESS != magma_tally3_zmalloc_cpu( &hu, 2*n )) {
        info = MAGMA_tally3_ERR_HOST_ALLOC;
        return info;
    }

    if (MAGMA_tally3_SUCCESS != magma_tally3_zmalloc_cpu( &hv, 2*n )) {
        info = MAGMA_tally3_ERR_HOST_ALLOC;
        return info;
    }
    ...
    info = magma_tally3_zgerbt_batched(Magma_tally3True, n, nrhs, dA_array, n, dB_array, n, hu, hv, &info, batchCount, queue);
    if (info != MAGMA_tally3_SUCCESS)  {
        return info;
    }


    info = magma_tally3_zgetrf_nopiv_batched( n, n, dA_array, ldda, info_array, batchCount, queue);
    if ( (info != MAGMA_tally3_SUCCESS) ){
        return info;
    }
    ...
    info = magma_tally3_zgetrs_nopiv_batched( Magma_tally3NoTrans, n, nrhs, dA_array, ldda, dB_array, lddb, info_array, batchCount, queue );
    ...
    magma_tally3_zsetvector(2*n, hv, 1, dv, 1);

    for(int i = 0; i < nrhs; i++)
        magma_tally3blas_zprbt_mv_batched(n, dv, dB_array+(i), batchCount, queue);
