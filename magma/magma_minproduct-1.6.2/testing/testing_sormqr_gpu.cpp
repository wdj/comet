...
int main( int argc, char** argv )
{
    ...
    float *C, *R, *A, *W, *tau;
    magma_minproductFloat_ptr dC, dA, dT;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int itran = 0; itran < 2; ++itran ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {        
            ...
            magma_minproduct_ssetmatrix( lda, k, A,  lda, dA, lda );
            magma_minproduct_sgeqrf_gpu( lda, k, dA, lda, tau, dT, &info );
            magma_minproduct_sgetmatrix( lda, k, dA, lda, A,  lda );
            if (info != 0)
            ...
            lwork = -1;
            magma_minproduct_sormqr_gpu( side[iside], trans[itran],
                              m, n, k,
                              dA, lda, tau, dC, ldc, W, lwork, dT, nb, &info );
            if (info != 0)
                printf("magma_minproduct_sormqr_gpu (lwork query) returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
            ...
            if (info != 0)
                printf("magma_minproduct_sormqr_gpu returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
