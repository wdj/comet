...
int main(int argc, char **argv)
{
    ...
    magma_tally2FloatComplex *A, *X, *Y, *Ylapack, *Ycublas, *Ymagma_tally2, *Ymagma_tally21, *hwork;
    magma_tally2FloatComplex_ptr dA, dX, dY;
    magma_tally2FloatComplex_ptr d_lA[Magma_tally2MaxGPUs], dwork[Magma_tally2MaxGPUs];
    ...
    magma_tally2_int_t nb = 64;  // required by magma_tally2blas_chemv_mgpu implementation
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int ioffset=0; ioffset < noffsets; ioffset += 1 ) {
        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( A,       magma_tally2FloatComplex, matsize );
            TESTING_MALLOC_CPU( Y,       magma_tally2FloatComplex, vecsize );
            TESTING_MALLOC_CPU( Ycublas, magma_tally2FloatComplex, vecsize );
            TESTING_MALLOC_CPU( Ymagma_tally2,  magma_tally2FloatComplex, vecsize );
            TESTING_MALLOC_CPU( Ymagma_tally21, magma_tally2FloatComplex, vecsize );
            TESTING_MALLOC_CPU( Ylapack, magma_tally2FloatComplex, vecsize );

            TESTING_MALLOC_PIN( X,       magma_tally2FloatComplex, vecsize );
            TESTING_MALLOC_PIN( hwork,   magma_tally2FloatComplex, lhwork  );
            
            magma_tally2_setdevice( opts.device );
            TESTING_MALLOC_DEV( dA, magma_tally2FloatComplex, matsize );
            TESTING_MALLOC_DEV( dX, magma_tally2FloatComplex, vecsize );
            TESTING_MALLOC_DEV( dY, magma_tally2FloatComplex, vecsize );
            
            // TODO make magma_tally2_cmalloc_bcyclic helper function?
            for( dev=0; dev < opts.ngpu; dev++ ) {
                ...
                magma_tally2_setdevice( dev );
                TESTING_MALLOC_DEV( d_lA[dev],  magma_tally2FloatComplex, ldda*n_local[dev] );
                TESTING_MALLOC_DEV( dwork[dev], magma_tally2FloatComplex, ldwork );
            }
            ...
            magma_tally2_cmake_hermitian( Noffset, A, lda );
            ...
            /* =====================================================================
               ...
               =================================================================== */
            magma_tally2_setdevice( opts.device );
            magma_tally2_csetmatrix( Noffset, Noffset, A, lda, dA, ldda );
            magma_tally2_csetvector( Noffset, X, incx, dX, incx );
            magma_tally2_csetvector( Noffset, Y, incx, dY, incx );
            
            cuda_time = magma_tally2_sync_wtime(0);
            cublasChemv( opts.handle, cublas_uplo_const_tally2(opts.uplo), N,
            ...
            magma_tally2_cgetvector( Noffset, dY, incx, Ycublas, incx );
            ...
            magma_tally2blas_chemv_work( opts.uplo, N,
                                  alpha, dA + offset + offset*ldda, ldda,
                                         dX + offset, incx,
            ...
            magma_tally2_cgetvector( Noffset, dY, incx, Ymagma_tally21, incx );
            
            /* =====================================================================
               Performs operation using MAGMA_tally2BLAS (multi-GPU)
               =================================================================== */
            magma_tally2_csetmatrix_1D_col_bcyclic( Noffset, Noffset, A, lda, d_lA, ldda, opts.ngpu, nb );
            blasf77_ccopy( &Noffset, Y, &incx, Ymagma_tally2, &incx );
            
            // workspaces do NOT need to be zero -- set to NAN to prove
            for( dev=0; dev < opts.ngpu; ++dev ) {
                magma_tally2_setdevice( dev );
                magma_tally2blas_claset( Magma_tally2Full, ldwork, 1, MAGMA_tally2_C_NAN, MAGMA_tally2_C_NAN, dwork[dev], ldwork );
            }
            lapackf77_claset( "Full", &lhwork, &ione, &MAGMA_tally2_C_NAN, &MAGMA_tally2_C_NAN, hwork, &lhwork );
            
            mgpu_time = magma_tally2_sync_wtime(0);
            
            magma_tally2_int_t info;
            info = magma_tally2blas_chemv_mgpu(
                opts.uplo, N,
                alpha,
                d_lA, ldda, offset,
                X + offset, incx,
                ...
                opts.ngpu, nb, queues );
            if ( info != 0 )
                printf("magma_tally2blas_chemv_mgpu returned error %d: %s.\n",
                       (int) info, magma_tally2_strerror( info ));
            
            info = magma_tally2blas_chemv_mgpu_sync(
                opts.uplo, N,
                alpha,
                d_lA, ldda, offset,
                X + offset, incx,
                ...
                opts.ngpu, nb, queues );
            if ( info != 0 )
                printf("magma_tally2blas_chemv_sync returned error %d: %s.\n",
                       (int) info, magma_tally2_strerror( info ));
            ...
            if ( opts.lapack ) {
                ...
                blasf77_chemv( lapack_uplo_const_tally2(opts.uplo), &N,
                ...
                blasf77_caxpy( &Noffset, &c_neg_one, Ymagma_tally2, &incx, Ylapack, &incx );
            ...
            blasf77_caxpy( &Noffset, &c_neg_one, Ymagma_tally2, &incx, Ycublas, &incx );
