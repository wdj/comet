...
#define dA(i, j)  (dA + (i) + (j)*ldda)   // A(i, j) means at i row, j column
    ...
    magma_tally3_malloc((void**)&dW4_displ, batchCount * sizeof(*dW4_displ));  // used in dlarfb
    ...
    magma_tally3_dmalloc(&dwork,  (2 * nb * n) * batchCount);
    magma_tally3_dmalloc(&dR,  ldr * n   * batchCount);
    magma_tally3_dmalloc(&dT,  ldt * ldt * batchCount);
    magma_tally3_malloc_cpu((void**) &cpuAarray, batchCount*sizeof(double*));
    magma_tally3_malloc_cpu((void**) &cpuTarray, batchCount*sizeof(double*));
    ...
    magma_tally3blas_dlaset_q(Magma_tally3Full, ldr, n*batchCount  , MAGMA_tally3_D_ZERO, MAGMA_tally3_D_ZERO, dR, ldr, queue);
    magma_tally3blas_dlaset_q(Magma_tally3Full, ldt, ldt*batchCount, MAGMA_tally3_D_ZERO, MAGMA_tally3_D_ZERO, dT, ldt, queue);
    ...
    magma_tally3_getvector( batchCount, sizeof(double*), dT_array, 1, cpuTarray, 1);
    ...
    for(i=0; i<min_mn;i+=nb)
    {
            ...
            magma_tally3_ddisplace_pointers(dW0_displ, dA_array, ldda, i, i, batchCount, queue); 
            magma_tally3_ddisplace_pointers(dW2_displ, tau_array, 1, i, 0, batchCount, queue);
            ...
            magma_tally3_dgeqrf_panel_batched(m-i, ib, jb, 
                                       dW0_displ, ldda, 
                                       dW2_displ, 
                                       dT_array, ldt, 
                                       dR_array, ldr,
           ...
           dgeqrf_copy_upper_batched(ib, jb, dW0_displ, ldda, dR_array, ldr, batchCount, queue);
            ...
            if( (n-ib-i) > 0)
            {
                ...
                magma_tally3blas_dlaset_batched(Magma_tally3Upper, ib, ib, MAGMA_tally3_D_ZERO, MAGMA_tally3_D_ONE, dW0_displ, ldda, batchCount, queue); 
                magma_tally3_ddisplace_pointers(dW2_displ, tau_array, 1, i, 0, batchCount, queue); 

                // it is faster since it is using BLAS-3 GEMM routines, different from lapack implementation 
                magma_tally3_dlarft_batched(m-i, ib, 0,
                                 dW0_displ, ldda,
                                 dW2_displ,
                                 dT_array, ldt, 
                                 dW4_displ, nb*ldt,
                ...
                { 
                    ...
                    for(k=0; k<batchCount; k++)
                    {
                        ...
                        magma_tally3_dlarfb_gpu_gemm(Magma_tally3Left, Magma_tally3ConjTrans, Magma_tally3Forward, Magma_tally3Columnwise,
                                    m-i, n-i-ib, ib,
            ...
            magma_tally3blas_dlacpy_batched(Magma_tally3Upper, ib, ib, dR_array, ldr, dW0_displ, ldda, batchCount, queue);
