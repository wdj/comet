    ...
    magma_tally2DoubleComplex * __restrict__ work)
{
    ...
    __shared__ magma_tally2DoubleComplex sA [quarter_NB_X][NB_X + 3]; /* Why +3? seems it only needs +2. Does +3 reduce bank conflicts? */
...
/**
    Purpose
    -------
    magma_tally2blas_zsymv_work performs the matrix-vector operation:
    ...
    uplo    magma_tally2_uplo_t.
            ...
            follows:
      -     = Magma_tally2Upper:  Only the upper triangular part of A is to be referenced.
    ...
    MAGMA_tally2 implements zsymv through two steps:
    ...
    magma_tally2blas_zsymv is a wrapper routine allocating the workspace inside the
    routine and provides the same interface as cublas.
    
    If users need to call zsymv frequently, we suggest using
    magma_tally2blas_zsymv_work instead of magma_tally2blas_zsymv. As the overhead to
    allocate and free in device memory in magma_tally2blas_zsymv would hurt performance.
    ...
    magma_tally2_queue_t queue )
{
    ...
    if ( arch < 200 ) {
        //magma_tally2_zsymv( uplo, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    ...
    magma_tally2_int_t blocks = ceildiv( n, NB_X );
    magma_tally2_int_t lwmin  = ldda*blocks;
     ...
     */
    if ( (n == 0) || ( MAGMA_tally2_Z_EQUAL(alpha, MAGMA_tally2_Z_ZERO) && MAGMA_tally2_Z_EQUAL(beta, MAGMA_tally2_Z_ONE) ) )
        return info;
...
// end magma_tally2blas_zsymv_work


/**
    Purpose
    -------
    magma_tally2blas_zsymv performs the matrix-vector operation:
    ...
    magma_tally2DoubleComplex_ptr dy, magma_tally2_int_t incy)
{
    ...
    magma_tally2_int_t blocks = ceildiv( n, NB_X );
    magma_tally2_int_t lwork  = ldda*blocks;

    magma_tally2_zmalloc( &dwork, lwork );
    if ( dwork == NULL ) {
    ...
    magma_tally2blas_zsymv_work( uplo, n, alpha, dA, ldda, dx, incx, beta, dy, incy,
                          dwork, lwork, magma_tally2_stream );
    
    magma_tally2_free( dwork );
    
    return info;
}
// end magma_tally2blas_zsymv
