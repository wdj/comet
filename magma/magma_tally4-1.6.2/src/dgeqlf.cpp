...
/**
    ...
    lwork   INTEGER
            ...
            through magma_tally4_get_dgeqlf_nb(M).
    \n
    ...
    magma_tally4_int_t *info)
{
    ...
    magma_tally4Double_ptr dA, dwork;
    ...
    nb = magma_tally4_get_dgeqlf_nb(m);
    *info = 0;
    lquery = (lwork == -1);
    ...
    if (*info == 0) {
        if (k == 0)
            ...
            work[0] = MAGMA_tally4_D_MAKE( max(n*nb, 2*nb*nb), 0 );
        }
    ...
    if (MAGMA_tally4_SUCCESS != magma_tally4_dmalloc( &dA, (n)*ldda + nb*lddwork )) {
    ...
    if ( (nb > 1) && (nb < k) ) {
        ...
        for (i = k - kk + ki; i >= k -kk; i -= nb) {
            ...
            if (i < k - kk + ki) {
                ...
                magma_tally4_dgetmatrix_async( m-rows, ib,
                                        dA(rows, n-k+i), ldda,
                ...
                magma_tally4_dlarfb_gpu( Magma_tally4Left, Magma_tally4ConjTrans, Magma_tally4Backward, Magma_tally4Columnwise,
                                  rows, cols, old_ib,
            ...
            if (cols > 0) {
                ...
                lapackf77_dlarft( Magma_tally4BackwardStr, Magma_tally4ColumnwiseStr,
                                  &rows, &ib,
                                  A(0, cols), &lda, tau + i, work, &ib);

                dpanel_to_q_tally4( Magma_tally4Lower, ib, A(rows-ib,cols), lda, work+ib*ib);
                magma_tally4_dsetmatrix( rows, ib,
                                  A(0,cols),  lda,
                                  dA(0,cols), ldda );
                dq_to_panel_tally4( Magma_tally4Lower, ib, A(rows-ib,cols), lda, work+ib*ib);

                // Send the triangular part on the GPU
                magma_tally4_dsetmatrix( ib, ib, work, ib, dwork(0), lddwork );
        ...
        magma_tally4_dgetmatrix( m, nu, dA(0,0), ldda, A(0,0), lda );
    } else {
        mu = m;
        nu = n;
    }
