    ...
    magma_tally3_int_t *info)
{
    ...
    *info = 0;
    ...
    work[0]  = MAGMA_tally3_C_MAKE( lwmin * one_eps, 0 );
    ...
    magma_tally3_int_t ldwork_real = max( ldwork*2, n );
    if ( wantz ) {
        // cstedx requrise 3n^2/2, in float
        ldwork_real = max( ldwork_real, 3*n*(n/2 + 1) );
    }
    if (MAGMA_tally3_SUCCESS != magma_tally3_smalloc( &dwork, ldwork_real )) {
        *info = MAGMA_tally3_ERR_DEVICE_ALLOC;
        return *info;
    }
    dC = (magma_tally3FloatComplex*) dwork;
    ...
    if (iscale == 1) {
        magma_tally3blas_clascl( uplo, 0, 0, 1., sigma, n, n, dA, ldda, info );
    }
...
#ifdef FAST_HEMV
    magma_tally3_chetrd2_gpu( uplo, n, dA, ldda, w, &rwork[inde],
                       &work[indtau], wA, ldwa, &work[indwrk], llwork,
                       dC, ldwork, &iinfo );
#else
    magma_tally3_chetrd_gpu ( uplo, n, dA, ldda, w, &rwork[inde],
                       &work[indtau], wA, ldwa, &work[indwrk], llwork,
                       &iinfo );
#endif
    ...
    if (! wantz) {
        ...
        magma_tally3_csetmatrix( n, *mout, &work[indwrk + n * (il-1) ], n, dC, lddc );

        magma_tally3_cunmtr_gpu( Magma_tally3Left, uplo, Magma_tally3NoTrans, n, *mout, dA, ldda, &work[indtau],
                          dC, lddc, wA, ldwa, &iinfo );

        magma_tally3_ccopymatrix( n, *mout, dC, lddc, dA, ldda );
