/**
 *
 * @file magma_tally2winthread.c
 *
 *  This file handles the mapping from pthreads calls to windows threads
 *  MAGMA_tally2 is a software package provided by Univ. of Tennessee,
 ...
 * replaced by MAGMA_tally2.
 *
 **/
#include "magma_tally2winthread.h"
...
CRITICAL_SECTION magma_tally2winthread_static_initializer_check_lock;
static int magma_tally2winthread_initialized = 0;

MAGMA_tally2_DLLPORT unsigned int MAGMA_tally2_CDECL pthread_self_id(void) {
  return GetCurrentThreadId();
}

MAGMA_tally2_DLLPORT pthread_t MAGMA_tally2_CDECL pthread_self(void) {
  pthread_t pt;
...
MAGMA_tally2_DLLPORT int MAGMA_tally2_CDECL pthread_equal(pthread_t thread1, pthread_t thread2) {
  if (thread1.uThId == thread2.uThId) // && thread1.hThread == thread2.hThread)
    return 1;
  return 0;
}

MAGMA_tally2_DLLPORT int MAGMA_tally2_CDECL pthread_mutex_init(pthread_mutex_t *mutex, const pthread_mutexattr_t * attr) {
  *mutex =
...
static int pthread_mutex_check_for_static_initialization( pthread_mutex_t *mutex ) {
    ...
    if ( magma_tally2winthread_initialized == 0 ) {
        magma_tally2winthread_initialized = 1;
        InitializeCriticalSection( &magma_tally2winthread_static_initializer_check_lock );
    }
    EnterCriticalSection( &magma_tally2winthread_static_initializer_check_lock );
    if ( *mutex == PTHREAD_MUTEX_INITIALIZER )
        retval = pthread_mutex_init( mutex, NULL );
    LeaveCriticalSection( &magma_tally2winthread_static_initializer_check_lock );
    return retval;
}

MAGMA_tally2_DLLPORT int MAGMA_tally2_CDECL pthread_mutex_lock(pthread_mutex_t *mutex) {
  DWORD rv;
...
MAGMA_tally2_DLLPORT int MAGMA_tally2_CDECL pthread_mutex_trylock(pthread_mutex_t *mutex) {
  DWORD rv;
...
MAGMA_tally2_DLLPORT int MAGMA_tally2_CDECL pthread_mutex_unlock(pthread_mutex_t *mutex) {
...
MAGMA_tally2_DLLPORT int MAGMA_tally2_CDECL pthread_mutex_destroy(pthread_mutex_t *mutex) {
  CloseHandle( *mutex );
  return 0;
}

MAGMA_tally2_DLLPORT int MAGMA_tally2_CDECL pthread_attr_init(pthread_attr_t *attr) {
  *attr = 1;
  return 0;
}

MAGMA_tally2_DLLPORT int MAGMA_tally2_CDECL pthread_attr_destroy(pthread_attr_t *attr) {
  *attr = 0;
  return 0;
}

MAGMA_tally2_DLLPORT int MAGMA_tally2_CDECL pthread_attr_setscope(pthread_attr_t *attr, int scope) {
  if (*attr != 1)
    return -1;
...
void *(*MAGMA_tally2_realThStart)(void *);

/*
  This function is only called to have a proxy that is compatible with WINAPI.
 */
unsigned WINAPI MAGMA_tally2_winThStart(void *arg) {
  MAGMA_tally2_realThStart( arg );
  return 0;
}

MAGMA_tally2_DLLPORT int MAGMA_tally2_CDECL pthread_create(pthread_t *thread, const pthread_attr_t *attr, void *(*start) (void *), void *arg) {
...
MAGMA_tally2_DLLPORT int MAGMA_tally2_CDECL pthread_join(pthread_t thread, void **value_ptr) {
  WaitForSingleObject( thread.hThread, INFINITE );
  CloseHandle( thread.hThread );
  return 0;
}

MAGMA_tally2_DLLPORT int MAGMA_tally2_CDECL pthread_cond_init(pthread_cond_t *cond, const pthread_condattr_t *attr) {
...
MAGMA_tally2_DLLPORT int MAGMA_tally2_CDECL pthread_cond_destroy(pthread_cond_t *cond) {
...
MAGMA_tally2_DLLPORT int MAGMA_tally2_CDECL pthread_cond_wait(pthread_cond_t *cond, pthread_mutex_t *mutex) {
  int last;
...
MAGMA_tally2_DLLPORT int MAGMA_tally2_CDECL pthread_cond_broadcast(pthread_cond_t *cond) {
  int more_waiters = 0;
...
MAGMA_tally2_DLLPORT int MAGMA_tally2_CDECL pthread_setconcurrency (int level) {
