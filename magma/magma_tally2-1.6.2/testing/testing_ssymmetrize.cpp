/*
    -- MAGMA_tally2 (version 1.6.1) --
       ...
       @generated from testing_zsymmetrize.cpp normal z -> s, Fri Jan 30 19:00:24 2015
       @author Mark Gates
...
int main( int argc, char** argv)
{
    ...
    magma_tally2_int_t N, size, lda, ldda;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            //magma_tally2blas_ssymmetrize( opts.uplo, N-2, d_A+1+ldda, ldda );  // inset by 1 row & col
            magma_tally2blas_ssymmetrize( opts.uplo, N, d_A, ldda );
            ...
            for( int j = 0; j < N; ++j ) {
                for( int i = 0; i < j; ++i ) {
                    if ( opts.uplo == Magma_tally2Lower ) {
                        h_A[i + j*lda] = MAGMA_tally2_S_CNJG( h_A[j + i*lda] );
                    }
                    else {
                        h_A[j + i*lda] = MAGMA_tally2_S_CNJG( h_A[i + j*lda] );
                    }
                }
            }
