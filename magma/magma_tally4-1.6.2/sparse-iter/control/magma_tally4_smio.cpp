/*
    -- MAGMA_tally4 (version 1.6.2) --
       ...
       @generated from magma_tally4_zmio.cpp normal z -> s, Sun May  3 11:23:01 2015
       @author Hartwig Anzt
*/
...
/**
    ...
    @param[out]
    type        magma_tally4_storage_t*
                storage type of matrix
                
    @param[out]
    location    magma_tally4_location_t*
                location of matrix
                
    @param[out]
    n_row       magma_tally4_int_t*
                number of rows in matrix
                
    @param[out]
    n_col       magma_tally4_int_t*
                number of columns in matrix
                
    @param[out]
    nnz         magma_tally4_int_t*
...
extern "C"
magma_tally4_int_t read_s_csr_from_mtx(
    ...
    magma_tally4_queue_t queue )
{
    ...
    magma_tally4_index_t *coo_col=NULL, *coo_row=NULL;
    float *coo_val=NULL;
    magma_tally4_index_t *new_col=NULL, *new_row=NULL;
    float *new_val=NULL;
    ...
    if (fid == NULL) {
        ...
        info = MAGMA_tally4_ERR_NOT_FOUND;
        goto cleanup;
    }
    ...
    magma_tally4_index_t num_rows, num_cols, num_nonzeros;
    if (mm_read_mtx_crd_size(fid, &num_rows, &num_cols, &num_nonzeros) != 0){
        info = MAGMA_tally4_ERR_NOT_SUPPORTED;
        goto cleanup;
    }
    
    *type     = Magma_tally4_CSR;
    *location = Magma_tally4_CPU;
    *n_row    = num_rows;
    *n_col    = num_cols;
    *nnz      = num_nonzeros;


    CHECK( magma_tally4_index_malloc_cpu( &coo_col, *nnz ) );
    CHECK( magma_tally4_index_malloc_cpu( &coo_row, *nnz ) );
    CHECK( magma_tally4_smalloc_cpu( &coo_val, *nnz ) );

    if (mm_is_real(matcode) || mm_is_integer(matcode)) {
        for(magma_tally4_int_t i = 0; i < *nnz; ++i) {
            ...
            coo_row[i] = ROW - 1;
            coo_col[i] = COL - 1;
            coo_val[i] = MAGMA_tally4_S_MAKE( VAL, 0.);
        }
        ...
        for(magma_tally4_int_t i = 0; i < *nnz; ++i) {
            ...
            coo_row[i] = ROW - 1;
            coo_col[i] = COL - 1;
            coo_val[i] = MAGMA_tally4_S_MAKE( 1.0, 0.);
        }
       ...
       for(magma_tally4_int_t i = 0; i < *nnz; ++i) {
            ...
            coo_row[i] = ROW - 1;
            coo_col[i] = COL - 1;
            coo_val[i] = MAGMA_tally4_S_MAKE( VAL, VALC);
        }
    ...
    if (mm_is_symmetric(matcode)) { // duplicate off diagonal entries
        ...
        magma_tally4_index_t off_diagonals = 0;
        for(magma_tally4_int_t i = 0; i < *nnz; ++i) {
            if (coo_row[i] != coo_col[i])
                ++off_diagonals;
        }
        
        magma_tally4_index_t true_nonzeros = 2*off_diagonals + (*nnz - off_diagonals);
        
        printf("total number of nonzeros: %d\n", (int) *nnz);

        
        CHECK( magma_tally4_index_malloc_cpu( &new_col, true_nonzeros ) );
        CHECK( magma_tally4_index_malloc_cpu( &new_row, true_nonzeros ) );
        CHECK( magma_tally4_smalloc_cpu( &new_val, true_nonzeros ) );
    
        magma_tally4_index_t ptr = 0;
        for(magma_tally4_int_t i = 0; i < *nnz; ++i) {
            if (coo_row[i] != coo_col[i]) {
        ...
        magma_tally4_free_cpu(coo_row); 
        magma_tally4_free_cpu(coo_col); 
        magma_tally4_free_cpu(coo_val);
    ...
    */
    CHECK( magma_tally4_smalloc_cpu( val, *nnz ) );
    
    CHECK( magma_tally4_index_malloc_cpu( col, *nnz ) );
    CHECK( magma_tally4_index_malloc_cpu( row, (*n_row+1) ) );
    CHECK( magma_tally4_smalloc_cpu( val, *nnz ) );

    // original code from  Nathan Bell and Michael Garland
    // the output CSR structure is NOT sorted!

    for (magma_tally4_index_t i = 0; i < num_rows; i++)
        (*row)[i] = 0;
    
    for (magma_tally4_index_t i = 0; i < *nnz; i++)
        (*row)[coo_row[i]]++;
    
    // cumsum the nnz per row to get Bp[]
    for(magma_tally4_int_t i = 0, cumsum = 0; i < num_rows; i++) {
        magma_tally4_index_t temp = (*row)[i];
        (*row)[i] = cumsum;
        cumsum += temp;
    }
    (*row)[num_rows] = *nnz;
    ...
    for(magma_tally4_int_t i = 0; i < *nnz; i++) {
        magma_tally4_index_t row_  = coo_row[i];
        magma_tally4_index_t dest = (*row)[row_];
    ...
    for (magma_tally4_index_t k=0; k < *n_row; ++k) {
        for (magma_tally4_index_t i=(*row)[k]; i < (*row)[k+1]-1; ++i) {
            for (magma_tally4_index_t j=(*row)[k]; j < (*row)[k+1]-1; ++j) {
                if ( (*col)[j] > (*col)[j+1] ) {
                    ti = (*col)[j];
                    (*col)[j] = (*col)[j+1];
                    (*col)[j+1] = ti;
...
{
    magma_tally4_int_t info = 0;
    // TODO: why does this hard code Magma_tally4ColMajor?
    CHECK( magma_tally4_swrite_csr_mtx( B, Magma_tally4ColMajor, filename, queue ));
cleanup:
    return info;
}
...
/**
    ...
    @param[in]
    MajorType   magma_tally4_index_t
                Row or Column sort
                default: 0 = RowMajor, 1 = ColMajor
                TODO: use named constants (e.g., Magma_tally4RowMajor), not numbers.
    ...
    magma_tally4_queue_t queue )
{
    ...
    if ( MajorType == Magma_tally4ColMajor ) {
        // to obtain ColMajor output we transpose the matrix
        // and flip the row and col pointer in the output
        
        CHECK( magma_tally4_s_cucsrtranspose( A, &B, queue ));
        magma_tally4_sprint_matrix( A, queue );
        
        
        magma_tally4_sprint_matrix( B, queue );
        ...
        magma_tally4_index_t i=0, j=0, rowindex=1;
        
        for(i=0; i < B.num_cols; i++) {
            magma_tally4_index_t rowtemp1 = B.row[i];
            magma_tally4_index_t rowtemp2 = B.row[i+1];
            for(j=0; j < rowtemp2 - rowtemp1; j++) {
                fprintf( fp, "%d %d %.16g %.16g\n",
                    ((B.col)[rowtemp1+j]+1), rowindex,
                    MAGMA_tally4_S_REAL((B.val)[rowtemp1+j]),
                    MAGMA_tally4_S_IMAG((B.val)[rowtemp1+j]) );
        ...
        for(i=0; i < A.num_cols; i++) {
            magma_tally4_index_t rowtemp1 = A.row[i];
            magma_tally4_index_t rowtemp2 = A.row[i+1];
            for(j=0; j < rowtemp2 - rowtemp1; j++) {
                fprintf( fp, "%d %d %.16g %.16g\n",
                    ((A.col)[rowtemp1+j]+1), rowindex,
                    MAGMA_tally4_S_REAL((A.val)[rowtemp1+j]),
                    MAGMA_tally4_S_IMAG((A.val)[rowtemp1+j]) );
    ...
    magma_tally4_queue_t queue )
{
    ...
    if ( MajorType == Magma_tally4ColMajor ) {
        ...
        magma_tally4_int_t new_n_row;
        magma_tally4_int_t new_n_col;
        ...
        for(i=0; i < n_col; i++) {
            magma_tally4_index_t rowtemp1 = (new_row)[i];
            magma_tally4_index_t rowtemp2 = (new_row)[i+1];
            for(j=0; j < rowtemp2 - rowtemp1; j++) {
                printf( "%d %d %.6e %.6e\n",
                    ((new_col)[rowtemp1+j]+1), rowindex,
                    MAGMA_tally4_S_REAL((new_val)[rowtemp1+j]),
                    MAGMA_tally4_S_IMAG((new_val)[rowtemp1+j]) );
        ...
        for(i=0; i < n_col; i++) {
            magma_tally4_index_t rowtemp1 = (*row)[i];
            magma_tally4_index_t rowtemp2 = (*row)[i+1];
            for(j=0; j < rowtemp2 - rowtemp1; j++) {
                printf( "%d %d %.6e %.6e\n",
                    rowindex, ((*col)[rowtemp1+j]+1),
                    MAGMA_tally4_S_REAL((*val)[rowtemp1+j]),
                    MAGMA_tally4_S_IMAG((*val)[rowtemp1+j]) );
    ...
    magma_tally4_queue_t queue )
{
    ...
    magma_tally4_index_t info = 0, i=0, j=0;
...
/**
    ...
    A           magma_tally4_s_matrix
                sparse matrix in Magma_tally4_CSR format
    @param[in]
    queue       magma_tally4_queue_t
                Queue to execute in.
    ...
    magma_tally4_queue_t queue )
{
    ...
    #ifdef COMPLEX
    #define magma_tally4_sprintval( tmp )       {                                  \
        if ( MAGMA_tally4_S_EQUAL( tmp, c_zero )) {                                \
            printf( "   0.              " );                                \
        }                                                                   \
        else {                                                              \
            printf( " %8.4f+%8.4fi",                                        \
                    MAGMA_tally4_S_REAL( tmp ), MAGMA_tally4_S_IMAG( tmp ));              \
        }                                                                   \
    }
    #else
    #define magma_tally4_sprintval( tmp )       {                                  \
        if ( MAGMA_tally4_S_EQUAL( tmp, c_zero )) {                                \
            ...
            printf( " %8.4f", MAGMA_tally4_S_REAL( tmp ));                         \
        }                                                                   \
    }
    #endif
    //**************************************************************
    
    magma_tally4_index_t i, j, k;
    float c_zero = MAGMA_tally4_S_ZERO;
    magma_tally4_s_matrix C={Magma_tally4_CSR};
        ...
        if ( A.storage_type == Magma_tally4_DENSE ) {
            for( i=0; i < (A.num_rows); i++ ) {
                for( j=0; j < A.num_cols; j++ ) {
                    magma_tally4_sprintval( A.val[i*(A.num_cols)+j] );
                }
                printf( "\n" );
            }
        }
        else if ( A.storage_type == Magma_tally4_CSR ) {
            // visualize only small matrices like dense
            if ( A.num_rows < 11 && A.num_cols < 11 ) {
                CHECK( magma_tally4_smconvert( A, &C, A.storage_type, Magma_tally4_DENSE, queue ));
                CHECK( magma_tally4_sprint_matrix(  C, queue ));
            ...
            // otherwise visualize only coners
                ...
                for( i=0; i < 4; i++ ) {
                    ...
                    for( j=0; j < 4; j++ ) {
                        ...
                        magma_tally4_index_t rbound = min( A.row[i]+4, A.row[i+1]);
                        magma_tally4_index_t lbound = max( A.row[i], A.row[i]);
                        for( k=lbound; k < rbound; k++ ) {
                            if ( A.col[k] == j ) {
                                tmp = A.val[k];
                            }
                        }
                        magma_tally4_sprintval( tmp );
                    }
                    if ( i == 0 ) {
                        printf( "    . . .    " );
                    } else {
                    ...
                    for( j=A.num_rows-4; j < A.num_rows; j++ ) {
                        float tmp = MAGMA_tally4_S_ZERO;
                        magma_tally4_index_t rbound = min( A.row[i+1], A.row[i+1]);
                        magma_tally4_index_t lbound = max( A.row[i+1]-4, A.row[i]);
                        for( k=lbound; k < rbound; k++ ) {
                            if ( A.col[k] == j ) {
                                tmp = A.val[k];
            ...
            CHECK( magma_tally4_smconvert( A, &C, A.storage_type, Magma_tally4_CSR, queue ));
            CHECK( magma_tally4_sprint_matrix(  C, queue ));
        }
    }
    else {
        magma_tally4_s_matrix C={Magma_tally4_CSR};
        CHECK( magma_tally4_smtransfer( A, &C, A.memory_location, Magma_tally4_CPU, queue ));
        CHECK( magma_tally4_sprint_matrix(  C, queue ));
    }
    ...
    magma_tally4_queue_t queue )
{
    ...
    magma_tally4_index_t *coo_col = NULL;
    magma_tally4_index_t *coo_row = NULL;
    float *coo_val = NULL;
    float *new_val = NULL;
    magma_tally4_index_t* new_row = NULL;
    magma_tally4_index_t* new_col = NULL;
    ...
    CHECK( magma_tally4_index_malloc_cpu( &coo_col, A->nnz ) );
    CHECK( magma_tally4_index_malloc_cpu( &coo_row, A->nnz ) );
    CHECK( magma_tally4_smalloc_cpu( &coo_val, A->nnz ) );
    ...
    A->sym = Magma_tally4_GENERAL;

    if (mm_is_symmetric(matcode)) { // duplicate off diagonal entries
        A->sym = Magma_tally4_SYMMETRIC;
        //printf("detected symmetric case\n");
        magma_tally4_index_t off_diagonals = 0;
        for(magma_tally4_int_t i = 0; i < A->nnz; ++i) {
            if (coo_row[i] != coo_col[i])
        ...
        magma_tally4_index_t true_nonzeros = 2 * off_diagonals + (A->nnz - off_diagonals);
    ...
    */
    CHECK( magma_tally4_smalloc_cpu( &A->val, A->nnz ));
    CHECK( magma_tally4_index_malloc_cpu( &A->col, A->nnz ));
    CHECK( magma_tally4_index_malloc_cpu( &A->row, A->num_rows+1 ));
    ...
    for (magma_tally4_index_t i = 0; i < A->nnz; i++)
        (A->row)[coo_row[i]]++;
        
    // cumsum the nnz per row to get Bp[]
    for(magma_tally4_int_t i = 0, cumsum = 0; i < num_rows; i++) {
        magma_tally4_index_t temp = (A->row)[i];
        (A->row)[i] = cumsum;
        cumsum += temp;
    }
    ...
    for(magma_tally4_int_t i = 0; i < A->nnz; i++) {
        magma_tally4_index_t row_  = coo_row[i];
        magma_tally4_index_t dest = (A->row)[row_];
    ...
    for (magma_tally4_index_t k=0; k < A->num_rows; ++k) {
        for (magma_tally4_index_t i=(A->row)[k]; i < (A->row)[k+1]-1; ++i) {
            for (magma_tally4_index_t j=(A->row)[k]; j < (A->row)[k+1]-1; ++j) {
    ...
    if ( csr_compressor > 0) { // run the CSR compressor to remove zeros
        ...
        CHECK( magma_tally4_smtransfer( *A, &B, Magma_tally4_CPU, Magma_tally4_CPU, queue ));
        CHECK( magma_tally4_s_csr_compressor(
            &(A->val), &(A->row), &(A->col),
        ...
        CHECK( magma_tally4_smtransfer( B, A, Magma_tally4_CPU, Magma_tally4_CPU, queue ));
        //printf("done.\n");
