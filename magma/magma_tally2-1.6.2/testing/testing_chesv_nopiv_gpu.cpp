...
int main(int argc, char **argv)
{
    ...
    magma_tally2FloatComplex *h_A, *h_B, *h_X, temp, *hwork;
    magma_tally2FloatComplex_ptr d_A, d_B;
    magma_tally2_int_t *ipiv;
    magma_tally2_int_t N, nrhs, lda, ldb, ldda, lddb, info, sizeA, sizeB, lwork;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            bool nopiv = true;
            if ( nopiv ) {
                magma_tally2_cmake_hpd( N, h_A, lda );  // SPD / HPD does not require pivoting
            }
            else {
                magma_tally2_cmake_hermitian( N, h_A, lda );  // symmetric/Hermitian generally requires pivoting
            }
            
            magma_tally2_csetmatrix( N, N,    h_A, lda, d_A, ldda );
            magma_tally2_csetmatrix( N, nrhs, h_B, ldb, d_B, lddb );
            ...
            magma_tally2_chesv_nopiv_gpu( opts.uplo, N, nrhs, d_A, ldda, d_B, lddb, &info );
            gpu_time = magma_tally2_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally2_cgesv_gpu returned error %d: %s.\n",
            ...
            if ( opts.lapack ) {
                lwork = -1;
                lapackf77_chesv( lapack_uplo_const_tally2(opts.uplo), &N,&nrhs,
                                 h_A, &lda, ipiv, h_B, &ldb, &temp, &lwork, &info );
                lwork = (magma_tally2_int_t) MAGMA_tally2_C_REAL( temp );
                TESTING_MALLOC_PIN( hwork, magma_tally2FloatComplex, lwork );
