    ...
    int block_offset )
{
    ...
    float psum, psum_t;
    float total = MAGMA_tally3_S_ZERO;
...
/**************************************************************
    ...
    Note beta*y is not included here; see magma_tally3blas_ssymv_mgpu_sync.
    ...
    int block_offset)
{
    ...
    if ( ind >= block_offset && ind < n+block_offset ) {
        ...
        y[ind * incy] = alpha*Ax;  // see magma_tally3blas_ssymv_sync for beta*y
    }
}
...
/**
    Purpose
    -------
    magma_tally3blas_ssymv_mgpu performs the matrix-vector operation:
...
magma_tally3blas_ssymv_mgpu(
    ...
    float alpha,
    magma_tally3Float_const_ptr const d_lA[], magma_tally3_int_t ldda,
    magma_tally3_int_t offset,
    float const *x,       magma_tally3_int_t incx,
    float beta,                                         // unused, see magma_tally3blas_ssymv_mgpu_sync
    float       *y,       magma_tally3_int_t incy,             // unused
    ...
    magma_tally3_queue_t queues[] )
{
    ...
    for(dev=0; dev < ngpu; dev++) {
        ...
        // copy x to each GPU
        magma_tally3_ssetvector_async( n, x, incx, dx_dev + block_offset, 1, queues[dev] );
    ...
    for(dev=0; dev < ngpu; dev++) {
        ...
        magma_tally3_setdevice( dev );
        float       *dx_dev    = dwork[dev];
        magma_tally3_sgetvector_async( n, dx_dev + block_offset, 1, &hwork[dev*n], 1, queues[dev] );
    }
    
    // see magma_tally3blas_ssymv_mgpu_sync for final row sums
...
/**
    ...
    For convenience, the parameters are identical to magma_tally3blas_ssymv_mgpu
    (though some are unused here).
    
    @see magma_tally3blas_ssymv_mgpu
    @ingroup magma_tally3_sblas2
    ********************************************************************/
extern "C" magma_tally3_int_t
magma_tally3blas_ssymv_mgpu_sync(
    magma_tally3_uplo_t uplo,                                               // unused, see magma_tally3blas_ssymv_mgpu
    magma_tally3_int_t n,
    float alpha,                                        // unused
    magma_tally3Float_const_ptr const d_lA[], magma_tally3_int_t ldda,
    magma_tally3_int_t offset,                                              // unused
    float const *x,       magma_tally3_int_t incx,             // unused
    float beta,
    float       *y,       magma_tally3_int_t incy,             // unused
    float       *hwork,   magma_tally3_int_t lhwork,
    magma_tally3Float_ptr    dwork[], magma_tally3_int_t ldwork,           // unused
    magma_tally3_int_t ngpu,
