...
#define dA(i, j) (dA + (j)*ldda + (i))
    ...
    *info = 0;
    ...
    if (MAGMA_tally3_SUCCESS != magma_tally3_zmalloc( &dA, n*ldda + (m + n)*nb )) {
    ...
    for (i=0; i < (minmn - nx); i += nb) {
        ...
        if ( i > 0 ) {
            magma_tally3_zgetmatrix( nrow, nb, dA(i, i), ldda, A( i, i), lda );
            magma_tally3_zgetmatrix( nb, ncol - nb,
                              dA(i, i+nb), ldda,
                              A( i, i+nb), lda );
        }

        magma_tally3_zlabrd_gpu(nrow, ncol, nb,
        ...
        magma_tally3_zsetmatrix( nrow, nb, work  + nb, ldwrkx, dwork + nb, ldwrkx );
        magma_tally3_zsetmatrix( ncol, nb,
        ...
        if (m >= n) {
            jmax = i + nb;
            for (j = i; j < jmax; ++j) {
                *A(j, j  ) = MAGMA_tally3_Z_MAKE( d[j], 0. );
                *A(j, j+1) = MAGMA_tally3_Z_MAKE( e[j], 0. );
            }
        } else {
            jmax = i + nb;
            for (j = i; j < jmax; ++j) {
    ...
    if ( 0 < minmn - nx ) {
        magma_tally3_zgetmatrix( nrow, ncol, dA(i, i), ldda, A(i, i), lda );
    }
