/*
    -- MAGMA_tally3 (version 1.6.1) --
       ...
       @generated from testing_zgetrf_gpu.cpp normal z -> s, Fri Jan 30 19:00:25 2015
       @author Mark Gates
*/
// includes, system
#include <stdlib.h>
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_tally3_sgetrf_gpu( M, N, d_A, ldda, ipiv, &info);
            gpu_time = magma_tally3_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally3_sgetrf_gpu returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
            ...
            if ( opts.check == 2 ) {
                magma_tally3_sgetmatrix( M, N, d_A, ldda, h_A, lda );
