...
int main( int argc, char** argv)
{
    ...
    magma_minproductFloatComplex *h_A, *h_R, *d_R, *h_work, aux_work[1];
    #ifdef COMPLEX
    float *rwork, aux_rwork[1];
    magma_minproduct_int_t lrwork;
    #endif
    float *w1, *w2, result[4]={0, 0, 0, 0}, eps;
    magma_minproduct_int_t *iwork, aux_iwork[1];
    magma_minproduct_int_t N, n2, info, lwork, liwork, lda, ldda;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_minproduct_cheevd_gpu( opts.jobz, opts.uplo,
                              N, NULL, ldda, NULL,
                              NULL, lda,
                              aux_work,  -1,
                              #ifdef COMPLEX
                              ...
                              aux_iwork, -1,
                              &info );
            lwork  = (magma_minproduct_int_t) MAGMA_minproduct_C_REAL( aux_work[0] );
            #ifdef COMPLEX
            lrwork = (magma_minproduct_int_t) aux_rwork[0];
            #endif
            liwork = aux_iwork[0];
            
            /* Allocate host memory for the matrix */
            TESTING_MALLOC_CPU( h_A,    magma_minproductFloatComplex, N*lda  );
            ...
            TESTING_MALLOC_PIN( h_R,    magma_minproductFloatComplex, N*lda  );
            TESTING_MALLOC_PIN( h_work, magma_minproductFloatComplex, lwork  );
            
            TESTING_MALLOC_DEV( d_R,    magma_minproductFloatComplex, N*ldda );
            
            /* Initialize the matrix */
            lapackf77_clarnv( &ione, ISEED, &n2, h_A );
            magma_minproduct_cmake_hermitian( N, h_A, N );
            
            magma_minproduct_csetmatrix( N, N, h_A, lda, d_R, ldda );
            ...
            if ( opts.warmup ) {
                magma_minproduct_cheevd_gpu( opts.jobz, opts.uplo,
                                  ...
                                  &info );
                if (info != 0)
                    printf("magma_minproduct_cheevd_gpu returned error %d: %s.\n",
            ...
            if ( opts.check && opts.jobz != Magma_minproductNoVec ) {
                /* =====================================================================
                   ...
                   =================================================================== */
                magma_minproduct_cgetmatrix( N, N, d_R, ldda, h_R, lda );
                
                magma_minproductFloatComplex *work;
                TESTING_MALLOC_CPU( work, magma_minproductFloatComplex, 2*N*N );
                ...
                //
                //magma_minproduct_csetmatrix( N, N, h_A, lda, d_R, ldda );
                //magma_minproduct_cheevd_gpu( Magma_minproductNoVec, opts.uplo,
                //                  N, d_R, ldda, w2,
                //                  h_R, lda,
                //                  h_work, lwork,
                //                  #ifdef COMPLEX
            ...
            if ( opts.lapack ) {
                printf("    | S_magma_minproduct - S_lapack | / |S| = %8.2e   %s\n\n", result[3], (result[3] < tolulp ? "ok" : "failed") );
