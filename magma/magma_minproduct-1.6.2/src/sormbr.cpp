...
/**
    Purpose
    -------
    If VECT = Magma_minproductQ, SORMBR overwrites the general real M-by-N matrix C with
    ...
    If VECT = Magma_minproductP, SORMBR overwrites the general real M-by-N matrix C with
                                 SIDE = Magma_minproductLeft     SIDE = Magma_minproductRight
    TRANS = Magma_minproductNoTrans:        P*C                  C*P
    TRANS = Magma_minproductTrans:     P**H*C               C*P**H
    ...
    trans   magma_minproduct_trans_t
      ...
      -     = Magma_minproductTrans: Conjugate transpose, apply Q**H or P**H.
    ...
    *info = 0;
    ...
    if (*info == 0) {
        if (nw > 0) {
            ...
            nb = magma_minproduct_get_sgebrd_nb( min( m, n ));
            lwkopt = max(1, nw*nb);
        }
        else {
            lwkopt = 1;
    ...
    if (applyq) {
        ...
        if (nq >= k) {
            ...
            #else
            magma_minproduct_sormqr( side, trans,
                          m, n, k, A, lda, tau, C, ldc, work, lwork, &iinfo);
            #endif
        }
        else if (nq > 1) {
        ...
        if (nq > k) {
            ...
            lapackf77_sormlq( lapack_side_const(side), lapack_trans_const(transt),
                              &m, &n, &k, A, &lda, tau, C, &ldc, work, &lwork, &iinfo);
            #else
            magma_minproduct_sormlq( side, transt,
                          m, n, k, A, lda, tau, C, ldc, work, lwork, &iinfo);
            #endif
        }
        else if (nq > 1) {
