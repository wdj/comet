...
int main( int argc, char** argv)
{
    ...
    magma_tally3DoubleComplex *h_A, *h_B, *h_X, *work, temp;
    real_Double_t   gflops, gpu_perf, gpu_time = 0.0, cpu_perf=0, cpu_time=0;
    double          error, error_lapack = 0.0;
    magma_tally3_int_t     *ipiv;
    magma_tally3_int_t     N, n2, lda, ldb, sizeB, lwork, info;
    magma_tally3_int_t     status = 0, ione = 1;
    magma_tally3_int_t     ISEED[4] = {0,0,0,1};
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_PIN( h_B,  magma_tally3DoubleComplex, sizeB );
            TESTING_MALLOC_PIN( h_X,  magma_tally3DoubleComplex, sizeB );
            ...
            if ( opts.lapack ) {
                lwork = -1;
                lapackf77_zhesv(lapack_uplo_const_tally3(opts.uplo), &N, &opts.nrhs, 
                ...
                lapackf77_zlacpy( Magma_tally3UpperLowerStr, &N, &opts.nrhs, h_B, &ldb, h_X, &ldb );
            ...
            magma_tally3_zhesv( opts.uplo, N, opts.nrhs, h_A, lda, ipiv, h_X, ldb, &info);
            gpu_time = magma_tally3_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally3_zhesv returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
