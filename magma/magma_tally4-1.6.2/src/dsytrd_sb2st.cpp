...
static void *magma_tally4_dsytrd_sb2st_parallel_section(void *arg);

static void magma_tally4_dtile_bulge_parallel(
    magma_tally4_int_t my_core_id, magma_tally4_int_t cores_num,
    double *A, magma_tally4_int_t lda,
    double *V, magma_tally4_int_t ldv,
    double *TAU, magma_tally4_int_t n, magma_tally4_int_t nb, magma_tally4_int_t nbtiles,
    magma_tally4_int_t grsiz, magma_tally4_int_t Vblksiz, volatile magma_tally4_int_t *prog);

static void magma_tally4_dtile_bulge_computeT_parallel(
    magma_tally4_int_t my_core_id, magma_tally4_int_t cores_num,
...
typedef struct magma_tally4_dbulge_data_s {
...
} magma_tally4_dbulge_data;

void magma_tally4_dbulge_data_init(
    magma_tally4_dbulge_data *dbulge_data_S,
...
void magma_tally4_dbulge_data_destroy(magma_tally4_dbulge_data *dbulge_data_S)
{
    pthread_barrier_destroy(&(dbulge_data_S->barrier));
}
typedef struct magma_tally4_dbulge_id_data_s {
    magma_tally4_int_t id;
    magma_tally4_dbulge_data* data;
} magma_tally4_dbulge_id_data;

void magma_tally4_dbulge_id_data_init(magma_tally4_dbulge_id_data *id_data, magma_tally4_int_t id, magma_tally4_dbulge_data* data)
{
    id_data->id = id;
    id_data->data = data;
}
...
magma_tally4_dsytrd_sb2st(
    ...
    double *A, magma_tally4_int_t lda, double *d, double *e,
    double *V, magma_tally4_int_t ldv, double *TAU,
    magma_tally4_int_t compT, double *T, magma_tally4_int_t ldt)
{
    #ifdef ENABLE_TIMER
    ...
    magma_tally4_dbulge_id_data* arg;
    magma_tally4_malloc_cpu((void**) &arg, threads*sizeof(magma_tally4_dbulge_id_data));

    pthread_t* thread_id;
    magma_tally4_malloc_cpu((void**) &thread_id, threads*sizeof(pthread_t));
    pthread_attr_t thread_attr;

    magma_tally4_dbulge_data data_bulge;
    magma_tally4_dbulge_data_init(&data_bulge, threads, n, nb, nbtiles, INgrsiz, Vblksiz, compT,
    ...
    for (magma_tally4_int_t thread = 1; thread < threads; thread++) {
        magma_tally4_dbulge_id_data_init(&(arg[thread]), thread, &data_bulge);
        pthread_create(&thread_id[thread], &thread_attr, magma_tally4_dsytrd_sb2st_parallel_section, &arg[thread]);
    }
    magma_tally4_dbulge_id_data_init(&(arg[0]), 0, &data_bulge);
    magma_tally4_dsytrd_sb2st_parallel_section(&arg[0]);
    ...
    magma_tally4_dbulge_data_destroy(&data_bulge);
    ...
    if (uplo == Magma_tally4Lower) {
        for (magma_tally4_int_t i=0; i < n-1; i++) {
            d[i] = MAGMA_tally4_D_REAL( A[i*lda  ] );
            e[i] = MAGMA_tally4_D_REAL( A[i*lda+1] );
        }
        d[n-1] = MAGMA_tally4_D_REAL(A[(n-1)*lda]);
    } else { /* Magma_tally4Upper not tested yet */
        for (magma_tally4_int_t i=0; i < n-1; i++) {
            d[i] = MAGMA_tally4_D_REAL( A[i*lda+nb]   );
            e[i] = MAGMA_tally4_D_REAL( A[i*lda+nb-1] );
        }
        d[n-1] = MAGMA_tally4_D_REAL(A[(n-1)*lda+nb]);
...
static void *magma_tally4_dsytrd_sb2st_parallel_section(void *arg)
{
    magma_tally4_int_t my_core_id  = ((magma_tally4_dbulge_id_data*)arg) -> id;
    magma_tally4_dbulge_data* data = ((magma_tally4_dbulge_id_data*)arg) -> data;
...
//#define PRINTAFFINITY
    ...
    if (compT == 1) {
        ...
        if (allcores_num == 1) {
            ...
            magma_tally4_dtile_bulge_parallel(0, 1, A, lda, V, ldv, TAU, n, nb, nbtiles, grsiz, Vblksiz, prog);
            ...
            magma_tally4_dtile_bulge_computeT_parallel(0, 1, V, ldv, TAU, T, ldt, n, nb, Vblksiz);
            ...
            magma_tally4_int_t tot = allcores_num;
                ...
                magma_tally4_dtile_bulge_parallel(id, tot, A, lda, V, ldv, TAU, n, nb, nbtiles, grsiz, Vblksiz, prog);
                ...
                magma_tally4_dtile_bulge_computeT_parallel(id, tot, V, ldv, TAU, T, ldt, n, nb, Vblksiz);
                pthread_barrier_wait(barrier);
        ...
        magma_tally4_dtile_bulge_parallel(my_core_id, allcores_num, A, lda, V, ldv, TAU, n, nb, nbtiles, grsiz, Vblksiz, prog);
        pthread_barrier_wait(barrier);
                            ...
                            while(fin == 0) {
                                if (myid == 1) {
                                    if ( prog[myid+shift-1] == (sweepid-1) ) {
                                        magma_tally4_dtrdtype1cbHLsym_withQ_v2(n, nb, A, lda, V, ldv, TAU, stind, edind, sweepid, Vblksiz, work);
                                    ...
                                    if ( (prog[myid-1] == sweepid) && (prog[myid+shift-1] == (sweepid-1)) ) {
                                        if (myid%2 == 0)
                                            magma_tally4_dtrdtype2cbHLsym_withQ_v2(n, nb, A, lda, V, ldv, TAU, stind, edind, sweepid, Vblksiz, work);
                                        else
                                            magma_tally4_dtrdtype3cbHLsym_withQ_v2(n, nb, A, lda, V, ldv, TAU, stind, edind, sweepid, Vblksiz, work);
