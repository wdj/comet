    ...
    magma_tally4_queue_t queue )
{
    ...
    // solver variables
    magma_tally4FloatComplex alpha, beta;
    ...
    CHECK( magma_tally4_cvinit( &rt,Magma_tally4_DEV, A.num_rows, b.num_cols, c_zero, queue ));
    CHECK( magma_tally4_cvinit( &p, Magma_tally4_DEV, A.num_rows, b.num_cols, c_zero, queue ));
    CHECK( magma_tally4_cvinit( &q, Magma_tally4_DEV, A.num_rows, b.num_cols, c_zero, queue ));
    CHECK( magma_tally4_cvinit( &h, Magma_tally4_DEV, A.num_rows, b.num_cols, c_zero, queue ));
    ...
    magma_tally4_ccopy( dofs, h.dval, 1, p.dval, 1 );                    // p = h
    nom = MAGMA_tally4_C_REAL( magma_tally4_cdotc(dofs, r.dval, 1, h.dval, 1) );
    CHECK( magma_tally4_c_spmv( c_one, A, p, c_zero, q, queue ));             // q = A p
    den = MAGMA_tally4_C_REAL( magma_tally4_cdotc(dofs, p.dval, 1, q.dval, 1) );// den = p dot q
    ...
    do
    {
        ...
        gammanew = MAGMA_tally4_C_REAL( magma_tally4_cdotc(dofs, r.dval, 1, h.dval, 1) );
                                                            // gn = < r,h>

        if ( solver_par->numiter==1 ) {
            magma_tally4_ccopy( dofs, h.dval, 1, p.dval, 1 );                    // p = h
        } else {
            beta = MAGMA_tally4_C_MAKE(gammanew/gammaold, 0.);       // beta = gn/go
            magma_tally4_cscal(dofs, beta, p.dval, 1);            // p = beta*p
            magma_tally4_caxpy(dofs, c_one, h.dval, 1, p.dval, 1); // p = p + h
        }

        CHECK( magma_tally4_c_spmv( c_one, A, p, c_zero, q, queue ));   // q = A p
        den = MAGMA_tally4_C_REAL(magma_tally4_cdotc(dofs, p.dval, 1, q.dval, 1));
                // den = p dot q

        alpha = MAGMA_tally4_C_MAKE(gammanew/den, 0.);
        magma_tally4_caxpy(dofs,  alpha, p.dval, 1, x->dval, 1);     // x = x + alpha p
        magma_tally4_caxpy(dofs, -alpha, q.dval, 1, r.dval, 1);      // r = r - alpha q
        gammaold = gammanew;

        res = magma_tally4_scnrm2( dofs, r.dval, 1 );
        if ( solver_par->verbose > 0 ) {
