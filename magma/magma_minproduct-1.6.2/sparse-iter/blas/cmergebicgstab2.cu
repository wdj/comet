...
__global__ void
magma_minproduct_creduce_kernel_spmv1(    
    int Gs,
    int n, 
    ...
    magma_minproductFloatComplex * vtmp)
{
    ...
    temp[ Idx ] = ( i < n ) ? v[ i ] * r[ i ] : MAGMA_minproduct_C_MAKE( 0.0, 0.0);
    __syncthreads();
    if ( Idx < 128 ){
...
__global__ void
magma_minproduct_cbicgstab_alphakernel(  
                    magma_minproductFloatComplex * skp ){
    int i = blockIdx.x * blockDim.x + threadIdx.x;

    if( i==0 ){
        magma_minproductFloatComplex tmp = skp[0];
        skp[0] = skp[4]/tmp;
    }
}
...
/**
    ...
    @param[in]
    dp          magma_minproductFloatComplex_ptr
                input vector p
    ...
    @param[in]
    dv          magma_minproductFloatComplex_ptr
                output vector v
    ...
    magma_minproduct_queue_t queue )
{
    ...
    if ( A.storage_type == Magma_minproduct_CSR)
        magma_minproduct_cbicgmerge_spmv1_kernel<<<Gs, Bs, Ms>>>
    ...
    while( Gs.x > 1 ) {
        ...
        magma_minproduct_creduce_kernel_spmv1<<< Gs_next.x/2, Bs.x/2, Ms/2 >>> 
                            ( Gs.x, n, aux1, aux2 );
    ...
    magma_minproduct_ccopyvector( 1, aux1, 1, skp, 1 );
    dim3 Bs2( 2 );
    dim3 Gs2( 1 );
    magma_minproduct_cbicgstab_alphakernel<<<Gs2, Bs2, 0>>>( skp );
...
__global__ void
magma_minproduct_creduce_kernel_spmv2( 
    int Gs,
    int n, 
    magma_minproductFloatComplex * vtmp,
    magma_minproductFloatComplex * vtmp2 )
{
    ...
    for( j=0; j<2; j++){
        int i = blockIdx.x * ( blockSize * 2 ) + Idx;   
        temp[Idx+j*(blockSize)] = MAGMA_minproduct_C_MAKE( 0.0, 0.0);
        while (i < Gs ) {
...
__global__ void
magma_minproduct_cbicgmerge_spmv2_kernel(  
    int n,
    magma_minproductFloatComplex * dval, 
    magma_minproduct_index_t * drowptr, 
    magma_minproduct_index_t * dcolind,
    magma_minproductFloatComplex * s,
    ...
    magma_minproductFloatComplex * vtmp )
{
    ...
    // 2 vectors 
    if (i<n){
            magma_minproductFloatComplex tmp2 = t[i];
            temp[Idx] = s[i] * tmp2;
            temp[Idx+blockDim.x] = tmp2 * tmp2;
    }
    else{
        for( j=0; j<2; j++)
            temp[Idx+j*blockDim.x] =MAGMA_minproduct_C_MAKE( 0.0, 0.0);
    }
    __syncthreads();
    if ( Idx < 128 ){
        for( j=0; j<2; j++){
...
__global__ void
magma_minproduct_cbicgstab_omegakernel(  
                    magma_minproductFloatComplex * skp ){
    int i = blockIdx.x * blockDim.x + threadIdx.x;
...
/**
    ...
    @param[in]
    ds          magma_minproductFloatComplex_ptr
                input vector s

    @param[in]
    dt          magma_minproductFloatComplex_ptr
                output vector t
    ...
    magma_minproduct_queue_t queue )
{
    ...
    int b = 1;        
    if ( A.storage_type == Magma_minproduct_CSR)
        magma_minproduct_cbicgmerge_spmv2_kernel<<<Gs, Bs, Ms>>>
    ...
    while( Gs.x > 1 ) {
        Gs_next.x = ( Gs.x+Bs.x-1 )/ Bs.x ;
        if ( Gs_next.x == 1 ) Gs_next.x = 2;
        magma_minproduct_creduce_kernel_spmv2<<< Gs_next.x/2, Bs.x/2, Ms/2 >>> 
                    ( Gs.x, n, aux1, aux2 );
        Gs_next.x = Gs_next.x /2;
        Gs.x = Gs_next.x;
        b = 1 - b;
    ...
    magma_minproduct_ccopyvector( 1, aux1, 1, skp+6, 1 );
    magma_minproduct_ccopyvector( 1, aux1+n, 1, skp+7, 1 );
    dim3 Bs2( 2 );
    dim3 Gs2( 1 );
    magma_minproduct_cbicgstab_omegakernel<<<Gs2, Bs2, 0>>>( skp );
...
__global__ void
magma_minproduct_cbicgmerge_xrbeta_kernel(  
    int n, 
    magma_minproductFloatComplex * rr,
    magma_minproductFloatComplex * r,
    magma_minproductFloatComplex * p,
    magma_minproductFloatComplex * s,
    magma_minproductFloatComplex * t,
    ...
    magma_minproductFloatComplex * vtmp )
{
    ...
    magma_minproductFloatComplex omega=skp[2];

    if( i<n ){
        magma_minproductFloatComplex sl;
        sl = s[i];
    ...
    // 2 vectors 
    if (i<n){
            magma_minproductFloatComplex tmp2 = r[i];
            temp[Idx] = rr[i] * tmp2;
            temp[Idx+blockDim.x] = tmp2 * tmp2;
    }
    else{
...
__global__ void
magma_minproduct_cbicgstab_betakernel(  
    magma_minproductFloatComplex * skp )
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;

    if( i==0 ){
        magma_minproductFloatComplex tmp1 = skp[4]/skp[3];
        magma_minproductFloatComplex tmp2 = skp[0] / skp[2];
        skp[1] =  tmp1*tmp2;
    }
}
...
/**
    ...
    @param[in]
    rr          magma_minproductFloatComplex_ptr
                input vector rr
    ...
    @param[in]
    p           magma_minproductFloatComplex_ptr
                input vector p

    @param[in]
    s           magma_minproductFloatComplex_ptr
                input vector s

    @param[in]
    t           magma_minproductFloatComplex_ptr
                input vector t

    @param[out]
    x           magma_minproductFloatComplex_ptr
                output vector x
...
magma_minproduct_cbicgmerge_xrbeta(
    ...
    magma_minproductFloatComplex_ptr d1,
    magma_minproductFloatComplex_ptr d2,
    magma_minproductFloatComplex_ptr rr,
    magma_minproductFloatComplex_ptr r,
    magma_minproductFloatComplex_ptr p,
    magma_minproductFloatComplex_ptr s,
    magma_minproductFloatComplex_ptr t,
    ...
    magma_minproduct_queue_t queue )
{
    ...
    magma_minproductFloatComplex_ptr aux1 = d1, aux2 = d2;
    int b = 1;        
    magma_minproduct_cbicgmerge_xrbeta_kernel<<<Gs, Bs, Ms>>>
    ...
    magma_minproduct_ccopyvector( 1, aux1+n, 1, skp+5, 1 );
    dim3 Bs2( 2 );
    dim3 Gs2( 1 );
    magma_minproduct_cbicgstab_betakernel<<<Gs2, Bs2, 0>>>( skp );
