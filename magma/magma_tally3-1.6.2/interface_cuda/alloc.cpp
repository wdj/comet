...
magma_tally3_malloc( magma_tally3_ptr* ptrPtr, size_t size )
{
    // CUDA can't allocate 0 bytes, so allocate some minimal size
    if ( size == 0 )
        size = sizeof(magma_tally3DoubleComplex);
...
// --------------------
// Free GPU memory allocated by magma_tally3_malloc.
extern "C" magma_tally3_int_t
magma_tally3_free_internal( magma_tally3_ptr ptr,
...
// Use magma_tally3_free_cpu() to free this memory.
extern "C" magma_tally3_int_t
magma_tally3_malloc_cpu( void** ptrPtr, size_t size )
{
    ...
    *ptrPtr = _aligned_malloc( size, 32 );
    if ( *ptrPtr == NULL ) {
        return MAGMA_tally3_ERR_HOST_ALLOC;
    }
#else
...
// --------------------
// Free CPU pinned memory previously allocated by magma_tally3_malloc_pinned.
...
extern "C" magma_tally3_int_t
magma_tally3_malloc_pinned( void** ptrPtr, size_t size )
{
    // CUDA can't allocate 0 bytes, so allocate some minimal size
...
// Free CPU pinned memory previously allocated by magma_tally3_malloc_pinned.
extern "C" magma_tally3_int_t
magma_tally3_free_pinned_internal( void* ptr,
    const char* func, const char* file, int line )
{
