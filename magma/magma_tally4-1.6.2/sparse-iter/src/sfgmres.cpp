...
GeneratePlaneRotation(float dx, float dy, float *cs, float *sn)
{
    ...
    if (dy == MAGMA_tally4_S_ZERO) {
        *cs = MAGMA_tally4_S_ONE;
        *sn = MAGMA_tally4_S_ZERO;
    } else if (MAGMA_tally4_S_ABS((dy)) > MAGMA_tally4_S_ABS((dx))) {
        float temp = dx / dy;
        *sn = MAGMA_tally4_S_ONE / magma_tally4_ssqrt( ( MAGMA_tally4_S_ONE + temp*temp)) ;
        *cs = temp * *sn;
    } else {
        float temp = dy / dx;
        *cs = MAGMA_tally4_S_ONE / magma_tally4_ssqrt( ( MAGMA_tally4_S_ONE + temp*temp )) ;
        *sn = temp * *cs;
    }

  //  real_Double_t rho = sqrt(MAGMA_tally4_S_REAL(MAGMA_tally4_S_CNJG(dx)*dx + MAGMA_tally4_S_CNJG(dy)*dy));
  //  *cs = dx / rho;
  //  *sn = dy / rho;
}
...
static void ApplyPlaneRotation(float *dx, float *dy, float cs, float sn)
{
 ...
 //   float temp  =  MAGMA_tally4_S_CNJG(cs) * *dx +  MAGMA_tally4_S_CNJG(sn) * *dy;
 //   *dy = -(sn) * *dx + cs * *dy;
 //   *dx = temp;
}
    ...
    magma_tally4_queue_t queue )
{
    ...
    magma_tally4_s_matrix v_t={Magma_tally4_CSR}, w_t={Magma_tally4_CSR}, t={Magma_tally4_CSR}, t2={Magma_tally4_CSR}, V={Magma_tally4_CSR}, W={Magma_tally4_CSR};
    ...
    CHECK( magma_tally4_svinit( &t, Magma_tally4_DEV, dofs, 1, MAGMA_tally4_S_ZERO, queue ));
    CHECK( magma_tally4_svinit( &t2, Magma_tally4_DEV, dofs, 1, MAGMA_tally4_S_ZERO, queue ));
    
    CHECK( magma_tally4_smalloc_pinned( &H, (dim+1)*dim ));
    CHECK( magma_tally4_smalloc_pinned( &s,  dim+1 ));
    CHECK( magma_tally4_smalloc_pinned( &cs, dim ));
    CHECK( magma_tally4_smalloc_pinned( &sn, dim ));
    
    
    CHECK( magma_tally4_svinit( &V, Magma_tally4_DEV, dofs*(dim+1), 1, MAGMA_tally4_S_ZERO, queue ));
    CHECK( magma_tally4_svinit( &W, Magma_tally4_DEV, dofs*dim, 1, MAGMA_tally4_S_ZERO, queue ));
    
    CHECK(  magma_tally4_sresidual( A, b, *x, &nom, queue));
    ...
    do
    {
        ...
        CHECK( magma_tally4_s_spmv( MAGMA_tally4_S_ONE, A, *x, MAGMA_tally4_S_ZERO, t, queue ));
        magma_tally4_scopy( dofs, t.dval, 1, V(0), 1 );
        
        temp = MAGMA_tally4_S_MAKE(-1.0, 0.0);
        magma_tally4_saxpy(dofs,temp, b.dval, 1, V(0), 1);           // V(0) = V(0) - b
        beta = MAGMA_tally4_S_MAKE( magma_tally4_snrm2( dofs, V(0), 1 ), 0.0); // beta = norm(V(0))
        if (solver_par->numiter == 0){
            solver_par->init_res = MAGMA_tally4_S_REAL( beta );
            resid0 = MAGMA_tally4_S_REAL( beta );
        ...
        temp = -1.0/beta;
        magma_tally4_sscal( dofs, temp, V(0), 1 );                 // V(0) = -V(0)/beta
        ...
        do
        {
            ...
            v_t.dval = V(i);
            CHECK( magma_tally4_s_applyprecond_left( A, v_t, &t, precond_par, queue ));
            CHECK( magma_tally4_s_applyprecond_right( A, t, &t2, precond_par, queue ));
            magma_tally4_scopy( dofs, t2.dval, 1, W(i), 1 );

            // A.mult(n, 1, W(i), n, V(i+1), n);
            w_t.dval = W(i);
            CHECK( magma_tally4_s_spmv( MAGMA_tally4_S_ONE, A, w_t, MAGMA_tally4_S_ZERO, t, queue ));
            magma_tally4_scopy( dofs, t.dval, 1, V(i+1), 1 );
            
            for (k = 0; k <= i; k++)
            {
                H(k, i) = magma_tally4_sdot(dofs, V(k), 1, V(i+1), 1);
                temp = -H(k,i);
                // V(i+1) -= H(k, i) * V(k);
                magma_tally4_saxpy(dofs,-H(k,i), V(k), 1, V(i+1), 1);
            }

            H(i+1, i) = MAGMA_tally4_S_MAKE( magma_tally4_snrm2(dofs, V(i+1), 1), 0. ); // H(i+1,i) = ||r||
            temp = 1.0 / H(i+1, i);
            // V(i+1) = V(i+1) / H(i+1, i)
            magma_tally4_sscal(dofs, temp, V(i+1), 1);    //  (to be fused)
            ...
            betanom = MAGMA_tally4_S_ABS( s[i+1] );
        ...
        {
            // x = x + s[j] * W(j)
            magma_tally4_saxpy(dofs, s[j], W(j), 1, x->dval, 1);
        }
    }
...
cleanup:
    ...
    magma_tally4_smfree( &W, queue);
    magma_tally4_smfree( &t, queue);
    magma_tally4_smfree( &t2, queue);
