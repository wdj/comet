...
int main(  int argc, char** argv )
{
    ...
    magma_tally4_c_matrix hA={Magma_tally4_CSR}, hA_SELLP={Magma_tally4_CSR}, hA_ELL={Magma_tally4_CSR}, 
    dA={Magma_tally4_CSR}, dA_SELLP={Magma_tally4_CSR}, dA_ELL={Magma_tally4_CSR};
    
    magma_tally4_c_matrix hx={Magma_tally4_CSR}, hy={Magma_tally4_CSR}, dx={Magma_tally4_CSR}, 
    ...
    magma_tally4FloatComplex c_one  = MAGMA_tally4_C_MAKE(1.0, 0.0);
    magma_tally4FloatComplex c_zero = MAGMA_tally4_C_MAKE(0.0, 0.0);
    ...
    while( i < argc ) {
        if ( strcmp("LAPLACE2D", argv[i]) == 0 && i+1 < argc ) {   // Laplace test
            ...
            CHECK( magma_tally4_cm_5stencil(  laplace_size, &hA, queue ));
        } else {                        // file-matrix test
            CHECK( magma_tally4_c_csr_mtx( &hA,  argv[i], queue ));
        }
        ...
        // init CPU vectors
        CHECK( magma_tally4_cvinit( &hx, Magma_tally4_CPU, hA.num_rows, 1, c_zero, queue ));
        CHECK( magma_tally4_cvinit( &hy, Magma_tally4_CPU, hA.num_rows, 1, c_zero, queue ));

        // init DEV vectors
        CHECK( magma_tally4_cvinit( &dx, Magma_tally4_DEV, hA.num_rows, 1, c_one, queue ));
        CHECK( magma_tally4_cvinit( &dy, Magma_tally4_DEV, hA.num_rows, 1, c_zero, queue ));
        ...
        // copy matrix to GPU
        CHECK( magma_tally4_cmtransfer( hA, &dA, Magma_tally4_CPU, Magma_tally4_DEV, queue ));
        // SpMV on GPU (CSR) -- this is the reference!
        start = magma_tally4_sync_wtime( queue );
        for (j=0; j<10; j++)
            CHECK( magma_tally4_c_spmv( c_one, dA, dx, c_zero, dy, queue ));
        end = magma_tally4_sync_wtime( queue );
        printf( " > MAGMA_tally4: %.2e seconds %.2e GFLOP/s    (standard CSR).\n",
                                        (end-start)/10, FLOPS*10/(end-start) );
        magma_tally4_cmfree(&dA, queue );
        CHECK( magma_tally4_cmtransfer( dy, &hrefvec , Magma_tally4_DEV, Magma_tally4_CPU, queue ));

        // convert to ELL and copy to GPU
        CHECK( magma_tally4_cmconvert(  hA, &hA_ELL, Magma_tally4_CSR, Magma_tally4_ELL, queue ));
        CHECK( magma_tally4_cmtransfer( hA_ELL, &dA_ELL, Magma_tally4_CPU, Magma_tally4_DEV, queue ));
        magma_tally4_cmfree(&hA_ELL, queue );
        magma_tally4_cmfree( &dy, queue );
        CHECK( magma_tally4_cvinit( &dy, Magma_tally4_DEV, hA.num_rows, 1, c_zero, queue ));
        // SpMV on GPU (ELL)
        start = magma_tally4_sync_wtime( queue );
        for (j=0; j<10; j++)
            CHECK( magma_tally4_c_spmv( c_one, dA_ELL, dx, c_zero, dy, queue ));
        end = magma_tally4_sync_wtime( queue );
        printf( " > MAGMA_tally4: %.2e seconds %.2e GFLOP/s    (standard ELL).\n",
                                        (end-start)/10, FLOPS*10/(end-start) );
        magma_tally4_cmfree(&dA_ELL, queue );
        CHECK( magma_tally4_cmtransfer( dy, &hcheck , Magma_tally4_DEV, Magma_tally4_CPU, queue ));
        res = 0.0;
        for(magma_tally4_int_t k=0; k<hA.num_rows; k++ )
            res=res + MAGMA_tally4_C_REAL(hcheck.val[k]) - MAGMA_tally4_C_REAL(hrefvec.val[k]);
        if ( res < .000001 )
            printf("# tester spmv ELL:  ok\n");
        else
            printf("# tester spmv ELL:  failed\n");
        magma_tally4_cmfree( &hcheck, queue );

        // convert to SELLP and copy to GPU
        CHECK( magma_tally4_cmconvert(  hA, &hA_SELLP, Magma_tally4_CSR, Magma_tally4_SELLP, queue ));
        CHECK( magma_tally4_cmtransfer( hA_SELLP, &dA_SELLP, Magma_tally4_CPU, Magma_tally4_DEV, queue ));
        magma_tally4_cmfree(&hA_SELLP, queue );
        magma_tally4_cmfree( &dy, queue );
        CHECK( magma_tally4_cvinit( &dy, Magma_tally4_DEV, hA.num_rows, 1, c_zero, queue ));
        // SpMV on GPU (SELLP)
        start = magma_tally4_sync_wtime( queue );
        for (j=0; j<10; j++)
            CHECK( magma_tally4_c_spmv( c_one, dA_SELLP, dx, c_zero, dy, queue ));
        end = magma_tally4_sync_wtime( queue );
        ...
        magma_tally4_cmfree(&dA_SELLP, queue );
        ...
        magma_tally4FloatComplex alpha = c_one;
        magma_tally4FloatComplex beta = c_zero;
        magma_tally4_cmfree( &dy, queue );
        CHECK( magma_tally4_cvinit( &dy, Magma_tally4_DEV, hA.num_rows, 1, c_zero, queue ));
        ...
        magma_tally4_cmfree(&hx, queue );
        magma_tally4_cmfree(&hy, queue );
        magma_tally4_cmfree(&hrefvec, queue );
        // free GPU memory
        magma_tally4_cmfree(&dA, queue );
        magma_tally4_cmfree(&dx, queue );
        magma_tally4_cmfree(&dy, queue );
