...
/**
    ...
    This version stores the T matrices, for later use in magma_tally2_cunghr.
    ...
    if (nb == 1 || nb >= nh) {
        ...
        for( d = 0; d < ngpu; ++d ) {
            ...
            ldwork = nlocal*ldda   // A
                   ...
                   + nb*ldda       // W
                   + nb*nb;        // Ti
            if ( MAGMA_tally2_SUCCESS != magma_tally2_cmalloc( &data.A[d], ldwork )) {
        ...
        magma_tally2_csetmatrix_1D_col_bcyclic( n, n, A, lda, data.A, ldda, ngpu, nb );
        ...
        for (i = ilo; i < ihi - 1 - nb; i += nb) {
            ...
            magma_tally2_clahr2_m( ihi, i+1, nb, A(0,i), lda,
                            &tau[i], &T[i*nb], nb, work, n, &data );
            
            magma_tally2_clahru_m( n, ihi, i, nb, A, lda, &data );
            
            // copy first i rows above panel to host
            magma_tally2_setdevice( dpanel );
            magma_tally2_cgetmatrix_async( i, nb,
                                    dA(dpanel, 0, di), ldda,
