...
int main( int argc, char** argv)
{
    ...
    magma_tally4DoubleComplex *h_x, *h_x2, *h_tau, *h_tau2;
    magma_tally4DoubleComplex_ptr d_x, d_tau;
    magma_tally4DoubleComplex c_neg_one = MAGMA_tally4_Z_NEG_ONE;
    double      error, error2, work[1];
    magma_tally4_int_t N, nb, lda, ldda, size;
    magma_tally4_int_t ione     = 1;
    magma_tally4_int_t ISEED[4] = {0,0,0,1};    magma_tally4_int_t status = 0;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_x,    magma_tally4DoubleComplex, N*nb );
            TESTING_MALLOC_CPU( h_x2,   magma_tally4DoubleComplex, N*nb );
            TESTING_MALLOC_CPU( h_tau,  magma_tally4DoubleComplex, nb   );
            TESTING_MALLOC_CPU( h_tau2, magma_tally4DoubleComplex, nb   );
        
            TESTING_MALLOC_DEV( d_x,   magma_tally4DoubleComplex, ldda*nb );
            TESTING_MALLOC_DEV( d_tau, magma_tally4DoubleComplex, nb      );
            ...
            /* =====================================================================
               ...
               =================================================================== */
            magma_tally4_zsetmatrix( N, nb, h_x, N, d_x, ldda );
    
            gpu_time = magma_tally4_sync_wtime( queue );
            for( int j = 0; j < nb; ++j ) {
                magma_tally4blas_zlarfg( N, &d_x[0+j*ldda], &d_x[1+j*ldda], ione, &d_tau[j] );
            }
            gpu_time = magma_tally4_sync_wtime( queue ) - gpu_time;
            gpu_perf = gflops / gpu_time;
            
            magma_tally4_zgetmatrix( N, nb, d_x, ldda, h_x2, N );
            magma_tally4_zgetvector( nb, d_tau, 1, h_tau2, 1 );
