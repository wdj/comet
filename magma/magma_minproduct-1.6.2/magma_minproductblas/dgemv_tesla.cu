...
extern "C" void
magma_minproductblas_dgemvt_tesla(
...
/**
    ...
    1) y =       A   x      if trans == Magma_minproductNoTrans, alpha == 1, beta == 0,
                            and incx == incy == 1 (using magma_minproductblas code)
    2) y = alpha A^T x      if trans == Magma_minproductTrans, beta == 0,
                            and incx == incy == 1 (using magma_minproductblas code)
    ...
    trans   magma_minproduct_trans_t
            ...
            follows:
      -     = Magma_minproductNoTrans:    y := alpha*A  *x + beta*y
      -     = Magma_minproductTrans:      y := alpha*A^T*x + beta*y
      -     = Magma_minproductConjTrans:  y := alpha*A^T*x + beta*y
    ...
    ********************************************************************/
extern "C" void
magma_minproductblas_dgemv_tesla(
    ...
    if ( incx == 1 && incy == 1 && beta == 0 ) {
        if ( trans == Magma_minproductNoTrans ) {
            if ( alpha == 1. ) {
                ...
                dgemv_kernel_tesla<<< grid, threads, 0, magma_minproduct_stream >>>
                    (m, n, (n/gemv_bs)*gemv_bs, A, lda, x, y);
            }
            else {
                magma_minproduct_dgemv( trans, m, n, alpha, A, lda, x, incx, beta, y, incy);
            }
        }
        else {
            magma_minproductblas_dgemvt_tesla(m, n, alpha, A, lda, x, y);
        }
    }
    else {
        magma_minproduct_dgemv( trans, m, n, alpha, A, lda, x, incx, beta, y, incy);
...
/**
    ...
    @ingroup magma_minproduct_dblas2_internal
    ********************************************************************/
extern "C" void
magma_minproductblas_dgemvt1_tesla(
    magma_minproduct_int_t m, magma_minproduct_int_t n, double alpha,
    const double *A, magma_minproduct_int_t lda,
    const double *x,
    double       *y )
{
    magma_minproduct_int_t blocks = (n - 1)/32 + 1;
    dim3 grid( blocks, 1, 1 );
    dim3 threads( 32, 2, 1 );
    dgemvt_kernel1_tesla<<< grid, threads, 0, magma_minproduct_stream >>>
...
/**
    ...
    ********************************************************************/
extern "C" void
magma_minproductblas_dgemvt2_tesla(
    magma_minproduct_int_t m, magma_minproduct_int_t n, double alpha,
    const double *A, magma_minproduct_int_t lda,
    const double *x,
    double       *y )
{
    magma_minproduct_int_t blocks = (n - 1)/16 + 1;
    dim3 grid( blocks, 1, 1 );
    dim3 threads( 16, 4, 1 );
    dgemvt_kernel2_tesla<<< grid, threads, 0, magma_minproduct_stream >>>
...
{
    if ( n <= 128 )
        magma_minproductblas_dgemvt2_tesla(m, n, alpha, A, lda, x, y);
    else
        magma_minproductblas_dgemvt1_tesla(m, n, alpha, A, lda, x, y);
}
