...
/**
    ...
    This version stores the T matrices, for later use in magma_minproduct_cunghr.
    ...
    if (nb == 1 || nb >= nh) {
        ...
        for( d = 0; d < ngpu; ++d ) {
            ...
            ldwork = nlocal*ldda   // A
                   ...
                   + nb*ldda       // W
                   + nb*nb;        // Ti
            if ( MAGMA_minproduct_SUCCESS != magma_minproduct_cmalloc( &data.A[d], ldwork )) {
        ...
        magma_minproduct_csetmatrix_1D_col_bcyclic( n, n, A, lda, data.A, ldda, ngpu, nb );
        ...
        for (i = ilo; i < ihi - 1 - nb; i += nb) {
            ...
            magma_minproduct_clahr2_m( ihi, i+1, nb, A(0,i), lda,
                            &tau[i], &T[i*nb], nb, work, n, &data );
            
            magma_minproduct_clahru_m( n, ihi, i, nb, A, lda, &data );
            
            // copy first i rows above panel to host
            magma_minproduct_setdevice( dpanel );
            magma_minproduct_cgetmatrix_async( i, nb,
                                    dA(dpanel, 0, di), ldda,
        ...
        for( i2 = i; i2 < n; i2 += nb ) {
            ...
            di = (i2 / nb) / ngpu * nb;
            magma_minproduct_setdevice( d );
            magma_minproduct_cgetmatrix( n, ib,
                              dA(d, 0, di), ldda,
                              A(0,i2),      lda );
        }
    }
