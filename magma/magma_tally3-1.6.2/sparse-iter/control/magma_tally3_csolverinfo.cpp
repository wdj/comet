/*
    -- MAGMA_tally3 (version 1.6.2) --
       ...
       @generated from magma_tally3_zsolverinfo.cpp normal z -> c, Sun May  3 11:23:01 2015
       @author Hartwig Anzt
    ...
    magma_tally3_queue_t queue )
{
    if( solver_par->verbose > 0 ){
        magma_tally3_int_t k = solver_par->verbose;
        printf("%%======================================================="
            "======%%\n");
        switch( solver_par->solver ) {
            ...
            case  Magma_tally3_BICGSTABMERGE:
                    printf("%%   BiCGSTAB (merged) performance analysis every %d iteration\n",
                                                                        (int) k); break;
            case  Magma_tally3_BICGSTABMERGE2:
                    printf("%%   BiCGSTAB (merged) performance analysis every %d iteration\n",
                                                                        (int) k); break;
    ...
    magma_tally3_queue_t queue )
{
    ...
    if ( solver_par->res_vec != NULL ) {
        magma_tally3_free_cpu( solver_par->res_vec );
        solver_par->res_vec = NULL;
    }
    if ( solver_par->timing != NULL ) {
        magma_tally3_free_cpu( solver_par->timing );
        solver_par->timing = NULL;
    }
    if ( solver_par->eigenvectors != NULL ) {
        magma_tally3_free( solver_par->eigenvectors );
        solver_par->eigenvectors = NULL;
    }
    if ( solver_par->eigenvalues != NULL ) {
        magma_tally3_free_cpu( solver_par->eigenvalues );
        solver_par->eigenvalues = NULL;
    }
    if ( precond_par->d.val != NULL ) {
        magma_tally3_free( precond_par->d.val );
        precond_par->d.val = NULL;
    }
    if ( precond_par->d2.val != NULL ) {
        magma_tally3_free( precond_par->d2.val );
        precond_par->d2.val = NULL;
    }
    if ( precond_par->work1.val != NULL ) {
        magma_tally3_free( precond_par->work1.val );
        precond_par->work1.val = NULL;
    }
    if ( precond_par->work2.val != NULL ) {
        magma_tally3_free( precond_par->work2.val );
        precond_par->work2.val = NULL;
    }
    if ( precond_par->M.val != NULL ) {
        if ( precond_par->M.memory_location == Magma_tally3_DEV )
            magma_tally3_free( precond_par->M.dval );
        else
            magma_tally3_free_cpu( precond_par->M.val );
        precond_par->M.val = NULL;
    }
    if ( precond_par->M.col != NULL ) {
        if ( precond_par->M.memory_location == Magma_tally3_DEV )
            magma_tally3_free( precond_par->M.dcol );
        else
            magma_tally3_free_cpu( precond_par->M.col );
        precond_par->M.col = NULL;
    }
    if ( precond_par->M.row != NULL ) {
        if ( precond_par->M.memory_location == Magma_tally3_DEV )
            magma_tally3_free( precond_par->M.drow );
        else
            magma_tally3_free_cpu( precond_par->M.row );
        precond_par->M.row = NULL;
    }
    if ( precond_par->M.blockinfo != NULL ) {
        magma_tally3_free_cpu( precond_par->M.blockinfo );
        precond_par->M.blockinfo = NULL;
    }
    if ( precond_par->L.val != NULL ) {
        if ( precond_par->L.memory_location == Magma_tally3_DEV )
            magma_tally3_free( precond_par->L.dval );
        else
            magma_tally3_free_cpu( precond_par->L.val );
        precond_par->L.val = NULL;
    }
    if ( precond_par->L.col != NULL ) {
        if ( precond_par->L.memory_location == Magma_tally3_DEV )
            magma_tally3_free( precond_par->L.col );
        else
            magma_tally3_free_cpu( precond_par->L.dcol );
        precond_par->L.col = NULL;
    }
    if ( precond_par->L.row != NULL ) {
        if ( precond_par->L.memory_location == Magma_tally3_DEV )
            magma_tally3_free( precond_par->L.drow );
        else
            magma_tally3_free_cpu( precond_par->L.row );
        precond_par->L.row = NULL;
    }
    if ( precond_par->L.blockinfo != NULL ) {
        magma_tally3_free_cpu( precond_par->L.blockinfo );
        precond_par->L.blockinfo = NULL;
    }
    if ( precond_par->U.val != NULL ) {
        if ( precond_par->U.memory_location == Magma_tally3_DEV )
            magma_tally3_free( precond_par->U.dval );
        else
            magma_tally3_free_cpu( precond_par->U.val );
        precond_par->U.val = NULL;
    }
    if ( precond_par->U.col != NULL ) {
        if ( precond_par->U.memory_location == Magma_tally3_DEV )
            magma_tally3_free( precond_par->U.dcol );
        else
            magma_tally3_free_cpu( precond_par->U.col );
        precond_par->U.col = NULL;
    }
    if ( precond_par->U.row != NULL ) {
        if ( precond_par->U.memory_location == Magma_tally3_DEV )
            magma_tally3_free( precond_par->U.drow );
        else
            magma_tally3_free_cpu( precond_par->U.row );
        precond_par->U.row = NULL;
    }
    if ( precond_par->U.blockinfo != NULL ) {
        magma_tally3_free_cpu( precond_par->U.blockinfo );
        precond_par->U.blockinfo = NULL;
    }
    if ( precond_par->solver == Magma_tally3_ILU ||
        precond_par->solver == Magma_tally3_AILU ||
        precond_par->solver == Magma_tally3_ICC||
        precond_par->solver == Magma_tally3_AICC ) {
    ...
    if ( precond_par->LD.val != NULL ) {
        if ( precond_par->LD.memory_location == Magma_tally3_DEV )
            magma_tally3_free( precond_par->LD.dval );
        else
            magma_tally3_free_cpu( precond_par->LD.val );
        precond_par->LD.val = NULL;
    }
    if ( precond_par->LD.col != NULL ) {
        if ( precond_par->LD.memory_location == Magma_tally3_DEV )
            magma_tally3_free( precond_par->LD.dcol );
        else
            magma_tally3_free_cpu( precond_par->LD.col );
        precond_par->LD.col = NULL;
    }
    if ( precond_par->LD.row != NULL ) {
        if ( precond_par->LD.memory_location == Magma_tally3_DEV )
            magma_tally3_free( precond_par->LD.drow );
        else
            magma_tally3_free_cpu( precond_par->LD.row );
        precond_par->LD.row = NULL;
    }
    if ( precond_par->LD.blockinfo != NULL ) {
        magma_tally3_free_cpu( precond_par->LD.blockinfo );
        precond_par->LD.blockinfo = NULL;
    }
    if ( precond_par->UD.val != NULL ) {
        if ( precond_par->UD.memory_location == Magma_tally3_DEV )
            magma_tally3_free( precond_par->UD.dval );
        else
            magma_tally3_free_cpu( precond_par->UD.val );
        precond_par->UD.val = NULL;
    }
    if ( precond_par->UD.col != NULL ) {
        if ( precond_par->UD.memory_location == Magma_tally3_DEV )
            magma_tally3_free( precond_par->UD.dcol );
        else
            magma_tally3_free_cpu( precond_par->UD.col );
        precond_par->UD.col = NULL;
    }
    if ( precond_par->UD.row != NULL ) {
        if ( precond_par->UD.memory_location == Magma_tally3_DEV )
            magma_tally3_free( precond_par->UD.drow );
        else
            magma_tally3_free_cpu( precond_par->UD.row );
        precond_par->UD.row = NULL;
    }
    if ( precond_par->UD.blockinfo != NULL ) {
        magma_tally3_free_cpu( precond_par->UD.blockinfo );
        precond_par->UD.blockinfo = NULL;
    }

    precond_par->solver = Magma_tally3_NONE;
    return MAGMA_tally3_SUCCESS;
}
    ...
    magma_tally3_queue_t queue )
{
    ...
    if ( solver_par->verbose > 0 ) {
        CHECK( magma_tally3_malloc_cpu( (void **)&solver_par->res_vec, sizeof(real_Double_t)
                * ( (solver_par->maxiter)/(solver_par->verbose)+1) ));
        CHECK( magma_tally3_malloc_cpu( (void **)&solver_par->timing, sizeof(real_Double_t)
                *( (solver_par->maxiter)/(solver_par->verbose)+1) ));
    } else {
        solver_par->res_vec = NULL;
        solver_par->timing = NULL;
...
cleanup:
    if( info != 0 ){
        magma_tally3_free( solver_par->timing );
        magma_tally3_free( solver_par->res_vec );
    }
    return info;
}
...
{
    magma_tally3_int_t info = 0;
    magma_tally3FloatComplex *initial_guess=NULL;
    solver_par->eigenvectors = NULL;
    solver_par->eigenvalues = NULL;
    if ( solver_par->solver == Magma_tally3_LOBPCG ) {
        CHECK( magma_tally3_smalloc_cpu( &solver_par->eigenvalues ,
                                3*solver_par->num_eigenvalues ));
        // setup initial guess EV using lapack
        // then copy to GPU
        magma_tally3_int_t ev = solver_par->num_eigenvalues * solver_par->ev_length;

        CHECK( magma_tally3_cmalloc_cpu( &initial_guess, ev ));
        CHECK( magma_tally3_cmalloc( &solver_par->eigenvectors, ev ));
        magma_tally3_int_t ISEED[4] = {0,0,0,1}, ione = 1;
        lapackf77_clarnv( &ione, ISEED, &ev, initial_guess );

        magma_tally3_csetmatrix( solver_par->ev_length, solver_par->num_eigenvalues,
...
cleanup:
    if( info != 0 ){
        magma_tally3_free( solver_par->eigenvectors );
        magma_tally3_free( solver_par->eigenvalues );
    }
    magma_tally3_free_cpu( initial_guess );
    return info;
}
