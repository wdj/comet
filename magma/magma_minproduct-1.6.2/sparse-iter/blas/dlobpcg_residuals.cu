...
// copied from dnrm2.cu in trunk/magma_minproductblas
// ----------------------------------------
...
template< int n >
__device__ void sum_reduce( /*int n,*/ int i, magma_minproductDouble_ptr  x )
{
    __syncthreads();
...
__global__ void
magma_minproduct_dlobpcg_res_kernel( 
    magma_minproduct_int_t num_rows, 
    magma_minproduct_int_t num_vecs, 
    magma_minproductDouble_ptr evals, 
    double * X, 
    double * R,
    magma_minproductDouble_ptr res)
{
    ...
    if( row<num_rows){
        for( int i=0; i<num_vecs; i++ ){ 
            ...
            R[row + i*num_rows] = R[row + i*num_rows] 
                                    + MAGMA_minproduct_D_MAKE( -evals[i], 0.0 )
                                                * X[ row + i*num_rows ];
        }
    }
}
...
/*
magma_minproductblas_dnrm2_kernel( 
    int m, 
    double * da, 
    int ldda, 
    double * dxnorm )
...
/**
    ...
    @param[in]
    evalues     magma_minproductDouble_ptr 
    ...
    @param[in]
    res         magma_minproductDouble_ptr 
                array of residuals
...
magma_minproduct_dlobpcg_res(
    magma_minproduct_int_t num_rows,
    magma_minproduct_int_t num_vecs, 
    magma_minproductDouble_ptr evalues, 
    ...
    magma_minproduct_queue_t queue )
{
    ...
    magma_minproduct_dlobpcg_res_kernel<<< grid, threads, 0, queue >>>
