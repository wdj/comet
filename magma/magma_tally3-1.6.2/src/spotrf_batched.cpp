...
#define A(i_, j_)  (A + (i_) + (j_)*ldda)   
    ...
    magma_tally3_int_t j, k, ib;
    magma_tally3_int_t nb = POTRF_NB;
    magma_tally3_int_t gemm_crossover = 127;//nb > 32 ? 127 : 160;
   ...
   /* check allocation */
    ...
    for(k=0; k<nbstreams; k++){
        magma_tally3_queue_create( &stream[k] );
    }
    ...
    if (uplo == Magma_tally3Upper) {
        ...
        for(j = 0; j < n; j+=nb) {
            ...
            #if 0
            arginfo = magma_tally3_spotrf_panel_batched(
                               uplo, n-j, ib,
                               dA_displ, ldda,
            ...
            #else
            //arginfo = magma_tally3_spotrf_rectile_batched(
            arginfo = magma_tally3_spotrf_recpanel_batched(
                               uplo, n-j, ib, 32,
                               dA_displ, ldda,
                               dwork_array, dwork_msize,
                               dinvA_array, invA_msize,
            ...
            if( (n-j-ib) > 0){
                ...
                { 
                    ...
                    for(k=0; k<batchCount; k++)
                    {
                        ...
                        magma_tally3_ssyrk(Magma_tally3Lower, Magma_tally3NoTrans, n-j-ib, ib, 
                            d_alpha, 
                ...
                else
                {
                    ...
                    //-------------------------------------------
                    magma_tally3_sdisplace_pointers(dA_displ, dA_array, ldda, j+ib, j, batchCount, queue);
                    magma_tally3_sdisplace_pointers(dW1_displ, dA_array, ldda, j+ib, j+ib, batchCount, queue);
                    magma_tally3blas_ssyrk_batched(uplo, Magma_tally3NoTrans, n-j-ib, ib,
