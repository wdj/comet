...
void zpotf2_zdscal(magma_minproduct_int_t n, magma_minproductDoubleComplex *x, magma_minproduct_int_t incx);
void zpotf2_zdotc(magma_minproduct_int_t n, magma_minproductDoubleComplex *x, magma_minproduct_int_t incx);

#if defined(PRECISION_z) || defined(PRECISION_c)
void zlacgv(magma_minproduct_int_t n, magma_minproductDoubleComplex *x, magma_minproduct_int_t incx);
#endif
    ...
    if (uplo == Magma_minproductUpper) {
        for(j = 0; j < n; j++) {
            ...
            if (j < n) {
                ...
                zlacgv(j, dA(0, j), 1);
                #endif
                magma_minproduct_zgemv( Magma_minproductTrans, j, n-j-1,
                             alpha, dA(0, j+1), ldda,
                                    dA(0, j),   1,
                             beta,  dA(j, j+1), ldda);
        ...
        for(j = 0; j < n; j++) {
            ...
            if (j < n) {
                ...
                #endif
                magma_minproduct_zgemv( Magma_minproductNoTrans, n-j-1, j,
                             alpha, dA(j+1, 0), ldda,
                                    dA(j,0),    ldda,
                             beta,  dA(j+1, j), 1 );
...
__global__ void kernel_zdotc(int n, magma_minproductDoubleComplex *x, int incx, int threadSize)
{
    int tx = threadIdx.x;
...
*/
    ...
    kernel_zdotc<<< 1, threadSize, threadSize * sizeof(double), magma_minproduct_stream>>> (n, x, incx, threadSize);
}

__global__ void kernel_zdscal(int n, magma_minproductDoubleComplex *x, int incx)
{
...
*/
    ...
    kernel_zdscal<<< grid, threads, 0, magma_minproduct_stream >>> (n, x, incx);
}


#if defined(PRECISION_z) || defined(PRECISION_c)

__global__ void kernel_zlacgv(int n, magma_minproductDoubleComplex *x, int incx)
{
...
void zlacgv(magma_minproduct_int_t n, magma_minproductDoubleComplex *x, magma_minproduct_int_t incx)
{
    ...
    dim3 grid(num_blocks,1);
    kernel_zlacgv<<< grid, threads, 0, magma_minproduct_stream >>> (n, x, incx);
}
