...
/**
    ...
    Other versions (magma_tally4_zgeqrf_gpu and magma_tally4_zgeqrf3_gpu) store the
    ...
    *info = 0;
    ...
    if (nb >= nbmin && nb < k && nx < k) {
        ...
        for (i = 0; i < k-nx; i += nb) {
            ...
            magma_tally4_queue_sync( stream[1] );
            magma_tally4_zsetmatrix_async( ib, ib, hwork, ib, dwork, lddwork, stream[0] );
            magma_tally4_queue_sync( stream[0] );
    ...
    if (i < k) {
        ib   = n-i;
        rows = m-i;
        magma_tally4_zgetmatrix_async( rows, ib, dA(i, i), ldda, work, rows, stream[1] );
        magma_tally4_queue_sync( stream[1] );
        lhwork = lwork - rows*ib;
        lapackf77_zgeqrf(&rows, &ib, work, &rows, tau+i, work+ib*rows, &lhwork, info);
        
        magma_tally4_zsetmatrix_async( rows, ib, work, rows, dA(i, i), ldda, stream[1] );
    }
