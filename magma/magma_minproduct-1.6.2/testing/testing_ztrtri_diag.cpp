...
int main( int argc, char** argv )
{
    ...
    double          magma_minproduct_error, norm_invA, work[1];
    ...
    magma_minproductDoubleComplex *h_A, *h_dinvA;
    magma_minproductDoubleComplex_ptr d_A, d_dinvA;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_DEV( d_dinvA, magma_minproductDoubleComplex, size_inv );
            TESTING_MALLOC_CPU( h_dinvA, magma_minproductDoubleComplex, size_inv );
            ...
            magma_minproductblas_ztrtri_diag( opts.uplo, opts.diag, N, d_A, ldda, d_dinvA );
            ...
            magma_minproduct_zgetvector( size_inv, d_dinvA, 1, h_dinvA, 1 );
            
            if ( opts.verbose ) {
                printf( "A%d=", (int) N );
                magma_minproduct_zprint( N, N, h_A, lda );
                printf( "d_dinvA%d=", (int) N );
                magma_minproduct_zprint( min(N+4, nb), min(N+4, nblock*nb), h_dinvA, nb );
            }
            ...
            if ( opts.check ) {
                ...
                for( int i=0; i < N; i += nb ) {
                    ...
                    magma_minproduct_error = max( magma_minproduct_error, lapackf77_zlantr( "M", uplo_, Magma_minproductNonUnitStr, &jb, &jb, h_dinvA(0, i), &nb,  work ));
                    norm_invA   = max( norm_invA,   lapackf77_zlantr( "M", uplo_, Magma_minproductNonUnitStr, &jb, &jb, h_A(i, i),     &lda, work ));
                }
