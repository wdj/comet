...
int main( int argc, char** argv )
{
    ...
    magma_tally3FloatComplex *C, *R, *A, *W, *tau;
    magma_tally3FloatComplex_ptr dC, dA, dT;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int itran = 0; itran < 2; ++itran ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {        
            ...
            TESTING_MALLOC_CPU( A,   magma_tally3FloatComplex, lda*k );
            TESTING_MALLOC_CPU( W,   magma_tally3FloatComplex, lwork_max );
            TESTING_MALLOC_CPU( tau, magma_tally3FloatComplex, k );
            
            TESTING_MALLOC_DEV( dC, magma_tally3FloatComplex, ldc*n );
            TESTING_MALLOC_DEV( dA, magma_tally3FloatComplex, lda*k );
            TESTING_MALLOC_DEV( dT, magma_tally3FloatComplex, dt_size );
            ...
            magma_tally3_csetmatrix( lda, k, A,  lda, dA, lda );
            magma_tally3_cgeqrf_gpu( lda, k, dA, lda, tau, dT, &info );
            magma_tally3_cgetmatrix( lda, k, dA, lda, A,  lda );
            if (info != 0)
            ...
            lapackf77_cunmqr( lapack_side_const_tally3( side[iside] ), lapack_trans_const_tally3( trans[itran] ),
                              &m, &n, &k,
            ...
            lwork = -1;
            magma_tally3_cunmqr_gpu( side[iside], trans[itran],
                              m, n, k,
                              dA, lda, tau, dC, ldc, W, lwork, dT, nb, &info );
            if (info != 0)
                printf("magma_tally3_cunmqr_gpu (lwork query) returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
            lwork = (magma_tally3_int_t) MAGMA_tally3_C_REAL( W[0] );
            ...
            if (info != 0)
                printf("magma_tally3_cunmqr_gpu returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
