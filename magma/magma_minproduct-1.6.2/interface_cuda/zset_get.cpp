    ...
    const char* func, const char* file, int line )
{
    ...
    status = cublasSetVector(
        n, sizeof(magma_minproductDoubleComplex),
        hx_src, incx,
        dy_dst, incy );
    ...
    const char* func, const char* file, int line )
{
    if ( incx == 1 && incy == 1 ) {
        ...
        status = cudaMemcpy(
            dy_dst,
            dx_src,
            n*sizeof(magma_minproductDoubleComplex), cudaMemcpyDeviceToDevice );
        check_xerror( status, func, file, line );
    }
    else {
    ...
    const char* func, const char* file, int line )
{
    if ( incx == 1 && incy == 1 ) {
        ...
        status = cudaMemcpyAsync(
            dy_dst,
            dx_src,
            n*sizeof(magma_minproductDoubleComplex), cudaMemcpyDeviceToDevice, queue );
        check_xerror( status, func, file, line );
    }
    else {
...
magma_minproduct_zsetmatrix_internal(
    ...
    magma_minproductDoubleComplex const* hA_src, magma_minproduct_int_t lda,
    magma_minproductDoubleComplex_ptr    dB_dst, magma_minproduct_int_t ldb,
    const char* func, const char* file, int line )
{
    cublasStatus_t status;
    status = cublasSetMatrix(
        m, n, sizeof(magma_minproductDoubleComplex),
        hA_src, lda,
        dB_dst, ldb );
    check_xerror( status, func, file, line );
}
...
magma_minproduct_zgetmatrix_internal(
    magma_minproduct_int_t m, magma_minproduct_int_t n,
    magma_minproductDoubleComplex_const_ptr dA_src, magma_minproduct_int_t lda,
    magma_minproductDoubleComplex*          hB_dst, magma_minproduct_int_t ldb,
    const char* func, const char* file, int line )
{
    cublasStatus_t status;
    ...
    const char* func, const char* file, int line )
{
    cudaError_t status;
    status = cudaMemcpy2D(
        dB_dst, ldb*sizeof(magma_minproductDoubleComplex),
        dA_src, lda*sizeof(magma_minproductDoubleComplex),
        m*sizeof(magma_minproductDoubleComplex), n, cudaMemcpyDeviceToDevice );
    check_xerror( status, func, file, line );
}
    ...
    const char* func, const char* file, int line )
{
    ...
    status = cudaMemcpy2DAsync(
        dB_dst, ldb*sizeof(magma_minproductDoubleComplex),
        dA_src, lda*sizeof(magma_minproductDoubleComplex),
        m*sizeof(magma_minproductDoubleComplex), n, cudaMemcpyDeviceToDevice, queue );
    check_xerror( status, func, file, line );
}
