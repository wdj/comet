...
/**
    ...
    @ingroup magma_tally3_dposv_driver
    ********************************************************************/
    ...
    magma_tally3_int_t *info)
{
    ...
    *info = 0;
    ...
    Anrm = magma_tally3blas_dlansy(Magma_tally3InfNorm, uplo, n, dA, ldda, (double*)dworkd );
    cte  = Anrm * eps * pow((double)n, 0.5) * BWDMAX;

    /*
     * Convert to single precision
     */
    magma_tally3blas_dlag2s( n, nrhs, dB, lddb, dSX, lddsx, info );
    if (*info != 0) {
        *iter = -2;
        goto FALLBACK;
    }

    magma_tally3blas_dlat2s( uplo, n, dA, ldda, dSA, lddsa, info );
    if (*info != 0) {
        *iter = -2;
        goto FALLBACK;
    }
    ...
    magma_tally3_spotrf_gpu( uplo, n, dSA, lddsa, info );
    if (*info != 0) {
        *iter = -3;
        goto FALLBACK;
    }
    ...
    magma_tally3_spotrs_gpu( uplo, n, nrhs, dSA, lddsa, dSX, lddsx, info );
    ...
    if ( nrhs == 1 ) {
        magma_tally3_dsymv( uplo, n,
                     c_neg_one, dA, ldda,
                                dX, 1,
                     c_one,     dR, 1 );
    }
    else {
        magma_tally3_dsymm( Magma_tally3Left, uplo, n, nrhs,
                     c_neg_one, dA, ldda,
                                dX, lddx,
                     c_one,     dR, lddr );
    }
...
REFINEMENT:
    for( iiter=1; iiter < ITERMAX; ) {
        *info = 0;
        ...
        magma_tally3blas_dlag2s( n, nrhs, dR, lddr, dSX, lddsx, info );
        if (*info != 0) {
            *iter = -2;
            goto FALLBACK;
        }
...
FALLBACK:
    ...
    magma_tally3_dpotrf_gpu( uplo, n, dA, ldda, info );
    if (*info == 0) {
        magma_tally3blas_dlacpy( Magma_tally3UpperLower, n, nrhs, dB, lddb, dX, lddx );
        magma_tally3_dpotrs_gpu( uplo, n, nrhs, dA, ldda, dX, lddx, info );
    }
