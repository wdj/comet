...
#define dA(i, j)  (dA + (i) + (j)*ldda)   // A(i, j) means at i row, j column
    ...
    magma_tally4_int_t nb = 32;
    magma_tally4_int_t nnb = 8;
    magma_tally4_int_t i, k, ib=nb, jb=nnb;
    magma_tally4_int_t ldw, ldt, ldr, offset; 

    cublasHandle_t myhandle;
    cublasCreate_v2(&myhandle);


    magma_tally4DoubleComplex **dW0_displ = NULL;
    ...
    magma_tally4DoubleComplex **dW5_displ = NULL;

    magma_tally4DoubleComplex *dwork = NULL;
    magma_tally4DoubleComplex *dT   = NULL;
    magma_tally4DoubleComplex *dR   = NULL;
    magma_tally4DoubleComplex **dR_array = NULL;
    magma_tally4DoubleComplex **dT_array = NULL; 
    magma_tally4DoubleComplex **cpuAarray = NULL;
    magma_tally4DoubleComplex **cpuTarray = NULL;
    ...
    magma_tally4_malloc((void**)&dW4_displ, batchCount * sizeof(*dW4_displ));  // used in zlarfb
    magma_tally4_malloc((void**)&dW5_displ, batchCount * sizeof(*dW5_displ));
    magma_tally4_malloc((void**)&dR_array, batchCount * sizeof(*dR_array));
    magma_tally4_malloc((void**)&dT_array, batchCount * sizeof(*dT_array));

    ldt = ldr = min(nb, min_mn);
    magma_tally4_zmalloc(&dwork,  (2 * nb * n) * batchCount);
    magma_tally4_zmalloc(&dR,  ldr * n   * batchCount);
    magma_tally4_zmalloc(&dT,  ldt * ldt * batchCount);
    magma_tally4_malloc_cpu((void**) &cpuAarray, batchCount*sizeof(magma_tally4DoubleComplex*));
    magma_tally4_malloc_cpu((void**) &cpuTarray, batchCount*sizeof(magma_tally4DoubleComplex*));
    ...
    if ( dW0_displ == NULL || dW1_displ == NULL || dW2_displ == NULL || dW3_displ == NULL || 
        ...
        magma_tally4_free(dR_array);
        magma_tally4_free(dT_array);
        magma_tally4_free(dR);
    ...
    magma_tally4blas_zlaset_q(Magma_tally4Full, ldr, n*batchCount  , MAGMA_tally4_Z_ZERO, MAGMA_tally4_Z_ZERO, dR, ldr, queue);
    magma_tally4blas_zlaset_q(Magma_tally4Full, ldt, ldt*batchCount, MAGMA_tally4_Z_ZERO, MAGMA_tally4_Z_ZERO, dT, ldt, queue);
    ...
    magma_tally4_getvector( batchCount, sizeof(magma_tally4DoubleComplex*), dA_array, 1, cpuAarray, 1);
    magma_tally4_getvector( batchCount, sizeof(magma_tally4DoubleComplex*), dT_array, 1, cpuTarray, 1);
    ...
    for(i=0; i<min_mn;i+=nb)
    {
            ...
            magma_tally4_zdisplace_pointers(dW0_displ, dA_array, ldda, i, i, batchCount, queue); 
            magma_tally4_zdisplace_pointers(dW2_displ, tau_array, 1, i, 0, batchCount, queue);
            ...
            magma_tally4_zgeqrf_panel_batched(m-i, ib, jb, 
                                       dW0_displ, ldda, 
                                       dW2_displ, 
                                       dT_array, ldt, 
                                       dR_array, ldr,
           ...
           zgeqrf_copy_upper_batched(ib, jb, dW0_displ, ldda, dR_array, ldr, batchCount, queue);
            ...
            if( (n-ib-i) > 0)
            {
                ...
                magma_tally4blas_zlaset_batched(Magma_tally4Upper, ib, ib, MAGMA_tally4_Z_ZERO, MAGMA_tally4_Z_ONE, dW0_displ, ldda, batchCount, queue); 
                magma_tally4_zdisplace_pointers(dW2_displ, tau_array, 1, i, 0, batchCount, queue); 

                // it is faster since it is using BLAS-3 GEMM routines, different from lapack implementation 
                magma_tally4_zlarft_batched(m-i, ib, 0,
                                 dW0_displ, ldda,
                                 dW2_displ,
                                 dT_array, ldt, 
                                 dW4_displ, nb*ldt,
                ...
                { 
                    ...
                    for(k=0; k<batchCount; k++)
                    {
                        ...
                        magma_tally4_zlarfb_gpu_gemm(Magma_tally4Left, Magma_tally4ConjTrans, Magma_tally4Forward, Magma_tally4Columnwise,
                                    m-i, n-i-ib, ib,
                ...
                else
                {
                    ...
                    magma_tally4_zdisplace_pointers(dW1_displ, dA_array, ldda, i, i+ib, batchCount, queue); 
            ...
            magma_tally4blas_zlacpy_batched(Magma_tally4Upper, ib, ib, dR_array, ldr, dW0_displ, ldda, batchCount, queue);
