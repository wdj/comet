    ...
    magma_tally2_queue_t queue )
{
    ...
    CHECK( magma_tally2_dvinit( &rt,Magma_tally2_DEV, A.num_rows, b.num_cols, c_zero, queue ));
    CHECK( magma_tally2_dvinit( &p, Magma_tally2_DEV, A.num_rows, b.num_cols, c_zero, queue ));
    CHECK( magma_tally2_dvinit( &q, Magma_tally2_DEV, A.num_rows, b.num_cols, c_zero, queue ));
    CHECK( magma_tally2_dvinit( &h, Magma_tally2_DEV, A.num_rows, b.num_cols, c_zero, queue ));
    ...
    magma_tally2_dcopy( dofs, h.dval, 1, p.dval, 1 );                    // p = h
    nom = MAGMA_tally2_D_REAL( magma_tally2_ddot(dofs, r.dval, 1, h.dval, 1) );
    CHECK( magma_tally2_d_spmv( c_one, A, p, c_zero, q, queue ));             // q = A p
    den = MAGMA_tally2_D_REAL( magma_tally2_ddot(dofs, p.dval, 1, q.dval, 1) );// den = p dot q
    ...
    do
    {
        ...
        gammanew = MAGMA_tally2_D_REAL( magma_tally2_ddot(dofs, r.dval, 1, h.dval, 1) );
                                                            // gn = < r,h>

        if ( solver_par->numiter==1 ) {
            magma_tally2_dcopy( dofs, h.dval, 1, p.dval, 1 );                    // p = h
        } else {
            beta = MAGMA_tally2_D_MAKE(gammanew/gammaold, 0.);       // beta = gn/go
            magma_tally2_dscal(dofs, beta, p.dval, 1);            // p = beta*p
            magma_tally2_daxpy(dofs, c_one, h.dval, 1, p.dval, 1); // p = p + h
        }

        CHECK( magma_tally2_d_spmv( c_one, A, p, c_zero, q, queue ));   // q = A p
        den = MAGMA_tally2_D_REAL(magma_tally2_ddot(dofs, p.dval, 1, q.dval, 1));
                // den = p dot q

        alpha = MAGMA_tally2_D_MAKE(gammanew/den, 0.);
        magma_tally2_daxpy(dofs,  alpha, p.dval, 1, x->dval, 1);     // x = x + alpha p
        magma_tally2_daxpy(dofs, -alpha, q.dval, 1, r.dval, 1);      // r = r - alpha q
        gammaold = gammanew;

        res = magma_tally2_dnrm2( dofs, r.dval, 1 );
        if ( solver_par->verbose > 0 ) {
