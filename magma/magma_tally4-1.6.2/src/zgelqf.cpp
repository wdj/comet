...
/**
    ...
    @ingroup magma_tally4_zgelqf_comp
    ********************************************************************/
    ...
    magma_tally4_int_t *info)
{
    ...
    magma_tally4DoubleComplex_ptr dA, dAT;
    ...
    /* Function Body */
    *info = 0;
    nb = magma_tally4_get_zgelqf_nb(m);
    min_mn = min(m,n);

    work[0] = MAGMA_tally4_Z_MAKE( (double)(m*nb), 0 );
    lquery = (lwork == -1);
    if (m < 0) {
        *info = -1;
    } else if (n < 0) {
    ...
    if (maxdim*maxdim < 2*maxm*maxn) {
        ...
        if (MAGMA_tally4_SUCCESS != magma_tally4_zmalloc( &dA, maxdim*maxdim )) {
            *info = MAGMA_tally4_ERR_DEVICE_ALLOC;
            return *info;
        }

        magma_tally4_zsetmatrix( m, n, A, lda, dA(0,0), ldda );
        dAT = dA;
        magma_tally4blas_ztranspose_inplace( lddat, dAT(0,0), lddat );
    }
    else {
        ...
        if (MAGMA_tally4_SUCCESS != magma_tally4_zmalloc( &dA, 2*maxn*maxm )) {
            *info = MAGMA_tally4_ERR_DEVICE_ALLOC;
            return *info;
        }
        ...
        dAT = dA + maxn * maxm;
        magma_tally4blas_ztranspose( m, n, dA(0,0), ldda, dAT(0,0), lddat );
    }

    // factor QR
    magma_tally4_zgeqrf2_gpu( n, m, dAT(0,0), lddat, tau, &iinfo );
    assert( iinfo >= 0 );
    if ( iinfo > 0 ) {
        *info = iinfo;
    }
    ...
    if (maxdim*maxdim < 2*maxm*maxn) {
        magma_tally4blas_ztranspose_inplace( lddat, dAT(0,0), lddat );
        magma_tally4_zgetmatrix( m, n, dA(0,0), ldda, A, lda );
    } else {
        magma_tally4blas_ztranspose( n, m, dAT(0,0), lddat, dA(0,0), ldda );
        magma_tally4_zgetmatrix( m, n, dA(0,0), ldda, A, lda );
    }
