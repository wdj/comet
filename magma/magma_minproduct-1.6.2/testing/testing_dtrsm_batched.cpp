...
int main( int argc, char** argv)
{
    ...
    double *h_A, *h_B, *h_Bcublas, *h_Bmagma_minproduct, *h_Blapack, *h_X;
    double *d_A, *d_B;
    ...
    double alpha = MAGMA_minproduct_D_MAKE(  0.29, -0.86 );
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_Bmagma_minproduct,  double, sizeB   );
            ...
            magma_minproduct_dmalloc( &dinvA, dinvA_batchSize * batchCount);
            magma_minproduct_dmalloc( &dwork, dwork_batchSize * batchCount );
    
            dset_pointer(dwork_array, dwork, lddb, 0, 0, dwork_batchSize, batchCount, opts.queue);
            dset_pointer(dinvA_array, dinvA, magma_minproduct_roundup( Ak, TRI_NB ), 0, 0, dinvA_batchSize, batchCount, opts.queue);

            memset(h_Bmagma_minproduct, 0, batchCount*ldb*N*sizeof(double));
            magma_minproductblas_dlaset( Magma_minproductFull, lddb, N*batchCount, c_zero, c_zero, dwork, lddb);
            ...
            /* =====================================================================
               ...
               =================================================================== */
            magma_minproduct_dsetmatrix( Ak, Ak*batchCount, h_A, lda, d_A, ldda );
            magma_minproduct_dsetmatrix( M,  N*batchCount, h_B, ldb, d_B, lddb );
            ...
            magma_minproduct_time = magma_minproduct_sync_wtime( NULL );
                ...
                magma_minproduct_dgetmatrix( M, N*batchCount, dwork, lddb, h_Bmagma_minproduct, ldb );
            #else
                ...
                magma_minproduct_time = magma_minproduct_sync_wtime( NULL ) - magma_minproduct_time;
                magma_minproduct_perf = gflops / magma_minproduct_time;
                magma_minproduct_dgetmatrix( M, N*batchCount, d_B, lddb, h_Bmagma_minproduct, ldb );
            #endif
            ...
            magma_minproduct_dgetmatrix( M, N*batchCount, d_B, lddb, h_Bcublas, ldb );
            ...
            /* =====================================================================
               ...
               =================================================================== */
            // ||b - 1/alpha*A*x|| / (||A||*||x||)
            double alpha2 = MAGMA_minproduct_D_DIV( c_one, alpha );
            double normR, normX, normA=0.;
            magma_minproduct_error  = 0.0;
            cublas_error = 0.0;

            memcpy( h_X, h_Bmagma_minproduct, sizeB*sizeof(double) );
            ...
            for (int s=0; s < batchCount; s++) {
                ...
                normX = lapackf77_dlange( "M", &M, &N, h_Bmagma_minproduct + s * ldb * N, &ldb, work );
