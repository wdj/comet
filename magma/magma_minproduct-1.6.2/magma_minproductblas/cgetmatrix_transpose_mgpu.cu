/*
    -- MAGMA_minproduct (version 1.6.1) --
       ...
       @generated from zgetmatrix_transpose_mgpu.cu normal z -> c, Fri Jan 30 19:00:09 2015
...
#define dAT(d, j)   (dAT[(d)]   + (j)*nb)
    ...
    magma_minproduct_int_t nstreams = 2, d, j, j_local, id, ib;
    ...
    if (lda < m || ngpu*ldda < n || lddw < m){
        printf( "Wrong arguments in magma_minproductblas_cgetmatrix_transpose_mgpu (%d<%d), (%d*%d<%d), or (%d<%d).\n",
    ...
    for(j=0; j<n; j+=nb){
       ...
       ib = min(n-j, nb);
       magma_minproductblas_ctranspose_q( ib, m, dAT(d,j_local), ldda, dwork(d,id), lddw, queues[d][id] );
