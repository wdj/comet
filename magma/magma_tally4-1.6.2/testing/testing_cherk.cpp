...
int main( int argc, char** argv)
{
    ...
    magma_tally4FloatComplex *h_A, *h_C, *h_Ccublas;
    magma_tally4FloatComplex_ptr d_A, d_C;
    magma_tally4FloatComplex c_neg_one = MAGMA_tally4_C_NEG_ONE;
    float alpha = MAGMA_tally4_D_MAKE(  0.29, -0.86 );
    float beta  = MAGMA_tally4_D_MAKE( -0.48,  0.38 );
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_Ccublas, magma_tally4FloatComplex, ldc*N  );
            ...
            /* =====================================================================
               ...
               =================================================================== */
            magma_tally4_csetmatrix( An, Ak, h_A, lda, d_A, ldda );
            magma_tally4_csetmatrix( N, N, h_C, ldc, d_C, lddc );

            cublas_time = magma_tally4_sync_wtime( NULL );
            cublasCherk( opts.handle, cublas_uplo_const_tally4(opts.uplo), cublas_trans_const_tally4(opts.transA), N, K,
                         &alpha, d_A, ldda,
                         &beta,  d_C, lddc );
            cublas_time = magma_tally4_sync_wtime( NULL ) - cublas_time;
            cublas_perf = gflops / cublas_time;
            
            magma_tally4_cgetmatrix( N, N, d_C, lddc, h_Ccublas, ldc );
            ...
            if ( opts.lapack ) {
                ...
                blasf77_cherk( lapack_uplo_const_tally4(opts.uplo), lapack_trans_const_tally4(opts.transA), &N, &K,
                               &alpha, h_A, &lda,
                               &beta,  h_C, &ldc );
            ...
            if ( opts.lapack ) {
                ...
                Cnorm = lapackf77_clanhe("fro", lapack_uplo_const_tally4(opts.uplo), &N, h_C, &ldc, work);

                blasf77_caxpy( &sizeC, &c_neg_one, h_C, &ione, h_Ccublas, &ione );
                cublas_error = lapackf77_clanhe( "fro", lapack_uplo_const_tally4(opts.uplo), &N, h_Ccublas, &ldc, work ) / Cnorm;
