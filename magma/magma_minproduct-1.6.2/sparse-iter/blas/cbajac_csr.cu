...
__global__ void
magma_minproduct_cbajac_csr_ls_kernel(int localiters, int n, 
                            magma_minproductFloatComplex * valD, 
                            magma_minproduct_index_t * rowD, 
                            magma_minproduct_index_t * colD, 
                            magma_minproductFloatComplex * valR, 
    ...
    if(index<n){
        ...
        magma_minproductFloatComplex bl, tmp = zero, v = zero; 
        ...
        __shared__ magma_minproductFloatComplex local_x[ BLOCKSIZE ];
...
__global__ void
magma_minproduct_cbajac_csr_kernel(    
    int n, 
    magma_minproductFloatComplex * valD, 
    magma_minproduct_index_t * rowD, 
    magma_minproduct_index_t * colD, 
    ...
    magma_minproduct_queue_t queue )
{
    ...
    if ( R.nnz > 0 ) { 
        if ( localiters == 1 )
        magma_minproduct_cbajac_csr_kernel<<< grid, block, 0, queue >>>
            ( D.num_rows, D.dval, D.drow, D.dcol, 
                            R.dval, R.drow, R.dcol, b.dval, x->dval );
        else
            magma_minproduct_cbajac_csr_ls_kernel<<< grid, block, 0, queue >>>
