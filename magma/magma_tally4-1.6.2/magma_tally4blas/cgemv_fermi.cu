...
cgemvn_kernel1_fermi(
    ...
    const magma_tally4FloatComplex * __restrict__ A, int lda,
    const magma_tally4FloatComplex * __restrict__ x, int incx, magma_tally4FloatComplex beta,
    ...
    magma_tally4FloatComplex       * __restrict__ y, int incy)
{
    ...
    __shared__ magma_tally4FloatComplex sx[BLK_Y];
    ...
    if ( threadIdx.x < npart ) {
        ...
        sx[threadIdx.x] = MAGMA_tally4_C_ZERO;
    }
    __syncthreads();
    ...
    magma_tally4FloatComplex       * __restrict__ y, int incy)
{
    ...
    __shared__ magma_tally4FloatComplex sdata[BLK_X];
    ...
    magma_tally4_sum_reduce< BLK_X >( tx, sdata );
...
/**
    ...
    trans   magma_tally4_trans_t
      ...
      -     = Magma_tally4ConjTrans:  y := alpha*A^H*x + beta*y
...
magma_tally4blas_cgemv(
    magma_tally4_trans_t trans, magma_tally4_int_t m, magma_tally4_int_t n, magma_tally4FloatComplex alpha,
    ...
    magma_tally4FloatComplex_ptr dy, magma_tally4_int_t incy)
{
    ...
    if ( arch < 200  ) {
        ...
        magma_tally4_cgemv( trans, m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
        #else
        magma_tally4blas_cgemv_tesla( trans, m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
        #endif
        return;
    }
    ...
    if ( trans == Magma_tally4NoTrans ) {
        ...
        cgemvn_kernel1_fermi<<< grid, threads, 0, magma_tally4_stream >>>
            ( m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    }
    else if ( trans == Magma_tally4Trans ) {
        dim3 grid    ( 1, n, 1 );
        dim3 threads ( BLK_X, 1, 1 );
        cgemvt_kernel_fermi<<< grid, threads, 0, magma_tally4_stream >>>
            ( m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    }
    else if ( trans == Magma_tally4ConjTrans ) {
        dim3 grid    ( 1, n, 1 );
        dim3 threads ( BLK_X, 1, 1 );
        cgemvc_kernel_fermi<<< grid, threads, 0, magma_tally4_stream >>>
            ( m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    }
}
