...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for( int j = 0; j < M; ++j ) {
                for( int i = 0; i < N; ++i ) {
                    h_B[i + j*ldb] = MAGMA_minproduct_S_MAKE( i + j/10000., j );
                }
            }
            magma_minproduct_ssetmatrix( N, M, h_B, ldb, d_B, lddb );
            ...
            //magma_minproductblas_stranspose( M-2, N-2, d_A+1+ldda, ldda, d_B+1+lddb, lddb );  // inset by 1 row & col
            magma_minproductblas_stranspose( M, N, d_A, ldda, d_B, lddb );
            ...
            if ( M == N ) {
                ...
                //magma_minproductblas_stranspose_inplace( N-2, d_A+1+ldda, ldda );  // inset by 1 row & col
                magma_minproductblas_stranspose_inplace( N, d_A, ldda );
