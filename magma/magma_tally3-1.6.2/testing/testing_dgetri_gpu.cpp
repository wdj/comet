...
int main( int argc, char** argv )
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            n2     = lda*N;
            ldda   = ((N+31)/32)*32;
            ldwork = N * magma_tally3_get_dgetri_nb( N );
            ...
            lwork = int( MAGMA_tally3_D_REAL( tmp ));
            ...
            magma_tally3_dgetrf_gpu( N, N, d_A, ldda, ipiv, &info );
            ...
            //h_A[ 10 + 10*lda ] = MAGMA_tally3_D_MAKE( 0.0, 0.0 );
            //magma_tally3_dsetmatrix( N, N, h_A, lda, d_A, ldda );
            ...
            magma_tally3_dgetri_gpu( N, d_A, ldda, ipiv, dwork, ldwork, &info );
            gpu_time = magma_tally3_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally3_dgetri_gpu returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
