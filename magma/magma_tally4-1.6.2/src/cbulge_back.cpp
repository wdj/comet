...
static void *magma_tally4_capplyQ_parallel_section(void *arg);

static void magma_tally4_ctile_bulge_applyQ(magma_tally4_int_t core_id, magma_tally4_side_t side, magma_tally4_int_t n_loc, magma_tally4_int_t n, magma_tally4_int_t nb, magma_tally4_int_t Vblksiz,
                                     magma_tally4FloatComplex *E, magma_tally4_int_t lde, magma_tally4FloatComplex *V, magma_tally4_int_t ldv,
                                     magma_tally4FloatComplex *TAU, magma_tally4FloatComplex *T, magma_tally4_int_t ldt);

//////////////////////////////////////////////////////////////////////////////////////////////////////////

typedef struct magma_tally4_capplyQ_data_s {
    magma_tally4_int_t threads_num;
    magma_tally4_int_t n;
    magma_tally4_int_t ne;
    magma_tally4_int_t n_gpu;
    magma_tally4_int_t nb;
    magma_tally4_int_t Vblksiz;
    magma_tally4FloatComplex* E;
    magma_tally4_int_t lde;
    ...
    magma_tally4_int_t ldde;
    pthread_barrier_t barrier;
} magma_tally4_capplyQ_data;

void magma_tally4_capplyQ_data_init(
    magma_tally4_capplyQ_data *zapplyQ_data, magma_tally4_int_t threads_num, magma_tally4_int_t n, magma_tally4_int_t ne, magma_tally4_int_t n_gpu,
    magma_tally4_int_t nb, magma_tally4_int_t Vblksiz, magma_tally4FloatComplex *E, magma_tally4_int_t lde,
    magma_tally4FloatComplex *V, magma_tally4_int_t ldv, magma_tally4FloatComplex *TAU,
    magma_tally4FloatComplex *T, magma_tally4_int_t ldt, magma_tally4FloatComplex *dE, magma_tally4_int_t ldde)
{
    ...
    magma_tally4_int_t count = zapplyQ_data->threads_num;
...
void magma_tally4_capplyQ_data_destroy(
    magma_tally4_capplyQ_data *zapplyQ_data)
{
    pthread_barrier_destroy(&(zapplyQ_data->barrier));
}

typedef struct magma_tally4_capplyQ_id_data {
    magma_tally4_int_t id;
    magma_tally4_capplyQ_data* data;
} magma_tally4_capplyQ_id_data;

void magma_tally4_capplyQ_id_data_init(
    magma_tally4_capplyQ_id_data *zapplyQ_id_data,
    magma_tally4_int_t id, magma_tally4_capplyQ_data* data)
{
...
//#endif
    ...
    magma_tally4_int_t gpu_cpu_perf = magma_tally4_get_cbulge_gcperf();
    if (threads > 1) {
        f = 1. / (1. + (float)(threads-1)/ ((float)gpu_cpu_perf)    );
        n_gpu = (magma_tally4_int_t)(f*ne);
    }
...
//$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
    ...
    if (n_gpu < ne) {
        ...
        #endif
        magma_tally4_capplyQ_data data_applyQ;
        magma_tally4_capplyQ_data_init(&data_applyQ, threads, n, ne, n_gpu, nb, Vblksiz, Z, ldz, V, ldv, TAU, T, ldt, dZ, lddz);

        magma_tally4_capplyQ_id_data* arg;
        magma_tally4_malloc_cpu((void**) &arg, threads*sizeof(magma_tally4_capplyQ_id_data));
        ...
        for (magma_tally4_int_t thread = 1; thread < threads; thread++) {
            magma_tally4_capplyQ_id_data_init(&(arg[thread]), thread, &data_applyQ);
            pthread_create(&thread_id[thread], &thread_attr, magma_tally4_capplyQ_parallel_section, &arg[thread]);
        }
        magma_tally4_capplyQ_id_data_init(&(arg[0]), 0, &data_applyQ);
        magma_tally4_capplyQ_parallel_section(&arg[0]);
        ...
        magma_tally4_capplyQ_data_destroy(&data_applyQ);


        magma_tally4_csetmatrix(n, ne-n_gpu, Z + n_gpu*ldz, ldz, dZ + n_gpu*ldz, lddz);

        /*============================
         *  use only GPU
         *==========================*/
    } else {
        magma_tally4_csetmatrix(n, ne, Z, ldz, dZ, lddz);
        magma_tally4_cbulge_applyQ_v2(Magma_tally4Left, ne, n, nb, Vblksiz, dZ, lddz, V, ldv, T, ldt, info);
        magma_tally4_device_sync();
    }

    timeaplQ2 = magma_tally4_wtime()-timeaplQ2;
...
static void *magma_tally4_capplyQ_parallel_section(void *arg)
{
    magma_tally4_int_t my_core_id   = ((magma_tally4_capplyQ_id_data*)arg) -> id;
    magma_tally4_capplyQ_data* data = ((magma_tally4_capplyQ_id_data*)arg) -> data;

    magma_tally4_int_t allcores_num   = data -> threads_num;
    magma_tally4_int_t n              = data -> n;
    magma_tally4_int_t ne             = data -> ne;
    magma_tally4_int_t n_gpu          = data -> n_gpu;
    magma_tally4_int_t nb             = data -> nb;
    magma_tally4_int_t Vblksiz        = data -> Vblksiz;
    magma_tally4FloatComplex *E         = data -> E;
    magma_tally4_int_t lde            = data -> lde;
    magma_tally4FloatComplex *V         = data -> V;
    magma_tally4_int_t ldv            = data -> ldv;
    magma_tally4FloatComplex *TAU       = data -> TAU;
    magma_tally4FloatComplex *T         = data -> T;
    magma_tally4_int_t ldt            = data -> ldt;
    magma_tally4FloatComplex *dE        = data -> dE;
    magma_tally4_int_t ldde           = data -> ldde;
    ...
    magma_tally4_int_t n_cpu = ne - n_gpu;
    ...
    if (my_core_id == 0) {
        ...
        //=============================================
        #ifdef ENABLE_TIMER
        timeQgpu = magma_tally4_wtime();
        #endif

        magma_tally4_csetmatrix(n, n_gpu, E, lde, dE, ldde);
        magma_tally4_cbulge_applyQ_v2(Magma_tally4Left, n_gpu, n, nb, Vblksiz, dE, ldde, V, ldv, T, ldt, &info);
        magma_tally4_device_sync();

        #ifdef ENABLE_TIMER
        timeQgpu = magma_tally4_wtime()-timeQgpu;
        ...
        #ifdef ENABLE_TIMER
        if (my_core_id == 1)
            timeQcpu = magma_tally4_wtime();
        #endif

        magma_tally4_int_t n_loc = magma_tally4_ceildiv(n_cpu, allcores_num-1);
        magma_tally4FloatComplex* E_loc = E + (n_gpu+ n_loc * (my_core_id-1))*lde;
        n_loc = min(n_loc,n_cpu - n_loc * (my_core_id-1));

        magma_tally4_ctile_bulge_applyQ(my_core_id, Magma_tally4Left, n_loc, n, nb, Vblksiz, E_loc, lde, V, ldv, TAU, T, ldt);
        pthread_barrier_wait(barrier);

        #ifdef ENABLE_TIMER
        if (my_core_id == 1) {
            timeQcpu = magma_tally4_wtime()-timeQcpu;
...
#define TAU(m)   &(TAU[(m)])
#define T(m)     &(T[(m)])
static void magma_tally4_ctile_bulge_applyQ(
    magma_tally4_int_t core_id, magma_tally4_side_t side, magma_tally4_int_t n_loc, magma_tally4_int_t n, magma_tally4_int_t nb, magma_tally4_int_t Vblksiz,
    ...
    //, magma_tally4_int_t* info)
{
    ...
    //%   local variables
    //%===========================
    magma_tally4_int_t firstcolj;
    magma_tally4_int_t bg, rownbm;
    magma_tally4_int_t st,ed,fst,vlen,vnb,colj;
    magma_tally4_int_t vpos,tpos;
    magma_tally4_int_t cur_blksiz,avai_blksiz, ncolinvolvd;
    magma_tally4_int_t nbgr, colst, coled;
    ...
    magma_tally4_int_t nb_loc = 128; //$$$$$$$$

    magma_tally4_int_t     lwork = 2*nb_loc*max(Vblksiz,64);
    magma_tally4FloatComplex *work, *work2;

    magma_tally4_cmalloc_cpu(&work, lwork);
    magma_tally4_cmalloc_cpu(&work2, lwork);

    magma_tally4_int_t nbchunk =  magma_tally4_ceildiv(n_loc, nb_loc);
    ...
    #endif
    for (magma_tally4_int_t i = 0; i < nbchunk; i++) {
        magma_tally4_int_t ib_loc = min(nb_loc, (n_loc - i*nb_loc));

        if (side == Magma_tally4Left) {
            for (bg = nbGblk; bg > 0; bg--) {
                firstcolj = (bg-1)*Vblksiz + 1;
                rownbm    = magma_tally4_ceildiv((n-(firstcolj+1)),nb);
                if (bg == nbGblk) rownbm = magma_tally4_ceildiv((n-(firstcolj)),nb);  // last blk has size=1 used for complex to handle A(N,N-1)
                for (magma_tally4_int_t j = rownbm; j > 0; j--) {
                    vlen = 0;
                    vnb  = 0;
                    colj = (bg-1)*Vblksiz; // for k=0; I compute the fst and then can remove it from the loop
                    fst  = (rownbm -j)*nb+colj +1;
                    for (magma_tally4_int_t k=0; k < Vblksiz; k++) {
                        colj = (bg-1)*Vblksiz + k;
                    ...
                    colst     = (bg-1)*Vblksiz;
                    magma_tally4_bulge_findVTpos(n, nb, Vblksiz, colst, fst, ldv, ldt, &vpos, &tpos);
        ...
        } else if (side == Magma_tally4Right) {
            rownbm    = magma_tally4_ceildiv((n-1),nb);
            for (magma_tally4_int_t k = 1; k <= rownbm; k++) {
                ncolinvolvd = min(n-1, k*nb);
                avai_blksiz = min(Vblksiz,ncolinvolvd);
                nbgr = magma_tally4_ceildiv(ncolinvolvd,avai_blksiz);
                for (magma_tally4_int_t j = 1; j <= nbgr; j++) {
                    vlen = 0;
                    vnb  = 0;
