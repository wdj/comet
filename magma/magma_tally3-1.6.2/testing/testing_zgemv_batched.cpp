...
int main( int argc, char** argv)
{
    ...
    double          magma_tally3_error, magma_tally3_err, Ynorm, work[1];
    ...
    magma_tally3DoubleComplex *h_A, *h_X, *h_Y, *h_Ymagma_tally3;
    magma_tally3DoubleComplex *d_A, *d_X, *d_Y;
    ...
    magma_tally3DoubleComplex **X_array = NULL;
    magma_tally3DoubleComplex **Y_array = NULL;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_X,  magma_tally3DoubleComplex, sizeX );
            TESTING_MALLOC_CPU( h_Y,  magma_tally3DoubleComplex, sizeY  );
            TESTING_MALLOC_CPU( h_Ymagma_tally3,  magma_tally3DoubleComplex, sizeY  );


            TESTING_MALLOC_DEV( d_A, magma_tally3DoubleComplex, ldda*N*batchCount );
            TESTING_MALLOC_DEV( d_X, magma_tally3DoubleComplex, sizeX );
            TESTING_MALLOC_DEV( d_Y, magma_tally3DoubleComplex, sizeY );
            ...
            /* =====================================================================
               ...
               =================================================================== */
            magma_tally3_zsetmatrix( M, N*batchCount, h_A, lda, d_A, ldda );
            magma_tally3_zsetvector( Xm*batchCount, h_X, incx, d_X, incx );
            magma_tally3_zsetvector( Ym*batchCount, h_Y, incy, d_Y, incy );
            
            zset_pointer(A_array, d_A, ldda, 0, 0, ldda*N, batchCount, magma_tally3_stream);
            zset_pointer(X_array, d_X, 1, 0, 0, incx*Xm, batchCount, magma_tally3_stream);
            zset_pointer(Y_array, d_Y, 1, 0, 0, incy*Ym, batchCount, magma_tally3_stream);

            magma_tally3_time = magma_tally3_sync_wtime( NULL );
            magma_tally3blas_zgemv_batched(opts.transA, M, N,
                             alpha, A_array, ldda,
                                    X_array, incx,
            ...
            magma_tally3_zgetvector( Ym*batchCount, d_Y, incy, h_Ymagma_tally3, incy );
            ...
            if ( opts.lapack ) {
                ...
                for(int s=0; s<batchCount; s++)
                {
                    ...
                    blasf77_zaxpy( &Ym, &c_neg_one, h_Y + s*Ym, &ione, h_Ymagma_tally3 + s*Ym, &ione );
                    magma_tally3_err = lapackf77_zlange( "M", &M, &ione, h_Ymagma_tally3 + s*Ym, &incy, work ) / Ynorm; 
