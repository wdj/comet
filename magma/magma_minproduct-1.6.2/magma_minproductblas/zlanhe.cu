...
zlanhe_inf_kernel_generic_lower(
    int n, const magma_minproductDoubleComplex* A, int lda, double *dwork,
    int n_full_block, int n_mod_bs )
{
    ...
    __shared__ magma_minproductDoubleComplex la[inf_bs][inf_bs+1];
    ...
    if ( blockIdx.x < n_full_block ) {
        ...
        for(int i=ty*8; i < ty*8 + 8; i++) {
            if ( i < tx ) {
                ...
                la[i][i] = MAGMA_minproduct_Z_MAKE( MAGMA_minproduct_Z_REAL( la[i][i] ), 0 );
            }
            #endif
        }
        __syncthreads();
        ...
        if ( n_mod_bs > 0 ) {
            ...
            for(int j=0; j < inf_bs; j += 4) {
                if ( tx < n_mod_bs ) {
                    ...
                    la[ty+j][tx] = MAGMA_minproduct_Z_ZERO;
                }
            }
            __syncthreads();
        ...
        la[tx][ty] = MAGMA_minproduct_Z_MAKE( res, 0. );
        __syncthreads();
        
        // first column of 32x1 threads computes final sum of each row
        if ( ty == 0 ) {
            res = res
                + MAGMA_minproduct_Z_REAL( la[tx][1] )
                + MAGMA_minproduct_Z_REAL( la[tx][2] )
                + MAGMA_minproduct_Z_REAL( la[tx][3] );
            dwork[ind] = res;
        }
    }
    else {
        ...
        if ( ty == 0 && tx < n_mod_bs ) {
            ...
            res += MAGMA_minproduct_D_ABS( MAGMA_minproduct_Z_REAL( *A ));
            A += 1;
        ...
        if ( n_mod_bs > 0 ) {
            ...
            for(int j=0; j < inf_bs; j += 4) {
                if ( ty+j < n_mod_bs ) {               //#
                    ...
                    la[tx][ty+j] = MAGMA_minproduct_Z_ZERO;       //#
                }
            }
            __syncthreads();
...
zlanhe_inf(
    ...
    magma_minproductDoubleComplex_const_ptr A, int lda,
    magma_minproductDouble_ptr dwork )
{
    ...
    if ( uplo == Magma_minproductLower) {
        zlanhe_inf_kernel_generic_lower<<< grid, threads, 0, magma_minproduct_stream >>>
            ( n, A, lda, dwork, n_full_block, n_mod_bs );
    }
    else {
        zlanhe_inf_kernel_generic_upper<<< grid, threads, 0, magma_minproduct_stream >>>
            ( n, A, lda, dwork, n_full_block, n_mod_bs );
    }
}
    ...
    int n, const magma_minproductDoubleComplex* A, int lda, double *dwork )
{
    ...
    if (ind < n) {
        ...
        res = fmax( res, MAGMA_minproduct_D_ABS( MAGMA_minproduct_Z_REAL( *A )));
        dwork[ind] = res;
    }
}
    ...
    magma_minproductDouble_ptr dwork )
{
    ...
    if ( uplo == Magma_minproductLower ) {
        zlanhe_max_kernel_lower<<< grid, threads, 0, magma_minproduct_stream >>>
            ( n, A, lda, dwork );
    }
    else {
        zlanhe_max_kernel_upper<<< grid, threads, 0, magma_minproduct_stream >>>
            ( n, A, lda, dwork );
    }
}
    ...
    magma_minproductDouble_ptr dwork )
{
    ...
    int i = magma_minproduct_idamax( n, dwork, 1 ) - 1;
