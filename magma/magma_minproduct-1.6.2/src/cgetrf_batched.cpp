...
#define A(i_, j_)  (A + (i_) + (j_)*ldda)   
    ...
    cudaMemset(info_array, 0, batchCount*sizeof(magma_minproduct_int_t));
...
//#define ENABLE_TIMER3
    ...
    tall   = magma_minproduct_sync_wtime(0);
    talloc = magma_minproduct_sync_wtime(0);
#endif

    magma_minproductFloatComplex neg_one = MAGMA_minproduct_C_NEG_ONE;
    magma_minproductFloatComplex one  = MAGMA_minproduct_C_ONE;
    magma_minproduct_int_t ib, i, k, pm;
    magma_minproduct_int_t nb = BATRF_NB;
    magma_minproduct_int_t gemm_crossover = nb > 32 ? 127 : 160;
    // magma_minproduct_int_t gemm_crossover = n;// use only stream gemm
    ...
    magma_minproductFloatComplex **dW0_displ  = NULL;
    ...
    magma_minproduct_int_t **pivinfo_array    = NULL;
    magma_minproduct_int_t *pivinfo           = NULL; 
    magma_minproductFloatComplex* dinvA      = NULL;
    magma_minproductFloatComplex* dwork      = NULL;// dinvA and dwork are workspace in ctrsm
    magma_minproductFloatComplex **cpuAarray = NULL;
    magma_minproduct_cmalloc( &dinvA, invA_msize * batchCount);
    magma_minproduct_cmalloc( &dwork, dwork_msize * batchCount );
    magma_minproduct_malloc((void**)&pivinfo_array, batchCount * sizeof(*pivinfo_array));
    magma_minproduct_malloc((void**)&pivinfo, batchCount * m * sizeof(magma_minproduct_int_t));
    magma_minproduct_malloc_cpu((void**) &cpuAarray, batchCount*sizeof(magma_minproductFloatComplex*));
   ...
   /* check allocation */
    if ( dA_displ  == NULL || dW0_displ == NULL || dW1_displ   == NULL || dW2_displ   == NULL || 
        ...
        magma_minproduct_free(dW0_displ);
        ...
        magma_minproduct_free(pivinfo_array);
        magma_minproduct_free(pivinfo);
        magma_minproduct_int_t info = MAGMA_minproduct_ERR_DEVICE_ALLOC;
        magma_minproduct_xerbla( __func__, -(info) );
        return info;
    }


    magma_minproductblas_claset_q(Magma_minproductFull, invA_msize, batchCount, MAGMA_minproduct_C_ZERO, MAGMA_minproduct_C_ZERO, dinvA, invA_msize, queue);
    magma_minproductblas_claset_q(Magma_minproductFull, dwork_msize, batchCount, MAGMA_minproduct_C_ZERO, MAGMA_minproduct_C_ZERO, dwork, dwork_msize, queue);
    ...
    magma_minproduct_int_t streamid;
    const magma_minproduct_int_t nbstreams=32;
    magma_minproduct_queue_t stream[nbstreams];
    for(i=0; i<nbstreams; i++){
        magma_minproduct_queue_create( &stream[i] );
    }
    magma_minproduct_getvector( batchCount, sizeof(magma_minproductFloatComplex*), dA_array, 1, cpuAarray, 1);



#if defined(ENABLE_TIMER3)
    printf(" I am after malloc\n");
    talloc = magma_minproduct_sync_wtime(0) - talloc;
    tloop  = magma_minproduct_sync_wtime(0);
#endif
    ...
    for(i = 0; i < min_mn; i+=nb) 
    {
        ...
        ib = min(nb, min_mn-i);
        pm = m-i;
        magma_minproduct_idisplace_pointers(dipiv_displ, ipiv_array, ldda, i, 0, batchCount, queue);
        magma_minproduct_cdisplace_pointers(dA_displ, dA_array, ldda, i, i, batchCount, queue);
        //===============================================
        //  panel factorization
        //===============================================
#if 0
        arginfo = magma_minproduct_cgetf2_batched(
                pm, ib,
                dA_displ, ldda,
...
#else
        arginfo = magma_minproduct_cgetrf_recpanel_batched(
                pm, ib, 16,
                dA_displ, ldda,
...
#define RUN_ALL
        ...
        magma_minproduct_cdisplace_pointers(dW0_displ, dA_array, ldda, i, 0, batchCount, queue);
        magma_minproduct_claswp_rowparallel_batched( i, dA_displ, ldda,
                dW0_displ, ldda,
                i, i+ib,
        ...
        if( (i + ib) < n)
        {
            ...
            magma_minproduct_cdisplace_pointers(dA_displ, dA_array, ldda, i, i+ib, batchCount, queue);
            cset_pointer(dwork_array, dwork, nb, 0, 0, dwork_msize, batchCount, queue); // I don't think it is needed Azzam
            magma_minproduct_claswp_rowparallel_batched( n-(i+ib), dA_displ, ldda,
                    dwork_array, nb,
                    i, i+ib,
                    pivinfo_array, batchCount, queue);
            ...
            magma_minproduct_cdisplace_pointers(dA_displ, dA_array, ldda, i, i, batchCount, queue);
            magma_minproduct_cdisplace_pointers(dW0_displ, dA_array, ldda, i, i+ib, batchCount, queue);
            magma_minproductblas_ctrsm_outofplace_batched(Magma_minproductLeft, Magma_minproductLower, Magma_minproductNoTrans, Magma_minproductUnit, 1,
                    ib, n-i-ib,
            ...
            {    
                ...
                { 
                    ...
                    for(k=0; k<batchCount; k++)
                    {
                        ...
                        magma_minproductblasSetKernelStream(stream[streamid]);
                ...
                {
                    magma_minproduct_cdisplace_pointers(dA_displ, dA_array,  ldda, i+ib,    i, batchCount, queue);
                    magma_minproduct_cdisplace_pointers(dW1_displ, dA_array, ldda,    i, i+ib, batchCount, queue);
                    magma_minproduct_cdisplace_pointers(dW2_displ, dA_array, ldda, i+ib, i+ib, batchCount, queue);
                    //printf("caling batched dgemm %d %d %d \n", m-i-ib, n-i-ib, ib);
                    magma_minproductblas_cgemm_batched( Magma_minproductNoTrans, Magma_minproductNoTrans, m-i-ib, n-i-ib, ib, 
...
#if defined(ENABLE_TIMER3)
    tloop   = magma_minproduct_sync_wtime(0) - tloop;
    tdalloc = magma_minproduct_sync_wtime(0);

#endif

    for(i=0; i<nbstreams; i++){
        magma_minproduct_queue_destroy( stream[i] );
    }
...
#if defined(ENABLE_TIMER3)
    tdalloc = magma_minproduct_sync_wtime(0) - tdalloc;
    tall = magma_minproduct_sync_wtime(0) - tall;
