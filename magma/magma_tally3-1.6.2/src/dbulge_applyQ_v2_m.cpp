    ...
    magma_tally3_int_t *info)
{
    ...
    */
    ...
    double *dE[Magma_tally3MaxGPUs];
    double *dwork[Magma_tally3MaxGPUs], *dwork0[Magma_tally3MaxGPUs], *dwork1[Magma_tally3MaxGPUs];
    //double *dwvt[Magma_tally3MaxGPUs];
    double *dwvt0[Magma_tally3MaxGPUs], *dwvt1[Magma_tally3MaxGPUs];
    double *dT0[Magma_tally3MaxGPUs], *dV0[Magma_tally3MaxGPUs], *dT1[Magma_tally3MaxGPUs], *dV1[Magma_tally3MaxGPUs];
    ...
    for (dev=0; dev < ngpu; ++dev) {
        magma_tally3_setdevice( dev );
        if (MAGMA_tally3_SUCCESS != magma_tally3_dmalloc( &dE[dev], ldde * ne_loc)) {
            printf ("!!!!  magma_tally3_dbulge_applyQ magma_tally3_alloc failed for: dE\n" );
            exit(-1);
        }
        if (MAGMA_tally3_SUCCESS != magma_tally3_dmalloc( &dwork[dev], 2*dworksiz + 2*dwVTsiz + 2*Vchunksiz* (Vblksiz* (lddv+lddt)) )) {
            printf ("!!!!  magma_tally3_dbulge_applyQ magma_tally3_alloc failed for: dwork\n" );
            exit(-1);
        }
        ...
        magma_tally3_dsetmatrix_async( N, ie_loc, E+lde*ne_loc*dev, lde, dE(dev, 0, 0), ldde, streams[dev][1] );
    }
                    ...
                    if (ncpy == 0) {
                        ...
                        if (mysiz > 0) {
                            ...
                            for( dev = 0; dev < ngpu; ++dev ) {
                                ...
                                magma_tally3_dsetmatrix_async(vld, Vblksiz, V(vpos), vld, dV1[dev], vld, streams[dev][1]);
                                magma_tally3_dsetmatrix_async(tld, Vblksiz, T(tpos), tld, dT1[dev], tld, streams[dev][1]);
                            }
                        ...
                        if (mysiz > 0) {
                            ...
                            if (flip == 0) { // now I am working on dV0 so copy the next and put it on dV1
                                ...
                                for( dev = 0; dev < ngpu; ++dev ) {
                                    ...
                                    magma_tally3_dsetmatrix_async(vld, Vblksiz, V(vpos), vld, dV0[dev], vld, streams[dev][0]);
                                    magma_tally3_dsetmatrix_async(tld, Vblksiz, T(tpos), tld, dT0[dev], tld, streams[dev][0]);
                                }
                            }
                        }
                    }
                        ...
                        if (flip == 0) {
                            for( dev = 0; dev < ngpu; ++dev ) {
                                ...
                                for (magma_tally3_int_t i=0; i < ie_loc; i += sz_bl) {
                                    ...
                                    //magma_tally3_dlarfb_gpu( Magma_tally3Left, Magma_tally3NoTrans, Magma_tally3Forward, Magma_tally3Columnwise, Vm, ib, Vn, dV0[dev]+lcvpos, lddv, dT0[dev]+lctpos, lddt, dE(dev,myrow,i), ldde, dwork0[dev], ib);
                                    magma_tally3_dlarfb_gpu_gemm( Magma_tally3Left, Magma_tally3NoTrans, Magma_tally3Forward, Magma_tally3Columnwise, Vm, ib, Vn, dV0[dev]+lcvpos, lddv, dT0[dev]+lctpos, lddt, dE(dev,myrow,i), ldde, dwork0[dev], ib, dwvt0[dev], Vm);
                                }
                            ...
                            for( dev = 0; dev < ngpu; ++dev ) {
                                ...
                                for (magma_tally3_int_t i=0; i < ie_loc; i += sz_bl) {
                                    ib = min(sz_bl, ie_loc-i);
                                    //magma_tally3_dlarfb_gpu( Magma_tally3Left, Magma_tally3NoTrans, Magma_tally3Forward, Magma_tally3Columnwise, Vm, ib, Vn, dV1[dev]+lcvpos, lddv, dT1[dev]+lctpos, lddt, dE(dev,myrow,i), ldde, dwork1[dev], ib);
                                    magma_tally3_dlarfb_gpu_gemm( Magma_tally3Left, Magma_tally3NoTrans, Magma_tally3Forward, Magma_tally3Columnwise, Vm, ib, Vn, dV1[dev]+lcvpos, lddv, dT1[dev]+lctpos, lddt, dE(dev,myrow,i), ldde, dwork1[dev], ib, dwvt1[dev], Vm);
                                }
            ...
            for (blki = mt; blki > 0; blki--) {
                ...
                for (blkj = nt-1; blkj >= 0; blkj--) {
                    ...
                    if ((Vm > 0) && (Vn > 0)) {
                        ...
                        magma_tally3_dsetmatrix_async(Vm, Vn, V(vpos), ldv, dV0, lddv, NULL);
                        magma_tally3_dsetmatrix_async(Vn,  Vn, T(tpos), ldt, dT0, lddt, NULL);
                        ...
                        for (magma_tally3_int_t i=0; i < NE; i += sz_bl) {
                            ib = min(sz_bl, NE-i);
                            magma_tally3_dlarfb_gpu( Magma_tally3Left, Magma_tally3NoTrans, Magma_tally3Forward, Magma_tally3Columnwise, Vm, ib, Vn, dV0, lddv, dT0, lddt, dE(myrow,i), ldde, dwork, NE);
                        }
                        */
                    } // end for (Vm &Vn) > 0
                } // end for blkj
            ...
            for (blkj=0; blkj < nt; blkj++) {
                ...
                for (blki=1; blki <= mt; blki++) {
                    ...
                    if ((Vm > 0) && (Vn > 0)) {
                        ...
                        magma_tally3_dsetmatrix_async(Vm, Vn, V(vpos), ldv, dV0, lddv, NULL);
                        magma_tally3_dsetmatrix_async(Vn,  Vn, T(tpos), ldt, dT0, lddt, NULL);
                        magma_tally3_dlarfb_gpu( Magma_tally3Right, Magma_tally3NoTrans, Magma_tally3Forward, Magma_tally3Columnwise, NE, Vm, Vn, dV0, lddv, dT0, lddt, dE(0, myrow), ldde, dwork, NE);
                        */
                    } // end for (Vm &Vn) > 0
                } // end for blki
            } // end fo blkj
    ...
    for( dev = 0; dev < ngpu; ++dev ) {
        ...
        magma_tally3_int_t ie_loc   = min(ne_loc, NE - ne_loc*dev);
        magma_tally3_dgetmatrix_async( N, ie_loc, dE(dev, 0, 0), ldde, E+lde*ne_loc*dev, lde, streams[dev][0] );
        magma_tally3_event_record( myevent[dev][0], streams[dev][0] );
    }
