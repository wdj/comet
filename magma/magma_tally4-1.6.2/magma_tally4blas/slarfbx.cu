                    ...
                    float *dwork)
{
    ...
    for( int j = i; j < m; j += BLOCK_SIZE )
       lsum += MAGMA_tally4_S_MUL( MAGMA_tally4_S_CNJG( dV[j] ), c[j] );
...
/*  ----------------------------------------------------------------------------- 
    Call 
        magma_tally4_sgemv_kernel3<<< n, BLOCK_SIZE>>>(m, V, ldv, c, dwork, tau)
    to compute
...
extern "C"
__global__ void
magma_tally4_sgemv_kernel3(int m, const float * __restrict__ V, int ldv, float *c,
...
{
    /* dwork = V**H c     */
    magma_tally4_sgemv_kernel1<<< k, BLOCK_SIZE, 0, magma_tally4_stream >>>(m, V, ldv, c, dwork); 

    /* dwork = T**H dwork */
    magma_tally4_strmv_tkernel<<< k, k, 0, magma_tally4_stream >>>( dT, ldt, dwork, dwork+k);
 
    /* c = c - V dwork    */
    dim3  blocks3( (m + BLOCK_SIZE-1) / BLOCK_SIZE );
    dim3 threads3( BLOCK_SIZE );     
    magma_tally4_sgemv_kernel2<<< blocks3, threads3, 0, magma_tally4_stream >>>( m, k, V, ldv, dwork+k, c);
}
