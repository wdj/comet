...
bool lessthan( magma_tally3FloatComplex a, magma_tally3FloatComplex b )
{
    return (MAGMA_tally3_C_REAL(a) < MAGMA_tally3_C_REAL(b)) ||
        (MAGMA_tally3_C_REAL(a) == MAGMA_tally3_C_REAL(b) && MAGMA_tally3_C_IMAG(a) < MAGMA_tally3_C_IMAG(b));
}
...
extern "C"
float magma_tally3_slapy2(float x, float y)
{
    float ret_val, d;
    float w, z, xabs, yabs;
    
    xabs = MAGMA_tally3_S_ABS(x);
    yabs = MAGMA_tally3_S_ABS(y);
    w    = max(xabs, yabs);
    z    = min(xabs, yabs);
    if (z == 0.) {
        ret_val = w;
...
extern "C"
float magma_tally3_szlapy2(magma_tally3FloatComplex x)
{
    return magma_tally3_slapy2( MAGMA_tally3_C_REAL(x), MAGMA_tally3_C_IMAG(x) );
}
...
int main( int argc, char** argv)
{
    ...
    float *w1i, *w2i;
    magma_tally3FloatComplex *w1copy, *w2copy;
    magma_tally3FloatComplex  c_neg_one = MAGMA_tally3_C_NEG_ONE;
    float tnrm, result[9];
    magma_tally3_int_t N, n2, lda, nb, lwork, info;
    ...
    if ( opts.check && !opts.lapack && opts.jobvl == Magma_tally3NoVec && opts.jobvr == Magma_tally3NoVec ) {
    ...
    printf("    N   CPU Time (sec)   GPU Time (sec)   |W_magma_tally3 - W_lapack| / |W_lapack|\n");
    printf("===========================================================================\n");
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lda   = N;
            n2    = lda*N;
            nb    = magma_tally3_get_sgehrd_nb(N);
            lwork = N*(2 + nb);
            // generous workspace - required by sget22
            lwork = max( lwork, N*(5 + 2*N) );
            
            TESTING_MALLOC_CPU( w1copy, magma_tally3FloatComplex, N );
            TESTING_MALLOC_CPU( w2copy, magma_tally3FloatComplex, N );
            ...
            lapackf77_slacpy( Magma_tally3UpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
            
            /* ====================================================================
               Performs operation using MAGMA_tally3
               =================================================================== */
            gpu_time = magma_tally3_wtime();
            magma_tally3_sgeev( opts.jobvl, opts.jobvr,
                         N, h_R, lda, w1, w1i,
                         VL, lda, VR, lda,
                         h_work, lwork, &info );
            gpu_time = magma_tally3_wtime() - gpu_time;
            if (info != 0)
                printf("magma_tally3_sgeev returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                if ( opts.jobvr == Magma_tally3Vec ) {
                    // Do test 1: | A * VR - VR * W | / ( n |A| )
                    // Note this writes result[1] also
                    lapackf77_sget22( Magma_tally3NoTransStr, Magma_tally3NoTransStr, Magma_tally3NoTransStr,
                    ...
                    for( int j = 0; j < N; ++j ) {
                        tnrm = 1.;
                        if (w1i[j] == 0.)
                            tnrm = magma_tally3_cblas_snrm2( N, &VR[j*lda], ione );
                        else if (w1i[j] > 0.)
                            tnrm = magma_tally3_slapy2( magma_tally3_cblas_snrm2( N, &VR[j*lda],     ione ),
                                                 magma_tally3_cblas_snrm2( N, &VR[(j+1)*lda], ione ));
                        
                        result[1] = max( result[1], min( ulpinv, MAGMA_tally3_S_ABS(tnrm-1.)/ulp ));
                        
                        if (w1i[j] > 0.) {
                            vmx  = vrmx = 0.;
                            for( int jj = 0; jj < N; ++jj ) {
                                vtst = magma_tally3_slapy2( VR[jj+j*lda], VR[jj+(j+1)*lda]);
                                if (vtst > vmx)
                                    vmx = vtst;
                                
                                if ( (VR[jj + (j+1)*lda])==0. &&
                                     MAGMA_tally3_S_ABS( VR[jj+j*lda] ) > vrmx)
                                {
                                    vrmx = MAGMA_tally3_S_ABS( VR[jj+j*lda] );
                                }
                            }
                            if (vrmx / vmx < 1. - ulp*2.)
                                result[1] = ulpinv;
                ...
                if ( opts.jobvl == Magma_tally3Vec ) {
                    // Do test 3: | A**T * VL - VL * W**T | / ( n |A| )
                    // Note this writes result[3] also
                    lapackf77_sget22( Magma_tally3TransStr, Magma_tally3NoTransStr, Magma_tally3TransStr,
                    ...
                    for( int j = 0; j < N; ++j ) {
                        tnrm = 1.;
                        if (w1i[j] == 0.)
                            tnrm = magma_tally3_cblas_snrm2( N, &VL[j*lda], ione );
                        else if (w1i[j] > 0.)
                            tnrm = magma_tally3_slapy2( magma_tally3_cblas_snrm2( N, &VL[j*lda],     ione ),
                                                 magma_tally3_cblas_snrm2( N, &VL[(j+1)*lda], ione ));
                        
                        result[3] = max( result[3], min( ulpinv, MAGMA_tally3_S_ABS(tnrm-1.)/ulp ));
                        
                        if (w1i[j] > 0.) {
                            vmx  = vrmx = 0.;
                            for( int jj = 0; jj < N; ++jj ) {
                                vtst = magma_tally3_slapy2( VL[jj+j*lda], VL[jj+(j+1)*lda]);
                                if (vtst > vmx)
                                    vmx = vtst;
                                
                                if ( (VL[jj + (j+1)*lda])==0. &&
                                     MAGMA_tally3_S_ABS( VL[jj+j*lda]) > vrmx)
                                {
                                    vrmx = MAGMA_tally3_S_ABS( VL[jj+j*lda] );
                                }
                            }
                            if (vrmx / vmx < 1. - ulp*2.)
                                result[3] = ulpinv;
            ...
            if ( opts.check == 2 ) {
                ...
                // this is really slow because it calls magma_tally3_zgeev multiple times
                float *LRE, DUM;
                ...
                magma_tally3_sgeev( Magma_tally3Vec, Magma_tally3Vec,
                             N, h_R, lda, w1, w1i,
                             VL, lda, VR, lda,
                             h_work, lwork, &info );
                if (info != 0)
                    printf("magma_tally3_zgeev (case V, V) returned error %d: %s.\n",
                           (int) info, magma_tally3_strerror( info ));
                ...
                //lapackf77_slacpy( Magma_tally3UpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                //magma_tally3_sgeev( Magma_tally3NoVec, Magma_tally3NoVec,
                //             N, h_R, lda, w2, w2i,
                //             &DUM, 1, &DUM, 1,
                //             h_work, lwork, &info );
                //if (info != 0)
                //    printf("magma_tally3_sgeev (case N, N) returned error %d: %s.\n",
                //           (int) info, magma_tally3_strerror( info ));
                //
                ...
                lapackf77_slacpy( Magma_tally3UpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                magma_tally3_sgeev( Magma_tally3NoVec, Magma_tally3Vec,
                             N, h_R, lda, w2, w2i,
                             &DUM, 1, LRE, lda,
                             h_work, lwork, &info );
                if (info != 0)
                    printf("magma_tally3_sgeev (case N, V) returned error %d: %s.\n",
                           (int) info, magma_tally3_strerror( info ));
                ...
                for( int j = 0; j < N; ++j )
                    for( int jj = 0; jj < N; ++jj )
                        if ( ! MAGMA_tally3_S_EQUAL( VR[j+jj*lda], LRE[j+jj*lda] ))
                            result[7] = 0;
                
                // ----------
                // Compute eigenvalues and left eigenvectors
                lapackf77_slacpy( Magma_tally3UpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                magma_tally3_sgeev( Magma_tally3Vec, Magma_tally3NoVec,
                             N, h_R, lda, w2, w2i,
                             LRE, lda, &DUM, 1,
                             h_work, lwork, &info );
                if (info != 0)
                    printf("magma_tally3_sgeev (case V, N) returned error %d: %s.\n",
                           (int) info, magma_tally3_strerror( info ));
                ...
                for( int j = 0; j < N; ++j )
                    for( int jj = 0; jj < N; ++jj )
                        if ( ! MAGMA_tally3_S_EQUAL( VL[j+jj*lda], LRE[j+jj*lda] ))
                            result[8] = 0;
            ...
            if ( opts.lapack ) {
                ...
                lapackf77_sgeev( lapack_vec_const_tally3(opts.jobvl), lapack_vec_const_tally3(opts.jobvr),
                                 &N, h_A, &lda, w2, w2i,
                                 VL, &lda, VR, &lda,
                                 h_work, &lwork, &info );
                ...
                // check | W_magma_tally3 - W_lapack | / | W |
                // need to sort eigenvalues first
                // copy them into complex vectors for ease
                for( int j=0; j < N; ++j ) {
                    w1copy[j] = MAGMA_tally3_C_MAKE( w1[j], w1i[j] );
                    w2copy[j] = MAGMA_tally3_C_MAKE( w2[j], w2i[j] );
                }
                ...
                for( int j=0; j < N; ++j ) {
                    for( int j2=j; j2 < N; ++j2 ) {
                        magma_tally3FloatComplex diff = MAGMA_tally3_C_SUB( w1copy[j], w2copy[j2] );
                        float diff2 = magma_tally3_szlapy2( diff ) / max( magma_tally3_szlapy2( w1copy[j] ), tol );
                        if ( diff2 < 100*tol ) {
                            if ( j != j2 ) {
                ...
                error  = magma_tally3_cblas_scnrm2( N, w1copy, 1 );
                error /= magma_tally3_cblas_scnrm2( N, w2copy, 1 );
