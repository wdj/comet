...
void init_matrix( int nopiv, int m, int n, magma_tally2FloatComplex *h_A, magma_tally2_int_t lda )
{
    assert( m == n );
    ...
    if (nopiv) {
        magma_tally2_cmake_hpd( n, h_A, lda );
    }
    else {
    ...
    magma_tally2_int_t *ipiv )
{
    ...
    magma_tally2FloatComplex *x, *b;
    
    // initialize RHS
    TESTING_MALLOC_CPU( x, magma_tally2FloatComplex, n );
    TESTING_MALLOC_CPU( b, magma_tally2FloatComplex, n );
    ...
    if (nopiv) {
        if (upper) {
            blasf77_ctrsm( Magma_tally2LeftStr, Magma_tally2UpperStr, 
                           Magma_tally2ConjTransStr, Magma_tally2UnitStr, 
                           &n, &ione, &c_one,
                           A, &lda, x, &n );
            for (int i=0; i<n; i++) x[i] = MAGMA_tally2_C_DIV( x[i], A[i+i*lda] );
            blasf77_ctrsm( Magma_tally2LeftStr, Magma_tally2UpperStr, 
                           Magma_tally2NoTransStr, Magma_tally2UnitStr, 
                           &n, &ione, &c_one,
                           A, &lda, x, &n );
        } else {
            blasf77_ctrsm( Magma_tally2LeftStr, Magma_tally2LowerStr, 
                           Magma_tally2NoTransStr, Magma_tally2UnitStr, 
                           &n, &ione, &c_one,
                           A, &lda, x, &n );
            for (int i=0; i<n; i++) x[i] = MAGMA_tally2_C_DIV( x[i], A[i+i*lda] );
        ...
        lapackf77_chetrs( lapack_uplo_const_tally2(uplo), &n, &ione, A, &lda, ipiv, x, &n, &info );
    }
    if (info != 0)
...
float get_LDLt_error(int nopiv, magma_tally2_uplo_t uplo, magma_tally2_int_t N,
                      magma_tally2FloatComplex *LD, magma_tally2_int_t lda,
                      magma_tally2_int_t *ipiv)
{
    ...
    magma_tally2FloatComplex *A, *L, *D;
    ...
    TESTING_MALLOC_CPU( A, magma_tally2FloatComplex, N*N );
    TESTING_MALLOC_CPU( L, magma_tally2FloatComplex, N*N );
    TESTING_MALLOC_CPU( D, magma_tally2FloatComplex, N*N );
    memset( L, 0, N*N*sizeof(magma_tally2FloatComplex) );
    memset( D, 0, N*N*sizeof(magma_tally2FloatComplex) );
    ...
    if (uplo == Magma_tally2Upper) {
        for (j=N-1; j>=0; j--) {
            ...
            if (piv < 0) {
                ...
                D(j,j)     = LD(j,j);
                D(j,j-1)   = MAGMA_tally2_C_CNJG(LD(j-1,j));
                D(j-1,j)   = LD(j-1,j);
                ...
                if (piv != j) {
                    ...
                    for (i=j+2; i<N; i++) {
                        magma_tally2FloatComplex val = L(j,i);
                        L(j,i) = L(piv,i);
                        L(piv,i) = val;
                    }
                    // apply row-pivoting to A
                    for (i=0; i<N; i++) {
                        magma_tally2FloatComplex val = A(j,i);
                        A(j,i) = A(piv,i);
                        A(piv,i) = val;
                    }
                    // apply col-pivoting to A
                    for (i=0; i<N; i++) {
                        magma_tally2FloatComplex val = A(i,j);
                        A(i,j) = A(i,piv);
                        A(i,piv) = val;
                    }
                }
        ...
        if (nopiv) {
            // compute W = D*U
            blasf77_cgemm(Magma_tally2NoTransStr, Magma_tally2NoTransStr, &N, &N, &N,
                          &c_one, D, &N, L, &N, &c_zero, LD, &lda);
            // compute D = U'*W
            blasf77_cgemm(Magma_tally2ConjTransStr, Magma_tally2NoTransStr, &N, &N, &N,
            ...
            // compute D = W*U'
            blasf77_cgemm(Magma_tally2NoTransStr, Magma_tally2ConjTransStr, &N, &N, &N,
    ...
    // compute norm of A
    matnorm = lapackf77_clange(Magma_tally2FullStr, &N, &N, A, &lda, work);

    for( j = 0; j < N; j++ ) {
        for( i = 0; i < N; i++ ) {
            D(i,j) = MAGMA_tally2_C_SUB( D(i,j), A(i,j) );
        }
    }
    residual = lapackf77_clange(Magma_tally2FullStr, &N, &N, D, &N, work);
...
int main( int argc, char** argv)
{
    ...
    magma_tally2FloatComplex *h_A, *work, temp;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( opts.lapack ) {
                lwork = -1;
                lapackf77_chetrf( lapack_uplo_const_tally2(opts.uplo), &N, h_A, &lda, ipiv, &temp, &lwork, &info);
                ...
                lapackf77_chetrf( lapack_uplo_const_tally2(opts.uplo), &N, h_A, &lda, ipiv, work, &lwork, &info);
            ...
            if (nopiv) {
                ...
                magma_tally2_chetrf_nopiv_tally2( opts.uplo, N, h_A, lda, &info);
                gpu_time = magma_tally2_wtime() - gpu_time;
            } else if (cpu) {
                // CPU-interface to Bunch-Kauffman LDLt
                magma_tally2_setdevice(0);
                gpu_time = magma_tally2_wtime();
                magma_tally2_chetrf( opts.uplo, N, h_A, lda, ipiv, &info);
                gpu_time = magma_tally2_wtime() - gpu_time;
            } else if (nopiv_gpu) {
                ...
                if (MAGMA_tally2_SUCCESS != magma_tally2_cmalloc( &d_A, N*ldda  )) {
                    printf( " failed to allocate d_A(%dx%d)\n", (int) N, (int) ldda );
                    return 0;
                }
                magma_tally2_csetmatrix(N, N, h_A, lda, d_A, ldda);
                gpu_time = magma_tally2_wtime();
                magma_tally2_chetrf_nopiv_tally2_gpu( opts.uplo, N, d_A, ldda, &info);
                gpu_time = magma_tally2_wtime() - gpu_time;
                magma_tally2_cgetmatrix(N, N, d_A, ldda, h_A, lda);
                magma_tally2_free( d_A );
            } else if (row) {
                magma_tally2_setdevice(0);
                //magma_tally2_chetrf_gpu_row( opts.uplo, N, h_A, lda, ipiv, work, lwork, &info);
            } else {
                magma_tally2_setdevice(0);
                //magma_tally2_chetrf_hybrid( opts.uplo, N, h_A, lda, ipiv, work, lwork, &info);
            }
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally2_chetrf returned error %d: %s.\n",
                       (int) info, magma_tally2_strerror( info ));
