      ...
      for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( uplo[iuplo] == Magma_minproductLower || uplo[iuplo] == Magma_minproductUpper ) {
                ...
                gbytes = sizeof(magma_minproductDoubleComplex) * 1.*N*(N+1) / 1e9;
            }
            else {
            ...
            TESTING_MALLOC_CPU( h_B, magma_minproductDoubleComplex, size   );
            ...
            for( int j = 0; j < N; ++j ) {
                for( int i = 0; i < M; ++i ) {
                    ...
                    h_B[i + j*ldb] = MAGMA_minproduct_Z_MAKE( i - j/10000. + 10000., j );
                }
            }
            ...
            //magma_minproductblas_zlacpy( uplo[iuplo], M-2, N-2, d_A+1+ldda, ldda, d_B+1+lddb, lddb );  // inset by 1 row & col
            magma_minproductblas_zlacpy( uplo[iuplo], M, N, d_A, ldda, d_B, lddb );
            ...
            if ( opts.verbose ) {
                ...
                printf( "B= " );  magma_minproduct_zprint(     M, N, h_B, lda );
                printf( "dA=" );  magma_minproduct_zprint_gpu( M, N, d_A, ldda );
                printf( "dB=" );  magma_minproduct_zprint_gpu( M, N, d_B, ldda );
            }
            
            /* =====================================================================
               Check the result
               =================================================================== */
            magma_minproduct_zgetmatrix( M, N, d_B, ldda, h_R, lda );
