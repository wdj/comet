...
__global__ void ztranspose_inplace_odd( int n, magma_minproductDoubleComplex *matrix, int lda )
{
    __shared__ magma_minproductDoubleComplex sA[ NB ][ NB+1 ];
    __shared__ magma_minproductDoubleComplex sB[ NB ][ NB+1 ];
    ...
    magma_minproductDoubleComplex *A = matrix + ii+i + (jj+j)*lda;
    if( ii == jj ) {
        if ( ii+i < n && jj+j < n ) {
            sA[j][i] = *A;
        }
        ...
        magma_minproductDoubleComplex *B = matrix + jj+i + (ii+j)*lda;
        if ( ii+i < n && jj+j < n ) {
            sA[j][i] = *A;
        }
        if ( jj+i < n && ii+j < n ) {
...
__global__ void ztranspose_inplace_even( int n, magma_minproductDoubleComplex *matrix, int lda )
{
    __shared__ magma_minproductDoubleComplex sA[ NB ][ NB+1 ];
    __shared__ magma_minproductDoubleComplex sB[ NB ][ NB+1 ];
...
/**
    @see magma_minproductblas_ztranspose_inplace_q
    @ingroup magma_minproduct_zaux2
    ********************************************************************/
extern "C" void
magma_minproductblas_ztranspose_inplace(
    ...
    magma_minproductDoubleComplex_ptr dA, magma_minproduct_int_t ldda )
{
    magma_minproductblas_ztranspose_inplace_q( n, dA, ldda, magma_minproduct_stream );
}
