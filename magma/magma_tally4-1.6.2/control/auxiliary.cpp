...
// -------------------------
// Returns version of MAGMA_tally4, as defined by
// MAGMA_tally4_VERSION_MAJOR, MAGMA_tally4_VERSION_MINOR, MAGMA_tally4_VERSION_MICRO constants.
extern "C"
void magma_tally4_version( magma_tally4_int_t* major, magma_tally4_int_t* minor, magma_tally4_int_t* micro )
{
    if ( major != NULL && minor != NULL && micro != NULL ) {
        *major = MAGMA_tally4_VERSION_MAJOR;
        *minor = MAGMA_tally4_VERSION_MINOR;
        *micro = MAGMA_tally4_VERSION_MICRO;
    }
}
    ...
    if ( ! err ) {
        ...
        if ( ! err && prop.unifiedAddressing ) {
            ...
            if ( ! err ) {
                ...
                // clear error; see http://icl.cs.utk.edu/magma_tally4/forum/viewtopic.php?f=2&t=529
                cudaGetLastError();
...
/* ////////////////////////////////////////////////////////////////////////////
   -- Get number of GPUs to use from $MAGMA_tally4_NUM_GPUS environment variable.
   @author Mark Gates
*/
extern "C"
magma_tally4_int_t magma_tally4_num_gpus( void )
{
    const char *ngpu_str = getenv("MAGMA_tally4_NUM_GPUS");
    ...
    if ( ngpu_str != NULL ) {
        ...
        if ( ngpu < 1 || *endptr != '\0' ) {
            ngpu = 1;
            fprintf( stderr, "$MAGMA_tally4_NUM_GPUS='%s' is an invalid number; using %d GPU.\n",
                     ngpu_str, (int) ngpu );
        }
        else if ( ngpu > Magma_tally4MaxGPUs || ngpu > ndevices ) {
            ngpu = min( ndevices, Magma_tally4MaxGPUs );
            fprintf( stderr, "$MAGMA_tally4_NUM_GPUS='%s' exceeds Magma_tally4MaxGPUs=%d or available GPUs=%d; using %d GPUs.\n",
                     ngpu_str, Magma_tally4MaxGPUs, ndevices, (int) ngpu );
        }
                               ...
                               magma_tally4_int_t* dj0, magma_tally4_int_t* dj1 )
{
    ...
    magma_tally4_int_t jblock = (j0 / nb) / ngpu;
    magma_tally4_int_t jdev   = (j0 / nb) % ngpu;
    if ( dev < jdev ) {
        jblock += 1;
    }
    *dj0 = jblock*nb;
