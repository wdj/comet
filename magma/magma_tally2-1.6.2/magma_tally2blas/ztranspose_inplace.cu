...
__global__ void ztranspose_inplace_odd( int n, magma_tally2DoubleComplex *matrix, int lda )
{
    __shared__ magma_tally2DoubleComplex sA[ NB ][ NB+1 ];
    __shared__ magma_tally2DoubleComplex sB[ NB ][ NB+1 ];
    ...
    magma_tally2DoubleComplex *A = matrix + ii+i + (jj+j)*lda;
    if( ii == jj ) {
        if ( ii+i < n && jj+j < n ) {
            sA[j][i] = *A;
        }
        ...
        magma_tally2DoubleComplex *B = matrix + jj+i + (ii+j)*lda;
        if ( ii+i < n && jj+j < n ) {
            sA[j][i] = *A;
        }
        if ( jj+i < n && ii+j < n ) {
...
__global__ void ztranspose_inplace_even( int n, magma_tally2DoubleComplex *matrix, int lda )
{
    __shared__ magma_tally2DoubleComplex sA[ NB ][ NB+1 ];
    __shared__ magma_tally2DoubleComplex sB[ NB ][ NB+1 ];
...
/**
    @see magma_tally2blas_ztranspose_inplace_q
    @ingroup magma_tally2_zaux2
    ********************************************************************/
extern "C" void
magma_tally2blas_ztranspose_inplace(
    ...
    magma_tally2DoubleComplex_ptr dA, magma_tally2_int_t ldda )
{
    magma_tally2blas_ztranspose_inplace_q( n, dA, ldda, magma_tally2_stream );
}
