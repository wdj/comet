...
magma_tally4_malloc( magma_tally4_ptr* ptrPtr, size_t size )
{
    // CUDA can't allocate 0 bytes, so allocate some minimal size
    if ( size == 0 )
        size = sizeof(magma_tally4DoubleComplex);
...
// --------------------
// Free GPU memory allocated by magma_tally4_malloc.
extern "C" magma_tally4_int_t
magma_tally4_free_internal( magma_tally4_ptr ptr,
...
// Use magma_tally4_free_cpu() to free this memory.
extern "C" magma_tally4_int_t
magma_tally4_malloc_cpu( void** ptrPtr, size_t size )
{
    ...
    *ptrPtr = _aligned_malloc( size, 32 );
    if ( *ptrPtr == NULL ) {
        return MAGMA_tally4_ERR_HOST_ALLOC;
    }
#else
...
// --------------------
// Free CPU pinned memory previously allocated by magma_tally4_malloc_pinned.
...
extern "C" magma_tally4_int_t
magma_tally4_malloc_pinned( void** ptrPtr, size_t size )
{
    // CUDA can't allocate 0 bytes, so allocate some minimal size
...
// Free CPU pinned memory previously allocated by magma_tally4_malloc_pinned.
extern "C" magma_tally4_int_t
magma_tally4_free_pinned_internal( void* ptr,
    const char* func, const char* file, int line )
{
