...
/**
    ...
    @ingroup magma_tally3_dgeqrf_aux
    ********************************************************************/
    ...
    if ( Aks == NULL ) {
        *info = MAGMA_tally3_ERR_DEVICE_ALLOC;
        ...
        for (i = 0; i < k; ++i) {
            ...
            magma_tally3_dlarfg_gpu(m-i, dA(i, i), dA(min(i+1,m), i), dtau+i, dwork, &Aks[i]);

            if (n-i-1 > 0) {
               /* Apply H(i)' to A(i:m,i+1:n) from the left */
               magma_tally3_dlarf_gpu(m-i, n-i-1, dA(i, i), dtau+i, dA(i, i+1), ldda);
            }
        }

        magma_tally3_dcopymatrix( 1, k, Aks, 1, dA(0, 0), ldda+1 );
    }
