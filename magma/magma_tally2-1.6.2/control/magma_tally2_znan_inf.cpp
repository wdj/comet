...
const magma_tally2DoubleComplex MAGMA_tally2_Z_NAN
    = MAGMA_tally2_Z_MAKE( std::numeric_limits<double>::quiet_NaN(),
                    std::numeric_limits<double>::quiet_NaN() );

const magma_tally2DoubleComplex MAGMA_tally2_Z_INF
    = MAGMA_tally2_Z_MAKE( std::numeric_limits<double>::infinity(),
                    std::numeric_limits<double>::infinity() );

/** @return true if either real(x) or imag(x) is NAN. */
inline bool magma_tally2_z_isnan( magma_tally2DoubleComplex x )
{
#ifdef COMPLEX
    return isnan( MAGMA_tally2_Z_REAL( x )) ||
           isnan( MAGMA_tally2_Z_IMAG( x ));
#else
    return isnan( x );
#endif
}
...
inline bool magma_tally2_z_isinf( magma_tally2DoubleComplex x )
{
#ifdef COMPLEX
    return isinf( MAGMA_tally2_Z_REAL( x )) ||
           isinf( MAGMA_tally2_Z_IMAG( x ));
#else
    return isinf( x );
#endif
}
...
/**
    ...
    magma_tally2_znan_inf checks a matrix that is located on the CPU host
    ...
    ********************************************************************/
extern "C"
magma_tally2_int_t magma_tally2_znan_inf(
    ...
    if ( uplo == Magma_tally2Lower ) {
        for( int j = 0; j < n; ++j ) {
            for( int i = j; i < m; ++i ) {  // i >= j
                if      ( magma_tally2_z_isnan( *A(i,j) )) { c_nan++; }
                else if ( magma_tally2_z_isinf( *A(i,j) )) { c_inf++; }
            }
        }
    }
...
/**
    ...
    ********************************************************************/
extern "C"
magma_tally2_int_t magma_tally2_znan_inf_gpu(
    magma_tally2_uplo_t uplo, magma_tally2_int_t m, magma_tally2_int_t n,
    ...
    magma_tally2_int_t *cnt_inf )
{
    ...
    magma_tally2_int_t cnt = magma_tally2_znan_inf( uplo, m, n, A, lda, cnt_nan, cnt_inf );
