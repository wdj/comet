                  ...
                  magma_tally4_int_t batchCount, magma_tally4_queue_t queue)
{
    ...
    float *hu, *hv;
    if (MAGMA_tally4_SUCCESS != magma_tally4_smalloc_cpu( &hu, 2*n )) {
        info = MAGMA_tally4_ERR_HOST_ALLOC;
        return info;
    }

    if (MAGMA_tally4_SUCCESS != magma_tally4_smalloc_cpu( &hv, 2*n )) {
        info = MAGMA_tally4_ERR_HOST_ALLOC;
        return info;
    }
    ...
    info = magma_tally4_sgerbt_batched(Magma_tally4True, n, nrhs, dA_array, n, dB_array, n, hu, hv, &info, batchCount, queue);
    if (info != MAGMA_tally4_SUCCESS)  {
        return info;
    }


    info = magma_tally4_sgetrf_nopiv_batched( n, n, dA_array, ldda, info_array, batchCount, queue);
    if ( (info != MAGMA_tally4_SUCCESS) ){
        return info;
    }
    ...
    info = magma_tally4_sgetrs_nopiv_batched( Magma_tally4NoTrans, n, nrhs, dA_array, ldda, dB_array, lddb, info_array, batchCount, queue );


    /* The solution of A.x = b is Vy computed on the GPU */
    float *dv;

    if (MAGMA_tally4_SUCCESS != magma_tally4_smalloc( &dv, 2*n )) {
        info = MAGMA_tally4_ERR_DEVICE_ALLOC;
        return info;
    }

    magma_tally4_ssetvector(2*n, hv, 1, dv, 1);

    for(int i = 0; i < nrhs; i++)
        magma_tally4blas_sprbt_mv_batched(n, dv, dB_array+(i), batchCount, queue);
