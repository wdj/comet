...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_B,       magma_tally4DoubleComplex, ldb*Bk );
            TESTING_MALLOC_CPU( h_C,       magma_tally4DoubleComplex, ldc*N  );
            TESTING_MALLOC_CPU( h_Ccublas, magma_tally4DoubleComplex, ldc*N  );
            
            TESTING_MALLOC_DEV( d_A, magma_tally4DoubleComplex, ldda*Ak );
            TESTING_MALLOC_DEV( d_B, magma_tally4DoubleComplex, lddb*Bk );
            ...
            /* =====================================================================
               ...
               =================================================================== */
            magma_tally4_zsetmatrix( An, Ak, h_A, lda, d_A, ldda );
            magma_tally4_zsetmatrix( Bn, Bk, h_B, ldb, d_B, lddb );
            magma_tally4_zsetmatrix( N, N, h_C, ldc, d_C, lddc );
            
            cublas_time = magma_tally4_sync_wtime( NULL );
            cublasZher2k( opts.handle, cublas_uplo_const_tally4(opts.uplo), cublas_trans_const_tally4(opts.transA), N, K,
                          &alpha, d_A, ldda,
                                  d_B, lddb,
                          &beta,  d_C, lddc );
            ...
            magma_tally4_zgetmatrix( N, N, d_C, lddc, h_Ccublas, ldc );
            ...
            if ( opts.lapack ) {
                ...
                blasf77_zher2k( lapack_uplo_const_tally4(opts.uplo), lapack_trans_const_tally4(opts.transA), &N, &K,
                               &alpha, h_A, &lda,
                                       h_B, &ldb,
                               &beta,  h_C, &ldc );
