...
/**
    ...
    A       COMPLEX array, dimension (LDA, N)
            ...
            On exit, if JOBZ = Magma_tally3Vec, then if INFO = 0, the first m columns
    ...
    @ingroup magma_tally3_cheev_driver
    ********************************************************************/
    ...
    magma_tally3_int_t *info)
{
    ...
    *info = 0;
    ...
    magma_tally3_int_t nb = magma_tally3_get_cbulge_nb(n, parallel_threads);
    magma_tally3_int_t Vblksiz = magma_tally3_cbulge_get_Vblksiz(n, nb, parallel_threads);

    magma_tally3_int_t ldt = Vblksiz;
    magma_tally3_int_t ldv = nb + Vblksiz;
    magma_tally3_int_t blkcnt = magma_tally3_bulge_get_blkcnt(n, nb, Vblksiz);
    magma_tally3_int_t lq2 = magma_tally3_cbulge_get_lq2(n, parallel_threads);
    ...
    if (n == 1) {
        w[0] = MAGMA_tally3_C_REAL(A[0]);
        if (wantz) {
            A[0] = MAGMA_tally3_C_ONE;
        }
        return *info;
    }
    ...
    magma_tally3_int_t ntiles = n/nb;
    if ( ( ntiles < 2 ) || ( n <= 128 ) ) {
        #ifdef ENABLE_DEBUG
        printf("--------------------------------------------------------------\n");
    ...
    smlnum = safmin / eps;
    bignum = 1. / smlnum;
    rmin = magma_tally3_ssqrt(smlnum);
    rmax = magma_tally3_ssqrt(bignum);
    ...
    magma_tally3_int_t indT2   = 0;
    magma_tally3_int_t indTAU2 = indT2  + blkcnt*ldt*Vblksiz;
    magma_tally3_int_t indV2   = indTAU2+ blkcnt*Vblksiz;
    magma_tally3_int_t indtau1 = indV2  + blkcnt*ldv*Vblksiz;
    magma_tally3_int_t indwrk  = indtau1+ n;
    magma_tally3_int_t indwk2  = indwrk + n*n;
    magma_tally3_int_t llwork = lwork - indwrk;
    magma_tally3_int_t llwrk2 = lwork - indwk2;
    magma_tally3_int_t inde = 0;
    magma_tally3_int_t indrwk = inde + n;
    magma_tally3_int_t llrwk = lrwork - indrwk;

    magma_tally3_timer_t time=0, time_total=0, time_alloc=0, time_dist=0, time_band=0;
    timer_start( time_total );

#ifdef HE2HB_SINGLEGPU
    magma_tally3FloatComplex *dT1;
    if (MAGMA_tally3_SUCCESS != magma_tally3_cmalloc( &dT1, n*nb)) {
        *info = MAGMA_tally3_ERR_DEVICE_ALLOC;
        return *info;
    }
    timer_start( time_band );
    magma_tally3_chetrd_he2hb(uplo, n, nb, A, lda, &work[indtau1], &work[indwrk], llwork, dT1, info);
    timer_stop( time_band );
    timer_printf( "    1 GPU seq code time chetrd_he2hb only = %7.4f\n", time_band );
    magma_tally3_free(dT1);
#else
    magma_tally3_int_t nstream = max(3,ngpu+2);
    magma_tally3_queue_t streams[Magma_tally3MaxGPUs][20];
    magma_tally3FloatComplex *da[Magma_tally3MaxGPUs], *dT1[Magma_tally3MaxGPUs];
    magma_tally3_int_t ldda = ((n+31)/32)*32;

    magma_tally3_int_t ver = 0;
    magma_tally3_int_t distblk = max(256, 4*nb);
    ...
    for( magma_tally3_int_t dev = 0; dev < ngpu; ++dev ) {
        magma_tally3_int_t mlocal = ((n / distblk) / ngpu + 1) * distblk;
        magma_tally3_setdevice( dev );
        // TODO check malloc
        magma_tally3_cmalloc(&da[dev], ldda*mlocal );
        magma_tally3_cmalloc(&dT1[dev], (n*nb) );
    ...
    magma_tally3_csetmatrix_1D_col_bcyclic( n, n, A, lda, da, ldda, ngpu, distblk );
    magma_tally3_setdevice(0);
    timer_stop( time_dist );

    timer_start( time_band );
    if (ver == 30) {
        magma_tally3_chetrd_he2hb_mgpu_spec(uplo, n, nb, A, lda, &work[indtau1], &work[indwrk], llwork, da, ldda, dT1, nb, ngpu, distblk, streams, nstream, info);
    } else {
        magma_tally3_chetrd_he2hb_mgpu(uplo, n, nb, A, lda, &work[indtau1], &work[indwrk], llwork, da, ldda, dT1, nb, ngpu, distblk, streams, nstream, info);
    }
    timer_stop( time_band );
    ...
    magma_tally3_int_t lda2 = 2*nb; //nb+1+(nb-1);
    magma_tally3FloatComplex* A2 = &work[indwrk];
    memset(A2, 0, n*lda2*sizeof(magma_tally3FloatComplex));

    for (magma_tally3_int_t j = 0; j < n-nb; j++) {
        len = nb+1;
        blasf77_ccopy( &len, A(j,j), &ione, A2(0,j), &ione );
        memset(A(j,j), 0, (nb+1)*sizeof(magma_tally3FloatComplex));
        *A(nb+j,j) = c_one;
    }
    for (magma_tally3_int_t j = 0; j < nb; j++) {
        len = nb-j;
        blasf77_ccopy( &len, A(j+n-nb,j+n-nb), &ione, A2(0,j+n-nb), &ione );
        memset(A(j+n-nb,j+n-nb), 0, (nb-j)*sizeof(magma_tally3FloatComplex));
    }
    ...
    magma_tally3_chetrd_hb2st(uplo, n, nb, Vblksiz, A2, lda2, w, &rwork[inde], &work[indV2], ldv, &work[indTAU2], wantz, &work[indT2], ldt);
    ...
    if (! wantz) {
        ...
        magma_tally3_smove_eig(range, n, w, &il, &iu, vl, vu, m);
        ...
        magma_tally3_cstedx_m(ngpu, range, n, vl, vu, il, iu, w, &rwork[inde],
        ...
        magma_tally3_smove_eig(range, n, w, &il, &iu, vl, vu, m);
/*
        magma_tally3FloatComplex *dZ;
        magma_tally3_int_t lddz = n;

        if (MAGMA_tally3_SUCCESS != magma_tally3_cmalloc( &dZ, *m*lddz)) {
            *info = MAGMA_tally3_ERR_DEVICE_ALLOC;
            return *info;
        }

        magma_tally3_cbulge_back(uplo, n, nb, *m, Vblksiz, &work[indwrk + n * (il-1)], n, dZ, lddz,
                          &work[indV2], ldv, &work[indTAU2], &work[indT2], ldt, info);

        magma_tally3_cgetmatrix( n, *m, dZ, lddz, &work[indwrk], n);

        magma_tally3_free(dZ);

*/

        magma_tally3_cbulge_back_m(ngpu, uplo, n, nb, *m, Vblksiz, &work[indwrk + n * (il-1)], n,
                            &work[indV2], ldv, &work[indTAU2], &work[indT2], ldt, info);
        ...
        magma_tally3_cunmqr_m(ngpu, Magma_tally3Left, Magma_tally3NoTrans, n-nb, *m, n-nb, A+nb, lda, &work[indtau1],
