...
/**
    ...
    This is an out-of-core (ooc) version that is similar to magma_tally2_dgeqrf but
    ...
    magma_tally2_int_t *info )
{
    ...
    *info = 0;
    ...
    if (NB >= n)
        return magma_tally2_dgeqrf(m, n, A, lda, tau, work, lwork, info);
    ...
    if (MAGMA_tally2_SUCCESS != magma_tally2_dmalloc( &dA, (NB + nb)*ldda + nb*lddwork )) {
    ...
    for (int i=0; i < n; i += NB) {
        ...
        magma_tally2_dsetmatrix_async( (m), IB,
                                A(0,i),  lda,
        ...
        for (int j=0; j < min(i,k); j += nb) {
            ...
            dpanel_to_q_tally2(Magma_tally2Upper, ib, A(j,j), lda, work+ib*ib);
            ...
            dq_to_panel_tally2(Magma_tally2Upper, ib, A(j,j), lda, work+ib*ib);
        }

        /* 3. Do a QR on the current part */
        if (i < k)
            magma_tally2_dgeqrf2_gpu(m-i, IB, dA(i,0), ldda, tau+i, info);

        /* 4. Copy the current part back to the CPU */
        magma_tally2_dgetmatrix_async( (m), IB,
                                dA(0,0), ldda,
                                A(0,i),  lda, stream[0] );
    }
