...
int main( int argc, char** argv )
{
    ...
    magma_tally4_int_t m, n, k, mi, ni, mm, nn, nq, size, info;
    magma_tally4_int_t ISEED[4] = {0,0,0,1};
    magma_tally4_int_t nb, ldc, lda, lwork, lwork_max;
    magma_tally4DoubleComplex *C, *R, *A, *work, *tau, *tauq, *taup;
    double *d, *e;
    ...
    magma_tally4_vect_t  vect [] = { Magma_tally4Q,          Magma_tally4P       };
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int itran = 0; itran < 2; ++itran ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            n = opts.nsize[itest];
            k = opts.ksize[itest];
            nb  = magma_tally4_get_zgebrd_nb( m );
            ldc = m;
            // A is nq x k (vect=Q) or k x nq (vect=P)
            // where nq=m (left) or nq=n (right)
            nq  = (side[iside] == Magma_tally4Left ? m  : n );
            mm  = (vect[ivect] == Magma_tally4Q    ? nq : k );
            nn  = (vect[ivect] == Magma_tally4Q    ? k  : nq);
            lda = mm;
            
            // MBR calls either MQR or MLQ in various ways
            if ( vect[ivect] == Magma_tally4Q ) {
                if ( nq >= k ) {
            ...
            TESTING_MALLOC_CPU( A,    magma_tally4DoubleComplex, lda*nn );
            TESTING_MALLOC_CPU( work, magma_tally4DoubleComplex, lwork_max );
            TESTING_MALLOC_CPU( d,    double,             min(mm,nn) );
            TESTING_MALLOC_CPU( e,    double,             min(mm,nn) );
            TESTING_MALLOC_CPU( tauq, magma_tally4DoubleComplex, min(mm,nn) );
            TESTING_MALLOC_CPU( taup, magma_tally4DoubleComplex, min(mm,nn) );
            ...
            magma_tally4_zgebrd( mm, nn, A, lda, d, e, tauq, taup, work, lwork_max, &info );
            if (info != 0)
                printf("magma_tally4_zgebrd returned error %d: %s.\n",
            ...
            lapackf77_zunmbr( lapack_vect_const_tally4( vect[ivect] ),
                              lapack_side_const_tally4( side[iside] ),
                              lapack_trans_const_tally4( trans[itran] ),
                              &m, &n, &k,
            ...
            lwork = -1;
            magma_tally4_zunmbr( vect[ivect], side[iside], trans[itran],
                          m, n, k,
                          A, lda, tau, R, ldc, work, lwork, &info );
            if (info != 0)
                printf("magma_tally4_zunmbr (lwork query) returned error %d: %s.\n",
                       (int) info, magma_tally4_strerror( info ));
            ...
            if (info != 0)
                printf("magma_tally4_zunmbr returned error %d: %s.\n",
                       (int) info, magma_tally4_strerror( info ));
            ...
            printf( "%5d %5d %5d   %c   %4c   %5c   %7.2f (%7.2f)   %7.2f (%7.2f)   %8.2e   %s\n",
                    (int) m, (int) n, (int) k,
                    lapacke_vect_const_tally4( vect[ivect] ),
