...
/////////////////////////////////////////////////////////////////////////////////////////////////
__global__ void sdot_kernel_batched(int n, float **x_array, int incx, int offset, magma_tally3_int_t *info_array, int gbstep)
{
    int tx = threadIdx.x;
    ...
    sdata[tx] = MAGMA_tally3_S_REAL(res * MAGMA_tally3_S_CNJG(res));
    ...
    if (tx == 0) {
        float xreal = MAGMA_tally3_S_REAL(x[n*incx]);        
        //MAGMA_tally3_S_SET2REAL(x[n*incx], sqrt(xreal - sdata[0]));
        x[n*incx] = MAGMA_tally3_S_MAKE(sqrt(xreal - sdata[0]), 0);
        if(x[n*incx] == MAGMA_tally3_S_ZERO){
...
void magma_tally3_spotf2_sdot_batched(magma_tally3_int_t n, float **x_array, magma_tally3_int_t incx, magma_tally3_int_t offset, magma_tally3_int_t *info_array, magma_tally3_int_t gbstep, magma_tally3_int_t batchCount, magma_tally3_queue_t queue)
{
/*
    Specialized Sdot
...
/////////////////////////////////////////////////////////////////////////////////////////////////
__global__ void sscal_kernel_batched(int n, float **x_array, int incx, int offset, magma_tally3_int_t *info_array)
{
    ...
    if (threadIdx.x == 0) {
        factor = MAGMA_tally3_S_MAKE(1.0/MAGMA_tally3_S_REAL(x[0]), 0.0);
    }
...
void magma_tally3_spotf2_sscal_batched(magma_tally3_int_t n, float **x_array, magma_tally3_int_t incx, magma_tally3_int_t offset, magma_tally3_int_t *info_array, magma_tally3_int_t batchCount, magma_tally3_queue_t queue)
{
/*
...
__global__ void slacgv_kernel_batched(int n, float **x_array, int incx, int offset)
{
    ...
    if ( id < n ) {
        x[id*incx] = MAGMA_tally3_S_CNJG(x[id*incx]);
    }
}

void magma_tally3_slacgv_batched(magma_tally3_int_t n, float **x_array, magma_tally3_int_t incx, int offset, int batchCount, magma_tally3_queue_t queue)
{
/*
    Purpose
    =======
...
static __device__ void spotf2_device(int m, int n, 
                              float *A, int lda, 
                              float alpha, 
                              float beta, magma_tally3_int_t *info, int gbstep)
{
/*
...
*/
    ...
    for(int iter=0; iter<n; iter++)
    {
        ...
        float res1 = MAGMA_tally3_S_ZERO;

        //1) performs sdot sum = A[iter, 0:iter-1]*conj(A[iter, 0:iter-1])
        //2) updates A[iter,iter] = sqrt(A[iter,iter]-sum);
        if(tx<iter)
        {
            res = MAGMA_tally3_S_REAL (sdata_A[iter + tx * m] * MAGMA_tally3_S_CNJG(sdata_A[iter + tx * m]));         
            sum[tx] = res;
        }
        else
        {
        ...
        __syncthreads();
        magma_tally3_sum_reduce<POTF2_TILE_SIZE>(tx, sum);//tried on K40: if m=32 n=32 the overall spotf2_device routine time is 60ms n=16 time=25 n=8 time=20ms 
        ...
        if (tx == 0) {
              float xreal = MAGMA_tally3_S_REAL(sdata_A[iter + iter * m]);        
              sdata_A[iter + iter * m] = MAGMA_tally3_S_MAKE(sqrt(xreal - sum[0]), 0);
              if(sdata_A[iter + iter * m] == MAGMA_tally3_S_ZERO){
                  *info = iter + gbstep + 1;
              }
        }
        __syncthreads();
        if(sdata_A[iter + iter * m] == MAGMA_tally3_S_ZERO) return;
        __syncthreads();
        ...
        if(tx < iter)
        {
             sdata_A[iter + tx * m] = MAGMA_tally3_S_CNJG(sdata_A[iter + tx * m]);
        }
        __syncthreads();  
        #endif
        ...
        if (tx == 0) {
            factor = MAGMA_tally3_S_MAKE(1.0/MAGMA_tally3_S_REAL(sdata_A[iter + iter * m]), 0.0);
        }
        __syncthreads();
...
/**
    ...
    @ingroup magma_tally3_sposv_aux
    ********************************************************************/
    ...
    magma_tally3_int_t *info_array, magma_tally3_int_t gbstep, magma_tally3_int_t batchCount, magma_tally3_queue_t queue)
{
    ...
    float alpha = MAGMA_tally3_S_NEG_ONE;
    float beta  = MAGMA_tally3_S_ONE;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
extern "C" magma_tally3_int_t
magma_tally3_spotf2_tile(
    magma_tally3_uplo_t uplo, magma_tally3_int_t m, magma_tally3_int_t n,
    float *dA, magma_tally3_int_t lda,
    magma_tally3_int_t *info)
{
    ...
    *info = 0;
    ...
    spotf2_kernel<<<dimGrid, threads, shared_mem_size, magma_tally3_stream >>>(m, n, dA, lda, alpha, beta, info);
