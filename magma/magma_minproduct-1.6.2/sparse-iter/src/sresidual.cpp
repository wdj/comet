    ...
    magma_minproduct_queue_t queue )
{
    ...
    float zero = MAGMA_minproduct_S_ZERO, one = MAGMA_minproduct_S_ONE,
                                            mone = MAGMA_minproduct_S_NEG_ONE;
    magma_minproduct_int_t dofs = A.num_rows;
    magma_minproduct_int_t num_vecs = b.num_rows*b.num_cols/A.num_rows;
    
    magma_minproduct_s_matrix r={Magma_minproduct_CSR};
    
    if ( A.num_rows == b.num_rows ) {
        CHECK( magma_minproduct_svinit( &r, Magma_minproduct_DEV, A.num_rows, b.num_cols, zero, queue ));

        CHECK( magma_minproduct_s_spmv( one, A, x, zero, r, queue ));           // r = A x
        magma_minproduct_saxpy(dofs, mone, b.dval, 1, r.dval, 1);          // r = r - b
        *res =  magma_minproduct_snrm2(dofs, r.dval, 1);            // res = ||r||
        //               /magma_minproduct_snrm2(dofs, b.dval, 1);               /||b||
        //printf( "relative residual: %e\n", *res );

    } else if ((b.num_rows*b.num_cols)%A.num_rows== 0 ) {
        
        CHECK( magma_minproduct_svinit( &r, Magma_minproduct_DEV, b.num_rows,b.num_cols, zero, queue ));

        CHECK( magma_minproduct_s_spmv( one, A, x, zero, r, queue ));           // r = A x

        for( magma_minproduct_int_t i=0; i<num_vecs; i++) {
            magma_minproduct_saxpy(dofs, mone, b(i), 1, r(i), 1);   // r = r - b
            res[i] =  magma_minproduct_snrm2(dofs, r(i), 1);        // res = ||r||
        }
        //               /magma_minproduct_snrm2(dofs, b.dval, 1);               /||b||
        //printf( "relative residual: %e\n", *res );
