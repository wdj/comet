...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lhwork2 = (magma_tally2_int_t) MAGMA_tally2_Z_REAL( tmp[0] );
            
            lhwork = -1;
            lapackf77_zunmqr( Magma_tally2LeftStr, Magma_tally2_ConjTransStr,
            ...
            magma_tally2_zgels3_gpu( Magma_tally2NoTrans, M, N, nrhs, d_A, ldda,
                              d_B, lddb, h_work, lworkgpu, &info);
            gpu_time = magma_tally2_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally2_zgels3_gpu returned error %d: %s.\n",
