...
/**
    ...
    @ingroup magma_minproduct_cgesv_tile
    ********************************************************************/
...
#define hL2(i)  (hL2 + (i)*ib*ldhl          )
    ...
    magma_minproductFloatComplex_ptr dAT, dUT;
    magma_minproductFloatComplex_ptr dAp, dUp;
#ifndef WITHOUTTRTRI
    magma_minproductFloatComplex_ptr dL2 = dL + ib;
    magma_minproductFloatComplex *hL2 = hL + ib;
    p = 2;
#endif
    ...
    *info = 0;
    ...
    if ( ib >= mindim ) {
        ...
        if ( order == Magma_minproductRowMajor ) {
            magma_minproduct_csetmatrix( m, n, hU, ldhu, dwork, lddwork );
            magma_minproductblas_ctranspose( m, n, dwork, lddwork, dU, lddu );

            magma_minproduct_csetmatrix( m, n, hA, ldha, dwork, lddwork );
            magma_minproductblas_ctranspose( m, n, dwork, lddwork, dA, ldda );
        } else {
            magma_minproduct_csetmatrix( m, n, hU, ldhu, dU, lddu );
            magma_minproduct_csetmatrix( m, n, hA, ldha, dA, ldda );
        }
        magma_minproduct_csetmatrix( p*ib, n, hL, ldhl, dL, lddl );
        ...
        if ( order == Magma_minproductColMajor ) {
            magma_minproductblas_cgetmo_in( dU, dUT, lddu, m,  n );
            magma_minproductblas_cgetmo_in( dA, dAT, ldda, m,  n );
        } else {
            dUT = dU; dAT = dA;
        }
        dAp = dwork;
        ...
        for( i=0; i < s; i++ ) {
            ...
            if ( i > 0 ) {
                // download i-th panel
                magma_minproductblas_ctranspose( sb, ii, UT(0,i), lddu, dUp, lddu );
                magma_minproductblas_ctranspose( sb, m,  AT(0,i), ldda, dAp, ldda );
                
                magma_minproduct_cgetmatrix( ii, sb, dUp, lddu, hU(0, i), ldhu );
                magma_minproduct_cgetmatrix( m,  sb, dAp, ldda, hA(0, i), ldha );
                
                // make sure that gpu queue is empty
                //magma_minproduct_device_sync();
                
#ifndef WITHOUTTRTRI
                magma_minproduct_ctrmm( Magma_minproductRight, Magma_minproductLower, Magma_minproductTrans, Magma_minproductUnit,
                             n-(ii+sb), ib,
                             c_one, L2(i-1),      lddl,
                                    UT(i-1, i+1), lddu);
#else
                magma_minproduct_ctrsm( Magma_minproductRight, Magma_minproductLower, Magma_minproductTrans, Magma_minproductUnit,
                             n-(ii+sb), ib,
                             c_one, L(i-1),       lddl,
                                    UT(i-1, i+1), lddu);
#endif
...
#ifndef NOSWAPBLK
            magma_minproductblas_cswapblk( Magma_minproductRowMajor, n-(ii+sb),
                                UT(i, i+1), lddu,
                                AT(0, i+1), ldda,
                                1, sb, ipiv+ii, 1, nb );
            ...
            for (j=0; j < ib; j++) {
                ...
                if ( im != (j) ) {
                    im = im - nb;
                    assert( (im >= 0) && (im < m) );
                    magma_minproductblas_cswap( n-(ii+sb), UT(i, i+1)+j*lddu, 1, AT(0, i+1)+im*ldda, 1 );
                } else {
                    ipiv[ip] += ii;
                }
                ip++;
...
#endif
            // upload i-th panel
            magma_minproduct_csetmatrix( sb, sb, hU(i, i), ldhu, dUp, lddu );
            magma_minproduct_csetmatrix( m, sb, hA(0, i), ldha, dAp, ldda );
            magma_minproduct_csetmatrix( p*ib, sb, hL(i), ldhl, L(i), lddl );
            magma_minproductblas_ctranspose( sb, sb, dUp, lddu, UT(i,i), lddu );
            magma_minproductblas_ctranspose( m,  sb, dAp, ldda, AT(0,i), ldda );
        ...
        if ( order == Magma_minproductColMajor ) {
            magma_minproductblas_cgetmo_out( dU, dUT, lddu, m,  n );
            magma_minproductblas_cgetmo_out( dA, dAT, ldda, m,  n );
        }
    }
    return *info;
}
