    ...
    magma_tally3FloatComplex **dC_array, magma_tally3_int_t lddc, magma_tally3_int_t batchCount, magma_tally3_queue_t queue )
{
    ...
    if      ( transB == Magma_tally3Trans )
        ...
        magma_tally3FloatComplex *cpuAarray[2], *cpuBarray[2];
        magma_tally3_getvector( min(2,batchCount), sizeof(magma_tally3FloatComplex*), dA_array, 1, cpuAarray, 1);
        magma_tally3_getvector( min(2,batchCount), sizeof(magma_tally3FloatComplex*), dB_array, 1, cpuBarray, 1);
        ...
        cudaError_t err;
        err = cudaBindTexture(&offsetA, tex_ref_A, cpuAarray[0], sizeA*sizeof(magma_tally3FloatComplex));
        if ( err != cudaSuccess ) {
            fprintf( stderr, "cannot bind A to texture: %s (%d)\n", cudaGetErrorString(err), err );
            return;
        }	
        err = cudaBindTexture(&offsetB, tex_ref_B, cpuBarray[0], sizeB*sizeof(magma_tally3FloatComplex));
        if ( err != cudaSuccess ) {
    ...
    offsetA = offsetA/sizeof(magma_tally3FloatComplex);
    offsetB = offsetB/sizeof(magma_tally3FloatComplex);
