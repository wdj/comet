...
magma_minproduct_malloc( magma_minproduct_ptr* ptrPtr, size_t size )
{
    // CUDA can't allocate 0 bytes, so allocate some minimal size
    if ( size == 0 )
        size = sizeof(magma_minproductDoubleComplex);
...
// --------------------
// Free GPU memory allocated by magma_minproduct_malloc.
extern "C" magma_minproduct_int_t
magma_minproduct_free_internal( magma_minproduct_ptr ptr,
...
// Use magma_minproduct_free_cpu() to free this memory.
extern "C" magma_minproduct_int_t
magma_minproduct_malloc_cpu( void** ptrPtr, size_t size )
{
    ...
    *ptrPtr = _aligned_malloc( size, 32 );
    if ( *ptrPtr == NULL ) {
        return MAGMA_minproduct_ERR_HOST_ALLOC;
    }
#else
...
// --------------------
// Free CPU pinned memory previously allocated by magma_minproduct_malloc_pinned.
...
extern "C" magma_minproduct_int_t
magma_minproduct_malloc_pinned( void** ptrPtr, size_t size )
{
    // CUDA can't allocate 0 bytes, so allocate some minimal size
...
// Free CPU pinned memory previously allocated by magma_minproduct_malloc_pinned.
extern "C" magma_minproduct_int_t
magma_minproduct_free_pinned_internal( void* ptr,
    const char* func, const char* file, int line )
{
