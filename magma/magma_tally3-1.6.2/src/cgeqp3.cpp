...
/**
    ...
    @ingroup magma_tally3_cgeqp3_comp
    ********************************************************************/
...
#define dA(i, j) (dwork + (i) + (j)*(ldda))
    ...
    magma_tally3FloatComplex   *dwork, *df;

    magma_tally3_int_t ione = 1;

    magma_tally3_int_t n_j, ldda, ldwork;
    magma_tally3_int_t j, jb, na, nb, sm, sn, fjb, nfxd, minmn;
    magma_tally3_int_t topbmn, sminmn, lwkopt=0, lquery;
    ...
    *info = 0;
    ...
    nb = magma_tally3_get_cgeqp3_nb(min(m, n));
    minmn = min(m,n);
    if (*info == 0) {
        if (minmn == 0) {
            lwkopt = 1;
    ...
    if (MAGMA_tally3_SUCCESS != magma_tally3_cmalloc( &dwork, ldwork )) {
    ...
    if (nfxd > 0) {
        ...
        if (na < n) {
            n_j = n - na;
            lapackf77_cunmqr( Magma_tally3LeftStr, Magma_tally3ConjTransStr, &m, &n_j, &na,
    ...
    if (nfxd < minmn) {
        ...
        if (nb < sminmn) {
            ...
            magma_tally3_csetmatrix_async( m, sn,
                                    A (0,j), lda,
                                    dA(0,j), ldda, stream );
        }
        ...
        for (j = nfxd; j < n; ++j) {
            rwork[j] = magma_tally3_cblas_scnrm2( sm, A(nfxd,j), ione );
            rwork[n + j] = rwork[j];
        }
        ...
        if (nb < sminmn) {
            ...
            while(j < topbmn) {
                ...
                if (j > nfxd) {
                    ...
                    magma_tally3_cgetmatrix( m-j, jb,
                                      dA(j,j), ldda,
                                      A (j,j), lda );
                    
                    // Get the rows
                    magma_tally3_cgetmatrix( jb, n_j - jb,
                                      dA(j,j + jb), ldda,
                                      A (j,j + jb), lda );
                }

                magma_tally3_claqps( m, n_j, j, jb, &fjb,
                              A (0, j), lda,
                              dA(0, j), ldda,
        ...
        if (j < minmn) {
            n_j = n - j;
            if (j > nfxd) {
                magma_tally3_cgetmatrix( m-j, n_j,
                                  dA(j,j), ldda,
                                  A (j,j), lda );
            }
