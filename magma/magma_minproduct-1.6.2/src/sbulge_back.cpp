...
static void *magma_minproduct_sapplyQ_parallel_section(void *arg);

static void magma_minproduct_stile_bulge_applyQ(magma_minproduct_int_t core_id, magma_minproduct_side_t side, magma_minproduct_int_t n_loc, magma_minproduct_int_t n, magma_minproduct_int_t nb, magma_minproduct_int_t Vblksiz,
                                     float *E, magma_minproduct_int_t lde, float *V, magma_minproduct_int_t ldv,
                                     float *TAU, float *T, magma_minproduct_int_t ldt);

//////////////////////////////////////////////////////////////////////////////////////////////////////////

typedef struct magma_minproduct_sapplyQ_data_s {
...
} magma_minproduct_sapplyQ_data;

void magma_minproduct_sapplyQ_data_init(
    magma_minproduct_sapplyQ_data *zapplyQ_data, magma_minproduct_int_t threads_num, magma_minproduct_int_t n, magma_minproduct_int_t ne, magma_minproduct_int_t n_gpu,
    magma_minproduct_int_t nb, magma_minproduct_int_t Vblksiz, float *E, magma_minproduct_int_t lde,
    float *V, magma_minproduct_int_t ldv, float *TAU,
    float *T, magma_minproduct_int_t ldt, float *dE, magma_minproduct_int_t ldde)
{
...
void magma_minproduct_sapplyQ_data_destroy(
    magma_minproduct_sapplyQ_data *zapplyQ_data)
{
    pthread_barrier_destroy(&(zapplyQ_data->barrier));
}

typedef struct magma_minproduct_sapplyQ_id_data {
    magma_minproduct_int_t id;
    magma_minproduct_sapplyQ_data* data;
} magma_minproduct_sapplyQ_id_data;

void magma_minproduct_sapplyQ_id_data_init(
    magma_minproduct_sapplyQ_id_data *zapplyQ_id_data,
    magma_minproduct_int_t id, magma_minproduct_sapplyQ_data* data)
{
...
//#endif
    ...
    magma_minproduct_int_t gpu_cpu_perf = magma_minproduct_get_sbulge_gcperf();
    if (threads > 1) {
...
//$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
    ...
    if (n_gpu < ne) {
        ...
        #endif
        magma_minproduct_sapplyQ_data data_applyQ;
        magma_minproduct_sapplyQ_data_init(&data_applyQ, threads, n, ne, n_gpu, nb, Vblksiz, Z, ldz, V, ldv, TAU, T, ldt, dZ, lddz);

        magma_minproduct_sapplyQ_id_data* arg;
        magma_minproduct_malloc_cpu((void**) &arg, threads*sizeof(magma_minproduct_sapplyQ_id_data));
        ...
        for (magma_minproduct_int_t thread = 1; thread < threads; thread++) {
            magma_minproduct_sapplyQ_id_data_init(&(arg[thread]), thread, &data_applyQ);
            pthread_create(&thread_id[thread], &thread_attr, magma_minproduct_sapplyQ_parallel_section, &arg[thread]);
        }
        magma_minproduct_sapplyQ_id_data_init(&(arg[0]), 0, &data_applyQ);
        magma_minproduct_sapplyQ_parallel_section(&arg[0]);
        ...
        magma_minproduct_sapplyQ_data_destroy(&data_applyQ);


        magma_minproduct_ssetmatrix(n, ne-n_gpu, Z + n_gpu*ldz, ldz, dZ + n_gpu*ldz, lddz);

        /*============================
         *  use only GPU
         *==========================*/
    } else {
        magma_minproduct_ssetmatrix(n, ne, Z, ldz, dZ, lddz);
        magma_minproduct_sbulge_applyQ_v2(Magma_minproductLeft, ne, n, nb, Vblksiz, dZ, lddz, V, ldv, T, ldt, info);
...
static void *magma_minproduct_sapplyQ_parallel_section(void *arg)
{
    magma_minproduct_int_t my_core_id   = ((magma_minproduct_sapplyQ_id_data*)arg) -> id;
    magma_minproduct_sapplyQ_data* data = ((magma_minproduct_sapplyQ_id_data*)arg) -> data;
    ...
    if (my_core_id == 0) {
        ...
        magma_minproduct_ssetmatrix(n, n_gpu, E, lde, dE, ldde);
        magma_minproduct_sbulge_applyQ_v2(Magma_minproductLeft, n_gpu, n, nb, Vblksiz, dE, ldde, V, ldv, T, ldt, &info);
        magma_minproduct_device_sync();
        ...
        magma_minproduct_stile_bulge_applyQ(my_core_id, Magma_minproductLeft, n_loc, n, nb, Vblksiz, E_loc, lde, V, ldv, TAU, T, ldt);
...
#define TAU(m)   &(TAU[(m)])
#define T(m)     &(T[(m)])
static void magma_minproduct_stile_bulge_applyQ(
    ...
    //, magma_minproduct_int_t* info)
{
    ...
    magma_minproduct_smalloc_cpu(&work2, lwork);
    ...
    for (magma_minproduct_int_t i = 0; i < nbchunk; i++) {
        ...
        if (side == Magma_minproductLeft) {
            for (bg = nbGblk; bg > 0; bg--) {
                ...
                if (bg == nbGblk) rownbm = magma_minproduct_ceildiv((n-(firstcolj)),nb);  // last blk has size=1 used for real to handle A(N,N-1)
