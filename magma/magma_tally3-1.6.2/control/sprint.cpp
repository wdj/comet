...
/**
    ...
    magma_tally3_sprint prints a matrix that is located on the CPU host.
    ...
    const float *A, magma_tally3_int_t lda )
{
    ...
    for( int i = 0; i < m; ++i ) {
        for( int j = 0; j < n; ++j ) {
            if ( MAGMA_tally3_S_EQUAL( *A(i,j), c_zero )) {
                #ifdef COMPLEX
                printf( "   0.              " );
                #else
                printf( "   0.    " );
                ...
                #ifdef COMPLEX
                printf( " %8.4f+%8.4fi", MAGMA_tally3_S_REAL( *A(i,j) ), MAGMA_tally3_S_IMAG( *A(i,j) ));
                #else
                printf( " %8.4f", MAGMA_tally3_S_REAL( *A(i,j) ));
                #endif
            }
        }
        if ( m > 1 ) {
...
/**
    Purpose
    -------
    magma_tally3_sprint_gpu prints a matrix that is located on the GPU device.
...
void magma_tally3_sprint_gpu(
    magma_tally3_int_t m, magma_tally3_int_t n,
    const float *dA, magma_tally3_int_t ldda )
{
    magma_tally3_int_t info = 0;
    if ( m < 0 )
        info = -1;
