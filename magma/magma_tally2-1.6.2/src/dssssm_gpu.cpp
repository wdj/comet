/*
    -- MAGMA_tally2 (version 1.6.1) --
       ...
       @generated from zssssm_gpu.cpp normal z -> d, Fri Jan 30 19:00:15 2015
...
/**
    ...
    @param[in,out]
    dA1     DOUBLE_PRECISION array, dimension(LDDA1, N), on gpu.
    ...
    @param[in,out]
    dA2     DOUBLE_PRECISION array, dimension(LDDA2, N), on gpu.
    ...
    @param[in]
    dL1     DOUBLE_PRECISION array, dimension(LDDL1, K), on gpu.
    ...
    @param[in]
    dL2     DOUBLE_PRECISION array, dimension(LDDL2, K)
            The M2-by-K tile as returned by DTSTRF.
...
#define L2(i,j)  (dL2  + (i)*lddl2i + (j)*lddl2j)
    ...
    int ip, ii, sb;
    magma_tally2Double_ptr dA1T, dA2T;
    magma_tally2_trans_t transL;
    int lddl2i, lddl2j;
    
    MAGMA_tally2_UNUSED( ip );  // used only if NOSWAPBLK
    ...
    *info = 0;
    ...
    if ( order == Magma_tally2ColMajor ) {
        magma_tally2blas_dgetmo_in( dA1, dA1T, ldda1, m1, n1 );
        magma_tally2blas_dgetmo_in( dA2, dA2T, ldda2, m2, n2 );
        transL = Magma_tally2Trans;
        lddl2i = 1; lddl2j = lddl2;
    } else {
        dA1T = dA1;
        dA2T = dA2;
        transL = Magma_tally2NoTrans;
    ...
    for( ii=0; ii < k; ii += ib ) {
...
#ifndef NOSWAPBLK
        magma_tally2blas_dswapblk( Magma_tally2RowMajor, n1,
                            A1T(0, 0), ldda1,
                            A2T(0, 0), ldda2,
...
#else
        {
            ...
            for (i=0; i < ib; i++) {
                ...
                if (im != (ii+i)) {
                    ...
                    magma_tally2blas_dswap( n1, A1T(ii+i, 0), 1, A2T(im, 0), 1 );
                }
                ip++;
            }
        }
        ...
        magma_tally2_dgemm( Magma_tally2NoTrans, transL,
                     n2, m2, sb,
    ...
    if ( order == Magma_tally2ColMajor ) {
        magma_tally2blas_dgetmo_out( dA1, dA1T, ldda1, m1, n1 );
        magma_tally2blas_dgetmo_out( dA2, dA2T, ldda2, m2, n2 );
    }
    return *info;
}
