...
int main( int argc, char** argv)
{
    ...
    magma_minproductFloatComplex *h_x, *h_x2, *h_tau, *h_tau2;
    magma_minproductFloatComplex_ptr d_x, d_tau;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_x,    magma_minproductFloatComplex, N*nb );
            TESTING_MALLOC_CPU( h_x2,   magma_minproductFloatComplex, N*nb );
            TESTING_MALLOC_CPU( h_tau,  magma_minproductFloatComplex, nb   );
            TESTING_MALLOC_CPU( h_tau2, magma_minproductFloatComplex, nb   );
        
            TESTING_MALLOC_DEV( d_x,   magma_minproductFloatComplex, ldda*nb );
            TESTING_MALLOC_DEV( d_tau, magma_minproductFloatComplex, nb      );
            ...
            /* =====================================================================
               ...
               =================================================================== */
            magma_minproduct_csetmatrix( N, nb, h_x, N, d_x, ldda );
    
            gpu_time = magma_minproduct_sync_wtime( queue );
            for( int j = 0; j < nb; ++j ) {
                magma_minproductblas_clarfg( N, &d_x[0+j*ldda], &d_x[1+j*ldda], ione, &d_tau[j] );
            }
            gpu_time = magma_minproduct_sync_wtime( queue ) - gpu_time;
            gpu_perf = gflops / gpu_time;
            
            magma_minproduct_cgetmatrix( N, nb, d_x, ldda, h_x2, N );
            magma_minproduct_cgetvector( nb, d_tau, 1, h_tau2, 1 );
