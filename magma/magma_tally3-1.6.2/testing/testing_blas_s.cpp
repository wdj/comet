/*
    -- MAGMA_tally3 (version 1.6.1) --
       ...
       correct, for example,
       magma_tally3_strmm(...) and cublasStrmm(...) produce /exactly/ the same results.
...
int main( int argc, char** argv )
{
    ...
    float  *A,  *B,  *C,   *C2, *LU;
    magma_tally3Float_ptr dA, dB, dC1, dC2;
    float alpha = MAGMA_tally3_S_MAKE( 0.5, 0.1 );
    float beta  = MAGMA_tally3_S_MAKE( 0.7, 0.2 );
    float dalpha = 0.6;
    float dbeta  = 0.8;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        ...
        err = magma_tally3_smalloc_pinned( &A,  size );  assert( err == 0 );
        err = magma_tally3_smalloc_pinned( &B,  size );  assert( err == 0 );
        err = magma_tally3_smalloc_pinned( &C,  size );  assert( err == 0 );
        err = magma_tally3_smalloc_pinned( &C2, size );  assert( err == 0 );
        err = magma_tally3_smalloc_pinned( &LU, size );  assert( err == 0 );
        err = magma_tally3_smalloc( &dA,  size );        assert( err == 0 );
        err = magma_tally3_smalloc( &dB,  size );        assert( err == 0 );
        err = magma_tally3_smalloc( &dC1, size );        assert( err == 0 );
        err = magma_tally3_smalloc( &dC2, size );        assert( err == 0 );
        ...
        if ( n >= 3 ) {
            magma_tally3_ssetmatrix( m, n, A, ld, dA, ld );
            magma_tally3_ssetmatrix( m, n, A, ld, dB, ld );
            magma_tally3_sswap( m, dA(0,1), 1, dA(0,2), 1 );
            magma_tally3_sswap( m, dB(0,1), 1, dB(0,2), 1 );
            
            // check results, storing diff between magma_tally3 and cuda calls in C2
            cublasSaxpy( opts.handle, ld*n, &c_neg_one, dA, 1, dB, 1 );
            magma_tally3_sgetmatrix( m, n, dB, ld, C2, ld );
        ...
        magma_tally3_ssetmatrix( m, k, A, ld, dA, ld );
        error = 0;
        for( int j = 0; j < k; ++j ) {
            magma_tally3_int_t i1 = magma_tally3_isamax( m, dA(0,j), 1 );
        ...
        for( int ia = 0; ia < 3; ++ia ) {
            magma_tally3_ssetmatrix( m, n, A,  ld, dA,  ld );
            magma_tally3_ssetvector( maxn, B, 1, dB,  1 );
            magma_tally3_ssetvector( maxn, C, 1, dC1, 1 );
            magma_tally3_ssetvector( maxn, C, 1, dC2, 1 );
            
            t1 = magma_tally3_sync_wtime( 0 );
            magma_tally3_sgemv( trans[ia], m, n, alpha, dA, ld, dB, 1, beta, dC1, 1 );
            t1 = magma_tally3_sync_wtime( 0 ) - t1;
            
            t2 = magma_tally3_sync_wtime( 0 );
            cublasSgemv( opts.handle, cublas_trans_const_tally3(trans[ia]),
            ...
            magma_tally3_sgetvector( size, dC2, 1, C2, 1 );
        ...
        for( int iu = 0; iu < 2; ++iu ) {
            magma_tally3_ssetmatrix( m, m, A, ld, dA, ld );
            magma_tally3_ssetvector( m, B, 1, dB,  1 );
            magma_tally3_ssetvector( m, C, 1, dC1, 1 );
            magma_tally3_ssetvector( m, C, 1, dC2, 1 );
            
            t1 = magma_tally3_sync_wtime( 0 );
            magma_tally3_ssymv( uplo[iu], m, alpha, dA, ld, dB, 1, beta, dC1, 1 );
            t1 = magma_tally3_sync_wtime( 0 ) - t1;
            
            t2 = magma_tally3_sync_wtime( 0 );
            cublasSsymv( opts.handle, cublas_uplo_const_tally3(uplo[iu]),
                         m, &alpha, dA, ld, dB, 1, &beta, dC2, 1 );
            t2 = magma_tally3_sync_wtime( 0 ) - t2;
            
            // check results, storing diff between magma_tally3 and cuda call in C2
            cublasSaxpy( opts.handle, m, &c_neg_one, dC1, 1, dC2, 1 );
            magma_tally3_sgetvector( m, dC2, 1, C2, 1 );
        ...
        for( int id = 0; id < 2; ++id ) {
            magma_tally3_ssetmatrix( m, m, LU, ld, dA, ld );
            magma_tally3_ssetvector( m, C, 1, dC1, 1 );
            magma_tally3_ssetvector( m, C, 1, dC2, 1 );
            
            t1 = magma_tally3_sync_wtime( 0 );
            magma_tally3_strsv( uplo[iu], trans[it], diag[id], m, dA, ld, dC1, 1 );
            t1 = magma_tally3_sync_wtime( 0 ) - t1;
            
            t2 = magma_tally3_sync_wtime( 0 );
            cublasStrsv( opts.handle, cublas_uplo_const_tally3(uplo[iu]), cublas_trans_const_tally3(trans[it]),
            ...
            error = lapackf77_slange( "F", &m, &ione, C2, &ld, work );
            total_error += error;
            gflops = FLOPS_STRSM( Magma_tally3Left, m, 1 ) / 1e9;
        ...
        for( int ib = 0; ib < 3; ++ib ) {
            ...
            magma_tally3_ssetmatrix( (nta ? m : k), (nta ? m : k), A, ld, dA,  ld );
            magma_tally3_ssetmatrix( (ntb ? k : n), (ntb ? n : k), B, ld, dB,  ld );
            magma_tally3_ssetmatrix( m, n, C, ld, dC1, ld );
            magma_tally3_ssetmatrix( m, n, C, ld, dC2, ld );
            
            t1 = magma_tally3_sync_wtime( 0 );
            magma_tally3_sgemm( trans[ia], trans[ib], m, n, k, alpha, dA, ld, dB, ld, beta, dC1, ld );
            t1 = magma_tally3_sync_wtime( 0 ) - t1;
            
            t2 = magma_tally3_sync_wtime( 0 );
            cublasSgemm( opts.handle, cublas_trans_const_tally3(trans[ia]), cublas_trans_const_tally3(trans[ib]),
                         m, n, k, &alpha, dA, ld, dB, ld, &beta, dC2, ld );
            t2 = magma_tally3_sync_wtime( 0 ) - t2;
            
            // check results, storing diff between magma_tally3 and cuda call in C2
            cublasSaxpy( opts.handle, ld*n, &c_neg_one, dC1, 1, dC2, 1 );
            magma_tally3_sgetmatrix( m, n, dC2, ld, C2, ld );
        ...
        for( int iu = 0; iu < 2; ++iu ) {
            magma_tally3_ssetmatrix( m, m, A, ld, dA,  ld );
            magma_tally3_ssetmatrix( m, n, B, ld, dB,  ld );
            magma_tally3_ssetmatrix( m, n, C, ld, dC1, ld );
            magma_tally3_ssetmatrix( m, n, C, ld, dC2, ld );
            
            t1 = magma_tally3_sync_wtime( 0 );
            magma_tally3_ssymm( side[is], uplo[iu], m, n, alpha, dA, ld, dB, ld, beta, dC1, ld );
            t1 = magma_tally3_sync_wtime( 0 ) - t1;
            
            t2 = magma_tally3_sync_wtime( 0 );
            cublasSsymm( opts.handle, cublas_side_const_tally3(side[is]), cublas_uplo_const_tally3(uplo[iu]),
        ...
        for( int iu = 0; iu < 2; ++iu ) {
        for( int it = 0; it < 3; ++it ) {
            magma_tally3_ssetmatrix( n, k, A, ld, dA,  ld );
            magma_tally3_ssetmatrix( n, n, C, ld, dC1, ld );
            magma_tally3_ssetmatrix( n, n, C, ld, dC2, ld );
            
            t1 = magma_tally3_sync_wtime( 0 );
            magma_tally3_ssyrk( uplo[iu], trans[it], n, k, dalpha, dA, ld, dbeta, dC1, ld );
            t1 = magma_tally3_sync_wtime( 0 ) - t1;
            
            t2 = magma_tally3_sync_wtime( 0 );
            cublasSsyrk( opts.handle, cublas_uplo_const_tally3(uplo[iu]), cublas_trans_const_tally3(trans[it]),
                         n, k, &dalpha, dA, ld, &dbeta, dC2, ld );
            t2 = magma_tally3_sync_wtime( 0 ) - t2;
            
            // check results, storing diff between magma_tally3 and cuda call in C2
            cublasSaxpy( opts.handle, ld*n, &c_neg_one, dC1, 1, dC2, 1 );
            magma_tally3_sgetmatrix( n, n, dC2, ld, C2, ld );
        ...
        for( int it = 0; it < 3; ++it ) {
            ...
            magma_tally3_ssetmatrix( (nt ? n : k), (nt ? n : k), A, ld, dA,  ld );
            magma_tally3_ssetmatrix( n, n, C, ld, dC1, ld );
            magma_tally3_ssetmatrix( n, n, C, ld, dC2, ld );
            
            t1 = magma_tally3_sync_wtime( 0 );
            magma_tally3_ssyr2k( uplo[iu], trans[it], n, k, alpha, dA, ld, dB, ld, dbeta, dC1, ld );
            t1 = magma_tally3_sync_wtime( 0 ) - t1;
            
            t2 = magma_tally3_sync_wtime( 0 );
            cublasSsyr2k( opts.handle, cublas_uplo_const_tally3(uplo[iu]), cublas_trans_const_tally3(trans[it]),
        ...
        for( int id = 0; id < 2; ++id ) {
            ...
            magma_tally3_ssetmatrix( (left ? m : n), (left ? m : n), A, ld, dA,  ld );
            magma_tally3_ssetmatrix( m, n, C, ld, dC1, ld );
            magma_tally3_ssetmatrix( m, n, C, ld, dC2, ld );
            
            t1 = magma_tally3_sync_wtime( 0 );
            magma_tally3_strmm( side[is], uplo[iu], trans[it], diag[id], m, n, alpha, dA, ld, dC1, ld );
            t1 = magma_tally3_sync_wtime( 0 ) - t1;
            
            // note cublas does trmm out-of-place (i.e., adds output matrix C),
            // but allows C=B to do in-place.
            t2 = magma_tally3_sync_wtime( 0 );
            cublasStrmm( opts.handle, cublas_side_const_tally3(side[is]), cublas_uplo_const_tally3(uplo[iu]),
        ...
        for( int id = 0; id < 2; ++id ) {
            bool left = (side[is] == Magma_tally3Left);
            magma_tally3_ssetmatrix( (left ? m : n), (left ? m : n), LU, ld, dA,  ld );
            magma_tally3_ssetmatrix( m, n, C, ld, dC1, ld );
            magma_tally3_ssetmatrix( m, n, C, ld, dC2, ld );
            
            t1 = magma_tally3_sync_wtime( 0 );
            magma_tally3_strsm( side[is], uplo[iu], trans[it], diag[id], m, n, alpha, dA, ld, dC1, ld );
            t1 = magma_tally3_sync_wtime( 0 ) - t1;
            
            t2 = magma_tally3_sync_wtime( 0 );
            cublasStrsm( opts.handle, cublas_side_const_tally3(side[is]), cublas_uplo_const_tally3(uplo[iu]),
                         cublas_trans_const_tally3(trans[it]), cublas_diag_const_tally3(diag[id]),
