    ...
    magma_tally4_int_t *info)
{
    ...
    *info = 0;
    ...
    #endif
    if (MAGMA_tally4_SUCCESS != magma_tally4_zmalloc( &dA, ldda*n + 2*lddw*nb + ldwork2 )) {
        *info = MAGMA_tally4_ERR_DEVICE_ALLOC;
        return *info;
    }

    magma_tally4DoubleComplex *dwork  = dA + ldda*n;
    #ifdef FAST_HEMV
    magma_tally4DoubleComplex *dwork2 = dwork + 2*lddw*nb;
    #endif
    ...
    if (upper) {
        ...
        for (i = n - nb; i >= kk; i -= nb) {
            ...
            magma_tally4_zlatrd2( uplo, i+nb, nb, A(0, 0), lda, e, tau,
                           ...
                           dwork2, ldwork2 );
            #else
            magma_tally4_zlatrd(  uplo, i+nb, nb, A(0, 0), lda, e, tau,
                           work, ldw, dA(0, 0), ldda, dwork, lddw );
            #endif
        ...
        for (i = 0; i < n-nx; i += nb) {
            ...
            magma_tally4_zlatrd2( uplo, n-i, nb, A(i, i), lda, &e[i], &tau[i],
                           ...
                           dwork2, ldwork2 );
            #else
            magma_tally4_zlatrd(  uplo, n-i, nb, A(i, i), lda, &e[i], &tau[i],
                           work, ldw, dA(i, i), ldda, dwork, lddw );
            #endif
