    ...
    magma_tally2_int_t *info)
{
    ...
    double *d_lAT[Magma_tally2MaxGPUs];
    double *d_panel[Magma_tally2MaxGPUs], *work;
    ...
    *info = 0;
    ...
    if (nb <= 1 || nb >= n) {
        ...
        magma_tally2_dgetmatrix( m, n, d_lA[0], ldda, work, m );
        lapackf77_dgetrf(&m, &n, work, &m, ipiv, info);
        magma_tally2_dsetmatrix( m, n, work, m, d_lA[0], ldda );
        ...
        for (i=0; i < ngpu; i++) {
            ...
            /* workspaces */
            if (MAGMA_tally2_SUCCESS != magma_tally2_dmalloc( &d_panel[i], (3+ngpu)*nb*maxm )) {
                for( j=0; j <= i; j++ ) {
            ...
            if (MAGMA_tally2_SUCCESS != magma_tally2_dmalloc( &d_lAT[i], lddat*maxm )) {
                for( j=0; j <= i; j++ ) {
                    magma_tally2_setdevice(j);
                    magma_tally2_free( d_panel[j] );
                }
            ...
            magma_tally2blas_dtranspose( m, n_local[i], d_lA[i], ldda, d_lAT[i], lddat );
        }
        for (i=0; i < ngpu; i++) {
            magma_tally2_setdevice(i);
        ...
        /* cpu workspace */
        lddwork = maxm;
        if (MAGMA_tally2_SUCCESS != magma_tally2_dmalloc_pinned( &work, lddwork*nb*ngpu )) {
            for (i=0; i < ngpu; i++ ) {
                magma_tally2_setdevice(i);
        ...
        magma_tally2_dgetrf2_mgpu(ngpu, m, n, nb, 0, d_lAT, lddat, ipiv, d_panel, work, maxm,
                           streaml, info);
        ...
        for( d=0; d < ngpu; d++ ) {
            ...
            /* save on output */
            magma_tally2blas_dtranspose( n_local[d], m, d_lAT[d], lddat, d_lA[d], ldda );
