    ...
    magma_minproduct_int_t *info )
{
    ...
    magma_minproductDouble_ptr dL = dwork;
    magma_minproduct_int_t lddl = n;
    magma_minproduct_int_t nb   = magma_minproduct_get_dgetri_nb(n);
    ...
    *info = 0;
    ...
    magma_minproduct_dtrtri_gpu( Magma_minproductUpper, Magma_minproductNonUnit, n, dA, ldda, info );
    if ( *info != 0 )
        return *info;
    ...
    for( j = jmax; j >= 0; j -= nb ) {
        ...
        magma_minproductblas_dlacpy( Magma_minproductFull, n-j, jb,
                          dA(j,j), ldda,
                          dL(j,0), lddl );
        magma_minproductblas_dlaset( Magma_minproductLower, n-j-1, jb, c_zero, c_zero, dA(j+1,j), ldda );
        ...
        if ( j+jb < n ) {
            magma_minproduct_dgemm( Magma_minproductNoTrans, Magma_minproductNoTrans, n, jb, n-j-jb,
                         c_neg_one, dA(0,j+jb), ldda,
                                    dL(j+jb,0), lddl,
                         c_one,     dA(0,j),    ldda );
        }
        // TODO use magma_minproductblas work interface
        magma_minproduct_dtrsm( Magma_minproductRight, Magma_minproductLower, Magma_minproductNoTrans, Magma_minproductUnit,
                     n, jb, c_one,
                     dL(j,0), lddl,
                     dA(0,j), ldda );
    }
    ...
    for( j = n-2; j >= 0; --j ) {
        jp = ipiv[j] - 1;
        if ( jp != j ) {
            magma_minproductblas_dswap( n, dA(0,j), 1, dA(0,jp), 1 );
        }
    }
