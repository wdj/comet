...
int main( int argc, char** argv)
{
    ...
    magma_minproductDoubleComplex *h_A, *h_R, *h_Q, *h_work, *tau, *twork;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_minproduct_zgehrd( N, ione, N, h_R, lda, tau, h_work, lwork, dT, &info);
            gpu_time = magma_minproduct_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_minproduct_zgehrd returned error %d: %s.\n",
