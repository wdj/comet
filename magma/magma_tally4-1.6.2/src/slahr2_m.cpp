    ...
    struct sgehrd_data *data )
{
    ...
    for( d = 0; d < ngpu; ++d ) {
        ...
        magma_tally4blas_slaset( Magma_tally4Full, nb, nb, c_zero, c_zero, dV(d,k,0), ldv );
    }
    ...
    for (i = 0; i < nb; ++i) {
        ...
        if (i > 0) {
            ...
            tmp = MAGMA_tally4_S_NEGATE( tau[i-1] );
        ...
        *A(k+i+1,i) = c_one;
        ...
        for( d = 0; d < ngpu; ++d ) {
            ...
            magma_tally4_ssetvector_async( n_k_i_1,
                                    A(k+i+1,i), 1,
            ...
            magma_tally4blas_slacpy( Magma_tally4Full, nb, nblocks-lblock,
            ...
            if ( dn-dki1 > 0 ) {
                magma_tally4_sgemv( Magma_tally4NoTrans, n-k, dn-dki1,
                ...
                magma_tally4_sgetvector_async( n-k,
                                        dY(d, k, i), 1,
        ...
        scale = MAGMA_tally4_S_NEGATE( tau[i] );
        blasf77_sgemv( "Conj", &n_k_i_1, &i,
                       &scale,  A(k+i+1,0), &lda,
                                A(k+i+1,i), &ione,
                       &c_zero, T(0,i), &ione );
    ...
    *A(k+nb,nb-1) = ei;
    ...
    for( d = 0; d < ngpu; ++d ) {
        ...
        // which are summed in separate loop below
        if ( dn-dki1 > 0 ) {
            magma_tally4_sgemm( Magma_tally4NoTrans, Magma_tally4NoTrans, k, nb, dn-dki1,
            ...
            magma_tally4_sgetmatrix_async( k, nb,
                                    dY(d, 0, 0),  ldda,
    ...
    for( d = 0; d < ngpu; ++d ) {
        magma_tally4_setdevice( d );
        magma_tally4_ssetmatrix_async( n, nb, Y, ldy, dY(d, 0, 0), ldda, data->streams[d] );
        magma_tally4_ssetmatrix_async( nb, nb, T, nb, dTi(d),      nb,   data->streams[d] );
    }
