        ...
        double* u, double* v)
{
    ...
    for (idx=0; idx<n; idx++){
        ...
        u[idx] = MAGMA_tally3_D_MAKE(u1,u1);

        v[idx] = MAGMA_tally3_D_MAKE(v1,v1);
    }
}
    ...
    magma_tally3_int_t *info, magma_tally3_int_t batchCount, magma_tally3_queue_t queue)
{
    ...
    *info = 0;
    ...
    if (MAGMA_tally3_SUCCESS != magma_tally3_dmalloc( &du, 2*n )) {
    ...
    magma_tally3_dsetvector( 2*n, U, 1, du, 1);
    magma_tally3_dsetvector( 2*n, V, 1, dv, 1);

    /* Perform Partial Random Butterfly Transformation on the GPU*/
    magma_tally3blas_dprbt_batched(n, dA_array, ldda, du, dv, batchCount, queue);

    /* Compute U^T.b on the GPU*/

    // TODO fix for multiple RHS
    for(int i= 0; i < nrhs; i++)
        magma_tally3blas_dprbt_mtv_batched(n, du, dB_array, batchCount, queue);
