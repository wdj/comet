...
typedef struct {
  ...
  /* Flags to wake up MAGMA_tally3 threads */
  volatile cuDoubleComplex **p;

  /* Synchronization flag */
  volatile magma_tally3_int_t sync0;

  /* One synchronization flag for each MAGMA_tally3 thread */
  volatile magma_tally3_int_t *sync1;
  
  /* Synchronization flag */
  volatile magma_tally3_int_t sync2;

  /* Work space */
  cuDoubleComplex *w;

} magma_tally3_qr_params;

//magma_tally3_qr_params MG;

typedef struct {
  magma_tally3_int_t tid;
  void *params;
} t_params;
...
extern "C" void *cpu_thread(void *a)
{
  ...
  magma_tally3_qr_params *mp = (magma_tally3_qr_params*)tp->params;

  //long magma_tally3_int_t t = (long int) a;
...
loop:
  ...
  for (i = 0; i < mp->np_gpu; i++) 
  {
    ...
    lapackf77_zlarfb(Magma_tally3LeftStr, Magma_tally3TransStr, Magma_tally3ForwardStr, Magma_tally3ColumnwiseStr,
...
void magma_tally3_qr_init(magma_tally3_qr_params *qr_params,
                   magma_tally3_int_t m, magma_tally3_int_t n, cuDoubleComplex *a, magma_tally3_int_t nthreads)
{
  magma_tally3_int_t i;
...
/* ////////////////////////////////////////////////////////////////////////////
   -- Testing zgeqrf
*/
int main( magma_tally3_int_t argc, char** argv) 
{
    magma_tally3_int_t nquarkthreads=2;
    magma_tally3_int_t nthreads=2;
    magma_tally3_int_t num_gpus  = 1;
    TRACE = 0;

    //magma_tally3_qr_params mp;

    cuDoubleComplex *h_A, *h_R, *h_work, *tau;
    double gpu_perf, cpu_perf, flops;

    magma_tally3_timestr_t start, end;

    magma_tally3_qr_params *mp = (magma_tally3_qr_params*)malloc(sizeof(magma_tally3_qr_params));

    /* Matrix size */
    magma_tally3_int_t M=0, N=0, n2;
    magma_tally3_int_t size[10] = {1024,2048,3072,4032,5184,6016,7040,8064,9088,10112};

    cublasStatus status;
    magma_tally3_int_t i, j, info;
    ...
    magma_tally3_int_t loop = argc;
    magma_tally3_int_t accuracyflag = 1;

    char precision;

    magma_tally3_int_t nc = -1;
    magma_tally3_int_t ncps = -1;
...
fprintf(stderr,"%d %d %d %d %d\n",mp->nb,mp->ob,mp->ib,nquarkthreads,nthreads);
    ...
    /* Initialize MAGMA_tally3 hardware context, seeting how many CPU cores
       and how many GPUs to be used in the consequent computations  */
    mp->sync0 = 0;
    magma_tally3_context *context;
    context = magma_tally3_init((void*)(mp),cpu_thread, nthreads, nquarkthreads, num_gpus, argc, argv);
    context->params = (void *)(mp);

    mp->sync1 = (volatile magma_tally3_int_t *) malloc (sizeof(int)*nthreads);
    ...
    magma_tally3_int_t nb = magma_tally3_get_zgeqrf_nb(min_mn);
    magma_tally3_int_t lwork = N*nb;
    ...
    for(i=0; i<10; i++){
        ...
        lapackf77_zlacpy( Magma_tally3UpperLowerStr, &M, &N, h_A, &M, h_R, &M );

        //magma_tally3_zgeqrf(M, N, h_R, M, tau, h_work, lwork, &info);
        ...
        /* ====================================================================
           ...
           =================================================================== */
        magma_tally3_qr_init(mp, M, N, h_R, nthreads);

        start = get_current_time();
        magma_tally3_zgeqrf3(context, M, N, h_R, M, tau, h_work, lwork, &info);
    ...
    /* Shut down the MAGMA_tally3 context */
    magma_tally3_finalize(context);
}
