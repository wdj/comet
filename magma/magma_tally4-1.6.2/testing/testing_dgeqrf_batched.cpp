...
void get_QR_error(magma_tally4_int_t M, magma_tally4_int_t N, magma_tally4_int_t min_mn,
                    double *h_R,  double *h_A, magma_tally4_int_t lda,
                    double *tau,
                    double *Q,  magma_tally4_int_t ldq,
                    double *R,  magma_tally4_int_t ldr,
                    double *h_work,  magma_tally4_int_t lwork,
...
int main( int argc, char** argv)
{
    ...
    double *h_A, *h_R, *h_Amagma_tally4, *tau, *h_work, tmp[1];
    double *d_A, *dtau_magma_tally4, *dtau_cublas;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_DEV( dtau_magma_tally4,  double, min_mn * batchCount);
			...
			{
				magma_tally4_dmake_hpd( N, h_A + i * lda * N, lda );// need modification
			}
			
			lapackf77_dlacpy( Magma_tally4UpperLowerStr, &M, &column, h_A, &lda, h_R, &lda );
       
            /* ====================================================================
               Performs operation using MAGMA_tally4
               =================================================================== */
            magma_tally4_dsetmatrix( M, column, h_R, lda,  d_A, ldda );
            dset_pointer(dA_array, d_A, 1, 0, 0, ldda*N, batchCount, opts.queue);
            dset_pointer(dtau_array, dtau_magma_tally4, 1, 0, 0, min_mn, batchCount, opts.queue);
    
            magma_tally4_time = magma_tally4_sync_wtime(0);
    
            info = magma_tally4_dgeqrf_batched(M, N, dA_array, ldda, dtau_array, dinfo_magma_tally4, batchCount, opts.queue);

            magma_tally4_time = magma_tally4_sync_wtime(0) - magma_tally4_time;
            magma_tally4_perf = gflops / magma_tally4_time;

            magma_tally4_dgetmatrix( M, column, d_A, ldda, h_Amagma_tally4, lda);

            if (info != 0)
                printf("magma_tally4_dgeqrf_batched returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                magma_tally4_dgetvector(min_mn*batchCount, dtau_magma_tally4, 1, tau, 1);
                for (int i=0; i < batchCount; i++)
                {
                ...
                #if CUDA_VERSION >= 6050
                magma_tally4_dgetvector(min_mn*batchCount, dtau_magma_tally4, 1, tau, 1);
                magma_tally4_dgetmatrix( M, column, d_A, ldda, h_A, lda);
                for (int i=0; i < batchCount; i++)
                {
                    get_QR_error(M, N, min_mn,
