...
__global__ void
magma_tally3_dswap_gemv_kernel(int m, int rk, int n, const double * __restrict__ V, int ldv,
                     const double * __restrict__ x, int ldx, double *c, double *b)
{
    ...
    lsum = MAGMA_tally3_D_ZERO;
    if (j < m){
       tmp  = b[j];
       b[j] = c[j];
       if (j>=rk) 
          for(int k=0; k<n; k++)
              lsum += MAGMA_tally3_D_MUL( V[k*ldv], MAGMA_tally3_D_CNJG(x[k*ldx]));
...
__global__ void
magma_tally3_dgemv_kernel(int m, int n, const double * __restrict__ V, int ldv,
                     const double * __restrict__ x, double *b, double *c)
{
    ...
    if (j < m){
        for(int k=0; k<n; k++)
            lsum += MAGMA_tally3_D_MUL( V[k*ldv], x[k]);
...
__global__
void magma_tally3_dscale_kernel(int n, double* dx0,
                         double *dtau, double *dxnorm, double* dAkk)
{
   ...
   for( int k = i; k < n; k += BLOCK_SIZE ) {
             ...
             lsum += re*re;
        #else
             re = MAGMA_tally3_D_REAL( dx[k] );
             double im = MAGMA_tally3_D_IMAG( dx[k] );
             lsum += re*re + im*im;
        #endif
   }
   sum[i] = lsum;
            ...
            if ( beta == 0 ) {
              *dtau = MAGMA_tally3_D_ZERO;
               ...
               *dAkk    = beta;
               ...
               scale = 1. / (tmp - beta);
#else
               double alphar =  MAGMA_tally3_D_REAL(tmp), alphai = MAGMA_tally3_D_IMAG(tmp);
               beta  = -copysign( beta, alphar );

               // todo: deal with badly scaled vectors (see lapack's larfg)
               *dtau = MAGMA_tally3_D_MAKE((beta - alphar)/beta, -alphai/beta);
               *dAkk = MAGMA_tally3_D_MAKE(beta, 0.);

               tmp = MAGMA_tally3_D_MAKE( alphar - beta, alphai);
               scale = MAGMA_tally3_D_DIV( MAGMA_tally3_D_ONE, tmp);
#endif
            }
   }
...
__global__ void
magma_tally3_dgemv_kernel_adjust(int n, int k, double * A, int lda, 
                          ...
                          double *xnorm, double *xnorm2, double *Akk, int *lsticc, int *lsticcs)
{
    ...
    if (j<n) {
      B += j;
      sum = MAGMA_tally3_D_CNJG( B[(k-1)*ldb] );
      // sum = MAGMA_tally3_D_ZERO;
      for(int m=0; m<k-1; m++) {
         sum += MAGMA_tally3_D_MUL( MAGMA_tally3_D_CNJG( B[m*ldb] ), A[m*lda] );
      }
      C[j*lda] -= sum;

      oldnorm = xnorm[j];
      temp = MAGMA_tally3_D_ABS( C[j*lda] ) / oldnorm;
...
__global__ void
magma_tally3blas_dnrm2_check_kernel(int m, double *da, int ldda, 
                              ...
                              int *dlsticc, int *dlsticcs)
{
    ...
    for( int j = i; j < m; j += BLOCK_SIZE ) {
        ...
        lsum += re*re;
#else
        re = MAGMA_tally3_D_REAL( dx[j] );
        double im = MAGMA_tally3_D_IMAG( dx[j] );
        lsum += re*re + im*im;
#endif
...
/**
    ...
    @ingroup magma_tally3_dgeqp3_aux
    ********************************************************************/
...
#define dF(i_, j_) (dF + (i_) + (j_)*(lddf))
    ...
    double c_zero    = MAGMA_tally3_D_MAKE( 0.,0.);
    double c_one     = MAGMA_tally3_D_MAKE( 1.,0.);
    double c_neg_one = MAGMA_tally3_D_MAKE(-1.,0.);
    ...
    magma_tally3Double_ptr dAkk = dauxv;
    dauxv += 1;
    ...
    while( k < nb && lsticc == 0 ) {
        ...
        if (pvt != k) {
            magma_tally3blas_dswap( k, dF(pvt,0), lddf, dF(k,0), lddf);
            itemp     = jpvt[pvt];
            jpvt[pvt] = jpvt[k];
            jpvt[k]   = itemp;
        ...
        magma_tally3_dswap_gemv_kernel<<< (m + BLOCK_SIZE1-1) / BLOCK_SIZE1, BLOCK_SIZE1, 0, magma_tally3_stream >>> 
                              ( m, rk, k, dA(0, 0), ldda, dF(k,  0), lddf, dA(0, k), dA(0,pvt));
                                 
        /*  Generate elementary reflector H(k). */
        magma_tally3_dscale_kernel<<< 1, BLOCK_SIZE, 0, magma_tally3_stream >>>
        ...
        if (k < n-1) {
            magma_tally3_dgetvector( 1, &dtau[k], 1, &tauk, 1 );
            magma_tally3_dgemv( Magma_tally3ConjTrans, m-rk, n,
                         tauk,   dA( rk,  0 ), ldda,
                                 dA( rk,  k   ), 1,
                         c_zero, dauxv, 1 );
            if (k==0) 
               magma_tally3blas_dlacpy(Magma_tally3UpperLower, n-k-1, 1, dauxv+k+1, n-k-1, dF( k+1, k   ), n-k-1);
        }
        ...
        if (k > 0) {
            ...
            magma_tally3_dgemv_kernel<<< (n-k-1 + BLOCK_SIZE1 -1)/BLOCK_SIZE1, BLOCK_SIZE1, 0, magma_tally3_stream >>>
        ...
        if (k < n-1) {
            ...
            magma_tally3_dgemv_kernel_adjust<<<(n-k-1 + BLOCK_SIZE2-1)/BLOCK_SIZE2, BLOCK_SIZE2, 0, magma_tally3_stream>>>
            ...
            // can be done in the kernel directly (magma_tally3blas_dnrm2_check_kernel)
            // if (k==16) lsticc = 1;
        }
...
/*
        if (rk < min(m, n+offset)-1){
           magma_tally3blas_dnrm2_row_check_adjust(n-k-1, tol3z, &dvn1[k+1], 
...
*/
        ...
        if (k>=n-1)
           magma_tally3blas_dlacpy(Magma_tally3UpperLower, 1, 1, dAkk, 1, dA(rk, k), 1);
    ...
    *kb = k + 1;
    ...
    if (*kb < min(n, m - offset)-1) {
        ...
        magma_tally3_dgemm( Magma_tally3NoTrans, Magma_tally3ConjTrans, i__1, i__2, *kb,
    ...
    if( lsticc > 0 ) {
        ...
        //magma_tally3blas_dnrm2_check(m-rk-1, n-*kb, A(rk+1,rk+1), lda,
        //                       &dvn1[rk+1], &dvn2[rk+1], dlsticcs);
       
        // There is a bug when we get to recompute  
        magma_tally3blas_dnrm2_check_kernel<<< n-*kb, BLOCK_SIZE >>>
