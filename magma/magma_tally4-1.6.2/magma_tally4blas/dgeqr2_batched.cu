...
void dlarfx_device( int m, int n,  double *v, double *tau,
                         double *dc, magma_tally4_int_t ldc, double* sum)
{


    if(n <=0) return ;
    if (MAGMA_tally4_D_EQUAL(*tau, MAGMA_tally4_D_ZERO) )  return; // check singularity
    ...
    for(int k=0;k<n;k++)
    {
        ...
        if(tx<BLOCK_SIZE)
        {
            ...
            for( int j = tx+1; j < m; j += BLOCK_SIZE ){
                lsum += MAGMA_tally4_D_MUL( MAGMA_tally4_D_CNJG( v[j] ), dc[j+ldc*k] );
            }
...
__global__
void dgeqr2_sm_kernel_batched( int m, int n, double** dA_array, magma_tally4_int_t lda,
                               double **dtau_array)
{
...
static __device__
void dgeqr2_device( magma_tally4_int_t m, magma_tally4_int_t n, double* dA, magma_tally4_int_t lda,
                               double *dtau, 
                               double *sdata,
                               double *sum,
                               double *swork,
...
__global__
void dgeqr2_kernel_batched( int m, int n, double** dA_array, magma_tally4_int_t lda,
                               double **dtau_array)
{
...
magma_tally4_dgeqr2_batched(magma_tally4_int_t m, magma_tally4_int_t n, double **dA_array,
                  magma_tally4_int_t lda, double **dtau_array,
