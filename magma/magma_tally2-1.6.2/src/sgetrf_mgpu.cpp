    ...
    magma_tally2_int_t *info)
{
    ...
    float *d_lAT[Magma_tally2MaxGPUs];
    float *d_panel[Magma_tally2MaxGPUs], *work;
    ...
    *info = 0;
    ...
    if (nb <= 1 || nb >= n) {
        ...
        magma_tally2_sgetmatrix( m, n, d_lA[0], ldda, work, m );
        lapackf77_sgetrf(&m, &n, work, &m, ipiv, info);
        magma_tally2_ssetmatrix( m, n, work, m, d_lA[0], ldda );
        ...
        for (i=0; i < ngpu; i++) {
            ...
            /* workspaces */
            if (MAGMA_tally2_SUCCESS != magma_tally2_smalloc( &d_panel[i], (3+ngpu)*nb*maxm )) {
                for( j=0; j <= i; j++ ) {
            ...
            if (MAGMA_tally2_SUCCESS != magma_tally2_smalloc( &d_lAT[i], lddat*maxm )) {
                for( j=0; j <= i; j++ ) {
                    magma_tally2_setdevice(j);
                    magma_tally2_free( d_panel[j] );
                }
            ...
            magma_tally2blas_stranspose( m, n_local[i], d_lA[i], ldda, d_lAT[i], lddat );
        }
        for (i=0; i < ngpu; i++) {
            magma_tally2_setdevice(i);
        ...
        /* cpu workspace */
        lddwork = maxm;
        if (MAGMA_tally2_SUCCESS != magma_tally2_smalloc_pinned( &work, lddwork*nb*ngpu )) {
            for (i=0; i < ngpu; i++ ) {
                magma_tally2_setdevice(i);
        ...
        magma_tally2_sgetrf2_mgpu(ngpu, m, n, nb, 0, d_lAT, lddat, ipiv, d_panel, work, maxm,
                           streaml, info);
        ...
        for( d=0; d < ngpu; d++ ) {
            ...
            /* save on output */
            magma_tally2blas_stranspose( n_local[d], m, d_lAT[d], lddat, d_lA[d], ldda );
