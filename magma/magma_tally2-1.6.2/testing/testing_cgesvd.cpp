/*
    -- MAGMA_tally2 (version 1.6.1) --
       ...
       @generated from testing_zgesvd.cpp normal z -> c, Fri Jan 30 19:00:26 2015
       @author Mark Gates
...
int main( int argc, char** argv)
{
    ...
    magma_tally2FloatComplex *h_A, *h_R, *U, *VT, *h_work;
    magma_tally2FloatComplex dummy[1];
    float *S1, *S2;
    #ifdef COMPLEX
    float *rwork;
    #endif
    magma_tally2_int_t M, N, N_U, M_VT, lda, ldu, ldv, n2, min_mn, info, nb, lwork;
    magma_tally2_int_t ione     = 1;
    magma_tally2_int_t ISEED[4] = {0,0,0,1};
    magma_tally2_vec_t jobu, jobvt;
    ...
    magma_tally2_vec_t jobs[] = { Magma_tally2NoVec, Magma_tally2SomeVec, Magma_tally2OverwriteVec, Magma_tally2AllVec };
    
    if ( opts.check && ! opts.all && (jobu == Magma_tally2NoVec || jobvt == Magma_tally2NoVec)) {
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( jobu == Magma_tally2OverwriteVec && jobvt == Magma_tally2OverwriteVec ) {
            ...
            N = opts.nsize[itest];
            min_mn = min(M, N);
            N_U  = (jobu  == Magma_tally2AllVec ? M : min_mn);
            M_VT = (jobvt == Magma_tally2AllVec ? N : min_mn);
            lda = M;
            ldu = M;
            ldv = M_VT;
            n2 = lda*N;
            ...
            switch( opts.svd_work ) {
                case 0: {  // query for workspace size
                    lwork = -1;
                    magma_tally2_cgesvd( jobu, jobvt, M, N,
                                  h_R, lda, S1, U, ldu, VT, ldv, dummy, lwork,
                                  #ifdef COMPLEX
                                  rwork,
                                  #endif
                                  &info );
                    lwork = (int) MAGMA_tally2_C_REAL( dummy[0] );
                    break;
                }
            ...
            TESTING_MALLOC_CPU( VT,    magma_tally2FloatComplex, ldv*N );   // N x N (jobvt=A) or min(M,N) x N
            TESTING_MALLOC_CPU( U,     magma_tally2FloatComplex, ldu*N_U ); // M x M (jobu=A)  or M x min(M,N)
            ...
            if (info != 0)
                printf("magma_tally2_cgesvd returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                magma_tally2FloatComplex *U2  = NULL;
                magma_tally2FloatComplex *VT2 = NULL;
                if ( jobu == Magma_tally2SomeVec || jobu == Magma_tally2AllVec ) {
                    U2 = U;
                }
                else if ( jobu == Magma_tally2OverwriteVec ) {
                    U2 = h_R;
                    ldu = lda;
                }
                if ( jobvt == Magma_tally2SomeVec || jobvt == Magma_tally2AllVec ) {
                    VT2 = VT;
                }
                else if ( jobvt == Magma_tally2OverwriteVec ) {
                    VT2 = h_R;
                    ldv = lda;
                }
                ...
                magma_tally2_int_t lwork_err = M+N;
                if ( U2 != NULL ) {
                ...
                magma_tally2FloatComplex *h_work_err;
                TESTING_MALLOC_CPU( h_work_err, magma_tally2FloatComplex, lwork_err );
            ...
            if ( opts.lapack ) {
                ...
                lapackf77_cgesvd( lapack_vec_const_tally2(jobu), lapack_vec_const_tally2(jobvt), &M, &N,
                ...
                printf("   %c    %c %5d %5d  %7.2f         %7.2f         %8.2e",
                       lapack_vec_const_tally2(jobu)[0], lapack_vec_const_tally2(jobvt)[0],
