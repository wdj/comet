    ...
    magma_minproduct_int_t batchCount, cublasHandle_t myhandle, magma_minproduct_queue_t queue )
{
    ...
    //===============================================
    if(m < nb){
        printf("magma_minproduct_dgetrf_panel_nopiv_batched_q m < nb %d < %d \n",(int) m, (int) nb);
        return -101;
    }

#if 0
    arginfo = magma_minproduct_dgetf2_nopiv_batched(
                       m, nb,
                       dA_array, ldda,
    ...
    if((m-nb) > 0){
        ...
        magma_minproductblas_dtrsm_work_batched(Magma_minproductRight, Magma_minproductUpper, Magma_minproductNoTrans, Magma_minproductNonUnit,
                              1, m-nb, nb, 
    ...
    magma_minproduct_int_t batchCount, cublasHandle_t myhandle, magma_minproduct_queue_t queue )
{
    ...
    if(panel_nb <= min_recpnb){
        ...
        //  panel factorization
        //magma_minproduct_ddisplace_pointers(dA_displ, dA_array, ldda, 0, 0, batchCount);
        ...
        magma_minproduct_ddisplace_pointers(dW5_displ, dA_array, ldda, p1, p2, batchCount, queue); 
        ...
        magma_minproduct_ddisplace_pointers(dW1_displ, dA_array, ldda, p2, 0, batchCount, queue); 
        magma_minproduct_ddisplace_pointers(dA_displ, dA_array, ldda, p2, p2, batchCount, queue); 
        magma_minproductblas_dgemm_batched( Magma_minproductNoTrans, Magma_minproductNoTrans, m2, n2, n1, 
                              MAGMA_minproduct_D_NEG_ONE, dW1_displ, ldda, 
                              dW5_displ, ldda, 
                              MAGMA_minproduct_D_ONE,  dA_displ, ldda, 
                              batchCount, queue);
        // panel on A2
