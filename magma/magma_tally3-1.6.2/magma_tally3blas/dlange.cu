...
max_nan_kernel( int n, T* x )
{
    ...
    magma_tally3_max_nan_reduce< 512 >( tx, smax );
    if ( tx == 0 ) {
        x[0] = smax[0];
    }
}
    ...
    int m, int n, const double *A, int lda, double *dwork )
{
    ...
    magma_tally3_sum_reduce< 64 >( tx, ssum );
    if ( tx == 0 ) {
...
magma_tally3blas_dlange(
    magma_tally3_norm_t norm, magma_tally3_int_t m, magma_tally3_int_t n,
    magma_tally3Double_const_ptr dA, magma_tally3_int_t ldda,
    magma_tally3Double_ptr dwork )  //, magma_tally3_int_t lwork )
{
    magma_tally3_int_t info = 0;
    if ( ! (norm == Magma_tally3InfNorm || norm == Magma_tally3MaxNorm || norm == Magma_tally3OneNorm) )
        info = -1;
    else if ( m < 0 )
        info = -2;
    else if ( n < 0 )
        ...
        info = -5;
    //else if ( ((norm == Magma_tally3InfNorm || norm == Magma_tally3MaxNorm) && (lwork < m)) ||
    //          ((norm == Magma_tally3OneNorm) && (lwork < n)) )
    //    info = -7;
    ...
    dim3 threads( 64 );
    double result = -1;
    if ( norm == Magma_tally3InfNorm ) {
        dim3 grid( (m-1)/64 + 1 );
        dlange_inf_kernel<<< grid, threads, 0, magma_tally3_stream >>>( m, n, dA, ldda, dwork );
        max_nan_kernel<<< 1, 512, 0, magma_tally3_stream >>>( m, dwork );
        cudaMemcpy( &result, &dwork[0], sizeof(double), cudaMemcpyDeviceToHost );
    }
    else if ( norm == Magma_tally3MaxNorm ) {
        dim3 grid( (m-1)/64 + 1 );
        dlange_max_kernel<<< grid, threads, 0, magma_tally3_stream >>>( m, n, dA, ldda, dwork );
        max_nan_kernel<<< 1, 512, 0, magma_tally3_stream >>>( m, dwork );
        cudaMemcpy( &result, &dwork[0], sizeof(double), cudaMemcpyDeviceToHost );
    }
    else if ( norm == Magma_tally3OneNorm ) {
        dim3 grid( n );
        dlange_one_kernel<<< grid, threads, 0, magma_tally3_stream >>>( m, n, dA, ldda, dwork );
        max_nan_kernel<<< 1, 512, 0, magma_tally3_stream >>>( n, dwork );
        cudaMemcpy( &result, &dwork[0], sizeof(double), cudaMemcpyDeviceToHost );
    }
