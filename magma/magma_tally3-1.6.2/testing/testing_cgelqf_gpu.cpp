...
int main( int argc, char** argv)
{
    ...
    magma_tally3_int_t M, N, n2, lda, ldda, lwork, info, min_mn, nb;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lapackf77_clacpy( Magma_tally3FullStr, &M, &N, h_A, &lda, h_R, &lda );
            ...
            magma_tally3_cgelqf_gpu( M, N, d_A, ldda, tau, h_work, lwork, &info);
            gpu_time = magma_tally3_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally3_cgelqf_gpu returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                magma_tally3_int_t ldq = min_mn;
                magma_tally3_int_t ldl = M;
                magma_tally3FloatComplex *Q, *L;
                float *work;
                TESTING_MALLOC_CPU( Q,    magma_tally3FloatComplex, ldq*N );       // K by N
                TESTING_MALLOC_CPU( L,    magma_tally3FloatComplex, ldl*min_mn );  // M by K
