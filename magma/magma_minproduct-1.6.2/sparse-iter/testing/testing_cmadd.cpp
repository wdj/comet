...
int main(  int argc, char** argv )
{
    ...
    real_Double_t res;
    magma_minproduct_c_matrix A={Magma_minproduct_CSR}, B={Magma_minproduct_CSR}, B2={Magma_minproduct_CSR}, 
    ...
    magma_minproductFloatComplex mone = MAGMA_minproduct_C_MAKE(-1.0, 0.0);
    ...
    if ( strcmp("LAPLACE2D", argv[i]) == 0 && i+1 < argc ) {   // Laplace test
        i++;
        magma_minproduct_int_t laplace_size = atoi( argv[i] );
        CHECK( magma_minproduct_cm_5stencil(  laplace_size, &B, queue ));
    } else {                        // file-matrix test
        CHECK( magma_minproduct_c_csr_mtx( &B,  argv[i], queue ));
    }
    printf( "# matrix info: %d-by-%d with %d nonzeros\n",
    ...
    CHECK( magma_minproduct_ccuspaxpy( &one, A_d, &one, B_d, &C_d, queue ));

    magma_minproduct_cmfree(&B_d, queue );

    CHECK( magma_minproduct_ccuspaxpy( &mone, A_d, &one, C_d, &B_d, queue ));
    
    CHECK( magma_minproduct_cmtransfer( B_d, &B2, Magma_minproduct_DEV, Magma_minproduct_CPU, queue ));

    magma_minproduct_cmfree(&A_d, queue );
    magma_minproduct_cmfree(&B_d, queue );
    magma_minproduct_cmfree(&C_d, queue );

    // check difference
    CHECK( magma_minproduct_cmdiff( B, B2, &res, queue ));
    printf("# ||A-B||_F = %8.2e\n", res);
    if ( res < .000001 )
    ...
    magma_minproduct_cmfree(&B2, queue );
