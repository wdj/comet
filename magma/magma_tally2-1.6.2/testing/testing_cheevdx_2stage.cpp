...
static magma_tally2_int_t check_orthogonality(magma_tally2_int_t M, magma_tally2_int_t N, magma_tally2FloatComplex *Q, magma_tally2_int_t LDQ, float eps);
static magma_tally2_int_t check_reduction(magma_tally2_uplo_t uplo, magma_tally2_int_t N, magma_tally2_int_t bw, magma_tally2FloatComplex *A, float *D, magma_tally2_int_t LDA, magma_tally2FloatComplex *Q, float eps );
static magma_tally2_int_t check_solution(magma_tally2_int_t N, float *E1, float *E2, float eps);
...
int main( int argc, char** argv)
{
    ...
    magma_tally2FloatComplex *h_A, *h_R, *h_work;
    ...
    magma_tally2_int_t N, n2, info, lwork, liwork;
    magma_tally2_int_t ione     = 1;
    magma_tally2_int_t ISEED[4] = {0,0,0,1};;
    magma_tally2_int_t info_ortho     = 0;
    magma_tally2_int_t info_solution  = 0;
    magma_tally2_int_t info_reduction = 0;
    ...
    if (opts.fraction != 1)
        range = Magma_tally2RangeI;

    if ( opts.check && opts.jobz == Magma_tally2NoVec ) {
        fprintf( stderr, "checking results requires vectors; setting jobz=V (option -JV)\n" );
        opts.jobz = Magma_tally2Vec;
    }

    printf("using: itype = %d, jobz = %s, range = %s, uplo = %s, check = %d, fraction = %6.4f\n",
           (int) opts.itype, lapack_vec_const_tally2(opts.jobz), lapack_range_const_tally2(range), lapack_uplo_const_tally2(opts.uplo),
           (int) opts.check, opts.fraction);

    printf("    N     M  GPU Time (sec)  ||I-Q'Q||/.  ||A-QDQ'||/.  ||D-D_magma_tally2||/.\n");
    printf("=======================================================================\n");
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            n2     = N*N;
            #if defined(PRECISION_z) || defined(PRECISION_c)
            lwork  = magma_tally2_cbulge_get_lq2(N, threads) + 2*N + N*N;
            lrwork = 1 + 5*N +2*N*N;
            #else
            lwork  = magma_tally2_cbulge_get_lq2(N, threads) + 1 + 6*N + 2*N*N;
            #endif
            liwork = 3 + 5*N;
            ...
            magma_tally2_cmake_hermitian( N, h_A, N );

            magma_tally2_int_t m1 = 0;
            float vl = 0;
            float vu = 0;
            magma_tally2_int_t il = 0;
            magma_tally2_int_t iu = 0;
            if (range == Magma_tally2RangeI) {
                il = 1;
            ...
            if (opts.warmup) {
                // ==================================================================
                // Warmup using MAGMA_tally2
                // ==================================================================
                lapackf77_clacpy( Magma_tally2UpperLowerStr, &N, &N, h_A, &N, h_R, &N );
                if (opts.ngpu == 1) {
                    //printf("calling cheevdx_2stage 1 GPU\n");
                    magma_tally2_cheevdx_2stage(opts.jobz, range, opts.uplo, N, 
                                    h_R, N, 
                                    vl, vu, il, iu, 
                                    &m1, w1, 
                                    h_work, lwork, 
                    ...
                    magma_tally2_cheevdx_2stage_m(opts.ngpu, opts.jobz, range, opts.uplo, N, 
                                    h_R, N, 
                                    vl, vu, il, iu, 
                                    &m1, w1, 
                                    h_work, lwork, 
            ...
            if ( opts.check && opts.jobz != Magma_tally2NoVec ) {
                ...
                //printf("\n");
                //printf("------ TESTS FOR MAGMA_tally2 CHEEVD ROUTINE -------  \n");
...
static magma_tally2_int_t check_orthogonality(magma_tally2_int_t M, magma_tally2_int_t N, magma_tally2FloatComplex *Q, magma_tally2_int_t LDQ, float eps)
{
    ...
    float  normQ, result;
    magma_tally2_int_t     info_ortho;
    magma_tally2_int_t     minMN = min(M, N);
    float *work;
    magma_tally2_smalloc_cpu( &work, minMN );

    /* Build the idendity matrix */
    magma_tally2FloatComplex *Id;
    magma_tally2_cmalloc_cpu( &Id, minMN*minMN );
...
static magma_tally2_int_t check_reduction(magma_tally2_uplo_t uplo, magma_tally2_int_t N, magma_tally2_int_t bw, magma_tally2FloatComplex *A, float *D, magma_tally2_int_t LDA, magma_tally2FloatComplex *Q, float eps )
{
    ...
    magma_tally2FloatComplex *TEMP, *Residual;
    float *work;
    float Anorm, Rnorm, result;
    magma_tally2_int_t info_reduction;
    magma_tally2_int_t i;
    magma_tally2_int_t ione=1;

    magma_tally2_cmalloc_cpu( &TEMP, N*N );
    magma_tally2_cmalloc_cpu( &Residual, N*N );
    ...
    Rnorm = lapackf77_clanhe("1", lapack_uplo_const_tally2(uplo), &N, Residual, &N, work);
    Anorm = lapackf77_clanhe("1", lapack_uplo_const_tally2(uplo), &N, A,        &LDA, work);
    ...
    magma_tally2_free_cpu(TEMP);
    magma_tally2_free_cpu(Residual);
    magma_tally2_free_cpu(work);
...
static magma_tally2_int_t check_solution(magma_tally2_int_t N, float *E1, float *E2, float eps)
{
    magma_tally2_int_t info_solution, i;
    float resid;
    float maxtmp;
