...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            // ==================================================================
            if ( opts.warmup ) {
                lapackf77_clacpy( Magma_tally3FullStr, &N, &N, h_A, &N, h_R, &N );
                lapackf77_clacpy( Magma_tally3FullStr, &N, &N, h_B, &N, h_S, &N );
                
                magma_tally3_chegvdx( opts.itype, opts.jobz, Magma_tally3RangeI, opts.uplo,
                               ...
                               &info );
                if (info != 0)
                    printf("magma_tally3_chegvdx returned error %d: %s.\n",
            ...
            if ( opts.check && opts.jobz != Magma_tally3NoVec ) {
                ...
                result[0] = 1.;
                result[0] /= lapackf77_clanhe("1", lapack_uplo_const_tally3(opts.uplo), &N, h_A, &N, rwork);
                result[0] /= lapackf77_clange("1", &N, &m1, h_R, &N, rwork);
                
                if (opts.itype == 1) {
                    blasf77_chemm("L", lapack_uplo_const_tally3(opts.uplo), &N, &m1, &c_one, h_A, &N, h_R, &N, &c_zero, h_work, &N);
                    for(int i=0; i < m1; ++i)
                        blasf77_csscal(&N, &w1[i], &h_R[i*N], &ione);
                    blasf77_chemm("L", lapack_uplo_const_tally3(opts.uplo), &N, &m1, &c_neg_one, h_B, &N, h_R, &N, &c_one, h_work, &N);
                    result[0] *= lapackf77_clange("1", &N, &m1, h_work, &N, rwork)/N;
                }
                else if (opts.itype == 2) {
                    blasf77_chemm("L", lapack_uplo_const_tally3(opts.uplo), &N, &m1, &c_one, h_B, &N, h_R, &N, &c_zero, h_work, &N);
                    for(int i=0; i < m1; ++i)
                        blasf77_csscal(&N, &w1[i], &h_R[i*N], &ione);
                    blasf77_chemm("L", lapack_uplo_const_tally3(opts.uplo), &N, &m1, &c_one, h_A, &N, h_work, &N, &c_neg_one, h_R, &N);
                    ...
                    for(int i=0; i < m1; ++i)
                        blasf77_csscal(&N, &w1[i], &h_R[i*N], &ione);
                    blasf77_chemm("L", lapack_uplo_const_tally3(opts.uplo), &N, &m1, &c_one, h_B, &N, h_work, &N, &c_neg_one, h_R, &N);
                    result[0] *= lapackf77_clange("1", &N, &m1, h_R, &N, rwork)/N;
                }
                ...
                //magma_tally3_chegvdx( opts.itype, Magma_tally3NoVec, Magma_tally3RangeI, opts.uplo,
