...
int main( int argc, char** argv)
{
    ...
    for( int i = 0; i < opts.ntest; ++i ) {
      ...
      for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_minproduct_dsetmatrix( M, columns, h_R, lda, d_A, ldda );
            ...
            info1 = magma_minproduct_dgetrf_batched( M, N, dA_array, ldda, dipiv_array, dinfo_array, batchCount, queue);
            info2 = magma_minproduct_dgetri_outofplace_batched( min(M,N), dA_array, ldda, dipiv_array, dinvA_array, ldda, dinfo_array, batchCount, queue);
            ...
            {
                if(cpu_info[i] != 0 ){
                    printf("magma_minproduct_dgetrf_batched matrix %d returned error %d\n", i, (int)cpu_info[i] );
                }
            }
            if (info1 != 0) printf("magma_minproduct_dgetrf_batched returned argument error %d: %s.\n", (int) info1, magma_minproduct_strerror( info1 ));
            if (info2 != 0) printf("magma_minproduct_dgetri_batched returned argument error %d: %s.\n", (int) info2, magma_minproduct_strerror( info2 ));
            /* =====================================================================
            ...
            if ( opts.lapack ) {
                ...
                lwork = magma_minproduct_int_t( MAGMA_minproduct_D_REAL( tmp ));
                TESTING_MALLOC_CPU( work,  double, lwork  );
                lapackf77_dlacpy( Magma_minproductUpperLowerStr, &M, &columns, h_R, &lda, h_A, &lda );
            ...
            if ( opts.check ) {
                ...
                magma_minproduct_dgetmatrix( min(M,N), N*batchCount, d_invA, ldda, h_R, lda );
                int stop=0;
                n2     = lda*N;
                for(int i=0; i < batchCount; i++)
                {
