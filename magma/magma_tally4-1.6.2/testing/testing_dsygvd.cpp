/*
    -- MAGMA_tally4 (version 1.6.1) --
       ...
       @generated from testing_zhegvd.cpp normal z -> d, Fri Jan 30 19:00:26 2015
...
int main( int argc, char** argv)
{
    ...
    magma_tally4_int_t N, n2, info, nb, lwork, liwork, lda;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lda    = N;
            n2     = N*lda;
            nb     = magma_tally4_get_dsytrd_nb(N);
            #ifdef COMPLEX
            ...
            //                 &d_one, &N, &N, lapack_uplo_const_tally4(opts.uplo), h_B, &lda, h_work, &info);
            //lapackf77_dlaset( "A", &N, &N, &c_zero, &c_one, h_B, &lda);
            lapackf77_dlarnv( &ione, ISEED, &n2, h_B );
            magma_tally4_dmake_hpd( N, h_B, lda );
            lapackf77_dlacpy( Magma_tally4FullStr, &N, &N, h_A, &lda, h_R, &lda );
            lapackf77_dlacpy( Magma_tally4FullStr, &N, &N, h_B, &lda, h_S, &lda );
            
            /* warmup */
            if ( opts.warmup ) {
                magma_tally4_dsygvd( opts.itype, opts.jobz, opts.uplo,
                              ...
                              &info );
                if (info != 0)
                    printf("magma_tally4_dsygvd returned error %d: %s.\n",
            ...
            if ( opts.check && opts.jobz != Magma_tally4NoVec ) {
                ...
                if ( opts.itype == 1 || opts.itype == 2 ) {
                    ...
                    blasf77_dsymm("R", lapack_uplo_const_tally4(opts.uplo), &N, &N, &c_neg_one, h_B, &lda, h_work, &N, &c_one, h_S, &lda);
                    result[1] = lapackf77_dlange("1", &N, &N, h_S, &lda, rwork) / N;
                }
                else if ( opts.itype == 3 ) {
                    lapackf77_dlacpy( Magma_tally4FullStr, &N, &N, h_B, &lda, h_S, &lda);
                    blasf77_dsyrk(lapack_uplo_const_tally4(opts.uplo), "N", &N, &N, &d_neg_one, h_R, &lda, &d_one, h_S, &lda);
                    result[1] = lapackf77_dlansy("1", lapack_uplo_const_tally4(opts.uplo), &N, h_S, &lda, rwork) / N
                              / lapackf77_dlansy("1", lapack_uplo_const_tally4(opts.uplo), &N, h_B, &lda, rwork);
                }
                
                result[0] = 1.;
                result[0] /= lapackf77_dlansy("1", lapack_uplo_const_tally4(opts.uplo), &N, h_A, &lda, rwork);
                result[0] /= lapackf77_dlange("1", &N, &N, h_R, &lda, rwork);
                
                if ( opts.itype == 1 ) {
                    blasf77_dsymm("L", lapack_uplo_const_tally4(opts.uplo), &N, &N, &c_one, h_A, &lda, h_R, &lda, &c_zero, h_work, &N);
                    for(int i=0; i < N; ++i)
                        blasf77_dscal(&N, &w1[i], &h_R[i*N], &ione);
                    blasf77_dsymm("L", lapack_uplo_const_tally4(opts.uplo), &N, &N, &c_neg_one, h_B, &lda, h_R, &lda, &c_one, h_work, &N);
                    result[0] *= lapackf77_dlange("1", &N, &N, h_work, &lda, rwork)/N;
                }
                else if ( opts.itype == 2 ) {
                    blasf77_dsymm("L", lapack_uplo_const_tally4(opts.uplo), &N, &N, &c_one, h_B, &lda, h_R, &lda, &c_zero, h_work, &N);
                    for(int i=0; i < N; ++i)
                        blasf77_dscal(&N, &w1[i], &h_R[i*N], &ione);
                    blasf77_dsymm("L", lapack_uplo_const_tally4(opts.uplo), &N, &N, &c_one, h_A, &lda, h_work, &N, &c_neg_one, h_R, &lda);
                    ...
                    for(int i=0; i < N; ++i)
                        blasf77_dscal(&N, &w1[i], &h_R[i*N], &ione);
                    blasf77_dsymm("L", lapack_uplo_const_tally4(opts.uplo), &N, &N, &c_one, h_B, &lda, h_work, &N, &c_neg_one, h_R, &lda);
                    result[0] *= lapackf77_dlange("1", &N, &N, h_R, &lda, rwork)/N;
                }
                
                /*
                assert( lwork >= 2*N*N );
                lapackf77_dsyt21( &ione, lapack_uplo_const_tally4(opts.uplo), &N, &izero,
                                  h_A, &lda,
                                  w1, w1,
                                  h_R, &lda,
                                  h_R, &lda,
                ...
                */
                ...
                //lapackf77_dlacpy( Magma_tally4FullStr, &N, &N, h_B, &lda, h_S, &lda );
                //
                //magma_tally4_dsygvd( opts.itype, Magma_tally4NoVec, opts.uplo,
            ...
            if ( opts.lapack ) {
                ...
                lapackf77_dsygvd( &opts.itype, lapack_vec_const_tally4(opts.jobz), lapack_uplo_const_tally4(opts.uplo),
            ...
            if ( opts.lapack ) {
                printf(    "    | D_magma_tally4 - D_lapack | / |D|  = %8.2e   %s\n\n", result[3], (result[3] < tolulp ? "ok" : "failed") );
