...
int main( int argc, char** argv)
{
    ...
    float *S1, *S2;
    #ifdef COMPLEX
    magma_tally4_int_t lrwork=0;
    float *rwork;
    #endif
    magma_tally4_int_t *iwork;
    
    magma_tally4_int_t M, N, N_U, M_VT, lda, ldu, ldv, n2, min_mn, max_mn, info, nb, lwork;
    magma_tally4_int_t ione     = 1;
    magma_tally4_int_t ISEED[4] = {0,0,0,1};
    magma_tally4_vec_t jobz;
    magma_tally4_int_t status = 0;

    MAGMA_tally4_UNUSED( max_mn );  // used only in complex
    ...
    if ( opts.check && ! opts.all && (jobz == Magma_tally4NoVec)) {
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            min_mn = min(M, N);
            max_mn = max(M, N);
            N_U  = (jobz == Magma_tally4AllVec ? M : min_mn);
            M_VT = (jobz == Magma_tally4AllVec ? N : min_mn);
            lda = M;
            ldu = M;
            ldv = M_VT;
            n2 = lda*N;
            ...
            magma_tally4_int_t x = max(M,N);
            magma_tally4_int_t y = min(M,N);
            #ifdef COMPLEX
            ...
            TESTING_MALLOC_CPU( VT,    magma_tally4FloatComplex, ldv*N );   // N x N (jobz=A) or min(M,N) x N
            TESTING_MALLOC_CPU( U,     magma_tally4FloatComplex, ldu*N_U ); // M x M (jobz=A) or M x min(M,N)
            TESTING_MALLOC_CPU( S1,    float, min_mn );
            TESTING_MALLOC_CPU( S2,    float, min_mn );
            TESTING_MALLOC_CPU( iwork, magma_tally4_int_t, 8*min_mn );
            ...
            if (info != 0)
                printf("magma_tally4_cgesdd returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                if ( jobz == Magma_tally4SomeVec || jobz == Magma_tally4AllVec ) {
                    U2  = U;
                    VT2 = VT;
                }
                else if ( jobz == Magma_tally4OverwriteVec ) {
                    if ( M >= N ) {
                        U2  = h_R;
                        ldu = lda;
                        VT2 = VT;
            ...
            if ( opts.lapack ) {
                ...
                lapackf77_cgesdd( lapack_vec_const_tally4(jobz), &M, &N,
                ...
                printf("   %c %5d %5d  %7.2f         %7.2f         %8.2e",
                       lapack_vec_const_tally4(jobz)[0],
