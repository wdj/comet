...
int main( int argc, char** argv)
{
    ...
    magma_tally3DoubleComplex *d_A, *d_invA;
    ...
    for( int i = 0; i < opts.ntest; ++i ) {
      ...
      for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_DEV(  d_invA,  magma_tally3DoubleComplex, ldda*N * batchCount);
            ...
            magma_tally3_zsetmatrix( M, columns, h_R, lda, d_A, ldda );
            ...
            info1 = magma_tally3_zgetrf_batched( M, N, dA_array, ldda, dipiv_array, dinfo_array, batchCount, queue);
            info2 = magma_tally3_zgetri_outofplace_batched( min(M,N), dA_array, ldda, dipiv_array, dinvA_array, ldda, dinfo_array, batchCount, queue);
            ...
            {
                if(cpu_info[i] != 0 ){
                    printf("magma_tally3_zgetrf_batched matrix %d returned error %d\n", i, (int)cpu_info[i] );
                }
            }
            if (info1 != 0) printf("magma_tally3_zgetrf_batched returned argument error %d: %s.\n", (int) info1, magma_tally3_strerror( info1 ));
            if (info2 != 0) printf("magma_tally3_zgetri_batched returned argument error %d: %s.\n", (int) info2, magma_tally3_strerror( info2 ));
            /* =====================================================================
            ...
            if ( opts.lapack ) {
                ...
                lwork = magma_tally3_int_t( MAGMA_tally3_Z_REAL( tmp ));
                TESTING_MALLOC_CPU( work,  magma_tally3DoubleComplex, lwork  );
                lapackf77_zlacpy( Magma_tally3UpperLowerStr, &M, &columns, h_R, &lda, h_A, &lda );
            ...
            if ( opts.check ) {
                ...
                magma_tally3_zgetmatrix( min(M,N), N*batchCount, d_invA, ldda, h_R, lda );
                int stop=0;
                n2     = lda*N;
                for(int i=0; i < batchCount; i++)
                {
