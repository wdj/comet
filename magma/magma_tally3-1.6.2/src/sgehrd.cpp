...
/**
    ...
    This version stores the T matrices in dT, for later use in magma_tally3_sorghr.
    ...
    magma_tally3_int_t *info)
{
    ...
    *info = 0;
    ...
    if (nb == 1 || nb > nh) {
        ...
        //   n*ldda  for dA
        float *dwork;
        if (MAGMA_tally3_SUCCESS != magma_tally3_smalloc( &dwork, 2*nb*ldda + n*ldda )) {
        ...
        float *T;
        magma_tally3_smalloc_cpu( &T, nb*nb );
        if ( T == NULL ) {
        ...
        magma_tally3blas_slaset( Magma_tally3Full, nb, nb, c_zero, c_zero, dV, ldda );
        ...
        magma_tally3blas_slaset( Magma_tally3Full, nb, n, c_zero, c_zero, dT, nb );
        
        // Copy the matrix to the GPU
        magma_tally3_ssetmatrix( n, n-ilo, A(0,ilo), lda, dA, ldda );
        ...
        for (i = ilo; i < ihi-1 - nb; i += nb) {
            ...
            magma_tally3_slahr2( ihi, i+1, nb,
                          dA(0,i-ilo), ldda,
                          dV,          ldda,
                          A(0,i),      lda,
                          &tau[i], T, nb, work, n);
            ...
            dTi = dT + (i - ilo)*nb;
            magma_tally3_ssetmatrix( nb, nb, T, nb, dTi, nb );
            
            magma_tally3_slahru( n, ihi, i, nb,
                          A(0,i),      lda,
                          dA(0,i-ilo), ldda, // dA
