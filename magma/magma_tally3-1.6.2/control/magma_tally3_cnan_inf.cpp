/*
    -- MAGMA_tally3 (version 1.6.1) --
       ...
       @author Mark Gates
       @generated from magma_tally3_znan_inf.cpp normal z -> c, Fri Jan 30 19:00:20 2015
...
const magma_tally3FloatComplex MAGMA_tally3_C_NAN
    = MAGMA_tally3_C_MAKE( std::numeric_limits<float>::quiet_NaN(),
                    std::numeric_limits<float>::quiet_NaN() );

const magma_tally3FloatComplex MAGMA_tally3_C_INF
    = MAGMA_tally3_C_MAKE( std::numeric_limits<float>::infinity(),
                    std::numeric_limits<float>::infinity() );

/** @return true if either real(x) or imag(x) is NAN. */
inline bool magma_tally3_c_isnan( magma_tally3FloatComplex x )
{
#ifdef COMPLEX
    return isnan( MAGMA_tally3_C_REAL( x )) ||
           isnan( MAGMA_tally3_C_IMAG( x ));
#else
    return isnan( x );
#endif
}
...
inline bool magma_tally3_c_isinf( magma_tally3FloatComplex x )
{
#ifdef COMPLEX
    return isinf( MAGMA_tally3_C_REAL( x )) ||
           isinf( MAGMA_tally3_C_IMAG( x ));
#else
    return isinf( x );
#endif
}
...
/**
    ...
    magma_tally3_cnan_inf checks a matrix that is located on the CPU host
    ...
    ********************************************************************/
extern "C"
magma_tally3_int_t magma_tally3_cnan_inf(
    ...
    if ( uplo == Magma_tally3Lower ) {
        for( int j = 0; j < n; ++j ) {
            for( int i = j; i < m; ++i ) {  // i >= j
                if      ( magma_tally3_c_isnan( *A(i,j) )) { c_nan++; }
                else if ( magma_tally3_c_isinf( *A(i,j) )) { c_inf++; }
            }
        }
    }
...
/**
    ...
    ********************************************************************/
extern "C"
magma_tally3_int_t magma_tally3_cnan_inf_gpu(
    magma_tally3_uplo_t uplo, magma_tally3_int_t m, magma_tally3_int_t n,
    ...
    magma_tally3_int_t *cnt_inf )
{
    ...
    magma_tally3_int_t cnt = magma_tally3_cnan_inf( uplo, m, n, A, lda, cnt_nan, cnt_inf );
