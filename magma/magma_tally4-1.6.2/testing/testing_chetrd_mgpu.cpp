...
int main( int argc, char** argv)
{
    ...
    magma_tally4_int_t k = 1;  // TODO: UNKNOWN, UNDOCUMENTED VARIABLE (number of streams?)
    ...
    printf("uplo = %s, ngpu %d\n", lapack_uplo_const_tally4(opts.uplo), (int) opts.ngpu );
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( opts.ngpu == 0 ) {
                magma_tally4_chetrd(opts.uplo, N, h_R, lda, diag, offdiag,
                             tau, h_work, lwork, &info);
            } else {
                magma_tally4_chetrd_mgpu(opts.ngpu, k, opts.uplo, N, h_R, lda, diag, offdiag,
                                  tau, h_work, lwork, &info);
            }
            gpu_time = magma_tally4_wtime() - gpu_time;
            if ( info != 0 ) {
                printf("magma_tally4_chetrd_mgpu returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                lapackf77_clacpy( lapack_uplo_const_tally4(opts.uplo), &N, &N, h_R, &lda, h_Q, &lda);
