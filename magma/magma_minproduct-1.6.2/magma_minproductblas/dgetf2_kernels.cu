...
idamax_devfunc(int length, const double *x, int incx, double *shared_x, int *shared_idx)
{
    ...
    for(int s =0 ; s < nchunk; s++)
    {
        if( (tx + s * zamax < length) && (tx < zamax) )
        {
            ...
            res1 = fabs(MAGMA_minproduct_D_REAL(res)) + fabs(MAGMA_minproduct_D_IMAG(res));
...
__global__ void
tree_idamax_kernel2_batched(int n, int step,  magma_minproduct_int_t** ipiv_array, magma_minproduct_int_t *info_array, int gbstep, double** data_pool_array, magma_minproduct_int_t** id_pool_array)
{
    __shared__ double shared_x[zamax];
    __shared__ int    shared_idx[zamax];
...
magma_minproduct_int_t magma_minproduct_idamax_lg_batched(magma_minproduct_int_t length, double **x_array, magma_minproduct_int_t incx, magma_minproduct_int_t step,  magma_minproduct_int_t lda,
...
extern "C"
magma_minproduct_int_t magma_minproduct_idamax_batched(magma_minproduct_int_t length, 
...
#else
    // the magma_minproduct_idamax_lg_batched is faster but when cuda launch it as 2 kernels the white space time between these 2 kernels and the next kernel is larger than using the idamax_kernel for that today we are using only idamax_kernel
    if( length <= 10 * zamax )
    {  
        dim3 grid(1, 1, batchCount);
        int chunk = (length-1)/zamax + 1;
        idamax_kernel_batched<<< grid, zamax, zamax * (sizeof(double) + sizeof(magma_minproduct_int_t)), queue >>>
                      (length, chunk, x_array, incx, step, lda, ipiv_array, info_array, gbstep);

    }
    else
    {
        magma_minproduct_idamax_lg_batched(length, x_array, incx, step, lda, ipiv_array, info_array, gbstep, batchCount);
    }
#endif
...
__global__
void dswap_kernel_batched(magma_minproduct_int_t n, double **x_array, magma_minproduct_int_t incx, magma_minproduct_int_t step, magma_minproduct_int_t** ipiv_array)
{
...
////////////////////////////////////////////////////////////////////////////////////////////////////
extern "C"
magma_minproduct_int_t magma_minproduct_dswap_batched(magma_minproduct_int_t n, double **x_array, magma_minproduct_int_t incx, magma_minproduct_int_t step, 
...
*/
    if( n  > MAX_NTHREADS) 
    {
       printf("magma_minproduct_dswap_batched nb=%d, > %d, not supported \n",n, MAX_NTHREADS);
       return -15;
    }
    dim3 grid(1,1, batchCount);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
__global__
void dscal_dger_kernel_batched(int m, int n, int step, double **dA_array, int lda, magma_minproduct_int_t *info_array, int gbstep)
{
    ...
    __syncthreads();
    if(shared_y[0] == MAGMA_minproduct_D_ZERO) {
        info_array[blockIdx.z] = step + gbstep + 1; 
        return;
    }

    if (gbidx < m && gbidx > 0) {
        double reg = MAGMA_minproduct_D_ZERO;
        reg = A[gbidx];
        reg *= MAGMA_minproduct_D_DIV(MAGMA_minproduct_D_ONE, shared_y[0]);
        A[gbidx] = reg;
        #pragma unroll
        for(int i=1; i < n; i++) {
            ...
            A[gbidx + i*lda] += (MAGMA_minproduct_D_NEG_ONE) * shared_y[i] * reg;
        }
    }
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////
extern "C"
magma_minproduct_int_t magma_minproduct_dscal_dger_batched(magma_minproduct_int_t m, magma_minproduct_int_t n, magma_minproduct_int_t step,
...
*/
    ...
    if( n  > MAX_NTHREADS) 
    {
       printf("magma_minproduct_dscal_dger_batched nb=%d, > %d, not supported \n",n, MAX_NTHREADS);
       return -15;
    }
...
void dgetf2trsm_kernel_batched(int ib, int n, double **dA_array, int step, int lda)
{
    ...
    if (tid < n) {
        ...
        for( d=0;  d<ib-1; d++) {
            for( i=d+1; i<ib; i++) {
                shared_b[i+tid*ib] += (MAGMA_minproduct_D_NEG_ONE) * shared_a[i+d*ib] * shared_b[d+tid*ib];
            }
        }
    }
    __syncthreads();
...
////////////////////////////////////////////////////////////////////////////////////////////////////
extern "C" void
magma_minproduct_dgetf2trsm_batched(magma_minproduct_int_t ib, magma_minproduct_int_t n, double **dA_array,  magma_minproduct_int_t step, magma_minproduct_int_t lda,
                       magma_minproduct_int_t batchCount, magma_minproduct_queue_t queue)
{
/*
...
dscal5_device(int m, double* x, double alpha)
{
    ...
    for(int s=0 ; s < nchunk; s++)
    {
        if( (tid + s * MAX_NTHREADS) < m ) {
            #if 0
            x[tid + s * MAX_NTHREADS] *= MAGMA_minproduct_D_DIV(MAGMA_minproduct_D_ONE, alpha);
            #else
...
////////////////////////////////////////////////////////////////////////////////////////////////////
__global__ void 
zcomputecolumn_kernel_shared_batched(int m, int paneloffset, int step, double **dA_array, int lda, magma_minproduct_int_t **ipiv_array, magma_minproduct_int_t *info_array, int gbstep)
{
...
////////////////////////////////////////////////////////////////////////////////////////////////////
extern "C"
magma_minproduct_int_t magma_minproduct_dcomputecolumn_batched(magma_minproduct_int_t m, magma_minproduct_int_t paneloffset, magma_minproduct_int_t step, 
                                        double **dA_array,  magma_minproduct_int_t lda,
...
*/
    ...
    {
        printf("magma_minproduct_dcomputecolumn_batched error out of shared memory \n");
        return -20;
    }
