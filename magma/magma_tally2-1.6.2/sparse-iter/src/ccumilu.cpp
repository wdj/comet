    ...
    magma_tally2_queue_t queue )
{
    ...
    magma_tally2_c_matrix hA={Magma_tally2_CSR}, hACSR={Magma_tally2_CSR};
    magma_tally2_c_matrix hL={Magma_tally2_CSR}, hU={Magma_tally2_CSR};
    CHECK( magma_tally2_cmtransfer( A, &hA, A.memory_location, Magma_tally2_CPU, queue ));
    CHECK( magma_tally2_cmconvert( hA, &hACSR, hA.storage_type, Magma_tally2_CSR, queue ));

        // in case using fill-in
    if( precond->levels > 0 ){
        magma_tally2_c_matrix hAL={Magma_tally2_CSR}, hAUt={Magma_tally2_CSR};
        CHECK( magma_tally2_csymbilu( &hACSR, precond->levels, &hAL, &hAUt,  queue ));
        magma_tally2_cmfree(&hAL, queue);
        magma_tally2_cmfree(&hAUt, queue);
    }

    CHECK( magma_tally2_cmtransfer(hACSR, &(precond->M), Magma_tally2_CPU, Magma_tally2_DEV, queue ));

    magma_tally2_cmfree( &hA, queue );
    magma_tally2_cmfree( &hACSR, queue );
    ...
    CHECK( magma_tally2_cmtransfer( precond->M, &hA, Magma_tally2_DEV, Magma_tally2_CPU, queue ));

    hL.diagorder_type = Magma_tally2_UNITY;
    CHECK( magma_tally2_cmconvert( hA, &hL , Magma_tally2_CSR, Magma_tally2_CSRL, queue ));
    hU.diagorder_type = Magma_tally2_VALUE;
    CHECK( magma_tally2_cmconvert( hA, &hU , Magma_tally2_CSR, Magma_tally2_CSRU, queue ));
    CHECK( magma_tally2_cmtransfer( hL, &(precond->L), Magma_tally2_CPU, Magma_tally2_DEV, queue ));
    CHECK( magma_tally2_cmtransfer( hU, &(precond->U), Magma_tally2_CPU, Magma_tally2_DEV, queue ));
    ...
    magma_tally2_queue_t queue )
{
    ...
    magma_tally2_c_matrix hA={Magma_tally2_CSR}, hL={Magma_tally2_CSR}, hU={Magma_tally2_CSR};
    
    if (precond->L.memory_location != Magma_tally2_DEV ){
        
        CHECK( magma_tally2_cmtransfer( precond->M, &hA,
    ...
    if( precond->maxiter < 50 ){
        ...
        // extract the diagonal of L into precond->d
        CHECK( magma_tally2_cjacobisetup_diagscal( precond->L, &precond->d, queue ));
        CHECK( magma_tally2_cvinit( &precond->work1, Magma_tally2_DEV, precond->U.num_rows, 1, MAGMA_tally2_C_ZERO, queue ));
        
        // extract the diagonal of U into precond->d2
        CHECK( magma_tally2_cjacobisetup_diagscal( precond->U, &precond->d2, queue ));
        CHECK( magma_tally2_cvinit( &precond->work2, Magma_tally2_DEV, precond->U.num_rows, 1, MAGMA_tally2_C_ZERO, queue ));
    }
    ...
    magma_tally2_queue_t queue )
{
    ...
    magma_tally2_c_matrix hA={Magma_tally2_CSR}, hACSR={Magma_tally2_CSR}, U={Magma_tally2_CSR};
    CHECK( magma_tally2_cmtransfer( A, &hA, A.memory_location, Magma_tally2_CPU, queue ));
    ...
    CHECK( magma_tally2_cmconvert( hACSR, &U, Magma_tally2_CSR, Magma_tally2_CSRL, queue ));
    magma_tally2_cmfree( &hACSR, queue );
    CHECK( magma_tally2_cmtransfer(U, &(precond->M), Magma_tally2_CPU, Magma_tally2_DEV, queue ));
...
/*
    ...
    magma_tally2_c_matrix hD, hR, hAt

    CHECK( magma_tally2_ccsrsplit( 256, hA, &hD, &hR, queue ));

    CHECK( magma_tally2_cmtransfer( hD, &precond->LD, Magma_tally2_CPU, Magma_tally2_DEV, queue ));
    CHECK( magma_tally2_cmtransfer( hR, &precond->L, Magma_tally2_CPU, Magma_tally2_DEV, queue ));

    magma_tally2_cmfree(&hD, queue );
    magma_tally2_cmfree(&hR, queue );

    CHECK( magma_tally2_c_cucsrtranspose(   hA, &hAt, queue ));

    CHECK( magma_tally2_ccsrsplit( 256, hAt, &hD, &hR, queue ));

    CHECK( magma_tally2_cmtransfer( hD, &precond->UD, Magma_tally2_CPU, Magma_tally2_DEV, queue ));
    CHECK( magma_tally2_cmtransfer( hR, &precond->U, Magma_tally2_CPU, Magma_tally2_DEV, queue ));
    
    magma_tally2_cmfree(&hD, queue );
    magma_tally2_cmfree(&hR, queue );
    magma_tally2_cmfree(&hA, queue );
    magma_tally2_cmfree(&hAt, queue );
*/
