...
__global__ void ztranspose_inplace_odd( int n, magma_tally3DoubleComplex *matrix, int lda )
{
    __shared__ magma_tally3DoubleComplex sA[ NB ][ NB+1 ];
    __shared__ magma_tally3DoubleComplex sB[ NB ][ NB+1 ];
    ...
    magma_tally3DoubleComplex *A = matrix + ii+i + (jj+j)*lda;
    if( ii == jj ) {
        if ( ii+i < n && jj+j < n ) {
            sA[j][i] = *A;
        }
        ...
        magma_tally3DoubleComplex *B = matrix + jj+i + (ii+j)*lda;
        if ( ii+i < n && jj+j < n ) {
            sA[j][i] = *A;
        }
        if ( jj+i < n && ii+j < n ) {
...
__global__ void ztranspose_inplace_even( int n, magma_tally3DoubleComplex *matrix, int lda )
{
    __shared__ magma_tally3DoubleComplex sA[ NB ][ NB+1 ];
    __shared__ magma_tally3DoubleComplex sB[ NB ][ NB+1 ];
...
/**
    @see magma_tally3blas_ztranspose_inplace_q
    @ingroup magma_tally3_zaux2
    ********************************************************************/
extern "C" void
magma_tally3blas_ztranspose_inplace(
    ...
    magma_tally3DoubleComplex_ptr dA, magma_tally3_int_t ldda )
{
    magma_tally3blas_ztranspose_inplace_q( n, dA, ldda, magma_tally3_stream );
}
