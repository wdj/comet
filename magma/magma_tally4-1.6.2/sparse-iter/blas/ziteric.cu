...
__global__ void 
magma_tally4_ziteric_csr_kernel(    
    magma_tally4_int_t n, 
    magma_tally4_int_t nnz, 
    magma_tally4_index_t *Arowidx, 
    magma_tally4_index_t *Acolidx, 
    const magma_tally4DoubleComplex * __restrict__  A_val,
    ...
    {     
        ...
        // modify entry
        if (i == j)
            val[il-1] = MAGMA_tally4_Z_MAKE(sqrt(abs(MAGMA_tally4_Z_REAL(s))), 0.0);
        else
            val[il-1] =  s / val[iu-1];
...
/**
    ...
    The input format of the initial guess matrix A is Magma_tally4_CSRCOO,
...
magma_tally4_ziteric_csr( 
    ...
    magma_tally4_ziteric_csr_kernel<<< grid, block, 0, magma_tally4_stream >>>
            ( A.num_rows, A.nnz, 
