...
/**
    ...
    lwork   INTEGER
            ...
            through magma_minproduct_get_zgeqlf_nb(M).
    \n
    ...
    @ingroup magma_minproduct_zgeqlf_comp
    ********************************************************************/
    ...
    magma_minproduct_int_t *info)
{
    ...
    magma_minproductDoubleComplex_ptr dA, dwork;
    magma_minproductDoubleComplex c_one = MAGMA_minproduct_Z_ONE;
    magma_minproduct_int_t i, k, lddwork, old_i, old_ib, nb;
    magma_minproduct_int_t rows, cols;
    magma_minproduct_int_t ib, ki, kk, mu, nu, iinfo, ldda;
    int lquery;

    nb = magma_minproduct_get_zgeqlf_nb(m);
    *info = 0;
    lquery = (lwork == -1);
    ...
    if (*info == 0) {
        if (k == 0)
            ...
            work[0] = MAGMA_minproduct_Z_MAKE( max(n*nb, 2*nb*nb), 0 );
        }
    ...
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc( &dA, (n)*ldda + nb*lddwork )) {
    ...
    if ( (nb > 1) && (nb < k) ) {
        ...
        for (i = k - kk + ki; i >= k -kk; i -= nb) {
            ...
            if (i < k - kk + ki) {
                ...
                magma_minproduct_zgetmatrix_async( m-rows, ib,
                                        dA(rows, n-k+i), ldda,
                ...
                magma_minproduct_zlarfb_gpu( Magma_minproductLeft, Magma_minproductConjTrans, Magma_minproductBackward, Magma_minproductColumnwise,
                                  rows, cols, old_ib,
                                  dA(0, cols+old_ib), ldda, dwork(0),      lddwork,
                                  dA(0, 0          ), ldda, dwork(old_ib), lddwork);
            }

            magma_minproduct_queue_sync( queues[1] );
            ...
            if (cols > 0) {
                ...
                lapackf77_zlarft( Magma_minproductBackwardStr, Magma_minproductColumnwiseStr,
                                  &rows, &ib,
                                  A(0, cols), &lda, tau + i, work, &ib);

                zpanel_to_q( Magma_minproductLower, ib, A(rows-ib,cols), lda, work+ib*ib);
                magma_minproduct_zsetmatrix( rows, ib,
                                  A(0,cols),  lda,
                                  dA(0,cols), ldda );
                zq_to_panel( Magma_minproductLower, ib, A(rows-ib,cols), lda, work+ib*ib);

                // Send the triangular part on the GPU
                magma_minproduct_zsetmatrix( ib, ib, work, ib, dwork(0), lddwork );
        ...
        magma_minproduct_zgetmatrix( m, nu, dA(0,0), ldda, A(0,0), lda );
    } else {
        mu = m;
        nu = n;
    }
