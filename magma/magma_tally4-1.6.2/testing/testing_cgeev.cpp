...
extern "C"
float magma_tally4_dlapy2(float x, float y)
{
    float ret_val, d;
    float w, z, xabs, yabs;
...
extern "C"
float magma_tally4_dzlapy2(magma_tally4FloatComplex x)
{
    return magma_tally4_dlapy2( MAGMA_tally4_C_REAL(x), MAGMA_tally4_C_IMAG(x) );
}
...
int main( int argc, char** argv)
{
    ...
    magma_tally4FloatComplex *h_A, *h_R, *VL, *VR, *h_work, *w1, *w2;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( w1,     magma_tally4FloatComplex, N );
            TESTING_MALLOC_CPU( w2,     magma_tally4FloatComplex, N );
            ...
            TESTING_MALLOC_PIN( VL,     magma_tally4FloatComplex, n2 );
            TESTING_MALLOC_PIN( VR,     magma_tally4FloatComplex, n2 );
            ...
            magma_tally4_cgeev( opts.jobvl, opts.jobvr,
                         N, h_R, lda, w1,
                         VL, lda, VR, lda,
                         h_work, lwork, rwork, &info );
            gpu_time = magma_tally4_wtime() - gpu_time;
            if (info != 0)
                printf("magma_tally4_cgeev returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                if ( opts.jobvr == Magma_tally4Vec ) {
                    ...
                    lapackf77_cget22( Magma_tally4NoTransStr, Magma_tally4NoTransStr, Magma_tally4NoTransStr,
                                      &N, h_A, &lda, VR, &lda, w1,
                    ...
                    result[1] = -1.;
                    for( int j = 0; j < N; ++j ) {
                        tnrm = magma_tally4_cblas_scnrm2( N, &VR[j*lda], ione );
                        ...
                        for( int jj = 0; jj <N; ++jj ) {
                            ...
                            if (MAGMA_tally4_C_IMAG(VR[jj + j*lda])==0. &&
                                fabs( MAGMA_tally4_C_REAL(VR[jj+j*lda]) ) > vrmx)
                            {
                                vrmx = fabs( MAGMA_tally4_C_REAL( VR[jj+j*lda] ) );
                            }
                        }
                        if (vrmx / vmx < 1. - ulp*2.)
                            result[1] = ulpinv;
                ...
                if ( opts.jobvl == Magma_tally4Vec ) {
                    ...
                    lapackf77_cget22( Magma_tally4ConjTransStr, Magma_tally4NoTransStr, Magma_tally4ConjTransStr,
                                      &N, h_A, &lda, VL, &lda, w1,
                    ...
                    result[3] = -1.;
                    for( int j = 0; j < N; ++j ) {
                        tnrm = magma_tally4_cblas_scnrm2( N, &VL[j*lda], ione );
                        ...
                        for( int jj = 0; jj < N; ++jj ) {
                            ...
                            if (MAGMA_tally4_C_IMAG(VL[jj + j*lda])==0. &&
                                fabs( MAGMA_tally4_C_REAL( VL[jj + j*lda] ) ) > vrmx)
                            {
                                vrmx = fabs( MAGMA_tally4_C_REAL( VL[jj+j*lda]) );
                            }
                        }
                        if (vrmx / vmx < 1. - ulp*2.)
                            result[3] = ulpinv;
            ...
            if ( opts.check == 2 ) {
                ...
                // this is really slow because it calls magma_tally4_cgeev multiple times
                magma_tally4FloatComplex *LRE, DUM;
                TESTING_MALLOC_PIN( LRE, magma_tally4FloatComplex, n2 );
                ...
                magma_tally4_cgeev( Magma_tally4Vec, Magma_tally4Vec,
                             N, h_R, lda, w1,
                             VL, lda, VR, lda,
                             h_work, lwork, rwork, &info );
                if (info != 0)
                    printf("magma_tally4_cgeev (case V, V) returned error %d: %s.\n",
                           (int) info, magma_tally4_strerror( info ));
                ...
                // lapackf77_clacpy( Magma_tally4UpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                // magma_tally4_cgeev( Magma_tally4NoVec, Magma_tally4NoVec,
                //              N, h_R, lda, w2,
                //              &DUM, 1, &DUM, 1,
                //              h_work, lwork, rwork, &info );
                // if (info != 0)
                //     printf("magma_tally4_cgeev (case N, N) returned error %d: %s.\n",
                //            (int) info, magma_tally4_strerror( info ));
                // 
                // // Do test 5: W(full) = W(partial, W only)
                // result[4] = 1;
                // for( int j = 0; j < N; ++j )
                //     if ( ! MAGMA_tally4_C_EQUAL( w1[j], w2[j] ))
                //         result[4] = 0;
                
                // ----------
                // Compute eigenvalues and right eigenvectors
                lapackf77_clacpy( Magma_tally4UpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                magma_tally4_cgeev( Magma_tally4NoVec, Magma_tally4Vec,
                             N, h_R, lda, w2,
                             &DUM, 1, LRE, lda,
                             h_work, lwork, rwork, &info );
                if (info != 0)
                    printf("magma_tally4_cgeev (case N, V) returned error %d: %s.\n",
                           (int) info, magma_tally4_strerror( info ));
                ...
                for( int j = 0; j < N; ++j )
                    for( int jj = 0; jj < N; ++jj )
                        if ( ! MAGMA_tally4_C_EQUAL( VR[j+jj*lda], LRE[j+jj*lda] ))
                            result[7] = 0;
                
                // ----------
                // Compute eigenvalues and left eigenvectors
                lapackf77_clacpy( Magma_tally4UpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                magma_tally4_cgeev( Magma_tally4Vec, Magma_tally4NoVec,
                             N, h_R, lda, w2,
                             LRE, lda, &DUM, 1,
                             h_work, lwork, rwork, &info );
                if (info != 0)
                    printf("magma_tally4_cgeev (case V, N) returned error %d: %s.\n",
                           (int) info, magma_tally4_strerror( info ));
                ...
                for( int j = 0; j < N; ++j )
                    for( int jj = 0; jj < N; ++jj )
                        if ( ! MAGMA_tally4_C_EQUAL( VL[j+jj*lda], LRE[j+jj*lda] ))
                            result[8] = 0;
            ...
            if ( opts.lapack ) {
                ...
                lapackf77_cgeev( lapack_vec_const_tally4(opts.jobvl), lapack_vec_const_tally4(opts.jobvr),
                                 &N, h_A, &lda, w2,
                                 VL, &lda, VR, &lda,
                                 h_work, &lwork, rwork, &info );
                ...
                for( int j=0; j < N; ++j ) {
                    for( int j2=j; j2 < N; ++j2 ) {
                        ...
                        float diff2 = magma_tally4_dzlapy2( diff ) / max( magma_tally4_dzlapy2( w1copy[j] ), tol );
                        if ( diff2 < 100*tol ) {
                            if ( j != j2 ) {
