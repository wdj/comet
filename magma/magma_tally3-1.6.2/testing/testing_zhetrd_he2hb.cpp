...
extern "C" void zcheck_eig_(char *JOBZ, int  *MATYPE, int  *N, int  *NB,
                       magma_tally3DoubleComplex* A, int  *LDA, double *AD, double *AE, double *D1, double *EIG,
                    magma_tally3DoubleComplex *Z, int  *LDZ, magma_tally3DoubleComplex *WORK, double *RWORK, double *RESU);
#endif
...
int main( int argc, char** argv)
{
    ...
    magma_tally3DoubleComplex *h_A, *h_R, *h_work, *dT1;
    magma_tally3DoubleComplex *tau;
    double *D, *E;

    /* Matrix size */
    magma_tally3_int_t N, n2, lda, lwork, lwork0;  //ldt
...
#if defined(CHECKEIG)
#if defined(PRECISION_z)  || defined(PRECISION_d)
    magma_tally3_int_t WANTZ=0;
    magma_tally3_int_t THREADS=1;
#endif
#endif

    magma_tally3_int_t NE = 0;
    magma_tally3_int_t NB = 0;
    magma_tally3_int_t ngpu = 1;
    ...
    NB = opts.nb;
    if (NB < 1)
        NB  = 64; //64; //magma_tally3_get_zhetrd_he2hb_nb(N);

    // what is NE ?
    if (NE < 1)
        NE  = 64; //N;  //magma_tally3_get_zhetrd_he2hb_nb(N);  // N not yet initialized
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( tau,    magma_tally3DoubleComplex, N-1    );
            
            TESTING_MALLOC_PIN( h_R,    magma_tally3DoubleComplex, lda*N  );
            TESTING_MALLOC_PIN( h_work, magma_tally3DoubleComplex, lwork0 );
            TESTING_MALLOC_PIN( D, double, N );
            TESTING_MALLOC_PIN( E, double, N );
            
            //TESTING_MALLOC_DEV( dT1, magma_tally3DoubleComplex, (2*min(N,N)+(N+31)/32*32)*NB );
            TESTING_MALLOC_DEV( dT1, magma_tally3DoubleComplex, (N*NB) );
            ...
            /*
            magma_tally3_zhetrd_he2hb( opts.uplo, N, NB, h_R, lda, tau, h_work, lwork0, dT1, THREADS, &info);
            tband = magma_tally3_wtime - gpu_time();
            printf("  Finish BAND  N %d  NB %d  ngpu %d timing= %f\n", N, NB, ngpu, tband);
            magma_tally3_zhetrd_bhe2trc_v5(THREADS, WANTZ, opts.uplo, NE, N, NB, h_R, lda, D, E, dT1, ldt);
            */
    
            /*
            magma_tally3_zhetrd_he2hb( opts.uplo, N, NB, h_R, lda, tau, h_work, lwork, dT1, THREADS, &info);
            tband = magma_tally3_wtime - gpu_time();
            printf("  Finish BAND  N %d  NB %d  ngpu %d timing= %f\n", N, NB, ngpu, tband);
            magma_tally3_zhetrd_bhe2trc(THREADS, WANTZ, opts.uplo, NE, N, NB, h_R, lda, D, E, dT1, ldt);
            */

            magma_tally3_range_t range = Magma_tally3RangeAll;
            magma_tally3_int_t fraction_ev = 100;
            magma_tally3_int_t il, iu, m1;
            double vl=0., vu=0.;
            ...
            magma_tally3DoubleComplex *hh_work;
            magma_tally3_int_t *iwork;
            magma_tally3_int_t /*nb,*/ /*lwork,*/ liwork;
            magma_tally3_int_t threads = magma_tally3_get_parallel_numthreads();
            #if defined(PRECISION_z) || defined(PRECISION_c)
                double *rwork;
                magma_tally3_int_t lrwork;
                lwork  = magma_tally3_zbulge_get_lq2(N, threads) + 2*N + N*N;
                lrwork = 1 + 5*N +2*N*N;
                TESTING_MALLOC_PIN( rwork, double, lrwork );
            #else
                lwork  = magma_tally3_zbulge_get_lq2(N, threads) + 1 + 6*N + 2*N*N;
            #endif
            liwork = 3 + 5*N;
            //nb = magma_tally3_get_zhetrd_nb(N);
            TESTING_MALLOC_PIN( hh_work, magma_tally3DoubleComplex, lwork  );
            TESTING_MALLOC_CPU( iwork,   magma_tally3_int_t,        liwork );
    
            if (ngpu == 1) {
                printf("calling zheevdx_2stage 1 GPU\n");
                magma_tally3_zheevdx_2stage( opts.jobz, range, opts.uplo, N,
                                h_R, lda,
                                vl, vu, il, iu,
                                &m1, D,
                                hh_work, lwork,
                ...
                magma_tally3_zheevdx_2stage_m(ngpu, opts.jobz, range, opts.uplo, N,
                                h_R, lda,
                                vl, vu, il, iu,
                                &m1, D,
                                hh_work, lwork,
            ...
            */
            ...
            if ( opts.check ) {
                ...
                int    lwork2 = 256*N;
                magma_tally3DoubleComplex *work2, *AINIT;
                double *rwork2, *D2;
                // TODO free this memory !
                magma_tally3_zmalloc_cpu( &work2, lwork2 );
                magma_tally3_dmalloc_cpu( &rwork2, N );
                magma_tally3_dmalloc_cpu( &D2, N );
                magma_tally3_zmalloc_cpu( &AINIT, N*lda );
                memcpy(AINIT, h_A, N*lda*sizeof(magma_tally3DoubleComplex));
                ...
                */
                ...
                //magma_tally3DoubleComplex mydz=0.0, mydo=1.0;
                //magma_tally3DoubleComplex *Z;
                // magma_tally3_zmalloc_cpu( &Z, N*lda );
                ...
                magma_tally3DoubleComplex *WORKAJETER;
                double *RWORKAJETER, *RESU;
                // TODO free this memory !
                magma_tally3_zmalloc_cpu( &WORKAJETER, (2* N * N + N)  );
                magma_tally3_dmalloc_cpu( &RWORKAJETER, N  );
                magma_tally3_dmalloc_cpu( &RESU, 10 );
                int MATYPE;
