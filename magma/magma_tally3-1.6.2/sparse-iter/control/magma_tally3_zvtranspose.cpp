    ...
    magma_tally3_queue_t queue )
{
    ...
    magma_tally3_z_matrix x_d={Magma_tally3_CSR}, y_d={Magma_tally3_CSR};
            
    if ( x.memory_location == Magma_tally3_DEV ) {
        CHECK( magma_tally3_zvinit( y, Magma_tally3_DEV, x.num_rows,x.num_cols, MAGMA_tally3_Z_ZERO, queue ));
        y->num_rows = x.num_rows;
        y->num_cols = x.num_cols;
        y->storage_type = x.storage_type;
        if ( x.major == Magma_tally3ColMajor) {
            y->major = Magma_tally3RowMajor;
            magma_tally3blas_ztranspose( m, n, x.val, m, y->val, n );
        }
        else {
            y->major = Magma_tally3ColMajor;
            magma_tally3blas_ztranspose( n, m, x.val, n, y->val, m );
        }
    } else {

        CHECK( magma_tally3_zmtransfer( x, &x_d, Magma_tally3_CPU, Magma_tally3_DEV, queue ));
        CHECK( magma_tally3_zvtranspose( x_d, &y_d, queue ));
        CHECK( magma_tally3_zmtransfer( y_d, y, Magma_tally3_DEV, Magma_tally3_CPU, queue ));
    }
...
cleanup:
    ...
    magma_tally3_zmfree( &x_d, queue );
    magma_tally3_zmfree( &y_d, queue );
