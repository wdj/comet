...
*********   (workspace) If FAST_HEMV is defined DZ should be (LDDZ, max(1,N)) in both cases.
    ...
    lddz    INTEGER
            ...
            JOBZ = Magma_tally4Vec, LDDZ >= max(1,N).
...
magma_tally4_cheevx_gpu(
    ...
    float *rwork, magma_tally4_int_t *iwork, magma_tally4_int_t *ifail,
    magma_tally4_int_t *info)
{
    ...
    const char* order_;
    magma_tally4_int_t indd, inde;
    magma_tally4_int_t imax;
    magma_tally4_int_t lopt, itmp1, indee;
    magma_tally4_int_t lower, wantz;
    magma_tally4_int_t i, j, jj, i__1;
    magma_tally4_int_t alleig, valeig, indeig;
    magma_tally4_int_t iscale, indibl;
    magma_tally4_int_t indiwk, indisp, indtau;
    magma_tally4_int_t indrwk, indwrk;
    magma_tally4_int_t llwork, nsplit;
    ...
    *info = 0;
    ...
    work[0] = MAGMA_tally4_C_MAKE( lopt, 0 );
    ...
    *m = 0;
    ...
    if (MAGMA_tally4_SUCCESS != magma_tally4_smalloc( &dwork, n )) {
        fprintf (stderr, "!!!! device memory allocation error (magma_tally4_cheevx_gpu)\n");
...
#ifdef FAST_HEMV
    magma_tally4_chetrd2_gpu(uplo, n, dA, ldda, &rwork[indd], &rwork[inde],
                      &work[indtau], wA, ldwa, &work[indwrk], llwork, dZ, lddz*n, &iinfo);
#else
    magma_tally4_chetrd_gpu (uplo, n, dA, ldda, &rwork[indd], &rwork[inde],
                      &work[indtau], wA, ldwa, &work[indwrk], llwork, &iinfo);
#endif

    lopt = n + (magma_tally4_int_t)MAGMA_tally4_C_REAL(work[indwrk]);
    ...
    if ((alleig || (indeig && il == 1 && iu == n)) && abstol <= 0.) {
        ...
        if (! wantz) {
            ...
            if (*info == 0) {
                ...
                magma_tally4_csetmatrix( n, n, wZ, ldwz, dZ, lddz );
            }
        }
        if (*info == 0) {
            *m = n;
