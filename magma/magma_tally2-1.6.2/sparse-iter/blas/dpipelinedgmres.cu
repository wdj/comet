...
__global__ void
magma_tally2_dpipelined_correction( 
    int n,  
    int k,
    double * skp, 
    double * r,
    double * v )
{
    ...
    temp[ i ] = ( i < k ) ? skp[ i ] * skp[ i ] : MAGMA_tally2_D_MAKE( 0.0, 0.0);
    __syncthreads();
    ...
    if( i == 0 ){
        tmp = MAGMA_tally2_D_REAL( temp[ i ] );
        zz = MAGMA_tally2_D_REAL( skp[(k)] );
        skp[k] = MAGMA_tally2_D_MAKE( sqrt(zz-tmp),0.0 );
    }
}

__global__ void
magma_tally2_dpipelined_copyscale( 
    int n,  
    int k,
    double * skp, 
    double * r,
...
__global__ void
magma_tally2_dpipelineddnrm2_kernel( 
    int m, 
    double * da, 
    int ldda, 
    double * dxnorm )
{
    ...
    magma_tally2Double_ptr dx = da + blockIdx.x * ldda;
    ...
    if (i==0)
        dxnorm[blockIdx.x] = MAGMA_tally2_D_MAKE( sqrt(sum[0]), 0.0 );
}

//----------------------------------------------------------------------------//

__global__ void
magma_tally2_dpipelinedscale( 
    int n, 
    double * r, 
    double * drnorm )
{
    ...
    magma_tally2_queue_t queue )
{
    ...
    magma_tally2_dpipelined_correction<<<Gs, Bs, Ms, queue >>>
                                            ( n, k, skp, r, v );
    magma_tally2_dpipelined_copyscale<<<Gs2, Bs, 0, queue >>>
                                            ( n, k, skp, r, v );
...
magma_tally2_dnrm2scale(
    ...
    magma_tally2Double_ptr r, 
    int lddr, 
    magma_tally2Double_ptr drnorm,
    magma_tally2_queue_t queue )
{
    dim3  blocks( 1 );
    dim3 threads( 512 );
    magma_tally2_dpipelineddnrm2_kernel<<< blocks, threads, 0, queue >>>
                                ( m, r, lddr, drnorm );

    dim3 Bs( BLOCK_SIZE );
    dim3 Gs2( magma_tally2_ceildiv( m, BLOCK_SIZE ) );
    magma_tally2_dpipelinedscale<<<Gs2, Bs, 0, queue >>>( m, r, drnorm );
