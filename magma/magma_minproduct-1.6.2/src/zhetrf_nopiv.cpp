    ...
    magma_minproduct_int_t *info)
{
    ...
    *info = 0;
    ...
    if ((MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc(&dA, n *ldda)) ||
        (MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc(&dW, nb*ldda))) {
    ...
    if (upper) {
        ...
        for (j=0; j<n; j+=nb) {
            ...
            magma_minproduct_zsetmatrix_async(j+jb, jb, A(0, j), lda, dA(0, j), ldda, stream[0]);
            trace_gpu_end( 0, 0 );
        }
        ...
        for (j=0; j<n; j += nb) {
            ...
            if ( j!=0) {
                ...
                magma_minproduct_zgetmatrix_async(jb, jb, dA(j, j), ldda, A(j,j), lda, stream[1]);
            }
            trace_gpu_end( 0, 0 );
            ...
            zhetrf_nopiv_cpu(Magma_minproductUpper, jb, ib, A(j, j), lda, info);
            trace_cpu_end( 0 );
            if (*info != 0){
                *info = *info + j;
                break;
            ...
            trace_gpu_start( 0, 0, "set", "set" );
            magma_minproduct_zsetmatrix_async(jb, jb, A(j, j), lda, dA(j, j), ldda, stream[0]);
            trace_gpu_end( 0, 0 );
            
            // copy j-th column of U back to CPU
            trace_gpu_start( 0, 1, "get", "get" );                                                                         
            magma_minproduct_zgetmatrix_async(j, jb, dA(0, j), ldda, A(0, j), lda, stream[1]);                                        
            trace_gpu_end( 0, 1 );
        ...
        for (j=0; j<n; j+=nb) {
            jb = min(nb, (n-j));
            trace_gpu_start( 0, 0, "set", "set" );
            magma_minproduct_zsetmatrix_async((n-j), jb, A(j, j), lda, dA(j, j), ldda, stream[0]);
            trace_gpu_end( 0, 0 );
        }
        ...
        for (j=0; j<n; j+=nb) {
            ...
            magma_minproduct_queue_sync(stream[1]);
            trace_cpu_start( 0, "potrf", "potrf" );
            zhetrf_nopiv_cpu(Magma_minproductLower, jb, ib, A(j, j), lda, info);
            trace_cpu_end( 0 );
            if (*info != 0){
                *info = *info + j;
                break;
            ...
            trace_gpu_start( 0, 1, "get", "get" );
            magma_minproduct_zgetmatrix_async(jb, j, dA(j, 0), ldda, A(j, 0), lda, stream[1]);
            trace_gpu_end( 0, 1 ); 
