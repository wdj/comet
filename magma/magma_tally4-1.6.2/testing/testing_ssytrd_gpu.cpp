...
int main( int argc, char** argv)
{
    ...
    magma_tally4Float_ptr d_R, dwork;
    float *tau;
    float          *diag, *offdiag;
    float           result[2] = {0., 0.};
    ...
    printf("2 - uses SSYMV from MAGMA_tally4 BLAS that requires extra space\n\n");
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_tally4_ssetmatrix( N, N, h_A, lda, d_R, ldda );
            ...
            if (opts.version == 1) {
                magma_tally4_ssytrd_gpu( opts.uplo, N, d_R, ldda, diag, offdiag,
                                  tau, h_R, lda, h_work, lwork, &info );
            }
            else {
                magma_tally4_ssytrd2_gpu( opts.uplo, N, d_R, ldda, diag, offdiag,
                                   tau, h_R, lda, h_work, lwork, dwork, ldwork, &info );
            }
            gpu_time = magma_tally4_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally4_ssytrd_gpu returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                magma_tally4_sgetmatrix( N, N, d_R, ldda, h_R, lda );
                magma_tally4_sgetmatrix( N, N, d_R, ldda, h_Q, lda );
