...
/**
    ...
    A       (workspace) DOUBLE_PRECISION array, dimension (LDA,N)
            On exit the diagonal, the  upper part (UPLO=Magma_tally4Upper)
            or the lower part (UPLO=Magma_tally4Lower) are copies of DA
    ...
    ldwork  INTEGER
            ...
            LDWORK >= ldda*ceil(n/64) + 2*ldda*nb, where nb = magma_tally4_get_dsytrd_nb(n),
            and 64 is for the blocksize of magma_tally4blas_dsymv.
    ...
    magma_tally4_int_t *info)
{
    ...
    magma_tally4_int_t nb = magma_tally4_get_dsytrd_nb( n );
    ...
    *info = 0;
    ...
    // rest of dwork (used as work in magma_tally4blas_dsymv) doesn't need to be cleared
    magma_tally4blas_dlaset( Magma_tally4Full, n, nb, c_zero, c_zero, dwork, lddw );
    ...
    if (upper) {
        ...
        for (i = n - nb; i >= kk; i -= nb) {
            ...
            magma_tally4_dgetmatrix( i+nb, nb, dA(0, i), ldda, A(0, i), lda );
            
            magma_tally4_dlatrd2( uplo, i+nb, nb, A(0, 0), lda, e, tau,
            ...
            magma_tally4_dsetmatrix( i + nb, nb, work, ldw, dwork, lddw );
            
            magma_tally4_dsyr2k( uplo, Magma_tally4NoTrans, i, nb, c_neg_one,
        ...
        for (i = 0; i < n-nx; i += nb) {
            ...
            /* Get the current panel */
            magma_tally4_dgetmatrix( n-i, nb, dA(i, i), ldda, A(i, i), lda );
            
            magma_tally4_dlatrd2( uplo, n-i, nb, A(i, i), lda, &e[i], &tau[i],
                           work, ldw, dA(i, i), ldda, dwork, lddw,
                           dwork + 2*lddw*nb, ldwork - 2*lddw*nb );
            
            /* Update the unreduced submatrix A(i+ib:n,i+ib:n), using
               an update of the form:  A := A - V*W' - W*V' */
            magma_tally4_dsetmatrix( n-i, nb, work, ldw, dwork, lddw );
            
            // cublas 6.5 crashes here if lddw % 32 != 0, e.g., N=250.
            magma_tally4_dsyr2k( Magma_tally4Lower, Magma_tally4NoTrans, n-i-nb, nb, c_neg_one,
        ...
        /* Use CPU code to reduce the last or only block */
        magma_tally4_dgetmatrix( n-i, n-i, dA(i, i), ldda, A(i, i), lda );
        
        i_n = n-i;
        lapackf77_dsytrd( uplo_, &i_n, A(i, i), &lda, &d[i], &e[i],
                          &tau[i], work, &lwork, &iinfo );
        
        magma_tally4_dsetmatrix( n-i, n-i, A(i, i), lda, dA(i, i), ldda );
    }
