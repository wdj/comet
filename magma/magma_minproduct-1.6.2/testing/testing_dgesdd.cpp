/*
    -- MAGMA_minproduct (version 1.6.1) --
       ...
       @generated from testing_zgesdd.cpp normal z -> d, Fri Jan 30 19:00:26 2015
       @author Mark Gates
...
int main( int argc, char** argv)
{
    ...
    MAGMA_minproduct_UNUSED( max_mn );  // used only in real
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            // x and y abbreviations used in dgesdd and dgesdd documentation
            ...
            switch( opts.svd_work ) {
                case 0: {  // query for workspace size
                    lwork = -1;
                    magma_minproduct_dgesdd( jobz, M, N,
                                  NULL, lda, NULL, NULL, ldu, NULL, ldv, dummy, lwork,
                                  #ifdef COMPLEX
                                  NULL,
                                  #endif
                                  NULL, &info );
                    lwork = (int) MAGMA_minproduct_D_REAL( dummy[0] );
                    break;
                }
                ...
                case 3: {  // optimal (for gesdd, 2 & 3 are same; for gesvd, they differ)
                    // formulas from dgesdd and dgesdd documentation
            ...
            TESTING_MALLOC_CPU( VT,    double, ldv*N );   // N x N (jobz=A) or min(M,N) x N
            TESTING_MALLOC_CPU( U,     double, ldu*N_U ); // M x M (jobz=A) or M x min(M,N)
            ...
            if (info != 0)
                printf("magma_minproduct_dgesdd returned error %d: %s.\n",
            ...
            if ( opts.lapack ) {
                ...
                if (info != 0)
                    printf("lapackf77_dgesdd returned error %d: %s.\n",
                           (int) info, magma_minproduct_strerror( info ));
