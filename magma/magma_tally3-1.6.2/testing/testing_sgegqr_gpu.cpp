...
int main( int argc, char** argv)
{
    ...
    magma_tally3Float_ptr d_A, dwork;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            // warmup
            if ( opts.warmup ) {
                magma_tally3_sgegqr_gpu( 1, M, N, d_A, ldda, dwork, h_work, &info );
                magma_tally3_ssetmatrix( M, N, h_R, lda, d_A, ldda );
            }
            ...
            magma_tally3_sgegqr_gpu( opts.version, M, N, d_A, ldda, dwork, h_rwork, &info );
            gpu_time = magma_tally3_sync_wtime( 0 ) - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally3_sgegqr returned error %d: %s.\n",
            ...
            // magma_tally3_sprint(N, N, h_work, N);
            ...
            if ( opts.lapack ) {
                ...
                for(int ii = 0; ii < N*N; ii += N+1 ) {
                    h_work[ii] = MAGMA_tally3_S_SUB(h_work[ii], c_one);
                }
