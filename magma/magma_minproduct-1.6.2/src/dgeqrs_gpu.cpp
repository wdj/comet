...
/**
    ...
    tau     DOUBLE_PRECISION array, dimension (N)
            ...
            reflector H(i), as returned by MAGMA_minproduct_DGEQRF_GPU.
    ...
    dT      DOUBLE_PRECISION array that is the output (the 6th argument)
            of magma_minproduct_dgeqrf_gpu of size
    ...
    @ingroup magma_minproduct_dgels_comp
    ********************************************************************/
    ...
    magma_minproduct_int_t *info)
{
    ...
    *info = 0;
    ...
    /* B := Q' * B */
    magma_minproduct_dormqr_gpu( Magma_minproductLeft, Magma_minproductTrans,
                      m, nrhs, n,
                      dA(0,0), ldda, tau,
    ...
    // TODO: this assumes that, on exit from magma_minproduct_dormqr_gpu, hwork contains
    ...
    // magma_minproduct_dtrsm and drop the dsetmatrix.
    if ( nrhs == 1 ) {
        blasf77_dtrsv( Magma_minproductUpperStr, Magma_minproductNoTransStr, Magma_minproductNonUnitStr,
                       &ib, hwork,         &rows,
                            hwork+rows*ib, &ione);
    } else {
        blasf77_dtrsm( Magma_minproductLeftStr, Magma_minproductUpperStr, Magma_minproductNoTransStr, Magma_minproductNonUnitStr,
                       &ib, &nrhs,
                       &c_one, hwork,         &rows,
                               hwork+rows*ib, &rows);
    }
    ...
    magma_minproduct_dsetmatrix( ib, nrhs, hwork+rows*ib, rows, dwork+i, lddwork );

    // update c
    if (nrhs == 1)
        magma_minproduct_dgemv( Magma_minproductNoTrans, i, ib,
    ...
    if (nb < k) {
        for (i = start; i >= 0; i -= nb) {
            ...
            if (i + ib < n) {
                if (nrhs == 1) {
                    magma_minproduct_dgemv( Magma_minproductNoTrans, ib, ib,
                                 c_one,  dT(i), ib,
                                         dB+i,      1,
                                 c_zero, dwork+i,  1);
                    magma_minproduct_dgemv( Magma_minproductNoTrans, i, ib,
    ...
    magma_minproduct_dcopymatrix( (n), nrhs,
                       dwork, lddwork,
                       dB,    lddb );
