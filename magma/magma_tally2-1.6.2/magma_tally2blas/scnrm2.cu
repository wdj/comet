...
__global__ void
magma_tally2blas_scnrm2_kernel( int m, magma_tally2FloatComplex *dA, int ldda, float *dxnorm )
{
...
//==============================================================================
__global__ void
magma_tally2blas_scnrm2_check_kernel( int m, magma_tally2FloatComplex *dA, int ldda, float *dxnorm, 
                               float *lsticc )
{
    const int tx = threadIdx.x;
    magma_tally2FloatComplex *dx = dA + blockIdx.x * ldda;
    ...
    magma_tally2Float_ptr dlsticc) 
{
    ...
    magma_tally2blas_scnrm2_check_kernel<<< blocks, threads >>>( m, dA, ldda, dxnorm, dlsticc );
}


//==============================================================================
__global__ void
magma_tally2blas_scnrm2_smkernel( int m, int n, magma_tally2FloatComplex *dA, int ldda,
                           float *dxnorm )
{
    const int tx = threadIdx.x;
    ...
    for( int k = ty; k < n; k += BLOCK_SIZEy ) {
        magma_tally2FloatComplex *dx = dA + k * ldda;
    ...
    float *dxnorm)
{
    ...
    magma_tally2blas_scnrm2_smkernel<<< blocks, threads, 0, magma_tally2_stream >>>( m, n, dA, ldda, dxnorm );
}
...
magma_tally2_scnrm2_adjust_kernel(float *xnorm, magma_tally2FloatComplex *c)
{
    ...
    temp = MAGMA_tally2_C_ABS( c[tx] ) / xnorm[0];
    sum[tx] = -temp * temp;
...
*/
extern "C" void
magma_tally2blas_scnrm2_adjust(magma_tally2_int_t k, magma_tally2Float_ptr dxnorm, magma_tally2FloatComplex_ptr dc)
{
    magma_tally2_scnrm2_adjust_kernel<<< 1, k, 0, magma_tally2_stream >>> (dxnorm, dc);
}
...
__global__ void
magma_tally2_scnrm2_row_check_adjust_kernel(
    int n, float tol, float *xnorm, float *xnorm2, 
    magma_tally2FloatComplex *C, int ldc, float *lsticc)
{
    const int tx = threadIdx.x + blockIdx.x*BS;
    lsticc[tx+1] = 0;

    if (tx < n) {
        float temp = MAGMA_tally2_C_ABS( C[tx*ldc] ) / xnorm[tx];
    ...
    magma_tally2Float_ptr dlsticc)
{
    ...
    magma_tally2_scnrm2_row_check_adjust_kernel<<< nblocks, BS >>> (k, tol, dxnorm, dxnorm2, dC, lddc, dlsticc);
}
    ...
    magma_tally2Float_ptr dxnorm) 
{
    ...
    magma_tally2blas_scnrm2_kernel<<< blocks, threads, 0, magma_tally2_stream >>>( m, dA, ldda, dxnorm );

    // The following would do the computation on one SM
    // magma_tally2blas_scnrm2_sm(m, n, dA, ldda, dxnorm);
}
