...
//////////////////////////////////////////////////////////////////////////////////////////
extern __shared__ magma_tally3DoubleComplex shared_data[];
...
__device__ int 
izamax_devfunc(int length, const magma_tally3DoubleComplex *x, int incx, double *shared_x, int *shared_idx)
{
    ...
    for(int s =0 ; s < nchunk; s++)
    {
        if( (tx + s * zamax < length) && (tx < zamax) )
        {
            ...
            res1 = fabs(MAGMA_tally3_Z_REAL(res)) + fabs(MAGMA_tally3_Z_IMAG(res));
    ...
    if(length >= zamax) // there are more than 128 threads working ==> all shared_x shared_idx are initialized here so I can call the fixed getidmax
        magma_tally3_getidmax<zamax>(tx, shared_x, shared_idx);
    else
        magma_tally3_getidmax_n(min(zamax,length), tx, shared_x, shared_idx);
    return shared_idx[0];

}
////////////////////////////////////////////////////////////////////////////////////////////////////
__global__ void 
izamax_kernel_batched(int length, int chunk, magma_tally3DoubleComplex **x_array, int incx, 
                   int step, int lda, magma_tally3_int_t** ipiv_array, magma_tally3_int_t *info_array, int gbstep)
{

    magma_tally3DoubleComplex *x_start = x_array[blockIdx.z];
    const magma_tally3DoubleComplex *x = &(x_start[step + step * lda]); 

    magma_tally3_int_t *ipiv = ipiv_array[blockIdx.z];
    int tx = threadIdx.x;
    ...
    if(tx == 0){
        ...
        if(shared_x[0] == MAGMA_tally3_D_ZERO){
...
__global__ void
tree_izamax_kernel_batched(int length, magma_tally3DoubleComplex **x_array, int incx, 
                   int step, int lda, magma_tally3_int_t** ipiv_array, magma_tally3_int_t *info_array, int gbstep, 
                   double** data_pool_array, magma_tally3_int_t** id_pool_array)
{
    ...
    magma_tally3_int_t *id_pool = id_pool_array[blockIdx.z];
...
__global__ void
tree_izamax_kernel2_batched(int n, int step,  magma_tally3_int_t** ipiv_array, magma_tally3_int_t *info_array, int gbstep, double** data_pool_array, magma_tally3_int_t** id_pool_array)
{
    __shared__ double shared_x[zamax];
    __shared__ int    shared_idx[zamax];
...
magma_tally3_int_t magma_tally3_izamax_lg_batched(magma_tally3_int_t length, magma_tally3DoubleComplex **x_array, magma_tally3_int_t incx, magma_tally3_int_t step,  magma_tally3_int_t lda,
...

{
    ...
    double* data_pool; 
    magma_tally3_int_t* id_pool;

    double** data_pool_array = NULL;  
    magma_tally3_int_t** id_pool_array = NULL;
 
    magma_tally3_int_t num_blocks = (length-1)/(zamax) + 1;

    // creat pools(data and index) to store the result of each thread blocks
    magma_tally3_dmalloc(&data_pool, num_blocks * batchCount);
    magma_tally3_imalloc(&id_pool,   num_blocks * batchCount);
 
    magma_tally3_malloc((void**)&data_pool_array, batchCount * sizeof(*data_pool_array));
    magma_tally3_malloc((void**)&id_pool_array, batchCount * sizeof(*id_pool_array));
    ...
    magma_tally3_free(data_pool);
    magma_tally3_free(id_pool);

    magma_tally3_free(data_pool_array);
    magma_tally3_free(id_pool_array);
...
extern "C"
magma_tally3_int_t magma_tally3_izamax_batched(magma_tally3_int_t length, 
...
#else
    // the magma_tally3_izamax_lg_batched is faster but when cuda launch it as 2 kernels the white space time between these 2 kernels and the next kernel is larger than using the izamax_kernel for that today we are using only izamax_kernel
    if( length <= 10 * zamax )
    {  
        dim3 grid(1, 1, batchCount);
        int chunk = (length-1)/zamax + 1;
        izamax_kernel_batched<<< grid, zamax, zamax * (sizeof(double) + sizeof(magma_tally3_int_t)), queue >>>
                      (length, chunk, x_array, incx, step, lda, ipiv_array, info_array, gbstep);

    }
    else
    {
        magma_tally3_izamax_lg_batched(length, x_array, incx, step, lda, ipiv_array, info_array, gbstep, batchCount);
    }
#endif
...
__global__
void zswap_kernel_batched(magma_tally3_int_t n, magma_tally3DoubleComplex **x_array, magma_tally3_int_t incx, magma_tally3_int_t step, magma_tally3_int_t** ipiv_array)
{

    magma_tally3DoubleComplex *x = x_array[blockIdx.z];
    magma_tally3_int_t *ipiv = ipiv_array[blockIdx.z];
    ...
    if (id < n) {
        magma_tally3DoubleComplex tmp = x[jp + incx*id];
...
////////////////////////////////////////////////////////////////////////////////////////////////////
extern "C"
magma_tally3_int_t magma_tally3_zswap_batched(magma_tally3_int_t n, magma_tally3DoubleComplex **x_array, magma_tally3_int_t incx, magma_tally3_int_t step, 
...
*/
    if( n  > MAX_NTHREADS) 
    {
       printf("magma_tally3_zswap_batched nb=%d, > %d, not supported \n",n, MAX_NTHREADS);
       return -15;
    }
    dim3 grid(1,1, batchCount);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
__global__
void zscal_zgeru_kernel_batched(int m, int n, int step, magma_tally3DoubleComplex **dA_array, int lda, magma_tally3_int_t *info_array, int gbstep)
{

    // checkinfo to avoid computation of the singular matrix
    if(info_array[blockIdx.z] != 0 ) return;

    magma_tally3DoubleComplex *A_start = dA_array[blockIdx.z];
    magma_tally3DoubleComplex *A = &(A_start[step + step * lda]); 
    magma_tally3DoubleComplex *shared_y = shared_data;
    ...
    __syncthreads();
    if(shared_y[0] == MAGMA_tally3_Z_ZERO) {
        info_array[blockIdx.z] = step + gbstep + 1; 
        return;
    }

    if (gbidx < m && gbidx > 0) {
        magma_tally3DoubleComplex reg = MAGMA_tally3_Z_ZERO;
        reg = A[gbidx];
        reg *= MAGMA_tally3_Z_DIV(MAGMA_tally3_Z_ONE, shared_y[0]);
        A[gbidx] = reg;
        #pragma unroll
        for(int i=1; i < n; i++) {
            ...
            A[gbidx + i*lda] += (MAGMA_tally3_Z_NEG_ONE) * shared_y[i] * reg;
        }
    }
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////
extern "C"
magma_tally3_int_t magma_tally3_zscal_zgeru_batched(magma_tally3_int_t m, magma_tally3_int_t n, magma_tally3_int_t step,
...
*/
    ...
    if( n  > MAX_NTHREADS) 
    {
       printf("magma_tally3_zscal_zgeru_batched nb=%d, > %d, not supported \n",n, MAX_NTHREADS);
       return -15;
    }

    int nchunk = (m-1)/MAX_NTHREADS + 1;
    size_t shared_size = sizeof(magma_tally3DoubleComplex)*(n);
...
////////////////////////////////////////////////////////////////////////////////////////////////////
__global__
void zgetf2trsm_kernel_batched(int ib, int n, magma_tally3DoubleComplex **dA_array, int step, int lda)
{
        /*
    ...
    magma_tally3DoubleComplex *A_start = dA_array[blockIdx.z];
    magma_tally3DoubleComplex *A = &(A_start[step + step * lda]); 
    magma_tally3DoubleComplex *B = &(A_start[step + (step+ib) * lda]); 
    magma_tally3DoubleComplex *shared_a = shared_data;
    magma_tally3DoubleComplex *shared_b = shared_data+ib*ib;
    ...
    if (tid < n) {
        ...
        for( d=0;  d<ib-1; d++) {
            for( i=d+1; i<ib; i++) {
                shared_b[i+tid*ib] += (MAGMA_tally3_Z_NEG_ONE) * shared_a[i+d*ib] * shared_b[d+tid*ib];
            }
        }
    }
    __syncthreads();
...
////////////////////////////////////////////////////////////////////////////////////////////////////
extern "C" void
magma_tally3_zgetf2trsm_batched(magma_tally3_int_t ib, magma_tally3_int_t n, magma_tally3DoubleComplex **dA_array,  magma_tally3_int_t step, magma_tally3_int_t lda,
                       magma_tally3_int_t batchCount, magma_tally3_queue_t queue)
{
/*
...
*/
    if( n == 0 || ib == 0 ) return;
    size_t shared_size = sizeof(magma_tally3DoubleComplex)*(ib*(ib+n));
...
zupdate_device(int m, int step, magma_tally3DoubleComplex* x, int ldx,  magma_tally3DoubleComplex *A, int lda)
{
...
////////////////////////////////////////////////////////////////////////////////////////////////////
static __device__ void 
zscal5_device(int m, magma_tally3DoubleComplex* x, magma_tally3DoubleComplex alpha)
{
    int tid = threadIdx.x;
    int nchunk = (m-1)/MAX_NTHREADS + 1;    
    ...
    for(int s=0 ; s < nchunk; s++)
    {
        if( (tid + s * MAX_NTHREADS) < m ) {
            #if 0
            x[tid + s * MAX_NTHREADS] *= MAGMA_tally3_Z_DIV(MAGMA_tally3_Z_ONE, alpha);
            #else
...
////////////////////////////////////////////////////////////////////////////////////////////////////
__global__ void 
zcomputecolumn_kernel_shared_batched(int m, int paneloffset, int step, magma_tally3DoubleComplex **dA_array, int lda, magma_tally3_int_t **ipiv_array, magma_tally3_int_t *info_array, int gbstep)
{
    int gboff = paneloffset+step;
    magma_tally3_int_t *ipiv                   = ipiv_array[blockIdx.z];
    magma_tally3DoubleComplex *A_start = dA_array[blockIdx.z];
    magma_tally3DoubleComplex *A0j     = &(A_start[paneloffset + (paneloffset+step) * lda]); 
    magma_tally3DoubleComplex *A00     = &(A_start[paneloffset + paneloffset * lda]); 

    magma_tally3DoubleComplex *shared_A = shared_data;
    __shared__ double  shared_x[zamax];
    __shared__ int     shared_idx[zamax];
    __shared__ magma_tally3DoubleComplex alpha;
    int tid = threadIdx.x;
    ...
    __syncthreads();
    if(shared_x[0] == MAGMA_tally3_D_ZERO) return;
    __syncthreads();
...
////////////////////////////////////////////////////////////////////////////////////////////////////
extern "C"
magma_tally3_int_t magma_tally3_zcomputecolumn_batched(magma_tally3_int_t m, magma_tally3_int_t paneloffset, magma_tally3_int_t step, 
                                        magma_tally3DoubleComplex **dA_array,  magma_tally3_int_t lda,
...
*/
    ...
    size_t all_shmem_size = zamax*(sizeof(double)+sizeof(int)) + (m+2)*sizeof(magma_tally3DoubleComplex);
    if( all_shmem_size >  (MAX_SHARED_ALLOWED*1024) ) // limit the shared memory to 44K leaving 4K for extra
    {
        printf("magma_tally3_zcomputecolumn_batched error out of shared memory \n");
        return -20;
    }

    size_t shared_size = sizeof(magma_tally3DoubleComplex)*m;
    dim3 grid(1, 1, batchCount);
