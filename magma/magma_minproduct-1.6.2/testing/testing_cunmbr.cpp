...
int main( int argc, char** argv )
{
    ...
    magma_minproductFloatComplex *C, *R, *A, *work, *tau, *tauq, *taup;
    float *d, *e;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int itran = 0; itran < 2; ++itran ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            n = opts.nsize[itest];
            k = opts.ksize[itest];
            nb  = magma_minproduct_get_cgebrd_nb( m );
            ldc = m;
            ...
            TESTING_MALLOC_CPU( A,    magma_minproductFloatComplex, lda*nn );
            TESTING_MALLOC_CPU( work, magma_minproductFloatComplex, lwork_max );
            TESTING_MALLOC_CPU( d,    float,             min(mm,nn) );
            TESTING_MALLOC_CPU( e,    float,             min(mm,nn) );
            TESTING_MALLOC_CPU( tauq, magma_minproductFloatComplex, min(mm,nn) );
            TESTING_MALLOC_CPU( taup, magma_minproductFloatComplex, min(mm,nn) );
            ...
            magma_minproduct_cgebrd( mm, nn, A, lda, d, e, tauq, taup, work, lwork_max, &info );
            if (info != 0)
            ...
            lwork = -1;
            magma_minproduct_cunmbr( vect[ivect], side[iside], trans[itran],
                          m, n, k,
                          A, lda, tau, R, ldc, work, lwork, &info );
            if (info != 0)
                printf("magma_minproduct_cunmbr (lwork query) returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
            lwork = (magma_minproduct_int_t) MAGMA_minproduct_C_REAL( work[0] );
            ...
            if (info != 0)
                printf("magma_minproduct_cunmbr returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
