...
int main( int argc, char** argv )
{
    ...
    magma_tally3DoubleComplex *C, *R, *A, *work, *tau, *tauq, *taup;
    double *d, *e;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int itran = 0; itran < 2; ++itran ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            n = opts.nsize[itest];
            k = opts.ksize[itest];
            nb  = magma_tally3_get_zgebrd_nb( m );
            ldc = m;
            ...
            TESTING_MALLOC_CPU( A,    magma_tally3DoubleComplex, lda*nn );
            TESTING_MALLOC_CPU( work, magma_tally3DoubleComplex, lwork_max );
            TESTING_MALLOC_CPU( d,    double,             min(mm,nn) );
            TESTING_MALLOC_CPU( e,    double,             min(mm,nn) );
            TESTING_MALLOC_CPU( tauq, magma_tally3DoubleComplex, min(mm,nn) );
            TESTING_MALLOC_CPU( taup, magma_tally3DoubleComplex, min(mm,nn) );
            ...
            magma_tally3_zgebrd( mm, nn, A, lda, d, e, tauq, taup, work, lwork_max, &info );
            if (info != 0)
            ...
            lapackf77_zunmbr( lapack_vect_const_tally3( vect[ivect] ),
            ...
            lwork = -1;
            magma_tally3_zunmbr( vect[ivect], side[iside], trans[itran],
                          m, n, k,
                          A, lda, tau, R, ldc, work, lwork, &info );
            if (info != 0)
                printf("magma_tally3_zunmbr (lwork query) returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
            ...
            if (info != 0)
                printf("magma_tally3_zunmbr returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
