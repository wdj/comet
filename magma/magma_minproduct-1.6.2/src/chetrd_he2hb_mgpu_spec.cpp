/*
    -- MAGMA_minproduct (version 1.6.1) --
       ...
       @generated from zhetrd_he2hb_mgpu_spec.cpp normal z -> c, Fri Jan 30 19:00:18 2015
...
/**
    ...
    A       COMPLEX array, dimension (LDA,N)
            ...
            On exit, if UPLO = Magma_minproductUpper, the Upper band-diagonal of A is
            ...
            = Magma_minproductLower, the the Lower band-diagonal of A is overwritten by
    ...
    magma_minproduct_int_t *info)
{
    ...
    magma_minproduct_int_t pm, pn, indi, indj, pk;
    magma_minproduct_int_t pm_old=0, pn_old=0, indi_old=0, flipV=-1;
    magma_minproduct_int_t iblock, idev, di;
    int i;
    int lwkopt;
    int lquery;
    ...
    *info = 0;
    ...
    // limit to 16 threads
    magma_minproduct_int_t orig_threads = magma_minproduct_get_lapack_numthreads();
    magma_minproduct_set_lapack_numthreads( min(orig_threads,16) );

    magma_minproduct_int_t gnode[Magma_minproductMaxGPUs][Magma_minproductMaxGPUs+2];
    magma_minproduct_int_t nbcmplx=0;
    magma_minproduct_buildconnection_mgpu(gnode, &nbcmplx,  ngpu);
    #ifdef ENABLE_DEBUG
    printf(" Initializing communication pattern.... GPU-ncmplx %d\n\n", nbcmplx);
    #endif

    magma_minproductFloatComplex *dspace[Magma_minproductMaxGPUs];
    magma_minproductFloatComplex *dwork[Magma_minproductMaxGPUs], *dworkbis[Magma_minproductMaxGPUs];
    magma_minproductFloatComplex *dvall[Magma_minproductMaxGPUs], *dv[Magma_minproductMaxGPUs], *dw[Magma_minproductMaxGPUs];
    magma_minproductFloatComplex *workngpu[Magma_minproductMaxGPUs+1];
    magma_minproduct_event_t     redevents[Magma_minproductMaxGPUs][Magma_minproductMaxGPUs*Magma_minproductMaxGPUs+10];
    magma_minproduct_int_t nbevents = Magma_minproductMaxGPUs*Magma_minproductMaxGPUs;

    magma_minproduct_int_t lddv        = ldda;
    magma_minproduct_int_t lddw        = lddv;
    magma_minproduct_int_t dwrk2siz    = ldda*nb*3;
    magma_minproduct_int_t worksiz     = n*nb;
    magma_minproduct_int_t devworksiz  = 2*nb*lddv + nb*lddw + nb*ldda + dwrk2siz; // 2*dv(dv0+dv1) + dw + dwork +dworkbis

    // local allocation and stream creation
    // TODO check malloc
    for( magma_minproduct_int_t dev = 0; dev < ngpu; ++dev ) {
        magma_minproduct_setdevice( dev );
        magma_minproduct_cmalloc( &dspace[dev], devworksiz );
        magma_minproduct_cmalloc_pinned ( &workngpu[dev], worksiz);
    ...
    magma_minproduct_cmalloc_pinned ( &workngpu[ngpu], worksiz);
    magma_minproductFloatComplex *worktest = NULL;
    //magma_minproduct_cmalloc_cpu( &worktest, n*nb ); // not used
    // ======================
  

    magma_minproductFloatComplex *hT = work + lwork - nb*nb;
    lwork -= nb*nb;
    memset( hT, 0, nb*nb*sizeof(magma_minproductFloatComplex));
    ...
    if (upper) {
        ...
        for (i = 1; i <= n-nb; i += nb) {
            ...
            if (i > 1 ) {
                ...
                cpanel_to_q(Magma_minproductUpper, pn-1, A(i, i+1), lda, work);
                ...
                magma_minproduct_cgetmatrix_async( (pm+pn), pn,
                                        dA(idev, i, di+1), ldda,
                ...
                magma_minproductblas_cher2k_mgpu_spec(
                    Magma_minproductLower, Magma_minproductNoTrans, pm_old-pn_old, pn_old,
                ...
                magma_minproduct_setdevice( idev );
                magma_minproduct_queue_sync( queues[idev][ nqueue-1 ] );
                //magma_minproduct_setdevice( 0 );
                cq_to_panel(Magma_minproductUpper, pn-1, A(i, i+1), lda, work);
            }
            ...
            cpanel_to_q(Magma_minproductUpper, pk, A(indi, indj), lda, work);
            ...
            for( magma_minproduct_int_t dev = 0; dev < ngpu; ++dev ) {
                magma_minproduct_setdevice( dev );
                // send V
                magma_minproduct_csetmatrix_async( pm, pk,
                                        A(indi, indj), lda,
                                        dv[dev], pm, queues[dev][nqueue-1] );

                // Send the triangular factor T to the GPU
                magma_minproduct_csetmatrix_async( pk, pk,
                                        hT, nb,
            ...
            for( magma_minproduct_int_t dev = 0; dev < ngpu; ++dev ) {
                // dwork = V T
                magma_minproduct_setdevice( dev );
                magma_minproductblasSetKernelStream( queues[ dev ][ nqueue-1 ] );
                magma_minproduct_queue_sync( queues[dev][nqueue-1] );
                magma_minproduct_cgemm(Magma_minproductNoTrans, Magma_minproductNoTrans, pm, pk, pk,
                        c_one, dv[dev], pm,
                        dT(dev, 1, i), lddt,
                        c_zero, dwork[dev], pm);
            }
            ...
            for( magma_minproduct_int_t dev = 0; dev < ngpu; ++dev ) {
                magma_minproduct_setdevice( dev );
                for( magma_minproduct_int_t s = 0; s < nqueue; ++s )
                magma_minproduct_queue_sync( queues[dev][s] );
            }
            ...
            // Broadcasting stream.
            if (ngpu == 1) {
                magma_minproductblasSetKernelStream( queues[ 0 ][ 0 ] );
                magma_minproduct_chemm(
                    Magma_minproductLeft, uplo, pm, pk,
            ...
            for( magma_minproduct_int_t dev = 0; dev < ngpu; ++dev ) {
                ...
                magma_minproduct_setdevice( dev );
                magma_minproductblasSetKernelStream( queues[ dev ][ dev ] );
                // magma_minproduct_queue_sync( queues[dev][0] );
                magma_minproduct_cgemm(Magma_minproductConjTrans, Magma_minproductNoTrans, pk, pk, pm,
                            c_one, dwork[dev], pm,
                            dw[dev], pm,
            ...
            cq_to_panel(Magma_minproductUpper, pk, A(indi, indj), lda, work);
            // ===============================================
            ...
            for( magma_minproduct_int_t dev = 0; dev < ngpu; ++dev ) {
                magma_minproduct_setdevice( dev );
                magma_minproduct_queue_sync( queues[dev][dev] );
            }
            ...
            if (i + nb <= n-nb) {
                ...
                magma_minproduct_setdevice( idev );
                magma_minproductblasSetKernelStream( queues[ idev ][ nqueue-1 ] );
                //magma_minproduct_queue_sync( queues[idev][0] ); removed because the sync has been done in the loop above
                magma_minproduct_cgemm(Magma_minproductNoTrans, Magma_minproductConjTrans, pm, pn, pn, c_neg_one,
                            dv[idev], pm,
                            dw[idev], pm, c_one,
                ...
                idev   = ((indi-1) / distblk) % ngpu;          // device with this block
                magma_minproduct_setdevice( idev );
                magma_minproductblasSetKernelStream( queues[ idev ][ 0 ] );
                //printf("LAST CHER2K idev %d on A(%d,%d) of size %d \n",idev, indi-1,di,pk);
                magma_minproduct_cher2k(Magma_minproductLower, Magma_minproductNoTrans, pk, pk, c_neg_one,
                             dv[idev], pm,
                             dw[idev], pm, d_one,
                             dA(idev, indi, di+1), ldda);
                ...
                cpanel_to_q(Magma_minproductUpper, pk-1, A(n-pk+1, n-pk+2), lda, work);
                magma_minproduct_cgetmatrix( pk, pk,
                                  dA(idev, indi, di+1), ldda,
                                  A(n-pk+1, n-pk+1),  lda );
                cq_to_panel(Magma_minproductUpper, pk-1, A(n-pk+1, n-pk+2), lda, work);
            }
    ...
    for( magma_minproduct_int_t dev = 0; dev < ngpu; ++dev ) {
        magma_minproduct_setdevice( dev );
        magma_minproduct_free( dspace[dev]);
        magma_minproduct_free_pinned(workngpu[dev]);
        for( magma_minproduct_int_t e = 0; e < nbevents; ++e ) {
            magma_minproduct_event_destroy( redevents[dev][e] );
        }
    }
    magma_minproduct_free_pinned(workngpu[ngpu]);
    magma_minproduct_free_cpu(worktest);

    magma_minproduct_setdevice( orig_dev );
    magma_minproductblasSetKernelStream( orig_stream );
    magma_minproduct_set_lapack_numthreads( orig_threads );
