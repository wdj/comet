...
/**
    ...
    as returned by SGEQRF. Q is of order M if SIDE = Magma_tally4Left and of order N
    ...
    lwork   INTEGER
            ...
            LWORK >= (M-K+NB)*(N+NB) + N*NB if SIDE = Magma_tally4Left, and
            LWORK >= (N-K+NB)*(M+NB) + M*NB if SIDE = Magma_tally4Right,
    ...
    dT      REAL array on the GPU that is the output
            (the 9th argument) of magma_tally4_sgeqrf_gpu.

    @param[in]
    nb      INTEGER
            This is the blocking size that was used in pre-computing DT, e.g.,
            the blocking size used in magma_tally4_sgeqrf_gpu.
    ...
    magma_tally4_int_t *info)
{
    ...
    magma_tally4_int_t i1, i2, step, ib, ic, jc, ma, mi, ni, nq, nw;
    ...
    *info = 0;
    ...
    hwork[0] = MAGMA_tally4_S_MAKE( lwkopt, 0 );
    ...
    if ( step < 0 ) {
        ...
        magma_tally4_int_t lhwork = lwork - (ma*ib + mi*ni);
        
        magma_tally4_sgetmatrix( ma, ib, dA(i,  i ), ldda, hA, ma );
        magma_tally4_sgetmatrix( mi, ni, dC(ic, jc), lddc, hC, mi );
        ...
        magma_tally4_ssetmatrix( mi, ni, hC, mi, dC(ic, jc), lddc );
    }
    ...
    if (nb < k) {
        for( i=i1; (step < 0 ? i >= i2 : i < i2); i += step ) {
            ...
            magma_tally4_slarfb_gpu( side, trans, Magma_tally4Forward, Magma_tally4Columnwise,
                              mi, ni, ib,
                              dA(i,  i ), ldda, dT(i), nb,
    ...
    // TODO sync. For cases Q*C and C*Q^T, last call is magma_tally4_slarfb_gpu,
    // which is async magma_tally4_gemm calls, so sormqr can be unfinished.
