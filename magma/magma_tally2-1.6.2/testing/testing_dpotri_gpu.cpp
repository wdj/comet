...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_tally2_dpotrf_gpu( opts.uplo, N, d_A, ldda, &info );
            
            // check for exact singularity
            //magma_tally2_dgetmatrix( N, N, d_A, ldda, h_R, lda );
            //h_R[ 10 + 10*lda ] = MAGMA_tally2_D_MAKE( 0.0, 0.0 );
            //magma_tally2_dsetmatrix( N, N, h_R, lda, d_A, ldda );
            
            gpu_time = magma_tally2_wtime();
            magma_tally2_dpotri_gpu( opts.uplo, N, d_A, ldda, &info );
            gpu_time = magma_tally2_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally2_dpotri_gpu returned error %d: %s.\n",
