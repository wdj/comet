...
int main( int argc, char** argv)
{
    ...
    magma_tally4DoubleComplex *h_A, *h_R, *h_Q, *h_work, *tau, *twork, *T, *dT;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_tally4_zgehrd_m( N, ione, N, h_R, lda, tau, h_work, lwork, T, &info);
            gpu_time = magma_tally4_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally4_zgehrd_m returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                TESTING_MALLOC_CPU( twork, magma_tally4DoubleComplex, ltwork );
                #if defined(PRECISION_z) || defined(PRECISION_c)
                TESTING_MALLOC_CPU( rwork, double, N );
                #endif
                
                lapackf77_zlacpy(Magma_tally4UpperLowerStr, &N, &N, h_R, &lda, h_Q, &lda);
                for( int j = 0; j < N-1; ++j )
                    for( int i = j+2; i < N; ++i )
                        h_R[i+j*lda] = MAGMA_tally4_Z_ZERO;
                
                magma_tally4_zsetmatrix( nb, N, T, nb, dT, nb );
                
                magma_tally4_zunghr(N, ione, N, h_Q, lda, tau, dT, nb, &info);
                if ( info != 0 ) {
                    printf("magma_tally4_zunghr returned error %d: %s.\n",
                           (int) info, magma_tally4_strerror( info ));
                    exit(1);
                }
                #if defined(PRECISION_z) || defined(PRECISION_c)
