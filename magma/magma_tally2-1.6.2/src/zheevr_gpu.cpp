...
magma_tally2_zheevr_gpu(
    ...
    double *w,
    magma_tally2DoubleComplex_ptr dZ, magma_tally2_int_t lddz, magma_tally2_int_t *isuppz,
    ...
    magma_tally2_int_t *info)
{
    ...
    *m = 0;
    ...
    if (n <= 128) {
        ...
        magma_tally2_zmalloc_cpu( &a, n*n );
        magma_tally2_zgetmatrix(n, n, dA, ldda, a, n);
        lapackf77_zheevr(jobz_, range_, uplo_,
                         &n, a, &n, &vl, &vu, &il, &iu, &abstol, m,
                         w, wZ, &ldwz, isuppz, work, &lwork,
                         rwork, &lrwork, iwork, &liwork, info);
        magma_tally2_zsetmatrix( n,  n,  a,    n, dA, ldda);
        magma_tally2_zsetmatrix( n, *m, wZ, ldwz, dZ, lddz);
        magma_tally2_free_cpu(a);
        return *info;
    }

    if (MAGMA_tally2_SUCCESS != magma_tally2_dmalloc( &dwork, n )) {
        fprintf (stderr, "!!!! device memory allocation error (magma_tally2_zheevr_gpu)\n");
    ...
    anrm = magma_tally2blas_zlanhe(Magma_tally2MaxNorm, uplo, n, dA, ldda, dwork);
    iscale = 0;
    sigma  = 1;
    if (anrm > 0. && anrm < rmin) {
        iscale = 1;
    ...
    if (iscale == 1) {
        d__1 = 1.;
        magma_tally2blas_zlascl(uplo, 0, 0, 1., sigma, n, n, dA,
                         ldda, info);
...
#ifdef FAST_HEMV
    magma_tally2_zhetrd2_gpu(uplo, n, dA, ldda, &rwork[indrd], &rwork[indre],
                      &work[indtau], wA, ldwa, &work[indwk], llwork, dZ, lddz*n, &iinfo);
#else
    magma_tally2_zhetrd_gpu (uplo, n, dA, ldda, &rwork[indrd], &rwork[indre],
                      &work[indtau], wA, ldwa, &work[indwk], llwork, &iinfo);
#endif

    lopt = n + (magma_tally2_int_t)MAGMA_tally2_Z_REAL(work[indwk]);
    ...
    if (! wantz) {
        ...
        if (*info == 0 && wantz) {
            magma_tally2_zsetmatrix( n, *m, wZ, ldwz, dZ, lddz );
            magma_tally2_zunmtr_gpu(Magma_tally2Left, uplo, Magma_tally2NoTrans, n, *m, dA, ldda, &work[indtau],
    ...
    if (wantz) {
        for (j = 1; j <= *m-1; ++j) {
            ...
            if (i != 0) {
                ...
                magma_tally2_zswap(n, dZ + (i-1)*lddz, ione, dZ + (j-1)*lddz, ione);
            }
        }
    }
    
    /* Set WORK[0] to optimal complex workspace size. */
    work[1] = MAGMA_tally2_Z_MAKE( lopt, 0 );
