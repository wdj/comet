/*
    -- MAGMA_minproduct (version 1.6.1) --
       ...
       @generated from zlahru.cpp normal z -> d, Fri Jan 30 19:00:18 2015
       @author Stan Tomov
       @author Mark Gates
*/
    ...
    magma_minproductDouble_ptr dwork )
{
    ...
    magma_minproduct_dgemm( Magma_minproductNoTrans, Magma_minproductNoTrans, k, nb, ihi-k,
                 c_one,  dA,  ldda,
                         dV,  lddv,
                 c_zero, dYm, ldda );
    ...
    magma_minproduct_dgemm( Magma_minproductNoTrans, Magma_minproductConjTrans, k, ihi-k, nb,
                 c_neg_one, dYm,   ldda,
                            dwork, ldda,
                 c_one,     dA,    ldda );
    
    // copy first nb columns of Am, A(0:k-1, 0:nb-1), to host
    magma_minproduct_dgetmatrix( k, nb, dA, ldda, A, lda );

    // -----
    // on right, A := A Q = A - A V T V'
    // Update Ag = Ag - Ag V T V' = Ag - Y W'
    // Ag = Ag - Y W' = A(k:ihi-1, nb:ihi-k-1) - Y(0:ihi-k-1, 0:nb-1) * W(nb:ihi-k-1, 0:nb-1)'
    magma_minproduct_dgemm( Magma_minproductNoTrans, Magma_minproductConjTrans, ihi-k, ihi-k-nb, nb,
                 c_neg_one, dY,         ldda,
                            dwork + nb, ldda,
                 c_one,     dA(k,nb),   ldda );
    ...
    magma_minproduct_dgemm( Magma_minproductConjTrans, Magma_minproductNoTrans, nb, n-k-nb, ihi-k,
                 c_one,  dV,       lddv,
                         dA(k,nb), ldda,
                 c_zero, dY,       nb );
    
    // Ag2 = Ag2 - W Z = A(k:ihi-1, nb:n-k-1) - W(nb:n-k-1, 0:nb-1) * Z(0:nb-1, nb:n-k-1)
    magma_minproduct_dgemm( Magma_minproductNoTrans, Magma_minproductNoTrans, ihi-k, n-k-nb, nb,
