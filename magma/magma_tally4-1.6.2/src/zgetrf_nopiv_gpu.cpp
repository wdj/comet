...
#define dA(i,j) (dA + (i)*nb + (j)*nb*ldda)
    ...
    magma_tally4_int_t i, rows, s, lddwork;
    ...
    *info = 0;
    ...
    if (nb <= 1 || nb >= min(m,n)) {
        ...
        magma_tally4_zgetrf_nopiv( m, n, work, m, info);
        ...
        if (MAGMA_tally4_SUCCESS != magma_tally4_zmalloc_pinned( &work, maxm*nb )) {
            *info = MAGMA_tally4_ERR_HOST_ALLOC;
            return *info;
        }
        ...
        for( i=0; i < s; i++ ) {
            ...
            magma_tally4_zgetmatrix_async( m-i*nb, nb, dA(i,i), ldda, work, lddwork, stream[0] );
            
            if ( i > 0 ) {
                magma_tally4_ztrsm( Magma_tally4Left, Magma_tally4Lower, Magma_tally4NoTrans, Magma_tally4Unit,
                             nb, n - (i+1)*nb,
            ...
            magma_tally4_zgetrf_nopiv( rows, nb, work, lddwork, &iinfo );
            if ( (*info == 0) && (iinfo > 0) )
                *info = iinfo + i*nb;

            // upload i-th panel
            magma_tally4_zsetmatrix_async( m-i*nb, nb, work, lddwork, dA(i, i), ldda, stream[0] );
            magma_tally4_queue_sync( stream[0] );
        ...
        rows = m - s*nb;
        magma_tally4_zgetmatrix( rows, nb0, dA(s,s), ldda, work, lddwork );

        // make sure that gpu queue is empty
        magma_tally4_device_sync();

        // do the cpu part
        magma_tally4_zgetrf_nopiv( rows, nb0, work, lddwork, &iinfo );
        if ( (*info == 0) && (iinfo > 0) )
            *info = iinfo + s*nb;

        // upload i-th panel
        magma_tally4_zsetmatrix( rows, nb0, work, lddwork, dA(s,s), ldda );
