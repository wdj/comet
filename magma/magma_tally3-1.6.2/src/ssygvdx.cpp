...
/**
    ...
    B       REAL array, dimension (LDB, N)
            On entry, the symmetric matrix B.  If UPLO = Magma_tally3Upper, the
    ...
    @ingroup magma_tally3_ssygv_driver
    ********************************************************************/
    ...
    magma_tally3_int_t *info)
{
    ...
    float d_one = MAGMA_tally3_S_ONE;
    ...
    magma_tally3_int_t wantz, lquery;
    ...
    *info = 0;
    ...
    if (MAGMA_tally3_SUCCESS != magma_tally3_smalloc( &dA, n*ldda ) ||
        MAGMA_tally3_SUCCESS != magma_tally3_smalloc( &dB, n*lddb )) {
    ...
    magma_tally3_ssetmatrix( n, n, B, ldb, dB, lddb );
    magma_tally3_ssetmatrix_async( n, n,
                            A,  lda,
                            dA, ldda, stream );
    ...
    magma_tally3_spotrf_gpu( uplo, n, dB, lddb, info );
    if (*info != 0) {
        *info = n + *info;
        return *info;
    }
    ...
    magma_tally3_sgetmatrix_async( n, n,
                            dB, lddb,
                            B,  ldb, stream );
    ...
    magma_tally3_ssygst_gpu( itype, uplo, n, dA, ldda, dB, lddb, info );
    ...
    if (n > 5000) {
        magma_tally3_queue_sync( stream );
        magma_tally3_free( dB );  dB=NULL;
    }

    timer_start( time );
    magma_tally3_ssyevdx_gpu( jobz, range, uplo, n, dA, ldda, vl, vu, il, iu, mout, w, A, lda,
    ...
    if (wantz && *info == 0) {
        ...
        if (dB == NULL) {
            if (MAGMA_tally3_SUCCESS != magma_tally3_smalloc( &dB, n*lddb ) ) {
                magma_tally3_free( dA );  dA=NULL;
                *info = MAGMA_tally3_ERR_DEVICE_ALLOC;
                return *info;
            }
            magma_tally3_ssetmatrix( n, n, B, ldb, dB, lddb );
        ...
        if (itype == 1 || itype == 2) {
            ...
            magma_tally3_strsm( Magma_tally3Left, uplo, trans, Magma_tally3NonUnit,
            ...
            magma_tally3_strmm( Magma_tally3Left, uplo, trans, Magma_tally3NonUnit,
                         n, *mout, d_one, dB, lddb, dA, ldda );
        }
        magma_tally3_sgetmatrix( n, *mout, dA, ldda, A, lda );
