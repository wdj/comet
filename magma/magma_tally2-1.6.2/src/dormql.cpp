...
/**
    ...
    as returned by DGEQLF. Q is of order M if SIDE = Magma_tally2Left and of order N
    ...
    magma_tally2_int_t *info)
{
    ...
    double *T, *T2;
    magma_tally2_int_t i, i1, i2, ib, nb, mi, ni, nq, nq_i, nw, step;
    ...
    *info  = 0;
    ...
    if (*info == 0) {
        nb = magma_tally2_get_dgelqf_nb( min( m, n ));
        lwkopt = max(1,nw)*nb;
    ...
    if ( nb >= k ) {
        /* Use CPU code */
        lapackf77_dormql( lapack_side_const_tally2(side), lapack_trans_const_tally2(trans),
        ...
        magma_tally2_dmalloc( &dwork, (nw + nq + nb)*nb + lddc*n );
        if ( dwork == NULL ) {
        ...
        magma_tally2_dmalloc_pinned( &T, 2*nb*nb );
        if ( T == NULL ) {
        ...
        magma_tally2_dsetmatrix( m, n, C, ldc, dC, lddc );
        ...
        for (i = i1; (step < 0 ? i >= i2 : i < i2); i += step) {
            ...
            /* 1) set lower triangle of panel in A to identity,
               ...
               3) restore A                                      */
            dpanel_to_q_tally2( Magma_tally2Lower, ib, A(nq_i-ib,i), lda, T2 );
            magma_tally2_dsetmatrix( nq_i,  ib, A(0,      i), lda, dV, nq_i );
            dq_to_panel_tally2( Magma_tally2Lower, ib, A(nq_i-ib,i), lda, T2 );
            ...
            magma_tally2_dlarfb_gpu( side, trans, Magma_tally2Backward, Magma_tally2Columnwise,
                              mi, ni, ib,
                              dV, nq_i,
                              dT, ib,
                              dC, lddc,
        ...
        magma_tally2_dgetmatrix( m, n, dC, lddc, C, ldc );

        magma_tally2_free( dwork );
        magma_tally2_free_pinned( T );
    }
    work[0] = MAGMA_tally2_D_MAKE( lwkopt, 0 );
