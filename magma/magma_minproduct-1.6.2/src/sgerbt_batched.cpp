        ...
        float* u, float* v)
{
    ...
    for (idx=0; idx<n; idx++){
        ...
        u[idx] = MAGMA_minproduct_S_MAKE(u1,u1);

        v[idx] = MAGMA_minproduct_S_MAKE(v1,v1);
    }
}
    ...
    magma_minproduct_int_t *info, magma_minproduct_int_t batchCount, magma_minproduct_queue_t queue)
{
    ...
    *info = 0;
    ...
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_smalloc( &du, 2*n )) {
    ...
    magma_minproduct_ssetvector( 2*n, U, 1, du, 1);
    magma_minproduct_ssetvector( 2*n, V, 1, dv, 1);

    /* Perform Partial Random Butterfly Transformation on the GPU*/
    magma_minproductblas_sprbt_batched(n, dA_array, ldda, du, dv, batchCount, queue);

    /* Compute U^T.b on the GPU*/

    // TODO fix for multiple RHS
    for(int i= 0; i < nrhs; i++)
        magma_minproductblas_sprbt_mtv_batched(n, du, dB_array, batchCount, queue);
