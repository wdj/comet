      ...
      for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
          for( int ival = 0; ival < 4; ++ival ) {
            ...
            // 2     1.23     0
            // 3     1.23     3.14
            offdiag = MAGMA_tally3_D_MAKE( 1.2345, 6.7890 ) * (ival / 2);
            diag    = MAGMA_tally3_D_MAKE( 3.1415, 2.7183 ) * (ival % 2);
            ...
            //magma_tally3blas_dlaset( uplo[iuplo], M-2, N-2, offdiag, diag, d_A+1+ldda, ldda );  // inset by 1 row & col
            magma_tally3blas_dlaset( uplo[iuplo], M, N, offdiag, diag, d_A, ldda );
            ...
            //lapackf77_dlaset( lapack_uplo_const_tally3( uplo[iuplo] ), &M2, &N2, &offdiag, &diag, h_A+1+lda, &lda );
            lapackf77_dlaset( lapack_uplo_const_tally3( uplo[iuplo] ), &M, &N, &offdiag, &diag, h_A, &lda );
