    ...
    magma_tally2_queue_t queue )
{
    ...
    magma_tally2DoubleComplex *tmp=NULL;
    ...
    if ( A->memory_location == Magma_tally2_CPU && A->storage_type == Magma_tally2_CSRCOO ) {
        if ( scaling == Magma_tally2_NOSCALE ) {
            ...
            CHECK( magma_tally2_zmalloc_cpu( &tmp, A->num_rows ));
            for( magma_tally2_int_t z=0; z<A->num_rows; z++ ) {
                magma_tally2DoubleComplex s = MAGMA_tally2_Z_MAKE( 0.0, 0.0 );
                for( magma_tally2_int_t f=A->row[z]; f<A->row[z+1]; f++ )
                    s+= MAGMA_tally2_Z_REAL(A->val[f])*MAGMA_tally2_Z_REAL(A->val[f]);
                tmp[z] = MAGMA_tally2_Z_MAKE( 1.0/sqrt(  MAGMA_tally2_Z_REAL( s )  ), 0.0 );
            }        printf("inhere1\n");
            ...
            for( magma_tally2_int_t z=0; z<A->num_rows; z++ ) {
                ...
                for( magma_tally2_int_t f=A->row[z]; f<A->row[z+1]; f++ ) {
                    if ( A->col[f]== z ) {
                        ...
                        //A->val[f] = A->val[f] +  MAGMA_tally2_Z_MAKE( 100000.0, 0.0 );
                        s = A->val[f];
                    }
                }
                if ( s == MAGMA_tally2_Z_MAKE( 0.0, 0.0 ) ){
        ...
        CHECK( magma_tally2_zmconvert( hA, &CSRA, hA.storage_type, Magma_tally2_CSRCOO, queue ));

        CHECK( magma_tally2_zmscale( &CSRA, scaling, queue ));

        magma_tally2_zmfree( &hA, queue );
        magma_tally2_zmfree( A, queue );
        CHECK( magma_tally2_zmconvert( CSRA, &hA, Magma_tally2_CSRCOO, A_storage, queue ));
    ...
    magma_tally2_queue_t queue )
{
    ...
    if ( A->memory_location == Magma_tally2_CPU && A->storage_type == Magma_tally2_CSRCOO ) {
        ...
        CHECK( magma_tally2_zmdiagadd( &CSRA, add, queue ));
