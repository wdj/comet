...
int main( int argc, char** argv)
{
    ...
    float          magma_tally4_error, magma_tally4_err, Ynorm, work[1];
    ...
    magma_tally4FloatComplex *h_A, *h_X, *h_Y, *h_Ymagma_tally4;
    magma_tally4FloatComplex *d_A, *d_X, *d_Y;
    ...
    magma_tally4FloatComplex **X_array = NULL;
    magma_tally4FloatComplex **Y_array = NULL;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_X,  magma_tally4FloatComplex, sizeX );
            TESTING_MALLOC_CPU( h_Y,  magma_tally4FloatComplex, sizeY  );
            TESTING_MALLOC_CPU( h_Ymagma_tally4,  magma_tally4FloatComplex, sizeY  );


            TESTING_MALLOC_DEV( d_A, magma_tally4FloatComplex, ldda*N*batchCount );
            TESTING_MALLOC_DEV( d_X, magma_tally4FloatComplex, sizeX );
            TESTING_MALLOC_DEV( d_Y, magma_tally4FloatComplex, sizeY );
            ...
            /* =====================================================================
               ...
               =================================================================== */
            magma_tally4_csetmatrix( M, N*batchCount, h_A, lda, d_A, ldda );
            magma_tally4_csetvector( Xm*batchCount, h_X, incx, d_X, incx );
            magma_tally4_csetvector( Ym*batchCount, h_Y, incy, d_Y, incy );
            
            cset_pointer(A_array, d_A, ldda, 0, 0, ldda*N, batchCount, magma_tally4_stream);
            cset_pointer(X_array, d_X, 1, 0, 0, incx*Xm, batchCount, magma_tally4_stream);
            cset_pointer(Y_array, d_Y, 1, 0, 0, incy*Ym, batchCount, magma_tally4_stream);

            magma_tally4_time = magma_tally4_sync_wtime( NULL );
            magma_tally4blas_cgemv_batched(opts.transA, M, N,
                             alpha, A_array, ldda,
                                    X_array, incx,
            ...
            magma_tally4_cgetvector( Ym*batchCount, d_Y, incy, h_Ymagma_tally4, incy );
            ...
            if ( opts.lapack ) {
                ...
                for(int s=0; s<batchCount; s++)
                {
                    ...
                    blasf77_caxpy( &Ym, &c_neg_one, h_Y + s*Ym, &ione, h_Ymagma_tally4 + s*Ym, &ione );
                    magma_tally4_err = lapackf77_clange( "M", &M, &ione, h_Ymagma_tally4 + s*Ym, &incy, work ) / Ynorm; 
