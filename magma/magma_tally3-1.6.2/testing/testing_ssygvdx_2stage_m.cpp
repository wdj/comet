...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            //magma_tally3_int_t sizvblg = magma_tally3_sbulge_get_lq2(N, threads);        
            ...
            if ( opts.warmup || opts.check ) {
                ...
                lapackf77_slacpy( Magma_tally3UpperLowerStr, &N, &N, h_A, &N, h_Ainit, &N );
                lapackf77_slacpy( Magma_tally3UpperLowerStr, &N, &N, h_B, &N, h_Binit, &N );
            }
            ...
            if ( opts.warmup ) {
                ...
                // ==================================================================
                magma_tally3_ssygvdx_2stage_m(opts.ngpu, opts.itype, opts.jobz, range, opts.uplo,
                                       ...
                                       iwork, liwork,
                                       &info);
                lapackf77_slacpy( Magma_tally3UpperLowerStr, &N, &N, h_Ainit, &N, h_A, &N );
                lapackf77_slacpy( Magma_tally3UpperLowerStr, &N, &N, h_Binit, &N, h_B, &N );
            }
            ...
            if ( opts.check && opts.jobz != Magma_tally3NoVec ) {
                ...
                #endif
                result = 1.;
                result /= lapackf77_slansy("1", lapack_uplo_const_tally3(opts.uplo), &N, h_Ainit, &N, rwork);
                result /= lapackf77_slange("1", &N , &m1, h_A, &N, rwork);

                if (opts.itype == 1) {
                    blasf77_ssymm("L", lapack_uplo_const_tally3(opts.uplo), &N, &m1, &c_one, h_Ainit, &N, h_A, &N, &c_zero, h_work, &N);
                    for(int i=0; i<m1; ++i)
                        blasf77_sscal(&N, &w1[i], &h_A[i*N], &ione);
                    blasf77_ssymm("L", lapack_uplo_const_tally3(opts.uplo), &N, &m1, &c_neg_one, h_Binit, &N, h_A, &N, &c_one, h_work, &N);
                    result *= lapackf77_slange("1", &N, &m1, h_work, &N, rwork)/N;
                }
                else if (opts.itype == 2) {
                    blasf77_ssymm("L", lapack_uplo_const_tally3(opts.uplo), &N, &m1, &c_one, h_Binit, &N, h_A, &N, &c_zero, h_work, &N);
                    for(int i=0; i<m1; ++i)
                        blasf77_sscal(&N, &w1[i], &h_A[i*N], &ione);
                    blasf77_ssymm("L", lapack_uplo_const_tally3(opts.uplo), &N, &m1, &c_one, h_Ainit, &N, h_work, &N, &c_neg_one, h_A, &N);
                    ...
                    for(int i=0; i<m1; ++i)
                        blasf77_sscal(&N, &w1[i], &h_A[i*N], &ione);
                    blasf77_ssymm("L", lapack_uplo_const_tally3(opts.uplo), &N, &m1, &c_one, h_Binit, &N, h_work, &N, &c_neg_one, h_A, &N);
                    result *= lapackf77_slange("1", &N, &m1, h_A, &N, rwork)/N;
                }
            }
