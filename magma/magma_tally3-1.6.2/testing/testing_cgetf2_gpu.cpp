...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            real_Double_t set_time = magma_tally3_wtime();
            magma_tally3_csetmatrix( M, N, h_R, lda, d_A, ldda );
            set_time =  magma_tally3_wtime() - set_time;
            ...
            magma_tally3_cgetf2_gpu( M, N, d_A, ldda, ipiv, &info);
            gpu_time = magma_tally3_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally3_cgetf2_gpu returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
            
            real_Double_t get_time = magma_tally3_wtime();
            magma_tally3_cgetmatrix( M, N, d_A, ldda, h_A, lda );
            get_time =  magma_tally3_wtime() - get_time;
