        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            switch( opts.svd_work ) {
                case 0: {  // query for workspace size
                    lwork = -1;
                    magma_minproduct_cgesdd( jobz, M, N,
            ...
            TESTING_MALLOC_CPU( VT,    magma_minproductFloatComplex, ldv*N );   // N x N (jobz=A) or min(M,N) x N
            TESTING_MALLOC_CPU( U,     magma_minproductFloatComplex, ldu*N_U ); // M x M (jobz=A) or M x min(M,N)
            ...
            if (info != 0)
                printf("magma_minproduct_cgesdd returned error %d: %s.\n",
