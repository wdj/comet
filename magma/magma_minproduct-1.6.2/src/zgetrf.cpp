    ...
    magma_minproduct_int_t *info)
{
    ...
    magma_minproductDoubleComplex *dAT, *dA, *da, *work;
    ...
    *info = 0;
    ...
    if ( (nb <= 1) || (nb >= min(m,n)) ) {
        ...
        if ( ngpu > 1 ) {
            ...
            magma_minproduct_zgetrf_m(ngpu, m, n, A, lda, ipiv, info);
            return *info;
        }
        ...
        if (maxdim*maxdim < 2*maxm*maxn) {
            ...
            if (MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc( &dA, nb*maxm + maxdim*maxdim )) {
            ...
            ldda = maxdim;
            magma_minproduct_zsetmatrix( m, n, A, lda, da, ldda );
            ...
            if (MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc( &dA, (nb + maxn)*maxm )) {
                /* alloc failed so call non-GPU-resident version */
                magma_minproduct_zgetrf_m(ngpu, m, n, A, lda, ipiv, info);
                return *info;
            }
            ...
            magma_minproduct_zsetmatrix( m, n, A, lda, da, maxm );
            
            if (MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc( &dAT, maxm*maxn )) {
                /* alloc failed so call non-GPU-resident version */
            ...
            magma_minproductblas_ztranspose( m, n, da, maxm, dAT, ldda );
        }
        ...
        for( j = 0; j < s; j++ ) {
            ...
            if (j > 0) {
                magma_minproductblas_ztranspose( nb, cols, dAT(j,j), ldda, dA, cols );

                // make sure that gpu queue is empty
                magma_minproduct_device_sync();

                magma_minproduct_zgetmatrix_async( m-j*nb, nb, dA, cols, work, lda,
                                        stream[0]);
            ...
            // upload j-th panel
            magma_minproduct_zsetmatrix_async( m-j*nb, nb, work, lda, dA, cols,
                                    stream[0]);

            for( i=j*nb; i < j*nb + nb; ++i ) {
                ipiv[i] += j*nb;
            }
            magma_minproductblas_zlaswp( n, dAT, ldda, j*nb + 1, j*nb + nb, ipiv, 1 );

            magma_minproduct_queue_sync( stream[0] );
            magma_minproductblas_ztranspose( cols, nb, dA, cols, dAT(j,j), ldda );
        ...
        if ( nb0 > 0 ) {
            ...
            magma_minproductblas_ztranspose( nb0, rows, dAT(s,s), ldda, dA, cols );
            magma_minproduct_zgetmatrix( rows, nb0, dA, cols, work, lda );
            ...
            magma_minproductblas_zlaswp( n, dAT, ldda, s*nb + 1, s*nb + nb0, ipiv, 1 );

            // upload j-th panel
            magma_minproduct_zsetmatrix( rows, nb0, work, lda, dA, cols );
            magma_minproductblas_ztranspose( rows, nb0, dA, cols, dAT(s,s), ldda );
        ...
        if (maxdim*maxdim < 2*maxm*maxn) {
            ...
            magma_minproductblas_ztranspose( n, m, dAT, ldda, da, maxm );
            magma_minproduct_zgetmatrix( m, n, da, maxm, A, lda );
