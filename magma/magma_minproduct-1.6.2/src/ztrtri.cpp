    ...
    magma_minproduct_int_t *info)
{
    ...
    *info = 0;
    ...
    if (nounit) {
        for (j=0; j < n; ++j) {
            if ( MAGMA_minproduct_Z_EQUAL( *A(j,j), c_zero )) {
    ...
    if (nb <= 1 || nb >= n)
        ...
        if (upper) {
            ...
            for (j=0; j < n; j += nb) {
                jb = min(nb, (n-j));
                magma_minproduct_zsetmatrix( jb, (n-j),
                                  A(j, j),  lda,
                                  dA(j, j), ldda );
                ...
                lapackf77_ztrtri(Magma_minproductUpperStr, diag_, &jb, A(j,j), &lda, info);

                magma_minproduct_zsetmatrix( jb, jb,
                                  A(j, j),  lda,
                                  dA(j, j), ldda );
            }
        }
            ...
            for (j=nn-1; j >= 0; j -= nb) {
                ...
                if ((j+jb) < n) {
                    magma_minproduct_zsetmatrix( (n-j), jb,
                                      A(j, j),  lda,
                                      dA(j, j), ldda );
                    ...
                    magma_minproduct_zgetmatrix_async( n-j-jb, jb,
                                            dA(j+jb, j), ldda,
                ...
                /* Compute inverse of current diagonal block */
                lapackf77_ztrtri(Magma_minproductLowerStr, diag_, &jb, A(j,j), &lda, info);
