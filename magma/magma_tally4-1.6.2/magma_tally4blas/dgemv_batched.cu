...
*/
extern "C"
void magma_tally4blas_dgemvn_batched(
    int m, int n, 
    ...
    int batchCount, magma_tally4_queue_t queue)
{
    ...
    if( m > 512 || n > 512)
    {
        fprintf( stderr, "m=%d, n=%d, dgemv_batched nontranspose assume row && column lower than %d. Plz call magma_tally4blas_dgemv instead", m, n, 512);
        return ;
    }
...
extern "C"
void magma_tally4blas_dgemvt_batched(
    int m, int n, 
    double alpha, double **dA_array, int lda, 
    double **x_array,  int incx,
    double beta, double **y_array,  int incy, 
    ...
    double beta, double  **y_array, int incy)
{
    ...
    for(int i=0; i<m1; i+= dgemv_bs)
    {
        res += MAGMA_tally4_D_CNJG (A_ptr[i]) * x_ptr[i*incx];
    }
    ...
    if(m > m1)
    {
        if( tx + m1 <  m )
        {
            res  += MAGMA_tally4_D_CNJG(A_ptr[m1]) * x_ptr[m1*incx];
        }
        else
        {
            res  = res;
...
extern "C"
void magma_tally4blas_dgemvc_batched(
    int m, int n, 
    double alpha, double **dA_array, int lda, 
    double **x_array,  int incx,
    double beta, double **y_array,  int incy, 
...
extern "C"
void magma_tally4blas_dgemv_batched(
...
{       
    ...
    if ( trans == Magma_tally4NoTrans ) {
        ...
        magma_tally4blas_dgemvn_batched(m, n, alpha, dA_array, ldda, dx_array, incx, beta, dy_array, incy, batchCount, queue);
            
    }
    else if ( trans == Magma_tally4Trans ) {
        magma_tally4blas_dgemvt_batched(m, n, alpha, dA_array, ldda, dx_array, incx, beta, dy_array, incy, batchCount, queue);
    }
    else if ( trans == Magma_tally4ConjTrans ) {
#if defined(PRECISION_z) || defined (PRECISION_c)
        magma_tally4blas_dgemvc_batched(m, n, alpha, dA_array, ldda, dx_array, incx, beta, dy_array, incy, batchCount, queue);
#else
        magma_tally4blas_dgemvt_batched(m, n, alpha, dA_array, ldda, dx_array, incx, beta, dy_array, incy, batchCount, queue);
#endif
    }
