...
int main( int argc, char** argv )
{
    ...
    magma_tally2DoubleComplex_ptr dC, dA, dT;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int itran = 0; itran < 2; ++itran ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {        
            ...
            TESTING_MALLOC_CPU( A,   magma_tally2DoubleComplex, lda*k );
            ...
            TESTING_MALLOC_DEV( dA, magma_tally2DoubleComplex, lda*k );
            TESTING_MALLOC_DEV( dT, magma_tally2DoubleComplex, dt_size );
            ...
            magma_tally2_zsetmatrix( lda, k, A,  lda, dA, lda );
            magma_tally2_zgeqrf_gpu( lda, k, dA, lda, tau, dT, &info );
            magma_tally2_zgetmatrix( lda, k, dA, lda, A,  lda );
            if (info != 0)
            ...
            lapackf77_zunmqr( lapack_side_const_tally2( side[iside] ), lapack_trans_const_tally2( trans[itran] ),
                              &m, &n, &k,
            ...
            lwork = -1;
            magma_tally2_zunmqr_gpu( side[iside], trans[itran],
                              m, n, k,
                              dA, lda, tau, dC, ldc, W, lwork, dT, nb, &info );
            if (info != 0)
                printf("magma_tally2_zunmqr_gpu (lwork query) returned error %d: %s.\n",
                       (int) info, magma_tally2_strerror( info ));
            ...
            if (info != 0)
                printf("magma_tally2_zunmqr_gpu returned error %d: %s.\n",
                       (int) info, magma_tally2_strerror( info ));
