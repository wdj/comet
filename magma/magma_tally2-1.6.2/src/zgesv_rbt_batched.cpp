                  ...
                  magma_tally2_int_t batchCount, magma_tally2_queue_t queue)
{
    ...
    magma_tally2DoubleComplex *hu, *hv;
    if (MAGMA_tally2_SUCCESS != magma_tally2_zmalloc_cpu( &hu, 2*n )) {
        info = MAGMA_tally2_ERR_HOST_ALLOC;
        return info;
    }

    if (MAGMA_tally2_SUCCESS != magma_tally2_zmalloc_cpu( &hv, 2*n )) {
        info = MAGMA_tally2_ERR_HOST_ALLOC;
        return info;
    }
    ...
    info = magma_tally2_zgerbt_batched(Magma_tally2True, n, nrhs, dA_array, n, dB_array, n, hu, hv, &info, batchCount, queue);
    if (info != MAGMA_tally2_SUCCESS)  {
        return info;
    }


    info = magma_tally2_zgetrf_nopiv_batched( n, n, dA_array, ldda, info_array, batchCount, queue);
    if ( (info != MAGMA_tally2_SUCCESS) ){
        return info;
    }
    ...
    info = magma_tally2_zgetrs_nopiv_batched( Magma_tally2NoTrans, n, nrhs, dA_array, ldda, dB_array, lddb, info_array, batchCount, queue );


    /* The solution of A.x = b is Vy computed on the GPU */
    magma_tally2DoubleComplex *dv;

    if (MAGMA_tally2_SUCCESS != magma_tally2_zmalloc( &dv, 2*n )) {
        info = MAGMA_tally2_ERR_DEVICE_ALLOC;
        return info;
    }

    magma_tally2_zsetvector(2*n, hv, 1, dv, 1);

    for(int i = 0; i < nrhs; i++)
        magma_tally2blas_zprbt_mv_batched(n, dv, dB_array+(i), batchCount, queue);
