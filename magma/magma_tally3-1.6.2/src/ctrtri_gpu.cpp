...
#define dA(i, j) (dA+(j)*ldda + (i))
    ...
    *info = 0;
    ...
    if (nounit) {
        for (j=0; j < n; ++j) {
            if ( MAGMA_tally3_C_EQUAL( *dA(j,j), c_zero )) {
    ...
    if (nb <= 1 || nb >= n) {
        ...
        if (upper) {
            ...
            for (j=0; j < n; j += nb) {
                ...
                lapackf77_ctrtri( Magma_tally3UpperStr, diag_, &jb, work, &jb, info );
            ...
            for (j=nn-1; j >= 0; j -= nb) {
                ...
                /* Compute inverse of current diagonal block */
                lapackf77_ctrtri( Magma_tally3LowerStr, diag_, &jb, work, &jb, info );
