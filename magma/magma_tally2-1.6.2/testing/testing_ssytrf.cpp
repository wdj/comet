...
void init_matrix( int nopiv, int m, int n, float *h_A, magma_tally2_int_t lda )
{
    assert( m == n );
    ...
    if (nopiv) {
        ...
        magma_tally2_smake_symmetric( n, h_A, lda );
    }
}
    ...
    magma_tally2_int_t *ipiv )
{
    ...
    if (nopiv) {
        if (upper) {
            blasf77_strsm( Magma_tally2LeftStr, Magma_tally2UpperStr, 
                           Magma_tally2ConjTransStr, Magma_tally2UnitStr, 
                           &n, &ione, &c_one,
                           A, &lda, x, &n );
            for (int i=0; i<n; i++) x[i] = MAGMA_tally2_S_DIV( x[i], A[i+i*lda] );
            blasf77_strsm( Magma_tally2LeftStr, Magma_tally2UpperStr, 
                           Magma_tally2NoTransStr, Magma_tally2UnitStr, 
                           &n, &ione, &c_one,
                           A, &lda, x, &n );
        } else {
            blasf77_strsm( Magma_tally2LeftStr, Magma_tally2LowerStr, 
                           Magma_tally2NoTransStr, Magma_tally2UnitStr, 
                           &n, &ione, &c_one,
                           A, &lda, x, &n );
            for (int i=0; i<n; i++) x[i] = MAGMA_tally2_S_DIV( x[i], A[i+i*lda] );
        ...
        lapackf77_ssytrs( lapack_uplo_const_tally2(uplo), &n, &ione, A, &lda, ipiv, x, &n, &info );
    }
    if (info != 0)
    ...
    norm_A = lapackf77_slange( Magma_tally2FullStr, &n, &n, A, &lda, work );
    norm_r = lapackf77_slange( Magma_tally2FullStr, &n, &ione, b, &n, work );
    norm_x = lapackf77_slange( Magma_tally2FullStr, &n, &ione, x, &n, work );
    
    //printf( "r=\n" ); magma_tally2_sprint( 1, n, b, 1 );
...
float get_LDLt_error(int nopiv, magma_tally2_uplo_t uplo, magma_tally2_int_t N,
                      float *LD, magma_tally2_int_t lda,
                      magma_tally2_int_t *ipiv)
{
    ...
    if (uplo == Magma_tally2Upper) {
        for (j=N-1; j>=0; j--) {
            ...
            if (piv < 0) {
                ...
                D(j,j)     = LD(j,j);
                D(j,j-1)   = MAGMA_tally2_S_CNJG(LD(j-1,j));
                D(j-1,j)   = LD(j-1,j);
        ...
        if (nopiv) {
            // compute W = D*U
            blasf77_sgemm(Magma_tally2NoTransStr, Magma_tally2NoTransStr, &N, &N, &N,
                          &c_one, D, &N, L, &N, &c_zero, LD, &lda);
            // compute D = U'*W
            blasf77_sgemm(Magma_tally2ConjTransStr, Magma_tally2NoTransStr, &N, &N, &N,
            ...
            // compute D = W*U'
            blasf77_sgemm(Magma_tally2NoTransStr, Magma_tally2ConjTransStr, &N, &N, &N,
    ...
    // compute norm of A
    matnorm = lapackf77_slange(Magma_tally2FullStr, &N, &N, A, &lda, work);

    for( j = 0; j < N; j++ ) {
        for( i = 0; i < N; i++ ) {
            D(i,j) = MAGMA_tally2_S_SUB( D(i,j), A(i,j) );
        }
    }
    residual = lapackf77_slange(Magma_tally2FullStr, &N, &N, D, &N, work);
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( opts.lapack ) {
                lwork = -1;
                lapackf77_ssytrf( lapack_uplo_const_tally2(opts.uplo), &N, h_A, &lda, ipiv, &temp, &lwork, &info);
                lwork = (int)MAGMA_tally2_S_REAL(temp);
                TESTING_MALLOC_CPU( work, float, lwork );

                init_matrix( nopiv, N, N, h_A, lda );
                cpu_time = magma_tally2_wtime();
                lapackf77_ssytrf( lapack_uplo_const_tally2(opts.uplo), &N, h_A, &lda, ipiv, work, &lwork, &info);
            ...
            if (nopiv) {
                ...
                magma_tally2_ssytrf_nopiv_tally2( opts.uplo, N, h_A, lda, &info);
                gpu_time = magma_tally2_wtime() - gpu_time;
            } else if (cpu) {
                // CPU-interface to Bunch-Kauffman LDLt
                magma_tally2_setdevice(0);
                gpu_time = magma_tally2_wtime();
                magma_tally2_ssytrf( opts.uplo, N, h_A, lda, ipiv, &info);
                gpu_time = magma_tally2_wtime() - gpu_time;
            } else if (nopiv_gpu) {
                ...
                if (MAGMA_tally2_SUCCESS != magma_tally2_smalloc( &d_A, N*ldda  )) {
                    printf( " failed to allocate d_A(%dx%d)\n", (int) N, (int) ldda );
                    return 0;
                }
                magma_tally2_ssetmatrix(N, N, h_A, lda, d_A, ldda);
                gpu_time = magma_tally2_wtime();
                magma_tally2_ssytrf_nopiv_tally2_gpu( opts.uplo, N, d_A, ldda, &info);
                gpu_time = magma_tally2_wtime() - gpu_time;
                magma_tally2_sgetmatrix(N, N, d_A, ldda, h_A, lda);
                magma_tally2_free( d_A );
            } else if (row) {
                magma_tally2_setdevice(0);
                //magma_tally2_ssytrf_gpu_row( opts.uplo, N, h_A, lda, ipiv, work, lwork, &info);
            } else {
                magma_tally2_setdevice(0);
                //magma_tally2_ssytrf_hybrid( opts.uplo, N, h_A, lda, ipiv, work, lwork, &info);
            }
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally2_ssytrf returned error %d: %s.\n",
                       (int) info, magma_tally2_strerror( info ));
