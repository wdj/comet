    ...
    magma_tally4_int_t *info )
{
    ...
    magma_tally4FloatComplex *dwork[Magma_tally4MaxGPUs]={NULL}, *dpanel[Magma_tally4MaxGPUs]={NULL};
    magma_tally4FloatComplex *hwork=NULL, *hpanel=NULL;
    ...
    *info = 0;
    ...
    for( dev=0; dev < ngpu; dev++ ) {
        ...
        if ( MAGMA_tally4_SUCCESS != magma_tally4_cmalloc( &(dwork[dev]), (lddwork + ldda)*nb )) {
    ...
    if ( MAGMA_tally4_SUCCESS != magma_tally4_cmalloc_pinned( &hwork, lwork )) {
    ...
    if ( nb < min_mn ) {
        ...
        for( i = 0; i < min_mn-nb; i += nb ) {
            ...
            cpanel_to_q_tally4( Magma_tally4Upper, ib, hpanel(i), ldhpanel, hwork + ib*ib );
            ...
            for( dev=0; dev < ngpu; dev++ ) {
                ...
                magma_tally4_csetmatrix_async( rows, ib,
                                        hpanel(i),   ldhpanel,
            ...
            // TODO: if cpanel_to_q_tally4 copied whole block, wouldn't need to restore
            // -- just send the copy to the GPUs.
            // TODO: also, could zero out the lower triangle and use Azzam's larfb w/ gemm.
            
            /* Restore the panel */
            cq_to_panel_tally4( Magma_tally4Upper, ib, hpanel(i), ldhpanel, hwork + ib*ib );

            if (i + ib < n) {
                /* Send the T matrix to the GPU. */
                for( dev=0; dev < ngpu; dev++ ) {
                    magma_tally4_setdevice( dev );
                    magma_tally4_csetmatrix_async( ib, ib,
                                            hwork,      ib,
    ...
    if (i < min_mn) {
        ...
        for( j=i; j < n; j += nb ) {
            ...
            ib = min( n-j, nb );
            magma_tally4_setdevice( panel_dev );
            magma_tally4_cgetmatrix( rows, ib,
                              dlA(panel_dev, i, i_local), ldda,
                              hwork + (j-i)*rows,         rows );
        }
