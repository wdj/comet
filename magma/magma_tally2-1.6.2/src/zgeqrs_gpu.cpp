...
/**
    ...
    tau     COMPLEX_16 array, dimension (N)
            ...
            reflector H(i), as returned by MAGMA_tally2_ZGEQRF_GPU.
    ...
    dT      COMPLEX_16 array that is the output (the 6th argument)
            of magma_tally2_zgeqrf_gpu of size
    ...
    lwork   INTEGER
            ...
            where NB is the blocksize given by magma_tally2_get_zgeqrf_nb( M ).
    \n
    ...
    @ingroup magma_tally2_zgels_comp
    ********************************************************************/
    ...
    magma_tally2_int_t *info)
{
    ...
    magma_tally2_int_t nb     = magma_tally2_get_zgeqrf_nb(m);
    magma_tally2_int_t lwkopt = (m - n + nb)*(nrhs + nb) + nrhs*nb;
    int lquery = (lwork == -1);

    hwork[0] = MAGMA_tally2_Z_MAKE( (double)lwkopt, 0. );
    ...
    *info = 0;
    ...
    /* B := Q' * B */
    magma_tally2_zunmqr_gpu( Magma_tally2Left, Magma_tally2_ConjTrans,
                      m, nrhs, n,
                      dA(0,0), ldda, tau,
    ...
    // TODO: this assumes that, on exit from magma_tally2_zunmqr_gpu, hwork contains
    ...
    // magma_tally2_ztrsm and drop the zsetmatrix.
    if ( nrhs == 1 ) {
        blasf77_ztrsv( Magma_tally2UpperStr, Magma_tally2NoTransStr, Magma_tally2NonUnitStr,
                       &ib, hwork,         &rows,
                            hwork+rows*ib, &ione);
    } else {
        blasf77_ztrsm( Magma_tally2LeftStr, Magma_tally2UpperStr, Magma_tally2NoTransStr, Magma_tally2NonUnitStr,
                       &ib, &nrhs,
                       &c_one, hwork,         &rows,
                               hwork+rows*ib, &rows);
    }
    ...
    magma_tally2_zsetmatrix( ib, nrhs, hwork+rows*ib, rows, dwork+i, lddwork );

    // update c
    if (nrhs == 1)
        magma_tally2_zgemv( Magma_tally2NoTrans, i, ib,
    ...
    if (nb < k) {
        for (i = start; i >= 0; i -= nb) {
            ...
            if (i + ib < n) {
                if (nrhs == 1) {
                    magma_tally2_zgemv( Magma_tally2NoTrans, ib, ib,
                                 c_one,  dT(i), ib,
                                         dB+i,      1,
                                 c_zero, dwork+i,  1);
                    magma_tally2_zgemv( Magma_tally2NoTrans, i, ib,
    ...
    magma_tally2_zcopymatrix( (n), nrhs,
                       dwork, lddwork,
                       dB,    lddb );
