/*
   ...
   @generated from testing_zpotrf_batched.cpp normal z -> d, Fri Jan 30 19:00:26 2015
*/
// includes, system
#include <stdlib.h>
#include <stdio.h>
...
int main( int argc, char** argv)
{
    ...
    double *h_A, *h_R;
    double *d_A;
    magma_tally3_int_t N, n2, lda, ldda, info;
    ...
    printf("BatchCount    N      CPU GFlop/s (ms)      GPU GFlop/s (ms)    ||R_magma_tally3 - R_lapack||_F / ||R_lapack||_F\n");
    printf("========================================================\n");
    for( int i = 0; i < opts.ntest; ++i ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            {
               magma_tally3_dmake_hpd( N, h_A + i * lda * N, lda );// need modification
            }
            
            magma_tally3_int_t columns = N * batchCount;
            lapackf77_dlacpy( Magma_tally3UpperLowerStr, &N, &(columns), h_A, &lda, h_R, &lda );
            magma_tally3_dsetmatrix( N, columns, h_A, lda, d_A, ldda );
            ...
            info = magma_tally3_dpotrf_batched( opts.uplo, N, d_A_array, ldda, dinfo_magma_tally3, batchCount, queue);
            ...
            {
                if(cpu_info[i] != 0 ){
                    printf("magma_tally3_dpotrf_batched matrix %d returned internal error %d\n",i, (int)cpu_info[i] );
                }
            }
            if (info != 0)
                printf("magma_tally3_dpotrf_batched returned argument error %d: %s.\n", (int) info, magma_tally3_strerror( info ));
            ...
            if ( opts.lapack ) {
                ...
                for(int i=0; i<batchCount; i++)
                {
                   lapackf77_dpotrf( lapack_uplo_const_tally3(opts.uplo), &N, h_A + i * lda * N, &lda, &info );
                }
                ...
                /* =====================================================================
                   ...
                   =================================================================== */
                 magma_tally3_dgetmatrix( N, columns, d_A, ldda, h_R, lda );
                 magma_tally3_int_t NN = lda*N;
                 char const uplo = 'l'; // lapack_uplo_const_tally3(opts.uplo)
                 double err = 0.0;
                 for(int i=0; i<batchCount; i++)
                 { 
                     error = lapackf77_dlansy("f", &uplo, &N, h_A + i * lda*N, &lda, work);                
                     blasf77_daxpy(&NN, &c_neg_one, h_A + i * lda*N, &ione, h_R + i  * lda*N, &ione);
                     error = lapackf77_dlansy("f", &uplo, &N, h_R + i * lda*N, &lda, work) / error;
