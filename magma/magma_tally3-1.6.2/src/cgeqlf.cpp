...
/**
    ...
    lwork   INTEGER
            ...
            through magma_tally3_get_cgeqlf_nb(M).
    \n
    ...
    @ingroup magma_tally3_cgeqlf_comp
    ********************************************************************/
    ...
    magma_tally3_int_t *info)
{
    ...
    magma_tally3FloatComplex_ptr dA, dwork;
    ...
    nb = magma_tally3_get_cgeqlf_nb(m);
    *info = 0;
    lquery = (lwork == -1);
    ...
    if (*info == 0) {
        if (k == 0)
            ...
            work[0] = MAGMA_tally3_C_MAKE( max(n*nb, 2*nb*nb), 0 );
        }
    ...
    if (MAGMA_tally3_SUCCESS != magma_tally3_cmalloc( &dA, (n)*ldda + nb*lddwork )) {
    ...
    if ( (nb > 1) && (nb < k) ) {
        ...
        for (i = k - kk + ki; i >= k -kk; i -= nb) {
            ...
            if (i < k - kk + ki) {
                /* 1. Copy asynchronously the current panel to the CPU.
                   ...
                   to the CPU)                                        */
                rows = m - k + i + ib;
                magma_tally3_cgetmatrix_async( rows, ib,
                                        dA(0, n-k+i), ldda,
                                        A(0, n-k+i),  lda, queues[1] );

                magma_tally3_cgetmatrix_async( m-rows, ib,
                                        dA(rows, n-k+i), ldda,
                ...
                magma_tally3_clarfb_gpu( Magma_tally3Left, Magma_tally3ConjTrans, Magma_tally3Backward, Magma_tally3Columnwise,
                                  rows, cols, old_ib,
            ...
            if (cols > 0) {
                ...
                lapackf77_clarft( Magma_tally3BackwardStr, Magma_tally3ColumnwiseStr,
                                  &rows, &ib,
                                  A(0, cols), &lda, tau + i, work, &ib);

                cpanel_to_q_tally3( Magma_tally3Lower, ib, A(rows-ib,cols), lda, work+ib*ib);
                magma_tally3_csetmatrix( rows, ib,
                                  A(0,cols),  lda,
                                  dA(0,cols), ldda );
                cq_to_panel_tally3( Magma_tally3Lower, ib, A(rows-ib,cols), lda, work+ib*ib);

                // Send the triangular part on the GPU
                magma_tally3_csetmatrix( ib, ib, work, ib, dwork(0), lddwork );
        ...
        magma_tally3_cgetmatrix( m, nu, dA(0,0), ldda, A(0,0), lda );
    } else {
        mu = m;
        nu = n;
    }
