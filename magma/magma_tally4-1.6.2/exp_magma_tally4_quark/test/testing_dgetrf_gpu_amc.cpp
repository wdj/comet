...
double get_LU_error(magma_tally4_int_t M, magma_tally4_int_t N,
                    ...
                    double *LU, magma_tally4_int_t *IPIV)
{
...
int main( int argc, char** argv)
{
    ...
    magma_tally4_int_t lapack_numthreads = 1;
    double *WORK;
    ...
    d_cpu = magma_tally4_amc_recommanded_dcpu(opts.nthread, CPU_PEAK, 1, GPU_PEAK); //assume 1 GPU
    #endif
    ...
    for( int i = 0; i < opts.ntest; ++i ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lapackf77_dlacpy( Magma_tally4UpperLowerStr, &M, &N, h_A, &lda, h_R, &lda );
            ...
            magma_tally4_dsetmatrix( M, N, h_R, lda, d_A, ldda );
            gpu_time1 = magma_tally4_wtime();
            magma_tally4_dgetrf_gpu( M, N, d_A, ldda, ipiv, &info);
            gpu_time1 = magma_tally4_wtime() - gpu_time1;
            gpu_perf1 = gflops / gpu_time1;
            if (info != 0)
                printf("\nmagma_tally4_dgetrf_gpu returned error %d: %s.\n     ",
                       (int) info, magma_tally4_strerror( info ));
            
            /* ====================================================================
               Performs operation using MAGMA_tally4_ASYNC
               =================================================================== */
            /*Save BLAS number of threads*/
            /*
            lapack_numthreads = magma_tally4_getlapack_numthreads();
            if(lapack_numthreads !=1){ 
                //Temporary set the number of threads to 1.
                printf("MAGMA_tally4 num threads set from %d to %d\n", lapack_numthreads, 1);
            ...
            magma_tally4_dgetrf_gpu_amc( M, N, d_A, ldda, ipiv, &info);
            gpu_time2 = magma_tally4_wtime() - gpu_time2;
            gpu_perf2 = gflops / gpu_time2;
            magma_tally4_amc_finalize();
            if (info != 0)
                printf("\nmagma_tally4_amc_dgetrf_rec_gpu returned error %d: %s.\n     ",
                       (int) info, magma_tally4_strerror( info ));
            
            /* ====================================================================
               Performs operation using MAGMA_tally4_ASYNC..._WORK (Workspace preallocated version)
               =================================================================== */
            ...
            magma_tally4_dgetrf_gpu_work_amc(M, N, d_A, ldda, ipiv, &info, WORK, WORK_LD, WORK_n);
            gpu_time3 = magma_tally4_wtime() - gpu_time3;
            gpu_perf3 = gflops / gpu_time3;
            magma_tally4_amc_finalize();

            if (info != 0)
                printf("magma_tally4_amc_dgetrf_rec_work_gpu returned error %d: %s.\n     ",
                       (int) info, magma_tally4_strerror( info ));
            ...
            if(lapack_numthreads !=1){ 
                /*Temporary set the number of threads to 1.*/
                printf("MAGMA_tally4 num threads set from %d to %d\n", 1, lapack_numthreads);
                magma_tally4_setlapack_numthreads(lapack_numthreads);
            }
            ...
            if ( opts.check ) {
                magma_tally4_dgetmatrix( M, N, d_A, ldda, h_A, lda );
