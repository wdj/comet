/*
    -- MAGMA_minproduct (version 1.6.2) --
       ...
       @generated from magma_minproduct_zvinit.cpp normal z -> s, Sun May  3 11:23:01 2015
       @author Hartwig Anzt
*/
...
/**
    ...
    Allocates memory for magma_minproduct_s_matrix and initializes it
    ...
    magma_minproduct_queue_t queue )
{
    ...
    if ( mem_loc == Magma_minproduct_CPU ) {
        x->memory_location = Magma_minproduct_CPU;
        CHECK( magma_minproduct_smalloc_cpu( &x->val, x->nnz ));
        for( magma_minproduct_int_t i=0; i<x->nnz; i++)
             x->val[i] = values;
    }
    else if ( mem_loc == Magma_minproduct_DEV ) {
        x->memory_location = Magma_minproduct_DEV;
        CHECK( magma_minproduct_smalloc( &x->val, x->nnz ));
        magma_minproductblas_slaset(Magma_minproductFull, x->num_rows, x->num_cols, values, values, x->val, x->num_rows);
    }
