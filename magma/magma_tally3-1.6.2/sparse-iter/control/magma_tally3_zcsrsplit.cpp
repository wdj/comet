    ...
    magma_tally3_queue_t queue )
{
    ...
    if (  A.memory_location == Magma_tally3_CPU &&
        ...
        magma_tally3_z_matrix Ah={Magma_tally3_CSR}, ACSR={Magma_tally3_CSR}, DCSR={Magma_tally3_CSR}, RCSR={Magma_tally3_CSR}, Dh={Magma_tally3_CSR}, Rh={Magma_tally3_CSR};
        CHECK( magma_tally3_zmtransfer( A, &Ah, A.memory_location, Magma_tally3_CPU, queue ));
        CHECK( magma_tally3_zmconvert( Ah, &ACSR, A.storage_type, Magma_tally3_CSR, queue ));

        CHECK( magma_tally3_zcsrsplit( bsize, ACSR, &DCSR, &RCSR, queue ));

        CHECK( magma_tally3_zmconvert( DCSR, &Dh, Magma_tally3_CSR, A.storage_type, queue ));
        CHECK( magma_tally3_zmconvert( RCSR, &Rh, Magma_tally3_CSR, A.storage_type, queue ));

        CHECK( magma_tally3_zmtransfer( Dh, D, Magma_tally3_CPU, A.memory_location, queue ));
        CHECK( magma_tally3_zmtransfer( Rh, R, Magma_tally3_CPU, A.memory_location, queue ));

        magma_tally3_zmfree( &Ah, queue );
        magma_tally3_zmfree( &ACSR, queue );
        magma_tally3_zmfree( &Dh, queue );
        magma_tally3_zmfree( &DCSR, queue );
        magma_tally3_zmfree( &Rh, queue );
        magma_tally3_zmfree( &RCSR, queue );
    }
cleanup:
    if( info != 0 ){
