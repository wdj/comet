...
magma_minproduct_ziterref(
    ...
    magma_minproduct_z_solver_par *solver_par, magma_minproduct_z_preconditioner *precond_par,
    magma_minproduct_queue_t queue )
{
    ...
    // workspace
    magma_minproduct_z_matrix r={Magma_minproduct_CSR}, z={Magma_minproduct_CSR};
    ...
    //CHECK(  magma_minproduct_zresidualvec( A, b, *x, &r, nom, queue));
    magma_minproduct_zcopy( dofs, b.dval, 1, r.dval, 1 );                    // r = b
    nom0 = magma_minproduct_dznrm2(dofs, r.dval, 1);                       // nom0 = || r ||
    nom = nom0 * nom0;
    ...
    for( solver_par->numiter= 1; solver_par->numiter<solver_par->maxiter;
        ...
        magma_minproduct_zscal( dofs, MAGMA_minproduct_Z_MAKE(1./nom, 0.), r.dval, 1) ;  // scale it
        CHECK( magma_minproduct_z_precond( A, r, &z, precond_par, queue )); // inner solver:  A * z = r
        magma_minproduct_zscal( dofs, MAGMA_minproduct_Z_MAKE(nom, 0.), z.dval, 1) ;  // scale it
        magma_minproduct_zaxpy(dofs,  c_one, z.dval, 1, x->dval, 1);        // x = x + z
        CHECK( magma_minproduct_z_spmv( c_mone, A, *x, c_zero, r, queue ));      // r = - A x
        magma_minproduct_zaxpy(dofs,  c_one, b.dval, 1, r.dval, 1);         // r = r + b
        nom = magma_minproduct_dznrm2(dofs, r.dval, 1);                    // nom = || r ||
