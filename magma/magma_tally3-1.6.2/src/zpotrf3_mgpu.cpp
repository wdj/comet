...
#define dlPT(id, i, j, k) (d_lP[(id)] + (k)*nb*lddp + (j)*nb   + (i))
    ...
    magma_tally3DoubleComplex *dlpanel;
    ...
    *info = 0;
    ...
    magma_tally3DoubleComplex *d_dinvA[Magma_tally3MaxGPUs];
    magma_tally3DoubleComplex *d_x[Magma_tally3MaxGPUs];
    ...
    for( d=0; d < ngpu; d++ ) {
        ...
        if ( (MAGMA_tally3_SUCCESS != magma_tally3_zmalloc( &d_dinvA[d], 2*trsm_nb*trsm_n )) ||
             (MAGMA_tally3_SUCCESS != magma_tally3_zmalloc( &d_x[d],     2*nb*(upper ? n : m) )) ) {
    ...
    if (upper) {
        ...
        for (j=0; j < m; j += nb) {
            ...
            lapackf77_zpotrf(Magma_tally3UpperStr, &jb, Aup(j,j), &lda, info);
            trace_cpu_end( 0 );
            if (*info != 0) {
                *info = *info + j;
                break;
            ...
            if ( (j+jb) < n) {
...
#if (defined(PRECISION_d) || defined(PRECISION_s)) && defined(ZTRSM_WORK)
                        magma_tally3blas_zlaset( Magma_tally3Full, trsm_nb, trsm_n, c_zero, c_zero, dinvA(d,0), trsm_nb );
                        magma_tally3blas_zlaset( Magma_tally3Full, nb0,     jb,     c_zero, c_zero, dx(d,0), nb0 );
                        magma_tally3blas_ztrsm_work( Magma_tally3Left, Magma_tally3Upper,
                                              ...
                                              1, dinvA(d,0), dx(d,0) );
#else
                        magma_tally3_ztrsm( Magma_tally3Left, Magma_tally3Upper,
                                     Magma_tally3ConjTrans, Magma_tally3NonUnit,
                                     jb, nb0, c_one,
                                     dlpanel,                ldpanel,
                                     dlA(d, j, nb*j_local2), ldda);
...
#if (defined(PRECISION_d) || defined(PRECISION_s)) && defined(ZTRSM_WORK)
                        magma_tally3blas_zlaset( Magma_tally3Full, trsm_nb, trsm_n, c_zero, c_zero, dinvA(d,0), trsm_nb );
                        magma_tally3blas_zlaset( Magma_tally3Full, nb2,     jb,     c_zero, c_zero, dx(d,0), nb2 );
                        magma_tally3blas_ztrsm_work( Magma_tally3Left, Magma_tally3Upper,
                                              Magma_tally3ConjTrans, Magma_tally3NonUnit,
                                              jb, nb2, c_one,
                                              dlpanel, ldpanel,
                ...
                if ( j+jb < m ) {
                    ...
                    magma_tally3_zgetmatrix_async( (j+jb), nb0,
                    ...
                    for( d2=0; d2 < ngpu; d2++ ) {
                        if ( d2 != d ) {
                            ...
                            magma_tally3_zsetmatrix_async( j+jb, nb0,
                                                    Aup(0,j+jb),       lda,
                    ...
                    if ( nb2 > 0 ) {
                        ...
                        if (flag == 0) {
                            ...
                            magma_tally3blas_zlaset( Magma_tally3Full, trsm_nb, trsm_n, c_zero, c_zero, dinvA(d,flag), trsm_nb );
                            magma_tally3_queue_wait_event( queues[d][stream2], events[d][1] ); // panel received
                        }
                        magma_tally3blas_zlaset( Magma_tally3Full, nb2, jb, c_zero, c_zero, dx(d,1), nb2 );
                        magma_tally3blas_ztrsm_work( Magma_tally3Left, Magma_tally3Upper, Magma_tally3ConjTrans, Magma_tally3NonUnit,
                                              jb, nb2, c_one,
                                              dlpanel, ldpanel,
        ...
        for (j=0; j < n; j += nb) {
            ...
            magma_tally3_setdevice(id);
            magma_tally3_queue_sync( queues[id][stream1] );
            lapackf77_zpotrf(Magma_tally3LowerStr, &jb, Alo(j,j), &lda, info);
            if (*info != 0) {
                *info = *info + j;
                break;
            }
            ...
            if ( (j+jb) < m) {
                ...
                for( dd=0; dd < ngpu; dd++ ) {
                    ...
                    if ( j+nb < n && d == (j/nb+1)%ngpu ) { /* owns next column, look-ahead next block on stream1 */
                        ...
                        magma_tally3blas_zlaset( Magma_tally3Full, trsm_nb, trsm_n, c_zero, c_zero, dinvA(d,0), trsm_nb );
                        magma_tally3blas_zlaset( Magma_tally3Full, nb0,     jb,     c_zero, c_zero, dx(d,0), nb0 );
                        magma_tally3blas_ztrsm_work( Magma_tally3Right, Magma_tally3Lower,
                                              Magma_tally3ConjTrans, Magma_tally3NonUnit,
                                              nb0, jb, c_one,
                                              dlpanel, ldpanel,
                        ...
                        magma_tally3blas_zlaset( Magma_tally3Full, trsm_nb, trsm_n, c_zero, c_zero, dinvA(d,0), trsm_nb );
                        magma_tally3blas_zlaset( Magma_tally3Full, nb2,     jb,     c_zero, c_zero, dx(d,0), nb2 );
                        magma_tally3blas_ztrsm_work( Magma_tally3Right, Magma_tally3Lower, Magma_tally3ConjTrans, Magma_tally3NonUnit,
                                              nb2, jb, c_one,
                                              dlpanel,                ldpanel,
                                              dlA(d, nb*j_local2, j), ldda,
                                              1, dinvA(d,0), dx(d,0) );
...
//#define ZPOTRF_DEVICE_TO_DEVICE
                    ...
                    for( d2=0; d2 < ngpu; d2++ ) {
                        if ( d2 != d ) {
                            ...
                            magma_tally3_zsetmatrix_async( nb0, j+jb,
                                                    Alo(j+jb,0),        lda,
                                                    dlPT(d2,0,nb,buf2), nb, // first nbxnb reserved for diagonal block
                                                    queues[d2][stream3] );
                            magma_tally3_event_record( events[d2][0], queues[d2][stream3] );
    ...
    magma_tally3_int_t *info)
{
    ...
    if (uplo == Magma_tally3Upper) {
        ...
        for (j=off_j; j < n; j += nb) {
            ...
            magma_tally3_zsetmatrix_async( mj, jb,
                                    A(off_i, j),     lda,
                                    dA(k, 0, jj*nb), ldda,
                                    queues[k][0] );
        }
        ...
        for (i=off_i; i < m; i += nb) {
            ...
            magma_tally3_setdevice(k);
            magma_tally3_zsetmatrix_async( ib, ni,
                                    A(i, off_j),     lda,
                                    dA(k, ii*nb, 0), ldda,
                                    queues[k][0] );
        }
    ...
    magma_tally3_int_t *info)
{
    ...
    if (uplo == Magma_tally3Upper) {
        ...
        for (j=off_j+NB; j < n; j += nb) {
            ...
            magma_tally3_setdevice(k);
            magma_tally3_zgetmatrix_async( mj, jb,
                                    dA(k, 0, jj*nb), ldda,
                                    A(off_i, j),     lda,
                                    queues[k][0] );
            magma_tally3_queue_sync( queues[k][0] );
        ...
        for (i=off_i+NB; i < m; i += nb) {
            ...
            magma_tally3_setdevice(k);
            magma_tally3_zgetmatrix_async( ib, ni,
                                    dA(k, ii*nb, 0), ldda,
                                    A(i, off_j),     lda,
                                    queues[k][0] );
            magma_tally3_queue_sync( queues[k][0] );
