...
{
    magma_tally3_int_t nb, n_local[Magma_tally3MaxGPUs];
    magma_tally3_int_t maxm;
    magma_tally3_int_t i, j, d, lddat, lddwork;
    magma_tally3FloatComplex *d_lAT[Magma_tally3MaxGPUs];
    magma_tally3FloatComplex *d_panel[Magma_tally3MaxGPUs], *work;
    magma_tally3_queue_t streaml[Magma_tally3MaxGPUs][2];
    ...
    *info = 0;
    ...
    if (nb <= 1 || nb >= n) {
        /* Use CPU code. */
        magma_tally3_cmalloc_cpu( &work, m * n );
        if ( work == NULL ) {
            *info = MAGMA_tally3_ERR_HOST_ALLOC;
            return *info;
        }
        magma_tally3_cgetmatrix( m, n, d_lA[0], ldda, work, m );
        lapackf77_cgetrf(&m, &n, work, &m, ipiv, info);
        magma_tally3_csetmatrix( m, n, work, m, d_lA[0], ldda );
        ...
        for (i=0; i < ngpu; i++) {
            ...
            /* workspaces */
            if (MAGMA_tally3_SUCCESS != magma_tally3_cmalloc( &d_panel[i], (3+ngpu)*nb*maxm )) {
                for( j=0; j <= i; j++ ) {
                    magma_tally3_setdevice(j);
                }
                for( j=0; j < i; j++ ) {
                    magma_tally3_setdevice(j);
                    magma_tally3_free( d_panel[j] );
                    magma_tally3_free( d_lAT[j]   );
                }
            ...
            if (MAGMA_tally3_SUCCESS != magma_tally3_cmalloc( &d_lAT[i], lddat*maxm )) {
                for( j=0; j <= i; j++ ) {
                    magma_tally3_setdevice(j);
                    magma_tally3_free( d_panel[j] );
                }
            ...
            /* create the streams */
            magma_tally3_queue_create( &streaml[i][0] );
            magma_tally3_queue_create( &streaml[i][1] );
            
            magma_tally3blasSetKernelStream(streaml[i][1]);
            magma_tally3blas_ctranspose( m, n_local[i], d_lA[i], ldda, d_lAT[i], lddat );
        }
        for (i=0; i < ngpu; i++) {
            magma_tally3_setdevice(i);
            magma_tally3_queue_sync(streaml[i][0]);
        ...
        /* cpu workspace */
        lddwork = maxm;
        if (MAGMA_tally3_SUCCESS != magma_tally3_cmalloc_pinned( &work, lddwork*nb*ngpu )) {
            for (i=0; i < ngpu; i++ ) {
                magma_tally3_setdevice(i);
                magma_tally3_free( d_panel[i] );
                magma_tally3_free( d_lAT[i]   );
            }
            *info = MAGMA_tally3_ERR_HOST_ALLOC;
            return *info;
        }
        ...
        magma_tally3_cgetrf2_mgpu(ngpu, m, n, nb, 0, d_lAT, lddat, ipiv, d_panel, work, maxm,
                           streaml, info);
        ...
        for( d=0; d < ngpu; d++ ) {
            ...
            /* save on output */
            magma_tally3blas_ctranspose( n_local[d], m, d_lAT[d], lddat, d_lA[d], ldda );
            magma_tally3_device_sync();
            magma_tally3_free( d_lAT[d]   );
            magma_tally3_free( d_panel[d] );
            magma_tally3_queue_destroy( streaml[d][0] );
            magma_tally3_queue_destroy( streaml[d][1] );
        } /* end of for d=1,..,ngpu */
