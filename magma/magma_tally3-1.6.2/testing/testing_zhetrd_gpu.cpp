...
int main( int argc, char** argv)
{
    ...
    double           eps;
    magma_tally3DoubleComplex *h_A, *h_R, *h_Q, *h_work, *work;
    magma_tally3DoubleComplex_ptr d_R, dwork;
    magma_tally3DoubleComplex *tau;
    double          *diag, *offdiag;
    double           result[2] = {0., 0.};
    magma_tally3_int_t N, n2, lda, ldda, lwork, info, nb, ldwork;
    ...
    printf("2 - uses ZHEMV from MAGMA_tally3 BLAS that requires extra space\n\n");
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lwork  = N*nb;  /* We suppose the magma_tally3 nb is bigger than lapack nb */
            ...
            TESTING_MALLOC_DEV( d_R,     magma_tally3DoubleComplex, ldda*N );
            ...
            magma_tally3_zsetmatrix( N, N, h_A, lda, d_R, ldda );
            ...
            if (opts.version == 1) {
                magma_tally3_zhetrd_gpu( opts.uplo, N, d_R, ldda, diag, offdiag,
                                  tau, h_R, lda, h_work, lwork, &info );
            }
            else {
                magma_tally3_zhetrd2_gpu( opts.uplo, N, d_R, ldda, diag, offdiag,
                                   tau, h_R, lda, h_work, lwork, dwork, ldwork, &info );
            }
            gpu_time = magma_tally3_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally3_zhetrd_gpu returned error %d: %s.\n",
            ...
            /* =====================================================================
               ...
               =================================================================== */
            if ( opts.check ) {
                TESTING_MALLOC_CPU( h_Q,  magma_tally3DoubleComplex, lda*N );
                TESTING_MALLOC_CPU( work, magma_tally3DoubleComplex, 2*N*N );
                #ifdef COMPLEX
                TESTING_MALLOC_CPU( rwork, double, N );
                #endif
                
                magma_tally3_zgetmatrix( N, N, d_R, ldda, h_R, lda );
                magma_tally3_zgetmatrix( N, N, d_R, ldda, h_Q, lda );
                lapackf77_zungtr( lapack_uplo_const_tally3(opts.uplo), &N, h_Q, &lda, tau, h_work, &lwork, &info );
                
                lapackf77_zhet21( &itwo, lapack_uplo_const_tally3(opts.uplo), &N, &ione,
                ...
                lapackf77_zhet21( &ithree, lapack_uplo_const_tally3(opts.uplo), &N, &ione,
                                  h_A, &lda, diag, offdiag,
                                  h_Q, &lda, h_R, &lda,
                                  tau, work,
                                  #ifdef COMPLEX
            ...
            if ( opts.lapack ) {
                ...
                lapackf77_zhetrd( lapack_uplo_const_tally3(opts.uplo), &N, h_A, &lda, diag, offdiag, tau,
                                  h_work, &lwork, &info );
