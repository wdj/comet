...
/**
    ...
    dT      REAL array on the GPU device.
            ...
            reflectors H(i) as returned by magma_tally4_ssytrd.
...
magma_tally4_sorgtr(
    ...
    float *dT, magma_tally4_int_t nb,
    ...
    *info = 0;
    ...
    if (upper) {
        ...
        for (j = 0; j < n-1; ++j) {
            ...
            *A(n-1, j) = MAGMA_tally4_S_ZERO;
        }
        for (i = 0; i < n-1; ++i) {
            *A(i, n-1) = MAGMA_tally4_S_ZERO;
        }
        *A(n-1, n-1) = MAGMA_tally4_S_ONE;
        ...
        /*  Q was determined by a call to SSYTRD with UPLO = 'L'.
            ...
            those of the unit matrix                                      */
        for (j = n-1; j > 0; --j) {
            *A(0, j) = MAGMA_tally4_S_ZERO;
            for (i = j; i < n-1; ++i)
                *A(i, j) = *A(i, j - 1);
        }

        *A(0, 0) = MAGMA_tally4_S_ONE;
        for (i = 1; i < n-1; ++i)
            *A(i, 0) = MAGMA_tally4_S_ZERO;
        
        if (n > 1) {
            /* Generate Q(2:n,2:n) */
            magma_tally4_sorgqr(n-1, n-1, n-1, A(1, 1), lda, tau, dT, nb, &iinfo);
        }
    }
