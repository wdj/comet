    ...
    if ( !MAGMA_tally2_D_EQUAL(*tau, MAGMA_tally2_D_ZERO) ) {
        if(thblk<step){    
            ...
            for( int j = tx+1; j < m; j += BLOCK_SIZE ){
                tmp +=  MAGMA_tally2_D_CNJG( v[j] ) * dc[j];
            }
            sum[tx] = tmp;
            magma_tally2_sum_reduce< BLOCK_SIZE >( tx, sum );
            #if defined (use_gemm_larft)
            *(T+thblk) = MAGMA_tally2_D_CNJG(sum[0]);
            #else
            tmp = - MAGMA_tally2_D_CNJG(*tau) * sum[0]; 
            *(T+thblk) = MAGMA_tally2_D_CNJG(tmp); // T = - tau(tx) * V(tx:n,1:tx-1)' * V(tx:n,tx) = tmp'
            //*(T+thblk) = - MAGMA_tally2_D_CNJG(sum[0]) * (*tau); // T = - tau(tx) * V(tx:n,1:tx-1)' * V(tx:n,tx) = tmp'
            #endif
        }
        else{
            #if defined (use_gemm_larft)
            *(T+thblk) = MAGMA_tally2_D_ONE;
            #else
            *(T+thblk) = *tau;
            #endif
        }
    ...
    double *tau)
{
    ...
    dlarft_gemvcolwise_kernel<<< grid, threads, 0, magma_tally2_stream >>>( m, v, ldv, tau, T, ldt, step);
    ...
    double *W, double* sdata)
{
    ...
    if(tx < dgemv_bs)
    {
        ...
        {
            res += MAGMA_tally2_D_CNJG (v_ptr[s]) * x_ptr[s*incx];
        }
    ...
    magma_tally2_sum_reduce<dgemv_bs>(tx, &(sdata[ty*dgemv_bs+0]));
    ...
    double *W)
{
    ...
    dlarft_gemvrowwise_kernel <<< grid, threads, sizeof(double)*dgemv_bs*(i+1), magma_tally2_stream>>>(m, i, tau, v, ldv, T, ldt);
}
//===================================================================================================
extern "C"
    ...
    double *T, int ldt)
{
    ...
    dlarft_gemv_loop_inside_kernel<<<grid, threads, sizeof(double) * (dgemv_bs*(k+1)), magma_tally2_stream>>>(n, k, tau, v, ldv, T, ldt); 
}
//===================================================================================================
extern "C"
    ...
    double *Tout, magma_tally2_int_t ldtout)
{
    ...
    dlarft_dtrmv_sm32x32_kernel <<< grid, threads, sizeof(double)*(m*m), magma_tally2_stream >>> (m, n,  tau, Tin, ldtin, Tout, ldtout);
}
//===================================================================================================
extern "C"
    ...
    double *Ttri, magma_tally2_int_t ldttri)
{
    ...
    dim3 grid(1);
    dim3 threads(max(m,1), 1, 1);
    dlarft_recdtrmv_sm32x32_kernel <<< grid, threads, sizeof(double)*(m*n), magma_tally2_stream >>> (m, n,  tau, Trec, ldtrec, Ttri, ldttri);
}
//===================================================================================================
extern "C"
