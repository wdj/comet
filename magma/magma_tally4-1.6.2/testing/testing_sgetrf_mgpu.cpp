...
void init_matrix( int m, int n, float *h_A, magma_tally4_int_t lda )
{
...
float get_LU_error(magma_tally4_int_t M, magma_tally4_int_t N,
                    float *LU, magma_tally4_int_t lda,
                    magma_tally4_int_t *ipiv)
{
...
int main( int argc, char** argv )
{
    ...
    float           error;
    float *h_A;
    magma_tally4Float_ptr d_lA[ Magma_tally4MaxGPUs ];
    magma_tally4_int_t *ipiv;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            /* ====================================================================
               ...
               =================================================================== */
            init_matrix( M, N, h_A, lda );
            magma_tally4_ssetmatrix_1D_col_bcyclic( M, N, h_A, lda, d_lA, ldda, ngpu, nb );
    
            gpu_time = magma_tally4_wtime();
            magma_tally4_sgetrf_mgpu( ngpu, M, N, d_lA, ldda, ipiv, &info );
            gpu_time = magma_tally4_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally4_sgetrf_mgpu returned error %d: %s.\n",
                       (int) info, magma_tally4_strerror( info ));
                       
            magma_tally4_sgetmatrix_1D_col_bcyclic( M, N, d_lA, ldda, h_A, lda, ngpu, nb );
