...
/**
    ...
    @ingroup magma_tally4_dposv_driver
    ********************************************************************/
    ...
    magma_tally4_int_t *info)
{
    ...
    *info = 0;
    ...
    Anrm = magma_tally4blas_dlansy(Magma_tally4InfNorm, uplo, n, dA, ldda, (double*)dworkd );
    cte  = Anrm * eps * pow((double)n, 0.5) * BWDMAX;

    /*
     * Convert to single precision
     */
    magma_tally4blas_dlag2s( n, nrhs, dB, lddb, dSX, lddsx, info );
    if (*info != 0) {
        *iter = -2;
        goto FALLBACK;
    }

    magma_tally4blas_dlat2s( uplo, n, dA, ldda, dSA, lddsa, info );
    if (*info != 0) {
        *iter = -2;
        goto FALLBACK;
    }
    ...
    magma_tally4_spotrf_gpu( uplo, n, dSA, lddsa, info );
    if (*info != 0) {
        *iter = -3;
        goto FALLBACK;
    }
    ...
    magma_tally4_spotrs_gpu( uplo, n, nrhs, dSA, lddsa, dSX, lddsx, info );
    ...
    if ( nrhs == 1 ) {
        magma_tally4_dsymv( uplo, n,
                     c_neg_one, dA, ldda,
                                dX, 1,
                     c_one,     dR, 1 );
    }
    else {
        magma_tally4_dsymm( Magma_tally4Left, uplo, n, nrhs,
                     c_neg_one, dA, ldda,
                                dX, lddx,
                     c_one,     dR, lddr );
    }
...
REFINEMENT:
    for( iiter=1; iiter < ITERMAX; ) {
        *info = 0;
        ...
        magma_tally4blas_dlag2s( n, nrhs, dR, lddr, dSX, lddsx, info );
        if (*info != 0) {
            *iter = -2;
            goto FALLBACK;
        }
...
FALLBACK:
    ...
    magma_tally4_dpotrf_gpu( uplo, n, dA, ldda, info );
    if (*info == 0) {
        magma_tally4blas_dlacpy( Magma_tally4UpperLower, n, nrhs, dB, lddb, dX, lddx );
        magma_tally4_dpotrs_gpu( uplo, n, nrhs, dA, ldda, dX, lddx, info );
    }
