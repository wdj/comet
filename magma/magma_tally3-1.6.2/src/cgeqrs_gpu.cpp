...
/**
    ...
    tau     COMPLEX array, dimension (N)
            ...
            reflector H(i), as returned by MAGMA_tally3_CGEQRF_GPU.
    ...
    dT      COMPLEX array that is the output (the 6th argument)
            of magma_tally3_cgeqrf_gpu of size
    ...
    @ingroup magma_tally3_cgels_comp
    ********************************************************************/
    ...
    magma_tally3_int_t *info)
{
    ...
    *info = 0;
    ...
    /* B := Q' * B */
    magma_tally3_cunmqr_gpu( Magma_tally3Left, Magma_tally3_ConjTrans,
                      m, nrhs, n,
                      dA(0,0), ldda, tau,
    ...
    // TODO: this assumes that, on exit from magma_tally3_cunmqr_gpu, hwork contains
    ...
    // magma_tally3_ctrsm and drop the csetmatrix.
    if ( nrhs == 1 ) {
        blasf77_ctrsv( Magma_tally3UpperStr, Magma_tally3NoTransStr, Magma_tally3NonUnitStr,
                       &ib, hwork,         &rows,
                            hwork+rows*ib, &ione);
    } else {
        blasf77_ctrsm( Magma_tally3LeftStr, Magma_tally3UpperStr, Magma_tally3NoTransStr, Magma_tally3NonUnitStr,
                       &ib, &nrhs,
                       &c_one, hwork,         &rows,
                               hwork+rows*ib, &rows);
    }
    ...
    magma_tally3_csetmatrix( ib, nrhs, hwork+rows*ib, rows, dwork+i, lddwork );

    // update c
    if (nrhs == 1)
        magma_tally3_cgemv( Magma_tally3NoTrans, i, ib,
    ...
    if (nb < k) {
        for (i = start; i >= 0; i -= nb) {
            ...
            if (i + ib < n) {
                if (nrhs == 1) {
                    magma_tally3_cgemv( Magma_tally3NoTrans, ib, ib,
                                 c_one,  dT(i), ib,
                                         dB+i,      1,
                                 c_zero, dwork+i,  1);
                    magma_tally3_cgemv( Magma_tally3NoTrans, i, ib,
    ...
    magma_tally3_ccopymatrix( (n), nrhs,
                       dwork, lddwork,
                       dB,    lddb );
