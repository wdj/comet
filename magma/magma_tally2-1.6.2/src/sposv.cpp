...
/**
    ...
    @ingroup magma_tally2_sposv_driver
    ********************************************************************/
    ...
    magma_tally2_int_t *info )
{
    ...
    *info = 0;
    ...
    ldda = ((n+31)/32)*32;
    lddb = ldda;
    if ( MAGMA_tally2_SUCCESS != magma_tally2_smalloc( &dA, ldda*n )) {
        goto CPU_INTERFACE;
    }
    if ( MAGMA_tally2_SUCCESS != magma_tally2_smalloc( &dB, lddb*nrhs )) {
        magma_tally2_free( dA );
        goto CPU_INTERFACE;
    }
    magma_tally2_ssetmatrix( n, n, A, lda, dA, ldda );
    magma_tally2_spotrf_gpu( uplo, n, dA, ldda, info );
    ...
    if ( *info == 0 ) {
        magma_tally2_ssetmatrix( n, nrhs, B, ldb, dB, lddb );
        magma_tally2_spotrs_gpu( uplo, n, nrhs, dA, ldda, dB, lddb, info );
        magma_tally2_sgetmatrix( n, nrhs, dB, lddb, B, ldb );
    }
    magma_tally2_free( dA );
    magma_tally2_free( dB );
    return *info;
...
CPU_INTERFACE:
    ...
    magma_tally2_spotrf( uplo, n, A, lda, info );
    if ( *info == 0 ) {
        lapackf77_spotrs( lapack_uplo_const_tally2(uplo), &n, &nrhs, A, &lda, B, &ldb, info );
    }
