    ...
    magma_tally4_int_t *info)
{
    ...
    magma_tally4_int_t pm_old=0, pn_old=0, indi_old=0, indj_old=0;
    ...
    *info = 0;
    ...
    if (MAGMA_tally4_SUCCESS != magma_tally4_cmalloc( &dA, (n + 2*nb)*ldda )) {
    ...
    magma_tally4FloatComplex *dwork = dA + n*ldda;
    magma_tally4FloatComplex *dW    = dwork + nb*ldda;
    ...
    magma_tally4FloatComplex *hT = work + lwork - nb*nb;
    lwork -= nb*nb;
    memset( hT, 0, nb*nb*sizeof(magma_tally4FloatComplex));

    magma_tally4blasSetKernelStream( stream[0] );
    magma_tally4_event_t Pupdate_event;
    cudaEventCreateWithFlags(&Pupdate_event,cudaEventDisableTiming);
    //magma_tally4_event_create(&Pupdate_event);
    ...
    if (upper) {
        ...
        if (1 <= n-nb) {
            ...
            magma_tally4_csetmatrix_async( (n-nb), (n-nb),
                                    A(nb+1, nb+1),  lda,
        ...
        for (i = 1; i <= n-nb; i += nb) {
             ...
             if (i > 1 ) {
                 // cpanel_to_q_tally4 copy the upper oof diagonal part of
                 ...
                 cpanel_to_q_tally4(Magma_tally4Upper, pn-1, A(i, i+1), lda, work);

                 trace_gpu_start( 0, 1, "get", "get panel" );
                 //magma_tally4_queue_sync( stream[0] );
                 magma_tally4_queue_wait_event(stream[1], Pupdate_event);  //, 0);
                 magma_tally4_cgetmatrix_async( (pm+pn), pn,
                                         dA( i, i), ldda,
                                         A ( i, i), lda, stream[1] );
                 trace_gpu_end( 0, 1 );

                 trace_gpu_start( 0, 2, "her2k", "her2k" );
                 magma_tally4_cher2k(Magma_tally4Lower, Magma_tally4NoTrans, pm_old-pn_old, pn_old, c_neg_one,
                 ...
                 trace_cpu_end( 0 );
                 cq_to_panel_tally4(Magma_tally4Upper, pn-1, A(i, i+1), lda, work);
             }
             ...
             cpanel_to_q_tally4(Magma_tally4Upper, pk, A(indi, indj), lda, work);
             trace_cpu_end( 0 );

             /* Send V from the CPU to the GPU */
             trace_gpu_start( 0, 0, "set", "set V and T" );
             magma_tally4_csetmatrix_async( pm, pk,
                                     A(indi, indj),  lda,
                                     dA(indi, indj), ldda, stream[0] );

             /* Send the triangular factor T to the GPU */
             magma_tally4_csetmatrix_async( pk, pk,
                                     hT,       nb,
                                     dT(i), lddt, stream[0] );
             trace_gpu_end( 0, 0 );
             ...
             // because below we made a restore cq_to_panel_tally4 and this restore need
             ...
             magma_tally4_cgemm(Magma_tally4NoTrans, Magma_tally4NoTrans, pm, pk, pk,
             ...
             magma_tally4_chemm(Magma_tally4Left, uplo, pm, pk,
                         c_one, dA(indi, indi), ldda,
                         dwork, pm,
                         c_zero, dW, pm);
             trace_gpu_end( 0, 2 );
             /* restore the panel */
             cq_to_panel_tally4(Magma_tally4Upper, pk, A(indi, indj), lda, work);
             
             /* dwork = V*T already ==> dwork' = T'*V'
              * compute T'*V'*X ==> dwork'*W ==>
              * dwork + pm*nb = ((T' * V') * X) = dwork' * X = dwork' * W */
             trace_gpu_start( 0, 2, "gemm", "work = T'*V'*X" );
             magma_tally4_cgemm(Magma_tally4ConjTrans, Magma_tally4NoTrans, pk, pk, pm,
                         c_one, dwork, pm,
                         dW, pm,
             ...
             if (i + nb <= n-nb) {
                 ...
                 magma_tally4_cgemm(Magma_tally4NoTrans, Magma_tally4ConjTrans, pm, pn, pn, c_neg_one,
                             dA(indi, indj), ldda,
                             dW,                 pm, c_one,
                             dA(indi, indi), ldda);
                 trace_gpu_end( 0, 2 );
                 ...
                 magma_tally4_cgemm(Magma_tally4NoTrans, Magma_tally4ConjTrans, pm, pn, pn, c_neg_one,
                             ...
                             dA(indi, indi), ldda);
                 trace_gpu_end( 0, 2 );
                 magma_tally4_event_record(Pupdate_event, stream[0]);
             }
             else {
                 /* no look-ahead as this is last iteration */
                 trace_gpu_start( 0, 2, "her2k", "her2k last iteration" );
                 magma_tally4_cher2k(Magma_tally4Lower, Magma_tally4NoTrans, pk, pk, c_neg_one,
                              dA(indi, indj), ldda,
                              dW,                 pm, d_one,
                              dA(indi, indi), ldda);
                 trace_gpu_end( 0, 2 );
        ...
        pk = min(pm,pn);
        if (1 <= n-nb) {
            cpanel_to_q_tally4(Magma_tally4Upper, pk-1, A(n-pk+1, n-pk+2), lda, work);
            trace_gpu_start( 0, 2, "get", "get last block" );
            magma_tally4_cgetmatrix( pk, pk,
                              dA(n-pk+1, n-pk+1), ldda,
                              A(n-pk+1, n-pk+1),  lda );
            trace_gpu_end( 0, 2 );
            cq_to_panel_tally4(Magma_tally4Upper, pk-1, A(n-pk+1, n-pk+2), lda, work);
        }
    }// end of LOWER
    
    trace_finalize( "chetrd_he2hb.svg", "trace.css" );

    magma_tally4_event_destroy( Pupdate_event );
