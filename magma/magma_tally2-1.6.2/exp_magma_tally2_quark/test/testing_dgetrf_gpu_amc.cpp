...
double get_LU_error(magma_tally2_int_t M, magma_tally2_int_t N,
                    ...
                    double *LU, magma_tally2_int_t *IPIV)
{
...
int main( int argc, char** argv)
{
    ...
    magma_tally2_int_t lapack_numthreads = 1;
    double *WORK;
    ...
    d_cpu = magma_tally2_amc_recommanded_dcpu(opts.nthread, CPU_PEAK, 1, GPU_PEAK); //assume 1 GPU
    #endif
    ...
    for( int i = 0; i < opts.ntest; ++i ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_tally2_dsetmatrix( M, N, h_R, lda, d_A, ldda );
            gpu_time1 = magma_tally2_wtime();
            magma_tally2_dgetrf_gpu( M, N, d_A, ldda, ipiv, &info);
            gpu_time1 = magma_tally2_wtime() - gpu_time1;
            gpu_perf1 = gflops / gpu_time1;
            if (info != 0)
                printf("\nmagma_tally2_dgetrf_gpu returned error %d: %s.\n     ",
                       (int) info, magma_tally2_strerror( info ));
            
            /* ====================================================================
               Performs operation using MAGMA_tally2_ASYNC
               =================================================================== */
            /*Save BLAS number of threads*/
            /*
            lapack_numthreads = magma_tally2_getlapack_numthreads();
            if(lapack_numthreads !=1){ 
                //Temporary set the number of threads to 1.
                printf("MAGMA_tally2 num threads set from %d to %d\n", lapack_numthreads, 1);
            ...
            magma_tally2_dgetrf_gpu_amc( M, N, d_A, ldda, ipiv, &info);
            gpu_time2 = magma_tally2_wtime() - gpu_time2;
            gpu_perf2 = gflops / gpu_time2;
            magma_tally2_amc_finalize();
            if (info != 0)
                printf("\nmagma_tally2_amc_dgetrf_rec_gpu returned error %d: %s.\n     ",
                       (int) info, magma_tally2_strerror( info ));
            
            /* ====================================================================
               Performs operation using MAGMA_tally2_ASYNC..._WORK (Workspace preallocated version)
               =================================================================== */
            ...
            magma_tally2_dgetrf_gpu_work_amc(M, N, d_A, ldda, ipiv, &info, WORK, WORK_LD, WORK_n);
            gpu_time3 = magma_tally2_wtime() - gpu_time3;
            gpu_perf3 = gflops / gpu_time3;
            magma_tally2_amc_finalize();

            if (info != 0)
                printf("magma_tally2_amc_dgetrf_rec_work_gpu returned error %d: %s.\n     ",
                       (int) info, magma_tally2_strerror( info ));
            ...
            if(lapack_numthreads !=1){ 
                /*Temporary set the number of threads to 1.*/
                printf("MAGMA_tally2 num threads set from %d to %d\n", 1, lapack_numthreads);
                magma_tally2_setlapack_numthreads(lapack_numthreads);
            }
            ...
            if ( opts.check ) {
                magma_tally2_dgetmatrix( M, N, d_A, ldda, h_A, lda );
