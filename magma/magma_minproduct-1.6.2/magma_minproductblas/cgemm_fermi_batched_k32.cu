    ...
    magma_minproductFloatComplex **dC_array, magma_minproduct_int_t lddc, magma_minproduct_int_t batchCount, magma_minproduct_queue_t queue )
{
    ...
    if      ( transB == Magma_minproductTrans )
        ...
        magma_minproductFloatComplex *cpuAarray[2], *cpuBarray[2];
        magma_minproduct_getvector( min(2,batchCount), sizeof(magma_minproductFloatComplex*), dA_array, 1, cpuAarray, 1);
        magma_minproduct_getvector( min(2,batchCount), sizeof(magma_minproductFloatComplex*), dB_array, 1, cpuBarray, 1);
        ...
        cudaError_t err;
        err = cudaBindTexture(&offsetA, tex_ref_A, cpuAarray[0], sizeA*sizeof(magma_minproductFloatComplex));
        if ( err != cudaSuccess ) {
            fprintf( stderr, "cannot bind A to texture: %s (%d)\n", cudaGetErrorString(err), err );
            return;
        }	
        err = cudaBindTexture(&offsetB, tex_ref_B, cpuBarray[0], sizeB*sizeof(magma_minproductFloatComplex));
        if ( err != cudaSuccess ) {
    ...
    offsetA = offsetA/sizeof(magma_minproductFloatComplex);
    offsetB = offsetB/sizeof(magma_minproductFloatComplex);
