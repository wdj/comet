/*
    -- MAGMA_tally4 (version 1.6.1) --
       ...
       @generated from testing_zposv_gpu.cpp normal z -> c, Fri Jan 30 19:00:24 2015
*/
// includes, system
#include <stdio.h>
#include <stdlib.h>
...
int main( int argc, char** argv)
{
    ...
    magma_tally4_int_t N, lda, ldb, ldda, lddb, info, sizeA, sizeB;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_B, magma_tally4FloatComplex, ldb*opts.nrhs );
            TESTING_MALLOC_CPU( h_X, magma_tally4FloatComplex, ldb*opts.nrhs );
            TESTING_MALLOC_CPU( work, float, N );
            
            TESTING_MALLOC_DEV( d_A, magma_tally4FloatComplex, ldda*N         );
            TESTING_MALLOC_DEV( d_B, magma_tally4FloatComplex, lddb*opts.nrhs );
            ...
            magma_tally4_csetmatrix( N, N,         h_A, N, d_A, ldda );
            magma_tally4_csetmatrix( N, opts.nrhs, h_B, N, d_B, lddb );
            
            /* ====================================================================
               Performs operation using MAGMA_tally4
               =================================================================== */
            gpu_time = magma_tally4_wtime();
            magma_tally4_cposv_gpu( opts.uplo, N, opts.nrhs, d_A, ldda, d_B, lddb, &info );
            gpu_time = magma_tally4_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally4_cpotrf_gpu returned error %d: %s.\n",
                       (int) info, magma_tally4_strerror( info ));

            /* =====================================================================
               Residual
               =================================================================== */
            magma_tally4_cgetmatrix( N, opts.nrhs, d_B, lddb, h_X, ldb );
            
            Anorm = lapackf77_clange("I", &N, &N,         h_A, &lda, work);
            Xnorm = lapackf77_clange("I", &N, &opts.nrhs, h_X, &ldb, work);
            
            blasf77_cgemm( Magma_tally4NoTransStr, Magma_tally4NoTransStr, &N, &opts.nrhs, &N,
                           &c_one,     h_A, &lda,
                                       h_X, &ldb,
                           &c_neg_one, h_B, &ldb );
            ...
            if ( opts.lapack ) {
                ...
                lapackf77_cposv( lapack_uplo_const_tally4(opts.uplo), &N, &opts.nrhs, h_A, &lda, h_B, &ldb, &info );
