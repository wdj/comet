...
/**
    Purpose
    -------
    If VECT = Magma_tally2Q, SORMBR overwrites the general real M-by-N matrix C with
                                 SIDE = Magma_tally2Left     SIDE = Magma_tally2Right
    TRANS = Magma_tally2NoTrans:        Q*C                  C*Q
    TRANS = Magma_tally2Trans:     Q**H*C               C*Q**H
    
    If VECT = Magma_tally2P, SORMBR overwrites the general real M-by-N matrix C with
                                 SIDE = Magma_tally2Left     SIDE = Magma_tally2Right
    TRANS = Magma_tally2NoTrans:        P*C                  C*P
    TRANS = Magma_tally2Trans:     P**H*C               C*P**H
    ...
    trans   magma_tally2_trans_t
      ...
      -     = Magma_tally2Trans: Conjugate transpose, apply Q**H or P**H.
    ...
    *info = 0;
    ...
    if (*info == 0) {
        if (nw > 0) {
            ...
            nb = magma_tally2_get_sgebrd_nb( min( m, n ));
            lwkopt = max(1, nw*nb);
        }
        else {
            lwkopt = 1;
    ...
    if (applyq) {
        ...
        if (nq >= k) {
            ...
            #if VERSION == 1
            lapackf77_sormqr( lapack_side_const_tally2(side), lapack_trans_const_tally2(trans),
                              &m, &n, &k, A, &lda, tau, C, &ldc, work, &lwork, &iinfo);
            #else
            magma_tally2_sormqr( side, trans,
                          m, n, k, A, lda, tau, C, ldc, work, lwork, &iinfo);
            #endif
        }
        else if (nq > 1) {
        ...
        if (nq > k) {
            ...
            #if VERSION == 1
            lapackf77_sormlq( lapack_side_const_tally2(side), lapack_trans_const_tally2(transt),
                              &m, &n, &k, A, &lda, tau, C, &ldc, work, &lwork, &iinfo);
            #else
            magma_tally2_sormlq( side, transt,
                          m, n, k, A, lda, tau, C, ldc, work, lwork, &iinfo);
            #endif
        }
        else if (nq > 1) {
