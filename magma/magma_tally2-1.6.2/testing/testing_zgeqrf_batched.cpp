...
void get_QR_error(magma_tally2_int_t M, magma_tally2_int_t N, magma_tally2_int_t min_mn,
                    magma_tally2DoubleComplex *h_R,  magma_tally2DoubleComplex *h_A, magma_tally2_int_t lda,
                    magma_tally2DoubleComplex *tau,
                    magma_tally2DoubleComplex *Q,  magma_tally2_int_t ldq,
                    magma_tally2DoubleComplex *R,  magma_tally2_int_t ldr,
                    magma_tally2DoubleComplex *h_work,  magma_tally2_int_t lwork,
...
int main( int argc, char** argv)
{
    ...
    double           magma_tally2_error=0.0, cublas_error=0.0, magma_tally2_error2=0.0, cublas_error2=0.0, error, error2;

    magma_tally2DoubleComplex *h_A, *h_R, *h_Amagma_tally2, *tau, *h_work, tmp[1];
    magma_tally2DoubleComplex *d_A, *dtau_magma_tally2, *dtau_cublas;

    magma_tally2DoubleComplex **dA_array = NULL;
    magma_tally2DoubleComplex **dtau_array = NULL;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( tau,   magma_tally2DoubleComplex, min_mn * batchCount );
            ...
            TESTING_MALLOC_DEV( dtau_magma_tally2,  magma_tally2DoubleComplex, min_mn * batchCount);
            TESTING_MALLOC_DEV( dtau_cublas,  magma_tally2DoubleComplex, min_mn * batchCount);
            ...
            TESTING_MALLOC_CPU( h_work, magma_tally2DoubleComplex, lwork * batchCount);
			...
			lapackf77_zlacpy( Magma_tally2UpperLowerStr, &M, &column, h_A, &lda, h_R, &lda );
       
            /* ====================================================================
               Performs operation using MAGMA_tally2
               =================================================================== */
            magma_tally2_zsetmatrix( M, column, h_R, lda,  d_A, ldda );
            zset_pointer(dA_array, d_A, 1, 0, 0, ldda*N, batchCount, opts.queue);
            zset_pointer(dtau_array, dtau_magma_tally2, 1, 0, 0, min_mn, batchCount, opts.queue);
    
            magma_tally2_time = magma_tally2_sync_wtime(0);
    
            info = magma_tally2_zgeqrf_batched(M, N, dA_array, ldda, dtau_array, dinfo_magma_tally2, batchCount, opts.queue);

            magma_tally2_time = magma_tally2_sync_wtime(0) - magma_tally2_time;
            magma_tally2_perf = gflops / magma_tally2_time;

            magma_tally2_zgetmatrix( M, column, d_A, ldda, h_Amagma_tally2, lda);

            if (info != 0)
                printf("magma_tally2_zgeqrf_batched returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                magma_tally2_zgetvector(min_mn*batchCount, dtau_magma_tally2, 1, tau, 1);
                for (int i=0; i < batchCount; i++)
                {
                ...
                #if CUDA_VERSION >= 6050
                magma_tally2_zgetvector(min_mn*batchCount, dtau_magma_tally2, 1, tau, 1);
                magma_tally2_zgetmatrix( M, column, d_A, ldda, h_A, lda);
                for (int i=0; i < batchCount; i++)
                {
                    get_QR_error(M, N, min_mn,
