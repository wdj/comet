...
int main(int argc, char **argv)
{
    ...
    magma_tally2FloatComplex *A, *X, *Y, *Yatomics, *Ycublas, *Ymagma_tally2;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( Yatomics, magma_tally2FloatComplex, sizeY );
            TESTING_MALLOC_CPU( Ycublas,  magma_tally2FloatComplex, sizeY );
            ...
            cublasChemv( opts.handle, cublas_uplo_const_tally2(opts.uplo),
                         N, &alpha, dA, ldda, dX, incx, &beta, dY, incy );
            cublas_time = magma_tally2_sync_wtime( 0 ) - cublas_time;
            cublas_perf = gflops / cublas_time;
            
            magma_tally2_cgetvector( N, dY, incy, Ycublas, incy );
            ...
            magma_tally2_cgetvector( N, dY, incy, Yatomics, incy );
            ...
            if ( opts.version == 1 ) {
                magma_tally2blas_chemv_work( opts.uplo, N, alpha, dA, ldda, dX, incx, beta, dY, incy, dwork, ldwork, opts.queue );
            }
            else {
                // non-work interface (has added overhead)
                magma_tally2blas_chemv( opts.uplo, N, alpha, dA, ldda, dX, incx, beta, dY, incy );
            }
            ...
            blasf77_chemv( lapack_uplo_const_tally2(opts.uplo), &N, &alpha, A, &lda, X, &incx, &beta, Y, &incy );
