...
int main( int argc, char** argv)
{
    ...
    magma_tally4DoubleComplex *h_A, *h_Q, *h_PT, *h_work;
    magma_tally4DoubleComplex *taup, *tauq;
    double      *diag, *offdiag;
    double      eps, result[3] = {0., 0., 0.};
    magma_tally4_int_t M, N, n2, lda, lhwork, info, minmn, nb;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            N = opts.nsize[itest];
            minmn  = min(M, N);
            nb     = magma_tally4_get_zgebrd_nb(N);
            lda    = M;
            n2     = lda*N;
            lhwork = (M + N)*nb;
            ...
            TESTING_MALLOC_CPU( tauq,    magma_tally4DoubleComplex, minmn );
            TESTING_MALLOC_CPU( taup,    magma_tally4DoubleComplex, minmn );
            TESTING_MALLOC_CPU( diag,    double, minmn   );
            TESTING_MALLOC_CPU( offdiag, double, minmn-1 );
            
            TESTING_MALLOC_PIN( h_Q,     magma_tally4DoubleComplex, lda*N  );
            TESTING_MALLOC_PIN( h_work,  magma_tally4DoubleComplex, lhwork );
            
            /* Initialize the matrices */
            lapackf77_zlarnv( &ione, ISEED, &n2, h_A );
            lapackf77_zlacpy( Magma_tally4UpperLowerStr, &M, &N, h_A, &lda, h_Q, &lda );
            
            /* ====================================================================
               Performs operation using MAGMA_tally4
               =================================================================== */
            gpu_time = magma_tally4_wtime();
            magma_tally4_zgebrd( M, N, h_Q, lda,
            ...
            if ( opts.check ) {
                ...
                lapackf77_zlacpy(Magma_tally4UpperLowerStr, &M, &N, h_Q, &lda, h_PT, &lda);
