...
/**
    ...
    This is an out-of-core (ooc) version that is similar to magma_tally4_cgeqrf but
    ...
    magma_tally4_int_t *info )
{
    ...
    magma_tally4FloatComplex *dA, *dwork;
    magma_tally4FloatComplex c_one = MAGMA_tally4_C_ONE;

    int  k, lddwork, ldda;

    *info = 0;
    int nb = magma_tally4_get_cgeqrf_nb(min(m, n));
    ...
    magma_tally4_int_t IB, NB = (magma_tally4_int_t)(0.8*freeMem/m);
    NB = (NB / nb) * nb;

    if (NB >= n)
        return magma_tally4_cgeqrf(m, n, A, lda, tau, work, lwork, info);
    ...
    if (MAGMA_tally4_SUCCESS != magma_tally4_cmalloc( &dA, (NB + nb)*ldda + nb*lddwork )) {
    ...
    magma_tally4FloatComplex *ptr = dA + ldda * NB;
    dwork = dA + ldda*(NB + nb);
    ...
    for (int i=0; i < n; i += NB) {
        ...
        magma_tally4_csetmatrix_async( (m), IB,
                                A(0,i),  lda,
        ...
        for (int j=0; j < min(i,k); j += nb) {
            magma_tally4_int_t ib = min(k-j, nb);
            ...
            cpanel_to_q_tally4(Magma_tally4Upper, ib, A(j,j), lda, work+ib*ib);
            ...
            cq_to_panel_tally4(Magma_tally4Upper, ib, A(j,j), lda, work+ib*ib);
        }

        /* 3. Do a QR on the current part */
        if (i < k)
            magma_tally4_cgeqrf2_gpu(m-i, IB, dA(i,0), ldda, tau+i, info);

        /* 4. Copy the current part back to the CPU */
        magma_tally4_cgetmatrix_async( (m), IB,
                                dA(0,0), ldda,
                                A(0,i),  lda, stream[0] );
    }
