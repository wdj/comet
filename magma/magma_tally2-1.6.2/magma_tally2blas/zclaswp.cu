...
__global__ void
zclaswp_kernel(int n, magma_tally2DoubleComplex *A, int lda, magma_tally2FloatComplex *SA, int m, const magma_tally2_int_t *ipiv)
{
    ...
    if (ind < m) {
        ...
        for(int i=0; i < n; i++) {
            res = MAGMA_tally2_C_MAKE( (float)cuCreal(A[newind+i*lda]),
...
__global__ void
zclaswp_inv_kernel(int n, magma_tally2DoubleComplex *A, int lda, magma_tally2FloatComplex *SA, int m, const magma_tally2_int_t *ipiv)
{
    int ind = blockIdx.x*NB + threadIdx.x;
    int newind;
    ...
    if (ind < m) {
        ...
        for(int i=0; i < n; i++) {
            res = MAGMA_tally2_Z_MAKE( (double)cuCrealf(SA[newind+i*lda]),
...
/**
    @see magma_tally2blas_zclaswp_q
    @ingroup magma_tally2_zaux2
    ********************************************************************/
extern "C" void
magma_tally2blas_zclaswp(
    ...
    const magma_tally2_int_t *ipiv, magma_tally2_int_t incx )
{
    magma_tally2blas_zclaswp_q( n, A, lda, SA, m, ipiv, incx, magma_tally2_stream );
}
