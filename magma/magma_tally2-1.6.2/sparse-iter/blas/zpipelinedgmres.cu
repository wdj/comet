...
__global__ void
magma_tally2_zpipelined_correction( 
    int n,  
    int k,
    ...
    magma_tally2DoubleComplex * v )
{
    ...
    temp[ i ] = ( i < k ) ? skp[ i ] * skp[ i ] : MAGMA_tally2_Z_MAKE( 0.0, 0.0);
    __syncthreads();
    ...
    if( i == 0 ){
        tmp = MAGMA_tally2_Z_REAL( temp[ i ] );
        zz = MAGMA_tally2_Z_REAL( skp[(k)] );
        skp[k] = MAGMA_tally2_Z_MAKE( sqrt(zz-tmp),0.0 );
    }
}

__global__ void
magma_tally2_zpipelined_copyscale( 
    int n,  
    int k,
    magma_tally2DoubleComplex * skp, 
    magma_tally2DoubleComplex * r,
    magma_tally2DoubleComplex * v )
{
    ...
    magma_tally2DoubleComplex rr=skp[k];
...
__global__ void
magma_tally2_zpipelineddznrm2_kernel( 
    int m, 
    magma_tally2DoubleComplex * da, 
    int ldda, 
    magma_tally2DoubleComplex * dxnorm )
{
    const int i = threadIdx.x;
    magma_tally2DoubleComplex_ptr dx = da + blockIdx.x * ldda;
    ...
    if (i==0)
        dxnorm[blockIdx.x] = MAGMA_tally2_Z_MAKE( sqrt(sum[0]), 0.0 );
}

//----------------------------------------------------------------------------//

__global__ void
magma_tally2_zpipelinedscale( 
    int n, 
    magma_tally2DoubleComplex * r, 
...
{
    dim3 Bs( BLOCK_SIZE );
    dim3 Gs( magma_tally2_ceildiv( k, BLOCK_SIZE ) );
    unsigned int Ms =   Bs.x * sizeof( magma_tally2DoubleComplex ); 

    dim3 Gs2( magma_tally2_ceildiv( n, BLOCK_SIZE ) );


    magma_tally2_zpipelined_correction<<<Gs, Bs, Ms, queue >>>
                                            ( n, k, skp, r, v );
    magma_tally2_zpipelined_copyscale<<<Gs2, Bs, 0, queue >>>
                                            ( n, k, skp, r, v );
...
magma_tally2_dznrm2scale(
    ...
    magma_tally2DoubleComplex_ptr r, 
    int lddr, 
    magma_tally2DoubleComplex_ptr drnorm,
    magma_tally2_queue_t queue )
{
    dim3  blocks( 1 );
    dim3 threads( 512 );
    magma_tally2_zpipelineddznrm2_kernel<<< blocks, threads, 0, queue >>>
                                ( m, r, lddr, drnorm );

    dim3 Bs( BLOCK_SIZE );
    dim3 Gs2( magma_tally2_ceildiv( m, BLOCK_SIZE ) );
    magma_tally2_zpipelinedscale<<<Gs2, Bs, 0, queue >>>( m, r, drnorm );
