...
/**
    ...
    lwork   INTEGER
            ...
            through magma_tally2_get_sgeqlf_nb(M).
    \n
    ...
    magma_tally2_int_t *info)
{
    ...
    magma_tally2Float_ptr dA, dwork;
    ...
    nb = magma_tally2_get_sgeqlf_nb(m);
    *info = 0;
    lquery = (lwork == -1);
    ...
    if (*info == 0) {
        if (k == 0)
            ...
            work[0] = MAGMA_tally2_S_MAKE( max(n*nb, 2*nb*nb), 0 );
        }
    ...
    if (MAGMA_tally2_SUCCESS != magma_tally2_smalloc( &dA, (n)*ldda + nb*lddwork )) {
    ...
    if ( (nb > 1) && (nb < k) ) {
        ...
        for (i = k - kk + ki; i >= k -kk; i -= nb) {
            ...
            if (i < k - kk + ki) {
                ...
                magma_tally2_sgetmatrix_async( m-rows, ib,
                                        dA(rows, n-k+i), ldda,
                ...
                magma_tally2_slarfb_gpu( Magma_tally2Left, Magma_tally2ConjTrans, Magma_tally2Backward, Magma_tally2Columnwise,
                                  rows, cols, old_ib,
            ...
            if (cols > 0) {
                ...
                lapackf77_slarft( Magma_tally2BackwardStr, Magma_tally2ColumnwiseStr,
                                  &rows, &ib,
                                  A(0, cols), &lda, tau + i, work, &ib);

                spanel_to_q_tally2( Magma_tally2Lower, ib, A(rows-ib,cols), lda, work+ib*ib);
                magma_tally2_ssetmatrix( rows, ib,
                                  A(0,cols),  lda,
                                  dA(0,cols), ldda );
                sq_to_panel_tally2( Magma_tally2Lower, ib, A(rows-ib,cols), lda, work+ib*ib);

                // Send the triangular part on the GPU
                magma_tally2_ssetmatrix( ib, ib, work, ib, dwork(0), lddwork );
        ...
        magma_tally2_sgetmatrix( m, nu, dA(0,0), ldda, A(0,0), lda );
    } else {
        mu = m;
        nu = n;
    }
