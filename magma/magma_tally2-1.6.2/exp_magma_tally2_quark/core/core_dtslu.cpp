...
if(!(tree_step = (int*) malloc(nbThreads*sizeof(int)))) magma_tally2_amc_abort("Memory allocation failed for tree_step"); /* private */

if(!(mutex_tree = (pthread_mutex_t*) malloc(nbThreads*sizeof(pthread_mutex_t)))) magma_tally2_amc_abort("Memory allocation failed for mutex_tree");
...
shared_LDT = max(shared_LDT,2*nb); /*At least 2 blocks needed for tslu*/
    ...
    /* tslu buffer */
if(!(shared_Tblock = (double*)    malloc(shared_LDT*nb*sizeof(double)))) magma_tally2_amc_abort("Memory allocation failed for shared_Tblock");

if(!(shared_Twork = (double*) malloc(shared_LDT*nb*sizeof(double)))) magma_tally2_amc_abort("Memory allocation failed for shared_Twork"); 

if(!(shared_IPwork =(int*)    malloc(shared_LDT*sizeof(int)))) magma_tally2_amc_abort("Memory allocation failed for shared_IPwork"); //descA[tid].M

if(!(shared_IP =  (int*)    malloc(shared_LDT*sizeof(int)))) magma_tally2_amc_abort("Memory allocation failed for shared_IP");
                ...
                printf("[%d] locNum:%d, tsluNum:%d update Global perm(step:%d)\n",tid, locNum,tslu_num, jblock);
                    ...
                    for(l=0;l<panel_MB;l++) //nb
                    {
                        ...
                        if(k>=M) 
                        {
                            ...
                            magma_tally2_amc_abort("***Error when computing permutation vector ! ");// Can never occur 
