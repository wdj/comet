...
/**
    ...
    ldwork  INTEGER
            ...
            LDWORK >= ldda*ceil(n/64) + 2*ldda*nb, where nb = magma_tally3_get_chetrd_nb(n),
            and 64 is for the blocksize of magma_tally3blas_chemv.
    ...
    magma_tally3_int_t *info)
{
    ...
    *info = 0;
    ...
    // rest of dwork (used as work in magma_tally3blas_chemv) doesn't need to be cleared
    magma_tally3blas_claset( Magma_tally3Full, n, nb, c_zero, c_zero, dwork, lddw );
    ...
    if (upper) {
        ...
        for (i = n - nb; i >= kk; i -= nb) {
            ...
            magma_tally3_cgetmatrix( i+nb, nb, dA(0, i), ldda, A(0, i), lda );
            
            magma_tally3_clatrd2( uplo, i+nb, nb, A(0, 0), lda, e, tau,
            ...
            magma_tally3_csetmatrix( i + nb, nb, work, ldw, dwork, lddw );
            
            magma_tally3_cher2k( uplo, Magma_tally3NoTrans, i, nb, c_neg_one,
        ...
        for (i = 0; i < n-nx; i += nb) {
            ...
            /* Get the current panel */
            magma_tally3_cgetmatrix( n-i, nb, dA(i, i), ldda, A(i, i), lda );
            
            magma_tally3_clatrd2( uplo, n-i, nb, A(i, i), lda, &e[i], &tau[i],
                           work, ldw, dA(i, i), ldda, dwork, lddw,
                           dwork + 2*lddw*nb, ldwork - 2*lddw*nb );
            
            /* Update the unreduced submatrix A(i+ib:n,i+ib:n), using
               an update of the form:  A := A - V*W' - W*V' */
            magma_tally3_csetmatrix( n-i, nb, work, ldw, dwork, lddw );
            
            // cublas 6.5 crashes here if lddw % 32 != 0, e.g., N=250.
            magma_tally3_cher2k( Magma_tally3Lower, Magma_tally3NoTrans, n-i-nb, nb, c_neg_one,
        ...
        /* Use CPU code to reduce the last or only block */
        magma_tally3_cgetmatrix( n-i, n-i, dA(i, i), ldda, A(i, i), lda );
        
        i_n = n-i;
        lapackf77_chetrd( uplo_, &i_n, A(i, i), &lda, &d[i], &e[i],
                          &tau[i], work, &lwork, &iinfo );
        
        magma_tally3_csetmatrix( n-i, n-i, A(i, i), lda, dA(i, i), ldda );
    }
