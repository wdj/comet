...
*/
void ssplit_diag_block(magma_minproduct_int_t ib, float *a, magma_minproduct_int_t lda, float *work)
{
    ...
    lapackf77_strtri( Magma_minproductUpperStr, Magma_minproductNonUnitStr, &ib, work, &ib, &info);
}
    ...
    *info = 0;
    ...
    if ( (nb > 1) && (nb < k) ) {
        ...
        for (i = 0; i < k-nb; i += nb) {
            ...
            magma_minproduct_ssetmatrix( rows, ib, work(i), ldwork, dA(i,i), ldda );

            if (i + ib < n) {
                /* Send the triangular factor T to the GPU */
                magma_minproduct_ssetmatrix( ib, ib, hwork, ib, dT(i), nb );
                ...
                if (i+nb < k-nb) {
                    ...
                    magma_minproduct_ssetmatrix( ib, ib, ut, ib, d_ref(i), ib );
                }
                old_i  = i;
                old_ib = ib;
            }
    ...
    if (i < k) {
        ib   = n-i;
        rows = m-i;
        magma_minproduct_sgetmatrix( rows, ib, dA(i, i), ldda, work, rows );
        lhwork = lwork - rows*ib;
        lapackf77_sgeqrf(&rows, &ib, work, &rows, tau+i, work+ib*rows, &lhwork, info);
        
        magma_minproduct_ssetmatrix( rows, ib, work, rows, dA(i, i), ldda );
    }
