...
int main( int argc, char** argv )
{
    ...
    float *h_A, *h_dinvA;
    magma_tally3Float_ptr d_A, d_dinvA;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_tally3blas_strtri_diag( opts.uplo, opts.diag, N, d_A, ldda, d_dinvA );
            magma_tally3_time = magma_tally3_sync_wtime( NULL ) - magma_tally3_time;
            magma_tally3_perf = gflops / magma_tally3_time;
            
            magma_tally3_sgetvector( size_inv, d_dinvA, 1, h_dinvA, 1 );
            
            if ( opts.verbose ) {
                printf( "A%d=", (int) N );
                magma_tally3_sprint( N, N, h_A, lda );
                printf( "d_dinvA%d=", (int) N );
                magma_tally3_sprint( min(N+4, nb), min(N+4, nblock*nb), h_dinvA, nb );
            }
            ...
            if ( opts.check ) {
                ...
                for( int i=0; i < N; i += nb ) {
                    ...
                    magma_tally3_error = max( magma_tally3_error, lapackf77_slantr( "M", uplo_, Magma_tally3NonUnitStr, &jb, &jb, h_dinvA(0, i), &nb,  work ));
                    norm_invA   = max( norm_invA,   lapackf77_slantr( "M", uplo_, Magma_tally3NonUnitStr, &jb, &jb, h_A(i, i),     &lda, work ));
                }
