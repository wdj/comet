    ...
    magma_minproduct_int_t resetozero, magma_minproduct_int_t batchCount, magma_minproduct_queue_t queue)
{
    ...
    if(resetozero)
    { 
        magma_minproductblas_zlaset_batched(Magma_minproductFull, ((n+NB-1)/NB)*NB, NB, MAGMA_minproduct_Z_ZERO, MAGMA_minproduct_Z_ZERO, dinvA_array, ((n+NB-1)/NB)*NB, batchCount, queue);
       //magma_minproductblas_zmemset_batched( dinvA_array, ((n+NB-1)/NB)*NB*NB, batchCount, queue);
    }
    ...
    //was not the largest size computed by the high API getrf_batched then it is bug and need to use magma_minproductblas_zlaset_batched
