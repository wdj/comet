...
__global__ void
zclaswp_kernel(int n, magma_minproductDoubleComplex *A, int lda, magma_minproductFloatComplex *SA, int m, const magma_minproduct_int_t *ipiv)
{
    ...
    if (ind < m) {
        ...
        for(int i=0; i < n; i++) {
            res = MAGMA_minproduct_C_MAKE( (float)cuCreal(A[newind+i*lda]),
...
__global__ void
zclaswp_inv_kernel(int n, magma_minproductDoubleComplex *A, int lda, magma_minproductFloatComplex *SA, int m, const magma_minproduct_int_t *ipiv)
{
    int ind = blockIdx.x*NB + threadIdx.x;
    int newind;
    ...
    if (ind < m) {
        ...
        for(int i=0; i < n; i++) {
            res = MAGMA_minproduct_Z_MAKE( (double)cuCrealf(SA[newind+i*lda]),
...
/**
    ...
    ********************************************************************/
extern "C" void
magma_minproductblas_zclaswp_q(
...
/**
    @see magma_minproductblas_zclaswp_q
    @ingroup magma_minproduct_zaux2
    ********************************************************************/
extern "C" void
magma_minproductblas_zclaswp(
    ...
    const magma_minproduct_int_t *ipiv, magma_minproduct_int_t incx )
{
    magma_minproductblas_zclaswp_q( n, A, lda, SA, m, ipiv, incx, magma_minproduct_stream );
}
