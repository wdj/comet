/*
    -- MAGMA_minproduct (version 1.6.1) --
       ...
       @generated from zlaswp.cu normal z -> c, Fri Jan 30 19:00:10 2015
...
__global__ void claswp_kernel(
    int n, magma_minproductFloatComplex *dAT, int ldda, claswp_params_t params )
{
    int tid = threadIdx.x + blockDim.x*blockIdx.x;
    if ( tid < n ) {
        dAT += tid;
        magma_minproductFloatComplex *A1  = dAT;
        
        for( int i1 = 0; i1 < params.npivots; ++i1 ) {
            int i2 = params.ipiv[i1];
            magma_minproductFloatComplex *A2 = dAT + i2*ldda;
            magma_minproductFloatComplex temp = *A1;
            *A1 = *A2;
            *A2 = temp;
...
/**
    ...
    ** Unlike LAPACK, here A is stored row-wise (hence dAT). **
    ...
    \param[in,out]
    dAT      COMPLEX array on GPU, stored row-wise, dimension (LDDA,N)
    ...
    magma_minproduct_queue_t queue )
{
    ...
    for( int k = k1-1; k < k2; k += MAX_PIVOTS ) {
        ...
        claswp_kernel<<< grid, threads, 0, queue >>>( n, dAT(k,0), ldda, params );
    }
...
/**
    @see magma_minproductblas_claswp_q
    @ingroup magma_minproduct_caux2
    ********************************************************************/
extern "C" void
magma_minproductblas_claswp(
    ...
    const magma_minproduct_int_t *ipiv, magma_minproduct_int_t inci )
{
    magma_minproductblas_claswp_q( n, dAT, ldda, k1, k2, ipiv, inci, magma_minproduct_stream );
}
...
__global__ void claswpx_kernel(
    int n, magma_minproductFloatComplex *dA, int ldx, int ldy, claswp_params_t params )
{
    int tid = threadIdx.x + blockDim.x*blockIdx.x;
    if ( tid < n ) {
        dA += tid*ldy;
        magma_minproductFloatComplex *A1  = dA;
        
        for( int i1 = 0; i1 < params.npivots; ++i1 ) {
            int i2 = params.ipiv[i1];
            magma_minproductFloatComplex *A2 = dA + i2*ldx;
            magma_minproductFloatComplex temp = *A1;
            *A1 = *A2;
            *A2 = temp;
            A1 += ldx;  // A1 = dA + i1*ldx
    ...
    magma_minproduct_queue_t queue )
{
    ...
    for( int k = k1-1; k < k2; k += MAX_PIVOTS ) {
        ...
        claswpx_kernel<<< grid, threads, 0, queue >>>( n, dA(k,0), ldx, ldy, params );
    }
...
/**
    @see magma_minproductblas_claswpx_q
    @ingroup magma_minproduct_caux2
    ********************************************************************/
extern "C" void
magma_minproductblas_claswpx(
    ...
    const magma_minproduct_int_t *ipiv, magma_minproduct_int_t inci )
{
    return magma_minproductblas_claswpx_q( n, dA, ldx, ldy, k1, k2, ipiv, inci, magma_minproduct_stream );
}
...
// batches of pivots. On Fermi, it is faster than magma_minproductblas_claswp
// (including copying pivots to the GPU).

__global__ void claswp2_kernel(
    int n, magma_minproductFloatComplex *dAT, int ldda, int npivots,
    const magma_minproduct_int_t *d_ipiv, int inci )
{
    int tid = threadIdx.x + blockDim.x*blockIdx.x;
    if ( tid < n ) {
        dAT += tid;
    ...
    magma_minproduct_queue_t queue )
{
    ...
    magma_minproduct_int_t nb = k2-(k1-1);
    
    dim3 grid( (n + NTHREADS - 1) / NTHREADS );
    dim3 threads( NTHREADS );
    claswp2_kernel<<< grid, threads, 0, queue >>>
        ( n, dAT(k1-1,0), ldda, nb, d_ipiv, inci );
}


/**
    @see magma_minproductblas_claswp2_q
    @ingroup magma_minproduct_caux2
    ********************************************************************/
extern "C" void
magma_minproductblas_claswp2(
    ...
    magma_minproductInt_const_ptr d_ipiv, magma_minproduct_int_t inci )
{
    magma_minproductblas_claswp2_q( n, dAT, ldda, k1, k2, d_ipiv, inci, magma_minproduct_stream );
}
