...
#define dTup(d, i, j) (   dt[(d)] + (j)*nb    + (i))
    ...
    double *dwork[Magma_minproductMaxGPUs], *dt[Magma_minproductMaxGPUs];
    ...
    *info = 0;
    ...
    for (d=0; d < ngpu; d++ ) {
        ...
        if (MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc( &dt[d], NB*lddla + max(2,ngpu)*nb*ldda )) {
    ...
    if (upper) {
        ...
        for( J=0; J < n; J += NB ) {
            ...
            magma_minproduct_dhtodpo( ngpu, uplo, JB, n, J, J, nb, A, lda, dwork, NB, stream, &iinfo);
            ...
            for( j=0; j < J; j += nb ) {
                ...
                for( d=0; d < ngpu; d++ ) {
                    magma_minproduct_setdevice(d);
                    magma_minproduct_dsetmatrix_async( nb, JB,
                                            A(j, J),       lda,
                                            dTup(d, 0, J), nb,
                                            stream[d][0] );
                    n_local[d] = 0;
                ...
                for( jj=0; jj < JB; jj += nb ) { // jj is 'local' column index within the big panel
                    ...
                    magma_minproduct_dsyrk(Magma_minproductUpper, Magma_minproductConjTrans, jb, nb,
            ...
            // using three streams
            magma_minproduct_dpotrf3_mgpu(ngpu, uplo, JB, n-J, J, J, nb,
            ...
            magma_minproduct_ddtohpo(ngpu, uplo, JB, n, J, J, nb, NB, A, lda, dwork, NB, stream, &iinfo);
            //magma_minproduct_ddtohpo(ngpu, uplo, JB, n, J, J, nb, 0, A, lda, dwork, NB, stream, &iinfo);
        }
    } else {
        /* ========================================================= *
        ...
        for( J=0; J < n; J += NB ) {
            ...
            magma_minproduct_dhtodpo( ngpu, uplo, n, JB, J, J, nb, A, lda, dwork, lddla, stream, &iinfo);
            ...
            for( j=0; j < J; j += nb ) {
                ...
                for( d=0; d < ngpu; d++ ) {
                    magma_minproduct_setdevice(d);
                    magma_minproduct_dsetmatrix_async( JB, nb,
                                            A(J, j),     lda,
                                            dT(d, J, 0), ldda,
                                            stream[d][0] );
                    n_local[d] = 0;
                ...
                for( jj=0; jj < JB; jj += nb ) { /* diagonal */
                    ...
                    magma_minproduct_dsyrk(Magma_minproductLower, Magma_minproductNoTrans, jb, nb,
            ...
            h = (JB+nb-1)/nb; // big diagonal of big panel will be on CPU
            // using three streams
            magma_minproduct_dpotrf3_mgpu(ngpu, uplo, n-J, JB, J, J, nb,
            ...
            magma_minproduct_ddtohpo( ngpu, uplo, n, JB, J, J, nb, JB, A, lda, dwork, lddla, stream, &iinfo);
