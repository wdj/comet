...
int main( int argc, char** argv)
{
    ...
    magma_tally4DoubleComplex *h_A, *h_B, *h_R;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            gbytes = sizeof(magma_tally4DoubleComplex) * 2.*M*N / 1e9;
            
            TESTING_MALLOC_CPU( h_A, magma_tally4DoubleComplex, lda*N  );  // input:  M x N
            TESTING_MALLOC_CPU( h_B, magma_tally4DoubleComplex, ldb*M  );  // output: N x M
            TESTING_MALLOC_CPU( h_R, magma_tally4DoubleComplex, ldb*M  );  // output: N x M
            
            TESTING_MALLOC_DEV( d_A, magma_tally4DoubleComplex, ldda*N );  // input:  M x N
            TESTING_MALLOC_DEV( d_B, magma_tally4DoubleComplex, lddb*M );  // output: N x M
            ...
            for( int j = 0; j < M; ++j ) {
                for( int i = 0; i < N; ++i ) {
                    h_B[i + j*ldb] = MAGMA_tally4_Z_MAKE( i + j/10000., j );
                }
            }
            magma_tally4_zsetmatrix( N, M, h_B, ldb, d_B, lddb );
            ...
            //magma_tally4blas_ztranspose( M-2, N-2, d_A+1+ldda, ldda, d_B+1+lddb, lddb );  // inset by 1 row & col
            magma_tally4blas_ztranspose( M, N, d_A, ldda, d_B, lddb );
            ...
            if ( M == N ) {
                ...
                //magma_tally4blas_ztranspose_inplace( N-2, d_A+1+ldda, ldda );  // inset by 1 row & col
                magma_tally4blas_ztranspose_inplace( N, d_A, ldda );
