    ...
    magma_minproductDoubleComplex **dC_array, magma_minproduct_int_t lddc, magma_minproduct_int_t batchCount, magma_minproduct_queue_t queue )
{
    ...
    if      ( transB == Magma_minproductTrans )
        ...
        magma_minproductDoubleComplex *cpuAarray[2], *cpuBarray[2];
        magma_minproduct_getvector( min(2,batchCount), sizeof(magma_minproductDoubleComplex*), dA_array, 1, cpuAarray, 1);
        magma_minproduct_getvector( min(2,batchCount), sizeof(magma_minproductDoubleComplex*), dB_array, 1, cpuBarray, 1);
        ...
        cudaError_t err;
        err = cudaBindTexture(&offsetA, tex_ref_A, cpuAarray[0], sizeA*sizeof(magma_minproductDoubleComplex));
        if ( err != cudaSuccess ) {
            fprintf( stderr, "cannot bind A to texture: %s (%d)\n", cudaGetErrorString(err), err );
            return;
        }	
        err = cudaBindTexture(&offsetB, tex_ref_B, cpuBarray[0], sizeB*sizeof(magma_minproductDoubleComplex));
        if ( err != cudaSuccess ) {
    ...
    offsetA = offsetA/sizeof(magma_minproductDoubleComplex);
    offsetB = offsetB/sizeof(magma_minproductDoubleComplex);
