...
void clag2z_kernel(
    int m, int n,
    const magma_tally3FloatComplex *SA, int ldsa,
    ...
    if ( ind < m ) {
        ...
        if ( full ) {
            ...
            for( int j=0; j < BLK_Y; ++j ) {
                A[j*lda] = MAGMA_tally3_Z_MAKE( MAGMA_tally3_C_REAL( SA[j*ldsa] ), MAGMA_tally3_C_IMAG( SA[j*ldsa] ));
            }
        }
        else {
...
magma_tally3blas_clag2z(
    ...
    magma_tally3_int_t *info)
{
    magma_tally3blas_clag2z_q( m, n, SA, ldsa, A, lda, info, magma_tally3_stream );
}
