    ...
    int block_offset )
{
    ...
    magma_minproductFloatComplex psum, psum_t;
    magma_minproductFloatComplex total = MAGMA_minproduct_C_ZERO;
    ...
    __shared__ magma_minproductFloatComplex sA [quarter_NB_X][NB_X + 2];  // TODO +3 used in chemv (single GPU); why?
    __shared__ magma_minproductFloatComplex sx_blk[NB_X];  // for x[ blk ]
    __shared__ magma_minproductFloatComplex sx_jj [NB_X];  // for x[ jj ], which cycles over all blocks left of diag

    magma_minproductFloatComplex rA[4];
    magma_minproductFloatComplex psums_t[4];
    ...
    if ( ty == 0 ) {
        ...
        if ( (partial && tx >= partial) ||
            ...
            sx_blk[tx] = MAGMA_minproduct_C_ZERO;
        }
        else {
            sx_blk[tx] = x[0];
        }
    ...
    if ( blk % ngpu == my_gpu_id ) {
        ...
        psum_t = MAGMA_minproduct_C_ZERO;
        #pragma unroll
        for(int j=0; j < 4; j++) {
    ...
    for(int jj=my_gpu_id; jj < blk; jj += ngpu) {
        ...
        if ( ty == 0 ) {
            if ( jj == 0 && tx < block_offset ) {
                sx_jj[tx] = MAGMA_minproduct_C_ZERO;
            }
            else {
...
/**************************************************************
    ...
    Note beta*y is not included here; see magma_minproductblas_chemv_mgpu_sync.
    ...
    int block_offset)
{
    ...
    if ( ind >= block_offset && ind < n+block_offset ) {
        magma_minproductFloatComplex Ax = MAGMA_minproduct_C_ZERO;
        ...
        y[ind * incy] = alpha*Ax;  // see magma_minproductblas_chemv_sync for beta*y
    }
}
...
/**
    Purpose
    -------
    magma_minproductblas_chemv_mgpu performs the matrix-vector operation:
    ...
    uplo    magma_minproduct_uplo_t.
            ...
            follows:
      -     = Magma_minproductUpper:  Only the upper triangular part of A is to be referenced. **Not currently supported.**
      -     = Magma_minproductLower:  Only the lower triangular part of A is to be referenced.
...
magma_minproductblas_chemv_mgpu(
    ...
    magma_minproductFloatComplex alpha,
    magma_minproductFloatComplex_const_ptr const d_lA[], magma_minproduct_int_t ldda,
    magma_minproduct_int_t offset,
    magma_minproductFloatComplex const *x,       magma_minproduct_int_t incx,
    magma_minproductFloatComplex beta,                                         // unused, see magma_minproductblas_chemv_mgpu_sync
    magma_minproductFloatComplex       *y,       magma_minproduct_int_t incy,             // unused
    ...
    magma_minproduct_queue_t queues[] )
{
    ...
    magma_minproduct_int_t offset_block_id = offset / NB_X;
    magma_minproduct_int_t offset_gpu_id   = offset_block_id % ngpu;
    magma_minproduct_int_t block_offset    = offset % NB_X;
    
    magma_minproduct_int_t blocks = ceildiv( n + block_offset, NB_X );
    magma_minproduct_int_t ldwmin = ldda*(blocks + 1);
    magma_minproduct_int_t lhwmin = n*ngpu;
    ...
    for(dev=0; dev < ngpu; dev++) {
        ...
        magma_minproduct_int_t num_blocks_skipped = offset_block_id / ngpu;
        if ( dev < offset_gpu_id ) {
            num_blocks_skipped += 1;
        }
        
        // shift dA to first block >= offset block that is owned by this GPU
        magma_minproductFloatComplex const *dA_dev    = d_lA[dev] + offset_block_id*NB_X + num_blocks_skipped*NB_X*ldda;
        ...
        magma_minproductFloatComplex       *dx_dev    = dwork[dev];
        magma_minproductFloatComplex       *dwork_dev = dwork[dev] + ldda;
        
        // renumber GPUs starting from the offset block
        magma_minproduct_int_t new_gpu_id = (dev + ngpu - offset_gpu_id) % ngpu;
        
        dim3 grid( blocks, 1 );
        
        // copy x to each GPU
        magma_minproduct_csetvector_async( n, x, incx, dx_dev + block_offset, 1, queues[dev] );
    ...
    for(dev=0; dev < ngpu; dev++) {
        ...
        magma_minproduct_setdevice( dev );
        magma_minproductFloatComplex       *dx_dev    = dwork[dev];
        magma_minproduct_cgetvector_async( n, dx_dev + block_offset, 1, &hwork[dev*n], 1, queues[dev] );
    }
    
    // see magma_minproductblas_chemv_mgpu_sync for final row sums
...
/**
    ...
    For convenience, the parameters are identical to magma_minproductblas_chemv_mgpu
    (though some are unused here).
    
    @see magma_minproductblas_chemv_mgpu
    @ingroup magma_minproduct_cblas2
    ********************************************************************/
extern "C" magma_minproduct_int_t
magma_minproductblas_chemv_mgpu_sync(
    magma_minproduct_uplo_t uplo,                                               // unused, see magma_minproductblas_chemv_mgpu
    magma_minproduct_int_t n,
    magma_minproductFloatComplex alpha,                                        // unused
    magma_minproductFloatComplex_const_ptr const d_lA[], magma_minproduct_int_t ldda,
    magma_minproduct_int_t offset,                                              // unused
    magma_minproductFloatComplex const *x,       magma_minproduct_int_t incx,             // unused
    magma_minproductFloatComplex beta,
    magma_minproductFloatComplex       *y,       magma_minproduct_int_t incy,             // unused
    magma_minproductFloatComplex       *hwork,   magma_minproduct_int_t lhwork,
    magma_minproductFloatComplex_ptr    dwork[], magma_minproduct_int_t ldwork,           // unused
    magma_minproduct_int_t ngpu,
    magma_minproduct_int_t nb,                                                  // unused
    magma_minproduct_queue_t queues[] )
{
     ...
     */
    magma_minproduct_int_t info = 0;
    //if ( (! upper) && (uplo != Magma_minproductLower) ) {  // unused
    //    info = -1;
    //} else
    if ( n < 0 ) {
        info = -2;
    ...
    for( dev=0; dev < ngpu; ++dev ) {
        magma_minproduct_setdevice( dev );
        magma_minproduct_queue_sync( queues[dev] );
