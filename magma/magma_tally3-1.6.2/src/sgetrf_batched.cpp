...
//#define ENABLE_TIMER3
    ...
    tall   = magma_tally3_sync_wtime(0);
    talloc = magma_tally3_sync_wtime(0);
#endif

    float neg_one = MAGMA_tally3_S_NEG_ONE;
    float one  = MAGMA_tally3_S_ONE;
    magma_tally3_int_t ib, i, k, pm;
    magma_tally3_int_t nb = BATRF_NB;
    magma_tally3_int_t gemm_crossover = nb > 32 ? 127 : 160;
    // magma_tally3_int_t gemm_crossover = n;// use only stream gemm
    ...
    magma_tally3_int_t     **dipiv_displ   = NULL;
    float **dA_displ   = NULL;
    ...
    magma_tally3_malloc((void**)&dipiv_displ,   batchCount * sizeof(*dipiv_displ));
    ...
    magma_tally3_int_t dwork_msize = n*nb;
    magma_tally3_int_t **pivinfo_array    = NULL;
    magma_tally3_int_t *pivinfo           = NULL; 
    float* dinvA      = NULL;
    float* dwork      = NULL;// dinvA and dwork are workspace in strsm
    float **cpuAarray = NULL;
    magma_tally3_smalloc( &dinvA, invA_msize * batchCount);
    magma_tally3_smalloc( &dwork, dwork_msize * batchCount );
    magma_tally3_malloc((void**)&pivinfo_array, batchCount * sizeof(*pivinfo_array));
    magma_tally3_malloc((void**)&pivinfo, batchCount * m * sizeof(magma_tally3_int_t));
   ...
   /* check allocation */
    if ( dA_displ  == NULL || dW0_displ == NULL || dW1_displ   == NULL || dW2_displ   == NULL || 
        ...
        free(cpuAarray);
        magma_tally3_free(dipiv_displ);
        magma_tally3_free(pivinfo_array);
        magma_tally3_free(pivinfo);
    ...
    magma_tally3blas_slaset_q(Magma_tally3Full, invA_msize, batchCount, MAGMA_tally3_S_ZERO, MAGMA_tally3_S_ZERO, dinvA, invA_msize, queue);
    magma_tally3blas_slaset_q(Magma_tally3Full, dwork_msize, batchCount, MAGMA_tally3_S_ZERO, MAGMA_tally3_S_ZERO, dwork, dwork_msize, queue);
    ...
    talloc = magma_tally3_sync_wtime(0) - talloc;
    tloop  = magma_tally3_sync_wtime(0);
#endif
    ...
    for(i = 0; i < min_mn; i+=nb) 
    {
        ...
        ib = min(nb, min_mn-i);
        pm = m-i;
        magma_tally3_idisplace_pointers(dipiv_displ, ipiv_array, ldda, i, 0, batchCount, queue);
        magma_tally3_sdisplace_pointers(dA_displ, dA_array, ldda, i, i, batchCount, queue);
        //===============================================
        //  panel factorization
        //===============================================
#if 0
        arginfo = magma_tally3_sgetf2_batched(
                pm, ib,
                dA_displ, ldda,
...
#else
        arginfo = magma_tally3_sgetrf_recpanel_batched(
                pm, ib, 16,
                dA_displ, ldda,
...
#define RUN_ALL
        ...
        magma_tally3_sdisplace_pointers(dW0_displ, dA_array, ldda, i, 0, batchCount, queue);
        magma_tally3_slaswp_rowparallel_batched( i, dA_displ, ldda,
                dW0_displ, ldda,
                i, i+ib,
        ...
        if( (i + ib) < n)
        {
            ...
            magma_tally3_sdisplace_pointers(dA_displ, dA_array, ldda, i, i+ib, batchCount, queue);
            sset_pointer(dwork_array, dwork, nb, 0, 0, dwork_msize, batchCount, queue); // I don't think it is needed Azzam
            magma_tally3_slaswp_rowparallel_batched( n-(i+ib), dA_displ, ldda,
                    dwork_array, nb,
                    i, i+ib,
                    pivinfo_array, batchCount, queue);
            ...
            magma_tally3_sdisplace_pointers(dA_displ, dA_array, ldda, i, i, batchCount, queue);
            magma_tally3_sdisplace_pointers(dW0_displ, dA_array, ldda, i, i+ib, batchCount, queue);
            magma_tally3blas_strsm_outofplace_batched(Magma_tally3Left, Magma_tally3Lower, Magma_tally3NoTrans, Magma_tally3Unit, 1,
                    ib, n-i-ib,
                    MAGMA_tally3_S_ONE,
                    dA_displ,    ldda, // dA
                    dwork_array,  nb, // dB
            ...
            {    
                ...
                { 
                    ...
                    //magma_tally3_queue_sync(NULL); 
                    //
                    for(k=0; k<batchCount; k++)
                    {
                        streamid = k%nbstreams;                                       
                        magma_tally3blasSetKernelStream(stream[streamid]);
                        magma_tally3_sgemm(Magma_tally3NoTrans, Magma_tally3NoTrans, 
                                m-i-ib, n-i-ib, ib,
                ...
                {
                    magma_tally3_sdisplace_pointers(dA_displ, dA_array,  ldda, i+ib,    i, batchCount, queue);
                    magma_tally3_sdisplace_pointers(dW1_displ, dA_array, ldda,    i, i+ib, batchCount, queue);
                    magma_tally3_sdisplace_pointers(dW2_displ, dA_array, ldda, i+ib, i+ib, batchCount, queue);
                    //printf("caling batched dgemm %d %d %d \n", m-i-ib, n-i-ib, ib);
                    magma_tally3blas_sgemm_batched( Magma_tally3NoTrans, Magma_tally3NoTrans, m-i-ib, n-i-ib, ib, 
...
#if defined(ENABLE_TIMER3)
    tloop   = magma_tally3_sync_wtime(0) - tloop;
    tdalloc = magma_tally3_sync_wtime(0);

#endif

    for(i=0; i<nbstreams; i++){
        magma_tally3_queue_destroy( stream[i] );
    }
...
#if defined(ENABLE_TIMER3)
    tdalloc = magma_tally3_sync_wtime(0) - tdalloc;
    tall = magma_tally3_sync_wtime(0) - tall;
