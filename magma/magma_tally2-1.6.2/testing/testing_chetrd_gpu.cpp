...
int main( int argc, char** argv)
{
    ...
    float           eps;
    magma_tally2FloatComplex *h_A, *h_R, *h_Q, *h_work, *work;
    magma_tally2FloatComplex_ptr d_R, dwork;
    magma_tally2FloatComplex *tau;
    float          *diag, *offdiag;
    float           result[2] = {0., 0.};
    magma_tally2_int_t N, n2, lda, ldda, lwork, info, nb, ldwork;
    ...
    printf("2 - uses CHEMV from MAGMA_tally2 BLAS that requires extra space\n\n");
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lwork  = N*nb;  /* We suppose the magma_tally2 nb is bigger than lapack nb */
            gflops = FLOPS_CHETRD( N ) / 1e9;
            ldwork = ldda*ceildiv(N,64) + 2*ldda*nb;
            
            TESTING_MALLOC_CPU( h_A,     magma_tally2FloatComplex, lda*N );
            TESTING_MALLOC_CPU( tau,     magma_tally2FloatComplex, N     );
            ...
            TESTING_MALLOC_DEV( d_R,     magma_tally2FloatComplex, ldda*N );
            ...
            magma_tally2_csetmatrix( N, N, h_A, lda, d_R, ldda );
            ...
            if (opts.version == 1) {
                magma_tally2_chetrd_gpu( opts.uplo, N, d_R, ldda, diag, offdiag,
                                  tau, h_R, lda, h_work, lwork, &info );
            }
            else {
                magma_tally2_chetrd2_gpu( opts.uplo, N, d_R, ldda, diag, offdiag,
                                   tau, h_R, lda, h_work, lwork, dwork, ldwork, &info );
            }
            gpu_time = magma_tally2_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally2_chetrd_gpu returned error %d: %s.\n",
            ...
            /* =====================================================================
               ...
               =================================================================== */
            if ( opts.check ) {
                TESTING_MALLOC_CPU( h_Q,  magma_tally2FloatComplex, lda*N );
                TESTING_MALLOC_CPU( work, magma_tally2FloatComplex, 2*N*N );
                #ifdef COMPLEX
                TESTING_MALLOC_CPU( rwork, float, N );
                #endif
                
                magma_tally2_cgetmatrix( N, N, d_R, ldda, h_R, lda );
                magma_tally2_cgetmatrix( N, N, d_R, ldda, h_Q, lda );
                lapackf77_cungtr( lapack_uplo_const_tally2(opts.uplo), &N, h_Q, &lda, tau, h_work, &lwork, &info );
                
                lapackf77_chet21( &itwo, lapack_uplo_const_tally2(opts.uplo), &N, &ione,
                ...
                lapackf77_chet21( &ithree, lapack_uplo_const_tally2(opts.uplo), &N, &ione,
                                  h_A, &lda, diag, offdiag,
                                  h_Q, &lda, h_R, &lda,
                                  tau, work,
                                  #ifdef COMPLEX
            ...
            if ( opts.lapack ) {
                ...
                lapackf77_chetrd( lapack_uplo_const_tally2(opts.uplo), &N, h_A, &lda, diag, offdiag, tau,
                                  h_work, &lwork, &info );
