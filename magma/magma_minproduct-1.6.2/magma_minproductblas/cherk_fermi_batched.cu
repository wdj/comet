    ...
    magma_minproductFloatComplex **dC_array, magma_minproduct_int_t lddc, magma_minproduct_int_t batchCount, magma_minproduct_queue_t queue )
{
    ...
    magma_minproductFloatComplex calpha = MAGMA_minproduct_C_MAKE( alpha, 0. );

    magma_minproduct_int_t info = 0;
    if      ( uplo != Magma_minproductUpper && uplo != Magma_minproductLower )
        info = -1;
    else if ( trans != Magma_minproductNoTrans && trans != Magma_minproductTrans && trans != Magma_minproductConjTrans )
        info = -2;
    else if ( n < 0 )
        info = -3;
    else if ( k < 0 )
        info = -4;
    else if ( trans == Magma_minproductNoTrans ? ldda < n : ldda < k )
        info = -7;
    else if ( lddc < n )
        info = -10;
    ...
    if      ( trans == Magma_minproductNoTrans )
        ...
        TransB = 1;
        #endif
    else if ( trans == Magma_minproductTrans || trans == Magma_minproductConjTrans)
        #if defined(PRECISION_z) || defined(PRECISION_c)     
        TransA = 2;
        #else
        TransA = 1;
        ...
        cudaError_t err;
        err = cudaBindTexture(&offsetA, tex_ref_A, dA_array[0], sizeA*sizeof(magma_minproductFloatComplex));
        if ( err != cudaSuccess ) {
    ...
    if ( TransA == 0 && TransB == 1 ) {
        dim3 dimGrid( (n - 1)/BLK_M_nt + 1,
                      (n - 1)/BLK_N_nt + 1 ,
                      batchCount );
        magma_minproductblas_c_herk_kernel_fermi_nt_batched<<< dimGrid, dimBlock, 0, queue >>>(
        ...
        dim3 dimGrid( (n - 1)/BLK_M_nc + 1,
                      (n - 1)/BLK_N_nc + 1 ,
                      batchCount );
         magma_minproductblas_c_herk_kernel_fermi_nc_batched<<< dimGrid, dimBlock, 0, queue >>>(
            uploA, n, k, dA_array, ldda, dA_array, ldda, dC_array, lddc, calpha, cbeta,
            (int)offsetA, (int)offsetA );
    }
        ...
        dim3 dimGrid( (n - 1)/BLK_M_tn + 1,
                      (n - 1)/BLK_N_tn + 1 ,
                      batchCount );
         magma_minproductblas_c_herk_kernel_fermi_tn_batched<<< dimGrid, dimBlock, 0, queue >>>(
            uploA, n, k, dA_array, ldda, dA_array, ldda, dC_array, lddc, calpha, cbeta,
            (int)offsetA, (int)offsetA );
    }
        ...
        dim3 dimGrid( (n - 1)/BLK_M_cn + 1,
                      (n - 1)/BLK_N_cn + 1 ,
                      batchCount );
         magma_minproductblas_c_herk_kernel_fermi_cn_batched<<< dimGrid, dimBlock, 0, queue >>>(
            uploA, n, k, dA_array, ldda, dA_array, ldda, dC_array, lddc, calpha, cbeta,
            (int)offsetA, (int)offsetA );
    }
