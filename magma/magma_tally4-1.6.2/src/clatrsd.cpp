...
/**
    ...
    @ingroup magma_tally4_cgeev_aux
    ********************************************************************/
extern "C"
magma_tally4_int_t magma_tally4_clatrsd(
...
#define A(i,j) (A + (i) + (j)*lda)
    ...
    float tmax;
    magma_tally4FloatComplex tjjs;
    float xmax, grow;

    float tscal;
    magma_tally4FloatComplex uscal;
    magma_tally4_int_t jlast;
    magma_tally4FloatComplex csumj;
    ...
    *scale = 1.;
    ...
    if ( normin == Magma_tally4False ) {
        ...
        if ( upper ) {
            ...
            cnorm[0] = 0.;
            for( j = 1; j < n; ++j ) {
                cnorm[j] = magma_tally4_cblas_scasum( j, A(0,j), ione );
            }
        }
        else {
            /* A is lower triangular. */
            for( j = 0; j < n-1; ++j ) {
                cnorm[j] = magma_tally4_cblas_scasum( n-(j+1), A(j+1,j), ione );
            }
            cnorm[n-1] = 0.;
        }
    }
    ...
    xmax = 0.;
    for( j = 0; j < n; ++j ) {
        xmax = max( xmax, 0.5*MAGMA_tally4_C_ABS1( x[j] ));
    }
    xbnd = xmax;
...
L110:
        ...
        for( j = jfirst; (jinc < 0 ? j >= jlast : j < jlast); j += jinc ) {
            ...
            /*                       k<>j             */
            xj = MAGMA_tally4_C_ABS1( x[j] );
            uscal = MAGMA_tally4_C_MAKE( tscal, 0. );
            rec = 1. / max( xmax, 1. );
            ...
            if ( uscal == c_one ) {
                ...
                if ( upper ) {
                    csumj = magma_tally4_cblas_cdotu( j, A(0,j), ione, &x[0], ione );
                }
                else if ( j < n-1 ) {
                    csumj = magma_tally4_cblas_cdotu( n-(j+1), A(j+1,j), ione, &x[j+1], ione );
                }
            }
            else {
            ...
            if ( uscal == MAGMA_tally4_C_MAKE( tscal, 0. )) {
...
L160:
            ...
            xmax = max( xmax, MAGMA_tally4_C_ABS1( x[j] ));
        }
    }
    else {
        /* ---------------------------------------- */
        ...
        for( j = jfirst; (jinc < 0 ? j >= jlast : j < jlast); j += jinc ) {
            ...
            if ( cnorm[j] > (bignum - xj) * rec ) {
                ...
                rec *= 0.5;
                if ( nounit ) {
                    tjjs = MAGMA_tally4_C_CNJG( *A(j,j) - lambda ) * tscal;
                }
                else {
                    tjjs = (c_one - lambda) * tscal;
                }
            ...
            if ( uscal == c_one ) {
                ...
                if ( upper ) {
                    csumj = magma_tally4_cblas_cdotc( j, A(0,j), ione, &x[0], ione );
                }
                else if ( j < n-1 ) {
                    csumj = magma_tally4_cblas_cdotc( n-(j+1), A(j+1,j), ione, &x[j+1], ione );
                }
            }
            else {
                /* Otherwise, use in-line code for the dot product. */
                if ( upper ) {
                    for( i = 0; i < j; ++i ) {
                        csumj += (MAGMA_tally4_C_CNJG( *A(i,j) ) * uscal) * x[i];
                    }
                }
                else if ( j < n-1 ) {
                    for( i = j + 1; i < n; ++i ) {
