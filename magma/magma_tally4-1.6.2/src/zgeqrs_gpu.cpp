...
/**
    ...
    tau     COMPLEX_16 array, dimension (N)
            ...
            reflector H(i), as returned by MAGMA_tally4_ZGEQRF_GPU.
    ...
    dT      COMPLEX_16 array that is the output (the 6th argument)
            of magma_tally4_zgeqrf_gpu of size
    ...
    @ingroup magma_tally4_zgels_comp
    ********************************************************************/
    ...
    magma_tally4_int_t *info)
{
    ...
    *info = 0;
    ...
    /* B := Q' * B */
    magma_tally4_zunmqr_gpu( Magma_tally4Left, Magma_tally4_ConjTrans,
                      m, nrhs, n,
                      dA(0,0), ldda, tau,
    ...
    // TODO: this assumes that, on exit from magma_tally4_zunmqr_gpu, hwork contains
    ...
    // magma_tally4_ztrsm and drop the zsetmatrix.
    if ( nrhs == 1 ) {
        blasf77_ztrsv( Magma_tally4UpperStr, Magma_tally4NoTransStr, Magma_tally4NonUnitStr,
                       &ib, hwork,         &rows,
                            hwork+rows*ib, &ione);
    } else {
        blasf77_ztrsm( Magma_tally4LeftStr, Magma_tally4UpperStr, Magma_tally4NoTransStr, Magma_tally4NonUnitStr,
                       &ib, &nrhs,
                       &c_one, hwork,         &rows,
                               hwork+rows*ib, &rows);
    }
    ...
    magma_tally4_zsetmatrix( ib, nrhs, hwork+rows*ib, rows, dwork+i, lddwork );

    // update c
    if (nrhs == 1)
        magma_tally4_zgemv( Magma_tally4NoTrans, i, ib,
    ...
    if (nb < k) {
        for (i = start; i >= 0; i -= nb) {
            ...
            if (i + ib < n) {
                if (nrhs == 1) {
                    magma_tally4_zgemv( Magma_tally4NoTrans, ib, ib,
                                 c_one,  dT(i), ib,
                                         dB+i,      1,
                                 c_zero, dwork+i,  1);
                    magma_tally4_zgemv( Magma_tally4NoTrans, i, ib,
    ...
    magma_tally4_zcopymatrix( (n), nrhs,
                       dwork, lddwork,
                       dB,    lddb );
