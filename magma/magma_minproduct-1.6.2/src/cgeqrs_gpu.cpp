    ...
    magma_minproduct_int_t *info)
{
    ...
    *info = 0;
    ...
    // TODO: this assumes that, on exit from magma_minproduct_cunmqr_gpu, hwork contains
    ...
    // magma_minproduct_ctrsm and drop the csetmatrix.
    if ( nrhs == 1 ) {
        blasf77_ctrsv( Magma_minproductUpperStr, Magma_minproductNoTransStr, Magma_minproductNonUnitStr,
                       &ib, hwork,         &rows,
                            hwork+rows*ib, &ione);
    } else {
        blasf77_ctrsm( Magma_minproductLeftStr, Magma_minproductUpperStr, Magma_minproductNoTransStr, Magma_minproductNonUnitStr,
                       &ib, &nrhs,
                       &c_one, hwork,         &rows,
                               hwork+rows*ib, &rows);
    }
    ...
    magma_minproduct_csetmatrix( ib, nrhs, hwork+rows*ib, rows, dwork+i, lddwork );

    // update c
    if (nrhs == 1)
        magma_minproduct_cgemv( Magma_minproductNoTrans, i, ib,
    ...
    if (nb < k) {
        for (i = start; i >= 0; i -= nb) {
            ...
            if (i + ib < n) {
                if (nrhs == 1) {
                    magma_minproduct_cgemv( Magma_minproductNoTrans, ib, ib,
                                 c_one,  dT(i), ib,
                                         dB+i,      1,
                                 c_zero, dwork+i,  1);
                    magma_minproduct_cgemv( Magma_minproductNoTrans, i, ib,
    ...
    magma_minproduct_ccopymatrix( (n), nrhs,
                       dwork, lddwork,
                       dB,    lddb );
