...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            N = opts.nsize[itest];
            n2     = N*N;
            nb     = magma_tally3_get_dsytrd_nb(N);
            #ifdef COMPLEX
            ...
            magma_tally3_dmake_hpd( N, h_B, N );
            ...
            if ( opts.warmup || opts.check ) {
                ...
                lapackf77_dlacpy( Magma_tally3FullStr, &N, &N, h_A, &N, h_Ainit, &N );
                lapackf77_dlacpy( Magma_tally3FullStr, &N, &N, h_B, &N, h_Binit, &N );
            }
            ...
            if (opts.warmup) {
                ...
                // ==================================================================
                magma_tally3_dsygvd_m( opts.ngpu, opts.itype, opts.jobz, opts.uplo,
                                N, h_A, N, h_B, N, w1,
                                h_work, lwork,
                                #ifdef COMPLEX
                                rwork, lrwork,
                                ...
                                iwork, liwork,
                                &info);
                lapackf77_dlacpy( Magma_tally3FullStr, &N, &N, h_Ainit, &N, h_A, &N );
                lapackf77_dlacpy( Magma_tally3FullStr, &N, &N, h_Binit, &N, h_B, &N );
            }
            ...
            if (info != 0)
                printf("magma_tally3_dsygvd_m returned error %d: %s.\n",
            ...
            if ( opts.check && opts.jobz != Magma_tally3NoVec ) {
                ...
                result[0] = 1.;
                result[0] /= lapackf77_dlansy("1", lapack_uplo_const_tally3(opts.uplo), &N, h_Ainit, &N, rwork);
                result[0] /= lapackf77_dlange("1", &N, &N, h_A, &N, rwork);

                if (opts.itype == 1) {
                    blasf77_dsymm("L", lapack_uplo_const_tally3(opts.uplo), &N, &N, &c_one, h_Ainit, &N, h_A, &N, &c_zero, h_work, &N);
                    for(int i=0; i < N; ++i)
                        blasf77_dscal(&N, &w1[i], &h_A[i*N], &ione);
                    blasf77_dsymm("L", lapack_uplo_const_tally3(opts.uplo), &N, &N, &c_neg_one, h_Binit, &N, h_A, &N, &c_one, h_work, &N);
                    result[0] *= lapackf77_dlange("1", &N, &N, h_work, &N, rwork)/N;
                }
                else if (opts.itype == 2) {
                    blasf77_dsymm("L", lapack_uplo_const_tally3(opts.uplo), &N, &N, &c_one, h_Binit, &N, h_A, &N, &c_zero, h_work, &N);
                    for(int i=0; i < N; ++i)
                        blasf77_dscal(&N, &w1[i], &h_A[i*N], &ione);
                    blasf77_dsymm("L", lapack_uplo_const_tally3(opts.uplo), &N, &N, &c_one, h_Ainit, &N, h_work, &N, &c_neg_one, h_A, &N);
                    ...
                    for(int i=0; i < N; ++i)
                        blasf77_dscal(&N, &w1[i], &h_A[i*N], &ione);
                    blasf77_dsymm("L", lapack_uplo_const_tally3(opts.uplo), &N, &N, &c_one, h_Binit, &N, h_work, &N, &c_neg_one, h_A, &N);
                    result[0] *= lapackf77_dlange("1", &N, &N, h_A, &N, rwork)/N;
                }
            }
            if ( opts.lapack ) {
                ...
                magma_tally3_dsygvd(opts.itype, opts.jobz, opts.uplo,
                             N, h_A, N, h_B, N, w2,
                             h_work, lwork,
                             #ifdef COMPLEX
                             rwork, lrwork,
                ...
                if (info != 0)
                    printf("magma_tally3_dsygvd returned error %d: %s.\n",
                           (int) info, magma_tally3_strerror( info ));
                ...
                lapackf77_dsygvd(&opts.itype, lapack_vec_const_tally3(opts.jobz), lapack_uplo_const_tally3(opts.uplo),
