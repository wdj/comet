...
__global__ void
magma_tally4_sreduce_kernel_spmv1(    
    int Gs,
    int n, 
    float * vtmp,
    float * vtmp2 )
...
__global__ void
magma_tally4_sbicgmerge_spmv1_kernel(  
    int n,
    float * dval, 
    ...
    float * vtmp)
{
    ...
    temp[ Idx ] = ( i < n ) ? v[ i ] * r[ i ] : MAGMA_tally4_S_MAKE( 0.0, 0.0);
    __syncthreads();
    if ( Idx < 128 ){
...
__global__ void
magma_tally4_sbicgstab_alphakernel(  
                    float * skp ){
    ...
    magma_tally4_queue_t queue )
{
    ...
    if ( A.storage_type == Magma_tally4_CSR)
        magma_tally4_sbicgmerge_spmv1_kernel<<<Gs, Bs, Ms>>>
    ...
    while( Gs.x > 1 ) {
        ...
        magma_tally4_sreduce_kernel_spmv1<<< Gs_next.x/2, Bs.x/2, Ms/2 >>> 
                            ( Gs.x, n, aux1, aux2 );
    ...
    magma_tally4_scopyvector( 1, aux1, 1, skp, 1 );
    dim3 Bs2( 2 );
    dim3 Gs2( 1 );
    magma_tally4_sbicgstab_alphakernel<<<Gs2, Bs2, 0>>>( skp );
...
__global__ void
magma_tally4_sreduce_kernel_spmv2( 
    int Gs,
    int n, 
    float * vtmp,
    float * vtmp2 )
{
    ...
    for( j=0; j<2; j++){
        int i = blockIdx.x * ( blockSize * 2 ) + Idx;   
        temp[Idx+j*(blockSize)] = MAGMA_tally4_S_MAKE( 0.0, 0.0);
        while (i < Gs ) {
...
__global__ void
magma_tally4_sbicgmerge_spmv2_kernel(  
    int n,
    float * dval, 
    magma_tally4_index_t * drowptr, 
    magma_tally4_index_t * dcolind,
    ...
    float * vtmp )
{
    ...
    if (i<n){
        ...
        for( j=0; j<2; j++)
            temp[Idx+j*blockDim.x] =MAGMA_tally4_S_MAKE( 0.0, 0.0);
    }
    __syncthreads();
    if ( Idx < 128 ){
        for( j=0; j<2; j++){
...
__global__ void
magma_tally4_sbicgstab_omegakernel(  
                    float * skp ){
    int i = blockIdx.x * blockDim.x + threadIdx.x;
...
/**
    ...
    @param[in]
    ds          magma_tally4Float_ptr
                input vector s

    @param[in]
    dt          magma_tally4Float_ptr
                output vector t
    ...
    magma_tally4_queue_t queue )
{
    ...
    int b = 1;        
    if ( A.storage_type == Magma_tally4_CSR)
        magma_tally4_sbicgmerge_spmv2_kernel<<<Gs, Bs, Ms>>>
    ...
    while( Gs.x > 1 ) {
        Gs_next.x = ( Gs.x+Bs.x-1 )/ Bs.x ;
        if ( Gs_next.x == 1 ) Gs_next.x = 2;
        magma_tally4_sreduce_kernel_spmv2<<< Gs_next.x/2, Bs.x/2, Ms/2 >>> 
                    ( Gs.x, n, aux1, aux2 );
        Gs_next.x = Gs_next.x /2;
        Gs.x = Gs_next.x;
        b = 1 - b;
    ...
    magma_tally4_scopyvector( 1, aux1, 1, skp+6, 1 );
    magma_tally4_scopyvector( 1, aux1+n, 1, skp+7, 1 );
    dim3 Bs2( 2 );
    dim3 Gs2( 1 );
    magma_tally4_sbicgstab_omegakernel<<<Gs2, Bs2, 0>>>( skp );
...
__global__ void
magma_tally4_sbicgmerge_xrbeta_kernel(  
    int n, 
    float * rr,
    float * r,
    float * p,
...
__global__ void
magma_tally4_sbicgstab_betakernel(  
    float * skp )
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
...
/**
    ...
    @param[in]
    p           magma_tally4Float_ptr
                input vector p

    @param[in]
    s           magma_tally4Float_ptr
                input vector s

    @param[in]
    t           magma_tally4Float_ptr
                input vector t

    @param[out]
    x           magma_tally4Float_ptr
                output vector x
...
magma_tally4_sbicgmerge_xrbeta(
    ...
    magma_tally4Float_ptr d1,
    magma_tally4Float_ptr d2,
    magma_tally4Float_ptr rr,
    magma_tally4Float_ptr r,
    magma_tally4Float_ptr p,
    magma_tally4Float_ptr s,
    magma_tally4Float_ptr t,
    ...
    magma_tally4_queue_t queue )
{
    ...
    magma_tally4Float_ptr aux1 = d1, aux2 = d2;
    int b = 1;        
    magma_tally4_sbicgmerge_xrbeta_kernel<<<Gs, Bs, Ms>>>
    ...
    magma_tally4_scopyvector( 1, aux1+n, 1, skp+5, 1 );
    dim3 Bs2( 2 );
    dim3 Gs2( 1 );
    magma_tally4_sbicgstab_betakernel<<<Gs2, Bs2, 0>>>( skp );
