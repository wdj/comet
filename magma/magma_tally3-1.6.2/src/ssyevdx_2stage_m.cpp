...
/**
    ...
    lwork   INTEGER
            ...
            If N <= 1,                      LWORK >= 1.
            If JOBZ = Magma_tally3NoVec and N > 1, LWORK >= LQ2 + 2*N + N*NB.
            If JOBZ = Magma_tally3Vec   and N > 1, LWORK >= LQ2 + 1 + 6*N + 2*N**2.
    ...
    @ingroup magma_tally3_ssyev_driver
    ********************************************************************/
    ...
    magma_tally3_int_t *info)
{
    ...
    float rmin, rmax;
    float sigma;
    magma_tally3_int_t lwmin, liwmin;
    ...
    *info = 0;
    ...
    magma_tally3_int_t nb = magma_tally3_get_sbulge_nb(n, parallel_threads);
    magma_tally3_int_t Vblksiz = magma_tally3_sbulge_get_Vblksiz(n, nb, parallel_threads);
    ...
    magma_tally3_int_t lq2 = magma_tally3_sbulge_get_lq2(n, parallel_threads);
    ...
    magma_tally3_int_t indT2   = inde + n;
    ...
    magma_tally3_timer_t time=0, time_total=0, time_alloc=0, time_sist=0, time_band=0;
    timer_start( time_total );

#ifdef HE2HB_SINGLEGPU
    float *dT1;
    if (MAGMA_tally3_SUCCESS != magma_tally3_smalloc( &dT1, n*nb)) {
        *info = MAGMA_tally3_ERR_DEVICE_ALLOC;
        return *info;
    }
    magma_tally3_ssytrd_sy2sb(uplo, n, nb, A, lda, &work[indtau1], &work[indwrk], llwork, dT1, info);
    magma_tally3_free(dT1);
#else
    magma_tally3_int_t nstream = max(3,ngpu+2);
    magma_tally3_queue_t streams[Magma_tally3MaxGPUs][20];
    float *dA[Magma_tally3MaxGPUs], *dT1[Magma_tally3MaxGPUs];
    ...
    for( magma_tally3_int_t dev = 0; dev < ngpu; ++dev ) {
        ...
        // TODO check malloc
        magma_tally3_smalloc(&dA[dev], ldda*mlocal );
        magma_tally3_smalloc(&dT1[dev], (n*nb) );
    ...
    magma_tally3_ssetmatrix_1D_col_bcyclic( n, n, A, lda, dA, ldda, ngpu, distblk );
    magma_tally3_setdevice(0);
    timer_stop( time_sist );

    timer_start( time_band );
    if (ver == 30) {
        magma_tally3_ssytrd_sy2sb_mgpu_spec(uplo, n, nb, A, lda, &work[indtau1], &work[indwrk], llwork, dA, ldda, dT1, nb, ngpu, distblk, streams, nstream, info);
    } else {
        magma_tally3_ssytrd_sy2sb_mgpu(uplo, n, nb, A, lda, &work[indtau1], &work[indwrk], llwork, dA, ldda, dT1, nb, ngpu, distblk, streams, nstream, info);
    }
    ...
    magma_tally3_ssytrd_sb2st(uplo, n, nb, Vblksiz, A2, lda2, w, &work[inde], &work[indV2], ldv, &work[indTAU2], wantz, &work[indT2], ldt);
    ...
    if (! wantz) {
        ...
        magma_tally3_sstedx_m(ngpu, range, n, vl, vu, il, iu, w, &work[inde],
        ...
        magma_tally3_sbulge_back_m(ngpu, uplo, n, nb, *m, Vblksiz, &work[indwrk + n * (il-1)], n,
        ...
        magma_tally3_sormqr_m(ngpu, Magma_tally3Left, Magma_tally3NoTrans, n-nb, *m, n-nb, A+nb, lda, &work[indtau1],
