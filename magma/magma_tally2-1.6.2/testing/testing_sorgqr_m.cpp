...
int main( int argc, char** argv )
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lapackf77_slacpy( Magma_tally2UpperLowerStr, &m, &n, hA, &lda, hR, &lda );
            ...
            // okay that magma_tally2_sgeqrf_gpu has special structure for R; R isn't used here.
            magma_tally2_ssetmatrix( m, n, hA, lda, dA, ldda );
            magma_tally2_sgeqrf_gpu( m, n, dA, ldda, tau, dT, &info );
            if ( info != 0 )
                printf("magma_tally2_sgeqrf_gpu returned error %d: %s.\n",
                       (int) info, magma_tally2_strerror( info ));
            magma_tally2_sgetmatrix( m, n, dA, ldda, hA, lda );
            lapackf77_slacpy( Magma_tally2FullStr, &m, &n, hA, &lda, hR, &lda );
            magma_tally2_sgetmatrix( nb, min_mn, dT, nb, hT, nb );
            
            gpu_time = magma_tally2_wtime();
            magma_tally2_sorgqr_m( m, n, k, hR, lda, tau, hT, nb, &info );
            gpu_time = magma_tally2_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if ( info != 0 )
                printf("magma_tally2_sorgqr_gpu returned error %d: %s.\n",
                       (int) info, magma_tally2_strerror( info ));
