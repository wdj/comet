    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for( int dev = 0; dev < ngpu; dev++ ) {
                ...
                TESTING_MALLOC_DEV( d_lA[dev], magma_minproductDoubleComplex, ldda*n_local );
            }
            ...
            /* =====================================================================
               ...
               =================================================================== */
            if ( opts.lapack ) {
                magma_minproductDoubleComplex *tau2;
                TESTING_MALLOC_CPU( tau2, magma_minproductDoubleComplex, min_mn );
            ...
            /* ====================================================================
               ...
               =================================================================== */
            magma_minproduct_zsetmatrix_1D_col_bcyclic( M, N, h_R, lda, d_lA, ldda, ngpu, nb );
            
            gpu_time = magma_minproduct_wtime();
            magma_minproduct_zgeqrf2_mgpu( ngpu, M, N, d_lA, ldda, tau, &info );
            gpu_time = magma_minproduct_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_minproduct_zgeqrf2 returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
            
            magma_minproduct_zgetmatrix_1D_col_bcyclic( M, N, d_lA, ldda, h_R, lda, ngpu, nb );
            ...
            if ( opts.check == 1 && M >= N ) {
                ...
                magma_minproductDoubleComplex *h_W1, *h_W2, *h_W3;
                double *h_RW, results[2];
            
                TESTING_MALLOC_CPU( h_W1, magma_minproductDoubleComplex, n2    ); // Q
                TESTING_MALLOC_CPU( h_W2, magma_minproductDoubleComplex, n2    ); // R
                TESTING_MALLOC_CPU( h_W3, magma_minproductDoubleComplex, lwork ); // WORK
