...
static void *magma_tally4_sapplyQ_parallel_section(void *arg);

static void magma_tally4_stile_bulge_applyQ(magma_tally4_int_t core_id, magma_tally4_side_t side, magma_tally4_int_t n_loc, magma_tally4_int_t n, magma_tally4_int_t nb, magma_tally4_int_t Vblksiz,
                                     float *E, magma_tally4_int_t lde, float *V, magma_tally4_int_t ldv,
                                     float *TAU, float *T, magma_tally4_int_t ldt);

//////////////////////////////////////////////////////////////////////////////////////////////////////////

typedef struct magma_tally4_sapplyQ_data_s {
...
} magma_tally4_sapplyQ_data;

void magma_tally4_sapplyQ_data_init(
    magma_tally4_sapplyQ_data *zapplyQ_data, magma_tally4_int_t threads_num, magma_tally4_int_t n, magma_tally4_int_t ne, magma_tally4_int_t n_gpu,
    magma_tally4_int_t nb, magma_tally4_int_t Vblksiz, float *E, magma_tally4_int_t lde,
    float *V, magma_tally4_int_t ldv, float *TAU,
    float *T, magma_tally4_int_t ldt, float *dE, magma_tally4_int_t ldde)
{
...
void magma_tally4_sapplyQ_data_destroy(
    magma_tally4_sapplyQ_data *zapplyQ_data)
{
    pthread_barrier_destroy(&(zapplyQ_data->barrier));
}

typedef struct magma_tally4_sapplyQ_id_data {
    magma_tally4_int_t id;
    magma_tally4_sapplyQ_data* data;
} magma_tally4_sapplyQ_id_data;

void magma_tally4_sapplyQ_id_data_init(
    magma_tally4_sapplyQ_id_data *zapplyQ_id_data,
    magma_tally4_int_t id, magma_tally4_sapplyQ_data* data)
{
...
//#endif
    ...
    magma_tally4_int_t gpu_cpu_perf = magma_tally4_get_sbulge_gcperf();
    if (threads > 1) {
...
//$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
    ...
    if (n_gpu < ne) {
        ...
        #endif
        magma_tally4_sapplyQ_data data_applyQ;
        magma_tally4_sapplyQ_data_init(&data_applyQ, threads, n, ne, n_gpu, nb, Vblksiz, Z, ldz, V, ldv, TAU, T, ldt, dZ, lddz);

        magma_tally4_sapplyQ_id_data* arg;
        magma_tally4_malloc_cpu((void**) &arg, threads*sizeof(magma_tally4_sapplyQ_id_data));
        ...
        for (magma_tally4_int_t thread = 1; thread < threads; thread++) {
            magma_tally4_sapplyQ_id_data_init(&(arg[thread]), thread, &data_applyQ);
            pthread_create(&thread_id[thread], &thread_attr, magma_tally4_sapplyQ_parallel_section, &arg[thread]);
        }
        magma_tally4_sapplyQ_id_data_init(&(arg[0]), 0, &data_applyQ);
        magma_tally4_sapplyQ_parallel_section(&arg[0]);
        ...
        magma_tally4_sapplyQ_data_destroy(&data_applyQ);


        magma_tally4_ssetmatrix(n, ne-n_gpu, Z + n_gpu*ldz, ldz, dZ + n_gpu*ldz, lddz);

        /*============================
         *  use only GPU
         *==========================*/
    } else {
        magma_tally4_ssetmatrix(n, ne, Z, ldz, dZ, lddz);
        magma_tally4_sbulge_applyQ_v2(Magma_tally4Left, ne, n, nb, Vblksiz, dZ, lddz, V, ldv, T, ldt, info);
...
static void *magma_tally4_sapplyQ_parallel_section(void *arg)
{
    magma_tally4_int_t my_core_id   = ((magma_tally4_sapplyQ_id_data*)arg) -> id;
    magma_tally4_sapplyQ_data* data = ((magma_tally4_sapplyQ_id_data*)arg) -> data;
    ...
    if (my_core_id == 0) {
        ...
        magma_tally4_ssetmatrix(n, n_gpu, E, lde, dE, ldde);
        magma_tally4_sbulge_applyQ_v2(Magma_tally4Left, n_gpu, n, nb, Vblksiz, dE, ldde, V, ldv, T, ldt, &info);
        magma_tally4_device_sync();
        ...
        magma_tally4_stile_bulge_applyQ(my_core_id, Magma_tally4Left, n_loc, n, nb, Vblksiz, E_loc, lde, V, ldv, TAU, T, ldt);
...
#define TAU(m)   &(TAU[(m)])
#define T(m)     &(T[(m)])
static void magma_tally4_stile_bulge_applyQ(
    ...
    //, magma_tally4_int_t* info)
{
    ...
    magma_tally4_smalloc_cpu(&work2, lwork);
    ...
    for (magma_tally4_int_t i = 0; i < nbchunk; i++) {
        ...
        if (side == Magma_tally4Left) {
            for (bg = nbGblk; bg > 0; bg--) {
                ...
                if (bg == nbGblk) rownbm = magma_tally4_ceildiv((n-(firstcolj)),nb);  // last blk has size=1 used for real to handle A(N,N-1)
