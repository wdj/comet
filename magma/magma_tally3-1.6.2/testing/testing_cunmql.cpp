/*
    -- MAGMA_tally3 (version 1.6.1) --
       ...
       @author Mark Gates
       @generated from testing_zunmql.cpp normal z -> c, Fri Jan 30 19:00:26 2015
*/
// includes, system
#include <stdlib.h>
#include <stdio.h>
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int itran = 0; itran < 2; ++itran ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_tally3_cgeqlf( mm, k, A, lda, tau, W, lwork_max, &info );
            if (info != 0)
                printf("magma_tally3_cgeqlf returned error %d: %s.\n",
            ...
            lapackf77_cunmql( lapack_side_const_tally3( side[iside] ), lapack_trans_const_tally3( trans[itran] ),
                              &m, &n, &k,
            ...
            lwork = -1;
            magma_tally3_cunmql( side[iside], trans[itran],
                          m, n, k,
                          A, lda, tau, R, ldc, W, lwork, &info );
            if (info != 0)
                printf("magma_tally3_cunmql (lwork query) returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
            ...
            if (info != 0)
                printf("magma_tally3_cunmql returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
