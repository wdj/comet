...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_tally3_ssytrd( opts.uplo, N, h_R, lda, diag, offdiag,
                          tau, h_work, lwork, &info );
            gpu_time = magma_tally3_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally3_ssytrd returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                lapackf77_slacpy( lapack_uplo_const_tally3(opts.uplo), &N, &N, h_R, &lda, h_Q, &lda );
