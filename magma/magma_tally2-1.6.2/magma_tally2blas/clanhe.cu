...
clanhe_inf_kernel_generic_lower(
    int n, const magma_tally2FloatComplex* A, int lda, float *dwork,
    int n_full_block, int n_mod_bs )
{
    ...
    __shared__ magma_tally2FloatComplex la[inf_bs][inf_bs+1];
    ...
    if ( blockIdx.x < n_full_block ) {
        ...
        for(int i=ty*8; i < ty*8 + 8; i++) {
            if ( i < tx ) {
                ...
                la[i][i] = MAGMA_tally2_C_MAKE( MAGMA_tally2_C_REAL( la[i][i] ), 0 );
            }
            #endif
        }
        __syncthreads();
        ...
        if ( n_mod_bs > 0 ) {
            ...
            for(int j=0; j < inf_bs; j += 4) {
                if ( tx < n_mod_bs ) {
                    ...
                    la[ty+j][tx] = MAGMA_tally2_C_ZERO;
                }
            }
            __syncthreads();
        ...
        la[tx][ty] = MAGMA_tally2_C_MAKE( res, 0. );
        __syncthreads();
        
        // first column of 32x1 threads computes final sum of each row
        if ( ty == 0 ) {
            res = res
                + MAGMA_tally2_C_REAL( la[tx][1] )
                + MAGMA_tally2_C_REAL( la[tx][2] )
                + MAGMA_tally2_C_REAL( la[tx][3] );
            dwork[ind] = res;
        }
    }
    else {
        ...
        if ( ty == 0 && tx < n_mod_bs ) {
            ...
            res += MAGMA_tally2_D_ABS( MAGMA_tally2_C_REAL( *A ));
            A += 1;
        ...
        if ( n_mod_bs > 0 ) {
            ...
            for(int j=0; j < inf_bs; j += 4) {
                if ( ty+j < n_mod_bs ) {               //#
                    ...
                    la[tx][ty+j] = MAGMA_tally2_C_ZERO;       //#
                }
            }
            __syncthreads();
...
extern "C" void
clanhe_inf(
    magma_tally2_uplo_t uplo, int n,
    magma_tally2FloatComplex_const_ptr A, int lda,
    magma_tally2Float_ptr dwork )
{
    ...
    if ( uplo == Magma_tally2Lower) {
        clanhe_inf_kernel_generic_lower<<< grid, threads, 0, magma_tally2_stream >>>
            ( n, A, lda, dwork, n_full_block, n_mod_bs );
    }
    else {
        clanhe_inf_kernel_generic_upper<<< grid, threads, 0, magma_tally2_stream >>>
            ( n, A, lda, dwork, n_full_block, n_mod_bs );
    }
}
    ...
    int n, const magma_tally2FloatComplex* A, int lda, float *dwork )
{
    ...
    if (ind < n) {
        ...
        res = fmax( res, MAGMA_tally2_D_ABS( MAGMA_tally2_C_REAL( *A )));
        dwork[ind] = res;
    }
}
    ...
    magma_tally2Float_ptr dwork )
{
    ...
    if ( uplo == Magma_tally2Lower ) {
        clanhe_max_kernel_lower<<< grid, threads, 0, magma_tally2_stream >>>
            ( n, A, lda, dwork );
    }
    else {
        clanhe_max_kernel_upper<<< grid, threads, 0, magma_tally2_stream >>>
            ( n, A, lda, dwork );
    }
}
...
/**
    ...
    uplo    magma_tally2_uplo_t
      ...
      -     = Magma_tally2Upper: Upper triangular part of A is referenced
      -     = Magma_tally2Lower: Lower triangular part of A is referenced
    ...
    A       COMPLEX array on the GPU, dimension (LDA,N)
            The Hermitian matrix A. If UPLO = Magma_tally2Upper, the leading n by n
            upper triangular part of A contains the upper triangular part
            of the matrix A, and the strictly lower triangular part of A
            is not referenced. If UPLO = Magma_tally2Lower, the leading n by n lower
...
magma_tally2blas_clanhe(
    magma_tally2_norm_t norm, magma_tally2_uplo_t uplo, magma_tally2_int_t n,
    ...
    magma_tally2Float_ptr dwork )
{
    ...
    bool inf_norm = (norm == Magma_tally2InfNorm || norm == Magma_tally2OneNorm);
    bool max_norm = (norm == Magma_tally2MaxNorm);
    
    // inf_norm Double-Complex requires > 16 KB shared data (arch >= 200)
    #if defined(PRECISION_z)
    const bool inf_implemented = (magma_tally2_getdevice_arch() >= 200);
    #else
    ...
    int i = magma_tally2_isamax( n, dwork, 1 ) - 1;
