/*
    -- MAGMA_tally3 (version 1.6.1) --
       ...
       @author Mark Gates
       @generated from testing_zunmqr.cpp normal z -> c, Fri Jan 30 19:00:25 2015
*/
// includes, system
#include <stdlib.h>
#include <stdio.h>
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int itran = 0; itran < 2; ++itran ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_tally3_cgeqrf( mm, k, A, lda, tau, W, lwork_max, &info );
            if (info != 0)
            ...
            lwork = -1;
            magma_tally3_cunmqr( side[iside], trans[itran],
                          m, n, k,
                          A, lda, tau, R, ldc, W, lwork, &info );
            if (info != 0)
                printf("magma_tally3_cunmqr (lwork query) returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
            ...
            if (info != 0)
                printf("magma_tally3_cunmqr returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
