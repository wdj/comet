...
int main( int argc, char** argv)
{
    ...
    magma_tally3DoubleComplex *h_A, *h_Q, *h_PT, *h_work;
    magma_tally3DoubleComplex *taup, *tauq;
    double      *diag, *offdiag;
    double      eps, result[3] = {0., 0., 0.};
    magma_tally3_int_t M, N, n2, lda, lhwork, info, minmn, nb;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            N = opts.nsize[itest];
            minmn  = min(M, N);
            nb     = magma_tally3_get_zgebrd_nb(N);
            lda    = M;
            n2     = lda*N;
            lhwork = (M + N)*nb;
            ...
            TESTING_MALLOC_CPU( tauq,    magma_tally3DoubleComplex, minmn );
            TESTING_MALLOC_CPU( taup,    magma_tally3DoubleComplex, minmn );
            ...
            TESTING_MALLOC_PIN( h_work,  magma_tally3DoubleComplex, lhwork );
            
            /* Initialize the matrices */
            lapackf77_zlarnv( &ione, ISEED, &n2, h_A );
            lapackf77_zlacpy( Magma_tally3UpperLowerStr, &M, &N, h_A, &lda, h_Q, &lda );
            
            /* ====================================================================
               Performs operation using MAGMA_tally3
               =================================================================== */
            gpu_time = magma_tally3_wtime();
            magma_tally3_zgebrd( M, N, h_Q, lda,
                          diag, offdiag, tauq, taup,
                          h_work, lhwork, &info);
            gpu_time = magma_tally3_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally3_zgebrd returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                lapackf77_zlacpy(Magma_tally3UpperLowerStr, &M, &N, h_Q, &lda, h_PT, &lda);
