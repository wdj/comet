...
int main( int argc, char** argv)
{
    ...
    magma_tally2FloatComplex offdiag, diag;
      ...
      for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
          for( int ival = 0; ival < 4; ++ival ) {
            ...
            // 2     1.23     0
            // 3     1.23     3.14
            offdiag = MAGMA_tally2_C_MAKE( 1.2345, 6.7890 ) * (ival / 2);
            diag    = MAGMA_tally2_C_MAKE( 3.1415, 2.7183 ) * (ival % 2);
            ...
            if ( uplo[iuplo] == Magma_tally2Lower || uplo[iuplo] == Magma_tally2Upper ) {
                ...
                gbytes = sizeof(magma_tally2FloatComplex) * 0.5*N*(N+1) / 1e9;
            }
            else {
                // save entire matrix
                gbytes = sizeof(magma_tally2FloatComplex) * 1.*M*N / 1e9;
            }
            ...
            //magma_tally2blas_claset( uplo[iuplo], M-2, N-2, offdiag, diag, d_A+1+ldda, ldda );  // inset by 1 row & col
            magma_tally2blas_claset( uplo[iuplo], M, N, offdiag, diag, d_A, ldda );
            ...
            //lapackf77_claset( lapack_uplo_const_tally2( uplo[iuplo] ), &M2, &N2, &offdiag, &diag, h_A+1+lda, &lda );
            lapackf77_claset( lapack_uplo_const_tally2( uplo[iuplo] ), &M, &N, &offdiag, &diag, h_A, &lda );
