...
#define dA(i,j) (dA + (i)*nb + (j)*nb*ldda)
    ...
    *info = 0;
    ...
    if (nb <= 1 || nb >= min(m,n)) {
        ...
        magma_minproduct_cgetrf_nopiv( m, n, work, m, info);
        ...
        if (MAGMA_minproduct_SUCCESS != magma_minproduct_cmalloc_pinned( &work, maxm*nb )) {
            *info = MAGMA_minproduct_ERR_HOST_ALLOC;
            return *info;
        }
        ...
        for( i=0; i < s; i++ ) {
            ...
            magma_minproduct_cgetmatrix_async( m-i*nb, nb, dA(i,i), ldda, work, lddwork, stream[0] );
            ...
            magma_minproduct_cgetrf_nopiv( rows, nb, work, lddwork, &iinfo );
            if ( (*info == 0) && (iinfo > 0) )
                *info = iinfo + i*nb;

            // upload i-th panel
            magma_minproduct_csetmatrix_async( m-i*nb, nb, work, lddwork, dA(i, i), ldda, stream[0] );
            magma_minproduct_queue_sync( stream[0] );
        ...
        rows = m - s*nb;
        magma_minproduct_cgetmatrix( rows, nb0, dA(s,s), ldda, work, lddwork );

        // make sure that gpu queue is empty
        magma_minproduct_device_sync();

        // do the cpu part
        magma_minproduct_cgetrf_nopiv( rows, nb0, work, lddwork, &iinfo );
        if ( (*info == 0) && (iinfo > 0) )
            *info = iinfo + s*nb;

        // upload i-th panel
        magma_minproduct_csetmatrix( rows, nb0, work, lddwork, dA(s,s), ldda );
