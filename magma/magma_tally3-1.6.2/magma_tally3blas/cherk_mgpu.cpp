...
#define STREAM_ID(i) (nqueue > 1 ? 1+((i)/nb)%(nqueue-1) : 0)
    ...
    magma_tally3FloatComplex z_alpha = MAGMA_tally3_C_MAKE(alpha,0.0);
    magma_tally3FloatComplex z_beta  = MAGMA_tally3_C_MAKE(beta, 0.0);
    ...
    for( i=0; i < n; i += nb ) {
        ...
        magma_tally3_cherk(uplo, trans, ib, k,
