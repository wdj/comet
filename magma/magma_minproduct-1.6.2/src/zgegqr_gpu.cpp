...
/**
    ...
    ikind   INTEGER
            ...
            2:  This version uses a standard LAPACK-based orthogonalization through
                MAGMA_minproduct's QR panel factorization (magma_minproduct_zgeqr2x3_gpu) and magma_minproduct_zungqr
            3:  MGS
    ...
    magma_minproduct_int_t *info )
{
    ...
    *info = 0;
    ...
    if (ikind == 1) {
        ...
        magma_minproductDoubleComplex *U, *VT, *vt, *R, *G, *hwork, *tau;
        double *S;
        ...
        magma_minproduct_zmalloc_cpu( &hwork, 32 + 2*n*n + 2*n);
        if ( hwork == NULL ) {
        ...
        do {
            ...
            magma_minproduct_zgemm(Magma_minproductConjTrans, Magma_minproductNoTrans, n, n, m, c_one, dA, ldda, dA, ldda, c_zero, dwork, n );
            magma_minproduct_zgetmatrix(n, n, dwork, n, G, n);
            ...
            magma_minproduct_zsetmatrix(n, n, VT, n, dwork, n);
            magma_minproduct_ztrsm( Magma_minproductRight, Magma_minproductUpper, Magma_minproductNoTrans, Magma_minproductNonUnit, m, n, c_one, dwork, n, dA, ldda);
            if (mins > 0.00001f)
                cn = maxs/mins;
        ...
        magma_minproductDoubleComplex_ptr dtau = dwork + 2*n*n;
        magma_minproductDoubleComplex_ptr d_T  = dwork;
        magma_minproductDoubleComplex_ptr ddA  = dwork + n*n;
        magma_minproductDoubleComplex *tau  = work+n*n;

        magma_minproductblas_zlaset( Magma_minproductFull, n, n, c_zero, c_zero, d_T, n );
        magma_minproduct_zgeqr2x3_gpu(m, n, dA, ldda, dtau, d_T, ddA,
                           (double *)(dwork+min_mn+2*n*n), info);
        magma_minproduct_zgetmatrix( min_mn, 1, dtau, min_mn, tau, min_mn);
        magma_minproduct_zgetmatrix( n, n, ddA, n, work, n);
        magma_minproduct_zungqr_gpu( m, n, n, dA, ldda, tau, d_T, nb, info );
        // ================== end of ikind == 2 ===================================================
    }
    else if (ikind == 3) {
        // ================== MGS               ===================================================
        for (magma_minproduct_int_t j = 0; j < n; j++) {
            for (magma_minproduct_int_t i = 0; i < j; i++) {
                *work(i, j) = magma_minproduct_zdotc(m, dA(0,i), 1, dA(0,j), 1);
                magma_minproduct_zaxpy(m, -(*work(i,j)),  dA(0,i), 1, dA(0,j), 1);
            }
            for (magma_minproduct_int_t i = j; i < n; i++)
                *work(i, j) = MAGMA_minproduct_Z_ZERO;
            //*work(j,j) = MAGMA_minproduct_Z_MAKE( magma_minproduct_dznrm2(m, dA(0,j), 1), 0. );
            *work(j,j) = magma_minproduct_zdotc(m, dA(0,j), 1, dA(0,j), 1);
            *work(j,j) = MAGMA_minproduct_Z_MAKE( sqrt(MAGMA_minproduct_Z_REAL( *work(j,j) )), 0.);
            magma_minproduct_zscal(m, 1./ *work(j,j), dA(0,j), 1);
        }
        // ================== end of ikind == 3 ===================================================
    }
    else if (ikind == 4) {
        // ================== Cholesky QR       ===================================================
        magma_minproduct_zgemm(Magma_minproductConjTrans, Magma_minproductNoTrans, n, n, m, c_one, dA, ldda, dA, ldda, c_zero, dwork, n );
        magma_minproduct_zgetmatrix(n, n, dwork, n, work, n);
        lapackf77_zpotrf("u", &n, work, &n, info);
        magma_minproduct_zsetmatrix(n, n, work, n, dwork, n);
        magma_minproduct_ztrsm( Magma_minproductRight, Magma_minproductUpper, Magma_minproductNoTrans, Magma_minproductNonUnit, m, n, c_one, dwork, n, dA, ldda);
        // ================== end of ikind == 4 ===================================================
    }
