...
/**
    Purpose
    -------
    If VECT = Magma_tally3Q, CUNMBR overwrites the general complex M-by-N matrix C with
                                 SIDE = Magma_tally3Left     SIDE = Magma_tally3Right
    TRANS = Magma_tally3NoTrans:        Q*C                  C*Q
    TRANS = Magma_tally3_ConjTrans:     Q**H*C               C*Q**H
    
    If VECT = Magma_tally3P, CUNMBR overwrites the general complex M-by-N matrix C with
                                 SIDE = Magma_tally3Left     SIDE = Magma_tally3Right
    ...
    *info = 0;
    ...
    if (*info == 0) {
        if (nw > 0) {
            ...
            nb = magma_tally3_get_cgebrd_nb( min( m, n ));
            lwkopt = max(1, nw*nb);
        }
        else {
            lwkopt = 1;
    ...
    if (applyq) {
        ...
        if (nq >= k) {
            ...
            #else
            magma_tally3_cunmqr( side, trans,
                          m, n, k, A, lda, tau, C, ldc, work, lwork, &iinfo);
            #endif
        }
        else if (nq > 1) {
        ...
        if (nq > k) {
            ...
            #if VERSION == 1
            lapackf77_cunmlq( lapack_side_const_tally3(side), lapack_trans_const_tally3(transt),
                              &m, &n, &k, A, &lda, tau, C, &ldc, work, &lwork, &iinfo);
            #else
            magma_tally3_cunmlq( side, transt,
                          m, n, k, A, lda, tau, C, ldc, work, lwork, &iinfo);
            #endif
        }
        else if (nq > 1) {
