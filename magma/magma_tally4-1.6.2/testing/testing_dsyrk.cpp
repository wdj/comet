...
int main( int argc, char** argv)
{
    ...
    magma_tally4Double_ptr d_A, d_C;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            /* =====================================================================
               ...
               =================================================================== */
            magma_tally4_dsetmatrix( An, Ak, h_A, lda, d_A, ldda );
            magma_tally4_dsetmatrix( N, N, h_C, ldc, d_C, lddc );

            cublas_time = magma_tally4_sync_wtime( NULL );
            cublasDsyrk( opts.handle, cublas_uplo_const_tally4(opts.uplo), cublas_trans_const_tally4(opts.transA), N, K,
                         &alpha, d_A, ldda,
                         &beta,  d_C, lddc );
            cublas_time = magma_tally4_sync_wtime( NULL ) - cublas_time;
            cublas_perf = gflops / cublas_time;
            
            magma_tally4_dgetmatrix( N, N, d_C, lddc, h_Ccublas, ldc );
            ...
            if ( opts.lapack ) {
                ...
                blasf77_dsyrk( lapack_uplo_const_tally4(opts.uplo), lapack_trans_const_tally4(opts.transA), &N, &K,
                               &alpha, h_A, &lda,
                               &beta,  h_C, &ldc );
            ...
            if ( opts.lapack ) {
                ...
                Cnorm = lapackf77_dlansy("fro", lapack_uplo_const_tally4(opts.uplo), &N, h_C, &ldc, work);

                blasf77_daxpy( &sizeC, &c_neg_one, h_C, &ione, h_Ccublas, &ione );
                cublas_error = lapackf77_dlansy( "fro", lapack_uplo_const_tally4(opts.uplo), &N, h_Ccublas, &ldc, work ) / Cnorm;
