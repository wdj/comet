...
/**
    ...
    This version stores the T matrices, for later use in magma_tally4_dorghr.
    ...
    magma_tally4_int_t *info)
{
    ...
    magma_tally4_int_t nb = magma_tally4_get_dgehrd_nb(n);
    ...
    *info = 0;
    iws = n*(nb + nb*ngpu);
    work[0] = MAGMA_tally4_D_MAKE( iws, 0 );
    ...
    if (nb == 1 || nb >= nh) {
        ...
        for( d = 0; d < ngpu; ++d ) {
            ...
            ldwork = nlocal*ldda   // A
                   ...
                   + nb*ldda       // W
                   + nb*nb;        // Ti
            if ( MAGMA_tally4_SUCCESS != magma_tally4_dmalloc( &data.A[d], ldwork )) {
        ...
        magma_tally4_dsetmatrix_1D_col_bcyclic( n, n, A, lda, data.A, ldda, ngpu, nb );
        ...
        for (i = ilo; i < ihi - 1 - nb; i += nb) {
            ...
            if ( i > ilo ) {
                ...
                magma_tally4_dgetmatrix( ihi-i, nb,
                                  dA(dpanel, i, di), ldda,
                                  A(i,i),            lda );
            }
            
            // add 1 to i for 1-based index
            magma_tally4_dlahr2_m( ihi, i+1, nb, A(0,i), lda,
                            &tau[i], &T[i*nb], nb, work, n, &data );
            
            magma_tally4_dlahru_m( n, ihi, i, nb, A, lda, &data );
            
            // copy first i rows above panel to host
            magma_tally4_setdevice( dpanel );
            magma_tally4_dgetmatrix_async( i, nb,
                                    dA(dpanel, 0, di), ldda,
