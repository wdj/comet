...
/**
    ...
    lwork   INTEGER
            ...
            through magma_tally3_get_sgeqlf_nb(M).
    \n
...
magma_tally3_sgeqlf(
    ...
    float *A,    magma_tally3_int_t lda, float *tau,
    ...
    magma_tally3_int_t *info)
{
    ...
    magma_tally3Float_ptr dA, dwork;
    float c_one = MAGMA_tally3_S_ONE;
    magma_tally3_int_t i, k, lddwork, old_i, old_ib, nb;
    magma_tally3_int_t rows, cols;
    magma_tally3_int_t ib, ki, kk, mu, nu, iinfo, ldda;
    int lquery;

    nb = magma_tally3_get_sgeqlf_nb(m);
    *info = 0;
    lquery = (lwork == -1);
    ...
    if (*info == 0) {
        if (k == 0)
            ...
            work[0] = MAGMA_tally3_S_MAKE( max(n*nb, 2*nb*nb), 0 );
        }
    ...
    if (MAGMA_tally3_SUCCESS != magma_tally3_smalloc( &dA, (n)*ldda + nb*lddwork )) {
    ...
    if ( (nb > 1) && (nb < k) ) {
        ...
        magma_tally3_ssetmatrix_async( m, n-nb,
                                A(0, 0),  lda,
        ...
        for (i = k - kk + ki; i >= k -kk; i -= nb) {
            ...
            if (i < k - kk + ki) {
                /* 1. Copy asynchronously the current panel to the CPU.
                   ...
                   to the CPU)                                        */
                rows = m - k + i + ib;
                magma_tally3_sgetmatrix_async( rows, ib,
                                        dA(0, n-k+i), ldda,
                                        A(0, n-k+i),  lda, queues[1] );

                magma_tally3_sgetmatrix_async( m-rows, ib,
                                        dA(rows, n-k+i), ldda,
                ...
                magma_tally3_slarfb_gpu( Magma_tally3Left, Magma_tally3ConjTrans, Magma_tally3Backward, Magma_tally3Columnwise,
                                  rows, cols, old_ib,
                                  dA(0, cols+old_ib), ldda, dwork(0),      lddwork,
                                  dA(0, 0          ), ldda, dwork(old_ib), lddwork);
            }

            magma_tally3_queue_sync( queues[1] );
            ...
            if (cols > 0) {
                ...
                lapackf77_slarft( Magma_tally3BackwardStr, Magma_tally3ColumnwiseStr,
                                  &rows, &ib,
                                  A(0, cols), &lda, tau + i, work, &ib);

                spanel_to_q_tally3( Magma_tally3Lower, ib, A(rows-ib,cols), lda, work+ib*ib);
                magma_tally3_ssetmatrix( rows, ib,
                                  A(0,cols),  lda,
                                  dA(0,cols), ldda );
                sq_to_panel_tally3( Magma_tally3Lower, ib, A(rows-ib,cols), lda, work+ib*ib);

                // Send the triangular part on the GPU
                magma_tally3_ssetmatrix( ib, ib, work, ib, dwork(0), lddwork );
        ...
        magma_tally3_sgetmatrix( m, nu, dA(0,0), ldda, A(0,0), lda );
    } else {
        mu = m;
        nu = n;
    }
