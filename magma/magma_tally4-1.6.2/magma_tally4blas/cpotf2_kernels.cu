...
/////////////////////////////////////////////////////////////////////////////////////////////////
__global__ void cdotc_kernel_batched(int n, magma_tally4FloatComplex **x_array, int incx, int offset, magma_tally4_int_t *info_array, int gbstep)
{
    int tx = threadIdx.x;

    magma_tally4FloatComplex *x = x_array[blockIdx.z]+offset;
    ...
    sdata[tx] = MAGMA_tally4_C_REAL(res * MAGMA_tally4_C_CNJG(res));
    ...
    if (tx == 0) {
        float xreal = MAGMA_tally4_C_REAL(x[n*incx]);        
        //MAGMA_tally4_C_SET2REAL(x[n*incx], sqrt(xreal - sdata[0]));
        x[n*incx] = MAGMA_tally4_C_MAKE(sqrt(xreal - sdata[0]), 0);
        if(x[n*incx] == MAGMA_tally4_C_ZERO){
...
void magma_tally4_cpotf2_cdotc_batched(magma_tally4_int_t n, magma_tally4FloatComplex **x_array, magma_tally4_int_t incx, magma_tally4_int_t offset, magma_tally4_int_t *info_array, magma_tally4_int_t gbstep, magma_tally4_int_t batchCount, magma_tally4_queue_t queue)
{
/*
    Specialized Cdotc
...
/////////////////////////////////////////////////////////////////////////////////////////////////
__global__ void csscal_kernel_batched(int n, magma_tally4FloatComplex **x_array, int incx, int offset, magma_tally4_int_t *info_array)
{
    ...
    __shared__ magma_tally4FloatComplex factor;

    if (threadIdx.x == 0) {
        factor = MAGMA_tally4_C_MAKE(1.0/MAGMA_tally4_C_REAL(x[0]), 0.0);
    }
...
void magma_tally4_cpotf2_csscal_batched(magma_tally4_int_t n, magma_tally4FloatComplex **x_array, magma_tally4_int_t incx, magma_tally4_int_t offset, magma_tally4_int_t *info_array, magma_tally4_int_t batchCount, magma_tally4_queue_t queue)
{
/*
...
__global__ void clacgv_kernel_batched(int n, magma_tally4FloatComplex **x_array, int incx, int offset)
{
    int id = threadIdx.x;
    ...
    if ( id < n ) {
        x[id*incx] = MAGMA_tally4_C_CNJG(x[id*incx]);
    }
}

void magma_tally4_clacgv_batched(magma_tally4_int_t n, magma_tally4FloatComplex **x_array, magma_tally4_int_t incx, int offset, int batchCount, magma_tally4_queue_t queue)
{
/*
    Purpose
    =======
...
static __device__ void cpotf2_device(int m, int n, 
                              magma_tally4FloatComplex *A, int lda, 
                              magma_tally4FloatComplex alpha, 
                              magma_tally4FloatComplex beta, magma_tally4_int_t *info, int gbstep)
{
/*
...
*/
    ...
    int tx = threadIdx.x;
    magma_tally4FloatComplex *sdata_A = shared_data;
    __shared__ magma_tally4FloatComplex factor;
    ...
    for(int iter=0; iter<n; iter++)
    {
        ...
        magma_tally4FloatComplex res1 = MAGMA_tally4_C_ZERO;

        //1) performs cdotc sum = A[iter, 0:iter-1]*conj(A[iter, 0:iter-1])
        //2) updates A[iter,iter] = sqrt(A[iter,iter]-sum);
        if(tx<iter)
        {
            res = MAGMA_tally4_C_REAL (sdata_A[iter + tx * m] * MAGMA_tally4_C_CNJG(sdata_A[iter + tx * m]));         
            sum[tx] = res;
        }
        else
        {
        ...
        __syncthreads();
        magma_tally4_sum_reduce<POTF2_TILE_SIZE>(tx, sum);//tried on K40: if m=32 n=32 the overall cpotf2_device routine time is 60ms n=16 time=25 n=8 time=20ms 
        ...
        if (tx == 0) {
              float xreal = MAGMA_tally4_C_REAL(sdata_A[iter + iter * m]);        
              sdata_A[iter + iter * m] = MAGMA_tally4_C_MAKE(sqrt(xreal - sum[0]), 0);
              if(sdata_A[iter + iter * m] == MAGMA_tally4_C_ZERO){
                  *info = iter + gbstep + 1;
              }
        }
        __syncthreads();
        if(sdata_A[iter + iter * m] == MAGMA_tally4_C_ZERO) return;
        __syncthreads();
        ...
        if(tx < iter)
        {
             sdata_A[iter + tx * m] = MAGMA_tally4_C_CNJG(sdata_A[iter + tx * m]);
        }
        __syncthreads();  
        #endif
        ...
        if (tx == 0) {
            factor = MAGMA_tally4_C_MAKE(1.0/MAGMA_tally4_C_REAL(sdata_A[iter + iter * m]), 0.0);
        }
        __syncthreads();
...
__global__ void cpotf2_kernel(int m, int n, 
                              magma_tally4FloatComplex *dA, int lda, 
                              magma_tally4FloatComplex alpha, 
                              magma_tally4FloatComplex beta,
...
/**
    ...
    @ingroup magma_tally4_cposv_aux
    ********************************************************************/
    ...
    magma_tally4_int_t *info_array, magma_tally4_int_t gbstep, magma_tally4_int_t batchCount, magma_tally4_queue_t queue)
{
    ...
    int shared_mem_size = sizeof(magma_tally4FloatComplex)*m*n; // + sizeof(float)*(POTF2_TILE_SIZE+1);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
extern "C" magma_tally4_int_t
magma_tally4_cpotf2_tile(
    magma_tally4_uplo_t uplo, magma_tally4_int_t m, magma_tally4_int_t n,
    magma_tally4FloatComplex *dA, magma_tally4_int_t lda,
    magma_tally4_int_t *info)
{
    ...
    *info = 0;
    ...
    cpotf2_kernel<<<dimGrid, threads, shared_mem_size, magma_tally4_stream >>>(m, n, dA, lda, alpha, beta, info);
