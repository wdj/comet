...
class magma_tally3_dlaqtrsd_task: public magma_tally3_task
{
public:
    magma_tally3_dlaqtrsd_task(
        magma_tally3_trans_t in_trans, magma_tally3_int_t in_n,
        const double *in_T, magma_tally3_int_t in_ldt,
        double       *in_x, magma_tally3_int_t in_incx,
    ...
    virtual void run()
    {
        ...
        magma_tally3_dlaqtrsd( trans, n, T, ldt, x, incx, cnorm, &info );
        if ( info != 0 ) {
...
class dgemm_task: public magma_tally3_task
{
public:
    dgemm_task(
        ...
        double  in_alpha,
        const double *in_A, magma_tally3_int_t in_lda,
        const double *in_B, magma_tally3_int_t in_ldb,
        double  in_beta,
        double *in_C, magma_tally3_int_t in_ldc
    ):
        transA( in_transA ),
        transB( in_transB ),
        m     ( in_m      ),
    ...
    virtual void run()
    {
        blasf77_dgemm( lapack_trans_const_tally3(transA), lapack_trans_const_tally3(transB),
...
/**
    ...
    @ingroup magma_tally3_dgeev_comp
    ********************************************************************/
extern "C"
magma_tally3_int_t magma_tally3_dtrevc3_mt(
...
#define work(i,j) (work + (i) + (j)*n)
    ...
    const magma_tally3_int_t nbmin = 16, nbmax = 256;
    
    // .. Local Scalars ..
    magma_tally3_int_t allv, bothv, leftv, over, pair, rightv, somev;
    magma_tally3_int_t i, ii, ip, is, j, k, ki, ki2,
                iv, n2, nb, nb2, version;
    double emax, remax;
    
    // .. Local Arrays ..
    // since iv is a 1-based index, allocate one extra here
    magma_tally3_int_t iscomplex[ nbmax+1 ];
    ...
    if ( rightv ) {
        ...
        for( ki=n-1; ki >= 0; --ki ) {
            ...
            if ( ip == 0 ) {
                ...
                queue.push_task( new magma_tally3_dlaqtrsd_task(
                    Magma_tally3NoTrans, ki+1, T(0,0), ldt, work(0,iv), n, work(0,0) ));
                ...
                queue.push_task( new magma_tally3_dlaqtrsd_task(
                    Magma_tally3NoTrans, ki+1, T(0,0), ldt, work(0,iv-1), n, work(0,0) ));
    ...
    if ( leftv ) {
        ...
        for( ki=0; ki < n; ++ki ) {
            ...
            if ( ip == 0 ) {
                ...
                queue.push_task( new magma_tally3_dlaqtrsd_task(
                    Magma_tally3Trans, n-ki, T(ki,ki), ldt, work(ki,iv), n, work(ki,0) ));
