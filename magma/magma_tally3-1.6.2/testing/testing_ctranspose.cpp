...
int main( int argc, char** argv)
{
    ...
    magma_tally3FloatComplex *h_A, *h_B, *h_R;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            gbytes = sizeof(magma_tally3FloatComplex) * 2.*M*N / 1e9;
            
            TESTING_MALLOC_CPU( h_A, magma_tally3FloatComplex, lda*N  );  // input:  M x N
            TESTING_MALLOC_CPU( h_B, magma_tally3FloatComplex, ldb*M  );  // output: N x M
            TESTING_MALLOC_CPU( h_R, magma_tally3FloatComplex, ldb*M  );  // output: N x M
            
            TESTING_MALLOC_DEV( d_A, magma_tally3FloatComplex, ldda*N );  // input:  M x N
            TESTING_MALLOC_DEV( d_B, magma_tally3FloatComplex, lddb*M );  // output: N x M
            ...
            for( int j = 0; j < M; ++j ) {
                for( int i = 0; i < N; ++i ) {
                    h_B[i + j*ldb] = MAGMA_tally3_C_MAKE( i + j/10000., j );
                }
            }
            magma_tally3_csetmatrix( N, M, h_B, ldb, d_B, lddb );
            ...
            //magma_tally3blas_ctranspose( M-2, N-2, d_A+1+ldda, ldda, d_B+1+lddb, lddb );  // inset by 1 row & col
            magma_tally3blas_ctranspose( M, N, d_A, ldda, d_B, lddb );
            ...
            if ( M == N ) {
                ...
                //magma_tally3blas_ctranspose_inplace( N-2, d_A+1+ldda, ldda );  // inset by 1 row & col
                magma_tally3blas_ctranspose_inplace( N, d_A, ldda );
