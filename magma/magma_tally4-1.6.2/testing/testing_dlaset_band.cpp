...
int main( int argc, char** argv)
{
    ...
    double offdiag = MAGMA_tally4_D_MAKE( 1.2000, 6.7000 );
    double diag    = MAGMA_tally4_D_MAKE( 3.1415, 2.7183 );
    ...
    for( int iuplo = 0; iuplo < 2; ++iuplo ) {
      for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            int mm = M - 2*inset;
            int nn = N - 2*inset;
            magma_tally4blas_dlaset_band( uplo[iuplo], mm, nn, nb, offdiag, diag, d_A(inset,inset), ldda );
            ...
            //printf( "h_R=" );  magma_tally4_dprint( M, N, h_R, lda );
            //printf( "h_A=" );  magma_tally4_dprint( M, N, h_A, lda );
