...
__global__ void ctranspose_inplace_odd( int n, magma_tally2FloatComplex *matrix, int lda )
{
    __shared__ magma_tally2FloatComplex sA[ NB ][ NB+1 ];
    __shared__ magma_tally2FloatComplex sB[ NB ][ NB+1 ];
    ...
    magma_tally2FloatComplex *A = matrix + ii+i + (jj+j)*lda;
    if( ii == jj ) {
        if ( ii+i < n && jj+j < n ) {
            sA[j][i] = *A;
        }
        ...
        magma_tally2FloatComplex *B = matrix + jj+i + (ii+j)*lda;
        if ( ii+i < n && jj+j < n ) {
            sA[j][i] = *A;
        }
        if ( jj+i < n && ii+j < n ) {
...
__global__ void ctranspose_inplace_even( int n, magma_tally2FloatComplex *matrix, int lda )
{
    __shared__ magma_tally2FloatComplex sA[ NB ][ NB+1 ];
    __shared__ magma_tally2FloatComplex sB[ NB ][ NB+1 ];
...
/**
    @see magma_tally2blas_ctranspose_inplace_q
    @ingroup magma_tally2_caux2
    ********************************************************************/
extern "C" void
magma_tally2blas_ctranspose_inplace(
    ...
    magma_tally2FloatComplex_ptr dA, magma_tally2_int_t ldda )
{
    magma_tally2blas_ctranspose_inplace_q( n, dA, ldda, magma_tally2_stream );
}
