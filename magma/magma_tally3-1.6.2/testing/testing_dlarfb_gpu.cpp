/*
    -- MAGMA_tally3 (version 1.6.1) --
       ...
       @author Mark Gates
       @generated from testing_zlarfb_gpu.cpp normal z -> d, Fri Jan 30 19:00:25 2015
*/
// includes, system
#include <stdlib.h>
#include <stdio.h>
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int itran = 0; itran < 2; ++itran ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {            
            ...
            magma_tally3Double_ptr dC, dV, dT, dW;
            ...
            //printf( "C=" );  magma_tally3_dprint( M, N, C, ldc );
            ...
            if ( storev[istor] == Magma_tally3Columnwise ) {
                if ( direct[idir] == Magma_tally3Forward ) {
                    lapackf77_dlaset( Magma_tally3UpperStr, &K, &K, &c_zero, &c_one, V, &ldv );
                }
                else {
                    lapackf77_dlaset( Magma_tally3LowerStr, &K, &K, &c_zero, &c_one, &V[(ldv-K)], &ldv );
                }
            }
            else {
                ...
                std::swap( ldv, nv );
                if ( direct[idir] == Magma_tally3Forward ) {
                    lapackf77_dlaset( Magma_tally3LowerStr, &K, &K, &c_zero, &c_one, V, &ldv );
                }
                else {
                    lapackf77_dlaset( Magma_tally3UpperStr, &K, &K, &c_zero, &c_one, &V[(nv-K)*ldv], &ldv );
                }
            }
            //printf( "# ldv %d, nv %d\n", ldv, nv );
            //printf( "V=" );  magma_tally3_dprint( ldv, nv, V, ldv );
            ...
            if ( direct[idir] == Magma_tally3Forward ) {
                lapackf77_dlaset( Magma_tally3LowerStr, &k1, &k1, &c_zero, &c_zero, &T[1], &ldt );
            }
            else {
                lapackf77_dlaset( Magma_tally3UpperStr, &k1, &k1, &c_zero, &c_zero, &T[1*ldt], &ldt );
            }
            //printf( "T=" );  magma_tally3_dprint( K, K, T, ldt );
            
            magma_tally3_dsetmatrix( M,   N,  C, ldc, dC, ldc );
            magma_tally3_dsetmatrix( ldv, nv, V, ldv, dV, ldv );
            magma_tally3_dsetmatrix( K,   K,  T, ldt, dT, ldt );
            
            lapackf77_dlarfb( lapack_side_const_tally3( side[iside] ), lapack_trans_const_tally3( trans[itran] ),
                              lapack_direct_const_tally3( direct[idir] ), lapack_storev_const_tally3( storev[istor] ),
                              &M, &N, &K,
                              V, &ldv, T, &ldt, C, &ldc, W, &ldw );
            //printf( "HC=" );  magma_tally3_dprint( M, N, C, ldc );
            
            magma_tally3_dlarfb_gpu( side[iside], trans[itran], direct[idir], storev[istor],
                              M, N, K,
                              dV, ldv, dT, ldt, dC, ldc, dW, ldw );
            magma_tally3_dgetmatrix( M, N, dC, ldc, R, ldc );
            //printf( "dHC=" );  magma_tally3_dprint( M, N, R, ldc );
