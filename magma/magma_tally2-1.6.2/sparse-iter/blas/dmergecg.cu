...
__global__ void
magma_tally2_dcgreduce_kernel_spmv1( 
    int Gs,
    int n, 
    double * vtmp,
    double * vtmp2 )
...
__global__ void
magma_tally2_dcgmerge_spmvcsr_kernel(  
    int n,
    double * dval, 
...
__global__ void
magma_tally2_dcgmerge_spmvell_kernel(  
    int n,
...
__global__ void
magma_tally2_dcgmerge_spmvellpack_kernel(  
    int n,
    int num_cols_per_row,
    double * dval, 
    magma_tally2_index_t * dcolind,
...
__global__ void
magma_tally2_dcgmerge_spmvellpackrt_kernel_8(  
    int n,
    double * dval, 
    magma_tally2_index_t * dcolind,
...
// computes the SpMV using ELLRT 8 threads per row
__global__ void
magma_tally2_dcgmerge_spmvellpackrt_kernel_16(  
    int n,
    double * dval, 
    magma_tally2_index_t * dcolind,
    magma_tally2_index_t * drowlength,
...
// computes the SpMV using ELLRT 8 threads per row
__global__ void
magma_tally2_dcgmerge_spmvellpackrt_kernel_32(  
    int n,
    double * dval, 
    magma_tally2_index_t * dcolind,
    magma_tally2_index_t * drowlength,
...
__global__ void
magma_tally2_dcgmerge_spmvellpackrt_kernel2(  
    int n,
    double * z,
    double * d,
    double * vtmp2 )
{
    ...
    temp[ Idx ] = ( i < n ) ? z[i]*d[i] : MAGMA_tally2_D_MAKE(0.0, 0.0);
    __syncthreads();
    if ( Idx < 128 ){
        temp[ Idx ] += temp[ Idx + 128 ];
    }
...
__global__ void
magma_tally2_dcgmerge_spmvsellc_kernel(   
    int num_rows, 
    int blocksize,
    double * dval, 
    magma_tally2_index_t * dcolind,
...
__global__ void
magma_tally2_dcgmerge_spmvsellpt_kernel_8( 
    int num_rows, 
    int blocksize,
    int T,
    double * dval, 
...
// SELLC SpMV kernel modified assigning multiple threads to each row - 2D kernel
__global__ void
magma_tally2_dcgmerge_spmvsellpt_kernel_16( 
    int num_rows, 
    int blocksize,
    int T,
    double * dval, 
...
// SELLC SpMV kernel modified assigning multiple threads to each row - 2D kernel
__global__ void
magma_tally2_dcgmerge_spmvsellpt_kernel_32( 
    int num_rows, 
    int blocksize,
    int T,
    double * dval, 
...
/**
    ...
    @param[in]
    dd          magma_tally2Double_ptr 
                input vector d

    @param[out]
    dz          magma_tally2Double_ptr 
                input vector z
    ...
    magma_tally2_queue_t queue )
{
    ...
    if ( A.storage_type == Magma_tally2_CSR )
        ...
        magma_tally2_dcgmerge_spmvellpackrt_kernel2<<<Gs, Bs, Ms, queue >>>
                              ( A.num_rows, dz, dd, d1 );
    ...
    while( Gs.x > 1 ) {
        ...
        magma_tally2_dcgreduce_kernel_spmv1<<< Gs_next.x/2, Bs.x/2, Ms/2 >>> 
    ...
    dim3 Bs2( 2 );
    dim3 Gs2( 1 );
    magma_tally2_dcg_rhokernel<<<Gs2, Bs2, 0>>>( skp );
...
__global__ void
magma_tally2_dcgmerge_xrbeta_kernel(  
    int n, 
    double * x, 
    double * r,
    double * d,
    ...
    double * vtmp )
{
    ...
    double rho = skp[3];
    double mrho = MAGMA_tally2_D_MAKE( -1.0, 0.0)*rho;
...
magma_tally2_dcg_alphabetakernel(  
    double * skp )
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if( i==0 ){
        double tmp1 = skp[1];
        skp[0] =  tmp1/skp[2];
        //printf("beta=%e\n", MAGMA_tally2_D_REAL(tmp1));
    }
}

// update search Krylov vector d
__global__ void
magma_tally2_dcg_d_kernel(  
    int n, 
    double * skp,
    double * r,
    double * d )
...
/**
    ...
    @ingroup magma_tally2sparse_dsygpuk
    ********************************************************************/
    ...
    magma_tally2_queue_t queue )
{
    ...
    magma_tally2Double_ptr aux1 = d1, aux2 = d2;
    int b = 1;        
    magma_tally2_dcgmerge_xrbeta_kernel<<<Gs, Bs, Ms>>>
    ...
    magma_tally2_dcopyvector( 1, aux1, 1, skp+1, 1 );
    dim3 Bs2( 2 );
    dim3 Gs2( 1 );
    magma_tally2_dcg_alphabetakernel<<<Gs2, Bs2, 0>>>( skp );

    dim3 Bs3( local_block_size );
    dim3 Gs3( magma_tally2_ceildiv( n, local_block_size ) );
    magma_tally2_dcg_d_kernel<<<Gs3, Bs3, 0>>>( n, skp, dr, dd );  
