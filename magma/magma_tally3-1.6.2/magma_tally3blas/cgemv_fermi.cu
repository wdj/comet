    ...
    magma_tally3FloatComplex       * __restrict__ y, int incy)
{
    ...
    __shared__ magma_tally3FloatComplex sx[BLK_Y];
    ...
    if ( threadIdx.x < npart ) {
        ...
        sx[threadIdx.x] = MAGMA_tally3_C_ZERO;
    }
    __syncthreads();
    ...
    magma_tally3FloatComplex       * __restrict__ y, int incy)
{
    ...
    __shared__ magma_tally3FloatComplex sdata[BLK_X];
...
magma_tally3blas_cgemv(
    magma_tally3_trans_t trans, magma_tally3_int_t m, magma_tally3_int_t n, magma_tally3FloatComplex alpha,
    ...
    magma_tally3FloatComplex_ptr dy, magma_tally3_int_t incy)
{
    ...
    if ( arch < 200  ) {
        ...
        magma_tally3_cgemv( trans, m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
        #else
        magma_tally3blas_cgemv_tesla( trans, m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
        #endif
        return;
    }
    ...
    if ( trans == Magma_tally3NoTrans ) {
        ...
        cgemvn_kernel1_fermi<<< grid, threads, 0, magma_tally3_stream >>>
            ( m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    }
    else if ( trans == Magma_tally3Trans ) {
        dim3 grid    ( 1, n, 1 );
        dim3 threads ( BLK_X, 1, 1 );
        cgemvt_kernel_fermi<<< grid, threads, 0, magma_tally3_stream >>>
            ( m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    }
    else if ( trans == Magma_tally3ConjTrans ) {
        dim3 grid    ( 1, n, 1 );
        dim3 threads ( BLK_X, 1, 1 );
        cgemvc_kernel_fermi<<< grid, threads, 0, magma_tally3_stream >>>
            ( m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    }
}
