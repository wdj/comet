...
magma_tally2_dgetrf(
    magma_tally2_int_t m, magma_tally2_int_t n, double *A, magma_tally2_int_t lda,
    ...
    *info = 0;
    ...
    if ( (nb <= 1) || (nb >= min(m,n)) ) {
        ...
        if ( ngpu > 1 ) {
            ...
            magma_tally2_dgetrf_m(ngpu, m, n, A, lda, ipiv, info);
            return *info;
        }
        ...
        if (maxdim*maxdim < 2*maxm*maxn) {
            ...
            if (MAGMA_tally2_SUCCESS != magma_tally2_dmalloc( &dA, nb*maxm + maxdim*maxdim )) {
            ...
            ldda = maxdim;
            magma_tally2_dsetmatrix( m, n, A, lda, da, ldda );
            ...
            if (MAGMA_tally2_SUCCESS != magma_tally2_dmalloc( &dA, (nb + maxn)*maxm )) {
                /* alloc failed so call non-GPU-resident version */
                magma_tally2_dgetrf_m(ngpu, m, n, A, lda, ipiv, info);
                return *info;
            }
            ...
            magma_tally2_dsetmatrix( m, n, A, lda, da, maxm );
            
            if (MAGMA_tally2_SUCCESS != magma_tally2_dmalloc( &dAT, maxm*maxn )) {
                /* alloc failed so call non-GPU-resident version */
            ...
            magma_tally2blas_dtranspose( m, n, da, maxm, dAT, ldda );
        }
        ...
        for( j = 0; j < s; j++ ) {
            ...
            if (j > 0) {
                magma_tally2blas_dtranspose( nb, cols, dAT(j,j), ldda, dA, cols );

                // make sure that gpu queue is empty
                magma_tally2_device_sync();

                magma_tally2_dgetmatrix_async( m-j*nb, nb, dA, cols, work, lda,
                                        stream[0]);
            ...
            // upload j-th panel
            magma_tally2_dsetmatrix_async( m-j*nb, nb, work, lda, dA, cols,
                                    stream[0]);

            for( i=j*nb; i < j*nb + nb; ++i ) {
                ipiv[i] += j*nb;
            }
            magma_tally2blas_dlaswp( n, dAT, ldda, j*nb + 1, j*nb + nb, ipiv, 1 );

            magma_tally2_queue_sync( stream[0] );
            magma_tally2blas_dtranspose( cols, nb, dA, cols, dAT(j,j), ldda );
        ...
        if ( nb0 > 0 ) {
            ...
            magma_tally2blas_dtranspose( nb0, rows, dAT(s,s), ldda, dA, cols );
            magma_tally2_dgetmatrix( rows, nb0, dA, cols, work, lda );
            ...
            magma_tally2blas_dlaswp( n, dAT, ldda, s*nb + 1, s*nb + nb0, ipiv, 1 );

            // upload j-th panel
            magma_tally2_dsetmatrix( rows, nb0, work, lda, dA, cols );
            magma_tally2blas_dtranspose( rows, nb0, dA, cols, dAT(s,s), ldda );
        ...
        if (maxdim*maxdim < 2*maxm*maxn) {
            ...
            magma_tally2blas_dtranspose( n, m, dAT, ldda, da, maxm );
            magma_tally2_dgetmatrix( m, n, da, maxm, A, lda );
