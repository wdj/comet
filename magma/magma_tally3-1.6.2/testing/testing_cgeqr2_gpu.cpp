...
int main( int argc, char** argv)
{
    ...
    magma_tally3FloatComplex *h_A, *h_R, *tau, *dtau, *h_work, tmp[1];
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            // warmup
            if ( opts.warmup ) {
                magma_tally3_cgeqr2_gpu( M, N, d_A, ldda, dtau, dwork, &info );
                magma_tally3_csetmatrix( M, N, h_R, lda, d_A, ldda );
            }
            ...
            if (info != 0)
                printf("magma_tally3_cgeqr2_gpu returned error %d: %s.\n",
