...
//
// We work around this bug by putting replacement routines in the magma_tally4/lib/libblas_fix.a library.
...
float test( magma_tally4_int_t m, magma_tally4_int_t n )
{
...
#define dA(i,j) (dA + (i) + (j)*lda)
    ...
    magma_tally4_int_t lda = MAX(m,n);
    ...
    for( magma_tally4_int_t i = 0; i < lda; ++i ) {
...
{
    magma_tally4_int_t m = 100;
    magma_tally4_int_t n = m;
    if ( argc > 1 ) {
        n = atoi( argv[1] );
    }
