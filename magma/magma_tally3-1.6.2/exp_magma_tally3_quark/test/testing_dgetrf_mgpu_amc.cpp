...
void init_matrix( int m, int n, double *h_A, magma_tally3_int_t lda )
{
...
double get_LU_error(magma_tally3_int_t M, magma_tally3_int_t N,
                    double *LU, magma_tally3_int_t lda,
                    magma_tally3_int_t *ipiv)
{
    magma_tally3_int_t min_mn = min(M,N);
    magma_tally3_int_t ione   = 1;
    magma_tally3_int_t i, j;
    double alpha = MAGMA_tally3_D_ONE;
    double beta  = MAGMA_tally3_D_ZERO;
    double *A, *L, *U;
    ...
    lapackf77_dlacpy( Magma_tally3LowerStr, &M, &min_mn, LU, &lda, L, &M      );
    lapackf77_dlacpy( Magma_tally3UpperStr, &min_mn, &N, LU, &lda, U, &min_mn );
    for(j=0; j<min_mn; j++)
        L[j+j*M] = MAGMA_tally3_D_MAKE( 1., 0. );
    ...
    for( j = 0; j < N; j++ ) {
        for( i = 0; i < M; i++ ) {
            LU[i+j*lda] = MAGMA_tally3_D_SUB( LU[i+j*lda], A[i+j*lda] );
        }
    }
...
int main( int argc, char** argv )
{
    ...
    double           error;
    double *h_A;
    double *d_lA[ Magma_tally3MaxGPUs ];
    magma_tally3_int_t *ipiv;
    magma_tally3_int_t M, N, n2, lda, ldda, n_local, ngpu;
    magma_tally3_int_t info, min_mn, nb, ldn_local;
    magma_tally3_int_t status = 0;

    magma_tally3_int_t P=-1;    /*Number of threads in the CPU part*/
    double d_cpu=-1;    /*pourcentgae of the matrix to allocate in the cpu part*/
    magma_tally3_int_t Pr=-1;  /*Number of threads for the panel*/
    magma_tally3_int_t async_nb; /*Block size*/
    
    double *WORK;
    magma_tally3_int_t WORK_LD, WORK_n;
    ...
    d_cpu = magma_tally3_amc_recommanded_dcpu(opts.nthread, CPU_PEAK, opts.ngpu, GPU_PEAK);
    #endif
    if(opts.fraction_dcpu!=0){ /*Overwrite the one computed with the model*/
    d_cpu = opts.fraction_dcpu;
    }
    magma_tally3_assert(d_cpu > 0 && d_cpu<=1.0,
    ...
    printf("  M     N     CPU GFlop/s (sec)   GPU GFlop/s (sec)   GPU_Async_v1 GFlop/s (sec)  GPU_Async_work_v1 GFlop/s (sec)");
    if ( opts.check == 2 ) {
        printf("   |Ax-b|/(N*|A|*|x|)\n");
    }
    else {
    ...
    for( int i = 0; i < opts.ntest; ++i ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU(    ipiv, magma_tally3_int_t,        min_mn );
            TESTING_MALLOC_CPU(    h_A,  double, n2     );
            
            /*set default number of threads for lapack*/
            magma_tally3_setlapack_numthreads(P);
            /* =====================================================================
            ...
            init_matrix( M, N, h_A, lda );
            magma_tally3_dsetmatrix_1D_col_bcyclic( M, N, h_A, lda, d_lA, ldda, ngpu, nb );
    
            gpu_time1 = magma_tally3_wtime();
            magma_tally3_dgetrf_mgpu( ngpu, M, N, d_lA, ldda, ipiv, &info );
            gpu_time1 = magma_tally3_wtime() - gpu_time1;
            gpu_perf1 = gflops / gpu_time1;
            if (info != 0)
                printf("magma_tally3_dgetrf_mgpu returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
                       
            magma_tally3_dgetmatrix_1D_col_bcyclic( M, N, d_lA, ldda, h_A, lda, ngpu, nb );
            ...
            /* ====================================================================
               Performs operation using MAGMA_tally3_Async: This interface allocate workspace internally
               =================================================================== */

            /*For the benchmark we have 2 approaches*/
            /*1. use directly magma_tally3_amc */
            /*2. use magma_tally3_amc_work and add pinned memory time*/
            ...
            magma_tally3_setlapack_numthreads(1);

            magma_tally3_amc_init(P, d_cpu, Pr, nb);
            gpu_time2 = magma_tally3_wtime();
            magma_tally3_dgetrf_async_mgpu( ngpu, M, N, d_lA, ldda, ipiv, &info );
            gpu_time2 = magma_tally3_wtime() - gpu_time2;
            gpu_perf2 = gflops / gpu_time2;
            magma_tally3_amc_finalize();
            if (info != 0)
                printf("magma_tally3_dgetrf_mgpu returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
            ...
            */
            ...
            /* ====================================================================
               Performs operation using MAGMA_tally3_Async_Work
               =================================================================== */
            ...
            //Allocate workspace
            alloc_time = magma_tally3_wtime();
            if (MAGMA_tally3_SUCCESS != magma_tally3_dmalloc_pinned(&WORK, WORK_LD*WORK_n)) { 
            //if (MAGMA_tally3_SUCCESS != magma_tally3_dmalloc_cpu(&WORK, WORK_LD*WORK_n)) {
                info = MAGMA_tally3_ERR_HOST_ALLOC;
                printf("magma_tally3_dmalloc_pinned returned error %d: %s.\n     ", (int) info);
            }
            ...
            dlpanelT_n = M;
             dlpanelT = (double **)    malloc(ngpu*sizeof(double*));
              for(int dev=0;dev<ngpu;dev++){
                 ...
                 if (MAGMA_tally3_SUCCESS != magma_tally3_dmalloc(&dlpanelT[dev], dlpanelT_m*dlpanelT_n)) { 
            
                        info = MAGMA_tally3_ERR_DEVICE_ALLOC; 
                        printf("magma_tally3_dmalloc returned error %d: %s.\n     ", (int) info);
                }
              }

            alloc_time = magma_tally3_wtime() - alloc_time;

            //First touch the workspace with each thread. This may be needed to avoid using numactl --interleave
            //magma_tally3_amc_dmemset(WORK, 0.0, WORK_LD*WORK_n, 256, P); //nb
            ...
            magma_tally3_amc_init(P, d_cpu, Pr, nb);
            gpu_time3 = magma_tally3_wtime();
            magma_tally3_dgetrf_mgpu_work_amc( ngpu, M, N, d_lA, ldda, ipiv, &info, WORK, WORK_LD, WORK_n, dlpanelT, dlpanelT_m, dlpanelT_n);
            gpu_time3 = magma_tally3_wtime() - gpu_time3;
            ...
            //Free workspace
            free_time = magma_tally3_wtime();
            magma_tally3_free_pinned(WORK);
            free_time = magma_tally3_wtime() - free_time;

            for(int dev=0;dev<ngpu;dev++){
                magma_tally3_setdevice(dev);
                magma_tally3_free(dlpanelT[dev]);
            }
