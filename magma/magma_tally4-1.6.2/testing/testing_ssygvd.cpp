...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lda    = N;
            n2     = N*lda;
            nb     = magma_tally4_get_ssytrd_nb(N);
            #ifdef COMPLEX
            ...
            //lapackf77_slatms( &N, &N, "U", ISEED, "P", w1, &five, &d_ten,
            ...
            magma_tally4_smake_hpd( N, h_B, lda );
            lapackf77_slacpy( Magma_tally4FullStr, &N, &N, h_A, &lda, h_R, &lda );
            lapackf77_slacpy( Magma_tally4FullStr, &N, &N, h_B, &lda, h_S, &lda );
            
            /* warmup */
            if ( opts.warmup ) {
                magma_tally4_ssygvd( opts.itype, opts.jobz, opts.uplo,
                              ...
                              &info );
                if (info != 0)
                    printf("magma_tally4_ssygvd returned error %d: %s.\n",
            ...
            if ( opts.check && opts.jobz != Magma_tally4NoVec ) {
                ...
                if ( opts.itype == 1 || opts.itype == 2 ) {
                    ...
                    blasf77_ssymm("R", lapack_uplo_const_tally4(opts.uplo), &N, &N, &c_neg_one, h_B, &lda, h_work, &N, &c_one, h_S, &lda);
                    result[1] = lapackf77_slange("1", &N, &N, h_S, &lda, rwork) / N;
                }
                else if ( opts.itype == 3 ) {
                    lapackf77_slacpy( Magma_tally4FullStr, &N, &N, h_B, &lda, h_S, &lda);
                    blasf77_ssyrk(lapack_uplo_const_tally4(opts.uplo), "N", &N, &N, &d_neg_one, h_R, &lda, &d_one, h_S, &lda);
                    result[1] = lapackf77_slansy("1", lapack_uplo_const_tally4(opts.uplo), &N, h_S, &lda, rwork) / N
                              / lapackf77_slansy("1", lapack_uplo_const_tally4(opts.uplo), &N, h_B, &lda, rwork);
                }
                
                result[0] = 1.;
                result[0] /= lapackf77_slansy("1", lapack_uplo_const_tally4(opts.uplo), &N, h_A, &lda, rwork);
                result[0] /= lapackf77_slange("1", &N, &N, h_R, &lda, rwork);
                
                if ( opts.itype == 1 ) {
                    blasf77_ssymm("L", lapack_uplo_const_tally4(opts.uplo), &N, &N, &c_one, h_A, &lda, h_R, &lda, &c_zero, h_work, &N);
                    for(int i=0; i < N; ++i)
                        blasf77_sscal(&N, &w1[i], &h_R[i*N], &ione);
                    blasf77_ssymm("L", lapack_uplo_const_tally4(opts.uplo), &N, &N, &c_neg_one, h_B, &lda, h_R, &lda, &c_one, h_work, &N);
                    result[0] *= lapackf77_slange("1", &N, &N, h_work, &lda, rwork)/N;
                }
                else if ( opts.itype == 2 ) {
                    blasf77_ssymm("L", lapack_uplo_const_tally4(opts.uplo), &N, &N, &c_one, h_B, &lda, h_R, &lda, &c_zero, h_work, &N);
                    for(int i=0; i < N; ++i)
                        blasf77_sscal(&N, &w1[i], &h_R[i*N], &ione);
                    blasf77_ssymm("L", lapack_uplo_const_tally4(opts.uplo), &N, &N, &c_one, h_A, &lda, h_work, &N, &c_neg_one, h_R, &lda);
                    ...
                    for(int i=0; i < N; ++i)
                        blasf77_sscal(&N, &w1[i], &h_R[i*N], &ione);
                    blasf77_ssymm("L", lapack_uplo_const_tally4(opts.uplo), &N, &N, &c_one, h_B, &lda, h_work, &N, &c_neg_one, h_R, &lda);
                    result[0] *= lapackf77_slange("1", &N, &N, h_R, &lda, rwork)/N;
                }
                
                /*
                assert( lwork >= 2*N*N );
                lapackf77_ssyt21( &ione, lapack_uplo_const_tally4(opts.uplo), &N, &izero,
                                  h_A, &lda,
                                  w1, w1,
                                  h_R, &lda,
                                  h_R, &lda,
                ...
                */
                ...
                //lapackf77_slacpy( Magma_tally4FullStr, &N, &N, h_B, &lda, h_S, &lda );
                //
                //magma_tally4_ssygvd( opts.itype, Magma_tally4NoVec, opts.uplo,
            ...
            if ( opts.lapack ) {
                ...
                lapackf77_ssygvd( &opts.itype, lapack_vec_const_tally4(opts.jobz), lapack_uplo_const_tally4(opts.uplo),
