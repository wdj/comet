...
#define W(j) (W + ((j)%ngpu)*nb*ldw)
    ...
    float *d_panel[Magma_minproductMaxGPUs], *panel_local[Magma_minproductMaxGPUs];
    ...
    *info = 0;
    ...
    magma_minproductblas_stranspose( nb0, m, dAT(0,0,0), lddat, d_lAP[0], maxm );
    magma_minproduct_sgetmatrix_async( m, nb0,
                            d_lAP[0], maxm,
                            W(0),     ldw, queues[0][1] );
    trace_gpu_end( 0, 1 );
    ...
    for( j=0; j < s; j++ ) {
        ...
        for( dd=0; dd < ngpu; dd++ ) {
            ...
            magma_minproduct_ssetmatrix_async( rows, nb,
                                    W(j),     ldw,
        ...
        for( dd=0; dd < ngpu; dd++ ) {
            ...
            magma_minproductblas_slaswp_q( lddat, dAT(d,0,0), lddat, j*nb + 1, j*nb + nb, ipiv, 1, queues[d][0] );
            trace_gpu_end( d, 1 );
            d = (d+1) % ngpu;
        }
        ...
        for( dd=0; dd < ngpu; dd++ ) {
            ...
            if ( d == (j+1) % ngpu) {
                ...
                magma_minproductblas_stranspose( rows, nb, &d_lAP[d][(j%h)*nb*maxm], cols, panel_local[d], ldpan[d] );
            ...
            if ( d == (j+1) % ngpu ) {
                ...
                if ( nb0 > 0 ) {
                    ...
                    magma_minproductblas_stranspose( nb0, m-(j+1)*nb, dAT(d,loff,j_local), lddat, &d_lAP[d][((j+1)%h)*nb*maxm], ldda );
             
                    /* send the panel to cpu */
                    magma_minproduct_sgetmatrix_async( cols, nb0,
    ...
    if ( nb0 > 0 ) {
        ...
        for( d=0; d < ngpu; d++ ) {
            ...
            if ( d == id || n_local[d] > j_local2*nb ) {
                magma_minproduct_ssetmatrix_async( rows, nb0,
                                        W(s),     ldw,
                                        &d_lAP[d][(s%h)*nb*maxm],
                                        cols, queues[d][1] );
            }
        ...
        for( d=0; d < ngpu; d++ ) {
            ...
            magma_minproductblas_slaswp_q( lddat, dAT(d,0,0), lddat, s*nb + 1, s*nb + nb0, ipiv, 1, queues[d][0] );
        }
        ...
        for( d=0; d < ngpu; d++ ) {
            ...
            if ( d == id ) {
                ...
                magma_minproductblas_stranspose( rows, nb0, &d_lAP[d][(s%h)*nb*maxm], cols, panel_local[d], lddat );
                ...
                magma_minproductblas_stranspose( rows, nb0, &d_lAP[d][(s%h)*nb*maxm], cols, panel_local[d], nb );
                magma_minproduct_strsm( Magma_minproductRight, Magma_minproductUpper, Magma_minproductNoTrans, Magma_minproductUnit,
                             nb1, nb0, c_one,
                             panel_local[d],     nb,
