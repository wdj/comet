...
int main( int argc, char** argv)
{
    ...
    magma_minproductDoubleComplex *h_A, *h_T, *h_R, *tau, *h_work, tmp[1];
    magma_minproductDoubleComplex_ptr d_A,  d_T, ddA, dtau;
    magma_minproductDoubleComplex_ptr d_A2, d_T2, ddA2, dtau2;
    magma_minproductDouble_ptr dwork, dwork2;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_T,   magma_minproductDoubleComplex, N*N    );
        
            TESTING_MALLOC_PIN( h_R,   magma_minproductDoubleComplex, n2     );
        
            TESTING_MALLOC_DEV( d_A,   magma_minproductDoubleComplex, ldda*N );
            TESTING_MALLOC_DEV( d_T,   magma_minproductDoubleComplex, N*N    );
            TESTING_MALLOC_DEV( ddA,   magma_minproductDoubleComplex, N*N    );
            TESTING_MALLOC_DEV( dtau,  magma_minproductDoubleComplex, min_mn );
        
            TESTING_MALLOC_DEV( d_A2,  magma_minproductDoubleComplex, ldda*N );
            TESTING_MALLOC_DEV( d_T2,  magma_minproductDoubleComplex, N*N    );
            TESTING_MALLOC_DEV( ddA2,  magma_minproductDoubleComplex, N*N    );
            TESTING_MALLOC_DEV( dtau2, magma_minproductDoubleComplex, min_mn );
            ...
            // todo replace with magma_minproduct_zlaset
            magma_minproductblas_zlaset( Magma_minproductFull, N, N, c_zero, c_zero, ddA,  N );
            magma_minproductblas_zlaset( Magma_minproductFull, N, N, c_zero, c_zero, d_T,  N );
            magma_minproductblas_zlaset( Magma_minproductFull, N, N, c_zero, c_zero, ddA2, N );
            magma_minproductblas_zlaset( Magma_minproductFull, N, N, c_zero, c_zero, d_T2, N );
            ...
            TESTING_MALLOC_CPU( h_work, magma_minproductDoubleComplex, lwork );

            /* Initialize the matrix */
            lapackf77_zlarnv( &ione, ISEED, &n2, h_A );
            lapackf77_zlacpy( Magma_minproductUpperLowerStr, &M, &N, h_A, &lda, h_R, &lda );
            magma_minproduct_zsetmatrix( M, N, h_R, lda,  d_A, ldda );
            magma_minproduct_zsetmatrix( M, N, h_R, lda, d_A2, ldda );
            ...
            if (opts.version == 1)
                magma_minproduct_zgeqr2x_gpu(M, N, d_A, ldda, dtau, d_T, ddA, dwork, &info);
            else if (opts.version == 2)
                magma_minproduct_zgeqr2x2_gpu(M, N, d_A, ldda, dtau, d_T, ddA, dwork, &info);
            else if (opts.version == 3)
                magma_minproduct_zgeqr2x3_gpu(M, N, d_A, ldda, dtau, d_T, ddA, dwork, &info);
            else {
                printf( "call magma_minproduct_zgeqr2x4_gpu\n" );
                /*
                ...
                */
                magma_minproduct_zgeqr2x4_gpu(M, N, d_A, ldda, dtau, d_T, ddA, dwork, NULL, &info);
                //magma_minproduct_zgeqr2x4_gpu(M, N, d_A, ldda, dtau, d_T, ddA, dwork, &info, stream[1]);
                //magma_minproduct_zgeqr2x4_gpu(M, N, d_A2, ldda, dtau2, d_T2, ddA2, dwork2, &info, stream[0]);
                //magma_minproduct_zgeqr2x4_gpu(M, N, d_A2, ldda, dtau2, d_T2, ddA2, dwork2, &info, NULL);
                //gflops *= 2;
            }
            ...
            if (info != 0) {
                printf("magma_minproduct_zgeqr2x_gpu version %d returned error %d: %s.\n",
                       (int) opts.version, (int) info, magma_minproduct_strerror( info ));
            } 
            else {
                if ( opts.check ) {
                    /* =====================================================================
                       ...
                       =================================================================== */
                    magma_minproduct_zgetmatrix( M, N, d_A, ldda, h_R, M );
                    magma_minproduct_zgetmatrix( N, N, ddA, N,    h_T, N );
                    magma_minproduct_zgetmatrix( min_mn, 1, dtau, min_mn,   tau, min_mn );
                    ...
                    lapackf77_zlacpy( Magma_minproductUpperLowerStr, &M, &N, h_R, &M, h_A, &lda );
                    lapackf77_zlarft( Magma_minproductForwardStr, Magma_minproductColumnwiseStr,
                                      &M, &N, h_A, &lda, tau, h_work, &N);
                    //magma_minproduct_zgeqr2(&M, &N, h_A, &lda, tau, h_work, &info);
                    ...
                    magma_minproduct_zgetmatrix( N, N, d_T, N, h_T, N );
    
                    double terr = 0.;
                    for(int col=0; col < N; col++)
                        for(int row=0; row <= col; row++)
                            terr += (  MAGMA_minproduct_Z_ABS(h_work[row + col*N] - h_T[row + col*N])*
                                       MAGMA_minproduct_Z_ABS(h_work[row + col*N] - h_T[row + col*N])  );
                    terr = sqrt( terr );
