...
int main( int argc, char** argv)
{
    ...
    magma_minproductFloatComplex *h_A, *h_Q, *h_PT, *h_work;
    magma_minproductFloatComplex *taup, *tauq;
    float      *diag, *offdiag;
    float      eps, result[3] = {0., 0., 0.};
    magma_minproduct_int_t M, N, n2, lda, lhwork, info, minmn, nb;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( tauq,    magma_minproductFloatComplex, minmn );
            TESTING_MALLOC_CPU( taup,    magma_minproductFloatComplex, minmn );
            ...
            TESTING_MALLOC_PIN( h_work,  magma_minproductFloatComplex, lhwork );
            
            /* Initialize the matrices */
            lapackf77_clarnv( &ione, ISEED, &n2, h_A );
            lapackf77_clacpy( Magma_minproductUpperLowerStr, &M, &N, h_A, &lda, h_Q, &lda );
            
            /* ====================================================================
               Performs operation using MAGMA_minproduct
               =================================================================== */
            gpu_time = magma_minproduct_wtime();
            magma_minproduct_cgebrd( M, N, h_Q, lda,
                          diag, offdiag, tauq, taup,
                          h_work, lhwork, &info);
            gpu_time = magma_minproduct_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_minproduct_cgebrd returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                lapackf77_clacpy(Magma_minproductUpperLowerStr, &M, &N, h_Q, &lda, h_PT, &lda);
