...
__global__ void
magma_tally4_ccgreduce_kernel_spmv1( 
    int Gs,
    int n, 
...
__global__ void
magma_tally4_ccgmerge_spmvell_kernel(  
    int n,
    ...
    magma_tally4FloatComplex * vtmp )
{
    ...
    if(i < n ){
        ...
        for ( int k = 0; k < num_cols_per_row ; k ++){
            ...
            magma_tally4FloatComplex val = dval [ n * k + i ];
            if( val != 0)
                dot += val * d[ col ];
        }
        z[ i ] =  dot;
...
__global__ void
magma_tally4_ccgmerge_spmvellpack_kernel(  
    int n,
    int num_cols_per_row,
    magma_tally4FloatComplex * dval, 
    magma_tally4_index_t * dcolind,
    ...
    magma_tally4FloatComplex * vtmp )
{
    ...
    if(i < n ){
        ...
        for ( int k = 0; k < num_cols_per_row ; k ++){
            ...
            magma_tally4FloatComplex val = dval [ num_cols_per_row * i + k ];
            if( val != 0)
                dot += val * d[ col ];
        }
        z[ i ] =  dot;
...
__global__ void
magma_tally4_ccgmerge_spmvellpackrt_kernel_8(  
    int n,
    magma_tally4FloatComplex * dval, 
    magma_tally4_index_t * dcolind,
...
extern __shared__ magma_tally4FloatComplex shared[];
    ...
    if(i < n ){
        ...
        for ( int k = 0; k < max_ ; k++ ){
            ...
            //magma_tally4FloatComplex val = dval[ k*(T*alignment)+(i*T)+idp ];  
            //int col = dcolind [ k*(T*alignment)+(i*T)+idp ];    

            // new code (working for me)        
            magma_tally4FloatComplex val = dval[ k*(T)+(i*alignment)+idp ];
...
// computes the SpMV using ELLRT 8 threads per row
__global__ void
magma_tally4_ccgmerge_spmvellpackrt_kernel_16(  
    int n,
    magma_tally4FloatComplex * dval, 
    magma_tally4_index_t * dcolind,
    magma_tally4_index_t * drowlength,
...
// computes the SpMV using ELLRT 8 threads per row
__global__ void
magma_tally4_ccgmerge_spmvellpackrt_kernel_32(  
    int n,
    magma_tally4FloatComplex * dval, 
    magma_tally4_index_t * dcolind,
    magma_tally4_index_t * drowlength,
...
__global__ void
magma_tally4_ccgmerge_spmvellpackrt_kernel2(  
    int n,
    magma_tally4FloatComplex * z,
    magma_tally4FloatComplex * d,
    magma_tally4FloatComplex * vtmp2 )
{
    ...
    temp[ Idx ] = ( i < n ) ? z[i]*d[i] : MAGMA_tally4_C_MAKE(0.0, 0.0);
    __syncthreads();
    if ( Idx < 128 ){
        temp[ Idx ] += temp[ Idx + 128 ];
    }
...
__global__ void
magma_tally4_ccgmerge_spmvsellc_kernel(   
    int num_rows, 
    int blocksize,
    magma_tally4FloatComplex * dval, 
    magma_tally4_index_t * dcolind,
 ...
 temp[ Idx ] = MAGMA_tally4_C_MAKE( 0.0, 0.0);
    ...
    if(i < num_rows ){
        ...
        for ( int n = 0; n < border; n ++){
            ...
            magma_tally4FloatComplex val = dval[offset+ blocksize * n + Idx];
            if( val != 0){
                  dot=dot+val*d[col];
            }
        }
...
__global__ void
magma_tally4_ccgmerge_spmvsellpt_kernel_8( 
    int num_rows, 
    int blocksize,
    int T,
    magma_tally4FloatComplex * dval, 
...
// SELLC SpMV kernel modified assigning multiple threads to each row - 2D kernel
__global__ void
magma_tally4_ccgmerge_spmvsellpt_kernel_16( 
    int num_rows, 
    int blocksize,
    int T,
    magma_tally4FloatComplex * dval, 
...
// SELLC SpMV kernel modified assigning multiple threads to each row - 2D kernel
__global__ void
magma_tally4_ccgmerge_spmvsellpt_kernel_32( 
    int num_rows, 
    int blocksize,
    int T,
    magma_tally4FloatComplex * dval, 
...
magma_tally4_ccg_rhokernel(  
    magma_tally4FloatComplex * skp ){
    int i = blockIdx.x * blockDim.x + threadIdx.x;

    if( i==0 ){
        magma_tally4FloatComplex tmp = skp[1];
        skp[3] = tmp/skp[4];
        skp[2] = tmp;
    }
}
...
/**
    ...
    @param[in]
    dd          magma_tally4FloatComplex_ptr 
                input vector d

    @param[out]
    dz          magma_tally4FloatComplex_ptr 
                input vector z
    ...
    magma_tally4_queue_t queue )
{
    ...
    if ( A.storage_type == Magma_tally4_CSR )
        ...
        ( A.num_rows, A.max_nnz_row, A.dval, A.dcol, dd, dz, d1 );
            ...
            int Mssellp = num_threadssellp * sizeof( magma_tally4FloatComplex );
        ...
        magma_tally4_ccgmerge_spmvellpackrt_kernel2<<<Gs, Bs, Ms, queue >>>
                              ( A.num_rows, dz, dd, d1 );
    ...
    int Mellrt = A.alignment * A.blocksize * sizeof( magma_tally4FloatComplex );
    ...
    while( Gs.x > 1 ) {
        ...
        magma_tally4_ccgreduce_kernel_spmv1<<< Gs_next.x/2, Bs.x/2, Ms/2 >>> 
    ...
    magma_tally4_ccopyvector( 1, aux1, 1, skp+4, 1 );
    dim3 Bs2( 2 );
    dim3 Gs2( 1 );
    magma_tally4_ccg_rhokernel<<<Gs2, Bs2, 0>>>( skp );
...
__global__ void
magma_tally4_ccgmerge_xrbeta_kernel(  
    int n, 
    ...
    magma_tally4FloatComplex * vtmp )
{
    ...
    magma_tally4FloatComplex rho = skp[3];
    magma_tally4FloatComplex mrho = MAGMA_tally4_C_MAKE( -1.0, 0.0)*rho;
...
magma_tally4_ccg_alphabetakernel(  
    magma_tally4FloatComplex * skp )
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;

    if( i==0 ){
        magma_tally4FloatComplex tmp1 = skp[1];
        skp[0] =  tmp1/skp[2];
        //printf("beta=%e\n", MAGMA_tally4_C_REAL(tmp1));
    }
}

// update search Krylov vector d
__global__ void
magma_tally4_ccg_d_kernel(  
    int n, 
    magma_tally4FloatComplex * skp,
    magma_tally4FloatComplex * r,
    magma_tally4FloatComplex * d )
...
/**
    ...
    @param[in/out]
    dr          magma_tally4FloatComplex_ptr 
    ...
    @ingroup magma_tally4sparse_csygpuk
    ********************************************************************/
    ...
    magma_tally4_queue_t queue )
{
    ...
    dim3 Gs_next;
    int Ms =  2*local_block_size * sizeof( magma_tally4FloatComplex ); 
    magma_tally4FloatComplex_ptr aux1 = d1, aux2 = d2;
    int b = 1;        
    magma_tally4_ccgmerge_xrbeta_kernel<<<Gs, Bs, Ms>>>
    ...
    magma_tally4_ccopyvector( 1, aux1, 1, skp+1, 1 );
    dim3 Bs2( 2 );
    dim3 Gs2( 1 );
    magma_tally4_ccg_alphabetakernel<<<Gs2, Bs2, 0>>>( skp );

    dim3 Bs3( local_block_size );
    dim3 Gs3( magma_tally4_ceildiv( n, local_block_size ) );
    magma_tally4_ccg_d_kernel<<<Gs3, Bs3, 0>>>( n, skp, dr, dd );  
