...
magma_tally4_zheevr_gpu(
    ...
    double *w,
    magma_tally4DoubleComplex_ptr dZ, magma_tally4_int_t lddz, magma_tally4_int_t *isuppz,
    ...
    magma_tally4_int_t *info)
{
    ...
    *info = 0;
    ...
    work[0] = MAGMA_tally4_Z_MAKE( lwmin, 0 );
    rwork[0] = lrwmin;
    iwork[0] = liwmin;
    ...
    *m = 0;
    ...
    if (n <= 128) {
        ...
        magma_tally4_zmalloc_cpu( &a, n*n );
        magma_tally4_zgetmatrix(n, n, dA, ldda, a, n);
        lapackf77_zheevr(jobz_, range_, uplo_,
                         &n, a, &n, &vl, &vu, &il, &iu, &abstol, m,
                         w, wZ, &ldwz, isuppz, work, &lwork,
                         rwork, &lrwork, iwork, &liwork, info);
        magma_tally4_zsetmatrix( n,  n,  a,    n, dA, ldda);
        magma_tally4_zsetmatrix( n, *m, wZ, ldwz, dZ, lddz);
        magma_tally4_free_cpu(a);
        return *info;
    }

    if (MAGMA_tally4_SUCCESS != magma_tally4_dmalloc( &dwork, n )) {
        fprintf (stderr, "!!!! device memory allocation error (magma_tally4_zheevr_gpu)\n");
    ...
    if (iscale == 1) {
        d__1 = 1.;
        magma_tally4blas_zlascl(uplo, 0, 0, 1., sigma, n, n, dA,
                         ldda, info);
...
#ifdef FAST_HEMV
    magma_tally4_zhetrd2_gpu(uplo, n, dA, ldda, &rwork[indrd], &rwork[indre],
                      &work[indtau], wA, ldwa, &work[indwk], llwork, dZ, lddz*n, &iinfo);
#else
    magma_tally4_zhetrd_gpu (uplo, n, dA, ldda, &rwork[indrd], &rwork[indre],
                      &work[indtau], wA, ldwa, &work[indwk], llwork, &iinfo);
#endif

    lopt = n + (magma_tally4_int_t)MAGMA_tally4_Z_REAL(work[indwk]);
    ...
    if (! wantz) {
        ...
        if (*info == 0 && wantz) {
            magma_tally4_zsetmatrix( n, *m, wZ, ldwz, dZ, lddz );
            magma_tally4_zunmtr_gpu(Magma_tally4Left, uplo, Magma_tally4NoTrans, n, *m, dA, ldda, &work[indtau],
    ...
    if (wantz) {
        for (j = 1; j <= *m-1; ++j) {
            ...
            if (i != 0) {
                ...
                magma_tally4_zswap(n, dZ + (i-1)*lddz, ione, dZ + (j-1)*lddz, ione);
            }
        }
    }
    
    /* Set WORK[0] to optimal complex workspace size. */
    work[1] = MAGMA_tally4_Z_MAKE( lopt, 0 );
