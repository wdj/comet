...
__global__ void
dslaswp_kernel(int n, double *A, int lda, float *SA, int m, const magma_tally4_int_t *ipiv)
{
    ...
    if (ind < m) {
        ...
        for(int i=0; i < n; i++) {
            res = MAGMA_tally4_S_MAKE( (float)(A[newind+i*lda]),
...
__global__ void
dslaswp_inv_kernel(int n, double *A, int lda, float *SA, int m, const magma_tally4_int_t *ipiv)
{
    int ind = blockIdx.x*NB + threadIdx.x;
    int newind;
    double res;
    ...
    if (ind < m) {
        ...
        for(int i=0; i < n; i++) {
            res = MAGMA_tally4_D_MAKE( (double)(SA[newind+i*lda]),
...
/**
    @see magma_tally4blas_dslaswp_q
    @ingroup magma_tally4_daux2
    ********************************************************************/
extern "C" void
magma_tally4blas_dslaswp(
    ...
    const magma_tally4_int_t *ipiv, magma_tally4_int_t incx )
{
    magma_tally4blas_dslaswp_q( n, A, lda, SA, m, ipiv, incx, magma_tally4_stream );
}
