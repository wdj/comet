...
int main( int argc, char** argv)
{
    ...
    magma_tally3FloatComplex_ptr d_R, dwork;
    ...
    printf("2 - uses CHEMV from MAGMA_tally3 BLAS that requires extra space\n\n");
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_DEV( d_R,     magma_tally3FloatComplex, ldda*N );
            ...
            if (opts.version == 1) {
                magma_tally3_chetrd_gpu( opts.uplo, N, d_R, ldda, diag, offdiag,
                                  tau, h_R, lda, h_work, lwork, &info );
            }
            else {
                magma_tally3_chetrd2_gpu( opts.uplo, N, d_R, ldda, diag, offdiag,
                                   tau, h_R, lda, h_work, lwork, dwork, ldwork, &info );
            }
            gpu_time = magma_tally3_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally3_chetrd_gpu returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                magma_tally3_cgetmatrix( N, N, d_R, ldda, h_Q, lda );
