...
int main( int argc, char** argv)
{
    ...
    magma_tally2FloatComplex *h_A, *h_Q, *h_PT, *h_work;
    magma_tally2FloatComplex *taup, *tauq;
    float      *diag, *offdiag;
    float      eps, result[3] = {0., 0., 0.};
    magma_tally2_int_t M, N, n2, lda, lhwork, info, minmn, nb;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( tauq,    magma_tally2FloatComplex, minmn );
            TESTING_MALLOC_CPU( taup,    magma_tally2FloatComplex, minmn );
            TESTING_MALLOC_CPU( diag,    float, minmn   );
            TESTING_MALLOC_CPU( offdiag, float, minmn-1 );
            
            TESTING_MALLOC_PIN( h_Q,     magma_tally2FloatComplex, lda*N  );
            TESTING_MALLOC_PIN( h_work,  magma_tally2FloatComplex, lhwork );
            
            /* Initialize the matrices */
            lapackf77_clarnv( &ione, ISEED, &n2, h_A );
            lapackf77_clacpy( Magma_tally2UpperLowerStr, &M, &N, h_A, &lda, h_Q, &lda );
            
            /* ====================================================================
               Performs operation using MAGMA_tally2
               =================================================================== */
            gpu_time = magma_tally2_wtime();
            magma_tally2_cgebrd( M, N, h_Q, lda,
                          diag, offdiag, tauq, taup,
                          h_work, lhwork, &info);
            gpu_time = magma_tally2_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally2_cgebrd returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                lapackf77_clacpy(Magma_tally2UpperLowerStr, &M, &N, h_Q, &lda, h_PT, &lda);
