    ...
    float* dtau,  float* swork, float* sscale, float* scale)
{
    ...
    if ( tx == 0 ) {
        ...
        if ( swork[0] == 0 && imag(alpha) == 0 ) {
            ...
            *dtau   = MAGMA_minproduct_S_MAKE( (beta - real(alpha)) / beta, -imag(alpha) / beta );
            *dalpha = MAGMA_minproduct_S_MAKE( beta, 0 );
            *scale = 1 / (alpha - beta);
        }
    }
...
void slarfx_device( int m, int n,  float *v, float *tau,
                         float *dc, magma_minproduct_int_t ldc, float* sum)
{


    if(n <=0) return ;
    if (MAGMA_minproduct_S_EQUAL(*tau, MAGMA_minproduct_S_ZERO) )  return; // check singularity
    ...
    for(int k=0;k<n;k++)
    {
        ...
        if(tx<BLOCK_SIZE)
        {
            ...
            for( int j = tx+1; j < m; j += BLOCK_SIZE ){
                lsum += MAGMA_minproduct_S_MUL( MAGMA_minproduct_S_CNJG( v[j] ), dc[j+ldc*k] );
            }
...
__global__
void sgeqr2_sm_kernel_batched( int m, int n, float** dA_array, magma_minproduct_int_t lda,
                               float **dtau_array)
{
...
static __device__
void sgeqr2_device( magma_minproduct_int_t m, magma_minproduct_int_t n, float* dA, magma_minproduct_int_t lda,
                               float *dtau, 
                               float *sdata,
                               float *sum,
                               float *swork,
...
__global__
void sgeqr2_kernel_batched( int m, int n, float** dA_array, magma_minproduct_int_t lda,
                               float **dtau_array)
{
...
magma_minproduct_sgeqr2_batched(magma_minproduct_int_t m, magma_minproduct_int_t n, float **dA_array,
                  magma_minproduct_int_t lda, float **dtau_array,
