...
__global__
void magma_tally3_dlarfx_kernel( int m, double *v, double *tau,
                         ...
                         double *T, int it )
{
    if ( !MAGMA_tally3_D_EQUAL(*tau, MAGMA_tally3_D_ZERO) ) {
        ...
        for( int j = tx; j < m; j += BLOCK_SIZE ){
            if (j==0){
               lsum += MAGMA_tally3_D_MUL( MAGMA_tally3_D_ONE, dc[j] );
               v[j] = MAGMA_tally3_D_ONE;
            }
            else
               lsum += MAGMA_tally3_D_MUL( MAGMA_tally3_D_CNJG( v[j] ), dc[j] );
        }
        sum[tx] = lsum;
        ...
        /*  C := C - v * w  */
        __syncthreads();
        double z__1 = - MAGMA_tally3_D_CNJG(*tau) * sum[0];
        if (blockIdx.x>it){
           ...
           /*
           if (tx==0){
             double temp = MAGMA_tally3_D_ABS( dc[0] ) / xnorm[blockIdx.x-it-1];
             temp = (temp + 1.) * (1. - temp);
...
extern "C"
__global__
void magma_tally3_dtrmv_kernel(const double *T, int ldt, double *t)
{
   const int tx = threadIdx.x;
   T += tx;

   __shared__ double tlocal[ BLOCK_SIZE ];
   double res = MAGMA_tally3_D_MAKE(0., 0.);
...
extern "C"
__global__
void magma_tally3_dtrmv_kernel2(const double *T, int ldt, double *t, 
                         double *y, double *tau)
{
   const int tx = threadIdx.x;
   T += blockIdx.x;
...
extern "C"
__global__
void magma_tally3_dtrmv_tkernel(double *T, int ldt, double *t, double *y)
{
   const int tx = threadIdx.x;
   T += blockIdx.x*ldt;
   ...
   sum[tx] = MAGMA_tally3_D_CNJG(T[tx])*t[tx];
   magma_tally3_sum_reduce_n(blockDim.x, tx, sum);
    ...
    magma_tally3Double_ptr work )
{
    ...
    if (iter==0)
        magma_tally3_dlarfx_kernel<<< N, BLOCK_SIZE, 0, magma_tally3_stream >>>( m, v, tau, C, ldc, xnorm, dT+iter*N, iter);
    else
        magma_tally3_dlarfx_kernel<<< N, BLOCK_SIZE, 0, magma_tally3_stream >>>( m, v, tau, C, ldc, xnorm, work, iter);

    if (iter > 0){
        //magma_tally3_dtrmv_kernel<<< 1, iter, 0, magma_tally3_stream >>>( dT, N, dT+iter*N);
        magma_tally3_dtrmv_kernel2<<< iter, iter, 0, magma_tally3_stream  >>>( dT, N, work, dT+iter*N, tau);
    }
}
