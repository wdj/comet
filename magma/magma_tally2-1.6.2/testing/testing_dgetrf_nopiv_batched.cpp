...
int main( int argc, char** argv)
{
    ...
    for( int i = 0; i < opts.ntest; ++i ) {
      ...
      for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_DEV(  dA_magma_tally2,  double, ldda*N * batchCount);
            ...
            magma_tally2_dsetmatrix( M, columns, h_R, lda, dA_magma_tally2, ldda );
            ...
            /* ====================================================================
               ...
               =================================================================== */
            dset_pointer(dA_array, dA_magma_tally2, ldda, 0, 0, ldda*N, batchCount, queue);
            magma_tally2_time = magma_tally2_sync_wtime(0);
            info = magma_tally2_dgetrf_nopiv_batched( M, N, dA_array, ldda, dinfo_magma_tally2, batchCount, queue);
            ...
            {
                if(cpu_info[i] != 0 ){
                    printf("magma_tally2_dgetrf_batched matrix %d returned internal error %d\n",i, (int)cpu_info[i] );
                }
            }
            if (info != 0)
                printf("magma_tally2_dgetrf_batched returned argument error %d: %s.\n", (int) info, magma_tally2_strerror( info ));
            ...
            if ( opts.check ) {
                ...
                magma_tally2_dgetmatrix( M, N*batchCount, dA_magma_tally2, ldda, h_A, lda );
                for(int i=0; i<batchCount; i++)
                {
