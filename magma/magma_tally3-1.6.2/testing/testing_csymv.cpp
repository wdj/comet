...
int main(int argc, char **argv)
{
    ...
    float          magma_tally3_error, work[1];
    ...
    magma_tally3FloatComplex *A, *X, *Y, *Ymagma_tally3;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( opts.version == 1 ) {
                magma_tally3blas_csymv_work( opts.uplo, N, alpha, dA, ldda, dX, incx, beta, dY, incy, dwork, ldwork, opts.queue );
            }
            else {
                // non-work interface (has added overhead)
                magma_tally3blas_csymv( opts.uplo, N, alpha, dA, ldda, dX, incx, beta, dY, incy );
            }
            ...
            lapackf77_csymv( lapack_uplo_const_tally3(opts.uplo), &N, &alpha, A, &lda, X, &incx, &beta, Y, &incy );
