...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lhwork2 = (magma_tally2_int_t) MAGMA_tally2_C_REAL( tmp[0] );
            
            lhwork = -1;
            lapackf77_cunmqr( Magma_tally2LeftStr, Magma_tally2_ConjTransStr,
                              &M, &nrhs, &min_mn, NULL, &lda, NULL,
                              NULL, &ldb, tmp, &lhwork, &info);
            lhwork = (magma_tally2_int_t) MAGMA_tally2_C_REAL( tmp[0] );
            ...
            TESTING_MALLOC_CPU( h_A2,   magma_tally2FloatComplex, lda*N     );
            TESTING_MALLOC_CPU( h_B,    magma_tally2FloatComplex, ldb*nrhs  );
            TESTING_MALLOC_CPU( h_X,    magma_tally2FloatComplex, ldb*nrhs  );
            TESTING_MALLOC_CPU( h_R,    magma_tally2FloatComplex, ldb*nrhs  );
            TESTING_MALLOC_CPU( h_work, magma_tally2FloatComplex, lhwork    );
            
            TESTING_MALLOC_DEV( d_A,    magma_tally2FloatComplex, ldda*N    );
            TESTING_MALLOC_DEV( d_B,    magma_tally2FloatComplex, lddb*nrhs );
            
            /* Initialize the matrices */
            lapackf77_clarnv( &ione, ISEED, &size, h_A );
            lapackf77_clacpy( Magma_tally2UpperLowerStr, &M, &N, h_A, &lda, h_A2, &lda );
            
            // make random RHS
            size = M*nrhs;
            lapackf77_clarnv( &ione, ISEED, &size, h_B );
            lapackf77_clacpy( Magma_tally2UpperLowerStr, &M, &nrhs, h_B, &ldb, h_R, &ldb );
            
            // make consistent RHS
            //size = N*nrhs;
            //lapackf77_clarnv( &ione, ISEED, &size, h_X );
            //blasf77_cgemm( Magma_tally2NoTransStr, Magma_tally2NoTransStr, &M, &nrhs, &N,
            //               &c_one,  h_A, &lda,
            //                        h_X, &ldb,
            //               &c_zero, h_B, &ldb );
            //lapackf77_clacpy( Magma_tally2UpperLowerStr, &M, &nrhs, h_B, &ldb, h_R, &ldb );
            ...
            magma_tally2_csetmatrix( M, nrhs, h_B, ldb, d_B, lddb );
            
            gpu_time = magma_tally2_wtime();
            magma_tally2_cgels3_gpu( Magma_tally2NoTrans, M, N, nrhs, d_A, ldda,
                              d_B, lddb, h_work, lworkgpu, &info);
            gpu_time = magma_tally2_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally2_cgels3_gpu returned error %d: %s.\n",
                       (int) info, magma_tally2_strerror( info ));
            
            // Get the solution in h_X
            magma_tally2_cgetmatrix( N, nrhs, d_B, lddb, h_X, ldb );
            ...
            /* =====================================================================
               ...
               =================================================================== */
            lapackf77_clacpy( Magma_tally2UpperLowerStr, &M, &nrhs, h_B, &ldb, h_X, &ldb );
            
            cpu_time = magma_tally2_wtime();
            lapackf77_cgels( Magma_tally2NoTransStr, &M, &N, &nrhs,
