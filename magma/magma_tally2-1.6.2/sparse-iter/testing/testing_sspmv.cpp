...
int main(  int argc, char** argv )
{
    ...
    magma_tally2_s_matrix hA={Magma_tally2_CSR}, hA_SELLP={Magma_tally2_CSR}, hA_ELL={Magma_tally2_CSR}, 
    dA={Magma_tally2_CSR}, dA_SELLP={Magma_tally2_CSR}, dA_ELL={Magma_tally2_CSR};
    
    magma_tally2_s_matrix hx={Magma_tally2_CSR}, hy={Magma_tally2_CSR}, dx={Magma_tally2_CSR}, 
    ...
    float c_one  = MAGMA_tally2_S_MAKE(1.0, 0.0);
    float c_zero = MAGMA_tally2_S_MAKE(0.0, 0.0);
    ...
    while( i < argc ) {
        if ( strcmp("LAPLACE2D", argv[i]) == 0 && i+1 < argc ) {   // Laplace test
            ...
            CHECK( magma_tally2_sm_5stencil(  laplace_size, &hA, queue ));
        } else {                        // file-matrix test
            CHECK( magma_tally2_s_csr_mtx( &hA,  argv[i], queue ));
        }
        ...
        // init CPU vectors
        CHECK( magma_tally2_svinit( &hx, Magma_tally2_CPU, hA.num_rows, 1, c_zero, queue ));
        CHECK( magma_tally2_svinit( &hy, Magma_tally2_CPU, hA.num_rows, 1, c_zero, queue ));

        // init DEV vectors
        CHECK( magma_tally2_svinit( &dx, Magma_tally2_DEV, hA.num_rows, 1, c_one, queue ));
        CHECK( magma_tally2_svinit( &dy, Magma_tally2_DEV, hA.num_rows, 1, c_zero, queue ));
        ...
        // copy matrix to GPU
        CHECK( magma_tally2_smtransfer( hA, &dA, Magma_tally2_CPU, Magma_tally2_DEV, queue ));
        // SpMV on GPU (CSR) -- this is the reference!
        start = magma_tally2_sync_wtime( queue );
        for (j=0; j<10; j++)
            CHECK( magma_tally2_s_spmv( c_one, dA, dx, c_zero, dy, queue ));
        end = magma_tally2_sync_wtime( queue );
        printf( " > MAGMA_tally2: %.2e seconds %.2e GFLOP/s    (standard CSR).\n",
                                        (end-start)/10, FLOPS*10/(end-start) );
        magma_tally2_smfree(&dA, queue );
        CHECK( magma_tally2_smtransfer( dy, &hrefvec , Magma_tally2_DEV, Magma_tally2_CPU, queue ));

        // convert to ELL and copy to GPU
        CHECK( magma_tally2_smconvert(  hA, &hA_ELL, Magma_tally2_CSR, Magma_tally2_ELL, queue ));
        CHECK( magma_tally2_smtransfer( hA_ELL, &dA_ELL, Magma_tally2_CPU, Magma_tally2_DEV, queue ));
        magma_tally2_smfree(&hA_ELL, queue );
        magma_tally2_smfree( &dy, queue );
        CHECK( magma_tally2_svinit( &dy, Magma_tally2_DEV, hA.num_rows, 1, c_zero, queue ));
        // SpMV on GPU (ELL)
        start = magma_tally2_sync_wtime( queue );
        for (j=0; j<10; j++)
            CHECK( magma_tally2_s_spmv( c_one, dA_ELL, dx, c_zero, dy, queue ));
        end = magma_tally2_sync_wtime( queue );
        printf( " > MAGMA_tally2: %.2e seconds %.2e GFLOP/s    (standard ELL).\n",
                                        (end-start)/10, FLOPS*10/(end-start) );
        magma_tally2_smfree(&dA_ELL, queue );
        CHECK( magma_tally2_smtransfer( dy, &hcheck , Magma_tally2_DEV, Magma_tally2_CPU, queue ));
        res = 0.0;
        for(magma_tally2_int_t k=0; k<hA.num_rows; k++ )
            res=res + MAGMA_tally2_S_REAL(hcheck.val[k]) - MAGMA_tally2_S_REAL(hrefvec.val[k]);
        if ( res < .000001 )
            printf("# tester spmv ELL:  ok\n");
        else
            printf("# tester spmv ELL:  failed\n");
        magma_tally2_smfree( &hcheck, queue );

        // convert to SELLP and copy to GPU
        CHECK( magma_tally2_smconvert(  hA, &hA_SELLP, Magma_tally2_CSR, Magma_tally2_SELLP, queue ));
        CHECK( magma_tally2_smtransfer( hA_SELLP, &dA_SELLP, Magma_tally2_CPU, Magma_tally2_DEV, queue ));
        magma_tally2_smfree(&hA_SELLP, queue );
        magma_tally2_smfree( &dy, queue );
        CHECK( magma_tally2_svinit( &dy, Magma_tally2_DEV, hA.num_rows, 1, c_zero, queue ));
        // SpMV on GPU (SELLP)
        start = magma_tally2_sync_wtime( queue );
        for (j=0; j<10; j++)
            CHECK( magma_tally2_s_spmv( c_one, dA_SELLP, dx, c_zero, dy, queue ));
        end = magma_tally2_sync_wtime( queue );
        ...
        magma_tally2_smfree(&dA_SELLP, queue );
        ...
        magma_tally2_smfree(&hx, queue );
        magma_tally2_smfree(&hy, queue );
        magma_tally2_smfree(&hrefvec, queue );
        // free GPU memory
        magma_tally2_smfree(&dA, queue );
        magma_tally2_smfree(&dx, queue );
        magma_tally2_smfree(&dy, queue );
