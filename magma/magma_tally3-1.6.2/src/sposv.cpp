...
/**
    ...
    @ingroup magma_tally3_sposv_driver
    ********************************************************************/
    ...
    magma_tally3_int_t *info )
{
    ...
    *info = 0;
    ...
    ldda = ((n+31)/32)*32;
    lddb = ldda;
    if ( MAGMA_tally3_SUCCESS != magma_tally3_smalloc( &dA, ldda*n )) {
        goto CPU_INTERFACE;
    }
    if ( MAGMA_tally3_SUCCESS != magma_tally3_smalloc( &dB, lddb*nrhs )) {
        magma_tally3_free( dA );
        goto CPU_INTERFACE;
    }
    magma_tally3_ssetmatrix( n, n, A, lda, dA, ldda );
    magma_tally3_spotrf_gpu( uplo, n, dA, ldda, info );
    ...
    if ( *info == 0 ) {
        magma_tally3_ssetmatrix( n, nrhs, B, ldb, dB, lddb );
        magma_tally3_spotrs_gpu( uplo, n, nrhs, dA, ldda, dB, lddb, info );
        magma_tally3_sgetmatrix( n, nrhs, dB, lddb, B, ldb );
    }
    magma_tally3_free( dA );
    magma_tally3_free( dB );
    return *info;
...
CPU_INTERFACE:
    ...
    magma_tally3_spotrf( uplo, n, A, lda, info );
    if ( *info == 0 ) {
        lapackf77_spotrs( lapack_uplo_const_tally3(uplo), &n, &nrhs, A, &lda, B, &ldb, info );
    }
