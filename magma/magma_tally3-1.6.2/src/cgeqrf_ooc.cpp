...
/**
    ...
    This is an out-of-core (ooc) version that is similar to magma_tally3_cgeqrf but
    ...
    magma_tally3_int_t *info )
{
    ...
    *info = 0;
    int nb = magma_tally3_get_cgeqrf_nb(min(m, n));
    ...
    if (NB >= n)
        return magma_tally3_cgeqrf(m, n, A, lda, tau, work, lwork, info);
    ...
    if (MAGMA_tally3_SUCCESS != magma_tally3_cmalloc( &dA, (NB + nb)*ldda + nb*lddwork )) {
    ...
    magma_tally3FloatComplex *ptr = dA + ldda * NB;
    dwork = dA + ldda*(NB + nb);
    ...
    for (int i=0; i < n; i += NB) {
        ...
        magma_tally3_csetmatrix_async( (m), IB,
                                A(0,i),  lda,
        ...
        for (int j=0; j < min(i,k); j += nb) {
            ...
            cpanel_to_q_tally3(Magma_tally3Upper, ib, A(j,j), lda, work+ib*ib);
            magma_tally3_csetmatrix_async( rows, ib,
                                    A(j,j), lda,
                                    ptr,        rows, stream[1] );
            ...
            cq_to_panel_tally3(Magma_tally3Upper, ib, A(j,j), lda, work+ib*ib);
        }

        /* 3. Do a QR on the current part */
        if (i < k)
            magma_tally3_cgeqrf2_gpu(m-i, IB, dA(i,0), ldda, tau+i, info);

        /* 4. Copy the current part back to the CPU */
        magma_tally3_cgetmatrix_async( (m), IB,
                                dA(0,0), ldda,
                                A(0,i),  lda, stream[0] );
    }
