...
int main( int argc, char** argv)
{
    ...
    magma_minproductDoubleComplex *h_A, *h_Ainit, *h_B, *h_Binit, *h_work;
    #ifdef COMPLEX
    double *rwork;
    #endif
    ...
    /* Matrix size */
    magma_minproduct_int_t N, n2, nb;
    ...
    opts.lapack |= (opts.check && opts.jobz == Magma_minproductNoVec);
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            #ifdef COMPLEX
                magma_minproduct_int_t lwork  = max( N + N*nb, 2*N + N*N );
                magma_minproduct_int_t lrwork = 1 + 5*N +2*N*N;
            #else
                magma_minproduct_int_t lwork  = max( 2*N + N*nb, 1 + 6*N + 2*N*N );
            #endif
            magma_minproduct_int_t liwork = 3 + 5*N;

            TESTING_MALLOC_PIN( h_A,    magma_minproductDoubleComplex, n2    );
            TESTING_MALLOC_PIN( h_B,    magma_minproductDoubleComplex, n2    );
            ...
            magma_minproduct_zmake_hpd( N, h_B, N );
            magma_minproduct_zmake_hermitian( N, h_A, N );

            if ( opts.warmup || opts.check ) {
                TESTING_MALLOC_CPU( h_Ainit, magma_minproductDoubleComplex, n2 );
                TESTING_MALLOC_CPU( h_Binit, magma_minproductDoubleComplex, n2 );
                lapackf77_zlacpy( Magma_minproductFullStr, &N, &N, h_A, &N, h_Ainit, &N );
                lapackf77_zlacpy( Magma_minproductFullStr, &N, &N, h_B, &N, h_Binit, &N );
            }
            ...
            if (opts.warmup) {
                ...
                // ==================================================================
                magma_minproduct_zhegvd_m( opts.ngpu, opts.itype, opts.jobz, opts.uplo,
                                N, h_A, N, h_B, N, w1,
                                h_work, lwork,
                                #ifdef COMPLEX
                                rwork, lrwork,
                                ...
                                iwork, liwork,
                                &info);
                lapackf77_zlacpy( Magma_minproductFullStr, &N, &N, h_Ainit, &N, h_A, &N );
                lapackf77_zlacpy( Magma_minproductFullStr, &N, &N, h_Binit, &N, h_B, &N );
            }

            // ===================================================================
            // Performs operation using MAGMA_minproduct
            // ===================================================================
            mgpu_time = magma_minproduct_wtime();
            magma_minproduct_zhegvd_m( opts.ngpu, opts.itype, opts.jobz, opts.uplo,
                            N, h_A, N, h_B, N, w1,
                            h_work, lwork,
                            #ifdef COMPLEX
                            ...
                            iwork, liwork,
                            &info);
            mgpu_time = magma_minproduct_wtime() - mgpu_time;

            if (info != 0)
                printf("magma_minproduct_zhegvd_m returned error %d: %s.\n",
            ...
            if ( opts.lapack ) {
                ...
                magma_minproduct_zhegvd(opts.itype, opts.jobz, opts.uplo,
                             N, h_A, N, h_B, N, w2,
                             h_work, lwork,
                             #ifdef COMPLEX
                             rwork, lrwork,
                ...
                if (info != 0)
                    printf("magma_minproduct_zhegvd returned error %d: %s.\n",
                           (int) info, magma_minproduct_strerror( info ));
