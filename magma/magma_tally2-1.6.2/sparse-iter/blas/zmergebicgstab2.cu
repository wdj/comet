...
__global__ void
magma_tally2_zreduce_kernel_spmv1(    
    int Gs,
    int n, 
    ...
    magma_tally2DoubleComplex * vtmp)
{
    ...
    temp[ Idx ] = ( i < n ) ? v[ i ] * r[ i ] : MAGMA_tally2_Z_MAKE( 0.0, 0.0);
    __syncthreads();
    if ( Idx < 128 ){
...
__global__ void
magma_tally2_zbicgstab_alphakernel(  
                    magma_tally2DoubleComplex * skp ){
    int i = blockIdx.x * blockDim.x + threadIdx.x;

    if( i==0 ){
        magma_tally2DoubleComplex tmp = skp[0];
        skp[0] = skp[4]/tmp;
    }
}
...
/**
    ...
    @param[in]
    dp          magma_tally2DoubleComplex_ptr
                input vector p
    ...
    @param[in]
    dv          magma_tally2DoubleComplex_ptr
                output vector v
    ...
    magma_tally2_queue_t queue )
{
    ...
    if ( A.storage_type == Magma_tally2_CSR)
        magma_tally2_zbicgmerge_spmv1_kernel<<<Gs, Bs, Ms>>>
    ...
    while( Gs.x > 1 ) {
        ...
        magma_tally2_zreduce_kernel_spmv1<<< Gs_next.x/2, Bs.x/2, Ms/2 >>> 
                            ( Gs.x, n, aux1, aux2 );
    ...
    magma_tally2_zcopyvector( 1, aux1, 1, skp, 1 );
    dim3 Bs2( 2 );
    dim3 Gs2( 1 );
    magma_tally2_zbicgstab_alphakernel<<<Gs2, Bs2, 0>>>( skp );
...
__global__ void
magma_tally2_zreduce_kernel_spmv2( 
    int Gs,
    int n, 
    magma_tally2DoubleComplex * vtmp,
    magma_tally2DoubleComplex * vtmp2 )
{
    ...
    for( j=0; j<2; j++){
        int i = blockIdx.x * ( blockSize * 2 ) + Idx;   
        temp[Idx+j*(blockSize)] = MAGMA_tally2_Z_MAKE( 0.0, 0.0);
        while (i < Gs ) {
...
__global__ void
magma_tally2_zbicgmerge_spmv2_kernel(  
    int n,
    magma_tally2DoubleComplex * dval, 
    magma_tally2_index_t * drowptr, 
    magma_tally2_index_t * dcolind,
    magma_tally2DoubleComplex * s,
    ...
    magma_tally2DoubleComplex * vtmp )
{
    ...
    // 2 vectors 
    if (i<n){
            magma_tally2DoubleComplex tmp2 = t[i];
            temp[Idx] = s[i] * tmp2;
            temp[Idx+blockDim.x] = tmp2 * tmp2;
    }
    else{
        for( j=0; j<2; j++)
            temp[Idx+j*blockDim.x] =MAGMA_tally2_Z_MAKE( 0.0, 0.0);
    }
    __syncthreads();
    if ( Idx < 128 ){
        for( j=0; j<2; j++){
...
__global__ void
magma_tally2_zbicgstab_omegakernel(  
                    magma_tally2DoubleComplex * skp ){
    int i = blockIdx.x * blockDim.x + threadIdx.x;
...
/**
    ...
    @param[in]
    ds          magma_tally2DoubleComplex_ptr
                input vector s

    @param[in]
    dt          magma_tally2DoubleComplex_ptr
                output vector t
    ...
    magma_tally2_queue_t queue )
{
    ...
    int b = 1;        
    if ( A.storage_type == Magma_tally2_CSR)
        magma_tally2_zbicgmerge_spmv2_kernel<<<Gs, Bs, Ms>>>
    ...
    while( Gs.x > 1 ) {
        Gs_next.x = ( Gs.x+Bs.x-1 )/ Bs.x ;
        if ( Gs_next.x == 1 ) Gs_next.x = 2;
        magma_tally2_zreduce_kernel_spmv2<<< Gs_next.x/2, Bs.x/2, Ms/2 >>> 
                    ( Gs.x, n, aux1, aux2 );
        Gs_next.x = Gs_next.x /2;
        Gs.x = Gs_next.x;
        b = 1 - b;
    ...
    magma_tally2_zcopyvector( 1, aux1, 1, skp+6, 1 );
    magma_tally2_zcopyvector( 1, aux1+n, 1, skp+7, 1 );
    dim3 Bs2( 2 );
    dim3 Gs2( 1 );
    magma_tally2_zbicgstab_omegakernel<<<Gs2, Bs2, 0>>>( skp );
...
__global__ void
magma_tally2_zbicgmerge_xrbeta_kernel(  
    int n, 
    magma_tally2DoubleComplex * rr,
    magma_tally2DoubleComplex * r,
    magma_tally2DoubleComplex * p,
    magma_tally2DoubleComplex * s,
    magma_tally2DoubleComplex * t,
    ...
    magma_tally2DoubleComplex * vtmp )
{
    ...
    magma_tally2DoubleComplex omega=skp[2];

    if( i<n ){
        magma_tally2DoubleComplex sl;
        sl = s[i];
    ...
    // 2 vectors 
    if (i<n){
            magma_tally2DoubleComplex tmp2 = r[i];
            temp[Idx] = rr[i] * tmp2;
            temp[Idx+blockDim.x] = tmp2 * tmp2;
    }
    else{
...
__global__ void
magma_tally2_zbicgstab_betakernel(  
    magma_tally2DoubleComplex * skp )
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;

    if( i==0 ){
        magma_tally2DoubleComplex tmp1 = skp[4]/skp[3];
        magma_tally2DoubleComplex tmp2 = skp[0] / skp[2];
        skp[1] =  tmp1*tmp2;
    }
}
...
/**
    ...
    @param[in]
    rr          magma_tally2DoubleComplex_ptr
                input vector rr

    @param[in]
    r           magma_tally2DoubleComplex_ptr
                input/output vector r

    @param[in]
    p           magma_tally2DoubleComplex_ptr
                input vector p

    @param[in]
    s           magma_tally2DoubleComplex_ptr
                input vector s

    @param[in]
    t           magma_tally2DoubleComplex_ptr
                input vector t
...
magma_tally2_zbicgmerge_xrbeta(
    ...
    magma_tally2DoubleComplex_ptr d1,
    magma_tally2DoubleComplex_ptr d2,
    magma_tally2DoubleComplex_ptr rr,
    magma_tally2DoubleComplex_ptr r,
    magma_tally2DoubleComplex_ptr p,
    magma_tally2DoubleComplex_ptr s,
    magma_tally2DoubleComplex_ptr t,
    ...
    magma_tally2_queue_t queue )
{
    ...
    magma_tally2DoubleComplex_ptr aux1 = d1, aux2 = d2;
    int b = 1;        
    magma_tally2_zbicgmerge_xrbeta_kernel<<<Gs, Bs, Ms>>>
    ...
    magma_tally2_zcopyvector( 1, aux1+n, 1, skp+5, 1 );
    dim3 Bs2( 2 );
    dim3 Gs2( 1 );
    magma_tally2_zbicgstab_betakernel<<<Gs2, Bs2, 0>>>( skp );
