/*
    -- MAGMA_minproduct (version 1.6.1) --
       ...
       @generated from ztrtri_diag.cu normal z -> s, Fri Jan 30 19:00:10 2015
    ...
    magma_minproduct_queue_t queue)
{
    ...
    if ( uplo == Magma_minproductLower ) {
        ...
        strtri_diag_lower_kernel<<< nblocks, IB, 0, queue >>>( diag, n, dA, ldda, d_dinvA );
        ...
        for( int jb=IB; jb < NB; jb *= 2 ) {
            ...
            switch (jb) {
                case 16:
                    triple_sgemm16_part1_lower_kernel<<< grid, threads, 0, queue >>>( n, dA, ldda, d_dinvA, jb, npages );
                    triple_sgemm16_part2_lower_kernel<<< grid, threads, 0, queue >>>( n, dA, ldda, d_dinvA, jb, npages );
                    break;
                case 32:
                    triple_sgemm32_part1_lower_kernel<<< grid, threads, 0, queue >>>( n, dA, ldda, d_dinvA, jb, npages );
                    triple_sgemm32_part2_lower_kernel<<< grid, threads, 0, queue >>>( n, dA, ldda, d_dinvA, jb, npages );
                    break;
                case 64:
                    triple_sgemm64_part1_lower_kernel<<< grid, threads, 0, queue >>>( n, dA, ldda, d_dinvA, jb, npages );
                    triple_sgemm64_part2_lower_kernel<<< grid, threads, 0, queue >>>( n, dA, ldda, d_dinvA, jb, npages );
                    break;
                default:
                    triple_sgemm_above64_part1_lower_kernel<<< grid, threads, 0, queue >>>( n, dA, ldda, d_dinvA, jb, npages );
                    triple_sgemm_above64_part2_lower_kernel<<< grid, threads, 0, queue >>>( n, dA, ldda, d_dinvA, jb, npages );
                    triple_sgemm_above64_part3_lower_kernel<<< grid, threads, 0, queue >>>( n, dA, ldda, d_dinvA, jb, npages );
                    break;
            }
            if ( kb >= n ) break;
        }
        ...
        strtri_diag_upper_kernel<<< nblocks, IB, 0, queue >>>( diag, n, dA, ldda, d_dinvA );
        ...
        for( int jb=IB; jb < NB; jb*=2 ) {
            ...
            switch (jb) {
                case 16:
                    triple_sgemm16_part1_upper_kernel<<< grid, threads, 0, queue >>>( n, dA, ldda, d_dinvA, jb, npages );
                    triple_sgemm16_part2_upper_kernel<<< grid, threads, 0, queue >>>( n, dA, ldda, d_dinvA, jb, npages );
                    break;
                case 32:
                    triple_sgemm32_part1_upper_kernel<<< grid, threads, 0, queue >>>( n, dA, ldda, d_dinvA, jb, npages );
                    triple_sgemm32_part2_upper_kernel<<< grid, threads, 0, queue >>>( n, dA, ldda, d_dinvA, jb, npages );
                    break;
                case 64:
                    triple_sgemm64_part1_upper_kernel<<< grid, threads, 0, queue >>>( n, dA, ldda, d_dinvA, jb, npages );
                    triple_sgemm64_part2_upper_kernel<<< grid, threads, 0, queue >>>( n, dA, ldda, d_dinvA, jb, npages );
                    break;
                default:
                    triple_sgemm_above64_part1_upper_kernel<<< grid, threads, 0, queue >>>( n, dA, ldda, d_dinvA, jb, npages );
                    triple_sgemm_above64_part2_upper_kernel<<< grid, threads, 0, queue >>>( n, dA, ldda, d_dinvA, jb, npages );
                    triple_sgemm_above64_part3_upper_kernel<<< grid, threads, 0, queue >>>( n, dA, ldda, d_dinvA, jb, npages );
                    break;
            }
            if ( kb >= n ) break;
        }
...
/**
    @see magma_minproductblas_strtri_diag_q
    @ingroup magma_minproduct_sblas3
    ********************************************************************/
extern "C" void
magma_minproductblas_strtri_diag(
    ...
    magma_minproductFloat_ptr d_dinvA)
{
    magma_minproductblas_strtri_diag_q( uplo, diag, n, dA, ldda, d_dinvA, magma_minproduct_stream );
}
