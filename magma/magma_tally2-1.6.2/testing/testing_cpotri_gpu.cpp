/*
    -- MAGMA_tally2 (version 1.6.1) --
       ...
       @generated from testing_zpotri_gpu.cpp normal z -> c, Fri Jan 30 19:00:24 2015
*/
// includes, system
#include <stdlib.h>
#include <stdio.h>
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_tally2_cpotrf_gpu( opts.uplo, N, d_A, ldda, &info );
            
            // check for exact singularity
            //magma_tally2_cgetmatrix( N, N, d_A, ldda, h_R, lda );
            //h_R[ 10 + 10*lda ] = MAGMA_tally2_C_MAKE( 0.0, 0.0 );
            //magma_tally2_csetmatrix( N, N, h_R, lda, d_A, ldda );
            
            gpu_time = magma_tally2_wtime();
            magma_tally2_cpotri_gpu( opts.uplo, N, d_A, ldda, &info );
            gpu_time = magma_tally2_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally2_cpotri_gpu returned error %d: %s.\n",
            ...
            if ( opts.lapack ) {
                ...
                lapackf77_cpotri( lapack_uplo_const_tally2(opts.uplo), &N, h_A, &lda, &info );
