...
int main( int argc, char** argv)
{
    ...
    double *h_A, *h_B, *h_C, *h_Cmagma_minproduct, *h_Cdev;
    magma_minproductDouble_ptr d_A, d_B, d_C;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_Cmagma_minproduct,  double, ldc*N  );
            ...
            magma_minproduct_dsetmatrix( Am, An, h_A, lda, d_A, ldda );
            magma_minproduct_dsetmatrix( Bm, Bn, h_B, ldb, d_B, lddb );
            
            /* =====================================================================
               Performs operation using MAGMA_minproductBLAS (currently only with CUDA)
               =================================================================== */
            #ifdef HAVE_CUBLAS
                magma_minproduct_dsetmatrix( M, N, h_C, ldc, d_C, lddc );
                
                magma_minproduct_time = magma_minproduct_sync_wtime( NULL );
                magma_minproductblas_dgemm( opts.transA, opts.transB, M, N, K,
                                 alpha, d_A, ldda,
                                        d_B, lddb,
                                 beta,  d_C, lddc );
                ...
                magma_minproduct_dgetmatrix( M, N, d_C, lddc, h_Cmagma_minproduct, ldc );
            #endif
            ...
            magma_minproduct_dsetmatrix( M, N, h_C, ldc, d_C, lddc );
                ...
                magma_minproduct_dgemm( opts.transA, opts.transB, M, N, K,
                             alpha, d_A, 0, ldda,
                                    d_B, 0, lddb,
            ...
            magma_minproduct_dgetmatrix( M, N, d_C, lddc, h_Cdev, ldc );
            ...
            if ( opts.lapack ) {
                ...
                dev_error = lapackf77_dlange( "F", &M, &N, h_Cdev, &ldc, work ) / Cnorm;
                ...
                #ifdef HAVE_CUBLAS
                    blasf77_daxpy( &sizeC, &c_neg_one, h_C, &ione, h_Cmagma_minproduct, &ione );
                    magma_minproduct_error = lapackf77_dlange( "F", &M, &N, h_Cmagma_minproduct, &ldc, work ) / Cnorm;
                    ...
                    blasf77_daxpy( &sizeC, &c_neg_one, h_Cdev, &ione, h_Cmagma_minproduct, &ione );
                    magma_minproduct_error = lapackf77_dlange( "F", &M, &N, h_Cmagma_minproduct, &ldc, work ) / Cnorm;
