...
void init_matrix( int nopiv, int m, int n, double *h_A, magma_tally3_int_t lda )
{
    assert( m == n );
    ...
    if (nopiv) {
        ...
        magma_tally3_dmake_symmetric( n, h_A, lda );
    }
}
    ...
    magma_tally3_int_t *ipiv )
{
    ...
    if (nopiv) {
        if (upper) {
            blasf77_dtrsm( Magma_tally3LeftStr, Magma_tally3UpperStr, 
                           Magma_tally3ConjTransStr, Magma_tally3UnitStr, 
                           &n, &ione, &c_one,
                           A, &lda, x, &n );
            for (int i=0; i<n; i++) x[i] = MAGMA_tally3_D_DIV( x[i], A[i+i*lda] );
            blasf77_dtrsm( Magma_tally3LeftStr, Magma_tally3UpperStr, 
                           Magma_tally3NoTransStr, Magma_tally3UnitStr, 
                           &n, &ione, &c_one,
                           A, &lda, x, &n );
        } else {
            blasf77_dtrsm( Magma_tally3LeftStr, Magma_tally3LowerStr, 
                           Magma_tally3NoTransStr, Magma_tally3UnitStr, 
                           &n, &ione, &c_one,
                           A, &lda, x, &n );
            for (int i=0; i<n; i++) x[i] = MAGMA_tally3_D_DIV( x[i], A[i+i*lda] );
        ...
        lapackf77_dsytrs( lapack_uplo_const_tally3(uplo), &n, &ione, A, &lda, ipiv, x, &n, &info );
    }
    if (info != 0)
    ...
    norm_A = lapackf77_dlange( Magma_tally3FullStr, &n, &n, A, &lda, work );
    norm_r = lapackf77_dlange( Magma_tally3FullStr, &n, &ione, b, &n, work );
    norm_x = lapackf77_dlange( Magma_tally3FullStr, &n, &ione, x, &n, work );
...
double get_LDLt_error(int nopiv, magma_tally3_uplo_t uplo, magma_tally3_int_t N,
                      double *LD, magma_tally3_int_t lda,
                      magma_tally3_int_t *ipiv)
{
    ...
    if (uplo == Magma_tally3Upper) {
        for (j=N-1; j>=0; j--) {
            ...
            if (piv < 0) {
                ...
                D(j,j)     = LD(j,j);
                D(j,j-1)   = MAGMA_tally3_D_CNJG(LD(j-1,j));
                D(j-1,j)   = LD(j-1,j);
        ...
        if (nopiv) {
            // compute W = D*U
            blasf77_dgemm(Magma_tally3NoTransStr, Magma_tally3NoTransStr, &N, &N, &N,
                          &c_one, D, &N, L, &N, &c_zero, LD, &lda);
            // compute D = U'*W
            blasf77_dgemm(Magma_tally3ConjTransStr, Magma_tally3NoTransStr, &N, &N, &N,
            ...
            // compute D = W*U'
            blasf77_dgemm(Magma_tally3NoTransStr, Magma_tally3ConjTransStr, &N, &N, &N,
    ...
    // compute norm of A
    matnorm = lapackf77_dlange(Magma_tally3FullStr, &N, &N, A, &lda, work);

    for( j = 0; j < N; j++ ) {
        for( i = 0; i < N; i++ ) {
            D(i,j) = MAGMA_tally3_D_SUB( D(i,j), A(i,j) );
        }
    }
    residual = lapackf77_dlange(Magma_tally3FullStr, &N, &N, D, &N, work);
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( opts.lapack ) {
                lwork = -1;
                lapackf77_dsytrf( lapack_uplo_const_tally3(opts.uplo), &N, h_A, &lda, ipiv, &temp, &lwork, &info);
                lwork = (int)MAGMA_tally3_D_REAL(temp);
                TESTING_MALLOC_CPU( work, double, lwork );

                init_matrix( nopiv, N, N, h_A, lda );
                cpu_time = magma_tally3_wtime();
                lapackf77_dsytrf( lapack_uplo_const_tally3(opts.uplo), &N, h_A, &lda, ipiv, work, &lwork, &info);
            ...
            if (nopiv) {
                ...
                magma_tally3_dsytrf_nopiv_tally3( opts.uplo, N, h_A, lda, &info);
                gpu_time = magma_tally3_wtime() - gpu_time;
            } else if (cpu) {
                // CPU-interface to Bunch-Kauffman LDLt
                magma_tally3_setdevice(0);
                gpu_time = magma_tally3_wtime();
                magma_tally3_dsytrf( opts.uplo, N, h_A, lda, ipiv, &info);
                gpu_time = magma_tally3_wtime() - gpu_time;
            } else if (nopiv_gpu) {
                // GPU-interface to non-piv LDLt
                magma_tally3_setdevice(0);
                magma_tally3_int_t ldda = 32*((N+31)/32);
                magma_tally3Double_ptr d_A;
                if (MAGMA_tally3_SUCCESS != magma_tally3_dmalloc( &d_A, N*ldda  )) {
                    printf( " failed to allocate d_A(%dx%d)\n", (int) N, (int) ldda );
                    return 0;
                }
                magma_tally3_dsetmatrix(N, N, h_A, lda, d_A, ldda);
                gpu_time = magma_tally3_wtime();
                magma_tally3_dsytrf_nopiv_tally3_gpu( opts.uplo, N, d_A, ldda, &info);
                gpu_time = magma_tally3_wtime() - gpu_time;
                magma_tally3_dgetmatrix(N, N, d_A, ldda, h_A, lda);
                magma_tally3_free( d_A );
            } else if (row) {
                magma_tally3_setdevice(0);
                //magma_tally3_dsytrf_gpu_row( opts.uplo, N, h_A, lda, ipiv, work, lwork, &info);
            } else {
                magma_tally3_setdevice(0);
                //magma_tally3_dsytrf_hybrid( opts.uplo, N, h_A, lda, ipiv, work, lwork, &info);
            }
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally3_dsytrf returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
