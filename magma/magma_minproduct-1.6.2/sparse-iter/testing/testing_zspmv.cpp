...
int main(  int argc, char** argv )
{
    ...
    while( i < argc ) {
        ...
        // init CPU vectors
        CHECK( magma_minproduct_zvinit( &hx, Magma_minproduct_CPU, hA.num_rows, 1, c_zero, queue ));
        CHECK( magma_minproduct_zvinit( &hy, Magma_minproduct_CPU, hA.num_rows, 1, c_zero, queue ));

        // init DEV vectors
        CHECK( magma_minproduct_zvinit( &dx, Magma_minproduct_DEV, hA.num_rows, 1, c_one, queue ));
        CHECK( magma_minproduct_zvinit( &dy, Magma_minproduct_DEV, hA.num_rows, 1, c_zero, queue ));
        ...
        CHECK( magma_minproduct_zmconvert(  hA, &hA_ELL, Magma_minproduct_CSR, Magma_minproduct_ELL, queue ));
        CHECK( magma_minproduct_zmtransfer( hA_ELL, &dA_ELL, Magma_minproduct_CPU, Magma_minproduct_DEV, queue ));
        ...
        start = magma_minproduct_sync_wtime( queue );
        for (j=0; j<10; j++)
            CHECK( magma_minproduct_z_spmv( c_one, dA_ELL, dx, c_zero, dy, queue ));
        end = magma_minproduct_sync_wtime( queue );
