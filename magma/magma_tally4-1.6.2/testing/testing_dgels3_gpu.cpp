/*
    -- MAGMA_tally4 (version 1.6.1) --
       ...
       @generated from testing_zgels3_gpu.cpp normal z -> d, Fri Jan 30 19:00:26 2015
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lhwork2 = (magma_tally4_int_t) MAGMA_tally4_D_REAL( tmp[0] );
            
            lhwork = -1;
            lapackf77_dormqr( Magma_tally4LeftStr, Magma_tally4TransStr,
            ...
            magma_tally4_dgels3_gpu( Magma_tally4NoTrans, M, N, nrhs, d_A, ldda,
                              d_B, lddb, h_work, lworkgpu, &info);
            gpu_time = magma_tally4_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally4_dgels3_gpu returned error %d: %s.\n",
                       (int) info, magma_tally4_strerror( info ));
            
            // Get the solution in h_X
            magma_tally4_dgetmatrix( N, nrhs, d_B, lddb, h_X, ldb );
