   ...
   /* check allocation */
    ...
    if (uplo == Magma_tally3Upper) {
        ...
        for(j = 0; j < n; j+=nb) {
            ...
            #if 0
            arginfo = magma_tally3_cpotrf_panel_batched(
                               uplo, n-j, ib,
                               dA_displ, ldda,
            ...
            #else
            //arginfo = magma_tally3_cpotrf_rectile_batched(
            arginfo = magma_tally3_cpotrf_recpanel_batched(
                               uplo, n-j, ib, 32,
                               dA_displ, ldda,
                               dwork_array, dwork_msize,
                               dinvA_array, invA_msize,
            ...
            if( (n-j-ib) > 0){
                ...
                { 
                    ...
                    for(k=0; k<batchCount; k++)
                    {
                        ...
                        magma_tally3_cherk(Magma_tally3Lower, Magma_tally3NoTrans, n-j-ib, ib, 
                            d_alpha, 
                            (const magma_tally3FloatComplex*) cpuAarray[k] + j+ib+j*ldda, ldda, 
                            d_beta,
                ...
                else
                {
                    ...
                    //-------------------------------------------
                    magma_tally3_cdisplace_pointers(dA_displ, dA_array, ldda, j+ib, j, batchCount, queue);
                    magma_tally3_cdisplace_pointers(dW1_displ, dA_array, ldda, j+ib, j+ib, batchCount, queue);
                    magma_tally3blas_cherk_batched(uplo, Magma_tally3NoTrans, n-j-ib, ib,
