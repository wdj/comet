/*
    -- MAGMA_tally4 (version 1.6.2) --
       ...
       @generated from magma_tally4_zmtranspose.cpp normal z -> c, Sun May  3 11:23:01 2015
       @author Hartwig Anzt
       @author Mark Gates
...
/**
    ...
    ---------
    @param[in]
    n_rows      magma_tally4_int_t
                number of rows in input matrix

    @param[in]
    n_cols      magma_tally4_int_t
    ...
    @param[in]
    values      magma_tally4FloatComplex*
    ...
    @param[in]
    colind      magma_tally4_index_t*
                column indices of input matrix

    @param[in]
    new_n_rows  magma_tally4_index_t*
                number of rows in transposed matrix

    @param[in]
    new_n_cols  magma_tally4_index_t*
                number of columns in transposed matrix

    @param[in]
    new_nnz     magma_tally4_index_t*
                number of nonzeros in transposed matrix

    @param[in]
    new_values  magma_tally4FloatComplex**
                value array of transposed matrix

    @param[in]
    new_rowptr  magma_tally4_index_t**
                row pointer of transposed matrix

    @param[in]
    new_colind  magma_tally4_index_t**
...
c_transpose_csr(
    ...
    magma_tally4_index_t *colind,
    magma_tally4_int_t *new_n_rows,
    magma_tally4_int_t *new_n_cols,
    magma_tally4_int_t *new_nnz,
    magma_tally4FloatComplex **new_values,
    magma_tally4_index_t **new_rowptr,
    magma_tally4_index_t **new_colind,
    magma_tally4_queue_t queue )
{
    ...
    magma_tally4FloatComplex *csc_values=NULL;
    magma_tally4_index_t *csc_colptr=NULL, *csc_rowind=NULL;
    
    // i, j are actual row & col indices (0 <= i < nrows, 0 <= j < ncols).
    // k is index into col and values (0 <= k < nnz).
    magma_tally4_int_t i, j, k, total, tmp;
    
    CHECK( magma_tally4_cmalloc_cpu( &csc_values, nnz ) );
    CHECK( magma_tally4_index_malloc_cpu( &csc_colptr, n_cols + 1 ) );
    CHECK( magma_tally4_index_malloc_cpu( &csc_rowind, nnz ) );
...
cleanup:
    magma_tally4_free_cpu( csc_values );
    magma_tally4_free_cpu( csc_colptr );
    magma_tally4_free_cpu( csc_rowind );
    return info;
}
    ...
    magma_tally4_queue_t queue )
{
    ...
    magma_tally4_c_matrix ACSR={Magma_tally4_CSR}, BCSR={Magma_tally4_CSR};
    magma_tally4_c_matrix A_d={Magma_tally4_CSR}, B_d={Magma_tally4_CSR};

    if( A.storage_type == Magma_tally4_CSR && A.memory_location == Magma_tally4_DEV ) {
        ...
        if ( A.fill_mode == Magma_tally4_FULL ) {
            B->fill_mode = Magma_tally4_FULL;
        }
        else if ( A.fill_mode == Magma_tally4_LOWER ) {
            B->fill_mode = Magma_tally4_UPPER;
        }
        else if ( A.fill_mode == Magma_tally4_UPPER ) {
    ...
    }else if( A.storage_type == Magma_tally4_CSR && A.memory_location == Magma_tally4_CPU ){
        CHECK( magma_tally4_cmtransfer( A, &A_d, A.memory_location, Magma_tally4_DEV, queue ));
        CHECK( magma_tally4_c_cucsrtranspose( A_d, &B_d, queue ));
        CHECK( magma_tally4_cmtransfer( B_d, B, Magma_tally4_DEV, A.memory_location, queue ));
                
    }else {
        CHECK( magma_tally4_cmconvert( A, &ACSR, A.storage_type, Magma_tally4_CSR, queue ));
        CHECK( magma_tally4_c_cucsrtranspose( ACSR, &BCSR, queue ));
        CHECK( magma_tally4_cmconvert( BCSR, B, Magma_tally4_CSR, A.storage_type, queue ));
    }
cleanup:
    ...
    magma_tally4_cmfree( &BCSR, queue );
    if( info != 0 ){
