...
#define dT(j)   (dT + (j)*nb)
    ...
    magma_minproductDoubleComplex_ptr dV, dW;
    ...
    *info = 0;
    ...
    lwork = n*nb;
    lpanel = (m - kk)*(n - kk);
    magma_minproduct_zmalloc_cpu( &work, lwork + lpanel );
    if ( work == NULL ) {
    ...
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc( &dV, ldda*nb )) {
    ...
    if (kk < n) {
        ...
        n_kk = n - kk;
        k_kk = k - kk;
        magma_minproduct_zgetmatrix( m_kk, k_kk,
    ...
    if (kk > 0) {
        ...
        for (i = ki; i >= 0; i -= nb) {
            ...
            magma_minproduct_zcopymatrix_async( mi, ib,
                                     dA(i,i), ldda,
                                     dV,      ldda, stream );
