...
magma_tally4_cresidualvec(
    ...
    magma_tally4_c_matrix *r, float *res,
    magma_tally4_queue_t queue )
{
    ...
    if ( A.num_rows == b.num_rows ) {
        ...
        CHECK( magma_tally4_c_spmv( mone, A, x, zero, *r, queue ));      // r = A x
        magma_tally4_caxpy(dofs, one, b.dval, 1, r->dval, 1);          // r = r - b
        *res =  magma_tally4_scnrm2(dofs, r->dval, 1);            // res = ||r||
        ...
        for( magma_tally4_int_t i=0; i<num_vecs; i++) {
            magma_tally4_caxpy(dofs, one, b(i), 1, r(i), 1);   // r = r - b
