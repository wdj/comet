    ...
    magma_minproduct_int_t *info)
{
    ...
    *info = 0;
    ...
    work[0]  = MAGMA_minproduct_Z_MAKE( lwmin * one_eps, 0 );
    ...
    magma_minproduct_int_t ldwork_real = max( ldwork*2, n );
    if ( wantz ) {
        // zstedx requrise 3n^2/2, in double
        ldwork_real = max( ldwork_real, 3*n*(n/2 + 1) );
    }
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc( &dwork, ldwork_real )) {
        *info = MAGMA_minproduct_ERR_DEVICE_ALLOC;
        return *info;
    }
    dC = (magma_minproductDoubleComplex*) dwork;
    ...
    if (iscale == 1) {
        magma_minproductblas_zlascl( uplo, 0, 0, 1., sigma, n, n, dA, ldda, info );
    }
...
#ifdef FAST_HEMV
    magma_minproduct_zhetrd2_gpu( uplo, n, dA, ldda, w, &rwork[inde],
                       &work[indtau], wA, ldwa, &work[indwrk], llwork,
                       dC, ldwork, &iinfo );
#else
    magma_minproduct_zhetrd_gpu ( uplo, n, dA, ldda, w, &rwork[inde],
                       &work[indtau], wA, ldwa, &work[indwrk], llwork,
                       &iinfo );
#endif
    ...
    if (! wantz) {
        ...
        magma_minproduct_dmove_eig( range, n, w, &il, &iu, vl, vu, mout );
    }
    else {
        timer_start( time );
        ...
        magma_minproduct_zsetmatrix( n, *mout, &work[indwrk + n * (il-1) ], n, dC, lddc );

        magma_minproduct_zunmtr_gpu( Magma_minproductLeft, uplo, Magma_minproductNoTrans, n, *mout, dA, ldda, &work[indtau],
                          dC, lddc, wA, ldwa, &iinfo );

        magma_minproduct_zcopymatrix( n, *mout, dC, lddc, dA, ldda );
