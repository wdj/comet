    ...
    magma_minproductFloatComplex       * __restrict__ y, int incy)
{
    ...
    __shared__ magma_minproductFloatComplex sx[BLK_Y];
    ...
    if ( threadIdx.x < npart ) {
        ...
        sx[threadIdx.x] = MAGMA_minproduct_C_ZERO;
    }
    __syncthreads();
    ...
    magma_minproductFloatComplex       * __restrict__ y, int incy)
{
    ...
    __shared__ magma_minproductFloatComplex sdata[BLK_X];
...
magma_minproductblas_cgemv(
    magma_minproduct_trans_t trans, magma_minproduct_int_t m, magma_minproduct_int_t n, magma_minproductFloatComplex alpha,
    ...
    magma_minproductFloatComplex_ptr dy, magma_minproduct_int_t incy)
{
    ...
    if ( arch < 200  ) {
        ...
        magma_minproduct_cgemv( trans, m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
        #else
        magma_minproductblas_cgemv_tesla( trans, m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
        #endif
        return;
    }
    ...
    if ( trans == Magma_minproductNoTrans ) {
        ...
        cgemvn_kernel1_fermi<<< grid, threads, 0, magma_minproduct_stream >>>
            ( m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    }
    else if ( trans == Magma_minproductTrans ) {
        dim3 grid    ( 1, n, 1 );
        dim3 threads ( BLK_X, 1, 1 );
        cgemvt_kernel_fermi<<< grid, threads, 0, magma_minproduct_stream >>>
            ( m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    }
    else if ( trans == Magma_minproductConjTrans ) {
        dim3 grid    ( 1, n, 1 );
        dim3 threads ( BLK_X, 1, 1 );
        cgemvc_kernel_fermi<<< grid, threads, 0, magma_minproduct_stream >>>
            ( m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    }
}
