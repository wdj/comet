/*
    -- MAGMA_minproduct (version 1.6.1) --
       ...
       @generated from zgeqrf_batched.cpp normal z -> d, Fri Jan 30 19:00:19 2015
...
#define dA(i, j)  (dA + (i) + (j)*ldda)   // A(i, j) means at i row, j column
    ...
    magma_minproduct_malloc((void**)&dW4_displ, batchCount * sizeof(*dW4_displ));  // used in dlarfb
    ...
    magma_minproduct_dmalloc(&dwork,  (2 * nb * n) * batchCount);
    magma_minproduct_dmalloc(&dR,  ldr * n   * batchCount);
    magma_minproduct_dmalloc(&dT,  ldt * ldt * batchCount);
    magma_minproduct_malloc_cpu((void**) &cpuAarray, batchCount*sizeof(double*));
    magma_minproduct_malloc_cpu((void**) &cpuTarray, batchCount*sizeof(double*));
    ...
    magma_minproductblas_dlaset_q(Magma_minproductFull, ldr, n*batchCount  , MAGMA_minproduct_D_ZERO, MAGMA_minproduct_D_ZERO, dR, ldr, queue);
    magma_minproductblas_dlaset_q(Magma_minproductFull, ldt, ldt*batchCount, MAGMA_minproduct_D_ZERO, MAGMA_minproduct_D_ZERO, dT, ldt, queue);
    dset_pointer(dR_array, dR, 1, 0, 0, ldr*min(nb, min_mn), batchCount, queue);
    dset_pointer(dT_array, dT, 1, 0, 0, ldt*min(nb, min_mn), batchCount, queue);
    ...
    magma_minproduct_getvector( batchCount, sizeof(double*), dA_array, 1, cpuAarray, 1);
    magma_minproduct_getvector( batchCount, sizeof(double*), dT_array, 1, cpuTarray, 1);
    ...
    for(i=0; i<min_mn;i+=nb)
    {
            ...
            magma_minproduct_ddisplace_pointers(dW0_displ, dA_array, ldda, i, i, batchCount, queue); 
            magma_minproduct_ddisplace_pointers(dW2_displ, tau_array, 1, i, 0, batchCount, queue);
            ...
            magma_minproduct_dgeqrf_panel_batched(m-i, ib, jb, 
                                       dW0_displ, ldda, 
                                       dW2_displ, 
                                       dT_array, ldt, 
                                       dR_array, ldr,
           ...
           dgeqrf_copy_upper_batched(ib, jb, dW0_displ, ldda, dR_array, ldr, batchCount, queue);
            ...
            //reset dW4_displ
            ldw = nb;
            dset_pointer(dW4_displ, dwork, 1, 0, 0,  ldw*n, batchCount, queue );
            offset = ldw*n*batchCount;
            dset_pointer(dW5_displ, dwork + offset, 1, 0, 0,  ldw*n, batchCount, queue );    

            if( (n-ib-i) > 0)
            {
    
                // set the diagonal of v as one and the upper triangular part as zero
                magma_minproductblas_dlaset_batched(Magma_minproductUpper, ib, ib, MAGMA_minproduct_D_ZERO, MAGMA_minproduct_D_ONE, dW0_displ, ldda, batchCount, queue); 
                magma_minproduct_ddisplace_pointers(dW2_displ, tau_array, 1, i, 0, batchCount, queue); 

                // it is faster since it is using BLAS-3 GEMM routines, different from lapack implementation 
                magma_minproduct_dlarft_batched(m-i, ib, 0,
                                 dW0_displ, ldda,
                                 dW2_displ,
                                 dT_array, ldt, 
                                 dW4_displ, nb*ldt,
                ...
                { 
                    ...
                    for(k=0; k<batchCount; k++)
                    {
                        ...
                        magma_minproduct_dlarfb_gpu_gemm(Magma_minproductLeft, Magma_minproductConjTrans, Magma_minproductForward, Magma_minproductColumnwise,
                                    m-i, n-i-ib, ib,
                ...
                else
                {
                    ...
                    magma_minproduct_ddisplace_pointers(dW1_displ, dA_array, ldda, i, i+ib, batchCount, queue); 
            ...
            magma_minproductblas_dlacpy_batched(Magma_minproductUpper, ib, ib, dR_array, ldr, dW0_displ, ldda, batchCount, queue);
