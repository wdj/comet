...
/**
    ...
    @ingroup magma_minproduct_dgeev_driver
    ********************************************************************/
    ...
    magma_minproduct_int_t *info )
{
    ...
    *info = 0;
    ...
    /* Compute workspace */
    nb = magma_minproduct_get_dgehrd_nb( n );
    if (*info == 0) {
        minwrk = (2 +   nb)*n;
        optwrk = (2 + 2*nb)*n;
        work[0] = MAGMA_minproduct_D_MAKE( (double) optwrk, 0. );
    ...
    double *dT;
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc( &dT, nb*n )) {
    ...
    double *T;
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc_cpu( &T, nb*n )) {
    ...
    if (wantvl) {
        ...
        lapackf77_dlacpy( Magma_minproductLowerStr, &n, &n, A, &lda, VL, &ldvl );
        ...
        flops_start( flop_unghr );
            ...
            // Version 3 - LAPACK consistent MAGMA_minproduct HRD + T matrices stored
            magma_minproduct_dorghr( n, ilo, ihi, VL, ldvl, &work[itau], dT, nb, &ierr );
        #elif defined(Version5)
            // Version 5 - Multi-GPU, T on host
            magma_minproduct_dorghr_m( n, ilo, ihi, VL, ldvl, &work[itau], T, nb, &ierr );
        #endif
        ...
        #elif defined(Version3) || defined(Version4)
            // Version 3 - LAPACK consistent MAGMA_minproduct HRD + T matrices stored
            magma_minproduct_dorghr( n, ilo, ihi, VR, ldvr, &work[itau], dT, nb, &ierr );
        #elif defined(Version5)
            // Version 5 - Multi-GPU, T on host
            magma_minproduct_dorghr_m( n, ilo, ihi, VR, ldvr, &work[itau], T, nb, &ierr );
        #endif
        time_sum += timer_stop( time_unghr );
        flop_sum += flops_stop( flop_unghr );
    ...
    if (wantvl || wantvr) {
        ...
        magma_minproduct_dtrevc3( side, Magma_minproductBacktransVec, select, n, A, lda, VL, ldvl,
                       VR, ldvr, n, &nout, &work[iwrk], liwrk, &ierr );
        #elif TREVC_VERSION == 4
        magma_minproduct_dtrevc3_mt( side, Magma_minproductBacktransVec, select, n, A, lda, VL, ldvl,
                          VR, ldvr, n, &nout, &work[iwrk], liwrk, &ierr );
        #elif TREVC_VERSION == 5
        magma_minproduct_dtrevc3_mt_gpu( side, Magma_minproductBacktransVec, select, n, A, lda, VL, ldvl,
                              VR, ldvr, n, &nout, &work[iwrk], liwrk, &ierr );
        #else
    ...
    if (wantvl) {
        ...
        for (i = 0; i < n; ++i) {
            if ( wi[i] == 0. ) {
                scl = 1. / magma_minproduct_cblas_dnrm2( n, VL(0,i), 1 );
                blasf77_dscal( &n, &scl, VL(0,i), &ione );
            }
            else if ( wi[i] > 0. ) {
                d__1 = magma_minproduct_cblas_dnrm2( n, VL(0,i), 1 );
                d__2 = magma_minproduct_cblas_dnrm2( n, VL(0,i+1), 1 );
    ...
    if (wantvr) {
        ...
        for (i = 0; i < n; ++i) {
            if ( wi[i] == 0. ) {
                scl = 1. / magma_minproduct_cblas_dnrm2( n, VR(0,i), 1 );
                blasf77_dscal( &n, &scl, VR(0,i), &ione );
            }
            else if ( wi[i] > 0. ) {
                d__1 = magma_minproduct_cblas_dnrm2( n, VR(0,i), 1 );
                d__2 = magma_minproduct_cblas_dnrm2( n, VR(0,i+1), 1 );
                scl = 1. / lapackf77_dlapy2( &d__1, &d__2 );
                blasf77_dscal( &n, &scl, VR(0,i), &ione );
