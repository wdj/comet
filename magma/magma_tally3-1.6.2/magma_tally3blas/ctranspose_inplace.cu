...
__global__ void ctranspose_inplace_odd( int n, magma_tally3FloatComplex *matrix, int lda )
{
    __shared__ magma_tally3FloatComplex sA[ NB ][ NB+1 ];
    __shared__ magma_tally3FloatComplex sB[ NB ][ NB+1 ];
    ...
    magma_tally3FloatComplex *A = matrix + ii+i + (jj+j)*lda;
    if( ii == jj ) {
        if ( ii+i < n && jj+j < n ) {
            sA[j][i] = *A;
        }
        ...
        magma_tally3FloatComplex *B = matrix + jj+i + (ii+j)*lda;
        if ( ii+i < n && jj+j < n ) {
            sA[j][i] = *A;
        }
        if ( jj+i < n && ii+j < n ) {
...
__global__ void ctranspose_inplace_even( int n, magma_tally3FloatComplex *matrix, int lda )
{
    __shared__ magma_tally3FloatComplex sA[ NB ][ NB+1 ];
    __shared__ magma_tally3FloatComplex sB[ NB ][ NB+1 ];
...
/**
    @see magma_tally3blas_ctranspose_inplace_q
    @ingroup magma_tally3_caux2
    ********************************************************************/
extern "C" void
magma_tally3blas_ctranspose_inplace(
    ...
    magma_tally3FloatComplex_ptr dA, magma_tally3_int_t ldda )
{
    magma_tally3blas_ctranspose_inplace_q( n, dA, ldda, magma_tally3_stream );
}
