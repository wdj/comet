...
/**
    ...
    This version stores the T matrices in dT, for later use in magma_tally2_zunghr.
    ...
    magma_tally2_int_t *info)
{
    ...
    magma_tally2_int_t nb = magma_tally2_get_zgehrd_nb(n);
    ...
    *info = 0;
    iws = n*nb;
    work[0] = MAGMA_tally2_Z_MAKE( iws, 0 );
    ...
    if (nb == 1 || nb > nh) {
        ...
        if (MAGMA_tally2_SUCCESS != magma_tally2_zmalloc( &dwork, 2*nb*ldda + n*ldda )) {
            *info = MAGMA_tally2_ERR_DEVICE_ALLOC;
            return *info;
        }
        magma_tally2DoubleComplex *dV = dwork + nb*ldda;
        magma_tally2DoubleComplex *dA = dwork + nb*ldda*2;
        magma_tally2DoubleComplex *dTi;
        
        magma_tally2DoubleComplex *T;
        magma_tally2_zmalloc_cpu( &T, nb*nb );
        if ( T == NULL ) {
        ...
        magma_tally2blas_zlaset( Magma_tally2Full, nb, nb, c_zero, c_zero, dV, ldda );
        ...
        magma_tally2blas_zlaset( Magma_tally2Full, nb, n, c_zero, c_zero, dT, nb );
        
        // Copy the matrix to the GPU
        magma_tally2_zsetmatrix( n, n-ilo, A(0,ilo), lda, dA, ldda );
        ...
        for (i = ilo; i < ihi-1 - nb; i += nb) {
            ...
            magma_tally2_zgetmatrix( ihi-i, nb,
                              dA(i,i-ilo), ldda,
                              A(i,i), lda );
            
            // add 1 to i for 1-based index
            magma_tally2_zlahr2( ihi, i+1, nb,
                          dA(0,i-ilo), ldda,
                          dV,          ldda,
                          A(0,i),      lda,
                          &tau[i], T, nb, work, n);
            ...
            dTi = dT + (i - ilo)*nb;
            magma_tally2_zsetmatrix( nb, nb, T, nb, dTi, nb );
            
            magma_tally2_zlahru( n, ihi, i, nb,
                          A(0,i),      lda,
                          dA(0,i-ilo), ldda, // dA
