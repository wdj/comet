...
/* ////////////////////////////////////////////////////////////////////////////
   -- Testing magma_tally3_zher2k_mgpu
*/
int main( int argc, char** argv)
{
    ...
    magma_tally3DoubleComplex alpha = MAGMA_tally3_Z_MAKE( 1.2345, 4.321 );
    double beta = 3.14159;
    
    real_Double_t    gflops, gpu_perf, cpu_perf, gpu_time, cpu_time;
    double           error, work[1];
    magma_tally3DoubleComplex *hA, *hR, *hR2, *hV, *hW;
    magma_tally3DoubleComplex_ptr dV[Magma_tally3MaxGPUs], dW[Magma_tally3MaxGPUs], dA[Magma_tally3MaxGPUs];
    magma_tally3_int_t n, k, size, lda, ldda, nb, ngpu, nstream;
    ...
    printf( "version 1: magma_tally3blas_zher2k_mgpu2     %s\n", (opts.version==1 ? "(enabled)" : ""));
    printf( "version 2: magma_tally3blas_zher2k_mgpu_spec %s\n", (opts.version==2 ? "(enabled)" : ""));
#ifdef ICHI
    printf( "version 3: magma_tally3_zher2k_mgpu (Ichi)   %s\n", (opts.version==3 ? "(enabled)" : ""));
#endif
    printf( "\n" );
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        ...
        for( int offset = 0; offset < n; offset += min(k,nb) ) {
            for( int iter = 0; iter < opts.niter; ++iter ) {
                ...
                TESTING_MALLOC_CPU( hA,  magma_tally3DoubleComplex, lda*n   );
                TESTING_MALLOC_CPU( hR,  magma_tally3DoubleComplex, lda*n   );
                TESTING_MALLOC_CPU( hR2, magma_tally3DoubleComplex, lda*n   );
                TESTING_MALLOC_CPU( hV,  magma_tally3DoubleComplex, lda*k*2 );
                //TESTING_MALLOC_CPU( hW,  magma_tally3DoubleComplex, lda*k   );
                for( int d = 0; d < ngpu; ++d ) {
                    magma_tally3_int_t nlocal = ((n / nb) / ngpu + 1) * nb;
                    magma_tally3_setdevice( d );
                    TESTING_MALLOC_DEV( dA[d], magma_tally3DoubleComplex, ldda*nlocal );
                    TESTING_MALLOC_DEV( dV[d], magma_tally3DoubleComplex, ldda*k*2    );
                    //TESTING_MALLOC_DEV( dW[d], magma_tally3DoubleComplex, ldda*k      );
                ...
                /* ====================================================================
                   ...
                   =================================================================== */
                magma_tally3_zsetmatrix_1D_col_bcyclic( n, n, hA, lda, dA, ldda, ngpu, nb );
                for( int d = 0; d < ngpu; ++d ) {
                    magma_tally3_setdevice( d );
                    dW[d] = dV[d] + ldda*k;
                    magma_tally3_zsetmatrix( n, k, hV, lda, dV[d], ldda );
                    magma_tally3_zsetmatrix( n, k, hW, lda, dW[d], ldda );
                }
                ...
                if ( opts.version == 1 ) {
                    magma_tally3blas_zher2k_mgpu2(
                        Magma_tally3Lower, Magma_tally3NoTrans, n-offset, k,
                        alpha, dV, ldda, 0,
                               dW, ldda, 0,
                        beta,  dA, ldda, offset,
                    ...
                    magma_tally3_zher2k_mgpu(
                        ngpu, Magma_tally3Lower, Magma_tally3NoTrans, nb, n-offset, k,
                        alpha, dV, ldda,
                               //dW, ldda,
                        beta,  dA, ldda, offset,
                        nstream, streams );
                ...
                magma_tally3_zgetmatrix_1D_col_bcyclic( n, n, dA, ldda, hR, lda, ngpu, nb );
                ...
                if ( opts.lapack || opts.check ) {
                    // store ||V||*||W|| + ||A||
                    magma_tally3_int_t n_offset = n - offset;
                    ...
                    // compute relative error ||R||/||A||, where R := A_magma_tally3 - A_lapack = R - A
                    size = lda*n;
