/*
    -- MAGMA_tally3 (version 1.6.2) --
       ...
       @generated from magma_tally3_zmilustruct.cpp normal z -> d, Sun May  3 11:23:01 2015
       @author Hartwig Anzt
*/
...
void magma_tally3_dshell_sort(
  const magma_tally3_int_t n,
  magma_tally3_int_t x[]);

void magma_tally3_dsymbolic_ilu(
...
void magma_tally3_dmexFunction(magma_tally3_int_t nlhs, magma_tally3_int_t n, double omega,
                 magma_tally3_int_t levfill, magma_tally3_int_t storage,
                magma_tally3_index_t * ial, magma_tally3_index_t *jal, double *al,
                magma_tally3_index_t * iau, magma_tally3_index_t *jau, double *au,
                magma_tally3_int_t nrhs,
                magma_tally3_index_t * ia, magma_tally3_index_t *ja, double *a ){
    ...
    magma_tally3_dsymbolic_ilu(levfill, n, &nzl, &nzu, ia, ja, ial, jal, iau, jau);
    ...
    for (i=0; i<n; i++)
    {
     ...
     //   printf("check line %d\n", i);
        ...
        for (j=ia[i]; j<ia[i+1]; j++)
            iwork[next++] = ja[j];
        magma_tally3_dshell_sort(len, iwork);
    ...
    magma_tally3_queue_t queue )
{
    ...
    magma_tally3_d_matrix A_copy={Magma_tally3_CSR}, B={Magma_tally3_CSR};
    magma_tally3_d_matrix hA={Magma_tally3_CSR}, CSRCOOA={Magma_tally3_CSR};
    
    if( A->memory_location == Magma_tally3_CPU && A->storage_type == Magma_tally3_CSR ){

        CHECK( magma_tally3_dmtransfer( *A, &A_copy, Magma_tally3_CPU, Magma_tally3_CPU, queue ));
        CHECK( magma_tally3_dmtransfer( *A, &B, Magma_tally3_CPU, Magma_tally3_CPU, queue ));

        // possibility to scale to unit diagonal
        //magma_tally3_dmscale( &B, Magma_tally3_UNITDIAG );

        CHECK( magma_tally3_dmconvert( B, L, Magma_tally3_CSR, Magma_tally3_CSR , queue));
        CHECK( magma_tally3_dmconvert( B, U, Magma_tally3_CSR, Magma_tally3_CSR, queue ));
        ...
        magma_tally3_dsymbolic_ilu( levels, A->num_rows, &num_lnnz, &num_unnz, B.row, B.col,
                                            L->row, L->col, U->row, U->col );
        L->nnz = num_lnnz;
        U->nnz = num_unnz;
        magma_tally3_free_cpu( L->val );
        magma_tally3_free_cpu( U->val );
        CHECK( magma_tally3_dmalloc_cpu( &L->val, L->nnz ));
        CHECK( magma_tally3_dmalloc_cpu( &U->val, U->nnz ));
        for( magma_tally3_int_t i=0; i<L->nnz; i++ )
            L->val[i] = MAGMA_tally3_D_MAKE( 0.0, 0.0 );

        for( magma_tally3_int_t i=0; i<U->nnz; i++ )
            U->val[i] = MAGMA_tally3_D_MAKE( 0.0, 0.0 );
        ...
        CHECK( magma_tally3_dmalloc_cpu( &A->val, L->nnz+U->nnz ));
        A->nnz = L->nnz+U->nnz ;
        ...
        CHECK( magma_tally3_dmconvert( hA, &CSRCOOA, hA.storage_type, Magma_tally3_CSR, queue ));

        CHECK( magma_tally3_dsymbilu( &CSRCOOA, levels, L, U, queue ));

        magma_tally3_dmfree( &hA, queue );
        magma_tally3_dmfree( A, queue );
        CHECK( magma_tally3_dmconvert( CSRCOOA, &hA, Magma_tally3_CSR, A_storage, queue ));
...
cleanup:
    ...
    magma_tally3_dmfree( &A_copy, queue );
    magma_tally3_dmfree( &B, queue );
    magma_tally3_dmfree( &hA, queue );
    magma_tally3_dmfree( &CSRCOOA, queue );
    return info;
}
