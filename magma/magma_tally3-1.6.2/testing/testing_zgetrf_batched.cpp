...
double get_LU_error(magma_tally3_int_t M, magma_tally3_int_t N,
                    ...
                    magma_tally3DoubleComplex *LU, magma_tally3_int_t *IPIV)
{
    magma_tally3_int_t min_mn = min(M,N);
    magma_tally3_int_t ione   = 1;
    magma_tally3_int_t i, j;
    magma_tally3DoubleComplex alpha = MAGMA_tally3_Z_ONE;
    magma_tally3DoubleComplex beta  = MAGMA_tally3_Z_ZERO;
    magma_tally3DoubleComplex *L, *U;
    double work[1], matnorm, residual;
    
    TESTING_MALLOC_CPU( L, magma_tally3DoubleComplex, M*min_mn);
    TESTING_MALLOC_CPU( U, magma_tally3DoubleComplex, min_mn*N);
    memset( L, 0, M*min_mn*sizeof(magma_tally3DoubleComplex) );
    memset( U, 0, min_mn*N*sizeof(magma_tally3DoubleComplex) );

    lapackf77_zlaswp( &N, A, &lda, &ione, &min_mn, IPIV, &ione);
    lapackf77_zlacpy( Magma_tally3LowerStr, &M, &min_mn, LU, &lda, L, &M      );
    lapackf77_zlacpy( Magma_tally3UpperStr, &min_mn, &N, LU, &lda, U, &min_mn );

    for(j=0; j<min_mn; j++)
        L[j+j*M] = MAGMA_tally3_Z_MAKE( 1., 0. );
    ...
    for( j = 0; j < N; j++ ) {
        for( i = 0; i < M; i++ ) {
            LU[i+j*lda] = MAGMA_tally3_Z_SUB( LU[i+j*lda], A[i+j*lda] );
        }
    }
...
int main( int argc, char** argv)
{
    ...
    magma_tally3DoubleComplex *h_A, *h_R, *h_Amagma_tally3;
    ...
    for( int i = 0; i < opts.ntest; ++i ) {
      ...
      for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_DEV(  dA,  magma_tally3DoubleComplex, ldda*N * batchCount);
            ...
            lapackf77_zlacpy( Magma_tally3UpperLowerStr, &M, &columns, h_A, &lda, h_R, &lda );
            
            /* ====================================================================
               Performs operation using MAGMA_tally3
               =================================================================== */

            magma_tally3_zsetmatrix( M, columns, h_R, lda, dA, ldda );
            ...
            info = magma_tally3_zgetrf_batched( M, N, dA_array, ldda, dipiv_array,  dinfo_magma_tally3, batchCount, queue);
            magma_tally3_time = magma_tally3_sync_wtime(0) - magma_tally3_time;
            magma_tally3_perf = gflops / magma_tally3_time;
            
            magma_tally3_zgetmatrix( M, N*batchCount, dA, ldda, h_Amagma_tally3, lda );
            ...
            {
                if(cpu_info[i] != 0 ){
                    printf("magma_tally3_zgetrf_batched matrix %d returned internal error %d\n",i, (int)cpu_info[i] );
                }
            }
            
            if (info != 0)
                printf("magma_tally3_zgetrf_batched returned argument error %d: %s.\n", (int) info, magma_tally3_strerror( info ));
