...
/**
    ...
    1) y =       A   x      if trans == 'N' or 'n', alpha == 1, beta == 0,
                            and incx == incy == 1 (using magma_tally3blas code)
    ...
    trans   magma_tally3_trans_t
      ...
      -     = Magma_tally3ConjTrans:  y := alpha*A^T*x + beta*y
    ...
    if ( incx == 1 && incy == 1 && beta == 0 ) {
        if ( trans == Magma_tally3NoTrans ) {
            if ( alpha == 1. ) {
                ...
                sgemv_kernel_tesla<<< grid, threads, 0, magma_tally3_stream >>>
                    (m, n, (n/gemv_bs)*gemv_bs, A, lda, x, y);
            }
            else {
                magma_tally3_sgemv( trans, m, n, alpha, A, lda, x, incx, beta, y, incy);
            }
        }
        else {
            magma_tally3blas_sgemvt_tesla(m, n, alpha, A, lda, x, y);
        }
    }
    else {
        magma_tally3_sgemv( trans, m, n, alpha, A, lda, x, incx, beta, y, incy);
...
/**
    ...
    @ingroup magma_tally3_sblas2_internal
    ********************************************************************/
extern "C" void
magma_tally3blas_sgemv2_tesla(
    ...
    float       *y )
{
    ...
    sgemv_kernel2_tesla<<< grid, threads, 0, magma_tally3_stream >>>
...
/**
    ...
    ********************************************************************/
extern "C" void
magma_tally3blas_sgemvt1_tesla(
    magma_tally3_int_t m, magma_tally3_int_t n, float alpha,
    const float *A, magma_tally3_int_t lda,
    const float *x,
    float       *y )
{
    magma_tally3_int_t blocks = (n - 1)/32 + 1;
    dim3 grid( blocks, 1, 1 );
    dim3 threads( 32, 2, 1 );
    sgemvt_kernel1_tesla<<< grid, threads, 0, magma_tally3_stream >>>
...
/**
    ...
    ********************************************************************/
extern "C" void
magma_tally3blas_sgemvt2_tesla(
    magma_tally3_int_t m, magma_tally3_int_t n, float alpha,
    const float *A, magma_tally3_int_t lda,
    const float *x,
    float       *y )
{
    magma_tally3_int_t blocks = (n - 1)/16 + 1;
    dim3 grid( blocks, 1, 1 );
    dim3 threads( 16, 4, 1 );
    sgemvt_kernel2_tesla<<< grid, threads, 0, magma_tally3_stream >>>
...
{
    if ( n <= 128 )
        magma_tally3blas_sgemvt2_tesla(m, n, alpha, A, lda, x, y);
    else
        magma_tally3blas_sgemvt1_tesla(m, n, alpha, A, lda, x, y);
}
