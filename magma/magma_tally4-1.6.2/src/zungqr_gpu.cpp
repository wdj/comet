...
#define dT(j)   (dT + (j)*nb)
    ...
    magma_tally4DoubleComplex_ptr dV, dW;
    ...
    *info = 0;
    ...
    lwork = n*nb;
    lpanel = (m - kk)*(n - kk);
    magma_tally4_zmalloc_cpu( &work, lwork + lpanel );
    if ( work == NULL ) {
    ...
    if (MAGMA_tally4_SUCCESS != magma_tally4_zmalloc( &dV, ldda*nb )) {
    ...
    if (kk < n) {
        ...
        n_kk = n - kk;
        k_kk = k - kk;
        magma_tally4_zgetmatrix( m_kk, k_kk,
    ...
    if (kk > 0) {
        ...
        for (i = ki; i >= 0; i -= nb) {
            ...
            magma_tally4_zcopymatrix_async( mi, ib,
                                     dA(i,i), ldda,
                                     dV,      ldda, stream );
