...
#if defined(MAGMA_tally4_WITH_ACML)
#include <acml.h>
#endif

// defining MAGMA_tally4_LAPACK_H is a hack to NOT include magma_tally4_lapack.h
// via common_magma_tally4.h here, since it conflicts with acml.h and we don't
// need lapack here, but we want acml.h for the acmlversion() function.
#define MAGMA_tally4_LAPACK_H
...
// --------------------
// subset of the CUDA device properties, set by magma_tally4_init()
struct magma_tally4_device
{
    size_t memory;
    magma_tally4_int_t cuda_arch;
};

int g_magma_tally4_devices_cnt = 0;
struct magma_tally4_device* g_magma_tally4_devices = NULL;
...
// When renumbering devices after calling magma_tally4_init,
// call magma_tally4_finalize, then cudaSetValidDevices, then magma_tally4_init again.
// Ideally magma_tally4_init is paired with magma_tally4_finalize, but this implementation
// ensures there isn't a memory leak if magma_tally4_init is called multiple times
// without calling magma_tally4_finalize.
extern "C"
magma_tally4_int_t magma_tally4_init()
{
    if ( g_magma_tally4_devices == NULL ) {
        cudaError_t err;
        err = cudaGetDeviceCount( &g_magma_tally4_devices_cnt );
        check_error( err );
        g_magma_tally4_devices = (struct magma_tally4_device*) malloc( g_magma_tally4_devices_cnt * sizeof(struct magma_tally4_device) );
        for( int i = 0; i < g_magma_tally4_devices_cnt; ++i ) {
            cudaDeviceProp prop;
            err = cudaGetDeviceProperties( &prop, i );
            check_error( err );
            g_magma_tally4_devices[i].memory = prop.totalGlobalMem;
            g_magma_tally4_devices[i].cuda_arch  = prop.major*100 + prop.minor*10;
        }
    }
...
extern "C"
magma_tally4_int_t magma_tally4_finalize()
{
    free( g_magma_tally4_devices );
    g_magma_tally4_devices = NULL;
    return MAGMA_tally4_SUCCESS;
}
...
{
    magma_tally4_int_t major, minor, micro;
    magma_tally4_version( &major, &minor, &micro );
    printf( "MAGMA_tally4 %d.%d.%d %s compiled for CUDA capability >= %.1f\n",
            (int) major, (int) minor, (int) micro, MAGMA_tally4_VERSION_STAGE, MIN_CUDA_ARCH/100. );
...
#else
    printf( "MAGMA_tally4 not compiled with OpenMP. " );
#endif
...
printf( "ndevices %d\n", ndevices );
    ...
    for( int dev = 0; dev < ndevices; dev++ ) {
        ...
        if ( arch < MIN_CUDA_ARCH ) {
            printf("\n"
                   "==============================================================================\n"
                   "WARNING: MAGMA_tally4 was compiled only for CUDA capability %.1f and higher;\n"
...
// This requires magma_tally4_init to be called first (to cache the information).
...
magma_tally4_int_t magma_tally4_getdevice_arch()
{
    ...
    check_error( err );
    if ( g_magma_tally4_devices == NULL || dev < 0 || dev >= g_magma_tally4_devices_cnt ) {
        fprintf( stderr, "Error in %s: MAGMA_tally4 not initialized (call magma_tally4_init() first) or bad device\n", __func__ );
        return 0;
    }
    return g_magma_tally4_devices[dev].cuda_arch;
}
...
// --------------------
extern "C"
void magma_tally4_getdevice( magma_tally4_device_t* device )
{
    cudaError_t err;
...
// --------------------
extern "C"
void magma_tally4_setdevice( magma_tally4_device_t device )
{
    cudaError_t err;
...
// ========================================
// queue support
// At the moment, MAGMA_tally4 queue == CUDA stream.
// In the future, MAGMA_tally4 queue may be CUBLAS handle.
// --------------------
extern "C"
...
// --------------------
extern "C"
void magma_tally4_event_create( magma_tally4_event_t* event )
{
    cudaError_t err;
