...
//===================================================================================================
static __device__
void clarft_gemvcolwise_device( int m, magma_tally4FloatComplex *v, magma_tally4FloatComplex *tau,
                         magma_tally4FloatComplex *c, int ldc, magma_tally4FloatComplex *T, int ldt, int step )
{
    ...
    if ( !MAGMA_tally4_C_EQUAL(*tau, MAGMA_tally4_C_ZERO) ) {
        if(thblk<step){    
            const int tx = threadIdx.x;
            magma_tally4FloatComplex *dc = c + blockIdx.x * ldc;
            ...
            if (tx==0)
                ...
                tmp = MAGMA_tally4_C_ZERO;
            for( int j = tx+1; j < m; j += BLOCK_SIZE ){
                tmp +=  MAGMA_tally4_C_CNJG( v[j] ) * dc[j];
            }
            sum[tx] = tmp;
            magma_tally4_sum_reduce< BLOCK_SIZE >( tx, sum );
            #if defined (use_gemm_larft)
            *(T+thblk) = MAGMA_tally4_C_CNJG(sum[0]);
            #else
            tmp = - MAGMA_tally4_C_CNJG(*tau) * sum[0]; 
            *(T+thblk) = MAGMA_tally4_C_CNJG(tmp); // T = - tau(tx) * V(tx:n,1:tx-1)' * V(tx:n,tx) = tmp'
            //*(T+thblk) = - MAGMA_tally4_C_CNJG(sum[0]) * (*tau); // T = - tau(tx) * V(tx:n,1:tx-1)' * V(tx:n,tx) = tmp'
            #endif
        }
        else{
            #if defined (use_gemm_larft)
            *(T+thblk) = MAGMA_tally4_C_ONE;
            #else
            *(T+thblk) = *tau;
            #endif
        }
    ...
    {
        *(T+thblk) = MAGMA_tally4_C_ZERO;
    }
}
//===================================================================================================
__global__
void clarft_gemvcolwise_kernel( int m, magma_tally4FloatComplex *v, int ldv, magma_tally4FloatComplex *tau,
                          magma_tally4FloatComplex *T, int ldt, int step )
{
    clarft_gemvcolwise_device(m, v+step+step*ldv, tau+step, v+step, ldv, T+step*ldt, ldt, step);
}
//===================================================================================================
__global__
void clarft_gemvcolwise_kernel_batched( int m, magma_tally4FloatComplex **v_array, int ldv, magma_tally4FloatComplex **tau_array,
                          magma_tally4FloatComplex **T_array, int ldt, int step )
{
    ...
    magma_tally4FloatComplex *tau)
{
    ...
    clarft_gemvcolwise_kernel<<< grid, threads, 0, magma_tally4_stream >>>( m, v, ldv, tau, T, ldt, step);
...
clarft_gemvrowwise_device(
    int m, int i,
    magma_tally4FloatComplex *tau, 
    magma_tally4FloatComplex *v_ptr, int ldv, 
    magma_tally4FloatComplex *x_ptr, int incx,
    magma_tally4FloatComplex *T_ptr, int ldt,
    magma_tally4FloatComplex *W, magma_tally4FloatComplex* sdata)
{
    int tx = threadIdx.x; 
    int ty = threadIdx.y; 
    ...
    if(tx < cgemv_bs)
    {
        ...
        {
            res += MAGMA_tally4_C_CNJG (v_ptr[s]) * x_ptr[s*incx];
        }
    ...
    magma_tally4_sum_reduce<cgemv_bs>(tx, &(sdata[ty*cgemv_bs+0]));
    ...
    magma_tally4FloatComplex *T, int ldt)
{
    ...
    magma_tally4FloatComplex *W =  T +i*ldt;
...
clarft_gemvrowwise_kernel_batched(
    ...
    magma_tally4FloatComplex **T_array, int ldt)
{

    int batchid = blockIdx.z;

    magma_tally4FloatComplex *W =  T_array[batchid] +i*ldt;
    ...
    magma_tally4FloatComplex *W)
{
    ...
    clarft_gemvrowwise_kernel <<< grid, threads, sizeof(magma_tally4FloatComplex)*cgemv_bs*(i+1), magma_tally4_stream>>>(m, i, tau, v, ldv, T, ldt);
}
//===================================================================================================
extern "C"
    ...
    magma_tally4_int_t batchCount, magma_tally4_queue_t queue)
{
    ...
    */
    clarft_gemvrowwise_kernel_batched <<< grid, threads, sizeof(magma_tally4FloatComplex)*cgemv_bs*(i+1), queue>>>(m, i,  tau_array, v_array, ldv, T_array, ldt);
}
//===================================================================================================
    ...
    magma_tally4FloatComplex *T, int ldt)
{
    ...
    for(int i=1; i<k;i++)
    {
        ...
        magma_tally4FloatComplex *v_ptr = v;

        v_ptr += i;

        magma_tally4FloatComplex *x_ptr = v_ptr + i * ldv;
            
        res = MAGMA_tally4_C_ZERO;
    ...
    magma_tally4FloatComplex *T, int ldt)
{
    ...
    clarft_gemv_loop_inside_kernel<<<grid, threads, sizeof(magma_tally4FloatComplex) * (cgemv_bs*(k+1)), magma_tally4_stream>>>(n, k, tau, v, ldv, T, ldt); 
}
//===================================================================================================
extern "C"
    ...
    magma_tally4FloatComplex **T_array, int ldt, magma_tally4_int_t batchCount, magma_tally4_queue_t queue)
{
    ...
    dim3 grid(1, 1, batchCount);
    dim3 threads(cgemv_bs, max(k,1), 1);
    clarft_gemv_loop_inside_kernel_batched<<<grid, threads, sizeof(magma_tally4FloatComplex) * (cgemv_bs*(k+1)), queue>>>(n, k, tau_array, v_array, ldv, T_array, ldt); 
}
//===================================================================================================
...
clarft_ctrmv_sm32x32_device(
    int n, int k, magma_tally4FloatComplex *tau,
    magma_tally4FloatComplex *Tin, int ldtin,  magma_tally4FloatComplex *Tout, int ldtout )
{
    int tx = threadIdx.x; 
    magma_tally4FloatComplex *sdata = (magma_tally4FloatComplex*)shared_data;
    magma_tally4FloatComplex res;
...
clarft_ctrmv_sm32x32_kernel_batched(
    int n, int k, magma_tally4FloatComplex **tau_array,
    magma_tally4FloatComplex **Tin_array, int ldtin,  magma_tally4FloatComplex **Tout_array, int ldtout )
{
    int batchId = blockIdx.z;
    ...
    magma_tally4FloatComplex *Tout, magma_tally4_int_t ldtout)
{
    ...
    clarft_ctrmv_sm32x32_kernel <<< grid, threads, sizeof(magma_tally4FloatComplex)*(m*m), magma_tally4_stream >>> (m, n,  tau, Tin, ldtin, Tout, ldtout);
}
//===================================================================================================
extern "C"
    ...
    magma_tally4_int_t batchCount, magma_tally4_queue_t queue)
{
    ...
    dim3 grid(1, 1, batchCount);
    dim3 threads(max(m,1), 1, 1);
    clarft_ctrmv_sm32x32_kernel_batched <<< grid, threads, sizeof(magma_tally4FloatComplex)*(m*m), queue >>> (m, n,  tau_array, Tin_array, ldtin, Tout_array, ldtout);
}
//===================================================================================================
...
clarft_recctrmv_sm32x32_device(
    int m, int n, magma_tally4FloatComplex *tau,
    magma_tally4FloatComplex *Trec, int ldtrec, magma_tally4FloatComplex *Ttri, int ldttri)
{
    int tx = threadIdx.x; 
    magma_tally4FloatComplex *sdata = (magma_tally4FloatComplex*)shared_data;
    magma_tally4FloatComplex res;
...
clarft_recctrmv_sm32x32_kernel_batched(
    int m, int n, magma_tally4FloatComplex **tau_array,
    magma_tally4FloatComplex **Trec_array, int ldtrec, magma_tally4FloatComplex **Ttri_array, int ldttri)
{
    int batchId = blockIdx.z;
    ...
    magma_tally4FloatComplex *Ttri, magma_tally4_int_t ldttri)
{
    ...
    dim3 grid(1);
    dim3 threads(max(m,1), 1, 1);
    clarft_recctrmv_sm32x32_kernel <<< grid, threads, sizeof(magma_tally4FloatComplex)*(m*n), magma_tally4_stream >>> (m, n,  tau, Trec, ldtrec, Ttri, ldttri);
}
//===================================================================================================
extern "C"
    ...
    magma_tally4_int_t batchCount, magma_tally4_queue_t queue)
{
    ...
    dim3 grid(1, 1, batchCount);
    dim3 threads(max(m,1), 1, 1);
    clarft_recctrmv_sm32x32_kernel_batched <<< grid, threads, sizeof(magma_tally4FloatComplex)*(m*n), queue >>> (m, n,  tau_array, Trec_array, ldtrec, Ttri_array, ldttri);
}
//===================================================================================================
