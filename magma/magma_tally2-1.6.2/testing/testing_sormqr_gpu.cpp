...
int main( int argc, char** argv )
{
    ...
    float *C, *R, *A, *W, *tau;
    magma_tally2Float_ptr dC, dA, dT;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int itran = 0; itran < 2; ++itran ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {        
            ...
            magma_tally2_ssetmatrix( lda, k, A,  lda, dA, lda );
            magma_tally2_sgeqrf_gpu( lda, k, dA, lda, tau, dT, &info );
            magma_tally2_sgetmatrix( lda, k, dA, lda, A,  lda );
            if (info != 0)
            ...
            lapackf77_sormqr( lapack_side_const_tally2( side[iside] ), lapack_trans_const_tally2( trans[itran] ),
                              &m, &n, &k,
            ...
            lwork = -1;
            magma_tally2_sormqr_gpu( side[iside], trans[itran],
                              m, n, k,
                              dA, lda, tau, dC, ldc, W, lwork, dT, nb, &info );
            if (info != 0)
                printf("magma_tally2_sormqr_gpu (lwork query) returned error %d: %s.\n",
                       (int) info, magma_tally2_strerror( info ));
            lwork = (magma_tally2_int_t) MAGMA_tally2_S_REAL( W[0] );
            ...
            if (info != 0)
                printf("magma_tally2_sormqr_gpu returned error %d: %s.\n",
                       (int) info, magma_tally2_strerror( info ));
