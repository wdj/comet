...
int main( int argc, char** argv)
{
    ...
    double cto, cfrom;
    magma_minproduct_int_t M, N, size, lda, ldda, info;
      ...
      for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( uplo[iuplo] == Magma_minproductLower || uplo[iuplo] == Magma_minproductUpper ) {
                ...
                gbytes = sizeof(magma_minproductDoubleComplex) * 0.5*2.*N*(N+1) / 1e9;
            }
            else {
            ...
            //magma_minproductblas_zlascl( uplo[iuplo], 1, 1, cfrom, cto, M-2, N-2, d_A+1+ldda, ldda, &info );  // inset by 1 row & col
            magma_minproductblas_zlascl( uplo[iuplo], 1, 1, cfrom, cto, M, N, d_A, ldda, &info );
            gpu_time = magma_minproduct_sync_wtime( 0 ) - gpu_time;
            gpu_perf = gbytes / gpu_time;
            if (info != 0)
                printf("magma_minproductblas_zlascl returned error %d: %s.\n",
            ...
            //magma_minproduct_zprint( M, N, h_R, lda );
