...
/**
    ...
    tau     COMPLEX_16 array, dimension (N)
            ...
            reflector H(i), as returned by MAGMA_minproduct_ZGEQRF_GPU.
    ...
    dT      COMPLEX_16 array that is the output (the 6th argument)
            of magma_minproduct_zgeqrf_gpu of size
    ...
    lwork   INTEGER
            ...
            where NB is the blocksize given by magma_minproduct_get_zgeqrf_nb( M ).
    \n
    ...
    @ingroup magma_minproduct_zgels_comp
    ********************************************************************/
    ...
    magma_minproduct_int_t *info)
{
    ...
    magma_minproduct_int_t i, k, lddwork, rows, ib;
    magma_minproduct_int_t ione = 1;

    magma_minproduct_int_t nb     = magma_minproduct_get_zgeqrf_nb(m);
    magma_minproduct_int_t lwkopt = (m - n + nb)*(nrhs + nb) + nrhs*nb;
    int lquery = (lwork == -1);

    hwork[0] = MAGMA_minproduct_Z_MAKE( (double)lwkopt, 0. );
    ...
    *info = 0;
    ...
    /* B := Q' * B */
    magma_minproduct_zunmqr_gpu( Magma_minproductLeft, Magma_minproduct_ConjTrans,
                      m, nrhs, n,
                      dA(0,0), ldda, tau,
    ...
    // TODO: this assumes that, on exit from magma_minproduct_zunmqr_gpu, hwork contains
    ...
    // magma_minproduct_ztrsm and drop the zsetmatrix.
    if ( nrhs == 1 ) {
        blasf77_ztrsv( Magma_minproductUpperStr, Magma_minproductNoTransStr, Magma_minproductNonUnitStr,
                       &ib, hwork,         &rows,
                            hwork+rows*ib, &ione);
    } else {
        blasf77_ztrsm( Magma_minproductLeftStr, Magma_minproductUpperStr, Magma_minproductNoTransStr, Magma_minproductNonUnitStr,
                       &ib, &nrhs,
                       &c_one, hwork,         &rows,
                               hwork+rows*ib, &rows);
    }
    ...
    magma_minproduct_zsetmatrix( ib, nrhs, hwork+rows*ib, rows, dwork+i, lddwork );

    // update c
    if (nrhs == 1)
        magma_minproduct_zgemv( Magma_minproductNoTrans, i, ib,
    ...
    if (nb < k) {
        for (i = start; i >= 0; i -= nb) {
            ...
            if (i + ib < n) {
                if (nrhs == 1) {
                    magma_minproduct_zgemv( Magma_minproductNoTrans, ib, ib,
                                 c_one,  dT(i), ib,
                                         dB+i,      1,
                                 c_zero, dwork+i,  1);
                    magma_minproduct_zgemv( Magma_minproductNoTrans, i, ib,
    ...
    magma_minproduct_zcopymatrix( (n), nrhs,
                       dwork, lddwork,
                       dB,    lddb );
