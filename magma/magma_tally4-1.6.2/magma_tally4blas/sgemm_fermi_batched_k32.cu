/*
    -- MAGMA_tally4 (version 1.6.1) --
       ...
       @generated from zgemm_fermi_batched_k32.cu normal z -> s, Fri Jan 30 19:00:10 2015
...
#include "sgemm_fermi_kernels_batched_k32.h"
    ...
    float **dC_array, magma_tally4_int_t lddc, magma_tally4_int_t batchCount, magma_tally4_queue_t queue )
{
    ...
    if      ( transB == Magma_tally4Trans )
        ...
        float *cpuAarray[2], *cpuBarray[2];
        magma_tally4_getvector( min(2,batchCount), sizeof(float*), dA_array, 1, cpuAarray, 1);
        magma_tally4_getvector( min(2,batchCount), sizeof(float*), dB_array, 1, cpuBarray, 1);
             ...
             sizeB >= CUBLAS_MAX_1DBUF_SIZE )
        {
            ...
            return;
            /*
            magma_tally4_sgemm( transA, transB, m, n, k, alpha,
        ...
        cudaError_t err;
        err = cudaBindTexture(&offsetA, tex_ref_A, cpuAarray[0], sizeA*sizeof(float));
        if ( err != cudaSuccess ) {
            fprintf( stderr, "cannot bind A to texture: %s (%d)\n", cudaGetErrorString(err), err );
            return;
        }	
        err = cudaBindTexture(&offsetB, tex_ref_B, cpuBarray[0], sizeB*sizeof(float));
        if ( err != cudaSuccess ) {
