...
extern "C" void magma_tally2_sbulge_applyQ(
    magma_tally2_int_t WANTZ, magma_tally2_side_t SIDE, magma_tally2_int_t NE, magma_tally2_int_t N, magma_tally2_int_t NB,
    magma_tally2_int_t Vblksiz, float *E, magma_tally2_int_t LDE,
    float *V, float *TAU, float *T,
    magma_tally2_int_t *INFO, float *dV, float *dT,
    float *dE, magma_tally2_int_t copytype )
{
    //%===========================
    //%   local variables
    //%===========================
    ...
    magma_tally2_int_t LDT, LDV, firstcolj;
    magma_tally2_int_t bg, nbGblk, rownbm, k, m, n;
    magma_tally2_int_t st, ed, fst, vlen, vnb, colj, len;
    magma_tally2_int_t blkid, vpos, taupos, tpos;
    //float *WORK;
    magma_tally2_int_t LWORK;
    magma_tally2_int_t  cur_blksiz, avai_blksiz, ncolinvolvd;
    magma_tally2_int_t  nbgr, colst, coled, versionL, versionR;
    magma_tally2_int_t blkcnt=-1;
    ...
    *INFO=0;
     ...
     * */
    if (copytype > 0) magma_tally2_ssetmatrix( LDV, blkcnt*Vblksiz, V, LDV, dV, LDV );
    if (copytype > 1) magma_tally2_ssetmatrix( LDT, blkcnt*Vblksiz, T, LDT, dT, LDT );
    if (copytype > 2) magma_tally2_ssetmatrix( N, NE, E, N, dE, N );
    float *dwork;
    //ldwork  = NE;
    LWORK   = 2*N*max(Vblksiz, 64);
    if (MAGMA_tally2_SUCCESS != magma_tally2_smalloc( &dwork, LWORK )) {
    ...
    if (WANTZ == 1) {
        ...
        magma_tally2blas_slaset( Magma_tally2Full, N, N, c_zero, c_one, dE, N );
    }
            ...
            for (bg = nbGblk; bg > 0; bg--) {
                ...
                for (m = rownbm; m > 0; m--) {
                    ...
                    if ((vlen > 0) && (vnb > 0)) {
                        if (WANTZ == 1) {
                            len =  N-colst;
                            magma_tally2_slarfb_gpu( Magma_tally2Left, Magma_tally2NoTrans, Magma_tally2Forward, Magma_tally2Columnwise, vlen, len, vnb, dV(vpos), LDV, dT(tpos), LDT, dE(fst,colst), LDE, dwork, len);
                        } else {
                            magma_tally2_slarfb_gpu( Magma_tally2Left, Magma_tally2NoTrans, Magma_tally2Forward, Magma_tally2Columnwise, vlen, NE, vnb, dV(vpos), LDV, dT(tpos), LDT, dE(fst,0), LDE, dwork, NE);
                        }
                    }
                }
            }
            ...
            for (m = rownbm; m > 0; m--) {
                ...
                for (n = nbgr; n > 0; n--) {
                    ...
                    if ((vlen > 0) && (vnb > 0)) {
                        ...
                        magma_tally2_slarfb_gpu( Magma_tally2Left, Magma_tally2NoTrans, Magma_tally2Forward, Magma_tally2Columnwise, vlen, N1, vnb, dV(vpos), LDV, dT(tpos), LDT, dE(fst,0), LDE, dwork, N1);
                        magma_tally2blasSetKernelStream(stream[1]);
                        magma_tally2_slarfb_gpu( Magma_tally2Left, Magma_tally2NoTrans, Magma_tally2Forward, Magma_tally2Columnwise, vlen, N2, vnb, dV(vpos), LDV, dT(tpos), LDT, dE(fst,N1), LDE, &dwork[N1*Vblksiz], N2);
                        #else
                        magma_tally2_slarfb_gpu( Magma_tally2Left, Magma_tally2NoTrans, Magma_tally2Forward, Magma_tally2Columnwise, vlen, NE, vnb, dV(vpos), LDV, dT(tpos), LDT, dE(fst,0), LDE, dwork, NE);
                        #endif
                    }
            ...
            for (bg =1; bg <= nbGblk; bg++) {
                ...
                for (m = 1; m <= rownbm; m++) {
                    ...
                    if ((vlen > 0) && (vnb > 0)) {
                        #if defined(USESTREAM)
                        magma_tally2blasSetKernelStream(stream[0]);
                        magma_tally2_slarfb_gpu( Magma_tally2Right, Magma_tally2NoTrans, Magma_tally2Forward, Magma_tally2Columnwise, N1, vlen, vnb, dV(vpos), LDV, dT(tpos), LDT, dE(0, fst), LDE, dwork, N1);
                        magma_tally2blasSetKernelStream(stream[1]);
                        magma_tally2_slarfb_gpu( Magma_tally2Right, Magma_tally2NoTrans, Magma_tally2Forward, Magma_tally2Columnwise, N2, vlen, vnb, dV(vpos), LDV, dT(tpos), LDT, dE(N1, fst), LDE, &dwork[N1*Vblksiz], N2);
                        #else
                        magma_tally2_slarfb_gpu( Magma_tally2Right, Magma_tally2NoTrans, Magma_tally2Forward, Magma_tally2Columnwise, NE, vlen, vnb, dV(vpos), LDV, dT(tpos), LDT, dE(0, fst), LDE, dwork, NE);
                        #endif
                    }
                }
            }
