...
int main( int argc, char** argv )
{
    ...
    magma_tally2FloatComplex *C, *R, *A, *work, *tau, *tauq, *taup;
    float *d, *e;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int itran = 0; itran < 2; ++itran ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            n = opts.nsize[itest];
            k = opts.ksize[itest];
            nb  = magma_tally2_get_cgebrd_nb( m );
            ldc = m;
            ...
            TESTING_MALLOC_CPU( A,    magma_tally2FloatComplex, lda*nn );
            TESTING_MALLOC_CPU( work, magma_tally2FloatComplex, lwork_max );
            TESTING_MALLOC_CPU( d,    float,             min(mm,nn) );
            TESTING_MALLOC_CPU( e,    float,             min(mm,nn) );
            TESTING_MALLOC_CPU( tauq, magma_tally2FloatComplex, min(mm,nn) );
            TESTING_MALLOC_CPU( taup, magma_tally2FloatComplex, min(mm,nn) );
            ...
            magma_tally2_cgebrd( mm, nn, A, lda, d, e, tauq, taup, work, lwork_max, &info );
            if (info != 0)
            ...
            lapackf77_cunmbr( lapack_vect_const_tally2( vect[ivect] ),
            ...
            lwork = -1;
            magma_tally2_cunmbr( vect[ivect], side[iside], trans[itran],
                          m, n, k,
                          A, lda, tau, R, ldc, work, lwork, &info );
            if (info != 0)
                printf("magma_tally2_cunmbr (lwork query) returned error %d: %s.\n",
                       (int) info, magma_tally2_strerror( info ));
            lwork = (magma_tally2_int_t) MAGMA_tally2_C_REAL( work[0] );
            ...
            if (info != 0)
                printf("magma_tally2_cunmbr returned error %d: %s.\n",
                       (int) info, magma_tally2_strerror( info ));
