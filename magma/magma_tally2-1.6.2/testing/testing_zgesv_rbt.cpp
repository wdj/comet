...
int main(int argc, char **argv)
{
    ...
    magma_tally2DoubleComplex *h_A, *h_LU, *h_B, *h_X;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_LU, magma_tally2DoubleComplex, lda*N    );
            ...
            magma_tally2_zgesv_rbt(Magma_tally2True, N, nrhs, h_LU, lda, h_X, ldb, &info );
            gpu_time = magma_tally2_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally2_zgesv_rbt returned error %d: %s.\n",
