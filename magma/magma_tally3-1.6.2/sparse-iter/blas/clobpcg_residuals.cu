...
// copied from scnrm2.cu in trunk/magma_tally3blas
// ----------------------------------------
...
template< int n >
__device__ void sum_reduce( /*int n,*/ int i, magma_tally3Float_ptr  x )
{
    __syncthreads();
    ...
    if( row<num_rows){
        for( int i=0; i<num_vecs; i++ ){ 
            ...
            R[row + i*num_rows] = R[row + i*num_rows] 
                                    + MAGMA_tally3_C_MAKE( -evals[i], 0.0 )
                                                * X[ row + i*num_rows ];
        }
    }
}
...
/*
magma_tally3blas_scnrm2_kernel( 
    int m, 
    ...
    magma_tally3_queue_t queue )
{
    ...
    magma_tally3_clobpcg_res_kernel<<< grid, threads, 0, queue >>>
