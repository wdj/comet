...
/**
    ...
    dA      DOUBLE_PRECISION array of DIMENSION ( LDA, ka ), where ka is
            k  when  transA = Magma_tally4NoTrans,  and is  m  otherwise.
            Before entry with  transA = Magma_tally4NoTrans,  the leading  m by k
    ...
    ldda    INTEGER.
            ...
            in the calling (sub) program. When  transA = Magma_tally4NoTrans then
            LDA must be at least  max( 1, m ), otherwise  LDA must be at
            least  max( 1, k ).
    
    @param[in]
    dB      DOUBLE_PRECISION array of DIMENSION ( LDB, kb ), where kb is
            n  when  transB = Magma_tally4NoTrans,  and is  k  otherwise.
            Before entry with  transB = Magma_tally4NoTrans,  the leading  k by n
    ...
    lddb    INTEGER.
            ...
            in the calling (sub) program. When  transB = Magma_tally4NoTrans then
    ...
    double **dC_array, magma_tally4_int_t lddc, magma_tally4_int_t batchCount, magma_tally4_queue_t queue )
{
    ...
    if      ( transA != Magma_tally4NoTrans && transA != Magma_tally4Trans && transA != Magma_tally4ConjTrans )
        info = -1;
    else if ( transB != Magma_tally4NoTrans && transB != Magma_tally4Trans && transB != Magma_tally4ConjTrans )
        info = -2;
    else if ( m < 0 )
        info = -3;
    else if ( n < 0 )
        ...
        info = -5;
    else if ( transA == Magma_tally4NoTrans ? ldda < m : ldda < k )
        info = -8;
    else if ( transB == Magma_tally4NoTrans ? lddb < k : lddb < n )
        info = -10;
    else if ( lddc < m )
        info = -13;
    ...
    if ( arch < 200  ) {
        ...
        // magma_tally4blas for [sd] precisions, cublas for [zc] precisions.
        #if defined(PRECISION_z) || defined(PRECISION_c)
        magma_tally4_dgemm(
            transA, transB,
            m, n, k, alpha, dA_array[0], ldda, dB_array[0], lddb, beta, dC_array[0], lddc );
        #else
        magma_tally4blas_dgemm_tesla(
    ...
    if      ( transA == Magma_tally4Trans )
        TransA = 1;
    else if ( transA == Magma_tally4NoTrans )
        TransA = 0;
                    
    if      ( transB == Magma_tally4Trans )
        TransB = 1;
    else if ( transB == Magma_tally4NoTrans )
        TransB = 0;
        ...
        magma_tally4_getvector( min(2,batchCount), sizeof(double*), dA_array, 1, cpuAarray, 1);
        magma_tally4_getvector( min(2,batchCount), sizeof(double*), dB_array, 1, cpuBarray, 1);
        magma_tally4_int_t matrixA_size = batchCount > 1 ? cpuAarray[1] - cpuAarray[0] : 0;
        magma_tally4_int_t matrixB_size = batchCount > 1 ? cpuBarray[1] - cpuBarray[0] : 0;
        magma_tally4_int_t Am = ( ! TransA ? m : k);
        magma_tally4_int_t An = (!TransA ? k : m);
        magma_tally4_int_t Bm = ( ! TransB ? k : n);
        magma_tally4_int_t Bn = (!TransB ? n : k);
             ...
             sizeB >= CUBLAS_MAX_1DBUF_SIZE )
        {
            ...
            return;
            /*
            magma_tally4_dgemm( transA, transB, m, n, k, alpha,
