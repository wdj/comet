    ...
    magma_minproduct_int_t *info)
{
    ...
    *info = 0;
    ...
    magma_minproduct_int_t Vblksiz = magma_minproduct_cbulge_get_Vblksiz(n, nb, parallel_threads);
    ...
    magma_minproductFloatComplex *dT1;
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_cmalloc( &dT1, n*nb)) {
        *info = MAGMA_minproduct_ERR_DEVICE_ALLOC;
        return *info;
    }
    timer_start( time_band );
    magma_minproduct_chetrd_he2hb(uplo, n, nb, A, lda, &work[indtau1], &work[indwrk], llwork, dT1, info);
    ...
    magma_minproductFloatComplex *da[Magma_minproductMaxGPUs], *dT1[Magma_minproductMaxGPUs];
    ...
    for( magma_minproduct_int_t dev = 0; dev < ngpu; ++dev ) {
        ...
        // TODO check malloc
        magma_minproduct_cmalloc(&da[dev], ldda*mlocal );
        magma_minproduct_cmalloc(&dT1[dev], (n*nb) );
    ...
    magma_minproduct_csetmatrix_1D_col_bcyclic( n, n, A, lda, da, ldda, ngpu, distblk );
    magma_minproduct_setdevice(0);
    timer_stop( time_dist );

    timer_start( time_band );
    if (ver == 30) {
        magma_minproduct_chetrd_he2hb_mgpu_spec(uplo, n, nb, A, lda, &work[indtau1], &work[indwrk], llwork, da, ldda, dT1, nb, ngpu, distblk, streams, nstream, info);
    } else {
        magma_minproduct_chetrd_he2hb_mgpu(uplo, n, nb, A, lda, &work[indtau1], &work[indwrk], llwork, da, ldda, dT1, nb, ngpu, distblk, streams, nstream, info);
    }
    timer_stop( time_band );
    ...
    magma_minproductFloatComplex* A2 = &work[indwrk];
    memset(A2, 0, n*lda2*sizeof(magma_minproductFloatComplex));

    for (magma_minproduct_int_t j = 0; j < n-nb; j++) {
        len = nb+1;
        blasf77_ccopy( &len, A(j,j), &ione, A2(0,j), &ione );
        memset(A(j,j), 0, (nb+1)*sizeof(magma_minproductFloatComplex));
        *A(nb+j,j) = c_one;
    }
    for (magma_minproduct_int_t j = 0; j < nb; j++) {
        len = nb-j;
        blasf77_ccopy( &len, A(j+n-nb,j+n-nb), &ione, A2(0,j+n-nb), &ione );
        memset(A(j+n-nb,j+n-nb), 0, (nb-j)*sizeof(magma_minproductFloatComplex));
    }
    ...
    magma_minproduct_chetrd_hb2st(uplo, n, nb, Vblksiz, A2, lda2, w, &rwork[inde], &work[indV2], ldv, &work[indTAU2], wantz, &work[indT2], ldt);
    ...
    if (! wantz) {
        ...
        magma_minproduct_cstedx_m(ngpu, range, n, vl, vu, il, iu, w, &rwork[inde],
...
/*
        ...
        if (MAGMA_minproduct_SUCCESS != magma_minproduct_cmalloc( &dZ, *m*lddz)) {
            *info = MAGMA_minproduct_ERR_DEVICE_ALLOC;
            return *info;
        }

        magma_minproduct_cbulge_back(uplo, n, nb, *m, Vblksiz, &work[indwrk + n * (il-1)], n, dZ, lddz,
                          &work[indV2], ldv, &work[indTAU2], &work[indT2], ldt, info);

        magma_minproduct_cgetmatrix( n, *m, dZ, lddz, &work[indwrk], n);

        magma_minproduct_free(dZ);

*/

        magma_minproduct_cbulge_back_m(ngpu, uplo, n, nb, *m, Vblksiz, &work[indwrk + n * (il-1)], n,
                            &work[indV2], ldv, &work[indTAU2], &work[indT2], ldt, info);
        ...
        magma_minproduct_cunmqr_m(ngpu, Magma_minproductLeft, Magma_minproductNoTrans, n-nb, *m, n-nb, A+nb, lda, &work[indtau1],
