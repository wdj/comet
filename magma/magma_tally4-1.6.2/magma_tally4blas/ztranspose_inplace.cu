...
__global__ void ztranspose_inplace_odd( int n, magma_tally4DoubleComplex *matrix, int lda )
{
    __shared__ magma_tally4DoubleComplex sA[ NB ][ NB+1 ];
    __shared__ magma_tally4DoubleComplex sB[ NB ][ NB+1 ];
    ...
    magma_tally4DoubleComplex *A = matrix + ii+i + (jj+j)*lda;
    if( ii == jj ) {
        if ( ii+i < n && jj+j < n ) {
            sA[j][i] = *A;
        }
        ...
        magma_tally4DoubleComplex *B = matrix + jj+i + (ii+j)*lda;
        if ( ii+i < n && jj+j < n ) {
            sA[j][i] = *A;
        }
        if ( jj+i < n && ii+j < n ) {
...
__global__ void ztranspose_inplace_even( int n, magma_tally4DoubleComplex *matrix, int lda )
{
    __shared__ magma_tally4DoubleComplex sA[ NB ][ NB+1 ];
    __shared__ magma_tally4DoubleComplex sB[ NB ][ NB+1 ];
...
/**
    @see magma_tally4blas_ztranspose_inplace_q
    @ingroup magma_tally4_zaux2
    ********************************************************************/
extern "C" void
magma_tally4blas_ztranspose_inplace(
    ...
    magma_tally4DoubleComplex_ptr dA, magma_tally4_int_t ldda )
{
    magma_tally4blas_ztranspose_inplace_q( n, dA, ldda, magma_tally4_stream );
}
