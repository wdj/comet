...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_tally2_zgeqlf( M, N, h_R, lda, tau, h_work, lwork, &info);
            gpu_time = magma_tally2_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally2_zgeqlf returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                magma_tally2_int_t ldl = min_mn;
                magma_tally2DoubleComplex *Q, *L;
                double *work;
                TESTING_MALLOC_CPU( Q,    magma_tally2DoubleComplex, ldq*min_mn );  // M by K
                TESTING_MALLOC_CPU( L,    magma_tally2DoubleComplex, ldl*N );       // K by N
