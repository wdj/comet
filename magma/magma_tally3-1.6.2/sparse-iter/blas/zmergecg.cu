...
__global__ void
magma_tally3_zcgreduce_kernel_spmv1( 
    int Gs,
    int n, 
    magma_tally3DoubleComplex * vtmp,
    magma_tally3DoubleComplex * vtmp2 )
{
    ...
    temp[Idx] = MAGMA_tally3_Z_MAKE( 0.0, 0.0);
    int i = blockIdx.x * ( blockSize * 2 ) + Idx;   
    while (i < Gs ) {
        temp[ Idx  ] += vtmp[ i ]; 
        temp[ Idx  ] += ( i + blockSize < Gs ) ? vtmp[ i + blockSize ] 
                                                : MAGMA_tally3_Z_MAKE( 0.0, 0.0); 
        i += gridSize;
    }
    __syncthreads();
    if ( Idx < 64 ){
...
__global__ void
magma_tally3_zcgmerge_spmvell_kernel(  
    int n,
    ...
    magma_tally3DoubleComplex * vtmp )
{
    ...
    if(i < n ){
        ...
        for ( int k = 0; k < num_cols_per_row ; k ++){
            ...
            magma_tally3DoubleComplex val = dval [ n * k + i ];
            if( val != 0)
                dot += val * d[ col ];
        }
        z[ i ] =  dot;
...
__global__ void
magma_tally3_zcgmerge_spmvellpack_kernel(  
    int n,
    int num_cols_per_row,
    magma_tally3DoubleComplex * dval, 
    magma_tally3_index_t * dcolind,
    ...
    magma_tally3DoubleComplex * vtmp )
{
    ...
    if(i < n ){
        ...
        for ( int k = 0; k < num_cols_per_row ; k ++){
            ...
            magma_tally3DoubleComplex val = dval [ num_cols_per_row * i + k ];
            if( val != 0)
                dot += val * d[ col ];
        }
        z[ i ] =  dot;
...
__global__ void
magma_tally3_zcgmerge_spmvellpackrt_kernel_8(  
    int n,
    magma_tally3DoubleComplex * dval, 
    magma_tally3_index_t * dcolind,
...
// computes the SpMV using ELLRT 8 threads per row
__global__ void
magma_tally3_zcgmerge_spmvellpackrt_kernel_16(  
    int n,
    magma_tally3DoubleComplex * dval, 
    magma_tally3_index_t * dcolind,
    magma_tally3_index_t * drowlength,
...
// computes the SpMV using ELLRT 8 threads per row
__global__ void
magma_tally3_zcgmerge_spmvellpackrt_kernel_32(  
    int n,
    magma_tally3DoubleComplex * dval, 
    magma_tally3_index_t * dcolind,
    magma_tally3_index_t * drowlength,
...
__global__ void
magma_tally3_zcgmerge_spmvellpackrt_kernel2(  
    int n,
    magma_tally3DoubleComplex * z,
    magma_tally3DoubleComplex * d,
    magma_tally3DoubleComplex * vtmp2 )
{
    ...
    temp[ Idx ] = ( i < n ) ? z[i]*d[i] : MAGMA_tally3_Z_MAKE(0.0, 0.0);
    __syncthreads();
    if ( Idx < 128 ){
        temp[ Idx ] += temp[ Idx + 128 ];
    }
...
__global__ void
magma_tally3_zcgmerge_spmvsellc_kernel(   
    int num_rows, 
    int blocksize,
    magma_tally3DoubleComplex * dval, 
    magma_tally3_index_t * dcolind,
 ...
 temp[ Idx ] = MAGMA_tally3_Z_MAKE( 0.0, 0.0);
    ...
    if(i < num_rows ){
        ...
        for ( int n = 0; n < border; n ++){
            ...
            magma_tally3DoubleComplex val = dval[offset+ blocksize * n + Idx];
            if( val != 0){
                  dot=dot+val*d[col];
            }
        }
...
__global__ void
magma_tally3_zcgmerge_spmvsellpt_kernel_8( 
    int num_rows, 
    int blocksize,
    int T,
    magma_tally3DoubleComplex * dval, 
...
// SELLC SpMV kernel modified assigning multiple threads to each row - 2D kernel
__global__ void
magma_tally3_zcgmerge_spmvsellpt_kernel_16( 
    int num_rows, 
    int blocksize,
    int T,
    magma_tally3DoubleComplex * dval, 
...
// SELLC SpMV kernel modified assigning multiple threads to each row - 2D kernel
__global__ void
magma_tally3_zcgmerge_spmvsellpt_kernel_32( 
    int num_rows, 
    int blocksize,
    int T,
    magma_tally3DoubleComplex * dval, 
...
magma_tally3_zcg_rhokernel(  
    magma_tally3DoubleComplex * skp ){
    int i = blockIdx.x * blockDim.x + threadIdx.x;

    if( i==0 ){
        magma_tally3DoubleComplex tmp = skp[1];
        skp[3] = tmp/skp[4];
        skp[2] = tmp;
    }
}
...
/**
    ...
    @param[in]
    d1          magma_tally3DoubleComplex_ptr 
                temporary vector

    @param[in]
    d2          magma_tally3DoubleComplex_ptr 
                temporary vector

    @param[in]
    dd          magma_tally3DoubleComplex_ptr 
                input vector d

    @param[out]
    dz          magma_tally3DoubleComplex_ptr 
                input vector z
    ...
    magma_tally3_queue_t queue )
{
    ...
    dim3 Gs_next;
    int Ms =  local_block_size * sizeof( magma_tally3DoubleComplex ); 
    magma_tally3DoubleComplex_ptr aux1 = d1, aux2 = d2;
    int b = 1;        

    if ( A.storage_type == Magma_tally3_CSR )
        magma_tally3_zcgmerge_spmvcsr_kernel<<<Gs, Bs, Ms, queue >>>
        ( A.num_rows, A.dval, A.drow, A.dcol, dd, dz, d1 );
    else if ( A.storage_type == Magma_tally3_ELLPACKT )
        magma_tally3_zcgmerge_spmvellpack_kernel<<<Gs, Bs, Ms, queue >>>
        ( A.num_rows, A.max_nnz_row, A.dval, A.dcol, dd, dz, d1 );
    else if ( A.storage_type == Magma_tally3_ELL )
        magma_tally3_zcgmerge_spmvell_kernel<<<Gs, Bs, Ms, queue >>>
        ( A.num_rows, A.max_nnz_row, A.dval, A.dcol, dd, dz, d1 );
            ...
            int Mssellp = num_threadssellp * sizeof( magma_tally3DoubleComplex );
        ...
        magma_tally3_zcgmerge_spmvellpackrt_kernel2<<<Gs, Bs, Ms, queue >>>
                              ( A.num_rows, dz, dd, d1 );
    ...
    int Mellrt = A.alignment * A.blocksize * sizeof( magma_tally3DoubleComplex );
    ...
    while( Gs.x > 1 ) {
        ...
        magma_tally3_zcgreduce_kernel_spmv1<<< Gs_next.x/2, Bs.x/2, Ms/2 >>> 
    ...
    magma_tally3_zcopyvector( 1, aux1, 1, skp+4, 1 );
    dim3 Bs2( 2 );
    dim3 Gs2( 1 );
    magma_tally3_zcg_rhokernel<<<Gs2, Bs2, 0>>>( skp );
...
__global__ void
magma_tally3_zcgmerge_xrbeta_kernel(  
    int n, 
    ...
    magma_tally3DoubleComplex * vtmp )
{
    ...
    magma_tally3DoubleComplex rho = skp[3];
    magma_tally3DoubleComplex mrho = MAGMA_tally3_Z_MAKE( -1.0, 0.0)*rho;
...
magma_tally3_zcg_alphabetakernel(  
    magma_tally3DoubleComplex * skp )
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;

    if( i==0 ){
        magma_tally3DoubleComplex tmp1 = skp[1];
        skp[0] =  tmp1/skp[2];
        //printf("beta=%e\n", MAGMA_tally3_Z_REAL(tmp1));
    }
}

// update search Krylov vector d
__global__ void
magma_tally3_zcg_d_kernel(  
    int n, 
    magma_tally3DoubleComplex * skp,
    magma_tally3DoubleComplex * r,
    magma_tally3DoubleComplex * d )
{
    ...
    magma_tally3DoubleComplex alpha = skp[0];
...
/**
    ...
    @param[in/out]
    dr          magma_tally3DoubleComplex_ptr 
    ...
    @ingroup magma_tally3sparse_zsygpuk
    ********************************************************************/
    ...
    magma_tally3_queue_t queue )
{
    ...
    dim3 Gs_next;
    int Ms =  2*local_block_size * sizeof( magma_tally3DoubleComplex ); 
    magma_tally3DoubleComplex_ptr aux1 = d1, aux2 = d2;
    int b = 1;        
    magma_tally3_zcgmerge_xrbeta_kernel<<<Gs, Bs, Ms>>>
    ...
    magma_tally3_zcopyvector( 1, aux1, 1, skp+1, 1 );
    dim3 Bs2( 2 );
    dim3 Gs2( 1 );
    magma_tally3_zcg_alphabetakernel<<<Gs2, Bs2, 0>>>( skp );

    dim3 Bs3( local_block_size );
    dim3 Gs3( magma_tally3_ceildiv( n, local_block_size ) );
    magma_tally3_zcg_d_kernel<<<Gs3, Bs3, 0>>>( n, skp, dr, dd );  
