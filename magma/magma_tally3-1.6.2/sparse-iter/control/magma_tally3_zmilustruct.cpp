...
void magma_tally3_zshell_sort(
  const magma_tally3_int_t n,
  magma_tally3_int_t x[]);

void magma_tally3_zsymbolic_ilu(
...
void magma_tally3_zmexFunction(magma_tally3_int_t nlhs, magma_tally3_int_t n, magma_tally3DoubleComplex omega,
                 magma_tally3_int_t levfill, magma_tally3_int_t storage,
                magma_tally3_index_t * ial, magma_tally3_index_t *jal, magma_tally3DoubleComplex *al,
                magma_tally3_index_t * iau, magma_tally3_index_t *jau, magma_tally3DoubleComplex *au,
                magma_tally3_int_t nrhs,
                magma_tally3_index_t * ia, magma_tally3_index_t *ja, magma_tally3DoubleComplex *a ){
    ...
    magma_tally3_zsymbolic_ilu(levfill, n, &nzl, &nzu, ia, ja, ial, jal, iau, jau);
    ...
    for (i=0; i<n; i++)
    {
     ...
     //   printf("check line %d\n", i);
        ...
        for (j=ia[i]; j<ia[i+1]; j++)
            iwork[next++] = ja[j];
        magma_tally3_zshell_sort(len, iwork);
    ...
    magma_tally3_queue_t queue )
{
    ...
    magma_tally3_z_matrix A_copy={Magma_tally3_CSR}, B={Magma_tally3_CSR};
    magma_tally3_z_matrix hA={Magma_tally3_CSR}, CSRCOOA={Magma_tally3_CSR};
    
    if( A->memory_location == Magma_tally3_CPU && A->storage_type == Magma_tally3_CSR ){

        CHECK( magma_tally3_zmtransfer( *A, &A_copy, Magma_tally3_CPU, Magma_tally3_CPU, queue ));
        CHECK( magma_tally3_zmtransfer( *A, &B, Magma_tally3_CPU, Magma_tally3_CPU, queue ));

        // possibility to scale to unit diagonal
        //magma_tally3_zmscale( &B, Magma_tally3_UNITDIAG );

        CHECK( magma_tally3_zmconvert( B, L, Magma_tally3_CSR, Magma_tally3_CSR , queue));
        CHECK( magma_tally3_zmconvert( B, U, Magma_tally3_CSR, Magma_tally3_CSR, queue ));
        ...
        magma_tally3_zsymbolic_ilu( levels, A->num_rows, &num_lnnz, &num_unnz, B.row, B.col,
                                            L->row, L->col, U->row, U->col );
        L->nnz = num_lnnz;
        U->nnz = num_unnz;
        magma_tally3_free_cpu( L->val );
        magma_tally3_free_cpu( U->val );
        CHECK( magma_tally3_zmalloc_cpu( &L->val, L->nnz ));
        CHECK( magma_tally3_zmalloc_cpu( &U->val, U->nnz ));
        for( magma_tally3_int_t i=0; i<L->nnz; i++ )
            L->val[i] = MAGMA_tally3_Z_MAKE( 0.0, 0.0 );

        for( magma_tally3_int_t i=0; i<U->nnz; i++ )
            U->val[i] = MAGMA_tally3_Z_MAKE( 0.0, 0.0 );
        ...
        CHECK( magma_tally3_zmalloc_cpu( &A->val, L->nnz+U->nnz ));
        A->nnz = L->nnz+U->nnz ;
        ...
        CHECK( magma_tally3_zmconvert( hA, &CSRCOOA, hA.storage_type, Magma_tally3_CSR, queue ));

        CHECK( magma_tally3_zsymbilu( &CSRCOOA, levels, L, U, queue ));

        magma_tally3_zmfree( &hA, queue );
        magma_tally3_zmfree( A, queue );
        CHECK( magma_tally3_zmconvert( CSRCOOA, &hA, Magma_tally3_CSR, A_storage, queue ));
...
cleanup:
    ...
    magma_tally3_zmfree( &A_copy, queue );
    magma_tally3_zmfree( &B, queue );
    magma_tally3_zmfree( &hA, queue );
    magma_tally3_zmfree( &CSRCOOA, queue );
    return info;
}
