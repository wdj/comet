...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_Ymagma_minproduct,  double, sizeY  );
            ...
            /* =====================================================================
               ...
               =================================================================== */
            magma_minproduct_dsetmatrix( M, N*batchCount, h_A, lda, d_A, ldda );
            magma_minproduct_dsetvector( Xm*batchCount, h_X, incx, d_X, incx );
            magma_minproduct_dsetvector( Ym*batchCount, h_Y, incy, d_Y, incy );
            
            dset_pointer(A_array, d_A, ldda, 0, 0, ldda*N, batchCount, magma_minproduct_stream);
            dset_pointer(X_array, d_X, 1, 0, 0, incx*Xm, batchCount, magma_minproduct_stream);
            dset_pointer(Y_array, d_Y, 1, 0, 0, incy*Ym, batchCount, magma_minproduct_stream);

            magma_minproduct_time = magma_minproduct_sync_wtime( NULL );
            magma_minproductblas_dgemv_batched(opts.transA, M, N,
                             alpha, A_array, ldda,
                                    X_array, incx,
            ...
            magma_minproduct_dgetvector( Ym*batchCount, d_Y, incy, h_Ymagma_minproduct, incy );
            ...
            if ( opts.lapack ) {
                ...
                for(int s=0; s<batchCount; s++)
                {
                    ...
                    blasf77_daxpy( &Ym, &c_neg_one, h_Y + s*Ym, &ione, h_Ymagma_minproduct + s*Ym, &ione );
                    magma_minproduct_err = lapackf77_dlange( "M", &M, &ione, h_Ymagma_minproduct + s*Ym, &incy, work ) / Ynorm; 
