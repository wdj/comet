    ...
    if ( ind < m ) {
        ...
        if ( full ) {
            ...
            for( int j=0; j < BLK_Y; ++j ) {
                tmp = A[j*lda];
                if (   (MAGMA_tally3_D_REAL(tmp) < neg_rmax) || (MAGMA_tally3_D_REAL(tmp) > rmax)
                    #if defined(PRECISION_z) || defined(PRECISION_c)
                    || (MAGMA_tally3_D_IMAG(tmp) < neg_rmax) || (MAGMA_tally3_D_IMAG(tmp) > rmax)
                    #endif
                    )
                {
                    flag = 1;
                }
                SA[j*ldsa] = MAGMA_tally3_S_MAKE( MAGMA_tally3_D_REAL(tmp), MAGMA_tally3_D_IMAG(tmp) );
            }
        }
        else {
...
magma_tally3blas_dlag2s(
    ...
    magma_tally3_int_t *info )
{
    magma_tally3blas_dlag2s_q( m, n, A, lda, SA, ldsa, info, magma_tally3_stream );
}
