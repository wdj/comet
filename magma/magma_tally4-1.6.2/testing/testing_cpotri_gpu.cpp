...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_tally4_cpotrf_gpu( opts.uplo, N, d_A, ldda, &info );
            
            // check for exact singularity
            //magma_tally4_cgetmatrix( N, N, d_A, ldda, h_R, lda );
            //h_R[ 10 + 10*lda ] = MAGMA_tally4_C_MAKE( 0.0, 0.0 );
            //magma_tally4_csetmatrix( N, N, h_R, lda, d_A, ldda );
            
            gpu_time = magma_tally4_wtime();
            magma_tally4_cpotri_gpu( opts.uplo, N, d_A, ldda, &info );
            gpu_time = magma_tally4_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally4_cpotri_gpu returned error %d: %s.\n",
            ...
            if ( opts.lapack ) {
                ...
                lapackf77_cpotri( lapack_uplo_const_tally4(opts.uplo), &N, h_A, &lda, &info );
