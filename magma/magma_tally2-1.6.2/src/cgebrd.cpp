...
#define dA(i, j) (dA + (j)*ldda + (i))
    ...
    magma_tally2_int_t ncol, nrow, jmax, nb, ldda;

    magma_tally2_int_t i, j, nx;
    magma_tally2_int_t iinfo;

    magma_tally2_int_t minmn;
    magma_tally2_int_t ldwrkx, ldwrky, lwkopt;
    magma_tally2_int_t lquery;

    nb   = magma_tally2_get_cgebrd_nb(n);
    ldda = m;
    ...
    *info = 0;
    ...
    if (MAGMA_tally2_SUCCESS != magma_tally2_cmalloc( &dA, n*ldda + (m + n)*nb )) {
    ...
    for (i=0; i < (minmn - nx); i += nb) {
        ...
        if ( i > 0 ) {
            magma_tally2_cgetmatrix( nrow, nb, dA(i, i), ldda, A( i, i), lda );
            magma_tally2_cgetmatrix( nb, ncol - nb,
                              dA(i, i+nb), ldda,
                              A( i, i+nb), lda );
        }

        magma_tally2_clabrd_gpu(nrow, ncol, nb,
        ...
        magma_tally2_csetmatrix( nrow, nb, work  + nb, ldwrkx, dwork + nb, ldwrkx );
        magma_tally2_csetmatrix( ncol, nb,
        ...
        if (m >= n) {
            jmax = i + nb;
            for (j = i; j < jmax; ++j) {
                *A(j, j  ) = MAGMA_tally2_C_MAKE( d[j], 0. );
                *A(j, j+1) = MAGMA_tally2_C_MAKE( e[j], 0. );
            }
        } else {
            jmax = i + nb;
            for (j = i; j < jmax; ++j) {
    ...
    if ( 0 < minmn - nx ) {
        magma_tally2_cgetmatrix( nrow, ncol, dA(i, i), ldda, A(i, i), lda );
    }
