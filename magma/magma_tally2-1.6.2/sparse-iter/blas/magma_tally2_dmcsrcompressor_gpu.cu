/*
    -- MAGMA_tally2 (version 1.6.2) --
       ...
       @generated from magma_tally2_zmcsrcompressor_gpu.cu normal z -> d, Sun May  3 11:22:58 2015
       @author Hartwig Anzt
...
__global__ void
magma_tally2_dmcsrgpu_kernel1( int num_rows,
                 double *A_val,
    ...
    if(row<num_rows){
        double zero = MAGMA_tally2_D_ZERO;
...
__global__ void
magma_tally2_dmcsrgpu_kernel2( int num_rows,
                 magma_tally2_index_t *B_rowptr,
                 magma_tally2_index_t *A_rowptr ){
...
__global__ void
magma_tally2_dmcsrgpu_kernel3( int num_rows,
                 double *B_val,
                 magma_tally2_index_t *B_rowptr,
                 magma_tally2_index_t *B_colind,
    ...
    magma_tally2_queue_t queue )
{
    ...
    magma_tally2_d_matrix B={Magma_tally2_CSR}, B2={Magma_tally2_CSR};
    magma_tally2_d_matrix dA={Magma_tally2_CSR}, CSRA={Magma_tally2_CSR};
    ...
    if ( A->memory_location == Magma_tally2_DEV && A->storage_type == Magma_tally2_CSR ) {
        ...
        magma_tally2_dmcsrgpu_kernel1<<< grid1, BLOCK_SIZE1, 0, queue >>>
                ( A->num_rows, A->dval, A->drow, A->dcol, B.dval, B.drow, B.dcol );

        // correct the row pointer
        dim3 grid2( 1, 1, 1);
        magma_tally2_dmcsrgpu_kernel2<<< grid2, BLOCK_SIZE2, 0, queue >>>
        ...
        magma_tally2_dmcsrgpu_kernel3<<< grid1, BLOCK_SIZE1, 0, queue >>>
        ...
        CHECK( magma_tally2_dmconvert( *A, &CSRA, A->storage_type, Magma_tally2_CSR, queue ));
        CHECK( magma_tally2_dmtransfer( *A, &dA, A->memory_location, Magma_tally2_DEV, queue ));

        CHECK( magma_tally2_dmcsrcompressor_gpu( &dA, queue ));

        magma_tally2_dmfree( &dA, queue );
        magma_tally2_dmfree( A, queue );
        CHECK( magma_tally2_dmtransfer( dA, &CSRA, Magma_tally2_DEV, A_location, queue ));
        CHECK( magma_tally2_dmconvert( CSRA, A, Magma_tally2_CSR, A_storage, queue ));
        magma_tally2_dmfree( &dA, queue );
