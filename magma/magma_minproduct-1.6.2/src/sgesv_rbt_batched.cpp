/*
    -- MAGMA_minproduct (version 1.6.1) --
       ...
       @generated from zgesv_rbt_batched.cpp normal z -> s, Fri Jan 30 19:00:19 2015
*/
                  ...
                  magma_minproduct_int_t batchCount, magma_minproduct_queue_t queue)
{
    ...
    float *hu, *hv;
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_smalloc_cpu( &hu, 2*n )) {
        info = MAGMA_minproduct_ERR_HOST_ALLOC;
        return info;
    }

    if (MAGMA_minproduct_SUCCESS != magma_minproduct_smalloc_cpu( &hv, 2*n )) {
        info = MAGMA_minproduct_ERR_HOST_ALLOC;
        return info;
    }
    ...
    info = magma_minproduct_sgerbt_batched(Magma_minproductTrue, n, nrhs, dA_array, n, dB_array, n, hu, hv, &info, batchCount, queue);
    if (info != MAGMA_minproduct_SUCCESS)  {
        return info;
    }


    info = magma_minproduct_sgetrf_nopiv_batched( n, n, dA_array, ldda, info_array, batchCount, queue);
    if ( (info != MAGMA_minproduct_SUCCESS) ){
        return info;
    }
    ...
    info = magma_minproduct_sgetrs_nopiv_batched( Magma_minproductNoTrans, n, nrhs, dA_array, ldda, dB_array, lddb, info_array, batchCount, queue );


    /* The solution of A.x = b is Vy computed on the GPU */
    float *dv;

    if (MAGMA_minproduct_SUCCESS != magma_minproduct_smalloc( &dv, 2*n )) {
        info = MAGMA_minproduct_ERR_DEVICE_ALLOC;
        return info;
    }

    magma_minproduct_ssetvector(2*n, hv, 1, dv, 1);

    for(int i = 0; i < nrhs; i++)
        magma_minproductblas_sprbt_mv_batched(n, dv, dB_array+(i), batchCount, queue);
