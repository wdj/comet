...
extern "C" void zcheck_eig_(char *JOBZ, int  *MATYPE, int  *N, int  *NB,
                       magma_minproductDoubleComplex* A, int  *LDA, double *AD, double *AE, double *D1, double *EIG,
                    magma_minproductDoubleComplex *Z, int  *LDZ, magma_minproductDoubleComplex *WORK, double *RWORK, double *RESU);
#endif
...
int main( int argc, char** argv)
{
    ...
    magma_minproductDoubleComplex *h_A, *h_R, *h_work, *dT1;
    ...
    NB = opts.nb;
    if (NB < 1)
        NB  = 64; //64; //magma_minproduct_get_zhetrd_he2hb_nb(N);

    // what is NE ?
    if (NE < 1)
        NE  = 64; //N;  //magma_minproduct_get_zhetrd_he2hb_nb(N);  // N not yet initialized
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( tau,    magma_minproductDoubleComplex, N-1    );
            
            TESTING_MALLOC_PIN( h_R,    magma_minproductDoubleComplex, lda*N  );
            TESTING_MALLOC_PIN( h_work, magma_minproductDoubleComplex, lwork0 );
            TESTING_MALLOC_PIN( D, double, N );
            TESTING_MALLOC_PIN( E, double, N );
            
            //TESTING_MALLOC_DEV( dT1, magma_minproductDoubleComplex, (2*min(N,N)+(N+31)/32*32)*NB );
            TESTING_MALLOC_DEV( dT1, magma_minproductDoubleComplex, (N*NB) );
            ...
            /*
            magma_minproduct_zhetrd_he2hb( opts.uplo, N, NB, h_R, lda, tau, h_work, lwork0, dT1, THREADS, &info);
            tband = magma_minproduct_wtime - gpu_time();
            printf("  Finish BAND  N %d  NB %d  ngpu %d timing= %f\n", N, NB, ngpu, tband);
            magma_minproduct_zhetrd_bhe2trc_v5(THREADS, WANTZ, opts.uplo, NE, N, NB, h_R, lda, D, E, dT1, ldt);
            */
    
            /*
            magma_minproduct_zhetrd_he2hb( opts.uplo, N, NB, h_R, lda, tau, h_work, lwork, dT1, THREADS, &info);
            tband = magma_minproduct_wtime - gpu_time();
            printf("  Finish BAND  N %d  NB %d  ngpu %d timing= %f\n", N, NB, ngpu, tband);
            magma_minproduct_zhetrd_bhe2trc(THREADS, WANTZ, opts.uplo, NE, N, NB, h_R, lda, D, E, dT1, ldt);
            */
            ...
            magma_minproductDoubleComplex *hh_work;
            ...
            #endif
            liwork = 3 + 5*N;
            //nb = magma_minproduct_get_zhetrd_nb(N);
            TESTING_MALLOC_PIN( hh_work, magma_minproductDoubleComplex, lwork  );
            ...
            if (ngpu == 1) {
                ...
                magma_minproduct_zheevdx_2stage_m(ngpu, opts.jobz, range, opts.uplo, N,
                                h_R, lda,
                                vl, vu, il, iu,
                                &m1, D,
                                hh_work, lwork,
            ...
            */
            ...
            if ( opts.check ) {
                ...
                int    lwork2 = 256*N;
                magma_minproductDoubleComplex *work2, *AINIT;
                double *rwork2, *D2;
                // TODO free this memory !
                magma_minproduct_zmalloc_cpu( &work2, lwork2 );
                magma_minproduct_dmalloc_cpu( &rwork2, N );
                magma_minproduct_dmalloc_cpu( &D2, N );
                magma_minproduct_zmalloc_cpu( &AINIT, N*lda );
                memcpy(AINIT, h_A, N*lda*sizeof(magma_minproductDoubleComplex));
                ...
                */
                ...
                //magma_minproductDoubleComplex mydz=0.0, mydo=1.0;
                //magma_minproductDoubleComplex *Z;
                // magma_minproduct_zmalloc_cpu( &Z, N*lda );
                ...
                magma_minproductDoubleComplex *WORKAJETER;
                double *RWORKAJETER, *RESU;
                // TODO free this memory !
                magma_minproduct_zmalloc_cpu( &WORKAJETER, (2* N * N + N)  );
                magma_minproduct_dmalloc_cpu( &RWORKAJETER, N  );
                magma_minproduct_dmalloc_cpu( &RESU, 10 );
                int MATYPE;
