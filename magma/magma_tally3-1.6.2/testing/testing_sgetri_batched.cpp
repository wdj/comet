/*
   ...
   @generated from testing_zgetri_batched.cpp normal z -> s, Fri Jan 30 19:00:26 2015
 */
// includes, system
#include <stdlib.h>
#include <stdio.h>
...
int main( int argc, char** argv)
{
    ...
    magma_tally3_int_t *dinfo_array = NULL;

    magma_tally3_int_t     *ipiv;
    magma_tally3_int_t     *d_ipiv, *d_info;
    magma_tally3_int_t M, N, n2, lda, ldda, min_mn, info, info1, info2;
    ...
    magma_tally3_int_t batchCount = opts.batchcount ;
    ...
    for( int i = 0; i < opts.ntest; ++i ) {
      ...
      for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            n2     = lda*N * batchCount;
            ldda   = ((M+31)/32)*32;
            //gflops = (FLOPS_SGETRF( M, N ) + FLOPS_SGETRI( min(M,N) ))/ 1e9 * batchCount; // This is the correct flops but since this getri_batched is based on 2 trsm = getrs and to know the real flops I am using the getrs one
            gflops = (FLOPS_SGETRF( M, N ) + FLOPS_SGETRS( min(M,N), min(M,N) ))/ 1e9 * batchCount;
            ...
            TESTING_MALLOC_DEV(  d_ipiv,  magma_tally3_int_t, min_mn * batchCount);
            TESTING_MALLOC_DEV(  d_info,  magma_tally3_int_t, batchCount);
            ...
            magma_tally3_malloc((void**)&dinfo_array, batchCount * sizeof(magma_tally3_int_t));
            ...
            magma_tally3_ssetmatrix( M, columns, h_R, lda, d_A, ldda );

            /* ====================================================================
               Performs operation using MAGMA_tally3
               =================================================================== */
            sset_pointer(dA_array, d_A, ldda, 0, 0, ldda * N, batchCount, queue);
            sset_pointer(dinvA_array, d_invA, ldda, 0, 0, ldda * N, batchCount, queue);
            set_ipointer(dipiv_array, d_ipiv, 1, 0, 0, min(M,N), batchCount, queue);

            gpu_time = magma_tally3_sync_wtime(0);
            info1 = magma_tally3_sgetrf_batched( M, N, dA_array, ldda, dipiv_array, dinfo_array, batchCount, queue);
            info2 = magma_tally3_sgetri_outofplace_batched( min(M,N), dA_array, ldda, dipiv_array, dinvA_array, ldda, dinfo_array, batchCount, queue);
            ...
            {
                if(cpu_info[i] != 0 ){
                    printf("magma_tally3_sgetrf_batched matrix %d returned error %d\n", i, (int)cpu_info[i] );
                }
            }
            if (info1 != 0) printf("magma_tally3_sgetrf_batched returned argument error %d: %s.\n", (int) info1, magma_tally3_strerror( info1 ));
            if (info2 != 0) printf("magma_tally3_sgetri_batched returned argument error %d: %s.\n", (int) info2, magma_tally3_strerror( info2 ));
            /* =====================================================================
            ...
            if ( opts.lapack ) {
                ...
                lwork = magma_tally3_int_t( MAGMA_tally3_S_REAL( tmp ));
                TESTING_MALLOC_CPU( work,  float, lwork  );
                lapackf77_slacpy( Magma_tally3UpperLowerStr, &M, &columns, h_R, &lda, h_A, &lda );
                ...
                for(int i=0; i<batchCount; i++)
                {
                    ...
                    if (info != 0)
                        printf("lapackf77_sgetrf returned error %d: %s.\n",
                           (int) info, magma_tally3_strerror( info ));
                    lapackf77_sgetri(&N, h_A + i * lda*N, &lda, ipiv + i * min_mn, work, &lwork, &info );
                    if (info != 0)
                        printf("lapackf77_sgetri returned error %d: %s.\n",
                           (int) info, magma_tally3_strerror( info ));
                }
            ...
            float err = 0.0;
            if ( opts.check ) {
                magma_tally3_getvector( min_mn * batchCount, sizeof(magma_tally3_int_t), d_ipiv, 1, ipiv, 1 );
                magma_tally3_sgetmatrix( min(M,N), N*batchCount, d_invA, ldda, h_R, lda );
                int stop=0;
                n2     = lda*N;
                for(int i=0; i < batchCount; i++)
                {
                    ...
                    error = lapackf77_slange( "f", &N, &N, h_A+ i * lda*N, &lda, rwork );
                    blasf77_saxpy( &n2, &c_neg_one, h_A+ i * lda*N, &ione, h_R+ i * lda*N, &ione );
                    error = lapackf77_slange( "f", &N, &N, h_R+ i * lda*N, &lda, rwork ) / (N*error);
