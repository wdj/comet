...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lhwork2 = (magma_tally2_int_t) MAGMA_tally2_S_REAL( tmp[0] );
            
            lhwork = -1;
            lapackf77_sormqr( Magma_tally2LeftStr, Magma_tally2TransStr,
                              &M, &nrhs, &min_mn, NULL, &lda, NULL,
                              NULL, &ldb, tmp, &lhwork, &info);
            lhwork = (magma_tally2_int_t) MAGMA_tally2_S_REAL( tmp[0] );
            ...
            lapackf77_slacpy( Magma_tally2UpperLowerStr, &M, &N, h_A, &lda, h_A2, &lda );
            
            // make random RHS
            size = M*nrhs;
            lapackf77_slarnv( &ione, ISEED, &size, h_B );
            lapackf77_slacpy( Magma_tally2UpperLowerStr, &M, &nrhs, h_B, &ldb, h_R, &ldb );
            
            // make consistent RHS
            //size = N*nrhs;
            //lapackf77_slarnv( &ione, ISEED, &size, h_X );
            //blasf77_sgemm( Magma_tally2NoTransStr, Magma_tally2NoTransStr, &M, &nrhs, &N,
            //               &c_one,  h_A, &lda,
            //                        h_X, &ldb,
            //               &c_zero, h_B, &ldb );
            //lapackf77_slacpy( Magma_tally2UpperLowerStr, &M, &nrhs, h_B, &ldb, h_R, &ldb );
            ...
            magma_tally2_ssetmatrix( M, nrhs, h_B, ldb, d_B, lddb );
            
            gpu_time = magma_tally2_wtime();
            magma_tally2_sgels3_gpu( Magma_tally2NoTrans, M, N, nrhs, d_A, ldda,
                              d_B, lddb, h_work, lworkgpu, &info);
            gpu_time = magma_tally2_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally2_sgels3_gpu returned error %d: %s.\n",
                       (int) info, magma_tally2_strerror( info ));
            
            // Get the solution in h_X
            magma_tally2_sgetmatrix( N, nrhs, d_B, lddb, h_X, ldb );
            ...
            /* =====================================================================
               ...
               =================================================================== */
            lapackf77_slacpy( Magma_tally2UpperLowerStr, &M, &nrhs, h_B, &ldb, h_X, &ldb );
            
            cpu_time = magma_tally2_wtime();
            lapackf77_sgels( Magma_tally2NoTransStr, &M, &N, &nrhs,
