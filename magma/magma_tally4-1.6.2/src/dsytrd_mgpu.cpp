...
/**
    ...
    lwork   INTEGER
            ...
            optimal blocksize given by magma_tally4_get_dsytrd_nb().
    \n
...
magma_tally4_dsytrd_mgpu(
    ...
    magma_tally4_int_t nqueue, magma_tally4_uplo_t uplo, magma_tally4_int_t n,
...
#define dW(id, i, j) (dW[(id)] + (j)*ldda + (i))
    ...
    magma_tally4_int_t i, ii, iii, j, dev, i_n;
    magma_tally4_int_t iinfo;
    magma_tally4_int_t ldwork, lddw, lwkopt, ldwork2, lhwork;
    magma_tally4_int_t lquery;
    
    // set pointers to NULL so it is safe to goto CLEANUP if any malloc fails.
    magma_tally4_queue_t queues[Magma_tally4MaxGPUs][10] = { { NULL, NULL } };
    magma_tally4_queue_t queues0[Magma_tally4MaxGPUs]    = { NULL };
    double *hwork = NULL;
    magma_tally4Double_ptr dwork2[Magma_tally4MaxGPUs] = { NULL };
    magma_tally4Double_ptr dA[Magma_tally4MaxGPUs]     = { NULL };
    magma_tally4Double_ptr dW[Magma_tally4MaxGPUs]     = { NULL };
    ...
    *info = 0;
    ...
    #ifdef PROFILE_SY2RK
    double times[11] = { 0 };
    magma_tally4_event_t start, stop;
    float etime;
    magma_tally4_setdevice( 0 );
    magma_tally4_event_create( &start );
    magma_tally4_event_create( &stop  );
    #endif
    ...
    for( dev=0; dev < ngpu; dev++ ) {
        ...
        if ( MAGMA_tally4_SUCCESS != magma_tally4_dmalloc( &dA[dev],     nlocal*ldda + 3*lddw*nb ) ||
             MAGMA_tally4_SUCCESS != magma_tally4_dmalloc( &dwork2[dev], ldwork2 ) ) {
        ...
        for( kk=0; kk < nqueue; kk++ ) {
            magma_tally4_queue_create( &queues[dev][kk] );
        }
        queues0[dev] = queues[dev][0];
    }
    
    lhwork = nqueue*ngpu*n;
    if ( MAGMA_tally4_SUCCESS != magma_tally4_dmalloc_pinned( &hwork, lhwork ) ) {
    ...
    if (upper) {
        ...
        if (1 <= n-nx) {
            magma_tally4_dhtodhe( ngpu, uplo, n, nb, A, lda, dA, ldda, queues, &iinfo );
        }
        ...
        for (i = nb*((n-1)/nb); i >= nx; i -= nb) {
            ...
            magma_tally4_dlatrd_mgpu( ngpu, uplo, i+ib, ib, nb,
                               A(0, 0), lda, e, tau,
                               work, ldwork,
                               dA, ldda, 0,
                               dW, i+ib,
            ...
            magma_tally4_dsyr2k_mgpu( ngpu, Magma_tally4Upper, Magma_tally4NoTrans, nb, i, ib,
                               c_neg_one, dW, i+ib, 0,
                               d_one,     dA, ldda, 0,
                               nqueue, queues);
            ...
            if (i-nb >= nx ) {
                ...
                magma_tally4_dgetmatrix_async( (i-nb)+ib2, ib2,
                                        dA(dev, 0, ii), ldda,
                                        A(0, i-nb),     lda,
                                        queues[dev][0] );
            }
            ...
            for (j = i; j < i+ib; ++j) {
                if ( j > 0 ) {
                    *A(j-1,j) = MAGMA_tally4_D_MAKE( e[j - 1], 0 );
                }
                d[j] = MAGMA_tally4_D_REAL( *A(j, j) );
            }
        } /* end of for i=... */
        ...
        if ( nx > 0 ) {
            if (1 <= n-nx) { /* else A is already on CPU */
                for (i=0; i < nx; i += nb) {
                    ...
                    magma_tally4_setdevice( dev );
                    magma_tally4_dgetmatrix_async( nx, ib,
                                            dA(dev, 0, ii), ldda,
                                            A(0, i),        lda,
                                            queues[dev][0] );
                }
        ...
        for (i = 0; i < n-nx; i += nb) {
            ...
            if (i != 0) {
                ...
                magma_tally4_dgetmatrix_async( n-i, ib,
                                        dA(dev, i, ii), ldda,
                                        A(i,i),         lda,
                                        queues[dev][0] );
                trace_gpu_end( dev, 0 );
            ...
            magma_tally4_dlatrd_mgpu( ngpu, uplo, n-i, ib, nb,
                               A(i, i), lda, &e[i], &tau[i],
                               work, ldwork,
                               dA, ldda, i,
                               dW, n-i,
            ...
            magma_tally4_event_record( start, 0 );
            #endif
            
            magma_tally4_dsyr2k_mgpu( ngpu, Magma_tally4Lower, Magma_tally4NoTrans, nb, n-i-ib, ib,
                               c_neg_one, dW, n-i, ib,
                               d_one, dA, ldda, i+ib, nqueue, queues);
            
            #ifdef PROFILE_SY2RK
            magma_tally4_setdevice( 0 );
            magma_tally4_event_record( stop, 0 );
            #endif
            ...
            for (j = i; j < i+ib; ++j) {
                if ( j+1 < n ) {
                    *A(j+1,j) = MAGMA_tally4_D_MAKE( e[j], 0 );
                }
                d[j] = MAGMA_tally4_D_REAL( *A(j, j) );
            }
        } /* for i=... */
        ...
        if ( i < n ) {
            ...
            if ( i > 0 ) {
                for (; i < n; i += nb) {
                    ...
                    magma_tally4_setdevice( dev );
                    magma_tally4_dgetmatrix_async( i_n, ib,
                                            dA(dev, iii, ii), ldda,
                                            A(iii, i),        lda,
                                            queues[dev][0] );
                }
    ...
    for( dev=0; dev < ngpu; dev++ ) {
        magma_tally4_setdevice( dev );
        for( kk=0; kk < nqueue; kk++ ) {
            magma_tally4_queue_sync( queues[dev][kk] );
        }
    }
    ...
    magma_tally4_event_destroy( start );
    magma_tally4_event_destroy( stop  );
    #endif
...
CLEANUP:
    for( dev=0; dev < ngpu; dev++ ) {
        magma_tally4_setdevice( dev );
        for( kk=0; kk < nqueue; kk++ ) {
            magma_tally4_queue_destroy( queues[dev][kk] );
        }
        magma_tally4_free( dA[dev] );
        magma_tally4_free( dwork2[dev] );
    }
...
#define dC(id, i, j)  (dC[(id)] + (j)*lddc + (i))
    ...
    for( i=0; i < n; i += nb ) {
        ...
        magma_tally4_dsyr2k( uplo, trans, ib, k,
    ...
    if (uplo == Magma_tally4Upper) {
        for( i=nb; i < n; i += nb ) {
            ...
            ib = min(nb, n-i);
            ii = nb*((i+c_offset)/(nb*ngpu));
            magma_tally4_dgemm( Magma_tally4NoTrans, Magma_tally4ConjTrans, i, ib, k,
        ...
        for( i=0; i < n-nb; i += nb ) {
            ...
            magma_tally4_dgemm( Magma_tally4NoTrans, Magma_tally4ConjTrans, n1, ib, k,
