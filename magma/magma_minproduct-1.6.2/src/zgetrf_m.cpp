...
#define dPT(d,i,j) (dPT[d] + (i)*nb*nb + (j)*nb*maxm)
    ...
    magma_minproduct_timer_t time=0, time_total=0, time_alloc=0, time_set=0, time_get=0, time_comp=0;
    ...
    magma_minproductDoubleComplex *dAT[Magma_minproductMaxGPUs], *dA[Magma_minproductMaxGPUs], *dPT[Magma_minproductMaxGPUs];
    magma_minproduct_int_t        iinfo = 0, nb, nbi, maxm, n_local[Magma_minproductMaxGPUs], ldn_local;
    magma_minproduct_int_t        N, M, NB, NBk, I, d, ngpu0 = ngpu;
    magma_minproduct_int_t        ii, jj, h, offset, ib, rows;
    ...
    *info = 0;
    ...
    /* initialize nb */
    nb = magma_minproduct_get_zgetrf_nb(m);
    maxm = ((m  + 31)/32)*32;

    /* figure out NB */
    size_t freeMem, totalMem;
    cudaMemGetInfo( &freeMem, &totalMem );
    freeMem /= sizeof(magma_minproductDoubleComplex);
    ...
    if ( (nb <= 1) || (nb >= min(m,n)) ) {
        ...
        for( d=0; d < ngpu; d++ ) {
            ...
            if (MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc( &dA[d], (ldn_local+h*nb)*maxm )) {
            ...
            magma_minproduct_queue_create( &stream[d][0] );
            magma_minproduct_queue_create( &stream[d][1] );
            magma_minproduct_event_create( &event[d][0] );
            magma_minproduct_event_create( &event[d][1] );
        }
        ...
        for( I=0; I < n; I += NB ) {
            ...
            timer_start( time );
            magma_minproductblas_zsetmatrix_transpose_mgpu(ngpu, stream, A(0,I), lda,
                                                dAT, ldn_local, dA, maxm, M, N, nb);
            for( d=0; d < ngpu; d++ ) {
                magma_minproduct_setdevice(d);
                magma_minproduct_queue_sync( stream[d][0] );
                magma_minproduct_queue_sync( stream[d][1] );
            ...
            for( offset = 0; offset < min(m,I); offset += NB ) {
                ...
                for( d=0; d < ngpu; d++ ) {
                    magma_minproduct_setdevice(d);
                    nbi  = min( nb, NBk );
                    magma_minproduct_zsetmatrix_async( (M-offset), nbi,
                                            A(offset,offset), lda,
                                            dA[d],            (maxm-offset), stream[d][0] );
                    
                    /* make sure the previous update finished */
                    magma_minproductblasSetKernelStream(stream[d][0]);
                    //magma_minproduct_queue_sync( stream[d][1] );
                    magma_minproduct_queue_wait_event( stream[d][0], event[d][0] );
                    
                    /* transpose */
                    magma_minproductblas_ztranspose( M-offset, nbi, dA[d], maxm-offset, dPT(d,0,0), nb );
                }
                
                /* applying the pivot from the previous big-panel */
                for( d=0; d < ngpu; d++ ) {
                    magma_minproduct_setdevice(d);
                    magma_minproductblas_zlaswp_q( ldn_local, dAT(d,0,0), ldn_local, offset+1, offset+NBk, ipiv, 1, stream[d][1] );
                }
                ...
                for( jj=0, ib=offset/nb; jj < NBk; jj += nb, ib++ ) {
                    ...
                    for( d=0; d < ngpu; d++ ) {
                        ...
                        if ( jj+nb < NBk ) {
                            magma_minproduct_zsetmatrix_async( (M-ii-nb), min(nb,NBk-jj-nb),
                                                    A(ii+nb,ii+nb), lda,
                            ...
                            magma_minproductblasSetKernelStream(stream[d][0]);
                            //magma_minproduct_queue_sync( stream[d][1] );
                            magma_minproduct_queue_wait_event( stream[d][0], event[d][(1+jj/nb)%2] );
                            
                            /* transpose next column */
                            magma_minproductblas_ztranspose( M-ii-nb, nb, dA[d], rows-nb, dPT(d,0,(1+jj/nb)%2), nb );
                        }
                        
                        /* update with the block column */
                        magma_minproductblasSetKernelStream(stream[d][1]);
                        magma_minproduct_ztrsm( Magma_minproductRight, Magma_minproductUpper, Magma_minproductNoTrans, Magma_minproductUnit,
                        ...
                        magma_minproduct_event_record( event[d][(jj/nb)%2], stream[d][1] );
            ...
            /* calling magma_minproduct-gpu interface to panel-factorize the big panel */
            if ( M > I ) {
                magma_minproduct_zgetrf2_mgpu(ngpu, M-I, N, nb, I, dAT, ldn_local, ipiv+I, dA, A(0,I), lda,
                                   stream, &iinfo);
                if ( iinfo < 0 ) {
                    *info = iinfo;
                    break;
            ...
            timer_start( time );
            magma_minproductblas_zgetmatrix_transpose_mgpu(ngpu, stream, dAT, ldn_local, A(0,I), lda, dA, maxm, M, N, nb);
            for( d=0; d < ngpu; d++ ) {
                magma_minproduct_setdevice(d);
                magma_minproduct_queue_sync( stream[d][0] );
                magma_minproduct_queue_sync( stream[d][1] );
        ...
        for( d=0; d < ngpu0; d++ ) {
            ...
            magma_minproduct_event_destroy( event[d][0] );
            magma_minproduct_event_destroy( event[d][1] );
            magma_minproduct_queue_destroy( stream[d][0] );
            magma_minproduct_queue_destroy( stream[d][1] );
        }
        magma_minproduct_setdevice( orig_dev );
        magma_minproductblasSetKernelStream( orig_stream );
    }
    if ( *info >= 0 )
        magma_minproduct_zgetrf_piv(m, n, NB, A, lda, ipiv, info);
    return *info;
} /* magma_minproduct_zgetrf_m */
...
magma_minproduct_zgetrf_piv(
    ...
    magma_minproduct_int_t *info)
{
    magma_minproduct_int_t I, k1, k2, incx, minmn;
    *info = 0;
    if (m < 0)
        *info = -1;
    else if (n < 0)
...
// ----------------------------------------------------------------------
extern "C" magma_minproduct_int_t
magma_minproduct_zgetrf2_piv(
    magma_minproduct_int_t m, magma_minproduct_int_t n, magma_minproduct_int_t start, magma_minproduct_int_t end,
    magma_minproductDoubleComplex *A, magma_minproduct_int_t lda, magma_minproduct_int_t *ipiv,
    magma_minproduct_int_t *info)
{
    magma_minproduct_int_t I, k1, k2, nb, incx, minmn;
