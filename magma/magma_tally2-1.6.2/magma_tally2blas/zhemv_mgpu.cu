    ...
    int block_offset )
{
    ...
    magma_tally2DoubleComplex psum, psum_t;
    magma_tally2DoubleComplex total = MAGMA_tally2_Z_ZERO;
    ...
    __shared__ magma_tally2DoubleComplex sA [quarter_NB_X][NB_X + 2];  // TODO +3 used in zhemv (single GPU); why?
    __shared__ magma_tally2DoubleComplex sx_blk[NB_X];  // for x[ blk ]
    __shared__ magma_tally2DoubleComplex sx_jj [NB_X];  // for x[ jj ], which cycles over all blocks left of diag

    magma_tally2DoubleComplex rA[4];
    magma_tally2DoubleComplex psums_t[4];
...
/**************************************************************
    ...
    Note beta*y is not included here; see magma_tally2blas_zhemv_mgpu_sync.
    ...
    int block_offset)
{
    ...
    if ( ind >= block_offset && ind < n+block_offset ) {
        magma_tally2DoubleComplex Ax = MAGMA_tally2_Z_ZERO;
        ...
        y[ind * incy] = alpha*Ax;  // see magma_tally2blas_zhemv_sync for beta*y
    }
}
...
/**
    Purpose
    -------
    magma_tally2blas_zhemv_mgpu performs the matrix-vector operation:
...
magma_tally2blas_zhemv_mgpu(
    ...
    magma_tally2DoubleComplex alpha,
    magma_tally2DoubleComplex_const_ptr const d_lA[], magma_tally2_int_t ldda,
    magma_tally2_int_t offset,
    magma_tally2DoubleComplex const *x,       magma_tally2_int_t incx,
    magma_tally2DoubleComplex beta,                                         // unused, see magma_tally2blas_zhemv_mgpu_sync
    magma_tally2DoubleComplex       *y,       magma_tally2_int_t incy,             // unused
    ...
    magma_tally2_queue_t queues[] )
{
    ...
    for(dev=0; dev < ngpu; dev++) {
        ...
        magma_tally2DoubleComplex const *dA_dev    = d_lA[dev] + offset_block_id*NB_X + num_blocks_skipped*NB_X*ldda;
        ...
        magma_tally2DoubleComplex       *dx_dev    = dwork[dev];
        magma_tally2DoubleComplex       *dwork_dev = dwork[dev] + ldda;
        ...
        // copy x to each GPU
        magma_tally2_zsetvector_async( n, x, incx, dx_dev + block_offset, 1, queues[dev] );
    ...
    for(dev=0; dev < ngpu; dev++) {
        ...
        magma_tally2_setdevice( dev );
        magma_tally2DoubleComplex       *dx_dev    = dwork[dev];
        magma_tally2_zgetvector_async( n, dx_dev + block_offset, 1, &hwork[dev*n], 1, queues[dev] );
    }
    
    // see magma_tally2blas_zhemv_mgpu_sync for final row sums
...
/**
    ...
    For convenience, the parameters are identical to magma_tally2blas_zhemv_mgpu
    (though some are unused here).
    
    @see magma_tally2blas_zhemv_mgpu
    @ingroup magma_tally2_zblas2
    ********************************************************************/
extern "C" magma_tally2_int_t
magma_tally2blas_zhemv_mgpu_sync(
    magma_tally2_uplo_t uplo,                                               // unused, see magma_tally2blas_zhemv_mgpu
    magma_tally2_int_t n,
    magma_tally2DoubleComplex alpha,                                        // unused
    magma_tally2DoubleComplex_const_ptr const d_lA[], magma_tally2_int_t ldda,
    magma_tally2_int_t offset,                                              // unused
    magma_tally2DoubleComplex const *x,       magma_tally2_int_t incx,             // unused
    magma_tally2DoubleComplex beta,
    magma_tally2DoubleComplex       *y,       magma_tally2_int_t incy,             // unused
    magma_tally2DoubleComplex       *hwork,   magma_tally2_int_t lhwork,
    magma_tally2DoubleComplex_ptr    dwork[], magma_tally2_int_t ldwork,           // unused
    magma_tally2_int_t ngpu,
