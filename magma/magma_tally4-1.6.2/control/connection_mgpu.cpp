...
{
    magma_tally4_int_t *deviceid = (magma_tally4_int_t *) malloc(ngpu*sizeof(magma_tally4_int_t));
    memset(deviceid, 0, ngpu*sizeof(magma_tally4_int_t));
    ...
    magma_tally4_int_t cmplxnb = 0;
    magma_tally4_int_t cmplxid = 0;
    magma_tally4_int_t lcgpunb = 0;
    for( magma_tally4_int_t d = 0; d < ngpu; ++d ) {
        ...
        if(deviceid[d]==0){
            cmplxnb           = cmplxnb+1;
            cmplxid           = cmplxnb-1;
            gnode[cmplxid][Magma_tally4MaxGPUs] = 1;
            lcgpunb           = gnode[cmplxid][Magma_tally4MaxGPUs]-1;
            gnode[cmplxid][lcgpunb] = d;
            deviceid[d]=-1;
        }
        //printf("DEVICE %d : \n", d);

        for( magma_tally4_int_t d2 = d+1; d2 < ngpu; ++d2 ) {
            // check for unified memory & enable peer memory access between all GPUs.            
            ...
            if(samecomplex==1){
                ...
                if((err==cudaSuccess)||(err==cudaErrorPeerAccessAlreadyEnabled)){
                    if(deviceid[d2]==0){
                        ...
                        gnode[cmplxid][Magma_tally4MaxGPUs] = gnode[cmplxid][Magma_tally4MaxGPUs]+1;
                        lcgpunb           = gnode[cmplxid][Magma_tally4MaxGPUs]-1;
