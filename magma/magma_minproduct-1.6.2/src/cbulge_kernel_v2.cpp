...
*/
    ...
    magma_minproduct_int_t ione = 1;
    magma_minproduct_int_t vpos, taupos, len, len2;

    magma_minproductFloatComplex c_one    =  MAGMA_minproduct_C_ONE;

    magma_minproduct_bulge_findVTAUpos(n, nb, Vblksiz, sweep-1, st-1, ldv, &vpos, &taupos);
    ...
    *V(vpos)  = c_one;
    ...
    magma_minproduct_clarfxsym_v2(len, A(st,st), lda-1, V(vpos), TAU(taupos), work);
}
#undef A
#undef V
#undef TAU
    ...
    magma_minproductFloatComplex *work)
{
    ...
    */
    ...
    magma_minproduct_int_t ione = 1;
    magma_minproduct_int_t vpos, taupos;

    magma_minproductFloatComplex conjtmp;

    magma_minproductFloatComplex c_one = MAGMA_minproduct_C_ONE;

    magma_minproduct_int_t ldx = lda-1;
    magma_minproduct_int_t len = ed - st + 1;
    magma_minproduct_int_t lem = min(ed+nb, n) - ed;
    magma_minproduct_int_t lem2;
    ...
    if (lem > 1) {
        magma_minproduct_bulge_findVTAUpos(n, nb, Vblksiz, sweep-1, ed, ldv, &vpos, &taupos);

        /* remove the first column of the created bulge */
        *V(vpos)  = c_one;
        //memcpy(V(vpos+1), A(ed+2, st), (lem-1)*sizeof(magma_minproductFloatComplex));
        lem2 = lem-1;
        blasf77_ccopy( &lem2, A(ed+2, st), &ione, V(vpos+1), &ione );
        memset(A(ed+2, st),0,(lem-1)*sizeof(magma_minproductFloatComplex));
