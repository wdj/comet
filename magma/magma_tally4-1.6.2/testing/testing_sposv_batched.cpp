...
int main(int argc, char **argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            info = magma_tally4_sposv_batched(opts.uplo, N, nrhs, dA_array, ldda, dB_array, lddb, dinfo_array, batchCount, queue); 
            ...
            {
                if(cpu_info[i] != 0 ){
                    printf("magma_tally4_sposv_batched matrix %d returned internal error %d\n",i, (int)cpu_info[i] );
                }
            }
            if (info != 0)
                printf("magma_tally4_sposv_batched returned argument error %d: %s.\n", (int) info, magma_tally4_strerror( info ));
            ...
            if ( opts.lapack ) {
                ...
                for(magma_tally4_int_t s=0; s<batchCount; s++)
                {
                    lapackf77_sposv( lapack_uplo_const_tally4(opts.uplo), &N, &nrhs, h_A + s * lda * N, &lda, h_B + s * ldb * nrhs, &ldb, &info );
                }
