...
/**
    ...
    Other versions (magma_minproduct_sgeqrf_gpu and magma_minproduct_sgeqrf3_gpu) store the
    ...
    magma_minproduct_int_t *info )
{
    ...
    *info = 0;
    ...
    if (nb >= nbmin && nb < k && nx < k) {
        ...
        for (i = 0; i < k-nx; i += nb) {
            ...
            if (i > 0) {
                ...
                magma_minproduct_ssetmatrix_async( old_ib, old_ib,
            ...
            spanel_to_q( Magma_minproductUpper, ib, work_ref(i), ldwork, hwork+ib*ib );

            /* download the i-th V matrix */
            magma_minproduct_ssetmatrix_async( rows, ib, work_ref(i), ldwork, dA(i,i), ldda, stream[0] );

            /* download the T matrix */
            magma_minproduct_queue_sync( stream[1] );
            magma_minproduct_ssetmatrix_async( ib, ib, hwork, ib, dwork, lddwork, stream[0] );
            magma_minproduct_queue_sync( stream[0] );
            ...
            if (i + ib < n) {
                if (i+nb < k-nx) {
                    ...
                    sq_to_panel( Magma_minproductUpper, ib, work_ref(i), ldwork, hwork+ib*ib );
                }
                else {
                    magma_minproduct_slarfb_gpu( Magma_minproductLeft, Magma_minproductConjTrans, Magma_minproductForward, Magma_minproductColumnwise,
                                      rows, n-i-ib, ib,
    ...
    if (i < k) {
        ib   = n-i;
        rows = m-i;
        magma_minproduct_sgetmatrix_async( rows, ib, dA(i, i), ldda, work, rows, stream[1] );
        magma_minproduct_queue_sync( stream[1] );
        lhwork = lwork - rows*ib;
        lapackf77_sgeqrf(&rows, &ib, work, &rows, tau+i, work+ib*rows, &lhwork, info);
        
        magma_minproduct_ssetmatrix_async( rows, ib, work, rows, dA(i, i), ldda, stream[1] );
    }
