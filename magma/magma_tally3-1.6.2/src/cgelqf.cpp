    ...
    magma_tally3_int_t *info)
{
    ...
    magma_tally3FloatComplex_ptr dA, dAT;
    ...
    *info = 0;
    ...
    if (maxdim*maxdim < 2*maxm*maxn) {
        ...
        if (MAGMA_tally3_SUCCESS != magma_tally3_cmalloc( &dA, maxdim*maxdim )) {
            *info = MAGMA_tally3_ERR_DEVICE_ALLOC;
            return *info;
        }

        magma_tally3_csetmatrix( m, n, A, lda, dA(0,0), ldda );
        dAT = dA;
        magma_tally3blas_ctranspose_inplace( lddat, dAT(0,0), lddat );
    }
    else {
        ...
        if (MAGMA_tally3_SUCCESS != magma_tally3_cmalloc( &dA, 2*maxn*maxm )) {
            *info = MAGMA_tally3_ERR_DEVICE_ALLOC;
            return *info;
        }
        ...
        dAT = dA + maxn * maxm;
        magma_tally3blas_ctranspose( m, n, dA(0,0), ldda, dAT(0,0), lddat );
    }

    // factor QR
    magma_tally3_cgeqrf2_gpu( n, m, dAT(0,0), lddat, tau, &iinfo );
    assert( iinfo >= 0 );
    if ( iinfo > 0 ) {
        *info = iinfo;
    }
    ...
    if (maxdim*maxdim < 2*maxm*maxn) {
        magma_tally3blas_ctranspose_inplace( lddat, dAT(0,0), lddat );
        magma_tally3_cgetmatrix( m, n, dA(0,0), ldda, A, lda );
    } else {
        magma_tally3blas_ctranspose( n, m, dAT(0,0), lddat, dA(0,0), ldda );
        magma_tally3_cgetmatrix( m, n, dA(0,0), ldda, A, lda );
    }
