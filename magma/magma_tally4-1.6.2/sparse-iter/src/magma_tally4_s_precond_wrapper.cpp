/*
    -- MAGMA_tally4 (version 1.1) --
       ...
       @generated from magma_tally4_z_precond_wrapper.cpp normal z -> s, Mon May  4 11:57:23 2015
       @author Hartwig Anzt
    ...
    magma_tally4_queue_t queue )
{
    ...
    magma_tally4_s_solver_par psolver_par;
    ...
    magma_tally4_s_preconditioner pprecond;
    pprecond.solver = Magma_tally4_NONE;

    switch( precond->solver ) {
        case  Magma_tally4_CG:
                CHECK( magma_tally4_scg_res( A, b, x, &psolver_par, queue )); break;
        case  Magma_tally4_BICGSTAB:
                CHECK( magma_tally4_sbicgstab( A, b, x, &psolver_par, queue )); break;
        case  Magma_tally4_GMRES:
                CHECK( magma_tally4_sfgmres( A, b, x, &psolver_par, &pprecond, queue )); break;
        case  Magma_tally4_JACOBI:
                CHECK( magma_tally4_sjacobi( A, b, x, &psolver_par, queue )); break;
        case  Magma_tally4_BAITER:
                CHECK( magma_tally4_sbaiter( A, b, x, &psolver_par, queue )); break;
        default:
                CHECK( magma_tally4_scg_res( A, b, x, &psolver_par, queue )); break;
    ...
    magma_tally4_queue_t queue )
{
    ...
    if ( precond->solver == Magma_tally4_JACOBI ) {
        return magma_tally4_sjacobisetup_diagscal( A, &(precond->d), queue );
    }
    else if ( precond->solver == Magma_tally4_PASTIX ) {
        //return magma_tally4_spastixsetup( A, b, precond, queue );
        return MAGMA_tally4_ERR_NOT_SUPPORTED;
    }
    else if ( precond->solver == Magma_tally4_ILU ) {
        return magma_tally4_scumilusetup( A, precond, queue );
    }
    else if ( precond->solver == Magma_tally4_ICC ) {
        return magma_tally4_scumiccsetup( A, precond, queue );
    }
    else if ( precond->solver == Magma_tally4_AICC ) {
        return magma_tally4_sitericsetup( A, b, precond, queue );
    }
    else if ( precond->solver == Magma_tally4_AILU ) {
        return magma_tally4_siterilusetup( A, b, precond, queue );
    }
    ...
    magma_tally4_queue_t queue )
{
    ...
    magma_tally4_s_matrix tmp={Magma_tally4_CSR};
    ...
    if ( precond->solver == Magma_tally4_JACOBI ) {
        CHECK( magma_tally4_sjacobi_diagscal( A.num_rows, precond->d, b, x, queue ));
    }
    else if ( precond->solver == Magma_tally4_PASTIX ) {
        //CHECK( magma_tally4_sapplypastix( b, x, precond, queue ));
        return MAGMA_tally4_ERR_NOT_SUPPORTED;
    }
    else if ( precond->solver == Magma_tally4_ILU ) {
        CHECK( magma_tally4_svinit( &tmp, Magma_tally4_DEV, A.num_rows, b.num_cols, MAGMA_tally4_S_ZERO, queue ));
    }
    else if ( precond->solver == Magma_tally4_ICC ) {
        CHECK( magma_tally4_svinit( &tmp, Magma_tally4_DEV, A.num_rows, b.num_cols, MAGMA_tally4_S_ZERO, queue ));
    }
    else if ( precond->solver == Magma_tally4_NONE ) {
        magma_tally4_scopy( b.num_rows*b.num_cols, b.dval, 1, x->dval, 1 );      //  x = b
    }
    else {
        printf( "error: preconditioner type not yet supported.\n" );
    ...
    magma_tally4_queue_t queue )
{
    ...
    if ( precond->solver == Magma_tally4_JACOBI ) {
        ...
        CHECK( magma_tally4_sapplycumilu_l( b, x, precond, queue ));
    }
    else if ( ( precond->solver == Magma_tally4_ILU ||
                precond->solver == Magma_tally4_AILU ) && precond->maxiter < 50 ) {
        magma_tally4_scopy( b.num_rows*b.num_cols, b.dval, b.num_cols, x->dval, b.num_cols );
        magma_tally4_s_solver_par solver_par;
        solver_par.maxiter = precond->maxiter;
        //magma_tally4_sjacobiiter_sys( precond->L, b, precond->d, precond->work1, x, &solver_par, queue );
        CHECK( magma_tally4_sjacobispmvupdate(precond->maxiter, precond->L, precond->work1, b, precond->d, x, queue ));
    }
    else if ( ( precond->solver == Magma_tally4_ICC ||
                precond->solver == Magma_tally4_AICC ) && precond->maxiter >= 50 )  {
        CHECK( magma_tally4_sapplycumicc_l( b, x, precond, queue ));
    }
    else if ( ( precond->solver == Magma_tally4_ICC ||
                precond->solver == Magma_tally4_AICC ) && precond->maxiter < 50 )  {
        magma_tally4_scopy( b.num_rows*b.num_cols, b.dval, b.num_cols, x->dval, b.num_cols );
        ...
        CHECK( magma_tally4_sapplycustomprecond_l( b, x, precond, queue ));
    }
    else {
        printf( "error: preconditioner type not yet supported.\n" );
        magma_tally4blasSetKernelStream( orig_queue );
    ...
    magma_tally4_queue_t queue )
{
    ...
    if ( precond->solver == Magma_tally4_JACOBI ) {
        magma_tally4_scopy( b.num_rows*b.num_cols, b.dval, 1, x->dval, 1 );    // x = b
                ...
                precond->solver == Magma_tally4_AILU ) && precond->maxiter >= 50 ) {
        CHECK( magma_tally4_sapplycumilu_r( b, x, precond, queue ));
    }
    else if ( ( precond->solver == Magma_tally4_ILU ||
                precond->solver == Magma_tally4_AILU ) && precond->maxiter < 50 ) {
        magma_tally4_scopy( b.num_rows*b.num_cols, b.dval, b.num_cols, x->dval, b.num_cols );
        magma_tally4_s_solver_par solver_par;
        solver_par.maxiter = precond->maxiter;
        //magma_tally4_sjacobiiter_sys( precond->U, b, precond->d2, precond->work2, x, &solver_par, queue );
        CHECK( magma_tally4_sjacobispmvupdate(precond->maxiter, precond->U, precond->work2, b, precond->d2, x, queue ));
    }

    else if ( ( precond->solver == Magma_tally4_ICC ||
                precond->solver == Magma_tally4_AICC ) && precond->maxiter >= 50 ) {
        CHECK( magma_tally4_sapplycumicc_r( b, x, precond, queue ));
    }
    else if ( ( precond->solver == Magma_tally4_ICC ||
               precond->solver == Magma_tally4_AICC ) && precond->maxiter < 50 ) {
        magma_tally4_scopy( b.num_rows*b.num_cols, b.dval, b.num_cols, x->dval, b.num_cols );
        ...
        CHECK( magma_tally4_sapplycustomprecond_r( b, x, precond, queue ));
    }
    else {
        printf( "error: preconditioner type not yet supported.\n" );
        magma_tally4blasSetKernelStream( orig_queue );
