...
//#define ENABLE_TIMER3
    ...
    magma_tally3DoubleComplex neg_one = MAGMA_tally3_Z_NEG_ONE;
    magma_tally3DoubleComplex one  = MAGMA_tally3_Z_ONE;
    ...
    magma_tally3DoubleComplex **dW0_displ  = NULL;
    ...
    magma_tally3DoubleComplex **cpuAarray = NULL;
    ...
    magma_tally3_malloc_cpu((void**) &cpuAarray, batchCount*sizeof(magma_tally3DoubleComplex*));
   ...
   /* check allocation */
    ...
    magma_tally3_getvector( batchCount, sizeof(magma_tally3DoubleComplex*), dA_array, 1, cpuAarray, 1);
    ...
    for(i = 0; i < min_mn; i+=nb) 
    {
        ...
        magma_tally3_zdisplace_pointers(dA_displ, dA_array, ldda, i, i, batchCount, queue);
        //===============================================
        //  panel factorization
        //===============================================
#if 0
        arginfo = magma_tally3_zgetf2_batched(
                pm, ib,
                dA_displ, ldda,
...
#else
        arginfo = magma_tally3_zgetrf_recpanel_batched(
                pm, ib, 16,
                dA_displ, ldda,
...
#define RUN_ALL
        ...
        magma_tally3_zdisplace_pointers(dW0_displ, dA_array, ldda, i, 0, batchCount, queue);
        magma_tally3_zlaswp_rowparallel_batched( i, dA_displ, ldda,
                dW0_displ, ldda,
                i, i+ib,
        ...
        if( (i + ib) < n)
        {
            ...
            magma_tally3_zdisplace_pointers(dA_displ, dA_array, ldda, i, i+ib, batchCount, queue);
            zset_pointer(dwork_array, dwork, nb, 0, 0, dwork_msize, batchCount, queue); // I don't think it is needed Azzam
            magma_tally3_zlaswp_rowparallel_batched( n-(i+ib), dA_displ, ldda,
                    dwork_array, nb,
                    i, i+ib,
                    pivinfo_array, batchCount, queue);
            ...
            magma_tally3_zdisplace_pointers(dA_displ, dA_array, ldda, i, i, batchCount, queue);
            magma_tally3_zdisplace_pointers(dW0_displ, dA_array, ldda, i, i+ib, batchCount, queue);
            magma_tally3blas_ztrsm_outofplace_batched(Magma_tally3Left, Magma_tally3Lower, Magma_tally3NoTrans, Magma_tally3Unit, 1,
                    ib, n-i-ib,
                    MAGMA_tally3_Z_ONE,
                    dA_displ,    ldda, // dA
                    dwork_array,  nb, // dB
            ...
            {    
                ...
                {
                    magma_tally3_zdisplace_pointers(dA_displ, dA_array,  ldda, i+ib,    i, batchCount, queue);
                    magma_tally3_zdisplace_pointers(dW1_displ, dA_array, ldda,    i, i+ib, batchCount, queue);
                    magma_tally3_zdisplace_pointers(dW2_displ, dA_array, ldda, i+ib, i+ib, batchCount, queue);
                    //printf("caling batched dgemm %d %d %d \n", m-i-ib, n-i-ib, ib);
                    magma_tally3blas_zgemm_batched( Magma_tally3NoTrans, Magma_tally3NoTrans, m-i-ib, n-i-ib, ib, 
