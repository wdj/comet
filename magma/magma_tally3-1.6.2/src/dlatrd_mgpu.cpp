            ...
            if (i > 0) {
                ...
                *A(i-1,i) = MAGMA_tally3_D_ONE;
                ...
                for( dev=0; dev < ngpu; dev++ ) {
                    ...
                    magma_tally3_dsetvector_async( n, A(0,i), 1, dW1(dev, 0, iw), 1, queues[dev] );
                }
                ...
                if (i < n-1) {
                    blasf77_dgemv( Magma_tally3ConjTransStr, &i, &n_i_1, &c_one,
                                   W(0,   iw+1), &ldw,
                                   A(0,   i),    &ione, &c_zero,
                                   W(i+1, iw),   &ione );
                }
                ...
                for( dev=0; dev < ngpu; dev++ ) {
                    magma_tally3_setdevice( dev );
                    magma_tally3_dsetvector_async( n, W(0,iw), 1, dW(dev, 0, iw), 1, queues[dev] );
                }
            }
        }
    } else {
        ...
        for (i = 0; i < nb; ++i) {
            ...
            if (i < n-1) {
                ...
                *A(i+1,i) = MAGMA_tally3_D_ONE;
                ...
                for( dev=0; dev < ngpu; dev++ ) {
                    magma_tally3_setdevice( dev );
                    magma_tally3_dsetvector_async( n, A(0,i), 1, dW1(dev, 0, i), 1, queues[dev] );
                }
                ...
                trace_cpu_start( 0, "gemv", "gemv" );
                blasf77_dgemv( Magma_tally3ConjTransStr, &n_i_1, &i, &c_one,
                               W(i+1, 0), &ldw,
                               A(i+1, i), &ione, &c_zero,
                               W(0,   i), &ione );
                ...
                value = magma_tally3_cblas_ddot( n_i_1, W(i+1,i), ione, A(i+1,i), ione );
                alpha = tau[i] * -0.5f * value;
                blasf77_daxpy( &n_i_1, &alpha, A(i+1, i), &ione, W(i+1,i), &ione );
                trace_cpu_end( 0 );
                for( dev=0; dev < ngpu; dev++ ) {
                    magma_tally3_setdevice( dev );
                    magma_tally3_dsetvector_async( n, W(0,i), 1, dW(dev, 0, i), 1, queues[dev] );
                }
            }
        }
    }
