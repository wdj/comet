...
int main( int argc, char** argv)
{
    ...
    magma_minproductDoubleComplex *d_A;
    ...
    for( int i = 0; i < opts.ntest; ++i ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lapackf77_zlacpy( Magma_minproductUpperLowerStr, &N, &(columns), h_A, &lda, h_R, &lda );
            ...
            info = magma_minproduct_zpotrf_batched( opts.uplo, N, d_A_array, ldda, dinfo_magma_minproduct, batchCount, queue);
            ...
            {
                if(cpu_info[i] != 0 ){
                    printf("magma_minproduct_zpotrf_batched matrix %d returned internal error %d\n",i, (int)cpu_info[i] );
                }
            }
            if (info != 0)
                printf("magma_minproduct_zpotrf_batched returned argument error %d: %s.\n", (int) info, magma_minproduct_strerror( info ));
            ...
            if ( opts.lapack ) {
                ...
                /* =====================================================================
                   ...
                   =================================================================== */
                 magma_minproduct_zgetmatrix( N, columns, d_A, ldda, h_R, lda );
