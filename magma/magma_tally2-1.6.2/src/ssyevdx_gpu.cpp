    ...
    magma_tally2_int_t *info)
{
    ...
    *info = 0;
    ...
    if (n <= 128) {
        ...
        float *A;
        magma_tally2_smalloc_cpu( &A, lda*n );
        magma_tally2_sgetmatrix( n, n, dA, ldda, A, lda );
        // TODO: deal with range and mout. Use lapack ssyevx? (There is no lapack ssyevdx.)
        lapackf77_ssyevd( lapack_vec_const_tally2(jobz), lapack_uplo_const_tally2(uplo),
                          &n, A, &lda,
                          w, work, &lwork,
                          iwork, &liwork, info );
        *mout = n;
    ...
    if (MAGMA_tally2_SUCCESS != magma_tally2_smalloc( &dwork, ldwork )) {
    ...
    anrm = magma_tally2blas_slansy( Magma_tally2MaxNorm, uplo, n, dA, ldda, dwork );
    iscale = 0;
    sigma  = 1;
    if (anrm > 0. && anrm < rmin) {
        iscale = 1;
    ...
    if (iscale == 1) {
        magma_tally2blas_slascl( uplo, 0, 0, 1., sigma, n, n, dA, ldda, info );
    }
...
#ifdef FAST_SYMV
    magma_tally2_ssytrd2_gpu( uplo, n, dA, ldda, w, &work[inde],
                       &work[indtau], wA, ldwa, &work[indwrk], llwork,
                       dwork, ldwork, &iinfo );
#else
    magma_tally2_ssytrd_gpu(  uplo, n, dA, ldda, w, &work[inde],
                       &work[indtau], wA, ldwa, &work[indwrk], llwork,
                       &iinfo );
#endif
    ...
    if (! wantz) {
        ...
        magma_tally2_ssetmatrix( n, *mout, &work[indwrk + n* (il-1) ], n, dwork, lddc );

        magma_tally2_sormtr_gpu( Magma_tally2Left, uplo, Magma_tally2NoTrans, n, *mout, dA, ldda, &work[indtau],
                          dwork, lddc, wA, ldwa, &iinfo );

        magma_tally2_scopymatrix( n, *mout, dwork, lddc, dA, ldda );
