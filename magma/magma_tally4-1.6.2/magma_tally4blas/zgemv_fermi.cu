...
zgemvn_kernel1_fermi(
    ...
    const magma_tally4DoubleComplex * __restrict__ A, int lda,
    const magma_tally4DoubleComplex * __restrict__ x, int incx, magma_tally4DoubleComplex beta,
    ...
    magma_tally4DoubleComplex       * __restrict__ y, int incy)
{
    ...
    __shared__ magma_tally4DoubleComplex sx[BLK_Y];
    ...
    if ( threadIdx.x < npart ) {
        ...
        sx[threadIdx.x] = MAGMA_tally4_Z_ZERO;
    }
    __syncthreads();
    ...
    magma_tally4DoubleComplex       * __restrict__ y, int incy)
{
    ...
    __shared__ magma_tally4DoubleComplex sdata[BLK_X];
...
magma_tally4blas_zgemv(
    magma_tally4_trans_t trans, magma_tally4_int_t m, magma_tally4_int_t n, magma_tally4DoubleComplex alpha,
    ...
    magma_tally4DoubleComplex_ptr dy, magma_tally4_int_t incy)
{
    ...
    if ( arch < 200  ) {
        ...
        magma_tally4_zgemv( trans, m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
        #else
        magma_tally4blas_zgemv_tesla( trans, m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
        #endif
        return;
    }
    ...
    if ( trans == Magma_tally4NoTrans ) {
        ...
        zgemvn_kernel1_fermi<<< grid, threads, 0, magma_tally4_stream >>>
            ( m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    }
    else if ( trans == Magma_tally4Trans ) {
        dim3 grid    ( 1, n, 1 );
        dim3 threads ( BLK_X, 1, 1 );
        zgemvt_kernel_fermi<<< grid, threads, 0, magma_tally4_stream >>>
            ( m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    }
    else if ( trans == Magma_tally4ConjTrans ) {
        dim3 grid    ( 1, n, 1 );
        dim3 threads ( BLK_X, 1, 1 );
        zgemvc_kernel_fermi<<< grid, threads, 0, magma_tally4_stream >>>
            ( m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    }
}
