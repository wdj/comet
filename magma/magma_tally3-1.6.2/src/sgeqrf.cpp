    ...
    magma_tally3_int_t *info )
{
    ...
    /* Function Body */
    *info = 0;
    magma_tally3_int_t nb = magma_tally3_get_sgeqrf_nb(min(m, n));
    ...
    if ( ngpu > 1 ) {
        ...
        return magma_tally3_sgeqrf4(ngpu, m, n, A, lda, tau, work, lwork, info);
    }

    // allocate space for dA, dwork, and dT
    if (MAGMA_tally3_SUCCESS != magma_tally3_smalloc( &dA, n*ldda + nb*lddwork + nb*nb )) {
        /* Switch to the "out-of-core" (out of GPU-memory) version */
        return magma_tally3_sgeqrf_ooc(m, n, A, lda, tau, work, lwork, info);
    }
    ...
    if ( (nb > 1) && (nb < k) ) {
        ...
        for (i = 0; i < k-nb; i += nb) {
            ...
            if (i > 0) {
                ...
                magma_tally3_sgetmatrix_async( m-i, ib,
                                        dA(i,i), ldda,
                                        A(i,i),  lda, stream[0] );
                ...
                magma_tally3_sgetmatrix_async( i, ib,
                                        dA(0,i), ldda,
                                        A(0,i),  lda, stream[1] );
            ...
            spanel_to_q_tally3(Magma_tally3Upper, ib, A(i,i), lda, work+ib*ib);

            /* download the i-th V matrix */
            magma_tally3_ssetmatrix_async( rows, ib, A(i,i), lda, dA(i,i), ldda, stream[0] );

            /* download the T matrix */
            magma_tally3_queue_sync( stream[1] );
            magma_tally3_ssetmatrix_async( ib, ib, work, ib, dT, nb, stream[0] );
            magma_tally3_queue_sync( stream[0] );
            ...
            if (i + ib < n) {
                if (i+ib < k-nb) {
                    ...
                    sq_to_panel_tally3(Magma_tally3Upper, ib, A(i,i), lda, work+ib*ib);
                }
                else {
    ...
    if (i < k) {
        ib = n-i;
        if (i != 0) {
            magma_tally3_sgetmatrix_async( m, ib, dA(0,i), ldda, A(0,i), lda, stream[1] );
            magma_tally3_queue_sync( stream[1] );
        }
        magma_tally3_int_t rows = m-i;
        lapackf77_sgeqrf(&rows, &ib, A(i,i), &lda, tau+i, work, &lwork, info);
