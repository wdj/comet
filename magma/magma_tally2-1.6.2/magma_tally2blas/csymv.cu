    ...
    magma_tally2FloatComplex * __restrict__ work)
{
    ...
    __shared__ magma_tally2FloatComplex sA [quarter_NB_X][NB_X + 3]; /* Why +3? seems it only needs +2. Does +3 reduce bank conflicts? */
...
/**
    Purpose
    -------
    magma_tally2blas_csymv_work performs the matrix-vector operation:
    ...
    MAGMA_tally2 implements csymv through two steps:
    ...
    magma_tally2blas_csymv is a wrapper routine allocating the workspace inside the
    routine and provides the same interface as cublas.
    
    If users need to call csymv frequently, we suggest using
    magma_tally2blas_csymv_work instead of magma_tally2blas_csymv. As the overhead to
    allocate and free in device memory in magma_tally2blas_csymv would hurt performance.
    ...
    magma_tally2_queue_t queue )
{
    ...
    if ( arch < 200 ) {
        //magma_tally2_csymv( uplo, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
     ...
     */
    if ( (n == 0) || ( MAGMA_tally2_C_EQUAL(alpha, MAGMA_tally2_C_ZERO) && MAGMA_tally2_C_EQUAL(beta, MAGMA_tally2_C_ONE) ) )
        return info;
...
// end magma_tally2blas_csymv_work


/**
    Purpose
    -------
    magma_tally2blas_csymv performs the matrix-vector operation:
    ...
    magma_tally2FloatComplex_ptr dy, magma_tally2_int_t incy)
{
    ...
    magma_tally2_cmalloc( &dwork, lwork );
    if ( dwork == NULL ) {
    ...
    magma_tally2blas_csymv_work( uplo, n, alpha, dA, ldda, dx, incx, beta, dy, incy,
