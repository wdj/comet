/*
    -- MAGMA_minproduct (version 1.6.1) --
       ...
       @generated from zgemm_reduce.cu normal z -> s, Fri Jan 30 19:00:08 2015
...
// BLK_K gets defined in magma_minproductblas_sgemm_reduce,
    ...
    float      * __restrict__ dC, int ldc)
{
    ...
    if (blockIdx.x*BLK_M + threadIdx.y < m && blockIdx.y*BLK_N + threadIdx.z < n){
        ...
        for( int j = tx; j < k; j += BLK_K )
            lsum += MAGMA_minproduct_S_CNJG( dA[j] )* dB[j];
        ...
        __syncthreads();
        if (threadIdx.x == 0) {
            if (MAGMA_minproduct_S_EQUAL(beta, MAGMA_minproduct_S_ZERO))
    ...
    magma_minproductFloat_ptr       dC, magma_minproduct_int_t lddc )
{
    ...
    if ( arch < 200  ) {
        ...
        sgemm_reduce_kernel<BLK_K> <<< blocks, threads, 0, magma_minproduct_stream >>>
