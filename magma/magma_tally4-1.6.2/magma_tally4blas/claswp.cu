...
__global__ void claswp_kernel(
    int n, magma_tally4FloatComplex *dAT, int ldda, claswp_params_t params )
{
    int tid = threadIdx.x + blockDim.x*blockIdx.x;
    if ( tid < n ) {
        dAT += tid;
        magma_tally4FloatComplex *A1  = dAT;
        
        for( int i1 = 0; i1 < params.npivots; ++i1 ) {
            int i2 = params.ipiv[i1];
            magma_tally4FloatComplex *A2 = dAT + i2*ldda;
            magma_tally4FloatComplex temp = *A1;
            *A1 = *A2;
            *A2 = temp;
...
magma_tally4blas_claswp(
    ...
    const magma_tally4_int_t *ipiv, magma_tally4_int_t inci )
{
    magma_tally4blas_claswp_q( n, dAT, ldda, k1, k2, ipiv, inci, magma_tally4_stream );
}
...
__global__ void claswpx_kernel(
    int n, magma_tally4FloatComplex *dA, int ldx, int ldy, claswp_params_t params )
{
    int tid = threadIdx.x + blockDim.x*blockIdx.x;
    if ( tid < n ) {
        dA += tid*ldy;
        magma_tally4FloatComplex *A1  = dA;
        
        for( int i1 = 0; i1 < params.npivots; ++i1 ) {
            int i2 = params.ipiv[i1];
            magma_tally4FloatComplex *A2 = dA + i2*ldx;
            magma_tally4FloatComplex temp = *A1;
            *A1 = *A2;
            *A2 = temp;
            A1 += ldx;  // A1 = dA + i1*ldx
...
/**
    @see magma_tally4blas_claswpx_q
    @ingroup magma_tally4_caux2
    ********************************************************************/
extern "C" void
magma_tally4blas_claswpx(
    ...
    const magma_tally4_int_t *ipiv, magma_tally4_int_t inci )
{
    return magma_tally4blas_claswpx_q( n, dA, ldx, ldy, k1, k2, ipiv, inci, magma_tally4_stream );
}
...
// batches of pivots. On Fermi, it is faster than magma_tally4blas_claswp
// (including copying pivots to the GPU).

__global__ void claswp2_kernel(
    int n, magma_tally4FloatComplex *dAT, int ldda, int npivots,
    const magma_tally4_int_t *d_ipiv, int inci )
{
    int tid = threadIdx.x + blockDim.x*blockIdx.x;
    if ( tid < n ) {
        dAT += tid;
...
/**
    @see magma_tally4blas_claswp2_q
    @ingroup magma_tally4_caux2
    ********************************************************************/
extern "C" void
magma_tally4blas_claswp2(
    ...
    magma_tally4Int_const_ptr d_ipiv, magma_tally4_int_t inci )
{
    magma_tally4blas_claswp2_q( n, dAT, ldda, k1, k2, d_ipiv, inci, magma_tally4_stream );
}
