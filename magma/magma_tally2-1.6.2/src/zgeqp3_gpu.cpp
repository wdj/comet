...
/**
    ...
    @ingroup magma_tally2_zgeqp3_comp
    ********************************************************************/
    ...
    magma_tally2_int_t *info )
{
    ...
    *info = 0;
    ...
    nb = magma_tally2_get_zgeqp3_nb(min(m, n));
    minmn = min(m,n);
    if (*info == 0) {
        if (minmn == 0) {
            lwkopt = 1;
    ...
    #endif
    magma_tally2DoubleComplex_ptr df;
    if (MAGMA_tally2_SUCCESS != magma_tally2_zmalloc( &df, (n+1)*nb )) {
        *info = MAGMA_tally2_ERR_DEVICE_ALLOC;
        return *info;
    }
    cudaMemset( df, 0, (n+1)*nb*sizeof(magma_tally2DoubleComplex) );
    ...
    if (nfxd > 0) {
        ...
        if (na < n) {
            n_j = n - na;
            lapackf77_zunmqr( Magma_tally2LeftStr, Magma_tally2ConjTransStr, &m, &n_j, &na,
    ...
    if (nfxd < minmn) {
        ...
        magma_tally2blas_dznrm2_cols(sm, sn, dA(nfxd,nfxd), ldda, &rwork[nfxd]);
        magma_tally2_dcopymatrix( sn, 1, &rwork[nfxd], sn, &rwork[n+nfxd], sn);
        ...
        //if (nb < sminmn)
        {
            ...
            while(j < topbmn) {
                ...
                //magma_tally2_zlaqps_gpu    // this is a cpp-file
                magma_tally2_zlaqps2_gpu   // this is a cuda-file
                    ( m, n_j, j, jb, &fjb,
                      dA(0, j), ldda,
        ...
        if (j < minmn) {
            n_j = n - j;
            if (j > nfxd) {
                magma_tally2_zgetmatrix( m-j, n_j,
                                  dA(j,j), ldda,
                                   A(j,j), lda );
            }
