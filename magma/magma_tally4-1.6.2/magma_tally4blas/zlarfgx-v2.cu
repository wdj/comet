...
__global__
void magma_tally4_zlarfgx_gpu_kernel( int n, magma_tally4DoubleComplex* dx0, magma_tally4DoubleComplex* dx,
                               magma_tally4DoubleComplex *dtau, double *dxnorm,
                               magma_tally4DoubleComplex *dA, int it)
{
    ...
    if ( i == 0 ) {
        ...
        double alpha = *dx0;
        double alphai = MAGMA_tally4_Z_ZERO;
        if ( (xnorm == 0 && alphai == MAGMA_tally4_Z_ZERO ) || n == 1 )
#else
        magma_tally4DoubleComplex alpha = *dx0;
        double alphar =  MAGMA_tally4_Z_REAL(alpha), alphai = MAGMA_tally4_Z_IMAG(alpha);
        if ( (xnorm == 0 && alphai == MAGMA_tally4_Z_ZERO ) || n == 0 )
#endif
        {
            ...
            *dA   = *dx0;
            ...
            if (j==0){
                *dtau = MAGMA_tally4_Z_MAKE((beta - alphar)/beta, -alphai/beta);
                //*dx0  = MAGMA_tally4_Z_MAKE(  1., 0.); //cannot be done here because raise condition all threadblock need to read it for alpha
                *dA   = MAGMA_tally4_Z_MAKE(beta, 0.);
            }
    ...
    if (j<it){
        *( dA-it+j) = *(dx0-it+j);
        *(dx0-it+j) = MAGMA_tally4_Z_MAKE(0., 0.);
    }
}
    ...
    magma_tally4DoubleComplex_ptr dA, magma_tally4_int_t iter)
{
    ...
    magma_tally4_zlarfgx_gpu_kernel<<< blocks, threads, 0, magma_tally4_stream >>>( n, dx0, dx, dtau, dxnorm, dA, iter);
}
    ...
    magma_tally4DoubleComplex_ptr dwork)
{
    ...
    magma_tally4_zlarfgx_gpu(n, dx0, dx, dtau, dxnorm, dA, iter);
    
    if (iter==0) {
        magma_tally4DoubleComplex tt = MAGMA_tally4_Z_ONE;
        magma_tally4blas_zlacpy(Magma_tally4UpperLower, 1, 1, dtau, 1, T+iter+iter*ldt, 1);
        magma_tally4_zsetmatrix(1,1, &tt,1, dx0,1);
    }
    else {
        /* Compute the iter-th column of T */
        magma_tally4_zgemv_kernel3<<< iter, BLOCK_SIZE, 0, magma_tally4_stream >>>( n, V, ldv, dx0, dwork, dtau );
        magma_tally4_ztrmv_kernel2<<< iter, iter,       0, magma_tally4_stream >>>( T, ldt, dwork, T+iter*ldt, dtau );
    }
}
