/*
    -- MAGMA_minproduct (version 1.6.1) --
       ...
       @generated from zgebrd.cpp normal z -> c, Fri Jan 30 19:00:19 2015
...
#define dA(i, j) (dA + (j)*ldda + (i))
    ...
    magma_minproduct_int_t ncol, nrow, jmax, nb, ldda;

    magma_minproduct_int_t i, j, nx;
    magma_minproduct_int_t iinfo;

    magma_minproduct_int_t minmn;
    magma_minproduct_int_t ldwrkx, ldwrky, lwkopt;
    magma_minproduct_int_t lquery;

    nb   = magma_minproduct_get_cgebrd_nb(n);
    ldda = m;
    ...
    *info = 0;
    ...
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_cmalloc( &dA, n*ldda + (m + n)*nb )) {
    ...
    for (i=0; i < (minmn - nx); i += nb) {
        ...
        if ( i > 0 ) {
            magma_minproduct_cgetmatrix( nrow, nb, dA(i, i), ldda, A( i, i), lda );
            magma_minproduct_cgetmatrix( nb, ncol - nb,
                              dA(i, i+nb), ldda,
                              A( i, i+nb), lda );
        }

        magma_minproduct_clabrd_gpu(nrow, ncol, nb,
        ...
        magma_minproduct_csetmatrix( nrow, nb, work  + nb, ldwrkx, dwork + nb, ldwrkx );
        magma_minproduct_csetmatrix( ncol, nb,
        ...
        if (m >= n) {
            jmax = i + nb;
            for (j = i; j < jmax; ++j) {
                *A(j, j  ) = MAGMA_minproduct_C_MAKE( d[j], 0. );
                *A(j, j+1) = MAGMA_minproduct_C_MAKE( e[j], 0. );
            }
        } else {
            jmax = i + nb;
            for (j = i; j < jmax; ++j) {
    ...
    if ( 0 < minmn - nx ) {
        magma_minproduct_cgetmatrix( nrow, ncol, dA(i, i), ldda, A(i, i), lda );
    }
