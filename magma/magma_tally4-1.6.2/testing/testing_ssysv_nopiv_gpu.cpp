...
int main(int argc, char **argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            bool nopiv = true;
            if ( nopiv ) {
                magma_tally4_smake_hpd( N, h_A, lda );  // SPD / HPD does not require pivoting
            }
            else {
                magma_tally4_smake_symmetric( N, h_A, lda );  // symmetric/symmetric generally requires pivoting
            }
            
            magma_tally4_ssetmatrix( N, N,    h_A, lda, d_A, ldda );
            magma_tally4_ssetmatrix( N, nrhs, h_B, ldb, d_B, lddb );
            ...
            magma_tally4_ssysv_nopiv_gpu( opts.uplo, N, nrhs, d_A, ldda, d_B, lddb, &info );
            gpu_time = magma_tally4_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally4_sgesv_gpu returned error %d: %s.\n",
            ...
            if ( opts.lapack ) {
                lwork = -1;
                lapackf77_ssysv( lapack_uplo_const_tally4(opts.uplo), &N,&nrhs,
                                 h_A, &lda, ipiv, h_B, &ldb, &temp, &lwork, &info );
                lwork = (magma_tally4_int_t) MAGMA_tally4_S_REAL( temp );
