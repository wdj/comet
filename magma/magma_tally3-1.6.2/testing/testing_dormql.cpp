    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int itran = 0; itran < 2; ++itran ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_tally3_dgeqlf( mm, k, A, lda, tau, W, lwork_max, &info );
            if (info != 0)
                printf("magma_tally3_dgeqlf returned error %d: %s.\n",
            ...
            lapackf77_dormql( lapack_side_const_tally3( side[iside] ), lapack_trans_const_tally3( trans[itran] ),
                              &m, &n, &k,
            ...
            lwork = -1;
            magma_tally3_dormql( side[iside], trans[itran],
                          m, n, k,
                          A, lda, tau, R, ldc, W, lwork, &info );
            if (info != 0)
                printf("magma_tally3_dormql (lwork query) returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
            ...
            if (info != 0)
                printf("magma_tally3_dormql returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
