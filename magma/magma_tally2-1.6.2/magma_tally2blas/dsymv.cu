...
/**
    Purpose
    -------
    magma_tally2blas_dsymv_work performs the matrix-vector operation:
    ...
    MAGMA_tally2 implements dsymv through two steps:
    ...
    magma_tally2blas_dsymv is a wrapper routine allocating the workspace inside the
    routine and provides the same interface as cublas.
    
    If users need to call dsymv frequently, we suggest using
    magma_tally2blas_dsymv_work instead of magma_tally2blas_dsymv. As the overhead to
    allocate and free in device memory in magma_tally2blas_dsymv would hurt performance.
    ...
    magma_tally2_queue_t queue )
{
    ...
    if ( arch < 200 ) {
        magma_tally2_dsymv( uplo, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
     ...
     */
    if ( (n == 0) || ( MAGMA_tally2_D_EQUAL(alpha, MAGMA_tally2_D_ZERO) && MAGMA_tally2_D_EQUAL(beta, MAGMA_tally2_D_ONE) ) )
        return info;
...
// end magma_tally2blas_dsymv_work


/**
    Purpose
    -------
    magma_tally2blas_dsymv performs the matrix-vector operation:
    ...
    magma_tally2Double_ptr dy, magma_tally2_int_t incy)
{
    ...
    magma_tally2_dmalloc( &dwork, lwork );
    if ( dwork == NULL ) {
    ...
    magma_tally2blas_dsymv_work( uplo, n, alpha, dA, ldda, dx, incx, beta, dy, incy,
