    ...
    magma_tally2DoubleComplex **dC_array, magma_tally2_int_t lddc, magma_tally2_int_t batchCount, magma_tally2_queue_t queue )
{
    ...
    if      ( transB == Magma_tally2Trans )
        ...
        magma_tally2DoubleComplex *cpuAarray[2], *cpuBarray[2];
        magma_tally2_getvector( min(2,batchCount), sizeof(magma_tally2DoubleComplex*), dA_array, 1, cpuAarray, 1);
        magma_tally2_getvector( min(2,batchCount), sizeof(magma_tally2DoubleComplex*), dB_array, 1, cpuBarray, 1);
        ...
        cudaError_t err;
        err = cudaBindTexture(&offsetA, tex_ref_A, cpuAarray[0], sizeA*sizeof(magma_tally2DoubleComplex));
        if ( err != cudaSuccess ) {
            fprintf( stderr, "cannot bind A to texture: %s (%d)\n", cudaGetErrorString(err), err );
            return;
        }	
        err = cudaBindTexture(&offsetB, tex_ref_B, cpuBarray[0], sizeB*sizeof(magma_tally2DoubleComplex));
        if ( err != cudaSuccess ) {
    ...
    offsetA = offsetA/sizeof(magma_tally2DoubleComplex);
    offsetB = offsetB/sizeof(magma_tally2DoubleComplex);
