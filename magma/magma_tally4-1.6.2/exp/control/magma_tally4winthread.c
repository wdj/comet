...
CRITICAL_SECTION magma_tally4winthread_static_initializer_check_lock;
static int magma_tally4winthread_initialized = 0;
...
static int pthread_mutex_check_for_static_initialization( pthread_mutex_t *mutex ) {
    ...
    if ( magma_tally4winthread_initialized == 0 ) {
        magma_tally4winthread_initialized = 1;
        InitializeCriticalSection( &magma_tally4winthread_static_initializer_check_lock );
    }
    EnterCriticalSection( &magma_tally4winthread_static_initializer_check_lock );
    if ( *mutex == PTHREAD_MUTEX_INITIALIZER )
        retval = pthread_mutex_init( mutex, NULL );
    LeaveCriticalSection( &magma_tally4winthread_static_initializer_check_lock );
    return retval;
}
...
void *(*MAGMA_tally4_realThStart)(void *);

/*
  This function is only called to have a proxy that is compatible with WINAPI.
 */
unsigned WINAPI MAGMA_tally4_winThStart(void *arg) {
  MAGMA_tally4_realThStart( arg );
  return 0;
}

MAGMA_tally4_DLLPORT int MAGMA_tally4_CDECL pthread_create(pthread_t *thread, const pthread_attr_t *attr, void *(*start) (void *), void *arg) {
...
MAGMA_tally4_DLLPORT int MAGMA_tally4_CDECL pthread_setconcurrency (int level) {
