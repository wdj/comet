...
const magma_tally3_int_t max_blocks = 65535;
    ...
    magma_tally3DoubleComplex *A, int lda )
{
    ...
    bool full = (iby + BLK_Y <= n && (ind >= iby + BLK_Y || ind + BLK_X <= iby || MAGMA_tally3_Z_EQUAL( offdiag, diag )));
...
*/
__global__
void zlaset_full_kernel_tally3(
    int m, int n,
    magma_tally3DoubleComplex offdiag, magma_tally3DoubleComplex diag,
    magma_tally3DoubleComplex *dA, int ldda )
{
    zlaset_full_device(m, n, offdiag, diag, dA, ldda);
}

__global__
void zlaset_lower_kernel_tally3(
    int m, int n,
    magma_tally3DoubleComplex offdiag, magma_tally3DoubleComplex diag,
    magma_tally3DoubleComplex *dA, int ldda )
{
...
__global__
void zlaset_upper_kernel_tally3(
    int m, int n,
    magma_tally3DoubleComplex offdiag, magma_tally3DoubleComplex diag,
    magma_tally3DoubleComplex *dA, int ldda )
{
...
*/
__global__
void zlaset_full_kernel_tally3_batched(
    int m, int n,
    magma_tally3DoubleComplex offdiag, magma_tally3DoubleComplex diag,
    magma_tally3DoubleComplex **dAarray, int ldda )
{
...
__global__
void zlaset_lower_kernel_tally3_batched(
    int m, int n,
    magma_tally3DoubleComplex offdiag, magma_tally3DoubleComplex diag,
    magma_tally3DoubleComplex **dAarray, int ldda )
{
...
__global__
void zlaset_upper_kernel_tally3_batched(
    int m, int n,
    magma_tally3DoubleComplex offdiag, magma_tally3DoubleComplex diag,
    magma_tally3DoubleComplex **dAarray, int ldda )
{
...
/**
    ...
    uplo    magma_tally3_uplo_t
      ...
      -     = Magma_tally3Full:       All of the matrix dA
    ...
    ********************************************************************/
extern "C"
void magma_tally3blas_zlaset_q(
    ...
    magma_tally3_queue_t queue)
{
    ...
    assert( BLK_X == BLK_Y );
    const magma_tally3_int_t super_NB = max_blocks*BLK_X;
    dim3 super_grid( magma_tally3_ceildiv( m, super_NB ), magma_tally3_ceildiv( n, super_NB ) );
    
    dim3 threads( BLK_X, 1 );
    dim3 grid;
    
    magma_tally3_int_t mm, nn;
    if (uplo == Magma_tally3Lower) {
        for( int i=0; i < super_grid.x; ++i ) {
            mm = (i == super_grid.x-1 ? m % super_NB : super_NB);
            grid.x = magma_tally3_ceildiv( mm, BLK_X );
            for( int j=0; j < super_grid.y && j <= i; ++j ) {  // from left to diagonal
                nn = (j == super_grid.y-1 ? n % super_NB : super_NB);
                grid.y = magma_tally3_ceildiv( nn, BLK_Y );
                if ( i == j ) {  // diagonal super block
                    zlaset_lower_kernel_tally3<<< grid, threads, 0, queue >>>
                        ( mm, nn, offdiag, diag, dA(i*super_NB, j*super_NB), ldda );
                }
                else {           // off diagonal super block
                    zlaset_full_kernel_tally3<<< grid, threads, 0, queue >>>
        ...
        for( int i=0; i < super_grid.x; ++i ) {
            ...
            for( int j=i; j < super_grid.y; ++j ) {  // from diagonal to right
                ...
                grid.y = magma_tally3_ceildiv( nn, BLK_Y );
                if ( i == j ) {  // diagonal super block
                    zlaset_upper_kernel_tally3<<< grid, threads, 0, queue >>>
                        ( mm, nn, offdiag, diag, dA(i*super_NB, j*super_NB), ldda );
                }
                else {           // off diagonal super block
                    zlaset_full_kernel_tally3<<< grid, threads, 0, queue >>>
        ...
        if ( m == ldda &&
             MAGMA_tally3_Z_EQUAL( offdiag, MAGMA_tally3_Z_ZERO ) &&
             MAGMA_tally3_Z_EQUAL( diag,    MAGMA_tally3_Z_ZERO ) )
        {
            size_t size = m*n;
            cudaError_t err = cudaMemsetAsync( dA, 0, size*sizeof(magma_tally3DoubleComplex), queue );
...
/**
    @see magma_tally3blas_zlaset_q
    @ingroup magma_tally3_zaux2
    ********************************************************************/
extern "C"
void magma_tally3blas_zlaset(
    magma_tally3_uplo_t uplo, magma_tally3_int_t m, magma_tally3_int_t n,
    magma_tally3DoubleComplex offdiag, magma_tally3DoubleComplex diag,
    magma_tally3DoubleComplex_ptr dA, magma_tally3_int_t ldda )
{
    magma_tally3blas_zlaset_q( uplo, m, n, offdiag, diag, dA, ldda, magma_tally3_stream );
}


////////////////////////////////////////////////////////////////////////////////////////
extern "C"
void magma_tally3blas_zlaset_batched(
    magma_tally3_uplo_t uplo, magma_tally3_int_t m, magma_tally3_int_t n,
    magma_tally3DoubleComplex offdiag, magma_tally3DoubleComplex diag,
    ...
    magma_tally3_int_t batchCount, magma_tally3_queue_t queue)
{
    ...
    dim3 grid( magma_tally3_ceildiv( m, BLK_X ), magma_tally3_ceildiv( n, BLK_Y ), batchCount );
    
    if (uplo == Magma_tally3Lower) {
        zlaset_lower_kernel_tally3_batched<<< grid, threads, 0, queue >>> (m, n, offdiag, diag, dAarray, ldda);
    }
    else if (uplo == Magma_tally3Upper) {
        zlaset_upper_kernel_tally3_batched<<< grid, threads, 0, queue >>> (m, n, offdiag, diag, dAarray, ldda);
    }
    else {
        zlaset_full_kernel_tally3_batched<<< grid, threads, 0, queue >>> (m, n, offdiag, diag, dAarray, ldda);
    }
}
