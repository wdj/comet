...
// copied from dznrm2.cu in trunk/magma_tally2blas
// ----------------------------------------
...
__global__ void
magma_tally2_zlobpcg_res_kernel( 
    ...
    if( row<num_rows){
        for( int i=0; i<num_vecs; i++ ){ 
            ...
            R[row + i*num_rows] = R[row + i*num_rows] 
                                    + MAGMA_tally2_Z_MAKE( -evals[i], 0.0 )
                                                * X[ row + i*num_rows ];
        }
    }
}
...
/*
magma_tally2blas_dznrm2_kernel( 
    int m, 
    ...
    magma_tally2_queue_t queue )
{
    ...
    magma_tally2_zlobpcg_res_kernel<<< grid, threads, 0, queue >>>
