...
zlarfg_device(
    int n,
    magma_tally4DoubleComplex* dalpha, magma_tally4DoubleComplex* dx, int incx,
    magma_tally4DoubleComplex* dtau,  double* swork, double* sscale, magma_tally4DoubleComplex* scale)
{
    ...
    if ( tx == 0 ) {
        magma_tally4DoubleComplex alpha = *dalpha;
        ...
        if ( swork[0] == 0 && imag(alpha) == 0 ) {
            ...
            *dtau   = MAGMA_tally4_Z_MAKE( (beta - real(alpha)) / beta, -imag(alpha) / beta );
            *dalpha = MAGMA_tally4_Z_MAKE( beta, 0 );
            *scale = 1 / (alpha - beta);
        }
    }
...
static __device__
void zlarfx_device( int m, int n,  magma_tally4DoubleComplex *v, magma_tally4DoubleComplex *tau,
                         magma_tally4DoubleComplex *dc, magma_tally4_int_t ldc, magma_tally4DoubleComplex* sum)
{


    if(n <=0) return ;
    if (MAGMA_tally4_Z_EQUAL(*tau, MAGMA_tally4_Z_ZERO) )  return; // check singularity
    ...
    for(int k=0;k<n;k++)
    {
        ...
        if(tx<BLOCK_SIZE)
        {
            ...
            for( int j = tx+1; j < m; j += BLOCK_SIZE ){
                lsum += MAGMA_tally4_Z_MUL( MAGMA_tally4_Z_CNJG( v[j] ), dc[j+ldc*k] );
            }
        ...
        magma_tally4DoubleComplex z__1 = - MAGMA_tally4_Z_CNJG(*tau) * sum[0];
        /*  C := C - v * w  */
        if(tx<BLOCK_SIZE)
        {    
           for( int j = tx+1; j<m ; j += BLOCK_SIZE )
...
__global__
void zgeqr2_sm_kernel_batched( int m, int n, magma_tally4DoubleComplex** dA_array, magma_tally4_int_t lda,
                               magma_tally4DoubleComplex **dtau_array)
{

    magma_tally4DoubleComplex* dA = dA_array[blockIdx.z];
    magma_tally4DoubleComplex* dtau = dtau_array[blockIdx.z];
...
static __device__
void zgeqr2_device( magma_tally4_int_t m, magma_tally4_int_t n, magma_tally4DoubleComplex* dA, magma_tally4_int_t lda,
                               magma_tally4DoubleComplex *dtau, 
                               magma_tally4DoubleComplex *sdata,
...
__global__
void zgeqr2_kernel_batched( int m, int n, magma_tally4DoubleComplex** dA_array, magma_tally4_int_t lda,
                               magma_tally4DoubleComplex **dtau_array)
{
...
/**
    ...
    @param
    dwork   (workspace) COMPLEX_16 array, dimension (N) * ( sizeof(double) + sizeof(magma_tally4DoubleComplex)) 
...
magma_tally4_zgeqr2_batched(magma_tally4_int_t m, magma_tally4_int_t n, magma_tally4DoubleComplex **dA_array,
                  magma_tally4_int_t lda, magma_tally4DoubleComplex **dtau_array,
                  magma_tally4_int_t *info_array, magma_tally4_int_t batchCount, magma_tally4_queue_t queue)
{
    ...
    if(sizeof(magma_tally4DoubleComplex)*(m*k) <= 128 /*sizeof(magma_tally4DoubleComplex) * 128 * k*/) // there are some static shared memory besides of dynamic ones 
    {   
        ...
        zgeqr2_sm_kernel_batched<<< blocks, threads, sizeof(magma_tally4DoubleComplex)*(m*k), queue >>>
    ...
    else
    {
        ...
        if(sizeof(magma_tally4DoubleComplex)*(m) < 42000)
            zgeqr2_kernel_batched<<< blocks, threads, sizeof(magma_tally4DoubleComplex)*(m), queue >>>
                                      (m, k, dA_array, lda, dtau_array);
        else
