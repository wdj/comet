...
int main( int argc, char** argv)
{
    ...
    magma_tally4_int_t N, nb, size, lda, ldda, mstride, nstride, ntile;
    ...
    printf("uplo = %s, nb = %d, mstride = %d, nstride = %d\n",
            lapack_uplo_const_tally4(opts.uplo), (int) nb, (int) mstride, (int) nstride );
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            gbytes = sizeof(magma_tally4DoubleComplex) * 2.*nb*nb*ntile / 1e9;
            ...
            magma_tally4blas_zsymmetrize_tiles( opts.uplo, nb, d_A, ldda, ntile, mstride, nstride );
                ...
                for( int j = 0; j < nb; ++j ) {
                    for( int i = 0; i < j; ++i ) {
                        if ( opts.uplo == Magma_tally4Lower ) {
                            h_A[offset + i + j*lda] = MAGMA_tally4_Z_CNJG( h_A[offset + j + i*lda] );
                        }
                        else {
                            h_A[offset + j + i*lda] = MAGMA_tally4_Z_CNJG( h_A[offset + i + j*lda] );
                        }
                    }
                }
            }
            ...
            /* =====================================================================
               Check the result
               =================================================================== */
            magma_tally4_zgetmatrix( N, N, d_A, ldda, h_R, lda );
