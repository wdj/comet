...
/**
    ...
    tau     DOUBLE_PRECISION array, dimension (N)
            ...
            reflector H(i), as returned by MAGMA_tally4_DGEQRF_GPU.
    ...
    dT      DOUBLE_PRECISION array that is the output (the 6th argument)
            of magma_tally4_dgeqrf_gpu of size
    ...
    lwork   INTEGER
            ...
            where NB is the blocksize given by magma_tally4_get_dgeqrf_nb( M ).
    \n
    ...
    @ingroup magma_tally4_dgels_comp
    ********************************************************************/
    ...
    magma_tally4_int_t *info)
{
    ...
    magma_tally4_int_t nb     = magma_tally4_get_dgeqrf_nb(m);
    magma_tally4_int_t lwkopt = (m - n + nb)*(nrhs + nb) + nrhs*nb;
    int lquery = (lwork == -1);

    hwork[0] = MAGMA_tally4_D_MAKE( (double)lwkopt, 0. );
    ...
    *info = 0;
    ...
    /* B := Q' * B */
    magma_tally4_dormqr_gpu( Magma_tally4Left, Magma_tally4Trans,
                      m, nrhs, n,
                      dA(0,0), ldda, tau,
    ...
    // TODO: this assumes that, on exit from magma_tally4_dormqr_gpu, hwork contains
    ...
    // magma_tally4_dtrsm and drop the dsetmatrix.
    if ( nrhs == 1 ) {
        blasf77_dtrsv( Magma_tally4UpperStr, Magma_tally4NoTransStr, Magma_tally4NonUnitStr,
                       &ib, hwork,         &rows,
                            hwork+rows*ib, &ione);
    } else {
        blasf77_dtrsm( Magma_tally4LeftStr, Magma_tally4UpperStr, Magma_tally4NoTransStr, Magma_tally4NonUnitStr,
                       &ib, &nrhs,
                       &c_one, hwork,         &rows,
                               hwork+rows*ib, &rows);
    }
    ...
    magma_tally4_dsetmatrix( ib, nrhs, hwork+rows*ib, rows, dwork+i, lddwork );

    // update c
    if (nrhs == 1)
        magma_tally4_dgemv( Magma_tally4NoTrans, i, ib,
    ...
    if (nb < k) {
        for (i = start; i >= 0; i -= nb) {
            ...
            if (i + ib < n) {
                if (nrhs == 1) {
                    magma_tally4_dgemv( Magma_tally4NoTrans, ib, ib,
                                 c_one,  dT(i), ib,
                                         dB+i,      1,
                                 c_zero, dwork+i,  1);
                    magma_tally4_dgemv( Magma_tally4NoTrans, i, ib,
    ...
    magma_tally4_dcopymatrix( (n), nrhs,
                       dwork, lddwork,
                       dB,    lddb );
