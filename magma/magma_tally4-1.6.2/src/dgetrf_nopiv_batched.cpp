...
#define A(i_, j_)  (A + (i_) + (j_)*lda)   
   ...
   /* check allocation */
    ...
    for(i = 0; i < min_mn; i+=nb) 
    {
        ...
        magma_tally4_ddisplace_pointers(dA_displ, dA_array, lda, i, i, batchCount, queue);
...
#define RUN_ALL
        ...
        if( (i + ib) < n)
        {
            ...
            //magma_tally4_ddisplace_pointers(dA_displ, dA_array, lda, i, i+ib, batchCount);
            dset_pointer(dwork_array, dwork, nb, 0, 0, dwork_msize, batchCount, queue); // I don't think it is needed Azzam

            magma_tally4_ddisplace_pointers(dA_displ, dA_array, lda, i, i, batchCount, queue);
            magma_tally4_ddisplace_pointers(dW0_displ, dA_array, lda, i, i+ib, batchCount, queue);
            ...
            {    
                ...
                {
                    magma_tally4_ddisplace_pointers(dA_displ, dA_array,  lda, i+ib,    i, batchCount, queue);
                    magma_tally4_ddisplace_pointers(dW1_displ, dA_array, lda,    i, i+ib, batchCount, queue);
                    magma_tally4_ddisplace_pointers(dW2_displ, dA_array, lda, i+ib, i+ib, batchCount, queue);
