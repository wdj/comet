    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int itran = 0; itran < 2; ++itran ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            n = opts.nsize[itest];
            k = opts.ksize[itest];
            nb  = magma_minproduct_get_dgebrd_nb( m );
            ldc = m;
            ...
            magma_minproduct_dgebrd( mm, nn, A, lda, d, e, tauq, taup, work, lwork_max, &info );
            if (info != 0)
                printf("magma_minproduct_dgebrd returned error %d: %s.\n",
            ...
            lwork = -1;
            magma_minproduct_dormbr( vect[ivect], side[iside], trans[itran],
                          m, n, k,
                          A, lda, tau, R, ldc, work, lwork, &info );
            if (info != 0)
                printf("magma_minproduct_dormbr (lwork query) returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
            lwork = (magma_minproduct_int_t) MAGMA_minproduct_D_REAL( work[0] );
            ...
            if (info != 0)
                printf("magma_minproduct_dormbr returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
