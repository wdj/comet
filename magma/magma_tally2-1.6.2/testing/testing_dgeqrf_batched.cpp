...
void get_QR_error(magma_tally2_int_t M, magma_tally2_int_t N, magma_tally2_int_t min_mn,
                    double *h_R,  double *h_A, magma_tally2_int_t lda,
                    double *tau,
                    double *Q,  magma_tally2_int_t ldq,
                    double *R,  magma_tally2_int_t ldr,
                    double *h_work,  magma_tally2_int_t lwork,
...
int main( int argc, char** argv)
{
    ...
    double *h_A, *h_R, *h_Amagma_tally2, *tau, *h_work, tmp[1];
    double *d_A, *dtau_magma_tally2, *dtau_cublas;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_Amagma_tally2,   double, n2     );
            TESTING_MALLOC_PIN( h_R,   double, n2     );
        
            TESTING_MALLOC_DEV( d_A,   double, ldda*N * batchCount );

            TESTING_MALLOC_DEV( dtau_magma_tally2,  double, min_mn * batchCount);
			...
			{
				magma_tally2_dmake_hpd( N, h_A + i * lda * N, lda );// need modification
			}
			
			lapackf77_dlacpy( Magma_tally2UpperLowerStr, &M, &column, h_A, &lda, h_R, &lda );
       
            /* ====================================================================
               Performs operation using MAGMA_tally2
               =================================================================== */
            magma_tally2_dsetmatrix( M, column, h_R, lda,  d_A, ldda );
            dset_pointer(dA_array, d_A, 1, 0, 0, ldda*N, batchCount, opts.queue);
            dset_pointer(dtau_array, dtau_magma_tally2, 1, 0, 0, min_mn, batchCount, opts.queue);
    
            magma_tally2_time = magma_tally2_sync_wtime(0);
    
            info = magma_tally2_dgeqrf_batched(M, N, dA_array, ldda, dtau_array, dinfo_magma_tally2, batchCount, opts.queue);

            magma_tally2_time = magma_tally2_sync_wtime(0) - magma_tally2_time;
            magma_tally2_perf = gflops / magma_tally2_time;

            magma_tally2_dgetmatrix( M, column, d_A, ldda, h_Amagma_tally2, lda);

            if (info != 0)
                printf("magma_tally2_dgeqrf_batched returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                magma_tally2_dgetvector(min_mn*batchCount, dtau_magma_tally2, 1, tau, 1);
                for (int i=0; i < batchCount; i++)
                {
                ...
                #if CUDA_VERSION >= 6050
                magma_tally2_dgetvector(min_mn*batchCount, dtau_magma_tally2, 1, tau, 1);
                magma_tally2_dgetmatrix( M, column, d_A, ldda, h_A, lda);
                for (int i=0; i < batchCount; i++)
                {
                    get_QR_error(M, N, min_mn,
