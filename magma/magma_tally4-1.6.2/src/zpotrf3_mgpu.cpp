...
#define ZTRSM_WORK
//#define magma_tally4_ztrsm magma_tally4blas_ztrsm
#endif
...
#define dlPT(id, i, j, k) (d_lP[(id)] + (k)*nb*lddp + (j)*nb   + (i))
    ...
    magma_tally4_int_t     j, jb, nb0, nb2, d, dd, id, j_local, j_local2, buf;
    ...
    magma_tally4DoubleComplex *dlpanel;
    magma_tally4_int_t n_local[Magma_tally4MaxGPUs], ldpanel;
    const magma_tally4_int_t stream1 = 0, stream2 = 1, stream3 = 2;
    ...
    *info = 0;
    ...
    magma_tally4DoubleComplex *d_dinvA[Magma_tally4MaxGPUs];
    magma_tally4DoubleComplex *d_x[Magma_tally4MaxGPUs];
    ...
    for( d=0; d < ngpu; d++ ) {
        ...
        if ( (MAGMA_tally4_SUCCESS != magma_tally4_zmalloc( &d_dinvA[d], 2*trsm_nb*trsm_n )) ||
             (MAGMA_tally4_SUCCESS != magma_tally4_zmalloc( &d_x[d],     2*nb*(upper ? n : m) )) ) {
    ...
    if (upper) {
        ...
        for (j=0; j < m; j += nb) {
            ...
            if ( j > 0 ) {
                magma_tally4blasSetKernelStream( queues[id][stream1] );
                trace_gpu_start( id, stream1, "syrk", "syrk" );
                magma_tally4_zherk(Magma_tally4Upper, Magma_tally4ConjTrans, jb, j,
            ...
            if ( j > 0 ) {
                ...
                for( dd=0; dd < ngpu; dd++ ) {
                    ...
                    if ( n_local[d] > nb0 ) {
                        magma_tally4_setdevice(d);
                        magma_tally4blasSetKernelStream( queues[d][stream2] );
                        if ( d == id ) {
                            dlpanel = dlA(d,0,nb*j_local);
                            ldpanel = ldda;
                            // the GPU owns the row from start, and no need of synch.
                            //magma_tally4_queue_wait_event( queues[d][stream2], events[d][0] ); // rows arrived at gpu
                            magma_tally4_queue_wait_event( queues[d][stream2], events[d][4] ); // wait for look-ahead trsm to finish
                        } else {
                        ...
                        magma_tally4_event_record( events[d][2], queues[d][stream2] );
                    }
                    d = (d+1)%ngpu;
                }
            }
            ...
            magma_tally4_setdevice(id);
            magma_tally4_queue_sync( queues[id][stream1] );
            trace_cpu_start( 0, "getrf", "getrf" );
            lapackf77_zpotrf(Magma_tally4UpperStr, &jb, Aup(j,j), &lda, info);
            trace_cpu_end( 0 );
            if (*info != 0) {
                *info = *info + j;
                break;
            ...
            if ( (j+jb) < n) {
                ...
                for( dd=0; dd < ngpu; dd++ ) {
                    ...
                    magma_tally4_event_record( events[d][1], queues[d][stream1] );
                    d = (d+1)%ngpu;
                }
            } else {
                magma_tally4_setdevice(id);
            ...
            if ( (j+jb) < n) {
                ...
                for( dd=0; dd < ngpu; dd++ ) {
                    ...
                    if ( j+jb < m && d == (j/nb+1)%ngpu ) {
                        ...
                        nb0 = min(nb, nb2);
                        magma_tally4blasSetKernelStream( queues[d][stream1] );
                        magma_tally4_queue_wait_event( queues[d][stream1], events[d][2] ); // wait for gemm update
                        trace_gpu_start( d, stream1, "trsm", "trsm" );
#if (defined(PRECISION_d) || defined(PRECISION_s)) && defined(ZTRSM_WORK)
                        magma_tally4blas_zlaset( Magma_tally4Full, trsm_nb, trsm_n, c_zero, c_zero, dinvA(d,0), trsm_nb );
                        magma_tally4blas_zlaset( Magma_tally4Full, nb0,     jb,     c_zero, c_zero, dx(d,0), nb0 );
                        magma_tally4blas_ztrsm_work( Magma_tally4Left, Magma_tally4Upper,
                                              Magma_tally4ConjTrans, Magma_tally4NonUnit,
                                              jb, nb0, c_one,
                                              dlpanel, ldpanel,
                                              dlA(d, j, nb*j_local2), ldda,
                                              1, dinvA(d,0), dx(d,0) );
#else
                        magma_tally4_ztrsm( Magma_tally4Left, Magma_tally4Upper,
                                     Magma_tally4ConjTrans, Magma_tally4NonUnit,
                                     jb, nb0, c_one,
                                     dlpanel,                ldpanel,
                                     dlA(d, j, nb*j_local2), ldda);
#endif
                        magma_tally4_event_record( events[d][4], queues[d][stream1] );
                        trace_gpu_end( d, stream1 );
                    } else if ( nb2 > 0 ) {
                        /* update all the blocks on stream2 */
                        magma_tally4_queue_wait_event( queues[d][stream2], events[d][1] ); // wait for cholesky factor
                        trace_gpu_start( d, stream2, "trsm", "trsm" );
                        magma_tally4blasSetKernelStream( queues[d][stream2] );
#if (defined(PRECISION_d) || defined(PRECISION_s)) && defined(ZTRSM_WORK)
                        magma_tally4blas_zlaset( Magma_tally4Full, trsm_nb, trsm_n, c_zero, c_zero, dinvA(d,0), trsm_nb );
                        magma_tally4blas_zlaset( Magma_tally4Full, nb2,     jb,     c_zero, c_zero, dx(d,0), nb2 );
                        magma_tally4blas_ztrsm_work( Magma_tally4Left, Magma_tally4Upper,
                                              Magma_tally4ConjTrans, Magma_tally4NonUnit,
                                              jb, nb2, c_one,
                                              dlpanel, ldpanel,
                ...
                if ( j+jb < m ) {
                    ...
                    magma_tally4_setdevice(d);
                    /* lookahead done */
                    magma_tally4_queue_wait_event( queues[d][stream3], events[d][4] );
                
                    trace_gpu_start( d, stream3, "comm", "row to CPU" );
                    magma_tally4_zgetmatrix_async( (j+jb), nb0,
                                            dlA(d, 0, nb*j_local2), ldda,
                                            Aup(0,j+jb),            lda,
                                            queues[d][stream3] );
                    trace_gpu_end( d, stream3 );
                    magma_tally4_event_record( events[d][3], queues[d][stream3] );
                    /* needed on pluto */
                    //magma_tally4_queue_sync( queues[d][stream3] );
                
                    /* broadcast rows to gpus on stream2 */
                    buf2 = ((j+jb)/nb)%ngpu;
                    for( d2=0; d2 < ngpu; d2++ ) {
                        if ( d2 != d ) {
                            magma_tally4_setdevice(d2);
                            trace_gpu_start( d2, stream3, "comm", "row to GPUs" );
                            magma_tally4_queue_wait_event( queues[d2][stream3], events[d][3] ); // rows arrived at cpu on stream3
                            magma_tally4_zsetmatrix_async( j+jb, nb0,
                                                    Aup(0,j+jb),       lda,
                                                    dlP(d2,nb,0,buf2), lddp,
                                                    queues[d2][stream3] );
                            trace_gpu_end( d2, stream3 );
                            magma_tally4_event_record( events[d2][0], queues[d2][stream3] );
                        }
                    }
                    ...
                    if ( nb2 > 0 ) {
                        ...
                        int flag = 0;
                        if (flag == 0) {
                            magma_tally4_queue_wait_event( queues[d][stream2], events[d][4] ); // lookahead -> diagonal inversion
                        } else {
                            magma_tally4blas_zlaset( Magma_tally4Full, trsm_nb, trsm_n, c_zero, c_zero, dinvA(d,flag), trsm_nb );
                            magma_tally4_queue_wait_event( queues[d][stream2], events[d][1] ); // panel received
                        }
                        magma_tally4blas_zlaset( Magma_tally4Full, nb2, jb, c_zero, c_zero, dx(d,1), nb2 );
                        magma_tally4blas_ztrsm_work( Magma_tally4Left, Magma_tally4Upper, Magma_tally4ConjTrans, Magma_tally4NonUnit,
                                              jb, nb2, c_one,
                                              dlpanel, ldpanel,
...
#else
                        magma_tally4_queue_wait_event( queues[d][stream2], events[d][1] ); // wait for cholesky factor
                        magma_tally4_ztrsm( Magma_tally4Left, Magma_tally4Upper, Magma_tally4ConjTrans, Magma_tally4NonUnit,
                                     jb, nb2, c_one,
                                     dlpanel, ldpanel,
                                     dlA(d, j, nb*j_local2+nb0), ldda);
#endif
        ...
        for (j=0; j < n; j += nb) {
            ...
            if ( j > 0 ) {
                magma_tally4blasSetKernelStream( queues[id][stream1] );
                magma_tally4_zherk(Magma_tally4Lower, Magma_tally4NoTrans, jb, j,
            ...
            magma_tally4_setdevice(id);
            magma_tally4_queue_sync( queues[id][stream1] );
            lapackf77_zpotrf(Magma_tally4LowerStr, &jb, Alo(j,j), &lda, info);
            if (*info != 0) {
                *info = *info + j;
                break;
            }
            ...
            if ( (j+jb) < m) {
                ...
                for( dd=0; dd < ngpu; dd++ ) {
                    ...
                    if ( j+nb < n && d == (j/nb+1)%ngpu ) { /* owns next column, look-ahead next block on stream1 */
                        if ( j > 0 ) magma_tally4_queue_wait_event( queues[d][stream1], events[d][2] ); // wait for gemm update
                        magma_tally4blasSetKernelStream( queues[d][stream1] );
#if (defined(PRECISION_d) || defined(PRECISION_s)) && defined(ZTRSM_WORK)
                        magma_tally4blas_zlaset( Magma_tally4Full, trsm_nb, trsm_n, c_zero, c_zero, dinvA(d,0), trsm_nb );
                        magma_tally4blas_zlaset( Magma_tally4Full, nb0,     jb,     c_zero, c_zero, dx(d,0), nb0 );
                        magma_tally4blas_ztrsm_work( Magma_tally4Right, Magma_tally4Lower,
                                              Magma_tally4ConjTrans, Magma_tally4NonUnit,
                                              nb0, jb, c_one,
                                              dlpanel, ldpanel,
                        ...
                        magma_tally4_queue_wait_event( queues[d][stream2], events[d][1] ); // wait for the cholesky factor
                        magma_tally4blasSetKernelStream( queues[d][stream2] );
#if (defined(PRECISION_d) || defined(PRECISION_s)) && defined(ZTRSM_WORK)
                        magma_tally4blas_zlaset( Magma_tally4Full, trsm_nb, trsm_n, c_zero, c_zero, dinvA(d,0), trsm_nb );
                        magma_tally4blas_zlaset( Magma_tally4Full, nb2,     jb,     c_zero, c_zero, dx(d,0), nb2 );
                        magma_tally4blas_ztrsm_work( Magma_tally4Right, Magma_tally4Lower, Magma_tally4ConjTrans, Magma_tally4NonUnit,
                                              nb2, jb, c_one,
                                              dlpanel,                ldpanel,
                                              dlA(d, nb*j_local2, j), ldda,
                                              1, dinvA(d,0), dx(d,0) );
#else
                        magma_tally4_ztrsm( Magma_tally4Right, Magma_tally4Lower, Magma_tally4ConjTrans, Magma_tally4NonUnit,
                                     nb2, jb, c_one,
                                     dlpanel,                ldpanel,
                                     dlA(d, nb*j_local2, j), ldda);
#endif
...
//#define ZPOTRF_DEVICE_TO_DEVICE
                    ...
                    for( d2=0; d2 < ngpu; d2++ ) {
                        magma_tally4_setdevice(d2);
                        magma_tally4_queue_wait_event( queues[d2][stream3], events[d][4] );
                        if ( d2 != d ) {
                            magma_tally4_zcopymatrix_async( nb0, j+jb,
                                                     dlPT(d2,0,nb,buf2), nb, // first nbxnb reserved for diagonal block
                                                     dlA(d, nb*j_local2, 0), ldda,
                                                     queues[d2][stream3] );
                            magma_tally4_event_record( events[d2][0], queues[d2][stream3] );
                        } else {
                            magma_tally4_zgetmatrix_async( nb0, j+jb,
                                                    dlA(d, nb*j_local2, 0), ldda,
                                                    Alo(j+jb,0),            lda,
                                                    queues[d][stream3] );
                        }
                    ...
                    for( d2=0; d2 < ngpu; d2++ ) {
                        if ( d2 != d ) {
                            magma_tally4_setdevice(d2);
                            magma_tally4_queue_wait_event( queues[d2][stream3], events[d][3] ); // getmatrix done
                            magma_tally4_zsetmatrix_async( nb0, j+jb,
                                                    Alo(j+jb,0),        lda,
                                                    dlPT(d2,0,nb,buf2), nb, // first nbxnb reserved for diagonal block
                                                    queues[d2][stream3] );
                            magma_tally4_event_record( events[d2][0], queues[d2][stream3] );
    ...
    for( d=0; d < ngpu; d++ ) {
        magma_tally4_setdevice(d);
        for( j=0; j < 3; j++ ) {
            magma_tally4_queue_sync( queues[d][j] );
        }
#if (defined(PRECISION_d) || defined(PRECISION_s)) && defined(ZTRSM_WORK)
        magma_tally4_free( d_dinvA[d] );
        magma_tally4_free( d_x[d] );
#endif
    }
    ...
    magma_tally4_int_t *info)
{
    ...
    if (uplo == Magma_tally4Upper) {
        ...
        for (j=off_j; j < n; j += nb) {
            ...
            magma_tally4_zsetmatrix_async( mj, jb,
                                    A(off_i, j),     lda,
                                    dA(k, 0, jj*nb), ldda,
                                    queues[k][0] );
        }
        ...
        for (i=off_i; i < m; i += nb) {
            ...
            magma_tally4_setdevice(k);
            magma_tally4_zsetmatrix_async( ib, ni,
                                    A(i, off_j),     lda,
                                    dA(k, ii*nb, 0), ldda,
                                    queues[k][0] );
        }
    ...
    for( k=0; k < ngpu; k++ ) {
        magma_tally4_setdevice(k);
        magma_tally4_queue_sync( queues[k][0] );
    }
    magma_tally4_setdevice( orig_dev );
    ...
    magma_tally4_int_t *info)
{
    ...
    if (uplo == Magma_tally4Upper) {
        ...
        for (j=off_j+NB; j < n; j += nb) {
            ...
            magma_tally4_setdevice(k);
            magma_tally4_zgetmatrix_async( mj, jb,
                                    dA(k, 0, jj*nb), ldda,
                                    A(off_i, j),     lda,
                                    queues[k][0] );
            magma_tally4_queue_sync( queues[k][0] );
        ...
        for (i=off_i+NB; i < m; i += nb) {
            ...
            magma_tally4_setdevice(k);
            magma_tally4_zgetmatrix_async( ib, ni,
                                    dA(k, ii*nb, 0), ldda,
                                    A(i, off_j),     lda,
                                    queues[k][0] );
            magma_tally4_queue_sync( queues[k][0] );
