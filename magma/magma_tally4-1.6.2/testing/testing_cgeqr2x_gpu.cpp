...
int main( int argc, char** argv)
{
    ...
    magma_tally4FloatComplex *h_A, *h_T, *h_R, *tau, *h_work, tmp[1];
    magma_tally4FloatComplex_ptr d_A,  d_T, ddA, dtau;
    magma_tally4FloatComplex_ptr d_A2, d_T2, ddA2, dtau2;
    magma_tally4Float_ptr dwork, dwork2;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_T,   magma_tally4FloatComplex, N*N    );
        
            TESTING_MALLOC_PIN( h_R,   magma_tally4FloatComplex, n2     );
        
            TESTING_MALLOC_DEV( d_A,   magma_tally4FloatComplex, ldda*N );
            TESTING_MALLOC_DEV( d_T,   magma_tally4FloatComplex, N*N    );
            TESTING_MALLOC_DEV( ddA,   magma_tally4FloatComplex, N*N    );
            TESTING_MALLOC_DEV( dtau,  magma_tally4FloatComplex, min_mn );
        
            TESTING_MALLOC_DEV( d_A2,  magma_tally4FloatComplex, ldda*N );
            TESTING_MALLOC_DEV( d_T2,  magma_tally4FloatComplex, N*N    );
            TESTING_MALLOC_DEV( ddA2,  magma_tally4FloatComplex, N*N    );
            TESTING_MALLOC_DEV( dtau2, magma_tally4FloatComplex, min_mn );
            ...
            // todo replace with magma_tally4_claset
            magma_tally4blas_claset( Magma_tally4Full, N, N, c_zero, c_zero, ddA,  N );
            magma_tally4blas_claset( Magma_tally4Full, N, N, c_zero, c_zero, d_T,  N );
            magma_tally4blas_claset( Magma_tally4Full, N, N, c_zero, c_zero, ddA2, N );
            magma_tally4blas_claset( Magma_tally4Full, N, N, c_zero, c_zero, d_T2, N );
            ...
            magma_tally4_csetmatrix( M, N, h_R, lda, d_A2, ldda );
            ...
            if (opts.version == 1)
                magma_tally4_cgeqr2x_gpu(M, N, d_A, ldda, dtau, d_T, ddA, dwork, &info);
            else if (opts.version == 2)
                magma_tally4_cgeqr2x2_gpu(M, N, d_A, ldda, dtau, d_T, ddA, dwork, &info);
            else if (opts.version == 3)
                magma_tally4_cgeqr2x3_gpu(M, N, d_A, ldda, dtau, d_T, ddA, dwork, &info);
            else {
                printf( "call magma_tally4_cgeqr2x4_gpu\n" );
                /*
                ...
                */
                magma_tally4_cgeqr2x4_gpu(M, N, d_A, ldda, dtau, d_T, ddA, dwork, NULL, &info);
                //magma_tally4_cgeqr2x4_gpu(M, N, d_A, ldda, dtau, d_T, ddA, dwork, &info, stream[1]);
                //magma_tally4_cgeqr2x4_gpu(M, N, d_A2, ldda, dtau2, d_T2, ddA2, dwork2, &info, stream[0]);
                //magma_tally4_cgeqr2x4_gpu(M, N, d_A2, ldda, dtau2, d_T2, ddA2, dwork2, &info, NULL);
                //gflops *= 2;
            }
            ...
            if (info != 0) {
                printf("magma_tally4_cgeqr2x_gpu version %d returned error %d: %s.\n",
                       (int) opts.version, (int) info, magma_tally4_strerror( info ));
            } 
            else {
                if ( opts.check ) {
                    /* =====================================================================
                    ...
                    magma_tally4_cgetmatrix( N, N, ddA, N,    h_T, N );
                    magma_tally4_cgetmatrix( min_mn, 1, dtau, min_mn,   tau, min_mn );
                    ...
                    magma_tally4FloatComplex *Q, *R;
                    float *work;
                    TESTING_MALLOC_CPU( Q,    magma_tally4FloatComplex, ldq*min_mn );  // M by K
                    TESTING_MALLOC_CPU( R,    magma_tally4FloatComplex, ldr*N );       // K by N
                    ...
                    lapackf77_clacpy( Magma_tally4UpperLowerStr, &M, &N, h_R, &M, h_A, &lda );
                    lapackf77_clarft( Magma_tally4ForwardStr, Magma_tally4ColumnwiseStr,
                                      &M, &N, h_A, &lda, tau, h_work, &N);
                    //magma_tally4_cgeqr2(&M, &N, h_A, &lda, tau, h_work, &info);
                    ...
                    magma_tally4_cgetmatrix( N, N, d_T, N, h_T, N );
    
                    float terr = 0.;
                    for(int col=0; col < N; col++)
                        for(int row=0; row <= col; row++)
                            terr += (  MAGMA_tally4_C_ABS(h_work[row + col*N] - h_T[row + col*N])*
                                       MAGMA_tally4_C_ABS(h_work[row + col*N] - h_T[row + col*N])  );
                    terr = sqrt( terr );
