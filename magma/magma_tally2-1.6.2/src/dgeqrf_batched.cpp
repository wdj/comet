...
#define dA(i, j)  (dA + (i) + (j)*ldda)   // A(i, j) means at i row, j column
    ...
    magma_tally2_int_t nb = 32;
    magma_tally2_int_t nnb = 8;
    magma_tally2_int_t i, k, ib=nb, jb=nnb;
    magma_tally2_int_t ldw, ldt, ldr, offset; 
    ...
    magma_tally2_malloc((void**)&dW4_displ, batchCount * sizeof(*dW4_displ));  // used in dlarfb
    magma_tally2_malloc((void**)&dW5_displ, batchCount * sizeof(*dW5_displ));
    magma_tally2_malloc((void**)&dR_array, batchCount * sizeof(*dR_array));
    magma_tally2_malloc((void**)&dT_array, batchCount * sizeof(*dT_array));

    ldt = ldr = min(nb, min_mn);
    magma_tally2_dmalloc(&dwork,  (2 * nb * n) * batchCount);
    magma_tally2_dmalloc(&dR,  ldr * n   * batchCount);
    magma_tally2_dmalloc(&dT,  ldt * ldt * batchCount);
    magma_tally2_malloc_cpu((void**) &cpuAarray, batchCount*sizeof(double*));
    magma_tally2_malloc_cpu((void**) &cpuTarray, batchCount*sizeof(double*));
    ...
    if ( dW0_displ == NULL || dW1_displ == NULL || dW2_displ == NULL || dW3_displ == NULL || 
        ...
        magma_tally2_free(dR_array);
        magma_tally2_free(dT_array);
        magma_tally2_free(dR);
    ...
    magma_tally2blas_dlaset_q(Magma_tally2Full, ldr, n*batchCount  , MAGMA_tally2_D_ZERO, MAGMA_tally2_D_ZERO, dR, ldr, queue);
    magma_tally2blas_dlaset_q(Magma_tally2Full, ldt, ldt*batchCount, MAGMA_tally2_D_ZERO, MAGMA_tally2_D_ZERO, dT, ldt, queue);
    ...
    magma_tally2_getvector( batchCount, sizeof(double*), dT_array, 1, cpuTarray, 1);
    ...
    for(i=0; i<min_mn;i+=nb)
    {
            ...
            magma_tally2_ddisplace_pointers(dW0_displ, dA_array, ldda, i, i, batchCount, queue); 
            magma_tally2_ddisplace_pointers(dW2_displ, tau_array, 1, i, 0, batchCount, queue);
            ...
            magma_tally2_dgeqrf_panel_batched(m-i, ib, jb, 
                                       dW0_displ, ldda, 
                                       dW2_displ, 
                                       dT_array, ldt, 
                                       dR_array, ldr,
           ...
           dgeqrf_copy_upper_batched(ib, jb, dW0_displ, ldda, dR_array, ldr, batchCount, queue);
            ...
            if( (n-ib-i) > 0)
            {
                ...
                magma_tally2blas_dlaset_batched(Magma_tally2Upper, ib, ib, MAGMA_tally2_D_ZERO, MAGMA_tally2_D_ONE, dW0_displ, ldda, batchCount, queue); 
                magma_tally2_ddisplace_pointers(dW2_displ, tau_array, 1, i, 0, batchCount, queue); 

                // it is faster since it is using BLAS-3 GEMM routines, different from lapack implementation 
                magma_tally2_dlarft_batched(m-i, ib, 0,
                                 dW0_displ, ldda,
                                 dW2_displ,
                                 dT_array, ldt, 
                                 dW4_displ, nb*ldt,
                ...
                { 
                    ...
                    for(k=0; k<batchCount; k++)
                    {
                        ...
                        magma_tally2_dlarfb_gpu_gemm(Magma_tally2Left, Magma_tally2ConjTrans, Magma_tally2Forward, Magma_tally2Columnwise,
                                    m-i, n-i-ib, ib,
            ...
            magma_tally2blas_dlacpy_batched(Magma_tally2Upper, ib, ib, dR_array, ldr, dW0_displ, ldda, batchCount, queue);

    }

    for(k=0; k<nbstreams; k++){
        magma_tally2_queue_destroy( stream[k] );
    }
