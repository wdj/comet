...
/**
    ...
    ikind   INTEGER
            ...
            2:  This version uses a standard LAPACK-based orthogonalization through
                MAGMA_tally4's QR panel factorization (magma_tally4_zgeqr2x3_gpu) and magma_tally4_zungqr
            3:  MGS
    ...
    magma_tally4_int_t *info )
{
    ...
    magma_tally4_int_t i = 0, j, k, n2 = n*n;
    ...
    *info = 0;
    ...
    if (ikind == 1) {
        ...
        magma_tally4DoubleComplex *U, *VT, *vt, *R, *G, *hwork, *tau;
        double *S;
        ...
        magma_tally4_zmalloc_cpu( &hwork, 32 + 2*n*n + 2*n);
        if ( hwork == NULL ) {
            *info = MAGMA_tally4_ERR_HOST_ALLOC;
            return *info;
        }
        
        magma_tally4_int_t lwork=n*n+32; // First part f hwork; used as workspace in svd
        ...
        double *rwork;
        magma_tally4_dmalloc_cpu( &rwork, 5*n);
        if ( rwork == NULL ) {
            *info = MAGMA_tally4_ERR_HOST_ALLOC;
            return *info;
        }
        ...
        do {
            ...
            magma_tally4_zgemm(Magma_tally4ConjTrans, Magma_tally4NoTrans, n, n, m, c_one, dA, ldda, dA, ldda, c_zero, dwork, n );
            magma_tally4_zgetmatrix(n, n, dwork, n, G, n);
            ...
            magma_tally4_zsetmatrix(n, n, VT, n, dwork, n);
            magma_tally4_ztrsm( Magma_tally4Right, Magma_tally4Upper, Magma_tally4NoTrans, Magma_tally4NonUnit, m, n, c_one, dwork, n, dA, ldda);
            if (mins > 0.00001f)
                cn = maxs/mins;
        ...
        magma_tally4_int_t nb = n;

        magma_tally4DoubleComplex_ptr dtau = dwork + 2*n*n;
        magma_tally4DoubleComplex_ptr d_T  = dwork;
        magma_tally4DoubleComplex_ptr ddA  = dwork + n*n;
        magma_tally4DoubleComplex *tau  = work+n*n;

        magma_tally4blas_zlaset( Magma_tally4Full, n, n, c_zero, c_zero, d_T, n );
        magma_tally4_zgeqr2x3_gpu(m, n, dA, ldda, dtau, d_T, ddA,
                           (double *)(dwork+min_mn+2*n*n), info);
        magma_tally4_zgetmatrix( min_mn, 1, dtau, min_mn, tau, min_mn);
        magma_tally4_zgetmatrix( n, n, ddA, n, work, n);
        magma_tally4_zungqr_gpu( m, n, n, dA, ldda, tau, d_T, nb, info );
        // ================== end of ikind == 2 ===================================================
    }
    else if (ikind == 3) {
        // ================== MGS               ===================================================
        for (magma_tally4_int_t j = 0; j < n; j++) {
            for (magma_tally4_int_t i = 0; i < j; i++) {
                *work(i, j) = magma_tally4_zdotc(m, dA(0,i), 1, dA(0,j), 1);
                magma_tally4_zaxpy(m, -(*work(i,j)),  dA(0,i), 1, dA(0,j), 1);
            }
            for (magma_tally4_int_t i = j; i < n; i++)
                *work(i, j) = MAGMA_tally4_Z_ZERO;
            //*work(j,j) = MAGMA_tally4_Z_MAKE( magma_tally4_dznrm2(m, dA(0,j), 1), 0. );
            *work(j,j) = magma_tally4_zdotc(m, dA(0,j), 1, dA(0,j), 1);
            *work(j,j) = MAGMA_tally4_Z_MAKE( sqrt(MAGMA_tally4_Z_REAL( *work(j,j) )), 0.);
            magma_tally4_zscal(m, 1./ *work(j,j), dA(0,j), 1);
        }
        // ================== end of ikind == 3 ===================================================
    }
    else if (ikind == 4) {
        // ================== Cholesky QR       ===================================================
        magma_tally4_zgemm(Magma_tally4ConjTrans, Magma_tally4NoTrans, n, n, m, c_one, dA, ldda, dA, ldda, c_zero, dwork, n );
        magma_tally4_zgetmatrix(n, n, dwork, n, work, n);
        lapackf77_zpotrf("u", &n, work, &n, info);
        magma_tally4_zsetmatrix(n, n, work, n, dwork, n);
        magma_tally4_ztrsm( Magma_tally4Right, Magma_tally4Upper, Magma_tally4NoTrans, Magma_tally4NonUnit, m, n, c_one, dwork, n, dA, ldda);
        // ================== end of ikind == 4 ===================================================
    }
