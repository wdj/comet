...
/**
    ...
    ikind   INTEGER
            ...
            2:  This version uses a standard LAPACK-based orthogonalization through
                MAGMA_tally2's QR panel factorization (magma_tally2_zgeqr2x3_gpu) and magma_tally2_zungqr
            3:  MGS
    ...
    magma_tally2_int_t *info )
{
    ...
    *info = 0;
    ...
    if (ikind == 1) {
        ...
        magma_tally2DoubleComplex *U, *VT, *vt, *R, *G, *hwork, *tau;
        double *S;
        ...
        magma_tally2_zmalloc_cpu( &hwork, 32 + 2*n*n + 2*n);
        if ( hwork == NULL ) {
        ...
        double *rwork;
        magma_tally2_dmalloc_cpu( &rwork, 5*n);
        if ( rwork == NULL ) {
            *info = MAGMA_tally2_ERR_HOST_ALLOC;
            return *info;
        }
        ...
        do {
            ...
            magma_tally2_zgemm(Magma_tally2ConjTrans, Magma_tally2NoTrans, n, n, m, c_one, dA, ldda, dA, ldda, c_zero, dwork, n );
            magma_tally2_zgetmatrix(n, n, dwork, n, G, n);
            ...
            magma_tally2_zsetmatrix(n, n, VT, n, dwork, n);
            magma_tally2_ztrsm( Magma_tally2Right, Magma_tally2Upper, Magma_tally2NoTrans, Magma_tally2NonUnit, m, n, c_one, dwork, n, dA, ldda);
            if (mins > 0.00001f)
                cn = maxs/mins;
        ...
        magma_tally2DoubleComplex_ptr dtau = dwork + 2*n*n;
        magma_tally2DoubleComplex_ptr d_T  = dwork;
        magma_tally2DoubleComplex_ptr ddA  = dwork + n*n;
        magma_tally2DoubleComplex *tau  = work+n*n;

        magma_tally2blas_zlaset( Magma_tally2Full, n, n, c_zero, c_zero, d_T, n );
        magma_tally2_zgeqr2x3_gpu(m, n, dA, ldda, dtau, d_T, ddA,
                           (double *)(dwork+min_mn+2*n*n), info);
        magma_tally2_zgetmatrix( min_mn, 1, dtau, min_mn, tau, min_mn);
        magma_tally2_zgetmatrix( n, n, ddA, n, work, n);
        magma_tally2_zungqr_gpu( m, n, n, dA, ldda, tau, d_T, nb, info );
        // ================== end of ikind == 2 ===================================================
    }
    else if (ikind == 3) {
        // ================== MGS               ===================================================
        for (magma_tally2_int_t j = 0; j < n; j++) {
            for (magma_tally2_int_t i = 0; i < j; i++) {
                *work(i, j) = magma_tally2_zdotc(m, dA(0,i), 1, dA(0,j), 1);
                magma_tally2_zaxpy(m, -(*work(i,j)),  dA(0,i), 1, dA(0,j), 1);
            }
            for (magma_tally2_int_t i = j; i < n; i++)
                *work(i, j) = MAGMA_tally2_Z_ZERO;
            //*work(j,j) = MAGMA_tally2_Z_MAKE( magma_tally2_dznrm2(m, dA(0,j), 1), 0. );
            *work(j,j) = magma_tally2_zdotc(m, dA(0,j), 1, dA(0,j), 1);
            *work(j,j) = MAGMA_tally2_Z_MAKE( sqrt(MAGMA_tally2_Z_REAL( *work(j,j) )), 0.);
            magma_tally2_zscal(m, 1./ *work(j,j), dA(0,j), 1);
        }
        // ================== end of ikind == 3 ===================================================
    }
    else if (ikind == 4) {
        // ================== Cholesky QR       ===================================================
        magma_tally2_zgemm(Magma_tally2ConjTrans, Magma_tally2NoTrans, n, n, m, c_one, dA, ldda, dA, ldda, c_zero, dwork, n );
        magma_tally2_zgetmatrix(n, n, dwork, n, work, n);
        lapackf77_zpotrf("u", &n, work, &n, info);
        magma_tally2_zsetmatrix(n, n, work, n, dwork, n);
        magma_tally2_ztrsm( Magma_tally2Right, Magma_tally2Upper, Magma_tally2NoTrans, Magma_tally2NonUnit, m, n, c_one, dwork, n, dA, ldda);
        // ================== end of ikind == 4 ===================================================
    }
