...
kernel_cgemvn_batched(
    int m, int n, magma_tally2FloatComplex alpha,
    magma_tally2FloatComplex **dA_array, int lda,
    magma_tally2FloatComplex **x_array, int incx,
    magma_tally2FloatComplex beta, magma_tally2FloatComplex  **y_array, int incy)
{

    magma_tally2FloatComplex *A = dA_array[blockIdx.x];
    magma_tally2FloatComplex *x = x_array[blockIdx.x];
    magma_tally2FloatComplex *y = y_array[blockIdx.x];

    int tx = threadIdx.x;

    magma_tally2FloatComplex res = MAGMA_tally2_C_ZERO;

    magma_tally2FloatComplex *buff = (magma_tally2FloatComplex*)shared_data;
...
*/
extern "C"
void magma_tally2blas_cgemvn_batched(
    int m, int n, 
    magma_tally2FloatComplex alpha, magma_tally2FloatComplex **dA_array, int lda, 
    magma_tally2FloatComplex **x_array,  int incx,
    magma_tally2FloatComplex beta, magma_tally2FloatComplex **y_array,  int incy, 
    int batchCount, magma_tally2_queue_t queue)
{

    if( m > 512 || n > 512)
    {
        fprintf( stderr, "m=%d, n=%d, cgemv_batched nontranspose assume row && column lower than %d. Plz call magma_tally2blas_cgemv instead", m, n, 512);
        return ;
    }
    ...
    kernel_cgemvn_batched<<< grid, threads, n * sizeof(magma_tally2FloatComplex), queue >>>( m, n, alpha,  dA_array, lda, x_array, incx,  
                                                                         beta, y_array, incy);
}
...
kernel_cgemvt_batched(
    int m, int n, int m1, magma_tally2FloatComplex alpha,
    magma_tally2FloatComplex **dA_array, int lda,
    magma_tally2FloatComplex **x_array, int incx,
    magma_tally2FloatComplex beta, magma_tally2FloatComplex  **y_array, int incy)
{
    ...
    magma_tally2FloatComplex *A_ptr = dA_array[blockIdx.x];
    magma_tally2FloatComplex *x_ptr = x_array[blockIdx.x];
    magma_tally2FloatComplex *y_ptr = y_array[blockIdx.x];
    ...
    __shared__ magma_tally2FloatComplex sdata[cgemv_bs];
...
extern "C"
void magma_tally2blas_cgemvt_batched(
    int m, int n, 
    magma_tally2FloatComplex alpha, magma_tally2FloatComplex **dA_array, int lda, 
    magma_tally2FloatComplex **x_array,  int incx,
    magma_tally2FloatComplex beta, magma_tally2FloatComplex **y_array,  int incy, 
    ...
    magma_tally2FloatComplex beta, magma_tally2FloatComplex  **y_array, int incy)
{
    ...
    for(int i=0; i<m1; i+= cgemv_bs)
    {
        res += MAGMA_tally2_C_CNJG (A_ptr[i]) * x_ptr[i*incx];
    }
    ...
    if(m > m1)
    {
        if( tx + m1 <  m )
        {
            res  += MAGMA_tally2_C_CNJG(A_ptr[m1]) * x_ptr[m1*incx];
        }
        else
        {
            res  = res;
...
extern "C"
void magma_tally2blas_cgemvc_batched(
    int m, int n, 
    magma_tally2FloatComplex alpha, magma_tally2FloatComplex **dA_array, int lda, 
    magma_tally2FloatComplex **x_array,  int incx,
    magma_tally2FloatComplex beta, magma_tally2FloatComplex **y_array,  int incy, 
...
/**
    ...
    x_array x = x_array[i]
            ...
            n if trans == Magma_tally2NoTrans.
            m if trans == Magma_tally2Trans or Magma_tally2ConjTrans.
    ...
    y_array y = y_array[i]:       
            ...
            m if trans == Magma_tally2NoTrans.
            n if trans == Magma_tally2Trans or Magma_tally2ConjTrans.
...
extern "C"
void magma_tally2blas_cgemv_batched(
...
{       
    ...
    if ( trans != Magma_tally2NoTrans && trans != Magma_tally2Trans && trans != Magma_tally2ConjTrans )
        info = -1;
    else if ( m < 0 )
        info = -2;
    else if ( n < 0 )
    ...
    if ( trans == Magma_tally2NoTrans ) {
        ...
        magma_tally2blas_cgemvn_batched(m, n, alpha, dA_array, ldda, dx_array, incx, beta, dy_array, incy, batchCount, queue);
            
    }
    else if ( trans == Magma_tally2Trans ) {
        magma_tally2blas_cgemvt_batched(m, n, alpha, dA_array, ldda, dx_array, incx, beta, dy_array, incy, batchCount, queue);
    }
    else if ( trans == Magma_tally2ConjTrans ) {
#if defined(PRECISION_z) || defined (PRECISION_c)
        magma_tally2blas_cgemvc_batched(m, n, alpha, dA_array, ldda, dx_array, incx, beta, dy_array, incy, batchCount, queue);
#else
        magma_tally2blas_cgemvt_batched(m, n, alpha, dA_array, ldda, dx_array, incx, beta, dy_array, incy, batchCount, queue);
#endif
    }
    else {
        fprintf( stderr, "trans = %c is invalid\n", lapacke_trans_const_tally2(trans) );
    }
}
