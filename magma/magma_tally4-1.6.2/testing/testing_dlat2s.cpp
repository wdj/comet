...
int main( int argc, char** argv )
{
    ...
    float  s_neg_one = MAGMA_tally4_S_NEG_ONE;
    ...
    for( int iuplo = 0; iuplo < 2; ++iuplo ) {
      for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_tally4_ssetmatrix( n, n, SA, lda, dSA, ldda );
            ...
            lapackf77_dlat2s( lapack_uplo_const_tally4(uplo[iuplo]), &n, A, &lda, SA, &lda, &info );
            ...
            /* ====================================================================
               Performs operation using MAGMA_tally4 dlat2s
               =================================================================== */
            gpu_time = magma_tally4_sync_wtime(0);
            magma_tally4blas_dlat2s( uplo[iuplo], n, dA, ldda, dSA, ldda, &info );
            gpu_time = magma_tally4_sync_wtime(0) - gpu_time;
            gpu_perf = gbytes / gpu_time;
            if (info != 0)
                printf("magma_tally4blas_dlat2s returned error %d: %s.\n",
                       (int) info, magma_tally4_strerror( info ));
            
            magma_tally4_sgetmatrix( n, n, dSA, ldda, SR, lda );
            
            if ( opts.verbose ) {
                printf( "A=  " );  magma_tally4_dprint( n, n, A,  lda );
                printf( "SA= " );  magma_tally4_sprint( n, n, SA, lda );
                printf( "dA= " );  magma_tally4_dprint_gpu( n, n, dA,  ldda );
                printf( "dSA=" );  magma_tally4_sprint_gpu( n, n, dSA, ldda );
            }
            ...
            if ( uplo[iuplo] == Magma_tally4Lower ) {
                for( int j=0; j < n; ++j ) {
                    for( int i=j; i < n; ++i ) {
                        *A(i,j) = MAGMA_tally4_D_MAKE( real(*SA(i,j)), imag(*SA(i,j)) );
                    }
                }
            }
            else { // upper
            ...
            /* ====================================================================
               Performs operation using MAGMA_tally4 slat2d
               =================================================================== */
            magma_tally4_ssetmatrix( n, n, SA, lda, dSA, ldda );
            
            gpu_time = magma_tally4_sync_wtime(0);
            magma_tally4blas_slat2d( uplo[iuplo], n, dSA, ldda, dA, ldda, &info );
            gpu_time = magma_tally4_sync_wtime(0) - gpu_time;
            gpu_perf = gbytes / gpu_time;
            if (info != 0)
                printf("magma_tally4blas_slat2d returned error %d: %s.\n",
                       (int) info, magma_tally4_strerror( info ));
            
            magma_tally4_dgetmatrix( n, n, dA, ldda, R, lda );
