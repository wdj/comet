...
int main( int argc, char** argv)
{
    ...
    if ( arch < 200 ) {
        ...
        for( int iuplo = 0; iuplo < 2; ++iuplo ) {
            printf( "Testing that magma_minproductblas_slansy( %s, %s, ... ) returns -1 error...\n",
                    lapack_norm_const( norm[inorm] ),
                    lapack_uplo_const( uplo[iuplo] ));
            norm_magma_minproduct = magma_minproductblas_slansy( norm[inorm], uplo[iuplo], 1, NULL, 1, NULL );
            if ( norm_magma_minproduct != -1 ) {
                printf( "expected magma_minproductblas_slansy to return -1 error, but got %f\n", norm_magma_minproduct );
                status = 1;
            }
        }}
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int iuplo = 0; iuplo < 2; ++iuplo ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            norm_magma_minproduct = magma_minproductblas_slansy( norm[inorm], uplo[iuplo], N, d_A, ldda, d_work );
            ...
            if (norm_magma_minproduct < 0)
                printf("magma_minproductblas_slansy returned error %f: %s.\n",
