...
int main( int argc, char** argv )
{
    ...
    magma_minproductDoubleComplex *hA, *hR, *hT, *tau, *h_work;
    magma_minproductDoubleComplex_ptr dA, dT;
    magma_minproduct_int_t m, n, k;
    magma_minproduct_int_t n2, lda, ldda, lwork, min_mn, nb, info;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            // okay that magma_minproduct_zgeqrf_gpu has special structure for R; R isn't used here.
            magma_minproduct_zsetmatrix( m, n, hA, lda, dA, ldda );
            magma_minproduct_zgeqrf_gpu( m, n, dA, ldda, tau, dT, &info );
            if ( info != 0 )
                printf("magma_minproduct_zgeqrf_gpu returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
            magma_minproduct_zgetmatrix( m, n, dA, ldda, hA, lda );
            lapackf77_zlacpy( Magma_minproductFullStr, &m, &n, hA, &lda, hR, &lda );
            magma_minproduct_zgetmatrix( nb, min_mn, dT, nb, hT, nb );
            
            gpu_time = magma_minproduct_wtime();
            magma_minproduct_zungqr_m( m, n, k, hR, lda, tau, hT, nb, &info );
            gpu_time = magma_minproduct_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if ( info != 0 )
                printf("magma_minproduct_zungqr_gpu returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
            ...
            if ( opts.lapack ) {
                ...
                // compute relative error |R|/|A| := |Q_magma_minproduct - Q_lapack|/|A|
