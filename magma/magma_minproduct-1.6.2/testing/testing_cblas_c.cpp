/*
    -- MAGMA_minproduct (version 1.6.1) --
       ...
       @generated from testing_cblas_z.cpp normal z -> c, Fri Jan 30 19:00:23 2015
       @author Mark Gates
       
       These tests ensure that the MAGMA_minproduct wrappers around (CPU) CBLAS calls are
       correct.
       This is derived from the testing_blas_c.cpp code that checks MAGMA_minproduct's
...
// hence why MAGMA_minproduct does not rely on them.
...
float blasf77_scasum( const magma_minproduct_int_t* n,
                       const magma_minproductFloatComplex* x, const magma_minproduct_int_t* incx );

float blasf77_scnrm2( const magma_minproduct_int_t* n,
                       const magma_minproductFloatComplex* x, const magma_minproduct_int_t* incx );

magma_minproductFloatComplex blasf77_cdotc( const magma_minproduct_int_t* n,
                                  const magma_minproductFloatComplex* x, const magma_minproduct_int_t* incx,
                                  const magma_minproductFloatComplex* y, const magma_minproduct_int_t* incy );

magma_minproductFloatComplex blasf77_cdotu( const magma_minproduct_int_t* n,
                                  const magma_minproductFloatComplex* x, const magma_minproduct_int_t* incx,
                                  const magma_minproductFloatComplex* y, const magma_minproduct_int_t* incy );
...
int main( int argc, char** argv )
{
    ...
    magma_minproductFloatComplex  *A, *B;
    float diff, error;
    magma_minproduct_int_t ISEED[4] = {0,0,0,1};
    magma_minproduct_int_t m, n, k, size, maxn, ld;
    magma_minproductFloatComplex x2_m, x2_c;  // complex x for magma_minproduct, cblas/fortran blas respectively
    float x_m, x_c;  // x for magma_minproduct, cblas/fortran blas respectively
    ...
    printf( "!! Calling these CBLAS and Fortran BLAS sometimes crashes (segfault), which !!\n"
            "!! is why we use wrappers. It does not necesarily indicate a bug in MAGMA_minproduct.  !!\n"
            "\n"
            "Diff  compares MAGMA_minproduct wrapper        to CBLAS and BLAS function; should be exactly 0.\n"
            "Error compares MAGMA_minproduct implementation to CBLAS and BLAS function; should be ~ machine epsilon.\n"
            "\n" );
    ...
    for( int iincx = 0; iincx < ninc; ++iincx ) {
        magma_minproduct_int_t incx = inc[iincx];
        
    for( int iincy = 0; iincy < ninc; ++iincy ) {
        magma_minproduct_int_t incy = inc[iincy];
        
        printf("=========================================================================\n");
        printf( "m=%d, n=%d, k=%d, incx = %d, incy = %d\n",
                (int) m, (int) n, (int) k, (int) incx, (int) incy );
        printf( "Function              MAGMA_minproduct     CBLAS     BLAS        Diff      Error\n"
                "                      msec      msec      msec\n" );
        ...
        ld = max( 1, maxn );
        size = ld*maxn;
        magma_minproduct_cmalloc_pinned( &A,  size );  assert( A   != NULL );
        magma_minproduct_cmalloc_pinned( &B,  size );  assert( B   != NULL );
        ...
        if ( incx > 0 && incx == incy ) {  // positive, no incy
            ...
            error = 0;
            for( int j = 0; j < k; ++j ) {
                x_m = magma_minproduct_cblas_scasum( m, A(0,j), incx );
        ...
        if ( incx > 0 && incx == incy ) {  // positive, no incy
            ...
            error = 0;
            for( int j = 0; j < k; ++j ) {
                x_m = magma_minproduct_cblas_scnrm2( m, A(0,j), incx );
        ...
        error = 0;
        for( int j = 0; j < k; ++j ) {
            // MAGMA_minproduct implementation, not just wrapper
            x2_m = magma_minproduct_cblas_cdotc( m, A(0,j), incx, B(0,j), incy );
        ...
        for( int j = 0; j < k; ++j ) {
            // MAGMA_minproduct implementation, not just wrapper
            x2_m = magma_minproduct_cblas_cdotu( m, A(0,j), incx, B(0,j), incy );
    ...
    printf( "sum diffs  = %8.2g, MAGMA_minproduct wrapper        compared to CBLAS and Fortran BLAS; should be exactly 0.\n"
            "sum errors = %8.2e, MAGMA_minproduct implementation compared to CBLAS and Fortran BLAS; should be ~ machine epsilon.\n\n",
