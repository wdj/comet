...
/**
    ...
    dA      COMPLEX array, dimension (LDDA, N)
            ...
            On exit, the lower triangle (if UPLO=Magma_tally3Lower) or the upper
            triangle (if UPLO=Magma_tally3Upper) of A, including the diagonal, is
            destroyed.
...
*********   (workspace) If FAST_HEMV is defined DZ should be (LDDZ, max(1,N)) in both cases.
    ...
    lddz    INTEGER
            ...
            JOBZ = Magma_tally3Vec, LDDZ >= max(1,N).
    ...
    ldwz    INTEGER
            ...
            JOBZ = Magma_tally3Vec, LDWZ >= max(1,N).
    ...
    ifail   INTEGER array, dimension (N)
            If JOBZ = Magma_tally3Vec, then if INFO = 0, the first M elements of
            IFAIL are zero.  If INFO > 0, then IFAIL contains the
            indices of the eigenvectors that failed to converge.
            If JOBZ = Magma_tally3NoVec, then IFAIL is not referenced.
...
magma_tally3_cheevx_gpu(
    ...
    float *rwork, magma_tally3_int_t *iwork, magma_tally3_int_t *ifail,
    magma_tally3_int_t *info)
{
    const char* uplo_  = lapack_uplo_const_tally3( uplo  );
    const char* jobz_  = lapack_vec_const_tally3( jobz  );
    const char* range_ = lapack_range_const_tally3( range );
    
    magma_tally3_int_t ione = 1;
    
    const char* order_;
    magma_tally3_int_t indd, inde;
    magma_tally3_int_t imax;
    magma_tally3_int_t lopt, itmp1, indee;
    magma_tally3_int_t lower, wantz;
    magma_tally3_int_t i, j, jj, i__1;
    magma_tally3_int_t alleig, valeig, indeig;
    magma_tally3_int_t iscale, indibl;
    magma_tally3_int_t indiwk, indisp, indtau;
    magma_tally3_int_t indrwk, indwrk;
    magma_tally3_int_t llwork, nsplit;
    ...
    *info = 0;
    ...
    work[0] = MAGMA_tally3_C_MAKE( lopt, 0 );
    ...
    *m = 0;
    ...
    if (n <= 128) {
        ...
        magma_tally3_cmalloc_cpu( &a, n*n );
        magma_tally3_cgetmatrix(n, n, dA, ldda, a, n);
        lapackf77_cheevx(jobz_, range_, uplo_,
                         &n, a, &n, &vl, &vu, &il, &iu, &abstol, m,
                         w, wZ, &ldwz, work, &lwork,
                         rwork, iwork, ifail, info);
        magma_tally3_csetmatrix( n,  n,  a,    n, dA, ldda);
        magma_tally3_csetmatrix( n, *m, wZ, ldwz, dZ, lddz);
        magma_tally3_free_cpu(a);
        return *info;
    }

    if (MAGMA_tally3_SUCCESS != magma_tally3_smalloc( &dwork, n )) {
        fprintf (stderr, "!!!! device memory allocation error (magma_tally3_cheevx_gpu)\n");
    ...
    anrm = magma_tally3blas_clanhe(Magma_tally3MaxNorm, uplo, n, dA, ldda, dwork);
    iscale = 0;
    sigma  = 1;
    if (anrm > 0. && anrm < rmin) {
        iscale = 1;
    ...
    if (iscale == 1) {
        d__1 = 1.;
        magma_tally3blas_clascl(uplo, 0, 0, 1., sigma, n, n, dA,
                         ldda, info);
...
#ifdef FAST_HEMV
    magma_tally3_chetrd2_gpu(uplo, n, dA, ldda, &rwork[indd], &rwork[inde],
                      &work[indtau], wA, ldwa, &work[indwrk], llwork, dZ, lddz*n, &iinfo);
#else
    magma_tally3_chetrd_gpu (uplo, n, dA, ldda, &rwork[indd], &rwork[inde],
                      &work[indtau], wA, ldwa, &work[indwrk], llwork, &iinfo);
#endif

    lopt = n + (magma_tally3_int_t)MAGMA_tally3_C_REAL(work[indwrk]);
    ...
    if ((alleig || (indeig && il == 1 && iu == n)) && abstol <= 0.) {
        ...
        if (! wantz) {
            ...
            if (*info == 0) {
                ...
                magma_tally3_csetmatrix( n, n, wZ, ldwz, dZ, lddz );
            }
        }
        if (*info == 0) {
            *m = n;
    ...
    if (*m == 0) {
        *info = 0;
        ...
        if (wantz) {
            ...
            magma_tally3_cunmtr_gpu(Magma_tally3Left, uplo, Magma_tally3NoTrans, n, *m, dA, ldda, &work[indtau],
    ...
    if (wantz) {
        for (j = 1; j <= *m-1; ++j) {
            ...
            if (i != 0) {
                ...
                magma_tally3_cswap(n, dZ + (i-1)*lddz, ione, dZ + (j-1)*lddz, ione);
                if (*info != 0) {
                    itmp1 = ifail[i];
                    ifail[i] = ifail[j];
                    ifail[j] = itmp1;
    ...
    work[1] = MAGMA_tally3_C_MAKE( lopt, 0 );
