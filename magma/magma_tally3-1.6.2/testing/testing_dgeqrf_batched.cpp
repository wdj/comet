...
void get_QR_error(magma_tally3_int_t M, magma_tally3_int_t N, magma_tally3_int_t min_mn,
                    double *h_R,  double *h_A, magma_tally3_int_t lda,
                    double *tau,
                    double *Q,  magma_tally3_int_t ldq,
                    double *R,  magma_tally3_int_t ldr,
                    double *h_work,  magma_tally3_int_t lwork,
...
int main( int argc, char** argv)
{
    ...
    double *h_A, *h_R, *h_Amagma_tally3, *tau, *h_work, tmp[1];
    double *d_A, *dtau_magma_tally3, *dtau_cublas;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_DEV( dtau_magma_tally3,  double, min_mn * batchCount);
			...
			lapackf77_dlacpy( Magma_tally3UpperLowerStr, &M, &column, h_A, &lda, h_R, &lda );
       
            /* ====================================================================
               Performs operation using MAGMA_tally3
               =================================================================== */
            magma_tally3_dsetmatrix( M, column, h_R, lda,  d_A, ldda );
            dset_pointer(dA_array, d_A, 1, 0, 0, ldda*N, batchCount, opts.queue);
            dset_pointer(dtau_array, dtau_magma_tally3, 1, 0, 0, min_mn, batchCount, opts.queue);
    
            magma_tally3_time = magma_tally3_sync_wtime(0);
    
            info = magma_tally3_dgeqrf_batched(M, N, dA_array, ldda, dtau_array, dinfo_magma_tally3, batchCount, opts.queue);

            magma_tally3_time = magma_tally3_sync_wtime(0) - magma_tally3_time;
            magma_tally3_perf = gflops / magma_tally3_time;

            magma_tally3_dgetmatrix( M, column, d_A, ldda, h_Amagma_tally3, lda);

            if (info != 0)
                printf("magma_tally3_dgeqrf_batched returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                magma_tally3_dgetvector(min_mn*batchCount, dtau_magma_tally3, 1, tau, 1);
                for (int i=0; i < batchCount; i++)
                {
                ...
                #if CUDA_VERSION >= 6050
                magma_tally3_dgetvector(min_mn*batchCount, dtau_magma_tally3, 1, tau, 1);
                magma_tally3_dgetmatrix( M, column, d_A, ldda, h_A, lda);
                for (int i=0; i < batchCount; i++)
                {
                    get_QR_error(M, N, min_mn,
