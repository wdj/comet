...
magma_tally4_zlahr2_m(
    ...
    struct zgehrd_data_tally4 *data )
{
    ...
    magma_tally4_int_t ldda = data->ldda;
    magma_tally4_int_t ldv  = data->ldv;
    magma_tally4_int_t ldvd = data->ldvd;
    
    magma_tally4_int_t ione = 1;
    
    magma_tally4_int_t d, dki1, dn, nblocks, gblock, lblock, lgid;
    magma_tally4_int_t n_k_i_1, n_k;
    magma_tally4DoubleComplex scale;

    magma_tally4_int_t i;
    magma_tally4DoubleComplex ei = MAGMA_tally4_Z_ZERO;

    magma_tally4_int_t info_data = 0;
    magma_tally4_int_t *info = &info_data;
    if (n < 0) {
        *info = -1;
    } else if (k < 0 || k >= n) {
        *info = -2;
    ...
    for( d = 0; d < ngpu; ++d ) {
        ...
        magma_tally4blasSetKernelStream( data->streams[d] );
        magma_tally4blas_zlaset( Magma_tally4Full, nb, nb, c_zero, c_zero, dV(d,k,0), ldv );
    }
    ...
    for (i = 0; i < nb; ++i) {
        ...
        if (i > 0) {
            ...
            tmp = MAGMA_tally4_Z_NEGATE( tau[i-1] );
        ...
        *A(k+i+1,i) = c_one;
        ...
        for( d = 0; d < ngpu; ++d ) {
            ...
            magma_tally4_zsetvector_async( n_k_i_1,
                                    A(k+i+1,i), 1,
            ...
            magma_tally4blas_zlacpy( Magma_tally4Full, nb, nblocks-lblock,
                              dV (d, d*nb + lblock*nb*ngpu, i), nb*ngpu,
                              dVd(d, 0    + lblock*nb,      i), nb );
            
            // convert global indices (k) to local indices (dk)
            magma_tally4_indices_1D_bcyclic( nb, ngpu, d, k+i+1, n, &dki1, &dn );
            ...
            if ( dn-dki1 > 0 ) {
                magma_tally4_zgemv( Magma_tally4NoTrans, n-k, dn-dki1,
                ...
                magma_tally4_zgetvector_async( n-k,
                                        dY(d, k, i), 1,
        ...
        scale = MAGMA_tally4_Z_NEGATE( tau[i] );
        blasf77_zgemv( "Conj", &n_k_i_1, &i,
                       &scale,  A(k+i+1,0), &lda,
                                A(k+i+1,i), &ione,
                       &c_zero, T(0,i), &ione );
        ...
        *T(i,i) = tau[i];
        ...
        for( d = 0; d < ngpu; ++d ) {
            magma_tally4_setdevice( d );
            magma_tally4_queue_sync( data->streams[d] );
            magma_tally4_indices_1D_bcyclic( nb, ngpu, d, k+i+1, n, &dki1, &dn );
            if ( dn-dki1 > 0 ) {
                // yi = yi + yi{d}
    ...
    *A(k+nb,nb-1) = ei;
    ...
    for( d = 0; d < ngpu; ++d ) {
        ...
        // convert global indices (k) to local indices (dk)
        magma_tally4_indices_1D_bcyclic( nb, ngpu, d, k+1, n, &dki1, &dn );
        ...
        // which are summed in separate loop below
        if ( dn-dki1 > 0 ) {
            magma_tally4_zgemm( Magma_tally4NoTrans, Magma_tally4NoTrans, k, nb, dn-dki1,
            ...
            magma_tally4_zgetmatrix_async( k, nb,
                                    dY(d, 0, 0),  ldda,
    ...
    for( d = 0; d < ngpu; ++d ) {
        magma_tally4_setdevice( d );
        magma_tally4_zsetmatrix_async( n, nb, Y, ldy, dY(d, 0, 0), ldda, data->streams[d] );
        magma_tally4_zsetmatrix_async( nb, nb, T, nb, dTi(d),      nb,   data->streams[d] );
    }
