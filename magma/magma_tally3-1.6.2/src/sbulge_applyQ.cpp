...
extern "C" void magma_tally3_sbulge_applyQ(
    magma_tally3_int_t WANTZ, magma_tally3_side_t SIDE, magma_tally3_int_t NE, magma_tally3_int_t N, magma_tally3_int_t NB,
    magma_tally3_int_t Vblksiz, float *E, magma_tally3_int_t LDE,
    float *V, float *TAU, float *T,
    magma_tally3_int_t *INFO, float *dV, float *dT,
    float *dE, magma_tally3_int_t copytype )
{
    //%===========================
    //%   local variables
    //%===========================
    ...
    *INFO=0;
     ...
     * */
    if (copytype > 0) magma_tally3_ssetmatrix( LDV, blkcnt*Vblksiz, V, LDV, dV, LDV );
    if (copytype > 1) magma_tally3_ssetmatrix( LDT, blkcnt*Vblksiz, T, LDT, dT, LDT );
    if (copytype > 2) magma_tally3_ssetmatrix( N, NE, E, N, dE, N );
    float *dwork;
    //ldwork  = NE;
    LWORK   = 2*N*max(Vblksiz, 64);
    if (MAGMA_tally3_SUCCESS != magma_tally3_smalloc( &dwork, LWORK )) {
    ...
    if (WANTZ == 1) {
        ...
        magma_tally3blas_slaset( Magma_tally3Full, N, N, c_zero, c_one, dE, N );
    }
            ...
            for (bg = nbGblk; bg > 0; bg--) {
                ...
                for (m = rownbm; m > 0; m--) {
                    ...
                    if ((vlen > 0) && (vnb > 0)) {
                        if (WANTZ == 1) {
                            len =  N-colst;
                            magma_tally3_slarfb_gpu( Magma_tally3Left, Magma_tally3NoTrans, Magma_tally3Forward, Magma_tally3Columnwise, vlen, len, vnb, dV(vpos), LDV, dT(tpos), LDT, dE(fst,colst), LDE, dwork, len);
                        } else {
                            magma_tally3_slarfb_gpu( Magma_tally3Left, Magma_tally3NoTrans, Magma_tally3Forward, Magma_tally3Columnwise, vlen, NE, vnb, dV(vpos), LDV, dT(tpos), LDT, dE(fst,0), LDE, dwork, NE);
                        }
                    }
                }
            }
            ...
            for (m = rownbm; m > 0; m--) {
                ...
                for (n = nbgr; n > 0; n--) {
                    ...
                    if ((vlen > 0) && (vnb > 0)) {
                        ...
                        magma_tally3_slarfb_gpu( Magma_tally3Left, Magma_tally3NoTrans, Magma_tally3Forward, Magma_tally3Columnwise, vlen, N1, vnb, dV(vpos), LDV, dT(tpos), LDT, dE(fst,0), LDE, dwork, N1);
                        magma_tally3blasSetKernelStream(stream[1]);
                        magma_tally3_slarfb_gpu( Magma_tally3Left, Magma_tally3NoTrans, Magma_tally3Forward, Magma_tally3Columnwise, vlen, N2, vnb, dV(vpos), LDV, dT(tpos), LDT, dE(fst,N1), LDE, &dwork[N1*Vblksiz], N2);
                        #else
                        magma_tally3_slarfb_gpu( Magma_tally3Left, Magma_tally3NoTrans, Magma_tally3Forward, Magma_tally3Columnwise, vlen, NE, vnb, dV(vpos), LDV, dT(tpos), LDT, dE(fst,0), LDE, dwork, NE);
                        #endif
                    }
            ...
            for (bg =1; bg <= nbGblk; bg++) {
                ...
                for (m = 1; m <= rownbm; m++) {
                    ...
                    if ((vlen > 0) && (vnb > 0)) {
                        #if defined(USESTREAM)
                        magma_tally3blasSetKernelStream(stream[0]);
                        magma_tally3_slarfb_gpu( Magma_tally3Right, Magma_tally3NoTrans, Magma_tally3Forward, Magma_tally3Columnwise, N1, vlen, vnb, dV(vpos), LDV, dT(tpos), LDT, dE(0, fst), LDE, dwork, N1);
                        magma_tally3blasSetKernelStream(stream[1]);
                        magma_tally3_slarfb_gpu( Magma_tally3Right, Magma_tally3NoTrans, Magma_tally3Forward, Magma_tally3Columnwise, N2, vlen, vnb, dV(vpos), LDV, dT(tpos), LDT, dE(N1, fst), LDE, &dwork[N1*Vblksiz], N2);
                        #else
                        magma_tally3_slarfb_gpu( Magma_tally3Right, Magma_tally3NoTrans, Magma_tally3Forward, Magma_tally3Columnwise, NE, vlen, vnb, dV(vpos), LDV, dT(tpos), LDT, dE(0, fst), LDE, dwork, NE);
                        #endif
                    }
                }
            }
