...
/**
    Purpose
    -------
    magma_tally3blas_ssymv_work performs the matrix-vector operation:
    ...
    MAGMA_tally3 implements ssymv through two steps:
    ...
    magma_tally3blas_ssymv is a wrapper routine allocating the workspace inside the
    routine and provides the same interface as cublas.
    
    If users need to call ssymv frequently, we suggest using
    magma_tally3blas_ssymv_work instead of magma_tally3blas_ssymv. As the overhead to
    allocate and free in device memory in magma_tally3blas_ssymv would hurt performance.
    ...
    magma_tally3_queue_t queue )
{
    ...
    if ( arch < 200 ) {
        magma_tally3_ssymv( uplo, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
     ...
     */
    if ( (n == 0) || ( MAGMA_tally3_S_EQUAL(alpha, MAGMA_tally3_S_ZERO) && MAGMA_tally3_S_EQUAL(beta, MAGMA_tally3_S_ONE) ) )
        return info;
...
// end magma_tally3blas_ssymv_work


/**
    Purpose
    -------
    magma_tally3blas_ssymv performs the matrix-vector operation:
    ...
    magma_tally3Float_ptr dy, magma_tally3_int_t incy)
{
    ...
    magma_tally3_smalloc( &dwork, lwork );
    if ( dwork == NULL ) {
    ...
    magma_tally3blas_ssymv_work( uplo, n, alpha, dA, ldda, dx, incx, beta, dy, incy,
