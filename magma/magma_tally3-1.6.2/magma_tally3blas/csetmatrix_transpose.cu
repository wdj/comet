...
#define dwork(i_, j_) (dwork + (i_) + (j_)*lddwork)
    ...
    for(i=nb; i < n; i += nb) {
        ...
        magma_tally3blas_ctranspose_q( m, nb, dwork(0,(j%2)*nb), lddwork, dAT(i-nb,0), ldda, queues[j%2] );
    }

    /* Transpose the last part of the matrix.                            */
    j++;
    magma_tally3blas_ctranspose_q( m, ib, dwork(0,(j%2)*nb), lddwork, dAT(i-nb,0), ldda, queues[j%2] );
}


// @see magma_tally3blas_csetmatrix_transpose_q
extern "C" void 
    ...
    magma_tally3FloatComplex_ptr     dwork, magma_tally3_int_t lddwork, magma_tally3_int_t nb )
{
    ...
    magma_tally3blas_csetmatrix_transpose_q( m, n, hA, lda, dAT, ldda, dwork, lddwork, nb, queues );
