...
int main( int argc, char** argv)
{
    ...
    NB = opts.nb;
    if (NB < 1)
        NB  = 64; //64; //magma_tally3_get_ssytrd_sy2sb_nb(N);

    // what is NE ?
    if (NE < 1)
        NE  = 64; //N;  //magma_tally3_get_ssytrd_sy2sb_nb(N);  // N not yet initialized
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            /*
            magma_tally3_ssytrd_sy2sb( opts.uplo, N, NB, h_R, lda, tau, h_work, lwork0, dT1, THREADS, &info);
            tband = magma_tally3_wtime - gpu_time();
            printf("  Finish BAND  N %d  NB %d  ngpu %d timing= %f\n", N, NB, ngpu, tband);
            magma_tally3_ssytrd_bsy2trc_v5(THREADS, WANTZ, opts.uplo, NE, N, NB, h_R, lda, D, E, dT1, ldt);
            */
    
            /*
            magma_tally3_ssytrd_sy2sb( opts.uplo, N, NB, h_R, lda, tau, h_work, lwork, dT1, THREADS, &info);
            tband = magma_tally3_wtime - gpu_time();
            printf("  Finish BAND  N %d  NB %d  ngpu %d timing= %f\n", N, NB, ngpu, tband);
            magma_tally3_ssytrd_bsy2trc(THREADS, WANTZ, opts.uplo, NE, N, NB, h_R, lda, D, E, dT1, ldt);
            */
            ...
            magma_tally3_int_t threads = magma_tally3_get_parallel_numthreads();
                ...
                lwork  = magma_tally3_sbulge_get_lq2(N, threads) + 2*N + N*N;
                lrwork = 1 + 5*N +2*N*N;
                TESTING_MALLOC_PIN( rwork, float, lrwork );
            #else
                lwork  = magma_tally3_sbulge_get_lq2(N, threads) + 1 + 6*N + 2*N*N;
            #endif
            liwork = 3 + 5*N;
            //nb = magma_tally3_get_ssytrd_nb(N);
            ...
            if (ngpu == 1) {
                ...
                magma_tally3_ssyevdx_2stage( opts.jobz, range, opts.uplo, N,
                                h_R, lda,
                                vl, vu, il, iu,
                                &m1, D,
                                hh_work, lwork,
                ...
                magma_tally3_ssyevdx_2stage_m(ngpu, opts.jobz, range, opts.uplo, N,
                                h_R, lda,
                                vl, vu, il, iu,
                                &m1, D,
                                hh_work, lwork,
            ...
            */
            ...
            if ( opts.check ) {
                ...
                magma_tally3_smalloc_cpu( &work2, lwork2 );
                magma_tally3_smalloc_cpu( &rwork2, N );
                magma_tally3_smalloc_cpu( &D2, N );
                magma_tally3_smalloc_cpu( &AINIT, N*lda );
                ...
                */
                ...
                //float *Z;
                // magma_tally3_smalloc_cpu( &Z, N*lda );
                ...
                // TODO free this memory !
                magma_tally3_smalloc_cpu( &WORKAJETER, (2* N * N + N)  );
