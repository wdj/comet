    ...
    magma_tally2_int_t *ipiv, magma_tally2_int_t *info)
{
    ...
    magma_tally2_int_t nb = magma_tally2_get_chetrf_nb(n);
    ...
    *info = 0;
    ...
    magma_tally2FloatComplex *dA, *dW;
    if ((MAGMA_tally2_SUCCESS != magma_tally2_cmalloc( &dA, n*ldda  )) ||
        (MAGMA_tally2_SUCCESS != magma_tally2_cmalloc( &dW, (1+nb)*ldda ))) {
    ...
    //magma_tally2_csetmatrix_async( n, n, A(0,0), lda, dA(0,0), ldda, stream[0] );
    if ( upper ) {
       for (int k = 0; k < n; k+=nb ) {
           kb = min(nb, n-k);
           magma_tally2_csetmatrix_async( k+kb, kb, A(0,k), lda, dA(0,k), ldda, stream[0] );
       }
    } else {
       for (int k = 0; k < n; k+=nb ) {
           kb = min(nb, n-k);
           magma_tally2_csetmatrix_async( n-k, kb, A(k,k), lda, dA(k,k), ldda, stream[0] );
       }
    }
    trace_gpu_end( 0, 0 );
    ...
    if ( upper ) {
        ...
        for (int k = n-1; k >= 0; k-=kb ) {
            ...
            if ( k+1 > nb ) {
                ...
                magma_tally2_clahef_gpu( Magma_tally2Upper, nk, kb, &kb, A( 0, 0 ), lda, dA( 0, 0 ), ldda, 
                ...
                magma_tally2_cgetmatrix( nk, nk, dA( 0, 0 ),ldda, A( 0, 0 ),lda );
                lapackf77_chetf2( Magma_tally2UpperStr, &nk, A( 0, 0 ), &lda, &ipiv[0], &iinfo );
                kb = k+1;
            }
        ...
        for (int k = 0; k < n; k += kb ) {
            ...
            if ( k < n-nb ) {
                ...
                magma_tally2_clahef_gpu( Magma_tally2Lower, nk, nb, &kb, A( k, k ), lda, dA( k, k ), ldda,
                                  &ipiv[k], dW, ldda, stream, event, &iinfo );

            } else {
                /* Use unblocked code to factorize columns k:n of A */
                magma_tally2_queue_sync( stream[0] );
                magma_tally2_cgetmatrix( nk,nk, dA(k,k),ldda, A(k,k),lda );
                lapackf77_chetf2( Magma_tally2LowerStr, &nk, A( k, k ), &lda, &ipiv[k], &iinfo );
            }
            /* Set INFO on the first occurrence of a zero pivot */
