    ...
    magma_minproductDoubleComplex       * __restrict__ y, int incy)
{
    ...
    __shared__ magma_minproductDoubleComplex sx[BLK_Y];
    ...
    if ( threadIdx.x < npart ) {
        ...
        sx[threadIdx.x] = MAGMA_minproduct_Z_ZERO;
    }
    __syncthreads();
    ...
    magma_minproductDoubleComplex       * __restrict__ y, int incy)
{
    ...
    __shared__ magma_minproductDoubleComplex sdata[BLK_X];
    ...
    magma_minproduct_sum_reduce< BLK_X >( tx, sdata );
...
/**
    ...
    trans   magma_minproduct_trans_t
      ...
      -     = Magma_minproductConjTrans:  y := alpha*A^H*x + beta*y
...
magma_minproductblas_zgemv(
    magma_minproduct_trans_t trans, magma_minproduct_int_t m, magma_minproduct_int_t n, magma_minproductDoubleComplex alpha,
    ...
    magma_minproductDoubleComplex_ptr dy, magma_minproduct_int_t incy)
{
    ...
    if ( arch < 200  ) {
        ...
        magma_minproduct_zgemv( trans, m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
        #else
        magma_minproductblas_zgemv_tesla( trans, m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
        #endif
        return;
    }
    ...
    if ( trans == Magma_minproductNoTrans ) {
        ...
        zgemvn_kernel1_fermi<<< grid, threads, 0, magma_minproduct_stream >>>
            ( m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    }
    else if ( trans == Magma_minproductTrans ) {
        dim3 grid    ( 1, n, 1 );
        dim3 threads ( BLK_X, 1, 1 );
        zgemvt_kernel_fermi<<< grid, threads, 0, magma_minproduct_stream >>>
            ( m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    }
    else if ( trans == Magma_minproductConjTrans ) {
        dim3 grid    ( 1, n, 1 );
        dim3 threads ( BLK_X, 1, 1 );
        zgemvc_kernel_fermi<<< grid, threads, 0, magma_minproduct_stream >>>
            ( m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    }
}
