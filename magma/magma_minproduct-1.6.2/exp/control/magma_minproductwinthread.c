/**
 *
 * @file magma_minproductwinthread.c
 *
 *  This file handles the mapping from pthreads calls to windows threads
 *  MAGMA_minproduct is a software package provided by Univ. of Tennessee,
...
CRITICAL_SECTION magma_minproductwinthread_static_initializer_check_lock;
static int magma_minproductwinthread_initialized = 0;
...
static int pthread_mutex_check_for_static_initialization( pthread_mutex_t *mutex ) {
    ...
    if ( magma_minproductwinthread_initialized == 0 ) {
        magma_minproductwinthread_initialized = 1;
        InitializeCriticalSection( &magma_minproductwinthread_static_initializer_check_lock );
    }
    EnterCriticalSection( &magma_minproductwinthread_static_initializer_check_lock );
    if ( *mutex == PTHREAD_MUTEX_INITIALIZER )
        retval = pthread_mutex_init( mutex, NULL );
    LeaveCriticalSection( &magma_minproductwinthread_static_initializer_check_lock );
    return retval;
}
...
void *(*MAGMA_minproduct_realThStart)(void *);

/*
  This function is only called to have a proxy that is compatible with WINAPI.
 */
unsigned WINAPI MAGMA_minproduct_winThStart(void *arg) {
  MAGMA_minproduct_realThStart( arg );
  return 0;
}

MAGMA_minproduct_DLLPORT int MAGMA_minproduct_CDECL pthread_create(pthread_t *thread, const pthread_attr_t *attr, void *(*start) (void *), void *arg) {
...
MAGMA_minproduct_DLLPORT int MAGMA_minproduct_CDECL pthread_setconcurrency (int level) {
