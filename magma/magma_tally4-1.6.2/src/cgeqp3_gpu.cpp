...
/**
    ...
    @ingroup magma_tally4_cgeqp3_comp
    ********************************************************************/
    ...
    magma_tally4_int_t *info )
{
    ...
    *info = 0;
    ...
    nb = magma_tally4_get_cgeqp3_nb(min(m, n));
    minmn = min(m,n);
    if (*info == 0) {
        if (minmn == 0) {
            lwkopt = 1;
    ...
    #endif
    magma_tally4FloatComplex_ptr df;
    if (MAGMA_tally4_SUCCESS != magma_tally4_cmalloc( &df, (n+1)*nb )) {
        *info = MAGMA_tally4_ERR_DEVICE_ALLOC;
        return *info;
    }
    cudaMemset( df, 0, (n+1)*nb*sizeof(magma_tally4FloatComplex) );
    ...
    if (nfxd > 0) {
        ...
        if (na < n) {
            n_j = n - na;
            lapackf77_cunmqr( Magma_tally4LeftStr, Magma_tally4ConjTransStr, &m, &n_j, &na,
    ...
    if (nfxd < minmn) {
        ...
        magma_tally4blas_scnrm2_cols(sm, sn, dA(nfxd,nfxd), ldda, &rwork[nfxd]);
        ...
        //if (nb < sminmn)
        {
            ...
            while(j < topbmn) {
                ...
                //magma_tally4_claqps_gpu    // this is a cpp-file
                magma_tally4_claqps2_gpu   // this is a cuda-file
                    ( m, n_j, j, jb, &fjb,
                      dA(0, j), ldda,
        ...
        if (j < minmn) {
            n_j = n - j;
            if (j > nfxd) {
                magma_tally4_cgetmatrix( m-j, n_j,
                                  dA(j,j), ldda,
                                   A(j,j), lda );
            }
