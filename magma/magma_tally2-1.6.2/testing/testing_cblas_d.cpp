/*
    -- MAGMA_tally2 (version 1.6.1) --
       ...
       correct.
       This is derived from the testing_blas_d.cpp code that checks MAGMA_tally2's
...
double blasf77_dasum( const magma_tally2_int_t* n,
                       const double* x, const magma_tally2_int_t* incx );

double blasf77_dnrm2( const magma_tally2_int_t* n,
                       const double* x, const magma_tally2_int_t* incx );

double blasf77_ddot( const magma_tally2_int_t* n,
                                  const double* x, const magma_tally2_int_t* incx,
...
int main( int argc, char** argv )
{
    ...
    double x2_m, x2_c;  // real x for magma_tally2, cblas/fortran blas respectively
    ...
    for( int iincy = 0; iincy < ninc; ++iincy ) {
        ...
        ld = max( 1, maxn );
        size = ld*maxn;
        magma_tally2_dmalloc_pinned( &A,  size );  assert( A   != NULL );
        magma_tally2_dmalloc_pinned( &B,  size );  assert( B   != NULL );
        ...
        if ( incx > 0 && incx == incy ) {  // positive, no incy
            ...
            error = 0;
            for( int j = 0; j < k; ++j ) {
                x_m = magma_tally2_cblas_dasum( m, A(0,j), incx );
        ...
        if ( incx > 0 && incx == incy ) {  // positive, no incy
            ...
            error = 0;
            for( int j = 0; j < k; ++j ) {
                x_m = magma_tally2_cblas_dnrm2( m, A(0,j), incx );
        ...
        for( int j = 0; j < k; ++j ) {
            ...
            x2_m = magma_tally2_cblas_ddot( m, A(0,j), incx, B(0,j), incy );
