...
/**
    ...
    dA      REAL array on the GPU,
            ...
            On exit, if JOBZ = Magma_minproductVec, then if INFO = 0, the first mout columns
    ...
    magma_minproduct_int_t *info)
{
    ...
    *info = 0;
    ...
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_smalloc( &dwork, ldwork )) {
    ...
    anrm = magma_minproductblas_slansy( Magma_minproductMaxNorm, uplo, n, dA, ldda, dwork );
    iscale = 0;
    sigma  = 1;
    if (anrm > 0. && anrm < rmin) {
        iscale = 1;
    ...
    if (iscale == 1) {
        magma_minproductblas_slascl( uplo, 0, 0, 1., sigma, n, n, dA, ldda, info );
    }
...
#ifdef FAST_SYMV
    magma_minproduct_ssytrd2_gpu( uplo, n, dA, ldda, w, &work[inde],
                       &work[indtau], wA, ldwa, &work[indwrk], llwork,
                       dwork, ldwork, &iinfo );
#else
    magma_minproduct_ssytrd_gpu(  uplo, n, dA, ldda, w, &work[inde],
                       &work[indtau], wA, ldwa, &work[indwrk], llwork,
                       &iinfo );
#endif
    ...
    if (! wantz) {
        ...
        magma_minproduct_smove_eig( range, n, w, &il, &iu, vl, vu, mout );
    }
    else {
        timer_start( time );
        ...
        magma_minproduct_ssetmatrix( n, *mout, &work[indwrk + n* (il-1) ], n, dwork, lddc );

        magma_minproduct_sormtr_gpu( Magma_minproductLeft, uplo, Magma_minproductNoTrans, n, *mout, dA, ldda, &work[indtau],
                          dwork, lddc, wA, ldwa, &iinfo );

        magma_minproduct_scopymatrix( n, *mout, dwork, lddc, dA, ldda );
