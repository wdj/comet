...
/* ////////////////////////////////////////////////////////////////////////////
   -- Testing magma_tally2_ssymm_mgpu
*/
int main( int argc, char** argv)
{
    ...
    float calpha    = MAGMA_tally2_S_MAKE( 3.456, 5.678 );
    float cbeta     = MAGMA_tally2_S_MAKE( 1.234, 2.456 );
    ...
    float *hA, *hX, *hB, *hR;
    magma_tally2Float_ptr dA[Magma_tally2MaxGPUs], dX[Magma_tally2MaxGPUs], dB[Magma_tally2MaxGPUs], dwork[Magma_tally2MaxGPUs], hwork[Magma_tally2MaxGPUs+1];
    magma_tally2Float_ptr dA2;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int offset = 0; offset < N; offset += min(N,nb) ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            ldda  = ((M + 31)/32)*32;
            size  = lda*M;
            gflops = FLOPS_SSYMM( Magma_tally2Left, (float)msize, (float)N ) / 1e9;
            ...
            magma_tally2_smake_symmetric( M, hA, lda );
            ...
            /* ====================================================================
               ...
               =================================================================== */
            magma_tally2_ssetmatrix_1D_col_bcyclic( M, M, hA, lda, dA, ldda, opts.ngpu, nb );
            for( int d = 0; d < opts.ngpu; ++d ) {
                magma_tally2_setdevice( d );
                //magma_tally2blasSetKernelStream( streams[ d ][  0 ] );
                magma_tally2_ssetmatrix( M, N, hX, lda, dX[d], ldda );
                //if (d == 0) magma_tally2_ssetmatrix( M, N, hB, lda, dB[d], ldda ); // this is wrong coz when offset != 0 the gpu who do the beta*C may be not 0 so this should be related to stdev(starting device who own i=0 first col)
                magma_tally2_ssetmatrix( M, N, hB, lda, dB[d], ldda );
            }
            ...
            if ( opts.check && iter == 0 ) {
                ...
                magma_tally2_ssetmatrix( M, M, hA, lda, dA2, ldda );
                magma_tally2_ssetmatrix( M, N, hX, lda, dX[0], ldda );
                magma_tally2_ssetmatrix( M, N, hB, lda, dwork[0], ldda );
            ...
            if ( opts.check ) {
                ...
                //printf( "A =" ); magma_tally2_sprint( M, M, hA, lda );
                //printf( "X =" ); magma_tally2_sprint( M, N, hX, lda );
                //printf( "B =" ); magma_tally2_sprint( M, N, hB, lda );
                ...
                */
                ...
                for(magma_tally2_int_t dev=0; dev < opts.ngpu; ++dev) {
                    ...
                    magma_tally2_sgetmatrix( M, N, dB[dev], ldda, hR, lda );
                    ...
                    //printf( "R ="  ); magma_tally2_sprint( M, N, hR, lda );
                    if (firstprint == 0) {
