...
//#define ENABLE_TIMER3
    ...
    tall   = magma_tally2_sync_wtime(0);
    talloc = magma_tally2_sync_wtime(0);
#endif

    double neg_one = MAGMA_tally2_D_NEG_ONE;
    double one  = MAGMA_tally2_D_ONE;
    magma_tally2_int_t ib, i, k, pm;
    magma_tally2_int_t nb = BATRF_NB;
    magma_tally2_int_t gemm_crossover = nb > 32 ? 127 : 160;
    // magma_tally2_int_t gemm_crossover = n;// use only stream gemm
    ...
    magma_tally2_int_t **pivinfo_array    = NULL;
    magma_tally2_int_t *pivinfo           = NULL; 
    double* dinvA      = NULL;
    double* dwork      = NULL;// dinvA and dwork are workspace in dtrsm
    double **cpuAarray = NULL;
    magma_tally2_dmalloc( &dinvA, invA_msize * batchCount);
    magma_tally2_dmalloc( &dwork, dwork_msize * batchCount );
    magma_tally2_malloc((void**)&pivinfo_array, batchCount * sizeof(*pivinfo_array));
    magma_tally2_malloc((void**)&pivinfo, batchCount * m * sizeof(magma_tally2_int_t));
    magma_tally2_malloc_cpu((void**) &cpuAarray, batchCount*sizeof(double*));
   ...
   /* check allocation */
    if ( dA_displ  == NULL || dW0_displ == NULL || dW1_displ   == NULL || dW2_displ   == NULL || 
        ...
        magma_tally2_free(pivinfo_array);
        magma_tally2_free(pivinfo);
    ...
    magma_tally2blas_dlaset_q(Magma_tally2Full, invA_msize, batchCount, MAGMA_tally2_D_ZERO, MAGMA_tally2_D_ZERO, dinvA, invA_msize, queue);
    magma_tally2blas_dlaset_q(Magma_tally2Full, dwork_msize, batchCount, MAGMA_tally2_D_ZERO, MAGMA_tally2_D_ZERO, dwork, dwork_msize, queue);
    ...
    magma_tally2_getvector( batchCount, sizeof(double*), dA_array, 1, cpuAarray, 1);



#if defined(ENABLE_TIMER3)
    printf(" I am after malloc\n");
    talloc = magma_tally2_sync_wtime(0) - talloc;
    tloop  = magma_tally2_sync_wtime(0);
#endif
    ...
    for(i = 0; i < min_mn; i+=nb) 
    {
        ...
        ib = min(nb, min_mn-i);
        pm = m-i;
        magma_tally2_idisplace_pointers(dipiv_displ, ipiv_array, ldda, i, 0, batchCount, queue);
        magma_tally2_ddisplace_pointers(dA_displ, dA_array, ldda, i, i, batchCount, queue);
        //===============================================
        //  panel factorization
        //===============================================
#if 0
        arginfo = magma_tally2_dgetf2_batched(
                pm, ib,
                dA_displ, ldda,
...
#else
        arginfo = magma_tally2_dgetrf_recpanel_batched(
                pm, ib, 16,
                dA_displ, ldda,
...
#define RUN_ALL
        ...
        magma_tally2_ddisplace_pointers(dW0_displ, dA_array, ldda, i, 0, batchCount, queue);
        magma_tally2_dlaswp_rowparallel_batched( i, dA_displ, ldda,
                dW0_displ, ldda,
                i, i+ib,
        ...
        if( (i + ib) < n)
        {
            ...
            magma_tally2_ddisplace_pointers(dA_displ, dA_array, ldda, i, i+ib, batchCount, queue);
            dset_pointer(dwork_array, dwork, nb, 0, 0, dwork_msize, batchCount, queue); // I don't think it is needed Azzam
            magma_tally2_dlaswp_rowparallel_batched( n-(i+ib), dA_displ, ldda,
                    dwork_array, nb,
                    i, i+ib,
                    pivinfo_array, batchCount, queue);
            ...
            magma_tally2_ddisplace_pointers(dA_displ, dA_array, ldda, i, i, batchCount, queue);
            magma_tally2_ddisplace_pointers(dW0_displ, dA_array, ldda, i, i+ib, batchCount, queue);
            magma_tally2blas_dtrsm_outofplace_batched(Magma_tally2Left, Magma_tally2Lower, Magma_tally2NoTrans, Magma_tally2Unit, 1,
                    ib, n-i-ib,
                    MAGMA_tally2_D_ONE,
                    dA_displ,    ldda, // dA
                    dwork_array,  nb, // dB
            ...
            {    
                ...
                {
                    magma_tally2_ddisplace_pointers(dA_displ, dA_array,  ldda, i+ib,    i, batchCount, queue);
                    magma_tally2_ddisplace_pointers(dW1_displ, dA_array, ldda,    i, i+ib, batchCount, queue);
                    magma_tally2_ddisplace_pointers(dW2_displ, dA_array, ldda, i+ib, i+ib, batchCount, queue);
                    //printf("caling batched dgemm %d %d %d \n", m-i-ib, n-i-ib, ib);
                    magma_tally2blas_dgemm_batched( Magma_tally2NoTrans, Magma_tally2NoTrans, m-i-ib, n-i-ib, ib, 
...
#if defined(ENABLE_TIMER3)
    tloop   = magma_tally2_sync_wtime(0) - tloop;
    tdalloc = magma_tally2_sync_wtime(0);
...
#if defined(ENABLE_TIMER3)
    tdalloc = magma_tally2_sync_wtime(0) - tdalloc;
    tall = magma_tally2_sync_wtime(0) - tall;
