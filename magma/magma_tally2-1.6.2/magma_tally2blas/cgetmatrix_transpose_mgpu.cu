...
#define dAT(d, j)   (dAT[(d)]   + (j)*nb)
    ...
    magma_tally2_int_t nstreams = 2, d, j, j_local, id, ib;
    ...
    if (lda < m || ngpu*ldda < n || lddw < m){
        printf( "Wrong arguments in magma_tally2blas_cgetmatrix_transpose_mgpu (%d<%d), (%d*%d<%d), or (%d<%d).\n",
    ...
    for(j=0; j<n; j+=nb){
       ...
       ib = min(n-j, nb);
       magma_tally2blas_ctranspose_q( ib, m, dAT(d,j_local), ldda, dwork(d,id), lddw, queues[d][id] );
