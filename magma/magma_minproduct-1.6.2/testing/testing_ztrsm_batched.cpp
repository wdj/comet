...
int main( int argc, char** argv)
{
    ...
    double          magma_minproduct_error, cublas_error, lapack_error, work[1];
    ...
    magma_minproductDoubleComplex *h_A, *h_B, *h_Bcublas, *h_Bmagma_minproduct, *h_Blapack, *h_X;
    magma_minproductDoubleComplex *d_A, *d_B;
    magma_minproductDoubleComplex **d_A_array = NULL;
    magma_minproductDoubleComplex **d_B_array = NULL;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_X,       magma_minproductDoubleComplex, sizeB   );
            TESTING_MALLOC_CPU( h_Blapack, magma_minproductDoubleComplex, sizeB   );
            TESTING_MALLOC_CPU( h_Bcublas, magma_minproductDoubleComplex, sizeB   );
            TESTING_MALLOC_CPU( h_Bmagma_minproduct,  magma_minproductDoubleComplex, sizeB   );
            TESTING_MALLOC_CPU( ipiv,      magma_minproduct_int_t,        Ak      );
            
            TESTING_MALLOC_DEV( d_A,       magma_minproductDoubleComplex, ldda*Ak*batchCount );
            TESTING_MALLOC_DEV( d_B,       magma_minproductDoubleComplex, lddb*N*batchCount  );
            ...
            magma_minproductDoubleComplex* dwork=NULL; // invA and work are workspace in ztrsm
            ...
            magma_minproduct_zmalloc( &dinvA, dinvA_batchSize * batchCount);
            magma_minproduct_zmalloc( &dwork, dwork_batchSize * batchCount );
    
            zset_pointer(dwork_array, dwork, lddb, 0, 0, dwork_batchSize, batchCount, opts.queue);
            zset_pointer(dinvA_array, dinvA, magma_minproduct_roundup( Ak, TRI_NB ), 0, 0, dinvA_batchSize, batchCount, opts.queue);

            memset(h_Bmagma_minproduct, 0, batchCount*ldb*N*sizeof(magma_minproductDoubleComplex));
            magma_minproductblas_zlaset( Magma_minproductFull, lddb, N*batchCount, c_zero, c_zero, dwork, lddb);
            ...
            memcpy( h_Blapack, h_B, sizeB*sizeof(magma_minproductDoubleComplex) );
            
            /* =====================================================================
               Performs operation using MAGMA_minproductBLAS
               =================================================================== */
            magma_minproduct_zsetmatrix( Ak, Ak*batchCount, h_A, lda, d_A, ldda );
            magma_minproduct_zsetmatrix( M,  N*batchCount, h_B, ldb, d_B, lddb );
            ...
            magma_minproduct_time = magma_minproduct_sync_wtime( NULL );
                ...
                magma_minproduct_zgetmatrix( M, N*batchCount, dwork, lddb, h_Bmagma_minproduct, ldb );
            #else
                ...
                magma_minproduct_time = magma_minproduct_sync_wtime( NULL ) - magma_minproduct_time;
                magma_minproduct_perf = gflops / magma_minproduct_time;
                magma_minproduct_zgetmatrix( M, N*batchCount, d_B, lddb, h_Bmagma_minproduct, ldb );
            #endif
            ...
            // CUBLAS version <= 6.0 has magma_minproductDoubleComplex **            dA_array, no cast needed.
            // CUBLAS version    6.5 has magma_minproductDoubleComplex const**       dA_array, requiring cast.
            // Correctly, it should be   magma_minproductDoubleComplex const* const* dA_array, to avoid requiring cast.
                ...
                cublasZtrsmBatched(
                    ...
                    M, N, &alpha,
                    (const magma_minproductDoubleComplex**) d_A_array, ldda,
                    d_B_array, lddb, batchCount);
                cublas_time = magma_minproduct_sync_wtime( NULL ) - cublas_time;
                cublas_perf = gflops / cublas_time;
            #endif

            magma_minproduct_zgetmatrix( M, N*batchCount, d_B, lddb, h_Bcublas, ldb );
            ...
            /* =====================================================================
               ...
               =================================================================== */
            // ||b - 1/alpha*A*x|| / (||A||*||x||)
            magma_minproductDoubleComplex alpha2 = MAGMA_minproduct_Z_DIV( c_one, alpha );
            double normR, normX, normA=0.;
            magma_minproduct_error  = 0.0;
            cublas_error = 0.0;

            memcpy( h_X, h_Bmagma_minproduct, sizeB*sizeof(magma_minproductDoubleComplex) );
            ...
            for (int s=0; s < batchCount; s++) {
                ...
                normX = lapackf77_zlange( "M", &M, &N, h_Bmagma_minproduct + s * ldb * N, &ldb, work );
                double magma_minproduct_err = normR/(normX*normA);
            ...
            memcpy( h_X, h_Bcublas, sizeB*sizeof(magma_minproductDoubleComplex) );
            // check cublas
            #if CUDA_VERSION >= 6050
            for (int s=0; s < batchCount; s++) {
                normA = lapackf77_zlange( "M", &Ak, &Ak, h_A + s * lda * Ak, &lda, work );
            ...
            if ( opts.lapack ) {
                ...
                lapack_error = 0.0;
                memcpy( h_X, h_Blapack, sizeB*sizeof(magma_minproductDoubleComplex) );
                for (int s=0; s < batchCount; s++) {
                    blasf77_ztrmm(
                        lapack_side_const(opts.side), lapack_uplo_const(opts.uplo),
                        lapack_trans_const(opts.transA), lapack_diag_const(opts.diag),
