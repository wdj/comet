...
extern "C"
void magma_tally2blas_dsymm_mgpu_com(
    ...
    double *work[],     magma_tally2_int_t worksiz,  // TODO unused
    ...
    magma_tally2_int_t gnode[Magma_tally2MaxGPUs][Magma_tally2MaxGPUs+2], magma_tally2_int_t nbcmplx )
{
    ...
    magma_tally2Double_ptr dwork2[Magma_tally2MaxGPUs];
    ...
    if(blockoffset>0){
        ...
        magma_tally2blas_dsymmetrize_tiles(  Magma_tally2Lower,  fstblksiz,  dA(stdev, offset, myblkoffst*nb+blockoffset),  ldda,  1,  ngpu*nb,  nb  );         
    }
    ...
    for( magma_tally2_int_t dev = 0; dev < ngpu; ++dev ) {
        ...
        magma_tally2_setdevice( dev );
        magma_tally2blasSetKernelStream( queues[ dev ][ 0 ] );
        magma_tally2blas_dsymmetrize_tiles(  Magma_tally2Lower,  nb,  dA(dev, newoffset+devperm*nb, myblkoffst*nb),  ldda,  myblk,  ngpu*nb,  nb  );
        if(remm%nb>0){
            ...
            if(devperm==devlstblk)
                magma_tally2blas_dsymmetrize(  Magma_tally2Lower,  remm % nb,  dA(dev, newoffset+nbblk*nb, myblkoffst*nb+ myblk*nb),  ldda );  // last partial tile
        }
    }
...
/*
    ...
    double *R;
    magma_tally2_dmalloc_cpu( &R, siz*siz );
    // collecte back A
    magma_tally2blas_dgetmatrix_1D_bcyclic( siz, siz, dA, ldda, R, siz, ngpu, nb );
    magma_tally2_setdevice( 0 );
    magma_tally2blasSetKernelStream( queues[ dev ][ 0 ] );
    //magma_tally2_dgetmatrix( siz, siz, dA[0], ldda, R, siz );
    FILE *trace_file;
...
*/
        ...
        for( magma_tally2_int_t i = fstblksiz; i < m; i += nb ) {
            ...
            for( magma_tally2_int_t dev = 0; dev < ngpu; ++dev ) {
                ...
                if(myrowsize>0){
                    ...
                    magma_tally2_dgemm( Magma_tally2ConjTrans, Magma_tally2NoTrans, myrowsize, n, ib,
    ...
    if(blockoffset>0){
        ...
        magma_tally2_dgemm( Magma_tally2NoTrans, Magma_tally2NoTrans, m, n, ib,
    ...
    for( magma_tally2_int_t i = fstblksiz; i < m; i += nb ) {
        ...
        magma_tally2blasSetKernelStream( queues[ dev ][ 0 ] );
        if(i==0){
           magma_tally2_dgemm( Magma_tally2NoTrans, Magma_tally2NoTrans, m-i, n, ib,
        ...
        if(ngpu==1){
            ...
            // magma_tally2blasSetKernelStream( queues[ dev ][ 0 ] );
            magma_tally2_dgemm( Magma_tally2ConjTrans, Magma_tally2NoTrans, i, n, ib,
        ...
        for( magma_tally2_int_t dev = 0; dev < ngpu; ++dev ) {
            ...
            if(myrowsize>0){
                ...
                for( magma_tally2_int_t blki = 0; blki < myblk; ++blki){
                    ...
                    magma_tally2blas_dgeadd(ib, n, c_one, 
            ...
            for( magma_tally2_int_t idev = 0; idev < myngpu; ++idev ) {
                ...
                if(mycolsize>0){
                    ...
                    if(dev!=masterdev){
                        ...
                        magma_tally2_dcopymatrix_async(
                            m, n,
                            &dC[dev][0], lddc,
        ...
        if(masterdev!=-1){ 
            ...
            for( magma_tally2_int_t l = 0; l < myngpu; ++l ) {
                ...
                if((lcdev!=masterdev)&&(lccolsize>0)){
                    ...
                    magma_tally2blas_dgeadd(m, n, c_one, 
