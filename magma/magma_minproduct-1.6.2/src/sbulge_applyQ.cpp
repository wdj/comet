 ...
 *     @author Stan Tomov
 *
 *     @generated from zbulge_applyQ.cpp normal z -> s, Fri Jan 30 19:00:18 2015
 *
 */
...
extern "C" void magma_minproduct_sbulge_applyQ(
    ...
    magma_minproduct_int_t Vblksiz, float *E, magma_minproduct_int_t LDE,
    float *V, float *TAU, float *T,
    magma_minproduct_int_t *INFO, float *dV, float *dT,
    float *dE, magma_minproduct_int_t copytype )
{
    //%===========================
    //%   local variables
    //%===========================
    ...
    *INFO=0;
     ...
     * */
    if (copytype > 0) magma_minproduct_ssetmatrix( LDV, blkcnt*Vblksiz, V, LDV, dV, LDV );
    if (copytype > 1) magma_minproduct_ssetmatrix( LDT, blkcnt*Vblksiz, T, LDT, dT, LDT );
    if (copytype > 2) magma_minproduct_ssetmatrix( N, NE, E, N, dE, N );
    float *dwork;
    //ldwork  = NE;
    LWORK   = 2*N*max(Vblksiz, 64);
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_smalloc( &dwork, LWORK )) {
    ...
    if (WANTZ == 1) {
        ...
        magma_minproductblas_slaset( Magma_minproductFull, N, N, c_zero, c_one, dE, N );
    }
            ...
            for (bg = nbGblk; bg > 0; bg--) {
                ...
                for (m = rownbm; m > 0; m--) {
                    ...
                    if ((vlen > 0) && (vnb > 0)) {
                        if (WANTZ == 1) {
                            len =  N-colst;
                            magma_minproduct_slarfb_gpu( Magma_minproductLeft, Magma_minproductNoTrans, Magma_minproductForward, Magma_minproductColumnwise, vlen, len, vnb, dV(vpos), LDV, dT(tpos), LDT, dE(fst,colst), LDE, dwork, len);
                        } else {
                            magma_minproduct_slarfb_gpu( Magma_minproductLeft, Magma_minproductNoTrans, Magma_minproductForward, Magma_minproductColumnwise, vlen, NE, vnb, dV(vpos), LDV, dT(tpos), LDT, dE(fst,0), LDE, dwork, NE);
                        }
                    }
                }
            }
            ...
            for (m = rownbm; m > 0; m--) {
                ...
                for (n = nbgr; n > 0; n--) {
                    ...
                    if ((vlen > 0) && (vnb > 0)) {
                        ...
                        magma_minproduct_slarfb_gpu( Magma_minproductLeft, Magma_minproductNoTrans, Magma_minproductForward, Magma_minproductColumnwise, vlen, N1, vnb, dV(vpos), LDV, dT(tpos), LDT, dE(fst,0), LDE, dwork, N1);
                        magma_minproductblasSetKernelStream(stream[1]);
                        magma_minproduct_slarfb_gpu( Magma_minproductLeft, Magma_minproductNoTrans, Magma_minproductForward, Magma_minproductColumnwise, vlen, N2, vnb, dV(vpos), LDV, dT(tpos), LDT, dE(fst,N1), LDE, &dwork[N1*Vblksiz], N2);
                        #else
                        magma_minproduct_slarfb_gpu( Magma_minproductLeft, Magma_minproductNoTrans, Magma_minproductForward, Magma_minproductColumnwise, vlen, NE, vnb, dV(vpos), LDV, dT(tpos), LDT, dE(fst,0), LDE, dwork, NE);
                        #endif
                    }
            ...
            for (bg =1; bg <= nbGblk; bg++) {
                ...
                for (m = 1; m <= rownbm; m++) {
                    ...
                    if ((vlen > 0) && (vnb > 0)) {
                        #if defined(USESTREAM)
                        magma_minproductblasSetKernelStream(stream[0]);
                        magma_minproduct_slarfb_gpu( Magma_minproductRight, Magma_minproductNoTrans, Magma_minproductForward, Magma_minproductColumnwise, N1, vlen, vnb, dV(vpos), LDV, dT(tpos), LDT, dE(0, fst), LDE, dwork, N1);
                        magma_minproductblasSetKernelStream(stream[1]);
                        magma_minproduct_slarfb_gpu( Magma_minproductRight, Magma_minproductNoTrans, Magma_minproductForward, Magma_minproductColumnwise, N2, vlen, vnb, dV(vpos), LDV, dT(tpos), LDT, dE(N1, fst), LDE, &dwork[N1*Vblksiz], N2);
                        #else
                        magma_minproduct_slarfb_gpu( Magma_minproductRight, Magma_minproductNoTrans, Magma_minproductForward, Magma_minproductColumnwise, NE, vlen, vnb, dV(vpos), LDV, dT(tpos), LDT, dE(0, fst), LDE, dwork, NE);
                        #endif
                    }
                }
            }
