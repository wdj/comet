...
/**
    ...
    @ingroup magma_tally3_zgesv_driver
    ********************************************************************/
extern "C" magma_tally3_int_t
magma_tally3_zgerfs_nopiv_gpu(
    ...
    magma_tally3DoubleComplex_ptr dworkd, magma_tally3DoubleComplex_ptr dAF,
    ...
    magma_tally3_int_t *info)
{
    ...
    magma_tally3DoubleComplex Xnrmv, Rnrmv;
    ...
    *info = 0;
    ...
    Anrm = magma_tally3blas_zlange(Magma_tally3InfNorm, n, n, dA, ldda, (double*)dworkd );
    cte  = Anrm * eps * pow( (double)n, (double)0.5 ) * BWDMAX;
    
    // residual dR = dB - dA*dX in double precision
    magma_tally3blas_zlacpy( Magma_tally3UpperLower, n, nrhs, dB, lddb, dR, lddr );
    if ( nrhs == 1 ) {
        magma_tally3_zgemv( trans, n, n,
                     c_neg_one, dA, ldda,
                                dX, 1,
                     c_one,     dR, 1 );
    }
    else {
        magma_tally3_zgemm( trans, Magma_tally3NoTrans, n, nrhs, n,
                     c_neg_one, dA, ldda,
                                dX, lddx,
                     c_one,     dR, lddr );
    }
    
    // TODO: use MAGMA_tally3_Z_ABS( dX(i,j) ) instead of zlange?
    for( j=0; j < nrhs; j++ ) {
        i = magma_tally3_izamax( n, dX(0,j), 1) - 1;
        magma_tally3_zgetmatrix( 1, 1, dX(i,j), 1, &Xnrmv, 1 );
        Xnrm = lapackf77_zlange( "F", &ione, &ione, &Xnrmv, &ione, NULL );
        
        i = magma_tally3_izamax ( n, dR(0,j), 1 ) - 1;
        magma_tally3_zgetmatrix( 1, 1, dR(i,j), 1, &Rnrmv, 1 );
...
REFINEMENT:
    for( iiter=1; iiter < ITERMAX; ) {
        *info = 0;
        ...
        magma_tally3_zgetrs_nopiv_gpu( trans, n, nrhs, dAF, lddsa, dR, lddr, info );
        if (*info != 0) {
            *iter = -3;
            goto FALLBACK;
        }
        ...
        for( j=0; j < nrhs; j++ ) {
            magma_tally3blas_zaxpycp2( n, dR(0,j), dX(0,j), dB(0,j) );
        }
    ...
    magma_tally3_int_t *info)
{
    ...
    *info = 0;
    ...
    magma_tally3DoubleComplex *dA, *hu, *hv, *db, *dAo, *dBo, *dwork;
    magma_tally3_int_t n2;

    magma_tally3_int_t iter;
    n2 = nn*nn;

    if (MAGMA_tally3_SUCCESS != magma_tally3_zmalloc( &dA, n2 )) {
        *info = MAGMA_tally3_ERR_DEVICE_ALLOC;
        return *info;
    }
    if (MAGMA_tally3_SUCCESS != magma_tally3_zmalloc( &db, nn*nrhs )) {
        *info = MAGMA_tally3_ERR_DEVICE_ALLOC;
        return *info;
    }

    if (ref == Magma_tally3True) {
        if (MAGMA_tally3_SUCCESS != magma_tally3_zmalloc( &dAo, n2 )) {
            *info = MAGMA_tally3_ERR_DEVICE_ALLOC;
            return *info;
        }
        if (MAGMA_tally3_SUCCESS != magma_tally3_zmalloc( &dwork, nn*nrhs )) {
            *info = MAGMA_tally3_ERR_DEVICE_ALLOC;
            return *info;
        }
        if (MAGMA_tally3_SUCCESS != magma_tally3_zmalloc( &dBo, nn*nrhs )) {
            *info = MAGMA_tally3_ERR_DEVICE_ALLOC;
            return *info;
        }
    }

    if (MAGMA_tally3_SUCCESS != magma_tally3_zmalloc_cpu( &hu, 2*nn )) {
        *info = MAGMA_tally3_ERR_HOST_ALLOC;
        return *info;
    }
    if (MAGMA_tally3_SUCCESS != magma_tally3_zmalloc_cpu( &hv, 2*nn )) {
        *info = MAGMA_tally3_ERR_HOST_ALLOC;
        return *info;
    }

    magma_tally3blas_zlaset(Magma_tally3Full, nn, nn, MAGMA_tally3_Z_ZERO, MAGMA_tally3_Z_ONE, dA, nn);

    /* Send matrix on the GPU*/
    magma_tally3_zsetmatrix(n, n, A, lda, dA, nn);

    /* Send b on the GPU*/
    magma_tally3_zsetmatrix(n, nrhs, B, ldb, db, nn);

    *info = magma_tally3_zgerbt_gpu(Magma_tally3True, nn, nrhs, dA, nn, db, nn, hu, hv, info);
    if (*info != MAGMA_tally3_SUCCESS)  {
        return *info;
    }

    if (ref == Magma_tally3True) {
        magma_tally3_zcopymatrix(nn, nn, dA, nn, dAo, nn);
        magma_tally3_zcopymatrix(nn, nrhs, db, nn, dBo, nn);
    }
    /* Solve the system U^TAV.y = U^T.b on the GPU*/ 
    magma_tally3_zgesv_nopiv_gpu( nn, nrhs, dA, nn, db, nn, info);


    /* Iterative refinement */
    if (ref == Magma_tally3True) {
        magma_tally3_zgerfs_nopiv_gpu(Magma_tally3NoTrans, nn, nrhs, dAo, nn, dBo, nn, db, nn, dwork, dA, &iter, info);
    }
    //printf("iter = %d\n", iter);

    /* The solution of A.x = b is Vy computed on the GPU */
    magma_tally3DoubleComplex *dv;

    if (MAGMA_tally3_SUCCESS != magma_tally3_zmalloc( &dv, 2*nn )) {
        *info = MAGMA_tally3_ERR_DEVICE_ALLOC;
        return *info;
    }

    magma_tally3_zsetvector(2*nn, hv, 1, dv, 1);
    
    for(int i = 0; i < nrhs; i++) {
        magma_tally3blas_zprbt_mv(nn, dv, db+(i*nn));
    }

    magma_tally3_zgetmatrix(n, nrhs, db, nn, B, ldb);
