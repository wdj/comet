...
/**
    ...
    @ingroup magma_tally2_dgesv_tile
    ********************************************************************/
...
#define hL2(i)  (hL2 + (i)*ib*ldhl          )
    ...
    magma_tally2Double_ptr dAT, dUT;
    magma_tally2Double_ptr dAp, dUp;
#ifndef WITHOUTTRTRI
    magma_tally2Double_ptr dL2 = dL + ib;
    double *hL2 = hL + ib;
    p = 2;
#endif
    ...
    *info = 0;
    ...
    if ( ib >= mindim ) {
        ...
        if ( order == Magma_tally2RowMajor ) {
            magma_tally2_dsetmatrix( m, n, hU, ldhu, dwork, lddwork );
            magma_tally2blas_dtranspose( m, n, dwork, lddwork, dU, lddu );

            magma_tally2_dsetmatrix( m, n, hA, ldha, dwork, lddwork );
            magma_tally2blas_dtranspose( m, n, dwork, lddwork, dA, ldda );
        } else {
            magma_tally2_dsetmatrix( m, n, hU, ldhu, dU, lddu );
            magma_tally2_dsetmatrix( m, n, hA, ldha, dA, ldda );
        }
        magma_tally2_dsetmatrix( p*ib, n, hL, ldhl, dL, lddl );
        ...
        if ( order == Magma_tally2ColMajor ) {
            magma_tally2blas_dgetmo_in( dU, dUT, lddu, m,  n );
            magma_tally2blas_dgetmo_in( dA, dAT, ldda, m,  n );
        } else {
            dUT = dU; dAT = dA;
        }
        dAp = dwork;
        ...
        for( i=0; i < s; i++ ) {
            ...
            if ( i > 0 ) {
                // download i-th panel
                magma_tally2blas_dtranspose( sb, ii, UT(0,i), lddu, dUp, lddu );
                magma_tally2blas_dtranspose( sb, m,  AT(0,i), ldda, dAp, ldda );
                
                magma_tally2_dgetmatrix( ii, sb, dUp, lddu, hU(0, i), ldhu );
                magma_tally2_dgetmatrix( m,  sb, dAp, ldda, hA(0, i), ldha );
                
                // make sure that gpu queue is empty
                //magma_tally2_device_sync();
                
#ifndef WITHOUTTRTRI
                magma_tally2_dtrmm( Magma_tally2Right, Magma_tally2Lower, Magma_tally2Trans, Magma_tally2Unit,
                             n-(ii+sb), ib,
                             c_one, L2(i-1),      lddl,
                                    UT(i-1, i+1), lddu);
#else
                magma_tally2_dtrsm( Magma_tally2Right, Magma_tally2Lower, Magma_tally2Trans, Magma_tally2Unit,
                             n-(ii+sb), ib,
                             c_one, L(i-1),       lddl,
                                    UT(i-1, i+1), lddu);
#endif
...
#ifndef NOSWAPBLK
            magma_tally2blas_dswapblk( Magma_tally2RowMajor, n-(ii+sb),
                                UT(i, i+1), lddu,
                                AT(0, i+1), ldda,
                                1, sb, ipiv+ii, 1, nb );
            ...
            for (j=0; j < ib; j++) {
                ...
                if ( im != (j) ) {
                    im = im - nb;
                    assert( (im >= 0) && (im < m) );
                    magma_tally2blas_dswap( n-(ii+sb), UT(i, i+1)+j*lddu, 1, AT(0, i+1)+im*ldda, 1 );
                } else {
                    ipiv[ip] += ii;
                }
                ip++;
...
#endif
            // upload i-th panel
            magma_tally2_dsetmatrix( sb, sb, hU(i, i), ldhu, dUp, lddu );
            magma_tally2_dsetmatrix( m, sb, hA(0, i), ldha, dAp, ldda );
            magma_tally2_dsetmatrix( p*ib, sb, hL(i), ldhl, L(i), lddl );
            magma_tally2blas_dtranspose( sb, sb, dUp, lddu, UT(i,i), lddu );
            magma_tally2blas_dtranspose( m,  sb, dAp, ldda, AT(0,i), ldda );
        ...
        if ( order == Magma_tally2ColMajor ) {
            magma_tally2blas_dgetmo_out( dU, dUT, lddu, m,  n );
            magma_tally2blas_dgetmo_out( dA, dAT, ldda, m,  n );
        }
    }
    return *info;
}
