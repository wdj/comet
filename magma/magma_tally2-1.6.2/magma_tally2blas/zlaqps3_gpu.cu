...
__global__ void
magma_tally2_zswap_gemv_kernel(int m, int rk, int n, const magma_tally2DoubleComplex * __restrict__ V, int ldv,
                     const magma_tally2DoubleComplex * __restrict__ x, int ldx, magma_tally2DoubleComplex *c, magma_tally2DoubleComplex *b)
{
    const int i = threadIdx.x;
    const int j = i + BLOCK_SIZE1 * blockIdx.x;
    magma_tally2DoubleComplex lsum, tmp;
    ...
    if (j < m){
       ...
       if (j>=rk) 
          for(int k=0; k<n; k++)
              lsum += MAGMA_tally2_Z_MUL( V[k*ldv], MAGMA_tally2_Z_CNJG(x[k*ldx]));
...
__global__ void
magma_tally2_zgemv_kernel(int m, int n, const magma_tally2DoubleComplex * __restrict__ V, int ldv,
                     const magma_tally2DoubleComplex * __restrict__ x, magma_tally2DoubleComplex *b, magma_tally2DoubleComplex *c)
{
    const int i = threadIdx.x;
    const int j = i + BLOCK_SIZE1 * blockIdx.x;
    ...
    if (j < m){
        for(int k=0; k<n; k++)
            lsum += MAGMA_tally2_Z_MUL( V[k*ldv], x[k]);
...
__global__
void magma_tally2_zscale_kernel(int n, magma_tally2DoubleComplex* dx0,
                         magma_tally2DoubleComplex *dtau, double *dxnorm, magma_tally2DoubleComplex* dAkk)
{
   ...
   magma_tally2DoubleComplex *dx = dx0;
   ...
   for( int k = i; k < n; k += BLOCK_SIZE ) {
             ...
             lsum += re*re;
        #else
             re = MAGMA_tally2_Z_REAL( dx[k] );
             double im = MAGMA_tally2_Z_IMAG( dx[k] );
             lsum += re*re + im*im;
        #endif
   }
   sum[i] = lsum;
            ...
            if ( beta == 0 ) {
              *dtau = MAGMA_tally2_Z_ZERO;
               ...
               *dAkk    = beta;
               ...
               scale = 1. / (tmp - beta);
#else
               double alphar =  MAGMA_tally2_Z_REAL(tmp), alphai = MAGMA_tally2_Z_IMAG(tmp);
               ...
               *dAkk = MAGMA_tally2_Z_MAKE(beta, 0.);
               ...
               tmp = MAGMA_tally2_Z_MAKE( alphar - beta, alphai);
               scale = MAGMA_tally2_Z_DIV( MAGMA_tally2_Z_ONE, tmp);
#endif
            }
   }
...
__global__ void
magma_tally2_zgemv_kernel_adjust(int n, int k, magma_tally2DoubleComplex * A, int lda, 
                          magma_tally2DoubleComplex *B, int ldb, magma_tally2DoubleComplex *C,
                          double *xnorm, double *xnorm2, magma_tally2DoubleComplex *Akk, int *lsticc, int *lsticcs)
{
    const int i = threadIdx.x;
    const int j = i + BLOCK_SIZE2 * blockIdx.x;
    magma_tally2DoubleComplex sum;
    double temp, oldnorm;

    if (j<n) {
      B += j;
      sum = MAGMA_tally2_Z_CNJG( B[(k-1)*ldb] );
      // sum = MAGMA_tally2_Z_ZERO;
      for(int m=0; m<k-1; m++) {
         sum += MAGMA_tally2_Z_MUL( MAGMA_tally2_Z_CNJG( B[m*ldb] ), A[m*lda] );
      }
      C[j*lda] -= sum;

      oldnorm = xnorm[j];
      temp = MAGMA_tally2_Z_ABS( C[j*lda] ) / oldnorm;
...
/*
    ...
    if (blockIdx.x==0) {
       //if (2.*temp < oldnorm) {
           ...
           magma_tally2DoubleComplex *dx = C+blockIdx.x*lda+1;
...
__global__ void
magma_tally2blas_dznrm2_check_kernel(int m, magma_tally2DoubleComplex *da, int ldda, 
...
#define dF(i_, j_) (dF + (i_) + (j_)*(lddf))
    ...
    magma_tally2DoubleComplex tauk;
    magma_tally2_int_t pvt, itemp;

    magma_tally2DoubleComplex_ptr dAkk = dauxv;
    dauxv += 1;
    ...
    // double tol3z = magma_tally2_dsqrt( lapackf77_dlamch("Epsilon"));
    ...
    while( k < nb && lsticc == 0 ) {
        ...
        pvt = k - 1 + magma_tally2_idamax( n-k, &dvn1[k], ione );

        if (pvt != k) {
            magma_tally2blas_zswap( k, dF(pvt,0), lddf, dF(k,0), lddf);
            itemp     = jpvt[pvt];
            jpvt[pvt] = jpvt[k];
            jpvt[k]   = itemp;
        ...
        magma_tally2_zswap_gemv_kernel<<< (m + BLOCK_SIZE1-1) / BLOCK_SIZE1, BLOCK_SIZE1, 0, magma_tally2_stream >>> 
                              ( m, rk, k, dA(0, 0), ldda, dF(k,  0), lddf, dA(0, k), dA(0,pvt));
                                 
        /*  Generate elementary reflector H(k). */
        magma_tally2_zscale_kernel<<< 1, BLOCK_SIZE, 0, magma_tally2_stream >>>
        ...
        if (k < n-1) {
            magma_tally2_zgetvector( 1, &dtau[k], 1, &tauk, 1 );
            magma_tally2_zgemv( Magma_tally2ConjTrans, m-rk, n,
                         tauk,   dA( rk,  0 ), ldda,
                                 dA( rk,  k   ), 1,
                         c_zero, dauxv, 1 );
            if (k==0) 
               magma_tally2blas_zlacpy(Magma_tally2UpperLower, n-k-1, 1, dauxv+k+1, n-k-1, dF( k+1, k   ), n-k-1);
        }
        ...
        if (k > 0) {
            ...
            magma_tally2_zgemv_kernel<<< (n-k-1 + BLOCK_SIZE1 -1)/BLOCK_SIZE1, BLOCK_SIZE1, 0, magma_tally2_stream >>>
        ...
        if (k < n-1) {
            ...
            magma_tally2_zgemv_kernel_adjust<<<(n-k-1 + BLOCK_SIZE2-1)/BLOCK_SIZE2, BLOCK_SIZE2, 0, magma_tally2_stream>>>
            ...
            // can be done in the kernel directly (magma_tally2blas_dznrm2_check_kernel)
            // if (k==16) lsticc = 1;
        }
...
/*
        if (rk < min(m, n+offset)-1){
           magma_tally2blas_dznrm2_row_check_adjust(n-k-1, tol3z, &dvn1[k+1], 
                                             &dvn2[k+1], dA(rk,k+1), ldda, lsticcs); 
        }

        #if defined(PRECISION_d) || defined(PRECISION_z)
            magma_tally2_dgetvector( 1, &lsticcs[0], 1, &lsticc, 1 );
        #else
...
*/
        ...
        if (k>=n-1)
           magma_tally2blas_zlacpy(Magma_tally2UpperLower, 1, 1, dAkk, 1, dA(rk, k), 1);
    ...
    *kb = k + 1;
    ...
    if( lsticc > 0 ) {
        ...
        //magma_tally2blas_dznrm2_check(m-rk-1, n-*kb, A(rk+1,rk+1), lda,
        //                       &dvn1[rk+1], &dvn2[rk+1], dlsticcs);
       
        // There is a bug when we get to recompute  
        magma_tally2blas_dznrm2_check_kernel<<< n-*kb, BLOCK_SIZE >>>
