...
*/
void zsplit_diag_block(magma_tally3_int_t ib, magma_tally3DoubleComplex *a, magma_tally3_int_t lda, magma_tally3DoubleComplex *work)
{
    magma_tally3_int_t i, j, info;
    magma_tally3DoubleComplex *cola, *colw;
    ...
    lapackf77_ztrtri( Magma_tally3UpperStr, Magma_tally3NonUnitStr, &ib, work, &ib, &info);
}
    ...
    magma_tally3_int_t *info )
{
    ...
    magma_tally3DoubleComplex *work, *ut;
    ...
    *info = 0;
    ...
    ut = hwork+nb*(n);
    memset( ut, 0, nb*nb*sizeof(magma_tally3DoubleComplex));
    ...
    if ( (nb > 1) && (nb < k) ) {
        ...
        for (i = 0; i < k-nb; i += nb) {
            ...
            if (i > 0) {
                ...
                /* store the diagonal */
                magma_tally3_zsetmatrix_async( old_ib, old_ib,
                                        ut,           old_ib,
            ...
            magma_tally3_zsetmatrix( rows, ib, work(i), ldwork, dA(i,i), ldda );

            if (i + ib < n) {
                /* Send the triangular factor T to the GPU */
                magma_tally3_zsetmatrix( ib, ib, hwork, ib, dT(i), nb );
                ...
                if (i+nb < k-nb) {
                    ...
                    magma_tally3_zsetmatrix( ib, ib, ut, ib, d_ref(i), ib );
                }
                old_i  = i;
                old_ib = ib;
            }
    ...
    if (i < k) {
        ib   = n-i;
        rows = m-i;
        magma_tally3_zgetmatrix( rows, ib, dA(i, i), ldda, work, rows );
        lhwork = lwork - rows*ib;
        lapackf77_zgeqrf(&rows, &ib, work, &rows, tau+i, work+ib*rows, &lhwork, info);
        
        magma_tally3_zsetmatrix( rows, ib, work, rows, dA(i, i), ldda );
    }
