...
#define dA(i, j)  (dA + (i) + (j)*ldda)   // A(i, j) means at i row, j column
    ...
    magma_tally3DoubleComplex *dwork = NULL;
    magma_tally3DoubleComplex *dT   = NULL;
    magma_tally3DoubleComplex *dR   = NULL;
    magma_tally3DoubleComplex **dR_array = NULL;
    magma_tally3DoubleComplex **dT_array = NULL; 
    magma_tally3DoubleComplex **cpuAarray = NULL;
    magma_tally3DoubleComplex **cpuTarray = NULL;
    ...
    magma_tally3_malloc((void**)&dW4_displ, batchCount * sizeof(*dW4_displ));  // used in zlarfb
    ...
    magma_tally3_zmalloc(&dwork,  (2 * nb * n) * batchCount);
    magma_tally3_zmalloc(&dR,  ldr * n   * batchCount);
    magma_tally3_zmalloc(&dT,  ldt * ldt * batchCount);
    magma_tally3_malloc_cpu((void**) &cpuAarray, batchCount*sizeof(magma_tally3DoubleComplex*));
    magma_tally3_malloc_cpu((void**) &cpuTarray, batchCount*sizeof(magma_tally3DoubleComplex*));
    ...
    magma_tally3blas_zlaset_q(Magma_tally3Full, ldr, n*batchCount  , MAGMA_tally3_Z_ZERO, MAGMA_tally3_Z_ZERO, dR, ldr, queue);
    magma_tally3blas_zlaset_q(Magma_tally3Full, ldt, ldt*batchCount, MAGMA_tally3_Z_ZERO, MAGMA_tally3_Z_ZERO, dT, ldt, queue);
    ...
    magma_tally3_getvector( batchCount, sizeof(magma_tally3DoubleComplex*), dT_array, 1, cpuTarray, 1);
    ...
    for(i=0; i<min_mn;i+=nb)
    {
            ...
            magma_tally3_zdisplace_pointers(dW0_displ, dA_array, ldda, i, i, batchCount, queue); 
            magma_tally3_zdisplace_pointers(dW2_displ, tau_array, 1, i, 0, batchCount, queue);
            ...
            magma_tally3_zgeqrf_panel_batched(m-i, ib, jb, 
                                       dW0_displ, ldda, 
                                       dW2_displ, 
                                       dT_array, ldt, 
                                       dR_array, ldr,
           ...
           zgeqrf_copy_upper_batched(ib, jb, dW0_displ, ldda, dR_array, ldr, batchCount, queue);
            ...
            if( (n-ib-i) > 0)
            {
                ...
                magma_tally3blas_zlaset_batched(Magma_tally3Upper, ib, ib, MAGMA_tally3_Z_ZERO, MAGMA_tally3_Z_ONE, dW0_displ, ldda, batchCount, queue); 
                magma_tally3_zdisplace_pointers(dW2_displ, tau_array, 1, i, 0, batchCount, queue); 

                // it is faster since it is using BLAS-3 GEMM routines, different from lapack implementation 
                magma_tally3_zlarft_batched(m-i, ib, 0,
                                 dW0_displ, ldda,
                                 dW2_displ,
                                 dT_array, ldt, 
                                 dW4_displ, nb*ldt,
                ...
                { 
                    ...
                    for(k=0; k<batchCount; k++)
                    {
                        ...
                        magma_tally3_zlarfb_gpu_gemm(Magma_tally3Left, Magma_tally3ConjTrans, Magma_tally3Forward, Magma_tally3Columnwise,
                                    m-i, n-i-ib, ib,
            ...
            magma_tally3blas_zlacpy_batched(Magma_tally3Upper, ib, ib, dR_array, ldr, dW0_displ, ldda, batchCount, queue);
