...
__global__
void magma_minproduct_slarfx_kernel( int m, float *v, float *tau,
                         float *c, int ldc, float *xnorm,
                         float *T, int it )
{
    if ( !MAGMA_minproduct_S_EQUAL(*tau, MAGMA_minproduct_S_ZERO) ) {
        ...
        for( int j = tx; j < m; j += BLOCK_SIZE ){
            if (j==0){
               ...
               v[j] = MAGMA_minproduct_S_ONE;
            }
            else
               lsum += MAGMA_minproduct_S_MUL( MAGMA_minproduct_S_CNJG( v[j] ), dc[j] );
        }
        sum[tx] = lsum;
        ...
        /*  C := C - v * w  */
        __syncthreads();
        float z__1 = - MAGMA_minproduct_S_CNJG(*tau) * sum[0];
        if (blockIdx.x>it){
           ...
           /*
           if (tx==0){
             float temp = MAGMA_minproduct_S_ABS( dc[0] ) / xnorm[blockIdx.x-it-1];
             temp = (temp + 1.) * (1. - temp);
...
extern "C"
__global__
void magma_minproduct_strmv_kernel(const float *T, int ldt, float *t)
{
   const int tx = threadIdx.x;
   T += tx;

   __shared__ float tlocal[ BLOCK_SIZE ];
   float res = MAGMA_minproduct_S_MAKE(0., 0.);
...
extern "C"
__global__
void magma_minproduct_strmv_kernel2(const float *T, int ldt, float *t, 
                         float *y, float *tau)
{
   const int tx = threadIdx.x;
   T += blockIdx.x;
...
extern "C"
__global__
void magma_minproduct_strmv_tkernel(float *T, int ldt, float *t, float *y)
{
   const int tx = threadIdx.x;
   T += blockIdx.x*ldt;
   ...
   sum[tx] = MAGMA_minproduct_S_CNJG(T[tx])*t[tx];
   magma_minproduct_sum_reduce_n(blockDim.x, tx, sum);
    ...
    magma_minproductFloat_ptr work )
{
    ...
    if (iter==0)
        magma_minproduct_slarfx_kernel<<< N, BLOCK_SIZE, 0, magma_minproduct_stream >>>( m, v, tau, C, ldc, xnorm, dT+iter*N, iter);
    else
        magma_minproduct_slarfx_kernel<<< N, BLOCK_SIZE, 0, magma_minproduct_stream >>>( m, v, tau, C, ldc, xnorm, work, iter);

    if (iter > 0){
        //magma_minproduct_strmv_kernel<<< 1, iter, 0, magma_minproduct_stream >>>( dT, N, dT+iter*N);
        magma_minproduct_strmv_kernel2<<< iter, iter, 0, magma_minproduct_stream  >>>( dT, N, work, dT+iter*N, tau);
    }
}
