...
*/
    ...
    *V(vpos)  = c_one;
    ...
    magma_minproduct_zlarfxsym_v2(len, A(st,st), lda-1, V(vpos), TAU(taupos), work);
}
#undef A
#undef V
#undef TAU
    ...
    magma_minproductDoubleComplex *work)
{
    ...
    */
    ...
    if (lem > 1) {
        ...
        *V(vpos)  = c_one;
        //memcpy(V(vpos+1), A(ed+2, st), (lem-1)*sizeof(magma_minproductDoubleComplex));
        lem2 = lem-1;
        blasf77_zcopy( &lem2, A(ed+2, st), &ione, V(vpos+1), &ione );
        memset(A(ed+2, st),0,(lem-1)*sizeof(magma_minproductDoubleComplex));
