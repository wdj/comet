    ...
    magma_minproduct_int_t *info)
{
    ...
    *info = 0;
    ...
    if (nounit) {
        for (j=0; j < n; ++j) {
            if ( MAGMA_minproduct_D_EQUAL( *A(j,j), c_zero )) {
    ...
    if (nb <= 1 || nb >= n)
        ...
        if (upper) {
            ...
            for (j=0; j < n; j += nb) {
                jb = min(nb, (n-j));
                magma_minproduct_dsetmatrix( jb, (n-j),
                                  A(j, j),  lda,
                                  dA(j, j), ldda );

                /* Compute rows 1:j-1 of current block column */
                magma_minproduct_dtrmm( Magma_minproductLeft, Magma_minproductUpper,
                             Magma_minproductNoTrans, Magma_minproductNonUnit, j, jb,
                             c_one, dA(0,0), ldda, dA(0, j),ldda);

                magma_minproduct_dtrsm( Magma_minproductRight, Magma_minproductUpper,
                             Magma_minproductNoTrans, Magma_minproductNonUnit, j, jb,
                ...
                magma_minproduct_dgetmatrix_async( j, jb,
                                        dA(0, j), ldda,
                                        A(0, j),  lda, stream[0] );
                ...
                lapackf77_dtrtri(Magma_minproductUpperStr, diag_, &jb, A(j,j), &lda, info);

                magma_minproduct_dsetmatrix( jb, jb,
                                  A(j, j),  lda,
                                  dA(j, j), ldda );
            }
        }
            ...
            for (j=nn-1; j >= 0; j -= nb) {
                ...
                if ((j+jb) < n) {
                    magma_minproduct_dsetmatrix( (n-j), jb,
                                      A(j, j),  lda,
                                      dA(j, j), ldda );
                    ...
                    magma_minproduct_dgetmatrix_async( n-j-jb, jb,
                                            dA(j+jb, j), ldda,
                ...
                /* Compute inverse of current diagonal block */
                lapackf77_dtrtri(Magma_minproductLowerStr, diag_, &jb, A(j,j), &lda, info);
