...
/**
    ...
    @ingroup magma_tally2_ssygv_driver
    ********************************************************************/
    ...
    magma_tally2_int_t *info)
{
    ...
    float d_one = MAGMA_tally2_S_ONE;
    ...
    *info = 0;
    ...
    magma_tally2_spotrf_m( ngpu, uplo, n, B, ldb, info );
    if (*info != 0) {
        *info = n + *info;
        return *info;
    }
    ...
    magma_tally2_ssygst_m( ngpu, itype, uplo, n, A, lda, B, ldb, info );

    timer_stop( time );
    timer_printf( "time ssygst_m = %6.2f\n", time );
    timer_start( time );

    magma_tally2_ssyevdx_2stage_m( ngpu, jobz, range, uplo, n, A, lda, vl, vu, il, iu, mout, w, work, lwork, iwork, liwork, info );
    ...
    if (wantz && *info == 0) {
        ...
        if (itype == 1 || itype == 2) {
            ...
            magma_tally2_strsm_m( ngpu, Magma_tally2Left, uplo, trans, Magma_tally2NonUnit, n, *mout, d_one, B, ldb, A, lda );
        }
        else if (itype == 3) {
            /* For B*A*x=(lambda)*x;
            ...
            //magma_tally2_strmm_m(ngpu, Magma_tally2Left, uplo, trans, Magma_tally2NonUnit, n, *mout, d_one, B, ldb, A, lda);
            #ifdef ENABLE_DEBUG
            ...
            if (MAGMA_tally2_SUCCESS != magma_tally2_smalloc( &dA, n*ldda ) ||
                MAGMA_tally2_SUCCESS != magma_tally2_smalloc( &dB, n*lddb ) ) {
                magma_tally2_free( dA );
                magma_tally2_free( dB );
                *info = MAGMA_tally2_ERR_DEVICE_ALLOC;
                return *info;
            }
            magma_tally2_ssetmatrix( n, n, B, ldb, dB, lddb );
            magma_tally2_ssetmatrix( n, n, A, lda, dA, ldda );
            magma_tally2_strmm( Magma_tally2Left, uplo, trans, Magma_tally2NonUnit,
