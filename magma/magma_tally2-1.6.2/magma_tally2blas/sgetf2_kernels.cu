...
isamax_devfunc(int length, const float *x, int incx, float *shared_x, int *shared_idx)
{
    ...
    for(int s =0 ; s < nchunk; s++)
    {
        if( (tx + s * zamax < length) && (tx < zamax) )
        {
            ...
            res1 = fabs(MAGMA_tally2_S_REAL(res)) + fabs(MAGMA_tally2_S_IMAG(res));
...
__global__ void
tree_isamax_kernel2_batched(int n, int step,  magma_tally2_int_t** ipiv_array, magma_tally2_int_t *info_array, int gbstep, float** data_pool_array, magma_tally2_int_t** id_pool_array)
{
    __shared__ float shared_x[zamax];
    __shared__ int    shared_idx[zamax];
...
magma_tally2_int_t magma_tally2_isamax_lg_batched(magma_tally2_int_t length, float **x_array, magma_tally2_int_t incx, magma_tally2_int_t step,  magma_tally2_int_t lda,
...
extern "C"
magma_tally2_int_t magma_tally2_isamax_batched(magma_tally2_int_t length, 
...
#else
    // the magma_tally2_isamax_lg_batched is faster but when cuda launch it as 2 kernels the white space time between these 2 kernels and the next kernel is larger than using the isamax_kernel for that today we are using only isamax_kernel
    if( length <= 10 * zamax )
    {  
        dim3 grid(1, 1, batchCount);
        int chunk = (length-1)/zamax + 1;
        isamax_kernel_batched<<< grid, zamax, zamax * (sizeof(float) + sizeof(magma_tally2_int_t)), queue >>>
                      (length, chunk, x_array, incx, step, lda, ipiv_array, info_array, gbstep);

    }
    else
    {
        magma_tally2_isamax_lg_batched(length, x_array, incx, step, lda, ipiv_array, info_array, gbstep, batchCount);
    }
#endif
...
__global__
void sswap_kernel_batched(magma_tally2_int_t n, float **x_array, magma_tally2_int_t incx, magma_tally2_int_t step, magma_tally2_int_t** ipiv_array)
{
...
////////////////////////////////////////////////////////////////////////////////////////////////////
extern "C"
magma_tally2_int_t magma_tally2_sswap_batched(magma_tally2_int_t n, float **x_array, magma_tally2_int_t incx, magma_tally2_int_t step, 
...
*/
    if( n  > MAX_NTHREADS) 
    {
       printf("magma_tally2_sswap_batched nb=%d, > %d, not supported \n",n, MAX_NTHREADS);
       return -15;
    }
    dim3 grid(1,1, batchCount);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
__global__
void sscal_sger_kernel_batched(int m, int n, int step, float **dA_array, int lda, magma_tally2_int_t *info_array, int gbstep)
{
    ...
    __syncthreads();
    if(shared_y[0] == MAGMA_tally2_S_ZERO) {
        info_array[blockIdx.z] = step + gbstep + 1; 
        return;
    }

    if (gbidx < m && gbidx > 0) {
        float reg = MAGMA_tally2_S_ZERO;
        reg = A[gbidx];
        reg *= MAGMA_tally2_S_DIV(MAGMA_tally2_S_ONE, shared_y[0]);
        A[gbidx] = reg;
        #pragma unroll
        for(int i=1; i < n; i++) {
            ...
            A[gbidx + i*lda] += (MAGMA_tally2_S_NEG_ONE) * shared_y[i] * reg;
        }
    }
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////
extern "C"
magma_tally2_int_t magma_tally2_sscal_sger_batched(magma_tally2_int_t m, magma_tally2_int_t n, magma_tally2_int_t step,
...
*/
    ...
    if( n  > MAX_NTHREADS) 
    {
       printf("magma_tally2_sscal_sger_batched nb=%d, > %d, not supported \n",n, MAX_NTHREADS);
       return -15;
    }
...
void sgetf2trsm_kernel_batched(int ib, int n, float **dA_array, int step, int lda)
{
    ...
    if (tid < n) {
        ...
        for( d=0;  d<ib-1; d++) {
            for( i=d+1; i<ib; i++) {
                shared_b[i+tid*ib] += (MAGMA_tally2_S_NEG_ONE) * shared_a[i+d*ib] * shared_b[d+tid*ib];
            }
        }
    }
    __syncthreads();
...
////////////////////////////////////////////////////////////////////////////////////////////////////
extern "C" void
magma_tally2_sgetf2trsm_batched(magma_tally2_int_t ib, magma_tally2_int_t n, float **dA_array,  magma_tally2_int_t step, magma_tally2_int_t lda,
                       magma_tally2_int_t batchCount, magma_tally2_queue_t queue)
{
/*
...
sscal5_device(int m, float* x, float alpha)
{
    ...
    for(int s=0 ; s < nchunk; s++)
    {
        if( (tid + s * MAX_NTHREADS) < m ) {
            #if 0
            x[tid + s * MAX_NTHREADS] *= MAGMA_tally2_S_DIV(MAGMA_tally2_S_ONE, alpha);
            #else
...
////////////////////////////////////////////////////////////////////////////////////////////////////
__global__ void 
zcomputecolumn_kernel_shared_batched(int m, int paneloffset, int step, float **dA_array, int lda, magma_tally2_int_t **ipiv_array, magma_tally2_int_t *info_array, int gbstep)
{
...
////////////////////////////////////////////////////////////////////////////////////////////////////
extern "C"
magma_tally2_int_t magma_tally2_scomputecolumn_batched(magma_tally2_int_t m, magma_tally2_int_t paneloffset, magma_tally2_int_t step, 
                                        float **dA_array,  magma_tally2_int_t lda,
...
*/
    ...
    {
        printf("magma_tally2_scomputecolumn_batched error out of shared memory \n");
        return -20;
    }
