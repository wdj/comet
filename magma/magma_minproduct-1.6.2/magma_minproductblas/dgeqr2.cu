/*
    -- MAGMA_minproduct (version 1.6.1) --
       ...
       @author Stan Tomov
       @generated from zgeqr2.cu normal z -> d, Fri Jan 30 19:00:10 2015
...
/**
    ...
    @ingroup magma_minproduct_dgeqrf_aux
    ********************************************************************/
    ...
    magma_minproduct_int_t *info)
{
    ...
    *info = 0;
    ...
    double *Aks;
    magma_minproduct_dmalloc( &Aks, k );
    if ( Aks == NULL ) {
        *info = MAGMA_minproduct_ERR_DEVICE_ALLOC;
        ...
        for (i = 0; i < k; ++i) {
            ...
            magma_minproduct_dlarfg_gpu(m-i, dA(i, i), dA(min(i+1,m), i), dtau+i, dwork, &Aks[i]);

            if (n-i-1 > 0) {
               /* Apply H(i)' to A(i:m,i+1:n) from the left */
               magma_minproduct_dlarf_gpu(m-i, n-i-1, dA(i, i), dtau+i, dA(i, i+1), ldda);
            }
        }

        magma_minproduct_dcopymatrix( 1, k, Aks, 1, dA(0, 0), ldda+1 );
    }
    
    magma_minproduct_free(Aks);
    return *info;
