...
int main( int argc, char** argv)
{
    ...
    double *h_A, *h_R, *h_Amagma_tally4;
    double *dA;
    ...
    for( int i = 0; i < opts.ntest; ++i ) {
      ...
      for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU(    h_Amagma_tally4,  double, n2     );
            ...
            magma_tally4_dsetmatrix( M, columns, h_R, lda, dA, ldda );
            ...
            info = magma_tally4_dgetrf_batched( M, N, dA_array, ldda, dipiv_array,  dinfo_magma_tally4, batchCount, queue);
            magma_tally4_time = magma_tally4_sync_wtime(0) - magma_tally4_time;
            magma_tally4_perf = gflops / magma_tally4_time;
            
            magma_tally4_dgetmatrix( M, N*batchCount, dA, ldda, h_Amagma_tally4, lda );
            ...
            {
                if(cpu_info[i] != 0 ){
                    printf("magma_tally4_dgetrf_batched matrix %d returned internal error %d\n",i, (int)cpu_info[i] );
                }
            }
            
            if (info != 0)
                printf("magma_tally4_dgetrf_batched returned argument error %d: %s.\n", (int) info, magma_tally4_strerror( info ));
