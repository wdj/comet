...
// copied from snrm2.cu in trunk/magma_minproductblas
// ----------------------------------------
...
template< int n >
__device__ void sum_reduce( /*int n,*/ int i, magma_minproductFloat_ptr  x )
{
    __syncthreads();
...
__global__ void
magma_minproduct_slobpcg_res_kernel( 
    magma_minproduct_int_t num_rows, 
    magma_minproduct_int_t num_vecs, 
    magma_minproductFloat_ptr evals, 
    float * X, 
    float * R,
    magma_minproductFloat_ptr res)
{
    ...
    if( row<num_rows){
        for( int i=0; i<num_vecs; i++ ){ 
            ...
            R[row + i*num_rows] = R[row + i*num_rows] 
                                    + MAGMA_minproduct_S_MAKE( -evals[i], 0.0 )
                                                * X[ row + i*num_rows ];
        }
    }
}
...
/*
magma_minproductblas_snrm2_kernel( 
    int m, 
    float * da, 
    int ldda, 
    float * dxnorm )
...
/**
    ...
    @param[in]
    evalues     magma_minproductFloat_ptr 
...
magma_minproduct_slobpcg_res(
    magma_minproduct_int_t num_rows,
    magma_minproduct_int_t num_vecs, 
    magma_minproductFloat_ptr evalues, 
    ...
    magma_minproduct_queue_t queue )
{
    ...
    magma_minproduct_slobpcg_res_kernel<<< grid, threads, 0, queue >>>
