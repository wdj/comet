/*
    -- MAGMA_tally2 (version 1.6.1) --
       ...
       @generated from magma_tally2_zutil.cpp normal z -> s, Fri Jan 30 19:00:26 2015
...
void magma_tally2_smake_symmetric( magma_tally2_int_t N, float* A, magma_tally2_int_t lda )
{
    ...
    for( i=0; i<N; ++i ) {
        A(i,i) = MAGMA_tally2_S_MAKE( MAGMA_tally2_S_REAL( A(i,i) ), 0. );
        for( j=0; j<i; ++j ) {
            A(j,i) = MAGMA_tally2_S_CNJG( A(i,j) );
        }
    }
}
...
void magma_tally2_smake_hpd( magma_tally2_int_t N, float* A, magma_tally2_int_t lda )
{
    magma_tally2_int_t i, j;
    for( i=0; i<N; ++i ) {
        A(i,i) = MAGMA_tally2_S_MAKE( MAGMA_tally2_S_REAL( A(i,i) ) + N, 0. );
        for( j=0; j<i; ++j ) {
            A(j,i) = MAGMA_tally2_S_CNJG( A(i,j) );
        }
    }
