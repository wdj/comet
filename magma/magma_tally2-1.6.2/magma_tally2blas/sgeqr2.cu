...
/**
    ...
    @ingroup magma_tally2_sgeqrf_aux
    ********************************************************************/
    ...
    if ( Aks == NULL ) {
        *info = MAGMA_tally2_ERR_DEVICE_ALLOC;
        ...
        for (i = 0; i < k; ++i) {
            ...
            magma_tally2_slarfg_gpu(m-i, dA(i, i), dA(min(i+1,m), i), dtau+i, dwork, &Aks[i]);

            if (n-i-1 > 0) {
               /* Apply H(i)' to A(i:m,i+1:n) from the left */
               magma_tally2_slarf_gpu(m-i, n-i-1, dA(i, i), dtau+i, dA(i, i+1), ldda);
            }
        }

        magma_tally2_scopymatrix( 1, k, Aks, 1, dA(0, 0), ldda+1 );
    }
    
    magma_tally2_free(Aks);
    return *info;
} /* magma_tally2_sgeqr2 */
