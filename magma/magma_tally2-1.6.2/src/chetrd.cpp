    ...
    magma_tally2_int_t *info)
{
    ...
    *info = 0;
    ...
    #endif
    if (MAGMA_tally2_SUCCESS != magma_tally2_cmalloc( &dA, ldda*n + 2*lddw*nb + ldwork2 )) {
        *info = MAGMA_tally2_ERR_DEVICE_ALLOC;
        return *info;
    }

    magma_tally2FloatComplex *dwork  = dA + ldda*n;
    #ifdef FAST_HEMV
    magma_tally2FloatComplex *dwork2 = dwork + 2*lddw*nb;
    #endif
    ...
    // rest of dwork (used as work in magma_tally2blas_chemv) doesn't need to be cleared
    magma_tally2blas_claset( Magma_tally2Full, n, nb, c_zero, c_zero, dwork, lddw );
    ...
    if (upper) {
        ...
        for (i = n - nb; i >= kk; i -= nb) {
            ...
            if (i != n-nb)
                magma_tally2_cgetmatrix( i+nb, nb, dA(0, i), ldda, A(0, i), lda );
            
            #ifdef FAST_HEMV
            magma_tally2_clatrd2( uplo, i+nb, nb, A(0, 0), lda, e, tau,
                           work, ldw, dA(0, 0), ldda, dwork, lddw,
                           dwork2, ldwork2 );
            #else
            magma_tally2_clatrd(  uplo, i+nb, nb, A(0, 0), lda, e, tau,
                           work, ldw, dA(0, 0), ldda, dwork, lddw );
            #endif

            /* Update the unreduced submatrix A(0:i-2,0:i-2), using an
               update of the form:  A := A - V*W' - W*V' */
            magma_tally2_csetmatrix( i + nb, nb, work, ldw, dwork, lddw );

            magma_tally2_cher2k( uplo, Magma_tally2NoTrans, i, nb, c_neg_one,
        ...
        magma_tally2_cgetmatrix( kk, kk, dA(0, 0), ldda, A(0, 0), lda );
        ...
        for (i = 0; i < n-nx; i += nb) {
            ...
            /* Get the current panel (no need for the 1st iteration) */
            if (i != 0)
                magma_tally2_cgetmatrix( n-i, nb, dA(i, i), ldda, A(i, i), lda );
            
            #ifdef FAST_HEMV
            magma_tally2_clatrd2( uplo, n-i, nb, A(i, i), lda, &e[i], &tau[i],
                           work, ldw, dA(i, i), ldda, dwork, lddw,
                           dwork2, ldwork2 );
            #else
            magma_tally2_clatrd(  uplo, n-i, nb, A(i, i), lda, &e[i], &tau[i],
                           work, ldw, dA(i, i), ldda, dwork, lddw );
            #endif
            
            /* Update the unreduced submatrix A(i+ib:n,i+ib:n), using
               an update of the form:  A := A - V*W' - W*V' */
            magma_tally2_csetmatrix( n-i, nb, work, ldw, dwork, lddw );

            magma_tally2_cher2k( Magma_tally2Lower, Magma_tally2NoTrans, n-i-nb, nb, c_neg_one,
        ...
        /* Use CPU code to reduce the last or only block */
        if (1 <= n-nx)
            magma_tally2_cgetmatrix( n-i, n-i, dA(i, i), ldda, A(i, i), lda );
