...
#define dF(i_, j_) (dF + (i_) + (j_)*(lddf))
    ...
    while( k < nb && lsticc == 0 ) {
        ...
        if (pvt != k) {
             magma_minproductblas_dswap( k+1, dF(pvt,0), lddf, dF(k,0), lddf);
        ...
        if (k > 0) {
            magma_minproductblas_dgemv_conjv( m-rk, k,
        ...
        magma_minproduct_dlarfg_gpu(m-rk, dA(rk, k), dA(rk + 1, k), &dtau[k], &dvn1[k], &dAkk[k]);
        magma_minproduct_dsetvector( 1, &c_one,   1, dA(rk, k), 1 );

        /* Compute Kth column of F:
           Compute  F(K+1:N,K) := tau(K)*A(RK:M,K+1:N)'*A(RK:M,K) on the GPU */
        if (k < n-1 || k > 0 ) magma_minproduct_dgetvector( 1, &dtau[k], 1, &tauk, 1 );
        if (k < n-1) {
        ...
        if (k > 0) {
            ...
            magma_minproduct_dgemv( Magma_minproductConjTrans, m-rk, k,
                         z__1,   dA(rk, 0), ldda,
                                 dA(rk, k), ione,
                         c_zero, dauxv, ione );*/

            magma_minproduct_dgemv_kernel3<<< k, BLOCK_SIZE, 0, magma_minproduct_stream >>>(m-rk, dA(rk, 0), ldda,
                                                                      dA(rk, k), dauxv, dtau+k);

            /* I think we only need stricly lower-triangular part */
            magma_minproduct_dgemv( Magma_minproductNoTrans, n-k-1, k,
                         c_one, dF(k+1,0), lddf,
                                dauxv,     ione,
                         c_one, dF(k+1,k), ione );
        }
       ...
       /* Update the current row of A:
        ...
        //*dA(rk, k) = Akk;
        //magma_minproduct_dsetvector( 1, &Akk, 1, dA(rk, k), 1 );
        //magma_minproductblas_dlacpy(Magma_minproductUpperLower, 1, 1, dAkk, 1, dA(rk, k), 1);

        ++k;
    }
    // restore the diagonals
    magma_minproduct_dcopymatrix( 1, k, dAkk, 1, dA(offset, 0), ldda+1 );
