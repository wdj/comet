...
extern "C"
void magma_minproductblas_chemm_mgpu_com(
    ...
    magma_minproductFloatComplex *work[],     magma_minproduct_int_t worksiz,  // TODO unused
    ...
    magma_minproduct_int_t gnode[Magma_minproductMaxGPUs][Magma_minproductMaxGPUs+2], magma_minproduct_int_t nbcmplx )
{
    ...
    magma_minproductFloatComplex_ptr dwork2[Magma_minproductMaxGPUs];
    ...
    for( magma_minproduct_int_t dev = 0; dev < ngpu; ++dev ) {
        ...
        cudaMemset(dwork(dev,0,0), 0, (lddwork)*(n)*sizeof(magma_minproductFloatComplex) );
        // put all dC on all dev to 0 except the one which
        // hold i==0 because this one has to multiply by beta.
        if(dev!=stdev){
           cudaMemset(dC(dev,0,0), 0, (lddc)*(n)*sizeof(magma_minproductFloatComplex) );
        }
    }
    ...
    if(blockoffset>0){
        ...
        magma_minproductblas_csymmetrize_tiles(  Magma_minproductLower,  fstblksiz,  dA(stdev, offset, myblkoffst*nb+blockoffset),  ldda,  1,  ngpu*nb,  nb  );         
    }
    ...
    for( magma_minproduct_int_t dev = 0; dev < ngpu; ++dev ) {
        ...
        magma_minproduct_setdevice( dev );
        magma_minproductblasSetKernelStream( queues[ dev ][ 0 ] );
        magma_minproductblas_csymmetrize_tiles(  Magma_minproductLower,  nb,  dA(dev, newoffset+devperm*nb, myblkoffst*nb),  ldda,  myblk,  ngpu*nb,  nb  );
        if(remm%nb>0){
            ...
            if(devperm==devlstblk)
                magma_minproductblas_csymmetrize(  Magma_minproductLower,  remm % nb,  dA(dev, newoffset+nbblk*nb, myblkoffst*nb+ myblk*nb),  ldda );  // last partial tile
        }
    }
...
/*
    ...
    magma_minproduct_cmalloc_cpu( &R, siz*siz );
    // collecte back A
    magma_minproductblas_cgetmatrix_1D_bcyclic( siz, siz, dA, ldda, R, siz, ngpu, nb );
    magma_minproduct_setdevice( 0 );
    magma_minproductblasSetKernelStream( queues[ dev ][ 0 ] );
    //magma_minproduct_cgetmatrix( siz, siz, dA[0], ldda, R, siz );
    FILE *trace_file;
...
*/
        ...
        for( magma_minproduct_int_t i = fstblksiz; i < m; i += nb ) {
            ...
            for( magma_minproduct_int_t dev = 0; dev < ngpu; ++dev ) {
                ...
                if(myrowsize>0){
                    ...
                    magma_minproduct_cgemm( Magma_minproductConjTrans, Magma_minproductNoTrans, myrowsize, n, ib,
    ...
    if(blockoffset>0){
        ...
        magma_minproduct_cgemm( Magma_minproductNoTrans, Magma_minproductNoTrans, m, n, ib,
    ...
    for( magma_minproduct_int_t i = fstblksiz; i < m; i += nb ) {
        ...
        magma_minproductblasSetKernelStream( queues[ dev ][ 0 ] );
        if(i==0){
           magma_minproduct_cgemm( Magma_minproductNoTrans, Magma_minproductNoTrans, m-i, n, ib,
        ...
        if(ngpu==1){
            ...
            // magma_minproductblasSetKernelStream( queues[ dev ][ 0 ] );
            magma_minproduct_cgemm( Magma_minproductConjTrans, Magma_minproductNoTrans, i, n, ib,
        ...
        for( magma_minproduct_int_t dev = 0; dev < ngpu; ++dev ) {
            ...
            if(myrowsize>0){
                ...
                for( magma_minproduct_int_t blki = 0; blki < myblk; ++blki){
                    ...
                    magma_minproductblas_cgeadd(ib, n, c_one, 
            ...
            for( magma_minproduct_int_t idev = 0; idev < myngpu; ++idev ) {
                ...
                if(mycolsize>0){
                    ...
                    if(dev!=masterdev){
                        ...
                        magma_minproduct_ccopymatrix_async(
                            m, n,
                            &dC[dev][0], lddc,
        ...
        if(masterdev!=-1){ 
            ...
            for( magma_minproduct_int_t l = 0; l < myngpu; ++l ) {
                ...
                if((lcdev!=masterdev)&&(lccolsize>0)){
                    ...
                    magma_minproductblas_cgeadd(m, n, c_one, 
