    ...
    magma_minproduct_int_t *info)
{
    ...
    *info = 0;
    ...
    if (nb <= 1 || nb >= min_mn) {
        /* Use unblocked code. */
        magma_minproduct_zgetf2_nopiv( m, n, A(1,1), lda, info );
    }
    else {
        /* Use blocked code. */
        for (j = 1; j <= min_mn; j += nb) {
            ...
            /* Factor diagonal and subdiagonal blocks and test for exact
               singularity. */
            i__3 = m - j + 1;
            //magma_minproduct_zgetf2_nopiv( i__3, jb, A(j,j), lda, &iinfo );

            i__3 -= jb;
            magma_minproduct_zgetf2_nopiv( jb, jb, A(j,j), lda, &iinfo );
