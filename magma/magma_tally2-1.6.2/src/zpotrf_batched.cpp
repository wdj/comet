   ...
   /* check allocation */
    ...
    if (uplo == Magma_tally2Upper) {
        ...
        for(j = 0; j < n; j+=nb) {
            ...
            #if 0
            arginfo = magma_tally2_zpotrf_panel_batched(
                               uplo, n-j, ib,
                               dA_displ, ldda,
            ...
            #else
            //arginfo = magma_tally2_zpotrf_rectile_batched(
            arginfo = magma_tally2_zpotrf_recpanel_batched(
                               uplo, n-j, ib, 32,
                               dA_displ, ldda,
                               dwork_array, dwork_msize,
                               dinvA_array, invA_msize,
            ...
            if( (n-j-ib) > 0){
                ...
                { 
                    ...
                    for(k=0; k<batchCount; k++)
                    {
                        ...
                        magma_tally2_zherk(Magma_tally2Lower, Magma_tally2NoTrans, n-j-ib, ib, 
                            d_alpha, 
                            (const magma_tally2DoubleComplex*) cpuAarray[k] + j+ib+j*ldda, ldda, 
                            d_beta,
                ...
                else
                {
                    ...
                    //-------------------------------------------
                    magma_tally2_zdisplace_pointers(dA_displ, dA_array, ldda, j+ib, j, batchCount, queue);
                    magma_tally2_zdisplace_pointers(dW1_displ, dA_array, ldda, j+ib, j+ib, batchCount, queue);
                    magma_tally2blas_zherk_batched(uplo, Magma_tally2NoTrans, n-j-ib, ib,
