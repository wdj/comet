...
/**
    ...
    1) y =       A   x      if trans == Magma_tally4NoTrans, alpha == 1, beta == 0,
                            and incx == incy == 1 (using magma_tally4blas code)
    2) y = alpha A^T x      if trans == Magma_tally4Trans, beta == 0,
                            and incx == incy == 1 (using magma_tally4blas code)
    ...
    trans   magma_tally4_trans_t
            ...
            follows:
      -     = Magma_tally4NoTrans:    y := alpha*A  *x + beta*y
      -     = Magma_tally4Trans:      y := alpha*A^T*x + beta*y
      -     = Magma_tally4ConjTrans:  y := alpha*A^T*x + beta*y
    ...
    double       *y, magma_tally4_int_t incy)
{
    ...
    if ( incx == 1 && incy == 1 && beta == 0 ) {
        if ( trans == Magma_tally4NoTrans ) {
            if ( alpha == 1. ) {
                magma_tally4_int_t blocks = (m - 1)/num_threads + 1;
                dim3 grid( blocks, 1, 1 );
                dim3 threads( num_threads, 1, 1 );
                dgemv_kernel_tesla<<< grid, threads, 0, magma_tally4_stream >>>
                    (m, n, (n/gemv_bs)*gemv_bs, A, lda, x, y);
            }
            else {
                magma_tally4_dgemv( trans, m, n, alpha, A, lda, x, incx, beta, y, incy);
            }
        }
        else {
            magma_tally4blas_dgemvt_tesla(m, n, alpha, A, lda, x, y);
        }
    }
    else {
        magma_tally4_dgemv( trans, m, n, alpha, A, lda, x, incx, beta, y, incy);
...
/**
    ...
    @ingroup magma_tally4_dblas2_internal
    ********************************************************************/
extern "C" void
magma_tally4blas_dgemvt1_tesla(
    magma_tally4_int_t m, magma_tally4_int_t n, double alpha,
    const double *A, magma_tally4_int_t lda,
    const double *x,
    double       *y )
{
    magma_tally4_int_t blocks = (n - 1)/32 + 1;
    dim3 grid( blocks, 1, 1 );
    dim3 threads( 32, 2, 1 );
    dgemvt_kernel1_tesla<<< grid, threads, 0, magma_tally4_stream >>>
...
/**
    ...
    ********************************************************************/
extern "C" void
magma_tally4blas_dgemvt2_tesla(
    magma_tally4_int_t m, magma_tally4_int_t n, double alpha,
    const double *A, magma_tally4_int_t lda,
    const double *x,
    double       *y )
{
    magma_tally4_int_t blocks = (n - 1)/16 + 1;
    dim3 grid( blocks, 1, 1 );
    dim3 threads( 16, 4, 1 );
    dgemvt_kernel2_tesla<<< grid, threads, 0, magma_tally4_stream >>>
...
{
    if ( n <= 128 )
        magma_tally4blas_dgemvt2_tesla(m, n, alpha, A, lda, x, y);
    else
        magma_tally4blas_dgemvt1_tesla(m, n, alpha, A, lda, x, y);
}
