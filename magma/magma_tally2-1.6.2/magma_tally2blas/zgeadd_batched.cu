...
zgeadd_batched_kernel(
    ...
    const magma_tally2DoubleComplex * const *dAarray, int ldda,
    magma_tally2DoubleComplex              **dBarray, int lddb )
{
    // dA and dB iterate across row i
    const magma_tally2DoubleComplex *dA = dAarray[ blockIdx.y ];
    magma_tally2DoubleComplex       *dB = dBarray[ blockIdx.y ];
    int i = blockIdx.x*blockDim.x + threadIdx.x;
    if ( i < m ) {
        dA += i;
        dB += i;
        const magma_tally2DoubleComplex *dAend = dA + n*ldda;
        while( dA < dAend ) {
            *dB = alpha*(*dA) + (*dB);
            dA += ldda;
            dB += lddb;
...
/**
    @see magma_tally2blas_zgeadd_batched_q
    @ingroup magma_tally2_zaux2
    ********************************************************************/
extern "C" void
