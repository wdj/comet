...
__global__ void
magma_tally2_zlobpcg_shift_kernel( 
    ...
    magma_tally2DoubleComplex * x )
{
    ...
    if( row<num_rows){
        magma_tally2DoubleComplex tmp = x[idx];
        __syncthreads();
...
{
    magma_tally2_int_t num_threads = num_vecs;
    ...
    int dimgrid2 = magma_tally2_ceildiv( num_rows, dimgrid1 );

    dim3 grid( dimgrid1, dimgrid2, 1);

    magma_tally2_zlobpcg_shift_kernel<<< grid, block, Ms, queue >>>
