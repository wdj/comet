...
/**
    ...
    1) y =       A   x      if trans == Magma_tally3NoTrans, alpha == 1, beta == 0,
                            and incx == incy == 1 (using magma_tally3blas code)
    2) y = alpha A^T x      if trans == Magma_tally3Trans, beta == 0,
                            and incx == incy == 1 (using magma_tally3blas code)
    ...
    if ( incx == 1 && incy == 1 && beta == 0 ) {
        if ( trans == Magma_tally3NoTrans ) {
            if ( alpha == 1. ) {
                ...
                dgemv_kernel_tesla<<< grid, threads, 0, magma_tally3_stream >>>
                    (m, n, (n/gemv_bs)*gemv_bs, A, lda, x, y);
            }
            else {
                magma_tally3_dgemv( trans, m, n, alpha, A, lda, x, incx, beta, y, incy);
            }
        }
        else {
            magma_tally3blas_dgemvt_tesla(m, n, alpha, A, lda, x, y);
        }
    }
    else {
        magma_tally3_dgemv( trans, m, n, alpha, A, lda, x, incx, beta, y, incy);
...
/**
    ...
    @ingroup magma_tally3_dblas2_internal
    ********************************************************************/
extern "C" void
magma_tally3blas_dgemvt1_tesla(
    magma_tally3_int_t m, magma_tally3_int_t n, double alpha,
    const double *A, magma_tally3_int_t lda,
    const double *x,
    double       *y )
{
    ...
    dim3 grid( blocks, 1, 1 );
    dim3 threads( 32, 2, 1 );
    dgemvt_kernel1_tesla<<< grid, threads, 0, magma_tally3_stream >>>
...
/**
    ...
    ********************************************************************/
extern "C" void
magma_tally3blas_dgemvt2_tesla(
    magma_tally3_int_t m, magma_tally3_int_t n, double alpha,
    const double *A, magma_tally3_int_t lda,
    const double *x,
    double       *y )
{
    ...
    dim3 grid( blocks, 1, 1 );
    dim3 threads( 16, 4, 1 );
    dgemvt_kernel2_tesla<<< grid, threads, 0, magma_tally3_stream >>>
...
{
    if ( n <= 128 )
        magma_tally3blas_dgemvt2_tesla(m, n, alpha, A, lda, x, y);
    else
        magma_tally3blas_dgemvt1_tesla(m, n, alpha, A, lda, x, y);
}
