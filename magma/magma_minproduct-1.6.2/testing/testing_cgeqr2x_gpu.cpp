...
int main( int argc, char** argv)
{
    ...
    magma_minproductFloatComplex *h_A, *h_T, *h_R, *tau, *h_work, tmp[1];
    magma_minproductFloatComplex_ptr d_A,  d_T, ddA, dtau;
    magma_minproductFloatComplex_ptr d_A2, d_T2, ddA2, dtau2;
    magma_minproductFloat_ptr dwork, dwork2;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_T,   magma_minproductFloatComplex, N*N    );
        
            TESTING_MALLOC_PIN( h_R,   magma_minproductFloatComplex, n2     );
        
            TESTING_MALLOC_DEV( d_A,   magma_minproductFloatComplex, ldda*N );
            TESTING_MALLOC_DEV( d_T,   magma_minproductFloatComplex, N*N    );
            TESTING_MALLOC_DEV( ddA,   magma_minproductFloatComplex, N*N    );
            TESTING_MALLOC_DEV( dtau,  magma_minproductFloatComplex, min_mn );
        
            TESTING_MALLOC_DEV( d_A2,  magma_minproductFloatComplex, ldda*N );
            TESTING_MALLOC_DEV( d_T2,  magma_minproductFloatComplex, N*N    );
            TESTING_MALLOC_DEV( ddA2,  magma_minproductFloatComplex, N*N    );
            TESTING_MALLOC_DEV( dtau2, magma_minproductFloatComplex, min_mn );
            ...
            // todo replace with magma_minproduct_claset
            magma_minproductblas_claset( Magma_minproductFull, N, N, c_zero, c_zero, ddA,  N );
            magma_minproductblas_claset( Magma_minproductFull, N, N, c_zero, c_zero, d_T,  N );
            magma_minproductblas_claset( Magma_minproductFull, N, N, c_zero, c_zero, ddA2, N );
            magma_minproductblas_claset( Magma_minproductFull, N, N, c_zero, c_zero, d_T2, N );
            ...
            magma_minproduct_csetmatrix( M, N, h_R, lda, d_A2, ldda );
            ...
            if (opts.version == 1)
                magma_minproduct_cgeqr2x_gpu(M, N, d_A, ldda, dtau, d_T, ddA, dwork, &info);
            else if (opts.version == 2)
                magma_minproduct_cgeqr2x2_gpu(M, N, d_A, ldda, dtau, d_T, ddA, dwork, &info);
            else if (opts.version == 3)
                magma_minproduct_cgeqr2x3_gpu(M, N, d_A, ldda, dtau, d_T, ddA, dwork, &info);
            else {
                printf( "call magma_minproduct_cgeqr2x4_gpu\n" );
                /*
                ...
                */
                magma_minproduct_cgeqr2x4_gpu(M, N, d_A, ldda, dtau, d_T, ddA, dwork, NULL, &info);
                //magma_minproduct_cgeqr2x4_gpu(M, N, d_A, ldda, dtau, d_T, ddA, dwork, &info, stream[1]);
                //magma_minproduct_cgeqr2x4_gpu(M, N, d_A2, ldda, dtau2, d_T2, ddA2, dwork2, &info, stream[0]);
                //magma_minproduct_cgeqr2x4_gpu(M, N, d_A2, ldda, dtau2, d_T2, ddA2, dwork2, &info, NULL);
                //gflops *= 2;
            }
            ...
            if (info != 0) {
                printf("magma_minproduct_cgeqr2x_gpu version %d returned error %d: %s.\n",
                ...
                if ( opts.check ) {
                    ...
                    magma_minproduct_cgetmatrix( N, N, ddA, N,    h_T, N );
                    ...
                    lapackf77_clacpy( Magma_minproductUpperLowerStr, &M, &N, h_R, &M, h_A, &lda );
                    lapackf77_clarft( Magma_minproductForwardStr, Magma_minproductColumnwiseStr,
                                      &M, &N, h_A, &lda, tau, h_work, &N);
                    //magma_minproduct_cgeqr2(&M, &N, h_A, &lda, tau, h_work, &info);
                    ...
                    magma_minproduct_cgetmatrix( N, N, d_T, N, h_T, N );
    
                    float terr = 0.;
                    for(int col=0; col < N; col++)
                        for(int row=0; row <= col; row++)
                            terr += (  MAGMA_minproduct_C_ABS(h_work[row + col*N] - h_T[row + col*N])*
                                       MAGMA_minproduct_C_ABS(h_work[row + col*N] - h_T[row + col*N])  );
                    terr = sqrt( terr );
