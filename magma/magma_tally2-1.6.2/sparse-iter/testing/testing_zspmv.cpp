...
int main(  int argc, char** argv )
{
    ...
    magma_tally2_z_matrix hA={Magma_tally2_CSR}, hA_SELLP={Magma_tally2_CSR}, hA_ELL={Magma_tally2_CSR}, 
    dA={Magma_tally2_CSR}, dA_SELLP={Magma_tally2_CSR}, dA_ELL={Magma_tally2_CSR};
    
    magma_tally2_z_matrix hx={Magma_tally2_CSR}, hy={Magma_tally2_CSR}, dx={Magma_tally2_CSR}, 
    ...
    magma_tally2DoubleComplex c_one  = MAGMA_tally2_Z_MAKE(1.0, 0.0);
    magma_tally2DoubleComplex c_zero = MAGMA_tally2_Z_MAKE(0.0, 0.0);
    ...
    while( i < argc ) {
        if ( strcmp("LAPLACE2D", argv[i]) == 0 && i+1 < argc ) {   // Laplace test
            ...
            CHECK( magma_tally2_zm_5stencil(  laplace_size, &hA, queue ));
        } else {                        // file-matrix test
            CHECK( magma_tally2_z_csr_mtx( &hA,  argv[i], queue ));
        }
        ...
        // init CPU vectors
        CHECK( magma_tally2_zvinit( &hx, Magma_tally2_CPU, hA.num_rows, 1, c_zero, queue ));
        CHECK( magma_tally2_zvinit( &hy, Magma_tally2_CPU, hA.num_rows, 1, c_zero, queue ));

        // init DEV vectors
        CHECK( magma_tally2_zvinit( &dx, Magma_tally2_DEV, hA.num_rows, 1, c_one, queue ));
        CHECK( magma_tally2_zvinit( &dy, Magma_tally2_DEV, hA.num_rows, 1, c_zero, queue ));
        ...
        // copy matrix to GPU
        CHECK( magma_tally2_zmtransfer( hA, &dA, Magma_tally2_CPU, Magma_tally2_DEV, queue ));
        // SpMV on GPU (CSR) -- this is the reference!
        start = magma_tally2_sync_wtime( queue );
        for (j=0; j<10; j++)
            CHECK( magma_tally2_z_spmv( c_one, dA, dx, c_zero, dy, queue ));
        end = magma_tally2_sync_wtime( queue );
        printf( " > MAGMA_tally2: %.2e seconds %.2e GFLOP/s    (standard CSR).\n",
                                        (end-start)/10, FLOPS*10/(end-start) );
        magma_tally2_zmfree(&dA, queue );
        CHECK( magma_tally2_zmtransfer( dy, &hrefvec , Magma_tally2_DEV, Magma_tally2_CPU, queue ));

        // convert to ELL and copy to GPU
        CHECK( magma_tally2_zmconvert(  hA, &hA_ELL, Magma_tally2_CSR, Magma_tally2_ELL, queue ));
        CHECK( magma_tally2_zmtransfer( hA_ELL, &dA_ELL, Magma_tally2_CPU, Magma_tally2_DEV, queue ));
        magma_tally2_zmfree(&hA_ELL, queue );
        magma_tally2_zmfree( &dy, queue );
        CHECK( magma_tally2_zvinit( &dy, Magma_tally2_DEV, hA.num_rows, 1, c_zero, queue ));
        // SpMV on GPU (ELL)
        start = magma_tally2_sync_wtime( queue );
        for (j=0; j<10; j++)
            CHECK( magma_tally2_z_spmv( c_one, dA_ELL, dx, c_zero, dy, queue ));
        end = magma_tally2_sync_wtime( queue );
        printf( " > MAGMA_tally2: %.2e seconds %.2e GFLOP/s    (standard ELL).\n",
                                        (end-start)/10, FLOPS*10/(end-start) );
        magma_tally2_zmfree(&dA_ELL, queue );
        CHECK( magma_tally2_zmtransfer( dy, &hcheck , Magma_tally2_DEV, Magma_tally2_CPU, queue ));
        res = 0.0;
        for(magma_tally2_int_t k=0; k<hA.num_rows; k++ )
            res=res + MAGMA_tally2_Z_REAL(hcheck.val[k]) - MAGMA_tally2_Z_REAL(hrefvec.val[k]);
        if ( res < .000001 )
            printf("# tester spmv ELL:  ok\n");
        else
            printf("# tester spmv ELL:  failed\n");
        magma_tally2_zmfree( &hcheck, queue );

        // convert to SELLP and copy to GPU
        CHECK( magma_tally2_zmconvert(  hA, &hA_SELLP, Magma_tally2_CSR, Magma_tally2_SELLP, queue ));
        CHECK( magma_tally2_zmtransfer( hA_SELLP, &dA_SELLP, Magma_tally2_CPU, Magma_tally2_DEV, queue ));
        magma_tally2_zmfree(&hA_SELLP, queue );
        magma_tally2_zmfree( &dy, queue );
        CHECK( magma_tally2_zvinit( &dy, Magma_tally2_DEV, hA.num_rows, 1, c_zero, queue ));
        // SpMV on GPU (SELLP)
        start = magma_tally2_sync_wtime( queue );
        for (j=0; j<10; j++)
            CHECK( magma_tally2_z_spmv( c_one, dA_SELLP, dx, c_zero, dy, queue ));
        end = magma_tally2_sync_wtime( queue );
        ...
        magma_tally2_zmfree(&dA_SELLP, queue );
        ...
        magma_tally2DoubleComplex alpha = c_one;
        magma_tally2DoubleComplex beta = c_zero;
        magma_tally2_zmfree( &dy, queue );
        CHECK( magma_tally2_zvinit( &dy, Magma_tally2_DEV, hA.num_rows, 1, c_zero, queue ));
        ...
        magma_tally2_zmfree(&hx, queue );
        magma_tally2_zmfree(&hy, queue );
        magma_tally2_zmfree(&hrefvec, queue );
        // free GPU memory
        magma_tally2_zmfree(&dA, queue );
        magma_tally2_zmfree(&dx, queue );
        magma_tally2_zmfree(&dy, queue );
