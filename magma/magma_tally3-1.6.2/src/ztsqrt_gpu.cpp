...
/**
    ...
    @ingroup magma_tally3_zgeqrf_tile
    ********************************************************************/
    ...
    magma_tally3_int_t *info )
{
    ...
    magma_tally3_int_t i, k, ldwork, lddwork, old_i, old_ib, rows, cols;
    magma_tally3_int_t nbmin, ib, ldda;
    
    /* Function Body */
    *info = 0;
    magma_tally3_int_t nb = magma_tally3_get_zgeqrf_nb(*m);
    
    magma_tally3_int_t lwkopt = (*n+*m) * nb;
    work[0] = (magma_tally3DoubleComplex) lwkopt;
    ...
    magma_tally3_int_t lhwork = *lwork - (*m)*nb;
    ...
    for (i = 0; i < k; i += nb) {
        ...
        magma_tally3_zgetmatrix_async( rows, ib,
                            ...
                            // A1(i,i), (*lda)*sizeof(magma_tally3DoubleComplex),
                            // the diagonal of A1 is in d_ref generated and
                            // passed from magma_tally3_zgeqrf_gpu
        magma_tally3_zgetmatrix_async( ib, ib,
                                d_ref(i), ib,
        ...
        if (i > 0) {
            ...
            magma_tally3_zssrfb(*m, cols, &old_ib,
        ...
        magma_tally3_zsetmatrix( *m, ib, work_A2, ldwork, A2(0,i), *lda );
        
        if (i + ib < *n) {
            // Send the triangular factor T from hwork to the GPU in t_ref(i)
            magma_tally3_zsetmatrix( ib, ib, hwork, ib, t_ref(i), lddwork );
            
            if (i+nb < k) {
                /* Apply H' to A(i:m,i+ib:i+2*ib) from the left */
                // if we can do one more step, first update T1
                magma_tally3_zssrfb(*m, ib, &ib,
