...
__global__
void magma_minproduct_clarfgx_gpu_kernel( int n, magma_minproductFloatComplex* dx0, magma_minproductFloatComplex* dx,
                               magma_minproductFloatComplex *dtau, float *dxnorm,
                               magma_minproductFloatComplex *dA, int it)
{
    ...
    if ( i == 0 ) {
        ...
        float alpha = *dx0;
        float alphai = MAGMA_minproduct_C_ZERO;
        if ( (xnorm == 0 && alphai == MAGMA_minproduct_C_ZERO ) || n == 1 )
#else
        magma_minproductFloatComplex alpha = *dx0;
        float alphar =  MAGMA_minproduct_C_REAL(alpha), alphai = MAGMA_minproduct_C_IMAG(alpha);
        if ( (xnorm == 0 && alphai == MAGMA_minproduct_C_ZERO ) || n == 0 )
#endif
        {
            ...
            *dA   = *dx0;
            ...
            if (j==0){
                *dtau = MAGMA_minproduct_C_MAKE((beta - alphar)/beta, -alphai/beta);
                //*dx0  = MAGMA_minproduct_C_MAKE(  1., 0.); //cannot be done here because raise condition all threadblock need to read it for alpha
                *dA   = MAGMA_minproduct_C_MAKE(beta, 0.);
            }
    ...
    if (j<it){
        *( dA-it+j) = *(dx0-it+j);
        *(dx0-it+j) = MAGMA_minproduct_C_MAKE(0., 0.);
    }
}
    ...
    magma_minproductFloatComplex_ptr dA, magma_minproduct_int_t iter)
{
    ...
    magma_minproduct_clarfgx_gpu_kernel<<< blocks, threads, 0, magma_minproduct_stream >>>( n, dx0, dx, dtau, dxnorm, dA, iter);
}
    ...
    magma_minproductFloatComplex_ptr dwork)
{
    ...
    magma_minproduct_clarfgx_gpu(n, dx0, dx, dtau, dxnorm, dA, iter);
    
    if (iter==0) {
        magma_minproductFloatComplex tt = MAGMA_minproduct_C_ONE;
        magma_minproductblas_clacpy(Magma_minproductUpperLower, 1, 1, dtau, 1, T+iter+iter*ldt, 1);
        magma_minproduct_csetmatrix(1,1, &tt,1, dx0,1);
    }
    else {
        /* Compute the iter-th column of T */
        magma_minproduct_cgemv_kernel3<<< iter, BLOCK_SIZE, 0, magma_minproduct_stream >>>( n, V, ldv, dx0, dwork, dtau );
        magma_minproduct_ctrmv_kernel2<<< iter, iter,       0, magma_minproduct_stream >>>( T, ldt, dwork, T+iter*ldt, dtau );
    }
}
