...
#define dAT(d, j)   (dAT[(d)]   + (j)*nb)
    ...
    if (lda < m || ngpu*ldda < n || lddw < m){
        printf( "Wrong arguments in magma_tally4blas_csetmatrix_transpose_mgpu (%d<%d), (%d*%d<%d), or (%d<%d).\n",
    ...
    for(j=0; j<n; j+=nb){
       ...
       magma_tally4blas_ctranspose_q( m, ib, dwork(d,id), lddw, dAT(d,j_local), ldda, queues[d][id] );
    }
}
