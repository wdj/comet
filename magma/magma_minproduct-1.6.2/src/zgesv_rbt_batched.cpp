                  ...
                  magma_minproduct_int_t batchCount, magma_minproduct_queue_t queue)
{
    ...
    magma_minproductDoubleComplex *hu, *hv;
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc_cpu( &hu, 2*n )) {
        info = MAGMA_minproduct_ERR_HOST_ALLOC;
        return info;
    }

    if (MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc_cpu( &hv, 2*n )) {
        info = MAGMA_minproduct_ERR_HOST_ALLOC;
        return info;
    }
    ...
    info = magma_minproduct_zgerbt_batched(Magma_minproductTrue, n, nrhs, dA_array, n, dB_array, n, hu, hv, &info, batchCount, queue);
    ...
    magma_minproduct_zsetvector(2*n, hv, 1, dv, 1);

    for(int i = 0; i < nrhs; i++)
        magma_minproductblas_zprbt_mv_batched(n, dv, dB_array+(i), batchCount, queue);
