...
zmgesellptmv_kernel_8_3D( 
    ...
    magma_tally2_index_t * dcolind,
    magma_tally2_index_t * drowptr,
    const magma_tally2DoubleComplex * __restrict__ dx,
    magma_tally2DoubleComplex beta, 
    magma_tally2DoubleComplex * dy)
{
   // T threads assigned to each row
    ...
    magma_tally2DoubleComplex * dy)
{
    ...
    if(row < num_rows ){
        magma_tally2DoubleComplex dot1 = MAGMA_tally2_Z_MAKE(0.0, 0.0);
        magma_tally2DoubleComplex dot2 = MAGMA_tally2_Z_MAKE(0.0, 0.0);
        int offset = drowptr[ bdx ];
        int max_ = (drowptr[ bdx+1 ]-offset)/blocksize;  
            // number of elements each thread handles
    ...
    magma_tally2_queue_t queue )
{
    ...
    if ( (texture==1) && (precision==1) && (kepler==1) ) {
        ...
        int Ms = num_vecs * blocksize*alignment * sizeof( magma_tally2DoubleComplex );


        if ( alignment == 1) {
            dim3 block( blocksize, num_vecs/2, 1 );
            if ( beta == MAGMA_tally2_Z_MAKE( 0.0, 0.0 ) )
        ...
        dim3 grid( dimgrid1, dimgrid2, 1);
        int Ms =  num_threads * sizeof( magma_tally2DoubleComplex );
