    ...
    magma_tally2_int_t *info)
{
    ...
    magma_tally2FloatComplex *dwork[Magma_tally2MaxGPUs];
    ...
    *info = 0;
    ...
    if (ngpu == 1 && ((nb <= 1) || (nb >= n)) ) {
        ...
        if (MAGMA_tally2_SUCCESS != magma_tally2_cmalloc_pinned( &work, n*nb )) {
            *info = MAGMA_tally2_ERR_HOST_ALLOC;
            return *info;
        }
        magma_tally2_cgetmatrix( n, n, d_lA[0], ldda, work, n );
        lapackf77_cpotrf(uplo_, &n, work, &n, info);
        magma_tally2_csetmatrix( n, n, work, n, d_lA[0], ldda );
        ...
        for( d=0; d < ngpu; d++ ) {
            ...
            if (MAGMA_tally2_SUCCESS != magma_tally2_cmalloc( &dwork[d], ngpu*nb*lddp )) {
                for( j=0; j < d; j++ ) {
        ...
        magma_tally2_setdevice(0);
        h = 1; //ngpu; //(n+nb-1)/nb;
        if (MAGMA_tally2_SUCCESS != magma_tally2_cmalloc_pinned( &work, n*nb*h )) {
            *info = MAGMA_tally2_ERR_HOST_ALLOC;
            return *info;
        }
        if (upper) {
            /* with three streams */
            magma_tally2_cpotrf3_mgpu(ngpu, uplo, n, n, 0, 0, nb, d_lA, ldda, dwork, lddp, work, n,
                               h, stream, event, info);
        } else {
            /* with three streams */
            magma_tally2_cpotrf3_mgpu(ngpu, uplo, n, n, 0, 0, nb, d_lA, ldda, dwork, lddp, work, nb*h,
                               h, stream, event, info);
        }
