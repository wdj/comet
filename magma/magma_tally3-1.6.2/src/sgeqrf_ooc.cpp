...
/**
    ...
    This is an out-of-core (ooc) version that is similar to magma_tally3_sgeqrf but
    ...
    magma_tally3_int_t *info )
{
    ...
    *info = 0;
    int nb = magma_tally3_get_sgeqrf_nb(min(m, n));

    int lwkopt = n * nb;
    work[0] = MAGMA_tally3_S_MAKE( (float)lwkopt, 0 );
    int lquery = (lwork == -1);
    if (m < 0) {
        *info = -1;
    } else if (n < 0) {
    ...
    magma_tally3_int_t IB, NB = (magma_tally3_int_t)(0.8*freeMem/m);
    NB = (NB / nb) * nb;

    if (NB >= n)
        return magma_tally3_sgeqrf(m, n, A, lda, tau, work, lwork, info);
    ...
    if (MAGMA_tally3_SUCCESS != magma_tally3_smalloc( &dA, (NB + nb)*ldda + nb*lddwork )) {
    ...
    for (int i=0; i < n; i += NB) {
        ...
        magma_tally3_ssetmatrix_async( (m), IB,
                                A(0,i),  lda,
        ...
        for (int j=0; j < min(i,k); j += nb) {
            magma_tally3_int_t ib = min(k-j, nb);
            ...
            spanel_to_q_tally3(Magma_tally3Upper, ib, A(j,j), lda, work+ib*ib);
            magma_tally3_ssetmatrix_async( rows, ib,
                                    A(j,j), lda,
                                    ptr,        rows, stream[1] );
            ...
            sq_to_panel_tally3(Magma_tally3Upper, ib, A(j,j), lda, work+ib*ib);
        }

        /* 3. Do a QR on the current part */
        if (i < k)
            magma_tally3_sgeqrf2_gpu(m-i, IB, dA(i,0), ldda, tau+i, info);

        /* 4. Copy the current part back to the CPU */
        magma_tally3_sgetmatrix_async( (m), IB,
                                dA(0,0), ldda,
                                A(0,i),  lda, stream[0] );
    }
