...
/**
    ...
    @param[out]
    dwork   (workspace) DOUBLE_PRECISION array on the GPU, dim (MAX(1,LDWORK))

    @param[in]
    ldwork  INTEGER
            The dimension of the array DWORK.
            LDWORK >= ldda*ceil(n/64) + 2*ldda*nb, where nb = magma_minproduct_get_dsytrd_nb(n),
            and 64 is for the blocksize of magma_minproductblas_dsymv.
    ...
    magma_minproduct_int_t *info)
{
    ...
    *info = 0;
    ...
    // rest of dwork (used as work in magma_minproductblas_dsymv) doesn't need to be cleared
    magma_minproductblas_dlaset( Magma_minproductFull, n, nb, c_zero, c_zero, dwork, lddw );
    ...
    if (upper) {
        ...
        for (i = n - nb; i >= kk; i -= nb) {
            ...
            magma_minproduct_dgetmatrix( i+nb, nb, dA(0, i), ldda, A(0, i), lda );
            
            magma_minproduct_dlatrd2( uplo, i+nb, nb, A(0, 0), lda, e, tau,
            ...
            magma_minproduct_dsetmatrix( i + nb, nb, work, ldw, dwork, lddw );
            
            magma_minproduct_dsyr2k( uplo, Magma_minproductNoTrans, i, nb, c_neg_one,
        ...
        magma_minproduct_dgetmatrix( kk, kk, dA(0, 0), ldda, A(0, 0), lda );
        
        /* Use CPU code to reduce the last or only block */
        lapackf77_dsytrd( uplo_, &kk, A(0, 0), &lda, d, e, tau, work, &lwork, &iinfo );
        
        magma_minproduct_dsetmatrix( kk, kk, A(0, 0), lda, dA(0, 0), ldda );
    }
    else {
        ...
        for (i = 0; i < n-nx; i += nb) {
            ...
            /* Get the current panel */
            magma_minproduct_dgetmatrix( n-i, nb, dA(i, i), ldda, A(i, i), lda );
            
            magma_minproduct_dlatrd2( uplo, n-i, nb, A(i, i), lda, &e[i], &tau[i],
                           work, ldw, dA(i, i), ldda, dwork, lddw,
                           dwork + 2*lddw*nb, ldwork - 2*lddw*nb );
            
            /* Update the unreduced submatrix A(i+ib:n,i+ib:n), using
               an update of the form:  A := A - V*W' - W*V' */
            magma_minproduct_dsetmatrix( n-i, nb, work, ldw, dwork, lddw );
            
            // cublas 6.5 crashes here if lddw % 32 != 0, e.g., N=250.
            magma_minproduct_dsyr2k( Magma_minproductLower, Magma_minproductNoTrans, n-i-nb, nb, c_neg_one,
        ...
        /* Use CPU code to reduce the last or only block */
        magma_minproduct_dgetmatrix( n-i, n-i, dA(i, i), ldda, A(i, i), lda );
        
        i_n = n-i;
        lapackf77_dsytrd( uplo_, &i_n, A(i, i), &lda, &d[i], &e[i],
                          &tau[i], work, &lwork, &iinfo );
        
        magma_minproduct_dsetmatrix( n-i, n-i, A(i, i), lda, dA(i, i), ldda );
    }
