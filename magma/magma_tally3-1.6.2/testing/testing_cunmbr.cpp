...
int main( int argc, char** argv )
{
    ...
    magma_tally3FloatComplex *C, *R, *A, *work, *tau, *tauq, *taup;
    float *d, *e;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int itran = 0; itran < 2; ++itran ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            n = opts.nsize[itest];
            k = opts.ksize[itest];
            nb  = magma_tally3_get_cgebrd_nb( m );
            ldc = m;
            ...
            TESTING_MALLOC_CPU( A,    magma_tally3FloatComplex, lda*nn );
            TESTING_MALLOC_CPU( work, magma_tally3FloatComplex, lwork_max );
            TESTING_MALLOC_CPU( d,    float,             min(mm,nn) );
            TESTING_MALLOC_CPU( e,    float,             min(mm,nn) );
            TESTING_MALLOC_CPU( tauq, magma_tally3FloatComplex, min(mm,nn) );
            TESTING_MALLOC_CPU( taup, magma_tally3FloatComplex, min(mm,nn) );
            ...
            magma_tally3_cgebrd( mm, nn, A, lda, d, e, tauq, taup, work, lwork_max, &info );
            if (info != 0)
                printf("magma_tally3_cgebrd returned error %d: %s.\n",
            ...
            lapackf77_cunmbr( lapack_vect_const_tally3( vect[ivect] ),
            ...
            lwork = -1;
            magma_tally3_cunmbr( vect[ivect], side[iside], trans[itran],
                          m, n, k,
                          A, lda, tau, R, ldc, work, lwork, &info );
            if (info != 0)
                printf("magma_tally3_cunmbr (lwork query) returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
            lwork = (magma_tally3_int_t) MAGMA_tally3_C_REAL( work[0] );
            ...
            if (info != 0)
                printf("magma_tally3_cunmbr returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
