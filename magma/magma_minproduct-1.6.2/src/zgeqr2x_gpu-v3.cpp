    ...
    magma_minproductDoubleComplex_ptr       dwork, magma_minproduct_int_t ldwork )
{
    ...
    // W = W T^H = C^H V T^H
    magma_minproduct_ztrmm( Magma_minproductRight, Magma_minproductUpper, Magma_minproductNoTrans, Magma_minproductNonUnit,
                 n, k,
                 c_one, dT,    lddt,
                        dwork, ldwork);
    ...
    *info = 0;
    ...
    for (int b=0; b < k; b += BLOCK_SIZE) {
        for (i = b; i < min(k, b+BLOCK_SIZE); ++i) {
            ...
            if ( i-b > 0)
                magma_minproduct_zlarfbx_gpu(m-b, i-b, dA(b, b), ldda,
                                  dT+b+b*k, k, dA(b, i), work);

            /*   Adjust the dnorm[i] to hold the norm of A(i:m,i) */
            //if ( i > 0 )
            //    magma_minproductblas_dznrm2_adjust(i, dnorm+i, dA(0, i));
            magma_minproductblas_dznrm2_cols(m-i, 1, dA(i,i), ldda, dnorm+i);
            ...
            /*  Generate elementary reflector H(i) to annihilate A(i+1:m,i)
                ...
                3. update T */
            magma_minproduct_zlarfgtx_gpu(m-i, dA(i, i), dA(min(i+1,m), i), dtau+i,
                               dnorm+i, ddA + i + i*(n), i,
