...
/**
    ...
    This version stores the T matrices in dT, for later use in magma_tally2_dorghr.
    ...
    magma_tally2_int_t *info)
{
    ...
    *info = 0;
    ...
    if (nb == 1 || nb > nh) {
        ...
        //   n*ldda  for dA
        double *dwork;
        if (MAGMA_tally2_SUCCESS != magma_tally2_dmalloc( &dwork, 2*nb*ldda + n*ldda )) {
        ...
        double *T;
        magma_tally2_dmalloc_cpu( &T, nb*nb );
        if ( T == NULL ) {
        ...
        magma_tally2blas_dlaset( Magma_tally2Full, nb, nb, c_zero, c_zero, dV, ldda );
        ...
        magma_tally2blas_dlaset( Magma_tally2Full, nb, n, c_zero, c_zero, dT, nb );
        
        // Copy the matrix to the GPU
        magma_tally2_dsetmatrix( n, n-ilo, A(0,ilo), lda, dA, ldda );
        ...
        for (i = ilo; i < ihi-1 - nb; i += nb) {
            ...
            magma_tally2_dlahr2( ihi, i+1, nb,
                          dA(0,i-ilo), ldda,
                          dV,          ldda,
                          A(0,i),      lda,
                          &tau[i], T, nb, work, n);
            ...
            dTi = dT + (i - ilo)*nb;
            magma_tally2_dsetmatrix( nb, nb, T, nb, dTi, nb );
            
            magma_tally2_dlahru( n, ihi, i, nb,
                          A(0,i),      lda,
                          dA(0,i-ilo), ldda, // dA
