    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for( int dev=0; dev < ngpu; dev++ ) {
                ...
                TESTING_MALLOC_DEV( d_lA[dev], magma_tally3FloatComplex, max_size );
            }
            ...
            if ( opts.lapack ) {
                ...
                lapackf77_cpotrf( lapack_uplo_const_tally3(opts.uplo), &N, h_A, &lda, &info );
            ...
            if ( opts.uplo == Magma_tally3Upper ) {
                ldda = ((N+nb-1)/nb)*nb;
                magma_tally3_csetmatrix_1D_col_bcyclic( N, N, h_R, lda, d_lA, ldda, ngpu, nb );
            } else {
                ldda = (1+N/(nb*ngpu))*nb;
                magma_tally3_csetmatrix_1D_row_bcyclic( N, N, h_R, lda, d_lA, ldda, ngpu, nb );
            }
            
            gpu_time = magma_tally3_wtime();
            magma_tally3_cpotrf_mgpu( ngpu, opts.uplo, N, d_lA, ldda, &info );
            gpu_time = magma_tally3_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally3_cpotrf_mgpu returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
            
            if ( opts.uplo == Magma_tally3Upper ) {
                magma_tally3_cgetmatrix_1D_col_bcyclic( N, N, d_lA, ldda, h_R, lda, ngpu, nb );
            } else {
                magma_tally3_cgetmatrix_1D_row_bcyclic( N, N, d_lA, ldda, h_R, lda, ngpu, nb );
            }
