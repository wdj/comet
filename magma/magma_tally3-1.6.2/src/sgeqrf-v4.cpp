/*
    -- MAGMA_tally3 (version 1.6.1) --
       ...
       @generated from zgeqrf-v4.cpp normal z -> s, Fri Jan 30 19:00:16 2015
    ...
    magma_tally3_int_t *info )
{
    ...
    *info = 0;
    ...
    int lquery = (lwork == -1);
    if (ngpu < 0 || ngpu > Magma_tally3MaxGPUs) {
        *info = -1;
    } else if (m < 0) {
        *info = -2;
    } else if (n < 0) {
    ...
    for (i=0; i < ngpu; i++) {
        ...
        if (MAGMA_tally3_SUCCESS != magma_tally3_smalloc( &da[i], ldda*n_local[i] )) {
    ...
    if (m > nb && n > nb) {
        ...
        magma_tally3_ssetmatrix_1D_col_bcyclic(m, n, A, lda, da, ldda, ngpu, nb);

        /* Factor using the GPU interface */
        magma_tally3_sgeqrf2_mgpu( ngpu, m, n, da, ldda, tau, info);
