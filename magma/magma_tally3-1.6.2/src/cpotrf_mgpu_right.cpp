    ...
    magma_tally3_int_t *info )
{
    ...
    magma_tally3FloatComplex z_one = MAGMA_tally3_C_MAKE(  1.0, 0.0 );
    magma_tally3FloatComplex mz_one = MAGMA_tally3_C_MAKE( -1.0, 0.0 );
    float             one =  1.0;
    float             m_one = -1.0;
    ...
    magma_tally3FloatComplex *panel, *tmppanel0, *tmppanel1, *tmppanel, *tmpprevpanel;
    magma_tally3FloatComplex *d_lP[Magma_tally3MaxGPUs], *dlpanel, *dlpanels[Magma_tally3MaxGPUs];
    ...
    *info = 0;
    ...
    if (MAGMA_tally3_SUCCESS != magma_tally3_cmalloc_pinned( &panel, 2 * nb * ldpanel )) {
    ...
    if ((nb <= 1) || (nb >= n)) {
        // Use unblocked code.
        magma_tally3_cgetmatrix( n, n, dlA(0, 0, 0), ldda, panel, ldpanel);
        lapackf77_cpotrf( uplo_, &n, panel, &ldpanel, info);
        magma_tally3_csetmatrix( n, n, panel, ldpanel, dlA(0, 0, 0), ldda );
    } else {
        for( d = 0; d < ngpu; d++ ) {
            ...
            if (MAGMA_tally3_SUCCESS != magma_tally3_cmalloc( &d_lP[d], nb * ldda )) {
                for( j = 0; j < d; j++ ) {
        ...
        if (uplo == Magma_tally3Upper) {
            ...
            if ( n > nb ) {
                ...
                magma_tally3_setdevice(0);
                tmppanel = tmppanel0;
                magma_tally3_cgetmatrix_async(n, nb,
                        dlA(0, 0, 0), ldda,
                        tmppanel(0),  ldpanel,
                        queues[0][0] );
            }
            ...
            for (j = 0; (j + nb) < n; j += nb) {
                ...
                if (j > 0 && prevtrsmrows > crosspoint) {
                    ...
                    blasf77_cgemm( Magma_tally3NoTransStr, Magma_tally3ConjTransStr,
                            &rows, &nb, &nb,
                ...
                #endif
                lapackf77_cpotrf(Magma_tally3LowerStr, &nb, tmppanel(j), &ldpanel, info);
                if (*info != 0) {
                    *info = *info + j;
                    break;
                }
                ...
                if (trsmrows > 0) {
                    blasf77_ctrsm(Magma_tally3RightStr, Magma_tally3LowerStr, Magma_tally3ConjTransStr, Magma_tally3NonUnitStr,
                                  &trsmrows, &nb,
                                  &z_one, tmppanel(j), &ldpanel,
                ...
                for (igpu = 0; igpu < ngpu; igpu++, d = (d + 1) % ngpu ) {
                    ...
                    if (myrows > 0) {
                        magma_tally3_setdevice(d);
                        magma_tally3_csetmatrix_async(myrows, nb,
                ...
                if ( (j + nb) < n) {
                    ...
                    if (j + nb + nb < n) {
                        ...
                        if (trsmrows <= crosspoint) {
                            ...
                            #ifdef  CHERK_ON_DIAG
                            magma_tally3_cherk( Magma_tally3Lower, Magma_tally3NoTrans,
                                    nb, nb,
                                    m_one, dlpanel, ldda,
                        ...
                        magma_tally3_cgetmatrix_async(rows-nb, nb,
                    ...
                    if (n - offset > 0) {
                        ...
                        //magma_tally3_cherk(Magma_tally3Lower, Magma_tally3NoTrans, n - offset, nb,
                        //        m_one, dlpanel, ldda,
            ...
            if (j < n && *info == 0) {
                ...
                #endif
                magma_tally3_cgetmatrix(rows, rows, dlA(id, j, j_local), ldda, panel(j), ldpanel);
                lapackf77_cpotrf(Magma_tally3LowerStr, &rows, panel(j), &ldpanel, info);
                magma_tally3_csetmatrix(rows, rows, panel(j), ldpanel, dlA(id, j, j_local), ldda);
                #if defined (ENABLE_TIMER)
