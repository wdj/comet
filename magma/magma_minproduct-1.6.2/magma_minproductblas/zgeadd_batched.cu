...
zgeadd_batched_kernel(
    ...
    const magma_minproductDoubleComplex * const *dAarray, int ldda,
    magma_minproductDoubleComplex              **dBarray, int lddb )
{
    // dA and dB iterate across row i
    const magma_minproductDoubleComplex *dA = dAarray[ blockIdx.y ];
    magma_minproductDoubleComplex       *dB = dBarray[ blockIdx.y ];
    int i = blockIdx.x*blockDim.x + threadIdx.x;
    if ( i < m ) {
        dA += i;
        dB += i;
        const magma_minproductDoubleComplex *dAend = dA + n*ldda;
        while( dA < dAend ) {
            *dB = alpha*(*dA) + (*dB);
            dA += ldda;
            dB += lddb;
...
/**
    @see magma_minproductblas_zgeadd_batched_q
    @ingroup magma_minproduct_zaux2
    ********************************************************************/
extern "C" void
...
{
    magma_minproductblas_zgeadd_batched_q(
        m, n, alpha, dAarray, ldda, dBarray, lddb, batchCount, magma_minproduct_stream );
}
