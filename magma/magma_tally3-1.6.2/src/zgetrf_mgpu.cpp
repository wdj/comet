    ...
    magma_tally3_int_t *info)
{
    ...
    magma_tally3DoubleComplex *d_lAT[Magma_tally3MaxGPUs];
    magma_tally3DoubleComplex *d_panel[Magma_tally3MaxGPUs], *work;
    ...
    *info = 0;
    ...
    if (nb <= 1 || nb >= n) {
        ...
        magma_tally3_zgetmatrix( m, n, d_lA[0], ldda, work, m );
        lapackf77_zgetrf(&m, &n, work, &m, ipiv, info);
        magma_tally3_zsetmatrix( m, n, work, m, d_lA[0], ldda );
        ...
        for (i=0; i < ngpu; i++) {
            ...
            /* workspaces */
            if (MAGMA_tally3_SUCCESS != magma_tally3_zmalloc( &d_panel[i], (3+ngpu)*nb*maxm )) {
                for( j=0; j <= i; j++ ) {
            ...
            if (MAGMA_tally3_SUCCESS != magma_tally3_zmalloc( &d_lAT[i], lddat*maxm )) {
                for( j=0; j <= i; j++ ) {
                    magma_tally3_setdevice(j);
                    magma_tally3_free( d_panel[j] );
                }
            ...
            magma_tally3blas_ztranspose( m, n_local[i], d_lA[i], ldda, d_lAT[i], lddat );
        }
        for (i=0; i < ngpu; i++) {
            magma_tally3_setdevice(i);
        ...
        /* cpu workspace */
        lddwork = maxm;
        if (MAGMA_tally3_SUCCESS != magma_tally3_zmalloc_pinned( &work, lddwork*nb*ngpu )) {
            for (i=0; i < ngpu; i++ ) {
                magma_tally3_setdevice(i);
        ...
        magma_tally3_zgetrf2_mgpu(ngpu, m, n, nb, 0, d_lAT, lddat, ipiv, d_panel, work, maxm,
                           streaml, info);
        ...
        for( d=0; d < ngpu; d++ ) {
            ...
            /* save on output */
            magma_tally3blas_ztranspose( n_local[d], m, d_lAT[d], lddat, d_lA[d], ldda );
