...
magma_tally3_dresidualvec(
    ...
    magma_tally3_d_matrix *r, double *res,
    magma_tally3_queue_t queue )
{
    ...
    if ( A.num_rows == b.num_rows ) {
        ...
        CHECK( magma_tally3_d_spmv( mone, A, x, zero, *r, queue ));      // r = A x
        magma_tally3_daxpy(dofs, one, b.dval, 1, r->dval, 1);          // r = r - b
        *res =  magma_tally3_dnrm2(dofs, r->dval, 1);            // res = ||r||
        ...
        for( magma_tally3_int_t i=0; i<num_vecs; i++) {
            magma_tally3_daxpy(dofs, one, b(i), 1, r(i), 1);   // r = r - b
