...
/**
    Purpose
    -------
    If VECT = Magma_tally2Q, ZUNMBR overwrites the general complex M-by-N matrix C with
                                 SIDE = Magma_tally2Left     SIDE = Magma_tally2Right
    TRANS = Magma_tally2NoTrans:        Q*C                  C*Q
    TRANS = Magma_tally2_ConjTrans:     Q**H*C               C*Q**H
    
    If VECT = Magma_tally2P, ZUNMBR overwrites the general complex M-by-N matrix C with
                                 SIDE = Magma_tally2Left     SIDE = Magma_tally2Right
    ...
    *info = 0;
    ...
    if (*info == 0) {
        if (nw > 0) {
            ...
            nb = magma_tally2_get_zgebrd_nb( min( m, n ));
            lwkopt = max(1, nw*nb);
        }
        else {
            lwkopt = 1;
    ...
    if (applyq) {
        ...
        if (nq >= k) {
            ...
            #else
            magma_tally2_zunmqr( side, trans,
                          m, n, k, A, lda, tau, C, ldc, work, lwork, &iinfo);
            #endif
        }
        else if (nq > 1) {
        ...
        if (nq > k) {
            ...
            #if VERSION == 1
            lapackf77_zunmlq( lapack_side_const_tally2(side), lapack_trans_const_tally2(transt),
                              &m, &n, &k, A, &lda, tau, C, &ldc, work, &lwork, &iinfo);
            #else
            magma_tally2_zunmlq( side, transt,
                          m, n, k, A, lda, tau, C, ldc, work, lwork, &iinfo);
            #endif
        }
        else if (nq > 1) {
