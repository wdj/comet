    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( norm[inorm] == Magma_tally4OneNorm )
                ...
                lwork = M;
            // read whole matrix
            gbytes = M*N*sizeof(magma_tally4DoubleComplex) / 1e9;
            ...
            //h_A[ 1 + 1*lda ] = MAGMA_tally4_Z_NAN;
            ...
            norm_magma_tally4 = magma_tally4blas_zlange( norm[inorm], M, N, d_A, ldda, d_work );
            gpu_time = magma_tally4_wtime() - gpu_time;
            gpu_perf = gbytes / gpu_time;
            if (norm_magma_tally4 < 0)
                printf("magma_tally4blas_zlange returned error %f: %s.\n",
            ...
            norm_lapack = lapackf77_zlange( lapack_norm_const_tally4(norm[inorm]), &M, &N, h_A, &lda, h_work );
            ...
            //         MAGMA_tally4_Z_REAL( h_A[1+1*lda] ),
            //         MAGMA_tally4_Z_IMAG( h_A[1+1*lda] ) );
