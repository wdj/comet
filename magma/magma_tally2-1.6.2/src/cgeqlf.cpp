...
/**
    ...
    lwork   INTEGER
            ...
            through magma_tally2_get_cgeqlf_nb(M).
    \n
    ...
    magma_tally2_int_t *info)
{
    ...
    magma_tally2FloatComplex_ptr dA, dwork;
    ...
    nb = magma_tally2_get_cgeqlf_nb(m);
    *info = 0;
    lquery = (lwork == -1);
    ...
    if (*info == 0) {
        if (k == 0)
            ...
            work[0] = MAGMA_tally2_C_MAKE( max(n*nb, 2*nb*nb), 0 );
        }
    ...
    if (MAGMA_tally2_SUCCESS != magma_tally2_cmalloc( &dA, (n)*ldda + nb*lddwork )) {
    ...
    if ( (nb > 1) && (nb < k) ) {
        ...
        for (i = k - kk + ki; i >= k -kk; i -= nb) {
            ...
            if (i < k - kk + ki) {
                ...
                magma_tally2_cgetmatrix_async( m-rows, ib,
                                        dA(rows, n-k+i), ldda,
                ...
                magma_tally2_clarfb_gpu( Magma_tally2Left, Magma_tally2ConjTrans, Magma_tally2Backward, Magma_tally2Columnwise,
                                  rows, cols, old_ib,
            ...
            if (cols > 0) {
                ...
                lapackf77_clarft( Magma_tally2BackwardStr, Magma_tally2ColumnwiseStr,
                                  &rows, &ib,
                                  A(0, cols), &lda, tau + i, work, &ib);

                cpanel_to_q_tally2( Magma_tally2Lower, ib, A(rows-ib,cols), lda, work+ib*ib);
                magma_tally2_csetmatrix( rows, ib,
                                  A(0,cols),  lda,
                                  dA(0,cols), ldda );
                cq_to_panel_tally2( Magma_tally2Lower, ib, A(rows-ib,cols), lda, work+ib*ib);

                // Send the triangular part on the GPU
                magma_tally2_csetmatrix( ib, ib, work, ib, dwork(0), lddwork );
        ...
        magma_tally2_cgetmatrix( m, nu, dA(0,0), ldda, A(0,0), lda );
    } else {
        mu = m;
        nu = n;
    }
