...
#define dA(i, j) (dwork + (i) + (j)*(ldda))
    ...
    magma_minproductDoubleComplex   *dwork, *df;
    ...
    *info = 0;
    ...
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc( &dwork, ldwork )) {
    ...
    if (nfxd < minmn) {
        ...
        if (nb < sminmn) {
            ...
            magma_minproduct_zsetmatrix_async( m, sn,
                                    A (0,j), lda,
                                    dA(0,j), ldda, stream );
        }
        ...
        for (j = nfxd; j < n; ++j) {
            rwork[j] = magma_minproduct_cblas_dznrm2( sm, A(nfxd,j), ione );
            rwork[n + j] = rwork[j];
        }
        ...
        if (nb < sminmn) {
            ...
            while(j < topbmn) {
                ...
                if (j > nfxd) {
                    ...
                    magma_minproduct_zgetmatrix( m-j, jb,
                                      dA(j,j), ldda,
                                      A (j,j), lda );
                    
                    // Get the rows
                    magma_minproduct_zgetmatrix( jb, n_j - jb,
                                      dA(j,j + jb), ldda,
                                      A (j,j + jb), lda );
                }

                magma_minproduct_zlaqps( m, n_j, j, jb, &fjb,
                              A (0, j), lda,
                              dA(0, j), ldda,
