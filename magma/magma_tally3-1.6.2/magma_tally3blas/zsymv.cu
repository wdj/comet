    ...
    magma_tally3DoubleComplex * __restrict__ work)
{
    ...
    __shared__ magma_tally3DoubleComplex sA [quarter_NB_X][NB_X + 3]; /* Why +3? seems it only needs +2. Does +3 reduce bank conflicts? */
...
/**
    Purpose
    -------
    magma_tally3blas_zsymv_work performs the matrix-vector operation:
    ...
    uplo    magma_tally3_uplo_t.
            ...
            follows:
      -     = Magma_tally3Upper:  Only the upper triangular part of A is to be referenced.
    ...
    MAGMA_tally3 implements zsymv through two steps:
    ...
    magma_tally3blas_zsymv is a wrapper routine allocating the workspace inside the
    routine and provides the same interface as cublas.
    
    If users need to call zsymv frequently, we suggest using
    magma_tally3blas_zsymv_work instead of magma_tally3blas_zsymv. As the overhead to
    allocate and free in device memory in magma_tally3blas_zsymv would hurt performance.
    ...
    magma_tally3_queue_t queue )
{
    ...
    if ( arch < 200 ) {
        //magma_tally3_zsymv( uplo, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    ...
    magma_tally3_int_t blocks = ceildiv( n, NB_X );
    magma_tally3_int_t lwmin  = ldda*blocks;
     ...
     */
    if ( (n == 0) || ( MAGMA_tally3_Z_EQUAL(alpha, MAGMA_tally3_Z_ZERO) && MAGMA_tally3_Z_EQUAL(beta, MAGMA_tally3_Z_ONE) ) )
        return info;
...
// end magma_tally3blas_zsymv_work


/**
    Purpose
    -------
    magma_tally3blas_zsymv performs the matrix-vector operation:
    ...
    magma_tally3DoubleComplex_ptr dy, magma_tally3_int_t incy)
{
    ...
    magma_tally3_int_t blocks = ceildiv( n, NB_X );
    magma_tally3_int_t lwork  = ldda*blocks;

    magma_tally3_zmalloc( &dwork, lwork );
    if ( dwork == NULL ) {
    ...
    magma_tally3blas_zsymv_work( uplo, n, alpha, dA, ldda, dx, incx, beta, dy, incy,
                          dwork, lwork, magma_tally3_stream );
    
    magma_tally3_free( dwork );
    
    return info;
}
// end magma_tally3blas_zsymv
