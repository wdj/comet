...
int main( int argc, char** argv)
{
    ...
    magma_tally3FloatComplex *h_A, *h_A2, *h_B, *h_X, *h_R, *tau, *h_work, tmp[1];
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lhwork2 = (magma_tally3_int_t) MAGMA_tally3_C_REAL( tmp[0] );
            
            lhwork = -1;
            lapackf77_cunmqr( Magma_tally3LeftStr, Magma_tally3_ConjTransStr,
                              &M, &nrhs, &min_mn, NULL, &lda, NULL,
                              NULL, &ldb, tmp, &lhwork, &info);
            lhwork = (magma_tally3_int_t) MAGMA_tally3_C_REAL( tmp[0] );
            ...
            TESTING_MALLOC_CPU( h_A2,   magma_tally3FloatComplex, lda*N     );
            TESTING_MALLOC_CPU( h_B,    magma_tally3FloatComplex, ldb*nrhs  );
            TESTING_MALLOC_CPU( h_X,    magma_tally3FloatComplex, ldb*nrhs  );
            TESTING_MALLOC_CPU( h_R,    magma_tally3FloatComplex, ldb*nrhs  );
            TESTING_MALLOC_CPU( h_work, magma_tally3FloatComplex, lhwork    );
            ...
            lapackf77_clacpy( Magma_tally3UpperLowerStr, &M, &N, h_A, &lda, h_A2, &lda );
            
            // make random RHS
            size = M*nrhs;
            lapackf77_clarnv( &ione, ISEED, &size, h_B );
            lapackf77_clacpy( Magma_tally3UpperLowerStr, &M, &nrhs, h_B, &ldb, h_R, &ldb );
            
            // make consistent RHS
            //size = N*nrhs;
            //lapackf77_clarnv( &ione, ISEED, &size, h_X );
            //blasf77_cgemm( Magma_tally3NoTransStr, Magma_tally3NoTransStr, &M, &nrhs, &N,
            //               &c_one,  h_A, &lda,
            //                        h_X, &ldb,
            //               &c_zero, h_B, &ldb );
            //lapackf77_clacpy( Magma_tally3UpperLowerStr, &M, &nrhs, h_B, &ldb, h_R, &ldb );
            ...
            magma_tally3_csetmatrix( M, nrhs, h_B, ldb, d_B, lddb );
            
            gpu_time = magma_tally3_wtime();
            magma_tally3_cgels3_gpu( Magma_tally3NoTrans, M, N, nrhs, d_A, ldda,
                              d_B, lddb, h_work, lworkgpu, &info);
            gpu_time = magma_tally3_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally3_cgels3_gpu returned error %d: %s.\n",
            ...
            /* =====================================================================
               ...
               =================================================================== */
            lapackf77_clacpy( Magma_tally3UpperLowerStr, &M, &nrhs, h_B, &ldb, h_X, &ldb );
            
            cpu_time = magma_tally3_wtime();
            lapackf77_cgels( Magma_tally3NoTransStr, &M, &N, &nrhs,
