...
int main( int argc, char** argv)
{
    ...
    magma_tally3Float_ptr d_R, dwork;
    float *tau;
    float          *diag, *offdiag;
    float           result[2] = {0., 0.};
    ...
    printf("2 - uses SSYMV from MAGMA_tally3 BLAS that requires extra space\n\n");
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if (opts.version == 1) {
                magma_tally3_ssytrd_gpu( opts.uplo, N, d_R, ldda, diag, offdiag,
                                  tau, h_R, lda, h_work, lwork, &info );
            }
            else {
                magma_tally3_ssytrd2_gpu( opts.uplo, N, d_R, ldda, diag, offdiag,
                                   tau, h_R, lda, h_work, lwork, dwork, ldwork, &info );
            }
            gpu_time = magma_tally3_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally3_ssytrd_gpu returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                magma_tally3_sgetmatrix( N, N, d_R, ldda, h_Q, lda );
                lapackf77_sorgtr( lapack_uplo_const_tally3(opts.uplo), &N, h_Q, &lda, tau, h_work, &lwork, &info );
                
                lapackf77_ssyt21( &itwo, lapack_uplo_const_tally3(opts.uplo), &N, &ione,
                ...
                lapackf77_ssyt21( &ithree, lapack_uplo_const_tally3(opts.uplo), &N, &ione,
                                  h_A, &lda, diag, offdiag,
                                  h_Q, &lda, h_R, &lda,
                                  tau, work,
                                  #ifdef COMPLEX
            ...
            if ( opts.lapack ) {
                ...
                lapackf77_ssytrd( lapack_uplo_const_tally3(opts.uplo), &N, h_A, &lda, diag, offdiag, tau,
                                  h_work, &lwork, &info );
