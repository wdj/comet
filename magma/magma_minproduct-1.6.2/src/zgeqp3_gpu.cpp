...
/**
    ...
    @ingroup magma_minproduct_zgeqp3_comp
    ********************************************************************/
    ...
    magma_minproduct_int_t *info )
{
    ...
    //magma_minproduct_int_t na;
    magma_minproduct_int_t n_j;
    magma_minproduct_int_t j, jb, nb, sm, sn, fjb, nfxd, minmn;
    magma_minproduct_int_t topbmn, lwkopt;
    ...
    *info = 0;
    ...
    nb = magma_minproduct_get_zgeqp3_nb(min(m, n));
    minmn = min(m,n);
    if (*info == 0) {
        if (minmn == 0) {
            lwkopt = 1;
    ...
    #endif
    magma_minproductDoubleComplex_ptr df;
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc( &df, (n+1)*nb )) {
        *info = MAGMA_minproduct_ERR_DEVICE_ALLOC;
        return *info;
    }
    cudaMemset( df, 0, (n+1)*nb*sizeof(magma_minproductDoubleComplex) );
    ...
    if (nfxd > 0) {
        ...
        if (na < n) {
            n_j = n - na;
            lapackf77_zunmqr( Magma_minproductLeftStr, Magma_minproductConjTransStr, &m, &n_j, &na,
    ...
    if (nfxd < minmn) {
        ...
        magma_minproductblas_dznrm2_cols(sm, sn, dA(nfxd,nfxd), ldda, &rwork[nfxd]);
        magma_minproduct_dcopymatrix( sn, 1, &rwork[nfxd], sn, &rwork[n+nfxd], sn);
        ...
        //if (nb < sminmn)
        {
            ...
            while(j < topbmn) {
                ...
                //magma_minproduct_zlaqps_gpu    // this is a cpp-file
                magma_minproduct_zlaqps2_gpu   // this is a cuda-file
                    ( m, n_j, j, jb, &fjb,
                      dA(0, j), ldda,
        ...
        if (j < minmn) {
            n_j = n - j;
            if (j > nfxd) {
                magma_minproduct_zgetmatrix( m-j, n_j,
                                  dA(j,j), ldda,
                                   A(j,j), lda );
            }
