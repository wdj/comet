    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( opts.version == 2 ) {
                ...
                magma_tally4_sgeqrf2_gpu( M, N, d_A, ldda, tau, &info );
            }
            else {
                ...
                if ( opts.version == 1 ) {
                    ...
                    magma_tally4_sgeqrf_gpu( M, N, d_A, ldda, tau, dT, &info );
                }
                #ifdef HAVE_CUBLAS
                else if ( opts.version == 3 ) {
                    // stores dT, V blocks have zeros, R blocks stored in dT
                    magma_tally4_sgeqrf3_gpu( M, N, d_A, ldda, tau, dT, &info );
                }
                #endif
                else {
            ...
            if (info != 0)
                printf("magma_tally4_sgeqrf returned error %d: %s.\n",
            ...
            if ( opts.check && opts.version == 2 ) {
                ...
                magma_tally4_ssetvector( M, b, 1, d_B, 1 );

                if ( opts.version == 1 ) {
                    // allocate hwork
                    magma_tally4_sgeqrs_gpu( M, N, 1,
                                      d_A, ldda, tau, dT,
                                      d_B, M, tmp, -1, &info );
                    lwork = (magma_tally4_int_t)MAGMA_tally4_S_REAL( tmp[0] );
                    ...
                    if (info != 0)
                        printf("magma_tally4_sgeqrs returned error %d: %s.\n",
                               (int) info, magma_tally4_strerror( info ));
                    ...
                    // allocate hwork
                    magma_tally4_sgeqrs3_gpu( M, N, 1,
                                       d_A, ldda, tau, dT,
                                       d_B, M, tmp, -1, &info );
                    lwork = (magma_tally4_int_t)MAGMA_tally4_S_REAL( tmp[0] );
                    TESTING_MALLOC_CPU( hwork, float, lwork );
                    ...
                    magma_tally4_sgeqrs3_gpu( M, N, 1,
                                       ...
                                       d_B, M, hwork, lwork, &info );
                    if (info != 0)
                        printf("magma_tally4_sgeqrs3 returned error %d: %s.\n",
                               (int) info, magma_tally4_strerror( info ));
                    TESTING_FREE_CPU( hwork );
                }
                #endif
                ...
                magma_tally4_sgetvector( N, d_B, 1, x, 1 );
