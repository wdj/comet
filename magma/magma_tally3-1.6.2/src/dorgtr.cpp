...
/**
    ...
    if UPLO = Magma_tally3Upper, Q = H(n-1) . . . H(2) H(1),

    if UPLO = Magma_tally3Lower, Q = H(1) H(2) . . . H(n-1).
    ...
    dT      DOUBLE_PRECISION array on the GPU device.
            ...
            reflectors H(i) as returned by magma_tally3_dsytrd.
...
magma_tally3_dorgtr(
    ...
    double *dT, magma_tally3_int_t nb,
    magma_tally3_int_t *info)
{
#define A(i,j) (A + (j)*lda+ (i))

    magma_tally3_int_t i__1;
    magma_tally3_int_t i, j;
    magma_tally3_int_t iinfo;
    magma_tally3_int_t upper, lwkopt, lquery;
    ...
    *info = 0;
    ...
    if (upper) {
        ...
        for (j = 0; j < n-1; ++j) {
            ...
            *A(n-1, j) = MAGMA_tally3_D_ZERO;
        }
        for (i = 0; i < n-1; ++i) {
            *A(i, n-1) = MAGMA_tally3_D_ZERO;
        }
        *A(n-1, n-1) = MAGMA_tally3_D_ONE;
        ...
        /*  Q was determined by a call to DSYTRD with UPLO = 'L'.
            ...
            those of the unit matrix                                      */
        for (j = n-1; j > 0; --j) {
            *A(0, j) = MAGMA_tally3_D_ZERO;
            for (i = j; i < n-1; ++i)
                *A(i, j) = *A(i, j - 1);
        }

        *A(0, 0) = MAGMA_tally3_D_ONE;
        for (i = 1; i < n-1; ++i)
            *A(i, 0) = MAGMA_tally3_D_ZERO;
        
        if (n > 1) {
            /* Generate Q(2:n,2:n) */
            magma_tally3_dorgqr(n-1, n-1, n-1, A(1, 1), lda, tau, dT, nb, &iinfo);
        }
    }
