    ...
    magma_tally2_queue_t queue )
{
    ...
    CHECK( magma_tally2_zvinit( &rt,Magma_tally2_DEV, A.num_rows, b.num_cols, c_zero, queue ));
    ...
    CHECK( magma_tally2_zvinit( &h, Magma_tally2_DEV, A.num_rows, b.num_cols, c_zero, queue ));
    ...
    magma_tally2_zcopy( dofs, h.dval, 1, p.dval, 1 );                    // p = h
    nom = MAGMA_tally2_Z_REAL( magma_tally2_zdotc(dofs, r.dval, 1, h.dval, 1) );
    ...
    do
    {
        ...
        gammanew = MAGMA_tally2_Z_REAL( magma_tally2_zdotc(dofs, r.dval, 1, h.dval, 1) );
                                                            // gn = < r,h>

        if ( solver_par->numiter==1 ) {
            magma_tally2_zcopy( dofs, h.dval, 1, p.dval, 1 );                    // p = h
        } else {
            beta = MAGMA_tally2_Z_MAKE(gammanew/gammaold, 0.);       // beta = gn/go
            magma_tally2_zscal(dofs, beta, p.dval, 1);            // p = beta*p
            magma_tally2_zaxpy(dofs, c_one, h.dval, 1, p.dval, 1); // p = p + h
        }

        CHECK( magma_tally2_z_spmv( c_one, A, p, c_zero, q, queue ));   // q = A p
        den = MAGMA_tally2_Z_REAL(magma_tally2_zdotc(dofs, p.dval, 1, q.dval, 1));
                // den = p dot q

        alpha = MAGMA_tally2_Z_MAKE(gammanew/den, 0.);
        magma_tally2_zaxpy(dofs,  alpha, p.dval, 1, x->dval, 1);     // x = x + alpha p
        magma_tally2_zaxpy(dofs, -alpha, q.dval, 1, r.dval, 1);      // r = r - alpha q
        gammaold = gammanew;

        res = magma_tally2_dznrm2( dofs, r.dval, 1 );
        if ( solver_par->verbose > 0 ) {
