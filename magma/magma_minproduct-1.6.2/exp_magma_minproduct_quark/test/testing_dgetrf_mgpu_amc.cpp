...
void init_matrix( int m, int n, double *h_A, magma_minproduct_int_t lda )
{
    ...
    magma_minproduct_int_t *ipiv )
{
    ...
    //printf( "r=\n" ); magma_minproduct_dprint( 1, n, b, 1 );
...
double get_LU_error(magma_minproduct_int_t M, magma_minproduct_int_t N,
                    double *LU, magma_minproduct_int_t lda,
                    magma_minproduct_int_t *ipiv)
{
...
int main( int argc, char** argv )
{
    ...
    double           error;
    double *h_A;
    double *d_lA[ Magma_minproductMaxGPUs ];
    magma_minproduct_int_t *ipiv;
    magma_minproduct_int_t M, N, n2, lda, ldda, n_local, ngpu;
    magma_minproduct_int_t info, min_mn, nb, ldn_local;
    magma_minproduct_int_t status = 0;

    magma_minproduct_int_t P=-1;    /*Number of threads in the CPU part*/
    double d_cpu=-1;    /*pourcentgae of the matrix to allocate in the cpu part*/
    magma_minproduct_int_t Pr=-1;  /*Number of threads for the panel*/
    magma_minproduct_int_t async_nb; /*Block size*/
    
    double *WORK;
    magma_minproduct_int_t WORK_LD, WORK_n;
    ...
    d_cpu = magma_minproduct_amc_recommanded_dcpu(opts.nthread, CPU_PEAK, opts.ngpu, GPU_PEAK);
    #endif
    if(opts.fraction_dcpu!=0){ /*Overwrite the one computed with the model*/
    d_cpu = opts.fraction_dcpu;
    }
    magma_minproduct_assert(d_cpu > 0 && d_cpu<=1.0,
    ...
    for( int i = 0; i < opts.ntest; ++i ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_minproduct_setlapack_numthreads(P);
            /* =====================================================================
            ...
            init_matrix( M, N, h_A, lda );
            magma_minproduct_dsetmatrix_1D_col_bcyclic( M, N, h_A, lda, d_lA, ldda, ngpu, nb );
    
            gpu_time1 = magma_minproduct_wtime();
            magma_minproduct_dgetrf_mgpu( ngpu, M, N, d_lA, ldda, ipiv, &info );
            gpu_time1 = magma_minproduct_wtime() - gpu_time1;
            gpu_perf1 = gflops / gpu_time1;
            if (info != 0)
                printf("magma_minproduct_dgetrf_mgpu returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
                       
            magma_minproduct_dgetmatrix_1D_col_bcyclic( M, N, d_lA, ldda, h_A, lda, ngpu, nb );
            ...
            /* ====================================================================
               Performs operation using MAGMA_minproduct_Async: This interface allocate workspace internally
               =================================================================== */

            /*For the benchmark we have 2 approaches*/
            /*1. use directly magma_minproduct_amc */
            /*2. use magma_minproduct_amc_work and add pinned memory time*/
            ...
            magma_minproduct_setlapack_numthreads(1);

            magma_minproduct_amc_init(P, d_cpu, Pr, nb);
            gpu_time2 = magma_minproduct_wtime();
            magma_minproduct_dgetrf_async_mgpu( ngpu, M, N, d_lA, ldda, ipiv, &info );
            gpu_time2 = magma_minproduct_wtime() - gpu_time2;
            gpu_perf2 = gflops / gpu_time2;
            magma_minproduct_amc_finalize();
            if (info != 0)
                printf("magma_minproduct_dgetrf_mgpu returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
            ...
            */
            ...
            /* ====================================================================
               Performs operation using MAGMA_minproduct_Async_Work
               =================================================================== */
            ...
            //Allocate workspace
            alloc_time = magma_minproduct_wtime();
            if (MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc_pinned(&WORK, WORK_LD*WORK_n)) { 
            //if (MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc_cpu(&WORK, WORK_LD*WORK_n)) {
                info = MAGMA_minproduct_ERR_HOST_ALLOC;
                printf("magma_minproduct_dmalloc_pinned returned error %d: %s.\n     ", (int) info);
            }
            ...
            dlpanelT_n = M;
             dlpanelT = (double **)    malloc(ngpu*sizeof(double*));
              for(int dev=0;dev<ngpu;dev++){
                 ...
                 if (MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc(&dlpanelT[dev], dlpanelT_m*dlpanelT_n)) { 
            
                        info = MAGMA_minproduct_ERR_DEVICE_ALLOC; 
                        printf("magma_minproduct_dmalloc returned error %d: %s.\n     ", (int) info);
                }
              }

            alloc_time = magma_minproduct_wtime() - alloc_time;

            //First touch the workspace with each thread. This may be needed to avoid using numactl --interleave
            //magma_minproduct_amc_dmemset(WORK, 0.0, WORK_LD*WORK_n, 256, P); //nb
            ...
            magma_minproduct_amc_init(P, d_cpu, Pr, nb);
            gpu_time3 = magma_minproduct_wtime();
            magma_minproduct_dgetrf_mgpu_work_amc( ngpu, M, N, d_lA, ldda, ipiv, &info, WORK, WORK_LD, WORK_n, dlpanelT, dlpanelT_m, dlpanelT_n);
            gpu_time3 = magma_minproduct_wtime() - gpu_time3;
            ...
            //Free workspace
            free_time = magma_minproduct_wtime();
            magma_minproduct_free_pinned(WORK);
            free_time = magma_minproduct_wtime() - free_time;

            for(int dev=0;dev<ngpu;dev++){
                magma_minproduct_setdevice(dev);
                magma_minproduct_free(dlpanelT[dev]);
            }
