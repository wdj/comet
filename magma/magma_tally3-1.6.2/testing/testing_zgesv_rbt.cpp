...
int main(int argc, char **argv)
{
    ...
    magma_tally3DoubleComplex *h_A, *h_LU, *h_B, *h_X;
    magma_tally3_int_t *ipiv;
    magma_tally3_int_t N, nrhs, lda, ldb, info, sizeA, sizeB;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_LU, magma_tally3DoubleComplex, lda*N    );
            ...
            magma_tally3_zgesv_rbt(Magma_tally3True, N, nrhs, h_LU, lda, h_X, ldb, &info );
            gpu_time = magma_tally3_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally3_zgesv_rbt returned error %d: %s.\n",
