/*
    -- MAGMA_tally3 (version 1.6.1) --
       ...
       @generated from dsyevdx_2stage.cpp normal d -> s, Fri Jan 30 19:00:18 2015
    ...
    magma_tally3_int_t *info)
{
    ...
    *info = 0;
    ...
    magma_tally3_timer_t time_total=0, time=0;
    ...
    timer_stop( time );
    timer_printf( "  time ssytrd_sy2sb = %6.2f\n", time );
    timer_start( time );
    ...
    if (! wantz) {
        ...
        magma_tally3_sstedx(range, n, vl, vu, il, iu, w, &work[inde],
        ...
        if (MAGMA_tally3_SUCCESS != magma_tally3_smalloc( &dZ, *m*lddz)) {
            *info = MAGMA_tally3_ERR_DEVICE_ALLOC;
            return *info;
        }
        if (MAGMA_tally3_SUCCESS != magma_tally3_smalloc( &dA, n*ldda )) {
            *info = MAGMA_tally3_ERR_DEVICE_ALLOC;
            return *info;
        }

        magma_tally3_sbulge_back(uplo, n, nb, *m, Vblksiz, &work[indwrk + n * (il-1)], n, dZ, lddz,
                          &work[indV2], ldv, &work[indTAU2], &work[indT2], ldt, info);

        timer_stop( time );
        timer_printf( "  time sbulge_back = %6.2f\n", time );
        timer_start( time );

        magma_tally3_ssetmatrix( n, n, A, lda, dA, ldda );

        magma_tally3_sormqr_gpu_2stages(Magma_tally3Left, Magma_tally3NoTrans, n-nb, *m, n-nb, dA+nb, ldda,
                                 dZ+nb, n, dT1, nb, info);

        magma_tally3_sgetmatrix( n, *m, dZ, lddz, A, lda );
        ...
        timer_stop( time );
        timer_stop( time_total );
        timer_printf( "  time sormqr + copy = %6.2f\n", time );
