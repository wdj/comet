...
__global__ void
magma_minproduct_dswap_gemv_kernel(int m, int rk, int n, const double * __restrict__ V, int ldv,
                     const double * __restrict__ x, int ldx, double *c, double *b)
{
    ...
    if (j < m){
       ...
       if (j>=rk) 
          for(int k=0; k<n; k++)
              lsum += MAGMA_minproduct_D_MUL( V[k*ldv], MAGMA_minproduct_D_CNJG(x[k*ldx]));
...
__global__ void
magma_minproduct_dgemv_kernel(int m, int n, const double * __restrict__ V, int ldv,
...
__global__
void magma_minproduct_dscale_kernel(int n, double* dx0,
                         double *dtau, double *dxnorm, double* dAkk)
{
   ...
   for( int k = i; k < n; k += BLOCK_SIZE ) {
             ...
             lsum += re*re;
        #else
             re = MAGMA_minproduct_D_REAL( dx[k] );
             double im = MAGMA_minproduct_D_IMAG( dx[k] );
             lsum += re*re + im*im;
        #endif
   }
   sum[i] = lsum;
            ...
            if ( beta == 0 ) {
              *dtau = MAGMA_minproduct_D_ZERO;
               ...
               *dAkk    = beta;
               ...
               scale = 1. / (tmp - beta);
#else
               double alphar =  MAGMA_minproduct_D_REAL(tmp), alphai = MAGMA_minproduct_D_IMAG(tmp);
               ...
               *dAkk = MAGMA_minproduct_D_MAKE(beta, 0.);
               ...
               tmp = MAGMA_minproduct_D_MAKE( alphar - beta, alphai);
               scale = MAGMA_minproduct_D_DIV( MAGMA_minproduct_D_ONE, tmp);
#endif
            }
   }
   ...
   for(int j=i; j<n; j+=BLOCK_SIZE)
      dx0[j] = MAGMA_minproduct_D_MUL(dx0[j], scale);

   /* === Make temporary the first element to 1; value is stored in dAkk === */
   if (i==0)
     dx0[0] = MAGMA_minproduct_D_ONE;
}
...
__global__ void
magma_minproduct_dgemv_kernel_adjust(int n, int k, double * A, int lda, 
                          ...
                          double *xnorm, double *xnorm2, double *Akk, int *lsticc, int *lsticcs)
{
    ...
    if (j<n) {
      B += j;
      sum = MAGMA_minproduct_D_CNJG( B[(k-1)*ldb] );
      // sum = MAGMA_minproduct_D_ZERO;
      for(int m=0; m<k-1; m++) {
         sum += MAGMA_minproduct_D_MUL( MAGMA_minproduct_D_CNJG( B[m*ldb] ), A[m*lda] );
      }
      C[j*lda] -= sum;

      oldnorm = xnorm[j];
      temp = MAGMA_minproduct_D_ABS( C[j*lda] ) / oldnorm;
...
__global__ void
magma_minproductblas_dnrm2_check_kernel(int m, double *da, int ldda, 
                              ...
                              int *dlsticc, int *dlsticcs)
{
    ...
    for( int j = i; j < m; j += BLOCK_SIZE ) {
        ...
        lsum += re*re;
#else
        re = MAGMA_minproduct_D_REAL( dx[j] );
        double im = MAGMA_minproduct_D_IMAG( dx[j] );
        lsum += re*re + im*im;
#endif
...
#define dF(i_, j_) (dF + (i_) + (j_)*(lddf))
    ...
    magma_minproductDouble_ptr dAkk = dauxv;
    dauxv += 1;
    ...
    while( k < nb && lsticc == 0 ) {
        ...
        if (pvt != k) {
            magma_minproductblas_dswap( k, dF(pvt,0), lddf, dF(k,0), lddf);
            itemp     = jpvt[pvt];
            jpvt[pvt] = jpvt[k];
            jpvt[k]   = itemp;
        ...
        magma_minproduct_dswap_gemv_kernel<<< (m + BLOCK_SIZE1-1) / BLOCK_SIZE1, BLOCK_SIZE1, 0, magma_minproduct_stream >>> 
                              ( m, rk, k, dA(0, 0), ldda, dF(k,  0), lddf, dA(0, k), dA(0,pvt));
                                 
        /*  Generate elementary reflector H(k). */
        magma_minproduct_dscale_kernel<<< 1, BLOCK_SIZE, 0, magma_minproduct_stream >>>
        ...
        if (k < n-1) {
            magma_minproduct_dgetvector( 1, &dtau[k], 1, &tauk, 1 );
            magma_minproduct_dgemv( Magma_minproductConjTrans, m-rk, n,
                         tauk,   dA( rk,  0 ), ldda,
                                 dA( rk,  k   ), 1,
                         c_zero, dauxv, 1 );
            if (k==0) 
               magma_minproductblas_dlacpy(Magma_minproductUpperLower, n-k-1, 1, dauxv+k+1, n-k-1, dF( k+1, k   ), n-k-1);
        }
        ...
        if (k > 0) {
            ...
            magma_minproduct_dgemv_kernel<<< (n-k-1 + BLOCK_SIZE1 -1)/BLOCK_SIZE1, BLOCK_SIZE1, 0, magma_minproduct_stream >>>
        ...
        if (k < n-1) {
            ...
            magma_minproduct_dgemv_kernel_adjust<<<(n-k-1 + BLOCK_SIZE2-1)/BLOCK_SIZE2, BLOCK_SIZE2, 0, magma_minproduct_stream>>>
            ...
            // can be done in the kernel directly (magma_minproductblas_dnrm2_check_kernel)
            // if (k==16) lsticc = 1;
        }
...
/*
        if (rk < min(m, n+offset)-1){
           magma_minproductblas_dnrm2_row_check_adjust(n-k-1, tol3z, &dvn1[k+1], 
...
*/
        ...
        if (k>=n-1)
           magma_minproductblas_dlacpy(Magma_minproductUpperLower, 1, 1, dAkk, 1, dA(rk, k), 1);
    ...
    *kb = k + 1;
    ...
    if( lsticc > 0 ) {
        ...
        //magma_minproductblas_dnrm2_check(m-rk-1, n-*kb, A(rk+1,rk+1), lda,
        //                       &dvn1[rk+1], &dvn2[rk+1], dlsticcs);
       
        // There is a bug when we get to recompute  
        magma_minproductblas_dnrm2_check_kernel<<< n-*kb, BLOCK_SIZE >>>
