    ...
    magma_tally2_queue_t queue )
{
    ...
    if (  A.memory_location == Magma_tally2_CPU &&
        ...
        magma_tally2_z_matrix Ah={Magma_tally2_CSR}, ACSR={Magma_tally2_CSR}, DCSR={Magma_tally2_CSR}, RCSR={Magma_tally2_CSR}, Dh={Magma_tally2_CSR}, Rh={Magma_tally2_CSR};
        CHECK( magma_tally2_zmtransfer( A, &Ah, A.memory_location, Magma_tally2_CPU, queue ));
        CHECK( magma_tally2_zmconvert( Ah, &ACSR, A.storage_type, Magma_tally2_CSR, queue ));

        CHECK( magma_tally2_zcsrsplit( bsize, ACSR, &DCSR, &RCSR, queue ));

        CHECK( magma_tally2_zmconvert( DCSR, &Dh, Magma_tally2_CSR, A.storage_type, queue ));
        CHECK( magma_tally2_zmconvert( RCSR, &Rh, Magma_tally2_CSR, A.storage_type, queue ));

        CHECK( magma_tally2_zmtransfer( Dh, D, Magma_tally2_CPU, A.memory_location, queue ));
        CHECK( magma_tally2_zmtransfer( Rh, R, Magma_tally2_CPU, A.memory_location, queue ));

        magma_tally2_zmfree( &Ah, queue );
        magma_tally2_zmfree( &ACSR, queue );
        magma_tally2_zmfree( &Dh, queue );
        magma_tally2_zmfree( &DCSR, queue );
        magma_tally2_zmfree( &Rh, queue );
        magma_tally2_zmfree( &RCSR, queue );
    }
cleanup:
    if( info != 0 ){
