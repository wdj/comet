    ...
    double **dC_array, magma_tally3_int_t lddc, magma_tally3_int_t batchCount, magma_tally3_queue_t queue )
{
    ...
    if      ( transA != Magma_tally3NoTrans && transA != Magma_tally3Trans && transA != Magma_tally3ConjTrans )
        ...
        info = -5;
    else if ( transA == Magma_tally3NoTrans ? ldda < m : ldda < k )
        info = -8;
    else if ( transB == Magma_tally3NoTrans ? lddb < k : lddb < n )
        info = -10;
    else if ( lddc < m )
        info = -13;
    ...
    if      ( transA == Magma_tally3Trans )
        TransA = 1;
    else if ( transA == Magma_tally3NoTrans )
        TransA = 0;
                    
    if      ( transB == Magma_tally3Trans )
        TransB = 1;
    else if ( transB == Magma_tally3NoTrans )
        TransB = 0;
        ...
        magma_tally3_getvector( min(2,batchCount), sizeof(double*), dA_array, 1, cpuAarray, 1);
        magma_tally3_getvector( min(2,batchCount), sizeof(double*), dB_array, 1, cpuBarray, 1);
        magma_tally3_int_t matrixA_size = batchCount > 1 ? cpuAarray[1] - cpuAarray[0] : 0;
        magma_tally3_int_t matrixB_size = batchCount > 1 ? cpuBarray[1] - cpuBarray[0] : 0;
        magma_tally3_int_t Am = ( ! TransA ? m : k);
        magma_tally3_int_t An = (!TransA ? k : m);
        magma_tally3_int_t Bm = ( ! TransB ? k : n);
        magma_tally3_int_t Bn = (!TransB ? n : k);
             ...
             sizeB >= CUBLAS_MAX_1DBUF_SIZE )
        {
            ...
            return;
            /*
            magma_tally3_dgemm( transA, transB, m, n, k, alpha,
