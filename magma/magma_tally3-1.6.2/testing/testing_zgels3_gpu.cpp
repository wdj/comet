...
int main( int argc, char** argv)
{
    ...
    magma_tally3DoubleComplex *h_A, *h_A2, *h_B, *h_X, *h_R, *tau, *h_work, tmp[1];
    magma_tally3DoubleComplex_ptr d_A, d_B;
    magma_tally3_int_t M, N, size, nrhs, lda, ldb, ldda, lddb, min_mn, max_mn, nb, info;
    magma_tally3_int_t lworkgpu, lhwork, lhwork2;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lhwork2 = (magma_tally3_int_t) MAGMA_tally3_Z_REAL( tmp[0] );
            
            lhwork = -1;
            lapackf77_zunmqr( Magma_tally3LeftStr, Magma_tally3_ConjTransStr,
            ...
            TESTING_MALLOC_CPU( h_work, magma_tally3DoubleComplex, lhwork    );
            ...
            magma_tally3_zgels3_gpu( Magma_tally3NoTrans, M, N, nrhs, d_A, ldda,
                              d_B, lddb, h_work, lworkgpu, &info);
            gpu_time = magma_tally3_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally3_zgels3_gpu returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
            
            // Get the solution in h_X
            magma_tally3_zgetmatrix( N, nrhs, d_B, lddb, h_X, ldb );
