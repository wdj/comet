/*
    -- MAGMA_tally3 (version 1.6.1) --
       ...
       correct.
       This is derived from the testing_blas_s.cpp code that checks MAGMA_tally3's
...
float blasf77_sasum( const magma_tally3_int_t* n,
                       const float* x, const magma_tally3_int_t* incx );

float blasf77_snrm2( const magma_tally3_int_t* n,
                       const float* x, const magma_tally3_int_t* incx );

float blasf77_sdot( const magma_tally3_int_t* n,
                                  const float* x, const magma_tally3_int_t* incx,
...
int main( int argc, char** argv )
{
    ...
    float x2_m, x2_c;  // real x for magma_tally3, cblas/fortran blas respectively
    ...
    for( int iincy = 0; iincy < ninc; ++iincy ) {
        ...
        ld = max( 1, maxn );
        size = ld*maxn;
        magma_tally3_smalloc_pinned( &A,  size );  assert( A   != NULL );
        magma_tally3_smalloc_pinned( &B,  size );  assert( B   != NULL );
        ...
        if ( incx > 0 && incx == incy ) {  // positive, no incy
            ...
            error = 0;
            for( int j = 0; j < k; ++j ) {
                x_m = magma_tally3_cblas_sasum( m, A(0,j), incx );
        ...
        if ( incx > 0 && incx == incy ) {  // positive, no incy
            ...
            error = 0;
            for( int j = 0; j < k; ++j ) {
                x_m = magma_tally3_cblas_snrm2( m, A(0,j), incx );
        ...
        for( int j = 0; j < k; ++j ) {
            ...
            x2_m = magma_tally3_cblas_sdot( m, A(0,j), incx, B(0,j), incy );
