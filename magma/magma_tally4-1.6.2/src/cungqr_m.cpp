...
/**
    ...
    T       COMPLEX array, dimension (NB, min(M,N)).
            ...
            magma_tally4_cgeqrf_gpu (except stored on the CPU, not the GPU).
...
#define dT(d,i,j) (dT[d] + (i) + (j)*nb)
    ...
    *info = 0;
    ...
    magma_tally4FloatComplex *dA[ Magma_tally4MaxGPUs ] = { NULL };
    magma_tally4FloatComplex *dT[ Magma_tally4MaxGPUs ] = { NULL };
    magma_tally4FloatComplex *dV[ Magma_tally4MaxGPUs ] = { NULL };
    magma_tally4FloatComplex *dW[ Magma_tally4MaxGPUs ] = { NULL };
    ...
    for( int d = 0; d < ngpu; ++d ) {
        ...
        ldwork = nlocal[d]*ldda  // dA
               ...
               + nb*ldda         // dV
               + nb*lddwork;     // dW
        if ( MAGMA_tally4_SUCCESS != magma_tally4_cmalloc( &dA[d], ldwork )) {
    ...
    if (kk < n) {
        ...
        magma_tally4blas_claset( Magma_tally4Full, kk, n - kk, c_zero, c_zero, dA(dpanel, 0, di), ldda );
        trace_cpu_end( 0 );
    }
    ...
    if (kk > 0) {
        ...
        for( int d = 0; d < ngpu; ++d ) {
            ...
            magma_tally4_csetmatrix_async( nb, min(m,n), T, nb, dT[d], nb, stream[d] );
            trace_gpu_end( d, 0 );
        }
        ...
        for( i = ki; i >= 0; i -= nb ) {
            ...
            magma_tally4blas_claset( Magma_tally4Full, i,  ib, c_zero, c_zero, dA(dpanel, 0, di), ldda );
            magma_tally4blas_claset( Magma_tally4Full, mi, ib, c_zero, c_one,  dA(dpanel, i, di), ldda );
    ...
    magma_tally4_cgetmatrix_1D_col_bcyclic( m, n, dA, ldda, A, lda, ngpu, nb );
    trace_cpu_end( 0 );
