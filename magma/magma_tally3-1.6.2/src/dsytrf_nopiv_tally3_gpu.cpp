/*
    -- MAGMA_tally3 (version 1.6.1) --
       ...
       @generated from zhetrf_nopiv_tally3_gpu.cpp normal z -> d, Fri Jan 30 19:00:17 2015
*/
    ...
    magma_tally3_int_t *info)
{
    ...
    *info = 0;
    ...
    // CPU workspace
    double *A;
    if (MAGMA_tally3_SUCCESS != magma_tally3_dmalloc_pinned( &A, nb*nb )) {
    ...
    if (MAGMA_tally3_SUCCESS != magma_tally3_dmalloc( &dW, (1+nb)*ldda )) {
    ...
    if (upper) {
        ...
        for (j=0; j<n; j += nb) {
            ...
            magma_tally3_dgetmatrix_async(jb, jb, dA(j, j), ldda, A(j,j), nb, stream[1]);
            trace_gpu_end( 0, 0 );

            // factorize the diagonal block
            magma_tally3_queue_sync(stream[1]);
            trace_cpu_start( 0, "potrf", "potrf" );
            dsytrf_nopiv_tally3_cpu(Magma_tally3Upper, jb, ib, A(j, j), nb, info);
            trace_cpu_end( 0 );
            if (*info != 0){
                *info = *info + j;
                break;
            ...
            magma_tally3_dsetmatrix_async(jb, jb, A(j, j), nb, dA(j, j), ldda, stream[0]);
            trace_gpu_end( 0, 0 );
        ...
        for (j=0; j<n; j+=nb) {
            ...
            magma_tally3_queue_sync(stream[1]);
            trace_cpu_start( 0, "potrf", "potrf" );
            dsytrf_nopiv_tally3_cpu(Magma_tally3Lower, jb, ib, A(j, j), nb, info);
            trace_cpu_end( 0 );
            if (*info != 0){
                *info = *info + j;
                break;
