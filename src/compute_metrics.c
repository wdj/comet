...
void gm_compute_metrics(GMMetrics* metrics, GMVectors* vectors, GMEnv* env) {
  ...
  switch (env->metric_type + GM_NUM_METRIC_TYPE * (
    ...
    case GM_METRIC_TYPE_SORENSON + GM_NUM_METRIC_TYPE*(
         GM_COMPUTE_METHOD_CPU + GM_NUM_COMPUTE_METHOD * (2)):
      gm_compute_metrics_sorenson_2way_cpu(metrics, vectors, env);
      break;

    case GM_METRIC_TYPE_SORENSON + GM_NUM_METRIC_TYPE*(
         GM_COMPUTE_METHOD_GPU + GM_NUM_COMPUTE_METHOD * (2)):
      gm_compute_metrics_sorenson_2way_gpu(metrics, vectors, env);
      break;

    case GM_METRIC_TYPE_SORENSON + GM_NUM_METRIC_TYPE*(
         GM_COMPUTE_METHOD_REF + GM_NUM_COMPUTE_METHOD * (2)):
      gm_compute_metrics_sorenson_2way_ref(metrics, vectors, env);
      break;

    case GM_METRIC_TYPE_SORENSON + GM_NUM_METRIC_TYPE*(
         GM_COMPUTE_METHOD_CPU + GM_NUM_COMPUTE_METHOD * (3)):
      gm_compute_metrics_sorenson_3way_cpu(metrics, vectors, env);
      break;

    case GM_METRIC_TYPE_SORENSON + GM_NUM_METRIC_TYPE*(
         GM_COMPUTE_METHOD_GPU + GM_NUM_COMPUTE_METHOD * (3)):
      gm_compute_metrics_sorenson_3way_gpu(metrics, vectors, env);
      break;

    case GM_METRIC_TYPE_SORENSON + GM_NUM_METRIC_TYPE*(
         GM_COMPUTE_METHOD_REF + GM_NUM_COMPUTE_METHOD * (3)):
      gm_compute_metrics_sorenson_3way_ref(metrics, vectors, env);
      break;

    case GM_METRIC_TYPE_CZEKANOWSKI + GM_NUM_METRIC_TYPE*(
         GM_COMPUTE_METHOD_CPU + GM_NUM_COMPUTE_METHOD * (2)):
    ...
    case GM_METRIC_TYPE_CCC + GM_NUM_METRIC_TYPE*(
         GM_COMPUTE_METHOD_CPU + GM_NUM_COMPUTE_METHOD * (2)):
