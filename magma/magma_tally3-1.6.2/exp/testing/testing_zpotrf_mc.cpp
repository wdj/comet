...
int main( magma_tally3_int_t argc, char** argv) 
{
    ...
    /* Matrix size */
    magma_tally3_int_t N=0, n2, lda;
    magma_tally3_int_t size[10] = {1024,2048,3072,4032,5184,6048,7200,8064,8928,10080};
    ...
    for(j=0; j<N; j++) 
      MAGMA_tally3_Z_SET2REAL( h_A[j*lda+j], ( MAGMA_tally3_Z_GET_X(h_A[j*lda+j]) + 2000. ) );
    ...
    //magma_tally3_zpotrf_mc(context, "L", &N, h_A2, &lda, info);
    magma_tally3_zpotrf_mc(context, "U", &N, h_A2, &lda, info);
    end = get_current_time();
    
    if (info[0] < 0)  
      printf("Argument %d of magma_tally3_zpotrf_mc had an illegal value.\n", -info[0]);     
