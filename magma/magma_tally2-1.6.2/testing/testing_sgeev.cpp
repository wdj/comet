...
extern "C"
float magma_tally2_slapy2(float x, float y)
{
    float ret_val, d;
    float w, z, xabs, yabs;
    
    xabs = MAGMA_tally2_S_ABS(x);
    yabs = MAGMA_tally2_S_ABS(y);
    w    = max(xabs, yabs);
    z    = min(xabs, yabs);
    if (z == 0.) {
        ret_val = w;
...
extern "C"
float magma_tally2_szlapy2(magma_tally2FloatComplex x)
{
    return magma_tally2_slapy2( MAGMA_tally2_C_REAL(x), MAGMA_tally2_C_IMAG(x) );
}
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lapackf77_slacpy( Magma_tally2UpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
            
            /* ====================================================================
               Performs operation using MAGMA_tally2
               =================================================================== */
            gpu_time = magma_tally2_wtime();
            magma_tally2_sgeev( opts.jobvl, opts.jobvr,
                         N, h_R, lda, w1, w1i,
                         VL, lda, VR, lda,
                         h_work, lwork, &info );
            gpu_time = magma_tally2_wtime() - gpu_time;
            if (info != 0)
                printf("magma_tally2_sgeev returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                if ( opts.jobvr == Magma_tally2Vec ) {
                    ...
                    lapackf77_sget22( Magma_tally2NoTransStr, Magma_tally2NoTransStr, Magma_tally2NoTransStr,
                    ...
                    for( int j = 0; j < N; ++j ) {
                        tnrm = 1.;
                        if (w1i[j] == 0.)
                            tnrm = magma_tally2_cblas_snrm2( N, &VR[j*lda], ione );
                        else if (w1i[j] > 0.)
                            tnrm = magma_tally2_slapy2( magma_tally2_cblas_snrm2( N, &VR[j*lda],     ione ),
                                                 magma_tally2_cblas_snrm2( N, &VR[(j+1)*lda], ione ));
                        
                        result[1] = max( result[1], min( ulpinv, MAGMA_tally2_S_ABS(tnrm-1.)/ulp ));
                        
                        if (w1i[j] > 0.) {
                            vmx  = vrmx = 0.;
                            for( int jj = 0; jj < N; ++jj ) {
                                vtst = magma_tally2_slapy2( VR[jj+j*lda], VR[jj+(j+1)*lda]);
                                if (vtst > vmx)
                                    vmx = vtst;
                                
                                if ( (VR[jj + (j+1)*lda])==0. &&
                                     MAGMA_tally2_S_ABS( VR[jj+j*lda] ) > vrmx)
                                {
                                    vrmx = MAGMA_tally2_S_ABS( VR[jj+j*lda] );
                                }
                            }
                            if (vrmx / vmx < 1. - ulp*2.)
                                result[1] = ulpinv;
                ...
                if ( opts.jobvl == Magma_tally2Vec ) {
                    ...
                    lapackf77_sget22( Magma_tally2TransStr, Magma_tally2NoTransStr, Magma_tally2TransStr,
                    ...
                    for( int j = 0; j < N; ++j ) {
                        tnrm = 1.;
                        if (w1i[j] == 0.)
                            tnrm = magma_tally2_cblas_snrm2( N, &VL[j*lda], ione );
                        else if (w1i[j] > 0.)
                            tnrm = magma_tally2_slapy2( magma_tally2_cblas_snrm2( N, &VL[j*lda],     ione ),
                                                 magma_tally2_cblas_snrm2( N, &VL[(j+1)*lda], ione ));
                        
                        result[3] = max( result[3], min( ulpinv, MAGMA_tally2_S_ABS(tnrm-1.)/ulp ));
                        
                        if (w1i[j] > 0.) {
                            vmx  = vrmx = 0.;
                            for( int jj = 0; jj < N; ++jj ) {
                                vtst = magma_tally2_slapy2( VL[jj+j*lda], VL[jj+(j+1)*lda]);
                                if (vtst > vmx)
                                    vmx = vtst;
                                
                                if ( (VL[jj + (j+1)*lda])==0. &&
                                     MAGMA_tally2_S_ABS( VL[jj+j*lda]) > vrmx)
                                {
                                    vrmx = MAGMA_tally2_S_ABS( VL[jj+j*lda] );
                                }
                            }
                            if (vrmx / vmx < 1. - ulp*2.)
                                result[3] = ulpinv;
            ...
            if ( opts.check == 2 ) {
                ...
                magma_tally2_sgeev( Magma_tally2Vec, Magma_tally2Vec,
                             N, h_R, lda, w1, w1i,
                             VL, lda, VR, lda,
                             h_work, lwork, &info );
                if (info != 0)
                ...
                //lapackf77_slacpy( Magma_tally2UpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                //magma_tally2_sgeev( Magma_tally2NoVec, Magma_tally2NoVec,
                //             N, h_R, lda, w2, w2i,
                //             &DUM, 1, &DUM, 1,
                //             h_work, lwork, &info );
                //if (info != 0)
                //    printf("magma_tally2_sgeev (case N, N) returned error %d: %s.\n",
                //           (int) info, magma_tally2_strerror( info ));
                //
                ...
                lapackf77_slacpy( Magma_tally2UpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                magma_tally2_sgeev( Magma_tally2NoVec, Magma_tally2Vec,
                             N, h_R, lda, w2, w2i,
                             &DUM, 1, LRE, lda,
                             h_work, lwork, &info );
                if (info != 0)
                    printf("magma_tally2_sgeev (case N, V) returned error %d: %s.\n",
                           (int) info, magma_tally2_strerror( info ));
                ...
                for( int j = 0; j < N; ++j )
                    for( int jj = 0; jj < N; ++jj )
                        if ( ! MAGMA_tally2_S_EQUAL( VR[j+jj*lda], LRE[j+jj*lda] ))
                            result[7] = 0;
                
                // ----------
                // Compute eigenvalues and left eigenvectors
                lapackf77_slacpy( Magma_tally2UpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                magma_tally2_sgeev( Magma_tally2Vec, Magma_tally2NoVec,
                             N, h_R, lda, w2, w2i,
                             LRE, lda, &DUM, 1,
                             h_work, lwork, &info );
                if (info != 0)
                    printf("magma_tally2_sgeev (case V, N) returned error %d: %s.\n",
                           (int) info, magma_tally2_strerror( info ));
                ...
                for( int j = 0; j < N; ++j )
                    for( int jj = 0; jj < N; ++jj )
                        if ( ! MAGMA_tally2_S_EQUAL( VL[j+jj*lda], LRE[j+jj*lda] ))
                            result[8] = 0;
            ...
            if ( opts.lapack ) {
                ...
                lapackf77_sgeev( lapack_vec_const_tally2(opts.jobvl), lapack_vec_const_tally2(opts.jobvr),
                                 &N, h_A, &lda, w2, w2i,
                                 VL, &lda, VR, &lda,
                                 h_work, &lwork, &info );
                ...
                for( int j=0; j < N; ++j ) {
                    w1copy[j] = MAGMA_tally2_C_MAKE( w1[j], w1i[j] );
                    w2copy[j] = MAGMA_tally2_C_MAKE( w2[j], w2i[j] );
                }
                ...
                for( int j=0; j < N; ++j ) {
                    for( int j2=j; j2 < N; ++j2 ) {
                        ...
                        float diff2 = magma_tally2_szlapy2( diff ) / max( magma_tally2_szlapy2( w1copy[j] ), tol );
                        if ( diff2 < 100*tol ) {
                            if ( j != j2 ) {
