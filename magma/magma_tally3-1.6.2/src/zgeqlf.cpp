...
/**
    ...
    lwork   INTEGER
            ...
            through magma_tally3_get_zgeqlf_nb(M).
    \n
    ...
    @ingroup magma_tally3_zgeqlf_comp
    ********************************************************************/
extern "C" magma_tally3_int_t
magma_tally3_zgeqlf(
    magma_tally3_int_t m, magma_tally3_int_t n,
    magma_tally3DoubleComplex *A,    magma_tally3_int_t lda, magma_tally3DoubleComplex *tau,
    ...
    magma_tally3_int_t *info)
{
    ...
    magma_tally3DoubleComplex_ptr dA, dwork;
    ...
    *info = 0;
    ...
    if (*info == 0) {
        if (k == 0)
            ...
            work[0] = MAGMA_tally3_Z_MAKE( max(n*nb, 2*nb*nb), 0 );
        }
    ...
    if (MAGMA_tally3_SUCCESS != magma_tally3_zmalloc( &dA, (n)*ldda + nb*lddwork )) {
    ...
    if ( (nb > 1) && (nb < k) ) {
        ...
        magma_tally3_zsetmatrix_async( m, n-nb,
                                A(0, 0),  lda,
        ...
        for (i = k - kk + ki; i >= k -kk; i -= nb) {
            ...
            if (i < k - kk + ki) {
                ...
                magma_tally3_zgetmatrix_async( m-rows, ib,
                                        dA(rows, n-k+i), ldda,
                ...
                magma_tally3_zlarfb_gpu( Magma_tally3Left, Magma_tally3ConjTrans, Magma_tally3Backward, Magma_tally3Columnwise,
                                  rows, cols, old_ib,
            ...
            if (cols > 0) {
                ...
                lapackf77_zlarft( Magma_tally3BackwardStr, Magma_tally3ColumnwiseStr,
                                  &rows, &ib,
                                  A(0, cols), &lda, tau + i, work, &ib);

                zpanel_to_q_tally3( Magma_tally3Lower, ib, A(rows-ib,cols), lda, work+ib*ib);
                magma_tally3_zsetmatrix( rows, ib,
                                  A(0,cols),  lda,
                                  dA(0,cols), ldda );
                zq_to_panel_tally3( Magma_tally3Lower, ib, A(rows-ib,cols), lda, work+ib*ib);

                // Send the triangular part on the GPU
                magma_tally3_zsetmatrix( ib, ib, work, ib, dwork(0), lddwork );
        ...
        magma_tally3_zgetmatrix( m, nu, dA(0,0), ldda, A(0,0), lda );
    } else {
        mu = m;
        nu = n;
    }
