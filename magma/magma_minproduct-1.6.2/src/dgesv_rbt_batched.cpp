                  ...
                  magma_minproduct_int_t batchCount, magma_minproduct_queue_t queue)
{
    ...
    double *hu, *hv;
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc_cpu( &hu, 2*n )) {
        info = MAGMA_minproduct_ERR_HOST_ALLOC;
        return info;
    }

    if (MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc_cpu( &hv, 2*n )) {
        info = MAGMA_minproduct_ERR_HOST_ALLOC;
        return info;
    }
    ...
    info = magma_minproduct_dgerbt_batched(Magma_minproductTrue, n, nrhs, dA_array, n, dB_array, n, hu, hv, &info, batchCount, queue);
    if (info != MAGMA_minproduct_SUCCESS)  {
        return info;
    }


    info = magma_minproduct_dgetrf_nopiv_batched( n, n, dA_array, ldda, info_array, batchCount, queue);
    if ( (info != MAGMA_minproduct_SUCCESS) ){
        return info;
    }
    ...
    info = magma_minproduct_dgetrs_nopiv_batched( Magma_minproductNoTrans, n, nrhs, dA_array, ldda, dB_array, lddb, info_array, batchCount, queue );
    ...
    magma_minproduct_dsetvector(2*n, hv, 1, dv, 1);

    for(int i = 0; i < nrhs; i++)
        magma_minproductblas_dprbt_mv_batched(n, dv, dB_array+(i), batchCount, queue);
