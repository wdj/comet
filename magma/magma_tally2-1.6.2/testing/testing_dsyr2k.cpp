...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_tally2_dsetmatrix( Bn, Bk, h_B, ldb, d_B, lddb );
            magma_tally2_dsetmatrix( N, N, h_C, ldc, d_C, lddc );
            
            cublas_time = magma_tally2_sync_wtime( NULL );
            cublasDsyr2k( opts.handle, cublas_uplo_const_tally2(opts.uplo), cublas_trans_const_tally2(opts.transA), N, K,
                          &alpha, d_A, ldda,
                                  d_B, lddb,
                          &beta,  d_C, lddc );
            ...
            if ( opts.lapack ) {
                ...
                blasf77_dsyr2k( lapack_uplo_const_tally2(opts.uplo), lapack_trans_const_tally2(opts.transA), &N, &K,
                               &alpha, h_A, &lda,
                                       h_B, &ldb,
                               &beta,  h_C, &ldc );
