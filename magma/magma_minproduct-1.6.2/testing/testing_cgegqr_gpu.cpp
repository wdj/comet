...
int main( int argc, char** argv)
{
    ...
    magma_minproductFloatComplex *h_A, *h_R, *tau, *dtau, *h_work, *h_rwork, tmp[1];

    magma_minproductFloatComplex_ptr d_A, dwork;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_PIN( tau,    magma_minproductFloatComplex, min_mn );
            TESTING_MALLOC_PIN( h_work, magma_minproductFloatComplex, lwork  );
            TESTING_MALLOC_PIN(h_rwork, magma_minproductFloatComplex, lwork  );

            TESTING_MALLOC_CPU( h_A,   magma_minproductFloatComplex, n2     );
            TESTING_MALLOC_CPU( h_R,   magma_minproductFloatComplex, n2     );
            ...
            // warmup
            if ( opts.warmup ) {
                magma_minproduct_cgegqr_gpu( 1, M, N, d_A, ldda, dwork, h_work, &info );
                magma_minproduct_csetmatrix( M, N, h_R, lda, d_A, ldda );
            }
            ...
            magma_minproduct_cgegqr_gpu( opts.version, M, N, d_A, ldda, dwork, h_rwork, &info );
            gpu_time = magma_minproduct_sync_wtime( 0 ) - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_minproduct_cgegqr returned error %d: %s.\n",
            ...
            // magma_minproduct_cprint(N, N, h_work, N);
            ...
            if ( opts.lapack ) {
                ...
                for(int ii = 0; ii < N*N; ii += N+1 ) {
                    h_work[ii] = MAGMA_minproduct_C_SUB(h_work[ii], c_one);
                }
