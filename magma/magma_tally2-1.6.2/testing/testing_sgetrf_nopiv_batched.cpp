...
float get_LU_error(magma_tally2_int_t M, magma_tally2_int_t N,
                    ...
                    float *LU, magma_tally2_int_t *IPIV)
{
    magma_tally2_int_t min_mn = min(M,N);
    magma_tally2_int_t ione   = 1;
    magma_tally2_int_t i, j;
    float alpha = MAGMA_tally2_S_ONE;
    float beta  = MAGMA_tally2_S_ZERO;
    float *L, *U;
    ...
    lapackf77_slacpy( Magma_tally2LowerStr, &M, &min_mn, LU, &lda, L, &M      );
    lapackf77_slacpy( Magma_tally2UpperStr, &min_mn, &N, LU, &lda, U, &min_mn );

    for(j=0; j<min_mn; j++)
        L[j+j*M] = MAGMA_tally2_S_MAKE( 1., 0. );
    ...
    for( j = 0; j < N; j++ ) {
        for( i = 0; i < M; i++ ) {
            LU[i+j*lda] = MAGMA_tally2_S_SUB( LU[i+j*lda], A[i+j*lda] );
        }
    }
...
int main( int argc, char** argv)
{
    ...
    for( int i = 0; i < opts.ntest; ++i ) {
      ...
      for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_DEV(  dA_magma_tally2,  float, ldda*N * batchCount);
            ...
            magma_tally2_ssetmatrix( M, columns, h_R, lda, dA_magma_tally2, ldda );
            ...
            /* ====================================================================
               ...
               =================================================================== */
            sset_pointer(dA_array, dA_magma_tally2, ldda, 0, 0, ldda*N, batchCount, queue);
            magma_tally2_time = magma_tally2_sync_wtime(0);
            info = magma_tally2_sgetrf_nopiv_batched( M, N, dA_array, ldda, dinfo_magma_tally2, batchCount, queue);
            ...
            {
                if(cpu_info[i] != 0 ){
                    printf("magma_tally2_sgetrf_batched matrix %d returned internal error %d\n",i, (int)cpu_info[i] );
                }
            }
            if (info != 0)
                printf("magma_tally2_sgetrf_batched returned argument error %d: %s.\n", (int) info, magma_tally2_strerror( info ));
            ...
            if ( opts.check ) {
                ...
                magma_tally2_sgetmatrix( M, N*batchCount, dA_magma_tally2, ldda, h_A, lda );
                for(int i=0; i<batchCount; i++)
                {
