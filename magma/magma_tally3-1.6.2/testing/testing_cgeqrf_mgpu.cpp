    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for( int dev = 0; dev < ngpu; dev++ ) {
                ...
                TESTING_MALLOC_DEV( d_lA[dev], magma_tally3FloatComplex, ldda*n_local );
            }
            ...
            /* =====================================================================
               ...
               =================================================================== */
            if ( opts.lapack ) {
                magma_tally3FloatComplex *tau2;
                TESTING_MALLOC_CPU( tau2, magma_tally3FloatComplex, min_mn );
            ...
            /* ====================================================================
               ...
               =================================================================== */
            magma_tally3_csetmatrix_1D_col_bcyclic( M, N, h_R, lda, d_lA, ldda, ngpu, nb );
            
            gpu_time = magma_tally3_wtime();
            magma_tally3_cgeqrf2_mgpu( ngpu, M, N, d_lA, ldda, tau, &info );
            gpu_time = magma_tally3_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally3_cgeqrf2 returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
            
            magma_tally3_cgetmatrix_1D_col_bcyclic( M, N, d_lA, ldda, h_R, lda, ngpu, nb );
            ...
            if ( opts.check == 1 && M >= N ) {
                ...
                magma_tally3FloatComplex *h_W1, *h_W2, *h_W3;
                float *h_RW, results[2];
            
                TESTING_MALLOC_CPU( h_W1, magma_tally3FloatComplex, n2    ); // Q
                TESTING_MALLOC_CPU( h_W2, magma_tally3FloatComplex, n2    ); // R
                TESTING_MALLOC_CPU( h_W3, magma_tally3FloatComplex, lwork ); // WORK
