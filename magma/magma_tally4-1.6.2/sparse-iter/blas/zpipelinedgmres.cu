...
__global__ void
magma_tally4_zpipelined_correction( 
    int n,  
    int k,
    ...
    magma_tally4DoubleComplex * v )
{
    ...
    temp[ i ] = ( i < k ) ? skp[ i ] * skp[ i ] : MAGMA_tally4_Z_MAKE( 0.0, 0.0);
    __syncthreads();
    ...
    if( i == 0 ){
        tmp = MAGMA_tally4_Z_REAL( temp[ i ] );
        zz = MAGMA_tally4_Z_REAL( skp[(k)] );
        skp[k] = MAGMA_tally4_Z_MAKE( sqrt(zz-tmp),0.0 );
    }
}

__global__ void
magma_tally4_zpipelined_copyscale( 
    int n,  
    int k,
    magma_tally4DoubleComplex * skp, 
    magma_tally4DoubleComplex * r,
    magma_tally4DoubleComplex * v )
{
    ...
    magma_tally4DoubleComplex rr=skp[k];
...
__global__ void
magma_tally4_zpipelineddznrm2_kernel( 
    int m, 
    magma_tally4DoubleComplex * da, 
    int ldda, 
    magma_tally4DoubleComplex * dxnorm )
{
    const int i = threadIdx.x;
    magma_tally4DoubleComplex_ptr dx = da + blockIdx.x * ldda;
    ...
    if (i==0)
        dxnorm[blockIdx.x] = MAGMA_tally4_Z_MAKE( sqrt(sum[0]), 0.0 );
}

//----------------------------------------------------------------------------//

__global__ void
magma_tally4_zpipelinedscale( 
    int n, 
    magma_tally4DoubleComplex * r, 
    ...
    magma_tally4_queue_t queue )
{
    ...
    unsigned int Ms =   Bs.x * sizeof( magma_tally4DoubleComplex ); 

    dim3 Gs2( magma_tally4_ceildiv( n, BLOCK_SIZE ) );


    magma_tally4_zpipelined_correction<<<Gs, Bs, Ms, queue >>>
                                            ( n, k, skp, r, v );
    magma_tally4_zpipelined_copyscale<<<Gs2, Bs, 0, queue >>>
                                            ( n, k, skp, r, v );
...
magma_tally4_dznrm2scale(
    ...
    magma_tally4DoubleComplex_ptr r, 
    int lddr, 
    magma_tally4DoubleComplex_ptr drnorm,
    magma_tally4_queue_t queue )
{
    dim3  blocks( 1 );
    dim3 threads( 512 );
    magma_tally4_zpipelineddznrm2_kernel<<< blocks, threads, 0, queue >>>
                                ( m, r, lddr, drnorm );

    dim3 Bs( BLOCK_SIZE );
    dim3 Gs2( magma_tally4_ceildiv( m, BLOCK_SIZE ) );
    magma_tally4_zpipelinedscale<<<Gs2, Bs, 0, queue >>>( m, r, drnorm );
