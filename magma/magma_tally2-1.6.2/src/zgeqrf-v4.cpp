...
{
    magma_tally2DoubleComplex *da[Magma_tally2MaxGPUs];
    ...
    *info = 0;
    ...
    for (i=0; i < ngpu; i++) {
        ...
        if (MAGMA_tally2_SUCCESS != magma_tally2_zmalloc( &da[i], ldda*n_local[i] )) {
    ...
    if (m > nb && n > nb) {
        ...
        magma_tally2_zsetmatrix_1D_col_bcyclic(m, n, A, lda, da, ldda, ngpu, nb);

        /* Factor using the GPU interface */
        magma_tally2_zgeqrf2_mgpu( ngpu, m, n, da, ldda, tau, info);
