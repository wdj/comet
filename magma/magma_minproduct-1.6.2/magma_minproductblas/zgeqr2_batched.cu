...
zlarfg_device(
    int n,
    magma_minproductDoubleComplex* dalpha, magma_minproductDoubleComplex* dx, int incx,
    magma_minproductDoubleComplex* dtau,  double* swork, double* sscale, magma_minproductDoubleComplex* scale)
{
    ...
    magma_minproduct_max_reduce<BLOCK_SIZE>( tx, swork );
    ...
    if ( *sscale > 0 ) {
        ...
        magma_minproduct_sum_reduce<BLOCK_SIZE>( tx, swork );

    }
    
    if ( tx == 0 ) {
        magma_minproductDoubleComplex alpha = *dalpha;
        ...
        if ( swork[0] == 0 && imag(alpha) == 0 ) {
            ...
            *dtau   = MAGMA_minproduct_Z_MAKE( (beta - real(alpha)) / beta, -imag(alpha) / beta );
            *dalpha = MAGMA_minproduct_Z_MAKE( beta, 0 );
            *scale = 1 / (alpha - beta);
        }
    }
...
static __device__
void zlarfx_device( int m, int n,  magma_minproductDoubleComplex *v, magma_minproductDoubleComplex *tau,
                         magma_minproductDoubleComplex *dc, magma_minproduct_int_t ldc, magma_minproductDoubleComplex* sum)
{


    if(n <=0) return ;
    if (MAGMA_minproduct_Z_EQUAL(*tau, MAGMA_minproduct_Z_ZERO) )  return; // check singularity
    ...
    for(int k=0;k<n;k++)
    {
        ...
        if(tx<BLOCK_SIZE)
        {
            ...
            for( int j = tx+1; j < m; j += BLOCK_SIZE ){
                lsum += MAGMA_minproduct_Z_MUL( MAGMA_minproduct_Z_CNJG( v[j] ), dc[j+ldc*k] );
            }
        ...
        magma_minproductDoubleComplex z__1 = - MAGMA_minproduct_Z_CNJG(*tau) * sum[0];
        /*  C := C - v * w  */
        if(tx<BLOCK_SIZE)
        {    
           for( int j = tx+1; j<m ; j += BLOCK_SIZE )
...
__global__
void zgeqr2_sm_kernel_batched( int m, int n, magma_minproductDoubleComplex** dA_array, magma_minproduct_int_t lda,
                               magma_minproductDoubleComplex **dtau_array)
{

    magma_minproductDoubleComplex* dA = dA_array[blockIdx.z];
    magma_minproductDoubleComplex* dtau = dtau_array[blockIdx.z];
...
static __device__
void zgeqr2_device( magma_minproduct_int_t m, magma_minproduct_int_t n, magma_minproductDoubleComplex* dA, magma_minproduct_int_t lda,
                               magma_minproductDoubleComplex *dtau, 
                               magma_minproductDoubleComplex *sdata,
...
__global__
void zgeqr2_kernel_batched( int m, int n, magma_minproductDoubleComplex** dA_array, magma_minproduct_int_t lda,
                               magma_minproductDoubleComplex **dtau_array)
{
...
/**
    ...
    @param
    dwork   (workspace) COMPLEX_16 array, dimension (N) * ( sizeof(double) + sizeof(magma_minproductDoubleComplex)) 
...
magma_minproduct_zgeqr2_batched(magma_minproduct_int_t m, magma_minproduct_int_t n, magma_minproductDoubleComplex **dA_array,
                  magma_minproduct_int_t lda, magma_minproductDoubleComplex **dtau_array,
                  magma_minproduct_int_t *info_array, magma_minproduct_int_t batchCount, magma_minproduct_queue_t queue)
{
    ...
    if(sizeof(magma_minproductDoubleComplex)*(m*k) <= 128 /*sizeof(magma_minproductDoubleComplex) * 128 * k*/) // there are some static shared memory besides of dynamic ones 
    {   
        ...
        zgeqr2_sm_kernel_batched<<< blocks, threads, sizeof(magma_minproductDoubleComplex)*(m*k), queue >>>
    ...
    else
    {
        ...
        if(sizeof(magma_minproductDoubleComplex)*(m) < 42000)
            zgeqr2_kernel_batched<<< blocks, threads, sizeof(magma_minproductDoubleComplex)*(m), queue >>>
                                      (m, k, dA_array, lda, dtau_array);
        else
