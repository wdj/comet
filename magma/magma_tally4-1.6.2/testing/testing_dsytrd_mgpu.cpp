/*
    -- MAGMA_tally4 (version 1.6.1) --
       ...
       @generated from testing_zhetrd_mgpu.cpp normal z -> d, Fri Jan 30 19:00:26 2015
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( opts.ngpu == 0 ) {
                magma_tally4_dsytrd(opts.uplo, N, h_R, lda, diag, offdiag,
                             tau, h_work, lwork, &info);
            } else {
                magma_tally4_dsytrd_mgpu(opts.ngpu, k, opts.uplo, N, h_R, lda, diag, offdiag,
                                  tau, h_work, lwork, &info);
            }
            gpu_time = magma_tally4_wtime() - gpu_time;
            if ( info != 0 ) {
                printf("magma_tally4_dsytrd_mgpu returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                lapackf77_dlacpy( lapack_uplo_const_tally4(opts.uplo), &N, &N, h_R, &lda, h_Q, &lda);
                lapackf77_dorgtr( lapack_uplo_const_tally4(opts.uplo), &N, h_Q, &lda, tau, h_work, &lwork, &info);

                lapackf77_dsyt21( &itwo, lapack_uplo_const_tally4(opts.uplo), &N, &ione,
                ...
                lapackf77_dsyt21( &ithree, lapack_uplo_const_tally4(opts.uplo), &N, &ione,
                                  h_A, &lda, diag, offdiag,
                                  h_Q, &lda, h_R, &lda,
                                  tau, work,
                                  #ifdef COMPLEX
            ...
            if ( opts.lapack ) {
                ...
                lapackf77_dsytrd(lapack_uplo_const_tally4(opts.uplo), &N, h_A, &lda, diag, offdiag, tau,
                                 h_work, &lwork, &info);
