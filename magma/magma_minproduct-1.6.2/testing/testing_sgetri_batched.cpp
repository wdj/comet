...
int main( int argc, char** argv)
{
    ...
    for( int i = 0; i < opts.ntest; ++i ) {
      ...
      for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_minproduct_ssetmatrix( M, columns, h_R, lda, d_A, ldda );
            ...
            info1 = magma_minproduct_sgetrf_batched( M, N, dA_array, ldda, dipiv_array, dinfo_array, batchCount, queue);
            info2 = magma_minproduct_sgetri_outofplace_batched( min(M,N), dA_array, ldda, dipiv_array, dinvA_array, ldda, dinfo_array, batchCount, queue);
            ...
            {
                if(cpu_info[i] != 0 ){
                    printf("magma_minproduct_sgetrf_batched matrix %d returned error %d\n", i, (int)cpu_info[i] );
                }
            }
            if (info1 != 0) printf("magma_minproduct_sgetrf_batched returned argument error %d: %s.\n", (int) info1, magma_minproduct_strerror( info1 ));
            if (info2 != 0) printf("magma_minproduct_sgetri_batched returned argument error %d: %s.\n", (int) info2, magma_minproduct_strerror( info2 ));
            /* =====================================================================
            ...
            if ( opts.lapack ) {
                ...
                lwork = magma_minproduct_int_t( MAGMA_minproduct_S_REAL( tmp ));
                TESTING_MALLOC_CPU( work,  float, lwork  );
                lapackf77_slacpy( Magma_minproductUpperLowerStr, &M, &columns, h_R, &lda, h_A, &lda );
            ...
            if ( opts.check ) {
                ...
                magma_minproduct_sgetmatrix( min(M,N), N*batchCount, d_invA, ldda, h_R, lda );
                int stop=0;
                n2     = lda*N;
                for(int i=0; i < batchCount; i++)
                {
