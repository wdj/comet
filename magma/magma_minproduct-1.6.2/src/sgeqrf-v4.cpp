...
{
    float *da[Magma_minproductMaxGPUs];
    ...
    *info = 0;
    ...
    for (i=0; i < ngpu; i++) {
        ...
        if (MAGMA_minproduct_SUCCESS != magma_minproduct_smalloc( &da[i], ldda*n_local[i] )) {
    ...
    if (m > nb && n > nb) {
        ...
        magma_minproduct_ssetmatrix_1D_col_bcyclic(m, n, A, lda, da, ldda, ngpu, nb);

        /* Factor using the GPU interface */
        magma_minproduct_sgeqrf2_mgpu( ngpu, m, n, da, ldda, tau, info);
