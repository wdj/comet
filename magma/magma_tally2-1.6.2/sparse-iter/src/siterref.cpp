...
magma_tally2_siterref(
    ...
    magma_tally2_s_solver_par *solver_par, magma_tally2_s_preconditioner *precond_par,
    magma_tally2_queue_t queue )
{
    ...
    // workspace
    magma_tally2_s_matrix r={Magma_tally2_CSR}, z={Magma_tally2_CSR};
    ...
    //CHECK(  magma_tally2_sresidualvec( A, b, *x, &r, nom, queue));
    magma_tally2_scopy( dofs, b.dval, 1, r.dval, 1 );                    // r = b
    nom0 = magma_tally2_snrm2(dofs, r.dval, 1);                       // nom0 = || r ||
    nom = nom0 * nom0;
    ...
    for( solver_par->numiter= 1; solver_par->numiter<solver_par->maxiter;
        ...
        magma_tally2_sscal( dofs, MAGMA_tally2_S_MAKE(1./nom, 0.), r.dval, 1) ;  // scale it
        CHECK( magma_tally2_s_precond( A, r, &z, precond_par, queue )); // inner solver:  A * z = r
        magma_tally2_sscal( dofs, MAGMA_tally2_S_MAKE(nom, 0.), z.dval, 1) ;  // scale it
        magma_tally2_saxpy(dofs,  c_one, z.dval, 1, x->dval, 1);        // x = x + z
        CHECK( magma_tally2_s_spmv( c_mone, A, *x, c_zero, r, queue ));      // r = - A x
        magma_tally2_saxpy(dofs,  c_one, b.dval, 1, r.dval, 1);         // r = r + b
        nom = magma_tally2_snrm2(dofs, r.dval, 1);                    // nom = || r ||
