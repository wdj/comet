...
/* ////////////////////////////////////////////////////////////////////////////
   -- Testing magma_tally3_ssyr2k_mgpu
*/
int main( int argc, char** argv)
{
    ...
    float alpha = MAGMA_tally3_S_MAKE( 1.2345, 4.321 );
    float beta = 3.14159;
    
    real_Double_t    gflops, gpu_perf, cpu_perf, gpu_time, cpu_time;
    float           error, work[1];
    float *hA, *hR, *hR2, *hV, *hW;
    magma_tally3Float_ptr dV[Magma_tally3MaxGPUs], dW[Magma_tally3MaxGPUs], dA[Magma_tally3MaxGPUs];
    ...
    printf( "version 1: magma_tally3blas_ssyr2k_mgpu2     %s\n", (opts.version==1 ? "(enabled)" : ""));
    printf( "version 2: magma_tally3blas_ssyr2k_mgpu_spec %s\n", (opts.version==2 ? "(enabled)" : ""));
#ifdef ICHI
    printf( "version 3: magma_tally3_ssyr2k_mgpu (Ichi)   %s\n", (opts.version==3 ? "(enabled)" : ""));
#endif
    printf( "\n" );
        ...
        for( int offset = 0; offset < n; offset += min(k,nb) ) {
            for( int iter = 0; iter < opts.niter; ++iter ) {
                ...
                /* ====================================================================
                   ...
                   =================================================================== */
                magma_tally3_ssetmatrix_1D_col_bcyclic( n, n, hA, lda, dA, ldda, ngpu, nb );
                for( int d = 0; d < ngpu; ++d ) {
                    magma_tally3_setdevice( d );
                    dW[d] = dV[d] + ldda*k;
                    magma_tally3_ssetmatrix( n, k, hV, lda, dV[d], ldda );
                    magma_tally3_ssetmatrix( n, k, hW, lda, dW[d], ldda );
                }
                ...
                magma_tally3_sgetmatrix_1D_col_bcyclic( n, n, dA, ldda, hR, lda, ngpu, nb );
                ...
                if ( opts.lapack || opts.check ) {
                    ...
                    error  = lapackf77_slange("f", &n_offset, &k, hV, &lda, work );
                    error *= lapackf77_slange("f", &n_offset, &k, hW, &lda, work );
                    error += lapackf77_slange("f", &n_offset, &n_offset, &hA[offset + offset*lda], &lda, work );
                    ...
                    error = lapackf77_slansy("fro", "Lower", &n_offset, &hR[offset + offset*lda], &lda, work) / error;
