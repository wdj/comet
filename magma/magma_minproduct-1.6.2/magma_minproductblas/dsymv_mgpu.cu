    ...
    int block_offset )
{
    ...
    double psum, psum_t;
    double total = MAGMA_minproduct_D_ZERO;
    ...
    if ( ty == 0 ) {
        ...
        if ( (partial && tx >= partial) ||
            ...
            sx_blk[tx] = MAGMA_minproduct_D_ZERO;
        }
        else {
            sx_blk[tx] = x[0];
        }
    ...
    if ( blk % ngpu == my_gpu_id ) {
        ...
        psum_t = MAGMA_minproduct_D_ZERO;
        #pragma unroll
        for(int j=0; j < 4; j++) {
    ...
    for(int jj=my_gpu_id; jj < blk; jj += ngpu) {
        ...
        if ( ty == 0 ) {
            if ( jj == 0 && tx < block_offset ) {
                sx_jj[tx] = MAGMA_minproduct_D_ZERO;
            }
            else {
...
/**************************************************************
    ...
    Note beta*y is not included here; see magma_minproductblas_dsymv_mgpu_sync.
    ...
    int block_offset)
{
    ...
    if ( ind >= block_offset && ind < n+block_offset ) {
        double Ax = MAGMA_minproduct_D_ZERO;
        ...
        y[ind * incy] = alpha*Ax;  // see magma_minproductblas_dsymv_sync for beta*y
    }
}
...
/**
    Purpose
    -------
    magma_minproductblas_dsymv_mgpu performs the matrix-vector operation:
...
magma_minproductblas_dsymv_mgpu(
    ...
    double alpha,
    magma_minproductDouble_const_ptr const d_lA[], magma_minproduct_int_t ldda,
    magma_minproduct_int_t offset,
    double const *x,       magma_minproduct_int_t incx,
    double beta,                                         // unused, see magma_minproductblas_dsymv_mgpu_sync
    double       *y,       magma_minproduct_int_t incy,             // unused
    ...
    magma_minproduct_queue_t queues[] )
{
    ...
    for(dev=0; dev < ngpu; dev++) {
        ...
        // copy x to each GPU
        magma_minproduct_dsetvector_async( n, x, incx, dx_dev + block_offset, 1, queues[dev] );
    ...
    for(dev=0; dev < ngpu; dev++) {
        ...
        magma_minproduct_setdevice( dev );
        double       *dx_dev    = dwork[dev];
        magma_minproduct_dgetvector_async( n, dx_dev + block_offset, 1, &hwork[dev*n], 1, queues[dev] );
    }
    
    // see magma_minproductblas_dsymv_mgpu_sync for final row sums
...
/**
    ...
    For convenience, the parameters are identical to magma_minproductblas_dsymv_mgpu
    (though some are unused here).
    
    @see magma_minproductblas_dsymv_mgpu
    @ingroup magma_minproduct_dblas2
    ********************************************************************/
extern "C" magma_minproduct_int_t
magma_minproductblas_dsymv_mgpu_sync(
    magma_minproduct_uplo_t uplo,                                               // unused, see magma_minproductblas_dsymv_mgpu
    magma_minproduct_int_t n,
    double alpha,                                        // unused
    magma_minproductDouble_const_ptr const d_lA[], magma_minproduct_int_t ldda,
    magma_minproduct_int_t offset,                                              // unused
    double const *x,       magma_minproduct_int_t incx,             // unused
    double beta,
    double       *y,       magma_minproduct_int_t incy,             // unused
    double       *hwork,   magma_minproduct_int_t lhwork,
    magma_minproductDouble_ptr    dwork[], magma_minproduct_int_t ldwork,           // unused
    magma_minproduct_int_t ngpu,
