...
/**
    ...
    lwork   INTEGER
            ...
            LWORK >= (M-K+NB)*(N+NB) + N*NB if SIDE = Magma_tally2Left, and
            LWORK >= (N-K+NB)*(M+NB) + M*NB if SIDE = Magma_tally2Right,
    ...
    dT      COMPLEX array on the GPU that is the output
            (the 9th argument) of magma_tally2_cgeqrf_gpu.

    @param[in]
    nb      INTEGER
            This is the blocking size that was used in pre-computing DT, e.g.,
            the blocking size used in magma_tally2_cgeqrf_gpu.
    ...
    magma_tally2_int_t *info)
{
    ...
    magma_tally2_int_t i, lddwork;
    magma_tally2_int_t i1, i2, step, ib, ic, jc, ma, mi, ni, nq, nw;
    ...
    *info = 0;
    ...
    hwork[0] = MAGMA_tally2_C_MAKE( lwkopt, 0 );
    ...
    if ( step < 0 ) {
        ...
        magma_tally2FloatComplex* hA = hwork;
        magma_tally2FloatComplex* hC = hwork + ma*ib;
        magma_tally2FloatComplex* hW = hwork + ma*ib + mi*ni;
        magma_tally2_int_t lhwork = lwork - (ma*ib + mi*ni);
        
        magma_tally2_cgetmatrix( ma, ib, dA(i,  i ), ldda, hA, ma );
        magma_tally2_cgetmatrix( mi, ni, dC(ic, jc), lddc, hC, mi );
        ...
        magma_tally2_csetmatrix( mi, ni, hC, mi, dC(ic, jc), lddc );
    }
    ...
    // TODO sync. For cases Q*C and C*Q^T, last call is magma_tally2_clarfb_gpu,
    // which is async magma_tally2_gemm calls, so cunmqr can be unfinished.
