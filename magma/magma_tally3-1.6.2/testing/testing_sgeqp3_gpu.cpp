...
int main( int argc, char** argv)
{
    ...
    magma_tally3Float_ptr d_A, dtau, d_work;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lda    = M;
            n2     = lda*N;
            nb     = magma_tally3_get_sgeqp3_nb( min_mn );
            ...
            /* Make h_A of rank K */
            float alpha = MAGMA_tally3_S_MAKE(  1., 0. );
            float beta  = MAGMA_tally3_S_MAKE(  0., 0. );
            ...
            /* copy A to gpu */
            magma_tally3_ssetmatrix( M, N, h_R, lda, d_A, lda );

            /* call gpu-interface */
            gpu_time = magma_tally3_wtime();
            #if defined(PRECISION_z) || defined(PRECISION_c)
            magma_tally3_sgeqp3_gpu(M, N, d_A, lda, jpvt, dtau, d_work, lwork, drwork, &info);
            #else
            magma_tally3_sgeqp3_gpu(M, N, d_A, lda, jpvt, dtau, d_work, lwork, &info);
            #endif
            gpu_time = magma_tally3_wtime() - gpu_time;
            
            /* copy outputs to cpu */
            magma_tally3_sgetmatrix( M, N, d_A, lda, h_R, lda );
            magma_tally3_sgetvector( min_mn, dtau, 1, tau, 1 );
            
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally3_sgeqp3 returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
