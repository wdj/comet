    ...
    magma_tally4_int_t *info )
{
    ...
    float z_one = MAGMA_tally4_S_MAKE(  1.0, 0.0 );
    float mz_one = MAGMA_tally4_S_MAKE( -1.0, 0.0 );
    float             one =  1.0;
    float             m_one = -1.0;
    ...
    float *d_lP[Magma_tally4MaxGPUs], *dlpanel, *dlpanels[Magma_tally4MaxGPUs];
    ...
    *info = 0;
    ...
    if (MAGMA_tally4_SUCCESS != magma_tally4_smalloc_pinned( &panel, 2 * nb * ldpanel )) {
    ...
    if ((nb <= 1) || (nb >= n)) {
        // Use unblocked code.
        magma_tally4_sgetmatrix( n, n, dlA(0, 0, 0), ldda, panel, ldpanel);
        lapackf77_spotrf( uplo_, &n, panel, &ldpanel, info);
        magma_tally4_ssetmatrix( n, n, panel, ldpanel, dlA(0, 0, 0), ldda );
    } else {
        for( d = 0; d < ngpu; d++ ) {
            ...
            if (MAGMA_tally4_SUCCESS != magma_tally4_smalloc( &d_lP[d], nb * ldda )) {
                for( j = 0; j < d; j++ ) {
        ...
        if (uplo == Magma_tally4Upper) {
            ...
            if ( n > nb ) {
                ...
                magma_tally4_setdevice(0);
                tmppanel = tmppanel0;
                magma_tally4_sgetmatrix_async(n, nb,
                        dlA(0, 0, 0), ldda,
                        tmppanel(0),  ldpanel,
                        queues[0][0] );
            }
            ...
            for (j = 0; (j + nb) < n; j += nb) {
                ...
                if (j > 0 && prevtrsmrows > crosspoint) {
                    ...
                    blasf77_sgemm( Magma_tally4NoTransStr, Magma_tally4ConjTransStr,
                            &rows, &nb, &nb,
                ...
                #endif
                lapackf77_spotrf(Magma_tally4LowerStr, &nb, tmppanel(j), &ldpanel, info);
                if (*info != 0) {
                    *info = *info + j;
                    break;
                }
                ...
                if (trsmrows > 0) {
                    blasf77_strsm(Magma_tally4RightStr, Magma_tally4LowerStr, Magma_tally4ConjTransStr, Magma_tally4NonUnitStr,
                                  &trsmrows, &nb,
                                  &z_one, tmppanel(j), &ldpanel,
                ...
                for (igpu = 0; igpu < ngpu; igpu++, d = (d + 1) % ngpu ) {
                    ...
                    if (myrows > 0) {
                        magma_tally4_setdevice(d);
                        magma_tally4_ssetmatrix_async(myrows, nb,
                ...
                if ( (j + nb) < n) {
                    ...
                    if (j + nb + nb < n) {
                        ...
                        if (trsmrows <= crosspoint) {
                            ...
                            #ifdef  SSYRK_ON_DIAG
                            magma_tally4_ssyrk( Magma_tally4Lower, Magma_tally4NoTrans,
                                    nb, nb,
                                    m_one, dlpanel, ldda,
                        ...
                        magma_tally4_sgetmatrix_async(rows-nb, nb,
                    ...
                    if (n - offset > 0) {
                        ...
                        //magma_tally4_ssyrk(Magma_tally4Lower, Magma_tally4NoTrans, n - offset, nb,
                        //        m_one, dlpanel, ldda,
            ...
            if (j < n && *info == 0) {
                ...
                #endif
                magma_tally4_sgetmatrix(rows, rows, dlA(id, j, j_local), ldda, panel(j), ldpanel);
                lapackf77_spotrf(Magma_tally4LowerStr, &rows, panel(j), &ldpanel, info);
                magma_tally4_ssetmatrix(rows, rows, panel(j), ldpanel, dlA(id, j, j_local), ldda);
                #if defined (ENABLE_TIMER)
