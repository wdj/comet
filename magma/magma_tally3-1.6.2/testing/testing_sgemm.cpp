...
int main( int argc, char** argv)
{
    ...
    float *h_A, *h_B, *h_C, *h_Cmagma_tally3, *h_Cdev;
    magma_tally3Float_ptr d_A, d_B, d_C;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_Cmagma_tally3,  float, ldc*N  );
            ...
            magma_tally3_ssetmatrix( Am, An, h_A, lda, d_A, ldda );
            magma_tally3_ssetmatrix( Bm, Bn, h_B, ldb, d_B, lddb );
            
            /* =====================================================================
               Performs operation using MAGMA_tally3BLAS (currently only with CUDA)
               =================================================================== */
            #ifdef HAVE_CUBLAS
                magma_tally3_ssetmatrix( M, N, h_C, ldc, d_C, lddc );
                
                magma_tally3_time = magma_tally3_sync_wtime( NULL );
                magma_tally3blas_sgemm( opts.transA, opts.transB, M, N, K,
                                 alpha, d_A, ldda,
                                        d_B, lddb,
                                 beta,  d_C, lddc );
                ...
                magma_tally3_sgetmatrix( M, N, d_C, lddc, h_Cmagma_tally3, ldc );
            #endif
            ...
            magma_tally3_ssetmatrix( M, N, h_C, ldc, d_C, lddc );
                ...
                cublasSgemm( opts.handle, cublas_trans_const_tally3(opts.transA), cublas_trans_const_tally3(opts.transB), M, N, K,
                             &alpha, d_A, ldda,
                                     d_B, lddb,
                             &beta,  d_C, lddc );
                ...
                magma_tally3_sgemm( opts.transA, opts.transB, M, N, K,
                             alpha, d_A, 0, ldda,
                                    d_B, 0, lddb,
            ...
            magma_tally3_sgetmatrix( M, N, d_C, lddc, h_Cdev, ldc );
            ...
            if ( opts.lapack ) {
                ...
                blasf77_sgemm( lapack_trans_const_tally3(opts.transA), lapack_trans_const_tally3(opts.transB), &M, &N, &K,
                               &alpha, h_A, &lda,
                                       h_B, &ldb,
                               &beta,  h_C, &ldc );
            ...
            if ( opts.lapack ) {
                ...
                dev_error = lapackf77_slange( "F", &M, &N, h_Cdev, &ldc, work ) / Cnorm;
                ...
                #ifdef HAVE_CUBLAS
                    blasf77_saxpy( &sizeC, &c_neg_one, h_C, &ione, h_Cmagma_tally3, &ione );
                    magma_tally3_error = lapackf77_slange( "F", &M, &N, h_Cmagma_tally3, &ldc, work ) / Cnorm;
                    ...
                    blasf77_saxpy( &sizeC, &c_neg_one, h_Cdev, &ione, h_Cmagma_tally3, &ione );
                    magma_tally3_error = lapackf77_slange( "F", &M, &N, h_Cmagma_tally3, &ldc, work ) / Cnorm;
