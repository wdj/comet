/*
    -- MAGMA_tally2 (version 1.6.2) --
       ...
       @generated from magma_tally2_zmgenerator.cpp normal z -> c, Sun May  3 11:23:01 2015
       @author Hartwig Anzt
*/
...
/**
    ...
    diag_offset magma_tally2_int_t*
                array containing the offsets
                                                ...
                                                (length offsets+1)
    @param[in]
    diag_vals   magma_tally2FloatComplex*
    ...
    magma_tally2_queue_t queue )
{
    ...
    CHECK( magma_tally2_cmalloc_cpu( &B.val, B.max_nnz_row*n ));
    ...
    for( int i=0; i<n; i++ ) { // stride over rows
        for( int j=0; j<B.max_nnz_row; j++ ) { // nonzeros in every row
            if ( (B.col[i*B.max_nnz_row + j] < 0) ||
                ...
                B.val[ i*B.max_nnz_row + j ] = MAGMA_tally2_C_MAKE( 0.0, 0.0 );
            }
        }
    ...
    for( int i=0; i<n; i++ ) { // stride over rows
        for( int j=0; j<B.max_nnz_row; j++ ) { // nonzeros in every row
            if ( MAGMA_tally2_C_REAL( B.val[i*B.max_nnz_row + j]) != 0.0 )
                B.nnz++;
        }
    ...
    CHECK( magma_tally2_cmconvert( B, A, Magma_tally2_ELLPACKT, Magma_tally2_CSR, queue ));
    ...
    magma_tally2_queue_t queue )
{
    ...
    magma_tally2_c_matrix hA={Magma_tally2_CSR};
    ...
    diag_vals[0] = MAGMA_tally2_C_MAKE( 26.0, 0.0 );
    diag_vals[1] = MAGMA_tally2_C_MAKE( -1.0, 0.0 );
    diag_vals[2] = MAGMA_tally2_C_MAKE( -1.0, 0.0 );
    diag_vals[3] = MAGMA_tally2_C_MAKE( -1.0, 0.0 );
    diag_vals[4] = MAGMA_tally2_C_MAKE( -1.0, 0.0 );
    diag_vals[5] = MAGMA_tally2_C_MAKE( -1.0, 0.0 );
    diag_vals[6] = MAGMA_tally2_C_MAKE( -1.0, 0.0 );
    diag_vals[7] = MAGMA_tally2_C_MAKE( -1.0, 0.0 );
    diag_vals[8] = MAGMA_tally2_C_MAKE( -1.0, 0.0 );
    diag_vals[9] = MAGMA_tally2_C_MAKE( -1.0, 0.0 );
    diag_vals[10] = MAGMA_tally2_C_MAKE( -1.0, 0.0 );
    diag_vals[11] = MAGMA_tally2_C_MAKE( -1.0, 0.0 );
    diag_vals[12] = MAGMA_tally2_C_MAKE( -1.0, 0.0 );
    diag_vals[13] = MAGMA_tally2_C_MAKE( -1.0, 0.0 );
    CHECK( magma_tally2_cmgenerator( nn, offdiags, diag_offset, diag_vals, &hA, queue ));
        ...
        for( k=hA.row[row]; k<hA.row[row+1]; k++) {
            ...
            if ((hA.col[k] == row-1 ||
                ...
                hA.col[k] == row+n*n+n-1 ) && (row+1)%n == 1 )
                    ...
                    hA.val[k] = MAGMA_tally2_C_MAKE( 0.0, 0.0 );
    ...
    CHECK( magma_tally2_cmconvert( hA, A, Magma_tally2_CSR, Magma_tally2_CSR, queue ));
    ...
    magma_tally2_queue_t queue )
{
    ...
    #ifdef COMPLEX
        // complex case
        diag_vals[0] = MAGMA_tally2_C_MAKE( 4.0, 4.0 );
        diag_vals[1] = MAGMA_tally2_C_MAKE( -1.0, -1.0 );
        diag_vals[2] = MAGMA_tally2_C_MAKE( -1.0, -1.0 );
        
    #else
        // real case
        diag_vals[0] = MAGMA_tally2_C_MAKE( 4.0, 0.0 );
        diag_vals[1] = MAGMA_tally2_C_MAKE( -1.0, 0.0 );
        diag_vals[2] = MAGMA_tally2_C_MAKE( -1.0, 0.0 );
    #endif
    CHECK( magma_tally2_cmgenerator( nn, offdiags, diag_offset, diag_vals, &hA, queue ));
