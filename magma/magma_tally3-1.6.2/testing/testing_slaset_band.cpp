/*
    -- MAGMA_tally3 (version 1.6.1) --
       ...
       @generated from testing_zlaset_band.cpp normal z -> s, Fri Jan 30 19:00:24 2015
       @author Mark Gates
*/
...
int main( int argc, char** argv)
{
    ...
    float offdiag = MAGMA_tally3_S_MAKE( 1.2000, 6.7000 );
    float diag    = MAGMA_tally3_S_MAKE( 3.1415, 2.7183 );
    magma_tally3_int_t M, N, nb, cnt, size, lda, ldda;
        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for( int j=inset; j < N-inset; ++j ) {
                for( int k=0; k < nb; ++k ) {  // set k-th sub- or super-diagonal
                    ...
                    else if ( uplo[iuplo] == Magma_tally3Lower && j+k < M-inset ) {
                        *h_A(j+k,j) = offdiag;
                        cnt += 1;
                    }
                    else if ( uplo[iuplo] == Magma_tally3Upper && j-k >= inset && j-k < M-inset ) {
                        *h_A(j-k,j) = offdiag;
                        cnt += 1;
                    }
                }
            ...
            int mm = M - 2*inset;
            int nn = N - 2*inset;
            magma_tally3blas_slaset_band( uplo[iuplo], mm, nn, nb, offdiag, diag, d_A(inset,inset), ldda );
            ...
            //printf( "h_R=" );  magma_tally3_sprint( M, N, h_R, lda );
            //printf( "h_A=" );  magma_tally3_sprint( M, N, h_A, lda );
