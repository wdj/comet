/*
    -- MAGMA_tally4 (version 1.6.1) --
       ...
       @generated from testing_zgels3_gpu.cpp normal z -> c, Fri Jan 30 19:00:25 2015
...
int main( int argc, char** argv)
{
    ...
    magma_tally4FloatComplex *h_A, *h_A2, *h_B, *h_X, *h_R, *tau, *h_work, tmp[1];
    magma_tally4FloatComplex_ptr d_A, d_B;
    magma_tally4_int_t M, N, size, nrhs, lda, ldb, ldda, lddb, min_mn, max_mn, nb, info;
    magma_tally4_int_t lworkgpu, lhwork, lhwork2;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lhwork2 = (magma_tally4_int_t) MAGMA_tally4_C_REAL( tmp[0] );
            
            lhwork = -1;
            lapackf77_cunmqr( Magma_tally4LeftStr, Magma_tally4_ConjTransStr,
            ...
            TESTING_MALLOC_CPU( h_A2,   magma_tally4FloatComplex, lda*N     );
            TESTING_MALLOC_CPU( h_B,    magma_tally4FloatComplex, ldb*nrhs  );
            TESTING_MALLOC_CPU( h_X,    magma_tally4FloatComplex, ldb*nrhs  );
            TESTING_MALLOC_CPU( h_R,    magma_tally4FloatComplex, ldb*nrhs  );
            TESTING_MALLOC_CPU( h_work, magma_tally4FloatComplex, lhwork    );
            
            TESTING_MALLOC_DEV( d_A,    magma_tally4FloatComplex, ldda*N    );
            TESTING_MALLOC_DEV( d_B,    magma_tally4FloatComplex, lddb*nrhs );
            
            /* Initialize the matrices */
            lapackf77_clarnv( &ione, ISEED, &size, h_A );
            lapackf77_clacpy( Magma_tally4UpperLowerStr, &M, &N, h_A, &lda, h_A2, &lda );
            
            // make random RHS
            size = M*nrhs;
            lapackf77_clarnv( &ione, ISEED, &size, h_B );
            lapackf77_clacpy( Magma_tally4UpperLowerStr, &M, &nrhs, h_B, &ldb, h_R, &ldb );
            
            // make consistent RHS
            //size = N*nrhs;
            //lapackf77_clarnv( &ione, ISEED, &size, h_X );
            //blasf77_cgemm( Magma_tally4NoTransStr, Magma_tally4NoTransStr, &M, &nrhs, &N,
            //               &c_one,  h_A, &lda,
            //                        h_X, &ldb,
            //               &c_zero, h_B, &ldb );
            //lapackf77_clacpy( Magma_tally4UpperLowerStr, &M, &nrhs, h_B, &ldb, h_R, &ldb );
            ...
            magma_tally4_csetmatrix( M, nrhs, h_B, ldb, d_B, lddb );
            
            gpu_time = magma_tally4_wtime();
            magma_tally4_cgels3_gpu( Magma_tally4NoTrans, M, N, nrhs, d_A, ldda,
                              d_B, lddb, h_work, lworkgpu, &info);
            gpu_time = magma_tally4_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally4_cgels3_gpu returned error %d: %s.\n",
                       (int) info, magma_tally4_strerror( info ));
            
            // Get the solution in h_X
            magma_tally4_cgetmatrix( N, nrhs, d_B, lddb, h_X, ldb );
            ...
            /* =====================================================================
               ...
               =================================================================== */
            lapackf77_clacpy( Magma_tally4UpperLowerStr, &M, &nrhs, h_B, &ldb, h_X, &ldb );
            
            cpu_time = magma_tally4_wtime();
            lapackf77_cgels( Magma_tally4NoTransStr, &M, &N, &nrhs,
