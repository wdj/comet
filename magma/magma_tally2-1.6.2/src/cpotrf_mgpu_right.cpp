    ...
    magma_tally2_int_t *info )
{
    ...
    magma_tally2FloatComplex z_one = MAGMA_tally2_C_MAKE(  1.0, 0.0 );
    magma_tally2FloatComplex mz_one = MAGMA_tally2_C_MAKE( -1.0, 0.0 );
    float             one =  1.0;
    float             m_one = -1.0;
    const char* uplo_ = lapack_uplo_const_tally2( uplo );

    magma_tally2_int_t j, nb, d, id, j_local, blkid, crosspoint, prevtrsmrows=0, nqueue = 5;
    magma_tally2FloatComplex *panel, *tmppanel0, *tmppanel1, *tmppanel, *tmpprevpanel;
    magma_tally2FloatComplex *d_lP[Magma_tally2MaxGPUs], *dlpanel, *dlpanels[Magma_tally2MaxGPUs];
    magma_tally2_int_t rows, trsmrows, igpu, n_local[Magma_tally2MaxGPUs], ldpanel;
    magma_tally2_queue_t queues[Magma_tally2MaxGPUs][10];
    ...
    *info = 0;
    ...
    nb = magma_tally2_get_cpotrf_nb(n);

    ldpanel = ldda;
    magma_tally2_setdevice(0);
    if (MAGMA_tally2_SUCCESS != magma_tally2_cmalloc_pinned( &panel, 2 * nb * ldpanel )) {
    ...
    if ((nb <= 1) || (nb >= n)) {
        // Use unblocked code.
        magma_tally2_cgetmatrix( n, n, dlA(0, 0, 0), ldda, panel, ldpanel);
        lapackf77_cpotrf( uplo_, &n, panel, &ldpanel, info);
        magma_tally2_csetmatrix( n, n, panel, ldpanel, dlA(0, 0, 0), ldda );
    } else {
        for( d = 0; d < ngpu; d++ ) {
            ...
            if (MAGMA_tally2_SUCCESS != magma_tally2_cmalloc( &d_lP[d], nb * ldda )) {
                for( j = 0; j < d; j++ ) {
                    magma_tally2_setdevice(j);
                    magma_tally2_free( d_lP[d] );
                }
                *info = MAGMA_tally2_ERR_DEVICE_ALLOC;
                return *info;
            }
            for( j=0; j < nqueue; j++ ) {
                magma_tally2_queue_create( &queues[d][j] );
            }
        }
        ...
        if (uplo == Magma_tally2Upper) {
            ...
            #if defined (ENABLE_TIMER)
            real_Double_t tget = magma_tally2_wtime(), tset = 0.0, ttot = 0.0;
            #endif
            if ( n > nb ) {
                // send first panel to cpu
                magma_tally2_setdevice(0);
                tmppanel = tmppanel0;
                magma_tally2_cgetmatrix_async(n, nb,
                        dlA(0, 0, 0), ldda,
                        tmppanel(0),  ldpanel,
                        queues[0][0] );
            }
            ...
            for (j = 0; (j + nb) < n; j += nb) {
                ...
                magma_tally2_queue_sync( queues[id][0] );
                if (j > 0 && prevtrsmrows > crosspoint) {
                    #if defined (ENABLE_TIMER)
                    tcnp = magma_tally2_wtime();
                    #endif

                    tmpprevpanel = ((blkid - 1) % 2) == 0 ? tmppanel0 : tmppanel1;

                    blasf77_cgemm( Magma_tally2NoTransStr, Magma_tally2ConjTransStr,
                            &rows, &nb, &nb,
                    ...
                    #if defined (ENABLE_TIMER)
                    tcnp = magma_tally2_wtime() - tcnp;
                    ttot_cnp += tcnp;
                    #endif
                }

                #if defined (ENABLE_TIMER)
                tcchol = magma_tally2_wtime();
                #endif
                lapackf77_cpotrf(Magma_tally2LowerStr, &nb, tmppanel(j), &ldpanel, info);
                if (*info != 0) {
                    *info = *info + j;
                    break;
                }
                ...
                #if defined (ENABLE_TIMER)
                tcchol = magma_tally2_wtime() - tcchol;
                ttot_cchol += tcchol;
                tctrsm = magma_tally2_wtime();
                #endif

                trsmrows = rows - nb;

                if (trsmrows > 0) {
                    blasf77_ctrsm(Magma_tally2RightStr, Magma_tally2LowerStr, Magma_tally2ConjTransStr, Magma_tally2NonUnitStr,
                                  &trsmrows, &nb,
                                  &z_one, tmppanel(j), &ldpanel,
                ...
                #if defined (ENABLE_TIMER)
                tctrsm = magma_tally2_wtime() - tctrsm;
                ttot_ctrsm += tctrsm;
                tctm = magma_tally2_wtime();
                #endif

                d = (id + 1) % ngpu;
                // send current panel to gpus
                for (igpu = 0; igpu < ngpu; igpu++, d = (d + 1) % ngpu ) {
                    magma_tally2_int_t myrows = 0;
                    magma_tally2_int_t row_offset = 0;
                    if ( d == id ) {
                    ...
                    if (myrows > 0) {
                        magma_tally2_setdevice(d);
                        magma_tally2_csetmatrix_async(myrows, nb,
                ...
                for (igpu = 0; igpu < ngpu; igpu++, d = (d + 1) % ngpu ) {
                    magma_tally2_setdevice(d);
                    magma_tally2_queue_sync( queues[d][0] );
                }

                #if defined (ENABLE_TIMER)
                tctm = magma_tally2_wtime() - tctm;
                ttot_ctm += tctm;
                #endif
                ...
                if ( (j + nb) < n) {
                    ...
                    if (j + nb + nb < n) {
                        d = (id + 1) % ngpu;
                        magma_tally2_setdevice(d);
                        magma_tally2_int_t j_local2 = (j + nb) / (nb * ngpu) * nb;
                        if (trsmrows <= crosspoint) {
                            #if defined (ENABLE_TIMER)
                            tmnp = magma_tally2_wtime();
                            #endif
                            ...
                            magma_tally2blasSetKernelStream( queues[d][STREAM_ID(j_local2)] );
                            #define CHERK_ON_DIAG
                            #ifdef  CHERK_ON_DIAG
                            magma_tally2_cherk( Magma_tally2Lower, Magma_tally2NoTrans,
                                    nb, nb,
                                    m_one, dlpanel, ldda,
                            ...
                            #if defined (ENABLE_TIMER)
                            magma_tally2_device_sync();
                            tmnp = magma_tally2_wtime() - tmnp;
                            ttot_mnp += tmnp;
                            #endif
                        }
                        // send next panel to cpu
                        magma_tally2_queue_sync( queues[d][STREAM_ID(j_local2)] ); // make sure lookahead is done
                        tmppanel = ((blkid+1) % 2 == 0) ? tmppanel0 : tmppanel1;
                        magma_tally2_cgetmatrix_async(rows-nb, nb,
                    ...
                    if (n - offset > 0) {
                        ...
                        #if defined (ENABLE_TIMER)
                        for( d=0; d < ngpu; d++ ) therk[d] = magma_tally2_wtime();
                        #endif

                        //magma_tally2blasSetKernelStream( queues[d] );
                        //magma_tally2_cherk(Magma_tally2Lower, Magma_tally2NoTrans, n - offset, nb,
                        //        m_one, dlpanel, ldda,
                        ...
                        #endif
                                        (ngpu, Magma_tally2Lower, Magma_tally2NoTrans,
                                         nb, n - offset, nb,
                        ...
                        for( d=0; d < ngpu; d++ ) {
                            magma_tally2_setdevice(d);
                            magma_tally2_device_sync();
                            therk[d] = magma_tally2_wtime() - therk[d];
                            ttot_herk[d] += therk[d];
                        }
                        #endif
                    }
            ...
            for( d = 0; d < ngpu; d++ ) {
                ...
                for( id=0; id < nqueue; id++ ) {
                    magma_tally2_queue_sync( queues[d][id] );
                }
            }
            #if defined (ENABLE_TIMER)
            ...
            if (j < n && *info == 0) {
                ...
                magma_tally2_setdevice(id);
                #if defined (ENABLE_TIMER)
                tset = magma_tally2_wtime();
                #endif
                magma_tally2_cgetmatrix(rows, rows, dlA(id, j, j_local), ldda, panel(j), ldpanel);
                lapackf77_cpotrf(Magma_tally2LowerStr, &rows, panel(j), &ldpanel, info);
                magma_tally2_csetmatrix(rows, rows, panel(j), ldpanel, dlA(id, j, j_local), ldda);
                #if defined (ENABLE_TIMER)
                tset = magma_tally2_wtime() - tset;
                #endif
            }
            #if defined (ENABLE_TIMER)
        ...
        for( d = 0; d < ngpu; d++ ) {
            magma_tally2_setdevice(d);
            for( j=0; j < nqueue; j++ ) {
                magma_tally2_queue_destroy( queues[d][j] );
            }
            magma_tally2_free( d_lP[d] );
        }
    } // end of not lapack
    ...
    // free workspace
    magma_tally2_free_pinned( panel );
