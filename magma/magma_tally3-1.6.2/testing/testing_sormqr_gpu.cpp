/*
    -- MAGMA_tally3 (version 1.6.1) --
       ...
       @author Mark Gates
       @generated from testing_zunmqr_gpu.cpp normal z -> s, Fri Jan 30 19:00:26 2015
*/
// includes, system
#include <stdlib.h>
#include <stdio.h>
...
int main( int argc, char** argv )
{
    ...
    float *C, *R, *A, *W, *tau;
    magma_tally3Float_ptr dC, dA, dT;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int itran = 0; itran < 2; ++itran ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {        
            ...
            magma_tally3_ssetmatrix( lda, k, A,  lda, dA, lda );
            magma_tally3_sgeqrf_gpu( lda, k, dA, lda, tau, dT, &info );
            magma_tally3_sgetmatrix( lda, k, dA, lda, A,  lda );
            if (info != 0)
                printf("magma_tally3_sgeqrf_gpu returned error %d: %s.\n",
            ...
            lapackf77_sormqr( lapack_side_const_tally3( side[iside] ), lapack_trans_const_tally3( trans[itran] ),
                              &m, &n, &k,
            ...
            lwork = -1;
            magma_tally3_sormqr_gpu( side[iside], trans[itran],
                              m, n, k,
                              dA, lda, tau, dC, ldc, W, lwork, dT, nb, &info );
            if (info != 0)
                printf("magma_tally3_sormqr_gpu (lwork query) returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
            lwork = (magma_tally3_int_t) MAGMA_tally3_S_REAL( W[0] );
            ...
            if (info != 0)
                printf("magma_tally3_sormqr_gpu returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
