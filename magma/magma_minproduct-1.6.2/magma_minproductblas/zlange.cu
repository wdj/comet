...
zlange_inf_kernel(
    int m, int n, const magma_minproductDoubleComplex *A, int lda, double *dwork )
{
    ...
    if ( i < m ) {
        ...
        if ( n >= 4 ) {
            const magma_minproductDoubleComplex *Aend = A + lda*n;
            magma_minproductDoubleComplex rA[4] = { A[0], A[lda], A[2*lda], A[3*lda] };
            A += 4*lda;
    ...
    magma_minproductDouble_ptr dwork )  //, magma_minproduct_int_t lwork )
{
    ...
    if ( norm == Magma_minproductInfNorm ) {
        dim3 grid( (m-1)/64 + 1 );
        zlange_inf_kernel<<< grid, threads, 0, magma_minproduct_stream >>>( m, n, dA, ldda, dwork );
        ...
        dim3 grid( (m-1)/64 + 1 );
        zlange_max_kernel<<< grid, threads, 0, magma_minproduct_stream >>>( m, n, dA, ldda, dwork );
        max_nan_kernel<<< 1, 512, 0, magma_minproduct_stream >>>( m, dwork );
        cudaMemcpy( &result, &dwork[0], sizeof(double), cudaMemcpyDeviceToHost );
    }
    else if ( norm == Magma_minproductOneNorm ) {
        dim3 grid( n );
        zlange_one_kernel<<< grid, threads, 0, magma_minproduct_stream >>>( m, n, dA, ldda, dwork );
