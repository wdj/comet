...
//FIX printf("HEY1  %i   %i   %i %i   %i %i\n", step_2way, J, K_min, K_max, I_min, I_max); //FIX

    enum {MF = METRIC_FORMAT};
    typedef MetricFormatType<MF> MFT;
    typedef typename MFT::TypeIn MFTTypeIn;
    ...
    for (int K = K_min; K < K_max; ++K) {
      ...
      for (int I = I_min; I < I_max; ++I) {
        ...
        if (is_I_in_range) {
          ...
          const bool init_numer = is_halved ? true : 0 == step_2way;
          ...
          MFTTypeIn r000, r001, r010, r011, r100, r101, r110, r111;
          ...
          MFTTypeIn r000_perm = r000;
          MFTTypeIn r100_perm = si->perm0(r100, r010, r001);
          MFTTypeIn r010_perm = si->perm1(r100, r010, r001);
          MFTTypeIn r001_perm = si->perm2(r100, r010, r001);
          MFTTypeIn r011_perm = si->perm0(r011, r101, r110);
          MFTTypeIn r101_perm = si->perm1(r011, r101, r110);
          MFTTypeIn r110_perm = si->perm2(r011, r101, r110);
          MFTTypeIn r111_perm = r111;
          ...
          if (is_halved) {
            ...
            // Only process if the relevant index is in range.

            //const bool is_I_mapped_lo_in_range
            //  = I_mapped_lo >= I_min && I_mapped_lo < I_max;

            //const bool is_I_mapped_hi_in_range
            //  = /I_mapped_hi >= I_min && I_mapped_hi < I_max;

            // Add in lo part.

//FIX printf("%i   %i %i   %i %i %i   %i %i    %i %i\n", step_2way, nvl,nvle, I,J,K, I_min,I_max, I_mapped_lo,I_mapped_hi); //FIX


            //if (is_I_mapped_lo_in_range) {
            {
              ...
              MFTTypeIn matB00_perm, matB01_perm, matB10_perm, matB11_perm;
              MFT::decode(matB00_perm, matB01_perm, matB_perm.data[0]);
              MFT::decode(matB10_perm, matB11_perm, matB_perm.data[1]);

//FIX printf("lo %f %f %f %f    %i %i %i\n", (double)matB00_perm, (double)matB01_perm, (double)matB10_perm, (double)matB11_perm, I_mapped_lo, K, (int)matB_buf->dim0);
#if 0
printf("%f %f %f %f    %f %f %f %f %f %f %f %f\n", (double)matB00_perm, (double)matB01_perm, (double)matB10_perm, (double)matB11_perm

, (double)(((float*)(matB_buf->h))[4])
, (double)(((float*)(matB_buf->h))[5])
, (double)(((float*)(matB_buf->h))[6])
, (double)(((float*)(matB_buf->h))[7])
, (double)(((float*)(matB_buf->h))[12])
, (double)(((float*)(matB_buf->h))[13])
, (double)(((float*)(matB_buf->h))[14])
, (double)(((float*)(matB_buf->h))[15])

); //FIX
#endif

//printf("1--- %i %i %i    %i %i %i %i\n", I, J, K, matB00_perm, matB01_perm, matB10_perm, matB11_perm);
...
//printf("Hey2\n");
// !is_I_in_range ?  GMTally2x2_null() :

              MFTTypeIn matB00_perm, matB01_perm, matB10_perm, matB11_perm;
              MFT::decode(matB00_perm, matB01_perm, matB_perm.data[0]);
              MFT::decode(matB10_perm, matB11_perm, matB_perm.data[1]);

//FIX printf("hi %f %f %f %f    %i %i %i\n", (double)matB00_perm, (double)matB01_perm, (double)matB10_perm, (double)matB11_perm, I_mapped_hi, K, (int)matB_buf->dim0);
...
//printf(">>>>>>>>>>>>>>>>>>>>>>>> %i %i %i %i %i\n", vars.J, vars.I_min, vars.I_max, vars.K_min, vars.K_max); //FIX
