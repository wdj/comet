...
int main( int argc, char** argv)
{
    ...
    magma_tally2DoubleComplex *h_A, *h_b, *h_x, *h_xcublas;
    magma_tally2DoubleComplex_ptr d_A, d_x;
    ...
    printf("uplo = %s, transA = %s, diag = %s\n",
           lapack_uplo_const_tally2(opts.uplo), lapack_trans_const_tally2(opts.transA), lapack_diag_const_tally2(opts.diag) );
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_b,       magma_tally2DoubleComplex, N     );
            TESTING_MALLOC_CPU( h_x,       magma_tally2DoubleComplex, N     );
            TESTING_MALLOC_CPU( h_xcublas, magma_tally2DoubleComplex, N     );
            
            TESTING_MALLOC_DEV( d_A, magma_tally2DoubleComplex, ldda*N );
            TESTING_MALLOC_DEV( d_x, magma_tally2DoubleComplex, N      );
            ...
            magma_tally2_zsetvector( N, h_x, 1, d_x, 1 );
            
            cublas_time = magma_tally2_sync_wtime( NULL );
            cublasZtrsv( opts.handle, cublas_uplo_const_tally2(opts.uplo),
            ...
            magma_tally2_zgetvector( N, d_x, 1, h_xcublas, 1 );
            ...
            if ( opts.lapack ) {
                ...
                blasf77_ztrsv( lapack_uplo_const_tally2(opts.uplo), lapack_trans_const_tally2(opts.transA), lapack_diag_const_tally2(opts.diag),
                               &N,
                               h_A, &lda,
                               h_x, &ione );
            ...
            blasf77_ztrmv( lapack_uplo_const_tally2(opts.uplo), lapack_trans_const_tally2(opts.transA), lapack_diag_const_tally2(opts.diag),
                           &N,
                           h_A, &lda,
                           h_xcublas, &ione );
