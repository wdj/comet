...
// Version 2 is MAGMA_tally3  slaln2, which is exactly the same, but thread safe.
#define VERSION 2
...
/**
    ...
    If trans = Magma_tally3NoTrans, lambda is an eigenvalue for the lower 1x1 or 2x2 block,
    and it solves
        ( [ That u      ] - lambda*I ) * x = 0,
        ( [ 0    lambda ]            )
    ...
    If trans = Magma_tally3Trans, lambda is an eigenvalue for the upper 1x1 or 2x2 block,
    and it solves
    ...
    trans   magma_tally3_trans_t
      ...
      -     = Magma_tally3NoTrans:    Solve (T - lambda*I)    * x = 0  (No transpose)
      -     = Magma_tally3Trans:      Solve (T - lambda*I)**T * x = 0  (Transpose)
    ...
    cnorm   (input) REAL array, dimension (N)
            ...
            of T.  If TRANS = Magma_tally3NoTrans, CNORM(j) must be greater than or equal
            to the infinity-norm, and if TRANS = Magma_tally3Trans or Magma_tally3ConjTrans, CNORM(j)
    ...
    @ingroup magma_tally3_sgeev_aux
    ********************************************************************/
extern "C"
magma_tally3_int_t magma_tally3_slaqtrsd(
...
#define W(i,j)  (W + (i) + (j)*2)
    ...
    // constants
    const magma_tally3_int_t c_false = false;
    const magma_tally3_int_t c_true  = true;
    const magma_tally3_int_t ione = 1;
    const magma_tally3_int_t itwo = 2;
    const float   c_zero = 0.;
    const float   c_one  = 1.;
    
    // .. Local Scalars ..
    magma_tally3_int_t notran;
    magma_tally3_int_t ierr, j, j1, j2, jnxt, k, len;
    ...
    *info = 0;
    if ( ! notran && trans != Magma_tally3Trans ) {
        *info = -1;
    }
    else if ( n < 0 ) {
        *info = -2;
    ...
    if ( notran ) {
        ...
        if ( wi == c_zero ) {
            ...
            *x(n-1,0) = c_one;
            ...
            //magma_tally3_s_snprint( buf, sizeof(buf), n, 1, x, ldx );
            ...
            *x(n-2,1) = c_zero;
            ...
            //magma_tally3_s_snprint( buf, sizeof(buf), n, 2, x, ldx );
            ...
            *x(0,0) = c_one;
            ...
            for( j=1; j < n; ++j ) {
                ...
                if ( j1 == j2 ) {
                    ...
                    len = j-1;
                    *x(j,0) -= magma_tally3_cblas_sdot( len, T(1,j), ione, x(1,0), ione );
                    ...
                    *x(j,0) = *W(0,0);
                    ...
                    len = j-1;
                    *x(j,  0) -= magma_tally3_cblas_sdot( len, T(1,j),   ione, x(1,0), ione );
                    *x(j+1,0) -= magma_tally3_cblas_sdot( len, T(1,j+1), ione, x(1,0), ione );
            ...
            *x(0,1) = c_zero;
            ...
            for( j=2; j < n; ++j ) {
                ...
                if ( j1 == j2 ) {
                    ...
                    len = j-2;
                    *x(j,0) -= magma_tally3_cblas_sdot( len, T(2,j), ione, x(2, 0), ione );
                    *x(j,1) -= magma_tally3_cblas_sdot( len, T(2,j), ione, x(2, 1), ione );
                    ...
                    *x(j,  0) -= magma_tally3_cblas_sdot( len, T(2,j),   ione, x(2,0), ione );
                    *x(j,  1) -= magma_tally3_cblas_sdot( len, T(2,j),   ione, x(2,1), ione );
                    *x(j+1,0) -= magma_tally3_cblas_sdot( len, T(2,j+1), ione, x(2,0), ione );
                    *x(j+1,1) -= magma_tally3_cblas_sdot( len, T(2,j+1), ione, x(2,1), ione );
