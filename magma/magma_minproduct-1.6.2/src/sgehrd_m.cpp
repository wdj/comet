/*
    -- MAGMA_minproduct (version 1.6.1) --
       ...
       @generated from zgehrd_m.cpp normal z -> s, Fri Jan 30 19:00:19 2015
       @author Mark Gates
*/
...
/**
    ...
    This version stores the T matrices, for later use in magma_minproduct_sorghr.
    ...
    magma_minproduct_int_t *info)
{
    ...
    magma_minproduct_int_t nb = magma_minproduct_get_sgehrd_nb(n);

    magma_minproduct_int_t nh, iws, ldda, min_lblocks, max_lblocks, last_dev, d;
    magma_minproduct_int_t dpanel, di, nlocal, i, i2, ib, ldwork;
    magma_minproduct_int_t iinfo;
    magma_minproduct_int_t lquery;
    struct sgehrd_data data;

    int ngpu = magma_minproduct_num_gpus();
    
    *info = 0;
    iws = n*(nb + nb*ngpu);
    work[0] = MAGMA_minproduct_S_MAKE( iws, 0 );
    ...
    if (nb == 1 || nb >= nh) {
        ...
        for( d = 0; d < ngpu; ++d ) {
            ...
            ldwork = nlocal*ldda   // A
                   ...
                   + nb*ldda       // W
                   + nb*nb;        // Ti
            if ( MAGMA_minproduct_SUCCESS != magma_minproduct_smalloc( &data.A[d], ldwork )) {
            ...
            magma_minproduct_queue_create( &data.streams[d] );
        }
        
        // Copy the matrix to GPUs
        magma_minproduct_ssetmatrix_1D_col_bcyclic( n, n, A, lda, data.A, ldda, ngpu, nb );
        ...
        for (i = ilo; i < ihi - 1 - nb; i += nb) {
            ...
            if ( i > ilo ) {
                ...
                magma_minproduct_sgetmatrix( ihi-i, nb,
                                  dA(dpanel, i, di), ldda,
                                  A(i,i),            lda );
            }
            
            // add 1 to i for 1-based index
            magma_minproduct_slahr2_m( ihi, i+1, nb, A(0,i), lda,
                            &tau[i], &T[i*nb], nb, work, n, &data );
            
            magma_minproduct_slahru_m( n, ihi, i, nb, A, lda, &data );
            
            // copy first i rows above panel to host
            magma_minproduct_setdevice( dpanel );
            magma_minproduct_sgetmatrix_async( i, nb,
                                    dA(dpanel, 0, di), ldda,
        ...
        for( i2 = i; i2 < n; i2 += nb ) {
            ...
            di = (i2 / nb) / ngpu * nb;
            magma_minproduct_setdevice( d );
            magma_minproduct_sgetmatrix( n, ib,
                              dA(d, 0, di), ldda,
                              A(0,i2),      lda );
        }
    }
...
CLEANUP:
    for( d = 0; d < ngpu; ++d ) {
        magma_minproduct_setdevice( d );
        magma_minproduct_free( data.A[d] );
        magma_minproduct_queue_destroy( data.streams[d] );
    }
