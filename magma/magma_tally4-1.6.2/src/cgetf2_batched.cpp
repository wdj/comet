...

{
    ...
    //magma_tally4FloatComplex **cpuAarray = (magma_tally4FloatComplex**) malloc(batchCount*sizeof(magma_tally4FloatComplex*));
    ...
    for( panelj=0; panelj < min_mn; panelj+=nb) 
    {
        ...
        for(step=0; step < ib; step++){
            gbj = panelj+step;
            //size_t required_shmem_size = zamax*(sizeof(float)+sizeof(int)) + (m-panelj+2)*sizeof(magma_tally4FloatComplex);
            //if( (m-panelj) > 0)
            ...
            //if( required_shmem_size >  (MAX_SHARED_ALLOWED*1024))
            {
                ...
                arginfo = magma_tally4_icamax_batched(m-gbj, dA_array, 1, gbj, lda, ipiv_array, info_array, gbstep, batchCount, queue);
                if(arginfo != 0 ) return arginfo;
                // Apply the interchange to columns 1:N. swap the whole row
                arginfo = magma_tally4_cswap_batched(n, dA_array, lda, gbj, ipiv_array, batchCount, queue);
                if(arginfo != 0 ) return arginfo;
                ...
                arginfo = magma_tally4_ccomputecolumn_batched(m-panelj, panelj, step, dA_array, lda, ipiv_array, info_array, gbstep, batchCount, queue);
                if(arginfo != 0 ) return arginfo;
                // Apply the interchange to columns 1:N. swap the whole row
                arginfo = magma_tally4_cswap_batched(n, dA_array, lda, gbj, ipiv_array, batchCount, queue);
                if(arginfo != 0 ) return arginfo;
