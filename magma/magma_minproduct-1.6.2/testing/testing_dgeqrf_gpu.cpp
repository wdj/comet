...
int main( int argc, char** argv)
{
    ...
    magma_minproductDouble_ptr d_A, dT;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( opts.version == 2 ) {
                ...
                magma_minproduct_dgeqrf2_gpu( M, N, d_A, ldda, tau, &info );
            }
            else {
                ...
                if ( opts.version == 1 ) {
                    ...
                    magma_minproduct_dgeqrf_gpu( M, N, d_A, ldda, tau, dT, &info );
                }
                #ifdef HAVE_CUBLAS
                else if ( opts.version == 3 ) {
                    // stores dT, V blocks have zeros, R blocks stored in dT
                    magma_minproduct_dgeqrf3_gpu( M, N, d_A, ldda, tau, dT, &info );
                }
                #endif
                else {
            ...
            if (info != 0)
                printf("magma_minproduct_dgeqrf returned error %d: %s.\n",
            ...
            if ( opts.check && opts.version == 2 ) {
                /* =====================================================================
                   ...
                   =================================================================== */
                magma_minproduct_dgetmatrix( M, N, d_A, ldda, h_R, lda );
                ...
                double *x, *b, *hwork;
                magma_minproductDouble_ptr d_B;
                const double c_zero    = MAGMA_minproduct_D_ZERO;
                const double c_one     = MAGMA_minproduct_D_ONE;
                const double c_neg_one = MAGMA_minproduct_D_NEG_ONE;
                const magma_minproduct_int_t ione = 1;
                ...
                magma_minproduct_dsetvector( M, b, 1, d_B, 1 );

                if ( opts.version == 1 ) {
                    // allocate hwork
                    magma_minproduct_dgeqrs_gpu( M, N, 1,
                                      d_A, ldda, tau, dT,
                                      d_B, M, tmp, -1, &info );
                    lwork = (magma_minproduct_int_t)MAGMA_minproduct_D_REAL( tmp[0] );
                    ...
                    if (info != 0)
                        printf("magma_minproduct_dgeqrs returned error %d: %s.\n",
                               (int) info, magma_minproduct_strerror( info ));
                    ...
                    // allocate hwork
                    magma_minproduct_dgeqrs3_gpu( M, N, 1,
                                       d_A, ldda, tau, dT,
                                       d_B, M, tmp, -1, &info );
                    lwork = (magma_minproduct_int_t)MAGMA_minproduct_D_REAL( tmp[0] );
                    TESTING_MALLOC_CPU( hwork, double, lwork );
                    ...
                    magma_minproduct_dgeqrs3_gpu( M, N, 1,
                                       ...
                                       d_B, M, hwork, lwork, &info );
                    if (info != 0)
                        printf("magma_minproduct_dgeqrs3 returned error %d: %s.\n",
                               (int) info, magma_minproduct_strerror( info ));
                    TESTING_FREE_CPU( hwork );
                }
                #endif
                ...
                magma_minproduct_dgetvector( N, d_B, 1, x, 1 );
