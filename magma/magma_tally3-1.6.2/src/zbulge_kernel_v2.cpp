...
*/
    ...
    magma_tally3_int_t ione = 1;
    magma_tally3_int_t vpos, taupos, len, len2;

    magma_tally3DoubleComplex c_one    =  MAGMA_tally3_Z_ONE;

    magma_tally3_bulge_findVTAUpos(n, nb, Vblksiz, sweep-1, st-1, ldv, &vpos, &taupos);
    ...
    *V(vpos)  = c_one;
    ...
    magma_tally3_zlarfxsym_v2(len, A(st,st), lda-1, V(vpos), TAU(taupos), work);
}
#undef A
#undef V
#undef TAU
    ...
    magma_tally3DoubleComplex *work)
{
    ...
    */
    ...
    magma_tally3_int_t ione = 1;
    magma_tally3_int_t vpos, taupos;

    magma_tally3DoubleComplex conjtmp;

    magma_tally3DoubleComplex c_one = MAGMA_tally3_Z_ONE;

    magma_tally3_int_t ldx = lda-1;
    magma_tally3_int_t len = ed - st + 1;
    magma_tally3_int_t lem = min(ed+nb, n) - ed;
    magma_tally3_int_t lem2;
    ...
    if (lem > 1) {
        magma_tally3_bulge_findVTAUpos(n, nb, Vblksiz, sweep-1, ed, ldv, &vpos, &taupos);

        /* remove the first column of the created bulge */
        *V(vpos)  = c_one;
        //memcpy(V(vpos+1), A(ed+2, st), (lem-1)*sizeof(magma_tally3DoubleComplex));
        lem2 = lem-1;
        blasf77_zcopy( &lem2, A(ed+2, st), &ione, V(vpos+1), &ione );
        memset(A(ed+2, st),0,(lem-1)*sizeof(magma_tally3DoubleComplex));
