...
int main( int argc, char** argv)
{
    ...
    magma_minproduct_int_t M, N, n2, lda, lwork, info, min_mn, nb;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_minproduct_zgeqlf( M, N, h_R, lda, tau, h_work, lwork, &info);
            gpu_time = magma_minproduct_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_minproduct_zgeqlf returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                magma_minproduct_int_t ldl = min_mn;
                magma_minproductDoubleComplex *Q, *L;
                double *work;
                TESTING_MALLOC_CPU( Q,    magma_minproductDoubleComplex, ldq*min_mn );  // M by K
                TESTING_MALLOC_CPU( L,    magma_minproductDoubleComplex, ldl*N );       // K by N
                ...
                if ( M >= N ) {
                    ...
                    //                   [ L L L ]
                    magma_minproduct_int_t M_N = M - N;
                    ...
                    magma_minproduct_int_t N_M = N - M;
