...
/**
    Purpose
    -------
    If VECT = Magma_minproductQ, DORMBR overwrites the general real M-by-N matrix C with
    ...
    If VECT = Magma_minproductP, DORMBR overwrites the general real M-by-N matrix C with
                                 SIDE = Magma_minproductLeft     SIDE = Magma_minproductRight
    ...
    *info = 0;
    ...
    if (*info == 0) {
        if (nw > 0) {
            ...
            nb = magma_minproduct_get_dgebrd_nb( min( m, n ));
            lwkopt = max(1, nw*nb);
        }
        else {
            lwkopt = 1;
    ...
    if (applyq) {
        ...
        if (nq >= k) {
            ...
            #else
            magma_minproduct_dormqr( side, trans,
                          m, n, k, A, lda, tau, C, ldc, work, lwork, &iinfo);
            #endif
        }
        else if (nq > 1) {
        ...
        if (nq > k) {
            ...
            lapackf77_dormlq( lapack_side_const(side), lapack_trans_const(transt),
                              &m, &n, &k, A, &lda, tau, C, &ldc, work, &lwork, &iinfo);
            #else
            magma_minproduct_dormlq( side, transt,
                          m, n, k, A, lda, tau, C, ldc, work, lwork, &iinfo);
            #endif
        }
        else if (nq > 1) {
