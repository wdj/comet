...
#define dwork(i_, j_) (dwork + (i_) + (j_)*lddwork)
    ...
    for(i=nb; i < n; i += nb) {
        ...
        magma_tally4blas_dtranspose_q( m, nb, dwork(0,(j%2)*nb), lddwork, dAT(i-nb,0), ldda, queues[j%2] );
    }

    /* Transpose the last part of the matrix.                            */
    j++;
    magma_tally4blas_dtranspose_q( m, ib, dwork(0,(j%2)*nb), lddwork, dAT(i-nb,0), ldda, queues[j%2] );
}


// @see magma_tally4blas_dsetmatrix_transpose_q
extern "C" void 
    ...
    magma_tally4Double_ptr     dwork, magma_tally4_int_t lddwork, magma_tally4_int_t nb )
{
    ...
    magma_tally4blas_dsetmatrix_transpose_q( m, n, hA, lda, dAT, ldda, dwork, lddwork, nb, queues );
