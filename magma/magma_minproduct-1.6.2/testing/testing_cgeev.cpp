...
bool lessthan( magma_minproductFloatComplex a, magma_minproductFloatComplex b )
{
    return (MAGMA_minproduct_C_REAL(a) < MAGMA_minproduct_C_REAL(b)) ||
        (MAGMA_minproduct_C_REAL(a) == MAGMA_minproduct_C_REAL(b) && MAGMA_minproduct_C_IMAG(a) < MAGMA_minproduct_C_IMAG(b));
}
...
extern "C"
float magma_minproduct_dlapy2(float x, float y)
{
    float ret_val, d;
    float w, z, xabs, yabs;
    
    xabs = MAGMA_minproduct_D_ABS(x);
    yabs = MAGMA_minproduct_D_ABS(y);
    w    = max(xabs, yabs);
    z    = min(xabs, yabs);
    if (z == 0.) {
        ret_val = w;
...
extern "C"
float magma_minproduct_dzlapy2(magma_minproductFloatComplex x)
{
    return magma_minproduct_dlapy2( MAGMA_minproduct_C_REAL(x), MAGMA_minproduct_C_IMAG(x) );
}
...
int main( int argc, char** argv)
{
    ...
    magma_minproductFloatComplex *h_A, *h_R, *VL, *VR, *h_work, *w1, *w2;
    magma_minproductFloatComplex *w1copy, *w2copy;
    magma_minproductFloatComplex  c_neg_one = MAGMA_minproduct_C_NEG_ONE;
    float *rwork;
    float tnrm, result[9];
    magma_minproduct_int_t N, n2, lda, nb, lwork, info;
    ...
    if ( opts.check && !opts.lapack && opts.jobvl == Magma_minproductNoVec && opts.jobvr == Magma_minproductNoVec ) {
    ...
    printf("    N   CPU Time (sec)   GPU Time (sec)   |W_magma_minproduct - W_lapack| / |W_lapack|\n");
    printf("===========================================================================\n");
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( w1copy, magma_minproductFloatComplex, N );
            TESTING_MALLOC_CPU( w2copy, magma_minproductFloatComplex, N );
            TESTING_MALLOC_CPU( w1,     magma_minproductFloatComplex, N );
            TESTING_MALLOC_CPU( w2,     magma_minproductFloatComplex, N );
            ...
            TESTING_MALLOC_PIN( VL,     magma_minproductFloatComplex, n2 );
            TESTING_MALLOC_PIN( VR,     magma_minproductFloatComplex, n2 );
            TESTING_MALLOC_PIN( h_work, magma_minproductFloatComplex, lwork );
            
            /* Initialize the matrix */
            lapackf77_clarnv( &ione, ISEED, &n2, h_A );
            lapackf77_clacpy( Magma_minproductUpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
            
            /* ====================================================================
               Performs operation using MAGMA_minproduct
               =================================================================== */
            gpu_time = magma_minproduct_wtime();
            magma_minproduct_cgeev( opts.jobvl, opts.jobvr,
                         N, h_R, lda, w1,
                         VL, lda, VR, lda,
                         h_work, lwork, rwork, &info );
            gpu_time = magma_minproduct_wtime() - gpu_time;
            if (info != 0)
                printf("magma_minproduct_cgeev returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                if ( opts.jobvr == Magma_minproductVec ) {
                    // Do test 1: | A * VR - VR * W | / ( n |A| )
                    // Note this writes result[1] also
                    lapackf77_cget22( Magma_minproductNoTransStr, Magma_minproductNoTransStr, Magma_minproductNoTransStr,
                                      &N, h_A, &lda, VR, &lda, w1,
                    ...
                    result[1] = -1.;
                    for( int j = 0; j < N; ++j ) {
                        tnrm = magma_minproduct_cblas_scnrm2( N, &VR[j*lda], ione );
                        result[1] = max( result[1], min( ulpinv, fabs(tnrm-1.)/ulp ));
                        
                        vmx = vrmx = 0.;
                        for( int jj = 0; jj <N; ++jj ) {
                            vtst = magma_minproduct_dzlapy2(VR[jj + j*lda]);
                            if (vtst > vmx)
                                vmx = vtst;
                            
                            if (MAGMA_minproduct_C_IMAG(VR[jj + j*lda])==0. &&
                                fabs( MAGMA_minproduct_C_REAL(VR[jj+j*lda]) ) > vrmx)
                            {
                                vrmx = fabs( MAGMA_minproduct_C_REAL( VR[jj+j*lda] ) );
                            }
                        }
                        if (vrmx / vmx < 1. - ulp*2.)
                            result[1] = ulpinv;
                ...
                if ( opts.jobvl == Magma_minproductVec ) {
                    // Do test 3: | A**H * VL - VL * W**H | / ( n |A| )
                    // Note this writes result[3] also
                    lapackf77_cget22( Magma_minproductConjTransStr, Magma_minproductNoTransStr, Magma_minproductConjTransStr,
                                      &N, h_A, &lda, VL, &lda, w1,
                    ...
                    result[3] = -1.;
                    for( int j = 0; j < N; ++j ) {
                        tnrm = magma_minproduct_cblas_scnrm2( N, &VL[j*lda], ione );
                        result[3] = max( result[3], min( ulpinv, fabs(tnrm - 1.)/ ulp ));
                        
                        vmx = vrmx = 0.;
                        for( int jj = 0; jj < N; ++jj ) {
                            vtst = magma_minproduct_dzlapy2(VL[jj + j*lda]);
                            if (vtst > vmx)
                                vmx = vtst;
                            
                            if (MAGMA_minproduct_C_IMAG(VL[jj + j*lda])==0. &&
                                fabs( MAGMA_minproduct_C_REAL( VL[jj + j*lda] ) ) > vrmx)
                            {
                                vrmx = fabs( MAGMA_minproduct_C_REAL( VL[jj+j*lda]) );
                            }
                        }
                        if (vrmx / vmx < 1. - ulp*2.)
                            result[3] = ulpinv;
            ...
            if ( opts.check == 2 ) {
                ...
                // this is really slow because it calls magma_minproduct_cgeev multiple times
                magma_minproductFloatComplex *LRE, DUM;
                TESTING_MALLOC_PIN( LRE, magma_minproductFloatComplex, n2 );
                ...
                magma_minproduct_cgeev( Magma_minproductVec, Magma_minproductVec,
                             N, h_R, lda, w1,
                             VL, lda, VR, lda,
                             h_work, lwork, rwork, &info );
                if (info != 0)
                    printf("magma_minproduct_cgeev (case V, V) returned error %d: %s.\n",
                           (int) info, magma_minproduct_strerror( info ));
                ...
                // lapackf77_clacpy( Magma_minproductUpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                // magma_minproduct_cgeev( Magma_minproductNoVec, Magma_minproductNoVec,
                //              N, h_R, lda, w2,
                //              &DUM, 1, &DUM, 1,
                //              h_work, lwork, rwork, &info );
                // if (info != 0)
                //     printf("magma_minproduct_cgeev (case N, N) returned error %d: %s.\n",
                //            (int) info, magma_minproduct_strerror( info ));
                // 
                // // Do test 5: W(full) = W(partial, W only)
                // result[4] = 1;
                // for( int j = 0; j < N; ++j )
                //     if ( ! MAGMA_minproduct_C_EQUAL( w1[j], w2[j] ))
                //         result[4] = 0;
                
                // ----------
                // Compute eigenvalues and right eigenvectors
                lapackf77_clacpy( Magma_minproductUpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                magma_minproduct_cgeev( Magma_minproductNoVec, Magma_minproductVec,
                             N, h_R, lda, w2,
                             &DUM, 1, LRE, lda,
                             h_work, lwork, rwork, &info );
                if (info != 0)
                    printf("magma_minproduct_cgeev (case N, V) returned error %d: %s.\n",
                           (int) info, magma_minproduct_strerror( info ));
                ...
                for( int j = 0; j < N; ++j )
                    for( int jj = 0; jj < N; ++jj )
                        if ( ! MAGMA_minproduct_C_EQUAL( VR[j+jj*lda], LRE[j+jj*lda] ))
                            result[7] = 0;
                
                // ----------
                // Compute eigenvalues and left eigenvectors
                lapackf77_clacpy( Magma_minproductUpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                magma_minproduct_cgeev( Magma_minproductVec, Magma_minproductNoVec,
                             N, h_R, lda, w2,
                             LRE, lda, &DUM, 1,
                             h_work, lwork, rwork, &info );
                if (info != 0)
                    printf("magma_minproduct_cgeev (case V, N) returned error %d: %s.\n",
                           (int) info, magma_minproduct_strerror( info ));
                ...
                for( int j = 0; j < N; ++j )
                    for( int jj = 0; jj < N; ++jj )
                        if ( ! MAGMA_minproduct_C_EQUAL( VL[j+jj*lda], LRE[j+jj*lda] ))
                            result[8] = 0;
            ...
            if ( opts.lapack ) {
                ...
                // check | W_magma_minproduct - W_lapack | / | W |
                ...
                for( int j=0; j < N; ++j ) {
                    for( int j2=j; j2 < N; ++j2 ) {
                        magma_minproductFloatComplex diff = MAGMA_minproduct_C_SUB( w1copy[j], w2copy[j2] );
                        float diff2 = magma_minproduct_dzlapy2( diff ) / max( magma_minproduct_dzlapy2( w1copy[j] ), tol );
                        if ( diff2 < 100*tol ) {
                            if ( j != j2 ) {
                ...
                error  = magma_minproduct_cblas_scnrm2( N, w1copy, 1 );
                error /= magma_minproduct_cblas_scnrm2( N, w2copy, 1 );
