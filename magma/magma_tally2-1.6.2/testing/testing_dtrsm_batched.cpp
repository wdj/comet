...
int main( int argc, char** argv)
{
    ...
    double *h_A, *h_B, *h_Bcublas, *h_Bmagma_tally2, *h_Blapack, *h_X;
    double *d_A, *d_B;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_Bmagma_tally2,  double, sizeB   );
            ...
            magma_tally2_dmalloc( &dinvA, dinvA_batchSize * batchCount);
            magma_tally2_dmalloc( &dwork, dwork_batchSize * batchCount );
    
            dset_pointer(dwork_array, dwork, lddb, 0, 0, dwork_batchSize, batchCount, opts.queue);
            dset_pointer(dinvA_array, dinvA, magma_tally2_roundup( Ak, TRI_NB ), 0, 0, dinvA_batchSize, batchCount, opts.queue);

            memset(h_Bmagma_tally2, 0, batchCount*ldb*N*sizeof(double));
            magma_tally2blas_dlaset( Magma_tally2Full, lddb, N*batchCount, c_zero, c_zero, dwork, lddb);
            ...
            /* =====================================================================
               ...
               =================================================================== */
            magma_tally2_dsetmatrix( Ak, Ak*batchCount, h_A, lda, d_A, ldda );
            magma_tally2_dsetmatrix( M,  N*batchCount, h_B, ldb, d_B, lddb );
            ...
            magma_tally2_time = magma_tally2_sync_wtime( NULL );
                ...
                magma_tally2_dgetmatrix( M, N*batchCount, dwork, lddb, h_Bmagma_tally2, ldb );
            #else
                ...
                magma_tally2_time = magma_tally2_sync_wtime( NULL ) - magma_tally2_time;
                magma_tally2_perf = gflops / magma_tally2_time;
                magma_tally2_dgetmatrix( M, N*batchCount, d_B, lddb, h_Bmagma_tally2, ldb );
            #endif
            ...
            magma_tally2_dgetmatrix( M, N*batchCount, d_B, lddb, h_Bcublas, ldb );
            ...
            /* =====================================================================
               ...
               =================================================================== */
            // ||b - 1/alpha*A*x|| / (||A||*||x||)
            double alpha2 = MAGMA_tally2_D_DIV( c_one, alpha );
            double normR, normX, normA=0.;
            magma_tally2_error  = 0.0;
            cublas_error = 0.0;

            memcpy( h_X, h_Bmagma_tally2, sizeB*sizeof(double) );
            ...
            for (int s=0; s < batchCount; s++) {
                ...
                normX = lapackf77_dlange( "M", &M, &N, h_Bmagma_tally2 + s * ldb * N, &ldb, work );
