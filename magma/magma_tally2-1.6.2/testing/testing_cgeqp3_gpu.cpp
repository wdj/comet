...
int main( int argc, char** argv)
{
    ...
    magma_tally2FloatComplex *h_A, *h_R, *tau, *h_work;
    magma_tally2FloatComplex_ptr d_A, dtau, d_work;
    magma_tally2_int_t *jpvt;
    magma_tally2_int_t M, N, K, n2, lda, lwork, j, info, min_mn, nb;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lda    = M;
            n2     = lda*N;
            nb     = magma_tally2_get_cgeqp3_nb( min_mn );
            ...
            TESTING_MALLOC_DEV( d_A,    magma_tally2FloatComplex, lda*N  );
            TESTING_MALLOC_DEV( d_work, magma_tally2FloatComplex, lwork  );
            
            /* Initialize the matrix */
            lapackf77_clarnv( &ione, ISEED, &n2, h_R );

            /* Make h_A of rank K */
            magma_tally2FloatComplex alpha = MAGMA_tally2_C_MAKE(  1., 0. );
            magma_tally2FloatComplex beta  = MAGMA_tally2_C_MAKE(  0., 0. );
            ...
            /* copy A to gpu */
            magma_tally2_csetmatrix( M, N, h_R, lda, d_A, lda );

            /* call gpu-interface */
            gpu_time = magma_tally2_wtime();
            #if defined(PRECISION_z) || defined(PRECISION_c)
            magma_tally2_cgeqp3_gpu(M, N, d_A, lda, jpvt, dtau, d_work, lwork, drwork, &info);
            #else
            magma_tally2_cgeqp3_gpu(M, N, d_A, lda, jpvt, dtau, d_work, lwork, &info);
            #endif
            gpu_time = magma_tally2_wtime() - gpu_time;
            
            /* copy outputs to cpu */
            magma_tally2_cgetmatrix( M, N, d_A, lda, h_R, lda );
            magma_tally2_cgetvector( min_mn, dtau, 1, tau, 1 );
            
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally2_cgeqp3 returned error %d: %s.\n",
                       (int) info, magma_tally2_strerror( info ));
