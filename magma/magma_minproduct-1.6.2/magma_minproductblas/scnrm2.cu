...
__global__ void
magma_minproductblas_scnrm2_kernel( int m, magma_minproductFloatComplex *dA, int ldda, float *dxnorm )
{
...
//==============================================================================
__global__ void
magma_minproductblas_scnrm2_check_kernel( int m, magma_minproductFloatComplex *dA, int ldda, float *dxnorm, 
                               float *lsticc )
{
    const int tx = threadIdx.x;
    magma_minproductFloatComplex *dx = dA + blockIdx.x * ldda;
    ...
    magma_minproductFloat_ptr dlsticc) 
{
    ...
    magma_minproductblas_scnrm2_check_kernel<<< blocks, threads >>>( m, dA, ldda, dxnorm, dlsticc );
}


//==============================================================================
__global__ void
magma_minproductblas_scnrm2_smkernel( int m, int n, magma_minproductFloatComplex *dA, int ldda,
                           float *dxnorm )
{
    const int tx = threadIdx.x;
    ...
    for( int k = ty; k < n; k += BLOCK_SIZEy ) {
        magma_minproductFloatComplex *dx = dA + k * ldda;
...
*/
extern "C" void
magma_minproductblas_scnrm2_sm(
    magma_minproduct_int_t m, magma_minproduct_int_t n,
    magma_minproductFloatComplex_ptr dA, magma_minproduct_int_t ldda,
    float *dxnorm)
{
    ...
    magma_minproductblas_scnrm2_smkernel<<< blocks, threads, 0, magma_minproduct_stream >>>( m, n, dA, ldda, dxnorm );
}
...
magma_minproduct_scnrm2_adjust_kernel(float *xnorm, magma_minproductFloatComplex *c)
{
    ...
    temp = MAGMA_minproduct_C_ABS( c[tx] ) / xnorm[0];
    sum[tx] = -temp * temp;
...
*/
extern "C" void
magma_minproductblas_scnrm2_adjust(magma_minproduct_int_t k, magma_minproductFloat_ptr dxnorm, magma_minproductFloatComplex_ptr dc)
{
    magma_minproduct_scnrm2_adjust_kernel<<< 1, k, 0, magma_minproduct_stream >>> (dxnorm, dc);
}
...
__global__ void
magma_minproduct_scnrm2_row_check_adjust_kernel(
    int n, float tol, float *xnorm, float *xnorm2, 
    magma_minproductFloatComplex *C, int ldc, float *lsticc)
{
    const int tx = threadIdx.x + blockIdx.x*BS;
    lsticc[tx+1] = 0;

    if (tx < n) {
        float temp = MAGMA_minproduct_C_ABS( C[tx*ldc] ) / xnorm[tx];
    ...
    magma_minproductFloat_ptr dlsticc)
{
    ...
    magma_minproduct_scnrm2_row_check_adjust_kernel<<< nblocks, BS >>> (k, tol, dxnorm, dxnorm2, dC, lddc, dlsticc);
}
    ...
    magma_minproductFloat_ptr dxnorm) 
{
    ...
    magma_minproductblas_scnrm2_kernel<<< blocks, threads, 0, magma_minproduct_stream >>>( m, dA, ldda, dxnorm );

    // The following would do the computation on one SM
    // magma_minproductblas_scnrm2_sm(m, n, dA, ldda, dxnorm);
}
