...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( opts.ngpu == 0 ) {
                magma_tally2_dsytrd(opts.uplo, N, h_R, lda, diag, offdiag,
                             tau, h_work, lwork, &info);
            } else {
                magma_tally2_dsytrd_mgpu(opts.ngpu, k, opts.uplo, N, h_R, lda, diag, offdiag,
                                  tau, h_work, lwork, &info);
            }
            gpu_time = magma_tally2_wtime() - gpu_time;
            if ( info != 0 ) {
                printf("magma_tally2_dsytrd_mgpu returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                lapackf77_dlacpy( lapack_uplo_const_tally2(opts.uplo), &N, &N, h_R, &lda, h_Q, &lda);
