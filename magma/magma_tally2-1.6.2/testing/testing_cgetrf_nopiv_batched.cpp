...
float get_LU_error(magma_tally2_int_t M, magma_tally2_int_t N,
                    magma_tally2FloatComplex *A,  magma_tally2_int_t lda,
                    magma_tally2FloatComplex *LU, magma_tally2_int_t *IPIV)
{
    magma_tally2_int_t min_mn = min(M,N);
    magma_tally2_int_t ione   = 1;
    magma_tally2_int_t i, j;
    magma_tally2FloatComplex alpha = MAGMA_tally2_C_ONE;
    magma_tally2FloatComplex beta  = MAGMA_tally2_C_ZERO;
    magma_tally2FloatComplex *L, *U;
    float work[1], matnorm, residual;
    
    TESTING_MALLOC_CPU( L, magma_tally2FloatComplex, M*min_mn);
    TESTING_MALLOC_CPU( U, magma_tally2FloatComplex, min_mn*N);
    memset( L, 0, M*min_mn*sizeof(magma_tally2FloatComplex) );
    memset( U, 0, min_mn*N*sizeof(magma_tally2FloatComplex) );

    lapackf77_claswp( &N, A, &lda, &ione, &min_mn, IPIV, &ione);
    lapackf77_clacpy( Magma_tally2LowerStr, &M, &min_mn, LU, &lda, L, &M      );
    lapackf77_clacpy( Magma_tally2UpperStr, &min_mn, &N, LU, &lda, U, &min_mn );

    for(j=0; j<min_mn; j++)
        L[j+j*M] = MAGMA_tally2_C_MAKE( 1., 0. );
    ...
    for( j = 0; j < N; j++ ) {
        for( i = 0; i < M; i++ ) {
            LU[i+j*lda] = MAGMA_tally2_C_SUB( LU[i+j*lda], A[i+j*lda] );
        }
    }
...
int main( int argc, char** argv)
{
    ...
    real_Double_t   gflops, magma_tally2_perf, magma_tally2_time, cublas_perf=0., cublas_time=0., cpu_perf=0, cpu_time=0;
    float          error; 
    magma_tally2_int_t cublas_enable = 0;
    magma_tally2FloatComplex *h_A, *h_R;
    magma_tally2FloatComplex *dA_magma_tally2;
    ...
    for( int i = 0; i < opts.ntest; ++i ) {
      ...
      for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_DEV(  dA_magma_tally2,  magma_tally2FloatComplex, ldda*N * batchCount);
            ...
            magma_tally2_csetmatrix( M, columns, h_R, lda, dA_magma_tally2, ldda );
            ...
            /* ====================================================================
               ...
               =================================================================== */
            cset_pointer(dA_array, dA_magma_tally2, ldda, 0, 0, ldda*N, batchCount, queue);
            magma_tally2_time = magma_tally2_sync_wtime(0);
            info = magma_tally2_cgetrf_nopiv_batched( M, N, dA_array, ldda, dinfo_magma_tally2, batchCount, queue);
            ...
            {
                if(cpu_info[i] != 0 ){
                    printf("magma_tally2_cgetrf_batched matrix %d returned internal error %d\n",i, (int)cpu_info[i] );
                }
            }
            if (info != 0)
                printf("magma_tally2_cgetrf_batched returned argument error %d: %s.\n", (int) info, magma_tally2_strerror( info ));
            ...
            if ( opts.check ) {
                ...
                magma_tally2_cgetmatrix( M, N*batchCount, dA_magma_tally2, ldda, h_A, lda );
                for(int i=0; i<batchCount; i++)
                {
            ...
            TESTING_FREE_DEV( dA_magma_tally2 );
