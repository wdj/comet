...
int main( int argc, char** argv)
{
    ...
    double          magma_minproduct_error, magma_minproduct_err, Ynorm, work[1];
    ...
    magma_minproductDoubleComplex *h_A, *h_X, *h_Y, *h_Ymagma_minproduct;
    magma_minproductDoubleComplex *d_A, *d_X, *d_Y;
    ...
    magma_minproductDoubleComplex **X_array = NULL;
    magma_minproductDoubleComplex **Y_array = NULL;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_X,  magma_minproductDoubleComplex, sizeX );
            TESTING_MALLOC_CPU( h_Y,  magma_minproductDoubleComplex, sizeY  );
            TESTING_MALLOC_CPU( h_Ymagma_minproduct,  magma_minproductDoubleComplex, sizeY  );


            TESTING_MALLOC_DEV( d_A, magma_minproductDoubleComplex, ldda*N*batchCount );
            TESTING_MALLOC_DEV( d_X, magma_minproductDoubleComplex, sizeX );
            TESTING_MALLOC_DEV( d_Y, magma_minproductDoubleComplex, sizeY );
            ...
            /* =====================================================================
               ...
               =================================================================== */
            magma_minproduct_zsetmatrix( M, N*batchCount, h_A, lda, d_A, ldda );
            magma_minproduct_zsetvector( Xm*batchCount, h_X, incx, d_X, incx );
            magma_minproduct_zsetvector( Ym*batchCount, h_Y, incy, d_Y, incy );
            
            zset_pointer(A_array, d_A, ldda, 0, 0, ldda*N, batchCount, magma_minproduct_stream);
            zset_pointer(X_array, d_X, 1, 0, 0, incx*Xm, batchCount, magma_minproduct_stream);
            zset_pointer(Y_array, d_Y, 1, 0, 0, incy*Ym, batchCount, magma_minproduct_stream);

            magma_minproduct_time = magma_minproduct_sync_wtime( NULL );
            magma_minproductblas_zgemv_batched(opts.transA, M, N,
                             alpha, A_array, ldda,
                                    X_array, incx,
            ...
            magma_minproduct_zgetvector( Ym*batchCount, d_Y, incy, h_Ymagma_minproduct, incy );
            ...
            if ( opts.lapack ) {
                ...
                for(int s=0; s<batchCount; s++)
                {
                    ...
                    blasf77_zaxpy( &Ym, &c_neg_one, h_Y + s*Ym, &ione, h_Ymagma_minproduct + s*Ym, &ione );
                    magma_minproduct_err = lapackf77_zlange( "M", &M, &ione, h_Ymagma_minproduct + s*Ym, &incy, work ) / Ynorm; 
