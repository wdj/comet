...
/**
    ...
    @ingroup magma_tally4_cgesv_tile
    ********************************************************************/
extern "C" magma_tally4_int_t
magma_tally4_ctstrf_gpu(
    magma_tally4_order_t order, magma_tally4_int_t m, magma_tally4_int_t n,
    magma_tally4_int_t ib, magma_tally4_int_t nb,
...
#define hL2(i)  (hL2 + (i)*ib*ldhl          )
    ...
    magma_tally4FloatComplex_ptr dAT, dUT;
    magma_tally4FloatComplex_ptr dAp, dUp;
#ifndef WITHOUTTRTRI
    magma_tally4FloatComplex_ptr dL2 = dL + ib;
    magma_tally4FloatComplex *hL2 = hL + ib;
    p = 2;
#endif
    ...
    *info = 0;
    ...
    if ( ib >= mindim ) {
        ...
        if ( order == Magma_tally4RowMajor ) {
            magma_tally4_csetmatrix( m, n, hU, ldhu, dwork, lddwork );
            magma_tally4blas_ctranspose( m, n, dwork, lddwork, dU, lddu );

            magma_tally4_csetmatrix( m, n, hA, ldha, dwork, lddwork );
            magma_tally4blas_ctranspose( m, n, dwork, lddwork, dA, ldda );
        } else {
            magma_tally4_csetmatrix( m, n, hU, ldhu, dU, lddu );
            magma_tally4_csetmatrix( m, n, hA, ldha, dA, ldda );
        }
        magma_tally4_csetmatrix( p*ib, n, hL, ldhl, dL, lddl );
        ...
        if ( order == Magma_tally4ColMajor ) {
            magma_tally4blas_cgetmo_in( dU, dUT, lddu, m,  n );
            magma_tally4blas_cgetmo_in( dA, dAT, ldda, m,  n );
        } else {
            dUT = dU; dAT = dA;
        }
        dAp = dwork;
        ...
        for( i=0; i < s; i++ ) {
            ...
            if ( i > 0 ) {
                // download i-th panel
                magma_tally4blas_ctranspose( sb, ii, UT(0,i), lddu, dUp, lddu );
                magma_tally4blas_ctranspose( sb, m,  AT(0,i), ldda, dAp, ldda );
                
                magma_tally4_cgetmatrix( ii, sb, dUp, lddu, hU(0, i), ldhu );
                magma_tally4_cgetmatrix( m,  sb, dAp, ldda, hA(0, i), ldha );
                
                // make sure that gpu queue is empty
                //magma_tally4_device_sync();
                
#ifndef WITHOUTTRTRI
                magma_tally4_ctrmm( Magma_tally4Right, Magma_tally4Lower, Magma_tally4Trans, Magma_tally4Unit,
                             n-(ii+sb), ib,
                             c_one, L2(i-1),      lddl,
                                    UT(i-1, i+1), lddu);
#else
                magma_tally4_ctrsm( Magma_tally4Right, Magma_tally4Lower, Magma_tally4Trans, Magma_tally4Unit,
                             n-(ii+sb), ib,
                             c_one, L(i-1),       lddl,
                                    UT(i-1, i+1), lddu);
#endif
...
#ifndef NOSWAPBLK
            magma_tally4blas_cswapblk( Magma_tally4RowMajor, n-(ii+sb),
                                UT(i, i+1), lddu,
                                AT(0, i+1), ldda,
                                1, sb, ipiv+ii, 1, nb );
            ...
            for (j=0; j < ib; j++) {
                ...
                if ( im != (j) ) {
                    im = im - nb;
                    assert( (im >= 0) && (im < m) );
                    magma_tally4blas_cswap( n-(ii+sb), UT(i, i+1)+j*lddu, 1, AT(0, i+1)+im*ldda, 1 );
                } else {
                    ipiv[ip] += ii;
                }
                ip++;
...
#endif
            // upload i-th panel
            magma_tally4_csetmatrix( sb, sb, hU(i, i), ldhu, dUp, lddu );
            magma_tally4_csetmatrix( m, sb, hA(0, i), ldha, dAp, ldda );
            magma_tally4_csetmatrix( p*ib, sb, hL(i), ldhl, L(i), lddl );
            magma_tally4blas_ctranspose( sb, sb, dUp, lddu, UT(i,i), lddu );
            magma_tally4blas_ctranspose( m,  sb, dAp, ldda, AT(0,i), ldda );
        ...
        if ( order == Magma_tally4ColMajor ) {
            magma_tally4blas_cgetmo_out( dU, dUT, lddu, m,  n );
            magma_tally4blas_cgetmo_out( dA, dAT, ldda, m,  n );
        }
    }
    return *info;
}

#endif /* MAGMA_tally4_WITH_PLASMA */
