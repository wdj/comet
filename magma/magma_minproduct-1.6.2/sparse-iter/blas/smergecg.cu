...
__global__ void
magma_minproduct_scgreduce_kernel_spmv1( 
    int Gs,
    int n, 
    float * vtmp,
    float * vtmp2 )
{
    ...
    temp[Idx] = MAGMA_minproduct_S_MAKE( 0.0, 0.0);
    int i = blockIdx.x * ( blockSize * 2 ) + Idx;   
    while (i < Gs ) {
        temp[ Idx  ] += vtmp[ i ]; 
        temp[ Idx  ] += ( i + blockSize < Gs ) ? vtmp[ i + blockSize ] 
                                                : MAGMA_minproduct_S_MAKE( 0.0, 0.0); 
        i += gridSize;
    }
    __syncthreads();
    if ( Idx < 64 ){
...
__global__ void
magma_minproduct_scgmerge_spmvcsr_kernel(  
    int n,
    float * dval, 
    ...
    float * vtmp )
{
    ...
    if( i<n ){
        float dot = MAGMA_minproduct_S_ZERO;
        int start = drowptr[ i ];
        int end = drowptr[ i+1 ];
        for( j=start; j<end; j++)
...
__global__ void
magma_minproduct_scgmerge_spmvell_kernel(  
    int n,
    ...
    float * vtmp )
{
    ...
    if(i < n ){
        float dot = MAGMA_minproduct_S_MAKE(0.0, 0.0);
...
__global__ void
magma_minproduct_scgmerge_spmvellpack_kernel(  
    int n,
    int num_cols_per_row,
    float * dval, 
    magma_minproduct_index_t * dcolind,
...
__global__ void
magma_minproduct_scgmerge_spmvellpackrt_kernel_8(  
    int n,
    float * dval, 
    magma_minproduct_index_t * dcolind,
...
// computes the SpMV using ELLRT 8 threads per row
__global__ void
magma_minproduct_scgmerge_spmvellpackrt_kernel_16(  
    int n,
    float * dval, 
    magma_minproduct_index_t * dcolind,
    magma_minproduct_index_t * drowlength,
...
// computes the SpMV using ELLRT 8 threads per row
__global__ void
magma_minproduct_scgmerge_spmvellpackrt_kernel_32(  
    int n,
    float * dval, 
    magma_minproduct_index_t * dcolind,
    magma_minproduct_index_t * drowlength,
...
__global__ void
magma_minproduct_scgmerge_spmvellpackrt_kernel2(  
    int n,
    float * z,
    float * d,
    float * vtmp2 )
{
    ...
    temp[ Idx ] = ( i < n ) ? z[i]*d[i] : MAGMA_minproduct_S_MAKE(0.0, 0.0);
    __syncthreads();
    if ( Idx < 128 ){
        temp[ Idx ] += temp[ Idx + 128 ];
    }
...
__global__ void
magma_minproduct_scgmerge_spmvsellc_kernel(   
    int num_rows, 
    int blocksize,
    float * dval, 
    magma_minproduct_index_t * dcolind,
...
__global__ void
magma_minproduct_scgmerge_spmvsellpt_kernel_8( 
    int num_rows, 
    int blocksize,
    int T,
    float * dval, 
...
// SELLC SpMV kernel modified assigning multiple threads to each row - 2D kernel
__global__ void
magma_minproduct_scgmerge_spmvsellpt_kernel_16( 
    int num_rows, 
    int blocksize,
    int T,
    float * dval, 
...
// SELLC SpMV kernel modified assigning multiple threads to each row - 2D kernel
__global__ void
magma_minproduct_scgmerge_spmvsellpt_kernel_32( 
    int num_rows, 
    int blocksize,
    int T,
    float * dval, 
...
magma_minproduct_scg_rhokernel(  
    float * skp ){
...
/**
    ...
    @param[in]
    d1          magma_minproductFloat_ptr 
                temporary vector

    @param[in]
    d2          magma_minproductFloat_ptr 
                temporary vector

    @param[in]
    dd          magma_minproductFloat_ptr 
                input vector d

    @param[out]
    dz          magma_minproductFloat_ptr 
                input vector z
    ...
    magma_minproduct_queue_t queue )
{
    ...
    if ( A.storage_type == Magma_minproduct_CSR )
        ...
        magma_minproduct_scgmerge_spmvellpackrt_kernel2<<<Gs, Bs, Ms, queue >>>
                              ( A.num_rows, dz, dd, d1 );
    ...
    while( Gs.x > 1 ) {
        ...
        magma_minproduct_scgreduce_kernel_spmv1<<< Gs_next.x/2, Bs.x/2, Ms/2 >>> 
    ...
    magma_minproduct_scopyvector( 1, aux1, 1, skp+4, 1 );
    dim3 Bs2( 2 );
    dim3 Gs2( 1 );
    magma_minproduct_scg_rhokernel<<<Gs2, Bs2, 0>>>( skp );
...
__global__ void
magma_minproduct_scgmerge_xrbeta_kernel(  
    int n, 
    float * x, 
    float * r,
    float * d,
    ...
    float * vtmp )
{
    ...
    float rho = skp[3];
    float mrho = MAGMA_minproduct_S_MAKE( -1.0, 0.0)*rho;
...
magma_minproduct_scg_alphabetakernel(  
    float * skp )
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if( i==0 ){
        float tmp1 = skp[1];
        skp[0] =  tmp1/skp[2];
        //printf("beta=%e\n", MAGMA_minproduct_S_REAL(tmp1));
    }
}

// update search Krylov vector d
__global__ void
magma_minproduct_scg_d_kernel(  
    int n, 
    float * skp,
    float * r,
    float * d )
...
/**
    ...
    @ingroup magma_minproductsparse_ssygpuk
    ********************************************************************/
    ...
    magma_minproduct_queue_t queue )
{
    ...
    magma_minproductFloat_ptr aux1 = d1, aux2 = d2;
    int b = 1;        
    magma_minproduct_scgmerge_xrbeta_kernel<<<Gs, Bs, Ms>>>
    ...
    magma_minproduct_scopyvector( 1, aux1, 1, skp+1, 1 );
    dim3 Bs2( 2 );
    dim3 Gs2( 1 );
    magma_minproduct_scg_alphabetakernel<<<Gs2, Bs2, 0>>>( skp );

    dim3 Bs3( local_block_size );
    dim3 Gs3( magma_minproduct_ceildiv( n, local_block_size ) );
    magma_minproduct_scg_d_kernel<<<Gs3, Bs3, 0>>>( n, skp, dr, dd );  
