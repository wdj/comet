...
extern "C"
void magma_tally3_assert( bool condition, const char* msg, ... )
{
    if ( ! condition ) {
...
extern "C"
void magma_tally3_assert_warn( bool condition, const char* msg, ... )
{
    if ( ! condition ) {
        printf( "Assert failed: " );
        va_list va;
...
"  --ngpu x         Number of GPUs, default 1. Also set with $MAGMA_tally3_NUM_GPUS.\n"
...
"  --fraction_dcpu x Percentage of the workload to schedule on the cpu. Used in magma_tally3_amc algorithms only, default 0.\n"
...
extern "C"
void parse_opts( int argc, char** argv, magma_tally3_opts *opts )
{
    ...
    opts->nrhs     = 1;
    opts->nstream  = 1;
    opts->ngpu     = magma_tally3_num_gpus();
    opts->niter    = 1;
    opts->nthread  = 1;
    opts->offset   = 0;
    opts->itype    = 1;
    ...
    opts->check     = (getenv("MAGMA_tally3_TESTINGS_CHECK") != NULL);
    opts->lapack    = (getenv("MAGMA_tally3_RUN_LAPACK")     != NULL);
    opts->warmup    = (getenv("MAGMA_tally3_WARMUP")         != NULL);
    opts->all       = (getenv("MAGMA_tally3_RUN_ALL")        != NULL);
    opts->verbose   = false;
    
    opts->uplo      = Magma_tally3Lower;      // potrf, etc.
    opts->transA    = Magma_tally3NoTrans;    // gemm, etc.
    opts->transB    = Magma_tally3NoTrans;    // gemm
    opts->side      = Magma_tally3Left;       // trsm, etc.
    opts->diag      = Magma_tally3NonUnit;    // trsm, etc.
    opts->jobu      = Magma_tally3NoVec;  // gesvd: no left  singular vectors
    opts->jobvt     = Magma_tally3NoVec;  // gesvd: no right singular vectors
    opts->jobz      = Magma_tally3NoVec;  // heev:  no eigen vectors
    opts->jobvr     = Magma_tally3NoVec;  // geev:  no right eigen vectors
    opts->jobvl     = Magma_tally3NoVec;  // geev:  no left  eigen vectors
    ...
    for( int i = 1; i < argc; ++i ) {
        ...
        if ( strcmp("-N", argv[i]) == 0 && i+1 < argc ) {
            magma_tally3_assert( ntest < MAX_NTEST, "error: -N %s, max number of tests exceeded, ntest=%d.\n",
                          argv[i], ntest );
            i++;
            int m2, n2, k2;
        ...
        // ----- scalar arguments
            ...
            magma_tally3_assert( opts->tolerance >= 0 && opts->tolerance <= 1000,
            ...
            magma_tally3_assert( opts->panel_nthread > 0,
            ...
            magma_tally3_assert( opts->fraction_dcpu > 0 && opts->fraction_dcpu<=1,
            ...
            magma_tally3_assert( opts->batchcount > 0,
        ...
        else if ( strcmp("-L",  argv[i]) == 0 ) { opts->uplo = Magma_tally3Lower; }
        else if ( strcmp("-U",  argv[i]) == 0 ) { opts->uplo = Magma_tally3Upper; }
        else if ( strcmp("-F",  argv[i]) == 0 ) { opts->uplo = Magma_tally3UpperLower; }
        
        else if ( strcmp("-NN", argv[i]) == 0 ) { opts->transA = Magma_tally3NoTrans;   opts->transB = Magma_tally3NoTrans;   }
        else if ( strcmp("-NT", argv[i]) == 0 ) { opts->transA = Magma_tally3NoTrans;   opts->transB = Magma_tally3Trans;     }
        else if ( strcmp("-NC", argv[i]) == 0 ) { opts->transA = Magma_tally3NoTrans;   opts->transB = Magma_tally3ConjTrans; }
        else if ( strcmp("-TN", argv[i]) == 0 ) { opts->transA = Magma_tally3Trans;     opts->transB = Magma_tally3NoTrans;   }
        else if ( strcmp("-TT", argv[i]) == 0 ) { opts->transA = Magma_tally3Trans;     opts->transB = Magma_tally3Trans;     }
        else if ( strcmp("-TC", argv[i]) == 0 ) { opts->transA = Magma_tally3Trans;     opts->transB = Magma_tally3ConjTrans; }
        else if ( strcmp("-CN", argv[i]) == 0 ) { opts->transA = Magma_tally3ConjTrans; opts->transB = Magma_tally3NoTrans;   }
        else if ( strcmp("-CT", argv[i]) == 0 ) { opts->transA = Magma_tally3ConjTrans; opts->transB = Magma_tally3Trans;     }
        else if ( strcmp("-CC", argv[i]) == 0 ) { opts->transA = Magma_tally3ConjTrans; opts->transB = Magma_tally3ConjTrans; }
        else if ( strcmp("-T",  argv[i]) == 0 ) { opts->transA = Magma_tally3Trans;     }
        else if ( strcmp("-C",  argv[i]) == 0 ) { opts->transA = Magma_tally3ConjTrans; }
        
        else if ( strcmp("-SL", argv[i]) == 0 ) { opts->side  = Magma_tally3Left;  }
        else if ( strcmp("-SR", argv[i]) == 0 ) { opts->side  = Magma_tally3Right; }
        
        else if ( strcmp("-DN", argv[i]) == 0 ) { opts->diag  = Magma_tally3NonUnit; }
        else if ( strcmp("-DU", argv[i]) == 0 ) { opts->diag  = Magma_tally3Unit;    }
        
        else if ( strcmp("-UA", argv[i]) == 0 ) { opts->jobu  = Magma_tally3AllVec;       }
        else if ( strcmp("-US", argv[i]) == 0 ) { opts->jobu  = Magma_tally3SomeVec;      }
        else if ( strcmp("-UO", argv[i]) == 0 ) { opts->jobu  = Magma_tally3OverwriteVec; }
        else if ( strcmp("-UN", argv[i]) == 0 ) { opts->jobu  = Magma_tally3NoVec;        }
        
        else if ( strcmp("-VA", argv[i]) == 0 ) { opts->jobvt = Magma_tally3AllVec;       }
        else if ( strcmp("-VS", argv[i]) == 0 ) { opts->jobvt = Magma_tally3SomeVec;      }
        else if ( strcmp("-VO", argv[i]) == 0 ) { opts->jobvt = Magma_tally3OverwriteVec; }
        else if ( strcmp("-VN", argv[i]) == 0 ) { opts->jobvt = Magma_tally3NoVec;        }
        
        else if ( strcmp("-JN", argv[i]) == 0 ) { opts->jobz  = Magma_tally3NoVec; }
        else if ( strcmp("-JV", argv[i]) == 0 ) { opts->jobz  = Magma_tally3Vec;   }
        
        else if ( strcmp("-LN", argv[i]) == 0 ) { opts->jobvl = Magma_tally3NoVec; }
        else if ( strcmp("-LV", argv[i]) == 0 ) { opts->jobvl = Magma_tally3Vec;   }
        
        else if ( strcmp("-RN", argv[i]) == 0 ) { opts->jobvr = Magma_tally3NoVec; }
        else if ( strcmp("-RV", argv[i]) == 0 ) { opts->jobvr = Magma_tally3Vec;   }
    ...
    if ( opts->jobu == Magma_tally3OverwriteVec && opts->jobvt == Magma_tally3OverwriteVec ) {
    ...
    magma_tally3_int_t num;
    magma_tally3_device_t devices[ Magma_tally3MaxGPUs ];
    magma_tally3_getdevices( devices, Magma_tally3MaxGPUs, &num );
    
    // 2 queues + 1 extra NULL entry to catch errors
    magma_tally3_queue_create( /*devices[ opts->device ],*/ &opts->queues2[ 0 ] );
    magma_tally3_queue_create( /*devices[ opts->device ],*/ &opts->queues2[ 1 ] );
    opts->queues2[ 2 ] = NULL;
