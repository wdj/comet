...
int main(int argc, char **argv)
{
    ...
    double          *h_workd;
    magma_tally4_int_t *h_ipiv, *d_ipiv;
    magma_tally4_int_t lda, ldb, ldx;
    magma_tally4_int_t ldda, lddb, lddx;
    magma_tally4_int_t N, nrhs, gesv_iter, info, size;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_ipiv,  magma_tally4_int_t,        N        );
            ...
            TESTING_MALLOC_DEV( d_ipiv,  magma_tally4_int_t,        N             );
            TESTING_MALLOC_DEV( d_WORKS, magma_tally4FloatComplex,  ldda*(N+nrhs) );
            ...
            lapackf77_zlacpy( Magma_tally4UpperLowerStr, &N, &nrhs, h_B, &ldb, h_X, &ldx);
            ...
            magma_tally4_zcgesv_gpu( opts.transA, N, nrhs,
            ...
            if (info != 0)
                printf("magma_tally4_zcgesv returned error %d: %s.\n",
                       (int) info, magma_tally4_strerror( info ));
            
            //=====================================================================
            //              ERROR DP vs MIXED  - GPU
            //=====================================================================
            magma_tally4_zgetmatrix( N, nrhs, d_X, lddx, h_X, ldx );
            
            Anorm = lapackf77_zlange("I", &N, &N, h_A, &lda, h_workd);
            blasf77_zgemm( lapack_trans_const_tally4(opts.transA), Magma_tally4NoTransStr,
                           &N, &nrhs, &N,
                           &c_one,     h_A, &lda,
                                       h_X, &ldx,
                           &c_neg_one, h_B, &ldb);
            ...
            gpu_time = magma_tally4_wtime();
            magma_tally4_zgetrf_gpu(N, N, d_A, ldda, h_ipiv, &info);
            gpu_time = magma_tally4_wtime() - gpu_time;
            gpu_perfdf = gflopsF / gpu_time;
            if (info != 0)
                printf("magma_tally4_zgetrf returned error %d: %s.\n",
                       (int) info, magma_tally4_strerror( info ));
            ...
            gpu_time = magma_tally4_wtime();
            magma_tally4_zgetrf_gpu(N, N, d_A, ldda, h_ipiv, &info);
            magma_tally4_zgetrs_gpu( opts.transA, N, nrhs, d_A, ldda, h_ipiv, d_B, lddb, &info );
            gpu_time = magma_tally4_wtime() - gpu_time;
            gpu_perfds = gflopsS / gpu_time;
            if (info != 0)
                printf("magma_tally4_zgetrs returned error %d: %s.\n",
                       (int) info, magma_tally4_strerror( info ));
            ...
            magma_tally4_zsetmatrix( N, N,    h_A, lda,  d_A,  ldda );
            magma_tally4_zsetmatrix( N, nrhs, h_B, ldb,  d_B,  lddb );
            magma_tally4blas_zlag2c( N, N,    d_A, ldda, d_As, ldda, &info );
            magma_tally4blas_zlag2c( N, nrhs, d_B, lddb, d_Bs, lddb, &info );
            
            gpu_time = magma_tally4_wtime();
            magma_tally4_cgetrf_gpu(N, N, d_As, ldda, h_ipiv, &info);
            gpu_time = magma_tally4_wtime() - gpu_time;
            gpu_perfsf = gflopsF / gpu_time;
            if (info != 0)
                printf("magma_tally4_cgetrf returned error %d: %s.\n",
                       (int) info, magma_tally4_strerror( info ));
            ...
            gpu_time = magma_tally4_wtime();
            magma_tally4_cgetrf_gpu( N, N,    d_As, ldda, h_ipiv, &info);
            magma_tally4_cgetrs_gpu( opts.transA, N, nrhs, d_As, ldda, h_ipiv,
                              d_Bs, lddb, &info);
            gpu_time = magma_tally4_wtime() - gpu_time;
            gpu_perfss = gflopsS / gpu_time;
            if (info != 0)
                printf("magma_tally4_cgetrs returned error %d: %s.\n",
                       (int) info, magma_tally4_strerror( info ));
