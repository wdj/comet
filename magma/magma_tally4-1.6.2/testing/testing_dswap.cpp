...
static int diff_matrix( magma_tally4_int_t m, magma_tally4_int_t n, double *A, magma_tally4_int_t lda, double *B, magma_tally4_int_t ldb )
{
    for( magma_tally4_int_t j = 0; j < n; j++ ) {
        for( magma_tally4_int_t i = 0; i < m; i++ ) {
            if ( ! MAGMA_tally4_D_EQUAL( A[lda*j+i], B[ldb*j+i] ) )
                return 1;
        }
    }
    return 0;
...
static void init_matrix( magma_tally4_int_t m, magma_tally4_int_t n, double *A, magma_tally4_int_t lda, magma_tally4_int_t offset )
{
    assert( lda >= m );
    for( magma_tally4_int_t j = 0; j < n; ++j ) {
        for( magma_tally4_int_t i=0; i < m; ++i ) {
            A[i + j*lda] = MAGMA_tally4_D_MAKE( offset + (i+1) + (j+1)/10000., 0 );
        }
    }
}
...
int main( int argc, char** argv)
{
    ...
    double *h_A1, *h_A2;
    double *h_R1, *h_R2;
    magma_tally4Double_ptr d_A1, d_A2;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lda    = N;
            ldda   = ((N+31)/32)*32;
            nb     = (opts.nb > 0 ? opts.nb : magma_tally4_get_dgetrf_nb( N ));
            nb     = min( N, nb );
            ...
            magma_tally4_dsetmatrix( N, N, h_A1, lda, d_A1, ldda );
            magma_tally4_dsetmatrix( N, N, h_A2, lda, d_A2, ldda );
            ...
            for( j=0; j < nb; j++) {
                if ( j != (ipiv[j]-1)) {
                    ...
                    #else
                        magma_tally4_dswap( N, d_A1, ldda*j, 1, d_A2, ldda*(ipiv[j]-1), 1, opts.queue );
                    #endif
                }
            }
            ...
            magma_tally4_dgetmatrix( N, N, d_A1, ldda, h_R1, lda );
            magma_tally4_dgetmatrix( N, N, d_A2, ldda, h_R2, lda );
            ...
            for( j=0; j < nb; j++) {
                if ( j != (ipiv[j]-1)) {
                    ...
                    #else
                        magma_tally4_dswap( N, d_A1, j, ldda, d_A2, ipiv[j]-1, ldda, opts.queue );
                    #endif
                }
            }
            time = magma_tally4_sync_wtime( queue ) - time;
            ...
            for( j=0; j < nb; j++) {
                if ( j != (ipiv[j]-1)) {
                    magma_tally4blas_dswap( N, d_A1+ldda*j, 1, d_A2+ldda*(ipiv[j]-1), 1);
                }
            }
            time = magma_tally4_sync_wtime( queue ) - time;
            ...
            for( j=0; j < nb; j++) {
                if ( j != (ipiv[j]-1)) {
                    magma_tally4blas_dswap( N, d_A1+j, ldda, d_A2+ipiv[j]-1, ldda );
                }
            }
            time = magma_tally4_sync_wtime( queue ) - time;
            ...
            time = magma_tally4_sync_wtime( queue );
            magma_tally4blas_dswapblk( Magma_tally4RowMajor, N, d_A1, ldda, d_A2, ldda, 1, nb, ipiv, 1, 0);
            time = magma_tally4_sync_wtime( queue ) - time;
            ...
            time = magma_tally4_sync_wtime( queue );
            magma_tally4blas_dswapblk( Magma_tally4ColMajor, N, d_A1, ldda, d_A2, ldda, 1, nb, ipiv, 1, 0);
            time = magma_tally4_sync_wtime( queue ) - time;
            ...
            time = magma_tally4_sync_wtime( queue );
            magma_tally4blas_dlaswp( N, d_A1, ldda, 1, nb, ipiv, 1);
            time = magma_tally4_sync_wtime( queue ) - time;
            ...
            magma_tally4blas_dlaswp2( N, d_A1, ldda, 1, nb, d_ipiv, 1 );
            time = magma_tally4_sync_wtime( queue ) - time;
            ...
            time = magma_tally4_sync_wtime( queue );
            magma_tally4blas_dlaswpx( N, d_A1, ldda, 1, 1, nb, ipiv, 1);
            time = magma_tally4_sync_wtime( queue ) - time;
            ...
            time = magma_tally4_sync_wtime( queue );
            magma_tally4blas_dlaswpx( N, d_A1, 1, ldda, 1, nb, ipiv, 1);
            time = magma_tally4_sync_wtime( queue ) - time;
            ...
            time = magma_tally4_sync_wtime( queue );
            magma_tally4_dcopymatrix( N, nb, d_A1, ldda, d_A2, ldda );
            time = magma_tally4_sync_wtime( queue ) - time;
            // copy reads 1 matrix and writes 1 matrix, so has half gbytes of swap
            col_perf6 = 0.5 * gbytes / time;
            
            time = magma_tally4_sync_wtime( queue );
            magma_tally4_dcopymatrix( nb, N, d_A1, ldda, d_A2, ldda );
            time = magma_tally4_sync_wtime( queue ) - time;
            // copy reads 1 matrix and writes 1 matrix, so has half gbytes of swap
