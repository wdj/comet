...
struct event_log
{
    ...
    int           ngpu;
    int           nqueue;
    magma_tally3_queue_t queues   [ MAX_GPU_QUEUES ];
    int           gpu_id   [ MAX_GPU_QUEUES ];
    magma_tally3_event_t gpu_first[ MAX_GPU_QUEUES ];
#if TRACE_METHOD == 2
    double        gpu_start[ MAX_GPU_QUEUES ][ MAX_EVENTS ];
#else
    magma_tally3_event_t gpu_start[ MAX_GPU_QUEUES ][ MAX_EVENTS ];
#endif
    magma_tally3_event_t gpu_end  [ MAX_GPU_QUEUES ][ MAX_EVENTS ];
...
// ----------------------------------------
void trace_init( int ncore, int ngpu, int nqueue, magma_tally3_queue_t* queues )
{
    if ( ncore > MAX_CORES ) {
    ...
    glog.cpu_first = magma_tally3_wtime();
}
...
void trace_cpu_start( int core, const char* tag, const char* lbl )
{
    ...
    glog.cpu_start[core][id] = magma_tally3_wtime();
    magma_tally3_strlcpy( glog.cpu_tag  [core][id], tag, MAX_LABEL_LEN );
    magma_tally3_strlcpy( glog.cpu_label[core][id], lbl, MAX_LABEL_LEN );
}
...
{
    int id = glog.cpu_id[core];
    glog.cpu_end[core][id] = magma_tally3_wtime();
    if ( id+1 < MAX_EVENTS ) {
...
void trace_gpu_start( int dev, int s, const char* tag, const char* lbl )
{
...
#if TRACE_METHOD == 2
    glog.gpu_start[t][id] = magma_tally3_wtime();
#else
    #ifdef HAVE_CUBLAS
    ...
    #endif
#endif
    magma_tally3_strlcpy( glog.gpu_tag  [t][id], tag, MAX_LABEL_LEN );
    magma_tally3_strlcpy( glog.gpu_label[t][id], lbl, MAX_LABEL_LEN );
}
...
void trace_finalize( const char* filename, const char* cssfile )
{
    ...
    double time = magma_tally3_wtime() - glog.cpu_first;
