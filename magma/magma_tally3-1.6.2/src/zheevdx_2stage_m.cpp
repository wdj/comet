    ...
    magma_tally3_int_t *info)
{
    ...
    *info = 0;
    ...
    magma_tally3_int_t Vblksiz = magma_tally3_zbulge_get_Vblksiz(n, nb, parallel_threads);
    ...
    magma_tally3DoubleComplex *dT1;
    if (MAGMA_tally3_SUCCESS != magma_tally3_zmalloc( &dT1, n*nb)) {
        *info = MAGMA_tally3_ERR_DEVICE_ALLOC;
        return *info;
    }
    timer_start( time_band );
    magma_tally3_zhetrd_he2hb(uplo, n, nb, A, lda, &work[indtau1], &work[indwrk], llwork, dT1, info);
    ...
    magma_tally3DoubleComplex *da[Magma_tally3MaxGPUs], *dT1[Magma_tally3MaxGPUs];
    ...
    for( magma_tally3_int_t dev = 0; dev < ngpu; ++dev ) {
        ...
        // TODO check malloc
        magma_tally3_zmalloc(&da[dev], ldda*mlocal );
        magma_tally3_zmalloc(&dT1[dev], (n*nb) );
    ...
    magma_tally3_zsetmatrix_1D_col_bcyclic( n, n, A, lda, da, ldda, ngpu, distblk );
    magma_tally3_setdevice(0);
    timer_stop( time_dist );

    timer_start( time_band );
    if (ver == 30) {
        magma_tally3_zhetrd_he2hb_mgpu_spec(uplo, n, nb, A, lda, &work[indtau1], &work[indwrk], llwork, da, ldda, dT1, nb, ngpu, distblk, streams, nstream, info);
    } else {
        magma_tally3_zhetrd_he2hb_mgpu(uplo, n, nb, A, lda, &work[indtau1], &work[indwrk], llwork, da, ldda, dT1, nb, ngpu, distblk, streams, nstream, info);
    }
    timer_stop( time_band );
    ...
    magma_tally3DoubleComplex* A2 = &work[indwrk];
    memset(A2, 0, n*lda2*sizeof(magma_tally3DoubleComplex));

    for (magma_tally3_int_t j = 0; j < n-nb; j++) {
        len = nb+1;
        blasf77_zcopy( &len, A(j,j), &ione, A2(0,j), &ione );
        memset(A(j,j), 0, (nb+1)*sizeof(magma_tally3DoubleComplex));
        *A(nb+j,j) = c_one;
    }
    for (magma_tally3_int_t j = 0; j < nb; j++) {
        len = nb-j;
        blasf77_zcopy( &len, A(j+n-nb,j+n-nb), &ione, A2(0,j+n-nb), &ione );
        memset(A(j+n-nb,j+n-nb), 0, (nb-j)*sizeof(magma_tally3DoubleComplex));
    }
    ...
    magma_tally3_zhetrd_hb2st(uplo, n, nb, Vblksiz, A2, lda2, w, &rwork[inde], &work[indV2], ldv, &work[indTAU2], wantz, &work[indT2], ldt);
...
/*
        ...
        if (MAGMA_tally3_SUCCESS != magma_tally3_zmalloc( &dZ, *m*lddz)) {
            *info = MAGMA_tally3_ERR_DEVICE_ALLOC;
            return *info;
        }

        magma_tally3_zbulge_back(uplo, n, nb, *m, Vblksiz, &work[indwrk + n * (il-1)], n, dZ, lddz,
                          &work[indV2], ldv, &work[indTAU2], &work[indT2], ldt, info);

        magma_tally3_zgetmatrix( n, *m, dZ, lddz, &work[indwrk], n);

        magma_tally3_free(dZ);

*/

        magma_tally3_zbulge_back_m(ngpu, uplo, n, nb, *m, Vblksiz, &work[indwrk + n * (il-1)], n,
                            &work[indV2], ldv, &work[indTAU2], &work[indT2], ldt, info);
        ...
        magma_tally3_zunmqr_m(ngpu, Magma_tally3Left, Magma_tally3NoTrans, n-nb, *m, n-nb, A+nb, lda, &work[indtau1],
