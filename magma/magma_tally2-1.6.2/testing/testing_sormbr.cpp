    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int itran = 0; itran < 2; ++itran ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            n = opts.nsize[itest];
            k = opts.ksize[itest];
            nb  = magma_tally2_get_sgebrd_nb( m );
            ldc = m;
            ...
            magma_tally2_sgebrd( mm, nn, A, lda, d, e, tauq, taup, work, lwork_max, &info );
            if (info != 0)
                printf("magma_tally2_sgebrd returned error %d: %s.\n",
            ...
            lapackf77_sormbr( lapack_vect_const_tally2( vect[ivect] ),
            ...
            lwork = -1;
            magma_tally2_sormbr( vect[ivect], side[iside], trans[itran],
                          m, n, k,
                          A, lda, tau, R, ldc, work, lwork, &info );
            if (info != 0)
                printf("magma_tally2_sormbr (lwork query) returned error %d: %s.\n",
                       (int) info, magma_tally2_strerror( info ));
            lwork = (magma_tally2_int_t) MAGMA_tally2_S_REAL( work[0] );
            ...
            if (info != 0)
                printf("magma_tally2_sormbr returned error %d: %s.\n",
                       (int) info, magma_tally2_strerror( info ));
