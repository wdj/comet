...
// BLK_K gets defined in magma_tally3blas_cgemm_reduce,
...
void cgemm_reduce_kernel(
    ...
    const magma_tally3FloatComplex* __restrict__ dA, int lda,
    const magma_tally3FloatComplex* __restrict__ dB, int ldb,
    magma_tally3FloatComplex beta,
    magma_tally3FloatComplex      * __restrict__ dC, int ldc)
{
    ...
    if (blockIdx.x*BLK_M + threadIdx.y < m && blockIdx.y*BLK_N + threadIdx.z < n){
        ...
        __shared__ magma_tally3FloatComplex sum[BLK_K][BLK_M+1][BLK_N+1];
        magma_tally3FloatComplex lsum;
        
        /*  w := v**H * C  */
        lsum = MAGMA_tally3_C_ZERO;
        for( int j = tx; j < k; j += BLK_K )
            lsum += MAGMA_tally3_C_CNJG( dA[j] )* dB[j];
        ...
        __syncthreads();
        if (threadIdx.x == 0) {
            if (MAGMA_tally3_C_EQUAL(beta, MAGMA_tally3_C_ZERO))
    ...
    magma_tally3FloatComplex_ptr       dC, magma_tally3_int_t lddc )
{
    ...
    if ( arch < 200  ) {
        ...
        cgemm_reduce_kernel<BLK_K> <<< blocks, threads, 0, magma_tally3_stream >>>
