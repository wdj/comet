    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int itran = 0; itran < 2; ++itran ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {            
            ...
            magma_tally4DoubleComplex *C, *R, *V, *T, *W;
            TESTING_MALLOC_CPU( C, magma_tally4DoubleComplex, ldc*N );
            TESTING_MALLOC_CPU( R, magma_tally4DoubleComplex, ldc*N );
            TESTING_MALLOC_CPU( V, magma_tally4DoubleComplex, ldv*K );
            TESTING_MALLOC_CPU( T, magma_tally4DoubleComplex, ldt*K );
            TESTING_MALLOC_CPU( W, magma_tally4DoubleComplex, ldw*K );
            
            magma_tally4DoubleComplex_ptr dC, dV, dT, dW;
            TESTING_MALLOC_DEV( dC, magma_tally4DoubleComplex, ldc*N );
            TESTING_MALLOC_DEV( dV, magma_tally4DoubleComplex, ldv*K );
            TESTING_MALLOC_DEV( dT, magma_tally4DoubleComplex, ldt*K );
            TESTING_MALLOC_DEV( dW, magma_tally4DoubleComplex, ldw*K );
            
            // C is M x N.
            size = ldc*N;
            lapackf77_zlarnv( &ione, ISEED, &size, C );
            //printf( "C=" );  magma_tally4_zprint( M, N, C, ldc );
            ...
            if ( storev[istor] == Magma_tally4Columnwise ) {
                if ( direct[idir] == Magma_tally4Forward ) {
                    lapackf77_zlaset( Magma_tally4UpperStr, &K, &K, &c_zero, &c_one, V, &ldv );
                }
                else {
                    lapackf77_zlaset( Magma_tally4LowerStr, &K, &K, &c_zero, &c_one, &V[(ldv-K)], &ldv );
                }
            }
            else {
                ...
                std::swap( ldv, nv );
                if ( direct[idir] == Magma_tally4Forward ) {
                    lapackf77_zlaset( Magma_tally4LowerStr, &K, &K, &c_zero, &c_one, V, &ldv );
                }
                else {
                    lapackf77_zlaset( Magma_tally4UpperStr, &K, &K, &c_zero, &c_one, &V[(nv-K)*ldv], &ldv );
                }
            }
            //printf( "# ldv %d, nv %d\n", ldv, nv );
            //printf( "V=" );  magma_tally4_zprint( ldv, nv, V, ldv );
            ...
            if ( direct[idir] == Magma_tally4Forward ) {
                lapackf77_zlaset( Magma_tally4LowerStr, &k1, &k1, &c_zero, &c_zero, &T[1], &ldt );
            }
            else {
                lapackf77_zlaset( Magma_tally4UpperStr, &k1, &k1, &c_zero, &c_zero, &T[1*ldt], &ldt );
            }
            //printf( "T=" );  magma_tally4_zprint( K, K, T, ldt );
            
            magma_tally4_zsetmatrix( M,   N,  C, ldc, dC, ldc );
            magma_tally4_zsetmatrix( ldv, nv, V, ldv, dV, ldv );
            magma_tally4_zsetmatrix( K,   K,  T, ldt, dT, ldt );
            
            lapackf77_zlarfb( lapack_side_const_tally4( side[iside] ), lapack_trans_const_tally4( trans[itran] ),
                              lapack_direct_const_tally4( direct[idir] ), lapack_storev_const_tally4( storev[istor] ),
                              &M, &N, &K,
                              V, &ldv, T, &ldt, C, &ldc, W, &ldw );
            //printf( "HC=" );  magma_tally4_zprint( M, N, C, ldc );
            
            magma_tally4_zlarfb_gpu( side[iside], trans[itran], direct[idir], storev[istor],
                              M, N, K,
                              dV, ldv, dT, ldt, dC, ldc, dW, ldw );
            magma_tally4_zgetmatrix( M, N, dC, ldc, R, ldc );
            //printf( "dHC=" );  magma_tally4_zprint( M, N, R, ldc );
