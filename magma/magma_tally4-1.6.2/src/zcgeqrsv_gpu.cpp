    ...
    magma_tally4_int_t *info)
{
    ...
    magma_tally4DoubleComplex *hworkd;
    magma_tally4FloatComplex  *hworks;
    magma_tally4DoubleComplex *tau;
    magma_tally4FloatComplex  *stau;
    magma_tally4DoubleComplex_ptr dworkd;
    magma_tally4FloatComplex_ptr  dworks;
    magma_tally4DoubleComplex_ptr dR, dT;
    magma_tally4FloatComplex_ptr  dSA, dSX, dST;
    ...
    *info = 0;
    ...
    if (MAGMA_tally4_SUCCESS != magma_tally4_cmalloc( &dworks, size )) {
    ...
    /* dworkd(dR) = lddr*nrhs */
    ldworkd = lddr*nrhs;
    if (MAGMA_tally4_SUCCESS != magma_tally4_zmalloc( &dworkd, ldworkd )) {
    ...
    size = lhwork + minmn;
    magma_tally4_cmalloc_cpu( &hworks, size );
    if ( hworks == NULL ) {
        magma_tally4_free( dworks );
    ...
    Anrm = magma_tally4blas_zlange(Magma_tally4InfNorm, m, n, dA, ldda, (double*)dworkd );
    cte  = Anrm * eps * pow((double)n, 0.5) * BWDMAX;

    /*
     * Convert to single precision
     */
    magma_tally4blas_zlag2c( m, nrhs, dB, lddb, dSX, lddsx, info );
    if (*info != 0) {
        *iter = -2;
        goto FALLBACK;
    }

    magma_tally4blas_zlag2c( m, n, dA, ldda, dSA, lddsa, info );
    if (*info != 0) {
        *iter = -2;
        goto FALLBACK;
    }
    ...
    magma_tally4_cgeqrf_gpu( m, n, dSA, lddsa, stau, dST, info );
    if (*info != 0) {
        *iter = -3;
        goto FALLBACK;
    }
    ...
    magma_tally4_cgeqrs_gpu( m, n, nrhs, dSA, lddsa, stau, dST, dSX, lddsx, hworks, lhwork, info );
    if (*info != 0) {
        *iter = -3;
        goto FALLBACK;
    }
    ...
    magma_tally4blas_zlacpy( Magma_tally4UpperLower, m, nrhs, dB, lddb, dR, lddr );
    if ( nrhs == 1 ) {
        magma_tally4_zgemv( Magma_tally4NoTrans, m, n,
                     c_neg_one, dA, ldda,
                                dX, 1,
                     c_one,     dR, 1 );
    }
    else {
        magma_tally4_zgemm( Magma_tally4NoTrans, Magma_tally4NoTrans, m, nrhs, n,
                     c_neg_one, dA, ldda,
                                dX, lddx,
                     c_one,     dR, lddr );
    }
    ...
    for( j=0; j < nrhs; j++ ) {
        ...
        i = magma_tally4_izamax ( m, dR(0,j), 1 ) - 1;
...
REFINEMENT:
    ...
    for( iiter=1; iiter < ITERMAX; ) {
        *info = 0;
        ...
        magma_tally4blas_zlag2c( m, nrhs, dR, lddr, dSX, lddsx, info );
        if (*info != 0) {
            *iter = -2;
            goto FALLBACK;
        }
...
FALLBACK:
    ...
    if ( size > ldworkd ) {
        magma_tally4_free( dworkd );
        if (MAGMA_tally4_SUCCESS != magma_tally4_zmalloc( &dworkd, size )) {
    ...
    size = lhwork + minmn;
    magma_tally4_zmalloc_cpu( &hworkd, size );
    if ( hworkd == NULL ) {
        magma_tally4_free( dworkd );
    ...
    magma_tally4_zgeqrf_gpu( m, n, dA, ldda, tau, dT, info );
    if (*info == 0) {
        // if m > n, then dB won't fit in dX, so solve with dB and copy n rows to dX
        magma_tally4_zgeqrs_gpu( m, n, nrhs, dA, ldda, tau, dT, dB, lddb, hworkd, lhwork, info );
