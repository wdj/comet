...
/**
    ...
    This is an out-of-core (ooc) version that is similar to magma_tally4_sgeqrf but
    ...
    magma_tally4_int_t *info )
{
    ...
    *info = 0;
    int nb = magma_tally4_get_sgeqrf_nb(min(m, n));
    ...
    if (NB >= n)
        return magma_tally4_sgeqrf(m, n, A, lda, tau, work, lwork, info);
    ...
    if (MAGMA_tally4_SUCCESS != magma_tally4_smalloc( &dA, (NB + nb)*ldda + nb*lddwork )) {
    ...
    for (int i=0; i < n; i += NB) {
        ...
        if (i < k)
            magma_tally4_sgeqrf2_gpu(m-i, IB, dA(i,0), ldda, tau+i, info);
