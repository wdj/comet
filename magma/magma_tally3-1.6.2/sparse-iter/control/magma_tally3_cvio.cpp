/*
    -- MAGMA_tally3 (version 1.6.2) --
       ...
       @generated from magma_tally3_zvio.cpp normal z -> c, Sun May  3 11:23:01 2015
       @author Hartwig Anzt
*/
...
/**
    ...
    Visualizes part of a vector of type magma_tally3_c_matrix.
    ...
    magma_tally3_queue_t queue )
{
    ...
    magma_tally3_c_matrix y={Magma_tally3_CSR};
    ...
    if ( x.memory_location == Magma_tally3_CPU ) {
        ...
        for( magma_tally3_int_t i=offset; i<offset + visulen; i++ )
            magma_tally3_cprintval(x.val[i]);
    }
    else if ( x.memory_location == Magma_tally3_DEV ) {
        printf("located on DEV:\n");
        CHECK( magma_tally3_cmtransfer( x, &y, Magma_tally3_DEV, Magma_tally3_CPU, queue ));
        for( magma_tally3_int_t i=offset; i<offset +  visulen; i++ )
            magma_tally3_cprintval(y.val[i]);
    ...
    magma_tally3_queue_t queue )
{
    ...
    CHECK( magma_tally3_cmalloc_cpu( &x->val, length ));
    ...
    while( i<length )  // eof() is 'true' at the end of data
    {
        ...
        #define COMPLEX
            ...
            float VAL2;
            fscanf(fid, " %f %f \n", &VAL1, &VAL2);
            VAL = MAGMA_tally3_C_MAKE(VAL1, VAL2);
        #else
            fscanf(fid, " %f \n", &VAL1);
            VAL = MAGMA_tally3_C_MAKE(VAL1, 0.0);
        #endif
        
        if ( VAL != MAGMA_tally3_C_ZERO )
            nnz++;
        x->val[i] = VAL;
        i++;
    }
    ...
    magma_tally3_queue_t queue )
{
    ...
    magma_tally3_c_matrix A={Magma_tally3_CSR}, B={Magma_tally3_CSR};
    magma_tally3_int_t entry=0;
     //   char *vfilename[] = {"/mnt/sparse_matrices/mtx/rail_79841_B.mtx"};
    CHECK( magma_tally3_c_csr_mtx( &A,  filename, queue  ));
    CHECK( magma_tally3_cmconvert( A, &B, Magma_tally3_CSR, Magma_tally3_DENSE, queue ));
    CHECK( magma_tally3_cvinit( x, Magma_tally3_CPU, A.num_cols, A.num_rows, MAGMA_tally3_C_ZERO, queue ));
