...
void dpotf2_dscal(magma_minproduct_int_t n, double *x, magma_minproduct_int_t incx);
void dpotf2_ddot(magma_minproduct_int_t n, double *x, magma_minproduct_int_t incx);

#if defined(PRECISION_z) || defined(PRECISION_c)
void dlacgv(magma_minproduct_int_t n, double *x, magma_minproduct_int_t incx);
#endif
    ...
    if (uplo == Magma_minproductUpper) {
        for(j = 0; j < n; j++) {
            ...
            if (j < n) {
                ...
                dlacgv(j, dA(0, j), 1);
                #endif
                magma_minproduct_dgemv( Magma_minproductTrans, j, n-j-1,
                             alpha, dA(0, j+1), ldda,
                                    dA(0, j),   1,
                             beta,  dA(j, j+1), ldda);
        ...
        for(j = 0; j < n; j++) {
            ...
            if (j < n) {
                ...
                #endif
                magma_minproduct_dgemv( Magma_minproductNoTrans, n-j-1, j,
                             alpha, dA(j+1, 0), ldda,
                                    dA(j,0),    ldda,
                             beta,  dA(j+1, j), 1 );
...
*/
    ...
    kernel_ddot<<< 1, threadSize, threadSize * sizeof(double), magma_minproduct_stream>>> (n, x, incx, threadSize);
}
...
*/
    ...
    kernel_dscal<<< grid, threads, 0, magma_minproduct_stream >>> (n, x, incx);
}
...
void dlacgv(magma_minproduct_int_t n, double *x, magma_minproduct_int_t incx)
{
    ...
    dim3 grid(num_blocks,1);
    kernel_dlacgv<<< grid, threads, 0, magma_minproduct_stream >>> (n, x, incx);
}
