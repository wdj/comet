...
void magma_tally4_task_core_void(Schedule* sched_obj)
{    
    /*nothing*/
}
...
/*Fill block of memory: sets the first n bytes of the block of memory pointed by ptr to the specified value*/
 void magma_tally4_task_core_dmemset(Schedule* sched_obj )
 {
    double *ptr;
    double value;
...
void magma_tally4_task_core_dgetrf(Schedule* sched_obj )
{
...
ca_trace_start();
    ...
    //magma_tally4_setlapack_numthreads(4); //testing
    

    dgetrf(&m,  &n, A,  &LDA,  ipiv,  info);

    //magma_tally4_setlapack_numthreads(1); //to be sure
    
     //magma_tally4_task_core_dgetrf_rec(A_m, nb, A(0,K), A_LD, &ipiv[0], iinfo);
    #if (dbglevel==10)
...
/*
void magma_tally4_task_core_dgetrf_rec(Schedule* sched_obj )
{
    magma_tally4_int_t m;
    magma_tally4_int_t n;
    double *A; 
...
*/
void magma_tally4_task_core_zgetrf_reclap(Schedule* sched_obj)
{
...
void magma_tally4_task_core_dtslu(Schedule* sched_obj)
{
    /*CORE_zgetrf_reclap_schedule(Schedule* sched_obj)*/
    int m;
    int n;
...
//    printf("end doing dgetrf\n");
 ...
 void magma_tally4_task_core_dlaswp(Schedule* sched_obj )
 {
    magma_tally4_int_t n;
    double *A;
    magma_tally4_int_t LDA;
    magma_tally4_int_t K1;
    magma_tally4_int_t K2;
    magma_tally4_int_t *ipiv;
...
ca_trace_start();
    ...
    dlaswp( &n, A,  &LDA,  &K1,  &K2,  ipiv,  &incx);
      //magma_tally4_task_core_dlaswp(ncols, A(K,J), A_LD, c_one, nb, &ipiv[K], c_one);
 #if (dbglevel >=1)
ca_trace_end_cpu('W');
#endif
 }
     
void  magma_tally4_task_core_dtrsm(Schedule* sched_obj )
{
    char side;
    char uplo;
    char transa;
...
ca_trace_start();
    ...
    //magma_tally4_task_core_dtrsm("L", "L", "N", "U", nb, ncols, c_one, A(K,K), A_LD, A(K,J), A_LD);
#if (dbglevel==10)
            if(side=='L')
...
void magma_tally4_task_core_dgemm(Schedule* sched_obj )
 {
    char transa;
    char transb; 
    magma_tally4_int_t m;
...
ca_trace_start();
    ...
    dgemm( &transa,  &transb, &m,  &n,  &k, &alpha, A,  &LDA, B,  &LDB, &beta, C,  &LDC);
     //magma_tally4_task_core_dgemm("N","N", nrows, ncols, nb, c_neg_one, A(I,K), A_LD, A(K,J), A_LD, c_one, A(I,J), A_LD);
    #if (dbglevel==10)
