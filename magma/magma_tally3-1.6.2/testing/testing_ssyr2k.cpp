...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_tally3_ssetmatrix( Bn, Bk, h_B, ldb, d_B, lddb );
            magma_tally3_ssetmatrix( N, N, h_C, ldc, d_C, lddc );
            
            cublas_time = magma_tally3_sync_wtime( NULL );
            cublasSsyr2k( opts.handle, cublas_uplo_const_tally3(opts.uplo), cublas_trans_const_tally3(opts.transA), N, K,
                          &alpha, d_A, ldda,
                                  d_B, lddb,
                          &beta,  d_C, lddc );
            ...
            if ( opts.lapack ) {
                ...
                blasf77_ssyr2k( lapack_uplo_const_tally3(opts.uplo), lapack_trans_const_tally3(opts.transA), &N, &K,
                               &alpha, h_A, &lda,
                                       h_B, &ldb,
                               &beta,  h_C, &ldc );
