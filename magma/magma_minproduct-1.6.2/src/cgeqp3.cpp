/*
    -- MAGMA_minproduct (version 1.6.1) --
       ...
       @generated from zgeqp3.cpp normal z -> c, Fri Jan 30 19:00:16 2015
...
#define dA(i, j) (dwork + (i) + (j)*(ldda))
    ...
    magma_minproductFloatComplex   *dwork, *df;

    magma_minproduct_int_t ione = 1;

    magma_minproduct_int_t n_j, ldda, ldwork;
    magma_minproduct_int_t j, jb, na, nb, sm, sn, fjb, nfxd, minmn;
    magma_minproduct_int_t topbmn, sminmn, lwkopt=0, lquery;
    ...
    *info = 0;
    ...
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_cmalloc( &dwork, ldwork )) {
    ...
    if (nfxd < minmn) {
        ...
        if (nb < sminmn) {
            ...
            magma_minproduct_csetmatrix_async( m, sn,
                                    A (0,j), lda,
                                    dA(0,j), ldda, stream );
        }
        ...
        for (j = nfxd; j < n; ++j) {
            rwork[j] = magma_minproduct_cblas_scnrm2( sm, A(nfxd,j), ione );
            rwork[n + j] = rwork[j];
        }
        ...
        if (nb < sminmn) {
            ...
            while(j < topbmn) {
                ...
                if (j > nfxd) {
                    ...
                    magma_minproduct_cgetmatrix( m-j, jb,
                                      dA(j,j), ldda,
                                      A (j,j), lda );
                    
                    // Get the rows
                    magma_minproduct_cgetmatrix( jb, n_j - jb,
                                      dA(j,j + jb), ldda,
                                      A (j,j + jb), lda );
                }

                magma_minproduct_claqps( m, n_j, j, jb, &fjb,
                              A (0, j), lda,
                              dA(0, j), ldda,
