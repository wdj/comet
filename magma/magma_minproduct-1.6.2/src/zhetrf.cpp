    ...
    magma_minproduct_int_t *ipiv, magma_minproduct_int_t *info)
{
    ...
    magma_minproduct_int_t nb = magma_minproduct_get_zhetrf_nb(n);
    ...
    *info = 0;
    ...
    magma_minproductDoubleComplex *dA, *dW;
    if ((MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc( &dA, n*ldda  )) ||
        (MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc( &dW, (1+nb)*ldda ))) {
    ...
    //magma_minproduct_zsetmatrix_async( n, n, A(0,0), lda, dA(0,0), ldda, stream[0] );
    if ( upper ) {
       for (int k = 0; k < n; k+=nb ) {
           kb = min(nb, n-k);
           magma_minproduct_zsetmatrix_async( k+kb, kb, A(0,k), lda, dA(0,k), ldda, stream[0] );
       }
    } else {
       for (int k = 0; k < n; k+=nb ) {
           kb = min(nb, n-k);
           magma_minproduct_zsetmatrix_async( n-k, kb, A(k,k), lda, dA(k,k), ldda, stream[0] );
       }
    }
    trace_gpu_end( 0, 0 );
    ...
    if ( upper ) {
        ...
        for (int k = n-1; k >= 0; k-=kb ) {
            ...
            if ( k+1 > nb ) {
                ...
                magma_minproduct_zlahef_gpu( Magma_minproductUpper, nk, kb, &kb, A( 0, 0 ), lda, dA( 0, 0 ), ldda, 
                ...
                magma_minproduct_zgetmatrix( nk, nk, dA( 0, 0 ),ldda, A( 0, 0 ),lda );
                lapackf77_zhetf2( Magma_minproductUpperStr, &nk, A( 0, 0 ), &lda, &ipiv[0], &iinfo );
                kb = k+1;
            }
        ...
        for (int k = 0; k < n; k += kb ) {
            ...
            if ( k < n-nb ) {
                ...
                magma_minproduct_zlahef_gpu( Magma_minproductLower, nk, nb, &kb, A( k, k ), lda, dA( k, k ), ldda,
                                  &ipiv[k], dW, ldda, stream, event, &iinfo );

            } else {
                /* Use unblocked code to factorize columns k:n of A */
                magma_minproduct_queue_sync( stream[0] );
                magma_minproduct_zgetmatrix( nk,nk, dA(k,k),ldda, A(k,k),lda );
                lapackf77_zhetf2( Magma_minproductLowerStr, &nk, A( k, k ), &lda, &ipiv[k], &iinfo );
            }
            /* Set INFO on the first occurrence of a zero pivot */
