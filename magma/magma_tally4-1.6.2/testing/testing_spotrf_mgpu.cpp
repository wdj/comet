    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( opts.lapack ) {
                ...
                lapackf77_spotrf( lapack_uplo_const_tally4(opts.uplo), &N, h_A, &lda, &info );
            ...
            if ( opts.uplo == Magma_tally4Upper ) {
                ldda = ((N+nb-1)/nb)*nb;
                magma_tally4_ssetmatrix_1D_col_bcyclic( N, N, h_R, lda, d_lA, ldda, ngpu, nb );
            } else {
                ldda = (1+N/(nb*ngpu))*nb;
                magma_tally4_ssetmatrix_1D_row_bcyclic( N, N, h_R, lda, d_lA, ldda, ngpu, nb );
            }
            
            gpu_time = magma_tally4_wtime();
            magma_tally4_spotrf_mgpu( ngpu, opts.uplo, N, d_lA, ldda, &info );
            gpu_time = magma_tally4_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally4_spotrf_mgpu returned error %d: %s.\n",
                       (int) info, magma_tally4_strerror( info ));
            
            if ( opts.uplo == Magma_tally4Upper ) {
                magma_tally4_sgetmatrix_1D_col_bcyclic( N, N, d_lA, ldda, h_R, lda, ngpu, nb );
            } else {
                magma_tally4_sgetmatrix_1D_row_bcyclic( N, N, d_lA, ldda, h_R, lda, ngpu, nb );
            }
