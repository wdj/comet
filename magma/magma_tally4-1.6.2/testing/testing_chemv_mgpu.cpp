...
int main(int argc, char **argv)
{
    ...
    magma_tally4FloatComplex *A, *X, *Y, *Ylapack, *Ycublas, *Ymagma_tally4, *Ymagma_tally41, *hwork;
    magma_tally4FloatComplex_ptr dA, dX, dY;
    magma_tally4FloatComplex_ptr d_lA[Magma_tally4MaxGPUs], dwork[Magma_tally4MaxGPUs];
    ...
    magma_tally4_int_t nb = 64;  // required by magma_tally4blas_chemv_mgpu implementation
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int ioffset=0; ioffset < noffsets; ioffset += 1 ) {
        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( A,       magma_tally4FloatComplex, matsize );
            TESTING_MALLOC_CPU( Y,       magma_tally4FloatComplex, vecsize );
            TESTING_MALLOC_CPU( Ycublas, magma_tally4FloatComplex, vecsize );
            TESTING_MALLOC_CPU( Ymagma_tally4,  magma_tally4FloatComplex, vecsize );
            TESTING_MALLOC_CPU( Ymagma_tally41, magma_tally4FloatComplex, vecsize );
            TESTING_MALLOC_CPU( Ylapack, magma_tally4FloatComplex, vecsize );

            TESTING_MALLOC_PIN( X,       magma_tally4FloatComplex, vecsize );
            TESTING_MALLOC_PIN( hwork,   magma_tally4FloatComplex, lhwork  );
            
            magma_tally4_setdevice( opts.device );
            TESTING_MALLOC_DEV( dA, magma_tally4FloatComplex, matsize );
            TESTING_MALLOC_DEV( dX, magma_tally4FloatComplex, vecsize );
            TESTING_MALLOC_DEV( dY, magma_tally4FloatComplex, vecsize );
            
            // TODO make magma_tally4_cmalloc_bcyclic helper function?
            for( dev=0; dev < opts.ngpu; dev++ ) {
                ...
                magma_tally4_setdevice( dev );
                TESTING_MALLOC_DEV( d_lA[dev],  magma_tally4FloatComplex, ldda*n_local[dev] );
                TESTING_MALLOC_DEV( dwork[dev], magma_tally4FloatComplex, ldwork );
            }
            ...
            magma_tally4_cmake_hermitian( Noffset, A, lda );
            ...
            /* =====================================================================
               ...
               =================================================================== */
            magma_tally4_setdevice( opts.device );
            magma_tally4_csetmatrix( Noffset, Noffset, A, lda, dA, ldda );
            magma_tally4_csetvector( Noffset, X, incx, dX, incx );
            magma_tally4_csetvector( Noffset, Y, incx, dY, incx );
            
            cuda_time = magma_tally4_sync_wtime(0);
            cublasChemv( opts.handle, cublas_uplo_const_tally4(opts.uplo), N,
            ...
            magma_tally4_cgetvector( Noffset, dY, incx, Ycublas, incx );
            ...
            magma_tally4blas_chemv_work( opts.uplo, N,
                                  alpha, dA + offset + offset*ldda, ldda,
                                         dX + offset, incx,
            ...
            magma_tally4_cgetvector( Noffset, dY, incx, Ymagma_tally41, incx );
            
            /* =====================================================================
               Performs operation using MAGMA_tally4BLAS (multi-GPU)
               =================================================================== */
            magma_tally4_csetmatrix_1D_col_bcyclic( Noffset, Noffset, A, lda, d_lA, ldda, opts.ngpu, nb );
            blasf77_ccopy( &Noffset, Y, &incx, Ymagma_tally4, &incx );
            
            // workspaces do NOT need to be zero -- set to NAN to prove
            for( dev=0; dev < opts.ngpu; ++dev ) {
                magma_tally4_setdevice( dev );
                magma_tally4blas_claset( Magma_tally4Full, ldwork, 1, MAGMA_tally4_C_NAN, MAGMA_tally4_C_NAN, dwork[dev], ldwork );
            }
            lapackf77_claset( "Full", &lhwork, &ione, &MAGMA_tally4_C_NAN, &MAGMA_tally4_C_NAN, hwork, &lhwork );
            
            mgpu_time = magma_tally4_sync_wtime(0);
            
            magma_tally4_int_t info;
            info = magma_tally4blas_chemv_mgpu(
                opts.uplo, N,
                alpha,
                d_lA, ldda, offset,
                X + offset, incx,
                ...
                opts.ngpu, nb, queues );
            if ( info != 0 )
                printf("magma_tally4blas_chemv_mgpu returned error %d: %s.\n",
                       (int) info, magma_tally4_strerror( info ));
            
            info = magma_tally4blas_chemv_mgpu_sync(
                opts.uplo, N,
                alpha,
                d_lA, ldda, offset,
                X + offset, incx,
                ...
                opts.ngpu, nb, queues );
            if ( info != 0 )
                printf("magma_tally4blas_chemv_sync returned error %d: %s.\n",
                       (int) info, magma_tally4_strerror( info ));
            ...
            if ( opts.lapack ) {
                ...
                blasf77_chemv( lapack_uplo_const_tally4(opts.uplo), &N,
                ...
                blasf77_caxpy( &Noffset, &c_neg_one, Ymagma_tally4, &incx, Ylapack, &incx );
            ...
            blasf77_caxpy( &Noffset, &c_neg_one, Ymagma_tally4, &incx, Ycublas, &incx );
