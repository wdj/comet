...
extern "C"
void magma_tally3blas_chemm_mgpu_spec(
    ...
    magma_tally3FloatComplex *work[],     magma_tally3_int_t worksiz,  // TODO unused
    ...
    magma_tally3_int_t gnode[Magma_tally3MaxGPUs][Magma_tally3MaxGPUs+2], magma_tally3_int_t nbcmplx )
{
    ...
    magma_tally3_int_t gdev, gcolsize, gmaster, gngpu;
    ...
    magma_tally3_int_t maxgsize    = n*nb*magma_tally3_ceildiv(nbblk, ngpu);
    ...
    if(ngpu ==1){
        ...
        magma_tally3_cgemm( Magma_tally3ConjTrans, Magma_tally3NoTrans, m, n, m,
    ...
    for( magma_tally3_int_t cmplxid = 0; cmplxid < nbcmplx; ++cmplxid ) {
        ...
        for( magma_tally3_int_t idev = 0; idev < myngpu; ++idev ) {
            ...
            if(mycolsize>0){
                ...
                magma_tally3_cgemm( Magma_tally3ConjTrans, Magma_tally3NoTrans, mycolsize, n, m,
                             alpha, dA(dev, offset, myblkoffst), ldda,
                                    dB(dev, 0, 0),    lddb,
                             beta,  &dwork[dev][maxgsize*dev], mycolsize );
                magma_tally3_event_record(redevents[dev][dev*ngpu+dev], queues[dev][dev]);
            }
            if(dev == masterdev){
...
*/
            ...
            if(mycolsize>0){
                ...
                for( magma_tally3_int_t k = 0; k < nbcmplx; ++k ) {
                    if(k!=cmplxid){
                        ...
                        if(gmaster!=-1){ //complex is active
                            ...
                            magma_tally3_queue_wait_event(queues[ dev ][ gmaster ], redevents[dev][dev*ngpu+dev]);
                            magma_tally3_ccopymatrix_async(
                                mycolsize, n,
                                &dwork[dev    ][maxgsize*dev], mycolsize,
                                &dwork[gmaster][maxgsize*dev], mycolsize, queues[dev][gmaster] );
                            magma_tally3_event_record(redevents[dev][gmaster*ngpu+dev], queues[dev][gmaster]);
                        }
                    }
                }
                //==============================================
                ...
                for( magma_tally3_int_t l = 0; l < myngpu; ++l ) {
                    ...
                    if((lcdev!=dev)&&(lccolsize>0)){
                        ...
                        magma_tally3_queue_wait_event(queues[ dev ][ lcdev ], redevents[dev][dev*ngpu+dev]);
                        magma_tally3_ccopymatrix_async(
                            mycolsize, n,
                            &dwork[dev  ][maxgsize*dev], mycolsize,
                            &dwork[lcdev][maxgsize*dev], mycolsize, queues[dev][lcdev] );
                        magma_tally3_event_record(redevents[dev][lcdev*ngpu+dev], queues[dev][lcdev]);
                    }
                }
                //==============================================
            }// end if mycolsize>0
    ...
    for( magma_tally3_int_t cmplxid = 0; cmplxid < nbcmplx; ++cmplxid ) {
        ...
        if(masterdev != -1){
            ...
            for( magma_tally3_int_t k = 0; k < nbcmplx; ++k ) {
                if(k!=cmplxid){
                    gngpu   = gnode[k][Magma_tally3MaxGPUs];
                    for( magma_tally3_int_t g = 0; g < gngpu; ++g ) {
                        gdev         = gnode[k][g];
                        gcolsize     = gpuisactive[gdev];
                        // check if I received from this GPU,
                        // if yes send it to my group
                        if(gcolsize>0){
                           magma_tally3_queue_wait_event(queues[ masterdev ][ gdev ], redevents[gdev][masterdev*ngpu+gdev]);
                           for( magma_tally3_int_t l = 0; l < myngpu; ++l ) {
                                lcdev         = gnode[cmplxid][l];
                                lccolsize     = gpuisactive[lcdev];
                                if((lcdev!=masterdev)&&(lccolsize>0)){
                                    ...
                                    magma_tally3_event_record(redevents[masterdev][lcdev*ngpu+gdev], queues[masterdev][gdev]);
                                }
                            }
                        }
                    }
...
/*
    ...
    for( magma_tally3_int_t dev = 0; dev < ngpu; ++dev ) {
        ...
        for( magma_tally3_int_t s = 0; s < ngpu; ++s ) {
...
*/
    ...
    magma_tally3_int_t lcblki, gbblki, gblk, ib;
        ...
        for( magma_tally3_int_t idev = 0; idev < myngpu; ++idev ) {
            ...
            if(mycolsize>0){ // I am an active GPU
                ...
                if(gcolsize>0){
                    ...
                    magma_tally3blasSetKernelStream( queues[ dev ][ gdev ] );
                    magma_tally3_queue_wait_event(queues[ dev ][ gdev ], redevents[gdev][dev*ngpu+gdev]);
                    //printf     ("              GPU %d stream %d doing clacpy\n", dev, queues[ dev ][ gdev ]);
                    for( magma_tally3_int_t blki = 0; blki < gblk; ++blki){
                        ...
                        magma_tally3blas_clacpy( Magma_tally3Full, ib, n, &dwork[dev][maxgsize*gdev+lcblki], gcolsize, &dC[dev][gbblki], lddc);
                    }// end blki
                }
                            ...
                            if(gcolsize>0){
                                ...
                                if(k==cmplxid){
                                    ...
                                    if(dev==masterdev)
                                        ...
                                        magma_tally3_queue_wait_event(queues[ dev ][ gdev ], redevents[masterdev][dev*ngpu+gdev]);
                                }
                                //printf     ("              GPU %d stream %d doing clacpy\n", dev, queues[ dev ][ gdev ]);
                                for( magma_tally3_int_t blki = 0; blki < gblk; ++blki){
                                    gbblki = (blki*ngpu + devperm)*nb - blockoffset;
