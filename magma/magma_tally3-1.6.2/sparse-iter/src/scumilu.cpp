    ...
    magma_tally3_queue_t queue )
{
    ...
    magma_tally3_s_matrix hA={Magma_tally3_CSR}, hACSR={Magma_tally3_CSR};
    magma_tally3_s_matrix hL={Magma_tally3_CSR}, hU={Magma_tally3_CSR};
    CHECK( magma_tally3_smtransfer( A, &hA, A.memory_location, Magma_tally3_CPU, queue ));
    CHECK( magma_tally3_smconvert( hA, &hACSR, hA.storage_type, Magma_tally3_CSR, queue ));

        // in case using fill-in
    if( precond->levels > 0 ){
        magma_tally3_s_matrix hAL={Magma_tally3_CSR}, hAUt={Magma_tally3_CSR};
        CHECK( magma_tally3_ssymbilu( &hACSR, precond->levels, &hAL, &hAUt,  queue ));
        magma_tally3_smfree(&hAL, queue);
        magma_tally3_smfree(&hAUt, queue);
    }

    CHECK( magma_tally3_smtransfer(hACSR, &(precond->M), Magma_tally3_CPU, Magma_tally3_DEV, queue ));

    magma_tally3_smfree( &hA, queue );
    magma_tally3_smfree( &hACSR, queue );
    ...
    CHECK( magma_tally3_smtransfer( precond->M, &hA, Magma_tally3_DEV, Magma_tally3_CPU, queue ));

    hL.diagorder_type = Magma_tally3_UNITY;
    CHECK( magma_tally3_smconvert( hA, &hL , Magma_tally3_CSR, Magma_tally3_CSRL, queue ));
    hU.diagorder_type = Magma_tally3_VALUE;
    CHECK( magma_tally3_smconvert( hA, &hU , Magma_tally3_CSR, Magma_tally3_CSRU, queue ));
    CHECK( magma_tally3_smtransfer( hL, &(precond->L), Magma_tally3_CPU, Magma_tally3_DEV, queue ));
    CHECK( magma_tally3_smtransfer( hU, &(precond->U), Magma_tally3_CPU, Magma_tally3_DEV, queue ));
    ...
    magma_tally3_queue_t queue )
{
    ...
    magma_tally3_s_matrix hA={Magma_tally3_CSR}, hL={Magma_tally3_CSR}, hU={Magma_tally3_CSR};
    
    if (precond->L.memory_location != Magma_tally3_DEV ){
        
        CHECK( magma_tally3_smtransfer( precond->M, &hA,
    ...
    if( precond->maxiter < 50 ){
        ...
        // extract the diagonal of L into precond->d
        CHECK( magma_tally3_sjacobisetup_diagscal( precond->L, &precond->d, queue ));
        CHECK( magma_tally3_svinit( &precond->work1, Magma_tally3_DEV, precond->U.num_rows, 1, MAGMA_tally3_S_ZERO, queue ));
        
        // extract the diagonal of U into precond->d2
        CHECK( magma_tally3_sjacobisetup_diagscal( precond->U, &precond->d2, queue ));
        CHECK( magma_tally3_svinit( &precond->work2, Magma_tally3_DEV, precond->U.num_rows, 1, MAGMA_tally3_S_ZERO, queue ));
    }
    ...
    magma_tally3_queue_t queue )
{
    ...
    magma_tally3_s_matrix hA={Magma_tally3_CSR}, hACSR={Magma_tally3_CSR}, U={Magma_tally3_CSR};
    CHECK( magma_tally3_smtransfer( A, &hA, A.memory_location, Magma_tally3_CPU, queue ));
    ...
    CHECK( magma_tally3_smconvert( hACSR, &U, Magma_tally3_CSR, Magma_tally3_CSRL, queue ));
    magma_tally3_smfree( &hACSR, queue );
    CHECK( magma_tally3_smtransfer(U, &(precond->M), Magma_tally3_CPU, Magma_tally3_DEV, queue ));
...
/*
    ...
    magma_tally3_s_matrix hD, hR, hAt

    CHECK( magma_tally3_scsrsplit( 256, hA, &hD, &hR, queue ));

    CHECK( magma_tally3_smtransfer( hD, &precond->LD, Magma_tally3_CPU, Magma_tally3_DEV, queue ));
    CHECK( magma_tally3_smtransfer( hR, &precond->L, Magma_tally3_CPU, Magma_tally3_DEV, queue ));

    magma_tally3_smfree(&hD, queue );
    magma_tally3_smfree(&hR, queue );

    CHECK( magma_tally3_s_cucsrtranspose(   hA, &hAt, queue ));

    CHECK( magma_tally3_scsrsplit( 256, hAt, &hD, &hR, queue ));

    CHECK( magma_tally3_smtransfer( hD, &precond->UD, Magma_tally3_CPU, Magma_tally3_DEV, queue ));
    CHECK( magma_tally3_smtransfer( hR, &precond->U, Magma_tally3_CPU, Magma_tally3_DEV, queue ));
    
    magma_tally3_smfree(&hD, queue );
    magma_tally3_smfree(&hR, queue );
    magma_tally3_smfree(&hA, queue );
    magma_tally3_smfree(&hAt, queue );
*/
