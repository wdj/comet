...
int main(int argc, char **argv)
{
    ...
    magma_tally3FloatComplex *h_A, *h_LU, *h_B, *h_X;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_LU, magma_tally3FloatComplex, lda*N    );
            ...
            magma_tally3_cgesv( N, nrhs, h_LU, lda, ipiv, h_X, ldb, &info );
            gpu_time = magma_tally3_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally3_cgesv returned error %d: %s.\n",
