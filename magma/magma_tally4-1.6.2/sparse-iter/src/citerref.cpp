...
magma_tally4_citerref(
    ...
    magma_tally4_c_solver_par *solver_par, magma_tally4_c_preconditioner *precond_par,
    magma_tally4_queue_t queue )
{
    ...
    // workspace
    magma_tally4_c_matrix r={Magma_tally4_CSR}, z={Magma_tally4_CSR};
    ...
    //CHECK(  magma_tally4_cresidualvec( A, b, *x, &r, nom, queue));
    magma_tally4_ccopy( dofs, b.dval, 1, r.dval, 1 );                    // r = b
    nom0 = magma_tally4_scnrm2(dofs, r.dval, 1);                       // nom0 = || r ||
    nom = nom0 * nom0;
    ...
    for( solver_par->numiter= 1; solver_par->numiter<solver_par->maxiter;
        ...
        magma_tally4_cscal( dofs, MAGMA_tally4_C_MAKE(1./nom, 0.), r.dval, 1) ;  // scale it
        CHECK( magma_tally4_c_precond( A, r, &z, precond_par, queue )); // inner solver:  A * z = r
        magma_tally4_cscal( dofs, MAGMA_tally4_C_MAKE(nom, 0.), z.dval, 1) ;  // scale it
        magma_tally4_caxpy(dofs,  c_one, z.dval, 1, x->dval, 1);        // x = x + z
        CHECK( magma_tally4_c_spmv( c_mone, A, *x, c_zero, r, queue ));      // r = - A x
        magma_tally4_caxpy(dofs,  c_one, b.dval, 1, r.dval, 1);         // r = r + b
        nom = magma_tally4_scnrm2(dofs, r.dval, 1);                    // nom = || r ||
