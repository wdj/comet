/*
    -- MAGMA_tally4 (version 1.6.2) --
       ...
       @generated from magma_tally4_zmcsrcompressor_gpu.cu normal z -> d, Sun May  3 11:22:58 2015
       @author Hartwig Anzt
...
__global__ void
magma_tally4_dmcsrgpu_kernel1( int num_rows,
                 double *A_val,
    ...
    if(row<num_rows){
        double zero = MAGMA_tally4_D_ZERO;
...
__global__ void
magma_tally4_dmcsrgpu_kernel2( int num_rows,
                 magma_tally4_index_t *B_rowptr,
                 magma_tally4_index_t *A_rowptr ){
...
__global__ void
magma_tally4_dmcsrgpu_kernel3( int num_rows,
                 double *B_val,
                 magma_tally4_index_t *B_rowptr,
                 magma_tally4_index_t *B_colind,
    ...
    magma_tally4_queue_t queue )
{
    ...
    magma_tally4_d_matrix B={Magma_tally4_CSR}, B2={Magma_tally4_CSR};
    magma_tally4_d_matrix dA={Magma_tally4_CSR}, CSRA={Magma_tally4_CSR};
    ...
    if ( A->memory_location == Magma_tally4_DEV && A->storage_type == Magma_tally4_CSR ) {
        ...
        magma_tally4_dmcsrgpu_kernel1<<< grid1, BLOCK_SIZE1, 0, queue >>>
                ( A->num_rows, A->dval, A->drow, A->dcol, B.dval, B.drow, B.dcol );

        // correct the row pointer
        dim3 grid2( 1, 1, 1);
        magma_tally4_dmcsrgpu_kernel2<<< grid2, BLOCK_SIZE2, 0, queue >>>
        ...
        magma_tally4_dmcsrgpu_kernel3<<< grid1, BLOCK_SIZE1, 0, queue >>>
        ...
        CHECK( magma_tally4_dmconvert( *A, &CSRA, A->storage_type, Magma_tally4_CSR, queue ));
        CHECK( magma_tally4_dmtransfer( *A, &dA, A->memory_location, Magma_tally4_DEV, queue ));

        CHECK( magma_tally4_dmcsrcompressor_gpu( &dA, queue ));

        magma_tally4_dmfree( &dA, queue );
        magma_tally4_dmfree( A, queue );
        CHECK( magma_tally4_dmtransfer( dA, &CSRA, Magma_tally4_DEV, A_location, queue ));
        CHECK( magma_tally4_dmconvert( CSRA, A, Magma_tally4_CSR, A_storage, queue ));
        magma_tally4_dmfree( &dA, queue );
