...
/**
    ...
    lwork   INTEGER
            ...
            NB can be obtained through magma_tally2_get_dsytrd_nb(N).
    \n
    ...
    magma_tally2_int_t *info)
{
    ...
    *info = 0;
    ...
    if (n <= 128) {
        ...
        magma_tally2_dgetmatrix( n, n, dA, ldda, A, lda );
        // TODO: deal with range and mout. Use lapack dsyevx? (There is no lapack dsyevdx.)
        lapackf77_dsyevd( lapack_vec_const_tally2(jobz), lapack_uplo_const_tally2(uplo),
                          &n, A, &lda,
                          w, work, &lwork,
                          iwork, &liwork, info );
        *mout = n;
    ...
    if (MAGMA_tally2_SUCCESS != magma_tally2_dmalloc( &dwork, ldwork )) {
    ...
    anrm = magma_tally2blas_dlansy( Magma_tally2MaxNorm, uplo, n, dA, ldda, dwork );
    iscale = 0;
    sigma  = 1;
    if (anrm > 0. && anrm < rmin) {
        iscale = 1;
    ...
    if (iscale == 1) {
        magma_tally2blas_dlascl( uplo, 0, 0, 1., sigma, n, n, dA, ldda, info );
    }
...
#ifdef FAST_SYMV
    magma_tally2_dsytrd2_gpu( uplo, n, dA, ldda, w, &work[inde],
                       &work[indtau], wA, ldwa, &work[indwrk], llwork,
                       dwork, ldwork, &iinfo );
#else
    magma_tally2_dsytrd_gpu(  uplo, n, dA, ldda, w, &work[inde],
                       &work[indtau], wA, ldwa, &work[indwrk], llwork,
                       &iinfo );
#endif
    ...
    if (! wantz) {
        ...
        magma_tally2_dstedx( range, n, vl, vu, il, iu, w, &work[inde],
        ...
        magma_tally2_dsetmatrix( n, *mout, &work[indwrk + n* (il-1) ], n, dwork, lddc );

        magma_tally2_dormtr_gpu( Magma_tally2Left, uplo, Magma_tally2NoTrans, n, *mout, dA, ldda, &work[indtau],
                          dwork, lddc, wA, ldwa, &iinfo );

        magma_tally2_dcopymatrix( n, *mout, dwork, lddc, dA, ldda );
