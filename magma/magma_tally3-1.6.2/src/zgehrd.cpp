...
/**
    ...
    This version stores the T matrices in dT, for later use in magma_tally3_zunghr.
    ...
    magma_tally3_int_t *info)
{
    ...
    magma_tally3_int_t nb = magma_tally3_get_zgehrd_nb(n);
    ...
    *info = 0;
    iws = n*nb;
    work[0] = MAGMA_tally3_Z_MAKE( iws, 0 );
    ...
    if (nb == 1 || nb > nh) {
        ...
        if (MAGMA_tally3_SUCCESS != magma_tally3_zmalloc( &dwork, 2*nb*ldda + n*ldda )) {
            *info = MAGMA_tally3_ERR_DEVICE_ALLOC;
            return *info;
        }
        magma_tally3DoubleComplex *dV = dwork + nb*ldda;
        magma_tally3DoubleComplex *dA = dwork + nb*ldda*2;
        magma_tally3DoubleComplex *dTi;
        
        magma_tally3DoubleComplex *T;
        magma_tally3_zmalloc_cpu( &T, nb*nb );
        if ( T == NULL ) {
        ...
        magma_tally3blas_zlaset( Magma_tally3Full, nb, nb, c_zero, c_zero, dV, ldda );
        ...
        magma_tally3blas_zlaset( Magma_tally3Full, nb, n, c_zero, c_zero, dT, nb );
        
        // Copy the matrix to the GPU
        magma_tally3_zsetmatrix( n, n-ilo, A(0,ilo), lda, dA, ldda );
        ...
        for (i = ilo; i < ihi-1 - nb; i += nb) {
            ...
            magma_tally3_zgetmatrix( ihi-i, nb,
                              dA(i,i-ilo), ldda,
                              A(i,i), lda );
            
            // add 1 to i for 1-based index
            magma_tally3_zlahr2( ihi, i+1, nb,
                          dA(0,i-ilo), ldda,
                          dV,          ldda,
                          A(0,i),      lda,
                          &tau[i], T, nb, work, n);
            ...
            dTi = dT + (i - ilo)*nb;
            magma_tally3_zsetmatrix( nb, nb, T, nb, dTi, nb );
            
            magma_tally3_zlahru( n, ihi, i, nb,
                          A(0,i),      lda,
                          dA(0,i-ilo), ldda, // dA
