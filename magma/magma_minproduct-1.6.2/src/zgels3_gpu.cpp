    ...
    magma_minproduct_int_t *info)
{
    ...
    *info = 0;
    ...
    k = min(m,n);
    if (k == 0) {
        hwork[0] = MAGMA_minproduct_Z_ONE;
        return *info;
    }
    ...
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc( &dT, ldtwork )) {
        *info = MAGMA_minproduct_ERR_DEVICE_ALLOC;
        return *info;
    }
    
    magma_minproduct_zmalloc_cpu( &tau, k );
    if ( tau == NULL ) {
    ...
    magma_minproduct_zgeqrf3_gpu( m, n, dA, ldda, tau, dT, info );
    if ( *info == 0 ) {
