...
#define dA(i,j) (dA + (i)*nb + (j)*nb*ldda)
    ...
    *info = 0;
    ...
    if (nb <= 1 || nb >= min(m,n)) {
        ...
        magma_tally3_dgetrf_nopiv( m, n, work, m, info);
        ...
        if (MAGMA_tally3_SUCCESS != magma_tally3_dmalloc_pinned( &work, maxm*nb )) {
            *info = MAGMA_tally3_ERR_HOST_ALLOC;
            return *info;
        }
        ...
        for( i=0; i < s; i++ ) {
            ...
            magma_tally3_dgetmatrix_async( m-i*nb, nb, dA(i,i), ldda, work, lddwork, stream[0] );
            
            if ( i > 0 ) {
                magma_tally3_dtrsm( Magma_tally3Left, Magma_tally3Lower, Magma_tally3NoTrans, Magma_tally3Unit,
                             nb, n - (i+1)*nb,
            ...
            magma_tally3_dgetrf_nopiv( rows, nb, work, lddwork, &iinfo );
            if ( (*info == 0) && (iinfo > 0) )
                *info = iinfo + i*nb;

            // upload i-th panel
            magma_tally3_dsetmatrix_async( m-i*nb, nb, work, lddwork, dA(i, i), ldda, stream[0] );
            magma_tally3_queue_sync( stream[0] );
        ...
        rows = m - s*nb;
        magma_tally3_dgetmatrix( rows, nb0, dA(s,s), ldda, work, lddwork );

        // make sure that gpu queue is empty
        magma_tally3_device_sync();

        // do the cpu part
        magma_tally3_dgetrf_nopiv( rows, nb0, work, lddwork, &iinfo );
        if ( (*info == 0) && (iinfo > 0) )
            *info = iinfo + s*nb;

        // upload i-th panel
        magma_tally3_dsetmatrix( rows, nb0, work, lddwork, dA(s,s), ldda );
