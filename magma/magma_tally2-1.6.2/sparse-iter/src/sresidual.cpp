    ...
    magma_tally2_queue_t queue )
{
    ...
    magma_tally2_s_matrix r={Magma_tally2_CSR};
    
    if ( A.num_rows == b.num_rows ) {
        CHECK( magma_tally2_svinit( &r, Magma_tally2_DEV, A.num_rows, b.num_cols, zero, queue ));

        CHECK( magma_tally2_s_spmv( one, A, x, zero, r, queue ));           // r = A x
        magma_tally2_saxpy(dofs, mone, b.dval, 1, r.dval, 1);          // r = r - b
        *res =  magma_tally2_snrm2(dofs, r.dval, 1);            // res = ||r||
        ...
        CHECK( magma_tally2_svinit( &r, Magma_tally2_DEV, b.num_rows,b.num_cols, zero, queue ));

        CHECK( magma_tally2_s_spmv( one, A, x, zero, r, queue ));           // r = A x

        for( magma_tally2_int_t i=0; i<num_vecs; i++) {
            magma_tally2_saxpy(dofs, mone, b(i), 1, r(i), 1);   // r = r - b
