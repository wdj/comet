...
magma_tally3_zheevx_gpu(
    ...
    double *rwork, magma_tally3_int_t *iwork, magma_tally3_int_t *ifail,
    magma_tally3_int_t *info)
{
    ...
    *info = 0;
    ...
    work[0] = MAGMA_tally3_Z_MAKE( lopt, 0 );
    ...
    *m = 0;
    ...
    if (n <= 128) {
        ...
        magma_tally3_zmalloc_cpu( &a, n*n );
        magma_tally3_zgetmatrix(n, n, dA, ldda, a, n);
        lapackf77_zheevx(jobz_, range_, uplo_,
                         &n, a, &n, &vl, &vu, &il, &iu, &abstol, m,
                         w, wZ, &ldwz, work, &lwork,
                         rwork, iwork, ifail, info);
        magma_tally3_zsetmatrix( n,  n,  a,    n, dA, ldda);
        magma_tally3_zsetmatrix( n, *m, wZ, ldwz, dZ, lddz);
        magma_tally3_free_cpu(a);
        return *info;
    }

    if (MAGMA_tally3_SUCCESS != magma_tally3_dmalloc( &dwork, n )) {
        fprintf (stderr, "!!!! device memory allocation error (magma_tally3_zheevx_gpu)\n");
    ...
    anrm = magma_tally3blas_zlanhe(Magma_tally3MaxNorm, uplo, n, dA, ldda, dwork);
    iscale = 0;
    sigma  = 1;
    if (anrm > 0. && anrm < rmin) {
        iscale = 1;
    ...
    if (iscale == 1) {
        d__1 = 1.;
        magma_tally3blas_zlascl(uplo, 0, 0, 1., sigma, n, n, dA,
                         ldda, info);
...
#ifdef FAST_HEMV
    magma_tally3_zhetrd2_gpu(uplo, n, dA, ldda, &rwork[indd], &rwork[inde],
                      &work[indtau], wA, ldwa, &work[indwrk], llwork, dZ, lddz*n, &iinfo);
#else
    magma_tally3_zhetrd_gpu (uplo, n, dA, ldda, &rwork[indd], &rwork[inde],
                      &work[indtau], wA, ldwa, &work[indwrk], llwork, &iinfo);
#endif

    lopt = n + (magma_tally3_int_t)MAGMA_tally3_Z_REAL(work[indwrk]);
    ...
    if ((alleig || (indeig && il == 1 && iu == n)) && abstol <= 0.) {
        ...
        if (! wantz) {
            ...
            if (*info == 0) {
                ...
                magma_tally3_zsetmatrix( n, n, wZ, ldwz, dZ, lddz );
            }
        }
        if (*info == 0) {
            *m = n;
    ...
    if (*m == 0) {
        *info = 0;
        ...
        if (wantz) {
            ...
            magma_tally3_zunmtr_gpu(Magma_tally3Left, uplo, Magma_tally3NoTrans, n, *m, dA, ldda, &work[indtau],
    ...
    if (wantz) {
        for (j = 1; j <= *m-1; ++j) {
            ...
            if (i != 0) {
                ...
                magma_tally3_zswap(n, dZ + (i-1)*lddz, ione, dZ + (j-1)*lddz, ione);
                if (*info != 0) {
                    itmp1 = ifail[i];
                    ifail[i] = ifail[j];
                    ifail[j] = itmp1;
    ...
    work[1] = MAGMA_tally3_Z_MAKE( lopt, 0 );
