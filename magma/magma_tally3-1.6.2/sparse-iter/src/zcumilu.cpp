    ...
    magma_tally3_queue_t queue )
{
    ...
    magma_tally3_z_matrix hA={Magma_tally3_CSR}, hACSR={Magma_tally3_CSR};
    magma_tally3_z_matrix hL={Magma_tally3_CSR}, hU={Magma_tally3_CSR};
    CHECK( magma_tally3_zmtransfer( A, &hA, A.memory_location, Magma_tally3_CPU, queue ));
    CHECK( magma_tally3_zmconvert( hA, &hACSR, hA.storage_type, Magma_tally3_CSR, queue ));

        // in case using fill-in
    if( precond->levels > 0 ){
        magma_tally3_z_matrix hAL={Magma_tally3_CSR}, hAUt={Magma_tally3_CSR};
        CHECK( magma_tally3_zsymbilu( &hACSR, precond->levels, &hAL, &hAUt,  queue ));
        magma_tally3_zmfree(&hAL, queue);
        magma_tally3_zmfree(&hAUt, queue);
    }

    CHECK( magma_tally3_zmtransfer(hACSR, &(precond->M), Magma_tally3_CPU, Magma_tally3_DEV, queue ));

    magma_tally3_zmfree( &hA, queue );
    magma_tally3_zmfree( &hACSR, queue );
    ...
    CHECK( magma_tally3_zmtransfer( precond->M, &hA, Magma_tally3_DEV, Magma_tally3_CPU, queue ));

    hL.diagorder_type = Magma_tally3_UNITY;
    CHECK( magma_tally3_zmconvert( hA, &hL , Magma_tally3_CSR, Magma_tally3_CSRL, queue ));
    hU.diagorder_type = Magma_tally3_VALUE;
    CHECK( magma_tally3_zmconvert( hA, &hU , Magma_tally3_CSR, Magma_tally3_CSRU, queue ));
    CHECK( magma_tally3_zmtransfer( hL, &(precond->L), Magma_tally3_CPU, Magma_tally3_DEV, queue ));
    CHECK( magma_tally3_zmtransfer( hU, &(precond->U), Magma_tally3_CPU, Magma_tally3_DEV, queue ));
    ...
    magma_tally3_queue_t queue )
{
    ...
    magma_tally3_z_matrix hA={Magma_tally3_CSR}, hL={Magma_tally3_CSR}, hU={Magma_tally3_CSR};
    
    if (precond->L.memory_location != Magma_tally3_DEV ){
        
        CHECK( magma_tally3_zmtransfer( precond->M, &hA,
    ...
    if( precond->maxiter < 50 ){
        ...
        // extract the diagonal of L into precond->d
        CHECK( magma_tally3_zjacobisetup_diagscal( precond->L, &precond->d, queue ));
        CHECK( magma_tally3_zvinit( &precond->work1, Magma_tally3_DEV, precond->U.num_rows, 1, MAGMA_tally3_Z_ZERO, queue ));
        
        // extract the diagonal of U into precond->d2
        CHECK( magma_tally3_zjacobisetup_diagscal( precond->U, &precond->d2, queue ));
        CHECK( magma_tally3_zvinit( &precond->work2, Magma_tally3_DEV, precond->U.num_rows, 1, MAGMA_tally3_Z_ZERO, queue ));
    }
    ...
    magma_tally3_queue_t queue )
{
    ...
    magma_tally3_z_matrix hA={Magma_tally3_CSR}, hACSR={Magma_tally3_CSR}, U={Magma_tally3_CSR};
    CHECK( magma_tally3_zmtransfer( A, &hA, A.memory_location, Magma_tally3_CPU, queue ));
    ...
    CHECK( magma_tally3_zmconvert( hACSR, &U, Magma_tally3_CSR, Magma_tally3_CSRL, queue ));
    magma_tally3_zmfree( &hACSR, queue );
    CHECK( magma_tally3_zmtransfer(U, &(precond->M), Magma_tally3_CPU, Magma_tally3_DEV, queue ));
...
/*
    ...
    magma_tally3_z_matrix hD, hR, hAt

    CHECK( magma_tally3_zcsrsplit( 256, hA, &hD, &hR, queue ));

    CHECK( magma_tally3_zmtransfer( hD, &precond->LD, Magma_tally3_CPU, Magma_tally3_DEV, queue ));
    CHECK( magma_tally3_zmtransfer( hR, &precond->L, Magma_tally3_CPU, Magma_tally3_DEV, queue ));

    magma_tally3_zmfree(&hD, queue );
    magma_tally3_zmfree(&hR, queue );

    CHECK( magma_tally3_z_cucsrtranspose(   hA, &hAt, queue ));

    CHECK( magma_tally3_zcsrsplit( 256, hAt, &hD, &hR, queue ));

    CHECK( magma_tally3_zmtransfer( hD, &precond->UD, Magma_tally3_CPU, Magma_tally3_DEV, queue ));
    CHECK( magma_tally3_zmtransfer( hR, &precond->U, Magma_tally3_CPU, Magma_tally3_DEV, queue ));
    
    magma_tally3_zmfree(&hD, queue );
    magma_tally3_zmfree(&hR, queue );
    magma_tally3_zmfree(&hA, queue );
    magma_tally3_zmfree(&hAt, queue );
*/
