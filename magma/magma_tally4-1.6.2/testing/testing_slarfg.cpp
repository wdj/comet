...
int main( int argc, char** argv)
{
    ...
    magma_tally4Float_ptr d_x, d_tau;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            /* =====================================================================
               ...
               =================================================================== */
            magma_tally4_ssetmatrix( N, nb, h_x, N, d_x, ldda );
    
            gpu_time = magma_tally4_sync_wtime( queue );
            for( int j = 0; j < nb; ++j ) {
                magma_tally4blas_slarfg( N, &d_x[0+j*ldda], &d_x[1+j*ldda], ione, &d_tau[j] );
            }
            gpu_time = magma_tally4_sync_wtime( queue ) - gpu_time;
            gpu_perf = gflops / gpu_time;
            
            magma_tally4_sgetmatrix( N, nb, d_x, ldda, h_x2, N );
            magma_tally4_sgetvector( nb, d_tau, 1, h_tau2, 1 );
