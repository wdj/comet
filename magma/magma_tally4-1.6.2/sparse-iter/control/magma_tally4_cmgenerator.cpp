/*
    -- MAGMA_tally4 (version 1.6.2) --
       ...
       @generated from magma_tally4_zmgenerator.cpp normal z -> c, Sun May  3 11:23:01 2015
       @author Hartwig Anzt
*/
...
/**
    ...
    @param[in]
    offdiags    magma_tally4_int_t
                number of offdiagonals

    @param[in]
    diag_offset magma_tally4_int_t*
                array containing the offsets

                                                (length offsets+1)
    @param[in]
    diag_vals   magma_tally4FloatComplex*
    ...
    magma_tally4_queue_t queue )
{
    ...
    B.num_rows = n;
    B.num_cols = n;
    B.fill_mode = Magma_tally4_FULL;
    B.memory_location = Magma_tally4_CPU;
    B.storage_type = Magma_tally4_ELLPACKT;
    B.max_nnz_row = (2*offdiags+1);

    CHECK( magma_tally4_cmalloc_cpu( &B.val, B.max_nnz_row*n ));
    CHECK( magma_tally4_index_malloc_cpu( &B.col, B.max_nnz_row*n ));
    ...
    for( int i=0; i<n; i++ ) { // stride over rows
        for( int j=0; j<B.max_nnz_row; j++ ) { // nonzeros in every row
            if ( (B.col[i*B.max_nnz_row + j] < 0) ||
                ...
                B.val[ i*B.max_nnz_row + j ] = MAGMA_tally4_C_MAKE( 0.0, 0.0 );
            }
        }
    ...
    for( int i=0; i<n; i++ ) { // stride over rows
        for( int j=0; j<B.max_nnz_row; j++ ) { // nonzeros in every row
            if ( MAGMA_tally4_C_REAL( B.val[i*B.max_nnz_row + j]) != 0.0 )
                B.nnz++;
        }
    ...
    CHECK( magma_tally4_cmconvert( B, A, Magma_tally4_ELLPACKT, Magma_tally4_CSR, queue ));
    ...
    magma_tally4_queue_t queue )
{
    ...
    magma_tally4_c_matrix hA={Magma_tally4_CSR};

    
    // generate matrix of desired structure and size (3d 27-point stencil)
    magma_tally4_int_t nn = n*n*n;
    magma_tally4_int_t offdiags = 13;
    magma_tally4_index_t *diag_offset=NULL;
    ...
    diag_vals[0] = MAGMA_tally4_C_MAKE( 26.0, 0.0 );
    diag_vals[1] = MAGMA_tally4_C_MAKE( -1.0, 0.0 );
    diag_vals[2] = MAGMA_tally4_C_MAKE( -1.0, 0.0 );
    diag_vals[3] = MAGMA_tally4_C_MAKE( -1.0, 0.0 );
    diag_vals[4] = MAGMA_tally4_C_MAKE( -1.0, 0.0 );
    diag_vals[5] = MAGMA_tally4_C_MAKE( -1.0, 0.0 );
    diag_vals[6] = MAGMA_tally4_C_MAKE( -1.0, 0.0 );
    diag_vals[7] = MAGMA_tally4_C_MAKE( -1.0, 0.0 );
    diag_vals[8] = MAGMA_tally4_C_MAKE( -1.0, 0.0 );
    diag_vals[9] = MAGMA_tally4_C_MAKE( -1.0, 0.0 );
    diag_vals[10] = MAGMA_tally4_C_MAKE( -1.0, 0.0 );
    diag_vals[11] = MAGMA_tally4_C_MAKE( -1.0, 0.0 );
    diag_vals[12] = MAGMA_tally4_C_MAKE( -1.0, 0.0 );
    diag_vals[13] = MAGMA_tally4_C_MAKE( -1.0, 0.0 );
    CHECK( magma_tally4_cmgenerator( nn, offdiags, diag_offset, diag_vals, &hA, queue ));


    // now set some entries to zero (boundary...)
    for( i=0; i<n*n; i++ ) {
    for( j=0; j<n; j++ ) {
        magma_tally4_index_t row = i*n+j;
        for( k=hA.row[row]; k<hA.row[row+1]; k++) {
            ...
            if ((hA.col[k] == row-1 ||
                ...
                hA.col[k] == row+n*n+n-1 ) && (row+1)%n == 1 )
                    ...
                    hA.val[k] = MAGMA_tally4_C_MAKE( 0.0, 0.0 );
    ...
    CHECK( magma_tally4_cmconvert( hA, A, Magma_tally4_CSR, Magma_tally4_CSR, queue ));

cleanup:
    magma_tally4_free_cpu( diag_vals );
    magma_tally4_free_cpu( diag_offset );
    ...
    magma_tally4_queue_t queue )
{
    ...
    magma_tally4_int_t nn = n*n;
    magma_tally4_int_t offdiags = 2;
    magma_tally4_index_t *diag_offset=NULL;
    magma_tally4FloatComplex *diag_vals=NULL;
    CHECK( magma_tally4_cmalloc_cpu( &diag_vals, offdiags+1 ));
    CHECK( magma_tally4_index_malloc_cpu( &diag_offset, offdiags+1 ));
    ...
    #ifdef COMPLEX
        // complex case
        diag_vals[0] = MAGMA_tally4_C_MAKE( 4.0, 4.0 );
        diag_vals[1] = MAGMA_tally4_C_MAKE( -1.0, -1.0 );
        diag_vals[2] = MAGMA_tally4_C_MAKE( -1.0, -1.0 );
        
    #else
        // real case
        diag_vals[0] = MAGMA_tally4_C_MAKE( 4.0, 0.0 );
        diag_vals[1] = MAGMA_tally4_C_MAKE( -1.0, 0.0 );
        diag_vals[2] = MAGMA_tally4_C_MAKE( -1.0, 0.0 );
    #endif
    CHECK( magma_tally4_cmgenerator( nn, offdiags, diag_offset, diag_vals, &hA, queue ));
