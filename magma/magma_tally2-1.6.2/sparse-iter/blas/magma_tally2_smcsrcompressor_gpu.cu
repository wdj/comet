/*
    -- MAGMA_tally2 (version 1.6.2) --
       ...
       @generated from magma_tally2_zmcsrcompressor_gpu.cu normal z -> s, Sun May  3 11:22:58 2015
       @author Hartwig Anzt
...
__global__ void
magma_tally2_smcsrgpu_kernel1( int num_rows,
                 float *A_val,
    ...
    if(row<num_rows){
        float zero = MAGMA_tally2_S_ZERO;
...
__global__ void
magma_tally2_smcsrgpu_kernel2( int num_rows,
                 magma_tally2_index_t *B_rowptr,
                 magma_tally2_index_t *A_rowptr ){
...
__global__ void
magma_tally2_smcsrgpu_kernel3( int num_rows,
                 float *B_val,
                 magma_tally2_index_t *B_rowptr,
                 magma_tally2_index_t *B_colind,
    ...
    magma_tally2_queue_t queue )
{
    ...
    magma_tally2_s_matrix B={Magma_tally2_CSR}, B2={Magma_tally2_CSR};
    magma_tally2_s_matrix dA={Magma_tally2_CSR}, CSRA={Magma_tally2_CSR};
    ...
    if ( A->memory_location == Magma_tally2_DEV && A->storage_type == Magma_tally2_CSR ) {
        ...
        magma_tally2_smcsrgpu_kernel1<<< grid1, BLOCK_SIZE1, 0, queue >>>
                ( A->num_rows, A->dval, A->drow, A->dcol, B.dval, B.drow, B.dcol );

        // correct the row pointer
        dim3 grid2( 1, 1, 1);
        magma_tally2_smcsrgpu_kernel2<<< grid2, BLOCK_SIZE2, 0, queue >>>
        ...
        magma_tally2_smcsrgpu_kernel3<<< grid1, BLOCK_SIZE1, 0, queue >>>
        ...
        CHECK( magma_tally2_smconvert( *A, &CSRA, A->storage_type, Magma_tally2_CSR, queue ));
        CHECK( magma_tally2_smtransfer( *A, &dA, A->memory_location, Magma_tally2_DEV, queue ));

        CHECK( magma_tally2_smcsrcompressor_gpu( &dA, queue ));

        magma_tally2_smfree( &dA, queue );
        magma_tally2_smfree( A, queue );
        CHECK( magma_tally2_smtransfer( dA, &CSRA, Magma_tally2_DEV, A_location, queue ));
        CHECK( magma_tally2_smconvert( CSRA, A, Magma_tally2_CSR, A_storage, queue ));
        magma_tally2_smfree( &dA, queue );
