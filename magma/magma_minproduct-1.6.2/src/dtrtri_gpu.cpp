...
#define dA(i, j) (dA+(j)*ldda + (i))
    ...
    *info = 0;
    ...
    if (nounit) {
        for (j=0; j < n; ++j) {
            if ( MAGMA_minproduct_D_EQUAL( *dA(j,j), c_zero )) {
    ...
    if (nb <= 1 || nb >= n) {
        ...
        if (upper) {
            ...
            for (j=0; j < n; j += nb) {
                ...
                lapackf77_dtrtri( Magma_minproductUpperStr, diag_, &jb, work, &jb, info );
            ...
            for (j=nn-1; j >= 0; j -= nb) {
                ...
                /* Compute inverse of current diagonal block */
                lapackf77_dtrtri( Magma_minproductLowerStr, diag_, &jb, work, &jb, info );
