...
int main( int argc, char** argv )
{
    ...
    double *h_A, *h_dinvA;
    magma_minproductDouble_ptr d_A, d_dinvA;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_minproductblas_dtrtri_diag( opts.uplo, opts.diag, N, d_A, ldda, d_dinvA );
            ...
            magma_minproduct_dgetvector( size_inv, d_dinvA, 1, h_dinvA, 1 );
            
            if ( opts.verbose ) {
                printf( "A%d=", (int) N );
                magma_minproduct_dprint( N, N, h_A, lda );
                printf( "d_dinvA%d=", (int) N );
                magma_minproduct_dprint( min(N+4, nb), min(N+4, nblock*nb), h_dinvA, nb );
            }
            ...
            if ( opts.check ) {
                ...
                for( int i=0; i < N; i += nb ) {
                    ...
                    magma_minproduct_error = max( magma_minproduct_error, lapackf77_dlantr( "M", uplo_, Magma_minproductNonUnitStr, &jb, &jb, h_dinvA(0, i), &nb,  work ));
                    norm_invA   = max( norm_invA,   lapackf77_dlantr( "M", uplo_, Magma_minproductNonUnitStr, &jb, &jb, h_A(i, i),     &lda, work ));
                }
