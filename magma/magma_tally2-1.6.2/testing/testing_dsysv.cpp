...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( opts.lapack ) {
                lwork = -1;
                lapackf77_dsysv(lapack_uplo_const_tally2(opts.uplo), &N, &opts.nrhs, 
                ...
                lapackf77_dlacpy( Magma_tally2UpperLowerStr, &N, &opts.nrhs, h_B, &ldb, h_X, &ldb );
            ...
            magma_tally2_dsysv( opts.uplo, N, opts.nrhs, h_A, lda, ipiv, h_X, ldb, &info);
            gpu_time = magma_tally2_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally2_dsysv returned error %d: %s.\n",
                       (int) info, magma_tally2_strerror( info ));
