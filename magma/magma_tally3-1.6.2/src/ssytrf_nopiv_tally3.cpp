/*
    -- MAGMA_tally3 (version 1.6.1) --
       ...
       @generated from zhetrf_nopiv_tally3.cpp normal z -> s, Fri Jan 30 19:00:16 2015
*/
    ...
    magma_tally3_int_t *info)
{
    ...
    *info = 0;
    ...
    if ((MAGMA_tally3_SUCCESS != magma_tally3_smalloc(&dA, n *ldda)) ||
        (MAGMA_tally3_SUCCESS != magma_tally3_smalloc(&dW, nb*ldda))) {
    ...
    if (upper) {
        ...
        for (j=0; j<n; j+=nb) {
            ...
            magma_tally3_ssetmatrix_async(j+jb, jb, A(0, j), lda, dA(0, j), ldda, stream[0]);
            trace_gpu_end( 0, 0 );
        }
        ...
        for (j=0; j<n; j += nb) {
            ...
            if ( j!=0) {
                ...
                magma_tally3_sgetmatrix_async(jb, jb, dA(j, j), ldda, A(j,j), lda, stream[1]);
            }
            trace_gpu_end( 0, 0 );
            ...
            ssytrf_nopiv_tally3_cpu(Magma_tally3Upper, jb, ib, A(j, j), lda, info);
            trace_cpu_end( 0 );
            if (*info != 0){
                *info = *info + j;
                break;
            ...
            trace_gpu_start( 0, 0, "set", "set" );
            magma_tally3_ssetmatrix_async(jb, jb, A(j, j), lda, dA(j, j), ldda, stream[0]);
            trace_gpu_end( 0, 0 );
            
            // copy j-th column of U back to CPU
            trace_gpu_start( 0, 1, "get", "get" );                                                                         
            magma_tally3_sgetmatrix_async(j, jb, dA(0, j), ldda, A(0, j), lda, stream[1]);                                        
            trace_gpu_end( 0, 1 );
        ...
        for (j=0; j<n; j+=nb) {
            jb = min(nb, (n-j));
            trace_gpu_start( 0, 0, "set", "set" );
            magma_tally3_ssetmatrix_async((n-j), jb, A(j, j), lda, dA(j, j), ldda, stream[0]);
            trace_gpu_end( 0, 0 );
        }
        ...
        for (j=0; j<n; j+=nb) {
            ...
            magma_tally3_queue_sync(stream[1]);
            trace_cpu_start( 0, "potrf", "potrf" );
            ssytrf_nopiv_tally3_cpu(Magma_tally3Lower, jb, ib, A(j, j), lda, info);
            trace_cpu_end( 0 );
            if (*info != 0){
                *info = *info + j;
                break;
            ...
            trace_gpu_start( 0, 1, "get", "get" );
            magma_tally3_sgetmatrix_async(jb, j, dA(j, 0), ldda, A(j, 0), lda, stream[1]);
            trace_gpu_end( 0, 1 ); 
