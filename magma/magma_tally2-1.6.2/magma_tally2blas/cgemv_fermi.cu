...
cgemvn_kernel1_fermi(
    ...
    const magma_tally2FloatComplex * __restrict__ A, int lda,
    const magma_tally2FloatComplex * __restrict__ x, int incx, magma_tally2FloatComplex beta,
    ...
    magma_tally2FloatComplex       * __restrict__ y, int incy)
{
    ...
    __shared__ magma_tally2FloatComplex sx[BLK_Y];
    ...
    if ( threadIdx.x < npart ) {
        ...
        sx[threadIdx.x] = MAGMA_tally2_C_ZERO;
    }
    __syncthreads();
    ...
    magma_tally2FloatComplex       * __restrict__ y, int incy)
{
    ...
    __shared__ magma_tally2FloatComplex sdata[BLK_X];
    ...
    magma_tally2_sum_reduce< BLK_X >( tx, sdata );
...
/**
    ...
    trans   magma_tally2_trans_t
            ...
            follows:
      -     = Magma_tally2NoTrans:    y := alpha*A  *x + beta*y
      -     = Magma_tally2Trans:      y := alpha*A^T*x + beta*y
      -     = Magma_tally2ConjTrans:  y := alpha*A^H*x + beta*y
...
magma_tally2blas_cgemv(
    magma_tally2_trans_t trans, magma_tally2_int_t m, magma_tally2_int_t n, magma_tally2FloatComplex alpha,
    ...
    magma_tally2FloatComplex_ptr dy, magma_tally2_int_t incy)
{
    ...
    if ( arch < 200  ) {
        ...
        magma_tally2_cgemv( trans, m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
        #else
        magma_tally2blas_cgemv_tesla( trans, m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
        #endif
        return;
    }
    ...
    if ( trans == Magma_tally2NoTrans ) {
        ...
        cgemvn_kernel1_fermi<<< grid, threads, 0, magma_tally2_stream >>>
            ( m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    }
    else if ( trans == Magma_tally2Trans ) {
        dim3 grid    ( 1, n, 1 );
        dim3 threads ( BLK_X, 1, 1 );
        cgemvt_kernel_fermi<<< grid, threads, 0, magma_tally2_stream >>>
            ( m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    }
    else if ( trans == Magma_tally2ConjTrans ) {
        dim3 grid    ( 1, n, 1 );
        dim3 threads ( BLK_X, 1, 1 );
        cgemvc_kernel_fermi<<< grid, threads, 0, magma_tally2_stream >>>
            ( m, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
    }
}
