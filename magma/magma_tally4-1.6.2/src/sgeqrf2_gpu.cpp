/*
    -- MAGMA_tally4 (version 1.6.1) --
       ...
       @author Stan Tomov
       @generated from zgeqrf2_gpu.cpp normal z -> s, Fri Jan 30 19:00:15 2015
...
/**
    ...
    Other versions (magma_tally4_sgeqrf_gpu and magma_tally4_sgeqrf3_gpu) store the
    ...
    magma_tally4_int_t *info )
{
    ...
    float *work;
    magma_tally4_int_t i, k, ldwork, lddwork, old_i, old_ib, rows;
    magma_tally4_int_t nbmin, nx, ib, nb;
    ...
    *info = 0;
    ...
    if (MAGMA_tally4_SUCCESS != magma_tally4_smalloc( &dwork, n*nb )) {
        *info = MAGMA_tally4_ERR_DEVICE_ALLOC;
        return *info;
    }

    if (MAGMA_tally4_SUCCESS != magma_tally4_smalloc_pinned( &work, lwork )) {
    ...
    if (nb >= nbmin && nb < k && nx < k) {
        ...
        for (i = 0; i < k-nx; i += nb) {
            ...
            if (i > 0) {
                ...
                magma_tally4_ssetmatrix_async( old_ib, old_ib,
            ...
            spanel_to_q_tally4( Magma_tally4Upper, ib, work_ref(i), ldwork, hwork+ib*ib );

            /* download the i-th V matrix */
            magma_tally4_ssetmatrix_async( rows, ib, work_ref(i), ldwork, dA(i,i), ldda, stream[0] );

            /* download the T matrix */
            magma_tally4_queue_sync( stream[1] );
            magma_tally4_ssetmatrix_async( ib, ib, hwork, ib, dwork, lddwork, stream[0] );
            magma_tally4_queue_sync( stream[0] );
            ...
            if (i + ib < n) {
                if (i+nb < k-nx) {
                    ...
                    sq_to_panel_tally4( Magma_tally4Upper, ib, work_ref(i), ldwork, hwork+ib*ib );
                }
                else {
                    magma_tally4_slarfb_gpu( Magma_tally4Left, Magma_tally4ConjTrans, Magma_tally4Forward, Magma_tally4Columnwise,
                                      rows, n-i-ib, ib,
    ...
    if (i < k) {
        ib   = n-i;
        rows = m-i;
        magma_tally4_sgetmatrix_async( rows, ib, dA(i, i), ldda, work, rows, stream[1] );
        magma_tally4_queue_sync( stream[1] );
        lhwork = lwork - rows*ib;
        lapackf77_sgeqrf(&rows, &ib, work, &rows, tau+i, work+ib*rows, &lhwork, info);
        
        magma_tally4_ssetmatrix_async( rows, ib, work, rows, dA(i, i), ldda, stream[1] );
    }
