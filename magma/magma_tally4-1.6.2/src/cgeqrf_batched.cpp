...
#define dA(i, j)  (dA + (i) + (j)*ldda)   // A(i, j) means at i row, j column
    ...
    magma_tally4FloatComplex *dwork = NULL;
    magma_tally4FloatComplex *dT   = NULL;
    magma_tally4FloatComplex *dR   = NULL;
    magma_tally4FloatComplex **dR_array = NULL;
    magma_tally4FloatComplex **dT_array = NULL; 
    magma_tally4FloatComplex **cpuAarray = NULL;
    magma_tally4FloatComplex **cpuTarray = NULL;
    ...
    magma_tally4_malloc((void**)&dW4_displ, batchCount * sizeof(*dW4_displ));  // used in clarfb
    ...
    magma_tally4_cmalloc(&dwork,  (2 * nb * n) * batchCount);
    magma_tally4_cmalloc(&dR,  ldr * n   * batchCount);
    magma_tally4_cmalloc(&dT,  ldt * ldt * batchCount);
    magma_tally4_malloc_cpu((void**) &cpuAarray, batchCount*sizeof(magma_tally4FloatComplex*));
    magma_tally4_malloc_cpu((void**) &cpuTarray, batchCount*sizeof(magma_tally4FloatComplex*));
    ...
    magma_tally4blas_claset_q(Magma_tally4Full, ldr, n*batchCount  , MAGMA_tally4_C_ZERO, MAGMA_tally4_C_ZERO, dR, ldr, queue);
    magma_tally4blas_claset_q(Magma_tally4Full, ldt, ldt*batchCount, MAGMA_tally4_C_ZERO, MAGMA_tally4_C_ZERO, dT, ldt, queue);
    ...
    magma_tally4_getvector( batchCount, sizeof(magma_tally4FloatComplex*), dT_array, 1, cpuTarray, 1);
    ...
    for(i=0; i<min_mn;i+=nb)
    {
            ...
            magma_tally4_cdisplace_pointers(dW0_displ, dA_array, ldda, i, i, batchCount, queue); 
            magma_tally4_cdisplace_pointers(dW2_displ, tau_array, 1, i, 0, batchCount, queue);
            ...
            magma_tally4_cgeqrf_panel_batched(m-i, ib, jb, 
                                       dW0_displ, ldda, 
                                       dW2_displ, 
                                       dT_array, ldt, 
                                       dR_array, ldr,
           ...
           cgeqrf_copy_upper_batched(ib, jb, dW0_displ, ldda, dR_array, ldr, batchCount, queue);
            ...
            if( (n-ib-i) > 0)
            {
                ...
                magma_tally4blas_claset_batched(Magma_tally4Upper, ib, ib, MAGMA_tally4_C_ZERO, MAGMA_tally4_C_ONE, dW0_displ, ldda, batchCount, queue); 
                magma_tally4_cdisplace_pointers(dW2_displ, tau_array, 1, i, 0, batchCount, queue); 

                // it is faster since it is using BLAS-3 GEMM routines, different from lapack implementation 
                magma_tally4_clarft_batched(m-i, ib, 0,
                                 dW0_displ, ldda,
                                 dW2_displ,
                                 dT_array, ldt, 
                                 dW4_displ, nb*ldt,
                ...
                { 
                    ...
                    for(k=0; k<batchCount; k++)
                    {
                        ...
                        magma_tally4_clarfb_gpu_gemm(Magma_tally4Left, Magma_tally4ConjTrans, Magma_tally4Forward, Magma_tally4Columnwise,
                                    m-i, n-i-ib, ib,
                ...
                else
                {
                    ...
                    magma_tally4_clarfb_gemm_batched(
                                ...
                                m-i, n-i-ib, ib,
                                (const magma_tally4FloatComplex**)dW0_displ, ldda,
                                (const magma_tally4FloatComplex**)dT_array, ldt,
                                dW1_displ,  ldda,
                                dW4_displ,  ldw,
                                dW5_displ, ldw,
                                batchCount, myhandle, queue);
            ...
            magma_tally4blas_clacpy_batched(Magma_tally4Upper, ib, ib, dR_array, ldr, dW0_displ, ldda, batchCount, queue);
