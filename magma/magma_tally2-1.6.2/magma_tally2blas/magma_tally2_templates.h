/*
    -- MAGMA_tally2 (version 1.6.1) --
       ...
       @author Mark Gates
*/
#ifndef MAGMA_tally2_TEMPLATES_H
#define MAGMA_tally2_TEMPLATES_H


// ----------------------------------------
template< int n, typename T, typename ID >
__device__ void 
magma_tally2_getidmax( /*int n,*/ int i, T* x, ID* ind )
{
    __syncthreads();
...
// end magma_tally2_getidmax2

template< typename T, typename ID >
__device__ void 
magma_tally2_getidmax_n( int n, int i, T* x, ID* ind )
{
    __syncthreads();
...
__device__ void
magma_tally2_max_reduce( /*int n,*/ int i, T* x )
{
    __syncthreads();
...
// ----------------------------------------
/// Same as magma_tally2_max_reduce, but propogates nan values.
///
/// Does max reduction of n-element array x, leaving total in x[0].
/// Contents of x are destroyed in the process.
/// With k threads, can reduce array up to 2*k in size.
...
template< int n, typename T >
__device__ void
magma_tally2_max_nan_reduce( /*int n,*/ int i, T* x )
{
    __syncthreads();
...
// ----------------------------------------
/// Same as magma_tally2_max_nan_reduce,
/// but takes n as runtime argument instead of compile-time template parameter.
template< typename T >
__device__ void
magma_tally2_max_nan_reduce_n( int n, int i, T* x )
{
    __syncthreads();
    if ( n > 1024 ) { if ( i < 1024 && i + 1024 < n ) { x[i] = max_nan( x[i], x[i+1024] ); }  __syncthreads(); }
    if ( n >  512 ) { if ( i <  512 && i +  512 < n ) { x[i] = max_nan( x[i], x[i+ 512] ); }  __syncthreads(); }
...
template< int n, typename T >
__device__ void
magma_tally2_sum_reduce( /*int n,*/ int i, T* x )
{
    __syncthreads();
...
// ----------------------------------------
/// Same as magma_tally2_sum_reduce,
/// but takes n as runtime argument instead of compile-time template parameter.
template< typename T >
__device__ void
magma_tally2_sum_reduce_n( int n, int i, T* x )
{
    __syncthreads();
    if ( n > 1024 ) { if ( i < 1024 && i + 1024 < n ) { x[i] += x[i+1024]; }  __syncthreads(); }
    if ( n >  512 ) { if ( i <  512 && i +  512 < n ) { x[i] += x[i+ 512]; }  __syncthreads(); }
...
__device__ void
magma_tally2_sum_reduce_2d( int i, int j, T x[m][n] )
{
    __syncthreads();
...
__device__ void
magma_tally2_sum_reduce_3d( int i, int j, int k, T x[m0][m1][m2] )
{
    __syncthreads();
...
#endif        //  #ifndef MAGMA_tally2_TEMPLATES_H
