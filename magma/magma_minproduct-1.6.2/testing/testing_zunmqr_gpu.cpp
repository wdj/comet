...
int main( int argc, char** argv )
{
    ...
    magma_minproductDoubleComplex *C, *R, *A, *W, *tau;
    magma_minproductDoubleComplex_ptr dC, dA, dT;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int itran = 0; itran < 2; ++itran ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {        
            ...
            TESTING_MALLOC_CPU( A,   magma_minproductDoubleComplex, lda*k );
            TESTING_MALLOC_CPU( W,   magma_minproductDoubleComplex, lwork_max );
            TESTING_MALLOC_CPU( tau, magma_minproductDoubleComplex, k );
            
            TESTING_MALLOC_DEV( dC, magma_minproductDoubleComplex, ldc*n );
            TESTING_MALLOC_DEV( dA, magma_minproductDoubleComplex, lda*k );
            TESTING_MALLOC_DEV( dT, magma_minproductDoubleComplex, dt_size );
            ...
            magma_minproduct_zsetmatrix( lda, k, A,  lda, dA, lda );
            magma_minproduct_zgeqrf_gpu( lda, k, dA, lda, tau, dT, &info );
            magma_minproduct_zgetmatrix( lda, k, dA, lda, A,  lda );
            if (info != 0)
            ...
            lwork = -1;
            magma_minproduct_zunmqr_gpu( side[iside], trans[itran],
                              m, n, k,
                              dA, lda, tau, dC, ldc, W, lwork, dT, nb, &info );
            if (info != 0)
                printf("magma_minproduct_zunmqr_gpu (lwork query) returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
            lwork = (magma_minproduct_int_t) MAGMA_minproduct_Z_REAL( W[0] );
            ...
            if (info != 0)
                printf("magma_minproduct_zunmqr_gpu returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
