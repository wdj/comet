...
/**
    ...
    This is an out-of-core (ooc) version that is similar to magma_minproduct_zgeqrf but
    ...
    magma_minproduct_int_t *info )
{
    ...
    magma_minproductDoubleComplex *dA, *dwork;
    magma_minproductDoubleComplex c_one = MAGMA_minproduct_Z_ONE;

    int  k, lddwork, ldda;

    *info = 0;
    int nb = magma_minproduct_get_zgeqrf_nb(min(m, n));
    ...
    if (NB >= n)
        return magma_minproduct_zgeqrf(m, n, A, lda, tau, work, lwork, info);
    ...
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc( &dA, (NB + nb)*ldda + nb*lddwork )) {
    ...
    magma_minproductDoubleComplex *ptr = dA + ldda * NB;
    dwork = dA + ldda*(NB + nb);
    ...
    for (int i=0; i < n; i += NB) {
        ...
        magma_minproduct_zsetmatrix_async( (m), IB,
                                A(0,i),  lda,
        ...
        for (int j=0; j < min(i,k); j += nb) {
            ...
            zpanel_to_q(Magma_minproductUpper, ib, A(j,j), lda, work+ib*ib);
            ...
            zq_to_panel(Magma_minproductUpper, ib, A(j,j), lda, work+ib*ib);
        }

        /* 3. Do a QR on the current part */
        if (i < k)
            magma_minproduct_zgeqrf2_gpu(m-i, IB, dA(i,0), ldda, tau+i, info);

        /* 4. Copy the current part back to the CPU */
        magma_minproduct_zgetmatrix_async( (m), IB,
                                dA(0,0), ldda,
                                A(0,i),  lda, stream[0] );
    }
