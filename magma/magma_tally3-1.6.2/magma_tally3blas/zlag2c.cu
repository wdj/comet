...
void zlag2c_kernel(
    ...
    magma_tally3FloatComplex *SA,       int ldsa,
    double rmax )
{
    ...
    if ( ind < m ) {
        ...
        if ( full ) {
            ...
            for( int j=0; j < BLK_Y; ++j ) {
                tmp = A[j*lda];
                if (   (MAGMA_tally3_Z_REAL(tmp) < neg_rmax) || (MAGMA_tally3_Z_REAL(tmp) > rmax)
                    #if defined(PRECISION_z) || defined(PRECISION_c)
                    || (MAGMA_tally3_Z_IMAG(tmp) < neg_rmax) || (MAGMA_tally3_Z_IMAG(tmp) > rmax)
                    #endif
                    )
                {
                    flag = 1;
                }
                SA[j*ldsa] = MAGMA_tally3_C_MAKE( MAGMA_tally3_Z_REAL(tmp), MAGMA_tally3_Z_IMAG(tmp) );
            }
        }
        else {
...
magma_tally3blas_zlag2c(
    ...
    magma_tally3_int_t *info )
{
    magma_tally3blas_zlag2c_q( m, n, A, lda, SA, ldsa, info, magma_tally3_stream );
}
