...
void Metrics_create(Metrics* metrics, int data_type_id, int num_vector_local,
    ...
    if (env->global_all2all) {
        ...
        if (env->num_way == 2) {
            ...
            metrics->num_elts_local =
                (num_vector_local*(num_vector_local-1)) / 2;
            ...
            /*---n choose 3---*/
            metrics->num_elts_local = (num_vector_local*
                (num_vector_local-1)*(num_vector_local-2)) / 6;
            metrics->index_map = malloc(metrics->num_elts_local*sizeof(int));
            int index = 0;
            int i = 0;
            for (i=0; i<num_vector_local; ++i) {
    ...
    if (data_type_id == DATA_TYPE_ID_FLOAT) {
        metrics->data
            = malloc(metrics->num_elts_local*sizeof(Float_t));
