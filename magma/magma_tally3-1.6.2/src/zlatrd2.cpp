        ...
        for (i = n-1; i >= n - nb; --i) {
            ...
            if (i > 0) {
                ...
                *A(i-1,i) = MAGMA_tally3_Z_ONE;
                ...
                magma_tally3_zsetvector_async( i, A(0, i), 1, dA(0, i), 1, stream );
                
                magma_tally3blas_zhemv_work( Magma_tally3Upper, i, c_one, dA(0, 0), ldda,
                                      dA(0, i), ione, c_zero, dW(0, iw), ione,
                                      dwork, ldwork, stream );
                
                // 2. Start getting the result back (asynchronously)
                magma_tally3_zgetmatrix_async( i, 1,
                                        dW(0, iw), lddw,
                                        W(0, iw),  ldw, stream );
                
                if (i < n-1) {
                    blasf77_zgemv( Magma_tally3ConjTransStr, &i, &i_n, &c_one, W(0, iw+1), &ldw,
                ...
                if (i < n-1) {
                    ...
                    blasf77_zgemv( Magma_tally3ConjTransStr, &i, &i_n, &c_one, A(0, i+1), &lda,
                                   A(0, i), &ione, &c_zero, W(i+1, iw), &ione );
        ...
        for (i = 0; i < nb; ++i) {
            ...
            if (i < n-1) {
                ...
                *A(i+1,i) = MAGMA_tally3_Z_ONE;
                ...
                magma_tally3_zsetvector_async( i_n, A(i+1, i), 1, dA(i+1, i), 1, stream );
                
                magma_tally3blas_zhemv_work( Magma_tally3Lower, i_n, c_one, dA(i+1, i+1), ldda,
                                      dA(i+1, i), ione, c_zero, dW(i+1, i), ione,
                                      dwork, ldwork, stream );
                
                // 2. Start getting the result back (asynchronously)
                magma_tally3_zgetmatrix_async( i_n, 1,
                                        dW(i+1, i), lddw,
                                        W(i+1, i),  ldw, stream );
                
                blasf77_zgemv( Magma_tally3ConjTransStr, &i_n, &i, &c_one, W(i+1, 0), &ldw,
                               A(i+1, i), &ione, &c_zero, W(0, i), &ione );
                
                blasf77_zgemv( "No transpose", &i_n, &i, &c_neg_one, A(i+1, 0), &lda,
                               W(0, i), &ione, &c_zero, f, &ione );
                
                blasf77_zgemv( Magma_tally3ConjTransStr, &i_n, &i, &c_one, A(i+1, 0), &lda,
                               A(i+1, i), &ione, &c_zero, W(0, i), &ione );
                ...
                value = magma_tally3_cblas_zdotc( i_n, W(i+1,i), ione, A(i+1,i), ione );
