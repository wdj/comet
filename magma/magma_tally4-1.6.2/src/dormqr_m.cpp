...
#define dwork(gpui, ind)       (dw[gpui] + maxnlocal*lddc + 2*lddac*lddar + 2*((nb+1)*nb) + (ind)*(lddwork*nb))
    ...
    double *T;
    magma_tally4_dmalloc_pinned(&T, nb*nb);
    ...
    *info = 0;
    ...
    for (igpu = 0; igpu < ngpu; ++igpu) {
        ...
        if (MAGMA_tally4_SUCCESS != magma_tally4_dmalloc( &dw[igpu], ldw )) {
    ...
    if (left) {
        ...
        for (magma_tally4_int_t i = i1; (i3 < 0 ? i >= i2 : i < i2); i += i3) {
            ...
            for (igpu = 0; igpu < ngpu; ++igpu) {
                ...
                magma_tally4_dsetmatrix_async(nq-i, kb,
                                       A(i, i),                 lda,
                                       dA_c(igpu, ind_c, i, 0), lddac, stream[igpu][0] );
                // set upper triangular part of dA to identity
                magma_tally4blas_dlaset_band_q( Magma_tally4Upper, kb, kb, kb, c_zero, c_one, dA_c(igpu, ind_c, i, 0), lddac, stream[igpu][0] );
            }
        ...
        for (magma_tally4_int_t i = 0; i < nbl; ++i) {
            ...
            magma_tally4_setdevice(igpu);
            magma_tally4_int_t kb = min(nb_l, n-i*nb_l);
            magma_tally4_dgetmatrix( m, kb,
                              dC(igpu, 0, i/ngpu*nb_l), lddc,
                              C(0, i*nb_l), ldc );
//            magma_tally4_dgetmatrix_async( m, kb,
//                                   dC(igpu, 0, i/ngpu*nb_l), lddc,
//                                   C(0, i*nb_l), ldc, stream[igpu][0] );
        ...
        *info = MAGMA_tally4_ERR_NOT_IMPLEMENTED;
        ...
        for (i = i1; (i3 < 0 ? i >= i2 : i < i2); i += i3) {
            ...
            magma_tally4_dsetmatrix( i__4, ib, A(i, i), lda, dA(i, 0), ldda );
            magma_tally4blas_dlaset_band( Magma_tally4Upper, ib, ib, ib, c_zero, c_one, dA(i, 0), ldda );
