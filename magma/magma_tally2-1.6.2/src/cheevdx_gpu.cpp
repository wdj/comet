/*
    -- MAGMA_tally2 (version 1.6.1) --
       ...
       @generated from zheevdx_gpu.cpp normal z -> c, Fri Jan 30 19:00:17 2015
...
/**
    ...
    dA      COMPLEX array on the GPU,
            ...
            On exit, if JOBZ = Magma_tally2Vec, then if INFO = 0, the first mout columns
    ...
    mout    INTEGER
            ...
            If RANGE = Magma_tally2RangeAll, MOUT = N, and if RANGE = Magma_tally2RangeI, MOUT = IU-IL+1.
...
magma_tally2_cheevdx_gpu(
    ...
    magma_tally2_int_t *mout, float *w,
    ...
    magma_tally2_int_t *info)
{
    ...
    magma_tally2_int_t llrwk;
    magma_tally2_int_t wantz;
    //magma_tally2_int_t indwk2;
    magma_tally2_int_t iscale;
    float safmin;
    float bignum;
    magma_tally2_int_t indtau;
    magma_tally2_int_t indrwk, indwrk, liwmin;
    magma_tally2_int_t lrwmin, llwork;
    float smlnum;
    ...
    *info = 0;
    ...
    work[0]  = MAGMA_tally2_C_MAKE( lwmin * one_eps, 0 );
    ...
    if (n <= 128) {
        ...
        magma_tally2_cmalloc_cpu( &A, lda*n );
    ...
    magma_tally2_int_t ldwork = max( ldda*ceildiv(n,64) + 2*ldda*nb, lddc*n );
    magma_tally2_int_t ldwork_real = max( ldwork*2, n );
    if ( wantz ) {
        // cstedx requrise 3n^2/2, in float
        ldwork_real = max( ldwork_real, 3*n*(n/2 + 1) );
    }
    if (MAGMA_tally2_SUCCESS != magma_tally2_smalloc( &dwork, ldwork_real )) {
        *info = MAGMA_tally2_ERR_DEVICE_ALLOC;
        return *info;
    }
    dC = (magma_tally2FloatComplex*) dwork;
    ...
    if (iscale == 1) {
        magma_tally2blas_clascl( uplo, 0, 0, 1., sigma, n, n, dA, ldda, info );
    }
...
#ifdef FAST_HEMV
    magma_tally2_chetrd2_gpu( uplo, n, dA, ldda, w, &rwork[inde],
                       &work[indtau], wA, ldwa, &work[indwrk], llwork,
                       dC, ldwork, &iinfo );
#else
    magma_tally2_chetrd_gpu ( uplo, n, dA, ldda, w, &rwork[inde],
                       &work[indtau], wA, ldwa, &work[indwrk], llwork,
                       &iinfo );
#endif
    ...
    if (! wantz) {
        ...
        magma_tally2_smove_eig( range, n, w, &il, &iu, vl, vu, mout );
    }
    else {
        timer_start( time );

        magma_tally2_cstedx( range, n, vl, vu, il, iu, w, &rwork[inde],
        ...
        magma_tally2_csetmatrix( n, *mout, &work[indwrk + n * (il-1) ], n, dC, lddc );

        magma_tally2_cunmtr_gpu( Magma_tally2Left, uplo, Magma_tally2NoTrans, n, *mout, dA, ldda, &work[indtau],
                          dC, lddc, wA, ldwa, &iinfo );

        magma_tally2_ccopymatrix( n, *mout, dC, lddc, dA, ldda );
