...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lhwork2 = (magma_minproduct_int_t) MAGMA_minproduct_D_REAL( tmp[0] );
            
            lhwork = -1;
            lapackf77_dormqr( Magma_minproductLeftStr, Magma_minproductTransStr,
            ...
            magma_minproduct_dgels3_gpu( Magma_minproductNoTrans, M, N, nrhs, d_A, ldda,
                              d_B, lddb, h_work, lworkgpu, &info);
            gpu_time = magma_minproduct_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_minproduct_dgels3_gpu returned error %d: %s.\n",
