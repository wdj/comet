...
#include "cudbg_magma_tally3_wrap.h" 
#else
...
void ca_dbg_printMat_gpu(int M, int N, double *dA,int dA_LDA, char desc[] )
{
    ...
    if ( magma_tally3_is_devptr( dA ) == 0 ) {
    ...
    //if(magma_tally3_malloc_pinned((void **) &A, max(LDA,M)*N)!=MAGMA_tally3_SUCCESS) {printf("Memory allocation failed for A in ca_dbg_printMat_gpu"); exit(1);}
   // if(magma_tally3_dmalloc_pinned(&A, max(LDA,M)*N)!=MAGMA_tally3_SUCCESS) {printf("Memory allocation failed for A in ca_dbg_printMat_gpu\n"); exit(1);}
    if(magma_tally3_dmalloc_cpu(&A, max(LDA,M)*N)!=MAGMA_tally3_SUCCESS) {printf("Memory allocation failed for A in ca_dbg_printMat_gpu\n"); exit(1);}
    magma_tally3_dgetmatrix(M, N, dA, dA_LDA, A, LDA);
...
int LDA=dA_LDA;
    ...
    /*TODO slow*/
    //if (MAGMA_tally3_SUCCESS != magma_tally3_dmalloc_cpu(&A, max(LDA,M)*N)) {printf("Memory allocation failed for A in cudbg_dprint_transpose_gpu\n");exit(1);}
    //if(!(A =    (double *)    malloc(max(LDA,M)*N*sizeof(double)))) {printf("Memory allocation failed for A in cudbg_dprint_transpose_gpu\n");exit(1);}
    //if(magma_tally3_dmalloc_pinned(&A, max(LDA,M)*N)!=MAGMA_tally3_SUCCESS) {printf("Memory allocation failed for A in cudbg_dprint_transpose_gpu\n");exit(1);}
    if(magma_tally3_dmalloc_cpu(&A, max(LDA,M)*N)!=MAGMA_tally3_SUCCESS) {printf("Memory allocation failed for A in ca_dbg_printMat_gpu\n"); exit(1);}
    magma_tally3_dgetmatrix(M, N, dA, dA_LDA, A, LDA);
