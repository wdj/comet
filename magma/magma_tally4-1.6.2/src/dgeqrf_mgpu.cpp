    ...
    magma_tally4_int_t *info )
{
    ...
    double *dwork[Magma_tally4MaxGPUs]={NULL}, *dpanel[Magma_tally4MaxGPUs]={NULL};
    ...
    *info = 0;
    ...
    for( dev=0; dev < ngpu; dev++ ) {
        ...
        if ( MAGMA_tally4_SUCCESS != magma_tally4_dmalloc( &(dwork[dev]), (lddwork + ldda)*nb )) {
    ...
    if ( MAGMA_tally4_SUCCESS != magma_tally4_dmalloc_pinned( &hwork, lwork )) {
    ...
    if ( nb < min_mn ) {
        ...
        for( i = 0; i < min_mn-nb; i += nb ) {
            ...
            magma_tally4_dgetmatrix_async( rows, ib,
            ...
            dpanel_to_q_tally4( Magma_tally4Upper, ib, hpanel(i), ldhpanel, hwork + ib*ib );
            ...
            for( dev=0; dev < ngpu; dev++ ) {
                ...
                magma_tally4_dsetmatrix_async( rows, ib,
                                        hpanel(i),   ldhpanel,
            ...
            // TODO: if dpanel_to_q_tally4 copied whole block, wouldn't need to restore
            // -- just send the copy to the GPUs.
            // TODO: also, could zero out the lower triangle and use Azzam's larfb w/ gemm.
            
            /* Restore the panel */
            dq_to_panel_tally4( Magma_tally4Upper, ib, hpanel(i), ldhpanel, hwork + ib*ib );
            ...
            if (i + ib < n) {
                ...
                for( dev=0; dev < ngpu; dev++ ) {
                    ...
                    if (dev == la_dev && i+nb < min_mn-nb) {
                        ...
                        magma_tally4_dlarfb_gpu( Magma_tally4Left, Magma_tally4ConjTrans, Magma_tally4Forward, Magma_tally4Columnwise,
                                          rows, ib, ib,
                                          dpanel[dev],             ldda,       // V
                                          dwork[dev],              lddwork,    // T
    ...
    if (i < min_mn) {
        ...
        for( j=i; j < n; j += nb ) {
            ...
            ib = min( n-j, nb );
            magma_tally4_setdevice( panel_dev );
            magma_tally4_dgetmatrix( rows, ib,
                              dlA(panel_dev, i, i_local), ldda,
                              hwork + (j-i)*rows,         rows );
        }
        ...
        for( j=i; j < n; j += nb ) {
            ...
            ib = min( n-j, nb );
            magma_tally4_setdevice( panel_dev );
            magma_tally4_dsetmatrix( rows, ib,
                              hwork + (j-i)*rows,         rows,
                              dlA(panel_dev, i, i_local), ldda );
        }
    }
