...
bool lessthan( magma_tally2DoubleComplex a, magma_tally2DoubleComplex b )
{
    return (MAGMA_tally2_Z_REAL(a) < MAGMA_tally2_Z_REAL(b)) ||
        (MAGMA_tally2_Z_REAL(a) == MAGMA_tally2_Z_REAL(b) && MAGMA_tally2_Z_IMAG(a) < MAGMA_tally2_Z_IMAG(b));
}
...
extern "C"
double magma_tally2_dlapy2(double x, double y)
{
    double ret_val, d;
...
extern "C"
double magma_tally2_dzlapy2(magma_tally2DoubleComplex x)
{
    return magma_tally2_dlapy2( MAGMA_tally2_Z_REAL(x), MAGMA_tally2_Z_IMAG(x) );
}
...
int main( int argc, char** argv)
{
    ...
    double *w1i, *w2i;
    magma_tally2DoubleComplex *w1copy, *w2copy;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( w1copy, magma_tally2DoubleComplex, N );
            TESTING_MALLOC_CPU( w2copy, magma_tally2DoubleComplex, N );
            ...
            lapackf77_dlacpy( Magma_tally2UpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
            
            /* ====================================================================
               Performs operation using MAGMA_tally2
               =================================================================== */
            gpu_time = magma_tally2_wtime();
            magma_tally2_dgeev( opts.jobvl, opts.jobvr,
                         N, h_R, lda, w1, w1i,
                         VL, lda, VR, lda,
                         h_work, lwork, &info );
            gpu_time = magma_tally2_wtime() - gpu_time;
            if (info != 0)
                printf("magma_tally2_dgeev returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                if ( opts.jobvr == Magma_tally2Vec ) {
                    ...
                    lapackf77_dget22( Magma_tally2NoTransStr, Magma_tally2NoTransStr, Magma_tally2NoTransStr,
                    ...
                    for( int j = 0; j < N; ++j ) {
                        tnrm = 1.;
                        if (w1i[j] == 0.)
                            tnrm = magma_tally2_cblas_dnrm2( N, &VR[j*lda], ione );
                        else if (w1i[j] > 0.)
                            tnrm = magma_tally2_dlapy2( magma_tally2_cblas_dnrm2( N, &VR[j*lda],     ione ),
                                                 magma_tally2_cblas_dnrm2( N, &VR[(j+1)*lda], ione ));
                        
                        result[1] = max( result[1], min( ulpinv, MAGMA_tally2_D_ABS(tnrm-1.)/ulp ));
                        
                        if (w1i[j] > 0.) {
                            vmx  = vrmx = 0.;
                            for( int jj = 0; jj < N; ++jj ) {
                                vtst = magma_tally2_dlapy2( VR[jj+j*lda], VR[jj+(j+1)*lda]);
                                if (vtst > vmx)
                                    vmx = vtst;
                                
                                if ( (VR[jj + (j+1)*lda])==0. &&
                                     MAGMA_tally2_D_ABS( VR[jj+j*lda] ) > vrmx)
                                {
                                    vrmx = MAGMA_tally2_D_ABS( VR[jj+j*lda] );
                                }
                            }
                            if (vrmx / vmx < 1. - ulp*2.)
                                result[1] = ulpinv;
                ...
                if ( opts.jobvl == Magma_tally2Vec ) {
                    ...
                    lapackf77_dget22( Magma_tally2TransStr, Magma_tally2NoTransStr, Magma_tally2TransStr,
                    ...
                    for( int j = 0; j < N; ++j ) {
                        tnrm = 1.;
                        if (w1i[j] == 0.)
                            tnrm = magma_tally2_cblas_dnrm2( N, &VL[j*lda], ione );
                        else if (w1i[j] > 0.)
                            tnrm = magma_tally2_dlapy2( magma_tally2_cblas_dnrm2( N, &VL[j*lda],     ione ),
                                                 magma_tally2_cblas_dnrm2( N, &VL[(j+1)*lda], ione ));
                        
                        result[3] = max( result[3], min( ulpinv, MAGMA_tally2_D_ABS(tnrm-1.)/ulp ));
                        
                        if (w1i[j] > 0.) {
                            vmx  = vrmx = 0.;
                            for( int jj = 0; jj < N; ++jj ) {
                                vtst = magma_tally2_dlapy2( VL[jj+j*lda], VL[jj+(j+1)*lda]);
                                if (vtst > vmx)
                                    vmx = vtst;
                                
                                if ( (VL[jj + (j+1)*lda])==0. &&
                                     MAGMA_tally2_D_ABS( VL[jj+j*lda]) > vrmx)
                                {
                                    vrmx = MAGMA_tally2_D_ABS( VL[jj+j*lda] );
                                }
                            }
                            if (vrmx / vmx < 1. - ulp*2.)
                                result[3] = ulpinv;
            ...
            if ( opts.check == 2 ) {
                ...
                // this is really slow because it calls magma_tally2_zgeev multiple times
                double *LRE, DUM;
                ...
                magma_tally2_dgeev( Magma_tally2Vec, Magma_tally2Vec,
                             N, h_R, lda, w1, w1i,
                             VL, lda, VR, lda,
                             h_work, lwork, &info );
                if (info != 0)
                    printf("magma_tally2_zgeev (case V, V) returned error %d: %s.\n",
                           (int) info, magma_tally2_strerror( info ));
                ...
                //lapackf77_dlacpy( Magma_tally2UpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                //magma_tally2_dgeev( Magma_tally2NoVec, Magma_tally2NoVec,
                //             N, h_R, lda, w2, w2i,
                //             &DUM, 1, &DUM, 1,
                //             h_work, lwork, &info );
                //if (info != 0)
                //    printf("magma_tally2_dgeev (case N, N) returned error %d: %s.\n",
                //           (int) info, magma_tally2_strerror( info ));
                //
                ...
                lapackf77_dlacpy( Magma_tally2UpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                magma_tally2_dgeev( Magma_tally2NoVec, Magma_tally2Vec,
                             N, h_R, lda, w2, w2i,
                             &DUM, 1, LRE, lda,
                             h_work, lwork, &info );
                if (info != 0)
                    printf("magma_tally2_dgeev (case N, V) returned error %d: %s.\n",
                           (int) info, magma_tally2_strerror( info ));
                ...
                for( int j = 0; j < N; ++j )
                    for( int jj = 0; jj < N; ++jj )
                        if ( ! MAGMA_tally2_D_EQUAL( VR[j+jj*lda], LRE[j+jj*lda] ))
                            result[7] = 0;
                
                // ----------
                // Compute eigenvalues and left eigenvectors
                lapackf77_dlacpy( Magma_tally2UpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                magma_tally2_dgeev( Magma_tally2Vec, Magma_tally2NoVec,
                             N, h_R, lda, w2, w2i,
                             LRE, lda, &DUM, 1,
                             h_work, lwork, &info );
                if (info != 0)
                    printf("magma_tally2_dgeev (case V, N) returned error %d: %s.\n",
                           (int) info, magma_tally2_strerror( info ));
                ...
                for( int j = 0; j < N; ++j )
                    for( int jj = 0; jj < N; ++jj )
                        if ( ! MAGMA_tally2_D_EQUAL( VL[j+jj*lda], LRE[j+jj*lda] ))
                            result[8] = 0;
            ...
            if ( opts.lapack ) {
                ...
                lapackf77_dgeev( lapack_vec_const_tally2(opts.jobvl), lapack_vec_const_tally2(opts.jobvr),
                                 &N, h_A, &lda, w2, w2i,
                                 VL, &lda, VR, &lda,
                                 h_work, &lwork, &info );
                ...
                for( int j=0; j < N; ++j ) {
                    w1copy[j] = MAGMA_tally2_Z_MAKE( w1[j], w1i[j] );
                    w2copy[j] = MAGMA_tally2_Z_MAKE( w2[j], w2i[j] );
                }
                ...
                for( int j=0; j < N; ++j ) {
                    for( int j2=j; j2 < N; ++j2 ) {
                        magma_tally2DoubleComplex diff = MAGMA_tally2_Z_SUB( w1copy[j], w2copy[j2] );
                        double diff2 = magma_tally2_dzlapy2( diff ) / max( magma_tally2_dzlapy2( w1copy[j] ), tol );
                        if ( diff2 < 100*tol ) {
                            if ( j != j2 ) {
                ...
                error  = magma_tally2_cblas_dznrm2( N, w1copy, 1 );
                error /= magma_tally2_cblas_dznrm2( N, w2copy, 1 );
