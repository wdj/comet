...
/**
    ...
    A       COMPLEX array, dimension (LDA,N)
            ...
            On exit, if UPLO = Magma_tally3Upper, the Upper band-diagonal of A is
            ...
            = Magma_tally3Lower, the the Lower band-diagonal of A is overwritten by
    ...
    magma_tally3_int_t *info)
{
    ...
    float  d_one = MAGMA_tally3_D_ONE;

    magma_tally3_int_t pm, pn, indi, indj, pk;
    magma_tally3_int_t pm_old=0, pn_old=0, indi_old=0, flipV=-1;
    magma_tally3_int_t iblock, idev, di;
    int i;
    int lwkopt;
    int lquery;
    ...
    *info = 0;
    ...
    // limit to 16 threads
    magma_tally3_int_t orig_threads = magma_tally3_get_lapack_numthreads();
    magma_tally3_set_lapack_numthreads( min(orig_threads,16) );

    magma_tally3_int_t gnode[Magma_tally3MaxGPUs][Magma_tally3MaxGPUs+2];
    magma_tally3_int_t nbcmplx=0;
    magma_tally3_buildconnection_mgpu(gnode, &nbcmplx,  ngpu);
    #ifdef ENABLE_DEBUG
    printf(" Initializing communication pattern.... GPU-ncmplx %d\n\n", nbcmplx);
    #endif

    magma_tally3FloatComplex *dspace[Magma_tally3MaxGPUs];
    magma_tally3FloatComplex *dwork[Magma_tally3MaxGPUs], *dworkbis[Magma_tally3MaxGPUs];
    magma_tally3FloatComplex *dvall[Magma_tally3MaxGPUs], *dv[Magma_tally3MaxGPUs], *dw[Magma_tally3MaxGPUs];
    magma_tally3FloatComplex *workngpu[Magma_tally3MaxGPUs+1];
    magma_tally3_event_t     redevents[Magma_tally3MaxGPUs][Magma_tally3MaxGPUs*Magma_tally3MaxGPUs+10];
    magma_tally3_int_t nbevents = Magma_tally3MaxGPUs*Magma_tally3MaxGPUs;

    magma_tally3_int_t lddv        = ldda;
    magma_tally3_int_t lddw        = lddv;
    magma_tally3_int_t dwrk2siz    = ldda*nb*3;
    magma_tally3_int_t worksiz     = n*nb;
    magma_tally3_int_t devworksiz  = 2*nb*lddv + nb*lddw + nb*ldda + dwrk2siz; // 2*dv(dv0+dv1) + dw + dwork +dworkbis

    // local allocation and stream creation
    // TODO check malloc
    for( magma_tally3_int_t dev = 0; dev < ngpu; ++dev ) {
        magma_tally3_setdevice( dev );
        magma_tally3_cmalloc( &dspace[dev], devworksiz );
        magma_tally3_cmalloc_pinned ( &workngpu[dev], worksiz);
    ...
    magma_tally3_cmalloc_pinned ( &workngpu[ngpu], worksiz);
    magma_tally3FloatComplex *worktest = NULL;
    //magma_tally3_cmalloc_cpu( &worktest, n*nb ); // not used
    // ======================
  

    magma_tally3FloatComplex *hT = work + lwork - nb*nb;
    lwork -= nb*nb;
    memset( hT, 0, nb*nb*sizeof(magma_tally3FloatComplex));
    ...
    if (upper) {
        ...
        for (i = 1; i <= n-nb; i += nb) {
            ...
            if (i > 1 ) {
                // cpanel_to_q_tally3 copy the upper oof diagonal part of
                ...
                cpanel_to_q_tally3(Magma_tally3Upper, pn-1, A(i, i+1), lda, work);
                ...
                magma_tally3_cgetmatrix_async( (pm+pn), pn,
                                        dA(idev, i, di+1), ldda,
                ...
                magma_tally3blas_cher2k_mgpu_spec(
                    Magma_tally3Lower, Magma_tally3NoTrans, pm_old-pn_old, pn_old,
                ...
                magma_tally3_setdevice( idev );
                magma_tally3_queue_sync( queues[idev][ nqueue-1 ] );
                //magma_tally3_setdevice( 0 );
                cq_to_panel_tally3(Magma_tally3Upper, pn-1, A(i, i+1), lda, work);
            }
            ...
            cpanel_to_q_tally3(Magma_tally3Upper, pk, A(indi, indj), lda, work);
            ...
            for( magma_tally3_int_t dev = 0; dev < ngpu; ++dev ) {
                magma_tally3_setdevice( dev );
                // send V
                magma_tally3_csetmatrix_async( pm, pk,
                                        A(indi, indj), lda,
                                        dv[dev], pm, queues[dev][nqueue-1] );

                // Send the triangular factor T to the GPU
                magma_tally3_csetmatrix_async( pk, pk,
                                        hT, nb,
            ...
            for( magma_tally3_int_t dev = 0; dev < ngpu; ++dev ) {
                // dwork = V T
                magma_tally3_setdevice( dev );
                magma_tally3blasSetKernelStream( queues[ dev ][ nqueue-1 ] );
                magma_tally3_queue_sync( queues[dev][nqueue-1] );
                magma_tally3_cgemm(Magma_tally3NoTrans, Magma_tally3NoTrans, pm, pk, pk,
                        c_one, dv[dev], pm,
                        dT(dev, 1, i), lddt,
                        c_zero, dwork[dev], pm);
            }
            ...
            for( magma_tally3_int_t dev = 0; dev < ngpu; ++dev ) {
                magma_tally3_setdevice( dev );
                for( magma_tally3_int_t s = 0; s < nqueue; ++s )
                magma_tally3_queue_sync( queues[dev][s] );
            }
            ...
            // Broadcasting stream.
            if (ngpu == 1) {
                magma_tally3blasSetKernelStream( queues[ 0 ][ 0 ] );
                magma_tally3_chemm(
                    Magma_tally3Left, uplo, pm, pk,
            ...
            for( magma_tally3_int_t dev = 0; dev < ngpu; ++dev ) {
                ...
                magma_tally3_setdevice( dev );
                magma_tally3blasSetKernelStream( queues[ dev ][ dev ] );
                // magma_tally3_queue_sync( queues[dev][0] );
                magma_tally3_cgemm(Magma_tally3ConjTrans, Magma_tally3NoTrans, pk, pk, pm,
                            c_one, dwork[dev], pm,
                            dw[dev], pm,
            ...
            cq_to_panel_tally3(Magma_tally3Upper, pk, A(indi, indj), lda, work);
            // ===============================================
            ...
            for( magma_tally3_int_t dev = 0; dev < ngpu; ++dev ) {
                magma_tally3_setdevice( dev );
                magma_tally3_queue_sync( queues[dev][dev] );
            }
            ...
            if (i + nb <= n-nb) {
                ...
                magma_tally3_setdevice( idev );
                magma_tally3blasSetKernelStream( queues[ idev ][ nqueue-1 ] );
                //magma_tally3_queue_sync( queues[idev][0] ); removed because the sync has been done in the loop above
                magma_tally3_cgemm(Magma_tally3NoTrans, Magma_tally3ConjTrans, pm, pn, pn, c_neg_one,
                            dv[idev], pm,
                            dw[idev], pm, c_one,
                ...
                idev   = ((indi-1) / distblk) % ngpu;          // device with this block
                magma_tally3_setdevice( idev );
                magma_tally3blasSetKernelStream( queues[ idev ][ 0 ] );
                //printf("LAST CHER2K idev %d on A(%d,%d) of size %d \n",idev, indi-1,di,pk);
                magma_tally3_cher2k(Magma_tally3Lower, Magma_tally3NoTrans, pk, pk, c_neg_one,
                             dv[idev], pm,
                             dw[idev], pm, d_one,
                             dA(idev, indi, di+1), ldda);
                ...
                cpanel_to_q_tally3(Magma_tally3Upper, pk-1, A(n-pk+1, n-pk+2), lda, work);
                magma_tally3_cgetmatrix( pk, pk,
                                  dA(idev, indi, di+1), ldda,
                                  A(n-pk+1, n-pk+1),  lda );
                cq_to_panel_tally3(Magma_tally3Upper, pk-1, A(n-pk+1, n-pk+2), lda, work);
            }
    ...
    for( magma_tally3_int_t dev = 0; dev < ngpu; ++dev ) {
        magma_tally3_setdevice( dev );
        magma_tally3_free( dspace[dev]);
        magma_tally3_free_pinned(workngpu[dev]);
        for( magma_tally3_int_t e = 0; e < nbevents; ++e ) {
            magma_tally3_event_destroy( redevents[dev][e] );
        }
    }
    magma_tally3_free_pinned(workngpu[ngpu]);
    magma_tally3_free_cpu(worktest);

    magma_tally3_setdevice( orig_dev );
    magma_tally3blasSetKernelStream( orig_stream );
    magma_tally3_set_lapack_numthreads( orig_threads );
