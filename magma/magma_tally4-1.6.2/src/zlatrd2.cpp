...
/**
    ...
    If UPLO = Magma_tally4Upper, ZLATRD reduces the last NB rows and columns of a
    matrix, of which the upper triangle is supplied;
    if UPLO = Magma_tally4Lower, ZLATRD reduces the first NB rows and columns of a
    ...
    A       COMPLEX_16 array, dimension (LDA,N)
            ...
            On exit:
      -     if UPLO = Magma_tally4Upper, the last NB columns have been reduced to
              tridiagonal form, with the diagonal elements overwriting
              the diagonal elements of A; the elements above the diagonal
              with the array TAU, represent the orthogonal matrix Q as a
              product of elementary reflectors;
      -     if UPLO = Magma_tally4Lower, the first NB columns have been reduced to
              tridiagonal form, with the diagonal elements overwriting
    ...
    e       COMPLEX_16 array, dimension (N-1)
            If UPLO = Magma_tally4Upper, E(n-nb:n-1) contains the superdiagonal
            elements of the last NB columns of the reduced matrix;
            if UPLO = Magma_tally4Lower, E(1:nb) contains the subdiagonal elements of
            the first NB columns of the reduced matrix.

    @param[out]
    tau     COMPLEX_16 array, dimension (N-1)
            The scalar factors of the elementary reflectors, stored in
            TAU(n-nb:n-1) if UPLO = Magma_tally4Upper, and in TAU(1:nb) if UPLO = Magma_tally4Lower.
            See Further Details.
    ...
    @ingroup magma_tally4_zheev_aux
    ********************************************************************/
    ...
    magma_tally4DoubleComplex_ptr dwork, magma_tally4_int_t ldwork)
{
    ...
    magma_tally4DoubleComplex alpha, value;
    magma_tally4_int_t i, i_n, i_1, iw;
    ...
    magma_tally4DoubleComplex *f;
    magma_tally4_zmalloc_cpu( &f, n );
    if ( f == NULL ) {
    ...
    if (uplo == Magma_tally4Upper) {
        ...
        for (i = n-1; i >= n - nb; --i) {
            ...
            if (i > 0) {
                ...
                e[i-1] = MAGMA_tally4_Z_REAL( alpha );
                *A(i-1,i) = MAGMA_tally4_Z_ONE;
                
                /* Compute W(1:i-1,i) */
                // 1. Send the block reflector  A(0:n-i-1,i) to the GPU
                magma_tally4_zsetvector_async( i, A(0, i), 1, dA(0, i), 1, stream );
                
                magma_tally4blas_zhemv_work( Magma_tally4Upper, i, c_one, dA(0, 0), ldda,
                                      dA(0, i), ione, c_zero, dW(0, iw), ione,
                                      dwork, ldwork, stream );
                
                // 2. Start getting the result back (asynchronously)
                magma_tally4_zgetmatrix_async( i, 1,
                                        dW(0, iw), lddw,
                                        W(0, iw),  ldw, stream );
                
                if (i < n-1) {
                    blasf77_zgemv( Magma_tally4ConjTransStr, &i, &i_n, &c_one, W(0, iw+1), &ldw,
                                   A(0, i), &ione, &c_zero, W(i+1, iw), &ione );
                }
                
                // 3. Here we need zhemv result W(0, iw)
                ...
                if (i < n-1) {
                    ...
                    blasf77_zgemv( Magma_tally4ConjTransStr, &i, &i_n, &c_one, A(0, i+1), &lda,
                                   A(0, i), &ione, &c_zero, W(i+1, iw), &ione );
                ...
                value = magma_tally4_cblas_zdotc( i, W(0,iw), ione, A(0,i), ione );
        ...
        for (i = 0; i < nb; ++i) {
            ...
            if (i < n-1) {
                ...
                e[i] = MAGMA_tally4_Z_REAL( alpha );
                *A(i+1,i) = MAGMA_tally4_Z_ONE;
                
                /* Compute W(i+1:n,i) */
                // 1. Send the block reflector  A(i+1:n,i) to the GPU
                magma_tally4_zsetvector_async( i_n, A(i+1, i), 1, dA(i+1, i), 1, stream );
                
                magma_tally4blas_zhemv_work( Magma_tally4Lower, i_n, c_one, dA(i+1, i+1), ldda,
                                      dA(i+1, i), ione, c_zero, dW(i+1, i), ione,
                                      dwork, ldwork, stream );
                
                // 2. Start getting the result back (asynchronously)
                magma_tally4_zgetmatrix_async( i_n, 1,
                                        dW(i+1, i), lddw,
                                        W(i+1, i),  ldw, stream );
                
                blasf77_zgemv( Magma_tally4ConjTransStr, &i_n, &i, &c_one, W(i+1, 0), &ldw,
                               A(i+1, i), &ione, &c_zero, W(0, i), &ione );
                
                blasf77_zgemv( "No transpose", &i_n, &i, &c_neg_one, A(i+1, 0), &lda,
                               W(0, i), &ione, &c_zero, f, &ione );
                
                blasf77_zgemv( Magma_tally4ConjTransStr, &i_n, &i, &c_one, A(i+1, 0), &lda,
                               A(i+1, i), &ione, &c_zero, W(0, i), &ione );
                
                // 3. Here we need zhemv result W(i+1, i)
                magma_tally4_queue_sync( stream );
                ...
                value = magma_tally4_cblas_zdotc( i_n, W(i+1,i), ione, A(i+1,i), ione );
