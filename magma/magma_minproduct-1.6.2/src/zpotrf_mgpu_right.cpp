    ...
    magma_minproduct_int_t *info )
{
    ...
    magma_minproductDoubleComplex z_one = MAGMA_minproduct_Z_MAKE(  1.0, 0.0 );
    magma_minproductDoubleComplex mz_one = MAGMA_minproduct_Z_MAKE( -1.0, 0.0 );
    double             one =  1.0;
    double             m_one = -1.0;
    const char* uplo_ = lapack_uplo_const( uplo );

    magma_minproduct_int_t j, nb, d, id, j_local, blkid, crosspoint, prevtrsmrows=0, nqueue = 5;
    magma_minproductDoubleComplex *panel, *tmppanel0, *tmppanel1, *tmppanel, *tmpprevpanel;
    magma_minproductDoubleComplex *d_lP[Magma_minproductMaxGPUs], *dlpanel, *dlpanels[Magma_minproductMaxGPUs];
    ...
    *info = 0;
    ...
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc_pinned( &panel, 2 * nb * ldpanel )) {
    ...
    if ((nb <= 1) || (nb >= n)) {
        // Use unblocked code.
        magma_minproduct_zgetmatrix( n, n, dlA(0, 0, 0), ldda, panel, ldpanel);
        lapackf77_zpotrf( uplo_, &n, panel, &ldpanel, info);
        magma_minproduct_zsetmatrix( n, n, panel, ldpanel, dlA(0, 0, 0), ldda );
    } else {
        for( d = 0; d < ngpu; d++ ) {
            ...
            if (MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc( &d_lP[d], nb * ldda )) {
                for( j = 0; j < d; j++ ) {
        ...
        if (uplo == Magma_minproductUpper) {
            ...
            if ( n > nb ) {
                ...
                magma_minproduct_setdevice(0);
                tmppanel = tmppanel0;
                magma_minproduct_zgetmatrix_async(n, nb,
                        dlA(0, 0, 0), ldda,
                        tmppanel(0),  ldpanel,
                        queues[0][0] );
            }
            ...
            for (j = 0; (j + nb) < n; j += nb) {
                ...
                if (j > 0 && prevtrsmrows > crosspoint) {
                    ...
                    blasf77_zgemm( Magma_minproductNoTransStr, Magma_minproductConjTransStr,
                            &rows, &nb, &nb,
                ...
                #endif
                lapackf77_zpotrf(Magma_minproductLowerStr, &nb, tmppanel(j), &ldpanel, info);
                if (*info != 0) {
                    *info = *info + j;
                    break;
                }
                ...
                if (trsmrows > 0) {
                    blasf77_ztrsm(Magma_minproductRightStr, Magma_minproductLowerStr, Magma_minproductConjTransStr, Magma_minproductNonUnitStr,
                                  &trsmrows, &nb,
                                  &z_one, tmppanel(j), &ldpanel,
                ...
                for (igpu = 0; igpu < ngpu; igpu++, d = (d + 1) % ngpu ) {
                    ...
                    if (myrows > 0) {
                        magma_minproduct_setdevice(d);
                        magma_minproduct_zsetmatrix_async(myrows, nb,
                ...
                if ( (j + nb) < n) {
                    ...
                    if (j + nb + nb < n) {
                        ...
                        if (trsmrows <= crosspoint) {
                            ...
                            #ifdef  ZHERK_ON_DIAG
                            magma_minproduct_zherk( Magma_minproductLower, Magma_minproductNoTrans,
                                    nb, nb,
                                    m_one, dlpanel, ldda,
                        ...
                        magma_minproduct_zgetmatrix_async(rows-nb, nb,
                    ...
                    if (n - offset > 0) {
                        ...
                        //magma_minproduct_zherk(Magma_minproductLower, Magma_minproductNoTrans, n - offset, nb,
                        //        m_one, dlpanel, ldda,
            ...
            if (j < n && *info == 0) {
                ...
                #endif
                magma_minproduct_zgetmatrix(rows, rows, dlA(id, j, j_local), ldda, panel(j), ldpanel);
                lapackf77_zpotrf(Magma_minproductLowerStr, &rows, panel(j), &ldpanel, info);
                magma_minproduct_zsetmatrix(rows, rows, panel(j), ldpanel, dlA(id, j, j_local), ldda);
                #if defined (ENABLE_TIMER)
