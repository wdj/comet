/*
    -- MAGMA_tally3 (version 1.6.1) --
       ...
       These tests ensure that the MAGMA_tally3 wrappers around (CPU) CBLAS calls are
       correct.
       This is derived from the testing_blas_z.cpp code that checks MAGMA_tally3's
...
// hence why MAGMA_tally3 does not rely on them.
...
double blasf77_dzasum( const magma_tally3_int_t* n,
                       const magma_tally3DoubleComplex* x, const magma_tally3_int_t* incx );

double blasf77_dznrm2( const magma_tally3_int_t* n,
                       const magma_tally3DoubleComplex* x, const magma_tally3_int_t* incx );

magma_tally3DoubleComplex blasf77_zdotc( const magma_tally3_int_t* n,
                                  const magma_tally3DoubleComplex* x, const magma_tally3_int_t* incx,
                                  const magma_tally3DoubleComplex* y, const magma_tally3_int_t* incy );

magma_tally3DoubleComplex blasf77_zdotu( const magma_tally3_int_t* n,
                                  const magma_tally3DoubleComplex* x, const magma_tally3_int_t* incx,
                                  const magma_tally3DoubleComplex* y, const magma_tally3_int_t* incy );
...
int main( int argc, char** argv )
{
    ...
    magma_tally3DoubleComplex  *A, *B;
    double diff, error;
    magma_tally3_int_t ISEED[4] = {0,0,0,1};
    magma_tally3_int_t m, n, k, size, maxn, ld;
    magma_tally3DoubleComplex x2_m, x2_c;  // complex x for magma_tally3, cblas/fortran blas respectively
    double x_m, x_c;  // x for magma_tally3, cblas/fortran blas respectively
    ...
    printf( "!! Calling these CBLAS and Fortran BLAS sometimes crashes (segfault), which !!\n"
            "!! is why we use wrappers. It does not necesarily indicate a bug in MAGMA_tally3.  !!\n"
            "\n"
            "Diff  compares MAGMA_tally3 wrapper        to CBLAS and BLAS function; should be exactly 0.\n"
            "Error compares MAGMA_tally3 implementation to CBLAS and BLAS function; should be ~ machine epsilon.\n"
            "\n" );
    ...
    for( int iincx = 0; iincx < ninc; ++iincx ) {
        magma_tally3_int_t incx = inc[iincx];
        
    for( int iincy = 0; iincy < ninc; ++iincy ) {
        magma_tally3_int_t incy = inc[iincy];
        
        printf("=========================================================================\n");
        printf( "m=%d, n=%d, k=%d, incx = %d, incy = %d\n",
                (int) m, (int) n, (int) k, (int) incx, (int) incy );
        printf( "Function              MAGMA_tally3     CBLAS     BLAS        Diff      Error\n"
                "                      msec      msec      msec\n" );
        ...
        ld = max( 1, maxn );
        size = ld*maxn;
        magma_tally3_zmalloc_pinned( &A,  size );  assert( A   != NULL );
        magma_tally3_zmalloc_pinned( &B,  size );  assert( B   != NULL );
        ...
        if ( incx > 0 && incx == incy ) {  // positive, no incy
            ...
            error = 0;
            for( int j = 0; j < k; ++j ) {
                x_m = magma_tally3_cblas_dzasum( m, A(0,j), incx );
        ...
        if ( incx > 0 && incx == incy ) {  // positive, no incy
            ...
            error = 0;
            for( int j = 0; j < k; ++j ) {
                x_m = magma_tally3_cblas_dznrm2( m, A(0,j), incx );
        ...
        error = 0;
        for( int j = 0; j < k; ++j ) {
            // MAGMA_tally3 implementation, not just wrapper
            x2_m = magma_tally3_cblas_zdotc( m, A(0,j), incx, B(0,j), incy );
        ...
        for( int j = 0; j < k; ++j ) {
            // MAGMA_tally3 implementation, not just wrapper
            x2_m = magma_tally3_cblas_zdotu( m, A(0,j), incx, B(0,j), incy );
    ...
    printf( "sum diffs  = %8.2g, MAGMA_tally3 wrapper        compared to CBLAS and Fortran BLAS; should be exactly 0.\n"
            "sum errors = %8.2e, MAGMA_tally3 implementation compared to CBLAS and Fortran BLAS; should be ~ machine epsilon.\n\n",
