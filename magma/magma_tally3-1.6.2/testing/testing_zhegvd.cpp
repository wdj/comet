...
int main( int argc, char** argv)
{
    ...
    magma_tally3DoubleComplex *h_A, *h_R, *h_B, *h_S, *h_work;
    double *w1, *w2;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_B,    magma_tally3DoubleComplex,  n2     );
            ...
            TESTING_MALLOC_PIN( h_S,    magma_tally3DoubleComplex,  n2     );
            ...
            magma_tally3_zmake_hpd( N, h_B, lda );
            lapackf77_zlacpy( Magma_tally3FullStr, &N, &N, h_A, &lda, h_R, &lda );
            lapackf77_zlacpy( Magma_tally3FullStr, &N, &N, h_B, &lda, h_S, &lda );
            ...
            if ( opts.check && opts.jobz != Magma_tally3NoVec ) {
                ...
                if ( opts.itype == 1 || opts.itype == 2 ) {
                    ...
                    blasf77_zhemm("R", lapack_uplo_const_tally3(opts.uplo), &N, &N, &c_neg_one, h_B, &lda, h_work, &N, &c_one, h_S, &lda);
                    result[1] = lapackf77_zlange("1", &N, &N, h_S, &lda, rwork) / N;
                }
                else if ( opts.itype == 3 ) {
                    lapackf77_zlacpy( Magma_tally3FullStr, &N, &N, h_B, &lda, h_S, &lda);
                    blasf77_zherk(lapack_uplo_const_tally3(opts.uplo), "N", &N, &N, &d_neg_one, h_R, &lda, &d_one, h_S, &lda);
                    result[1] = lapackf77_zlanhe("1", lapack_uplo_const_tally3(opts.uplo), &N, h_S, &lda, rwork) / N
                              / lapackf77_zlanhe("1", lapack_uplo_const_tally3(opts.uplo), &N, h_B, &lda, rwork);
                }
                
                result[0] = 1.;
                result[0] /= lapackf77_zlanhe("1", lapack_uplo_const_tally3(opts.uplo), &N, h_A, &lda, rwork);
                result[0] /= lapackf77_zlange("1", &N, &N, h_R, &lda, rwork);
                
                if ( opts.itype == 1 ) {
                    blasf77_zhemm("L", lapack_uplo_const_tally3(opts.uplo), &N, &N, &c_one, h_A, &lda, h_R, &lda, &c_zero, h_work, &N);
                    for(int i=0; i < N; ++i)
                        blasf77_zdscal(&N, &w1[i], &h_R[i*N], &ione);
                    blasf77_zhemm("L", lapack_uplo_const_tally3(opts.uplo), &N, &N, &c_neg_one, h_B, &lda, h_R, &lda, &c_one, h_work, &N);
                    result[0] *= lapackf77_zlange("1", &N, &N, h_work, &lda, rwork)/N;
                }
                else if ( opts.itype == 2 ) {
                    blasf77_zhemm("L", lapack_uplo_const_tally3(opts.uplo), &N, &N, &c_one, h_B, &lda, h_R, &lda, &c_zero, h_work, &N);
                    for(int i=0; i < N; ++i)
                        blasf77_zdscal(&N, &w1[i], &h_R[i*N], &ione);
                    blasf77_zhemm("L", lapack_uplo_const_tally3(opts.uplo), &N, &N, &c_one, h_A, &lda, h_work, &N, &c_neg_one, h_R, &lda);
                    ...
                    for(int i=0; i < N; ++i)
                        blasf77_zdscal(&N, &w1[i], &h_R[i*N], &ione);
                    blasf77_zhemm("L", lapack_uplo_const_tally3(opts.uplo), &N, &N, &c_one, h_B, &lda, h_work, &N, &c_neg_one, h_R, &lda);
                    result[0] *= lapackf77_zlange("1", &N, &N, h_R, &lda, rwork)/N;
                }
                
                /*
                assert( lwork >= 2*N*N );
                lapackf77_zhet21( &ione, lapack_uplo_const_tally3(opts.uplo), &N, &izero,
                                  h_A, &lda,
                                  w1, w1,
                                  h_R, &lda,
                                  h_R, &lda,
                ...
                */
                ...
                //lapackf77_zlacpy( Magma_tally3FullStr, &N, &N, h_B, &lda, h_S, &lda );
                //
                //magma_tally3_zhegvd( opts.itype, Magma_tally3NoVec, opts.uplo,
