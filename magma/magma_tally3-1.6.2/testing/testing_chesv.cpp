...
int main( int argc, char** argv)
{
    ...
    magma_tally3FloatComplex *h_A, *h_B, *h_X, *work, temp;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_PIN( h_B,  magma_tally3FloatComplex, sizeB );
            TESTING_MALLOC_PIN( h_X,  magma_tally3FloatComplex, sizeB );
            ...
            if ( opts.lapack ) {
                lwork = -1;
                lapackf77_chesv(lapack_uplo_const_tally3(opts.uplo), &N, &opts.nrhs, 
            ...
            magma_tally3_chesv( opts.uplo, N, opts.nrhs, h_A, lda, ipiv, h_X, ldb, &info);
            gpu_time = magma_tally3_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally3_chesv returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
