...
int main( int argc, char** argv )
{
    ...
    double *C, *R, *A, *W, *tau;
    magma_tally3Double_ptr dC, dA, dT;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int itran = 0; itran < 2; ++itran ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {        
            ...
            magma_tally3_dsetmatrix( lda, k, A,  lda, dA, lda );
            magma_tally3_dgeqrf_gpu( lda, k, dA, lda, tau, dT, &info );
            magma_tally3_dgetmatrix( lda, k, dA, lda, A,  lda );
            if (info != 0)
            ...
            lapackf77_dormqr( lapack_side_const_tally3( side[iside] ), lapack_trans_const_tally3( trans[itran] ),
                              &m, &n, &k,
            ...
            lwork = -1;
            magma_tally3_dormqr_gpu( side[iside], trans[itran],
                              m, n, k,
                              dA, lda, tau, dC, ldc, W, lwork, dT, nb, &info );
            if (info != 0)
                printf("magma_tally3_dormqr_gpu (lwork query) returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
            lwork = (magma_tally3_int_t) MAGMA_tally3_D_REAL( W[0] );
            ...
            if (info != 0)
                printf("magma_tally3_dormqr_gpu returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
