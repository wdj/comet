/*
    -- MAGMA_minproduct (version 1.6.1) --
       ...
       @generated from zlange.cu normal z -> d, Fri Jan 30 19:00:09 2015
       @author Mark Gates
*/
...
max_nan_kernel( int n, T* x )
{
    ...
    magma_minproduct_max_nan_reduce< 512 >( tx, smax );
    if ( tx == 0 ) {
        x[0] = smax[0];
    }
}
    ...
    int m, int n, const double *A, int lda, double *dwork )
{
    ...
    if ( i < m ) {
        ...
        if ( n >= 4 ) {
            ...
            while( A < Aend ) {
                rsum[0] += fabs( rA[0] );  rA[0] = A[0];
                rsum[1] += fabs( rA[1] );  rA[1] = A[lda];
                rsum[2] += fabs( rA[2] );  rA[2] = A[2*lda];
                rsum[3] += fabs( rA[3] );  rA[3] = A[3*lda];
                A += 4*lda;
            }
    ...
    int m, int n, const double *A, int lda, double *dwork )
{
    ...
    if ( i < m ) {
        ...
        if ( n >= 4 ) {
            ...
            while( A < Aend ) {
                rmax[0] = max_nan( rmax[0], fabs( rA[0] ));  rA[0] = A[0];
                rmax[1] = max_nan( rmax[1], fabs( rA[1] ));  rA[1] = A[lda];
                rmax[2] = max_nan( rmax[2], fabs( rA[2] ));  rA[2] = A[2*lda];
                rmax[3] = max_nan( rmax[3], fabs( rA[3] ));  rA[3] = A[3*lda];
                A += 4*lda;
            }
            
            rmax[0] = max_nan( rmax[0], fabs( rA[0] ));
            rmax[1] = max_nan( rmax[1], fabs( rA[1] ));
            rmax[2] = max_nan( rmax[2], fabs( rA[2] ));
            rmax[3] = max_nan( rmax[3], fabs( rA[3] ));
        }
        ...
        switch( n_mod_4 ) {
            ...
            case 1:
                rmax[0] = max_nan( rmax[0], fabs( A[0] ));
                break;                          
                                                
            case 2:                             
                rmax[0] = max_nan( rmax[0], fabs( A[  0] ));
                rmax[1] = max_nan( rmax[1], fabs( A[lda] ));
                break;                          
                                                
            case 3:                             
                rmax[0] = max_nan( rmax[0], fabs( A[    0] ));
                rmax[1] = max_nan( rmax[1], fabs( A[  lda] ));
                rmax[2] = max_nan( rmax[2], fabs( A[2*lda] ));
                break;
        }
    ...
    int m, int n, const double *A, int lda, double *dwork )
{
    ...
    magma_minproduct_sum_reduce< 64 >( tx, ssum );
    if ( tx == 0 ) {
...
magma_minproductblas_dlange(
    magma_minproduct_norm_t norm, magma_minproduct_int_t m, magma_minproduct_int_t n,
    ...
    magma_minproductDouble_ptr dwork )  //, magma_minproduct_int_t lwork )
{
    ...
    if ( ! (norm == Magma_minproductInfNorm || norm == Magma_minproductMaxNorm || norm == Magma_minproductOneNorm) )
        info = -1;
    else if ( m < 0 )
        info = -2;
    else if ( n < 0 )
        ...
        info = -5;
    //else if ( ((norm == Magma_minproductInfNorm || norm == Magma_minproductMaxNorm) && (lwork < m)) ||
    //          ((norm == Magma_minproductOneNorm) && (lwork < n)) )
    //    info = -7;
    ...
    dim3 threads( 64 );
    double result = -1;
    if ( norm == Magma_minproductInfNorm ) {
        dim3 grid( (m-1)/64 + 1 );
        dlange_inf_kernel<<< grid, threads, 0, magma_minproduct_stream >>>( m, n, dA, ldda, dwork );
        max_nan_kernel<<< 1, 512, 0, magma_minproduct_stream >>>( m, dwork );
        cudaMemcpy( &result, &dwork[0], sizeof(double), cudaMemcpyDeviceToHost );
    }
    else if ( norm == Magma_minproductMaxNorm ) {
        dim3 grid( (m-1)/64 + 1 );
        dlange_max_kernel<<< grid, threads, 0, magma_minproduct_stream >>>( m, n, dA, ldda, dwork );
        max_nan_kernel<<< 1, 512, 0, magma_minproduct_stream >>>( m, dwork );
        cudaMemcpy( &result, &dwork[0], sizeof(double), cudaMemcpyDeviceToHost );
    }
    else if ( norm == Magma_minproductOneNorm ) {
        dim3 grid( n );
        dlange_one_kernel<<< grid, threads, 0, magma_minproduct_stream >>>( m, n, dA, ldda, dwork );
        max_nan_kernel<<< 1, 512, 0, magma_minproduct_stream >>>( n, dwork );
        cudaMemcpy( &result, &dwork[0], sizeof(double), cudaMemcpyDeviceToHost );
    }
