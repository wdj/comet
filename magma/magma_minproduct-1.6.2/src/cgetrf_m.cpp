...
#define dPT(d,i,j) (dPT[d] + (i)*nb*nb + (j)*nb*maxm)
    ...
    magma_minproductFloatComplex *dAT[Magma_minproductMaxGPUs], *dA[Magma_minproductMaxGPUs], *dPT[Magma_minproductMaxGPUs];
    ...
    *info = 0;
    ...
    /* initialize nb */
    nb = magma_minproduct_get_cgetrf_nb(m);
    maxm = ((m  + 31)/32)*32;

    /* figure out NB */
    size_t freeMem, totalMem;
    cudaMemGetInfo( &freeMem, &totalMem );
    freeMem /= sizeof(magma_minproductFloatComplex);
    ...
    if ( (nb <= 1) || (nb >= min(m,n)) ) {
        ...
        for( d=0; d < ngpu; d++ ) {
            ...
            if (MAGMA_minproduct_SUCCESS != magma_minproduct_cmalloc( &dA[d], (ldn_local+h*nb)*maxm )) {
        ...
        for( I=0; I < n; I += NB ) {
            ...
            timer_start( time );
            magma_minproductblas_csetmatrix_transpose_mgpu(ngpu, stream, A(0,I), lda,
            ...
            for( offset = 0; offset < min(m,I); offset += NB ) {
                ...
                for( d=0; d < ngpu; d++ ) {
                    magma_minproduct_setdevice(d);
                    nbi  = min( nb, NBk );
                    magma_minproduct_csetmatrix_async( (M-offset), nbi,
                                            A(offset,offset), lda,
                    ...
                    /* transpose */
                    magma_minproductblas_ctranspose( M-offset, nbi, dA[d], maxm-offset, dPT(d,0,0), nb );
                }
                
                /* applying the pivot from the previous big-panel */
                for( d=0; d < ngpu; d++ ) {
                    magma_minproduct_setdevice(d);
                    magma_minproductblas_claswp_q( ldn_local, dAT(d,0,0), ldn_local, offset+1, offset+NBk, ipiv, 1, stream[d][1] );
                }
                ...
                for( jj=0, ib=offset/nb; jj < NBk; jj += nb, ib++ ) {
                    ...
                    for( d=0; d < ngpu; d++ ) {
                        ...
                        if ( jj+nb < NBk ) {
                            magma_minproduct_csetmatrix_async( (M-ii-nb), min(nb,NBk-jj-nb),
                                                    A(ii+nb,ii+nb), lda,
                            ...
                            magma_minproductblas_ctranspose( M-ii-nb, nb, dA[d], rows-nb, dPT(d,0,(1+jj/nb)%2), nb );
                        }
                        
                        /* update with the block column */
                        magma_minproductblasSetKernelStream(stream[d][1]);
                        magma_minproduct_ctrsm( Magma_minproductRight, Magma_minproductUpper, Magma_minproductNoTrans, Magma_minproductUnit,
            ...
            if ( M > I ) {
                magma_minproduct_cgetrf2_mgpu(ngpu, M-I, N, nb, I, dAT, ldn_local, ipiv+I, dA, A(0,I), lda,
                                   stream, &iinfo);
                if ( iinfo < 0 ) {
                    *info = iinfo;
                    break;
            ...
            timer_start( time );
            magma_minproductblas_cgetmatrix_transpose_mgpu(ngpu, stream, dAT, ldn_local, A(0,I), lda, dA, maxm, M, N, nb);
            for( d=0; d < ngpu; d++ ) {
                magma_minproduct_setdevice(d);
                magma_minproduct_queue_sync( stream[d][0] );
                magma_minproduct_queue_sync( stream[d][1] );
    ...
    if ( *info >= 0 )
        magma_minproduct_cgetrf_piv(m, n, NB, A, lda, ipiv, info);
    return *info;
} /* magma_minproduct_cgetrf_m */
