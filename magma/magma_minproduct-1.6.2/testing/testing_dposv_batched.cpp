...
int main(int argc, char **argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            info = magma_minproduct_dposv_batched(opts.uplo, N, nrhs, dA_array, ldda, dB_array, lddb, dinfo_array, batchCount, queue); 
            ...
            {
                if(cpu_info[i] != 0 ){
                    printf("magma_minproduct_dposv_batched matrix %d returned internal error %d\n",i, (int)cpu_info[i] );
                }
            }
            if (info != 0)
                printf("magma_minproduct_dposv_batched returned argument error %d: %s.\n", (int) info, magma_minproduct_strerror( info ));
