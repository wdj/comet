    ...
    magma_minproduct_int_t *info )
{
    ...
    double *dwork[Magma_minproductMaxGPUs]={NULL}, *dpanel[Magma_minproductMaxGPUs]={NULL};
    double *hwork=NULL, *hpanel=NULL;
    magma_minproduct_queue_t stream[Magma_minproductMaxGPUs][2]={{NULL}};
    magma_minproduct_event_t panel_event[Magma_minproductMaxGPUs]={NULL};

    magma_minproduct_int_t i, j, min_mn, dev, ldhpanel, lddwork, rows;
    magma_minproduct_int_t ib, nb;
    magma_minproduct_int_t lhwork, lwork;
    magma_minproduct_int_t panel_dev, i_local, i_nb_local, n_local[Magma_minproductMaxGPUs], la_dev, dpanel_offset;
    ...
    *info = 0;
    ...
    for( dev=0; dev < ngpu; dev++ ) {
        ...
        if ( MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc( &(dwork[dev]), (lddwork + ldda)*nb )) {
    ...
    if ( MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc_pinned( &hwork, lwork )) {
    ...
    for( dev=0; dev < ngpu; dev++ ) {
        magma_minproduct_setdevice( dev );
        magma_minproduct_queue_create( &stream[dev][0] );
        magma_minproduct_queue_create( &stream[dev][1] );
        magma_minproduct_event_create( &panel_event[dev] );
    }
    ...
    if ( nb < min_mn ) {
        ...
        for( i = 0; i < min_mn-nb; i += nb ) {
            ...
            magma_minproduct_setdevice( panel_dev );
            magma_minproduct_queue_wait_event( stream[panel_dev][1], panel_event[panel_dev] );
            magma_minproduct_dgetmatrix_async( rows, ib,
                                    dlA(panel_dev, i, i_local), ldda,
                                    hpanel(i),                  ldhpanel, stream[panel_dev][1] );
            magma_minproduct_queue_sync( stream[panel_dev][1] );
            ...
            dpanel_to_q( Magma_minproductUpper, ib, hpanel(i), ldhpanel, hwork + ib*ib );
            ...
            for( dev=0; dev < ngpu; dev++ ) {
                ...
                magma_minproduct_dsetmatrix_async( rows, ib,
                                        hpanel(i),   ldhpanel,
                                        dpanel[dev], ldda, stream[dev][0] );
            }
            for( dev=0; dev < ngpu; dev++ ) {
                magma_minproduct_setdevice( dev );
                magma_minproduct_queue_sync( stream[dev][0] );
            }
            ...
            /* Restore the panel */
            dq_to_panel( Magma_minproductUpper, ib, hpanel(i), ldhpanel, hwork + ib*ib );

            if (i + ib < n) {
                /* Send the T matrix to the GPU. */
                for( dev=0; dev < ngpu; dev++ ) {
                    magma_minproduct_setdevice( dev );
                    magma_minproduct_dsetmatrix_async( ib, ib,
                                            hwork,      ib,
                ...
                for( dev=0; dev < ngpu; dev++ ) {
                    magma_minproduct_setdevice( dev );
                    magma_minproductblasSetKernelStream( stream[dev][0] );
                    if (dev == la_dev && i+nb < min_mn-nb) {
                        // If not last panel,
                        // for look-ahead panel, apply H' to A(i:m,i+ib:i+2*ib)
                        i_nb_local = (i+nb)/(nb*ngpu)*nb;
                        magma_minproduct_dlarfb_gpu( Magma_minproductLeft, Magma_minproductConjTrans, Magma_minproductForward, Magma_minproductColumnwise,
                                          rows, ib, ib,
                                          dpanel[dev],             ldda,       // V
                                          dwork[dev],              lddwork,    // T
                                          dlA(dev, i, i_nb_local), ldda,       // C
                                          dwork[dev]+ib,           lddwork );  // work
                        magma_minproduct_event_record( panel_event[dev], stream[dev][0] );
    ...
    if (i < min_mn) {
        ...
        for( j=i; j < n; j += nb ) {
            ...
            ib = min( n-j, nb );
            magma_minproduct_setdevice( panel_dev );
            magma_minproduct_dgetmatrix( rows, ib,
                              dlA(panel_dev, i, i_local), ldda,
                              hwork + (j-i)*rows,         rows );
        }
        ...
        for( j=i; j < n; j += nb ) {
            ...
            ib = min( n-j, nb );
            magma_minproduct_setdevice( panel_dev );
            magma_minproduct_dsetmatrix( rows, ib,
                              hwork + (j-i)*rows,         rows,
                              dlA(panel_dev, i, i_local), ldda );
        }
    }
...
CLEANUP:
    ...
    for( dev=0; dev < ngpu; dev++ ) {
        magma_minproduct_setdevice( dev );
        magma_minproduct_queue_destroy( stream[dev][0]   );
        magma_minproduct_queue_destroy( stream[dev][1]   );
        magma_minproduct_event_destroy( panel_event[dev] );
