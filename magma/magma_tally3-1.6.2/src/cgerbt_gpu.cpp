        ...
        magma_tally3FloatComplex* u, magma_tally3FloatComplex* v)
{
    ...
    for (idx=0; idx<n; idx++){
        ...
        u[idx] = MAGMA_tally3_C_MAKE(u1,u1);

        v[idx] = MAGMA_tally3_C_MAKE(v1,v1);
    }
}
    ...
    magma_tally3_int_t *info)
{
    ...
    *info = 0;
    ...
    magma_tally3FloatComplex *du, *dv;

    /* Allocate memory for the buterfly matrices */
    if (MAGMA_tally3_SUCCESS != magma_tally3_cmalloc( &du, 2*n )) {
    ...
    magma_tally3_csetvector( 2*n, U, 1, du, 1);
    magma_tally3_csetvector( 2*n, V, 1, dv, 1);

    /* Perform Partial Random Butterfly Transformation on the GPU*/
    magma_tally3blas_cprbt(n, dA, ldda, du, dv);

    /* Compute U^T.b on the GPU*/
    for(int i= 0; i < nrhs; i++)
        magma_tally3blas_cprbt_mtv(n, du, dB+(i*lddb));
