                  ...
                  magma_tally4_int_t batchCount, magma_tally4_queue_t queue)
{
    ...
    double *hu, *hv;
    if (MAGMA_tally4_SUCCESS != magma_tally4_dmalloc_cpu( &hu, 2*n )) {
        info = MAGMA_tally4_ERR_HOST_ALLOC;
        return info;
    }

    if (MAGMA_tally4_SUCCESS != magma_tally4_dmalloc_cpu( &hv, 2*n )) {
        info = MAGMA_tally4_ERR_HOST_ALLOC;
        return info;
    }
    ...
    info = magma_tally4_dgerbt_batched(Magma_tally4True, n, nrhs, dA_array, n, dB_array, n, hu, hv, &info, batchCount, queue);
    if (info != MAGMA_tally4_SUCCESS)  {
        return info;
    }


    info = magma_tally4_dgetrf_nopiv_batched( n, n, dA_array, ldda, info_array, batchCount, queue);
    if ( (info != MAGMA_tally4_SUCCESS) ){
        return info;
    }
    ...
    info = magma_tally4_dgetrs_nopiv_batched( Magma_tally4NoTrans, n, nrhs, dA_array, ldda, dB_array, lddb, info_array, batchCount, queue );
    ...
    magma_tally4_dsetvector(2*n, hv, 1, dv, 1);

    for(int i = 0; i < nrhs; i++)
        magma_tally4blas_dprbt_mv_batched(n, dv, dB_array+(i), batchCount, queue);
