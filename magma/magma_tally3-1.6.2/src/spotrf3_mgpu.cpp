...
#define STRSM_WORK
//#define magma_tally3_strsm magma_tally3blas_strsm
#endif
...
/**
    ...
    dA      REAL array on the GPU, dimension (LDDA,N)
            On entry, the symmetric matrix dA.  If UPLO = Magma_tally3Upper, the leading
    ...
    @ingroup magma_tally3_sposv_comp
    ********************************************************************/
...
#define dlPT(id, i, j, k) (d_lP[(id)] + (k)*nb*lddp + (j)*nb   + (i))
    ...
    magma_tally3_int_t     j, jb, nb0, nb2, d, dd, id, j_local, j_local2, buf;
    ...
    float *dlpanel;
    magma_tally3_int_t n_local[Magma_tally3MaxGPUs], ldpanel;
    const magma_tally3_int_t stream1 = 0, stream2 = 1, stream3 = 2;
    ...
    *info = 0;
    ...
    float *d_dinvA[Magma_tally3MaxGPUs];
    float *d_x[Magma_tally3MaxGPUs];
    ...
    for( d=0; d < ngpu; d++ ) {
        ...
        if ( (MAGMA_tally3_SUCCESS != magma_tally3_smalloc( &d_dinvA[d], 2*trsm_nb*trsm_n )) ||
             (MAGMA_tally3_SUCCESS != magma_tally3_smalloc( &d_x[d],     2*nb*(upper ? n : m) )) ) {
    ...
    if (upper) {
        ...
        for (j=0; j < m; j += nb) {
            ...
            if ( j > 0 ) {
                magma_tally3blasSetKernelStream( queues[id][stream1] );
                trace_gpu_start( id, stream1, "syrk", "syrk" );
                magma_tally3_ssyrk(Magma_tally3Upper, Magma_tally3ConjTrans, jb, j,
            ...
            magma_tally3_sgetmatrix_async( jb, jb,
            ...
            if ( j > 0 ) {
                ...
                for( dd=0; dd < ngpu; dd++ ) {
                    ...
                    if ( n_local[d] > nb0 ) {
                        magma_tally3_setdevice(d);
                        magma_tally3blasSetKernelStream( queues[d][stream2] );
                        if ( d == id ) {
                            dlpanel = dlA(d,0,nb*j_local);
                            ldpanel = ldda;
                            // the GPU owns the row from start, and no need of synch.
                            //magma_tally3_queue_wait_event( queues[d][stream2], events[d][0] ); // rows arrived at gpu
                            magma_tally3_queue_wait_event( queues[d][stream2], events[d][4] ); // wait for look-ahead trsm to finish
                        } else {
                        ...
                        magma_tally3_sgemm(Magma_tally3ConjTrans, Magma_tally3NoTrans,
                                    jb, n_local[d]-nb0, j,
                                    c_neg_one, dlpanel,        ldpanel,
                                               dlA(d, 0, nb0), ldda,
                                    c_one,     dlA(d, j, nb0), ldda);
                        trace_gpu_end( d, stream2 );
                        magma_tally3_event_record( events[d][2], queues[d][stream2] );
                    }
                    d = (d+1)%ngpu;
                }
            }
            ...
            magma_tally3_setdevice(id);
            magma_tally3_queue_sync( queues[id][stream1] );
            trace_cpu_start( 0, "getrf", "getrf" );
            lapackf77_spotrf(Magma_tally3UpperStr, &jb, Aup(j,j), &lda, info);
            trace_cpu_end( 0 );
            if (*info != 0) {
                *info = *info + j;
                break;
            ...
            if ( (j+jb) < n) {
                ...
                for( dd=0; dd < ngpu; dd++ ) {
                    ...
                    magma_tally3_ssetmatrix_async( jb, jb,
                                            Aup(j,j), lda,
                                            dlpanel,  ldpanel,
                                            queues[d][stream1] );
                    trace_gpu_end( d, stream1 );
                    magma_tally3_event_record( events[d][1], queues[d][stream1] );
                    d = (d+1)%ngpu;
                }
            } else {
                magma_tally3_setdevice(id);
            ...
            if ( (j+jb) < n) {
                ...
                for( dd=0; dd < ngpu; dd++ ) {
                    ...
                    if ( j+jb < m && d == (j/nb+1)%ngpu ) {
                        ...
                        nb0 = min(nb, nb2);
                        magma_tally3blasSetKernelStream( queues[d][stream1] );
                        magma_tally3_queue_wait_event( queues[d][stream1], events[d][2] ); // wait for gemm update
                        trace_gpu_start( d, stream1, "trsm", "trsm" );
#if (defined(PRECISION_d) || defined(PRECISION_s)) && defined(STRSM_WORK)
                        magma_tally3blas_slaset( Magma_tally3Full, trsm_nb, trsm_n, c_zero, c_zero, dinvA(d,0), trsm_nb );
                        magma_tally3blas_slaset( Magma_tally3Full, nb0,     jb,     c_zero, c_zero, dx(d,0), nb0 );
                        magma_tally3blas_strsm_work( Magma_tally3Left, Magma_tally3Upper,
                                              Magma_tally3ConjTrans, Magma_tally3NonUnit,
                                              jb, nb0, c_one,
                                              dlpanel, ldpanel,
                                              dlA(d, j, nb*j_local2), ldda,
                                              1, dinvA(d,0), dx(d,0) );
#else
                        magma_tally3_strsm( Magma_tally3Left, Magma_tally3Upper,
                                     Magma_tally3ConjTrans, Magma_tally3NonUnit,
                                     jb, nb0, c_one,
                                     dlpanel,                ldpanel,
                                     dlA(d, j, nb*j_local2), ldda);
#endif
                        magma_tally3_event_record( events[d][4], queues[d][stream1] );
                        trace_gpu_end( d, stream1 );
                    } else if ( nb2 > 0 ) {
                        /* update all the blocks on stream2 */
                        magma_tally3_queue_wait_event( queues[d][stream2], events[d][1] ); // wait for cholesky factor
                        trace_gpu_start( d, stream2, "trsm", "trsm" );
                        magma_tally3blasSetKernelStream( queues[d][stream2] );
#if (defined(PRECISION_d) || defined(PRECISION_s)) && defined(STRSM_WORK)
                        magma_tally3blas_slaset( Magma_tally3Full, trsm_nb, trsm_n, c_zero, c_zero, dinvA(d,0), trsm_nb );
                        magma_tally3blas_slaset( Magma_tally3Full, nb2,     jb,     c_zero, c_zero, dx(d,0), nb2 );
                        magma_tally3blas_strsm_work( Magma_tally3Left, Magma_tally3Upper,
                                              Magma_tally3ConjTrans, Magma_tally3NonUnit,
                                              jb, nb2, c_one,
                                              dlpanel, ldpanel,
                ...
                if ( j+jb < m ) {
                    ...
                    magma_tally3_setdevice(d);
                    /* lookahead done */
                    magma_tally3_queue_wait_event( queues[d][stream3], events[d][4] );
                
                    trace_gpu_start( d, stream3, "comm", "row to CPU" );
                    magma_tally3_sgetmatrix_async( (j+jb), nb0,
                                            dlA(d, 0, nb*j_local2), ldda,
                                            Aup(0,j+jb),            lda,
                                            queues[d][stream3] );
                    trace_gpu_end( d, stream3 );
                    magma_tally3_event_record( events[d][3], queues[d][stream3] );
                    /* needed on pluto */
                    //magma_tally3_queue_sync( queues[d][stream3] );
                
                    /* broadcast rows to gpus on stream2 */
                    buf2 = ((j+jb)/nb)%ngpu;
                    for( d2=0; d2 < ngpu; d2++ ) {
                        if ( d2 != d ) {
                            magma_tally3_setdevice(d2);
                            trace_gpu_start( d2, stream3, "comm", "row to GPUs" );
                            magma_tally3_queue_wait_event( queues[d2][stream3], events[d][3] ); // rows arrived at cpu on stream3
                            magma_tally3_ssetmatrix_async( j+jb, nb0,
                                                    Aup(0,j+jb),       lda,
                                                    dlP(d2,nb,0,buf2), lddp,
                                                    queues[d2][stream3] );
                            trace_gpu_end( d2, stream3 );
                            magma_tally3_event_record( events[d2][0], queues[d2][stream3] );
                        }
                    }
                    ...
                    if ( nb2 > 0 ) {
                        ...
                        int flag = 0;
                        if (flag == 0) {
                            magma_tally3_queue_wait_event( queues[d][stream2], events[d][4] ); // lookahead -> diagonal inversion
                        } else {
                            magma_tally3blas_slaset( Magma_tally3Full, trsm_nb, trsm_n, c_zero, c_zero, dinvA(d,flag), trsm_nb );
                            magma_tally3_queue_wait_event( queues[d][stream2], events[d][1] ); // panel received
                        }
                        magma_tally3blas_slaset( Magma_tally3Full, nb2, jb, c_zero, c_zero, dx(d,1), nb2 );
                        magma_tally3blas_strsm_work( Magma_tally3Left, Magma_tally3Upper, Magma_tally3ConjTrans, Magma_tally3NonUnit,
                                              jb, nb2, c_one,
                                              dlpanel, ldpanel,
        ...
        for (j=0; j < n; j += nb) {
            ...
            if ( j > 0 ) {
                magma_tally3blasSetKernelStream( queues[id][stream1] );
                magma_tally3_ssyrk(Magma_tally3Lower, Magma_tally3NoTrans, jb, j,
            ...
            if ( j > 0 ) {
                ...
                for( dd=0; dd < ngpu; dd++ ) {
                    ...
                    if ( nb0 < n_local[d] ) {
                        ...
                        magma_tally3_sgemm( Magma_tally3NoTrans, Magma_tally3ConjTrans,
                                     n_local[d]-nb0, jb, j,
            ...
            magma_tally3_setdevice(id);
            magma_tally3_queue_sync( queues[id][stream1] );
            lapackf77_spotrf(Magma_tally3LowerStr, &jb, Alo(j,j), &lda, info);
            if (*info != 0) {
                *info = *info + j;
                break;
            }
            ...
            if ( (j+jb) < m) {
                ...
                for( dd=0; dd < ngpu; dd++ ) {
                    ...
                    if ( j+nb < n && d == (j/nb+1)%ngpu ) { /* owns next column, look-ahead next block on stream1 */
                        if ( j > 0 ) magma_tally3_queue_wait_event( queues[d][stream1], events[d][2] ); // wait for gemm update
                        magma_tally3blasSetKernelStream( queues[d][stream1] );
#if (defined(PRECISION_d) || defined(PRECISION_s)) && defined(STRSM_WORK)
                        magma_tally3blas_slaset( Magma_tally3Full, trsm_nb, trsm_n, c_zero, c_zero, dinvA(d,0), trsm_nb );
                        magma_tally3blas_slaset( Magma_tally3Full, nb0,     jb,     c_zero, c_zero, dx(d,0), nb0 );
                        magma_tally3blas_strsm_work( Magma_tally3Right, Magma_tally3Lower,
                                              Magma_tally3ConjTrans, Magma_tally3NonUnit,
                                              nb0, jb, c_one,
                                              dlpanel, ldpanel,
                                              ...
                                              1, dinvA(d,0), dx(d,0) );
#else
                        magma_tally3_strsm( Magma_tally3Right, Magma_tally3Lower,
                                     Magma_tally3ConjTrans, Magma_tally3NonUnit,
                                     nb0, jb, c_one,
                                     dlpanel, ldpanel,
                                     dlA(d, nb*j_local2, j), ldda);
                        ...
                        magma_tally3_queue_wait_event( queues[d][stream2], events[d][1] ); // wait for the cholesky factor
                        magma_tally3blasSetKernelStream( queues[d][stream2] );
#if (defined(PRECISION_d) || defined(PRECISION_s)) && defined(STRSM_WORK)
                        magma_tally3blas_slaset( Magma_tally3Full, trsm_nb, trsm_n, c_zero, c_zero, dinvA(d,0), trsm_nb );
                        magma_tally3blas_slaset( Magma_tally3Full, nb2,     jb,     c_zero, c_zero, dx(d,0), nb2 );
                        magma_tally3blas_strsm_work( Magma_tally3Right, Magma_tally3Lower, Magma_tally3ConjTrans, Magma_tally3NonUnit,
                                              nb2, jb, c_one,
                                              dlpanel,                ldpanel,
                                              dlA(d, nb*j_local2, j), ldda,
                                              1, dinvA(d,0), dx(d,0) );
...
//#define SPOTRF_DEVICE_TO_DEVICE
                    ...
                    for( d2=0; d2 < ngpu; d2++ ) {
                        magma_tally3_setdevice(d2);
                        magma_tally3_queue_wait_event( queues[d2][stream3], events[d][4] );
                        if ( d2 != d ) {
                            magma_tally3_scopymatrix_async( nb0, j+jb,
                                                     dlPT(d2,0,nb,buf2), nb, // first nbxnb reserved for diagonal block
                                                     dlA(d, nb*j_local2, 0), ldda,
                                                     queues[d2][stream3] );
                            magma_tally3_event_record( events[d2][0], queues[d2][stream3] );
                        } else {
                            magma_tally3_sgetmatrix_async( nb0, j+jb,
                                                    dlA(d, nb*j_local2, 0), ldda,
                                                    Alo(j+jb,0),            lda,
                                                    queues[d][stream3] );
                        }
                    ...
                    for( d2=0; d2 < ngpu; d2++ ) {
                        if ( d2 != d ) {
                            magma_tally3_setdevice(d2);
                            magma_tally3_queue_wait_event( queues[d2][stream3], events[d][3] ); // getmatrix done
                            magma_tally3_ssetmatrix_async( nb0, j+jb,
                                                    Alo(j+jb,0),        lda,
                                                    dlPT(d2,0,nb,buf2), nb, // first nbxnb reserved for diagonal block
                                                    queues[d2][stream3] );
                            magma_tally3_event_record( events[d2][0], queues[d2][stream3] );
    ...
    for( d=0; d < ngpu; d++ ) {
        magma_tally3_setdevice(d);
        for( j=0; j < 3; j++ ) {
            magma_tally3_queue_sync( queues[d][j] );
        }
#if (defined(PRECISION_d) || defined(PRECISION_s)) && defined(STRSM_WORK)
        magma_tally3_free( d_dinvA[d] );
        magma_tally3_free( d_x[d] );
#endif
    }
    ...
    magma_tally3_int_t *info)
{
    ...
    if (uplo == Magma_tally3Upper) {
        ...
        for (j=off_j; j < n; j += nb) {
            ...
            magma_tally3_ssetmatrix_async( mj, jb,
                                    A(off_i, j),     lda,
                                    dA(k, 0, jj*nb), ldda,
                                    queues[k][0] );
        }
        ...
        for (i=off_i; i < m; i += nb) {
            ...
            magma_tally3_setdevice(k);
            magma_tally3_ssetmatrix_async( ib, ni,
                                    A(i, off_j),     lda,
                                    dA(k, ii*nb, 0), ldda,
                                    queues[k][0] );
        }
    ...
    magma_tally3_int_t *info)
{
    ...
    if (uplo == Magma_tally3Upper) {
        ...
        for (j=off_j+NB; j < n; j += nb) {
            ...
            magma_tally3_setdevice(k);
            magma_tally3_sgetmatrix_async( mj, jb,
                                    dA(k, 0, jj*nb), ldda,
                                    A(off_i, j),     lda,
                                    queues[k][0] );
            magma_tally3_queue_sync( queues[k][0] );
        ...
        for (i=off_i+NB; i < m; i += nb) {
            ...
            magma_tally3_setdevice(k);
            magma_tally3_sgetmatrix_async( ib, ni,
                                    dA(k, ii*nb, 0), ldda,
                                    A(i, off_j),     lda,
                                    queues[k][0] );
            magma_tally3_queue_sync( queues[k][0] );
