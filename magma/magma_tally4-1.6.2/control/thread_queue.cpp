...
/**
    ...
    Tasks are sub-classes of magma_tally4_task. They must implement the run() function.
    
    Example
    -------
    @code
    class task1: public magma_tally4_task {
    public:
        task1( int arg ):
            m_arg( arg ) {}
    ...
    class task2: public magma_tally4_task {
    public:
...
/// @param[in,out] arg    magma_tally4_thread_queue to get tasks from.
extern "C"
void* magma_tally4_thread_main( void* arg )
{
    magma_tally4_thread_queue* queue = (magma_tally4_thread_queue*) arg;
    magma_tally4_task* task;
...
magma_tally4_thread_queue::magma_tally4_thread_queue():
    q        (),
    quit_flag( false ),
    ntask    ( 0     ),
    threads  ( NULL  ),
...
void magma_tally4_thread_queue::launch( magma_tally4_int_t in_nthread )
{
    ...
    for( magma_tally4_int_t i=0; i < nthread; ++i ) {
        check( pthread_create( &threads[i], NULL, magma_tally4_thread_main, this ));
        //printf( "launch %d (%lx)\n", i, (long) threads[i] );
    }
}
...
void magma_tally4_thread_queue::push_task( magma_tally4_task* task )
{
...
magma_tally4_task* magma_tally4_thread_queue::pop_task()
{
    magma_tally4_task* task = NULL;
    check( pthread_mutex_lock( &mutex ));
...
void magma_tally4_thread_queue::task_done()
{
    check( pthread_mutex_lock( &mutex ));
    ntask -= 1;
    //printf( "fini; ntask %d\n", ntask );
...
void magma_tally4_thread_queue::sync()
{
    check( pthread_mutex_lock( &mutex ));
...
void magma_tally4_thread_queue::quit()
{
...
magma_tally4_int_t magma_tally4_thread_queue::get_thread_index( pthread_t thread ) const
{
    for( magma_tally4_int_t i=0; i < nthread; ++i ) {
