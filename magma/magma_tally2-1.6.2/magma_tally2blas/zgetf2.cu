...
void magma_tally2_zgetf2_swap(
    magma_tally2_int_t n, magma_tally2DoubleComplex *x, magma_tally2_int_t i, magma_tally2_int_t j, magma_tally2_int_t incx);

void magma_tally2_zscal_zgeru(
    ...
    magma_tally2_int_t *info )
{
    ...
    *info = 0;
    ...
    for( j=0; j < min_mn; j++ ) {
        ...
        jp = j - 1 + magma_tally2_izamax(m-j, dA(j,j), 1);
        ...
        //if ( dA(jp, j) != 0.0) {
            ...
            if (jp != j) {
                magma_tally2_zgetf2_swap(n, dA, j, jp, ldda);
            }
            
            // Compute elements J+1:M of J-th column.
            if (j < m) {
                magma_tally2_zscal_zgeru(m-j, n-j, dA(j, j), ldda);
            }
        //}
        //else if (*info == 0) {
        //    *info = j;
...
__global__
void kernel_zswap(int n, magma_tally2DoubleComplex *x, int i, int j, int incx)
{
    int id = blockIdx.x * zswap_bs + threadIdx.x;

    if (id < n) {
        magma_tally2DoubleComplex tmp = x[i + incx*id];
...
void magma_tally2_zgetf2_swap(magma_tally2_int_t n, magma_tally2DoubleComplex *x, magma_tally2_int_t i, magma_tally2_int_t j, magma_tally2_int_t incx)
{
/*
...
*/
    ...
    kernel_zswap<<< grid, threads, 0, magma_tally2_stream >>>(n, x, i, j, incx);
}
...
__global__
void kernel_zscal_zgeru(int m, int n, magma_tally2DoubleComplex *A, int lda)
{
    ...
    if (tid < m && tid > 0) {
        ...
        #pragma unroll
        for(int i=1; i < n; i++) {
            A[tid + i*lda] += (MAGMA_tally2_Z_NEG_ONE) * shared_y[i] * reg;
        }
    }
}


void magma_tally2_zscal_zgeru(magma_tally2_int_t m, magma_tally2_int_t n, magma_tally2DoubleComplex *A, magma_tally2_int_t lda)
{
/*
...
*/
    ...
    kernel_zscal_zgeru<<< grid, threads, shared_size, magma_tally2_stream>>>(m, n, A, lda);
}
