...
/**
    ...
    tau     DOUBLE_PRECISION array, dimension (N)
            ...
            reflector H(i), as returned by MAGMA_tally3_DGEQRF_GPU.
    ...
    dT      DOUBLE_PRECISION array that is the output (the 6th argument)
            of magma_tally3_dgeqrf_gpu of size
    ...
    lwork   INTEGER
            ...
            where NB is the blocksize given by magma_tally3_get_dgeqrf_nb( M ).
    \n
    ...
    @ingroup magma_tally3_dgels_comp
    ********************************************************************/
    ...
    magma_tally3_int_t *info)
{
    ...
    magma_tally3_int_t nb     = magma_tally3_get_dgeqrf_nb(m);
    magma_tally3_int_t lwkopt = (m - n + nb)*(nrhs + nb) + nrhs*nb;
    int lquery = (lwork == -1);

    hwork[0] = MAGMA_tally3_D_MAKE( (double)lwkopt, 0. );
    ...
    *info = 0;
    ...
    /* B := Q' * B */
    magma_tally3_dormqr_gpu( Magma_tally3Left, Magma_tally3Trans,
                      m, nrhs, n,
                      dA(0,0), ldda, tau,
    ...
    // TODO: this assumes that, on exit from magma_tally3_dormqr_gpu, hwork contains
    ...
    // magma_tally3_dtrsm and drop the dsetmatrix.
    if ( nrhs == 1 ) {
        blasf77_dtrsv( Magma_tally3UpperStr, Magma_tally3NoTransStr, Magma_tally3NonUnitStr,
                       &ib, hwork,         &rows,
                            hwork+rows*ib, &ione);
    } else {
        blasf77_dtrsm( Magma_tally3LeftStr, Magma_tally3UpperStr, Magma_tally3NoTransStr, Magma_tally3NonUnitStr,
                       &ib, &nrhs,
                       &c_one, hwork,         &rows,
                               hwork+rows*ib, &rows);
    }
    ...
    magma_tally3_dsetmatrix( ib, nrhs, hwork+rows*ib, rows, dwork+i, lddwork );

    // update c
    if (nrhs == 1)
        magma_tally3_dgemv( Magma_tally3NoTrans, i, ib,
    ...
    if (nb < k) {
        for (i = start; i >= 0; i -= nb) {
            ...
            if (i + ib < n) {
                if (nrhs == 1) {
                    magma_tally3_dgemv( Magma_tally3NoTrans, ib, ib,
                                 c_one,  dT(i), ib,
                                         dB+i,      1,
                                 c_zero, dwork+i,  1);
                    magma_tally3_dgemv( Magma_tally3NoTrans, i, ib,
    ...
    magma_tally3_dcopymatrix( (n), nrhs,
                       dwork, lddwork,
                       dB,    lddb );
