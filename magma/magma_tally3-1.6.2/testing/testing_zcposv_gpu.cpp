...
int main(int argc, char **argv)
{
    ...
    magma_tally3_int_t N, nrhs, posv_iter, info, size;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_DEV( d_A,     magma_tally3DoubleComplex, lda*N        );
            TESTING_MALLOC_DEV( d_B,     magma_tally3DoubleComplex, ldb*nrhs     );
            TESTING_MALLOC_DEV( d_X,     magma_tally3DoubleComplex, ldx*nrhs     );
            TESTING_MALLOC_DEV( d_works, magma_tally3FloatComplex,  lda*(N+nrhs) );
            ...
            magma_tally3_zsetmatrix( N, N,    h_A, lda, d_A, lda );
            magma_tally3_zsetmatrix( N, nrhs, h_B, ldb, d_B, ldb );
            
            //=====================================================================
            //              Mixed Precision Iterative Refinement - GPU
            //=====================================================================
            gpu_time = magma_tally3_wtime();
            magma_tally3_zcposv_gpu(opts.uplo, N, nrhs, d_A, lda, d_B, ldb, d_X, ldx,
                             d_workd, d_works, &posv_iter, &info);
            gpu_time = magma_tally3_wtime() - gpu_time;
            gpu_perf = gflopsS / gpu_time;
            if (info != 0)
                printf("magma_tally3_zcposv returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
            
            //=====================================================================
            //                 Error Computation
            //=====================================================================
            magma_tally3_zgetmatrix( N, nrhs, d_X, ldx, h_X, ldx ) ;
            
            Anorm = lapackf77_zlanhe( "I", lapack_uplo_const_tally3(opts.uplo), &N, h_A, &N, h_workd);
            blasf77_zhemm( "L", lapack_uplo_const_tally3(opts.uplo), &N, &nrhs,
                           &c_one,     h_A, &lda,
                                       h_X, &ldx,
                           &c_neg_one, h_B, &ldb);
            ...
            gpu_time = magma_tally3_wtime();
            magma_tally3_zpotrf_gpu(opts.uplo, N, d_A, lda, &info);
            gpu_time = magma_tally3_wtime() - gpu_time;
            gpu_perfdf = gflopsF / gpu_time;
            if (info != 0)
                printf("magma_tally3_zpotrf returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
            ...
            gpu_time = magma_tally3_wtime();
            magma_tally3_zpotrf_gpu(opts.uplo, N, d_A, lda, &info);
            magma_tally3_zpotrs_gpu(opts.uplo, N, nrhs, d_A, lda, d_B, ldb, &info);
            gpu_time = magma_tally3_wtime() - gpu_time;
            gpu_perfds = gflopsS / gpu_time;
            if (info != 0)
                printf("magma_tally3_zpotrs returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
            ...
            magma_tally3_zsetmatrix( N, N,    h_A, lda, d_A, lda );
            magma_tally3_zsetmatrix( N, nrhs, h_B, ldb, d_B, ldb );
            magma_tally3blas_zlag2c( N, N,    d_A, lda, d_As, N, &info );
            magma_tally3blas_zlag2c( N, nrhs, d_B, ldb, d_Bs, N, &info );
            
            gpu_time = magma_tally3_wtime();
            magma_tally3_cpotrf_gpu(opts.uplo, N, d_As, N, &info);
            gpu_time = magma_tally3_wtime() - gpu_time;
            ...
            gpu_time = magma_tally3_wtime();
            magma_tally3_cpotrf_gpu(opts.uplo, N, d_As, lda, &info);
            magma_tally3_cpotrs_gpu(opts.uplo, N, nrhs, d_As, N, d_Bs, N, &info);
            gpu_time = magma_tally3_wtime() - gpu_time;
            gpu_perfss = gflopsS / gpu_time;
            if (info != 0)
                printf("magma_tally3_cpotrs returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
