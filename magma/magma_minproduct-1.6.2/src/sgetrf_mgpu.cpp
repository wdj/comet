    ...
    magma_minproduct_int_t *info)
{
    ...
    float *d_lAT[Magma_minproductMaxGPUs];
    float *d_panel[Magma_minproductMaxGPUs], *work;
    ...
    *info = 0;
    ...
    if (nb <= 1 || nb >= n) {
        /* Use CPU code. */
        magma_minproduct_smalloc_cpu( &work, m * n );
        if ( work == NULL ) {
            *info = MAGMA_minproduct_ERR_HOST_ALLOC;
            return *info;
        }
        magma_minproduct_sgetmatrix( m, n, d_lA[0], ldda, work, m );
        lapackf77_sgetrf(&m, &n, work, &m, ipiv, info);
        magma_minproduct_ssetmatrix( m, n, work, m, d_lA[0], ldda );
        ...
        for (i=0; i < ngpu; i++) {
            ...
            /* workspaces */
            if (MAGMA_minproduct_SUCCESS != magma_minproduct_smalloc( &d_panel[i], (3+ngpu)*nb*maxm )) {
                for( j=0; j <= i; j++ ) {
            ...
            if (MAGMA_minproduct_SUCCESS != magma_minproduct_smalloc( &d_lAT[i], lddat*maxm )) {
                for( j=0; j <= i; j++ ) {
                    magma_minproduct_setdevice(j);
                    magma_minproduct_free( d_panel[j] );
                }
            ...
            magma_minproductblas_stranspose( m, n_local[i], d_lA[i], ldda, d_lAT[i], lddat );
        }
        for (i=0; i < ngpu; i++) {
            magma_minproduct_setdevice(i);
        ...
        /* cpu workspace */
        lddwork = maxm;
        if (MAGMA_minproduct_SUCCESS != magma_minproduct_smalloc_pinned( &work, lddwork*nb*ngpu )) {
            for (i=0; i < ngpu; i++ ) {
                magma_minproduct_setdevice(i);
        ...
        magma_minproduct_sgetrf2_mgpu(ngpu, m, n, nb, 0, d_lAT, lddat, ipiv, d_panel, work, maxm,
                           streaml, info);
        ...
        for( d=0; d < ngpu; d++ ) {
            ...
            /* save on output */
            magma_minproductblas_stranspose( n_local[d], m, d_lAT[d], lddat, d_lA[d], ldda );
