...
zmgesellptmv_kernel_8_3D( 
    ...
    magma_tally4_index_t * dcolind,
    magma_tally4_index_t * drowptr,
    const magma_tally4FloatComplex * __restrict__ dx,
    magma_tally4FloatComplex beta, 
    magma_tally4FloatComplex * dy)
{
   // T threads assigned to each row
    ...
    magma_tally4FloatComplex * dy)
{
    ...
    if(row < num_rows ){
        magma_tally4FloatComplex dot1 = MAGMA_tally4_C_MAKE(0.0, 0.0);
        magma_tally4FloatComplex dot2 = MAGMA_tally4_C_MAKE(0.0, 0.0);
        int offset = drowptr[ bdx ];
        int max_ = (drowptr[ bdx+1 ]-offset)/blocksize;  
            // number of elements each thread handles
    ...
    magma_tally4_queue_t queue )
{
    ...
    if ( (texture==1) && (precision==1) && (kepler==1) ) {
        ...
        int Ms = num_vecs * blocksize*alignment * sizeof( magma_tally4FloatComplex );


        if ( alignment == 1) {
            dim3 block( blocksize, num_vecs/2, 1 );
            if ( beta == MAGMA_tally4_C_MAKE( 0.0, 0.0 ) )
        ...
        dim3 grid( dimgrid1, dimgrid2, 1);
        int Ms =  num_threads * sizeof( magma_tally4FloatComplex );
