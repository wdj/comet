    ...
    magma_tally2_queue_t queue)
{
    ...
    magma_tally2blas_dapply_transpose_vector_kernel<<< grid, threads, 0, queue >>>(n/2, du, n, db, 0);
    magma_tally2blas_dapply_transpose_vector_kernel<<< grid, threads, 0, queue >>>(n/2, du, n+n/2, db, n/2);

    threads = block_length;
    grid = n/(2*block_length) + ((n%(2*block_length))!=0);
    magma_tally2blas_dapply_transpose_vector_kernel<<< grid, threads, 0, queue >>>(n, du, 0, db, 0);
}

/**
    @see magma_tally2blas_dprbt_mtv_q
    ********************************************************************/
extern "C" void
magma_tally2blas_dprbt_mtv(
    ...
    double *du, double *db)
{
    magma_tally2blas_dprbt_mtv_q(n, du, db, magma_tally2_stream);
}
    ...
    magma_tally2_queue_t queue)
{
    ...
    magma_tally2blas_dapply_vector_kernel<<< grid, threads, 0, queue >>>(n, dv, 0, db, 0);


    threads = block_length;
    grid = n/(4*block_length) + ((n%(4*block_length))!=0);

    magma_tally2blas_dapply_vector_kernel<<< grid, threads, 0, queue >>>(n/2, dv, n, db, 0);
    magma_tally2blas_dapply_vector_kernel<<< grid, threads, 0, queue >>>(n/2, dv, n+n/2, db, n/2);
}
...
magma_tally2blas_dprbt_mv(
    ...
    double *dv, double *db)
{
    magma_tally2blas_dprbt_mv_q(n, dv, db, magma_tally2_stream);
}
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
/**
    Purpose
    ...
    magma_tally2_queue_t queue)
{
    ...
    magma_tally2blas_delementary_multiplication_kernel<<< grid, threads, 0, queue >>>(n/2, dA,            0, ldda, du,   0, dv,   0);
    magma_tally2blas_delementary_multiplication_kernel<<< grid, threads, 0, queue >>>(n/2, dA,     ldda*n/2, ldda, du,   0, dv, n/2);
    magma_tally2blas_delementary_multiplication_kernel<<< grid, threads, 0, queue >>>(n/2, dA,          n/2, ldda, du, n/2, dv,   0);
    magma_tally2blas_delementary_multiplication_kernel<<< grid, threads, 0, queue >>>(n/2, dA, ldda*n/2+n/2, ldda, du, n/2, dv, n/2);

    dim3 threads2(block_height, block_width);
    dim3 grid2(n/(2*block_height) + ((n%(2*block_height))!=0), 
            n/(2*block_width)  + ((n%(2*block_width))!=0));
    magma_tally2blas_delementary_multiplication_kernel<<< grid2, threads2, 0, queue >>>(n, dA, 0, ldda, du, -ldda, dv, -ldda);
}


/**
    @see magma_tally2blas_dprbt_q
    ********************************************************************/
extern "C" void 
magma_tally2blas_dprbt(
    ...
    double *du, double *dv)
{
    magma_tally2blas_dprbt_q(n, dA, ldda, du, dv, magma_tally2_stream);
}
    ...
    const double *b)
{
    ...
    if ( i < m ) {
        x[i] = MAGMA_tally2_D_ADD( x[i], r[i] );
        r[i] = b[i];
    }
}
...
magma_tally2blas_daxpycp2(
    ...
    const double *b)
{
    magma_tally2blas_daxpycp2_q( m, r, x, b, magma_tally2_stream );
}
