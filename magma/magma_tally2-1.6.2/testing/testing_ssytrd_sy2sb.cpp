...
int main( int argc, char** argv)
{
    ...
    NB = opts.nb;
    if (NB < 1)
        NB  = 64; //64; //magma_tally2_get_ssytrd_sy2sb_nb(N);

    // what is NE ?
    if (NE < 1)
        NE  = 64; //N;  //magma_tally2_get_ssytrd_sy2sb_nb(N);  // N not yet initialized
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            /*
            magma_tally2_ssytrd_sy2sb( opts.uplo, N, NB, h_R, lda, tau, h_work, lwork0, dT1, THREADS, &info);
            tband = magma_tally2_wtime - gpu_time();
            printf("  Finish BAND  N %d  NB %d  ngpu %d timing= %f\n", N, NB, ngpu, tband);
            magma_tally2_ssytrd_bsy2trc_v5(THREADS, WANTZ, opts.uplo, NE, N, NB, h_R, lda, D, E, dT1, ldt);
            */
    
            /*
            magma_tally2_ssytrd_sy2sb( opts.uplo, N, NB, h_R, lda, tau, h_work, lwork, dT1, THREADS, &info);
            tband = magma_tally2_wtime - gpu_time();
            printf("  Finish BAND  N %d  NB %d  ngpu %d timing= %f\n", N, NB, ngpu, tband);
            magma_tally2_ssytrd_bsy2trc(THREADS, WANTZ, opts.uplo, NE, N, NB, h_R, lda, D, E, dT1, ldt);
            */
            ...
            if (ngpu == 1) {
                ...
                magma_tally2_ssyevdx_2stage_m(ngpu, opts.jobz, range, opts.uplo, N,
                                h_R, lda,
                                vl, vu, il, iu,
                                &m1, D,
                                hh_work, lwork,
            ...
            */
            ...
            if ( opts.check ) {
                ...
                magma_tally2_smalloc_cpu( &work2, lwork2 );
                magma_tally2_smalloc_cpu( &rwork2, N );
                magma_tally2_smalloc_cpu( &D2, N );
                magma_tally2_smalloc_cpu( &AINIT, N*lda );
                ...
                */
                ...
                //float *Z;
                // magma_tally2_smalloc_cpu( &Z, N*lda );
                ...
                // TODO free this memory !
                magma_tally2_smalloc_cpu( &WORKAJETER, (2* N * N + N)  );
