...
__global__ void
magma_tally4_clobpcg_shift_kernel( 
    ...
    magma_tally4FloatComplex * x )
{
    ...
    if( row<num_rows){
        magma_tally4FloatComplex tmp = x[idx];
        __syncthreads();
...
{
    magma_tally4_int_t num_threads = num_vecs;
    ...
    int dimgrid2 = magma_tally4_ceildiv( num_rows, dimgrid1 );

    dim3 grid( dimgrid1, dimgrid2, 1);

    magma_tally4_clobpcg_shift_kernel<<< grid, block, Ms, queue >>>
