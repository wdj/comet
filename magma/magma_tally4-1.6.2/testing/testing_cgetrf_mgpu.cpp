    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for( int dev=0; dev < ngpu; dev++ ) {
                ...
                TESTING_MALLOC_DEV( d_lA[dev], magma_tally4FloatComplex, ldda*ldn_local );
            }
            ...
            /* ====================================================================
               ...
               =================================================================== */
            init_matrix( M, N, h_A, lda );
            magma_tally4_csetmatrix_1D_col_bcyclic( M, N, h_A, lda, d_lA, ldda, ngpu, nb );
    
            gpu_time = magma_tally4_wtime();
            magma_tally4_cgetrf_mgpu( ngpu, M, N, d_lA, ldda, ipiv, &info );
            gpu_time = magma_tally4_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally4_cgetrf_mgpu returned error %d: %s.\n",
                       (int) info, magma_tally4_strerror( info ));
                       
            magma_tally4_cgetmatrix_1D_col_bcyclic( M, N, d_lA, ldda, h_A, lda, ngpu, nb );
