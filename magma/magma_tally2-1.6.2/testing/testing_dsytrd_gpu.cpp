...
int main( int argc, char** argv)
{
    ...
    magma_tally2Double_ptr d_R, dwork;
    double *tau;
    double          *diag, *offdiag;
    double           result[2] = {0., 0.};
    ...
    printf("2 - uses DSYMV from MAGMA_tally2 BLAS that requires extra space\n\n");
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_tally2_dsetmatrix( N, N, h_A, lda, d_R, ldda );
            ...
            if (opts.version == 1) {
                magma_tally2_dsytrd_gpu( opts.uplo, N, d_R, ldda, diag, offdiag,
                                  tau, h_R, lda, h_work, lwork, &info );
            }
            else {
                magma_tally2_dsytrd2_gpu( opts.uplo, N, d_R, ldda, diag, offdiag,
                                   tau, h_R, lda, h_work, lwork, dwork, ldwork, &info );
            }
            gpu_time = magma_tally2_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally2_dsytrd_gpu returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                magma_tally2_dgetmatrix( N, N, d_R, ldda, h_R, lda );
                magma_tally2_dgetmatrix( N, N, d_R, ldda, h_Q, lda );
                lapackf77_dorgtr( lapack_uplo_const_tally2(opts.uplo), &N, h_Q, &lda, tau, h_work, &lwork, &info );
                
                lapackf77_dsyt21( &itwo, lapack_uplo_const_tally2(opts.uplo), &N, &ione,
                ...
                lapackf77_dsyt21( &ithree, lapack_uplo_const_tally2(opts.uplo), &N, &ione,
                                  h_A, &lda, diag, offdiag,
                                  h_Q, &lda, h_R, &lda,
                                  tau, work,
                                  #ifdef COMPLEX
            ...
            if ( opts.lapack ) {
                ...
                lapackf77_dsytrd( lapack_uplo_const_tally2(opts.uplo), &N, h_A, &lda, diag, offdiag, tau,
                                  h_work, &lwork, &info );
