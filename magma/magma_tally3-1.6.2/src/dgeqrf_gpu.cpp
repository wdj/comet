...
*/
void dsplit_diag_block(magma_tally3_int_t ib, double *a, magma_tally3_int_t lda, double *work)
{
    ...
    lapackf77_dtrtri( Magma_tally3UpperStr, Magma_tally3NonUnitStr, &ib, work, &ib, &info);
}
    ...
    *info = 0;
    ...
    if ( (nb > 1) && (nb < k) ) {
        ...
        for (i = 0; i < k-nb; i += nb) {
            ...
            magma_tally3_dsetmatrix( rows, ib, work(i), ldwork, dA(i,i), ldda );

            if (i + ib < n) {
                /* Send the triangular factor T to the GPU */
                magma_tally3_dsetmatrix( ib, ib, hwork, ib, dT(i), nb );
                ...
                if (i+nb < k-nb) {
                    ...
                    magma_tally3_dsetmatrix( ib, ib, ut, ib, d_ref(i), ib );
                }
                old_i  = i;
                old_ib = ib;
            }
    ...
    if (i < k) {
        ib   = n-i;
        rows = m-i;
        magma_tally3_dgetmatrix( rows, ib, dA(i, i), ldda, work, rows );
        lhwork = lwork - rows*ib;
        lapackf77_dgeqrf(&rows, &ib, work, &rows, tau+i, work+ib*rows, &lhwork, info);
        
        magma_tally3_dsetmatrix( rows, ib, work, rows, dA(i, i), ldda );
    }
