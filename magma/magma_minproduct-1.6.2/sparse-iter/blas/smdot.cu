    ...
    int k )
{
   ...
   if( i < n ){
    for( int j=0; j<k; j++)
      d[ i+j*n ] = MAGMA_minproduct_S_MAKE( 0.0, 0.0 );
    }
}

// dot product
__global__ void
magma_minproduct_sdot_kernel( 
    int Gs,
    int n, 
    float * v,
    float * r,
...
__global__ void
magma_minproduct_sblockdot_kernel( 
    int Gs,
    int n, 
    int k,
    float * v,
...
__global__ void
magma_minproduct_sblockreduce_kernel( 
    int Gs,
    int n, 
    int k,
    float * vtmp,
...
__global__ void
magma_minproduct_sreduce_kernel_fast( int Gs,
                           int n, 
                           float * vtmp,
                           float * vtmp2 ){
...
__global__ void
magma_minproduct_sblockreduce_kernel_fast( 
    int Gs,
    int n, 
    int k,
    float * vtmp,
    ...
    magma_minproduct_queue_t queue )
{
    ...
    if (k>1) {
        magma_minproduct_sblockdot_kernel<<<Gs, Bs, Ms>>>( Gs.x, n, k, v, r, d1 );
    }
    else {
        magma_minproduct_sdot_kernel<<<Gs, Bs, Ms>>>( Gs.x, n, v, r, d1 );
    }
/*
    // not necessary to zero GPU mem
    magma_minproduct_sgpumemzero<<<Gs, Bs, 0>>>( d1, n*k,1 );
    magma_minproduct_sgpumemzero<<<Gs, Bs, 0>>>( d2, n*k,1 );
    //magma_minproductblas_slaset( Magma_minproductUpperLower, n, k, d1, n );
    //magma_minproductblas_slaset( Magma_minproductUpperLower, n, k, d2, n );
    while( Gs.x > 1 ) {
        Gs_next.x = ( Gs.x+Bs.x-1 )/ Bs.x ;
        magma_minproduct_sblockreduce_kernel<<< Gs_next.x, Bs.x, Ms >>> 
                                        ( Gs.x, n, k, aux1, aux2 );
        Gs.x = Gs_next.x;
        b = 1 - b;
    ...
    for( int j=0; j<k; j++) {
            magma_minproduct_scopyvector( 1, aux1+j*n, 1, skp+j, 1 );
    }
*/
    ...
    if ( k>1) {
        while( Gs.x > 1 ) {
            ...
            magma_minproduct_sblockreduce_kernel_fast<<< Gs_next.x/2, Bs.x/2, Ms/2 >>> 
                        ( Gs.x, n, k, aux1, aux2 );
        ...
        while( Gs.x > 1 ) {
            Gs_next.x = ( Gs.x+Bs.x-1 )/ Bs.x ;
            if ( Gs_next.x == 1 ) Gs_next.x = 2;
            magma_minproduct_sreduce_kernel_fast<<< Gs_next.x/2, Bs.x/2, Ms/2 >>> 
                        ( Gs.x, n, aux1, aux2 );
            Gs_next.x = Gs_next.x /2;
            Gs.x = Gs_next.x;
            b = 1 - b;
    ...
    magma_minproduct_queue_t queue )
{
    ...
    while( rows_left > (chunk_size) ) {
        magma_minproduct_smdotc( n, chunk_size, v+offset*n, r, d1, d2, skp+offset, queue );
        offset = offset + chunk_size;
        rows_left = rows_left-chunk_size;

    }
    // process rest
    magma_minproduct_smdotc( n, rows_left, v+offset*n, r, d1, d2, skp+offset, queue ); 
