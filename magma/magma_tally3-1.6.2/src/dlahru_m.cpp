...
/**
    Purpose
    -------
    DLAHRU is an auxiliary MAGMA_tally3 routine that is used in DGEHRD to update
    ...
    struct dgehrd_data_tally3 *data )
{
    ...
    for( d = 0; d < ngpu; ++d ) {
        ...
        magma_tally3_dgemm( Magma_tally3NoTrans, Magma_tally3ConjTrans, dkhi-dk, nb, nb,
                     c_one,  dVd(d, dk, 0), ldvd,
                             dTi(d),        nb,
                     c_zero, dW (d, dk, 0), ldda );
        
        // Am = Am - Ym Wd' = A(0:k-1, k:ihi-1) - Ym(0:k-1, 0:nb-1) * W(k:ihi-1, 0:nb-1)'
        magma_tally3_dgemm( Magma_tally3NoTrans, Magma_tally3ConjTrans, k, dkhi-dk, nb,
        ...
        magma_tally3_dgemm( Magma_tally3NoTrans, Magma_tally3ConjTrans, ihi-k, dkhi-dknb, nb,
        ...
        magma_tally3_dgemm( Magma_tally3NoTrans, Magma_tally3ConjTrans, ihi-k, nb, nb,
                     c_one,  dV (d, k, 0), ldv,
                             dTi(d),       nb,
                     c_zero, dW (d, k, 0), ldda );
        
        // Z = V(k:ihi-1, 0:nb-1)' * A(k:ihi-1, nb:n-k-1);  Z is stored over Y
        magma_tally3_dgemm( Magma_tally3ConjTrans, Magma_tally3NoTrans, nb, dn-dknb, ihi-k,
                     c_one,  dV(d, k, 0),    ldv,
                             dA(d, k, dknb), ldda,
                     c_zero, dY(d, 0, 0),    nb );
        
        // Ag2 = Ag2 - W Z = A(k:ihi-1, k+nb:n-1) - W(k+nb:n-1, 0:nb-1) * Z(0:nb-1, k+nb:n-1)
        magma_tally3_dgemm( Magma_tally3NoTrans, Magma_tally3NoTrans, ihi-k, dn-dknb, nb,
