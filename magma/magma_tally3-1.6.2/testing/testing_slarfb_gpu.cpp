    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int itran = 0; itran < 2; ++itran ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {            
            ...
            magma_tally3Float_ptr dC, dV, dT, dW;
            ...
            //printf( "C=" );  magma_tally3_sprint( M, N, C, ldc );
            ...
            if ( storev[istor] == Magma_tally3Columnwise ) {
                if ( direct[idir] == Magma_tally3Forward ) {
                    lapackf77_slaset( Magma_tally3UpperStr, &K, &K, &c_zero, &c_one, V, &ldv );
                }
                else {
                    lapackf77_slaset( Magma_tally3LowerStr, &K, &K, &c_zero, &c_one, &V[(ldv-K)], &ldv );
                }
            }
            else {
                ...
                std::swap( ldv, nv );
                if ( direct[idir] == Magma_tally3Forward ) {
                    lapackf77_slaset( Magma_tally3LowerStr, &K, &K, &c_zero, &c_one, V, &ldv );
                }
                else {
                    lapackf77_slaset( Magma_tally3UpperStr, &K, &K, &c_zero, &c_one, &V[(nv-K)*ldv], &ldv );
                }
            }
            //printf( "# ldv %d, nv %d\n", ldv, nv );
            //printf( "V=" );  magma_tally3_sprint( ldv, nv, V, ldv );
            ...
            if ( direct[idir] == Magma_tally3Forward ) {
                lapackf77_slaset( Magma_tally3LowerStr, &k1, &k1, &c_zero, &c_zero, &T[1], &ldt );
            }
            else {
                lapackf77_slaset( Magma_tally3UpperStr, &k1, &k1, &c_zero, &c_zero, &T[1*ldt], &ldt );
            }
            //printf( "T=" );  magma_tally3_sprint( K, K, T, ldt );
            
            magma_tally3_ssetmatrix( M,   N,  C, ldc, dC, ldc );
            magma_tally3_ssetmatrix( ldv, nv, V, ldv, dV, ldv );
            magma_tally3_ssetmatrix( K,   K,  T, ldt, dT, ldt );
            
            lapackf77_slarfb( lapack_side_const_tally3( side[iside] ), lapack_trans_const_tally3( trans[itran] ),
                              lapack_direct_const_tally3( direct[idir] ), lapack_storev_const_tally3( storev[istor] ),
                              &M, &N, &K,
                              V, &ldv, T, &ldt, C, &ldc, W, &ldw );
            //printf( "HC=" );  magma_tally3_sprint( M, N, C, ldc );
            
            magma_tally3_slarfb_gpu( side[iside], trans[itran], direct[idir], storev[istor],
                              M, N, K,
                              dV, ldv, dT, ldt, dC, ldc, dW, ldw );
            magma_tally3_sgetmatrix( M, N, dC, ldc, R, ldc );
            //printf( "dHC=" );  magma_tally3_sprint( M, N, R, ldc );
