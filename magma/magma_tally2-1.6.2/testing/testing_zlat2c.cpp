...
int main( int argc, char** argv )
{
    ...
    magma_tally2FloatComplex  s_neg_one = MAGMA_tally2_C_NEG_ONE;
    ...
    magma_tally2FloatComplex   *SA, *SR;
    magma_tally2DoubleComplex   *A,  *R;
    magma_tally2FloatComplex  *dSA;
    ...
    for( int iuplo = 0; iuplo < 2; ++iuplo ) {
      for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            gbytes = (real_Double_t) 0.5*(n+1)*n * (sizeof(magma_tally2DoubleComplex) + sizeof(magma_tally2FloatComplex)) / 1e9;
            size = ldda*n;  // ldda >= lda
            
            TESTING_MALLOC_CPU(  SA, magma_tally2FloatComplex,  size );
            TESTING_MALLOC_CPU(   A, magma_tally2DoubleComplex, size );
            TESTING_MALLOC_CPU(  SR, magma_tally2FloatComplex,  size );
            TESTING_MALLOC_CPU(   R, magma_tally2DoubleComplex, size );
            
            TESTING_MALLOC_DEV( dSA, magma_tally2FloatComplex,  size );
            TESTING_MALLOC_DEV(  dA, magma_tally2DoubleComplex, size );
            
            lapackf77_zlarnv( &ione, ISEED, &size,  A );
            lapackf77_clarnv( &ione, ISEED, &size, SA );
            
            magma_tally2_zsetmatrix( n, n, A,  lda, dA,  ldda );
            magma_tally2_csetmatrix( n, n, SA, lda, dSA, ldda );
            ...
            lapackf77_zlat2c( lapack_uplo_const_tally2(uplo[iuplo]), &n, A, &lda, SA, &lda, &info );
            ...
            /* ====================================================================
               Performs operation using MAGMA_tally2 zlat2c
               =================================================================== */
            gpu_time = magma_tally2_sync_wtime(0);
            magma_tally2blas_zlat2c( uplo[iuplo], n, dA, ldda, dSA, ldda, &info );
            gpu_time = magma_tally2_sync_wtime(0) - gpu_time;
            gpu_perf = gbytes / gpu_time;
            if (info != 0)
                printf("magma_tally2blas_zlat2c returned error %d: %s.\n",
                       (int) info, magma_tally2_strerror( info ));
            
            magma_tally2_cgetmatrix( n, n, dSA, ldda, SR, lda );
            
            if ( opts.verbose ) {
                printf( "A=  " );  magma_tally2_zprint( n, n, A,  lda );
                printf( "SA= " );  magma_tally2_cprint( n, n, SA, lda );
                printf( "dA= " );  magma_tally2_zprint_gpu( n, n, dA,  ldda );
                printf( "dSA=" );  magma_tally2_cprint_gpu( n, n, dSA, ldda );
            }
            ...
            if ( uplo[iuplo] == Magma_tally2Lower ) {
                for( int j=0; j < n; ++j ) {
                    for( int i=j; i < n; ++i ) {
                        *A(i,j) = MAGMA_tally2_Z_MAKE( real(*SA(i,j)), imag(*SA(i,j)) );
                    }
                }
            }
            else { // upper
            ...
            /* ====================================================================
               Performs operation using MAGMA_tally2 clat2z
               =================================================================== */
            magma_tally2_csetmatrix( n, n, SA, lda, dSA, ldda );
            
            gpu_time = magma_tally2_sync_wtime(0);
            magma_tally2blas_clat2z( uplo[iuplo], n, dSA, ldda, dA, ldda, &info );
            gpu_time = magma_tally2_sync_wtime(0) - gpu_time;
            gpu_perf = gbytes / gpu_time;
            if (info != 0)
                printf("magma_tally2blas_clat2z returned error %d: %s.\n",
                       (int) info, magma_tally2_strerror( info ));
            
            magma_tally2_zgetmatrix( n, n, dA, ldda, R, lda );
