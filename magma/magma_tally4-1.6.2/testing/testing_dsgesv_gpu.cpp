...
int main(int argc, char **argv)
{
    ...
    double *h_A, *h_B, *h_X;
    magma_tally4Double_ptr d_A, d_B, d_X, d_WORKD;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lapackf77_dlacpy( Magma_tally4UpperLowerStr, &N, &nrhs, h_B, &ldb, h_X, &ldx);
            
            magma_tally4_dsetmatrix( N, N,    h_A, lda, d_A, ldda );
            magma_tally4_dsetmatrix( N, nrhs, h_B, ldb, d_B, lddb );
            
            //=====================================================================
            //              MIXED - GPU
            //=====================================================================
            gpu_time = magma_tally4_wtime();
            magma_tally4_dsgesv_gpu( opts.transA, N, nrhs,
            ...
            if (info != 0)
                printf("magma_tally4_dsgesv returned error %d: %s.\n",
                       (int) info, magma_tally4_strerror( info ));
            
            //=====================================================================
            //              ERROR DP vs MIXED  - GPU
            //=====================================================================
            magma_tally4_dgetmatrix( N, nrhs, d_X, lddx, h_X, ldx );
            
            Anorm = lapackf77_dlange("I", &N, &N, h_A, &lda, h_workd);
            blasf77_dgemm( lapack_trans_const_tally4(opts.transA), Magma_tally4NoTransStr,
                           &N, &nrhs, &N,
                           &c_one,     h_A, &lda,
                                       h_X, &ldx,
                           &c_neg_one, h_B, &ldb);
            ...
            gpu_time = magma_tally4_wtime();
            magma_tally4_dgetrf_gpu(N, N, d_A, ldda, h_ipiv, &info);
            gpu_time = magma_tally4_wtime() - gpu_time;
            gpu_perfdf = gflopsF / gpu_time;
            if (info != 0)
                printf("magma_tally4_dgetrf returned error %d: %s.\n",
                       (int) info, magma_tally4_strerror( info ));
            ...
            gpu_time = magma_tally4_wtime();
            magma_tally4_dgetrf_gpu(N, N, d_A, ldda, h_ipiv, &info);
            magma_tally4_dgetrs_gpu( opts.transA, N, nrhs, d_A, ldda, h_ipiv, d_B, lddb, &info );
            gpu_time = magma_tally4_wtime() - gpu_time;
            gpu_perfds = gflopsS / gpu_time;
            if (info != 0)
                printf("magma_tally4_dgetrs returned error %d: %s.\n",
                       (int) info, magma_tally4_strerror( info ));
            ...
            magma_tally4_dsetmatrix( N, N,    h_A, lda,  d_A,  ldda );
            magma_tally4_dsetmatrix( N, nrhs, h_B, ldb,  d_B,  lddb );
            magma_tally4blas_dlag2s( N, N,    d_A, ldda, d_As, ldda, &info );
            magma_tally4blas_dlag2s( N, nrhs, d_B, lddb, d_Bs, lddb, &info );
            
            gpu_time = magma_tally4_wtime();
            magma_tally4_sgetrf_gpu(N, N, d_As, ldda, h_ipiv, &info);
            gpu_time = magma_tally4_wtime() - gpu_time;
            gpu_perfsf = gflopsF / gpu_time;
            if (info != 0)
                printf("magma_tally4_sgetrf returned error %d: %s.\n",
                       (int) info, magma_tally4_strerror( info ));
            ...
            gpu_time = magma_tally4_wtime();
            magma_tally4_sgetrf_gpu( N, N,    d_As, ldda, h_ipiv, &info);
            magma_tally4_sgetrs_gpu( opts.transA, N, nrhs, d_As, ldda, h_ipiv,
                              d_Bs, lddb, &info);
            gpu_time = magma_tally4_wtime() - gpu_time;
            gpu_perfss = gflopsS / gpu_time;
            if (info != 0)
                printf("magma_tally4_sgetrs returned error %d: %s.\n",
                       (int) info, magma_tally4_strerror( info ));
