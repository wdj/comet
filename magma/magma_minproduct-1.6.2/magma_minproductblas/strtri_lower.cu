/*
    -- MAGMA_minproduct (version 1.6.1) --
       ...
       @generated from ztrtri_lower.cu normal z -> s, Fri Jan 30 19:00:10 2015
...
strtri_diag_lower_device(
    magma_minproduct_diag_t diag, int n, const float *A, int lda, float *d_dinvA)
{
    int tx = threadIdx.x;
    int bx = blockIdx.x;
    int blk_ind = bx*IB;
    ...
    for( int j=0; j < IB; j++ ) {
        if (tx >= j && ind < n) {
            ...
            sB[tx + j*IB] = MAGMA_minproduct_S_ZERO;
        }
    }
    __syncthreads();
    ...
    if (diag == Magma_minproductUnit) {
        sB[tx + tx*IB] = MAGMA_minproduct_S_ONE;
    }
    else {
        if ( sB[tx + tx*IB] == MAGMA_minproduct_S_ZERO ) {  // singular or outside matrix
            sB[tx + tx*IB] = MAGMA_minproduct_S_ONE;
        }
        else {
            sB[tx + tx*IB] = MAGMA_minproduct_S_ONE / sB[tx + tx*IB];
        }
    }
    ...
    int n, const float *Ain, int lda, float *d_dinvA, int jb, int npages)
{
    ...
    //--------------------------part three---------------------------//
    {
        ...
        #pragma unroll
        for( int i = 0; i < 16; i++ ) {
            B12[i*ldb] = MAGMA_minproduct_S_ZERO;
        }
    }
}
...
strtri_diag_lower_kernel_batched(
    magma_minproduct_diag_t diag, int n, float const * const * dA_array, int lda, float **dinvA_array)
{
