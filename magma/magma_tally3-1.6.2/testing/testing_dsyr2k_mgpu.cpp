/*
    -- MAGMA_tally3 (version 1.6.1) --
       ...
       @generated from testing_zher2k_mgpu.cpp normal z -> d, Fri Jan 30 19:00:23 2015
...
/* ////////////////////////////////////////////////////////////////////////////
   -- Testing magma_tally3_dsyr2k_mgpu
*/
int main( int argc, char** argv)
{
    ...
    double alpha = MAGMA_tally3_D_MAKE( 1.2345, 4.321 );
    double beta = 3.14159;
    
    real_Double_t    gflops, gpu_perf, cpu_perf, gpu_time, cpu_time;
    double           error, work[1];
    double *hA, *hR, *hR2, *hV, *hW;
    magma_tally3Double_ptr dV[Magma_tally3MaxGPUs], dW[Magma_tally3MaxGPUs], dA[Magma_tally3MaxGPUs];
    ...
    printf( "version 1: magma_tally3blas_dsyr2k_mgpu2     %s\n", (opts.version==1 ? "(enabled)" : ""));
    printf( "version 2: magma_tally3blas_dsyr2k_mgpu_spec %s\n", (opts.version==2 ? "(enabled)" : ""));
#ifdef ICHI
    printf( "version 3: magma_tally3_dsyr2k_mgpu (Ichi)   %s\n", (opts.version==3 ? "(enabled)" : ""));
#endif
    printf( "\n" );
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        ...
        for( int offset = 0; offset < n; offset += min(k,nb) ) {
            for( int iter = 0; iter < opts.niter; ++iter ) {
                ...
                TESTING_MALLOC_CPU( hR2, double, lda*n   );
                TESTING_MALLOC_CPU( hV,  double, lda*k*2 );
                //TESTING_MALLOC_CPU( hW,  double, lda*k   );
                ...
                size = lda*k*2;
                lapackf77_dlarnv( &ione, ISEED, &size, hV );
                hW = hV + lda*k;
                //lapackf77_dlarnv( &ione, ISEED, &size, hW );
                
                /* ====================================================================
                   Performs operation using MAGMA_tally3
                   =================================================================== */
                magma_tally3_dsetmatrix_1D_col_bcyclic( n, n, hA, lda, dA, ldda, ngpu, nb );
                for( int d = 0; d < ngpu; ++d ) {
                    magma_tally3_setdevice( d );
                    dW[d] = dV[d] + ldda*k;
                    magma_tally3_dsetmatrix( n, k, hV, lda, dV[d], ldda );
                    magma_tally3_dsetmatrix( n, k, hW, lda, dW[d], ldda );
                }
                ...
                magma_tally3_dgetmatrix_1D_col_bcyclic( n, n, dA, ldda, hR, lda, ngpu, nb );
