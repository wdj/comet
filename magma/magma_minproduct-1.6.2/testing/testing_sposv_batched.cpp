/*
   ...
   @generated from testing_zposv_batched.cpp normal z -> s, Fri Jan 30 19:00:26 2015
 */
// includes, system
#include <stdio.h>
#include <stdlib.h>
...
int main(int argc, char **argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_minproduct_ssetmatrix( N, N*batchCount,    h_A, lda, d_A, ldda );
            magma_minproduct_ssetmatrix( N, nrhs*batchCount, h_B, ldb, d_B, lddb );
            ...
            info = magma_minproduct_sposv_batched(opts.uplo, N, nrhs, dA_array, ldda, dB_array, lddb, dinfo_array, batchCount, queue); 
            ...
            {
                if(cpu_info[i] != 0 ){
                    printf("magma_minproduct_sposv_batched matrix %d returned internal error %d\n",i, (int)cpu_info[i] );
                }
            }
            if (info != 0)
                printf("magma_minproduct_sposv_batched returned argument error %d: %s.\n", (int) info, magma_minproduct_strerror( info ));
            
            //=====================================================================
            // Residual
            //=====================================================================
            magma_minproduct_sgetmatrix( N, nrhs*batchCount, d_B, lddb, h_X, ldb );
            ...
            for(magma_minproduct_int_t s=0; s<batchCount; s++)
            {
                ...
                blasf77_sgemm( Magma_minproductNoTransStr, Magma_minproductNoTransStr, &N, &nrhs, &N,
            ...
            if ( opts.lapack ) {
                ...
                for(magma_minproduct_int_t s=0; s<batchCount; s++)
                {
                    lapackf77_sposv( lapack_uplo_const(opts.uplo), &N, &nrhs, h_A + s * lda * N, &lda, h_B + s * ldb * nrhs, &ldb, &info );
                }
                cpu_time = magma_minproduct_wtime() - cpu_time;
                cpu_perf = gflops / cpu_time;
                if (info != 0)
                    printf("lapackf77_sposv returned err %d: %s.\n",
                           (int) info, magma_minproduct_strerror( info ));
