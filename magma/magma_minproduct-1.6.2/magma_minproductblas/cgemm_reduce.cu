...
// BLK_K gets defined in magma_minproductblas_cgemm_reduce,
...
void cgemm_reduce_kernel(
    ...
    const magma_minproductFloatComplex* __restrict__ dA, int lda,
    const magma_minproductFloatComplex* __restrict__ dB, int ldb,
    magma_minproductFloatComplex beta,
    magma_minproductFloatComplex      * __restrict__ dC, int ldc)
{
    ...
    if (blockIdx.x*BLK_M + threadIdx.y < m && blockIdx.y*BLK_N + threadIdx.z < n){
        ...
        __shared__ magma_minproductFloatComplex sum[BLK_K][BLK_M+1][BLK_N+1];
        magma_minproductFloatComplex lsum;
        
        /*  w := v**H * C  */
        lsum = MAGMA_minproduct_C_ZERO;
        for( int j = tx; j < k; j += BLK_K )
            lsum += MAGMA_minproduct_C_CNJG( dA[j] )* dB[j];
        ...
        __syncthreads();
        if (threadIdx.x == 0) {
            if (MAGMA_minproduct_C_EQUAL(beta, MAGMA_minproduct_C_ZERO))
    ...
    magma_minproductFloatComplex_ptr       dC, magma_minproduct_int_t lddc )
{
    ...
    if ( arch < 200  ) {
        ...
        cgemm_reduce_kernel<BLK_K> <<< blocks, threads, 0, magma_minproduct_stream >>>
