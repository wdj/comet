/*
    -- MAGMA_tally4 (version 1.6.1) --
       ...
       @generated from testing_zlacpy.cpp normal z -> c, Fri Jan 30 19:00:23 2015
       @author Mark Gates
*/
...
int main( int argc, char** argv)
{
    ...
    magma_tally4_int_t M, N, size, lda, ldb, ldda, lddb;
    ...
    for( int iuplo = 0; iuplo < 3; ++iuplo ) {
      for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_B, magma_tally4FloatComplex, size   );
            ...
            for( int j = 0; j < N; ++j ) {
                for( int i = 0; i < M; ++i ) {
                    ...
                    h_B[i + j*ldb] = MAGMA_tally4_C_MAKE( i - j/10000. + 10000., j );
                }
            }
            ...
            //magma_tally4blas_clacpy( uplo[iuplo], M-2, N-2, d_A+1+ldda, ldda, d_B+1+lddb, lddb );  // inset by 1 row & col
            magma_tally4blas_clacpy( uplo[iuplo], M, N, d_A, ldda, d_B, lddb );
            ...
            //lapackf77_clacpy( lapack_uplo_const_tally4(uplo[iuplo]), &M2, &N2, h_A+1+lda, &lda, h_B+1+ldb, &ldb );
            lapackf77_clacpy( lapack_uplo_const_tally4(uplo[iuplo]), &M, &N, h_A, &lda, h_B, &ldb );
            cpu_time = magma_tally4_wtime() - cpu_time;
            cpu_perf = gbytes / cpu_time;
            
            if ( opts.verbose ) {
                printf( "A= " );  magma_tally4_cprint(     M, N, h_A, lda );
                printf( "B= " );  magma_tally4_cprint(     M, N, h_B, lda );
                printf( "dA=" );  magma_tally4_cprint_gpu( M, N, d_A, ldda );
                printf( "dB=" );  magma_tally4_cprint_gpu( M, N, d_B, ldda );
            }
            
            /* =====================================================================
               Check the result
               =================================================================== */
            magma_tally4_cgetmatrix( M, N, d_B, ldda, h_R, lda );
