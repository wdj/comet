...
/**
    ...
    dT      COMPLEX_16 array on the GPU that is the output
            (the 9th argument) of magma_tally2_zgeqrf_gpu.

    @param[in]
    nb      INTEGER
            This is the blocking size that was used in pre-computing DT, e.g.,
            the blocking size used in magma_tally2_zgeqrf_gpu.
    ...
    magma_tally2_int_t *info)
{
    ...
    *info = 0;
    ...
    hwork[0] = MAGMA_tally2_Z_MAKE( lwkopt, 0 );
    ...
    if ( step < 0 ) {
        ...
        magma_tally2DoubleComplex* hA = hwork;
        magma_tally2DoubleComplex* hC = hwork + ma*ib;
        magma_tally2DoubleComplex* hW = hwork + ma*ib + mi*ni;
        magma_tally2_int_t lhwork = lwork - (ma*ib + mi*ni);
        
        magma_tally2_zgetmatrix( ma, ib, dA(i,  i ), ldda, hA, ma );
        magma_tally2_zgetmatrix( mi, ni, dC(ic, jc), lddc, hC, mi );
        ...
        magma_tally2_zsetmatrix( mi, ni, hC, mi, dC(ic, jc), lddc );
    }
    ...
    // TODO sync. For cases Q*C and C*Q^T, last call is magma_tally2_zlarfb_gpu,
    // which is async magma_tally2_gemm calls, so zunmqr can be unfinished.
