...
int main(int argc, char **argv)
{
    ...
    double *A, *X, *Y, *Ylapack, *Ycublas, *Ymagma_tally3, *Ymagma_tally31, *hwork;
    magma_tally3Double_ptr dA, dX, dY;
    magma_tally3Double_ptr d_lA[Magma_tally3MaxGPUs], dwork[Magma_tally3MaxGPUs];
    ...
    magma_tally3_int_t nb = 64;  // required by magma_tally3blas_dsymv_mgpu implementation
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int ioffset=0; ioffset < noffsets; ioffset += 1 ) {
        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( Ymagma_tally3,  double, vecsize );
            TESTING_MALLOC_CPU( Ymagma_tally31, double, vecsize );
            ...
            // TODO make magma_tally3_dmalloc_bcyclic helper function?
            for( dev=0; dev < opts.ngpu; dev++ ) {
            ...
            magma_tally3_dmake_symmetric( Noffset, A, lda );
            ...
            /* =====================================================================
               ...
               =================================================================== */
            magma_tally3_setdevice( opts.device );
            magma_tally3_dsetmatrix( Noffset, Noffset, A, lda, dA, ldda );
            magma_tally3_dsetvector( Noffset, X, incx, dX, incx );
            magma_tally3_dsetvector( Noffset, Y, incx, dY, incx );
            
            cuda_time = magma_tally3_sync_wtime(0);
            cublasDsymv( opts.handle, cublas_uplo_const_tally3(opts.uplo), N,
            ...
            magma_tally3_dgetvector( Noffset, dY, incx, Ycublas, incx );
            ...
            magma_tally3blas_dsymv_work( opts.uplo, N,
                                  alpha, dA + offset + offset*ldda, ldda,
                                         dX + offset, incx,
            ...
            magma_tally3_dgetvector( Noffset, dY, incx, Ymagma_tally31, incx );
            
            /* =====================================================================
               Performs operation using MAGMA_tally3BLAS (multi-GPU)
               =================================================================== */
            magma_tally3_dsetmatrix_1D_col_bcyclic( Noffset, Noffset, A, lda, d_lA, ldda, opts.ngpu, nb );
            blasf77_dcopy( &Noffset, Y, &incx, Ymagma_tally3, &incx );
            
            // workspaces do NOT need to be zero -- set to NAN to prove
            for( dev=0; dev < opts.ngpu; ++dev ) {
                magma_tally3_setdevice( dev );
                magma_tally3blas_dlaset( Magma_tally3Full, ldwork, 1, MAGMA_tally3_D_NAN, MAGMA_tally3_D_NAN, dwork[dev], ldwork );
            }
            lapackf77_dlaset( "Full", &lhwork, &ione, &MAGMA_tally3_D_NAN, &MAGMA_tally3_D_NAN, hwork, &lhwork );
            
            mgpu_time = magma_tally3_sync_wtime(0);
            
            magma_tally3_int_t info;
            info = magma_tally3blas_dsymv_mgpu(
                opts.uplo, N,
                alpha,
                d_lA, ldda, offset,
                X + offset, incx,
                ...
                opts.ngpu, nb, queues );
            if ( info != 0 )
                printf("magma_tally3blas_dsymv_mgpu returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
            
            info = magma_tally3blas_dsymv_mgpu_sync(
                opts.uplo, N,
                alpha,
                d_lA, ldda, offset,
                X + offset, incx,
                ...
                opts.ngpu, nb, queues );
            if ( info != 0 )
                printf("magma_tally3blas_dsymv_sync returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
            ...
            if ( opts.lapack ) {
                ...
                blasf77_dsymv( lapack_uplo_const_tally3(opts.uplo), &N,
                ...
                blasf77_daxpy( &Noffset, &c_neg_one, Ymagma_tally3, &incx, Ylapack, &incx );
            ...
            blasf77_daxpy( &Noffset, &c_neg_one, Ymagma_tally3, &incx, Ycublas, &incx );
