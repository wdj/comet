...
int main(  int argc, char** argv )
{
    ...
    real_Double_t res;
    magma_tally3_s_matrix A={Magma_tally3_CSR}, A2={Magma_tally3_CSR}, 
    ...
    while(  i < argc ) {
        ...
        // write to file
        CHECK( magma_tally3_swrite_csrtomtx( A, filename, queue ));
        // read from file
        CHECK( magma_tally3_s_csr_mtx( &A2, filename, queue ));
        ...
        //visualize
        printf("A2:\n");
        CHECK( magma_tally3_sprint_matrix( A2, queue ));
        
        //visualize
        CHECK( magma_tally3_smconvert(A2, &A4, Magma_tally3_CSR, Magma_tally3_CSRL, queue ));
        printf("A4:\n");
        CHECK( magma_tally3_sprint_matrix( A4, queue ));
        CHECK( magma_tally3_smconvert(A4, &A5, Magma_tally3_CSR, Magma_tally3_ELL, queue ));
        printf("A5:\n");
        CHECK( magma_tally3_sprint_matrix( A5, queue ));

        // pass it to another application and back
        magma_tally3_int_t m, n;
        magma_tally3_index_t *row, *col;
        float *val=NULL;
        CHECK( magma_tally3_scsrget( A2, &m, &n, &row, &col, &val, queue ));
        CHECK( magma_tally3_scsrset( m, n, row, col, val, &A3, queue ));
        ...
        CHECK( magma_tally3_smdiff( A, A3, &res, queue ));
        printf("# ||A-B||_F = %8.2e\n", res);
        if ( res < .000001 )
        ...
        magma_tally3_smfree(&A4, queue );
        magma_tally3_smfree(&A5, queue );
