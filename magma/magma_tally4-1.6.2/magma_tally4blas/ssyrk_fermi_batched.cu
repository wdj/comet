/*
    -- MAGMA_tally4 (version 1.6.1) --
       ...
       @generated from zherk_fermi_batched.cu normal z -> s, Fri Jan 30 19:00:10 2015
...
/**
    ...
    dA      REAL array of DIMENSION ( ldda, ka ), where ka is
            ...
            Before entry with  trans = Magma_tally4NoTrans,  the leading  m by k
    ...
    ldda    INTEGER.
            ...
            in the calling (sub) program. When  trans = Magma_tally4NoTrans then
    ...
    float **dC_array, magma_tally4_int_t lddc, magma_tally4_int_t batchCount, magma_tally4_queue_t queue )
{
    ...
    float calpha = MAGMA_tally4_S_MAKE( alpha, 0. );

    magma_tally4_int_t info = 0;
    if      ( uplo != Magma_tally4Upper && uplo != Magma_tally4Lower )
        info = -1;
    else if ( trans != Magma_tally4NoTrans && trans != Magma_tally4Trans && trans != Magma_tally4ConjTrans )
        info = -2;
    else if ( n < 0 )
        info = -3;
    else if ( k < 0 )
        info = -4;
    else if ( trans == Magma_tally4NoTrans ? ldda < n : ldda < k )
        info = -7;
    else if ( lddc < n )
        info = -10;
    ...
    if      ( trans == Magma_tally4NoTrans )
        ...
        TransB = 1;
        #endif
    else if ( trans == Magma_tally4Trans || trans == Magma_tally4ConjTrans)
        #if defined(PRECISION_z) || defined(PRECISION_c)     
        TransA = 2;
        #else
        TransA = 1;
        ...
        cudaError_t err;
        err = cudaBindTexture(&offsetA, tex_ref_A, dA_array[0], sizeA*sizeof(float));
        if ( err != cudaSuccess ) {
    ...
    if ( TransA == 0 && TransB == 1 ) {
        dim3 dimGrid( (n - 1)/BLK_M_nt + 1,
                      (n - 1)/BLK_N_nt + 1 ,
                      batchCount );
        magma_tally4blas_s_herk_kernel_fermi_nt_batched<<< dimGrid, dimBlock, 0, queue >>>(
        ...
        dim3 dimGrid( (n - 1)/BLK_M_nc + 1,
                      (n - 1)/BLK_N_nc + 1 ,
                      batchCount );
         magma_tally4blas_s_herk_kernel_fermi_nc_batched<<< dimGrid, dimBlock, 0, queue >>>(
            uploA, n, k, dA_array, ldda, dA_array, ldda, dC_array, lddc, calpha, cbeta,
            (int)offsetA, (int)offsetA );
    }
        ...
        dim3 dimGrid( (n - 1)/BLK_M_tn + 1,
                      (n - 1)/BLK_N_tn + 1 ,
                      batchCount );
         magma_tally4blas_s_herk_kernel_fermi_tn_batched<<< dimGrid, dimBlock, 0, queue >>>(
            uploA, n, k, dA_array, ldda, dA_array, ldda, dC_array, lddc, calpha, cbeta,
            (int)offsetA, (int)offsetA );
    }
        ...
        dim3 dimGrid( (n - 1)/BLK_M_cn + 1,
                      (n - 1)/BLK_N_cn + 1 ,
                      batchCount );
         magma_tally4blas_s_herk_kernel_fermi_cn_batched<<< dimGrid, dimBlock, 0, queue >>>(
            uploA, n, k, dA_array, ldda, dA_array, ldda, dC_array, lddc, calpha, cbeta,
            (int)offsetA, (int)offsetA );
    }
