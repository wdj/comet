...
extern "C"
void magma_tally3blas_dsymm_mgpu_com(
    ...
    magma_tally3_int_t gnode[Magma_tally3MaxGPUs][Magma_tally3MaxGPUs+2], magma_tally3_int_t nbcmplx )
{
    ...
    magma_tally3Double_ptr dwork2[Magma_tally3MaxGPUs];
    ...
    if(blockoffset>0){
        ...
        magma_tally3blas_dsymmetrize_tiles(  Magma_tally3Lower,  fstblksiz,  dA(stdev, offset, myblkoffst*nb+blockoffset),  ldda,  1,  ngpu*nb,  nb  );         
    }
    ...
    for( magma_tally3_int_t dev = 0; dev < ngpu; ++dev ) {
        ...
        magma_tally3_setdevice( dev );
        magma_tally3blasSetKernelStream( queues[ dev ][ 0 ] );
        magma_tally3blas_dsymmetrize_tiles(  Magma_tally3Lower,  nb,  dA(dev, newoffset+devperm*nb, myblkoffst*nb),  ldda,  myblk,  ngpu*nb,  nb  );
        if(remm%nb>0){
            ...
            if(devperm==devlstblk)
                magma_tally3blas_dsymmetrize(  Magma_tally3Lower,  remm % nb,  dA(dev, newoffset+nbblk*nb, myblkoffst*nb+ myblk*nb),  ldda );  // last partial tile
        }
    }
...
/*
    ...
    double *R;
    magma_tally3_dmalloc_cpu( &R, siz*siz );
    // collecte back A
    magma_tally3blas_dgetmatrix_1D_bcyclic( siz, siz, dA, ldda, R, siz, ngpu, nb );
    magma_tally3_setdevice( 0 );
    magma_tally3blasSetKernelStream( queues[ dev ][ 0 ] );
    //magma_tally3_dgetmatrix( siz, siz, dA[0], ldda, R, siz );
    FILE *trace_file;
...
*/
        ...
        for( magma_tally3_int_t i = fstblksiz; i < m; i += nb ) {
            ...
            for( magma_tally3_int_t dev = 0; dev < ngpu; ++dev ) {
                ...
                if(myrowsize>0){
                    ...
                    magma_tally3_dgemm( Magma_tally3ConjTrans, Magma_tally3NoTrans, myrowsize, n, ib,
    ...
    if(blockoffset>0){
        ...
        magma_tally3_dgemm( Magma_tally3NoTrans, Magma_tally3NoTrans, m, n, ib,
    ...
    for( magma_tally3_int_t i = fstblksiz; i < m; i += nb ) {
        ...
        magma_tally3blasSetKernelStream( queues[ dev ][ 0 ] );
        if(i==0){
           magma_tally3_dgemm( Magma_tally3NoTrans, Magma_tally3NoTrans, m-i, n, ib,
        ...
        if(ngpu==1){
            ...
            // magma_tally3blasSetKernelStream( queues[ dev ][ 0 ] );
            magma_tally3_dgemm( Magma_tally3ConjTrans, Magma_tally3NoTrans, i, n, ib,
        ...
        for( magma_tally3_int_t dev = 0; dev < ngpu; ++dev ) {
            ...
            if(myrowsize>0){
                ...
                for( magma_tally3_int_t blki = 0; blki < myblk; ++blki){
                    ...
                    magma_tally3blas_dgeadd(ib, n, c_one, 
        ...
        if(masterdev!=-1){ 
            ...
            for( magma_tally3_int_t l = 0; l < myngpu; ++l ) {
                ...
                if((lcdev!=masterdev)&&(lccolsize>0)){
                    ...
                    magma_tally3blas_dgeadd(m, n, c_one, 
