...
kernel_zgemvn_batched(
    ...
    magma_tally2DoubleComplex **x_array, int incx,
    magma_tally2DoubleComplex beta, magma_tally2DoubleComplex  **y_array, int incy)
{

    magma_tally2DoubleComplex *A = dA_array[blockIdx.x];
    magma_tally2DoubleComplex *x = x_array[blockIdx.x];
    magma_tally2DoubleComplex *y = y_array[blockIdx.x];

    int tx = threadIdx.x;

    magma_tally2DoubleComplex res = MAGMA_tally2_Z_ZERO;

    magma_tally2DoubleComplex *buff = (magma_tally2DoubleComplex*)shared_data;
...
*/
extern "C"
void magma_tally2blas_zgemvn_batched(
    int m, int n, 
    ...
    int batchCount, magma_tally2_queue_t queue)
{
    ...
    if( m > 512 || n > 512)
    {
        fprintf( stderr, "m=%d, n=%d, zgemv_batched nontranspose assume row && column lower than %d. Plz call magma_tally2blas_zgemv instead", m, n, 512);
        return ;
    }
    ...
    kernel_zgemvn_batched<<< grid, threads, n * sizeof(magma_tally2DoubleComplex), queue >>>( m, n, alpha,  dA_array, lda, x_array, incx,  
                                                                         beta, y_array, incy);
}
...
kernel_zgemvt_batched(
    int m, int n, int m1, magma_tally2DoubleComplex alpha,
    magma_tally2DoubleComplex **dA_array, int lda,
    magma_tally2DoubleComplex **x_array, int incx,
    magma_tally2DoubleComplex beta, magma_tally2DoubleComplex  **y_array, int incy)
{
    ...
    magma_tally2DoubleComplex *A_ptr = dA_array[blockIdx.x];
    magma_tally2DoubleComplex *x_ptr = x_array[blockIdx.x];
    magma_tally2DoubleComplex *y_ptr = y_array[blockIdx.x];
    ...
    __shared__ magma_tally2DoubleComplex sdata[zgemv_bs];
...
extern "C"
void magma_tally2blas_zgemvt_batched(
    int m, int n, 
    magma_tally2DoubleComplex alpha, magma_tally2DoubleComplex **dA_array, int lda, 
    magma_tally2DoubleComplex **x_array,  int incx,
    magma_tally2DoubleComplex beta, magma_tally2DoubleComplex **y_array,  int incy, 
    ...
    magma_tally2DoubleComplex beta, magma_tally2DoubleComplex  **y_array, int incy)
{
    ...
    for(int i=0; i<m1; i+= zgemv_bs)
    {
        res += MAGMA_tally2_Z_CNJG (A_ptr[i]) * x_ptr[i*incx];
    }
    ...
    if(m > m1)
    {
        if( tx + m1 <  m )
        {
            res  += MAGMA_tally2_Z_CNJG(A_ptr[m1]) * x_ptr[m1*incx];
        }
        else
        {
            res  = res;
...
extern "C"
void magma_tally2blas_zgemvc_batched(
    int m, int n, 
    magma_tally2DoubleComplex alpha, magma_tally2DoubleComplex **dA_array, int lda, 
    magma_tally2DoubleComplex **x_array,  int incx,
    magma_tally2DoubleComplex beta, magma_tally2DoubleComplex **y_array,  int incy, 
...
extern "C"
void magma_tally2blas_zgemv_batched(
...
{       
    ...
    if ( trans == Magma_tally2NoTrans ) {
        ...
        magma_tally2blas_zgemvn_batched(m, n, alpha, dA_array, ldda, dx_array, incx, beta, dy_array, incy, batchCount, queue);
            
    }
    else if ( trans == Magma_tally2Trans ) {
        magma_tally2blas_zgemvt_batched(m, n, alpha, dA_array, ldda, dx_array, incx, beta, dy_array, incy, batchCount, queue);
    }
    else if ( trans == Magma_tally2ConjTrans ) {
#if defined(PRECISION_z) || defined (PRECISION_c)
        magma_tally2blas_zgemvc_batched(m, n, alpha, dA_array, ldda, dx_array, incx, beta, dy_array, incy, batchCount, queue);
#else
        magma_tally2blas_zgemvt_batched(m, n, alpha, dA_array, ldda, dx_array, incx, beta, dy_array, incy, batchCount, queue);
#endif
    }
