...
int main( int argc, char** argv )
{
    ...
    magma_tally3DoubleComplex *h_A, *h_R, *tau, *h_work, tmp[1];
    magma_tally3DoubleComplex_ptr d_lA[ Magma_tally3MaxGPUs ];
    magma_tally3_int_t M, N, n2, lda, ldda, n_local, ngpu;
    magma_tally3_int_t info, min_mn, nb, lhwork;
    magma_tally3_int_t ione     = 1;
    magma_tally3_int_t ISEED[4] = {0,0,0,1}, ISEED2[4];
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for( int dev = 0; dev < ngpu; dev++ ) {
                ...
                TESTING_MALLOC_DEV( d_lA[dev], magma_tally3DoubleComplex, ldda*n_local );
            }
            ...
            /* =====================================================================
               ...
               =================================================================== */
            if ( opts.lapack ) {
                magma_tally3DoubleComplex *tau2;
                TESTING_MALLOC_CPU( tau2, magma_tally3DoubleComplex, min_mn );
            ...
            /* ====================================================================
               ...
               =================================================================== */
            magma_tally3_zsetmatrix_1D_col_bcyclic( M, N, h_R, lda, d_lA, ldda, ngpu, nb );
            
            gpu_time = magma_tally3_wtime();
            magma_tally3_zgeqrf2_mgpu( ngpu, M, N, d_lA, ldda, tau, &info );
            gpu_time = magma_tally3_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally3_zgeqrf2 returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
            
            magma_tally3_zgetmatrix_1D_col_bcyclic( M, N, d_lA, ldda, h_R, lda, ngpu, nb );
            ...
            if ( opts.check == 1 && M >= N ) {
                /* =====================================================================
                   ...
                   =================================================================== */
                magma_tally3_int_t lwork = n2+N;
                magma_tally3DoubleComplex *h_W1, *h_W2, *h_W3;
                double *h_RW, results[2];
            
                TESTING_MALLOC_CPU( h_W1, magma_tally3DoubleComplex, n2    ); // Q
                TESTING_MALLOC_CPU( h_W2, magma_tally3DoubleComplex, n2    ); // R
                TESTING_MALLOC_CPU( h_W3, magma_tally3DoubleComplex, lwork ); // WORK
