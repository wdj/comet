...
/**
    ...
    lwork   INTEGER
            ...
            where NB can be obtained through magma_tally2_get_cgeqrf_nb(M).
    \n
...
magma_tally2_cgeqrf(
    ...
    magma_tally2FloatComplex *A,    magma_tally2_int_t lda, magma_tally2FloatComplex *tau,
    ...
    magma_tally2_int_t *info )
{
    ...
    magma_tally2FloatComplex *dA, *dwork, *dT;
    ...
    *info = 0;
    ...
    work[0] = MAGMA_tally2_C_MAKE( (float)lwkopt, 0 );
    int lquery = (lwork == -1);
    if (m < 0) {
        *info = -1;
    } else if (n < 0) {
    ...
    if ( ngpu > 1 ) {
        ...
        return magma_tally2_cgeqrf4(ngpu, m, n, A, lda, tau, work, lwork, info);
    }

    // allocate space for dA, dwork, and dT
    if (MAGMA_tally2_SUCCESS != magma_tally2_cmalloc( &dA, n*ldda + nb*lddwork + nb*nb )) {
        /* Switch to the "out-of-core" (out of GPU-memory) version */
        return magma_tally2_cgeqrf_ooc(m, n, A, lda, tau, work, lwork, info);
    }
    ...
    if ( (nb > 1) && (nb < k) ) {
        ...
        magma_tally2_csetmatrix_async( m, n-nb,
                                A(0,nb),  lda,
        ...
        for (i = 0; i < k-nb; i += nb) {
            ...
            if (i > 0) {
                ...
                magma_tally2_cgetmatrix_async( m-i, ib,
                                        dA(i,i), ldda,
                                        A(i,i),  lda, stream[0] );
                ...
                magma_tally2_cgetmatrix_async( i, ib,
                                        dA(0,i), ldda,
                                        A(0,i),  lda, stream[1] );
            ...
            cpanel_to_q_tally2(Magma_tally2Upper, ib, A(i,i), lda, work+ib*ib);

            /* download the i-th V matrix */
            magma_tally2_csetmatrix_async( rows, ib, A(i,i), lda, dA(i,i), ldda, stream[0] );

            /* download the T matrix */
            magma_tally2_queue_sync( stream[1] );
            magma_tally2_csetmatrix_async( ib, ib, work, ib, dT, nb, stream[0] );
            magma_tally2_queue_sync( stream[0] );
            ...
            if (i + ib < n) {
                if (i+ib < k-nb) {
                    ...
                    cq_to_panel_tally2(Magma_tally2Upper, ib, A(i,i), lda, work+ib*ib);
                }
                else {
    ...
    if (i < k) {
        ib = n-i;
        if (i != 0) {
            magma_tally2_cgetmatrix_async( m, ib, dA(0,i), ldda, A(0,i), lda, stream[1] );
            magma_tally2_queue_sync( stream[1] );
        }
        magma_tally2_int_t rows = m-i;
        lapackf77_cgeqrf(&rows, &ib, A(i,i), &lda, tau+i, work, &lwork, info);
