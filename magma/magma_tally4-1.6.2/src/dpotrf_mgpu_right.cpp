...
/**
    ...
    d_lA    DOUBLE_PRECISION array of pointers on the GPU, dimension (ngpu)
            ...
            block size of nb) if UPLO = Magma_tally4Upper or Magma_tally4Lower, respectively.
            If UPLO = Magma_tally4Upper, the leading N-by-N upper triangular
            ...
            If UPLO = Magma_tally4Lower, the leading N-by-N lower triangular part
    ...
    magma_tally4_int_t *info )
{
    ...
    double z_one = MAGMA_tally4_D_MAKE(  1.0, 0.0 );
    double mz_one = MAGMA_tally4_D_MAKE( -1.0, 0.0 );
    double             one =  1.0;
    double             m_one = -1.0;
    ...
    double *d_lP[Magma_tally4MaxGPUs], *dlpanel, *dlpanels[Magma_tally4MaxGPUs];
    magma_tally4_int_t rows, trsmrows, igpu, n_local[Magma_tally4MaxGPUs], ldpanel;
    magma_tally4_queue_t queues[Magma_tally4MaxGPUs][10];
    ...
    *info = 0;
    ...
    if (MAGMA_tally4_SUCCESS != magma_tally4_dmalloc_pinned( &panel, 2 * nb * ldpanel )) {
    ...
    if ((nb <= 1) || (nb >= n)) {
        // Use unblocked code.
        magma_tally4_dgetmatrix( n, n, dlA(0, 0, 0), ldda, panel, ldpanel);
        lapackf77_dpotrf( uplo_, &n, panel, &ldpanel, info);
        magma_tally4_dsetmatrix( n, n, panel, ldpanel, dlA(0, 0, 0), ldda );
    } else {
        for( d = 0; d < ngpu; d++ ) {
            ...
            if (MAGMA_tally4_SUCCESS != magma_tally4_dmalloc( &d_lP[d], nb * ldda )) {
                for( j = 0; j < d; j++ ) {
                    magma_tally4_setdevice(j);
                    magma_tally4_free( d_lP[d] );
                }
                *info = MAGMA_tally4_ERR_DEVICE_ALLOC;
                return *info;
            }
            for( j=0; j < nqueue; j++ ) {
                magma_tally4_queue_create( &queues[d][j] );
            }
        }
        ...
        if (uplo == Magma_tally4Upper) {
            ...
            #if defined (ENABLE_TIMER)
            real_Double_t tget = magma_tally4_wtime(), tset = 0.0, ttot = 0.0;
            #endif
            if ( n > nb ) {
                // send first panel to cpu
                magma_tally4_setdevice(0);
                tmppanel = tmppanel0;
                magma_tally4_dgetmatrix_async(n, nb,
                        dlA(0, 0, 0), ldda,
                        tmppanel(0),  ldpanel,
                        queues[0][0] );
            }
            ...
            for (j = 0; (j + nb) < n; j += nb) {
                ...
                magma_tally4_queue_sync( queues[id][0] );
                if (j > 0 && prevtrsmrows > crosspoint) {
                    #if defined (ENABLE_TIMER)
                    tcnp = magma_tally4_wtime();
                    #endif

                    tmpprevpanel = ((blkid - 1) % 2) == 0 ? tmppanel0 : tmppanel1;

                    blasf77_dgemm( Magma_tally4NoTransStr, Magma_tally4ConjTransStr,
                            &rows, &nb, &nb,
                    ...
                    #if defined (ENABLE_TIMER)
                    tcnp = magma_tally4_wtime() - tcnp;
                    ttot_cnp += tcnp;
                    #endif
                }

                #if defined (ENABLE_TIMER)
                tcchol = magma_tally4_wtime();
                #endif
                lapackf77_dpotrf(Magma_tally4LowerStr, &nb, tmppanel(j), &ldpanel, info);
                if (*info != 0) {
                    *info = *info + j;
                    break;
                }
                ...
                #if defined (ENABLE_TIMER)
                tcchol = magma_tally4_wtime() - tcchol;
                ttot_cchol += tcchol;
                tctrsm = magma_tally4_wtime();
                #endif

                trsmrows = rows - nb;

                if (trsmrows > 0) {
                    blasf77_dtrsm(Magma_tally4RightStr, Magma_tally4LowerStr, Magma_tally4ConjTransStr, Magma_tally4NonUnitStr,
                                  &trsmrows, &nb,
                                  &z_one, tmppanel(j), &ldpanel,
                ...
                #if defined (ENABLE_TIMER)
                tctrsm = magma_tally4_wtime() - tctrsm;
                ttot_ctrsm += tctrsm;
                tctm = magma_tally4_wtime();
                #endif

                d = (id + 1) % ngpu;
                // send current panel to gpus
                for (igpu = 0; igpu < ngpu; igpu++, d = (d + 1) % ngpu ) {
                    magma_tally4_int_t myrows = 0;
                    magma_tally4_int_t row_offset = 0;
                    if ( d == id ) {
                    ...
                    if (myrows > 0) {
                        magma_tally4_setdevice(d);
                        magma_tally4_dsetmatrix_async(myrows, nb,
                ...
                for (igpu = 0; igpu < ngpu; igpu++, d = (d + 1) % ngpu ) {
                    magma_tally4_setdevice(d);
                    magma_tally4_queue_sync( queues[d][0] );
                }

                #if defined (ENABLE_TIMER)
                tctm = magma_tally4_wtime() - tctm;
                ttot_ctm += tctm;
                #endif
                ...
                if ( (j + nb) < n) {
                    ...
                    if (j + nb + nb < n) {
                        d = (id + 1) % ngpu;
                        magma_tally4_setdevice(d);
                        magma_tally4_int_t j_local2 = (j + nb) / (nb * ngpu) * nb;
                        if (trsmrows <= crosspoint) {
                            #if defined (ENABLE_TIMER)
                            tmnp = magma_tally4_wtime();
                            #endif
                            ...
                            magma_tally4blasSetKernelStream( queues[d][STREAM_ID(j_local2)] );
                            #define DSYRK_ON_DIAG
                            #ifdef  DSYRK_ON_DIAG
                            magma_tally4_dsyrk( Magma_tally4Lower, Magma_tally4NoTrans,
                                    nb, nb,
                                    m_one, dlpanel, ldda,
                            ...
                            #if defined (ENABLE_TIMER)
                            magma_tally4_device_sync();
                            tmnp = magma_tally4_wtime() - tmnp;
                            ttot_mnp += tmnp;
                            #endif
                        }
                        // send next panel to cpu
                        magma_tally4_queue_sync( queues[d][STREAM_ID(j_local2)] ); // make sure lookahead is done
                        tmppanel = ((blkid+1) % 2 == 0) ? tmppanel0 : tmppanel1;
                        magma_tally4_dgetmatrix_async(rows-nb, nb,
                    ...
                    if (n - offset > 0) {
                        ...
                        #if defined (ENABLE_TIMER)
                        for( d=0; d < ngpu; d++ ) therk[d] = magma_tally4_wtime();
                        #endif

                        //magma_tally4blasSetKernelStream( queues[d] );
                        //magma_tally4_dsyrk(Magma_tally4Lower, Magma_tally4NoTrans, n - offset, nb,
                        //        m_one, dlpanel, ldda,
                        ...
                        #endif
                                        (ngpu, Magma_tally4Lower, Magma_tally4NoTrans,
                                         nb, n - offset, nb,
                        ...
                        for( d=0; d < ngpu; d++ ) {
                            magma_tally4_setdevice(d);
                            magma_tally4_device_sync();
                            therk[d] = magma_tally4_wtime() - therk[d];
                            ttot_herk[d] += therk[d];
                        }
                        #endif
                    }
            ...
            for( d = 0; d < ngpu; d++ ) {
                ...
                for( id=0; id < nqueue; id++ ) {
                    magma_tally4_queue_sync( queues[d][id] );
                }
            }
            #if defined (ENABLE_TIMER)
            ...
            if (j < n && *info == 0) {
                ...
                magma_tally4_setdevice(id);
                #if defined (ENABLE_TIMER)
                tset = magma_tally4_wtime();
                #endif
                magma_tally4_dgetmatrix(rows, rows, dlA(id, j, j_local), ldda, panel(j), ldpanel);
                lapackf77_dpotrf(Magma_tally4LowerStr, &rows, panel(j), &ldpanel, info);
                magma_tally4_dsetmatrix(rows, rows, panel(j), ldpanel, dlA(id, j, j_local), ldda);
                #if defined (ENABLE_TIMER)
                tset = magma_tally4_wtime() - tset;
                #endif
            }
            #if defined (ENABLE_TIMER)
        ...
        for( d = 0; d < ngpu; d++ ) {
            magma_tally4_setdevice(d);
            for( j=0; j < nqueue; j++ ) {
                magma_tally4_queue_destroy( queues[d][j] );
            }
            magma_tally4_free( d_lP[d] );
        }
    } // end of not lapack
    ...
    // free workspace
    magma_tally4_free_pinned( panel );
