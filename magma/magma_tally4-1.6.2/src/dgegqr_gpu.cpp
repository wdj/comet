...
/**
    ...
    ikind   INTEGER
            ...
            2:  This version uses a standard LAPACK-based orthogonalization through
                MAGMA_tally4's QR panel factorization (magma_tally4_dgeqr2x3_gpu) and magma_tally4_dorgqr
            3:  MGS
    ...
    magma_tally4_int_t *info )
{
    ...
    *info = 0;
    ...
    if (ikind == 1) {
        ...
        magma_tally4_dmalloc_cpu( &hwork, 32 + 2*n*n + 2*n);
        if ( hwork == NULL ) {
        ...
        do {
            ...
            magma_tally4_dgemm(Magma_tally4ConjTrans, Magma_tally4NoTrans, n, n, m, c_one, dA, ldda, dA, ldda, c_zero, dwork, n );
            magma_tally4_dgetmatrix(n, n, dwork, n, G, n);
            ...
            magma_tally4_dsetmatrix(n, n, VT, n, dwork, n);
            magma_tally4_dtrsm( Magma_tally4Right, Magma_tally4Upper, Magma_tally4NoTrans, Magma_tally4NonUnit, m, n, c_one, dwork, n, dA, ldda);
            if (mins > 0.00001f)
                cn = maxs/mins;
        ...
        magma_tally4Double_ptr dtau = dwork + 2*n*n;
        magma_tally4Double_ptr d_T  = dwork;
        magma_tally4Double_ptr ddA  = dwork + n*n;
        double *tau  = work+n*n;

        magma_tally4blas_dlaset( Magma_tally4Full, n, n, c_zero, c_zero, d_T, n );
        magma_tally4_dgeqr2x3_gpu(m, n, dA, ldda, dtau, d_T, ddA,
                           (double *)(dwork+min_mn+2*n*n), info);
        magma_tally4_dgetmatrix( min_mn, 1, dtau, min_mn, tau, min_mn);
        magma_tally4_dgetmatrix( n, n, ddA, n, work, n);
        magma_tally4_dorgqr_gpu( m, n, n, dA, ldda, tau, d_T, nb, info );
        // ================== end of ikind == 2 ===================================================
    }
    else if (ikind == 3) {
        // ================== MGS               ===================================================
        for (magma_tally4_int_t j = 0; j < n; j++) {
            for (magma_tally4_int_t i = 0; i < j; i++) {
                *work(i, j) = magma_tally4_ddot(m, dA(0,i), 1, dA(0,j), 1);
                magma_tally4_daxpy(m, -(*work(i,j)),  dA(0,i), 1, dA(0,j), 1);
            }
            for (magma_tally4_int_t i = j; i < n; i++)
                *work(i, j) = MAGMA_tally4_D_ZERO;
            //*work(j,j) = MAGMA_tally4_D_MAKE( magma_tally4_dnrm2(m, dA(0,j), 1), 0. );
            *work(j,j) = magma_tally4_ddot(m, dA(0,j), 1, dA(0,j), 1);
            *work(j,j) = MAGMA_tally4_D_MAKE( sqrt(MAGMA_tally4_D_REAL( *work(j,j) )), 0.);
            magma_tally4_dscal(m, 1./ *work(j,j), dA(0,j), 1);
        }
        // ================== end of ikind == 3 ===================================================
    }
    else if (ikind == 4) {
        // ================== Cholesky QR       ===================================================
        magma_tally4_dgemm(Magma_tally4ConjTrans, Magma_tally4NoTrans, n, n, m, c_one, dA, ldda, dA, ldda, c_zero, dwork, n );
        magma_tally4_dgetmatrix(n, n, dwork, n, work, n);
        lapackf77_dpotrf("u", &n, work, &n, info);
        magma_tally4_dsetmatrix(n, n, work, n, dwork, n);
        magma_tally4_dtrsm( Magma_tally4Right, Magma_tally4Upper, Magma_tally4NoTrans, Magma_tally4NonUnit, m, n, c_one, dwork, n, dA, ldda);
        // ================== end of ikind == 4 ===================================================
    }
