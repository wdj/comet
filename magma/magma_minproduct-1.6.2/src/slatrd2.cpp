...
/**
    ...
    If UPLO = Magma_minproductUpper, SLATRD reduces the last NB rows and columns of a
    matrix, of which the upper triangle is supplied;
    if UPLO = Magma_minproductLower, SLATRD reduces the first NB rows and columns of a
    ...
    magma_minproductFloat_ptr dwork, magma_minproduct_int_t ldwork)
{
    ...
    float *f;
    magma_minproduct_smalloc_cpu( &f, n );
    if ( f == NULL ) {
    ...
    if (uplo == Magma_minproductUpper) {
        ...
        for (i = n-1; i >= n - nb; --i) {
            ...
            if (i > 0) {
                ...
                e[i-1] = MAGMA_minproduct_S_REAL( alpha );
                *A(i-1,i) = MAGMA_minproduct_S_ONE;
                
                /* Compute W(1:i-1,i) */
                // 1. Send the block reflector  A(0:n-i-1,i) to the GPU
                magma_minproduct_ssetvector_async( i, A(0, i), 1, dA(0, i), 1, stream );
                
                magma_minproductblas_ssymv_work( Magma_minproductUpper, i, c_one, dA(0, 0), ldda,
                                      dA(0, i), ione, c_zero, dW(0, iw), ione,
                                      dwork, ldwork, stream );
                
                // 2. Start getting the result back (asynchronously)
                magma_minproduct_sgetmatrix_async( i, 1,
                                        dW(0, iw), lddw,
                                        W(0, iw),  ldw, stream );
                
                if (i < n-1) {
                    blasf77_sgemv( Magma_minproductConjTransStr, &i, &i_n, &c_one, W(0, iw+1), &ldw,
                ...
                if (i < n-1) {
                    ...
                    blasf77_sgemv( Magma_minproductConjTransStr, &i, &i_n, &c_one, A(0, i+1), &lda,
                                   A(0, i), &ione, &c_zero, W(i+1, iw), &ione );
                ...
                value = magma_minproduct_cblas_sdot( i, W(0,iw), ione, A(0,i), ione );
        ...
        for (i = 0; i < nb; ++i) {
            ...
            if (i < n-1) {
                ...
                e[i] = MAGMA_minproduct_S_REAL( alpha );
                *A(i+1,i) = MAGMA_minproduct_S_ONE;
                
                /* Compute W(i+1:n,i) */
                // 1. Send the block reflector  A(i+1:n,i) to the GPU
                magma_minproduct_ssetvector_async( i_n, A(i+1, i), 1, dA(i+1, i), 1, stream );
                
                magma_minproductblas_ssymv_work( Magma_minproductLower, i_n, c_one, dA(i+1, i+1), ldda,
                                      dA(i+1, i), ione, c_zero, dW(i+1, i), ione,
                                      dwork, ldwork, stream );
                
                // 2. Start getting the result back (asynchronously)
                magma_minproduct_sgetmatrix_async( i_n, 1,
                                        dW(i+1, i), lddw,
                                        W(i+1, i),  ldw, stream );
                
                blasf77_sgemv( Magma_minproductConjTransStr, &i_n, &i, &c_one, W(i+1, 0), &ldw,
                               A(i+1, i), &ione, &c_zero, W(0, i), &ione );
                
                blasf77_sgemv( "No transpose", &i_n, &i, &c_neg_one, A(i+1, 0), &lda,
                               W(0, i), &ione, &c_zero, f, &ione );
                
                blasf77_sgemv( Magma_minproductConjTransStr, &i_n, &i, &c_one, A(i+1, 0), &lda,
                               A(i+1, i), &ione, &c_zero, W(0, i), &ione );
                ...
                value = magma_minproduct_cblas_sdot( i_n, W(i+1,i), ione, A(i+1,i), ione );
