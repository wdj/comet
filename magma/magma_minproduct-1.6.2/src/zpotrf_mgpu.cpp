    ...
    magma_minproduct_int_t *info)
{
    ...
    magma_minproductDoubleComplex *dwork[Magma_minproductMaxGPUs];
    ...
    *info = 0;
    ...
    if (ngpu == 1 && ((nb <= 1) || (nb >= n)) ) {
        ...
        if (MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc_pinned( &work, n*nb )) {
            *info = MAGMA_minproduct_ERR_HOST_ALLOC;
            return *info;
        }
        magma_minproduct_zgetmatrix( n, n, d_lA[0], ldda, work, n );
        lapackf77_zpotrf(uplo_, &n, work, &n, info);
        magma_minproduct_zsetmatrix( n, n, work, n, d_lA[0], ldda );
        ...
        for( d=0; d < ngpu; d++ ) {
            ...
            if (MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc( &dwork[d], ngpu*nb*lddp )) {
                for( j=0; j < d; j++ ) {
        ...
        magma_minproduct_setdevice(0);
        h = 1; //ngpu; //(n+nb-1)/nb;
        if (MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc_pinned( &work, n*nb*h )) {
            *info = MAGMA_minproduct_ERR_HOST_ALLOC;
            return *info;
        }
        if (upper) {
            /* with three streams */
            magma_minproduct_zpotrf3_mgpu(ngpu, uplo, n, n, 0, 0, nb, d_lA, ldda, dwork, lddp, work, n,
                               h, stream, event, info);
        } else {
            /* with three streams */
            magma_minproduct_zpotrf3_mgpu(ngpu, uplo, n, n, 0, 0, nb, d_lA, ldda, dwork, lddp, work, nb*h,
                               h, stream, event, info);
        }
