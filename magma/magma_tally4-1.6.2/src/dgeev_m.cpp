...
/**
    ...
    @ingroup magma_tally4_dgeev_driver
    ********************************************************************/
    ...
    magma_tally4_int_t *info )
{
    ...
    *info = 0;
    ...
    if (*info == 0) {
        minwrk = (2 +   nb)*n;
        optwrk = (2 + 2*nb)*n;
        work[0] = MAGMA_tally4_D_MAKE( (double) optwrk, 0. );
    ...
    double *dT;
    if (MAGMA_tally4_SUCCESS != magma_tally4_dmalloc( &dT, nb*n )) {
    ...
    double *T;
    if (MAGMA_tally4_SUCCESS != magma_tally4_dmalloc_cpu( &T, nb*n )) {
    ...
    if (wantvl) {
        ...
        lapackf77_dlacpy( Magma_tally4LowerStr, &n, &n, A, &lda, VL, &ldvl );
        ...
        flops_start( flop_unghr );
            ...
            // Version 3 - LAPACK consistent MAGMA_tally4 HRD + T matrices stored
            magma_tally4_dorghr( n, ilo, ihi, VL, ldvl, &work[itau], dT, nb, &ierr );
        #elif defined(Version5)
            // Version 5 - Multi-GPU, T on host
            magma_tally4_dorghr_m( n, ilo, ihi, VL, ldvl, &work[itau], T, nb, &ierr );
        #endif
        ...
        #elif defined(Version3) || defined(Version4)
            // Version 3 - LAPACK consistent MAGMA_tally4 HRD + T matrices stored
            magma_tally4_dorghr( n, ilo, ihi, VR, ldvr, &work[itau], dT, nb, &ierr );
        #elif defined(Version5)
            // Version 5 - Multi-GPU, T on host
            magma_tally4_dorghr_m( n, ilo, ihi, VR, ldvr, &work[itau], T, nb, &ierr );
        #endif
        time_sum += timer_stop( time_unghr );
        flop_sum += flops_stop( flop_unghr );
    ...
    if (wantvl || wantvr) {
        ...
        magma_tally4_dtrevc3_mt( side, Magma_tally4BacktransVec, select, n, A, lda, VL, ldvl,
                          VR, ldvr, n, &nout, &work[iwrk], liwrk, &ierr );
        #elif TREVC_VERSION == 5
        magma_tally4_dtrevc3_mt_gpu( side, Magma_tally4BacktransVec, select, n, A, lda, VL, ldvl,
                              VR, ldvr, n, &nout, &work[iwrk], liwrk, &ierr );
        #else
    ...
    if (wantvl) {
        ...
        for (i = 0; i < n; ++i) {
            if ( wi[i] == 0. ) {
                scl = 1. / magma_tally4_cblas_dnrm2( n, VL(0,i), 1 );
                blasf77_dscal( &n, &scl, VL(0,i), &ione );
            }
            else if ( wi[i] > 0. ) {
                d__1 = magma_tally4_cblas_dnrm2( n, VL(0,i), 1 );
                d__2 = magma_tally4_cblas_dnrm2( n, VL(0,i+1), 1 );
    ...
    if (wantvr) {
        ...
        for (i = 0; i < n; ++i) {
            if ( wi[i] == 0. ) {
                scl = 1. / magma_tally4_cblas_dnrm2( n, VR(0,i), 1 );
                blasf77_dscal( &n, &scl, VR(0,i), &ione );
            }
            else if ( wi[i] > 0. ) {
                d__1 = magma_tally4_cblas_dnrm2( n, VR(0,i), 1 );
                d__2 = magma_tally4_cblas_dnrm2( n, VR(0,i+1), 1 );
                scl = 1. / lapackf77_dlapy2( &d__1, &d__2 );
                blasf77_dscal( &n, &scl, VR(0,i), &ione );
