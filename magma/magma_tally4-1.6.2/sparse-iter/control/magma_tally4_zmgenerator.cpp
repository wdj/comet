...
/**
    ...
    diag_offset magma_tally4_int_t*
                array containing the offsets
                                                ...
                                                (length offsets+1)
    @param[in]
    diag_vals   magma_tally4DoubleComplex*
    ...
    magma_tally4_queue_t queue )
{
    ...
    CHECK( magma_tally4_zmalloc_cpu( &B.val, B.max_nnz_row*n ));
    ...
    for( int i=0; i<n; i++ ) { // stride over rows
        for( int j=0; j<B.max_nnz_row; j++ ) { // nonzeros in every row
            if ( (B.col[i*B.max_nnz_row + j] < 0) ||
                ...
                B.val[ i*B.max_nnz_row + j ] = MAGMA_tally4_Z_MAKE( 0.0, 0.0 );
            }
        }
    ...
    for( int i=0; i<n; i++ ) { // stride over rows
        for( int j=0; j<B.max_nnz_row; j++ ) { // nonzeros in every row
            if ( MAGMA_tally4_Z_REAL( B.val[i*B.max_nnz_row + j]) != 0.0 )
                B.nnz++;
        }
    ...
    CHECK( magma_tally4_zmconvert( B, A, Magma_tally4_ELLPACKT, Magma_tally4_CSR, queue ));
    ...
    magma_tally4_queue_t queue )
{
    ...
    magma_tally4_z_matrix hA={Magma_tally4_CSR};
    ...
    diag_vals[0] = MAGMA_tally4_Z_MAKE( 26.0, 0.0 );
    diag_vals[1] = MAGMA_tally4_Z_MAKE( -1.0, 0.0 );
    diag_vals[2] = MAGMA_tally4_Z_MAKE( -1.0, 0.0 );
    diag_vals[3] = MAGMA_tally4_Z_MAKE( -1.0, 0.0 );
    diag_vals[4] = MAGMA_tally4_Z_MAKE( -1.0, 0.0 );
    diag_vals[5] = MAGMA_tally4_Z_MAKE( -1.0, 0.0 );
    diag_vals[6] = MAGMA_tally4_Z_MAKE( -1.0, 0.0 );
    diag_vals[7] = MAGMA_tally4_Z_MAKE( -1.0, 0.0 );
    diag_vals[8] = MAGMA_tally4_Z_MAKE( -1.0, 0.0 );
    diag_vals[9] = MAGMA_tally4_Z_MAKE( -1.0, 0.0 );
    diag_vals[10] = MAGMA_tally4_Z_MAKE( -1.0, 0.0 );
    diag_vals[11] = MAGMA_tally4_Z_MAKE( -1.0, 0.0 );
    diag_vals[12] = MAGMA_tally4_Z_MAKE( -1.0, 0.0 );
    diag_vals[13] = MAGMA_tally4_Z_MAKE( -1.0, 0.0 );
    CHECK( magma_tally4_zmgenerator( nn, offdiags, diag_offset, diag_vals, &hA, queue ));
        ...
        for( k=hA.row[row]; k<hA.row[row+1]; k++) {
            ...
            if ((hA.col[k] == row-1 ||
                ...
                hA.col[k] == row+n*n+n-1 ) && (row+1)%n == 1 )
                    ...
                    hA.val[k] = MAGMA_tally4_Z_MAKE( 0.0, 0.0 );
    ...
    CHECK( magma_tally4_zmconvert( hA, A, Magma_tally4_CSR, Magma_tally4_CSR, queue ));
    ...
    magma_tally4_queue_t queue )
{
    ...
    #ifdef COMPLEX
        // complex case
        diag_vals[0] = MAGMA_tally4_Z_MAKE( 4.0, 4.0 );
        diag_vals[1] = MAGMA_tally4_Z_MAKE( -1.0, -1.0 );
        diag_vals[2] = MAGMA_tally4_Z_MAKE( -1.0, -1.0 );
        
    #else
        // real case
        diag_vals[0] = MAGMA_tally4_Z_MAKE( 4.0, 0.0 );
        diag_vals[1] = MAGMA_tally4_Z_MAKE( -1.0, 0.0 );
        diag_vals[2] = MAGMA_tally4_Z_MAKE( -1.0, 0.0 );
    #endif
    CHECK( magma_tally4_zmgenerator( nn, offdiags, diag_offset, diag_vals, &hA, queue ));
