...
int main( int argc, char** argv)
{
    ...
    float          magma_tally2_error, cublas_error, lapack_error, work[1];
    ...
    magma_tally2FloatComplex *h_A, *h_B, *h_Bcublas, *h_Bmagma_tally2, *h_Blapack, *h_X;
    magma_tally2FloatComplex *d_A, *d_B;
    magma_tally2FloatComplex **d_A_array = NULL;
    magma_tally2FloatComplex **d_B_array = NULL;
    ...
    magma_tally2FloatComplex alpha = MAGMA_tally2_C_MAKE(  0.29, -0.86 );
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_Blapack, magma_tally2FloatComplex, sizeB   );
            TESTING_MALLOC_CPU( h_Bcublas, magma_tally2FloatComplex, sizeB   );
            TESTING_MALLOC_CPU( h_Bmagma_tally2,  magma_tally2FloatComplex, sizeB   );
            TESTING_MALLOC_CPU( ipiv,      magma_tally2_int_t,        Ak      );
            
            TESTING_MALLOC_DEV( d_A,       magma_tally2FloatComplex, ldda*Ak*batchCount );
            TESTING_MALLOC_DEV( d_B,       magma_tally2FloatComplex, lddb*N*batchCount  );
            ...
            magma_tally2FloatComplex* dwork=NULL; // invA and work are workspace in ctrsm
 
            magma_tally2_int_t dinvA_batchSize = magma_tally2_roundup( Ak, TRI_NB )*TRI_NB;
            magma_tally2_int_t dwork_batchSize = lddb*N;
            magma_tally2_cmalloc( &dinvA, dinvA_batchSize * batchCount);
            magma_tally2_cmalloc( &dwork, dwork_batchSize * batchCount );
    
            cset_pointer(dwork_array, dwork, lddb, 0, 0, dwork_batchSize, batchCount, opts.queue);
            cset_pointer(dinvA_array, dinvA, magma_tally2_roundup( Ak, TRI_NB ), 0, 0, dinvA_batchSize, batchCount, opts.queue);

            memset(h_Bmagma_tally2, 0, batchCount*ldb*N*sizeof(magma_tally2FloatComplex));
            magma_tally2blas_claset( Magma_tally2Full, lddb, N*batchCount, c_zero, c_zero, dwork, lddb);
            ...
            memcpy( h_Blapack, h_B, sizeB*sizeof(magma_tally2FloatComplex) );
            
            /* =====================================================================
               Performs operation using MAGMA_tally2BLAS
               =================================================================== */
            magma_tally2_csetmatrix( Ak, Ak*batchCount, h_A, lda, d_A, ldda );
            magma_tally2_csetmatrix( M,  N*batchCount, h_B, ldb, d_B, lddb );
            ...
            magma_tally2_time = magma_tally2_sync_wtime( NULL );
                ...
                magma_tally2_cgetmatrix( M, N*batchCount, dwork, lddb, h_Bmagma_tally2, ldb );
            #else
                ...
                magma_tally2_time = magma_tally2_sync_wtime( NULL ) - magma_tally2_time;
                magma_tally2_perf = gflops / magma_tally2_time;
                magma_tally2_cgetmatrix( M, N*batchCount, d_B, lddb, h_Bmagma_tally2, ldb );
            #endif
            ...
            // CUBLAS version <= 6.0 has magma_tally2FloatComplex **            dA_array, no cast needed.
            // CUBLAS version    6.5 has magma_tally2FloatComplex const**       dA_array, requiring cast.
            // Correctly, it should be   magma_tally2FloatComplex const* const* dA_array, to avoid requiring cast.
                ...
                cublasCtrsmBatched(
                    ...
                    M, N, &alpha,
                    (const magma_tally2FloatComplex**) d_A_array, ldda,
                    d_B_array, lddb, batchCount);
                cublas_time = magma_tally2_sync_wtime( NULL ) - cublas_time;
                cublas_perf = gflops / cublas_time;
            #endif

            magma_tally2_cgetmatrix( M, N*batchCount, d_B, lddb, h_Bcublas, ldb );
            ...
            /* =====================================================================
               ...
               =================================================================== */
            // ||b - 1/alpha*A*x|| / (||A||*||x||)
            magma_tally2FloatComplex alpha2 = MAGMA_tally2_C_DIV( c_one, alpha );
            float normR, normX, normA=0.;
            magma_tally2_error  = 0.0;
            cublas_error = 0.0;

            memcpy( h_X, h_Bmagma_tally2, sizeB*sizeof(magma_tally2FloatComplex) );
            ...
            for (int s=0; s < batchCount; s++) {
                ...
                normX = lapackf77_clange( "M", &M, &N, h_Bmagma_tally2 + s * ldb * N, &ldb, work );
                float magma_tally2_err = normR/(normX*normA);
            ...
            memcpy( h_X, h_Bcublas, sizeB*sizeof(magma_tally2FloatComplex) );
            // check cublas
            #if CUDA_VERSION >= 6050
            for (int s=0; s < batchCount; s++) {
                normA = lapackf77_clange( "M", &Ak, &Ak, h_A + s * lda * Ak, &lda, work );
            ...
            if ( opts.lapack ) {
                ...
                lapack_error = 0.0;
                memcpy( h_X, h_Blapack, sizeB*sizeof(magma_tally2FloatComplex) );
                for (int s=0; s < batchCount; s++) {
                    blasf77_ctrmm(
                        lapack_side_const_tally2(opts.side), lapack_uplo_const_tally2(opts.uplo),
                        lapack_trans_const_tally2(opts.transA), lapack_diag_const_tally2(opts.diag),
