...
int main( int argc, char** argv)
{
    ...
    magma_tally3FloatComplex *h_A, *h_T, *h_R, *tau, *h_work, tmp[1];
    magma_tally3FloatComplex_ptr d_A,  d_T, ddA, dtau;
    magma_tally3FloatComplex_ptr d_A2, d_T2, ddA2, dtau2;
    magma_tally3Float_ptr dwork, dwork2;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_T,   magma_tally3FloatComplex, N*N    );
        
            TESTING_MALLOC_PIN( h_R,   magma_tally3FloatComplex, n2     );
        
            TESTING_MALLOC_DEV( d_A,   magma_tally3FloatComplex, ldda*N );
            TESTING_MALLOC_DEV( d_T,   magma_tally3FloatComplex, N*N    );
            TESTING_MALLOC_DEV( ddA,   magma_tally3FloatComplex, N*N    );
            TESTING_MALLOC_DEV( dtau,  magma_tally3FloatComplex, min_mn );
        
            TESTING_MALLOC_DEV( d_A2,  magma_tally3FloatComplex, ldda*N );
            TESTING_MALLOC_DEV( d_T2,  magma_tally3FloatComplex, N*N    );
            TESTING_MALLOC_DEV( ddA2,  magma_tally3FloatComplex, N*N    );
            TESTING_MALLOC_DEV( dtau2, magma_tally3FloatComplex, min_mn );
            ...
            // todo replace with magma_tally3_claset
            magma_tally3blas_claset( Magma_tally3Full, N, N, c_zero, c_zero, ddA,  N );
            magma_tally3blas_claset( Magma_tally3Full, N, N, c_zero, c_zero, d_T,  N );
            magma_tally3blas_claset( Magma_tally3Full, N, N, c_zero, c_zero, ddA2, N );
            magma_tally3blas_claset( Magma_tally3Full, N, N, c_zero, c_zero, d_T2, N );
        
            lwork = -1;
            lapackf77_cgeqrf(&M, &N, NULL, &M, NULL, tmp, &lwork, &info);
            lwork = (magma_tally3_int_t)MAGMA_tally3_C_REAL( tmp[0] );
            lwork = max(lwork, N*N);
        
            TESTING_MALLOC_CPU( h_work, magma_tally3FloatComplex, lwork );

            /* Initialize the matrix */
            lapackf77_clarnv( &ione, ISEED, &n2, h_A );
            lapackf77_clacpy( Magma_tally3UpperLowerStr, &M, &N, h_A, &lda, h_R, &lda );
            magma_tally3_csetmatrix( M, N, h_R, lda,  d_A, ldda );
            magma_tally3_csetmatrix( M, N, h_R, lda, d_A2, ldda );
            ...
            if (opts.version == 1)
                magma_tally3_cgeqr2x_gpu(M, N, d_A, ldda, dtau, d_T, ddA, dwork, &info);
            else if (opts.version == 2)
                magma_tally3_cgeqr2x2_gpu(M, N, d_A, ldda, dtau, d_T, ddA, dwork, &info);
            else if (opts.version == 3)
                magma_tally3_cgeqr2x3_gpu(M, N, d_A, ldda, dtau, d_T, ddA, dwork, &info);
            else {
                printf( "call magma_tally3_cgeqr2x4_gpu\n" );
                /*
                ...
                */
                magma_tally3_cgeqr2x4_gpu(M, N, d_A, ldda, dtau, d_T, ddA, dwork, NULL, &info);
                //magma_tally3_cgeqr2x4_gpu(M, N, d_A, ldda, dtau, d_T, ddA, dwork, &info, stream[1]);
                //magma_tally3_cgeqr2x4_gpu(M, N, d_A2, ldda, dtau2, d_T2, ddA2, dwork2, &info, stream[0]);
                //magma_tally3_cgeqr2x4_gpu(M, N, d_A2, ldda, dtau2, d_T2, ddA2, dwork2, &info, NULL);
                //gflops *= 2;
            }
            ...
            if (info != 0) {
                printf("magma_tally3_cgeqr2x_gpu version %d returned error %d: %s.\n",
                       (int) opts.version, (int) info, magma_tally3_strerror( info ));
            } 
            else {
                if ( opts.check ) {
                    /* =====================================================================
                       ...
                       =================================================================== */
                    magma_tally3_cgetmatrix( M, N, d_A, ldda, h_R, M );
                    magma_tally3_cgetmatrix( N, N, ddA, N,    h_T, N );
                    magma_tally3_cgetmatrix( min_mn, 1, dtau, min_mn,   tau, min_mn );
                    ...
                    magma_tally3FloatComplex *Q, *R;
                    float *work;
                    TESTING_MALLOC_CPU( Q,    magma_tally3FloatComplex, ldq*min_mn );  // M by K
                    TESTING_MALLOC_CPU( R,    magma_tally3FloatComplex, ldr*N );       // K by N
                    ...
                    lapackf77_clacpy( Magma_tally3UpperLowerStr, &M, &N, h_R, &M, h_A, &lda );
                    lapackf77_clarft( Magma_tally3ForwardStr, Magma_tally3ColumnwiseStr,
                                      &M, &N, h_A, &lda, tau, h_work, &N);
                    //magma_tally3_cgeqr2(&M, &N, h_A, &lda, tau, h_work, &info);
                    ...
                    magma_tally3_cgetmatrix( N, N, d_T, N, h_T, N );
    
                    float terr = 0.;
                    for(int col=0; col < N; col++)
                        for(int row=0; row <= col; row++)
                            terr += (  MAGMA_tally3_C_ABS(h_work[row + col*N] - h_T[row + col*N])*
                                       MAGMA_tally3_C_ABS(h_work[row + col*N] - h_T[row + col*N])  );
                    terr = sqrt( terr );
