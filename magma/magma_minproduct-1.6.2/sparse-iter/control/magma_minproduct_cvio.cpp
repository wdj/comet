/*
    -- MAGMA_minproduct (version 1.6.2) --
       ...
       @generated from magma_minproduct_zvio.cpp normal z -> c, Sun May  3 11:23:01 2015
       @author Hartwig Anzt
*/
...
/**
    ...
    Visualizes part of a vector of type magma_minproduct_c_matrix.
    ...
    magma_minproduct_queue_t queue )
{
    ...
    magma_minproduct_c_matrix y={Magma_minproduct_CSR};
    ...
    if ( x.memory_location == Magma_minproduct_CPU ) {
        ...
        for( magma_minproduct_int_t i=offset; i<offset + visulen; i++ )
            magma_minproduct_cprintval(x.val[i]);
    }
    else if ( x.memory_location == Magma_minproduct_DEV ) {
        printf("located on DEV:\n");
        CHECK( magma_minproduct_cmtransfer( x, &y, Magma_minproduct_DEV, Magma_minproduct_CPU, queue ));
        for( magma_minproduct_int_t i=offset; i<offset +  visulen; i++ )
            magma_minproduct_cprintval(y.val[i]);
    ...
    magma_minproduct_queue_t queue )
{
    ...
    CHECK( magma_minproduct_cmalloc_cpu( &x->val, length ));
    ...
    while( i<length )  // eof() is 'true' at the end of data
    {
        ...
        #define COMPLEX
            ...
            float VAL2;
            fscanf(fid, " %f %f \n", &VAL1, &VAL2);
            VAL = MAGMA_minproduct_C_MAKE(VAL1, VAL2);
        #else
            fscanf(fid, " %f \n", &VAL1);
            VAL = MAGMA_minproduct_C_MAKE(VAL1, 0.0);
        #endif
        
        if ( VAL != MAGMA_minproduct_C_ZERO )
            nnz++;
        x->val[i] = VAL;
        i++;
    }
    ...
    magma_minproduct_queue_t queue )
{
    ...
    magma_minproduct_c_matrix A={Magma_minproduct_CSR}, B={Magma_minproduct_CSR};
    magma_minproduct_int_t entry=0;
     //   char *vfilename[] = {"/mnt/sparse_matrices/mtx/rail_79841_B.mtx"};
    CHECK( magma_minproduct_c_csr_mtx( &A,  filename, queue  ));
    CHECK( magma_minproduct_cmconvert( A, &B, Magma_minproduct_CSR, Magma_minproduct_DENSE, queue ));
    CHECK( magma_minproduct_cvinit( x, Magma_minproduct_CPU, A.num_cols, A.num_rows, MAGMA_minproduct_C_ZERO, queue ));
