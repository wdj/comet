...
/**
    ...
    @ingroup magma_minproduct_sgesv_driver
    ********************************************************************/
extern "C" magma_minproduct_int_t
magma_minproduct_sgerfs_nopiv_gpu(
    ...
    magma_minproductFloat_ptr dworkd, magma_minproductFloat_ptr dAF,
    ...
    magma_minproduct_int_t *info)
{
    ...
    *info = 0;
    ...
    Anrm = magma_minproductblas_slange(Magma_minproductInfNorm, n, n, dA, ldda, (float*)dworkd );
    cte  = Anrm * eps * pow( (float)n, (float)0.5 ) * BWDMAX;
    
    // residual dR = dB - dA*dX in real
    magma_minproductblas_slacpy( Magma_minproductUpperLower, n, nrhs, dB, lddb, dR, lddr );
    if ( nrhs == 1 ) {
        magma_minproduct_sgemv( trans, n, n,
                     c_neg_one, dA, ldda,
                                dX, 1,
                     c_one,     dR, 1 );
    }
    else {
        magma_minproduct_sgemm( trans, Magma_minproductNoTrans, n, nrhs, n,
                     c_neg_one, dA, ldda,
                                dX, lddx,
                     c_one,     dR, lddr );
    }
    
    // TODO: use MAGMA_minproduct_S_ABS( dX(i,j) ) instead of slange?
    for( j=0; j < nrhs; j++ ) {
        i = magma_minproduct_isamax( n, dX(0,j), 1) - 1;
        magma_minproduct_sgetmatrix( 1, 1, dX(i,j), 1, &Xnrmv, 1 );
        Xnrm = lapackf77_slange( "F", &ione, &ione, &Xnrmv, &ione, NULL );
        
        i = magma_minproduct_isamax ( n, dR(0,j), 1 ) - 1;
        magma_minproduct_sgetmatrix( 1, 1, dR(i,j), 1, &Rnrmv, 1 );
...
REFINEMENT:
    for( iiter=1; iiter < ITERMAX; ) {
        *info = 0;
        ...
        magma_minproduct_sgetrs_nopiv_gpu( trans, n, nrhs, dAF, lddsa, dR, lddr, info );
        if (*info != 0) {
            *iter = -3;
            goto FALLBACK;
        }
        ...
        for( j=0; j < nrhs; j++ ) {
            magma_minproductblas_saxpycp2( n, dR(0,j), dX(0,j), dB(0,j) );
        }
    ...
    magma_minproduct_int_t *info)
{
    ...
    *info = 0;
    ...
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_smalloc( &dA, n2 )) {
        *info = MAGMA_minproduct_ERR_DEVICE_ALLOC;
        return *info;
    }
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_smalloc( &db, nn*nrhs )) {
        *info = MAGMA_minproduct_ERR_DEVICE_ALLOC;
        return *info;
    }

    if (ref == Magma_minproductTrue) {
        if (MAGMA_minproduct_SUCCESS != magma_minproduct_smalloc( &dAo, n2 )) {
            *info = MAGMA_minproduct_ERR_DEVICE_ALLOC;
            return *info;
        }
        if (MAGMA_minproduct_SUCCESS != magma_minproduct_smalloc( &dwork, nn*nrhs )) {
            *info = MAGMA_minproduct_ERR_DEVICE_ALLOC;
            return *info;
        }
        if (MAGMA_minproduct_SUCCESS != magma_minproduct_smalloc( &dBo, nn*nrhs )) {
            *info = MAGMA_minproduct_ERR_DEVICE_ALLOC;
            return *info;
        }
    }

    if (MAGMA_minproduct_SUCCESS != magma_minproduct_smalloc_cpu( &hu, 2*nn )) {
        *info = MAGMA_minproduct_ERR_HOST_ALLOC;
        return *info;
    }
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_smalloc_cpu( &hv, 2*nn )) {
        *info = MAGMA_minproduct_ERR_HOST_ALLOC;
        return *info;
    }

    magma_minproductblas_slaset(Magma_minproductFull, nn, nn, MAGMA_minproduct_S_ZERO, MAGMA_minproduct_S_ONE, dA, nn);

    /* Send matrix on the GPU*/
    magma_minproduct_ssetmatrix(n, n, A, lda, dA, nn);

    /* Send b on the GPU*/
    magma_minproduct_ssetmatrix(n, nrhs, B, ldb, db, nn);

    *info = magma_minproduct_sgerbt_gpu(Magma_minproductTrue, nn, nrhs, dA, nn, db, nn, hu, hv, info);
    if (*info != MAGMA_minproduct_SUCCESS)  {
        return *info;
    }

    if (ref == Magma_minproductTrue) {
        magma_minproduct_scopymatrix(nn, nn, dA, nn, dAo, nn);
        magma_minproduct_scopymatrix(nn, nrhs, db, nn, dBo, nn);
    }
    /* Solve the system U^TAV.y = U^T.b on the GPU*/ 
    magma_minproduct_sgesv_nopiv_gpu( nn, nrhs, dA, nn, db, nn, info);


    /* Iterative refinement */
    if (ref == Magma_minproductTrue) {
        magma_minproduct_sgerfs_nopiv_gpu(Magma_minproductNoTrans, nn, nrhs, dAo, nn, dBo, nn, db, nn, dwork, dA, &iter, info);
    }
    //printf("iter = %d\n", iter);
    ...
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_smalloc( &dv, 2*nn )) {
        *info = MAGMA_minproduct_ERR_DEVICE_ALLOC;
        return *info;
    }

    magma_minproduct_ssetvector(2*nn, hv, 1, dv, 1);
    
    for(int i = 0; i < nrhs; i++) {
        magma_minproductblas_sprbt_mv(nn, dv, db+(i*nn));
    }

    magma_minproduct_sgetmatrix(n, nrhs, db, nn, B, ldb);
