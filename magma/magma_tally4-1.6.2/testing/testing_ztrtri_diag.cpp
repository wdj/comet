...
int main( int argc, char** argv )
{
    ...
    double          magma_tally4_error, norm_invA, work[1];
    ...
    magma_tally4DoubleComplex *h_A, *h_dinvA;
    magma_tally4DoubleComplex_ptr d_A, d_dinvA;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_DEV( d_dinvA, magma_tally4DoubleComplex, size_inv );
            TESTING_MALLOC_CPU( h_dinvA, magma_tally4DoubleComplex, size_inv );
            ...
            magma_tally4blas_ztrtri_diag( opts.uplo, opts.diag, N, d_A, ldda, d_dinvA );
            magma_tally4_time = magma_tally4_sync_wtime( NULL ) - magma_tally4_time;
            magma_tally4_perf = gflops / magma_tally4_time;
            
            magma_tally4_zgetvector( size_inv, d_dinvA, 1, h_dinvA, 1 );
            
            if ( opts.verbose ) {
                printf( "A%d=", (int) N );
                magma_tally4_zprint( N, N, h_A, lda );
                printf( "d_dinvA%d=", (int) N );
                magma_tally4_zprint( min(N+4, nb), min(N+4, nblock*nb), h_dinvA, nb );
            }
            ...
            if ( opts.check ) {
                ...
                for( int i=0; i < N; i += nb ) {
                    ...
                    magma_tally4_error = max( magma_tally4_error, lapackf77_zlantr( "M", uplo_, Magma_tally4NonUnitStr, &jb, &jb, h_dinvA(0, i), &nb,  work ));
                    norm_invA   = max( norm_invA,   lapackf77_zlantr( "M", uplo_, Magma_tally4NonUnitStr, &jb, &jb, h_A(i, i),     &lda, work ));
                }
