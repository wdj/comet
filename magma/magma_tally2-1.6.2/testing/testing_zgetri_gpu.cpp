...
int main( int argc, char** argv )
{
    ...
    magma_tally2DoubleComplex *h_A, *h_R, *work;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            n2     = lda*N;
            ldda   = ((N+31)/32)*32;
            ldwork = N * magma_tally2_get_zgetri_nb( N );
            ...
            lwork = int( MAGMA_tally2_Z_REAL( tmp ));
            ...
            magma_tally2_zgetrf_gpu( N, N, d_A, ldda, ipiv, &info );
            magma_tally2_zgetmatrix( N, N, d_A, ldda, h_A, lda );
            if ( info != 0 )
                printf("magma_tally2_zgetrf_gpu returned error %d: %s.\n",
                       (int) info, magma_tally2_strerror( info ));
            
            // check for exact singularity
            //h_A[ 10 + 10*lda ] = MAGMA_tally2_Z_MAKE( 0.0, 0.0 );
            //magma_tally2_zsetmatrix( N, N, h_A, lda, d_A, ldda );
            ...
            magma_tally2_zgetri_gpu( N, d_A, ldda, ipiv, dwork, ldwork, &info );
            gpu_time = magma_tally2_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally2_zgetri_gpu returned error %d: %s.\n",
                       (int) info, magma_tally2_strerror( info ));
