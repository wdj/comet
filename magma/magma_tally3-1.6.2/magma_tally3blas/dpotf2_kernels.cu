/*
    -- MAGMA_tally3 (version 1.6.1) --
       ...
       @generated from zpotf2_kernels.cu normal z -> d, Fri Jan 30 19:00:10 2015
*/
...
/////////////////////////////////////////////////////////////////////////////////////////////////
__global__ void ddot_kernel_batched(int n, double **x_array, int incx, int offset, magma_tally3_int_t *info_array, int gbstep)
{
    int tx = threadIdx.x;
    ...
    sdata[tx] = MAGMA_tally3_D_REAL(res * MAGMA_tally3_D_CNJG(res));
    ...
    if (tx == 0) {
        double xreal = MAGMA_tally3_D_REAL(x[n*incx]);        
        //MAGMA_tally3_D_SET2REAL(x[n*incx], sqrt(xreal - sdata[0]));
        x[n*incx] = MAGMA_tally3_D_MAKE(sqrt(xreal - sdata[0]), 0);
        if(x[n*incx] == MAGMA_tally3_D_ZERO){
...
void magma_tally3_dpotf2_ddot_batched(magma_tally3_int_t n, double **x_array, magma_tally3_int_t incx, magma_tally3_int_t offset, magma_tally3_int_t *info_array, magma_tally3_int_t gbstep, magma_tally3_int_t batchCount, magma_tally3_queue_t queue)
{
/*
    Specialized Ddot
...
/////////////////////////////////////////////////////////////////////////////////////////////////
__global__ void dscal_kernel_batched(int n, double **x_array, int incx, int offset, magma_tally3_int_t *info_array)
{
    ...
    if (threadIdx.x == 0) {
        factor = MAGMA_tally3_D_MAKE(1.0/MAGMA_tally3_D_REAL(x[0]), 0.0);
    }
...
void magma_tally3_dpotf2_dscal_batched(magma_tally3_int_t n, double **x_array, magma_tally3_int_t incx, magma_tally3_int_t offset, magma_tally3_int_t *info_array, magma_tally3_int_t batchCount, magma_tally3_queue_t queue)
{
/*
...
__global__ void dlacgv_kernel_batched(int n, double **x_array, int incx, int offset)
{
    ...
    if ( id < n ) {
        x[id*incx] = MAGMA_tally3_D_CNJG(x[id*incx]);
    }
}

void magma_tally3_dlacgv_batched(magma_tally3_int_t n, double **x_array, magma_tally3_int_t incx, int offset, int batchCount, magma_tally3_queue_t queue)
{
/*
    Purpose
    =======
...
static __device__ void dpotf2_device(int m, int n, 
                              double *A, int lda, 
                              double alpha, 
                              double beta, magma_tally3_int_t *info, int gbstep)
{
/*
...
*/
    ...
    {
        double res = MAGMA_tally3_D_ZERO;
        double res1 = MAGMA_tally3_D_ZERO;

        //1) performs ddot sum = A[iter, 0:iter-1]*conj(A[iter, 0:iter-1])
        //2) updates A[iter,iter] = sqrt(A[iter,iter]-sum);
        if(tx<iter)
        {
            res = MAGMA_tally3_D_REAL (sdata_A[iter + tx * m] * MAGMA_tally3_D_CNJG(sdata_A[iter + tx * m]));         
            sum[tx] = res;
        }
        else
        {
        ...
        __syncthreads();
        magma_tally3_sum_reduce<POTF2_TILE_SIZE>(tx, sum);//tried on K40: if m=32 n=32 the overall dpotf2_device routine time is 60ms n=16 time=25 n=8 time=20ms 
        ...
        if (tx == 0) {
              double xreal = MAGMA_tally3_D_REAL(sdata_A[iter + iter * m]);        
              sdata_A[iter + iter * m] = MAGMA_tally3_D_MAKE(sqrt(xreal - sum[0]), 0);
              if(sdata_A[iter + iter * m] == MAGMA_tally3_D_ZERO){
                  *info = iter + gbstep + 1;
              }
        }
        __syncthreads();
        if(sdata_A[iter + iter * m] == MAGMA_tally3_D_ZERO) return;
        __syncthreads();
        ...
        if(tx < iter)
        {
             sdata_A[iter + tx * m] = MAGMA_tally3_D_CNJG(sdata_A[iter + tx * m]);
        }
        __syncthreads();  
        #endif
        ...
        if (tx == 0) {
            factor = MAGMA_tally3_D_MAKE(1.0/MAGMA_tally3_D_REAL(sdata_A[iter + iter * m]), 0.0);
        }
        __syncthreads();
    ...
    magma_tally3_int_t *info_array, magma_tally3_int_t gbstep, magma_tally3_int_t batchCount, magma_tally3_queue_t queue)
{
    ...
    double alpha = MAGMA_tally3_D_NEG_ONE;
    double beta  = MAGMA_tally3_D_ONE;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
extern "C" magma_tally3_int_t
magma_tally3_dpotf2_tile(
    magma_tally3_uplo_t uplo, magma_tally3_int_t m, magma_tally3_int_t n,
    double *dA, magma_tally3_int_t lda,
    magma_tally3_int_t *info)
{
    ...
    *info = 0;
    ...
    dpotf2_kernel<<<dimGrid, threads, shared_mem_size, magma_tally3_stream >>>(m, n, dA, lda, alpha, beta, info);
