/*
    -- MAGMA_tally4 (version 1.6.2) --
       ...
       @generated from magma_tally4_zmscale.cpp normal z -> s, Sun May  3 11:23:01 2015
       @author Hartwig Anzt
    ...
    magma_tally4_queue_t queue )
{
    ...
    if ( A->memory_location == Magma_tally4_CPU && A->storage_type == Magma_tally4_CSRCOO ) {
        if ( scaling == Magma_tally4_NOSCALE ) {
            ...
            CHECK( magma_tally4_smalloc_cpu( &tmp, A->num_rows ));
            for( magma_tally4_int_t z=0; z<A->num_rows; z++ ) {
                float s = MAGMA_tally4_S_MAKE( 0.0, 0.0 );
                for( magma_tally4_int_t f=A->row[z]; f<A->row[z+1]; f++ )
                    s+= MAGMA_tally4_S_REAL(A->val[f])*MAGMA_tally4_S_REAL(A->val[f]);
                tmp[z] = MAGMA_tally4_S_MAKE( 1.0/sqrt(  MAGMA_tally4_S_REAL( s )  ), 0.0 );
            }        printf("inhere1\n");
            ...
            for( magma_tally4_int_t z=0; z<A->num_rows; z++ ) {
                ...
                for( magma_tally4_int_t f=A->row[z]; f<A->row[z+1]; f++ ) {
                    if ( A->col[f]== z ) {
                        ...
                        //A->val[f] = A->val[f] +  MAGMA_tally4_S_MAKE( 100000.0, 0.0 );
                        s = A->val[f];
                    }
                }
                if ( s == MAGMA_tally4_S_MAKE( 0.0, 0.0 ) ){
        ...
        CHECK( magma_tally4_smconvert( hA, &CSRA, hA.storage_type, Magma_tally4_CSRCOO, queue ));

        CHECK( magma_tally4_smscale( &CSRA, scaling, queue ));

        magma_tally4_smfree( &hA, queue );
        magma_tally4_smfree( A, queue );
        CHECK( magma_tally4_smconvert( CSRA, &hA, Magma_tally4_CSRCOO, A_storage, queue ));
    ...
    magma_tally4_queue_t queue )
{
    ...
    if ( A->memory_location == Magma_tally4_CPU && A->storage_type == Magma_tally4_CSRCOO ) {
        ...
        CHECK( magma_tally4_smdiagadd( &CSRA, add, queue ));
