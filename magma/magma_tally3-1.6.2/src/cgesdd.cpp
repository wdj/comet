...
/**
    ...
    @ingroup magma_tally3_cgesvd_driver
    ********************************************************************/
    ...
    magma_tally3_int_t *info)
{
    ...
    *info = 0;
        ...
        if (m >= n) {
            ...
            if (m >= mnthr1) {
                if (wantqn) {
                    ...
                    maxwrk =                n +   n * nb;   //magma_tally3_ilaenv( 1, "CGEQRF", " ",   m, n, -1, -1 );
                    maxwrk = max( maxwrk, 2*n + 2*n * nb ); // cgebrd
                    minwrk = maxwrk;  // lapack was: 3*n
                }
                else if (wantqo) {
                    /* Path 2 (M much larger than N, JOBZ='O') */
                    wrkbl  =               n +   n * nb;   //magma_tally3_ilaenv( 1, "CGEQRF", " ",   m, n, -1, -1 );
                    wrkbl  = max( wrkbl,   n +   n * nb ); //magma_tally3_ilaenv( 1, "CUNGQR", " ",   m, n,  n, -1 ));
                    wrkbl  = max( wrkbl, 2*n + 2*n * nb ); // cgebrd
                    wrkbl  = max( wrkbl, 2*n +   n * nb ); //magma_tally3_ilaenv( 1, "CUNMBR", "QLN", n, n,  n, -1 ));
                    wrkbl  = max( wrkbl, 2*n +   n * nb ); //magma_tally3_ilaenv( 1, "CUNMBR", "PRC", n, n,  n, -1 ));
                    ...
                    wrkbl  = max( wrkbl,   n +   m * nb ); //magma_tally3_ilaenv( 1, "CUNGQR", " ",   m, m,  n, -1 ));
                    maxwrk = n*n + wrkbl;
                }
            }
            else if (m >= mnthr2) {
                ...
                if (wantqo) {
                    maxwrk = max( maxwrk, 2*n + n * nb );  //magma_tally3_ilaenv( 1, "CUNGBR", "P", n, n, n, -1 ));
                    maxwrk = max( maxwrk, 2*n + n * nb );  //magma_tally3_ilaenv( 1, "CUNGBR", "Q", m, n, n, -1 ));
                    ...
                    maxwrk = max( maxwrk, 2*n + n * nb );  //magma_tally3_ilaenv( 1, "CUNGBR", "P", n, n, n, -1 ));
                    maxwrk = max( maxwrk, 2*n + m * nb );  //magma_tally3_ilaenv( 1, "CUNGBR", "Q", m, m, n, -1 ));
                }
            }
            else {
                ...
                minwrk = maxwrk;  // lapack was: 2*n + m
                if (wantqo) {
                    maxwrk = max( maxwrk, 2*n + n*n + n * nb ); //magma_tally3_ilaenv( 1, "CUNMBR", "PRC", n, n, n, -1 ));
                    maxwrk = max( maxwrk, 2*n + m*n + n * nb ); //magma_tally3_ilaenv( 1, "CUNMBR", "QLN", m, n, n, -1 ));
                    // lapack was maxwrk += m*n n*n and m*n put into unmbr MAX above
                    minwrk = max( minwrk, 2*n + n*n );  // lapack was minwrk += n*n
                }
                else if (wantqs) {
                    maxwrk = max( maxwrk, 2*n + n * nb ); //magma_tally3_ilaenv( 1, "CUNMBR", "PRC", n, n, n, -1 ));
                    maxwrk = max( maxwrk, 2*n + n * nb ); //magma_tally3_ilaenv( 1, "CUNMBR", "QLN", m, n, n, -1 ));
                }
                else if (wantqa) {
                    maxwrk = max( maxwrk, 2*n + n * nb ); //magma_tally3_ilaenv( 1, "CUNGBR", "PRC", n, n, n, -1 ));
                    maxwrk = max( maxwrk, 2*n + m * nb ); //magma_tally3_ilaenv( 1, "CUNGBR", "QLN", m, m, n, -1 ));
                }
            }
        }
        else {
            ...
            if (n >= mnthr1) {
                if (wantqn) {
                    ...
                    maxwrk =                m +   m * nb;   //magma_tally3_ilaenv( 1, "CGELQF", " ",   m, n, -1, -1 );
                    maxwrk = max( maxwrk, 2*m + 2*m * nb ); // cgebrd
                    minwrk = maxwrk;  // lapack was: 3*m
                }
                else if (wantqo) {
                    /* Path 2t (N much larger than M, JOBZ='O') */
                    wrkbl  =               m +   m * nb;   //magma_tally3_ilaenv( 1, "CGELQF", " ",   m, n, -1, -1 );
                    wrkbl  = max( wrkbl,   m +   m * nb ); //magma_tally3_ilaenv( 1, "CUNGLQ", " ",   m, n,  m, -1 ));
                    wrkbl  = max( wrkbl, 2*m + 2*m * nb ); // cgebrd
                    wrkbl  = max( wrkbl, 2*m +   m * nb ); //magma_tally3_ilaenv( 1, "CUNMBR", "PRC", m, m,  m, -1 ));
                    wrkbl  = max( wrkbl, 2*m +   m * nb ); //magma_tally3_ilaenv( 1, "CUNMBR", "QLN", m, m,  m, -1 ));
                    ...
                    // include preferred size for cungqr
                    wrkbl  = max( wrkbl,   m +   n * nb ); //magma_tally3_ilaenv( 1, "CUNGLQ", " ",   n, n,  m, -1 ));
                    maxwrk = m*m + wrkbl;
                }
            }
            else if (n >= mnthr2) {
                ...
                if (wantqo) {
                    maxwrk = max( maxwrk, 2*m + m * nb );  //magma_tally3_ilaenv( 1, "CUNGBR", "P", m, n, m, -1 ));
                    maxwrk = max( maxwrk, 2*m + m * nb );  //magma_tally3_ilaenv( 1, "CUNGBR", "Q", m, m, n, -1 ));
                    ...
                    maxwrk = max( maxwrk, 2*m + n * nb ); //magma_tally3_ilaenv( 1, "CUNGBR", "P", n, n, m, -1 ));
                    maxwrk = max( maxwrk, 2*m + m * nb ); //magma_tally3_ilaenv( 1, "CUNGBR", "Q", m, m, n, -1 ));
                }
            }
            else {
                ...
                minwrk = maxwrk;  // lapack was: 2*m + n
                if (wantqo) {
                    maxwrk = max( maxwrk, 2*m + m*m + m * nb ); //magma_tally3_ilaenv( 1, "CUNMBR", "PRC", m, n, m, -1 ));
                    maxwrk = max( maxwrk, 2*m + m*n + m * nb ); //magma_tally3_ilaenv( 1, "CUNMBR", "QLN", m, m, n, -1 ));
                    // lapack was: maxwrk += m*n; m*m and m*n put into unmbr MAX above
                    minwrk = max( minwrk, 2*m + m*m );  // lapack was: minwrk += m*m;
                }
                else if (wantqs) {
                    maxwrk = max( maxwrk, 2*m + m * nb ); //magma_tally3_ilaenv( 1, "CUNMBR", "PRC", m, n, m, -1 ));  // lapack was GBR
                    maxwrk = max( maxwrk, 2*m + m * nb ); //magma_tally3_ilaenv( 1, "CUNMBR", "QLN", m, m, n, -1 ));  // lapack was GBR
                }
                else if (wantqa) {
                    maxwrk = max( maxwrk, 2*m + n * nb ); //magma_tally3_ilaenv( 1, "CUNGBR", "PRC", n, n, m, -1 ));
                    maxwrk = max( maxwrk, 2*m + m * nb ); //magma_tally3_ilaenv( 1, "CUNGBR", "QLN", m, m, n, -1 ));
                }
            }
        }
    ...
    if (*info == 0) {
        work[1] = MAGMA_tally3_C_MAKE( maxwrk, 0 );
    ...
    if (m >= n) {
        ...
        if (m >= mnthr1) {
            if (wantqn) {
                ...
                #else
                magma_tally3_cgebrd(n, n, A(1,1), lda, s, &rwork[ie], &work[itauq], &work[itaup], &work[nwork], lnwork, &ierr);
                #endif
                nrwork = ie + n;
                ...
                #else
                magma_tally3_cgebrd(n, n, &work[ir], ldwrkr, s, &rwork[ie], &work[itauq], &work[itaup], &work[nwork], lnwork, &ierr);
                #endif
                ...
                #else
                magma_tally3_cunmbr(Magma_tally3Q, Magma_tally3Left, Magma_tally3NoTrans, n, n, n, &work[ir], ldwrkr, &work[itauq], &work[iu], ldwrku, &work[nwork], lnwork, &ierr);
                #endif
                ...
                #else
                magma_tally3_cunmbr(Magma_tally3P, Magma_tally3Right, Magma_tally3ConjTrans, n, n, n, &work[ir], ldwrkr, &work[itaup], VT, ldvt, &work[nwork], lnwork, &ierr);
                #endif
                ...
                #else
                magma_tally3_cunmbr(Magma_tally3Q, Magma_tally3Left, Magma_tally3NoTrans, n, n, n, &work[ir], ldwrkr, &work[itauq], U, ldu, &work[nwork], lnwork, &ierr);
                #endif
                ...
                #else
                magma_tally3_cunmbr(Magma_tally3Q, Magma_tally3Left, Magma_tally3NoTrans, n, n, n, A(1,1), lda, &work[itauq], &work[iu], ldwrku, &work[nwork], lnwork, &ierr);
                #endif
                ...
                #else
                magma_tally3_cunmbr(Magma_tally3P, Magma_tally3Right, Magma_tally3ConjTrans, n, n, n, A(1,1), lda, &work[itaup], VT, ldvt, &work[nwork], lnwork, &ierr);
                #endif
            ...
            #else
            magma_tally3_cgebrd(m, n, A(1,1), lda, s, &rwork[ie], &work[itauq], &work[itaup], &work[nwork], lnwork, &ierr);
            #endif
            ...
            if (wantqn) {
                ...
                if (lwork >= m*n + 3*n) {
                    ...
                    #else
                    magma_tally3_cunmbr(Magma_tally3Q, Magma_tally3Left, Magma_tally3NoTrans, m, n, n, A(1,1), lda, &work[itauq], &work[iu], ldwrku, &work[nwork], lnwork, &ierr);
                    #endif
                ...
                #else
                magma_tally3_cunmbr(Magma_tally3Q, Magma_tally3Left, Magma_tally3NoTrans, m, n, n, A(1,1), lda, &work[itauq], U, ldu, &work[nwork], lnwork, &ierr);
                #endif
                ...
                #else
                magma_tally3_cunmbr(Magma_tally3Q, Magma_tally3Left, Magma_tally3NoTrans, m, m, n, A(1,1), lda, &work[itauq], U, ldu, &work[nwork], lnwork, &ierr);
                #endif
        ...
        if (n >= mnthr1) {
            if (wantqn) {
                ...
                #else
                magma_tally3_cgebrd(m, m, A(1,1), lda, s, &rwork[ie], &work[itauq], &work[itaup], &work[nwork], lnwork, &ierr);
                #endif
                nrwork = ie + m;
                ...
                #else
                magma_tally3_cgebrd(m, m, &work[il], ldwrkl, s, &rwork[ie], &work[itauq], &work[itaup], &work[nwork], lnwork, &ierr);
                #endif
                ...
                #else
                magma_tally3_cunmbr(Magma_tally3Q, Magma_tally3Left, Magma_tally3NoTrans, m, m, m, &work[il], ldwrkl, &work[itauq], U, ldu, &work[nwork], lnwork, &ierr);
                #endif
                ...
                #else
                magma_tally3_cunmbr(Magma_tally3P, Magma_tally3Right, Magma_tally3ConjTrans, m, m, m, &work[il], ldwrkl, &work[itaup], &work[ivt], ldwkvt, &work[nwork], lnwork, &ierr);
                #endif
                ...
                #else
                magma_tally3_cunmbr(Magma_tally3P, Magma_tally3Right, Magma_tally3ConjTrans, m, m, m, &work[il], ldwrkl, &work[itaup], VT, ldvt, &work[nwork], lnwork, &ierr);
                #endif
                ...
                #else
                magma_tally3_cunmbr(Magma_tally3Q, Magma_tally3Left, Magma_tally3NoTrans, m, m, m, A(1,1), lda, &work[itauq], U, ldu, &work[nwork], lnwork, &ierr);
                #endif
                ...
                #else
                magma_tally3_cunmbr(Magma_tally3P, Magma_tally3Right, Magma_tally3ConjTrans, m, m, m, A(1,1), lda, &work[itaup], &work[ivt], ldwkvt, &work[nwork], lnwork, &ierr);
                #endif
            ...
            if (wantqn) {
                ...
                if (lwork >= m*n + 3*m) {
                    ...
                    #else
                    magma_tally3_cunmbr(Magma_tally3P, Magma_tally3Right, Magma_tally3ConjTrans, m, n, m, A(1,1), lda, &work[itaup], &work[ivt], ldwkvt, &work[nwork], lnwork, &ierr);
                    #endif
                ...
                #else
                magma_tally3_cunmbr(Magma_tally3P, Magma_tally3Right, Magma_tally3ConjTrans, m, n, m, A(1,1), lda, &work[itaup], VT, ldvt, &work[nwork], lnwork, &ierr);
                #endif
            }
            else {
                /* Path 6ta (N >= M, JOBZ=A) */
                ...
                #else
                magma_tally3_cunmbr(Magma_tally3P, Magma_tally3Right, Magma_tally3ConjTrans, n, n, m, A(1,1), lda, &work[itaup], VT, ldvt, &work[nwork], lnwork, &ierr);
                #endif
            }
        }
    }
