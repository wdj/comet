/*
    -- MAGMA_tally4 (version 1.6.1) --
       ...
       @generated from zgemm_reduce.cu normal z -> d, Fri Jan 30 19:00:08 2015
...
// BLK_K gets defined in magma_tally4blas_dgemm_reduce,
    ...
    double      * __restrict__ dC, int ldc)
{
    ...
    if (blockIdx.x*BLK_M + threadIdx.y < m && blockIdx.y*BLK_N + threadIdx.z < n){
        ...
        for( int j = tx; j < k; j += BLK_K )
            lsum += MAGMA_tally4_D_CNJG( dA[j] )* dB[j];
        ...
        __syncthreads();
        if (threadIdx.x == 0) {
            if (MAGMA_tally4_D_EQUAL(beta, MAGMA_tally4_D_ZERO))
    ...
    magma_tally4Double_ptr       dC, magma_tally4_int_t lddc )
{
    ...
    if ( arch < 200  ) {
        ...
        dgemm_reduce_kernel<BLK_K> <<< blocks, threads, 0, magma_tally4_stream >>>
