/*
    -- MAGMA_minproduct (version 1.6.1) --
       ...
       @generated from zgebrd.cpp normal z -> d, Fri Jan 30 19:00:19 2015
...
/**
    ...
    @ingroup magma_minproduct_dgesvd_comp
    ********************************************************************/
...
#define dA(i, j) (dA + (j)*ldda + (i))
    ...
    nb   = magma_minproduct_get_dgebrd_nb(n);
    ldda = m;
    ...
    *info = 0;
    ...
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc( &dA, n*ldda + (m + n)*nb )) {
    ...
    for (i=0; i < (minmn - nx); i += nb) {
        ...
        if ( i > 0 ) {
            magma_minproduct_dgetmatrix( nrow, nb, dA(i, i), ldda, A( i, i), lda );
            magma_minproduct_dgetmatrix( nb, ncol - nb,
                              dA(i, i+nb), ldda,
                              A( i, i+nb), lda );
        }

        magma_minproduct_dlabrd_gpu(nrow, ncol, nb,
        ...
        magma_minproduct_dsetmatrix( nrow, nb, work  + nb, ldwrkx, dwork + nb, ldwrkx );
        magma_minproduct_dsetmatrix( ncol, nb,
        ...
        if (m >= n) {
            jmax = i + nb;
            for (j = i; j < jmax; ++j) {
                *A(j, j  ) = MAGMA_minproduct_D_MAKE( d[j], 0. );
                *A(j, j+1) = MAGMA_minproduct_D_MAKE( e[j], 0. );
            }
        } else {
            jmax = i + nb;
            for (j = i; j < jmax; ++j) {
    ...
    if ( 0 < minmn - nx ) {
        magma_minproduct_dgetmatrix( nrow, ncol, dA(i, i), ldda, A(i, i), lda );
    }
