...
/**
    Purpose
    -------
    If VECT = Magma_tally4Q, ZUNMBR overwrites the general complex M-by-N matrix C with
                                 SIDE = Magma_tally4Left     SIDE = Magma_tally4Right
    TRANS = Magma_tally4NoTrans:        Q*C                  C*Q
    TRANS = Magma_tally4_ConjTrans:     Q**H*C               C*Q**H
    
    If VECT = Magma_tally4P, ZUNMBR overwrites the general complex M-by-N matrix C with
                                 SIDE = Magma_tally4Left     SIDE = Magma_tally4Right
    TRANS = Magma_tally4NoTrans:        P*C                  C*P
    TRANS = Magma_tally4_ConjTrans:     P**H*C               C*P**H
    ...
    trans   magma_tally4_trans_t
      ...
      -     = Magma_tally4_ConjTrans: Conjugate transpose, apply Q**H or P**H.
    ...
    *info = 0;
    ...
    if (*info == 0) {
        if (nw > 0) {
            ...
            nb = magma_tally4_get_zgebrd_nb( min( m, n ));
            lwkopt = max(1, nw*nb);
        }
        else {
            lwkopt = 1;
    ...
    if (applyq) {
        ...
        if (nq >= k) {
            ...
            #else
            magma_tally4_zunmqr( side, trans,
                          m, n, k, A, lda, tau, C, ldc, work, lwork, &iinfo);
            #endif
        }
        else if (nq > 1) {
        ...
        if (nq > k) {
            ...
            #if VERSION == 1
            lapackf77_zunmlq( lapack_side_const_tally4(side), lapack_trans_const_tally4(transt),
                              &m, &n, &k, A, &lda, tau, C, &ldc, work, &lwork, &iinfo);
            #else
            magma_tally4_zunmlq( side, transt,
                          m, n, k, A, lda, tau, C, ldc, work, lwork, &iinfo);
            #endif
        }
        else if (nq > 1) {
