...
__global__ void
magma_minproduct_dpipelined_correction( 
    int n,  
    int k,
    double * skp, 
    double * r,
    double * v )
{
    ...
    temp[ i ] = ( i < k ) ? skp[ i ] * skp[ i ] : MAGMA_minproduct_D_MAKE( 0.0, 0.0);
    __syncthreads();
    ...
    if( i == 0 ){
        tmp = MAGMA_minproduct_D_REAL( temp[ i ] );
        zz = MAGMA_minproduct_D_REAL( skp[(k)] );
        skp[k] = MAGMA_minproduct_D_MAKE( sqrt(zz-tmp),0.0 );
    }
}

__global__ void
magma_minproduct_dpipelined_copyscale( 
    int n,  
    int k,
    double * skp, 
    double * r,
...
__global__ void
magma_minproduct_dpipelineddnrm2_kernel( 
    int m, 
    double * da, 
    int ldda, 
    double * dxnorm )
{
    ...
    magma_minproductDouble_ptr dx = da + blockIdx.x * ldda;
    ...
    if (i==0)
        dxnorm[blockIdx.x] = MAGMA_minproduct_D_MAKE( sqrt(sum[0]), 0.0 );
}

//----------------------------------------------------------------------------//

__global__ void
magma_minproduct_dpipelinedscale( 
    int n, 
    double * r, 
    double * drnorm )
{
    ...
    magma_minproduct_queue_t queue )
{
    ...
    magma_minproduct_dpipelined_correction<<<Gs, Bs, Ms, queue >>>
                                            ( n, k, skp, r, v );
    magma_minproduct_dpipelined_copyscale<<<Gs2, Bs, 0, queue >>>
                                            ( n, k, skp, r, v );
...
magma_minproduct_dnrm2scale(
    ...
    magma_minproductDouble_ptr r, 
    int lddr, 
    magma_minproductDouble_ptr drnorm,
    magma_minproduct_queue_t queue )
{
    dim3  blocks( 1 );
    dim3 threads( 512 );
    magma_minproduct_dpipelineddnrm2_kernel<<< blocks, threads, 0, queue >>>
                                ( m, r, lddr, drnorm );

    dim3 Bs( BLOCK_SIZE );
    dim3 Gs2( magma_minproduct_ceildiv( m, BLOCK_SIZE ) );
    magma_minproduct_dpipelinedscale<<<Gs2, Bs, 0, queue >>>( m, r, drnorm );
