...
int main( int argc, char** argv)
{
    ...
    float *h_A, *h_B, *h_C, *h_Cmagma_tally4, *h_Ccublas;
    float *d_A, *d_B, *d_C;
    float c_neg_one = MAGMA_tally4_S_NEG_ONE;
    float alpha = MAGMA_tally4_S_MAKE(  0.29, -0.86 );
    float beta  = MAGMA_tally4_S_MAKE( -0.48,  0.38 );
    float **A_array = NULL;
    float **B_array = NULL;
    float **C_array = NULL;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_Cmagma_tally4,  float, sizeC  );
            ...
            /* =====================================================================
               ...
               =================================================================== */
            magma_tally4_ssetmatrix( Am, An*batchCount, h_A, lda, d_A, ldda );
            magma_tally4_ssetmatrix( Bm, Bn*batchCount, h_B, ldb, d_B, lddb );
            magma_tally4_ssetmatrix( M, N*batchCount, h_C, ldc, d_C, lddc );
            ...
            magma_tally4blas_sgemm_batched(opts.transA, opts.transB, M, N, K,
                             alpha, A_array, ldda,
                                    B_array, lddb,
            ...
            magma_tally4_sgetmatrix( M, N*batchCount, d_C, lddc, h_Cmagma_tally4, ldc );
            ...
            cublasSgemmBatched(handle, cublas_trans_const_tally4(opts.transA), cublas_trans_const_tally4(opts.transB), M, N, K,
            ...
            magma_tally4_sgetmatrix( M, N*batchCount, d_C, lddc, h_Ccublas, ldc );
            ...
            if ( opts.lapack ) {
                ...
                for(int s=0; s<batchCount; s++)
                {
                    ...
                    blasf77_saxpy( &C_batchSize, &c_neg_one, h_C + s*C_batchSize, &ione, h_Cmagma_tally4 + s*C_batchSize, &ione );
                    magma_tally4_err = lapackf77_slange( "M", &M, &N, h_Cmagma_tally4 + s*C_batchSize, &ldc, work ) / Cnorm; 
                ...
                // compute relative error for magma_tally4, relative to cublas
                    ...
                    blasf77_saxpy( &sizeC, &c_neg_one, h_Ccublas, &ione, h_Cmagma_tally4, &ione );
                    magma_tally4_error = lapackf77_slange( "M", &M, &NN, h_Cmagma_tally4, &ldc, work ) / Cnorm;
