    ...
    magma_tally3_int_t resetozero, magma_tally3_int_t batchCount, magma_tally3_queue_t queue)
{
    ...
    if(resetozero)
    { 
        magma_tally3blas_claset_batched(Magma_tally3Full, ((n+NB-1)/NB)*NB, NB, MAGMA_tally3_C_ZERO, MAGMA_tally3_C_ZERO, dinvA_array, ((n+NB-1)/NB)*NB, batchCount, queue);
       //magma_tally3blas_cmemset_batched( dinvA_array, ((n+NB-1)/NB)*NB*NB, batchCount, queue);
    }
    ...
    //was not the largest size computed by the high API getrf_batched then it is bug and need to use magma_tally3blas_claset_batched
