    ...
    magma_tally2_queue_t queue )
{
    ...
    CHECK( magma_tally2_svinit( &rt,Magma_tally2_DEV, A.num_rows, b.num_cols, c_zero, queue ));
    ...
    CHECK( magma_tally2_svinit( &h, Magma_tally2_DEV, A.num_rows, b.num_cols, c_zero, queue ));
    ...
    magma_tally2_scopy( dofs, h.dval, 1, p.dval, 1 );                    // p = h
    nom = MAGMA_tally2_S_REAL( magma_tally2_sdot(dofs, r.dval, 1, h.dval, 1) );
    ...
    do
    {
        ...
        gammanew = MAGMA_tally2_S_REAL( magma_tally2_sdot(dofs, r.dval, 1, h.dval, 1) );
                                                            // gn = < r,h>

        if ( solver_par->numiter==1 ) {
            magma_tally2_scopy( dofs, h.dval, 1, p.dval, 1 );                    // p = h
        } else {
            beta = MAGMA_tally2_S_MAKE(gammanew/gammaold, 0.);       // beta = gn/go
            magma_tally2_sscal(dofs, beta, p.dval, 1);            // p = beta*p
            magma_tally2_saxpy(dofs, c_one, h.dval, 1, p.dval, 1); // p = p + h
        }
