...
void get_QR_error(magma_minproduct_int_t M, magma_minproduct_int_t N, magma_minproduct_int_t min_mn,
                    float *h_R,  float *h_A, magma_minproduct_int_t lda,
                    float *tau,
                    float *Q,  magma_minproduct_int_t ldq,
                    float *R,  magma_minproduct_int_t ldr,
                    float *h_work,  magma_minproduct_int_t lwork,
...
int main( int argc, char** argv)
{
    ...
    float *h_A, *h_R, *h_Amagma_minproduct, *tau, *h_work, tmp[1];
    float *d_A, *dtau_magma_minproduct, *dtau_cublas;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_Amagma_minproduct,   float, n2     );
            TESTING_MALLOC_PIN( h_R,   float, n2     );
        
            TESTING_MALLOC_DEV( d_A,   float, ldda*N * batchCount );

            TESTING_MALLOC_DEV( dtau_magma_minproduct,  float, min_mn * batchCount);
			...
			{
				magma_minproduct_smake_hpd( N, h_A + i * lda * N, lda );// need modification
			}
			
			lapackf77_slacpy( Magma_minproductUpperLowerStr, &M, &column, h_A, &lda, h_R, &lda );
       
            /* ====================================================================
               Performs operation using MAGMA_minproduct
               =================================================================== */
            magma_minproduct_ssetmatrix( M, column, h_R, lda,  d_A, ldda );
            sset_pointer(dA_array, d_A, 1, 0, 0, ldda*N, batchCount, opts.queue);
            sset_pointer(dtau_array, dtau_magma_minproduct, 1, 0, 0, min_mn, batchCount, opts.queue);
    
            magma_minproduct_time = magma_minproduct_sync_wtime(0);
    
            info = magma_minproduct_sgeqrf_batched(M, N, dA_array, ldda, dtau_array, dinfo_magma_minproduct, batchCount, opts.queue);
            ...
            magma_minproduct_sgetmatrix( M, column, d_A, ldda, h_Amagma_minproduct, lda);

            if (info != 0)
                printf("magma_minproduct_sgeqrf_batched returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                magma_minproduct_sgetvector(min_mn*batchCount, dtau_magma_minproduct, 1, tau, 1);
                for (int i=0; i < batchCount; i++)
                {
                ...
                #if CUDA_VERSION >= 6050
                magma_minproduct_sgetvector(min_mn*batchCount, dtau_magma_minproduct, 1, tau, 1);
                magma_minproduct_sgetmatrix( M, column, d_A, ldda, h_A, lda);
                for (int i=0; i < batchCount; i++)
                {
                    get_QR_error(M, N, min_mn,
