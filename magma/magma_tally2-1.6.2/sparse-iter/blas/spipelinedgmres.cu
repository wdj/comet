...
__global__ void
magma_tally2_spipelined_correction( 
    int n,  
    int k,
    float * skp, 
    float * r,
    float * v )
{
    ...
    temp[ i ] = ( i < k ) ? skp[ i ] * skp[ i ] : MAGMA_tally2_S_MAKE( 0.0, 0.0);
    __syncthreads();
    ...
    if( i == 0 ){
        tmp = MAGMA_tally2_S_REAL( temp[ i ] );
        zz = MAGMA_tally2_S_REAL( skp[(k)] );
        skp[k] = MAGMA_tally2_S_MAKE( sqrt(zz-tmp),0.0 );
    }
}

__global__ void
magma_tally2_spipelined_copyscale( 
    int n,  
    int k,
    float * skp, 
    float * r,
...
__global__ void
magma_tally2_spipelinedsnrm2_kernel( 
    int m, 
    float * da, 
    int ldda, 
    float * dxnorm )
{
    ...
    magma_tally2Float_ptr dx = da + blockIdx.x * ldda;
    ...
    if (i==0)
        dxnorm[blockIdx.x] = MAGMA_tally2_S_MAKE( sqrt(sum[0]), 0.0 );
}

//----------------------------------------------------------------------------//

__global__ void
magma_tally2_spipelinesscale( 
    int n, 
    float * r, 
    float * drnorm )
{
    ...
    magma_tally2_queue_t queue )
{
    ...
    magma_tally2_spipelined_correction<<<Gs, Bs, Ms, queue >>>
                                            ( n, k, skp, r, v );
    magma_tally2_spipelined_copyscale<<<Gs2, Bs, 0, queue >>>
                                            ( n, k, skp, r, v );
...
magma_tally2_snrm2scale(
    ...
    magma_tally2Float_ptr r, 
    int lddr, 
    magma_tally2Float_ptr drnorm,
    magma_tally2_queue_t queue )
{
    dim3  blocks( 1 );
    dim3 threads( 512 );
    magma_tally2_spipelinedsnrm2_kernel<<< blocks, threads, 0, queue >>>
                                ( m, r, lddr, drnorm );

    dim3 Bs( BLOCK_SIZE );
    dim3 Gs2( magma_tally2_ceildiv( m, BLOCK_SIZE ) );
    magma_tally2_spipelinesscale<<<Gs2, Bs, 0, queue >>>( m, r, drnorm );
