...
float get_LU_error(magma_tally4_int_t M, magma_tally4_int_t N,
                    ...
                    float *LU, magma_tally4_int_t *IPIV)
{
    magma_tally4_int_t min_mn = min(M,N);
    magma_tally4_int_t ione   = 1;
    magma_tally4_int_t i, j;
    float alpha = MAGMA_tally4_S_ONE;
    float beta  = MAGMA_tally4_S_ZERO;
    float *L, *U;
    ...
    lapackf77_slacpy( Magma_tally4LowerStr, &M, &min_mn, LU, &lda, L, &M      );
    lapackf77_slacpy( Magma_tally4UpperStr, &min_mn, &N, LU, &lda, U, &min_mn );

    for(j=0; j<min_mn; j++)
        L[j+j*M] = MAGMA_tally4_S_MAKE( 1., 0. );
    ...
    for( j = 0; j < N; j++ ) {
        for( i = 0; i < M; i++ ) {
            LU[i+j*lda] = MAGMA_tally4_S_SUB( LU[i+j*lda], A[i+j*lda] );
        }
    }
...
int main( int argc, char** argv)
{
    ...
    real_Double_t   gflops, magma_tally4_perf, magma_tally4_time, cublas_perf, cublas_time, cpu_perf=0, cpu_time=0;
    float          error=0.0; 
    magma_tally4_int_t cublas_enable = 0;
    float *h_A, *h_R, *h_Amagma_tally4;
    float *dA;
    float **dA_array = NULL;
    ...
    for( int i = 0; i < opts.ntest; ++i ) {
      ...
      for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU(    h_Amagma_tally4,  float, n2     );
            ...
            TESTING_MALLOC_DEV(  dipiv_cublas,  magma_tally4_int_t, min_mn * batchCount);
            ...
            magma_tally4_ssetmatrix( M, columns, h_R, lda, dA, ldda );
            sset_pointer(dA_array, dA, ldda, 0, 0, ldda*N, batchCount, queue);
            set_ipointer(dipiv_array, dipiv_magma_tally4, 1, 0, 0, min_mn, batchCount, queue);
            
            magma_tally4_time = magma_tally4_sync_wtime(0);
            info = magma_tally4_sgetrf_batched( M, N, dA_array, ldda, dipiv_array,  dinfo_magma_tally4, batchCount, queue);
            magma_tally4_time = magma_tally4_sync_wtime(0) - magma_tally4_time;
            magma_tally4_perf = gflops / magma_tally4_time;
            
            magma_tally4_sgetmatrix( M, N*batchCount, dA, ldda, h_Amagma_tally4, lda );
            ...
            {
                if(cpu_info[i] != 0 ){
                    printf("magma_tally4_sgetrf_batched matrix %d returned internal error %d\n",i, (int)cpu_info[i] );
                }
            }
            
            if (info != 0)
                printf("magma_tally4_sgetrf_batched returned argument error %d: %s.\n", (int) info, magma_tally4_strerror( info ));
            ...
            float err = 0.0;
            if ( opts.check ) {
                magma_tally4_getvector( min_mn * batchCount, sizeof(magma_tally4_int_t), dipiv_magma_tally4, 1, ipiv, 1 );
                int stop=0;
                for(int i=0; i<batchCount; i++)
                {
                    ...
                    error = get_LU_error( M, N, h_R + i * lda*N, lda, h_Amagma_tally4 + i * lda*N, ipiv + i * min_mn);                 
