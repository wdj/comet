...
int main(  int argc, char** argv )
{
    ...
    while( i < argc ) {
        ...
        // init CPU vectors
        CHECK( magma_minproduct_cvinit( &hx, Magma_minproduct_CPU, m, n, c_one, queue ));
        CHECK( magma_minproduct_cvinit( &hy, Magma_minproduct_CPU, m, n, c_zero, queue ));

        // init DEV vectors
        CHECK( magma_minproduct_cvinit( &dx, Magma_minproduct_DEV, m, n, c_one, queue ));
        CHECK( magma_minproduct_cvinit( &dy, Magma_minproduct_DEV, m, n, c_zero, queue ));
        ...
        CHECK( magma_minproduct_cvinit( &dy, Magma_minproduct_DEV, dx.num_rows, dx.num_cols, c_zero, queue ));
        // SpMV on GPU (SELLP)
        start = magma_minproduct_sync_wtime( queue );
        for (j=0; j<10; j++)
