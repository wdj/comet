...
cgeadd_batched_kernel(
    ...
    const magma_minproductFloatComplex * const *dAarray, int ldda,
    magma_minproductFloatComplex              **dBarray, int lddb )
{
    // dA and dB iterate across row i
    const magma_minproductFloatComplex *dA = dAarray[ blockIdx.y ];
    magma_minproductFloatComplex       *dB = dBarray[ blockIdx.y ];
    int i = blockIdx.x*blockDim.x + threadIdx.x;
    if ( i < m ) {
        dA += i;
        dB += i;
        const magma_minproductFloatComplex *dAend = dA + n*ldda;
        while( dA < dAend ) {
            *dB = alpha*(*dA) + (*dB);
            dA += ldda;
            dB += lddb;
...
/**
    @see magma_minproductblas_cgeadd_batched_q
    @ingroup magma_minproduct_caux2
    ********************************************************************/
extern "C" void
