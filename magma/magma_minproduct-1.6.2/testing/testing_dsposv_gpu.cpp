...
int main(int argc, char **argv)
{
    ...
    double *h_A, *h_B, *h_X;
    magma_minproductDouble_ptr d_A, d_B, d_X, d_workd;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_minproduct_dsetmatrix( N, N,    h_A, lda, d_A, lda );
            magma_minproduct_dsetmatrix( N, nrhs, h_B, ldb, d_B, ldb );
            
            //=====================================================================
            //              Mixed Precision Iterative Refinement - GPU
            //=====================================================================
            gpu_time = magma_minproduct_wtime();
            magma_minproduct_dsposv_gpu(opts.uplo, N, nrhs, d_A, lda, d_B, ldb, d_X, ldx,
                             d_workd, d_works, &posv_iter, &info);
            gpu_time = magma_minproduct_wtime() - gpu_time;
            gpu_perf = gflopsS / gpu_time;
            if (info != 0)
                printf("magma_minproduct_dsposv returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
            
            //=====================================================================
            //                 Error Computation
            //=====================================================================
            magma_minproduct_dgetmatrix( N, nrhs, d_X, ldx, h_X, ldx ) ;
            ...
            gpu_time = magma_minproduct_wtime();
            magma_minproduct_dpotrf_gpu(opts.uplo, N, d_A, lda, &info);
            gpu_time = magma_minproduct_wtime() - gpu_time;
            gpu_perfdf = gflopsF / gpu_time;
            if (info != 0)
                printf("magma_minproduct_dpotrf returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
            ...
            gpu_time = magma_minproduct_wtime();
            magma_minproduct_dpotrf_gpu(opts.uplo, N, d_A, lda, &info);
            magma_minproduct_dpotrs_gpu(opts.uplo, N, nrhs, d_A, lda, d_B, ldb, &info);
            gpu_time = magma_minproduct_wtime() - gpu_time;
            gpu_perfds = gflopsS / gpu_time;
            if (info != 0)
                printf("magma_minproduct_dpotrs returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
            ...
            magma_minproduct_dsetmatrix( N, N,    h_A, lda, d_A, lda );
            magma_minproduct_dsetmatrix( N, nrhs, h_B, ldb, d_B, ldb );
            magma_minproductblas_dlag2s( N, N,    d_A, lda, d_As, N, &info );
            magma_minproductblas_dlag2s( N, nrhs, d_B, ldb, d_Bs, N, &info );
            
            gpu_time = magma_minproduct_wtime();
            magma_minproduct_spotrf_gpu(opts.uplo, N, d_As, N, &info);
            gpu_time = magma_minproduct_wtime() - gpu_time;
            gpu_perfsf = gflopsF / gpu_time;
            if (info != 0)
                printf("magma_minproduct_spotrf returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
            ...
            gpu_time = magma_minproduct_wtime();
            magma_minproduct_spotrf_gpu(opts.uplo, N, d_As, lda, &info);
            magma_minproduct_spotrs_gpu(opts.uplo, N, nrhs, d_As, N, d_Bs, N, &info);
            gpu_time = magma_minproduct_wtime() - gpu_time;
            gpu_perfss = gflopsS / gpu_time;
            if (info != 0)
                printf("magma_minproduct_spotrs returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
