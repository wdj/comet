                    ...
                    magma_minproductFloatComplex *dwork)
{
    ...
    const magma_minproductFloatComplex *dV = V + (blockIdx.x) * ldv;
    ...
    for( int j = i; j < m; j += BLOCK_SIZE )
       lsum += MAGMA_minproduct_C_MUL( MAGMA_minproduct_C_CNJG( dV[j] ), c[j] );
...
/*  ----------------------------------------------------------------------------- 
    Call 
        magma_minproduct_cgemv_kernel3<<< n, BLOCK_SIZE>>>(m, V, ldv, c, dwork, tau)
    to compute
...
extern "C"
__global__ void
magma_minproduct_cgemv_kernel3(int m, const magma_minproductFloatComplex * __restrict__ V, int ldv, magma_minproductFloatComplex *c,
                    magma_minproductFloatComplex *dwork, magma_minproductFloatComplex *tau)
{
    const int i = threadIdx.x;
    const magma_minproductFloatComplex *dV = V + (blockIdx.x) * ldv;
...
{
    /* dwork = V**H c     */
    magma_minproduct_cgemv_kernel1<<< k, BLOCK_SIZE, 0, magma_minproduct_stream >>>(m, V, ldv, c, dwork); 

    /* dwork = T**H dwork */
    magma_minproduct_ctrmv_tkernel<<< k, k, 0, magma_minproduct_stream >>>( dT, ldt, dwork, dwork+k);
 
    /* c = c - V dwork    */
    dim3  blocks3( (m + BLOCK_SIZE-1) / BLOCK_SIZE );
    dim3 threads3( BLOCK_SIZE );     
    magma_minproduct_cgemv_kernel2<<< blocks3, threads3, 0, magma_minproduct_stream >>>( m, k, V, ldv, dwork+k, c);
}
