/*
    -- MAGMA_minproduct (version 1.6.1) --
       ...
       @generated from zgeqrf_ooc.cpp normal z -> d, Fri Jan 30 19:00:16 2015
...
/**
    ...
    This is an out-of-core (ooc) version that is similar to magma_minproduct_dgeqrf but
    ...
    magma_minproduct_int_t *info )
{
    ...
    *info = 0;
    int nb = magma_minproduct_get_dgeqrf_nb(min(m, n));
    ...
    if (NB >= n)
        return magma_minproduct_dgeqrf(m, n, A, lda, tau, work, lwork, info);
    ...
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc( &dA, (NB + nb)*ldda + nb*lddwork )) {
    ...
    for (int i=0; i < n; i += NB) {
        ...
        magma_minproduct_dsetmatrix_async( (m), IB,
                                A(0,i),  lda,
        ...
        for (int j=0; j < min(i,k); j += nb) {
            ...
            dpanel_to_q(Magma_minproductUpper, ib, A(j,j), lda, work+ib*ib);
            ...
            dq_to_panel(Magma_minproductUpper, ib, A(j,j), lda, work+ib*ib);
        }

        /* 3. Do a QR on the current part */
        if (i < k)
            magma_minproduct_dgeqrf2_gpu(m-i, IB, dA(i,0), ldda, tau+i, info);

        /* 4. Copy the current part back to the CPU */
        magma_minproduct_dgetmatrix_async( (m), IB,
                                dA(0,0), ldda,
                                A(0,i),  lda, stream[0] );
    }
