...
void init_matrix( int nopiv, int m, int n, magma_tally2DoubleComplex *h_A, magma_tally2_int_t lda )
{
    assert( m == n );
...
double get_residual(
    int nopiv, magma_tally2_uplo_t uplo, magma_tally2_int_t n,
    ...
    magma_tally2_int_t *ipiv )
{
    ...
    magma_tally2_int_t ISEED[4] = {0,0,0,2};
    magma_tally2_int_t info = 0;
    magma_tally2DoubleComplex *x, *b;
    
    // initialize RHS
    TESTING_MALLOC_CPU( x, magma_tally2DoubleComplex, n );
    TESTING_MALLOC_CPU( b, magma_tally2DoubleComplex, n );
    ...
    if (nopiv) {
        if (upper) {
            blasf77_ztrsm( Magma_tally2LeftStr, Magma_tally2UpperStr, 
                           Magma_tally2ConjTransStr, Magma_tally2UnitStr, 
                           &n, &ione, &c_one,
                           A, &lda, x, &n );
            for (int i=0; i<n; i++) x[i] = MAGMA_tally2_Z_DIV( x[i], A[i+i*lda] );
            blasf77_ztrsm( Magma_tally2LeftStr, Magma_tally2UpperStr, 
                           Magma_tally2NoTransStr, Magma_tally2UnitStr, 
                           &n, &ione, &c_one,
                           A, &lda, x, &n );
        } else {
            blasf77_ztrsm( Magma_tally2LeftStr, Magma_tally2LowerStr, 
                           Magma_tally2NoTransStr, Magma_tally2UnitStr, 
                           &n, &ione, &c_one,
                           A, &lda, x, &n );
            for (int i=0; i<n; i++) x[i] = MAGMA_tally2_Z_DIV( x[i], A[i+i*lda] );
        ...
        lapackf77_zhetrs( lapack_uplo_const_tally2(uplo), &n, &ione, A, &lda, ipiv, x, &n, &info );
    }
    if (info != 0)
    ...
    norm_A = lapackf77_zlange( Magma_tally2FullStr, &n, &n, A, &lda, work );
    norm_r = lapackf77_zlange( Magma_tally2FullStr, &n, &ione, b, &n, work );
    norm_x = lapackf77_zlange( Magma_tally2FullStr, &n, &ione, x, &n, work );
    
    //printf( "r=\n" ); magma_tally2_zprint( 1, n, b, 1 );
...
double get_LDLt_error(int nopiv, magma_tally2_uplo_t uplo, magma_tally2_int_t N,
                      magma_tally2DoubleComplex *LD, magma_tally2_int_t lda,
                      magma_tally2_int_t *ipiv)
{
    ...
    magma_tally2DoubleComplex *A, *L, *D;
    ...
    TESTING_MALLOC_CPU( A, magma_tally2DoubleComplex, N*N );
    TESTING_MALLOC_CPU( L, magma_tally2DoubleComplex, N*N );
    TESTING_MALLOC_CPU( D, magma_tally2DoubleComplex, N*N );
    memset( L, 0, N*N*sizeof(magma_tally2DoubleComplex) );
    memset( D, 0, N*N*sizeof(magma_tally2DoubleComplex) );
    ...
    if (uplo == Magma_tally2Upper) {
        for (j=N-1; j>=0; j--) {
            ...
            if (piv < 0) {
                ...
                D(j,j)     = LD(j,j);
                D(j,j-1)   = MAGMA_tally2_Z_CNJG(LD(j-1,j));
                D(j-1,j)   = LD(j-1,j);
                ...
                if (piv != j) {
                    ...
                    for (i=j+2; i<N; i++) {
                        magma_tally2DoubleComplex val = L(j,i);
                        L(j,i) = L(piv,i);
                        L(piv,i) = val;
                    }
                    // apply row-pivoting to A
                    for (i=0; i<N; i++) {
                        magma_tally2DoubleComplex val = A(j,i);
                        A(j,i) = A(piv,i);
                        A(piv,i) = val;
                    }
                    // apply col-pivoting to A
                    for (i=0; i<N; i++) {
                        magma_tally2DoubleComplex val = A(i,j);
                        A(i,j) = A(i,piv);
                        A(i,piv) = val;
                    }
                }
        ...
        if (nopiv) {
            // compute W = D*U
            blasf77_zgemm(Magma_tally2NoTransStr, Magma_tally2NoTransStr, &N, &N, &N,
                          &c_one, D, &N, L, &N, &c_zero, LD, &lda);
            // compute D = U'*W
            blasf77_zgemm(Magma_tally2ConjTransStr, Magma_tally2NoTransStr, &N, &N, &N,
            ...
            // compute D = W*U'
            blasf77_zgemm(Magma_tally2NoTransStr, Magma_tally2ConjTransStr, &N, &N, &N,
    ...
    // compute norm of A
    matnorm = lapackf77_zlange(Magma_tally2FullStr, &N, &N, A, &lda, work);

    for( j = 0; j < N; j++ ) {
        for( i = 0; i < N; i++ ) {
            D(i,j) = MAGMA_tally2_Z_SUB( D(i,j), A(i,j) );
        }
    }
    residual = lapackf77_zlange(Magma_tally2FullStr, &N, &N, D, &N, work);
...
int main( int argc, char** argv)
{
    ...
    magma_tally2DoubleComplex *h_A, *work, temp;
    real_Double_t   gflops, gpu_perf, gpu_time = 0.0, cpu_perf=0, cpu_time=0;
    double          error, error_lapack = 0.0;
    magma_tally2_int_t     *ipiv;
    magma_tally2_int_t     N, n2, lda, lwork, info;
    magma_tally2_int_t     status = 0;
    magma_tally2_int_t     cpu = 0, nopiv = 0, nopiv_gpu = 0, row = 0;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_PIN( h_A,  magma_tally2DoubleComplex, n2 );
            ...
            if ( opts.lapack ) {
                lwork = -1;
                lapackf77_zhetrf( lapack_uplo_const_tally2(opts.uplo), &N, h_A, &lda, ipiv, &temp, &lwork, &info);
                lwork = (int)MAGMA_tally2_Z_REAL(temp);
                TESTING_MALLOC_CPU( work, magma_tally2DoubleComplex, lwork );

                init_matrix( nopiv, N, N, h_A, lda );
                cpu_time = magma_tally2_wtime();
                lapackf77_zhetrf( lapack_uplo_const_tally2(opts.uplo), &N, h_A, &lda, ipiv, work, &lwork, &info);
            ...
            if (nopiv) {
                ...
                magma_tally2_zhetrf_nopiv_tally2( opts.uplo, N, h_A, lda, &info);
                gpu_time = magma_tally2_wtime() - gpu_time;
            } else if (cpu) {
                // CPU-interface to Bunch-Kauffman LDLt
                magma_tally2_setdevice(0);
                gpu_time = magma_tally2_wtime();
                magma_tally2_zhetrf( opts.uplo, N, h_A, lda, ipiv, &info);
                gpu_time = magma_tally2_wtime() - gpu_time;
            } else if (nopiv_gpu) {
                ...
                if (MAGMA_tally2_SUCCESS != magma_tally2_zmalloc( &d_A, N*ldda  )) {
                    printf( " failed to allocate d_A(%dx%d)\n", (int) N, (int) ldda );
                    return 0;
                }
                magma_tally2_zsetmatrix(N, N, h_A, lda, d_A, ldda);
                gpu_time = magma_tally2_wtime();
                magma_tally2_zhetrf_nopiv_tally2_gpu( opts.uplo, N, d_A, ldda, &info);
                gpu_time = magma_tally2_wtime() - gpu_time;
                magma_tally2_zgetmatrix(N, N, d_A, ldda, h_A, lda);
                magma_tally2_free( d_A );
            } else if (row) {
                magma_tally2_setdevice(0);
                //magma_tally2_zhetrf_gpu_row( opts.uplo, N, h_A, lda, ipiv, work, lwork, &info);
            } else {
                magma_tally2_setdevice(0);
                //magma_tally2_zhetrf_hybrid( opts.uplo, N, h_A, lda, ipiv, work, lwork, &info);
            }
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally2_zhetrf returned error %d: %s.\n",
                       (int) info, magma_tally2_strerror( info ));
