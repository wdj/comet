...
__global__ void 
magma_tally3_diteric_csr_kernel(    
    ...
    {     
        ...
        // modify entry
        if (i == j)
            val[il-1] = MAGMA_tally3_D_MAKE(sqrt(abs(MAGMA_tally3_D_REAL(s))), 0.0);
        else
            val[il-1] =  s / val[iu-1];
...
magma_tally3_diteric_csr( 
    ...
    magma_tally3_diteric_csr_kernel<<< grid, block, 0, magma_tally3_stream >>>
            ( A.num_rows, A.nnz, 
