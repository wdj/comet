...
// BLK_K gets defined in magma_tally4blas_zgemm_reduce,
...
void zgemm_reduce_kernel(
    ...
    const magma_tally4DoubleComplex* __restrict__ dA, int lda,
    const magma_tally4DoubleComplex* __restrict__ dB, int ldb,
    magma_tally4DoubleComplex beta,
    magma_tally4DoubleComplex      * __restrict__ dC, int ldc)
{
    ...
    if (blockIdx.x*BLK_M + threadIdx.y < m && blockIdx.y*BLK_N + threadIdx.z < n){
        ...
        // moved 3rd dimension to 1st dimension to make magma_tally4_sum_reduce_3d interface nicer.
        __shared__ magma_tally4DoubleComplex sum[BLK_K][BLK_M+1][BLK_N+1];
        magma_tally4DoubleComplex lsum;
        
        /*  w := v**H * C  */
        lsum = MAGMA_tally4_Z_ZERO;
        for( int j = tx; j < k; j += BLK_K )
            lsum += MAGMA_tally4_Z_CNJG( dA[j] )* dB[j];
        
        sum[tx][threadIdx.y][threadIdx.z] = lsum;
        magma_tally4_sum_reduce_3d< BLK_K, BLK_M+1, BLK_N+1 >( tx, threadIdx.y, threadIdx.z, sum );
        
        /*  C := C - v * w  */
        __syncthreads();
        if (threadIdx.x == 0) {
            if (MAGMA_tally4_Z_EQUAL(beta, MAGMA_tally4_Z_ZERO))
    ...
    magma_tally4DoubleComplex_ptr       dC, magma_tally4_int_t lddc )
{
    ...
    if ( arch < 200  ) {
        ...
        zgemm_reduce_kernel<BLK_K> <<< blocks, threads, 0, magma_tally4_stream >>>
