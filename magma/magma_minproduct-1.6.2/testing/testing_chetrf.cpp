...
void init_matrix( int nopiv, int m, int n, magma_minproductFloatComplex *h_A, magma_minproduct_int_t lda )
{
    assert( m == n );
    ...
    if (nopiv) {
        magma_minproduct_cmake_hpd( n, h_A, lda );
    }
    else {
    ...
    magma_minproduct_int_t *ipiv )
{
    ...
    if (nopiv) {
        if (upper) {
            blasf77_ctrsm( Magma_minproductLeftStr, Magma_minproductUpperStr, 
                           Magma_minproductConjTransStr, Magma_minproductUnitStr, 
                           &n, &ione, &c_one,
                           A, &lda, x, &n );
            for (int i=0; i<n; i++) x[i] = MAGMA_minproduct_C_DIV( x[i], A[i+i*lda] );
            blasf77_ctrsm( Magma_minproductLeftStr, Magma_minproductUpperStr, 
                           Magma_minproductNoTransStr, Magma_minproductUnitStr, 
                           &n, &ione, &c_one,
                           A, &lda, x, &n );
        } else {
            blasf77_ctrsm( Magma_minproductLeftStr, Magma_minproductLowerStr, 
                           Magma_minproductNoTransStr, Magma_minproductUnitStr, 
                           &n, &ione, &c_one,
                           A, &lda, x, &n );
            for (int i=0; i<n; i++) x[i] = MAGMA_minproduct_C_DIV( x[i], A[i+i*lda] );
...
float get_LDLt_error(int nopiv, magma_minproduct_uplo_t uplo, magma_minproduct_int_t N,
                      magma_minproductFloatComplex *LD, magma_minproduct_int_t lda,
                      magma_minproduct_int_t *ipiv)
{
    ...
    magma_minproductFloatComplex *A, *L, *D;
    ...
    TESTING_MALLOC_CPU( A, magma_minproductFloatComplex, N*N );
    TESTING_MALLOC_CPU( L, magma_minproductFloatComplex, N*N );
    TESTING_MALLOC_CPU( D, magma_minproductFloatComplex, N*N );
    memset( L, 0, N*N*sizeof(magma_minproductFloatComplex) );
    memset( D, 0, N*N*sizeof(magma_minproductFloatComplex) );
    ...
    if (uplo == Magma_minproductUpper) {
        for (j=N-1; j>=0; j--) {
            ...
            if (piv < 0) {
                ...
                D(j,j)     = LD(j,j);
                D(j,j-1)   = MAGMA_minproduct_C_CNJG(LD(j-1,j));
                D(j-1,j)   = LD(j-1,j);
                ...
                if (piv != j) {
                    ...
                    for (i=j+2; i<N; i++) {
                        magma_minproductFloatComplex val = L(j,i);
                        L(j,i) = L(piv,i);
                        L(piv,i) = val;
                    }
                    // apply row-pivoting to A
                    for (i=0; i<N; i++) {
                        magma_minproductFloatComplex val = A(j,i);
                        A(j,i) = A(piv,i);
                        A(piv,i) = val;
                    }
                    // apply col-pivoting to A
                    for (i=0; i<N; i++) {
                        magma_minproductFloatComplex val = A(i,j);
                        A(i,j) = A(i,piv);
                        A(i,piv) = val;
                    }
                }
        ...
        if (nopiv) {
            // compute W = D*U
            blasf77_cgemm(Magma_minproductNoTransStr, Magma_minproductNoTransStr, &N, &N, &N,
                          &c_one, D, &N, L, &N, &c_zero, LD, &lda);
            // compute D = U'*W
            blasf77_cgemm(Magma_minproductConjTransStr, Magma_minproductNoTransStr, &N, &N, &N,
            ...
            // compute D = W*U'
            blasf77_cgemm(Magma_minproductNoTransStr, Magma_minproductConjTransStr, &N, &N, &N,
    ...
    // compute norm of A
    matnorm = lapackf77_clange(Magma_minproductFullStr, &N, &N, A, &lda, work);

    for( j = 0; j < N; j++ ) {
        for( i = 0; i < N; i++ ) {
            D(i,j) = MAGMA_minproduct_C_SUB( D(i,j), A(i,j) );
        }
    }
    residual = lapackf77_clange(Magma_minproductFullStr, &N, &N, D, &N, work);
...
int main( int argc, char** argv)
{
    ...
    magma_minproductFloatComplex *h_A, *work, temp;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if (nopiv) {
                ...
                magma_minproduct_chetrf_nopiv( opts.uplo, N, h_A, lda, &info);
                gpu_time = magma_minproduct_wtime() - gpu_time;
            } else if (cpu) {
                // CPU-interface to Bunch-Kauffman LDLt
                magma_minproduct_setdevice(0);
                gpu_time = magma_minproduct_wtime();
                magma_minproduct_chetrf( opts.uplo, N, h_A, lda, ipiv, &info);
                gpu_time = magma_minproduct_wtime() - gpu_time;
            } else if (nopiv_gpu) {
                ...
                if (MAGMA_minproduct_SUCCESS != magma_minproduct_cmalloc( &d_A, N*ldda  )) {
                    printf( " failed to allocate d_A(%dx%d)\n", (int) N, (int) ldda );
                    return 0;
                }
                magma_minproduct_csetmatrix(N, N, h_A, lda, d_A, ldda);
                gpu_time = magma_minproduct_wtime();
                magma_minproduct_chetrf_nopiv_gpu( opts.uplo, N, d_A, ldda, &info);
                gpu_time = magma_minproduct_wtime() - gpu_time;
                magma_minproduct_cgetmatrix(N, N, d_A, ldda, h_A, lda);
                magma_minproduct_free( d_A );
            } else if (row) {
                magma_minproduct_setdevice(0);
                //magma_minproduct_chetrf_gpu_row( opts.uplo, N, h_A, lda, ipiv, work, lwork, &info);
            } else {
                magma_minproduct_setdevice(0);
                //magma_minproduct_chetrf_hybrid( opts.uplo, N, h_A, lda, ipiv, work, lwork, &info);
            }
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_minproduct_chetrf returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
