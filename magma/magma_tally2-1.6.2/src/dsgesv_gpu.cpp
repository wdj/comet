    ...
    magma_tally2_int_t *info)
{
    ...
    *info = 0;
     ...
     */
    //magma_tally2blas_dlag2s( n, nrhs, dB, lddb, dSX, lddsx, info );  // done inside dsgetrs with pivots
    if (*info != 0) {
        *iter = -2;
        goto FALLBACK;
    }
    
    magma_tally2blas_dlag2s( n, n, dA, ldda, dSA, lddsa, info );
    if (*info != 0) {
        *iter = -2;
        goto FALLBACK;
    }
    ...
    magma_tally2_sgetrf_gpu( n, n, dSA, lddsa, ipiv, info );
    if (*info != 0) {
        *iter = -3;
        goto FALLBACK;
    }
    ...
    {
        magma_tally2_int_t *newipiv;
        magma_tally2_imalloc_cpu( &newipiv, n );
        if ( newipiv == NULL ) {
            *iter = -3;
            goto FALLBACK;
        }
        swp2pswp_tally2( trans, n, ipiv, newipiv );
        magma_tally2_setvector( n, sizeof(magma_tally2_int_t), newipiv, 1, dipiv, 1 );
        magma_tally2_free_cpu( newipiv );
    }
    
    // solve dSA*dSX = dB in single precision
    // converts dB to dSX and applies pivots, solves, then converts result back to dX
    magma_tally2_dsgetrs_gpu( trans, n, nrhs, dSA, lddsa, dipiv, dB, lddb, dX, lddx, dSX, info );
...
REFINEMENT:
    for( iiter=1; iiter < ITERMAX; ) {
        *info = 0;
        ...
        magma_tally2_dsgetrs_gpu( trans, n, nrhs, dSA, lddsa, dipiv, dR, lddr, dR, lddr, dSX, info );
        if (*info != 0) {
            *iter = -3;
            goto FALLBACK;
        }
        ...
        for( j=0; j < nrhs; j++ ) {
            magma_tally2blas_daxpycp( n, dR(0,j), dX(0,j), dB(0,j) );
        }
...
FALLBACK:
    ...
    magma_tally2_dgetrf_gpu( n, n, dA, ldda, ipiv, info );
    if (*info == 0) {
        magma_tally2blas_dlacpy( Magma_tally2UpperLower, n, nrhs, dB, lddb, dX, lddx );
        magma_tally2_dgetrs_gpu( trans, n, nrhs, dA, ldda, ipiv, dX, lddx, info );
    }
