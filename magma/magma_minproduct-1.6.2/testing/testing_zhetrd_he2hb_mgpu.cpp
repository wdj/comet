...
int main( int argc, char** argv)
{
    ...
    double *D, *E;
    magma_minproduct_int_t N, n2, lda, ldda, lwork, ldt, info, nstream;
    ...
    magma_minproduct_int_t distblk = 0;

    magma_minproduct_opts opts;
    parse_opts( argc, argv, &opts );
    
    magma_minproduct_int_t WANTZ = (opts.jobz == Magma_minproductVec);
    double tol = opts.tolerance * lapackf77_dlamch("E");
    if (opts.nb == 0)
        opts.nb = 64; //magma_minproduct_get_zhetrd_he2hb_nb(N);

    if (NE < 1)
        NE = N; //64; //magma_minproduct_get_zhetrd_he2hb_nb(N);

    nstream = max(3, opts.ngpu+2);
    magma_minproduct_queue_t streams[Magma_minproductMaxGPUs][20];
    magma_minproductDoubleComplex_ptr da[Magma_minproductMaxGPUs], dT1[Magma_minproductMaxGPUs];
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_PIN( h_A,    magma_minproductDoubleComplex, lda*N );
            ...
            for( magma_minproduct_int_t dev = 0; dev < opts.ngpu; ++dev ) {
                magma_minproduct_int_t mlocal = ((N / distblk) / opts.ngpu + 1) * distblk;
                magma_minproduct_setdevice( dev );
                TESTING_MALLOC_DEV( da[dev],  magma_minproductDoubleComplex, ldda*mlocal );
                TESTING_MALLOC_DEV( dT1[dev], magma_minproductDoubleComplex, N*opts.nb        );
            }
            ...
            magma_minproduct_zsetmatrix_1D_col_bcyclic( N, N, h_R, lda, da, ldda, opts.ngpu, distblk);
            //magma_minproductDoubleComplex_ptr dabis;
            //TESTING_MALLOC_DEV( dabis,  magma_minproductDoubleComplex, ldda*N );
            //magma_minproduct_zsetmatrix(N, N, h_R, lda, dabis, ldda);
            ...
            for (int count=0; count < 1; ++count) {
                ...
                // magma_minproduct_zhetrd_he2hb(opts.uplo, N, opts.nb, h_R, lda, tau, h_work, lwork, dT1[0], &info);
            ...
            // magma_minproduct_zhetrd_bhe2trc_v5(opts.nthread, WANTZ, opts.uplo, NE, N, opts.nb, h_R, lda, D, E, dT1[0], ldt);
            // magma_minproduct_zhetrd_bhe2trc(opts.nthread, WANTZ, opts.uplo, NE, N, opts.nb, h_R, lda, D, E, dT1[0], ldt);
            ...
            if (info != 0)
                printf("magma_minproduct_zhetrd_he2hb returned error %d: %s.\n",
