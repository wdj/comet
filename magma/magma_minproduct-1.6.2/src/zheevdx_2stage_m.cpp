...
/**
    ...
    @ingroup magma_minproduct_zheev_driver
    ********************************************************************/
    ...
    magma_minproduct_int_t *info)
{
    ...
    magma_minproduct_int_t lwmin, lrwmin, liwmin;
    ...
    *info = 0;
    ...
    magma_minproduct_int_t nb = magma_minproduct_get_zbulge_nb(n, parallel_threads);
    magma_minproduct_int_t Vblksiz = magma_minproduct_zbulge_get_Vblksiz(n, nb, parallel_threads);

    magma_minproduct_int_t ldt = Vblksiz;
    magma_minproduct_int_t ldv = nb + Vblksiz;
    magma_minproduct_int_t blkcnt = magma_minproduct_bulge_get_blkcnt(n, nb, Vblksiz);
    magma_minproduct_int_t lq2 = magma_minproduct_zbulge_get_lq2(n, parallel_threads);
    ...
    if (n == 1) {
        w[0] = MAGMA_minproduct_Z_REAL(A[0]);
        if (wantz) {
            A[0] = MAGMA_minproduct_Z_ONE;
        }
        return *info;
    }
    ...
    magma_minproduct_int_t ntiles = n/nb;
    if ( ( ntiles < 2 ) || ( n <= 128 ) ) {
        #ifdef ENABLE_DEBUG
        printf("--------------------------------------------------------------\n");
    ...
    magma_minproduct_int_t indT2   = 0;
    magma_minproduct_int_t indTAU2 = indT2  + blkcnt*ldt*Vblksiz;
    magma_minproduct_int_t indV2   = indTAU2+ blkcnt*Vblksiz;
    magma_minproduct_int_t indtau1 = indV2  + blkcnt*ldv*Vblksiz;
    magma_minproduct_int_t indwrk  = indtau1+ n;
    magma_minproduct_int_t indwk2  = indwrk + n*n;
    magma_minproduct_int_t llwork = lwork - indwrk;
    magma_minproduct_int_t llwrk2 = lwork - indwk2;
    magma_minproduct_int_t inde = 0;
    magma_minproduct_int_t indrwk = inde + n;
    magma_minproduct_int_t llrwk = lrwork - indrwk;

    magma_minproduct_timer_t time=0, time_total=0, time_alloc=0, time_dist=0, time_band=0;
    timer_start( time_total );

#ifdef HE2HB_SINGLEGPU
    magma_minproductDoubleComplex *dT1;
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc( &dT1, n*nb)) {
        *info = MAGMA_minproduct_ERR_DEVICE_ALLOC;
        return *info;
    }
    timer_start( time_band );
    magma_minproduct_zhetrd_he2hb(uplo, n, nb, A, lda, &work[indtau1], &work[indwrk], llwork, dT1, info);
    timer_stop( time_band );
    timer_printf( "    1 GPU seq code time zhetrd_he2hb only = %7.4f\n", time_band );
    magma_minproduct_free(dT1);
#else
    magma_minproduct_int_t nstream = max(3,ngpu+2);
    magma_minproduct_queue_t streams[Magma_minproductMaxGPUs][20];
    magma_minproductDoubleComplex *da[Magma_minproductMaxGPUs], *dT1[Magma_minproductMaxGPUs];
    magma_minproduct_int_t ldda = ((n+31)/32)*32;

    magma_minproduct_int_t ver = 0;
    magma_minproduct_int_t distblk = max(256, 4*nb);
    ...
    for( magma_minproduct_int_t dev = 0; dev < ngpu; ++dev ) {
        magma_minproduct_int_t mlocal = ((n / distblk) / ngpu + 1) * distblk;
        magma_minproduct_setdevice( dev );
        // TODO check malloc
        magma_minproduct_zmalloc(&da[dev], ldda*mlocal );
        magma_minproduct_zmalloc(&dT1[dev], (n*nb) );
    ...
    magma_minproduct_zsetmatrix_1D_col_bcyclic( n, n, A, lda, da, ldda, ngpu, distblk );
    magma_minproduct_setdevice(0);
    timer_stop( time_dist );

    timer_start( time_band );
    if (ver == 30) {
        magma_minproduct_zhetrd_he2hb_mgpu_spec(uplo, n, nb, A, lda, &work[indtau1], &work[indwrk], llwork, da, ldda, dT1, nb, ngpu, distblk, streams, nstream, info);
    } else {
        magma_minproduct_zhetrd_he2hb_mgpu(uplo, n, nb, A, lda, &work[indtau1], &work[indwrk], llwork, da, ldda, dT1, nb, ngpu, distblk, streams, nstream, info);
    }
    timer_stop( time_band );
    ...
    for( magma_minproduct_int_t dev = 0; dev < ngpu; ++dev ) {
        magma_minproduct_setdevice( dev );
        magma_minproduct_free( da[dev] );
        magma_minproduct_free( dT1[dev] );
        for( int i = 0; i < nstream; ++i ) {
    ...
    magma_minproduct_int_t lda2 = 2*nb; //nb+1+(nb-1);
    magma_minproductDoubleComplex* A2 = &work[indwrk];
    memset(A2, 0, n*lda2*sizeof(magma_minproductDoubleComplex));

    for (magma_minproduct_int_t j = 0; j < n-nb; j++) {
        len = nb+1;
        blasf77_zcopy( &len, A(j,j), &ione, A2(0,j), &ione );
        memset(A(j,j), 0, (nb+1)*sizeof(magma_minproductDoubleComplex));
        *A(nb+j,j) = c_one;
    }
    for (magma_minproduct_int_t j = 0; j < nb; j++) {
        len = nb-j;
        blasf77_zcopy( &len, A(j+n-nb,j+n-nb), &ione, A2(0,j+n-nb), &ione );
        memset(A(j+n-nb,j+n-nb), 0, (nb-j)*sizeof(magma_minproductDoubleComplex));
    }
    ...
    magma_minproduct_zhetrd_hb2st(uplo, n, nb, Vblksiz, A2, lda2, w, &rwork[inde], &work[indV2], ldv, &work[indTAU2], wantz, &work[indT2], ldt);
    ...
    if (! wantz) {
        ...
        magma_minproduct_dmove_eig(range, n, w, &il, &iu, vl, vu, m);
        ...
        magma_minproduct_zstedx_m(ngpu, range, n, vl, vu, il, iu, w, &rwork[inde],
        ...
        magma_minproduct_dmove_eig(range, n, w, &il, &iu, vl, vu, m);
/*
        magma_minproductDoubleComplex *dZ;
        magma_minproduct_int_t lddz = n;

        if (MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc( &dZ, *m*lddz)) {
            *info = MAGMA_minproduct_ERR_DEVICE_ALLOC;
            return *info;
        }

        magma_minproduct_zbulge_back(uplo, n, nb, *m, Vblksiz, &work[indwrk + n * (il-1)], n, dZ, lddz,
                          &work[indV2], ldv, &work[indTAU2], &work[indT2], ldt, info);

        magma_minproduct_zgetmatrix( n, *m, dZ, lddz, &work[indwrk], n);

        magma_minproduct_free(dZ);

*/

        magma_minproduct_zbulge_back_m(ngpu, uplo, n, nb, *m, Vblksiz, &work[indwrk + n * (il-1)], n,
                            &work[indV2], ldv, &work[indTAU2], &work[indT2], ldt, info);
        ...
        magma_minproduct_zunmqr_m(ngpu, Magma_minproductLeft, Magma_minproductNoTrans, n-nb, *m, n-nb, A+nb, lda, &work[indtau1],
