...
void getro (char *trans, const magma_tally3_int_t m, const magma_tally3_int_t n, 
        const cuDoubleComplex *A, const magma_tally3_int_t LDA, 
        cuDoubleComplex *B, const magma_tally3_int_t LDB) 
{
...
void SCHED_zlarfb(Quark* quark)
{
  ...
  *W = (cuDoubleComplex*) malloc(LDW*MM*sizeof(cuDoubleComplex));
  ...
  getro(Magma_tally3ConjTransStr, MM, NN, C, LDC, *W, LDW);

  cuDoubleComplex c_one = MAGMA_tally3_Z_ONE;

  blasf77_ztrmm("r","l","n","u",
    &NN, &MM, &c_one, V, &LDV, *W, &LDW);

  magma_tally3_int_t K=M-MM;

  blasf77_zgemm(Magma_tally3ConjTransStr, "n", &NN, &MM, &K,
...
void SCHED_zgeqrt(Quark* quark)
{
  ...
  lwork=(magma_tally3_int_t)MAGMA_tally3_Z_REAL(WORK[0]);
  lapackf77_zgeqrf(&M, &N, A, &LDA, TAU, WORK, &lwork, &iinfo);
...
void SCHED_ztrmm(Quark *quark)
{
  ...
  getro(Magma_tally3ConjTransStr, n, m, *b, ldb, work, m);
  ...
  magma_tally3_int_t dkdk)
{
  ...
  QUARK_Insert_Task(quark, SCHED_zgemm, task_flags,
    ...
    sizeof(magma_tally3_int_t),           &priority, VALUE | TASK_PRIORITY,
    sizeof(magma_tally3_int_t),&dkdk,VALUE,
  ...
  char *dag_label)
{
  ...
  magma_tally3_int_t priority = 1000;

  QUARK_Insert_Task(quark, SCHED_zgeqrt, task_flags,
    sizeof(magma_tally3_int_t),           &m,        VALUE,
    sizeof(magma_tally3_int_t),           &n,        VALUE,
    sizeof(magma_tally3_int_t),           &ldt,      VALUE,
...

{
  ...
  QUARK_Insert_Task(quark, SCHED_zlarfb, task_flags,
    sizeof(magma_tally3_int_t),         &m,        VALUE,
    sizeof(magma_tally3_int_t),         &n,        VALUE,
    sizeof(magma_tally3_int_t),         &mm,       VALUE,
    sizeof(magma_tally3_int_t),         &nn,       VALUE,
    sizeof(magma_tally3_int_t),         &ib,       VALUE,
    sizeof(cuDoubleComplex)*m*n,   v,         INPUT,
    sizeof(magma_tally3_int_t),         &ldv,      VALUE,
    sizeof(cuDoubleComplex)*m*n,   c,         INPUT,
    sizeof(magma_tally3_int_t),         &ldc,      VALUE,
    sizeof(cuDoubleComplex)*ib*ib, t,         INPUT,
    sizeof(magma_tally3_int_t),         &ldt,      VALUE,
    sizeof(cuDoubleComplex*),      w,         OUTPUT | LOCALITY,
    sizeof(magma_tally3_int_t),         &ldw,      VALUE,
    sizeof(magma_tally3_int_t),         &priority, VALUE | TASK_PRIORITY,
    strlen(dag_label)+1, dag_label, VALUE | TASKLABEL,
...
magma_tally3_zgeqrf_mc( magma_tally3_context *cntxt, magma_tally3_int_t *m, magma_tally3_int_t *n,
...
/*  -- MAGMA_tally3 (version 1.6.1) --
    ...
    M       (input) magma_tally3_int_tEGER   
            The number of rows of the matrix A.  M >= 0.   

    N       (input) magma_tally3_int_tEGER   
    ...
    LDA     (input) magma_tally3_int_tEGER   
    ...
    LWORK   (input) magma_tally3_int_tEGER   
    ...
    INFO    (output) magma_tally3_int_tEGER   
            = 0:  successful exit   
  ...
  {
    //magma_tally3_int_t result = magma_tally3_zgeqrf(*m, *n, a, *lda, tau, work, *lwork, info);
    //return result;
  }

  magma_tally3_int_t i,j,l;

  magma_tally3_int_t ii=-1,jj=-1,ll=-1;
  ...
  *info = 0;
  ...
  cuDoubleComplex c_one = MAGMA_tally3_Z_ONE;
  cuDoubleComplex c_neg_one = MAGMA_tally3_Z_NEG_ONE;

  magma_tally3_int_t nb = (cntxt->nb ==-1)? magma_tally3_get_zpotrf_nb(*n): cntxt->nb;

  magma_tally3_int_t lwkopt = *n * nb;
  work[0] = MAGMA_tally3_Z_MAKE( (double)lwkopt, 0 );
  ...
  magma_tally3_int_t k = min(*m,*n);
  if (k == 0) {
    work[0] = c_one;
    return 0;
  }

  magma_tally3_int_t nt = (((*n)%nb) == 0) ? (*n)/nb : (*n)/nb + 1;
  magma_tally3_int_t mt = (((*m)%nb) == 0) ? (*m)/nb : (*m)/nb + 1;
