    ...
    magma_minproduct_int_t *info)
{
    ...
    *info = 0;
    ...
    #ifdef FAST_HEMV
    magma_minproduct_int_t ldwork2 = ldda*ceildiv(n,64);
    #else
    magma_minproduct_int_t ldwork2 = 0;
    #endif
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc( &dA, ldda*n + 2*lddw*nb + ldwork2 )) {
        *info = MAGMA_minproduct_ERR_DEVICE_ALLOC;
        return *info;
    }

    magma_minproductDoubleComplex *dwork  = dA + ldda*n;
    #ifdef FAST_HEMV
    magma_minproductDoubleComplex *dwork2 = dwork + 2*lddw*nb;
    #endif
    ...
    // rest of dwork (used as work in magma_minproductblas_zhemv) doesn't need to be cleared
    magma_minproductblas_zlaset( Magma_minproductFull, n, nb, c_zero, c_zero, dwork, lddw );
    ...
    if (upper) {
        ...
        for (i = n - nb; i >= kk; i -= nb) {
            ...
            if (i != n-nb)
                magma_minproduct_zgetmatrix( i+nb, nb, dA(0, i), ldda, A(0, i), lda );
            
            #ifdef FAST_HEMV
            magma_minproduct_zlatrd2( uplo, i+nb, nb, A(0, 0), lda, e, tau,
                           work, ldw, dA(0, 0), ldda, dwork, lddw,
                           dwork2, ldwork2 );
            #else
            magma_minproduct_zlatrd(  uplo, i+nb, nb, A(0, 0), lda, e, tau,
                           work, ldw, dA(0, 0), ldda, dwork, lddw );
            #endif

            /* Update the unreduced submatrix A(0:i-2,0:i-2), using an
               update of the form:  A := A - V*W' - W*V' */
            magma_minproduct_zsetmatrix( i + nb, nb, work, ldw, dwork, lddw );

            magma_minproduct_zher2k( uplo, Magma_minproductNoTrans, i, nb, c_neg_one,
        ...
        magma_minproduct_zgetmatrix( kk, kk, dA(0, 0), ldda, A(0, 0), lda );
        ...
        for (i = 0; i < n-nx; i += nb) {
            ...
            /* Get the current panel (no need for the 1st iteration) */
            if (i != 0)
                magma_minproduct_zgetmatrix( n-i, nb, dA(i, i), ldda, A(i, i), lda );
            
            #ifdef FAST_HEMV
            magma_minproduct_zlatrd2( uplo, n-i, nb, A(i, i), lda, &e[i], &tau[i],
                           work, ldw, dA(i, i), ldda, dwork, lddw,
                           dwork2, ldwork2 );
            #else
            magma_minproduct_zlatrd(  uplo, n-i, nb, A(i, i), lda, &e[i], &tau[i],
                           work, ldw, dA(i, i), ldda, dwork, lddw );
            #endif
            
            /* Update the unreduced submatrix A(i+ib:n,i+ib:n), using
               an update of the form:  A := A - V*W' - W*V' */
            magma_minproduct_zsetmatrix( n-i, nb, work, ldw, dwork, lddw );

            magma_minproduct_zher2k( Magma_minproductLower, Magma_minproductNoTrans, n-i-nb, nb, c_neg_one,
        ...
        /* Use CPU code to reduce the last or only block */
        if (1 <= n-nx)
            magma_minproduct_zgetmatrix( n-i, n-i, dA(i, i), ldda, A(i, i), lda );
