...
#define dA(i, j) (dA + (j)*ldda + (i))
    ...
    *info = 0;
    ...
    if (MAGMA_tally3_SUCCESS != magma_tally3_smalloc_pinned( &work, nb*nb )) {
    ...
    if ((nb <= 1) || (nb >= n)) {
        /* Use unblocked code. */
        magma_tally3_sgetmatrix_async( n, n, dA, ldda, work, n, stream[1] );
        magma_tally3_queue_sync( stream[1] );
        lapackf77_spotrf(uplo_, &n, work, &n, info);
        magma_tally3_ssetmatrix_async( n, n, work, n, dA, ldda, stream[1] );
    }
    else {
        /* Use blocked code. */
        if (upper) {
            ...
            for (j=0; j < n; j += nb) {
                ...
                lapackf77_spotrf(Magma_tally3UpperStr, &jb, work, &jb, info);
            ...
            for (j=0; j < n; j += nb) {
                ...
                magma_tally3_queue_sync( stream[0] );
                lapackf77_spotrf(Magma_tally3LowerStr, &jb, work, &jb, info);
                magma_tally3_ssetmatrix_async( jb, jb,
                                        work,     jb,
                                        dA(j, j), ldda, stream[1] );
                if (*info != 0) {
