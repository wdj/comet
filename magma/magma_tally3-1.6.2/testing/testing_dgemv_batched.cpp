...
int main( int argc, char** argv)
{
    ...
    double *h_A, *h_X, *h_Y, *h_Ymagma_tally3;
    double *d_A, *d_X, *d_Y;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_Ymagma_tally3,  double, sizeY  );
            ...
            /* =====================================================================
               ...
               =================================================================== */
            magma_tally3_dsetmatrix( M, N*batchCount, h_A, lda, d_A, ldda );
            magma_tally3_dsetvector( Xm*batchCount, h_X, incx, d_X, incx );
            magma_tally3_dsetvector( Ym*batchCount, h_Y, incy, d_Y, incy );
            
            dset_pointer(A_array, d_A, ldda, 0, 0, ldda*N, batchCount, magma_tally3_stream);
            dset_pointer(X_array, d_X, 1, 0, 0, incx*Xm, batchCount, magma_tally3_stream);
            dset_pointer(Y_array, d_Y, 1, 0, 0, incy*Ym, batchCount, magma_tally3_stream);

            magma_tally3_time = magma_tally3_sync_wtime( NULL );
            magma_tally3blas_dgemv_batched(opts.transA, M, N,
                             alpha, A_array, ldda,
                                    X_array, incx,
            ...
            magma_tally3_dgetvector( Ym*batchCount, d_Y, incy, h_Ymagma_tally3, incy );
            ...
            if ( opts.lapack ) {
                ...
                for(int s=0; s<batchCount; s++)
                {
                    ...
                    blasf77_daxpy( &Ym, &c_neg_one, h_Y + s*Ym, &ione, h_Ymagma_tally3 + s*Ym, &ione );
                    magma_tally3_err = lapackf77_dlange( "M", &M, &ione, h_Ymagma_tally3 + s*Ym, &incy, work ) / Ynorm; 
