...
/**
    ...
    as returned by SGEQRF. Q is of order M if SIDE = Magma_minproductLeft and of order N
    ...
    dT      REAL array on the GPU that is the output
            (the 9th argument) of magma_minproduct_sgeqrf_gpu.

    @param[in]
    nb      INTEGER
            This is the blocking size that was used in pre-computing DT, e.g.,
            the blocking size used in magma_minproduct_sgeqrf_gpu.
    ...
    magma_minproduct_int_t *info)
{
    ...
    *info = 0;
    ...
    hwork[0] = MAGMA_minproduct_S_MAKE( lwkopt, 0 );
    ...
    if ( step < 0 ) {
        ...
        magma_minproduct_sgetmatrix( ma, ib, dA(i,  i ), ldda, hA, ma );
        magma_minproduct_sgetmatrix( mi, ni, dC(ic, jc), lddc, hC, mi );
        ...
        magma_minproduct_ssetmatrix( mi, ni, hC, mi, dC(ic, jc), lddc );
    }
    ...
    if (nb < k) {
        for( i=i1; (step < 0 ? i >= i2 : i < i2); i += step ) {
            ...
            magma_minproduct_slarfb_gpu( side, trans, Magma_minproductForward, Magma_minproductColumnwise,
                              mi, ni, ib,
                              dA(i,  i ), ldda, dT(i), nb,
    ...
    // TODO sync. For cases Q*C and C*Q^T, last call is magma_minproduct_slarfb_gpu,
    // which is async magma_minproduct_gemm calls, so sormqr can be unfinished.
