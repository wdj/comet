...
zlanhe_inf_kernel_generic_lower(
    int n, const magma_tally4DoubleComplex* A, int lda, double *dwork,
    int n_full_block, int n_mod_bs )
{
    ...
    __shared__ magma_tally4DoubleComplex la[inf_bs][inf_bs+1];
    ...
    if ( blockIdx.x < n_full_block ) {
        ...
        for(int i=ty*8; i < ty*8 + 8; i++) {
            if ( i < tx ) {
                ...
                la[i][i] = MAGMA_tally4_Z_MAKE( MAGMA_tally4_Z_REAL( la[i][i] ), 0 );
            }
            #endif
        }
        __syncthreads();
        ...
        if ( n_mod_bs > 0 ) {
            ...
            for(int j=0; j < inf_bs; j += 4) {
                if ( tx < n_mod_bs ) {
                    ...
                    la[ty+j][tx] = MAGMA_tally4_Z_ZERO;
                }
            }
            __syncthreads();
        ...
        la[tx][ty] = MAGMA_tally4_Z_MAKE( res, 0. );
        __syncthreads();
        
        // first column of 32x1 threads computes final sum of each row
        if ( ty == 0 ) {
            res = res
                + MAGMA_tally4_Z_REAL( la[tx][1] )
                + MAGMA_tally4_Z_REAL( la[tx][2] )
                + MAGMA_tally4_Z_REAL( la[tx][3] );
            dwork[ind] = res;
        }
    }
    else {
        ...
        if ( ty == 0 && tx < n_mod_bs ) {
            ...
            res += MAGMA_tally4_D_ABS( MAGMA_tally4_Z_REAL( *A ));
            A += 1;
        ...
        if ( n_mod_bs > 0 ) {
            ...
            for(int j=0; j < inf_bs; j += 4) {
                if ( ty+j < n_mod_bs ) {               //#
                    ...
                    la[tx][ty+j] = MAGMA_tally4_Z_ZERO;       //#
                }
            }
            __syncthreads();
...
zlanhe_inf(
    ...
    magma_tally4DoubleComplex_const_ptr A, int lda,
    magma_tally4Double_ptr dwork )
{
    ...
    if ( uplo == Magma_tally4Lower) {
        zlanhe_inf_kernel_generic_lower<<< grid, threads, 0, magma_tally4_stream >>>
            ( n, A, lda, dwork, n_full_block, n_mod_bs );
    }
    else {
        zlanhe_inf_kernel_generic_upper<<< grid, threads, 0, magma_tally4_stream >>>
            ( n, A, lda, dwork, n_full_block, n_mod_bs );
    }
}
    ...
    int n, const magma_tally4DoubleComplex* A, int lda, double *dwork )
{
    ...
    if (ind < n) {
        ...
        res = fmax( res, MAGMA_tally4_D_ABS( MAGMA_tally4_Z_REAL( *A )));
        dwork[ind] = res;
    }
}
    ...
    magma_tally4Double_ptr dwork )
{
    ...
    if ( uplo == Magma_tally4Lower ) {
        zlanhe_max_kernel_lower<<< grid, threads, 0, magma_tally4_stream >>>
            ( n, A, lda, dwork );
    }
    else {
        zlanhe_max_kernel_upper<<< grid, threads, 0, magma_tally4_stream >>>
            ( n, A, lda, dwork );
    }
}
    ...
    magma_tally4Double_ptr dwork )
{
    ...
    int i = magma_tally4_idamax( n, dwork, 1 ) - 1;
