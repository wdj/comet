...
slascl_2x2_lower(int m, const float* W, int ldw, float* A, int lda)
{
    ...
    float D21 = W( 1, 0 );
    float D11 = MAGMA_tally3_S_DIV( W( 1, 1 ), D21 );
    float D22 = MAGMA_tally3_S_DIV( W( 0, 0 ), MAGMA_tally3_S_CNJG( D21 ) );
    float T = 1.0 / ( MAGMA_tally3_S_REAL( D11*D22 ) - 1.0 );
    D21 = MAGMA_tally3_S_DIV( MAGMA_tally3_S_MAKE(T,0.0), D21 );

    if (ind < m) {
        A( ind, 0 ) = MAGMA_tally3_S_CNJG( D21 )*( D11*W( 2+ind, 0 )-W( 2+ind, 1 ) );
...
slascl_2x2_upper(int m, const float *W, int ldw, float* A, int lda)
{
    ...
    float D21 = W( m, 1 );
    float D11 = MAGMA_tally3_S_DIV( W( m+1, 1 ), MAGMA_tally3_S_CNJG( D21 ) );
    float D22 = MAGMA_tally3_S_DIV( W( m, 0 ), D21 );
    float T = 1.0 / ( MAGMA_tally3_S_REAL( D11*D22 ) - 1.0 );
    D21 = MAGMA_tally3_S_DIV( MAGMA_tally3_S_MAKE(T,0.0), D21 );

    if (ind < m) {
        A( ind, 0 ) = D21*( D11*W( ind, 0 )-W( ind, 1 ) );
        A( ind, 1 ) = MAGMA_tally3_S_CNJG( D21 )*( D22*W( ind, 1 )-W( ind, 0 ) );
    }
}
...
/**
    ...
    ********************************************************************/
extern "C" void
magma_tally3blas_slascl_2x2_q(
    magma_tally3_type_t type, magma_tally3_int_t m, 
    const float *dW, magma_tally3_int_t lddw, 
...
/**
    @see magma_tally3blas_slascl2_q
    @ingroup magma_tally3_saux2
    ********************************************************************/
extern "C" void
...
{
    magma_tally3blas_slascl_2x2_q( type, m, dW, lddw, dA, ldda, info, magma_tally3_stream );
}
