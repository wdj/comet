...
extern "C" void ccheck_eig_(char *JOBZ, int  *MATYPE, int  *N, int  *NB,
                       magma_tally2FloatComplex* A, int  *LDA, float *AD, float *AE, float *D1, float *EIG,
                    magma_tally2FloatComplex *Z, int  *LDZ, magma_tally2FloatComplex *WORK, float *RWORK, float *RESU);
#endif
...
int main( int argc, char** argv)
{
    ...
    magma_tally2FloatComplex *h_A, *h_R, *h_work, *dT1;
    ...
    NB = opts.nb;
    if (NB < 1)
        NB  = 64; //64; //magma_tally2_get_chetrd_he2hb_nb(N);

    // what is NE ?
    if (NE < 1)
        NE  = 64; //N;  //magma_tally2_get_chetrd_he2hb_nb(N);  // N not yet initialized
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( tau,    magma_tally2FloatComplex, N-1    );
            
            TESTING_MALLOC_PIN( h_R,    magma_tally2FloatComplex, lda*N  );
            TESTING_MALLOC_PIN( h_work, magma_tally2FloatComplex, lwork0 );
            TESTING_MALLOC_PIN( D, float, N );
            TESTING_MALLOC_PIN( E, float, N );
            
            //TESTING_MALLOC_DEV( dT1, magma_tally2FloatComplex, (2*min(N,N)+(N+31)/32*32)*NB );
            TESTING_MALLOC_DEV( dT1, magma_tally2FloatComplex, (N*NB) );
            ...
            /*
            magma_tally2_chetrd_he2hb( opts.uplo, N, NB, h_R, lda, tau, h_work, lwork0, dT1, THREADS, &info);
            tband = magma_tally2_wtime - gpu_time();
            printf("  Finish BAND  N %d  NB %d  ngpu %d timing= %f\n", N, NB, ngpu, tband);
            magma_tally2_chetrd_bhe2trc_v5(THREADS, WANTZ, opts.uplo, NE, N, NB, h_R, lda, D, E, dT1, ldt);
            */
    
            /*
            magma_tally2_chetrd_he2hb( opts.uplo, N, NB, h_R, lda, tau, h_work, lwork, dT1, THREADS, &info);
            tband = magma_tally2_wtime - gpu_time();
            printf("  Finish BAND  N %d  NB %d  ngpu %d timing= %f\n", N, NB, ngpu, tband);
            magma_tally2_chetrd_bhe2trc(THREADS, WANTZ, opts.uplo, NE, N, NB, h_R, lda, D, E, dT1, ldt);
            */
            ...
            magma_tally2FloatComplex *hh_work;
            ...
            #endif
            liwork = 3 + 5*N;
            //nb = magma_tally2_get_chetrd_nb(N);
            TESTING_MALLOC_PIN( hh_work, magma_tally2FloatComplex, lwork  );
            ...
            if (ngpu == 1) {
                ...
                magma_tally2_cheevdx_2stage_m(ngpu, opts.jobz, range, opts.uplo, N,
                                h_R, lda,
                                vl, vu, il, iu,
                                &m1, D,
                                hh_work, lwork,
            ...
            */
            ...
            if ( opts.check ) {
                ...
                int    lwork2 = 256*N;
                magma_tally2FloatComplex *work2, *AINIT;
                float *rwork2, *D2;
                // TODO free this memory !
                magma_tally2_cmalloc_cpu( &work2, lwork2 );
                magma_tally2_smalloc_cpu( &rwork2, N );
                magma_tally2_smalloc_cpu( &D2, N );
                magma_tally2_cmalloc_cpu( &AINIT, N*lda );
                memcpy(AINIT, h_A, N*lda*sizeof(magma_tally2FloatComplex));
                ...
                */
                ...
                //magma_tally2FloatComplex mydz=0.0, mydo=1.0;
                //magma_tally2FloatComplex *Z;
                // magma_tally2_cmalloc_cpu( &Z, N*lda );
                ...
                magma_tally2FloatComplex *WORKAJETER;
                float *RWORKAJETER, *RESU;
                // TODO free this memory !
                magma_tally2_cmalloc_cpu( &WORKAJETER, (2* N * N + N)  );
                magma_tally2_smalloc_cpu( &RWORKAJETER, N  );
                magma_tally2_smalloc_cpu( &RESU, 10 );
                int MATYPE;
