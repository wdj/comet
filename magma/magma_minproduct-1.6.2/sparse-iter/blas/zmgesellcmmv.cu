...
zmgesellptmv_kernel_8_3D( 
    ...
    magma_minproduct_index_t * dcolind,
    magma_minproduct_index_t * drowptr,
    const magma_minproductDoubleComplex * __restrict__ dx,
    magma_minproductDoubleComplex beta, 
    magma_minproductDoubleComplex * dy)
{
   // T threads assigned to each row
    ...
    magma_minproductDoubleComplex * dy)
{
    ...
    if(row < num_rows ){
        magma_minproductDoubleComplex dot1 = MAGMA_minproduct_Z_MAKE(0.0, 0.0);
        magma_minproductDoubleComplex dot2 = MAGMA_minproduct_Z_MAKE(0.0, 0.0);
        int offset = drowptr[ bdx ];
        int max_ = (drowptr[ bdx+1 ]-offset)/blocksize;  
            // number of elements each thread handles
    ...
    magma_minproduct_queue_t queue )
{
    ...
    if ( (texture==1) && (precision==1) && (kepler==1) ) {
        ...
        int Ms = num_vecs * blocksize*alignment * sizeof( magma_minproductDoubleComplex );


        if ( alignment == 1) {
            dim3 block( blocksize, num_vecs/2, 1 );
            if ( beta == MAGMA_minproduct_Z_MAKE( 0.0, 0.0 ) )
