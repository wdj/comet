...
/**
    ...
    Other versions (magma_tally2_sgeqrf_gpu and magma_tally2_sgeqrf3_gpu) store the
    ...
    magma_tally2_int_t *info )
{
    ...
    *info = 0;
    ...
    if (nb >= nbmin && nb < k && nx < k) {
        ...
        for (i = 0; i < k-nx; i += nb) {
            ...
            if (i > 0) {
                ...
                magma_tally2_ssetmatrix_async( old_ib, old_ib,
            ...
            spanel_to_q_tally2( Magma_tally2Upper, ib, work_ref(i), ldwork, hwork+ib*ib );

            /* download the i-th V matrix */
            magma_tally2_ssetmatrix_async( rows, ib, work_ref(i), ldwork, dA(i,i), ldda, stream[0] );

            /* download the T matrix */
            magma_tally2_queue_sync( stream[1] );
            magma_tally2_ssetmatrix_async( ib, ib, hwork, ib, dwork, lddwork, stream[0] );
            magma_tally2_queue_sync( stream[0] );
            ...
            if (i + ib < n) {
                if (i+nb < k-nx) {
                    ...
                    sq_to_panel_tally2( Magma_tally2Upper, ib, work_ref(i), ldwork, hwork+ib*ib );
                }
                else {
                    magma_tally2_slarfb_gpu( Magma_tally2Left, Magma_tally2ConjTrans, Magma_tally2Forward, Magma_tally2Columnwise,
                                      rows, n-i-ib, ib,
    ...
    if (i < k) {
        ib   = n-i;
        rows = m-i;
        magma_tally2_sgetmatrix_async( rows, ib, dA(i, i), ldda, work, rows, stream[1] );
        magma_tally2_queue_sync( stream[1] );
        lhwork = lwork - rows*ib;
        lapackf77_sgeqrf(&rows, &ib, work, &rows, tau+i, work+ib*rows, &lhwork, info);
        
        magma_tally2_ssetmatrix_async( rows, ib, work, rows, dA(i, i), ldda, stream[1] );
    }
