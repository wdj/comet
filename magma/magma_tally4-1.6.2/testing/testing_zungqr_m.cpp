...
int main( int argc, char** argv )
{
    ...
    magma_tally4DoubleComplex *hA, *hR, *hT, *tau, *h_work;
    magma_tally4DoubleComplex_ptr dA, dT;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            // okay that magma_tally4_zgeqrf_gpu has special structure for R; R isn't used here.
            ...
            magma_tally4_zgetmatrix( m, n, dA, ldda, hA, lda );
            lapackf77_zlacpy( Magma_tally4FullStr, &m, &n, hA, &lda, hR, &lda );
            magma_tally4_zgetmatrix( nb, min_mn, dT, nb, hT, nb );
            
            gpu_time = magma_tally4_wtime();
            magma_tally4_zungqr_m( m, n, k, hR, lda, tau, hT, nb, &info );
            gpu_time = magma_tally4_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if ( info != 0 )
                printf("magma_tally4_zungqr_gpu returned error %d: %s.\n",
                       (int) info, magma_tally4_strerror( info ));
