...
// BLK_K gets defined in magma_tally4blas_sgemm_reduce,
    ...
    float      * __restrict__ dC, int ldc)
{
    ...
    if (blockIdx.x*BLK_M + threadIdx.y < m && blockIdx.y*BLK_N + threadIdx.z < n){
        ...
        for( int j = tx; j < k; j += BLK_K )
            lsum += MAGMA_tally4_S_CNJG( dA[j] )* dB[j];
        ...
        __syncthreads();
        if (threadIdx.x == 0) {
            if (MAGMA_tally4_S_EQUAL(beta, MAGMA_tally4_S_ZERO))
    ...
    magma_tally4Float_ptr       dC, magma_tally4_int_t lddc )
{
    ...
    if ( arch < 200  ) {
        ...
        sgemm_reduce_kernel<BLK_K> <<< blocks, threads, 0, magma_tally4_stream >>>
