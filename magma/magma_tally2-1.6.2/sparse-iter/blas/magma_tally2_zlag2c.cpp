...
/**
    ...
    convertes magma_tally2_z_matrix from Z to C
...
magma_tally2_vector_zlag2c(
    magma_tally2_z_matrix x, magma_tally2_c_vector *y,
    magma_tally2_queue_t queue )
{
    ...
    if ( x.memory_location == Magma_tally2_DEV) {
        ...
        y->nnz = x.nnz;
        magma_tally2_cmalloc( &y->val, x.num_rows );
        magma_tally2blas_zlag2c_sparse( x.num_rows, 1, x.dval, x.num_rows, y->val,
        ...
        y->num_rows = x.num_rows;
        y->nnz = x.nnz;
        magma_tally2_cmalloc_cpu( &y->val, x.num_rows );
...
/**
    ...
    @param
    B           magma_tally2_c_sparse_matrix*
...
magma_tally2_sparse_matrix_zlag2c(
    magma_tally2_z_matrix A, magma_tally2_c_sparse_matrix *B,
    magma_tally2_queue_t queue )
{
    magma_tally2_int_t info;
    if ( A.memory_location == Magma_tally2_DEV) {
        ...
        if ( A.storage_type == Magma_tally2_CSR ) {
            magma_tally2_cmalloc( &B->val, A.nnz );
            magma_tally2blas_zlag2c_sparse( A.nnz, 1, A.dval, A.nnz, B->val,
                    A.nnz, &info, queue );
            B->row = A.drow;
            B->col = A.dcol;
        }
        if ( A.storage_type == Magma_tally2_ELLPACK ) {
            magma_tally2_cmalloc( &B->val, A.num_rows*A.max_nnz_row );
            magma_tally2blas_zlag2c_sparse( A.num_rows*A.max_nnz_row, 1, A.dval,
        ...
        if ( A.storage_type == Magma_tally2_DENSE ) {
            magma_tally2_cmalloc( &B->val, A.num_rows*A.num_cols );
            magma_tally2blas_zlag2c_sparse(  A.num_rows, A.num_cols, A.dval, A.num_rows,
