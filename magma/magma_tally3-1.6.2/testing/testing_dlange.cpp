...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            //h_A[ 1 + 1*lda ] = MAGMA_tally3_D_NAN;
            ...
            norm_magma_tally3 = magma_tally3blas_dlange( norm[inorm], M, N, d_A, ldda, d_work );
            gpu_time = magma_tally3_wtime() - gpu_time;
            gpu_perf = gbytes / gpu_time;
            if (norm_magma_tally3 < 0)
                printf("magma_tally3blas_dlange returned error %f: %s.\n",
            ...
            norm_lapack = lapackf77_dlange( lapack_norm_const_tally3(norm[inorm]), &M, &N, h_A, &lda, h_work );
            ...
            //         MAGMA_tally3_D_REAL( h_A[1+1*lda] ),
            //         MAGMA_tally3_D_IMAG( h_A[1+1*lda] ) );
