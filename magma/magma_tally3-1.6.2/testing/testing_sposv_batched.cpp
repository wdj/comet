...
int main(int argc, char **argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_tally3_ssetmatrix( N, N*batchCount,    h_A, lda, d_A, ldda );
            magma_tally3_ssetmatrix( N, nrhs*batchCount, h_B, ldb, d_B, lddb );
            ...
            info = magma_tally3_sposv_batched(opts.uplo, N, nrhs, dA_array, ldda, dB_array, lddb, dinfo_array, batchCount, queue); 
            ...
            {
                if(cpu_info[i] != 0 ){
                    printf("magma_tally3_sposv_batched matrix %d returned internal error %d\n",i, (int)cpu_info[i] );
                }
            }
            if (info != 0)
                printf("magma_tally3_sposv_batched returned argument error %d: %s.\n", (int) info, magma_tally3_strerror( info ));
            
            //=====================================================================
            // Residual
            //=====================================================================
            magma_tally3_sgetmatrix( N, nrhs*batchCount, d_B, lddb, h_X, ldb );
            ...
            for(magma_tally3_int_t s=0; s<batchCount; s++)
            {
                ...
                blasf77_sgemm( Magma_tally3NoTransStr, Magma_tally3NoTransStr, &N, &nrhs, &N,
            ...
            if ( opts.lapack ) {
                ...
                for(magma_tally3_int_t s=0; s<batchCount; s++)
                {
                    lapackf77_sposv( lapack_uplo_const_tally3(opts.uplo), &N, &nrhs, h_A + s * lda * N, &lda, h_B + s * ldb * nrhs, &ldb, &info );
                }
