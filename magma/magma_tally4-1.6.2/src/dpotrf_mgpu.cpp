    ...
    magma_tally4_int_t *info)
{
    ...
    double *dwork[Magma_tally4MaxGPUs];
    ...
    *info = 0;
    ...
    if (ngpu == 1 && ((nb <= 1) || (nb >= n)) ) {
        ...
        if (MAGMA_tally4_SUCCESS != magma_tally4_dmalloc_pinned( &work, n*nb )) {
            *info = MAGMA_tally4_ERR_HOST_ALLOC;
            return *info;
        }
        magma_tally4_dgetmatrix( n, n, d_lA[0], ldda, work, n );
        lapackf77_dpotrf(uplo_, &n, work, &n, info);
        magma_tally4_dsetmatrix( n, n, work, n, d_lA[0], ldda );
        ...
        for( d=0; d < ngpu; d++ ) {
            ...
            if (MAGMA_tally4_SUCCESS != magma_tally4_dmalloc( &dwork[d], ngpu*nb*lddp )) {
                for( j=0; j < d; j++ ) {
        ...
        magma_tally4_setdevice(0);
        h = 1; //ngpu; //(n+nb-1)/nb;
        if (MAGMA_tally4_SUCCESS != magma_tally4_dmalloc_pinned( &work, n*nb*h )) {
            *info = MAGMA_tally4_ERR_HOST_ALLOC;
            return *info;
        }
        if (upper) {
            /* with three streams */
            magma_tally4_dpotrf3_mgpu(ngpu, uplo, n, n, 0, 0, nb, d_lA, ldda, dwork, lddp, work, n,
                               h, stream, event, info);
        } else {
            /* with three streams */
            magma_tally4_dpotrf3_mgpu(ngpu, uplo, n, n, 0, 0, nb, d_lA, ldda, dwork, lddp, work, nb*h,
                               h, stream, event, info);
        }
