...
/**
    Purpose
    -------
    magma_tally2blas_ssymv_work performs the matrix-vector operation:
    ...
    MAGMA_tally2 implements ssymv through two steps:
    ...
    magma_tally2blas_ssymv is a wrapper routine allocating the workspace inside the
    routine and provides the same interface as cublas.
    
    If users need to call ssymv frequently, we suggest using
    magma_tally2blas_ssymv_work instead of magma_tally2blas_ssymv. As the overhead to
    allocate and free in device memory in magma_tally2blas_ssymv would hurt performance.
    ...
    magma_tally2_queue_t queue )
{
    ...
    if ( arch < 200 ) {
        magma_tally2_ssymv( uplo, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
     ...
     */
    if ( (n == 0) || ( MAGMA_tally2_S_EQUAL(alpha, MAGMA_tally2_S_ZERO) && MAGMA_tally2_S_EQUAL(beta, MAGMA_tally2_S_ONE) ) )
        return info;
...
// end magma_tally2blas_ssymv_work


/**
    Purpose
    -------
    magma_tally2blas_ssymv performs the matrix-vector operation:
    ...
    magma_tally2Float_ptr dy, magma_tally2_int_t incy)
{
    ...
    magma_tally2_smalloc( &dwork, lwork );
    if ( dwork == NULL ) {
    ...
    magma_tally2blas_ssymv_work( uplo, n, alpha, dA, ldda, dx, incx, beta, dy, incy,
