...
int main(int argc, char **argv)
{
    ...
    magma_tally3DoubleComplex *A, *X, *Y, *Yatomics, *Ycublas, *Ymagma_tally3;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( Yatomics, magma_tally3DoubleComplex, sizeY );
            TESTING_MALLOC_CPU( Ycublas,  magma_tally3DoubleComplex, sizeY );
            ...
            cublasZhemv( opts.handle, cublas_uplo_const_tally3(opts.uplo),
                         N, &alpha, dA, ldda, dX, incx, &beta, dY, incy );
            cublas_time = magma_tally3_sync_wtime( 0 ) - cublas_time;
            cublas_perf = gflops / cublas_time;
            
            magma_tally3_zgetvector( N, dY, incy, Ycublas, incy );
            ...
            magma_tally3_zgetvector( N, dY, incy, Yatomics, incy );
            ...
            if ( opts.version == 1 ) {
                magma_tally3blas_zhemv_work( opts.uplo, N, alpha, dA, ldda, dX, incx, beta, dY, incy, dwork, ldwork, opts.queue );
            }
            else {
                // non-work interface (has added overhead)
                magma_tally3blas_zhemv( opts.uplo, N, alpha, dA, ldda, dX, incx, beta, dY, incy );
            }
            ...
            blasf77_zhemv( lapack_uplo_const_tally3(opts.uplo), &N, &alpha, A, &lda, X, &incx, &beta, Y, &incy );
