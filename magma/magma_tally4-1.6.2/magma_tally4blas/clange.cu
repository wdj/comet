...
clange_inf_kernel(
    int m, int n, const magma_tally4FloatComplex *A, int lda, float *dwork )
{
    ...
    if ( i < m ) {
        ...
        if ( n >= 4 ) {
            const magma_tally4FloatComplex *Aend = A + lda*n;
            magma_tally4FloatComplex rA[4] = { A[0], A[lda], A[2*lda], A[3*lda] };
            A += 4*lda;
    ...
    magma_tally4Float_ptr dwork )  //, magma_tally4_int_t lwork )
{
    ...
    if ( norm == Magma_tally4InfNorm ) {
        dim3 grid( (m-1)/64 + 1 );
        clange_inf_kernel<<< grid, threads, 0, magma_tally4_stream >>>( m, n, dA, ldda, dwork );
        ...
        dim3 grid( (m-1)/64 + 1 );
        clange_max_kernel<<< grid, threads, 0, magma_tally4_stream >>>( m, n, dA, ldda, dwork );
        max_nan_kernel<<< 1, 512, 0, magma_tally4_stream >>>( m, dwork );
        cudaMemcpy( &result, &dwork[0], sizeof(float), cudaMemcpyDeviceToHost );
    }
    else if ( norm == Magma_tally4OneNorm ) {
        dim3 grid( n );
        clange_one_kernel<<< grid, threads, 0, magma_tally4_stream >>>( m, n, dA, ldda, dwork );
