    ...
    int block_offset )
{
    ...
    magma_minproductDoubleComplex psum, psum_t;
    magma_minproductDoubleComplex total = MAGMA_minproduct_Z_ZERO;
    ...
    __shared__ magma_minproductDoubleComplex sA [quarter_NB_X][NB_X + 2];  // TODO +3 used in zhemv (single GPU); why?
    __shared__ magma_minproductDoubleComplex sx_blk[NB_X];  // for x[ blk ]
    __shared__ magma_minproductDoubleComplex sx_jj [NB_X];  // for x[ jj ], which cycles over all blocks left of diag

    magma_minproductDoubleComplex rA[4];
    magma_minproductDoubleComplex psums_t[4];
    ...
    if ( ty == 0 ) {
        ...
        if ( (partial && tx >= partial) ||
            ...
            sx_blk[tx] = MAGMA_minproduct_Z_ZERO;
        }
        else {
            sx_blk[tx] = x[0];
        }
    ...
    if ( blk % ngpu == my_gpu_id ) {
        ...
        psum_t = MAGMA_minproduct_Z_ZERO;
        #pragma unroll
        for(int j=0; j < 4; j++) {
    ...
    for(int jj=my_gpu_id; jj < blk; jj += ngpu) {
        ...
        if ( ty == 0 ) {
            if ( jj == 0 && tx < block_offset ) {
                sx_jj[tx] = MAGMA_minproduct_Z_ZERO;
            }
            else {
...
/**************************************************************
    ...
    Note beta*y is not included here; see magma_minproductblas_zhemv_mgpu_sync.
    ...
    int block_offset)
{
    ...
    if ( ind >= block_offset && ind < n+block_offset ) {
        magma_minproductDoubleComplex Ax = MAGMA_minproduct_Z_ZERO;
        ...
        y[ind * incy] = alpha*Ax;  // see magma_minproductblas_zhemv_sync for beta*y
    }
}
...
/**
    Purpose
    -------
    magma_minproductblas_zhemv_mgpu performs the matrix-vector operation:
...
magma_minproductblas_zhemv_mgpu(
    ...
    magma_minproductDoubleComplex alpha,
    magma_minproductDoubleComplex_const_ptr const d_lA[], magma_minproduct_int_t ldda,
    magma_minproduct_int_t offset,
    magma_minproductDoubleComplex const *x,       magma_minproduct_int_t incx,
    magma_minproductDoubleComplex beta,                                         // unused, see magma_minproductblas_zhemv_mgpu_sync
    magma_minproductDoubleComplex       *y,       magma_minproduct_int_t incy,             // unused
    ...
    magma_minproduct_queue_t queues[] )
{
    ...
    for(dev=0; dev < ngpu; dev++) {
        ...
        magma_minproductDoubleComplex const *dA_dev    = d_lA[dev] + offset_block_id*NB_X + num_blocks_skipped*NB_X*ldda;
        ...
        magma_minproductDoubleComplex       *dx_dev    = dwork[dev];
        magma_minproductDoubleComplex       *dwork_dev = dwork[dev] + ldda;
        ...
        // copy x to each GPU
        magma_minproduct_zsetvector_async( n, x, incx, dx_dev + block_offset, 1, queues[dev] );
    ...
    for(dev=0; dev < ngpu; dev++) {
        ...
        magma_minproduct_setdevice( dev );
        magma_minproductDoubleComplex       *dx_dev    = dwork[dev];
        magma_minproduct_zgetvector_async( n, dx_dev + block_offset, 1, &hwork[dev*n], 1, queues[dev] );
    }
    
    // see magma_minproductblas_zhemv_mgpu_sync for final row sums
...
/**
    ...
    For convenience, the parameters are identical to magma_minproductblas_zhemv_mgpu
    (though some are unused here).
    
    @see magma_minproductblas_zhemv_mgpu
    @ingroup magma_minproduct_zblas2
    ********************************************************************/
extern "C" magma_minproduct_int_t
magma_minproductblas_zhemv_mgpu_sync(
    magma_minproduct_uplo_t uplo,                                               // unused, see magma_minproductblas_zhemv_mgpu
    magma_minproduct_int_t n,
    magma_minproductDoubleComplex alpha,                                        // unused
    magma_minproductDoubleComplex_const_ptr const d_lA[], magma_minproduct_int_t ldda,
    magma_minproduct_int_t offset,                                              // unused
    magma_minproductDoubleComplex const *x,       magma_minproduct_int_t incx,             // unused
    magma_minproductDoubleComplex beta,
    magma_minproductDoubleComplex       *y,       magma_minproduct_int_t incy,             // unused
    magma_minproductDoubleComplex       *hwork,   magma_minproduct_int_t lhwork,
    magma_minproductDoubleComplex_ptr    dwork[], magma_minproduct_int_t ldwork,           // unused
    magma_minproduct_int_t ngpu,
