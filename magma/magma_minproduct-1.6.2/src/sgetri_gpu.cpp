    ...
    magma_minproduct_int_t *info )
{
    ...
    magma_minproductFloat_ptr dL = dwork;
    magma_minproduct_int_t lddl = n;
    magma_minproduct_int_t nb   = magma_minproduct_get_sgetri_nb(n);
    ...
    *info = 0;
    ...
    magma_minproduct_strtri_gpu( Magma_minproductUpper, Magma_minproductNonUnit, n, dA, ldda, info );
    if ( *info != 0 )
        return *info;
    ...
    for( j = jmax; j >= 0; j -= nb ) {
        ...
        magma_minproductblas_slacpy( Magma_minproductFull, n-j, jb,
                          dA(j,j), ldda,
                          dL(j,0), lddl );
        magma_minproductblas_slaset( Magma_minproductLower, n-j-1, jb, c_zero, c_zero, dA(j+1,j), ldda );
        ...
        if ( j+jb < n ) {
            magma_minproduct_sgemm( Magma_minproductNoTrans, Magma_minproductNoTrans, n, jb, n-j-jb,
                         c_neg_one, dA(0,j+jb), ldda,
                                    dL(j+jb,0), lddl,
                         c_one,     dA(0,j),    ldda );
        }
        // TODO use magma_minproductblas work interface
        magma_minproduct_strsm( Magma_minproductRight, Magma_minproductLower, Magma_minproductNoTrans, Magma_minproductUnit,
                     n, jb, c_one,
                     dL(j,0), lddl,
                     dA(0,j), ldda );
    }
    ...
    for( j = n-2; j >= 0; --j ) {
        jp = ipiv[j] - 1;
        if ( jp != j ) {
            magma_minproductblas_sswap( n, dA(0,j), 1, dA(0,jp), 1 );
        }
    }
