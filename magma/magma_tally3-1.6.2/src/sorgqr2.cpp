...
#define dA(i,j) (dA + (i) + (j)*ldda)
    ...
    *info = 0;
    ...
    ldda    = ((m + 31) / 32) * 32;
    lddwork = ((n + 31) / 32) * 32;
    if (MAGMA_tally3_SUCCESS != magma_tally3_smalloc( &dA, ldda*n + ldda*nb + lddwork*nb + nb*nb)) {
    ...
    if (kk < n) {
        ...
        */
        lapackf77_slacpy( Magma_tally3UpperLowerStr, &m_kk, &k_kk, A(kk,kk), &lda, V, &m_kk);
        lapackf77_slaset( Magma_tally3UpperLowerStr, &m_kk, &n_kk, &c_zero, &c_one, A(kk, kk), &lda );

        lapackf77_slarft( Magma_tally3ForwardStr, Magma_tally3ColumnwiseStr,
                          &m_kk, &k_kk,
                          V, &m_kk, &tau[kk], work, &k_kk);
        lapackf77_slarfb( Magma_tally3LeftStr, Magma_tally3NoTransStr, Magma_tally3ForwardStr, Magma_tally3ColumnwiseStr,
                          &m_kk, &n_kk, &k_kk,
