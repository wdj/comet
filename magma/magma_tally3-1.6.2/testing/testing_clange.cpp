    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( norm[inorm] == Magma_tally3OneNorm )
                ...
                lwork = M;
            // read whole matrix
            gbytes = M*N*sizeof(magma_tally3FloatComplex) / 1e9;
            ...
            //h_A[ 1 + 1*lda ] = MAGMA_tally3_C_NAN;
            ...
            norm_magma_tally3 = magma_tally3blas_clange( norm[inorm], M, N, d_A, ldda, d_work );
            gpu_time = magma_tally3_wtime() - gpu_time;
            gpu_perf = gbytes / gpu_time;
            if (norm_magma_tally3 < 0)
                printf("magma_tally3blas_clange returned error %f: %s.\n",
            ...
            norm_lapack = lapackf77_clange( lapack_norm_const_tally3(norm[inorm]), &M, &N, h_A, &lda, h_work );
            ...
            //         MAGMA_tally3_C_REAL( h_A[1+1*lda] ),
            //         MAGMA_tally3_C_IMAG( h_A[1+1*lda] ) );
