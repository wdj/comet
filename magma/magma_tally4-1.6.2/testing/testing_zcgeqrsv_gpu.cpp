...
int main( int argc, char** argv)
{
    ...
    magma_tally4DoubleComplex *h_A, *h_A2, *h_B, *h_X, *h_R;
    magma_tally4DoubleComplex_ptr d_A, d_B, d_X, d_T;
    magma_tally4FloatComplex  *d_SA, *d_SB;
    magma_tally4DoubleComplex *h_workd, *tau, tmp[1];
    magma_tally4FloatComplex  *h_works;
    magma_tally4_int_t lda,  ldb, lhwork, lworkgpu;
    magma_tally4_int_t ldda, lddb, lddx;
    magma_tally4_int_t M, N, nrhs, qrsv_iters, info, size, min_mn, max_mn, nb;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lddx   = ((N+31)/32) * 32;
            nb     = max( magma_tally4_get_zgeqrf_nb( M ), magma_tally4_get_cgeqrf_nb( M ) );
            ...
            lhwork = -1;
            lapackf77_zgels( Magma_tally4NoTransStr, &M, &N, &nrhs,
            ...
            TESTING_MALLOC_CPU( h_A2,    magma_tally4DoubleComplex, lda*N    );
            TESTING_MALLOC_CPU( h_B,     magma_tally4DoubleComplex, ldb*nrhs );
            TESTING_MALLOC_CPU( h_X,     magma_tally4DoubleComplex, ldb*nrhs );
            TESTING_MALLOC_CPU( h_R,     magma_tally4DoubleComplex, ldb*nrhs );
            TESTING_MALLOC_CPU( h_workd, magma_tally4DoubleComplex, lhwork   );
            h_works = (magma_tally4FloatComplex*)h_workd;
            
            TESTING_MALLOC_DEV( d_A, magma_tally4DoubleComplex, ldda*N      );
            TESTING_MALLOC_DEV( d_B, magma_tally4DoubleComplex, lddb*nrhs   );
            TESTING_MALLOC_DEV( d_X, magma_tally4DoubleComplex, lddx*nrhs   );
            TESTING_MALLOC_DEV( d_T, magma_tally4DoubleComplex, ( 2*min_mn + (N+31)/32*32 )*nb );
            
            /* Initialize the matrices */
            size = lda*N;
            lapackf77_zlarnv( &ione, ISEED, &size, h_A );
            lapackf77_zlacpy( Magma_tally4UpperLowerStr, &M, &N, h_A, &lda, h_A2, &lda );
            
            // make random RHS
            size = ldb*nrhs;
            lapackf77_zlarnv( &ione, ISEED, &size, h_B );
            lapackf77_zlacpy( Magma_tally4UpperLowerStr, &M, &nrhs, h_B, &ldb, h_R, &ldb );
            
            magma_tally4_zsetmatrix( M, N,    h_A, lda, d_A, ldda );
            magma_tally4_zsetmatrix( M, nrhs, h_B, ldb, d_B, lddb );
            
            //=====================================================================
            //              Mixed Precision Iterative Refinement - GPU
            //=====================================================================
            gpu_time = magma_tally4_wtime();
            magma_tally4_zcgeqrsv_gpu( M, N, nrhs,
                                d_A, ldda, d_B, lddb,
                                d_X, lddx, &qrsv_iters, &info );
            gpu_time = magma_tally4_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally4_zcgeqrsv returned error %d: %s.\n",
                       (int) info, magma_tally4_strerror( info ));
            
            // compute the residual
            magma_tally4_zgetmatrix( N, nrhs, d_X, lddx, h_X, ldb );
            blasf77_zgemm( Magma_tally4NoTransStr, Magma_tally4NoTransStr, &M, &nrhs, &N,
                           &c_neg_one, h_A, &lda,
                                       h_X, &ldb,
                           &c_one,     h_R, &ldb);
            ...
            gpu_time = magma_tally4_wtime();
            magma_tally4_zgels_gpu( Magma_tally4NoTrans, M, N, nrhs, d_A, ldda,
            ...
            TESTING_MALLOC_DEV( d_SA, magma_tally4FloatComplex, ldda*N    );
            TESTING_MALLOC_DEV( d_SB, magma_tally4FloatComplex, lddb*nrhs );
            magma_tally4blas_zlag2c( M, N,    d_A, ldda, d_SA, ldda, &info );
            magma_tally4blas_zlag2c( N, nrhs, d_B, lddb, d_SB, lddb, &info );
            
            gpu_time = magma_tally4_wtime();
            magma_tally4_cgels_gpu( Magma_tally4NoTrans, M, N, nrhs, d_SA, ldda,
            ...
            /* =====================================================================
               ...
               =================================================================== */
            lapackf77_zlacpy( Magma_tally4UpperLowerStr, &M, &nrhs, h_B, &ldb, h_X, &ldb );
