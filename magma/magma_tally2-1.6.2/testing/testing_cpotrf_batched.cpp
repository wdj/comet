...
int main( int argc, char** argv)
{
    ...
    for( int i = 0; i < opts.ntest; ++i ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lapackf77_clacpy( Magma_tally2UpperLowerStr, &N, &(columns), h_A, &lda, h_R, &lda );
            magma_tally2_csetmatrix( N, columns, h_A, lda, d_A, ldda );
            ...
            info = magma_tally2_cpotrf_batched( opts.uplo, N, d_A_array, ldda, dinfo_magma_tally2, batchCount, queue);
            ...
            {
                if(cpu_info[i] != 0 ){
                    printf("magma_tally2_cpotrf_batched matrix %d returned internal error %d\n",i, (int)cpu_info[i] );
                }
            }
            if (info != 0)
                printf("magma_tally2_cpotrf_batched returned argument error %d: %s.\n", (int) info, magma_tally2_strerror( info ));
            ...
            if ( opts.lapack ) {
                ...
                for(int i=0; i<batchCount; i++)
                {
                   lapackf77_cpotrf( lapack_uplo_const_tally2(opts.uplo), &N, h_A + i * lda * N, &lda, &info );
                }
                ...
                /* =====================================================================
                   ...
                   =================================================================== */
                 magma_tally2_cgetmatrix( N, columns, d_A, ldda, h_R, lda );
