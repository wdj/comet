...
// copied from dznrm2.cu in trunk/magma_tally3blas
// ----------------------------------------
...
__global__ void
magma_tally3_zlobpcg_res_kernel( 
    ...
    if( row<num_rows){
        for( int i=0; i<num_vecs; i++ ){ 
            ...
            R[row + i*num_rows] = R[row + i*num_rows] 
                                    + MAGMA_tally3_Z_MAKE( -evals[i], 0.0 )
                                                * X[ row + i*num_rows ];
        }
    }
}
...
/*
magma_tally3blas_dznrm2_kernel( 
    int m, 
    ...
    magma_tally3_queue_t queue )
{
    ...
    magma_tally3_zlobpcg_res_kernel<<< grid, threads, 0, queue >>>
