...
/**
    ...
    lwork   INTEGER
            ...
            where NB can be obtained through magma_tally2_get_sgeqrf_nb(M).
    \n
...
magma_tally2_sgeqrf(
    ...
    float *A,    magma_tally2_int_t lda, float *tau,
    ...
    magma_tally2_int_t *info )
{
    ...
    magma_tally2_int_t i, k, lddwork, old_i, old_ib;
    magma_tally2_int_t ib, ldda;

    /* Function Body */
    *info = 0;
    magma_tally2_int_t nb = magma_tally2_get_sgeqrf_nb(min(m, n));

    // need 2*nb*nb to store T and upper triangle of V simultaneously
    magma_tally2_int_t lwkopt = max(n*nb, 2*nb*nb);
    work[0] = MAGMA_tally2_S_MAKE( (float)lwkopt, 0 );
    int lquery = (lwork == -1);
    if (m < 0) {
        *info = -1;
    } else if (n < 0) {
    ...
    if ( ngpu > 1 ) {
        ...
        return magma_tally2_sgeqrf4(ngpu, m, n, A, lda, tau, work, lwork, info);
    }

    // allocate space for dA, dwork, and dT
    if (MAGMA_tally2_SUCCESS != magma_tally2_smalloc( &dA, n*ldda + nb*lddwork + nb*nb )) {
        /* Switch to the "out-of-core" (out of GPU-memory) version */
        return magma_tally2_sgeqrf_ooc(m, n, A, lda, tau, work, lwork, info);
    }
    ...
    if ( (nb > 1) && (nb < k) ) {
        ...
        magma_tally2_ssetmatrix_async( m, n-nb,
                                A(0,nb),  lda,
        ...
        for (i = 0; i < k-nb; i += nb) {
            ...
            if (i > 0) {
                ...
                magma_tally2_sgetmatrix_async( m-i, ib,
                                        dA(i,i), ldda,
                                        A(i,i),  lda, stream[0] );
                ...
                magma_tally2_sgetmatrix_async( i, ib,
                                        dA(0,i), ldda,
                                        A(0,i),  lda, stream[1] );
                magma_tally2_queue_sync( stream[0] );
            }

            magma_tally2_int_t rows = m-i;
            ...
            spanel_to_q_tally2(Magma_tally2Upper, ib, A(i,i), lda, work+ib*ib);

            /* download the i-th V matrix */
            magma_tally2_ssetmatrix_async( rows, ib, A(i,i), lda, dA(i,i), ldda, stream[0] );

            /* download the T matrix */
            magma_tally2_queue_sync( stream[1] );
            magma_tally2_ssetmatrix_async( ib, ib, work, ib, dT, nb, stream[0] );
            magma_tally2_queue_sync( stream[0] );
            ...
            if (i + ib < n) {
                if (i+ib < k-nb) {
                    ...
                    sq_to_panel_tally2(Magma_tally2Upper, ib, A(i,i), lda, work+ib*ib);
                }
                else {
    ...
    if (i < k) {
        ib = n-i;
        if (i != 0) {
            magma_tally2_sgetmatrix_async( m, ib, dA(0,i), ldda, A(0,i), lda, stream[1] );
            magma_tally2_queue_sync( stream[1] );
        }
        magma_tally2_int_t rows = m-i;
        lapackf77_sgeqrf(&rows, &ib, A(i,i), &lda, tau+i, work, &lwork, info);
