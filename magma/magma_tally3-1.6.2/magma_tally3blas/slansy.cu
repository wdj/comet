    ...
    if ( blockIdx.x < n_full_block ) {
        ...
        for(int i=ty*8; i < ty*8 + 8; i++) {
            if ( i < tx ) {
                ...
                la[i][i] = MAGMA_tally3_S_MAKE( MAGMA_tally3_S_REAL( la[i][i] ), 0 );
            }
            #endif
        }
        __syncthreads();
        ...
        if ( n_mod_bs > 0 ) {
            ...
            for(int j=0; j < inf_bs; j += 4) {
                if ( tx < n_mod_bs ) {
                    ...
                    la[ty+j][tx] = MAGMA_tally3_S_ZERO;
                }
            }
            __syncthreads();
        ...
        la[tx][ty] = MAGMA_tally3_S_MAKE( res, 0. );
        __syncthreads();
        
        // first column of 32x1 threads computes final sum of each row
        if ( ty == 0 ) {
            res = res
                + MAGMA_tally3_S_REAL( la[tx][1] )
                + MAGMA_tally3_S_REAL( la[tx][2] )
                + MAGMA_tally3_S_REAL( la[tx][3] );
            dwork[ind] = res;
        }
    }
    else {
        ...
        if ( ty == 0 && tx < n_mod_bs ) {
            ...
            res += MAGMA_tally3_D_ABS( MAGMA_tally3_S_REAL( *A ));
            A += 1;
        ...
        if ( n_mod_bs > 0 ) {
            ...
            for(int j=0; j < inf_bs; j += 4) {
                if ( ty+j < n_mod_bs ) {               //#
                    ...
                    la[tx][ty+j] = MAGMA_tally3_S_ZERO;       //#
                }
            }
            __syncthreads();
...
slansy_inf(
    ...
    magma_tally3Float_const_ptr A, int lda,
    magma_tally3Float_ptr dwork )
{
    ...
    if ( uplo == Magma_tally3Lower) {
        slansy_inf_kernel_generic_lower<<< grid, threads, 0, magma_tally3_stream >>>
            ( n, A, lda, dwork, n_full_block, n_mod_bs );
    }
    else {
        slansy_inf_kernel_generic_upper<<< grid, threads, 0, magma_tally3_stream >>>
            ( n, A, lda, dwork, n_full_block, n_mod_bs );
    }
}
    ...
    int n, const float* A, int lda, float *dwork )
{
    ...
    if (ind < n) {
        ...
        res = fmax( res, MAGMA_tally3_D_ABS( MAGMA_tally3_S_REAL( *A )));
        dwork[ind] = res;
    }
}
    ...
    magma_tally3Float_ptr dwork )
{
    ...
    if ( uplo == Magma_tally3Lower ) {
        slansy_max_kernel_lower<<< grid, threads, 0, magma_tally3_stream >>>
            ( n, A, lda, dwork );
    }
    else {
        slansy_max_kernel_upper<<< grid, threads, 0, magma_tally3_stream >>>
            ( n, A, lda, dwork );
    }
}
...
/**
    ...
    A       REAL array on the GPU, dimension (LDA,N)
            The symmetric matrix A. If UPLO = Magma_tally3Upper, the leading n by n
