...
{
    const magma_tally4_int_t nrhs = 1, n = r_blocks*size_b, ione = 1, inc = 1;

   magma_tally4DoubleComplex_ptr work; 
    magma_tally4_zmalloc_cpu( &work, r_blocks*size_b );

    // first shift the pivot elements
    for( magma_tally4_int_t k=0; k<r_blocks; k++) {
            for( magma_tally4_int_t l=0; l<size_b; l++)
            ipiv[ k*size_b+l ] = ipiv[ k*size_b+l ] + k*size_b;
    }

    // now the usual pivoting
    magma_tally4_zgetmatrix(n, 1, x, n, work, n);
    lapackf77_zlaswp(&nrhs, work, &n, &ione, &n, ipiv, &inc);
    magma_tally4_zsetmatrix(n, 1, work, n, x, n);
