...
#define dA(i_, j_) (dA + (j_)*ldda + (i_))
    ...
    *info = 0;
    ...
    if ( ngpu > 1 ) {
        ...
        return magma_tally2_cpotrf_m(ngpu, uplo, n, A, lda, info);
    }
    ...
    if (nb <= 1 || nb >= n) {
        ...
        if (upper) {
            ...
            for (j=0; j < n; j += nb) {
                ...
                jb = min(nb, (n-j));
                magma_tally2_csetmatrix_async( jb, (n-j), A(j, j), lda, dA(j, j), ldda, stream[1]);
                ...
                magma_tally2_cgetmatrix_async( j, jb,
                                        dA(0, j), ldda,
                                        A (0, j),  lda, stream[2] );

                magma_tally2_queue_sync( stream[0] );
                lapackf77_cpotrf(Magma_tally2UpperStr, &jb, A(j, j), &lda, info);
                if (*info != 0) {
                    *info = *info + j;
                    break;
                }
            ...
            for (j=0; j < n; j += nb) {
                ...
                //  for non-positive-definiteness. Computing MIN
                jb = min(nb, (n-j));
                magma_tally2_csetmatrix_async( (n-j), jb, A(j, j), lda, dA(j, j), ldda, stream[1]);
                ...
                magma_tally2_cgetmatrix_async( jb, j,
                                        dA(j, 0), ldda,
                                        A(j, 0),  lda, stream[2] );

                magma_tally2_queue_sync( stream[0] );
                lapackf77_cpotrf(Magma_tally2LowerStr, &jb, A(j, j), &lda, info);
                if (*info != 0) {
                    *info = *info + j;
                    break;
                }
