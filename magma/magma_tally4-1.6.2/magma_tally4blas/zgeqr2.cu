...
/**
    ...
    @ingroup magma_tally4_zgeqrf_aux
    ********************************************************************/
    ...
    magma_tally4_int_t *info)
{
    ...
    *info = 0;
    ...
    magma_tally4DoubleComplex *Aks;
    magma_tally4_zmalloc( &Aks, k );
    if ( Aks == NULL ) {
        *info = MAGMA_tally4_ERR_DEVICE_ALLOC;
        ...
        for (i = 0; i < k; ++i) {
            ...
            magma_tally4_zlarfg_gpu(m-i, dA(i, i), dA(min(i+1,m), i), dtau+i, dwork, &Aks[i]);

            if (n-i-1 > 0) {
               /* Apply H(i)' to A(i:m,i+1:n) from the left */
               magma_tally4_zlarf_gpu(m-i, n-i-1, dA(i, i), dtau+i, dA(i, i+1), ldda);
            }
        }

        magma_tally4_zcopymatrix( 1, k, Aks, 1, dA(0, 0), ldda+1 );
    }
