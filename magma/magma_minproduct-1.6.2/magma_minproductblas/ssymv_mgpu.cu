    ...
    int block_offset )
{
    ...
    float psum, psum_t;
    float total = MAGMA_minproduct_S_ZERO;
    ...
    if ( ty == 0 ) {
        ...
        if ( (partial && tx >= partial) ||
            ...
            sx_blk[tx] = MAGMA_minproduct_S_ZERO;
        }
        else {
            sx_blk[tx] = x[0];
        }
    ...
    if ( blk % ngpu == my_gpu_id ) {
        ...
        psum_t = MAGMA_minproduct_S_ZERO;
        #pragma unroll
        for(int j=0; j < 4; j++) {
    ...
    for(int jj=my_gpu_id; jj < blk; jj += ngpu) {
        ...
        if ( ty == 0 ) {
            if ( jj == 0 && tx < block_offset ) {
                sx_jj[tx] = MAGMA_minproduct_S_ZERO;
            }
            else {
...
/**************************************************************
    ...
    Note beta*y is not included here; see magma_minproductblas_ssymv_mgpu_sync.
    ...
    int block_offset)
{
    ...
    if ( ind >= block_offset && ind < n+block_offset ) {
        float Ax = MAGMA_minproduct_S_ZERO;
        ...
        y[ind * incy] = alpha*Ax;  // see magma_minproductblas_ssymv_sync for beta*y
    }
}
...
/**
    Purpose
    -------
    magma_minproductblas_ssymv_mgpu performs the matrix-vector operation:
...
magma_minproductblas_ssymv_mgpu(
    ...
    float alpha,
    magma_minproductFloat_const_ptr const d_lA[], magma_minproduct_int_t ldda,
    magma_minproduct_int_t offset,
    float const *x,       magma_minproduct_int_t incx,
    float beta,                                         // unused, see magma_minproductblas_ssymv_mgpu_sync
    float       *y,       magma_minproduct_int_t incy,             // unused
    ...
    magma_minproduct_queue_t queues[] )
{
    ...
    for(dev=0; dev < ngpu; dev++) {
        ...
        // copy x to each GPU
        magma_minproduct_ssetvector_async( n, x, incx, dx_dev + block_offset, 1, queues[dev] );
    ...
    for(dev=0; dev < ngpu; dev++) {
        ...
        magma_minproduct_setdevice( dev );
        float       *dx_dev    = dwork[dev];
        magma_minproduct_sgetvector_async( n, dx_dev + block_offset, 1, &hwork[dev*n], 1, queues[dev] );
    }
    
    // see magma_minproductblas_ssymv_mgpu_sync for final row sums
...
/**
    ...
    For convenience, the parameters are identical to magma_minproductblas_ssymv_mgpu
    (though some are unused here).
    
    @see magma_minproductblas_ssymv_mgpu
    @ingroup magma_minproduct_sblas2
    ********************************************************************/
extern "C" magma_minproduct_int_t
magma_minproductblas_ssymv_mgpu_sync(
    magma_minproduct_uplo_t uplo,                                               // unused, see magma_minproductblas_ssymv_mgpu
    magma_minproduct_int_t n,
    float alpha,                                        // unused
    magma_minproductFloat_const_ptr const d_lA[], magma_minproduct_int_t ldda,
    magma_minproduct_int_t offset,                                              // unused
    float const *x,       magma_minproduct_int_t incx,             // unused
    float beta,
    float       *y,       magma_minproduct_int_t incy,             // unused
    float       *hwork,   magma_minproduct_int_t lhwork,
    magma_minproductFloat_ptr    dwork[], magma_minproduct_int_t ldwork,           // unused
    magma_minproduct_int_t ngpu,
