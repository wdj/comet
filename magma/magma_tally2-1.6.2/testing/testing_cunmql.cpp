    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int itran = 0; itran < 2; ++itran ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_tally2_cgeqlf( mm, k, A, lda, tau, W, lwork_max, &info );
            if (info != 0)
            ...
            lapackf77_cunmql( lapack_side_const_tally2( side[iside] ), lapack_trans_const_tally2( trans[itran] ),
                              &m, &n, &k,
            ...
            lwork = -1;
            magma_tally2_cunmql( side[iside], trans[itran],
                          m, n, k,
                          A, lda, tau, R, ldc, W, lwork, &info );
            if (info != 0)
                printf("magma_tally2_cunmql (lwork query) returned error %d: %s.\n",
                       (int) info, magma_tally2_strerror( info ));
            ...
            if (info != 0)
                printf("magma_tally2_cunmql returned error %d: %s.\n",
                       (int) info, magma_tally2_strerror( info ));
