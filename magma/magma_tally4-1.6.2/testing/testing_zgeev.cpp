...
bool lessthan( magma_tally4DoubleComplex a, magma_tally4DoubleComplex b )
{
    return (MAGMA_tally4_Z_REAL(a) < MAGMA_tally4_Z_REAL(b)) ||
        (MAGMA_tally4_Z_REAL(a) == MAGMA_tally4_Z_REAL(b) && MAGMA_tally4_Z_IMAG(a) < MAGMA_tally4_Z_IMAG(b));
}
...
extern "C"
double magma_tally4_dlapy2(double x, double y)
{
    double ret_val, d;
    double w, z, xabs, yabs;
    
    xabs = MAGMA_tally4_D_ABS(x);
    yabs = MAGMA_tally4_D_ABS(y);
    w    = max(xabs, yabs);
    z    = min(xabs, yabs);
    if (z == 0.) {
        ret_val = w;
...
extern "C"
double magma_tally4_dzlapy2(magma_tally4DoubleComplex x)
{
    return magma_tally4_dlapy2( MAGMA_tally4_Z_REAL(x), MAGMA_tally4_Z_IMAG(x) );
}
...
int main( int argc, char** argv)
{
    ...
    magma_tally4DoubleComplex *h_A, *h_R, *VL, *VR, *h_work, *w1, *w2;
    magma_tally4DoubleComplex *w1copy, *w2copy;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( w1copy, magma_tally4DoubleComplex, N );
            TESTING_MALLOC_CPU( w2copy, magma_tally4DoubleComplex, N );
            TESTING_MALLOC_CPU( w1,     magma_tally4DoubleComplex, N );
            TESTING_MALLOC_CPU( w2,     magma_tally4DoubleComplex, N );
            ...
            TESTING_MALLOC_PIN( VL,     magma_tally4DoubleComplex, n2 );
            TESTING_MALLOC_PIN( VR,     magma_tally4DoubleComplex, n2 );
            ...
            magma_tally4_zgeev( opts.jobvl, opts.jobvr,
                         N, h_R, lda, w1,
                         VL, lda, VR, lda,
                         h_work, lwork, rwork, &info );
            gpu_time = magma_tally4_wtime() - gpu_time;
            if (info != 0)
                printf("magma_tally4_zgeev returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                if ( opts.jobvr == Magma_tally4Vec ) {
                    ...
                    lapackf77_zget22( Magma_tally4NoTransStr, Magma_tally4NoTransStr, Magma_tally4NoTransStr,
                                      &N, h_A, &lda, VR, &lda, w1,
                    ...
                    result[1] = -1.;
                    for( int j = 0; j < N; ++j ) {
                        tnrm = magma_tally4_cblas_dznrm2( N, &VR[j*lda], ione );
                        result[1] = max( result[1], min( ulpinv, fabs(tnrm-1.)/ulp ));
                        
                        vmx = vrmx = 0.;
                        for( int jj = 0; jj <N; ++jj ) {
                            vtst = magma_tally4_dzlapy2(VR[jj + j*lda]);
                            if (vtst > vmx)
                                vmx = vtst;
                            
                            if (MAGMA_tally4_Z_IMAG(VR[jj + j*lda])==0. &&
                                fabs( MAGMA_tally4_Z_REAL(VR[jj+j*lda]) ) > vrmx)
                            {
                                vrmx = fabs( MAGMA_tally4_Z_REAL( VR[jj+j*lda] ) );
                            }
                        }
                        if (vrmx / vmx < 1. - ulp*2.)
                            result[1] = ulpinv;
                ...
                if ( opts.jobvl == Magma_tally4Vec ) {
                    ...
                    lapackf77_zget22( Magma_tally4ConjTransStr, Magma_tally4NoTransStr, Magma_tally4ConjTransStr,
                                      &N, h_A, &lda, VL, &lda, w1,
                    ...
                    result[3] = -1.;
                    for( int j = 0; j < N; ++j ) {
                        tnrm = magma_tally4_cblas_dznrm2( N, &VL[j*lda], ione );
                        result[3] = max( result[3], min( ulpinv, fabs(tnrm - 1.)/ ulp ));
                        
                        vmx = vrmx = 0.;
                        for( int jj = 0; jj < N; ++jj ) {
                            vtst = magma_tally4_dzlapy2(VL[jj + j*lda]);
                            if (vtst > vmx)
                                vmx = vtst;
                            
                            if (MAGMA_tally4_Z_IMAG(VL[jj + j*lda])==0. &&
                                fabs( MAGMA_tally4_Z_REAL( VL[jj + j*lda] ) ) > vrmx)
                            {
                                vrmx = fabs( MAGMA_tally4_Z_REAL( VL[jj+j*lda]) );
                            }
                        }
                        if (vrmx / vmx < 1. - ulp*2.)
                            result[3] = ulpinv;
            ...
            if ( opts.check == 2 ) {
                ...
                magma_tally4DoubleComplex *LRE, DUM;
                TESTING_MALLOC_PIN( LRE, magma_tally4DoubleComplex, n2 );
                ...
                magma_tally4_zgeev( Magma_tally4Vec, Magma_tally4Vec,
                             N, h_R, lda, w1,
                             VL, lda, VR, lda,
                             h_work, lwork, rwork, &info );
                if (info != 0)
                ...
                // lapackf77_zlacpy( Magma_tally4UpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                // magma_tally4_zgeev( Magma_tally4NoVec, Magma_tally4NoVec,
                //              N, h_R, lda, w2,
                //              &DUM, 1, &DUM, 1,
                //              h_work, lwork, rwork, &info );
                // if (info != 0)
                //     printf("magma_tally4_zgeev (case N, N) returned error %d: %s.\n",
                //            (int) info, magma_tally4_strerror( info ));
                // 
                // // Do test 5: W(full) = W(partial, W only)
                // result[4] = 1;
                // for( int j = 0; j < N; ++j )
                //     if ( ! MAGMA_tally4_Z_EQUAL( w1[j], w2[j] ))
                //         result[4] = 0;
                
                // ----------
                // Compute eigenvalues and right eigenvectors
                lapackf77_zlacpy( Magma_tally4UpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                magma_tally4_zgeev( Magma_tally4NoVec, Magma_tally4Vec,
                             N, h_R, lda, w2,
                             &DUM, 1, LRE, lda,
                             h_work, lwork, rwork, &info );
                if (info != 0)
                    printf("magma_tally4_zgeev (case N, V) returned error %d: %s.\n",
                           (int) info, magma_tally4_strerror( info ));
                ...
                for( int j = 0; j < N; ++j )
                    for( int jj = 0; jj < N; ++jj )
                        if ( ! MAGMA_tally4_Z_EQUAL( VR[j+jj*lda], LRE[j+jj*lda] ))
                            result[7] = 0;
                
                // ----------
                // Compute eigenvalues and left eigenvectors
                lapackf77_zlacpy( Magma_tally4UpperLowerStr, &N, &N, h_A, &lda, h_R, &lda );
                magma_tally4_zgeev( Magma_tally4Vec, Magma_tally4NoVec,
                             N, h_R, lda, w2,
                             LRE, lda, &DUM, 1,
                             h_work, lwork, rwork, &info );
                if (info != 0)
                    printf("magma_tally4_zgeev (case V, N) returned error %d: %s.\n",
                           (int) info, magma_tally4_strerror( info ));
                ...
                for( int j = 0; j < N; ++j )
                    for( int jj = 0; jj < N; ++jj )
                        if ( ! MAGMA_tally4_Z_EQUAL( VL[j+jj*lda], LRE[j+jj*lda] ))
                            result[8] = 0;
            ...
            if ( opts.lapack ) {
                ...
                lapackf77_zgeev( lapack_vec_const_tally4(opts.jobvl), lapack_vec_const_tally4(opts.jobvr),
                                 &N, h_A, &lda, w2,
                                 VL, &lda, VR, &lda,
                                 h_work, &lwork, rwork, &info );
                ...
                for( int j=0; j < N; ++j ) {
                    for( int j2=j; j2 < N; ++j2 ) {
                        magma_tally4DoubleComplex diff = MAGMA_tally4_Z_SUB( w1copy[j], w2copy[j2] );
                        double diff2 = magma_tally4_dzlapy2( diff ) / max( magma_tally4_dzlapy2( w1copy[j] ), tol );
                        if ( diff2 < 100*tol ) {
                            if ( j != j2 ) {
                ...
                error  = magma_tally4_cblas_dznrm2( N, w1copy, 1 );
                error /= magma_tally4_cblas_dznrm2( N, w2copy, 1 );
