...
#define dAT(d, j)   (dAT[(d)]   + (j)*nb)
    ...
    if (lda < m || ngpu*ldda < n || lddw < m){
        printf( "Wrong arguments in magma_minproductblas_zsetmatrix_transpose_mgpu (%d<%d), (%d*%d<%d), or (%d<%d).\n",
    ...
    for(j=0; j<n; j+=nb){
       ...
       magma_minproductblas_ztranspose_q( m, ib, dwork(d,id), lddw, dAT(d,j_local), ldda, queues[d][id] );
    }
}
