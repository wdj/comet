...
__global__ void 
magma_tally3_ziteric_csr_kernel(    
    magma_tally3_int_t n, 
    magma_tally3_int_t nnz, 
    magma_tally3_index_t *Arowidx, 
    magma_tally3_index_t *Acolidx, 
    const magma_tally3DoubleComplex * __restrict__  A_val,
    ...
    magma_tally3DoubleComplex s, sp;
    int il, iu, jl, ju;
    ...
    {     
        ...
        // modify entry
        if (i == j)
            val[il-1] = MAGMA_tally3_Z_MAKE(sqrt(abs(MAGMA_tally3_Z_REAL(s))), 0.0);
        else
            val[il-1] =  s / val[iu-1];
...
/**
    ...
    The input format of the initial guess matrix A is Magma_tally3_CSRCOO,
...
magma_tally3_ziteric_csr( 
    ...
    magma_tally3_ziteric_csr_kernel<<< grid, block, 0, magma_tally3_stream >>>
            ( A.num_rows, A.nnz, 
