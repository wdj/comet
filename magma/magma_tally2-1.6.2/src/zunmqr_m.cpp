...
#define dwork(gpui, ind)       (dw[gpui] + maxnlocal*lddc + 2*lddac*lddar + 2*((nb+1)*nb) + (ind)*(lddwork*nb))
    ...
    magma_tally2_zmalloc_pinned(&T, nb*nb);
    ...
    *info = 0;
    ...
    for (igpu = 0; igpu < ngpu; ++igpu) {
        ...
        if (MAGMA_tally2_SUCCESS != magma_tally2_zmalloc( &dw[igpu], ldw )) {
    ...
    if (left) {
        ...
        for (magma_tally2_int_t i = i1; (i3 < 0 ? i >= i2 : i < i2); i += i3) {
            ...
            for (igpu = 0; igpu < ngpu; ++igpu) {
                ...
                magma_tally2_zsetmatrix_async(nq-i, kb,
                                       A(i, i),                 lda,
                                       dA_c(igpu, ind_c, i, 0), lddac, stream[igpu][0] );
                // set upper triangular part of dA to identity
                magma_tally2blas_zlaset_band_q( Magma_tally2Upper, kb, kb, kb, c_zero, c_one, dA_c(igpu, ind_c, i, 0), lddac, stream[igpu][0] );
            }
        ...
        for (magma_tally2_int_t i = 0; i < nbl; ++i) {
            ...
            magma_tally2_setdevice(igpu);
            magma_tally2_int_t kb = min(nb_l, n-i*nb_l);
            magma_tally2_zgetmatrix( m, kb,
                              dC(igpu, 0, i/ngpu*nb_l), lddc,
                              C(0, i*nb_l), ldc );
//            magma_tally2_zgetmatrix_async( m, kb,
//                                   dC(igpu, 0, i/ngpu*nb_l), lddc,
//                                   C(0, i*nb_l), ldc, stream[igpu][0] );
        ...
        *info = MAGMA_tally2_ERR_NOT_IMPLEMENTED;
        ...
        for (i = i1; (i3 < 0 ? i >= i2 : i < i2); i += i3) {
            ...
            magma_tally2_zsetmatrix( i__4, ib, A(i, i), lda, dA(i, 0), ldda );
            magma_tally2blas_zlaset_band( Magma_tally2Upper, ib, ib, ib, c_zero, c_one, dA(i, 0), ldda );
            ...
            // Apply H or H'; First copy T to the GPU
            magma_tally2_zsetmatrix( ib, ib, T, ib, dT, ib );
            magma_tally2_zlarfb_gpu( side, trans, Magma_tally2Forward, Magma_tally2Columnwise,
            mi, ni, ib,
            dA(i, 0), ldda, dT, ib,
            dC(ic, jc), lddc,
