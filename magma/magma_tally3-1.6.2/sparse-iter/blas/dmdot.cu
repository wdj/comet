    ...
    int k )
{
   ...
   if( i < n ){
    for( int j=0; j<k; j++)
      d[ i+j*n ] = MAGMA_tally3_D_MAKE( 0.0, 0.0 );
    }
}

// dot product
__global__ void
magma_tally3_ddot_kernel( 
    int Gs,
    int n, 
    double * v,
    double * r,
...
__global__ void
magma_tally3_dblockreduce_kernel( 
    int Gs,
    int n, 
    int k,
    double * vtmp,
...
__global__ void
magma_tally3_dreduce_kernel_fast( int Gs,
                           int n, 
                           double * vtmp,
                           double * vtmp2 ){
...
__global__ void
magma_tally3_dblockreduce_kernel_fast( 
    int Gs,
    int n, 
    int k,
    double * vtmp,
    ...
    magma_tally3_queue_t queue )
{
    ...
    if (k>1) {
        magma_tally3_dblockdot_kernel<<<Gs, Bs, Ms>>>( Gs.x, n, k, v, r, d1 );
    }
    else {
        magma_tally3_ddot_kernel<<<Gs, Bs, Ms>>>( Gs.x, n, v, r, d1 );
    }
/*
    // not necessary to zero GPU mem
    magma_tally3_dgpumemzero<<<Gs, Bs, 0>>>( d1, n*k,1 );
    magma_tally3_dgpumemzero<<<Gs, Bs, 0>>>( d2, n*k,1 );
    //magma_tally3blas_dlaset( Magma_tally3UpperLower, n, k, d1, n );
    //magma_tally3blas_dlaset( Magma_tally3UpperLower, n, k, d2, n );
    while( Gs.x > 1 ) {
        Gs_next.x = ( Gs.x+Bs.x-1 )/ Bs.x ;
        magma_tally3_dblockreduce_kernel<<< Gs_next.x, Bs.x, Ms >>> 
                                        ( Gs.x, n, k, aux1, aux2 );
        Gs.x = Gs_next.x;
        b = 1 - b;
    ...
    for( int j=0; j<k; j++) {
            magma_tally3_dcopyvector( 1, aux1+j*n, 1, skp+j, 1 );
    }
*/
    ...
    if ( k>1) {
        while( Gs.x > 1 ) {
            ...
            magma_tally3_dblockreduce_kernel_fast<<< Gs_next.x/2, Bs.x/2, Ms/2 >>> 
                        ( Gs.x, n, k, aux1, aux2 );
        ...
        while( Gs.x > 1 ) {
            Gs_next.x = ( Gs.x+Bs.x-1 )/ Bs.x ;
            if ( Gs_next.x == 1 ) Gs_next.x = 2;
            magma_tally3_dreduce_kernel_fast<<< Gs_next.x/2, Bs.x/2, Ms/2 >>> 
                        ( Gs.x, n, aux1, aux2 );
            Gs_next.x = Gs_next.x /2;
            Gs.x = Gs_next.x;
            b = 1 - b;
    ...
    magma_tally3_queue_t queue )
{
    ...
    while( rows_left > (chunk_size) ) {
        magma_tally3_dmdotc( n, chunk_size, v+offset*n, r, d1, d2, skp+offset, queue );
        offset = offset + chunk_size;
        rows_left = rows_left-chunk_size;

    }
    // process rest
    magma_tally3_dmdotc( n, rows_left, v+offset*n, r, d1, d2, skp+offset, queue ); 
