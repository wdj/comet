...
/**
    ...
    T       DOUBLE_PRECISION array, dimension (NB, min(M,N)).
            ...
            magma_minproduct_dgeqrf_gpu (except stored on the CPU, not the GPU).
...
#define dT(d,i,j) (dT[d] + (i) + (j)*nb)
    ...
    *info = 0;
    ...
    double *dA[ Magma_minproductMaxGPUs ] = { NULL };
    double *dT[ Magma_minproductMaxGPUs ] = { NULL };
    double *dV[ Magma_minproductMaxGPUs ] = { NULL };
    double *dW[ Magma_minproductMaxGPUs ] = { NULL };
    ...
    for( int d = 0; d < ngpu; ++d ) {
        ...
        ldwork = nlocal[d]*ldda  // dA
               ...
               + nb*ldda         // dV
               + nb*lddwork;     // dW
        if ( MAGMA_minproduct_SUCCESS != magma_minproduct_dmalloc( &dA[d], ldwork )) {
    ...
    if (kk < n) {
        ...
        magma_minproductblas_dlaset( Magma_minproductFull, kk, n - kk, c_zero, c_zero, dA(dpanel, 0, di), ldda );
        trace_cpu_end( 0 );
    }
    ...
    if (kk > 0) {
        ...
        for( int d = 0; d < ngpu; ++d ) {
            ...
            magma_minproduct_dsetmatrix_async( nb, min(m,n), T, nb, dT[d], nb, stream[d] );
            trace_gpu_end( d, 0 );
        }
        ...
        for( i = ki; i >= 0; i -= nb ) {
            ...
            for( int d = 0; d < ngpu; ++d ) {
                ...
                magma_minproduct_dsetmatrix_async( mi, ib,
                                        A(i, i), lda,
                                        dV[d],   ldda, stream[d] );
                trace_gpu_end( d, 0 );
            }
            ...
            magma_minproductblas_dlaset( Magma_minproductFull, i,  ib, c_zero, c_zero, dA(dpanel, 0, di), ldda );
            magma_minproductblas_dlaset( Magma_minproductFull, mi, ib, c_zero, c_one,  dA(dpanel, i, di), ldda );
    ...
    magma_minproduct_dgetmatrix_1D_col_bcyclic( m, n, dA, ldda, A, lda, ngpu, nb );
    trace_cpu_end( 0 );
