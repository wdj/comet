    ...
    magma_tally3_int_t *info)
{
    ...
    *info = 0;
    ...
    magma_tally3DoubleComplex *dA, *dB;
    if ( ngpu > 1 ) {
        goto CPU_INTERFACE;
    }
    ldda = ((n+31)/32)*32;
    lddb = ldda;
    if ( MAGMA_tally3_SUCCESS != magma_tally3_zmalloc( &dA, ldda*n )) {
        goto CPU_INTERFACE;
    }
    if ( MAGMA_tally3_SUCCESS != magma_tally3_zmalloc( &dB, lddb*nrhs )) {
    ...
    if ( *info == 0 ) {
        magma_tally3_zsetmatrix( n, nrhs, B, ldb, dB, lddb );
        magma_tally3_zgetrs_gpu( Magma_tally3NoTrans, n, nrhs, dA, ldda, ipiv, dB, lddb, info );
        magma_tally3_zgetmatrix( n, nrhs, dB, lddb, B, ldb );
    }
    magma_tally3_free( dA );
    magma_tally3_free( dB );
    return *info;
...
CPU_INTERFACE:
    ...
    magma_tally3_zgetrf( n, n, A, lda, ipiv, info );
    if ( *info == 0 ) {
        lapackf77_zgetrs( Magma_tally3NoTransStr, &n, &nrhs, A, &lda, ipiv, B, &ldb, info );
    }
    return *info;
}
