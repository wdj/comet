    ...
    magma_minproduct_queue_t queue )
{
    ...
    CHECK( magma_minproduct_svinit( &rt,Magma_minproduct_DEV, A.num_rows, b.num_cols, c_zero, queue ));
    CHECK( magma_minproduct_svinit( &p, Magma_minproduct_DEV, A.num_rows, b.num_cols, c_zero, queue ));
    CHECK( magma_minproduct_svinit( &q, Magma_minproduct_DEV, A.num_rows, b.num_cols, c_zero, queue ));
    CHECK( magma_minproduct_svinit( &h, Magma_minproduct_DEV, A.num_rows, b.num_cols, c_zero, queue ));
    ...
    magma_minproduct_scopy( dofs, h.dval, 1, p.dval, 1 );                    // p = h
    nom = MAGMA_minproduct_S_REAL( magma_minproduct_sdot(dofs, r.dval, 1, h.dval, 1) );
    CHECK( magma_minproduct_s_spmv( c_one, A, p, c_zero, q, queue ));             // q = A p
    den = MAGMA_minproduct_S_REAL( magma_minproduct_sdot(dofs, p.dval, 1, q.dval, 1) );// den = p dot q
    ...
    do
    {
        ...
        gammanew = MAGMA_minproduct_S_REAL( magma_minproduct_sdot(dofs, r.dval, 1, h.dval, 1) );
                                                            // gn = < r,h>

        if ( solver_par->numiter==1 ) {
            magma_minproduct_scopy( dofs, h.dval, 1, p.dval, 1 );                    // p = h
        } else {
            beta = MAGMA_minproduct_S_MAKE(gammanew/gammaold, 0.);       // beta = gn/go
            magma_minproduct_sscal(dofs, beta, p.dval, 1);            // p = beta*p
            magma_minproduct_saxpy(dofs, c_one, h.dval, 1, p.dval, 1); // p = p + h
        }

        CHECK( magma_minproduct_s_spmv( c_one, A, p, c_zero, q, queue ));   // q = A p
        den = MAGMA_minproduct_S_REAL(magma_minproduct_sdot(dofs, p.dval, 1, q.dval, 1));
                // den = p dot q

        alpha = MAGMA_minproduct_S_MAKE(gammanew/den, 0.);
        magma_minproduct_saxpy(dofs,  alpha, p.dval, 1, x->dval, 1);     // x = x + alpha p
        magma_minproduct_saxpy(dofs, -alpha, q.dval, 1, r.dval, 1);      // r = r - alpha q
        gammaold = gammanew;

        res = magma_minproduct_snrm2( dofs, r.dval, 1 );
        if ( solver_par->verbose > 0 ) {
