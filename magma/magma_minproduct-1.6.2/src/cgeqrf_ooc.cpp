...
/**
    ...
    This is an out-of-core (ooc) version that is similar to magma_minproduct_cgeqrf but
    ...
    magma_minproduct_int_t *info )
{
    ...
    magma_minproductFloatComplex *dA, *dwork;
    magma_minproductFloatComplex c_one = MAGMA_minproduct_C_ONE;

    int  k, lddwork, ldda;

    *info = 0;
    int nb = magma_minproduct_get_cgeqrf_nb(min(m, n));
    ...
    if (NB >= n)
        return magma_minproduct_cgeqrf(m, n, A, lda, tau, work, lwork, info);
    ...
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_cmalloc( &dA, (NB + nb)*ldda + nb*lddwork )) {
    ...
    magma_minproductFloatComplex *ptr = dA + ldda * NB;
    dwork = dA + ldda*(NB + nb);
    ...
    for (int i=0; i < n; i += NB) {
        ...
        for (int j=0; j < min(i,k); j += nb) {
            ...
            cpanel_to_q(Magma_minproductUpper, ib, A(j,j), lda, work+ib*ib);
            ...
            cq_to_panel(Magma_minproductUpper, ib, A(j,j), lda, work+ib*ib);
        }

        /* 3. Do a QR on the current part */
        if (i < k)
            magma_minproduct_cgeqrf2_gpu(m-i, IB, dA(i,0), ldda, tau+i, info);

        /* 4. Copy the current part back to the CPU */
        magma_minproduct_cgetmatrix_async( (m), IB,
                                dA(0,0), ldda,
                                A(0,i),  lda, stream[0] );
    }
