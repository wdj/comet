    ...
    magma_minproduct_int_t *info)
{
    ...
    *info = 0;
    ...
    if (nounit) {
        for (j=0; j < n; ++j) {
            if ( MAGMA_minproduct_S_EQUAL( *A(j,j), c_zero )) {
    ...
    if (nb <= 1 || nb >= n)
        ...
        if (upper) {
            ...
            for (j=0; j < n; j += nb) {
                jb = min(nb, (n-j));
                magma_minproduct_ssetmatrix( jb, (n-j),
                                  A(j, j),  lda,
                                  dA(j, j), ldda );

                /* Compute rows 1:j-1 of current block column */
                magma_minproduct_strmm( Magma_minproductLeft, Magma_minproductUpper,
                             Magma_minproductNoTrans, Magma_minproductNonUnit, j, jb,
                             c_one, dA(0,0), ldda, dA(0, j),ldda);

                magma_minproduct_strsm( Magma_minproductRight, Magma_minproductUpper,
                             Magma_minproductNoTrans, Magma_minproductNonUnit, j, jb,
                ...
                lapackf77_strtri(Magma_minproductUpperStr, diag_, &jb, A(j,j), &lda, info);

                magma_minproduct_ssetmatrix( jb, jb,
                                  A(j, j),  lda,
                                  dA(j, j), ldda );
            }
        }
            ...
            for (j=nn-1; j >= 0; j -= nb) {
                ...
                if ((j+jb) < n) {
                    magma_minproduct_ssetmatrix( (n-j), jb,
                                      A(j, j),  lda,
                                      dA(j, j), ldda );
                    ...
                    magma_minproduct_sgetmatrix_async( n-j-jb, jb,
                                            dA(j+jb, j), ldda,
                ...
                /* Compute inverse of current diagonal block */
                lapackf77_strtri(Magma_minproductLowerStr, diag_, &jb, A(j,j), &lda, info);
