    ...
    magma_minproduct_queue_t queue )
{
    ...
    magma_minproduct_z_matrix r={Magma_minproduct_CSR};
    
    if ( A.num_rows == b.num_rows ) {
        CHECK( magma_minproduct_zvinit( &r, Magma_minproduct_DEV, A.num_rows, b.num_cols, zero, queue ));

        CHECK( magma_minproduct_z_spmv( one, A, x, zero, r, queue ));           // r = A x
        magma_minproduct_zaxpy(dofs, mone, b.dval, 1, r.dval, 1);          // r = r - b
        *res =  magma_minproduct_dznrm2(dofs, r.dval, 1);            // res = ||r||
        ...
        CHECK( magma_minproduct_zvinit( &r, Magma_minproduct_DEV, b.num_rows,b.num_cols, zero, queue ));

        CHECK( magma_minproduct_z_spmv( one, A, x, zero, r, queue ));           // r = A x

        for( magma_minproduct_int_t i=0; i<num_vecs; i++) {
            magma_minproduct_zaxpy(dofs, mone, b(i), 1, r(i), 1);   // r = r - b
