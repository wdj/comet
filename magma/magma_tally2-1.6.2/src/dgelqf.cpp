    ...
    magma_tally2_int_t *info)
{
    ...
    /* Function Body */
    *info = 0;
    nb = magma_tally2_get_dgelqf_nb(m);
    min_mn = min(m,n);

    work[0] = MAGMA_tally2_D_MAKE( (double)(m*nb), 0 );
    lquery = (lwork == -1);
    if (m < 0) {
        *info = -1;
    } else if (n < 0) {
    ...
    if (maxdim*maxdim < 2*maxm*maxn) {
        ...
        if (MAGMA_tally2_SUCCESS != magma_tally2_dmalloc( &dA, maxdim*maxdim )) {
            *info = MAGMA_tally2_ERR_DEVICE_ALLOC;
            return *info;
        }

        magma_tally2_dsetmatrix( m, n, A, lda, dA(0,0), ldda );
        dAT = dA;
        magma_tally2blas_dtranspose_inplace( lddat, dAT(0,0), lddat );
    }
    else {
        ...
        if (MAGMA_tally2_SUCCESS != magma_tally2_dmalloc( &dA, 2*maxn*maxm )) {
            *info = MAGMA_tally2_ERR_DEVICE_ALLOC;
            return *info;
        }
        ...
        dAT = dA + maxn * maxm;
        magma_tally2blas_dtranspose( m, n, dA(0,0), ldda, dAT(0,0), lddat );
    }

    // factor QR
    magma_tally2_dgeqrf2_gpu( n, m, dAT(0,0), lddat, tau, &iinfo );
    assert( iinfo >= 0 );
    if ( iinfo > 0 ) {
        *info = iinfo;
    }
    ...
    if (maxdim*maxdim < 2*maxm*maxn) {
        magma_tally2blas_dtranspose_inplace( lddat, dAT(0,0), lddat );
        magma_tally2_dgetmatrix( m, n, dA(0,0), ldda, A, lda );
    } else {
        magma_tally2blas_dtranspose( n, m, dAT(0,0), lddat, dA(0,0), ldda );
        magma_tally2_dgetmatrix( m, n, dA(0,0), ldda, A, lda );
    }
