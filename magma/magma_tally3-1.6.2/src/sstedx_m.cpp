...
#define Z(i_,j_) (Z + (i_) + (j_)*ldz)
    ...
    magma_tally3_int_t alleig, indeig, valeig, lquery;
    magma_tally3_int_t i, j, k, m;
    magma_tally3_int_t liwmin, lwmin;
    magma_tally3_int_t start, end, smlsiz;
    ...
    *info = 0;
    ...
    if (*info == 0) {
        ...
        smlsiz = magma_tally3_get_smlsize_divideconquer();
        if ( n <= 1 ) {
            lwmin = 1;
            liwmin = 1;
        } else {
        ...
        if (alleig) {
            ...
            while ( start < n ) {
                ...
                for (end = start+1; end < n; ++end) {
                    tiny = eps * sqrt( MAGMA_tally3_S_ABS(d[end-1]*d[end]));
                    if (MAGMA_tally3_S_ABS(e[end-1]) <= tiny)
                        break;
                }
                ...
                if (m > smlsiz) {
                    ...
                    magma_tally3_int_t mm = m-1;
                    lapackf77_slascl("G", &izero, &izero, &orgnrm, &d_one, &mm, &ione, &e[start], &mm, info);

                    magma_tally3_slaex0_m( ngpu, m, &d[start], &e[start], Z(start, start), ldz, work, iwork, Magma_tally3RangeAll, vl, vu, il, iu, info);
            ...
            magma_tally3_int_t nm = n-1;
            lapackf77_slascl("G", &izero, &izero, &orgnrm, &d_one, &nm, &ione, e, &nm, info);

            magma_tally3_slaex0_m(ngpu, n, d, e, Z, ldz, work, iwork, range, vl, vu, il, iu, info);
