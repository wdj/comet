...
/* ////////////////////////////////////////////////////////////////////////////
   -- Testing magma_minproduct_cher2k_mgpu
*/
int main( int argc, char** argv)
{
    ...
    magma_minproductFloatComplex alpha = MAGMA_minproduct_C_MAKE( 1.2345, 4.321 );
    float beta = 3.14159;
    
    real_Double_t    gflops, gpu_perf, cpu_perf, gpu_time, cpu_time;
    float           error, work[1];
    magma_minproductFloatComplex *hA, *hR, *hR2, *hV, *hW;
    magma_minproductFloatComplex_ptr dV[Magma_minproductMaxGPUs], dW[Magma_minproductMaxGPUs], dA[Magma_minproductMaxGPUs];
    magma_minproduct_int_t n, k, size, lda, ldda, nb, ngpu, nstream;
    ...
    printf( "version 1: magma_minproductblas_cher2k_mgpu2     %s\n", (opts.version==1 ? "(enabled)" : ""));
    printf( "version 2: magma_minproductblas_cher2k_mgpu_spec %s\n", (opts.version==2 ? "(enabled)" : ""));
#ifdef ICHI
    printf( "version 3: magma_minproduct_cher2k_mgpu (Ichi)   %s\n", (opts.version==3 ? "(enabled)" : ""));
#endif
    printf( "\n" );
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        ...
        for( int offset = 0; offset < n; offset += min(k,nb) ) {
            for( int iter = 0; iter < opts.niter; ++iter ) {
                ...
                TESTING_MALLOC_CPU( hA,  magma_minproductFloatComplex, lda*n   );
                TESTING_MALLOC_CPU( hR,  magma_minproductFloatComplex, lda*n   );
                TESTING_MALLOC_CPU( hR2, magma_minproductFloatComplex, lda*n   );
                TESTING_MALLOC_CPU( hV,  magma_minproductFloatComplex, lda*k*2 );
                //TESTING_MALLOC_CPU( hW,  magma_minproductFloatComplex, lda*k   );
                for( int d = 0; d < ngpu; ++d ) {
                    magma_minproduct_int_t nlocal = ((n / nb) / ngpu + 1) * nb;
                    magma_minproduct_setdevice( d );
                    TESTING_MALLOC_DEV( dA[d], magma_minproductFloatComplex, ldda*nlocal );
                    TESTING_MALLOC_DEV( dV[d], magma_minproductFloatComplex, ldda*k*2    );
                    //TESTING_MALLOC_DEV( dW[d], magma_minproductFloatComplex, ldda*k      );
                ...
                /* ====================================================================
                   ...
                   =================================================================== */
                magma_minproduct_csetmatrix_1D_col_bcyclic( n, n, hA, lda, dA, ldda, ngpu, nb );
                for( int d = 0; d < ngpu; ++d ) {
                    magma_minproduct_setdevice( d );
                    dW[d] = dV[d] + ldda*k;
                    magma_minproduct_csetmatrix( n, k, hV, lda, dV[d], ldda );
                    magma_minproduct_csetmatrix( n, k, hW, lda, dW[d], ldda );
                }
                ...
                if ( opts.version == 1 ) {
                    magma_minproductblas_cher2k_mgpu2(
                        Magma_minproductLower, Magma_minproductNoTrans, n-offset, k,
                        alpha, dV, ldda, 0,
                               dW, ldda, 0,
                        beta,  dA, ldda, offset,
                    ...
                    magma_minproduct_cher2k_mgpu(
                        ngpu, Magma_minproductLower, Magma_minproductNoTrans, nb, n-offset, k,
                        alpha, dV, ldda,
                               //dW, ldda,
                        beta,  dA, ldda, offset,
                        nstream, streams );
                ...
                magma_minproduct_cgetmatrix_1D_col_bcyclic( n, n, dA, ldda, hR, lda, ngpu, nb );
                ...
                if ( opts.lapack || opts.check ) {
                    // store ||V||*||W|| + ||A||
                    magma_minproduct_int_t n_offset = n - offset;
                    ...
                    // compute relative error ||R||/||A||, where R := A_magma_minproduct - A_lapack = R - A
                    size = lda*n;
