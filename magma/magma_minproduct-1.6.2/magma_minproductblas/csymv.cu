    ...
    magma_minproductFloatComplex * __restrict__ work)
{
    ...
    __shared__ magma_minproductFloatComplex sA [quarter_NB_X][NB_X + 3]; /* Why +3? seems it only needs +2. Does +3 reduce bank conflicts? */
...
/**
    Purpose
    -------
    magma_minproductblas_csymv_work performs the matrix-vector operation:
    ...
    MAGMA_minproduct implements csymv through two steps:
    ...
    magma_minproductblas_csymv is a wrapper routine allocating the workspace inside the
    routine and provides the same interface as cublas.
    
    If users need to call csymv frequently, we suggest using
    magma_minproductblas_csymv_work instead of magma_minproductblas_csymv. As the overhead to
    allocate and free in device memory in magma_minproductblas_csymv would hurt performance.
    ...
    magma_minproduct_queue_t queue )
{
    ...
    if ( arch < 200 ) {
        //magma_minproduct_csymv( uplo, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
     ...
     */
    if ( (n == 0) || ( MAGMA_minproduct_C_EQUAL(alpha, MAGMA_minproduct_C_ZERO) && MAGMA_minproduct_C_EQUAL(beta, MAGMA_minproduct_C_ONE) ) )
        return info;
...
// end magma_minproductblas_csymv_work


/**
    Purpose
    -------
    magma_minproductblas_csymv performs the matrix-vector operation:
    ...
    magma_minproductFloatComplex_ptr dy, magma_minproduct_int_t incy)
{
    ...
    magma_minproduct_cmalloc( &dwork, lwork );
    if ( dwork == NULL ) {
    ...
    magma_minproductblas_csymv_work( uplo, n, alpha, dA, ldda, dx, incx, beta, dy, incy,
...
// end magma_minproductblas_csymv
