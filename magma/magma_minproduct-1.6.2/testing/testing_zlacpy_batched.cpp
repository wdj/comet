...
int main( int argc, char** argv)
{
    ...
    magma_minproductDoubleComplex *h_A, *h_B;
    magma_minproductDoubleComplex_ptr d_A, d_B;
    magma_minproductDoubleComplex **hAarray, **hBarray, **dAarray, **dBarray;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( hAarray, magma_minproductDoubleComplex*, ntile );
            TESTING_MALLOC_CPU( hBarray, magma_minproductDoubleComplex*, ntile );
            TESTING_MALLOC_DEV( dAarray, magma_minproductDoubleComplex*, ntile );
            TESTING_MALLOC_DEV( dBarray, magma_minproductDoubleComplex*, ntile );
            ...
            magma_minproduct_setvector( ntile, sizeof(magma_minproductDoubleComplex*), hAarray, 1, dAarray, 1 );
            magma_minproduct_setvector( ntile, sizeof(magma_minproductDoubleComplex*), hBarray, 1, dBarray, 1 );
            
            gpu_time = magma_minproduct_sync_wtime( 0 );
            magma_minproductblas_zlacpy_batched( Magma_minproductUpperLower, mb, nb, dAarray, ldda, dBarray, ldda, ntile, queue );
            ...
            for( int tile = 0; tile < ntile; ++tile ) {
                ...
                lapackf77_zlacpy( Magma_minproductUpperLowerStr, &mb, &nb,
                                  &h_A[offset], &lda,
                                  &h_B[offset], &lda );
            }
