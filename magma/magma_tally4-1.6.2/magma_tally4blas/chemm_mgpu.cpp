...
extern "C"
void magma_tally4blas_chemm_mgpu_com(
    ...
    magma_tally4FloatComplex *work[],     magma_tally4_int_t worksiz,  // TODO unused
    ...
    magma_tally4_int_t gnode[Magma_tally4MaxGPUs][Magma_tally4MaxGPUs+2], magma_tally4_int_t nbcmplx )
{
    ...
    magma_tally4FloatComplex_ptr dwork2[Magma_tally4MaxGPUs];
    ...
    for( magma_tally4_int_t dev = 0; dev < ngpu; ++dev ) {
        ...
        cudaMemset(dwork(dev,0,0), 0, (lddwork)*(n)*sizeof(magma_tally4FloatComplex) );
        // put all dC on all dev to 0 except the one which
        // hold i==0 because this one has to multiply by beta.
        if(dev!=stdev){
           cudaMemset(dC(dev,0,0), 0, (lddc)*(n)*sizeof(magma_tally4FloatComplex) );
        }
    }
    ...
    if(blockoffset>0){
        ...
        magma_tally4blas_csymmetrize_tiles(  Magma_tally4Lower,  fstblksiz,  dA(stdev, offset, myblkoffst*nb+blockoffset),  ldda,  1,  ngpu*nb,  nb  );         
    }
    ...
    for( magma_tally4_int_t dev = 0; dev < ngpu; ++dev ) {
        ...
        magma_tally4_setdevice( dev );
        magma_tally4blasSetKernelStream( queues[ dev ][ 0 ] );
        magma_tally4blas_csymmetrize_tiles(  Magma_tally4Lower,  nb,  dA(dev, newoffset+devperm*nb, myblkoffst*nb),  ldda,  myblk,  ngpu*nb,  nb  );
        if(remm%nb>0){
            ...
            if(devperm==devlstblk)
                magma_tally4blas_csymmetrize(  Magma_tally4Lower,  remm % nb,  dA(dev, newoffset+nbblk*nb, myblkoffst*nb+ myblk*nb),  ldda );  // last partial tile
        }
    }
...
/*
    ...
    magma_tally4_cmalloc_cpu( &R, siz*siz );
    // collecte back A
    magma_tally4blas_cgetmatrix_1D_bcyclic( siz, siz, dA, ldda, R, siz, ngpu, nb );
    magma_tally4_setdevice( 0 );
    magma_tally4blasSetKernelStream( queues[ dev ][ 0 ] );
    //magma_tally4_cgetmatrix( siz, siz, dA[0], ldda, R, siz );
    FILE *trace_file;
...
*/
        ...
        for( magma_tally4_int_t i = fstblksiz; i < m; i += nb ) {
            ...
            for( magma_tally4_int_t dev = 0; dev < ngpu; ++dev ) {
                ...
                if(myrowsize>0){
                    ...
                    magma_tally4_cgemm( Magma_tally4ConjTrans, Magma_tally4NoTrans, myrowsize, n, ib,
    ...
    if(blockoffset>0){
        ...
        magma_tally4_cgemm( Magma_tally4NoTrans, Magma_tally4NoTrans, m, n, ib,
    ...
    for( magma_tally4_int_t i = fstblksiz; i < m; i += nb ) {
        ...
        magma_tally4blasSetKernelStream( queues[ dev ][ 0 ] );
        if(i==0){
           magma_tally4_cgemm( Magma_tally4NoTrans, Magma_tally4NoTrans, m-i, n, ib,
        ...
        if(ngpu==1){
            ...
            // magma_tally4blasSetKernelStream( queues[ dev ][ 0 ] );
            magma_tally4_cgemm( Magma_tally4ConjTrans, Magma_tally4NoTrans, i, n, ib,
        ...
        for( magma_tally4_int_t dev = 0; dev < ngpu; ++dev ) {
            ...
            if(myrowsize>0){
                ...
                for( magma_tally4_int_t blki = 0; blki < myblk; ++blki){
                    ...
                    magma_tally4blas_cgeadd(ib, n, c_one, 
            ...
            for( magma_tally4_int_t idev = 0; idev < myngpu; ++idev ) {
                ...
                if(mycolsize>0){
                    ...
                    if(dev!=masterdev){
                        ...
                        magma_tally4_ccopymatrix_async(
                            m, n,
                            &dC[dev][0], lddc,
        ...
        if(masterdev!=-1){ 
            ...
            for( magma_tally4_int_t l = 0; l < myngpu; ++l ) {
                ...
                if((lcdev!=masterdev)&&(lccolsize>0)){
                    ...
                    magma_tally4blas_cgeadd(m, n, c_one, 
