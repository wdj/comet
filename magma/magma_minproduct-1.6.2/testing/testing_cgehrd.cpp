/*
    -- MAGMA_minproduct (version 1.6.1) --
       ...
       @generated from testing_zgehrd.cpp normal z -> c, Fri Jan 30 19:00:26 2015
...
int main( int argc, char** argv)
{
    ...
    magma_minproductFloatComplex *h_A, *h_R, *h_Q, *h_work, *tau, *twork;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_minproduct_cgehrd( N, ione, N, h_R, lda, tau, h_work, lwork, dT, &info);
            gpu_time = magma_minproduct_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_minproduct_cgehrd returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ltwork = 2*(N*N);
                TESTING_MALLOC_PIN( h_Q,   magma_minproductFloatComplex, lda*N  );
                TESTING_MALLOC_CPU( twork, magma_minproductFloatComplex, ltwork );
                #if defined(PRECISION_z) || defined(PRECISION_c)
                TESTING_MALLOC_CPU( rwork, float, N );
                #endif
                
                lapackf77_clacpy(Magma_minproductUpperLowerStr, &N, &N, h_R, &lda, h_Q, &lda);
                for( int j = 0; j < N-1; ++j )
                    for( int i = j+2; i < N; ++i )
                        h_R[i+j*lda] = MAGMA_minproduct_C_ZERO;
                
                magma_minproduct_cunghr(N, ione, N, h_Q, lda, tau, dT, nb, &info);
                if (info != 0) {
                    printf("magma_minproduct_cunghr returned error %d: %s.\n",
                           (int) info, magma_minproduct_strerror( info ));
                    exit(1);
                }
                #if defined(PRECISION_z) || defined(PRECISION_c)
