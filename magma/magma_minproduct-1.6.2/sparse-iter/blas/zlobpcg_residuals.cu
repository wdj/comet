...
// copied from dznrm2.cu in trunk/magma_minproductblas
// ----------------------------------------
...
__global__ void
magma_minproduct_zlobpcg_res_kernel( 
    ...
    if( row<num_rows){
        for( int i=0; i<num_vecs; i++ ){ 
            ...
            R[row + i*num_rows] = R[row + i*num_rows] 
                                    + MAGMA_minproduct_Z_MAKE( -evals[i], 0.0 )
                                                * X[ row + i*num_rows ];
        }
    }
}
...
/*
magma_minproductblas_dznrm2_kernel( 
    int m, 
    ...
    magma_minproduct_queue_t queue )
{
    ...
    magma_minproduct_zlobpcg_res_kernel<<< grid, threads, 0, queue >>>
