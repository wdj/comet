...
/**
    ...
    @ingroup magma_minproduct_sgesvd_aux
    ********************************************************************/
    ...
    magma_minproductFloat_ptr dY, magma_minproduct_int_t lddy)
{
    ...
    magma_minproduct_smalloc_cpu( &f, max(n,m) );
    if ( f == NULL ) {
    ...
    if (m >= n) {
        ...
        for (i = 1; i <= nb; ++i) {
            ...
            d[i] = MAGMA_minproduct_S_REAL( alpha );
            if (i < n) {
                *A(i,i) = c_one;
                ...
                magma_minproduct_ssetvector( i__2,
                                  A(i,i), 1,
                                  dA(i-1,i-1), 1 );
                // 2. Multiply ---------------------------------------------
                magma_minproduct_sgemv( Magma_minproductConjTrans, i__2, i__3, c_one,
                             dA(i-1,i),   ldda,
                             dA(i-1,i-1), ione, c_zero,
                             dY(i+1,i),   ione );
                
                // 3. Put the result back ----------------------------------
                magma_minproduct_sgetmatrix_async( i__3, 1,
                                        dY(i+1,i), lddy,
                                        Y(i+1,i),  ldy, stream );
                i__2 = m - i + 1;
                i__3 = i - 1;
                blasf77_sgemv( Magma_minproductConjTransStr, &i__2, &i__3, &c_one,
                               A(i,1), &lda,
                               A(i,i), &ione, &c_zero,
                               Y(1,i), &ione );
                ...
                i__2 = i - 1;
                i__3 = n - i;
                blasf77_sgemv( Magma_minproductConjTransStr, &i__2, &i__3, &c_neg_one,
                               A(1,i+1), &lda,
                               Y(1,i),   &ione, &c_one,
                               Y(i+1,i), &ione );
                i__2 = n - i;
                ...
                *A(i,i+1) = c_one;
                ...
                i__3 = n - i;
                // 1. Send the block reflector  A(i+1:m,i) to the GPU ------
                magma_minproduct_ssetvector( i__3,
                                  A(i,i+1), lda,
                                  dA(i-1,i), ldda );
                // 2. Multiply ---------------------------------------------
                //magma_minproduct_scopy( i__3, dA(i-1,i), ldda, dY(1,1), 1 );
                magma_minproduct_sgemv( Magma_minproductNoTrans, i__2, i__3, c_one,
                             dA(i,i), ldda,
                             dA(i-1,i), ldda,
                             //dY(1,1), 1,
                             c_zero,
                ...
                // 3. Put the result back ----------------------------------
                magma_minproduct_sgetmatrix_async( i__2, 1,
                                        dX(i+1,i), lddx,
                                        X(i+1,i),  ldx, stream );

                i__2 = n - i;
                blasf77_sgemv( Magma_minproductConjTransStr, &i__2, &i, &c_one,
                               Y(i+1,1), &ldy,
                               A(i,i+1), &lda, &c_zero,
                               X(1,i),   &ione );
        ...
        for (i = 1; i <= nb; ++i) {
            ...
            if (i < m) {
                *A(i,i) = c_one;
                ...
                // 2. Multiply ---------------------------------------------
                //magma_minproduct_scopy( i__3, dA(i-1,i-1), ldda, dY(1,1), 1 );
                magma_minproduct_sgemv( Magma_minproductNoTrans, i__2, i__3, c_one,
                             dA(i,i-1), ldda,
                             dA(i-1,i-1), ldda,
                             //dY(1,1), 1,
                ...
                *A(i+1,i) = c_one;
                ...
                i__2 = n - i;
                blasf77_sgemv( Magma_minproductConjTransStr, &i, &i__2, &c_neg_one,
                               A(1,i+1), &lda,
                               Y(1,i),   &ione, &c_one,
                               Y(i+1,i), &ione );
                i__2 = n - i;
