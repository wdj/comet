...
dtrtri_diag_lower_device(
    magma_minproduct_diag_t diag, int n, const double *A, int lda, double *d_dinvA)
{
    int tx = threadIdx.x;
    int bx = blockIdx.x;
    int blk_ind = bx*IB;
    ...
    for( int j=0; j < IB; j++ ) {
        if (tx >= j && ind < n) {
            ...
            sB[tx + j*IB] = MAGMA_minproduct_D_ZERO;
        }
    }
    __syncthreads();
    ...
    if (diag == Magma_minproductUnit) {
        sB[tx + tx*IB] = MAGMA_minproduct_D_ONE;
    }
    else {
        if ( sB[tx + tx*IB] == MAGMA_minproduct_D_ZERO ) {  // singular or outside matrix
            sB[tx + tx*IB] = MAGMA_minproduct_D_ONE;
        }
        else {
            sB[tx + tx*IB] = MAGMA_minproduct_D_ONE / sB[tx + tx*IB];
        }
    }
    ...
    int n, const double *Ain, int lda, double *d_dinvA, int jb, int npages)
{
    ...
    //--------------------------part three---------------------------//
    {
        ...
        #pragma unroll
        for( int i = 0; i < 16; i++ ) {
            B12[i*ldb] = MAGMA_minproduct_D_ZERO;
        }
    }
}
...
dtrtri_diag_lower_kernel_batched(
    magma_minproduct_diag_t diag, int n, double const * const * dA_array, int lda, double **dinvA_array)
{
