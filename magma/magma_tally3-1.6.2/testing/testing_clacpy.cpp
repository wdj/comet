      ...
      for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( uplo[iuplo] == Magma_tally3Lower || uplo[iuplo] == Magma_tally3Upper ) {
                ...
                gbytes = sizeof(magma_tally3FloatComplex) * 1.*N*(N+1) / 1e9;
            }
            else {
            ...
            TESTING_MALLOC_CPU( h_B, magma_tally3FloatComplex, size   );
            TESTING_MALLOC_CPU( h_R, magma_tally3FloatComplex, size   );
            
            TESTING_MALLOC_DEV( d_A, magma_tally3FloatComplex, ldda*N );
            TESTING_MALLOC_DEV( d_B, magma_tally3FloatComplex, ldda*N );
            
            /* Initialize the matrix */
            for( int j = 0; j < N; ++j ) {
                for( int i = 0; i < M; ++i ) {
                    h_A[i + j*lda] = MAGMA_tally3_C_MAKE( i + j/10000., j );
                    h_B[i + j*ldb] = MAGMA_tally3_C_MAKE( i - j/10000. + 10000., j );
                }
            }
            ...
            //magma_tally3blas_clacpy( uplo[iuplo], M-2, N-2, d_A+1+ldda, ldda, d_B+1+lddb, lddb );  // inset by 1 row & col
            magma_tally3blas_clacpy( uplo[iuplo], M, N, d_A, ldda, d_B, lddb );
            ...
            //lapackf77_clacpy( lapack_uplo_const_tally3(uplo[iuplo]), &M2, &N2, h_A+1+lda, &lda, h_B+1+ldb, &ldb );
            lapackf77_clacpy( lapack_uplo_const_tally3(uplo[iuplo]), &M, &N, h_A, &lda, h_B, &ldb );
            cpu_time = magma_tally3_wtime() - cpu_time;
            cpu_perf = gbytes / cpu_time;
            
            if ( opts.verbose ) {
                printf( "A= " );  magma_tally3_cprint(     M, N, h_A, lda );
                printf( "B= " );  magma_tally3_cprint(     M, N, h_B, lda );
                printf( "dA=" );  magma_tally3_cprint_gpu( M, N, d_A, ldda );
                printf( "dB=" );  magma_tally3_cprint_gpu( M, N, d_B, ldda );
            }
            
            /* =====================================================================
               Check the result
               =================================================================== */
            magma_tally3_cgetmatrix( M, N, d_B, ldda, h_R, lda );
