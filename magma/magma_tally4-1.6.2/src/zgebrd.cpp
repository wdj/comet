...
/**
    ...
    @ingroup magma_tally4_zgesvd_comp
    ********************************************************************/
...
#define dA(i, j) (dA + (j)*ldda + (i))
    ...
    magma_tally4DoubleComplex *dA, *dwork;
    ...
    *info = 0;
    ...
    if (MAGMA_tally4_SUCCESS != magma_tally4_zmalloc( &dA, n*ldda + (m + n)*nb )) {
    ...
    for (i=0; i < (minmn - nx); i += nb) {
        ...
        if ( i > 0 ) {
            magma_tally4_zgetmatrix( nrow, nb, dA(i, i), ldda, A( i, i), lda );
            magma_tally4_zgetmatrix( nb, ncol - nb,
                              dA(i, i+nb), ldda,
                              A( i, i+nb), lda );
        }

        magma_tally4_zlabrd_gpu(nrow, ncol, nb,
        ...
        magma_tally4_zsetmatrix( nrow, nb, work  + nb, ldwrkx, dwork + nb, ldwrkx );
        magma_tally4_zsetmatrix( ncol, nb,
        ...
        if (m >= n) {
            jmax = i + nb;
            for (j = i; j < jmax; ++j) {
                *A(j, j  ) = MAGMA_tally4_Z_MAKE( d[j], 0. );
                *A(j, j+1) = MAGMA_tally4_Z_MAKE( e[j], 0. );
            }
        } else {
            jmax = i + nb;
            for (j = i; j < jmax; ++j) {
    ...
    if ( 0 < minmn - nx ) {
        magma_tally4_zgetmatrix( nrow, ncol, dA(i, i), ldda, A(i, i), lda );
    }
