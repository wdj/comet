...
#define dA(i, j) (dwork + (i) + (j)*(ldda))
    ...
    magma_tally4FloatComplex   *dwork, *df;

    magma_tally4_int_t ione = 1;

    magma_tally4_int_t n_j, ldda, ldwork;
    magma_tally4_int_t j, jb, na, nb, sm, sn, fjb, nfxd, minmn;
    magma_tally4_int_t topbmn, sminmn, lwkopt=0, lquery;
    ...
    *info = 0;
    ...
    if (MAGMA_tally4_SUCCESS != magma_tally4_cmalloc( &dwork, ldwork )) {
    ...
    if (nfxd < minmn) {
        ...
        if (nb < sminmn) {
            ...
            magma_tally4_csetmatrix_async( m, sn,
                                    A (0,j), lda,
                                    dA(0,j), ldda, stream );
        }
        ...
        for (j = nfxd; j < n; ++j) {
            rwork[j] = magma_tally4_cblas_scnrm2( sm, A(nfxd,j), ione );
            rwork[n + j] = rwork[j];
        }
        ...
        if (nb < sminmn) {
            ...
            while(j < topbmn) {
                ...
                if (j > nfxd) {
                    ...
                    magma_tally4_cgetmatrix( m-j, jb,
                                      dA(j,j), ldda,
                                      A (j,j), lda );
                    
                    // Get the rows
                    magma_tally4_cgetmatrix( jb, n_j - jb,
                                      dA(j,j + jb), ldda,
                                      A (j,j + jb), lda );
                }

                magma_tally4_claqps( m, n_j, j, jb, &fjb,
                              A (0, j), lda,
                              dA(0, j), ldda,
