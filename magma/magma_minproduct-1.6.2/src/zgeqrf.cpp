...
/**
    ...
    lwork   INTEGER
            ...
            where NB can be obtained through magma_minproduct_get_zgeqrf_nb(M).
    \n
...
magma_minproduct_zgeqrf(
    ...
    magma_minproductDoubleComplex *A,    magma_minproduct_int_t lda, magma_minproductDoubleComplex *tau,
    ...
    magma_minproduct_int_t *info )
{
    ...
    magma_minproductDoubleComplex *dA, *dwork, *dT;
    magma_minproductDoubleComplex c_one = MAGMA_minproduct_Z_ONE;

    magma_minproduct_int_t i, k, lddwork, old_i, old_ib;
    magma_minproduct_int_t ib, ldda;

    /* Function Body */
    *info = 0;
    magma_minproduct_int_t nb = magma_minproduct_get_zgeqrf_nb(min(m, n));

    // need 2*nb*nb to store T and upper triangle of V simultaneously
    magma_minproduct_int_t lwkopt = max(n*nb, 2*nb*nb);
    ...
    if ( ngpu > 1 ) {
        ...
        return magma_minproduct_zgeqrf4(ngpu, m, n, A, lda, tau, work, lwork, info);
    }

    // allocate space for dA, dwork, and dT
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_zmalloc( &dA, n*ldda + nb*lddwork + nb*nb )) {
        /* Switch to the "out-of-core" (out of GPU-memory) version */
        return magma_minproduct_zgeqrf_ooc(m, n, A, lda, tau, work, lwork, info);
    }
    ...
    if ( (nb > 1) && (nb < k) ) {
        ...
        magma_minproduct_zsetmatrix_async( m, n-nb,
                                A(0,nb),  lda,
        ...
        for (i = 0; i < k-nb; i += nb) {
            ...
            if (i > 0) {
                ...
                magma_minproduct_zgetmatrix_async( m-i, ib,
                                        dA(i,i), ldda,
                                        A(i,i),  lda, stream[0] );
                ...
                magma_minproduct_zgetmatrix_async( i, ib,
                                        dA(0,i), ldda,
                                        A(0,i),  lda, stream[1] );
                magma_minproduct_queue_sync( stream[0] );
            }

            magma_minproduct_int_t rows = m-i;
            ...
            zpanel_to_q(Magma_minproductUpper, ib, A(i,i), lda, work+ib*ib);

            /* download the i-th V matrix */
            magma_minproduct_zsetmatrix_async( rows, ib, A(i,i), lda, dA(i,i), ldda, stream[0] );

            /* download the T matrix */
            magma_minproduct_queue_sync( stream[1] );
            magma_minproduct_zsetmatrix_async( ib, ib, work, ib, dT, nb, stream[0] );
            magma_minproduct_queue_sync( stream[0] );
            ...
            if (i + ib < n) {
                if (i+ib < k-nb) {
                    ...
                    zq_to_panel(Magma_minproductUpper, ib, A(i,i), lda, work+ib*ib);
                }
                else {
    ...
    if (i < k) {
        ib = n-i;
        if (i != 0) {
            magma_minproduct_zgetmatrix_async( m, ib, dA(0,i), ldda, A(0,i), lda, stream[1] );
            magma_minproduct_queue_sync( stream[1] );
        }
        magma_minproduct_int_t rows = m-i;
        lapackf77_zgeqrf(&rows, &ib, A(i,i), &lda, tau+i, work, &lwork, info);
