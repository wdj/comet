...
double get_LU_error(magma_minproduct_int_t M, magma_minproduct_int_t N,
                    ...
                    magma_minproductDoubleComplex *LU, magma_minproduct_int_t *IPIV)
{
    ...
    magma_minproductDoubleComplex *L, *U;
...
int main( int argc, char** argv)
{
    ...
    magma_minproductDoubleComplex *dA_magma_minproduct;
    ...
    for( int i = 0; i < opts.ntest; ++i ) {
      ...
      for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_DEV(  dA_magma_minproduct,  magma_minproductDoubleComplex, ldda*N * batchCount);
            ...
            magma_minproduct_zsetmatrix( M, columns, h_R, lda, dA_magma_minproduct, ldda );
            ...
            /* ====================================================================
               ...
               =================================================================== */
            zset_pointer(dA_array, dA_magma_minproduct, ldda, 0, 0, ldda*N, batchCount, queue);
            magma_minproduct_time = magma_minproduct_sync_wtime(0);
            info = magma_minproduct_zgetrf_nopiv_batched( M, N, dA_array, ldda, dinfo_magma_minproduct, batchCount, queue);
            ...
            {
                if(cpu_info[i] != 0 ){
                    printf("magma_minproduct_zgetrf_batched matrix %d returned internal error %d\n",i, (int)cpu_info[i] );
                }
            }
            if (info != 0)
                printf("magma_minproduct_zgetrf_batched returned argument error %d: %s.\n", (int) info, magma_minproduct_strerror( info ));
            ...
            if ( opts.check ) {
                ...
                magma_minproduct_zgetmatrix( M, N*batchCount, dA_magma_minproduct, ldda, h_A, lda );
                for(int i=0; i<batchCount; i++)
                {
