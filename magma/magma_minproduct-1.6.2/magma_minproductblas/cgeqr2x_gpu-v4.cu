/*
    -- MAGMA_minproduct (version 1.6.1) --
       ...
       @generated from zgeqr2x_gpu-v4.cu normal z -> c, Fri Jan 30 19:00:10 2015
    ...
    magma_minproduct_int_t *info)
{
    ...
    magma_minproductFloatComplex *work = (magma_minproductFloatComplex *)(dwork+2*n);
    ...
    *info = 0;
    ...
    k = min(m,n);
    magma_minproductblas_scnrm2_cols(m, k, dA(0,0), ldda, dnorm);
    ...
    for (magma_minproduct_int_t b=0; b < k; b += BS) {
        for (i = b; i < min(k, b+BS); ++i) {
            ...
            if (i-b > 0) {
                ...
                if ( i-1 > 0 ) {
                    magma_minproduct_cgemv_kernel3<<< i-1, BLOCK_SIZE, 0, magma_minproduct_stream >>>
                        ( m-i+1, dA(i-1,0), ldda, dA(i-1, i-1), work, dtau+i-1);
                    magma_minproduct_ctrmv_kernel2<<< i-1, i-1, 0, magma_minproduct_stream >>>
                        ( dT(0,0), k, work, dT(0,i-1), dtau+i-1);
                }

                /* dwork = V**H c */
                magma_minproduct_cgemv_kernel1<<< i-b, BLOCK_SIZE, 0, magma_minproduct_stream >>>
                    (m-b, dA(b, b),  ldda, dA(b,i), work);

                /* dwork = T**H work */
                magma_minproduct_ctrmv_tkernel<<< i-b, i-b, 0, magma_minproduct_stream >>>
                ...
                if ( m-b > 0 ) {
                    ...
                    magma_minproduct_cgemv_kernel2<<< blocks3, threads3, 0, magma_minproduct_stream >>>
            ...
            if ( i > 0 ) {
                magma_minproduct_scnrm2_adjust_kernel<<< 1, i, 0, magma_minproduct_stream >>>(dnorm+i, dA(0, i));
            }
            ...
            /*  Generate elementary reflector H(i) to annihilate A(i+1:m,i)
                ...
                3. update T */
            magma_minproduct_clarfgx_gpu(m-i, dA(i, i), dA(min(i+1,m),i), dtau+i,
                              dnorm+i, ddA + i + i*n, i);

            if (i == 0) {
                magma_minproductFloatComplex tt = MAGMA_minproduct_C_ONE;
                magma_minproductblas_clacpy(Magma_minproductUpperLower, 1, 1, dtau, 1, dT(0,0), 1);
                magma_minproduct_csetmatrix_async(1, 1, &tt, 1, dA(i, i), 1, magma_minproduct_stream);
            }
        }
        if ( i-1 > 0 ) {
            magma_minproduct_cgemv_kernel3<<< i-1, BLOCK_SIZE, 0, magma_minproduct_stream >>>
        ...
        //magma_minproduct_clarfb2_gpu( Magma_minproductLeft, Magma_minproductConjTrans, Magma_minproductForward, Magma_minproductColumnwise,
