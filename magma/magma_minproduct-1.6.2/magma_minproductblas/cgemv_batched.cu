...
kernel_cgemvn_batched(
    ...
    magma_minproductFloatComplex **x_array, int incx,
    magma_minproductFloatComplex beta, magma_minproductFloatComplex  **y_array, int incy)
{

    magma_minproductFloatComplex *A = dA_array[blockIdx.x];
    magma_minproductFloatComplex *x = x_array[blockIdx.x];
    magma_minproductFloatComplex *y = y_array[blockIdx.x];

    int tx = threadIdx.x;

    magma_minproductFloatComplex res = MAGMA_minproduct_C_ZERO;

    magma_minproductFloatComplex *buff = (magma_minproductFloatComplex*)shared_data;
...
*/
extern "C"
void magma_minproductblas_cgemvn_batched(
    int m, int n, 
    ...
    int batchCount, magma_minproduct_queue_t queue)
{
    ...
    if( m > 512 || n > 512)
    {
        fprintf( stderr, "m=%d, n=%d, cgemv_batched nontranspose assume row && column lower than %d. Plz call magma_minproductblas_cgemv instead", m, n, 512);
        return ;
    }
    ...
    kernel_cgemvn_batched<<< grid, threads, n * sizeof(magma_minproductFloatComplex), queue >>>( m, n, alpha,  dA_array, lda, x_array, incx,  
                                                                         beta, y_array, incy);
}
...
kernel_cgemvt_batched(
    int m, int n, int m1, magma_minproductFloatComplex alpha,
    magma_minproductFloatComplex **dA_array, int lda,
    magma_minproductFloatComplex **x_array, int incx,
    magma_minproductFloatComplex beta, magma_minproductFloatComplex  **y_array, int incy)
{
    ...
    magma_minproductFloatComplex *A_ptr = dA_array[blockIdx.x];
    magma_minproductFloatComplex *x_ptr = x_array[blockIdx.x];
    magma_minproductFloatComplex *y_ptr = y_array[blockIdx.x];
    ...
    __shared__ magma_minproductFloatComplex sdata[cgemv_bs];
...
extern "C"
void magma_minproductblas_cgemvt_batched(
    int m, int n, 
    magma_minproductFloatComplex alpha, magma_minproductFloatComplex **dA_array, int lda, 
    magma_minproductFloatComplex **x_array,  int incx,
    magma_minproductFloatComplex beta, magma_minproductFloatComplex **y_array,  int incy, 
    ...
    magma_minproductFloatComplex beta, magma_minproductFloatComplex  **y_array, int incy)
{
    ...
    for(int i=0; i<m1; i+= cgemv_bs)
    {
        res += MAGMA_minproduct_C_CNJG (A_ptr[i]) * x_ptr[i*incx];
    }
    ...
    if(m > m1)
    {
        if( tx + m1 <  m )
        {
            res  += MAGMA_minproduct_C_CNJG(A_ptr[m1]) * x_ptr[m1*incx];
        }
        else
        {
            res  = res;
...
extern "C"
void magma_minproductblas_cgemvc_batched(
    int m, int n, 
    magma_minproductFloatComplex alpha, magma_minproductFloatComplex **dA_array, int lda, 
    magma_minproductFloatComplex **x_array,  int incx,
    magma_minproductFloatComplex beta, magma_minproductFloatComplex **y_array,  int incy, 
...
extern "C"
void magma_minproductblas_cgemv_batched(
...
{       
    ...
    if ( trans == Magma_minproductNoTrans ) {
        ...
        magma_minproductblas_cgemvn_batched(m, n, alpha, dA_array, ldda, dx_array, incx, beta, dy_array, incy, batchCount, queue);
            
    }
    else if ( trans == Magma_minproductTrans ) {
        magma_minproductblas_cgemvt_batched(m, n, alpha, dA_array, ldda, dx_array, incx, beta, dy_array, incy, batchCount, queue);
    }
    else if ( trans == Magma_minproductConjTrans ) {
#if defined(PRECISION_z) || defined (PRECISION_c)
        magma_minproductblas_cgemvc_batched(m, n, alpha, dA_array, ldda, dx_array, incx, beta, dy_array, incy, batchCount, queue);
#else
        magma_minproductblas_cgemvt_batched(m, n, alpha, dA_array, ldda, dx_array, incx, beta, dy_array, incy, batchCount, queue);
#endif
    }
