...
void init_matrix( int nopiv, int m, int n, magma_tally3DoubleComplex *h_A, magma_tally3_int_t lda )
{
    assert( m == n );
    magma_tally3_int_t ione = 1;
    magma_tally3_int_t ISEED[4] = {0,0,0,1};
    magma_tally3_int_t n2 = lda*n;
    lapackf77_zlarnv( &ione, ISEED, &n2, h_A );
    if (nopiv) {
        magma_tally3_zmake_hpd( n, h_A, lda );
    }
    else {
        magma_tally3_zmake_hermitian( n, h_A, lda );
    }
}
...
double get_residual(
    int nopiv, magma_tally3_uplo_t uplo, magma_tally3_int_t n,
    ...
    magma_tally3_int_t *ipiv )
{
    ...
    magma_tally3_int_t ISEED[4] = {0,0,0,2};
    magma_tally3_int_t info = 0;
    magma_tally3DoubleComplex *x, *b;
    
    // initialize RHS
    TESTING_MALLOC_CPU( x, magma_tally3DoubleComplex, n );
    TESTING_MALLOC_CPU( b, magma_tally3DoubleComplex, n );
    ...
    if (nopiv) {
        if (upper) {
            blasf77_ztrsm( Magma_tally3LeftStr, Magma_tally3UpperStr, 
                           Magma_tally3ConjTransStr, Magma_tally3UnitStr, 
                           &n, &ione, &c_one,
                           A, &lda, x, &n );
            for (int i=0; i<n; i++) x[i] = MAGMA_tally3_Z_DIV( x[i], A[i+i*lda] );
            blasf77_ztrsm( Magma_tally3LeftStr, Magma_tally3UpperStr, 
                           Magma_tally3NoTransStr, Magma_tally3UnitStr, 
                           &n, &ione, &c_one,
                           A, &lda, x, &n );
        } else {
            blasf77_ztrsm( Magma_tally3LeftStr, Magma_tally3LowerStr, 
                           Magma_tally3NoTransStr, Magma_tally3UnitStr, 
                           &n, &ione, &c_one,
                           A, &lda, x, &n );
            for (int i=0; i<n; i++) x[i] = MAGMA_tally3_Z_DIV( x[i], A[i+i*lda] );
        ...
        lapackf77_zhetrs( lapack_uplo_const_tally3(uplo), &n, &ione, A, &lda, ipiv, x, &n, &info );
    }
    if (info != 0)
    ...
    norm_A = lapackf77_zlange( Magma_tally3FullStr, &n, &n, A, &lda, work );
    norm_r = lapackf77_zlange( Magma_tally3FullStr, &n, &ione, b, &n, work );
    norm_x = lapackf77_zlange( Magma_tally3FullStr, &n, &ione, x, &n, work );
    
    //printf( "r=\n" ); magma_tally3_zprint( 1, n, b, 1 );
...
double get_LDLt_error(int nopiv, magma_tally3_uplo_t uplo, magma_tally3_int_t N,
                      magma_tally3DoubleComplex *LD, magma_tally3_int_t lda,
                      magma_tally3_int_t *ipiv)
{
    ...
    magma_tally3DoubleComplex *A, *L, *D;
    ...
    TESTING_MALLOC_CPU( A, magma_tally3DoubleComplex, N*N );
    TESTING_MALLOC_CPU( L, magma_tally3DoubleComplex, N*N );
    TESTING_MALLOC_CPU( D, magma_tally3DoubleComplex, N*N );
    memset( L, 0, N*N*sizeof(magma_tally3DoubleComplex) );
    memset( D, 0, N*N*sizeof(magma_tally3DoubleComplex) );
    ...
    if (uplo == Magma_tally3Upper) {
        for (j=N-1; j>=0; j--) {
            ...
            if (piv < 0) {
                ...
                D(j,j)     = LD(j,j);
                D(j,j-1)   = MAGMA_tally3_Z_CNJG(LD(j-1,j));
                D(j-1,j)   = LD(j-1,j);
                ...
                if (piv != j) {
                    ...
                    for (i=j+2; i<N; i++) {
                        magma_tally3DoubleComplex val = L(j,i);
                        L(j,i) = L(piv,i);
                        L(piv,i) = val;
                    }
                    // apply row-pivoting to A
                    for (i=0; i<N; i++) {
                        magma_tally3DoubleComplex val = A(j,i);
                        A(j,i) = A(piv,i);
                        A(piv,i) = val;
                    }
                    // apply col-pivoting to A
                    for (i=0; i<N; i++) {
                        magma_tally3DoubleComplex val = A(i,j);
                        A(i,j) = A(i,piv);
                        A(i,piv) = val;
                    }
                }
        ...
        if (nopiv) {
            // compute W = D*U
            blasf77_zgemm(Magma_tally3NoTransStr, Magma_tally3NoTransStr, &N, &N, &N,
                          &c_one, D, &N, L, &N, &c_zero, LD, &lda);
            // compute D = U'*W
            blasf77_zgemm(Magma_tally3ConjTransStr, Magma_tally3NoTransStr, &N, &N, &N,
            ...
            // compute D = W*U'
            blasf77_zgemm(Magma_tally3NoTransStr, Magma_tally3ConjTransStr, &N, &N, &N,
    ...
    // compute norm of A
    matnorm = lapackf77_zlange(Magma_tally3FullStr, &N, &N, A, &lda, work);

    for( j = 0; j < N; j++ ) {
        for( i = 0; i < N; i++ ) {
            D(i,j) = MAGMA_tally3_Z_SUB( D(i,j), A(i,j) );
        }
    }
    residual = lapackf77_zlange(Magma_tally3FullStr, &N, &N, D, &N, work);
...
int main( int argc, char** argv)
{
    ...
    magma_tally3DoubleComplex *h_A, *work, temp;
    real_Double_t   gflops, gpu_perf, gpu_time = 0.0, cpu_perf=0, cpu_time=0;
    double          error, error_lapack = 0.0;
    magma_tally3_int_t     *ipiv;
    magma_tally3_int_t     N, n2, lda, lwork, info;
    magma_tally3_int_t     status = 0;
    magma_tally3_int_t     cpu = 0, nopiv = 0, nopiv_gpu = 0, row = 0;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_PIN( h_A,  magma_tally3DoubleComplex, n2 );
            ...
            if ( opts.lapack ) {
                lwork = -1;
                lapackf77_zhetrf( lapack_uplo_const_tally3(opts.uplo), &N, h_A, &lda, ipiv, &temp, &lwork, &info);
                lwork = (int)MAGMA_tally3_Z_REAL(temp);
                TESTING_MALLOC_CPU( work, magma_tally3DoubleComplex, lwork );

                init_matrix( nopiv, N, N, h_A, lda );
                cpu_time = magma_tally3_wtime();
                lapackf77_zhetrf( lapack_uplo_const_tally3(opts.uplo), &N, h_A, &lda, ipiv, work, &lwork, &info);
            ...
            if (nopiv) {
                ...
                magma_tally3_zhetrf_nopiv_tally3( opts.uplo, N, h_A, lda, &info);
                gpu_time = magma_tally3_wtime() - gpu_time;
            } else if (cpu) {
                // CPU-interface to Bunch-Kauffman LDLt
                magma_tally3_setdevice(0);
                gpu_time = magma_tally3_wtime();
                magma_tally3_zhetrf( opts.uplo, N, h_A, lda, ipiv, &info);
                gpu_time = magma_tally3_wtime() - gpu_time;
            } else if (nopiv_gpu) {
                // GPU-interface to non-piv LDLt
                magma_tally3_setdevice(0);
                magma_tally3_int_t ldda = 32*((N+31)/32);
                magma_tally3DoubleComplex_ptr d_A;
                if (MAGMA_tally3_SUCCESS != magma_tally3_zmalloc( &d_A, N*ldda  )) {
                    printf( " failed to allocate d_A(%dx%d)\n", (int) N, (int) ldda );
                    return 0;
                }
                magma_tally3_zsetmatrix(N, N, h_A, lda, d_A, ldda);
                gpu_time = magma_tally3_wtime();
                magma_tally3_zhetrf_nopiv_tally3_gpu( opts.uplo, N, d_A, ldda, &info);
                gpu_time = magma_tally3_wtime() - gpu_time;
                magma_tally3_zgetmatrix(N, N, d_A, ldda, h_A, lda);
                magma_tally3_free( d_A );
            } else if (row) {
                magma_tally3_setdevice(0);
                //magma_tally3_zhetrf_gpu_row( opts.uplo, N, h_A, lda, ipiv, work, lwork, &info);
            } else {
                magma_tally3_setdevice(0);
                //magma_tally3_zhetrf_hybrid( opts.uplo, N, h_A, lda, ipiv, work, lwork, &info);
            }
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_tally3_zhetrf returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
