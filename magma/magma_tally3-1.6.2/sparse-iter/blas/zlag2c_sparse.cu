...
__global__ void
magma_tally3int_zlag2c_sparse(
    int M,
    int N,
    const magma_tally3DoubleComplex_ptr A,
    ...
    magma_tally3_queue_t queue )
{
    ...
    *info = 0;
    ...
    dim3 grid( magma_tally3_ceildiv( M, BLOCKSIZE ) );


    cudaMemcpyToSymbol( flag, info, sizeof(flag) );    // flag = 0
    magma_tally3int_zlag2c_sparse<<< grid, BLOCKSIZE, 0, queue >>>
                                        ( M, N, A, SA ) ;
...
__global__ void
magma_tally3_zlag2c_CSR_DENSE_kernel(
    int num_rows,
    int num_cols,
    magma_tally3DoubleComplex_ptr Aval,
    magma_tally3Index_ptr Arow,
    magma_tally3Index_ptr Acol,
    magma_tally3FloatComplex *Bval )
{
    ...
    if(row<num_rows){
        for( j=0; j<num_cols; j++)
            Bval[ j ] = MAGMA_tally3_C_MAKE(0.0, 0.0);
        int start = Arow[ row ];
        int end = Arow[ row+1 ];
        for( j=start; j<end; j++ )
...
__global__ void
magma_tally3_zlag2c_CSR_DENSE_kernel_1(
    int num_rows,
    int num_cols,
    magma_tally3FloatComplex_ptr Bval )
{
...
__global__ void
magma_tally3_zlag2c_CSR_DENSE_kernel_2(
...
magma_tally3_zlag2c_CSR_DENSE(
    ...
    magma_tally3_c_sparse_matrix *B,
    magma_tally3_queue_t queue )
{
    magma_tally3_int_t stat;
    ...
    if ( A.memory_location == Magma_tally3_DEV && A.storage_type == Magma_tally3_CSR) {
        ...
        B->nnz = A.nnz;
        stat = magma_tally3_cmalloc( &B->val, A.num_rows* A.num_cols );
        if ( stat != 0 )
        {printf("Memory Allocation Error converting matrix\n"); return -1;}
        
        dim3 Bs( BLOCKSIZE );
        dim3 Gs( magma_tally3_ceildiv( A.num_rows, BLOCKSIZE ) );

        magma_tally3_zlag2c_CSR_DENSE_kernel<<< Bs, Gs, 0, queue >>>
    ...
    magma_tally3_queue_t queue )
{
    ...
    if ( A.memory_location == Magma_tally3_DEV && A.storage_type == Magma_tally3_CSR) {
        ...
        magma_tally3_zlag2c_CSR_DENSE_kernel_1<<< Bs, Gs, 0, queue >>>
...
magma_tally3_zlag2c_CSR_DENSE_convert(
    ...
    magma_tally3_queue_t queue )
{
    if ( B->memory_location == Magma_tally3_DEV && B->storage_type == Magma_tally3_DENSE) {
        dim3 Bs( BLOCKSIZE );
        dim3 Gs( magma_tally3_ceildiv( A.num_rows, BLOCKSIZE ) );

        magma_tally3_zlag2c_CSR_DENSE_kernel_2<<< Bs, Gs, 0, queue >>>
        ( A.num_rows, A.num_cols, A.dval, A.drow, A.dcol, B->val );
    }
}
