...
int main( int argc, char** argv)
{
    ...
    magma_tally4FloatComplex *h_x, *h_x2, *h_tau, *h_tau2;
    magma_tally4FloatComplex_ptr d_x, d_tau;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_x,    magma_tally4FloatComplex, N*nb );
            TESTING_MALLOC_CPU( h_x2,   magma_tally4FloatComplex, N*nb );
            TESTING_MALLOC_CPU( h_tau,  magma_tally4FloatComplex, nb   );
            TESTING_MALLOC_CPU( h_tau2, magma_tally4FloatComplex, nb   );
        
            TESTING_MALLOC_DEV( d_x,   magma_tally4FloatComplex, ldda*nb );
            TESTING_MALLOC_DEV( d_tau, magma_tally4FloatComplex, nb      );
            ...
            /* =====================================================================
               ...
               =================================================================== */
            magma_tally4_csetmatrix( N, nb, h_x, N, d_x, ldda );
    
            gpu_time = magma_tally4_sync_wtime( queue );
            for( int j = 0; j < nb; ++j ) {
                magma_tally4blas_clarfg( N, &d_x[0+j*ldda], &d_x[1+j*ldda], ione, &d_tau[j] );
            }
            gpu_time = magma_tally4_sync_wtime( queue ) - gpu_time;
            gpu_perf = gflops / gpu_time;
            
            magma_tally4_cgetmatrix( N, nb, d_x, ldda, h_x2, N );
            magma_tally4_cgetvector( nb, d_tau, 1, h_tau2, 1 );
