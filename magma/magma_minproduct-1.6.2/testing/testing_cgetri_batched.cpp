...
int main( int argc, char** argv)
{
    ...
    magma_minproductFloatComplex *d_A, *d_invA;
    magma_minproductFloatComplex **dA_array = NULL;
    magma_minproductFloatComplex **dinvA_array = NULL;
    magma_minproductFloatComplex **C_array = NULL;
    magma_minproduct_int_t  **dipiv_array = NULL;
    magma_minproduct_int_t *dinfo_array = NULL;

    magma_minproduct_int_t     *ipiv;
    magma_minproduct_int_t     *d_ipiv, *d_info;
    magma_minproduct_int_t M, N, n2, lda, ldda, min_mn, info, info1, info2;
    ...
    magma_minproduct_int_t batchCount = opts.batchcount ;
    ...
    for( int i = 0; i < opts.ntest; ++i ) {
      ...
      for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            n2     = lda*N * batchCount;
            ldda   = ((M+31)/32)*32;
            //gflops = (FLOPS_CGETRF( M, N ) + FLOPS_CGETRI( min(M,N) ))/ 1e9 * batchCount; // This is the correct flops but since this getri_batched is based on 2 trsm = getrs and to know the real flops I am using the getrs one
            gflops = (FLOPS_CGETRF( M, N ) + FLOPS_CGETRS( min(M,N), min(M,N) ))/ 1e9 * batchCount;
            ...
            TESTING_MALLOC_DEV(  d_invA,  magma_minproductFloatComplex, ldda*N * batchCount);
            TESTING_MALLOC_DEV(  d_ipiv,  magma_minproduct_int_t, min_mn * batchCount);
            TESTING_MALLOC_DEV(  d_info,  magma_minproduct_int_t, batchCount);
            ...
            magma_minproduct_malloc((void**)&dinfo_array, batchCount * sizeof(magma_minproduct_int_t));
            ...
            lapackf77_clacpy( Magma_minproductUpperLowerStr, &M, &columns, h_A, &lda, h_R, &lda );
            magma_minproduct_csetmatrix( M, columns, h_R, lda, d_A, ldda );
            ...
            info1 = magma_minproduct_cgetrf_batched( M, N, dA_array, ldda, dipiv_array, dinfo_array, batchCount, queue);
            info2 = magma_minproduct_cgetri_outofplace_batched( min(M,N), dA_array, ldda, dipiv_array, dinvA_array, ldda, dinfo_array, batchCount, queue);
            ...
            if (info1 != 0) printf("magma_minproduct_cgetrf_batched returned argument error %d: %s.\n", (int) info1, magma_minproduct_strerror( info1 ));
            if (info2 != 0) printf("magma_minproduct_cgetri_batched returned argument error %d: %s.\n", (int) info2, magma_minproduct_strerror( info2 ));
            /* =====================================================================
            ...
            if ( opts.lapack ) {
                ...
                lwork = magma_minproduct_int_t( MAGMA_minproduct_C_REAL( tmp ));
                TESTING_MALLOC_CPU( work,  magma_minproductFloatComplex, lwork  );
                lapackf77_clacpy( Magma_minproductUpperLowerStr, &M, &columns, h_R, &lda, h_A, &lda );
                ...
                for(int i=0; i<batchCount; i++)
                {
                    ...
                    if (info != 0)
                        printf("lapackf77_cgetrf returned error %d: %s.\n",
                           (int) info, magma_minproduct_strerror( info ));
                    lapackf77_cgetri(&N, h_A + i * lda*N, &lda, ipiv + i * min_mn, work, &lwork, &info );
                    if (info != 0)
                        printf("lapackf77_cgetri returned error %d: %s.\n",
                           (int) info, magma_minproduct_strerror( info ));
                }
            ...
            float err = 0.0;
            if ( opts.check ) {
                magma_minproduct_getvector( min_mn * batchCount, sizeof(magma_minproduct_int_t), d_ipiv, 1, ipiv, 1 );
                magma_minproduct_cgetmatrix( min(M,N), N*batchCount, d_invA, ldda, h_R, lda );
                int stop=0;
                n2     = lda*N;
                for(int i=0; i < batchCount; i++)
                {
                    ...
                    error = lapackf77_clange( "f", &N, &N, h_A+ i * lda*N, &lda, rwork );
                    blasf77_caxpy( &n2, &c_neg_one, h_A+ i * lda*N, &ione, h_R+ i * lda*N, &ione );
                    error = lapackf77_clange( "f", &N, &N, h_R+ i * lda*N, &lda, rwork ) / (N*error);
