...
__global__ void
magma_minproduct_zswap_gemv_kernel(int m, int rk, int n, const magma_minproductDoubleComplex * __restrict__ V, int ldv,
                     const magma_minproductDoubleComplex * __restrict__ x, int ldx, magma_minproductDoubleComplex *c, magma_minproductDoubleComplex *b)
{
    const int i = threadIdx.x;
    const int j = i + BLOCK_SIZE1 * blockIdx.x;
    magma_minproductDoubleComplex lsum, tmp;
    ...
    if (j < m){
       ...
       if (j>=rk) 
          for(int k=0; k<n; k++)
              lsum += MAGMA_minproduct_Z_MUL( V[k*ldv], MAGMA_minproduct_Z_CNJG(x[k*ldx]));
...
__global__ void
magma_minproduct_zgemv_kernel(int m, int n, const magma_minproductDoubleComplex * __restrict__ V, int ldv,
                     const magma_minproductDoubleComplex * __restrict__ x, magma_minproductDoubleComplex *b, magma_minproductDoubleComplex *c)
{
    const int i = threadIdx.x;
    const int j = i + BLOCK_SIZE1 * blockIdx.x;
...
__global__
void magma_minproduct_zscale_kernel(int n, magma_minproductDoubleComplex* dx0,
                         magma_minproductDoubleComplex *dtau, double *dxnorm, magma_minproductDoubleComplex* dAkk)
{
   const int i = threadIdx.x;
   ...
   magma_minproductDoubleComplex *dx = dx0;
   ...
   for( int k = i; k < n; k += BLOCK_SIZE ) {
             ...
             lsum += re*re;
        #else
             re = MAGMA_minproduct_Z_REAL( dx[k] );
             double im = MAGMA_minproduct_Z_IMAG( dx[k] );
             lsum += re*re + im*im;
        #endif
   }
   sum[i] = lsum;
            ...
            if ( beta == 0 ) {
              *dtau = MAGMA_minproduct_Z_ZERO;
               ...
               *dAkk    = beta;
               ...
               scale = 1. / (tmp - beta);
#else
               double alphar =  MAGMA_minproduct_Z_REAL(tmp), alphai = MAGMA_minproduct_Z_IMAG(tmp);
               beta  = -copysign( beta, alphar );

               // todo: deal with badly scaled vectors (see lapack's larfg)
               *dtau = MAGMA_minproduct_Z_MAKE((beta - alphar)/beta, -alphai/beta);
               *dAkk = MAGMA_minproduct_Z_MAKE(beta, 0.);

               tmp = MAGMA_minproduct_Z_MAKE( alphar - beta, alphai);
               scale = MAGMA_minproduct_Z_DIV( MAGMA_minproduct_Z_ONE, tmp);
#endif
            }
   }
   ...
   for(int j=i; j<n; j+=BLOCK_SIZE)
      dx0[j] = MAGMA_minproduct_Z_MUL(dx0[j], scale);

   /* === Make temporary the first element to 1; value is stored in dAkk === */
   if (i==0)
     dx0[0] = MAGMA_minproduct_Z_ONE;
}
...
__global__ void
magma_minproduct_zgemv_kernel_adjust(int n, int k, magma_minproductDoubleComplex * A, int lda, 
                          magma_minproductDoubleComplex *B, int ldb, magma_minproductDoubleComplex *C,
                          double *xnorm, double *xnorm2, magma_minproductDoubleComplex *Akk, int *lsticc, int *lsticcs)
{
    const int i = threadIdx.x;
    const int j = i + BLOCK_SIZE2 * blockIdx.x;
    magma_minproductDoubleComplex sum;
    double temp, oldnorm;

    if (j<n) {
      B += j;
      sum = MAGMA_minproduct_Z_CNJG( B[(k-1)*ldb] );
      // sum = MAGMA_minproduct_Z_ZERO;
      for(int m=0; m<k-1; m++) {
         sum += MAGMA_minproduct_Z_MUL( MAGMA_minproduct_Z_CNJG( B[m*ldb] ), A[m*lda] );
      }
      C[j*lda] -= sum;

      oldnorm = xnorm[j];
      temp = MAGMA_minproduct_Z_ABS( C[j*lda] ) / oldnorm;
...
/*
    ...
    if (blockIdx.x==0) {
       //if (2.*temp < oldnorm) {
           ...
           magma_minproductDoubleComplex *dx = C+blockIdx.x*lda+1;
...
__global__ void
magma_minproductblas_dznrm2_check_kernel(int m, magma_minproductDoubleComplex *da, int ldda, 
                              double *dxnorm, double *dxnorm2, 
                              int *dlsticc, int *dlsticcs)
{
    const int i = threadIdx.x;
    magma_minproductDoubleComplex *dx = da + blockIdx.x * ldda;
    ...
    for( int j = i; j < m; j += BLOCK_SIZE ) {
        ...
        lsum += re*re;
#else
        re = MAGMA_minproduct_Z_REAL( dx[j] );
        double im = MAGMA_minproduct_Z_IMAG( dx[j] );
        lsum += re*re + im*im;
#endif
...
#define dF(i_, j_) (dF + (i_) + (j_)*(lddf))
    ...
    magma_minproductDoubleComplex tauk;
    magma_minproduct_int_t pvt, itemp;

    magma_minproductDoubleComplex_ptr dAkk = dauxv;
    dauxv += 1;
    ...
    // double tol3z = magma_minproduct_dsqrt( lapackf77_dlamch("Epsilon"));
    ...
    while( k < nb && lsticc == 0 ) {
        ...
        pvt = k - 1 + magma_minproduct_idamax( n-k, &dvn1[k], ione );

        if (pvt != k) {
            magma_minproductblas_zswap( k, dF(pvt,0), lddf, dF(k,0), lddf);
            itemp     = jpvt[pvt];
            jpvt[pvt] = jpvt[k];
            jpvt[k]   = itemp;
        ...
        magma_minproduct_zswap_gemv_kernel<<< (m + BLOCK_SIZE1-1) / BLOCK_SIZE1, BLOCK_SIZE1, 0, magma_minproduct_stream >>> 
                              ( m, rk, k, dA(0, 0), ldda, dF(k,  0), lddf, dA(0, k), dA(0,pvt));
                                 
        /*  Generate elementary reflector H(k). */
        magma_minproduct_zscale_kernel<<< 1, BLOCK_SIZE, 0, magma_minproduct_stream >>>
        ...
        if (k < n-1) {
            magma_minproduct_zgetvector( 1, &dtau[k], 1, &tauk, 1 );
            magma_minproduct_zgemv( Magma_minproductConjTrans, m-rk, n,
                         tauk,   dA( rk,  0 ), ldda,
                                 dA( rk,  k   ), 1,
                         c_zero, dauxv, 1 );
            if (k==0) 
               magma_minproductblas_zlacpy(Magma_minproductUpperLower, n-k-1, 1, dauxv+k+1, n-k-1, dF( k+1, k   ), n-k-1);
        }
        ...
        if (k > 0) {
            ...
            magma_minproduct_zgemv_kernel<<< (n-k-1 + BLOCK_SIZE1 -1)/BLOCK_SIZE1, BLOCK_SIZE1, 0, magma_minproduct_stream >>>
        ...
        if (k < n-1) {
            ...
            magma_minproduct_zgemv_kernel_adjust<<<(n-k-1 + BLOCK_SIZE2-1)/BLOCK_SIZE2, BLOCK_SIZE2, 0, magma_minproduct_stream>>>
            ...
            // can be done in the kernel directly (magma_minproductblas_dznrm2_check_kernel)
            // if (k==16) lsticc = 1;
        }
...
/*
        if (rk < min(m, n+offset)-1){
           magma_minproductblas_dznrm2_row_check_adjust(n-k-1, tol3z, &dvn1[k+1], 
                                             &dvn2[k+1], dA(rk,k+1), ldda, lsticcs); 
        }

        #if defined(PRECISION_d) || defined(PRECISION_z)
            magma_minproduct_dgetvector( 1, &lsticcs[0], 1, &lsticc, 1 );
        #else
...
*/
        ...
        if (k>=n-1)
           magma_minproductblas_zlacpy(Magma_minproductUpperLower, 1, 1, dAkk, 1, dA(rk, k), 1);
    ...
    *kb = k + 1;
    ...
    if( lsticc > 0 ) {
        ...
        //magma_minproductblas_dznrm2_check(m-rk-1, n-*kb, A(rk+1,rk+1), lda,
        //                       &dvn1[rk+1], &dvn2[rk+1], dlsticcs);
       
        // There is a bug when we get to recompute  
        magma_minproductblas_dznrm2_check_kernel<<< n-*kb, BLOCK_SIZE >>>
