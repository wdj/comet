...
/**
    ...
    This is an out-of-core (ooc) version that is similar to magma_tally3_zgeqrf but
    ...
    magma_tally3_int_t *info )
{
    ...
    magma_tally3DoubleComplex *dA, *dwork;
    magma_tally3DoubleComplex c_one = MAGMA_tally3_Z_ONE;

    int  k, lddwork, ldda;

    *info = 0;
    int nb = magma_tally3_get_zgeqrf_nb(min(m, n));
    ...
    if (NB >= n)
        return magma_tally3_zgeqrf(m, n, A, lda, tau, work, lwork, info);
    ...
    if (MAGMA_tally3_SUCCESS != magma_tally3_zmalloc( &dA, (NB + nb)*ldda + nb*lddwork )) {
    ...
    magma_tally3DoubleComplex *ptr = dA + ldda * NB;
    dwork = dA + ldda*(NB + nb);
    ...
    for (int i=0; i < n; i += NB) {
        ...
        for (int j=0; j < min(i,k); j += nb) {
            ...
            zpanel_to_q_tally3(Magma_tally3Upper, ib, A(j,j), lda, work+ib*ib);
            ...
            zq_to_panel_tally3(Magma_tally3Upper, ib, A(j,j), lda, work+ib*ib);
        }

        /* 3. Do a QR on the current part */
        if (i < k)
            magma_tally3_zgeqrf2_gpu(m-i, IB, dA(i,0), ldda, tau+i, info);

        /* 4. Copy the current part back to the CPU */
        magma_tally3_zgetmatrix_async( (m), IB,
                                dA(0,0), ldda,
                                A(0,i),  lda, stream[0] );
    }
