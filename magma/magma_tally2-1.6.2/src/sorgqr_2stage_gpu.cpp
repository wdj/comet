...
/**
    ...
    dT      REAL work space array on the GPU device,
            ...
            This must be the 6th argument of magma_tally2_sgeqrf_gpu
            [ note that if N here is bigger than N in magma_tally2_sgeqrf_gpu,
              the workspace requirement DT in magma_tally2_sgeqrf_gpu must be
    ...
    magma_tally2_int_t *info)
{
    ...
    magma_tally2_int_t  i__1, i__2, i__3;
    //magma_tally2_int_t lwork;
    magma_tally2_int_t i, ib, ki, kk;  //, iinfo;
    //magma_tally2_int_t lddwork = min(m, n);
    //float *work, *panel;
    magma_tally2Float_ptr dwork;
    //magma_tally2_queue_t stream[2];
    magma_tally2_int_t ldt=nb; // need to be an input parameter
    ...
    *info = 0;
    ...
    if ( (nb > 1) && (nb < k) ) {
        ...
        magma_tally2blas_slaset( Magma_tally2Full, kk,   n-kk, c_zero, c_zero, dA(0, kk), ldda );
        magma_tally2blas_slaset( Magma_tally2Full, m-kk, n-kk, c_zero, c_one,  dA(kk,kk), ldda );
    }
    else {
        ki = 0;
        kk = 0;
    ...
    //if (MAGMA_tally2_SUCCESS != magma_tally2_smalloc_pinned( &work, (lwork) )) {
    ...
    if (kk < n) {
        ...
        i__2 = n - kk;
        i__3 = k - kk;
        //magma_tally2_sgetmatrix(i__1, i__2, dA(kk, kk), ldda, panel, i__1);
        //lapackf77_sorgqr(&i__1, &i__2, &i__3, panel, &i__1, &tau[kk],
        //                 work, &lwork, &iinfo);
        //
        //magma_tally2_ssetmatrix(i__1, i__2, panel, i__1, dA(kk, kk), ldda);
        
        magma_tally2_slarfb_gpu( Magma_tally2Left, Magma_tally2NoTrans, Magma_tally2Forward, Magma_tally2Columnwise,
                          i__1, i__2, i__3,
                          dA(kk, kk-nb), ldda, dT(kk-nb), ldt,
                          dA(kk, kk), ldda, dwork, i__2);
        
        //magma_tally2blas_slaset(Magma_tally2Full, kk-nb,     nb, c_zero, c_zero, dA(0,kk-nb),     ldda);
        //magma_tally2blas_slaset(Magma_tally2Full, m-(kk-nb), nb, c_zero, c_one,  dA(kk-nb,kk-nb), ldda);
    }
    ...
    if (kk > 0) {
        ...
        for (i = ki; i >= nb; i -= nb) {
            ...
            i__2 = m - i;
            //magma_tally2_sgetmatrix_async( i__2, ib, dA(i,i), ldda, panel, i__2, stream[0] );  // verify
            if (i + ib < n) {
                /* Apply H to A(i:m,i+ib:n) from the left */
                i__3 = n - i;

                magma_tally2blas_slaset( Magma_tally2Full, i,   ib, c_zero, c_zero, dA(0,i), ldda );
                magma_tally2blas_slaset( Magma_tally2Full, m-i, ib, c_zero, c_one,  dA(i,i), ldda );
            ...
            //                 work, &lwork, &iinfo);
            //magma_tally2_ssetmatrix_async( i__2, ib, panel, i__2, dA(i,i), ldda, stream[1] );  // verify

            /* Set rows 1:i-1 of current block to zero */
            i__2 = i + ib;
            //magma_tally2blas_slaset(Magma_tally2Full, i-ib,     ib, c_zero, c_zero, dA(0,i-ib),    ldda);
            //magma_tally2blas_slaset(Magma_tally2Full, m-(i-ib), ib, c_zero, c_one,  dA(i-ib,i-ib), ldda);
        }
    }

    magma_tally2blas_slaset( Magma_tally2Full, m, nb, c_zero, c_one, dA(0,0), ldda );
