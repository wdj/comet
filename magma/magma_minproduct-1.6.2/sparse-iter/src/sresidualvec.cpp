...
magma_minproduct_sresidualvec(
    ...
    magma_minproduct_s_matrix *r, float *res,
    magma_minproduct_queue_t queue )
{
    ...
    if ( A.num_rows == b.num_rows ) {
        ...
        CHECK( magma_minproduct_s_spmv( mone, A, x, zero, *r, queue ));      // r = A x
        magma_minproduct_saxpy(dofs, one, b.dval, 1, r->dval, 1);          // r = r - b
        *res =  magma_minproduct_snrm2(dofs, r->dval, 1);            // res = ||r||
        ...
        for( magma_minproduct_int_t i=0; i<num_vecs; i++) {
            magma_minproduct_saxpy(dofs, one, b(i), 1, r(i), 1);   // r = r - b
