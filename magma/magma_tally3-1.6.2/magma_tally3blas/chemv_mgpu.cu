    ...
    int block_offset )
{
    ...
    magma_tally3FloatComplex psum, psum_t;
    magma_tally3FloatComplex total = MAGMA_tally3_C_ZERO;
    ...
    __shared__ magma_tally3FloatComplex sA [quarter_NB_X][NB_X + 2];  // TODO +3 used in chemv (single GPU); why?
    __shared__ magma_tally3FloatComplex sx_blk[NB_X];  // for x[ blk ]
    __shared__ magma_tally3FloatComplex sx_jj [NB_X];  // for x[ jj ], which cycles over all blocks left of diag

    magma_tally3FloatComplex rA[4];
    magma_tally3FloatComplex psums_t[4];
...
/**************************************************************
    ...
    Note beta*y is not included here; see magma_tally3blas_chemv_mgpu_sync.
    ...
    int block_offset)
{
    ...
    if ( ind >= block_offset && ind < n+block_offset ) {
        magma_tally3FloatComplex Ax = MAGMA_tally3_C_ZERO;
        ...
        y[ind * incy] = alpha*Ax;  // see magma_tally3blas_chemv_sync for beta*y
    }
}
...
/**
    Purpose
    -------
    magma_tally3blas_chemv_mgpu performs the matrix-vector operation:
    ...
    uplo    magma_tally3_uplo_t.
            ...
            follows:
      -     = Magma_tally3Upper:  Only the upper triangular part of A is to be referenced. **Not currently supported.**
      -     = Magma_tally3Lower:  Only the lower triangular part of A is to be referenced.
...
magma_tally3blas_chemv_mgpu(
    ...
    magma_tally3FloatComplex alpha,
    magma_tally3FloatComplex_const_ptr const d_lA[], magma_tally3_int_t ldda,
    magma_tally3_int_t offset,
    magma_tally3FloatComplex const *x,       magma_tally3_int_t incx,
    magma_tally3FloatComplex beta,                                         // unused, see magma_tally3blas_chemv_mgpu_sync
    magma_tally3FloatComplex       *y,       magma_tally3_int_t incy,             // unused
    ...
    magma_tally3_queue_t queues[] )
{
    ...
    magma_tally3_int_t offset_block_id = offset / NB_X;
    magma_tally3_int_t offset_gpu_id   = offset_block_id % ngpu;
    magma_tally3_int_t block_offset    = offset % NB_X;
    
    magma_tally3_int_t blocks = ceildiv( n + block_offset, NB_X );
    magma_tally3_int_t ldwmin = ldda*(blocks + 1);
    magma_tally3_int_t lhwmin = n*ngpu;
    ...
    for(dev=0; dev < ngpu; dev++) {
        ...
        magma_tally3_int_t num_blocks_skipped = offset_block_id / ngpu;
        if ( dev < offset_gpu_id ) {
            num_blocks_skipped += 1;
        }
        
        // shift dA to first block >= offset block that is owned by this GPU
        magma_tally3FloatComplex const *dA_dev    = d_lA[dev] + offset_block_id*NB_X + num_blocks_skipped*NB_X*ldda;
        ...
        magma_tally3FloatComplex       *dx_dev    = dwork[dev];
        magma_tally3FloatComplex       *dwork_dev = dwork[dev] + ldda;
        
        // renumber GPUs starting from the offset block
        magma_tally3_int_t new_gpu_id = (dev + ngpu - offset_gpu_id) % ngpu;
        
        dim3 grid( blocks, 1 );
        
        // copy x to each GPU
        magma_tally3_csetvector_async( n, x, incx, dx_dev + block_offset, 1, queues[dev] );
    ...
    for(dev=0; dev < ngpu; dev++) {
        ...
        magma_tally3_setdevice( dev );
        magma_tally3FloatComplex       *dx_dev    = dwork[dev];
        magma_tally3_cgetvector_async( n, dx_dev + block_offset, 1, &hwork[dev*n], 1, queues[dev] );
    }
    
    // see magma_tally3blas_chemv_mgpu_sync for final row sums
...
/**
    ...
    For convenience, the parameters are identical to magma_tally3blas_chemv_mgpu
    (though some are unused here).
    
    @see magma_tally3blas_chemv_mgpu
    @ingroup magma_tally3_cblas2
    ********************************************************************/
extern "C" magma_tally3_int_t
magma_tally3blas_chemv_mgpu_sync(
    magma_tally3_uplo_t uplo,                                               // unused, see magma_tally3blas_chemv_mgpu
    magma_tally3_int_t n,
    magma_tally3FloatComplex alpha,                                        // unused
    magma_tally3FloatComplex_const_ptr const d_lA[], magma_tally3_int_t ldda,
    magma_tally3_int_t offset,                                              // unused
    magma_tally3FloatComplex const *x,       magma_tally3_int_t incx,             // unused
    magma_tally3FloatComplex beta,
    magma_tally3FloatComplex       *y,       magma_tally3_int_t incy,             // unused
    magma_tally3FloatComplex       *hwork,   magma_tally3_int_t lhwork,
    magma_tally3FloatComplex_ptr    dwork[], magma_tally3_int_t ldwork,           // unused
    magma_tally3_int_t ngpu,
    magma_tally3_int_t nb,                                                  // unused
    magma_tally3_queue_t queues[] )
{
     ...
     */
    magma_tally3_int_t info = 0;
    //if ( (! upper) && (uplo != Magma_tally3Lower) ) {  // unused
    //    info = -1;
    //} else
    if ( n < 0 ) {
        info = -2;
    ...
    for( dev=0; dev < ngpu; ++dev ) {
        magma_tally3_setdevice( dev );
        magma_tally3_queue_sync( queues[dev] );
