...
int main( int argc, char** argv)
{
    ...
    float          magma_tally3_error, cublas_error, magma_tally3_err, cublas_err, Cnorm, work[1];
    ...
    magma_tally3FloatComplex *h_A, *h_B, *h_C, *h_Cmagma_tally3, *h_Ccublas;
    magma_tally3FloatComplex *d_A, *d_B, *d_C;
    ...
    magma_tally3FloatComplex **B_array = NULL;
    magma_tally3FloatComplex **C_array = NULL;
    ...
    printf("If running lapack (option --lapack), MAGMA_tally3 and CUBLAS error are both computed\n"
           "relative to CPU BLAS result. Else, MAGMA_tally3 error is computed relative to CUBLAS result.\n\n"
           "transA = %s, transB = %s\n", 
           lapack_trans_const_tally3(opts.transA),
           lapack_trans_const_tally3(opts.transB));
    printf("BatchCount    M     N     K   MAGMA_tally3 Gflop/s (ms)  CUBLAS Gflop/s (ms)  CPU Gflop/s (ms)  MAGMA_tally3 error  CUBLAS error\n");
    printf("=========================================================================================================\n");
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_B,  magma_tally3FloatComplex, sizeB );
            TESTING_MALLOC_CPU( h_C,  magma_tally3FloatComplex, sizeC  );
            TESTING_MALLOC_CPU( h_Cmagma_tally3,  magma_tally3FloatComplex, sizeC  );
            TESTING_MALLOC_CPU( h_Ccublas, magma_tally3FloatComplex, sizeC  );

            TESTING_MALLOC_DEV( d_A, magma_tally3FloatComplex, ldda*An*batchCount );
            TESTING_MALLOC_DEV( d_B, magma_tally3FloatComplex, lddb*Bn*batchCount );
            TESTING_MALLOC_DEV( d_C, magma_tally3FloatComplex, lddc*N*batchCount  );

            magma_tally3_malloc((void**)&A_array, batchCount * sizeof(*A_array));
            magma_tally3_malloc((void**)&B_array, batchCount * sizeof(*B_array));
            ...
            /* =====================================================================
               ...
               =================================================================== */
            magma_tally3_csetmatrix( Am, An*batchCount, h_A, lda, d_A, ldda );
            magma_tally3_csetmatrix( Bm, Bn*batchCount, h_B, ldb, d_B, lddb );
            magma_tally3_csetmatrix( M, N*batchCount, h_C, ldc, d_C, lddc );
            ...
            magma_tally3blas_cgemm_batched(opts.transA, opts.transB, M, N, K,
                             alpha, A_array, ldda,
                                    B_array, lddb,
            ...
            magma_tally3_cgetmatrix( M, N*batchCount, d_C, lddc, h_Cmagma_tally3, ldc );
            ...
            cublasCgemmBatched(handle, cublas_trans_const_tally3(opts.transA), cublas_trans_const_tally3(opts.transB), M, N, K,
                               &alpha, (const magma_tally3FloatComplex**) A_array, ldda,
                               (const magma_tally3FloatComplex**) B_array, lddb,
                               &beta,  C_array, lddc, batchCount );

            cublas_time = magma_tally3_sync_wtime( NULL ) - cublas_time;
            cublas_perf = gflops / cublas_time;
            
            magma_tally3_cgetmatrix( M, N*batchCount, d_C, lddc, h_Ccublas, ldc );
            ...
            if ( opts.lapack ) {
                ...
                {
                   blasf77_cgemm(
                               lapack_trans_const_tally3(opts.transA), lapack_trans_const_tally3(opts.transB),
                               &M, &N, &K,
            ...
            if ( opts.lapack ) {
                ...
                {
                    magma_tally3_int_t C_batchSize = ldc * N;
 
                    Cnorm = lapackf77_clange( "M", &M, &N, h_C + s*C_batchSize, &ldc, work );

                    blasf77_caxpy( &C_batchSize, &c_neg_one, h_C + s*C_batchSize, &ione, h_Cmagma_tally3 + s*C_batchSize, &ione );
                    magma_tally3_err = lapackf77_clange( "M", &M, &N, h_Cmagma_tally3 + s*C_batchSize, &ldc, work ) / Cnorm; 
                ...
                // compute relative error for magma_tally3, relative to cublas

                    Cnorm = lapackf77_clange( "M", &M, &NN, h_Ccublas, &ldc, work );
                    blasf77_caxpy( &sizeC, &c_neg_one, h_Ccublas, &ione, h_Cmagma_tally3, &ione );
                    magma_tally3_error = lapackf77_clange( "M", &M, &NN, h_Cmagma_tally3, &ldc, work ) / Cnorm;
