...
extern "C"
magma_tally2_int_t read_z_csr_from_mtx(
    ...
    magma_tally2_queue_t queue )
{
    ...
    magma_tally2DoubleComplex *coo_val=NULL;
    magma_tally2_index_t *new_col=NULL, *new_row=NULL;
    magma_tally2DoubleComplex *new_val=NULL;
    ...
    *nnz      = num_nonzeros;
    ...
    CHECK( magma_tally2_zmalloc_cpu( &coo_val, *nnz ) );
    ...
    if (mm_is_real(matcode) || mm_is_integer(matcode)) {
        for(magma_tally2_int_t i = 0; i < *nnz; ++i) {
            ...
            coo_row[i] = ROW - 1;
            coo_col[i] = COL - 1;
            coo_val[i] = MAGMA_tally2_Z_MAKE( VAL, 0.);
        }
        ...
        for(magma_tally2_int_t i = 0; i < *nnz; ++i) {
            ...
            coo_row[i] = ROW - 1;
            coo_col[i] = COL - 1;
            coo_val[i] = MAGMA_tally2_Z_MAKE( 1.0, 0.);
        }
       ...
       for(magma_tally2_int_t i = 0; i < *nnz; ++i) {
            ...
            coo_row[i] = ROW - 1;
            coo_col[i] = COL - 1;
            coo_val[i] = MAGMA_tally2_Z_MAKE( VAL, VALC);
        }
    ...
    if (mm_is_symmetric(matcode)) { // duplicate off diagonal entries
        ...
        CHECK( magma_tally2_zmalloc_cpu( &new_val, true_nonzeros ) );
    ...
    */
    CHECK( magma_tally2_zmalloc_cpu( val, *nnz ) );
    ...
    magma_tally2_queue_t queue )
{
    ...
    CHECK( magma_tally2_zwrite_csr_mtx( B, Magma_tally2ColMajor, filename, queue ));
cleanup:
    return info;
}
    ...
    magma_tally2_queue_t queue )
{
    ...
    if ( MajorType == Magma_tally2ColMajor ) {
        ...
        CHECK( magma_tally2_z_cucsrtranspose( A, &B, queue ));
        magma_tally2_zprint_matrix( A, queue );
        
        
        magma_tally2_zprint_matrix( B, queue );
        ...
        for(i=0; i < B.num_cols; i++) {
            ...
            for(j=0; j < rowtemp2 - rowtemp1; j++) {
                fprintf( fp, "%d %d %.16g %.16g\n",
                    ...
                    MAGMA_tally2_Z_REAL((B.val)[rowtemp1+j]),
                    MAGMA_tally2_Z_IMAG((B.val)[rowtemp1+j]) );
        ...
        for(i=0; i < A.num_cols; i++) {
            ...
            for(j=0; j < rowtemp2 - rowtemp1; j++) {
                fprintf( fp, "%d %d %.16g %.16g\n",
                    ...
                    MAGMA_tally2_Z_REAL((A.val)[rowtemp1+j]),
                    MAGMA_tally2_Z_IMAG((A.val)[rowtemp1+j]) );
        ...
        for(i=0; i < n_col; i++) {
            ...
            for(j=0; j < rowtemp2 - rowtemp1; j++) {
                printf( "%d %d %.6e %.6e\n",
                    ...
                    MAGMA_tally2_Z_REAL((new_val)[rowtemp1+j]),
                    MAGMA_tally2_Z_IMAG((new_val)[rowtemp1+j]) );
        ...
        for(i=0; i < n_col; i++) {
            ...
            for(j=0; j < rowtemp2 - rowtemp1; j++) {
                printf( "%d %d %.6e %.6e\n",
                    ...
                    MAGMA_tally2_Z_REAL((*val)[rowtemp1+j]),
                    MAGMA_tally2_Z_IMAG((*val)[rowtemp1+j]) );
    ...
    magma_tally2_queue_t queue )
{
    ...
    #define magma_tally2_zprintval( tmp )       {                                  \
        if ( MAGMA_tally2_Z_EQUAL( tmp, c_zero )) {                                \
            ...
            printf( " %8.4f", MAGMA_tally2_Z_REAL( tmp ));                         \
        }                                                                   \
    }
    #endif
    //**************************************************************
        ...
        if ( A.storage_type == Magma_tally2_DENSE ) {
            for( i=0; i < (A.num_rows); i++ ) {
                for( j=0; j < A.num_cols; j++ ) {
                    magma_tally2_zprintval( A.val[i*(A.num_cols)+j] );
                }
                printf( "\n" );
            }
        }
            ...
            if ( A.num_rows < 11 && A.num_cols < 11 ) {
                CHECK( magma_tally2_zmconvert( A, &C, A.storage_type, Magma_tally2_DENSE, queue ));
                CHECK( magma_tally2_zprint_matrix(  C, queue ));
            ...
            CHECK( magma_tally2_zmconvert( A, &C, A.storage_type, Magma_tally2_CSR, queue ));
            CHECK( magma_tally2_zprint_matrix(  C, queue ));
        }
    }
    else {
        magma_tally2_z_matrix C={Magma_tally2_CSR};
        CHECK( magma_tally2_zmtransfer( A, &C, A.memory_location, Magma_tally2_CPU, queue ));
        CHECK( magma_tally2_zprint_matrix(  C, queue ));
    }
    ...
    magma_tally2_queue_t queue )
{
    ...
    CHECK( magma_tally2_zmalloc_cpu( &coo_val, A->nnz ) );
    ...
    */
    CHECK( magma_tally2_zmalloc_cpu( &A->val, A->nnz ));
    ...
    if ( csr_compressor > 0) { // run the CSR compressor to remove zeros
        ...
        CHECK( magma_tally2_zmtransfer( *A, &B, Magma_tally2_CPU, Magma_tally2_CPU, queue ));
        CHECK( magma_tally2_z_csr_compressor(
            &(A->val), &(A->row), &(A->col),
        ...
        CHECK( magma_tally2_zmtransfer( B, A, Magma_tally2_CPU, Magma_tally2_CPU, queue ));
        //printf("done.\n");
