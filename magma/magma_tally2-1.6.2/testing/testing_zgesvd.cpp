        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            switch( opts.svd_work ) {
                case 0: {  // query for workspace size
                    lwork = -1;
                    magma_tally2_zgesvd( jobu, jobvt, M, N,
            ...
            TESTING_MALLOC_CPU( VT,    magma_tally2DoubleComplex, ldv*N );   // N x N (jobvt=A) or min(M,N) x N
            TESTING_MALLOC_CPU( U,     magma_tally2DoubleComplex, ldu*N_U ); // M x M (jobu=A)  or M x min(M,N)
            ...
            if (info != 0)
                printf("magma_tally2_zgesvd returned error %d: %s.\n",
            ...
            if ( opts.lapack ) {
                ...
                lapackf77_zgesvd( lapack_vec_const_tally2(jobu), lapack_vec_const_tally2(jobvt), &M, &N,
