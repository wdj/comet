...
/**
    ...
    ikind   INTEGER
            ...
            2:  This version uses a standard LAPACK-based orthogonalization through
                MAGMA_tally3's QR panel factorization (magma_tally3_sgeqr2x3_gpu) and magma_tally3_sorgqr
            3:  MGS
    ...
    magma_tally3_int_t *info )
{
    ...
    *info = 0;
    ...
    if (ikind == 1) {
        ...
        magma_tally3_smalloc_cpu( &hwork, 32 + 2*n*n + 2*n);
        if ( hwork == NULL ) {
        ...
        do {
            ...
            magma_tally3_sgemm(Magma_tally3ConjTrans, Magma_tally3NoTrans, n, n, m, c_one, dA, ldda, dA, ldda, c_zero, dwork, n );
            magma_tally3_sgetmatrix(n, n, dwork, n, G, n);
            ...
            magma_tally3_ssetmatrix(n, n, VT, n, dwork, n);
            magma_tally3_strsm( Magma_tally3Right, Magma_tally3Upper, Magma_tally3NoTrans, Magma_tally3NonUnit, m, n, c_one, dwork, n, dA, ldda);
            if (mins > 0.00001f)
                cn = maxs/mins;
        ...
        magma_tally3Float_ptr dtau = dwork + 2*n*n;
        magma_tally3Float_ptr d_T  = dwork;
        magma_tally3Float_ptr ddA  = dwork + n*n;
        float *tau  = work+n*n;

        magma_tally3blas_slaset( Magma_tally3Full, n, n, c_zero, c_zero, d_T, n );
        magma_tally3_sgeqr2x3_gpu(m, n, dA, ldda, dtau, d_T, ddA,
                           (float *)(dwork+min_mn+2*n*n), info);
        magma_tally3_sgetmatrix( min_mn, 1, dtau, min_mn, tau, min_mn);
        magma_tally3_sgetmatrix( n, n, ddA, n, work, n);
        magma_tally3_sorgqr_gpu( m, n, n, dA, ldda, tau, d_T, nb, info );
        // ================== end of ikind == 2 ===================================================
    }
    else if (ikind == 3) {
        // ================== MGS               ===================================================
        for (magma_tally3_int_t j = 0; j < n; j++) {
            for (magma_tally3_int_t i = 0; i < j; i++) {
                *work(i, j) = magma_tally3_sdot(m, dA(0,i), 1, dA(0,j), 1);
                magma_tally3_saxpy(m, -(*work(i,j)),  dA(0,i), 1, dA(0,j), 1);
            }
            for (magma_tally3_int_t i = j; i < n; i++)
                *work(i, j) = MAGMA_tally3_S_ZERO;
            //*work(j,j) = MAGMA_tally3_S_MAKE( magma_tally3_snrm2(m, dA(0,j), 1), 0. );
            *work(j,j) = magma_tally3_sdot(m, dA(0,j), 1, dA(0,j), 1);
            *work(j,j) = MAGMA_tally3_S_MAKE( sqrt(MAGMA_tally3_S_REAL( *work(j,j) )), 0.);
            magma_tally3_sscal(m, 1./ *work(j,j), dA(0,j), 1);
        }
        // ================== end of ikind == 3 ===================================================
    }
    else if (ikind == 4) {
        // ================== Cholesky QR       ===================================================
        magma_tally3_sgemm(Magma_tally3ConjTrans, Magma_tally3NoTrans, n, n, m, c_one, dA, ldda, dA, ldda, c_zero, dwork, n );
        magma_tally3_sgetmatrix(n, n, dwork, n, work, n);
        lapackf77_spotrf("u", &n, work, &n, info);
        magma_tally3_ssetmatrix(n, n, work, n, dwork, n);
        magma_tally3_strsm( Magma_tally3Right, Magma_tally3Upper, Magma_tally3NoTrans, Magma_tally3NonUnit, m, n, c_one, dwork, n, dA, ldda);
        // ================== end of ikind == 4 ===================================================
    }
