    ...
    int block_offset)
{
    ...
    for(int jj=next; jj < gridDim.x; jj += ngpu) {
        ...
        if ( ty4 < 4 && (partial == 0 || tx4 + ty4*quarter_NB_X < partial) ) {
            ...
            work[jj*NB_X + tx4 + ty4_nb4] = psum_t;  //MAGMA_tally4_D_MAKE( tx4, blk );  // store at work( jj*NB_X + tx4 + ty4*16, blk )
        }
        __syncthreads();
    }
    ...
    if ( ty == 0 && (partial == 0 || tx < partial) ) {
        total = sA16(0, tx)
              ...
              + sA16(2, tx)
              + sA16(3, tx);
        work[blk*NB_X + tx] = total;  //MAGMA_tally4_D_MAKE( tx, blk );  // store at work( blk*NB_X + tx, blk )
    }
