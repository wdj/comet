...
int main( int argc, char** argv)
{
    ...
    printf("    N   CPU GFlop/s (ms)    GPU GFlop/s (ms)    ||R_magma_minproduct - R_lapack||_F / ||R_lapack||_F\n");
    printf("========================================================\n");
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_minproduct_cpotf2_gpu( opts.uplo, N, d_A, ldda, &info );
            gpu_time = magma_minproduct_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_minproduct_cpotf2_gpu returned error %d: %s.\n",
