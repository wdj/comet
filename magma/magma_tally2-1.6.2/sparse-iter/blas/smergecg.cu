...
__global__ void
magma_tally2_scgreduce_kernel_spmv1( 
    int Gs,
    int n, 
    float * vtmp,
    float * vtmp2 )
{
    ...
    temp[Idx] = MAGMA_tally2_S_MAKE( 0.0, 0.0);
    int i = blockIdx.x * ( blockSize * 2 ) + Idx;   
    while (i < Gs ) {
        temp[ Idx  ] += vtmp[ i ]; 
        temp[ Idx  ] += ( i + blockSize < Gs ) ? vtmp[ i + blockSize ] 
                                                : MAGMA_tally2_S_MAKE( 0.0, 0.0); 
        i += gridSize;
    }
    __syncthreads();
    if ( Idx < 64 ){
...
__global__ void
magma_tally2_scgmerge_spmvcsr_kernel(  
    int n,
    float * dval, 
...
__global__ void
magma_tally2_scgmerge_spmvell_kernel(  
    int n,
...
__global__ void
magma_tally2_scgmerge_spmvellpack_kernel(  
    int n,
    int num_cols_per_row,
    float * dval, 
    magma_tally2_index_t * dcolind,
...
__global__ void
magma_tally2_scgmerge_spmvellpackrt_kernel_8(  
    int n,
    float * dval, 
    magma_tally2_index_t * dcolind,
...
// computes the SpMV using ELLRT 8 threads per row
__global__ void
magma_tally2_scgmerge_spmvellpackrt_kernel_16(  
    int n,
    float * dval, 
    magma_tally2_index_t * dcolind,
    magma_tally2_index_t * drowlength,
...
// computes the SpMV using ELLRT 8 threads per row
__global__ void
magma_tally2_scgmerge_spmvellpackrt_kernel_32(  
    int n,
    float * dval, 
    magma_tally2_index_t * dcolind,
    magma_tally2_index_t * drowlength,
...
__global__ void
magma_tally2_scgmerge_spmvellpackrt_kernel2(  
    int n,
    float * z,
    float * d,
    float * vtmp2 )
{
    ...
    temp[ Idx ] = ( i < n ) ? z[i]*d[i] : MAGMA_tally2_S_MAKE(0.0, 0.0);
    __syncthreads();
    if ( Idx < 128 ){
        temp[ Idx ] += temp[ Idx + 128 ];
    }
...
__global__ void
magma_tally2_scgmerge_spmvsellc_kernel(   
    int num_rows, 
    int blocksize,
    float * dval, 
    magma_tally2_index_t * dcolind,
...
__global__ void
magma_tally2_scgmerge_spmvsellpt_kernel_8( 
    int num_rows, 
    int blocksize,
    int T,
    float * dval, 
...
// SELLC SpMV kernel modified assigning multiple threads to each row - 2D kernel
__global__ void
magma_tally2_scgmerge_spmvsellpt_kernel_16( 
    int num_rows, 
    int blocksize,
    int T,
    float * dval, 
...
// SELLC SpMV kernel modified assigning multiple threads to each row - 2D kernel
__global__ void
magma_tally2_scgmerge_spmvsellpt_kernel_32( 
    int num_rows, 
    int blocksize,
    int T,
    float * dval, 
...
/**
    ...
    @param[in]
    d1          magma_tally2Float_ptr 
                temporary vector

    @param[in]
    d2          magma_tally2Float_ptr 
                temporary vector

    @param[in]
    dd          magma_tally2Float_ptr 
                input vector d

    @param[out]
    dz          magma_tally2Float_ptr 
                input vector z

    @param[out]
    skp         magma_tally2Float_ptr 
    ...
    @ingroup magma_tally2sparse_sgegpuk
    ********************************************************************/
    ...
    magma_tally2_queue_t queue )
{
    ...
    if ( A.storage_type == Magma_tally2_CSR )
        ...
        magma_tally2_scgmerge_spmvellpackrt_kernel2<<<Gs, Bs, Ms, queue >>>
                              ( A.num_rows, dz, dd, d1 );
    ...
    while( Gs.x > 1 ) {
        ...
        magma_tally2_scgreduce_kernel_spmv1<<< Gs_next.x/2, Bs.x/2, Ms/2 >>> 
    ...
    magma_tally2_scopyvector( 1, aux1, 1, skp+4, 1 );
    dim3 Bs2( 2 );
    dim3 Gs2( 1 );
    magma_tally2_scg_rhokernel<<<Gs2, Bs2, 0>>>( skp );
...
__global__ void
magma_tally2_scgmerge_xrbeta_kernel(  
    int n, 
    float * x, 
    float * r,
    float * d,
    ...
    float * vtmp )
{
    ...
    float rho = skp[3];
    float mrho = MAGMA_tally2_S_MAKE( -1.0, 0.0)*rho;
...
magma_tally2_scg_alphabetakernel(  
    float * skp )
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if( i==0 ){
        float tmp1 = skp[1];
        skp[0] =  tmp1/skp[2];
        //printf("beta=%e\n", MAGMA_tally2_S_REAL(tmp1));
    }
}

// update search Krylov vector d
__global__ void
magma_tally2_scg_d_kernel(  
    int n, 
    float * skp,
    float * r,
    float * d )
...
/**
    ...
    @ingroup magma_tally2sparse_ssygpuk
    ********************************************************************/
    ...
    magma_tally2_queue_t queue )
{
    ...
    magma_tally2Float_ptr aux1 = d1, aux2 = d2;
    int b = 1;        
    magma_tally2_scgmerge_xrbeta_kernel<<<Gs, Bs, Ms>>>
    ...
    magma_tally2_scopyvector( 1, aux1, 1, skp+1, 1 );
    dim3 Bs2( 2 );
    dim3 Gs2( 1 );
    magma_tally2_scg_alphabetakernel<<<Gs2, Bs2, 0>>>( skp );

    dim3 Bs3( local_block_size );
    dim3 Gs3( magma_tally2_ceildiv( n, local_block_size ) );
    magma_tally2_scg_d_kernel<<<Gs3, Bs3, 0>>>( n, skp, dr, dd );  
