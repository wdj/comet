...
int main( int argc, char** argv)
{
    ...
    float *h_A, *h_B, *h_Bcublas, *h_Bmagma_minproduct, *h_Blapack, *h_X;
    float *d_A, *d_B;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_Bmagma_minproduct,  float, sizeB   );
            ...
            magma_minproduct_smalloc( &dinvA, dinvA_batchSize * batchCount);
            magma_minproduct_smalloc( &dwork, dwork_batchSize * batchCount );
    
            sset_pointer(dwork_array, dwork, lddb, 0, 0, dwork_batchSize, batchCount, opts.queue);
            sset_pointer(dinvA_array, dinvA, magma_minproduct_roundup( Ak, TRI_NB ), 0, 0, dinvA_batchSize, batchCount, opts.queue);

            memset(h_Bmagma_minproduct, 0, batchCount*ldb*N*sizeof(float));
            magma_minproductblas_slaset( Magma_minproductFull, lddb, N*batchCount, c_zero, c_zero, dwork, lddb);
            ...
            /* =====================================================================
               ...
               =================================================================== */
            magma_minproduct_ssetmatrix( Ak, Ak*batchCount, h_A, lda, d_A, ldda );
            magma_minproduct_ssetmatrix( M,  N*batchCount, h_B, ldb, d_B, lddb );
            ...
            magma_minproduct_time = magma_minproduct_sync_wtime( NULL );
                ...
                magma_minproduct_sgetmatrix( M, N*batchCount, dwork, lddb, h_Bmagma_minproduct, ldb );
            #else
                ...
                magma_minproduct_time = magma_minproduct_sync_wtime( NULL ) - magma_minproduct_time;
                magma_minproduct_perf = gflops / magma_minproduct_time;
                magma_minproduct_sgetmatrix( M, N*batchCount, d_B, lddb, h_Bmagma_minproduct, ldb );
            #endif
            ...
            magma_minproduct_sgetmatrix( M, N*batchCount, d_B, lddb, h_Bcublas, ldb );
            ...
            /* =====================================================================
               ...
               =================================================================== */
            // ||b - 1/alpha*A*x|| / (||A||*||x||)
            float alpha2 = MAGMA_minproduct_S_DIV( c_one, alpha );
            float normR, normX, normA=0.;
            magma_minproduct_error  = 0.0;
            cublas_error = 0.0;

            memcpy( h_X, h_Bmagma_minproduct, sizeB*sizeof(float) );
            ...
            for (int s=0; s < batchCount; s++) {
                ...
                normX = lapackf77_slange( "M", &M, &N, h_Bmagma_minproduct + s * ldb * N, &ldb, work );
