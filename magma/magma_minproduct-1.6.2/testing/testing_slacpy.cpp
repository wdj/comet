        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for( int j = 0; j < N; ++j ) {
                for( int i = 0; i < M; ++i ) {
                    ...
                    h_B[i + j*ldb] = MAGMA_minproduct_S_MAKE( i - j/10000. + 10000., j );
                }
            }
            ...
            //magma_minproductblas_slacpy( uplo[iuplo], M-2, N-2, d_A+1+ldda, ldda, d_B+1+lddb, lddb );  // inset by 1 row & col
            magma_minproductblas_slacpy( uplo[iuplo], M, N, d_A, ldda, d_B, lddb );
            ...
            if ( opts.verbose ) {
                ...
                printf( "B= " );  magma_minproduct_sprint(     M, N, h_B, lda );
                printf( "dA=" );  magma_minproduct_sprint_gpu( M, N, d_A, ldda );
                printf( "dB=" );  magma_minproduct_sprint_gpu( M, N, d_B, ldda );
            }
            
            /* =====================================================================
               Check the result
               =================================================================== */
            magma_minproduct_sgetmatrix( M, N, d_B, ldda, h_R, lda );
