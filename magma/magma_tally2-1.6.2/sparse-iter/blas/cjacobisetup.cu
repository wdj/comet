...
/**
    ...
    @param[in]
    num_cols    magma_tally2_int_t
                number of cols
                
    @param[in]
    t           magma_tally2_c_matrix
                t = A*x
    ...
    magma_tally2_queue_t queue )
{
    ...
    dim3 grid( magma_tally2_ceildiv( t.num_rows, BLOCK_SIZE ));
    magma_tally2_int_t threads = BLOCK_SIZE;
    ...
    magma_tally2_queue_t queue )
{
    ...
    for( magma_tally2_int_t i=0; i<maxiter; i++ ) {
        // distinct routines imply synchronization
        // magma_tally2_c_spmv( c_one, A, *x, c_zero, t, queue );                // t =  A * x
        // cjacobiupdate_kernel<<< grid, threads, 0 >>>( t.num_rows, t.num_cols, t.dval, b.dval, d.dval, x->dval );
