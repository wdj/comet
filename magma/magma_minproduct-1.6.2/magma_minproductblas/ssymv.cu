...
/**
    Purpose
    -------
    magma_minproductblas_ssymv_work performs the matrix-vector operation:
    ...
    MAGMA_minproduct implements ssymv through two steps:
    ...
    magma_minproductblas_ssymv is a wrapper routine allocating the workspace inside the
    routine and provides the same interface as cublas.
    
    If users need to call ssymv frequently, we suggest using
    magma_minproductblas_ssymv_work instead of magma_minproductblas_ssymv. As the overhead to
    allocate and free in device memory in magma_minproductblas_ssymv would hurt performance.
    ...
    magma_minproduct_queue_t queue )
{
    ...
    if ( arch < 200 ) {
        magma_minproduct_ssymv( uplo, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
     ...
     */
    if ( (n == 0) || ( MAGMA_minproduct_S_EQUAL(alpha, MAGMA_minproduct_S_ZERO) && MAGMA_minproduct_S_EQUAL(beta, MAGMA_minproduct_S_ONE) ) )
        return info;
...
// end magma_minproductblas_ssymv_work


/**
    Purpose
    -------
    magma_minproductblas_ssymv performs the matrix-vector operation:
    ...
    magma_minproductFloat_ptr dy, magma_minproduct_int_t incy)
{
    ...
    magma_minproduct_smalloc( &dwork, lwork );
    if ( dwork == NULL ) {
    ...
    magma_minproductblas_ssymv_work( uplo, n, alpha, dA, ldda, dx, incx, beta, dy, incy,
...
// end magma_minproductblas_ssymv
