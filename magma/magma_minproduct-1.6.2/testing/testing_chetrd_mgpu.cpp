...
int main( int argc, char** argv)
{
    ...
    #endif
    float result[2] = {0., 0.};
    magma_minproduct_int_t N, n2, lda, lwork, info, nb;
    ...
    magma_minproduct_int_t k = 1;  // TODO: UNKNOWN, UNDOCUMENTED VARIABLE (number of streams?)
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( opts.ngpu == 0 ) {
                magma_minproduct_chetrd(opts.uplo, N, h_R, lda, diag, offdiag,
                             tau, h_work, lwork, &info);
            } else {
                magma_minproduct_chetrd_mgpu(opts.ngpu, k, opts.uplo, N, h_R, lda, diag, offdiag,
                                  tau, h_work, lwork, &info);
            }
            gpu_time = magma_minproduct_wtime() - gpu_time;
            if ( info != 0 ) {
                printf("magma_minproduct_chetrd_mgpu returned error %d: %s.\n",
