    ...
    magma_minproduct_int_t *info)
{
    ...
    *info = 0;
    ...
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_cmalloc_pinned( &A, nb*nb )) {
    ...
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_cmalloc( &dW, (1+nb)*ldda )) {
    ...
    if (upper) {
        ...
        for (j=0; j<n; j += nb) {
            ...
            magma_minproduct_cgetmatrix_async(jb, jb, dA(j, j), ldda, A(j,j), nb, stream[1]);
            trace_gpu_end( 0, 0 );

            // factorize the diagonal block
            magma_minproduct_queue_sync(stream[1]);
            trace_cpu_start( 0, "potrf", "potrf" );
            chetrf_nopiv_cpu(Magma_minproductUpper, jb, ib, A(j, j), nb, info);
            trace_cpu_end( 0 );
            if (*info != 0){
                *info = *info + j;
                break;
            ...
            magma_minproduct_csetmatrix_async(jb, jb, A(j, j), nb, dA(j, j), ldda, stream[0]);
            trace_gpu_end( 0, 0 );
        ...
        for (j=0; j<n; j+=nb) {
            ...
            magma_minproduct_queue_sync(stream[1]);
            trace_cpu_start( 0, "potrf", "potrf" );
            chetrf_nopiv_cpu(Magma_minproductLower, jb, ib, A(j, j), nb, info);
            trace_cpu_end( 0 );
            if (*info != 0){
                *info = *info + j;
                break;
