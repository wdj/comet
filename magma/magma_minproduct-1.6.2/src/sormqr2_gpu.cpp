    ...
    magma_minproduct_int_t *info)
{
    ...
    *info = 0;
    ...
    if (left) {
        nq = m;
        //nw = n;
        magma_minproduct_smalloc( &dwork, (n + 64)*64 );  // TODO after checking args, else memory leak!
    } else {
        nq = n;
        //nw = m;
        magma_minproduct_smalloc( &dwork, (m + 64)*64 );  // TODO after checking args, else memory leak!
    }
    ...
    magma_minproductblas_slaset_band( Magma_minproductUpper, k, k, nb, c_zero, c_one, dA, ldda );
    ...
    for (i = i1; (step < 0 ? i >= i2 : i <= i2); i += step) {
        ...
        magma_minproduct_ssetmatrix( ib, ib, T, ib, dwork, ib );
