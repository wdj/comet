...
int main( int argc, char** argv)
{
    ...
    double          magma_tally3_error, cublas_error, magma_tally3_err, cublas_err, Cnorm, work[1];
    ...
    magma_tally3DoubleComplex *h_A, *h_B, *h_C, *h_Cmagma_tally3, *h_Ccublas;
    magma_tally3DoubleComplex *d_A, *d_B, *d_C;
    ...
    magma_tally3DoubleComplex **A_array = NULL;
    magma_tally3DoubleComplex **B_array = NULL;
    magma_tally3DoubleComplex **C_array = NULL;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( h_C,  magma_tally3DoubleComplex, sizeC  );
            TESTING_MALLOC_CPU( h_Cmagma_tally3,  magma_tally3DoubleComplex, sizeC  );
            TESTING_MALLOC_CPU( h_Ccublas, magma_tally3DoubleComplex, sizeC  );

            TESTING_MALLOC_DEV( d_A, magma_tally3DoubleComplex, ldda*An*batchCount );
            TESTING_MALLOC_DEV( d_B, magma_tally3DoubleComplex, lddb*Bn*batchCount );
            TESTING_MALLOC_DEV( d_C, magma_tally3DoubleComplex, lddc*N*batchCount  );
            ...
            /* =====================================================================
               ...
               =================================================================== */
            magma_tally3_zsetmatrix( Am, An*batchCount, h_A, lda, d_A, ldda );
            magma_tally3_zsetmatrix( Bm, Bn*batchCount, h_B, ldb, d_B, lddb );
            magma_tally3_zsetmatrix( M, N*batchCount, h_C, ldc, d_C, lddc );
            ...
            magma_tally3blas_zgemm_batched(opts.transA, opts.transB, M, N, K,
                             alpha, A_array, ldda,
                                    B_array, lddb,
            ...
            magma_tally3_zgetmatrix( M, N*batchCount, d_C, lddc, h_Cmagma_tally3, ldc );
            ...
            cublasZgemmBatched(handle, cublas_trans_const_tally3(opts.transA), cublas_trans_const_tally3(opts.transB), M, N, K,
                               &alpha, (const magma_tally3DoubleComplex**) A_array, ldda,
                               (const magma_tally3DoubleComplex**) B_array, lddb,
                               &beta,  C_array, lddc, batchCount );

            cublas_time = magma_tally3_sync_wtime( NULL ) - cublas_time;
            cublas_perf = gflops / cublas_time;
            
            magma_tally3_zgetmatrix( M, N*batchCount, d_C, lddc, h_Ccublas, ldc );
            ...
            if ( opts.lapack ) {
                ...
                for(int s=0; s<batchCount; s++)
                {
                    ...
                    blasf77_zaxpy( &C_batchSize, &c_neg_one, h_C + s*C_batchSize, &ione, h_Cmagma_tally3 + s*C_batchSize, &ione );
                    magma_tally3_err = lapackf77_zlange( "M", &M, &N, h_Cmagma_tally3 + s*C_batchSize, &ldc, work ) / Cnorm; 
                ...
                // compute relative error for magma_tally3, relative to cublas
                    ...
                    blasf77_zaxpy( &sizeC, &c_neg_one, h_Ccublas, &ione, h_Cmagma_tally3, &ione );
                    magma_tally3_error = lapackf77_zlange( "M", &M, &NN, h_Cmagma_tally3, &ldc, work ) / Cnorm;
