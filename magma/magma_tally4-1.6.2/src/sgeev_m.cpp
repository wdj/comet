 ...
 * Version2 - MAGMA_tally4 without dT    (magma_tally4_zgehrd2 and lapack_zunghr)
 * Version3 - MAGMA_tally4 with dT       (magma_tally4_zgehrd  and magma_tally4_zunghr)
 * Version4 - Multi-GPU magma_tally4_zgehrd_m with T on CPU, copied to dT, single-GPU magma_tally4_zunghr
 * Version5 - Multi-GPU magma_tally4_zgehrd_m with T on CPU, multi-GPU magma_tally4_zunghr_m
 */
#define Version5
 ...
 * TREVC version 3 - blocked, single-threaded (MAGMA_tally4)
 * TREVC version 4 - blocked, multi-threaded  (MAGMA_tally4)
 * TREVC version 5 - blocked, multi-threaded, GPU (MAGMA_tally4)
 */
...
/**
    ...
    ---------
    @param[in]
    jobvl   magma_tally4_vec_t
      -     = Magma_tally4NoVec: left eigenvectors of A are not computed;
      -     = Magma_tally4Vec:   left eigenvectors of are computed.

    @param[in]
    jobvr   magma_tally4_vec_t
      -     = Magma_tally4NoVec: right eigenvectors of A are not computed;
      -     = Magma_tally4Vec:   right eigenvectors of A are computed.
    ...
    VL      REAL array, dimension (LDVL,N)
            If JOBVL = Magma_tally4Vec, the left eigenvectors u(j) are stored one
            after another in the columns of VL, in the same order
            as their eigenvalues.
            If JOBVL = Magma_tally4NoVec, VL is not referenced.
            u(j) = VL(:,j), the j-th column of VL.

    @param[in]
    ldvl    INTEGER
            The leading dimension of the array VL.  LDVL >= 1; if
            JOBVL = Magma_tally4Vec, LDVL >= N.

    @param[out]
    VR      REAL array, dimension (LDVR,N)
            If JOBVR = Magma_tally4Vec, the right eigenvectors v(j) are stored one
            after another in the columns of VR, in the same order
            as their eigenvalues.
            If JOBVR = Magma_tally4NoVec, VR is not referenced.
            v(j) = VR(:,j), the j-th column of VR.

    @param[in]
    ldvr    INTEGER
            The leading dimension of the array VR.  LDVR >= 1; if
            JOBVR = Magma_tally4Vec, LDVR >= N.
    ...
    @ingroup magma_tally4_sgeev_driver
    ********************************************************************/
    ...
    magma_tally4_int_t *info )
{
    ...
    magma_tally4_int_t i, k, ilo, ihi;
    magma_tally4_int_t ibal, ierr, itau, iwrk, nout, liwrk, nb;
    magma_tally4_int_t scalea, minwrk, optwrk, lquery, wantvl, wantvr, select[1];
    
    magma_tally4_side_t side = Magma_tally4Right;
    
    magma_tally4_timer_t time_total=0, time_gehrd=0, time_unghr=0, time_hseqr=0, time_trevc=0, time_sum=0;
    magma_tally4_flops_t flop_total=0, flop_gehrd=0, flop_unghr=0, flop_hseqr=0, flop_trevc=0, flop_sum=0;
    timer_start( time_total );
    flops_start( flop_total );
    
    *info = 0;
    lquery = (lwork == -1);
    wantvl = (jobvl == Magma_tally4Vec);
    wantvr = (jobvr == Magma_tally4Vec);
    if (! wantvl && jobvl != Magma_tally4NoVec) {
        *info = -1;
    } else if (! wantvr && jobvr != Magma_tally4NoVec) {
        *info = -2;
    } else if (n < 0) {
        *info = -3;
    } else if (lda < max(1,n)) {
    ...
    /* Compute workspace */
    nb = magma_tally4_get_sgehrd_nb( n );
    if (*info == 0) {
        minwrk = (2 +   nb)*n;
        optwrk = (2 + 2*nb)*n;
        work[0] = MAGMA_tally4_S_MAKE( (float) optwrk, 0. );
    ...
    float *dT;
    if (MAGMA_tally4_SUCCESS != magma_tally4_smalloc( &dT, nb*n )) {
    ...
    float *T;
    if (MAGMA_tally4_SUCCESS != magma_tally4_smalloc_cpu( &T, nb*n )) {
    ...
    smlnum = magma_tally4_ssqrt( smlnum ) / eps;
    bignum = 1. / smlnum;
    ...
    flops_start( flop_gehrd );
        ...
        magma_tally4_sgehrd2( n, ilo, ihi, A, lda,
                       &work[itau], &work[iwrk], liwrk, &ierr );
    #elif defined(Version3)
        // Version 3 - LAPACK consistent MAGMA_tally4 HRD + T matrices stored,
        magma_tally4_sgehrd( n, ilo, ihi, A, lda,
                      &work[itau], &work[iwrk], liwrk, dT, &ierr );
    #elif defined(Version4) || defined(Version5)
        // Version 4 - Multi-GPU, T on host
        magma_tally4_sgehrd_m( n, ilo, ihi, A, lda,
                        &work[itau], &work[iwrk], liwrk, T, &ierr );
        magma_tally4_ssetmatrix( nb, n, T, nb, dT, nb );
    #endif
    ...
    if (wantvl) {
        ...
        lapackf77_slacpy( Magma_tally4LowerStr, &n, &n, A, &lda, VL, &ldvl );
        ...
        flops_start( flop_unghr );
            ...
            // Version 3 - LAPACK consistent MAGMA_tally4 HRD + T matrices stored
            magma_tally4_sorghr( n, ilo, ihi, VL, ldvl, &work[itau], dT, nb, &ierr );
        #elif defined(Version5)
            // Version 5 - Multi-GPU, T on host
            magma_tally4_sorghr_m( n, ilo, ihi, VL, ldvl, &work[itau], T, nb, &ierr );
        #endif
        ...
        #elif defined(Version3) || defined(Version4)
            // Version 3 - LAPACK consistent MAGMA_tally4 HRD + T matrices stored
            magma_tally4_sorghr( n, ilo, ihi, VR, ldvr, &work[itau], dT, nb, &ierr );
        #elif defined(Version5)
            // Version 5 - Multi-GPU, T on host
            magma_tally4_sorghr_m( n, ilo, ihi, VR, ldvr, &work[itau], T, nb, &ierr );
        #endif
        time_sum += timer_stop( time_unghr );
        flop_sum += flops_stop( flop_unghr );
    ...
    if (wantvl || wantvr) {
        ...
        liwrk = lwork - iwrk;
        #if TREVC_VERSION == 1
        lapackf77_strevc( lapack_side_const_tally4(side), "B", select, &n, A, &lda, VL, &ldvl,
                          VR, &ldvr, &n, &nout, &work[iwrk], &ierr );
        #elif TREVC_VERSION == 2
        lapackf77_strevc3( lapack_side_const_tally4(side), "B", select, &n, A, &lda, VL, &ldvl,
                           VR, &ldvr, &n, &nout, &work[iwrk], &liwrk, &ierr );
        #elif TREVC_VERSION == 3
        magma_tally4_strevc3( side, Magma_tally4BacktransVec, select, n, A, lda, VL, ldvl,
                       VR, ldvr, n, &nout, &work[iwrk], liwrk, &ierr );
        #elif TREVC_VERSION == 4
        magma_tally4_strevc3_mt( side, Magma_tally4BacktransVec, select, n, A, lda, VL, ldvl,
                          VR, ldvr, n, &nout, &work[iwrk], liwrk, &ierr );
        #elif TREVC_VERSION == 5
        magma_tally4_strevc3_mt_gpu( side, Magma_tally4BacktransVec, select, n, A, lda, VL, ldvl,
                              VR, ldvr, n, &nout, &work[iwrk], liwrk, &ierr );
        #else
    ...
    if (wantvl) {
        ...
        for (i = 0; i < n; ++i) {
            if ( wi[i] == 0. ) {
                scl = 1. / magma_tally4_cblas_snrm2( n, VL(0,i), 1 );
                blasf77_sscal( &n, &scl, VL(0,i), &ione );
            }
            else if ( wi[i] > 0. ) {
                d__1 = magma_tally4_cblas_snrm2( n, VL(0,i), 1 );
                d__2 = magma_tally4_cblas_snrm2( n, VL(0,i+1), 1 );
    ...
    if (wantvr) {
        ...
        for (i = 0; i < n; ++i) {
            if ( wi[i] == 0. ) {
                scl = 1. / magma_tally4_cblas_snrm2( n, VR(0,i), 1 );
                blasf77_sscal( &n, &scl, VR(0,i), &ione );
            }
            else if ( wi[i] > 0. ) {
                d__1 = magma_tally4_cblas_snrm2( n, VR(0,i), 1 );
                d__2 = magma_tally4_cblas_snrm2( n, VR(0,i+1), 1 );
                scl = 1. / lapackf77_slapy2( &d__1, &d__2 );
                blasf77_sscal( &n, &scl, VR(0,i), &ione );
...
CLEANUP:
    ...
    if (scalea) {
        ...
        magma_tally4_int_t nval = n - (*info);
        magma_tally4_int_t ld = max( nval, 1 );
