...
{
    magma_tally4DoubleComplex *da[Magma_tally4MaxGPUs];
    ...
    *info = 0;
    ...
    for (i=0; i < ngpu; i++) {
        ...
        if (MAGMA_tally4_SUCCESS != magma_tally4_zmalloc( &da[i], ldda*n_local[i] )) {
    ...
    if (m > nb && n > nb) {
        ...
        magma_tally4_zsetmatrix_1D_col_bcyclic(m, n, A, lda, da, ldda, ngpu, nb);

        /* Factor using the GPU interface */
        magma_tally4_zgeqrf2_mgpu( ngpu, m, n, da, ldda, tau, info);
