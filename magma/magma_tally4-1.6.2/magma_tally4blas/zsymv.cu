    ...
    magma_tally4DoubleComplex * __restrict__ work)
{
    ...
    __shared__ magma_tally4DoubleComplex sA [quarter_NB_X][NB_X + 3]; /* Why +3? seems it only needs +2. Does +3 reduce bank conflicts? */
...
/**
    Purpose
    -------
    magma_tally4blas_zsymv_work performs the matrix-vector operation:
    ...
    MAGMA_tally4 implements zsymv through two steps:
    ...
    magma_tally4blas_zsymv is a wrapper routine allocating the workspace inside the
    routine and provides the same interface as cublas.
    
    If users need to call zsymv frequently, we suggest using
    magma_tally4blas_zsymv_work instead of magma_tally4blas_zsymv. As the overhead to
    allocate and free in device memory in magma_tally4blas_zsymv would hurt performance.
    ...
    magma_tally4_queue_t queue )
{
    ...
    if ( arch < 200 ) {
        //magma_tally4_zsymv( uplo, n, alpha, dA, ldda, dx, incx, beta, dy, incy );
     ...
     */
    if ( (n == 0) || ( MAGMA_tally4_Z_EQUAL(alpha, MAGMA_tally4_Z_ZERO) && MAGMA_tally4_Z_EQUAL(beta, MAGMA_tally4_Z_ONE) ) )
        return info;
...
// end magma_tally4blas_zsymv_work


/**
    Purpose
    -------
    magma_tally4blas_zsymv performs the matrix-vector operation:
    ...
    magma_tally4DoubleComplex_ptr dy, magma_tally4_int_t incy)
{
    ...
    magma_tally4_zmalloc( &dwork, lwork );
    if ( dwork == NULL ) {
    ...
    magma_tally4blas_zsymv_work( uplo, n, alpha, dA, ldda, dx, incx, beta, dy, incy,
