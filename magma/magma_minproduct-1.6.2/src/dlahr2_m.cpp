    ...
    struct dgehrd_data *data )
{
    ...
    double ei = MAGMA_minproduct_D_ZERO;
    ...
    for( d = 0; d < ngpu; ++d ) {
        ...
        magma_minproductblas_dlaset( Magma_minproductFull, nb, nb, c_zero, c_zero, dV(d,k,0), ldv );
    }
    ...
    for (i = 0; i < nb; ++i) {
        ...
        if (i > 0) {
            ...
            tmp = MAGMA_minproduct_D_NEGATE( tau[i-1] );
        ...
        *A(k+i+1,i) = c_one;
        ...
        for( d = 0; d < ngpu; ++d ) {
            ...
            magma_minproduct_dsetvector_async( n_k_i_1,
                                    A(k+i+1,i), 1,
            ...
            magma_minproductblas_dlacpy( Magma_minproductFull, nb, nblocks-lblock,
            ...
            if ( dn-dki1 > 0 ) {
                magma_minproduct_dgemv( Magma_minproductNoTrans, n-k, dn-dki1,
                ...
                magma_minproduct_dgetvector_async( n-k,
                                        dY(d, k, i), 1,
        ...
        scale = MAGMA_minproduct_D_NEGATE( tau[i] );
        blasf77_dgemv( "Conj", &n_k_i_1, &i,
                       &scale,  A(k+i+1,0), &lda,
                                A(k+i+1,i), &ione,
                       &c_zero, T(0,i), &ione );
    ...
    *A(k+nb,nb-1) = ei;
    ...
    for( d = 0; d < ngpu; ++d ) {
        ...
        // which are summed in separate loop below
        if ( dn-dki1 > 0 ) {
            magma_minproduct_dgemm( Magma_minproductNoTrans, Magma_minproductNoTrans, k, nb, dn-dki1,
            ...
            magma_minproduct_dgetmatrix_async( k, nb,
                                    dY(d, 0, 0),  ldda,
    ...
    for( d = 0; d < ngpu; ++d ) {
        magma_minproduct_setdevice( d );
        magma_minproduct_dsetmatrix_async( n, nb, Y, ldy, dY(d, 0, 0), ldda, data->streams[d] );
        magma_minproduct_dsetmatrix_async( nb, nb, T, nb, dTi(d),      nb,   data->streams[d] );
    }
