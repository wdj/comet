...
{
    magma_tally2FloatComplex *da[Magma_tally2MaxGPUs];
    ...
    *info = 0;
    ...
    for (i=0; i < ngpu; i++) {
        ...
        if (MAGMA_tally2_SUCCESS != magma_tally2_cmalloc( &da[i], ldda*n_local[i] )) {
    ...
    if (m > nb && n > nb) {
        ...
        magma_tally2_csetmatrix_1D_col_bcyclic(m, n, A, lda, da, ldda, ngpu, nb);

        /* Factor using the GPU interface */
        magma_tally2_cgeqrf2_mgpu( ngpu, m, n, da, ldda, tau, info);
