...
int main( int argc, char** argv)
{
    ...
    magma_minproductFloatComplex *h_A, *h_R, *U, *VT, *h_work;
    magma_minproductFloatComplex dummy[1];
    float *S1, *S2;
    #ifdef COMPLEX
    float *rwork;
    #endif
    magma_minproduct_int_t M, N, N_U, M_VT, lda, ldu, ldv, n2, min_mn, info, nb, lwork;
    magma_minproduct_int_t ione     = 1;
    magma_minproduct_int_t ISEED[4] = {0,0,0,1};
    magma_minproduct_vec_t jobu, jobvt;
    ...
    if ( opts.check && ! opts.all && (jobu == Magma_minproductNoVec || jobvt == Magma_minproductNoVec)) {
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( jobu == Magma_minproductOverwriteVec && jobvt == Magma_minproductOverwriteVec ) {
            ...
            N = opts.nsize[itest];
            min_mn = min(M, N);
            N_U  = (jobu  == Magma_minproductAllVec ? M : min_mn);
            M_VT = (jobvt == Magma_minproductAllVec ? N : min_mn);
            lda = M;
            ldu = M;
            ldv = M_VT;
            n2 = lda*N;
            ...
            switch( opts.svd_work ) {
                case 0: {  // query for workspace size
                    lwork = -1;
                    magma_minproduct_cgesvd( jobu, jobvt, M, N,
                                  h_R, lda, S1, U, ldu, VT, ldv, dummy, lwork,
                                  #ifdef COMPLEX
                                  rwork,
                                  #endif
                                  &info );
                    lwork = (int) MAGMA_minproduct_C_REAL( dummy[0] );
                    break;
                }
            ...
            TESTING_MALLOC_CPU( VT,    magma_minproductFloatComplex, ldv*N );   // N x N (jobvt=A) or min(M,N) x N
            TESTING_MALLOC_CPU( U,     magma_minproductFloatComplex, ldu*N_U ); // M x M (jobu=A)  or M x min(M,N)
            ...
            if (info != 0)
                printf("magma_minproduct_cgesvd returned error %d: %s.\n",
            ...
            if ( opts.check ) {
                ...
                magma_minproductFloatComplex *U2  = NULL;
                magma_minproductFloatComplex *VT2 = NULL;
                if ( jobu == Magma_minproductSomeVec || jobu == Magma_minproductAllVec ) {
                    U2 = U;
                }
                else if ( jobu == Magma_minproductOverwriteVec ) {
                    U2 = h_R;
                    ldu = lda;
                }
                if ( jobvt == Magma_minproductSomeVec || jobvt == Magma_minproductAllVec ) {
                    VT2 = VT;
                }
                else if ( jobvt == Magma_minproductOverwriteVec ) {
                    VT2 = h_R;
                    ldv = lda;
                }
                ...
                magma_minproductFloatComplex *h_work_err;
                TESTING_MALLOC_CPU( h_work_err, magma_minproductFloatComplex, lwork_err );
            ...
            if ( opts.lapack ) {
                ...
                lapackf77_cgesvd( lapack_vec_const(jobu), lapack_vec_const(jobvt), &M, &N,
