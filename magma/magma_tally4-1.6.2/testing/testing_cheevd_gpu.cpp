...
int main( int argc, char** argv)
{
    ...
    magma_tally4FloatComplex *h_A, *h_R, *d_R, *h_work, aux_work[1];
    #ifdef COMPLEX
    float *rwork, aux_rwork[1];
    magma_tally4_int_t lrwork;
    #endif
    float *w1, *w2, result[4]={0, 0, 0, 0}, eps;
    magma_tally4_int_t *iwork, aux_iwork[1];
    magma_tally4_int_t N, n2, info, lwork, liwork, lda, ldda;
    ...
    printf("using: jobz = %s, uplo = %s\n",
           lapack_vec_const_tally4(opts.jobz), lapack_uplo_const_tally4(opts.uplo));
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_tally4_cheevd_gpu( opts.jobz, opts.uplo,
                              N, NULL, ldda, NULL,
                              NULL, lda,
                              aux_work,  -1,
                              #ifdef COMPLEX
                              ...
                              aux_iwork, -1,
                              &info );
            lwork  = (magma_tally4_int_t) MAGMA_tally4_C_REAL( aux_work[0] );
            #ifdef COMPLEX
            lrwork = (magma_tally4_int_t) aux_rwork[0];
            #endif
            liwork = aux_iwork[0];
            
            /* Allocate host memory for the matrix */
            TESTING_MALLOC_CPU( h_A,    magma_tally4FloatComplex, N*lda  );
            ...
            TESTING_MALLOC_PIN( h_R,    magma_tally4FloatComplex, N*lda  );
            TESTING_MALLOC_PIN( h_work, magma_tally4FloatComplex, lwork  );
            
            TESTING_MALLOC_DEV( d_R,    magma_tally4FloatComplex, N*ldda );
            ...
            if ( opts.warmup ) {
                magma_tally4_cheevd_gpu( opts.jobz, opts.uplo,
                                  ...
                                  &info );
                if (info != 0)
                    printf("magma_tally4_cheevd_gpu returned error %d: %s.\n",
            ...
            if ( opts.check && opts.jobz != Magma_tally4NoVec ) {
                ...
                //
                //magma_tally4_csetmatrix( N, N, h_A, lda, d_R, ldda );
                //magma_tally4_cheevd_gpu( Magma_tally4NoVec, opts.uplo,
                //                  N, d_R, ldda, w2,
                //                  h_R, lda,
                //                  h_work, lwork,
                //                  #ifdef COMPLEX
            ...
            if ( opts.lapack ) {
                ...
                lapackf77_cheevd( lapack_vec_const_tally4(opts.jobz), lapack_uplo_const_tally4(opts.uplo),
                                  &N, h_A, &lda, w2,
                                  h_work, &lwork,
                                  #ifdef COMPLEX
                                  rwork, &lrwork,
            ...
            if ( opts.lapack ) {
                printf("    | S_magma_tally4 - S_lapack | / |S| = %8.2e   %s\n\n", result[3], (result[3] < tolulp ? "ok" : "failed") );
