    ...
    magma_tally4DoubleComplex * dy)
{
    ...
    if(row < num_rows ){
        magma_tally4DoubleComplex dot1 = MAGMA_tally4_Z_MAKE(0.0, 0.0);
        magma_tally4DoubleComplex dot2 = MAGMA_tally4_Z_MAKE(0.0, 0.0);
        int offset = drowptr[ bdx ];
        int max_ = (drowptr[ bdx+1 ]-offset)/blocksize;  
            // number of elements each thread handles
    ...
    magma_tally4_queue_t queue )
{
    ...
    if ( (texture==1) && (precision==1) && (kepler==1) ) {
        ...
        int Ms = num_vecs * blocksize*alignment * sizeof( magma_tally4DoubleComplex );


        if ( alignment == 1) {
            dim3 block( blocksize, num_vecs/2, 1 );
            if ( beta == MAGMA_tally4_Z_MAKE( 0.0, 0.0 ) )
