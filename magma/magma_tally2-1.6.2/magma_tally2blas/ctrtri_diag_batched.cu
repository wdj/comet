    ...
    magma_tally2_int_t resetozero, magma_tally2_int_t batchCount, magma_tally2_queue_t queue)
{
    ...
    if(resetozero)
    { 
        magma_tally2blas_claset_batched(Magma_tally2Full, ((n+NB-1)/NB)*NB, NB, MAGMA_tally2_C_ZERO, MAGMA_tally2_C_ZERO, dinvA_array, ((n+NB-1)/NB)*NB, batchCount, queue);
       //magma_tally2blas_cmemset_batched( dinvA_array, ((n+NB-1)/NB)*NB*NB, batchCount, queue);
    }
    ...
    //was not the largest size computed by the high API getrf_batched then it is bug and need to use magma_tally2blas_claset_batched
