...
int main( int argc, char** argv)
{
    ...
    magma_tally2FloatComplex_ptr d_A, dT;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( opts.version == 2 ) {
                ...
                magma_tally2_cgeqrf2_gpu( M, N, d_A, ldda, tau, &info );
            }
            else {
                nb = magma_tally2_get_cgeqrf_nb( M );
                size = (2*min(M, N) + (N+31)/32*32 )*nb;
                TESTING_MALLOC_DEV( dT, magma_tally2FloatComplex, size );
                if ( opts.version == 1 ) {
                    // stores dT, V blocks have zeros, R blocks inverted & stored in dT
                    magma_tally2_cgeqrf_gpu( M, N, d_A, ldda, tau, dT, &info );
                }
                #ifdef HAVE_CUBLAS
                else if ( opts.version == 3 ) {
                    // stores dT, V blocks have zeros, R blocks stored in dT
                    magma_tally2_cgeqrf3_gpu( M, N, d_A, ldda, tau, dT, &info );
                }
                #endif
                else {
            ...
            if (info != 0)
                printf("magma_tally2_cgeqrf returned error %d: %s.\n",
            ...
            if ( opts.check && opts.version == 2 ) {
                ...
                magma_tally2FloatComplex *x, *b, *hwork;
                magma_tally2FloatComplex_ptr d_B;
                const magma_tally2FloatComplex c_zero    = MAGMA_tally2_C_ZERO;
                const magma_tally2FloatComplex c_one     = MAGMA_tally2_C_ONE;
                const magma_tally2FloatComplex c_neg_one = MAGMA_tally2_C_NEG_ONE;
                const magma_tally2_int_t ione = 1;
                ...
                TESTING_MALLOC_CPU( b, magma_tally2FloatComplex, M );
                lapackf77_clarnv( &ione, ISEED, &N, x );
                blasf77_cgemv( "Notrans", &M, &N, &c_one, h_A, &lda, x, &ione, &c_zero, b, &ione );
                // copy to GPU
                TESTING_MALLOC_DEV( d_B, magma_tally2FloatComplex, M );
                magma_tally2_csetvector( M, b, 1, d_B, 1 );

                if ( opts.version == 1 ) {
                    // allocate hwork
                    magma_tally2_cgeqrs_gpu( M, N, 1,
                                      d_A, ldda, tau, dT,
                                      d_B, M, tmp, -1, &info );
                    lwork = (magma_tally2_int_t)MAGMA_tally2_C_REAL( tmp[0] );
                    TESTING_MALLOC_CPU( hwork, magma_tally2FloatComplex, lwork );
                    ...
                    if (info != 0)
                        printf("magma_tally2_cgeqrs returned error %d: %s.\n",
                               (int) info, magma_tally2_strerror( info ));
                    ...
                    // allocate hwork
                    magma_tally2_cgeqrs3_gpu( M, N, 1,
                                       d_A, ldda, tau, dT,
                                       d_B, M, tmp, -1, &info );
                    lwork = (magma_tally2_int_t)MAGMA_tally2_C_REAL( tmp[0] );
                    TESTING_MALLOC_CPU( hwork, magma_tally2FloatComplex, lwork );
                    ...
                    magma_tally2_cgeqrs3_gpu( M, N, 1,
                                       ...
                                       d_B, M, hwork, lwork, &info );
                    if (info != 0)
                        printf("magma_tally2_cgeqrs3 returned error %d: %s.\n",
                               (int) info, magma_tally2_strerror( info ));
                    TESTING_FREE_CPU( hwork );
                }
                #endif
                ...
                magma_tally2_cgetvector( N, d_B, 1, x, 1 );
