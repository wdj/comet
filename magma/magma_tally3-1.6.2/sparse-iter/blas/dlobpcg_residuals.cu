...
// copied from dnrm2.cu in trunk/magma_tally3blas
// ----------------------------------------
...
template< int n >
__device__ void sum_reduce( /*int n,*/ int i, magma_tally3Double_ptr  x )
{
    __syncthreads();
...
__global__ void
magma_tally3_dlobpcg_res_kernel( 
    magma_tally3_int_t num_rows, 
    magma_tally3_int_t num_vecs, 
    magma_tally3Double_ptr evals, 
    double * X, 
    double * R,
    magma_tally3Double_ptr res)
{
    ...
    if( row<num_rows){
        for( int i=0; i<num_vecs; i++ ){ 
            ...
            R[row + i*num_rows] = R[row + i*num_rows] 
                                    + MAGMA_tally3_D_MAKE( -evals[i], 0.0 )
                                                * X[ row + i*num_rows ];
        }
    }
}
...
/**
    ...
    @param[in]
    evalues     magma_tally3Double_ptr 
...
magma_tally3_dlobpcg_res(
    magma_tally3_int_t num_rows,
    magma_tally3_int_t num_vecs, 
    magma_tally3Double_ptr evalues, 
    ...
    magma_tally3_queue_t queue )
{
    ...
    magma_tally3_dlobpcg_res_kernel<<< grid, threads, 0, queue >>>
