/*
    -- MAGMA_tally2 (version 1.6.1) --
       ...
       @generated from zgeqp3.cpp normal z -> d, Fri Jan 30 19:00:16 2015
...
/**
    ...
    @ingroup magma_tally2_dgeqp3_comp
    ********************************************************************/
...
#define dA(i, j) (dwork + (i) + (j)*(ldda))
    ...
    *info = 0;
    ...
    nb = magma_tally2_get_dgeqp3_nb(min(m, n));
    minmn = min(m,n);
    if (*info == 0) {
        if (minmn == 0) {
            lwkopt = 1;
    ...
    if (nfxd > 0) {
        ...
        if (na < n) {
            n_j = n - na;
            lapackf77_dormqr( Magma_tally2LeftStr, Magma_tally2ConjTransStr, &m, &n_j, &na,
    ...
    if (nfxd < minmn) {
        ...
        if (nb < sminmn) {
            ...
            magma_tally2_dsetmatrix_async( m, sn,
                                    A (0,j), lda,
                                    dA(0,j), ldda, stream );
        }
        ...
        for (j = nfxd; j < n; ++j) {
            rwork[j] = magma_tally2_cblas_dnrm2( sm, A(nfxd,j), ione );
            rwork[n + j] = rwork[j];
        }
        ...
        if (nb < sminmn) {
            ...
            while(j < topbmn) {
                ...
                if (j > nfxd) {
                    ...
                    magma_tally2_dgetmatrix( m-j, jb,
                                      dA(j,j), ldda,
                                      A (j,j), lda );
                    
                    // Get the rows
                    magma_tally2_dgetmatrix( jb, n_j - jb,
                                      dA(j,j + jb), ldda,
                                      A (j,j + jb), lda );
                }

                magma_tally2_dlaqps( m, n_j, j, jb, &fjb,
                              A (0, j), lda,
                              dA(0, j), ldda,
        ...
        if (j < minmn) {
            n_j = n - j;
            if (j > nfxd) {
                magma_tally2_dgetmatrix( m-j, n_j,
                                  dA(j,j), ldda,
                                  A (j,j), lda );
            }
