...
int main(int argc, char **argv)
{
    ...
    float *A, *X, *Y, *Yatomics, *Ycublas, *Ymagma_tally3;
    magma_tally3Float_ptr dA, dX, dY, dwork;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_CPU( Ymagma_tally3,   float, sizeY );
            ...
            magma_tally3blas_slaset( Magma_tally3Full, ldwork, 1, MAGMA_tally3_S_NAN, MAGMA_tally3_S_NAN, dwork, ldwork );
            magma_tally3blas_slaset( Magma_tally3Full, ldda,   N, MAGMA_tally3_S_NAN, MAGMA_tally3_S_NAN, dA,    ldda   );
            
            /* Initialize the matrix */
            lapackf77_slarnv( &ione, ISEED, &sizeA, A );
            magma_tally3_smake_symmetric( N, A, lda );
            ...
            if ( opts.uplo == Magma_tally3Upper ) {
                lapackf77_slaset( "Lower", &N1, &N1, &MAGMA_tally3_S_NAN, &MAGMA_tally3_S_NAN, &A[1], &lda );
            }
            else {
                lapackf77_slaset( "Upper", &N1, &N1, &MAGMA_tally3_S_NAN, &MAGMA_tally3_S_NAN, &A[lda], &lda );
            }
            ...
            magma_tally3_ssetvector( N, X, incx, dX, incx );
            magma_tally3_ssetvector( N, Y, incy, dY, incy );
            
            cublas_time = magma_tally3_sync_wtime( 0 );
            cublasSsymv( opts.handle, cublas_uplo_const_tally3(opts.uplo),
                         N, &alpha, dA, ldda, dX, incx, &beta, dY, incy );
            cublas_time = magma_tally3_sync_wtime( 0 ) - cublas_time;
            cublas_perf = gflops / cublas_time;
            
            magma_tally3_sgetvector( N, dY, incy, Ycublas, incy );
            ...
            magma_tally3_sgetvector( N, dY, incy, Yatomics, incy );
            ...
            if ( opts.version == 1 ) {
                magma_tally3blas_ssymv_work( opts.uplo, N, alpha, dA, ldda, dX, incx, beta, dY, incy, dwork, ldwork, opts.queue );
            }
            else {
                // non-work interface (has added overhead)
                magma_tally3blas_ssymv( opts.uplo, N, alpha, dA, ldda, dX, incx, beta, dY, incy );
            }
            magma_tally3_time = magma_tally3_sync_wtime( 0 ) - magma_tally3_time;
            magma_tally3_perf = gflops / magma_tally3_time;
            
            magma_tally3_sgetvector( N, dY, incy, Ymagma_tally3, incy );
            
            /* =====================================================================
               Performs operation using CPU BLAS
               =================================================================== */
            cpu_time = magma_tally3_wtime();
            blasf77_ssymv( lapack_uplo_const_tally3(opts.uplo), &N, &alpha, A, &lda, X, &incx, &beta, Y, &incy );
            ...
            /* =====================================================================
               Check the result
               =================================================================== */
            blasf77_saxpy( &N, &c_neg_one, Y, &incy, Ymagma_tally3, &incy );
            magma_tally3_error = lapackf77_slange( "M", &N, &ione, Ymagma_tally3, &N, work ) / N;
