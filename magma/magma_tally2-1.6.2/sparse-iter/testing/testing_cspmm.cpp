...
int main(  int argc, char** argv )
{
    ...
    while( i < argc ) {
        ...
        // init CPU vectors
        CHECK( magma_tally2_cvinit( &hx, Magma_tally2_CPU, m, n, c_one, queue ));
        CHECK( magma_tally2_cvinit( &hy, Magma_tally2_CPU, m, n, c_zero, queue ));

        // init DEV vectors
        CHECK( magma_tally2_cvinit( &dx, Magma_tally2_DEV, m, n, c_one, queue ));
        CHECK( magma_tally2_cvinit( &dy, Magma_tally2_DEV, m, n, c_zero, queue ));


        // calling MKL with CSR
        #ifdef MAGMA_tally2_WITH_MKL
            magma_tally2_int_t *pntre=NULL;
            CHECK( magma_tally2_imalloc_cpu( &pntre, m + 1 ) );
            pntre[0] = 0;
            for (j=0; j < m; j++ ) {
                pntre[j] = hA.row[j+1];
            }
        ...
        CHECK( magma_tally2_cvinit( &dy, Magma_tally2_DEV, dx.num_rows, dx.num_cols, c_zero, queue ));
        // SpMV on GPU (SELLP)
        start = magma_tally2_sync_wtime( queue );
        for (j=0; j<10; j++)
