...
/* ////////////////////////////////////////////////////////////////////////////
   -- Testing magma_tally2_zher2k_mgpu
*/
int main( int argc, char** argv)
{
    ...
    magma_tally2DoubleComplex alpha = MAGMA_tally2_Z_MAKE( 1.2345, 4.321 );
    double beta = 3.14159;
    
    real_Double_t    gflops, gpu_perf, cpu_perf, gpu_time, cpu_time;
    double           error, work[1];
    magma_tally2DoubleComplex *hA, *hR, *hR2, *hV, *hW;
    magma_tally2DoubleComplex_ptr dV[Magma_tally2MaxGPUs], dW[Magma_tally2MaxGPUs], dA[Magma_tally2MaxGPUs];
    ...
    printf( "version 1: magma_tally2blas_zher2k_mgpu2     %s\n", (opts.version==1 ? "(enabled)" : ""));
    printf( "version 2: magma_tally2blas_zher2k_mgpu_spec %s\n", (opts.version==2 ? "(enabled)" : ""));
#ifdef ICHI
    printf( "version 3: magma_tally2_zher2k_mgpu (Ichi)   %s\n", (opts.version==3 ? "(enabled)" : ""));
#endif
    printf( "\n" );
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        ...
        for( int offset = 0; offset < n; offset += min(k,nb) ) {
            for( int iter = 0; iter < opts.niter; ++iter ) {
                ...
                TESTING_MALLOC_CPU( hR,  magma_tally2DoubleComplex, lda*n   );
                TESTING_MALLOC_CPU( hR2, magma_tally2DoubleComplex, lda*n   );
                TESTING_MALLOC_CPU( hV,  magma_tally2DoubleComplex, lda*k*2 );
                //TESTING_MALLOC_CPU( hW,  magma_tally2DoubleComplex, lda*k   );
                for( int d = 0; d < ngpu; ++d ) {
                    magma_tally2_int_t nlocal = ((n / nb) / ngpu + 1) * nb;
                    magma_tally2_setdevice( d );
                    TESTING_MALLOC_DEV( dA[d], magma_tally2DoubleComplex, ldda*nlocal );
                    TESTING_MALLOC_DEV( dV[d], magma_tally2DoubleComplex, ldda*k*2    );
                    //TESTING_MALLOC_DEV( dW[d], magma_tally2DoubleComplex, ldda*k      );
                ...
                /* ====================================================================
                   ...
                   =================================================================== */
                magma_tally2_zsetmatrix_1D_col_bcyclic( n, n, hA, lda, dA, ldda, ngpu, nb );
                for( int d = 0; d < ngpu; ++d ) {
                    magma_tally2_setdevice( d );
                    dW[d] = dV[d] + ldda*k;
                    magma_tally2_zsetmatrix( n, k, hV, lda, dV[d], ldda );
                    magma_tally2_zsetmatrix( n, k, hW, lda, dW[d], ldda );
                }
                ...
                magma_tally2_zgetmatrix_1D_col_bcyclic( n, n, dA, ldda, hR, lda, ngpu, nb );
