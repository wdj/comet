    ...
    magma_tally4_queue_t queue )
{
    ...
    magma_tally4_s_matrix r={Magma_tally4_CSR}, d={Magma_tally4_CSR}, z={Magma_tally4_CSR};
    ...
    CHECK( magma_tally4_svinit( &d, Magma_tally4_DEV, A.num_rows, b.num_cols, c_zero, queue ));
    CHECK( magma_tally4_svinit( &z, Magma_tally4_DEV, A.num_rows, b.num_cols, c_zero, queue ));
    
    CHECK( magma_tally4_smalloc( &d1, dofs*(1) ));
    CHECK( magma_tally4_smalloc( &d2, dofs*(1) ));
    // array for the parameters
    CHECK( magma_tally4_smalloc( &skp, 6 ));
    ...
    magma_tally4_scopy( dofs, r.dval, 1, d.dval, 1 );                    // d = r
    nom0 = betanom = magma_tally4_snrm2( dofs, r.dval, 1 );
    nom = nom0 * nom0;                                           // nom = r' * r
    CHECK( magma_tally4_s_spmv( c_one, A, d, c_zero, z, queue ));              // z = A d
    den = MAGMA_tally4_S_REAL( magma_tally4_sdot(dofs, d.dval, 1, z.dval, 1) ); // den = d'* z
    solver_par->init_res = nom0;
    
    // array on host for the parameters
    CHECK( magma_tally4_smalloc_cpu( &skp_h, 6 ));
    
    alpha = rho = gamma = tmp1 = c_one;
    beta =  magma_tally4_sdot(dofs, r.dval, 1, r.dval, 1);
    skp_h[0]=alpha;
    skp_h[1]=beta;
    skp_h[2]=gamma;
    skp_h[3]=rho;
    ...
    magma_tally4_ssetvector( 6, skp_h, 1, skp, 1 );
    ...
    do
    {
        ...
        CHECK( magma_tally4_scgmerge_spmv1(  A, d1, d2, d.dval, z.dval, skp, queue ));
            
        // updates x, r, computes scalars and updates d
        CHECK( magma_tally4_scgmerge_xrbeta( dofs, d1, d2, x->dval, r.dval, d.dval, z.dval, skp, queue ));

        // check stopping criterion (asynchronous copy)
        magma_tally4_sgetvector_async( 1 , skp+1, 1,
                                                    skp_h+1, 1, stream[1] );
        betanom = sqrt(MAGMA_tally4_S_REAL(skp_h[1]));
