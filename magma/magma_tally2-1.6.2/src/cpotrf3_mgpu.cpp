...
#define CTRSM_WORK
//#define magma_tally2_ctrsm magma_tally2blas_ctrsm
#endif
...
#define dlPT(id, i, j, k) (d_lP[(id)] + (k)*nb*lddp + (j)*nb   + (i))
    ...
    magma_tally2FloatComplex *dlpanel;
    ...
    *info = 0;
    ...
    magma_tally2FloatComplex *d_dinvA[Magma_tally2MaxGPUs];
    magma_tally2FloatComplex *d_x[Magma_tally2MaxGPUs];
    ...
    for( d=0; d < ngpu; d++ ) {
        ...
        if ( (MAGMA_tally2_SUCCESS != magma_tally2_cmalloc( &d_dinvA[d], 2*trsm_nb*trsm_n )) ||
             (MAGMA_tally2_SUCCESS != magma_tally2_cmalloc( &d_x[d],     2*nb*(upper ? n : m) )) ) {
    ...
    if (upper) {
        ...
        for (j=0; j < m; j += nb) {
            ...
            if ( j > 0 ) {
                ...
                magma_tally2_cherk(Magma_tally2Upper, Magma_tally2ConjTrans, jb, j,
            ...
            magma_tally2_cgetmatrix_async( jb, jb,
            ...
            lapackf77_cpotrf(Magma_tally2UpperStr, &jb, Aup(j,j), &lda, info);
            trace_cpu_end( 0 );
            if (*info != 0) {
                *info = *info + j;
                break;
            ...
            if ( (j+jb) < n) {
                ...
                for( dd=0; dd < ngpu; dd++ ) {
                    ...
                    magma_tally2_csetmatrix_async( jb, jb,
                                            Aup(j,j), lda,
                                            dlpanel,  ldpanel,
                                            queues[d][stream1] );
            ...
            if ( (j+jb) < n) {
...
#if (defined(PRECISION_d) || defined(PRECISION_s)) && defined(CTRSM_WORK)
                        magma_tally2blas_claset( Magma_tally2Full, trsm_nb, trsm_n, c_zero, c_zero, dinvA(d,0), trsm_nb );
                        magma_tally2blas_claset( Magma_tally2Full, nb0,     jb,     c_zero, c_zero, dx(d,0), nb0 );
                        magma_tally2blas_ctrsm_work( Magma_tally2Left, Magma_tally2Upper,
                                              ...
                                              1, dinvA(d,0), dx(d,0) );
#else
                        magma_tally2_ctrsm( Magma_tally2Left, Magma_tally2Upper,
                                     Magma_tally2ConjTrans, Magma_tally2NonUnit,
                                     jb, nb0, c_one,
                                     dlpanel,                ldpanel,
                                     dlA(d, j, nb*j_local2), ldda);
...
#if (defined(PRECISION_d) || defined(PRECISION_s)) && defined(CTRSM_WORK)
                        magma_tally2blas_claset( Magma_tally2Full, trsm_nb, trsm_n, c_zero, c_zero, dinvA(d,0), trsm_nb );
                        magma_tally2blas_claset( Magma_tally2Full, nb2,     jb,     c_zero, c_zero, dx(d,0), nb2 );
                        magma_tally2blas_ctrsm_work( Magma_tally2Left, Magma_tally2Upper,
                                              Magma_tally2ConjTrans, Magma_tally2NonUnit,
                                              jb, nb2, c_one,
                                              dlpanel, ldpanel,
                ...
                if ( j+jb < m ) {
                    ...
                    magma_tally2_cgetmatrix_async( (j+jb), nb0,
                    ...
                    for( d2=0; d2 < ngpu; d2++ ) {
                        if ( d2 != d ) {
                            ...
                            magma_tally2_csetmatrix_async( j+jb, nb0,
                                                    Aup(0,j+jb),       lda,
                    ...
                    if ( nb2 > 0 ) {
                        ...
                        if (flag == 0) {
                            ...
                            magma_tally2blas_claset( Magma_tally2Full, trsm_nb, trsm_n, c_zero, c_zero, dinvA(d,flag), trsm_nb );
                            magma_tally2_queue_wait_event( queues[d][stream2], events[d][1] ); // panel received
                        }
                        magma_tally2blas_claset( Magma_tally2Full, nb2, jb, c_zero, c_zero, dx(d,1), nb2 );
                        magma_tally2blas_ctrsm_work( Magma_tally2Left, Magma_tally2Upper, Magma_tally2ConjTrans, Magma_tally2NonUnit,
                                              jb, nb2, c_one,
                                              dlpanel, ldpanel,
        ...
        for (j=0; j < n; j += nb) {
            ...
            if ( j > 0 ) {
                magma_tally2blasSetKernelStream( queues[id][stream1] );
                magma_tally2_cherk(Magma_tally2Lower, Magma_tally2NoTrans, jb, j,
            ...
            magma_tally2_setdevice(id);
            magma_tally2_queue_sync( queues[id][stream1] );
            lapackf77_cpotrf(Magma_tally2LowerStr, &jb, Alo(j,j), &lda, info);
            if (*info != 0) {
                *info = *info + j;
                break;
            }
            ...
            if ( (j+jb) < m) {
                ...
                for( dd=0; dd < ngpu; dd++ ) {
                    ...
                    if ( j+nb < n && d == (j/nb+1)%ngpu ) { /* owns next column, look-ahead next block on stream1 */
                        ...
                        magma_tally2blas_claset( Magma_tally2Full, trsm_nb, trsm_n, c_zero, c_zero, dinvA(d,0), trsm_nb );
                        magma_tally2blas_claset( Magma_tally2Full, nb0,     jb,     c_zero, c_zero, dx(d,0), nb0 );
                        magma_tally2blas_ctrsm_work( Magma_tally2Right, Magma_tally2Lower,
                                              Magma_tally2ConjTrans, Magma_tally2NonUnit,
                                              nb0, jb, c_one,
                                              dlpanel, ldpanel,
                                              ...
                                              1, dinvA(d,0), dx(d,0) );
#else
                        magma_tally2_ctrsm( Magma_tally2Right, Magma_tally2Lower,
                                     Magma_tally2ConjTrans, Magma_tally2NonUnit,
                                     nb0, jb, c_one,
                                     dlpanel, ldpanel,
                                     dlA(d, nb*j_local2, j), ldda);
                        ...
                        magma_tally2blas_claset( Magma_tally2Full, trsm_nb, trsm_n, c_zero, c_zero, dinvA(d,0), trsm_nb );
                        magma_tally2blas_claset( Magma_tally2Full, nb2,     jb,     c_zero, c_zero, dx(d,0), nb2 );
                        magma_tally2blas_ctrsm_work( Magma_tally2Right, Magma_tally2Lower, Magma_tally2ConjTrans, Magma_tally2NonUnit,
                                              nb2, jb, c_one,
                                              dlpanel,                ldpanel,
                                              dlA(d, nb*j_local2, j), ldda,
                                              1, dinvA(d,0), dx(d,0) );
...
//#define CPOTRF_DEVICE_TO_DEVICE
                    ...
                    for( d2=0; d2 < ngpu; d2++ ) {
                        if ( d2 != d ) {
                            ...
                            magma_tally2_csetmatrix_async( nb0, j+jb,
                                                    Alo(j+jb,0),        lda,
                                                    dlPT(d2,0,nb,buf2), nb, // first nbxnb reserved for diagonal block
                                                    queues[d2][stream3] );
                            magma_tally2_event_record( events[d2][0], queues[d2][stream3] );
    ...
    magma_tally2_int_t *info)
{
    ...
    if (uplo == Magma_tally2Upper) {
        ...
        for (j=off_j; j < n; j += nb) {
            ...
            magma_tally2_csetmatrix_async( mj, jb,
                                    A(off_i, j),     lda,
                                    dA(k, 0, jj*nb), ldda,
                                    queues[k][0] );
        }
        ...
        for (i=off_i; i < m; i += nb) {
            ...
            magma_tally2_setdevice(k);
            magma_tally2_csetmatrix_async( ib, ni,
                                    A(i, off_j),     lda,
                                    dA(k, ii*nb, 0), ldda,
                                    queues[k][0] );
        }
    ...
    magma_tally2_int_t *info)
{
    ...
    if (uplo == Magma_tally2Upper) {
        ...
        for (j=off_j+NB; j < n; j += nb) {
            ...
            magma_tally2_setdevice(k);
            magma_tally2_cgetmatrix_async( mj, jb,
                                    dA(k, 0, jj*nb), ldda,
                                    A(off_i, j),     lda,
                                    queues[k][0] );
            magma_tally2_queue_sync( queues[k][0] );
        ...
        for (i=off_i+NB; i < m; i += nb) {
            ...
            magma_tally2_setdevice(k);
            magma_tally2_cgetmatrix_async( ib, ni,
                                    dA(k, ii*nb, 0), ldda,
                                    A(i, off_j),     lda,
                                    queues[k][0] );
            magma_tally2_queue_sync( queues[k][0] );
