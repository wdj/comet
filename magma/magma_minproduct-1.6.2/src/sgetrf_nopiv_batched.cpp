...
#define A(i_, j_)  (A + (i_) + (j_)*lda)   
   ...
   /* check allocation */
    ...
    for(i = 0; i < min_mn; i+=nb) 
    {
        ...
        magma_minproduct_sdisplace_pointers(dA_displ, dA_array, lda, i, i, batchCount, queue);
        sset_pointer(dwork_array, dwork, nb, 0, 0, dwork_msize, batchCount, queue);
#if 0
        /* buggy: TODO */
        arginfo = magma_minproduct_sgetrf_panel_nopiv_batched(
                pm, ib,
                dA_displ, lda,
                dwork_array, nb, 
...
#else
        arginfo = magma_minproduct_sgetrf_recpanel_nopiv_batched(
                pm, ib, 32,
                dA_displ, lda,
                dwork_array, nb, 
                dinvA_array, invA_msize, 
...
#define RUN_ALL
        ...
        if( (i + ib) < n)
        {
            ...
            //magma_minproduct_sdisplace_pointers(dA_displ, dA_array, lda, i, i+ib, batchCount);
            sset_pointer(dwork_array, dwork, nb, 0, 0, dwork_msize, batchCount, queue); // I don't think it is needed Azzam

            magma_minproduct_sdisplace_pointers(dA_displ, dA_array, lda, i, i, batchCount, queue);
            magma_minproduct_sdisplace_pointers(dW0_displ, dA_array, lda, i, i+ib, batchCount, queue);
            magma_minproductblas_strsm_work_batched(Magma_minproductLeft, Magma_minproductLower, Magma_minproductNoTrans, Magma_minproductUnit, 1,
                    ib, n-i-ib,
            ...
            {    
                ...
                {
                    magma_minproduct_sdisplace_pointers(dA_displ, dA_array,  lda, i+ib,    i, batchCount, queue);
                    magma_minproduct_sdisplace_pointers(dW1_displ, dA_array, lda,    i, i+ib, batchCount, queue);
                    magma_minproduct_sdisplace_pointers(dW2_displ, dA_array, lda, i+ib, i+ib, batchCount, queue);
