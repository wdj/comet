...
/**
    ...
    T       DOUBLE_PRECISION array, dimension (NB, min(M,N)).
            ...
            magma_tally2_dgeqrf_gpu (except stored on the CPU, not the GPU).
...
#define dT(d,i,j) (dT[d] + (i) + (j)*nb)
    ...
    *info = 0;
    ...
    double *dA[ Magma_tally2MaxGPUs ] = { NULL };
    double *dT[ Magma_tally2MaxGPUs ] = { NULL };
    double *dV[ Magma_tally2MaxGPUs ] = { NULL };
    double *dW[ Magma_tally2MaxGPUs ] = { NULL };
    ...
    for( int d = 0; d < ngpu; ++d ) {
        ...
        ldwork = nlocal[d]*ldda  // dA
               ...
               + nb*ldda         // dV
               + nb*lddwork;     // dW
        if ( MAGMA_tally2_SUCCESS != magma_tally2_dmalloc( &dA[d], ldwork )) {
    ...
    if (kk < n) {
        ...
        magma_tally2blas_dlaset( Magma_tally2Full, kk, n - kk, c_zero, c_zero, dA(dpanel, 0, di), ldda );
        trace_cpu_end( 0 );
    }
    ...
    if (kk > 0) {
        ...
        for( int d = 0; d < ngpu; ++d ) {
            ...
            magma_tally2_dsetmatrix_async( nb, min(m,n), T, nb, dT[d], nb, stream[d] );
            trace_gpu_end( d, 0 );
        }
        ...
        for( i = ki; i >= 0; i -= nb ) {
            ...
            for( int d = 0; d < ngpu; ++d ) {
                ...
                magma_tally2_dsetmatrix_async( mi, ib,
                                        A(i, i), lda,
                                        dV[d],   ldda, stream[d] );
                trace_gpu_end( d, 0 );
            }
            ...
            magma_tally2blas_dlaset( Magma_tally2Full, i,  ib, c_zero, c_zero, dA(dpanel, 0, di), ldda );
            magma_tally2blas_dlaset( Magma_tally2Full, mi, ib, c_zero, c_one,  dA(dpanel, i, di), ldda );
