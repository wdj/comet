...
#define dW1(dev, i, j) (dW[(dev)] + ((j)+nb)     *lddw + (i))
    ...
    magma_tally2_int_t i, n_i, n_i_1, ip1, iw;
    ...
    if (uplo == Magma_tally2Upper) {
        ...
        for (i = n-1; i >= n - nb; --i) {
            ...
            if (i < n-1) {
                /* Update A(1:i,i) */
                magma_tally2DoubleComplex wii = -conj( *W(i, iw+1) );
            ...
            if (i > 0) {
                ...
                *A(i-1,i) = MAGMA_tally2_Z_ONE;
                ...
                for( dev=0; dev < ngpu; dev++ ) {
                    ...
                    magma_tally2_zsetvector_async( n, A(0,i), 1, dW1(dev, 0, iw), 1, queues[dev] );
                }
                magma_tally2blas_zhemv_mgpu(
                    Magma_tally2Upper, i, c_one, dA, ldda, 0,
                    A(0,i), 1, c_zero, W(0, iw), 1,
                    hwork, lhwork, dwork, ldwork, ngpu, nb0, queues );

                if (i < n-1) {
                    blasf77_zgemv( Magma_tally2ConjTransStr, &i, &n_i_1, &c_one,
                                   W(0,   iw+1), &ldw,
                                   A(0,   i),    &ione, &c_zero,
                                   W(i+1, iw),   &ione );
                }
                ...
                for( dev=0; dev < ngpu; dev++ ) {
                    magma_tally2_setdevice( dev );
                    magma_tally2_zsetvector_async( n, W(0,iw), 1, dW(dev, 0, iw), 1, queues[dev] );
                }
            }
        }
    } else {
        ...
        for (i = 0; i < nb; ++i) {
            ...
            if ( i > 0 ) {
                ...
                magma_tally2DoubleComplex wii = -conj( *W(i, i-1) );
            ...
            if (i < n-1) {
                ...
                *A(i+1,i) = MAGMA_tally2_Z_ONE;
                ...
                for( dev=0; dev < ngpu; dev++ ) {
                    magma_tally2_setdevice( dev );
                    magma_tally2_zsetvector_async( n, A(0,i), 1, dW1(dev, 0, i), 1, queues[dev] );
                }
                
                magma_tally2blas_zhemv_mgpu(
                    Magma_tally2Lower, n_i_1, c_one, dA, ldda, offset+i+1,
                    A(i+1, i), 1, c_zero, W(i+1, i), 1,
                    hwork, lhwork, dwork, ldwork, ngpu, nb0, queues );
                
                trace_cpu_start( 0, "gemv", "gemv" );
                blasf77_zgemv( Magma_tally2ConjTransStr, &n_i_1, &i, &c_one,
                               W(i+1, 0), &ldw,
                               A(i+1, i), &ione, &c_zero,
                               W(0,   i), &ione );
                ...
                value = magma_tally2_cblas_zdotc( n_i_1, W(i+1,i), ione, A(i+1,i), ione );
                alpha = tau[i] * -0.5f * value;
                blasf77_zaxpy( &n_i_1, &alpha, A(i+1, i), &ione, W(i+1,i), &ione );
                trace_cpu_end( 0 );
                for( dev=0; dev < ngpu; dev++ ) {
                    magma_tally2_setdevice( dev );
                    magma_tally2_zsetvector_async( n, W(0,i), 1, dW(dev, 0, i), 1, queues[dev] );
                }
            }
        }
    }
