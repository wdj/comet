...
int main( int argc, char** argv)
{
    ...
    magma_minproduct_int_t N, nb, size, lda, ldda, mstride, nstride, ntile;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            gbytes = sizeof(magma_minproductFloatComplex) * 2.*nb*nb*ntile / 1e9;
            ...
            magma_minproductblas_csymmetrize_tiles( opts.uplo, nb, d_A, ldda, ntile, mstride, nstride );
                ...
                for( int j = 0; j < nb; ++j ) {
                    for( int i = 0; i < j; ++i ) {
                        if ( opts.uplo == Magma_minproductLower ) {
                            h_A[offset + i + j*lda] = MAGMA_minproduct_C_CNJG( h_A[offset + j + i*lda] );
                        }
                        else {
                            h_A[offset + j + i*lda] = MAGMA_minproduct_C_CNJG( h_A[offset + i + j*lda] );
                        }
                    }
                }
            }
