                  ...
                  magma_tally3_int_t batchCount, magma_tally3_queue_t queue)
{
    ...
    magma_tally3FloatComplex *hu, *hv;
    if (MAGMA_tally3_SUCCESS != magma_tally3_cmalloc_cpu( &hu, 2*n )) {
        info = MAGMA_tally3_ERR_HOST_ALLOC;
        return info;
    }

    if (MAGMA_tally3_SUCCESS != magma_tally3_cmalloc_cpu( &hv, 2*n )) {
        info = MAGMA_tally3_ERR_HOST_ALLOC;
        return info;
    }
    ...
    info = magma_tally3_cgerbt_batched(Magma_tally3True, n, nrhs, dA_array, n, dB_array, n, hu, hv, &info, batchCount, queue);
    ...
    if (MAGMA_tally3_SUCCESS != magma_tally3_cmalloc( &dv, 2*n )) {
        info = MAGMA_tally3_ERR_DEVICE_ALLOC;
        return info;
    }

    magma_tally3_csetvector(2*n, hv, 1, dv, 1);

    for(int i = 0; i < nrhs; i++)
        magma_tally3blas_cprbt_mv_batched(n, dv, dB_array+(i), batchCount, queue);
