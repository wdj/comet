...
/**
    ...
    @ingroup magma_minproduct_sgesvd_comp
    ********************************************************************/
...
#define dA(i, j) (dA + (j)*ldda + (i))
    ...
    nb   = magma_minproduct_get_sgebrd_nb(n);
    ldda = m;
    ...
    *info = 0;
    ...
    if (MAGMA_minproduct_SUCCESS != magma_minproduct_smalloc( &dA, n*ldda + (m + n)*nb )) {
    ...
    for (i=0; i < (minmn - nx); i += nb) {
        ...
        if ( i > 0 ) {
            magma_minproduct_sgetmatrix( nrow, nb, dA(i, i), ldda, A( i, i), lda );
            magma_minproduct_sgetmatrix( nb, ncol - nb,
                              dA(i, i+nb), ldda,
                              A( i, i+nb), lda );
        }

        magma_minproduct_slabrd_gpu(nrow, ncol, nb,
        ...
        magma_minproduct_ssetmatrix( nrow, nb, work  + nb, ldwrkx, dwork + nb, ldwrkx );
        magma_minproduct_ssetmatrix( ncol, nb,
        ...
        if (m >= n) {
            jmax = i + nb;
            for (j = i; j < jmax; ++j) {
                *A(j, j  ) = MAGMA_minproduct_S_MAKE( d[j], 0. );
                *A(j, j+1) = MAGMA_minproduct_S_MAKE( e[j], 0. );
            }
        } else {
            jmax = i + nb;
            for (j = i; j < jmax; ++j) {
    ...
    if ( 0 < minmn - nx ) {
        magma_minproduct_sgetmatrix( nrow, ncol, dA(i, i), ldda, A(i, i), lda );
    }
