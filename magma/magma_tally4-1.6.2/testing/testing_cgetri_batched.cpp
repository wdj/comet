...
int main( int argc, char** argv)
{
    ...
    magma_tally4FloatComplex *d_A, *d_invA;
    magma_tally4FloatComplex **dA_array = NULL;
    magma_tally4FloatComplex **dinvA_array = NULL;
    magma_tally4FloatComplex **C_array = NULL;
    ...
    for( int i = 0; i < opts.ntest; ++i ) {
      ...
      for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_DEV(  d_invA,  magma_tally4FloatComplex, ldda*N * batchCount);
            ...
            lapackf77_clacpy( Magma_tally4UpperLowerStr, &M, &columns, h_A, &lda, h_R, &lda );
            magma_tally4_csetmatrix( M, columns, h_R, lda, d_A, ldda );
            ...
            info1 = magma_tally4_cgetrf_batched( M, N, dA_array, ldda, dipiv_array, dinfo_array, batchCount, queue);
            info2 = magma_tally4_cgetri_outofplace_batched( min(M,N), dA_array, ldda, dipiv_array, dinvA_array, ldda, dinfo_array, batchCount, queue);
            ...
            if (info1 != 0) printf("magma_tally4_cgetrf_batched returned argument error %d: %s.\n", (int) info1, magma_tally4_strerror( info1 ));
            if (info2 != 0) printf("magma_tally4_cgetri_batched returned argument error %d: %s.\n", (int) info2, magma_tally4_strerror( info2 ));
            /* =====================================================================
            ...
            if ( opts.lapack ) {
                ...
                lwork = magma_tally4_int_t( MAGMA_tally4_C_REAL( tmp ));
                TESTING_MALLOC_CPU( work,  magma_tally4FloatComplex, lwork  );
                lapackf77_clacpy( Magma_tally4UpperLowerStr, &M, &columns, h_R, &lda, h_A, &lda );
            ...
            if ( opts.check ) {
                ...
                magma_tally4_cgetmatrix( min(M,N), N*batchCount, d_invA, ldda, h_R, lda );
                int stop=0;
                n2     = lda*N;
                for(int i=0; i < batchCount; i++)
                {
