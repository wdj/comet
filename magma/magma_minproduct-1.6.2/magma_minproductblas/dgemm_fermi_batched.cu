    ...
    double **dC_array, magma_minproduct_int_t lddc, magma_minproduct_int_t batchCount, magma_minproduct_queue_t queue )
{
    ...
    if      ( transA != Magma_minproductNoTrans && transA != Magma_minproductTrans && transA != Magma_minproductConjTrans )
        ...
        info = -5;
    else if ( transA == Magma_minproductNoTrans ? ldda < m : ldda < k )
        info = -8;
    else if ( transB == Magma_minproductNoTrans ? lddb < k : lddb < n )
        info = -10;
    else if ( lddc < m )
        info = -13;
    ...
    if ( arch < 200  ) {
        ...
        // magma_minproductblas for [sd] precisions, cublas for [zc] precisions.
    ...
    if      ( transA == Magma_minproductTrans )
        TransA = 1;
    else if ( transA == Magma_minproductNoTrans )
        TransA = 0;
                    
    if      ( transB == Magma_minproductTrans )
        TransB = 1;
    else if ( transB == Magma_minproductNoTrans )
        TransB = 0;
        ...
        magma_minproduct_getvector( min(2,batchCount), sizeof(double*), dA_array, 1, cpuAarray, 1);
        magma_minproduct_getvector( min(2,batchCount), sizeof(double*), dB_array, 1, cpuBarray, 1);
        magma_minproduct_int_t matrixA_size = batchCount > 1 ? cpuAarray[1] - cpuAarray[0] : 0;
        magma_minproduct_int_t matrixB_size = batchCount > 1 ? cpuBarray[1] - cpuBarray[0] : 0;
             ...
             sizeB >= CUBLAS_MAX_1DBUF_SIZE )
        {
            ...
            return;
            /*
            magma_minproduct_dgemm( transA, transB, m, n, k, alpha,
