    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int itran = 0; itran < 2; ++itran ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            n = opts.nsize[itest];
            k = opts.ksize[itest];
            nb  = magma_tally4_get_cgeqlf_nb( m );
            ldc = m;
            ...
            magma_tally4_cgeqlf( mm, k, A, lda, tau, W, lwork_max, &info );
            if (info != 0)
                printf("magma_tally4_cgeqlf returned error %d: %s.\n",
            ...
            lapackf77_cunmql( lapack_side_const_tally4( side[iside] ), lapack_trans_const_tally4( trans[itran] ),
                              &m, &n, &k,
            ...
            lwork = -1;
            magma_tally4_cunmql( side[iside], trans[itran],
                          m, n, k,
                          A, lda, tau, R, ldc, W, lwork, &info );
            if (info != 0)
                printf("magma_tally4_cunmql (lwork query) returned error %d: %s.\n",
                       (int) info, magma_tally4_strerror( info ));
            ...
            if (info != 0)
                printf("magma_tally4_cunmql returned error %d: %s.\n",
                       (int) info, magma_tally4_strerror( info ));
