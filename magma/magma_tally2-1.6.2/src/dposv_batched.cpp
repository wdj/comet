                  ...
                  magma_tally2_int_t batchCount, magma_tally2_queue_t queue)
{
    ...
    info = magma_tally2_dpotrf_batched( uplo, n, dA_array, ldda, dinfo_array, batchCount, queue);
    ...
    {
        if(cpu_info[i] != 0 ){
            printf("magma_tally2_dpotrf_batched matrix %d returned error %d\n",i, (int)cpu_info[i] );
            info = cpu_info[i];
            free (cpu_info);
            return info;
        }
    ...
    info = magma_tally2_dpotrs_batched( uplo, n, nrhs, dA_array, ldda, dB_array, lddb,  batchCount, queue );
    return info;
}
