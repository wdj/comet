...
} magma_tally4gpu_sswapblk_params_t;

__global__ void magma_tally4gpu_sswapblkrm( magma_tally4gpu_sswapblk_params_t params )
{
...
__global__ void magma_tally4gpu_sswapblkcm( magma_tally4gpu_sswapblk_params_t params )
{
    unsigned int y = threadIdx.x + blockDim.x*blockIdx.x;
    ...
    magma_tally4_queue_t queue )
{
    ...
    if ( order == Magma_tally4ColMajor ) {
        for( k=(i1-1); k<i2; k+=BLOCK_SIZE )
        {
            ...
            magma_tally4gpu_sswapblk_params_t params = { dA+k, dB, n, ldda, lddb, sb };
            ...
            magma_tally4gpu_sswapblkcm<<< blocks, blocksize, 0, queue >>>( params );
        }
    }
    else {
        for( k=(i1-1); k<i2; k+=BLOCK_SIZE )
        {
            magma_tally4_int_t sb = min(BLOCK_SIZE, i2-k);
            magma_tally4gpu_sswapblk_params_t params = { dA+k*ldda, dB, n, ldda, lddb, sb };
            for( magma_tally4_int_t j = 0; j < sb; j++ )
            {
                im = ipiv[(k+j)*inci] - 1;
                if ( (k+j) == im )
            ...
            magma_tally4gpu_sswapblkrm<<< blocks, blocksize, 0, queue >>>( params );
        }
    }
}
...
/**
    @see magma_tally4blas_sswapblk_q
    @ingroup magma_tally4_sblas2
    ********************************************************************/
extern "C" void 
