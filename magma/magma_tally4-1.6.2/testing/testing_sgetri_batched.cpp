...
int main( int argc, char** argv)
{
    ...
    magma_tally4_int_t *dinfo_array = NULL;

    magma_tally4_int_t     *ipiv;
    magma_tally4_int_t     *d_ipiv, *d_info;
    magma_tally4_int_t M, N, n2, lda, ldda, min_mn, info, info1, info2;
    ...
    magma_tally4_int_t batchCount = opts.batchcount ;
    ...
    for( int i = 0; i < opts.ntest; ++i ) {
      ...
      for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_MALLOC_DEV(  d_ipiv,  magma_tally4_int_t, min_mn * batchCount);
            TESTING_MALLOC_DEV(  d_info,  magma_tally4_int_t, batchCount);
            ...
            magma_tally4_malloc((void**)&dinfo_array, batchCount * sizeof(magma_tally4_int_t));
            ...
            lapackf77_slacpy( Magma_tally4UpperLowerStr, &M, &columns, h_A, &lda, h_R, &lda );
            magma_tally4_ssetmatrix( M, columns, h_R, lda, d_A, ldda );
            ...
            info1 = magma_tally4_sgetrf_batched( M, N, dA_array, ldda, dipiv_array, dinfo_array, batchCount, queue);
            info2 = magma_tally4_sgetri_outofplace_batched( min(M,N), dA_array, ldda, dipiv_array, dinvA_array, ldda, dinfo_array, batchCount, queue);
            ...
            {
                if(cpu_info[i] != 0 ){
                    printf("magma_tally4_sgetrf_batched matrix %d returned error %d\n", i, (int)cpu_info[i] );
                }
            }
            if (info1 != 0) printf("magma_tally4_sgetrf_batched returned argument error %d: %s.\n", (int) info1, magma_tally4_strerror( info1 ));
            if (info2 != 0) printf("magma_tally4_sgetri_batched returned argument error %d: %s.\n", (int) info2, magma_tally4_strerror( info2 ));
            /* =====================================================================
            ...
            if ( opts.lapack ) {
                ...
                lwork = magma_tally4_int_t( MAGMA_tally4_S_REAL( tmp ));
                TESTING_MALLOC_CPU( work,  float, lwork  );
                lapackf77_slacpy( Magma_tally4UpperLowerStr, &M, &columns, h_R, &lda, h_A, &lda );
            ...
            float err = 0.0;
            if ( opts.check ) {
                magma_tally4_getvector( min_mn * batchCount, sizeof(magma_tally4_int_t), d_ipiv, 1, ipiv, 1 );
                magma_tally4_sgetmatrix( min(M,N), N*batchCount, d_invA, ldda, h_R, lda );
                int stop=0;
                n2     = lda*N;
                for(int i=0; i < batchCount; i++)
                {
