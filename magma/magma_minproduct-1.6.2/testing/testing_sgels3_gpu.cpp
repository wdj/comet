...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lhwork2 = (magma_minproduct_int_t) MAGMA_minproduct_S_REAL( tmp[0] );
            
            lhwork = -1;
            lapackf77_sormqr( Magma_minproductLeftStr, Magma_minproductTransStr,
                              &M, &nrhs, &min_mn, NULL, &lda, NULL,
                              NULL, &ldb, tmp, &lhwork, &info);
            lhwork = (magma_minproduct_int_t) MAGMA_minproduct_S_REAL( tmp[0] );
            ...
            lapackf77_slacpy( Magma_minproductUpperLowerStr, &M, &N, h_A, &lda, h_A2, &lda );
            
            // make random RHS
            size = M*nrhs;
            lapackf77_slarnv( &ione, ISEED, &size, h_B );
            lapackf77_slacpy( Magma_minproductUpperLowerStr, &M, &nrhs, h_B, &ldb, h_R, &ldb );
            
            // make consistent RHS
            //size = N*nrhs;
            //lapackf77_slarnv( &ione, ISEED, &size, h_X );
            //blasf77_sgemm( Magma_minproductNoTransStr, Magma_minproductNoTransStr, &M, &nrhs, &N,
            //               &c_one,  h_A, &lda,
            //                        h_X, &ldb,
            //               &c_zero, h_B, &ldb );
            //lapackf77_slacpy( Magma_minproductUpperLowerStr, &M, &nrhs, h_B, &ldb, h_R, &ldb );
            ...
            magma_minproduct_ssetmatrix( M, nrhs, h_B, ldb, d_B, lddb );
            
            gpu_time = magma_minproduct_wtime();
            magma_minproduct_sgels3_gpu( Magma_minproductNoTrans, M, N, nrhs, d_A, ldda,
                              d_B, lddb, h_work, lworkgpu, &info);
            gpu_time = magma_minproduct_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0)
                printf("magma_minproduct_sgels3_gpu returned error %d: %s.\n",
                       (int) info, magma_minproduct_strerror( info ));
            
            // Get the solution in h_X
            magma_minproduct_sgetmatrix( N, nrhs, d_B, lddb, h_X, ldb );
            ...
            /* =====================================================================
               ...
               =================================================================== */
            lapackf77_slacpy( Magma_minproductUpperLowerStr, &M, &nrhs, h_B, &ldb, h_X, &ldb );
            
            cpu_time = magma_minproduct_wtime();
            lapackf77_sgels( Magma_minproductNoTransStr, &M, &N, &nrhs,
