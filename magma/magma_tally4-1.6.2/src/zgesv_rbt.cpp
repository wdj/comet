...
magma_tally4_zgerfs_nopiv_gpu(
    ...
    magma_tally4DoubleComplex_ptr dworkd, magma_tally4DoubleComplex_ptr dAF,
    ...
    magma_tally4_int_t *info)
{
    ...
    *info = 0;
    ...
    Anrm = magma_tally4blas_zlange(Magma_tally4InfNorm, n, n, dA, ldda, (double*)dworkd );
    ...
    if ( nrhs == 1 ) {
        magma_tally4_zgemv( trans, n, n,
                     c_neg_one, dA, ldda,
                                dX, 1,
                     c_one,     dR, 1 );
    }
    else {
        magma_tally4_zgemm( trans, Magma_tally4NoTrans, n, nrhs, n,
                     c_neg_one, dA, ldda,
                                dX, lddx,
                     c_one,     dR, lddr );
    }
...
REFINEMENT:
    for( iiter=1; iiter < ITERMAX; ) {
        *info = 0;
        ...
        magma_tally4_zgetrs_nopiv_gpu( trans, n, nrhs, dAF, lddsa, dR, lddr, info );
        if (*info != 0) {
            *iter = -3;
            goto FALLBACK;
        }
        ...
        for( j=0; j < nrhs; j++ ) {
            magma_tally4blas_zaxpycp2( n, dR(0,j), dX(0,j), dB(0,j) );
        }
    ...
    magma_tally4_int_t *info)
{
    ...
    *info = 0;
    ...
    magma_tally4DoubleComplex *dA, *hu, *hv, *db, *dAo, *dBo, *dwork;
    magma_tally4_int_t n2;

    magma_tally4_int_t iter;
    n2 = nn*nn;

    if (MAGMA_tally4_SUCCESS != magma_tally4_zmalloc( &dA, n2 )) {
        *info = MAGMA_tally4_ERR_DEVICE_ALLOC;
        return *info;
    }
    if (MAGMA_tally4_SUCCESS != magma_tally4_zmalloc( &db, nn*nrhs )) {
        *info = MAGMA_tally4_ERR_DEVICE_ALLOC;
        return *info;
    }

    if (ref == Magma_tally4True) {
        if (MAGMA_tally4_SUCCESS != magma_tally4_zmalloc( &dAo, n2 )) {
            *info = MAGMA_tally4_ERR_DEVICE_ALLOC;
            return *info;
        }
        if (MAGMA_tally4_SUCCESS != magma_tally4_zmalloc( &dwork, nn*nrhs )) {
            *info = MAGMA_tally4_ERR_DEVICE_ALLOC;
            return *info;
        }
        if (MAGMA_tally4_SUCCESS != magma_tally4_zmalloc( &dBo, nn*nrhs )) {
            *info = MAGMA_tally4_ERR_DEVICE_ALLOC;
            return *info;
        }
    }

    if (MAGMA_tally4_SUCCESS != magma_tally4_zmalloc_cpu( &hu, 2*nn )) {
        *info = MAGMA_tally4_ERR_HOST_ALLOC;
        return *info;
    }
    if (MAGMA_tally4_SUCCESS != magma_tally4_zmalloc_cpu( &hv, 2*nn )) {
        *info = MAGMA_tally4_ERR_HOST_ALLOC;
        return *info;
    }

    magma_tally4blas_zlaset(Magma_tally4Full, nn, nn, MAGMA_tally4_Z_ZERO, MAGMA_tally4_Z_ONE, dA, nn);

    /* Send matrix on the GPU*/
    magma_tally4_zsetmatrix(n, n, A, lda, dA, nn);

    /* Send b on the GPU*/
    magma_tally4_zsetmatrix(n, nrhs, B, ldb, db, nn);

    *info = magma_tally4_zgerbt_gpu(Magma_tally4True, nn, nrhs, dA, nn, db, nn, hu, hv, info);
    if (*info != MAGMA_tally4_SUCCESS)  {
        return *info;
    }

    if (ref == Magma_tally4True) {
        magma_tally4_zcopymatrix(nn, nn, dA, nn, dAo, nn);
        magma_tally4_zcopymatrix(nn, nrhs, db, nn, dBo, nn);
    }
    /* Solve the system U^TAV.y = U^T.b on the GPU*/ 
    magma_tally4_zgesv_nopiv_gpu( nn, nrhs, dA, nn, db, nn, info);


    /* Iterative refinement */
    if (ref == Magma_tally4True) {
        magma_tally4_zgerfs_nopiv_gpu(Magma_tally4NoTrans, nn, nrhs, dAo, nn, dBo, nn, db, nn, dwork, dA, &iter, info);
    }
    //printf("iter = %d\n", iter);
    ...
    if (MAGMA_tally4_SUCCESS != magma_tally4_zmalloc( &dv, 2*nn )) {
        *info = MAGMA_tally4_ERR_DEVICE_ALLOC;
        return *info;
    }

    magma_tally4_zsetvector(2*nn, hv, 1, dv, 1);
    
    for(int i = 0; i < nrhs; i++) {
        magma_tally4blas_zprbt_mv(nn, dv, db+(i*nn));
    }
