    ...
    magma_tally4_int_t *info )
{
    ...
    magma_tally4DoubleComplex *dA, *dwork, *dT;
    ...
    *info = 0;
    ...
    if ( ngpu > 1 ) {
        ...
        return magma_tally4_zgeqrf4(ngpu, m, n, A, lda, tau, work, lwork, info);
    }

    // allocate space for dA, dwork, and dT
    if (MAGMA_tally4_SUCCESS != magma_tally4_zmalloc( &dA, n*ldda + nb*lddwork + nb*nb )) {
        /* Switch to the "out-of-core" (out of GPU-memory) version */
        return magma_tally4_zgeqrf_ooc(m, n, A, lda, tau, work, lwork, info);
    }
    ...
    if ( (nb > 1) && (nb < k) ) {
        ...
        for (i = 0; i < k-nb; i += nb) {
            ...
            if (i > 0) {
                ...
                magma_tally4_zgetmatrix_async( m-i, ib,
                                        dA(i,i), ldda,
                                        A(i,i),  lda, stream[0] );
                ...
                magma_tally4_zgetmatrix_async( i, ib,
                                        dA(0,i), ldda,
                                        A(0,i),  lda, stream[1] );
            ...
            magma_tally4_queue_sync( stream[1] );
            magma_tally4_zsetmatrix_async( ib, ib, work, ib, dT, nb, stream[0] );
            magma_tally4_queue_sync( stream[0] );
    ...
    if (i < k) {
        ib = n-i;
        if (i != 0) {
            magma_tally4_zgetmatrix_async( m, ib, dA(0,i), ldda, A(0,i), lda, stream[1] );
            magma_tally4_queue_sync( stream[1] );
        }
        magma_tally4_int_t rows = m-i;
        lapackf77_zgeqrf(&rows, &ib, A(i,i), &lda, tau+i, work, &lwork, info);
