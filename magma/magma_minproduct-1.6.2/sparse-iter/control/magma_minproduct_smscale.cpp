/*
    -- MAGMA_minproduct (version 1.6.2) --
       ...
       @generated from magma_minproduct_zmscale.cpp normal z -> s, Sun May  3 11:23:01 2015
       @author Hartwig Anzt
    ...
    magma_minproduct_queue_t queue )
{
    ...
    if ( A->memory_location == Magma_minproduct_CPU && A->storage_type == Magma_minproduct_CSRCOO ) {
        if ( scaling == Magma_minproduct_NOSCALE ) {
            ...
            CHECK( magma_minproduct_smalloc_cpu( &tmp, A->num_rows ));
            for( magma_minproduct_int_t z=0; z<A->num_rows; z++ ) {
                float s = MAGMA_minproduct_S_MAKE( 0.0, 0.0 );
                for( magma_minproduct_int_t f=A->row[z]; f<A->row[z+1]; f++ )
                    s+= MAGMA_minproduct_S_REAL(A->val[f])*MAGMA_minproduct_S_REAL(A->val[f]);
                tmp[z] = MAGMA_minproduct_S_MAKE( 1.0/sqrt(  MAGMA_minproduct_S_REAL( s )  ), 0.0 );
            }        printf("inhere1\n");
            ...
            for( magma_minproduct_int_t z=0; z<A->num_rows; z++ ) {
                ...
                for( magma_minproduct_int_t f=A->row[z]; f<A->row[z+1]; f++ ) {
                    if ( A->col[f]== z ) {
                        ...
                        //A->val[f] = A->val[f] +  MAGMA_minproduct_S_MAKE( 100000.0, 0.0 );
                        s = A->val[f];
                    }
                }
                if ( s == MAGMA_minproduct_S_MAKE( 0.0, 0.0 ) ){
        ...
        CHECK( magma_minproduct_smconvert( hA, &CSRA, hA.storage_type, Magma_minproduct_CSRCOO, queue ));

        CHECK( magma_minproduct_smscale( &CSRA, scaling, queue ));

        magma_minproduct_smfree( &hA, queue );
        magma_minproduct_smfree( A, queue );
        CHECK( magma_minproduct_smconvert( CSRA, &hA, Magma_minproduct_CSRCOO, A_storage, queue ));
    ...
    magma_minproduct_queue_t queue )
{
    ...
    if ( A->memory_location == Magma_minproduct_CPU && A->storage_type == Magma_minproduct_CSRCOO ) {
        ...
        CHECK( magma_minproduct_smdiagadd( &CSRA, add, queue ));
