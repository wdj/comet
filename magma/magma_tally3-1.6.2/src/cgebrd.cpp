...
/**
    ...
    @ingroup magma_tally3_cgesvd_comp
    ********************************************************************/
...
#define dA(i, j) (dA + (j)*ldda + (i))
    ...
    magma_tally3FloatComplex *dA, *dwork;
    ...
    nb   = magma_tally3_get_cgebrd_nb(n);
    ldda = m;
    ...
    *info = 0;
    ...
    if (MAGMA_tally3_SUCCESS != magma_tally3_cmalloc( &dA, n*ldda + (m + n)*nb )) {
    ...
    for (i=0; i < (minmn - nx); i += nb) {
        ...
        if ( i > 0 ) {
            magma_tally3_cgetmatrix( nrow, nb, dA(i, i), ldda, A( i, i), lda );
            magma_tally3_cgetmatrix( nb, ncol - nb,
                              dA(i, i+nb), ldda,
                              A( i, i+nb), lda );
        }

        magma_tally3_clabrd_gpu(nrow, ncol, nb,
        ...
        magma_tally3_csetmatrix( nrow, nb, work  + nb, ldwrkx, dwork + nb, ldwrkx );
        magma_tally3_csetmatrix( ncol, nb,
        ...
        if (m >= n) {
            jmax = i + nb;
            for (j = i; j < jmax; ++j) {
                *A(j, j  ) = MAGMA_tally3_C_MAKE( d[j], 0. );
                *A(j, j+1) = MAGMA_tally3_C_MAKE( e[j], 0. );
            }
        } else {
            jmax = i + nb;
            for (j = i; j < jmax; ++j) {
    ...
    if ( 0 < minmn - nx ) {
        magma_tally3_cgetmatrix( nrow, ncol, dA(i, i), ldda, A(i, i), lda );
    }
