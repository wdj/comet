/*
    -- MAGMA_minproduct (version 1.6.1) --
       ...
       @generated from zheevr_gpu.cpp normal z -> c, Fri Jan 30 19:00:17 2015
...
magma_minproduct_cheevr_gpu(
    ...
    float *w,
    magma_minproductFloatComplex_ptr dZ, magma_minproduct_int_t lddz, magma_minproduct_int_t *isuppz,
    ...
    magma_minproduct_int_t *info)
{
    ...
    *m = 0;
    ...
    if (n <= 128) {
        ...
        magma_minproduct_cmalloc_cpu( &a, n*n );
        magma_minproduct_cgetmatrix(n, n, dA, ldda, a, n);
        lapackf77_cheevr(jobz_, range_, uplo_,
                         &n, a, &n, &vl, &vu, &il, &iu, &abstol, m,
                         w, wZ, &ldwz, isuppz, work, &lwork,
                         rwork, &lrwork, iwork, &liwork, info);
        magma_minproduct_csetmatrix( n,  n,  a,    n, dA, ldda);
        magma_minproduct_csetmatrix( n, *m, wZ, ldwz, dZ, lddz);
        magma_minproduct_free_cpu(a);
        return *info;
    }

    if (MAGMA_minproduct_SUCCESS != magma_minproduct_smalloc( &dwork, n )) {
        fprintf (stderr, "!!!! device memory allocation error (magma_minproduct_cheevr_gpu)\n");
    ...
    anrm = magma_minproductblas_clanhe(Magma_minproductMaxNorm, uplo, n, dA, ldda, dwork);
    iscale = 0;
    sigma  = 1;
    if (anrm > 0. && anrm < rmin) {
        iscale = 1;
    ...
    if (iscale == 1) {
        d__1 = 1.;
        magma_minproductblas_clascl(uplo, 0, 0, 1., sigma, n, n, dA,
                         ldda, info);
...
#ifdef FAST_HEMV
    magma_minproduct_chetrd2_gpu(uplo, n, dA, ldda, &rwork[indrd], &rwork[indre],
                      &work[indtau], wA, ldwa, &work[indwk], llwork, dZ, lddz*n, &iinfo);
#else
    magma_minproduct_chetrd_gpu (uplo, n, dA, ldda, &rwork[indrd], &rwork[indre],
                      &work[indtau], wA, ldwa, &work[indwk], llwork, &iinfo);
#endif

    lopt = n + (magma_minproduct_int_t)MAGMA_minproduct_C_REAL(work[indwk]);
    ...
    if (! wantz) {
        ...
        if (*info == 0 && wantz) {
            magma_minproduct_csetmatrix( n, *m, wZ, ldwz, dZ, lddz );
            magma_minproduct_cunmtr_gpu(Magma_minproductLeft, uplo, Magma_minproductNoTrans, n, *m, dA, ldda, &work[indtau],
    ...
    if (wantz) {
        for (j = 1; j <= *m-1; ++j) {
            ...
            if (i != 0) {
                ...
                magma_minproduct_cswap(n, dZ + (i-1)*lddz, ione, dZ + (j-1)*lddz, ione);
            }
        }
    }
    
    /* Set WORK[0] to optimal complex workspace size. */
    work[1] = MAGMA_minproduct_C_MAKE( lopt, 0 );
