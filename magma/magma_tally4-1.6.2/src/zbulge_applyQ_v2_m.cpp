    ...
    magma_tally4_int_t *info)
{
    ...
    */
    ...
    magma_tally4DoubleComplex *dE[Magma_tally4MaxGPUs];
    magma_tally4DoubleComplex *dwork[Magma_tally4MaxGPUs], *dwork0[Magma_tally4MaxGPUs], *dwork1[Magma_tally4MaxGPUs];
    //magma_tally4DoubleComplex *dwvt[Magma_tally4MaxGPUs];
    magma_tally4DoubleComplex *dwvt0[Magma_tally4MaxGPUs], *dwvt1[Magma_tally4MaxGPUs];
    magma_tally4DoubleComplex *dT0[Magma_tally4MaxGPUs], *dV0[Magma_tally4MaxGPUs], *dT1[Magma_tally4MaxGPUs], *dV1[Magma_tally4MaxGPUs];
    ...
    for (dev=0; dev < ngpu; ++dev) {
        magma_tally4_setdevice( dev );
        if (MAGMA_tally4_SUCCESS != magma_tally4_zmalloc( &dE[dev], ldde * ne_loc)) {
            printf ("!!!!  magma_tally4_zbulge_applyQ magma_tally4_alloc failed for: dE\n" );
            exit(-1);
        }
        if (MAGMA_tally4_SUCCESS != magma_tally4_zmalloc( &dwork[dev], 2*dworksiz + 2*dwVTsiz + 2*Vchunksiz* (Vblksiz* (lddv+lddt)) )) {
            printf ("!!!!  magma_tally4_zbulge_applyQ magma_tally4_alloc failed for: dwork\n" );
            exit(-1);
        }
        ...
        magma_tally4_zsetmatrix_async( N, ie_loc, E+lde*ne_loc*dev, lde, dE(dev, 0, 0), ldde, streams[dev][1] );
    }
                    ...
                    if (ncpy == 0) {
                        ...
                        if (mysiz > 0) {
                            ...
                            for( dev = 0; dev < ngpu; ++dev ) {
                                ...
                                magma_tally4_zsetmatrix_async(vld, Vblksiz, V(vpos), vld, dV1[dev], vld, streams[dev][1]);
                                magma_tally4_zsetmatrix_async(tld, Vblksiz, T(tpos), tld, dT1[dev], tld, streams[dev][1]);
                            }
                        ...
                        if (mysiz > 0) {
                            ...
                            if (flip == 0) { // now I am working on dV0 so copy the next and put it on dV1
                                ...
                                for( dev = 0; dev < ngpu; ++dev ) {
                                    ...
                                    magma_tally4_zsetmatrix_async(vld, Vblksiz, V(vpos), vld, dV0[dev], vld, streams[dev][0]);
                                    magma_tally4_zsetmatrix_async(tld, Vblksiz, T(tpos), tld, dT0[dev], tld, streams[dev][0]);
                                }
                            }
                        }
                    }
                        ...
                        if (flip == 0) {
                            for( dev = 0; dev < ngpu; ++dev ) {
                                ...
                                for (magma_tally4_int_t i=0; i < ie_loc; i += sz_bl) {
                                    ...
                                    //magma_tally4_zlarfb_gpu( Magma_tally4Left, Magma_tally4NoTrans, Magma_tally4Forward, Magma_tally4Columnwise, Vm, ib, Vn, dV0[dev]+lcvpos, lddv, dT0[dev]+lctpos, lddt, dE(dev,myrow,i), ldde, dwork0[dev], ib);
                                    magma_tally4_zlarfb_gpu_gemm( Magma_tally4Left, Magma_tally4NoTrans, Magma_tally4Forward, Magma_tally4Columnwise, Vm, ib, Vn, dV0[dev]+lcvpos, lddv, dT0[dev]+lctpos, lddt, dE(dev,myrow,i), ldde, dwork0[dev], ib, dwvt0[dev], Vm);
                                }
                            ...
                            for( dev = 0; dev < ngpu; ++dev ) {
                                ...
                                for (magma_tally4_int_t i=0; i < ie_loc; i += sz_bl) {
                                    ib = min(sz_bl, ie_loc-i);
                                    //magma_tally4_zlarfb_gpu( Magma_tally4Left, Magma_tally4NoTrans, Magma_tally4Forward, Magma_tally4Columnwise, Vm, ib, Vn, dV1[dev]+lcvpos, lddv, dT1[dev]+lctpos, lddt, dE(dev,myrow,i), ldde, dwork1[dev], ib);
                                    magma_tally4_zlarfb_gpu_gemm( Magma_tally4Left, Magma_tally4NoTrans, Magma_tally4Forward, Magma_tally4Columnwise, Vm, ib, Vn, dV1[dev]+lcvpos, lddv, dT1[dev]+lctpos, lddt, dE(dev,myrow,i), ldde, dwork1[dev], ib, dwvt1[dev], Vm);
                                }
            ...
            for (blki = mt; blki > 0; blki--) {
                ...
                for (blkj = nt-1; blkj >= 0; blkj--) {
                    ...
                    if ((Vm > 0) && (Vn > 0)) {
                        ...
                        magma_tally4_zsetmatrix_async(Vm, Vn, V(vpos), ldv, dV0, lddv, NULL);
                        magma_tally4_zsetmatrix_async(Vn,  Vn, T(tpos), ldt, dT0, lddt, NULL);
                        ...
                        for (magma_tally4_int_t i=0; i < NE; i += sz_bl) {
                            ib = min(sz_bl, NE-i);
                            magma_tally4_zlarfb_gpu( Magma_tally4Left, Magma_tally4NoTrans, Magma_tally4Forward, Magma_tally4Columnwise, Vm, ib, Vn, dV0, lddv, dT0, lddt, dE(myrow,i), ldde, dwork, NE);
                        }
                        */
                    } // end for (Vm &Vn) > 0
                } // end for blkj
            ...
            for (blkj=0; blkj < nt; blkj++) {
                ...
                for (blki=1; blki <= mt; blki++) {
                    ...
                    if ((Vm > 0) && (Vn > 0)) {
                        ...
                        magma_tally4_zsetmatrix_async(Vm, Vn, V(vpos), ldv, dV0, lddv, NULL);
                        magma_tally4_zsetmatrix_async(Vn,  Vn, T(tpos), ldt, dT0, lddt, NULL);
                        magma_tally4_zlarfb_gpu( Magma_tally4Right, Magma_tally4NoTrans, Magma_tally4Forward, Magma_tally4Columnwise, NE, Vm, Vn, dV0, lddv, dT0, lddt, dE(0, myrow), ldde, dwork, NE);
                        */
                    } // end for (Vm &Vn) > 0
                } // end for blki
            } // end fo blkj
    ...
    for( dev = 0; dev < ngpu; ++dev ) {
        ...
        magma_tally4_int_t ie_loc   = min(ne_loc, NE - ne_loc*dev);
        magma_tally4_zgetmatrix_async( N, ie_loc, dE(dev, 0, 0), ldde, E+lde*ne_loc*dev, lde, streams[dev][0] );
        magma_tally4_event_record( myevent[dev][0], streams[dev][0] );
    }
