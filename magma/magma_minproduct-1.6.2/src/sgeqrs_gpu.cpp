/*
    -- MAGMA_minproduct (version 1.6.1) --
       ...
       @generated from zgeqrs_gpu.cpp normal z -> s, Fri Jan 30 19:00:15 2015
...
/**
    ...
    tau     REAL array, dimension (N)
            ...
            reflector H(i), as returned by MAGMA_minproduct_SGEQRF_GPU.
    ...
    dT      REAL array that is the output (the 6th argument)
            of magma_minproduct_sgeqrf_gpu of size
    ...
    lwork   INTEGER
            ...
            where NB is the blocksize given by magma_minproduct_get_sgeqrf_nb( M ).
    \n
    ...
    @ingroup magma_minproduct_sgels_comp
    ********************************************************************/
    ...
    magma_minproduct_int_t *info)
{
    ...
    magma_minproduct_int_t nb     = magma_minproduct_get_sgeqrf_nb(m);
    magma_minproduct_int_t lwkopt = (m - n + nb)*(nrhs + nb) + nrhs*nb;
    int lquery = (lwork == -1);

    hwork[0] = MAGMA_minproduct_S_MAKE( (float)lwkopt, 0. );
    ...
    *info = 0;
    ...
    /* B := Q' * B */
    magma_minproduct_sormqr_gpu( Magma_minproductLeft, Magma_minproductTrans,
                      m, nrhs, n,
                      dA(0,0), ldda, tau,
    ...
    // TODO: this assumes that, on exit from magma_minproduct_sormqr_gpu, hwork contains
    ...
    // magma_minproduct_strsm and drop the ssetmatrix.
    if ( nrhs == 1 ) {
        blasf77_strsv( Magma_minproductUpperStr, Magma_minproductNoTransStr, Magma_minproductNonUnitStr,
                       &ib, hwork,         &rows,
                            hwork+rows*ib, &ione);
    } else {
        blasf77_strsm( Magma_minproductLeftStr, Magma_minproductUpperStr, Magma_minproductNoTransStr, Magma_minproductNonUnitStr,
                       &ib, &nrhs,
                       &c_one, hwork,         &rows,
                               hwork+rows*ib, &rows);
    }
    ...
    magma_minproduct_ssetmatrix( ib, nrhs, hwork+rows*ib, rows, dwork+i, lddwork );

    // update c
    if (nrhs == 1)
        magma_minproduct_sgemv( Magma_minproductNoTrans, i, ib,
    ...
    if (nb < k) {
        for (i = start; i >= 0; i -= nb) {
            ...
            if (i + ib < n) {
                if (nrhs == 1) {
                    magma_minproduct_sgemv( Magma_minproductNoTrans, ib, ib,
                                 c_one,  dT(i), ib,
                                         dB+i,      1,
                                 c_zero, dwork+i,  1);
                    magma_minproduct_sgemv( Magma_minproductNoTrans, i, ib,
    ...
    magma_minproduct_scopymatrix( (n), nrhs,
                       dwork, lddwork,
                       dB,    lddb );
