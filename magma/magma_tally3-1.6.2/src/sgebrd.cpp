/*
    -- MAGMA_tally3 (version 1.6.1) --
       ...
       @generated from zgebrd.cpp normal z -> s, Fri Jan 30 19:00:19 2015
...
/**
    ...
    @ingroup magma_tally3_sgesvd_comp
    ********************************************************************/
...
#define dA(i, j) (dA + (j)*ldda + (i))
    ...
    magma_tally3_int_t ncol, nrow, jmax, nb, ldda;

    magma_tally3_int_t i, j, nx;
    magma_tally3_int_t iinfo;

    magma_tally3_int_t minmn;
    magma_tally3_int_t ldwrkx, ldwrky, lwkopt;
    magma_tally3_int_t lquery;

    nb   = magma_tally3_get_sgebrd_nb(n);
    ldda = m;
    ...
    *info = 0;
    ...
    if (MAGMA_tally3_SUCCESS != magma_tally3_smalloc( &dA, n*ldda + (m + n)*nb )) {
    ...
    for (i=0; i < (minmn - nx); i += nb) {
        ...
        if ( i > 0 ) {
            magma_tally3_sgetmatrix( nrow, nb, dA(i, i), ldda, A( i, i), lda );
            magma_tally3_sgetmatrix( nb, ncol - nb,
                              dA(i, i+nb), ldda,
                              A( i, i+nb), lda );
        }

        magma_tally3_slabrd_gpu(nrow, ncol, nb,
        ...
        magma_tally3_ssetmatrix( nrow, nb, work  + nb, ldwrkx, dwork + nb, ldwrkx );
        magma_tally3_ssetmatrix( ncol, nb,
        ...
        if (m >= n) {
            jmax = i + nb;
            for (j = i; j < jmax; ++j) {
                *A(j, j  ) = MAGMA_tally3_S_MAKE( d[j], 0. );
                *A(j, j+1) = MAGMA_tally3_S_MAKE( e[j], 0. );
            }
        } else {
            jmax = i + nb;
            for (j = i; j < jmax; ++j) {
    ...
    if ( 0 < minmn - nx ) {
        magma_tally3_sgetmatrix( nrow, ncol, dA(i, i), ldda, A(i, i), lda );
    }
