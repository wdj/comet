                         ...
                         float *c, int ldc, float *T, int ldt, int step )
{
    ...
    if ( !MAGMA_tally2_S_EQUAL(*tau, MAGMA_tally2_S_ZERO) ) {
        if(thblk<step){    
            ...
            for( int j = tx+1; j < m; j += BLOCK_SIZE ){
                tmp +=  MAGMA_tally2_S_CNJG( v[j] ) * dc[j];
            }
            sum[tx] = tmp;
            magma_tally2_sum_reduce< BLOCK_SIZE >( tx, sum );
            #if defined (use_gemm_larft)
            *(T+thblk) = MAGMA_tally2_S_CNJG(sum[0]);
            #else
            tmp = - MAGMA_tally2_S_CNJG(*tau) * sum[0]; 
            *(T+thblk) = MAGMA_tally2_S_CNJG(tmp); // T = - tau(tx) * V(tx:n,1:tx-1)' * V(tx:n,tx) = tmp'
            //*(T+thblk) = - MAGMA_tally2_S_CNJG(sum[0]) * (*tau); // T = - tau(tx) * V(tx:n,1:tx-1)' * V(tx:n,tx) = tmp'
            #endif
        }
        else{
            #if defined (use_gemm_larft)
            *(T+thblk) = MAGMA_tally2_S_ONE;
            #else
            *(T+thblk) = *tau;
            #endif
        }
    ...
    float *tau)
{
    ...
    slarft_gemvcolwise_kernel<<< grid, threads, 0, magma_tally2_stream >>>( m, v, ldv, tau, T, ldt, step);
    ...
    float *W, float* sdata)
{
    ...
    if(tx < sgemv_bs)
    {
        ...
        {
            res += MAGMA_tally2_S_CNJG (v_ptr[s]) * x_ptr[s*incx];
        }
    ...
    magma_tally2_sum_reduce<sgemv_bs>(tx, &(sdata[ty*sgemv_bs+0]));
    ...
    float *W)
{
    ...
    slarft_gemvrowwise_kernel <<< grid, threads, sizeof(float)*sgemv_bs*(i+1), magma_tally2_stream>>>(m, i, tau, v, ldv, T, ldt);
}
//===================================================================================================
extern "C"
    ...
    float *T, int ldt)
{
    ...
    slarft_gemv_loop_inside_kernel<<<grid, threads, sizeof(float) * (sgemv_bs*(k+1)), magma_tally2_stream>>>(n, k, tau, v, ldv, T, ldt); 
}
//===================================================================================================
extern "C"
    ...
    float *Tout, magma_tally2_int_t ldtout)
{
    ...
    slarft_strmv_sm32x32_kernel <<< grid, threads, sizeof(float)*(m*m), magma_tally2_stream >>> (m, n,  tau, Tin, ldtin, Tout, ldtout);
}
//===================================================================================================
extern "C"
    ...
    float *Ttri, magma_tally2_int_t ldttri)
{
    ...
    dim3 grid(1);
    dim3 threads(max(m,1), 1, 1);
    slarft_recstrmv_sm32x32_kernel <<< grid, threads, sizeof(float)*(m*n), magma_tally2_stream >>> (m, n,  tau, Trec, ldtrec, Ttri, ldttri);
}
//===================================================================================================
extern "C"
