    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int itran = 0; itran < 2; ++itran ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_tally3_sgeqrf( mm, k, A, lda, tau, W, lwork_max, &info );
            if (info != 0)
            ...
            lwork = -1;
            magma_tally3_sormqr( side[iside], trans[itran],
                          m, n, k,
                          A, lda, tau, R, ldc, W, lwork, &info );
            if (info != 0)
                printf("magma_tally3_sormqr (lwork query) returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
            ...
            if (info != 0)
                printf("magma_tally3_sormqr returned error %d: %s.\n",
                       (int) info, magma_tally3_strerror( info ));
